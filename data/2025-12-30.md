...
### [1] [Scalable Residual Feature Aggregation Framework with Hybrid Metaheuristic Optimization for Robust Early Pancreatic Neoplasm Detection in Multimodal CT Imaging](https://arxiv.org/abs/2512.23597)
...
Motivation: 胰腺肿瘤早期检测面临临床困境，主要因为肿瘤在CT扫描中对比度低、解剖结构变异大，需要能增强细微视觉线索并具有良好泛化能力的系统。

Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割；2)DenseNet-121提取特征；3)混合HHO-BA元启发式特征选择；4)ViT与EfficientNet-B3混合模型分类；5)SSA和GWO双优化机制调参。

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，显著优于传统CNN和当代transformer模型。

Conclusion: SRFA框架作为胰腺肿瘤早期检测的有效工具具有潜力，其多阶段处理和优化策略能有效处理复杂医学影像数据。
...
TL;DR: 本文提出了一种实时美国手语识别系统，采用3D CNN与LSTM混合深度学习架构，通过摄像头视频流识别单词级ASL手势，为全球7000多万聋哑人士解决沟通障碍。


<details>
  <summary>Details</summary>
Motivation: 为全球超过7000万聋哑和听力障碍人士解决沟通障碍，通过实时手语识别技术促进无障碍交流。

Method: 采用混合深度学习架构：3D CNN提取视频帧的时空特征，LSTM层建模手语手势的序列依赖关系。在WLASL数据集（2000个常用词）、ASL-LEX词汇数据库（约2700个手势）和100个专家标注的ASL手势数据集上进行训练。

Result: 系统在不同手势类别上获得0.71到0.99的F1分数，性能表现良好。模型部署在AWS基础设施上，并支持在OAK-D相机上进行边缘部署，实现实时推理。

Conclusion: 该混合深度学习架构能够有效识别实时美国手语手势，为实际无障碍应用提供了可行的技术方案，包括架构设计、训练方法、评估指标和部署考虑等方面的完整解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（包括医疗账单和转录）创造了机会。传统方法在处理高维医疗数据时存在准确性和效率方面的挑战，需要创新的解决方案来减少人为错误并简化操作。

Method: 开发了一种AI增强的LLE模型，专门针对医疗账单系统和转录服务进行优化。该方法通过自动化处理流程，提供全面的数学模型，并在真实医疗场景中进行了一系列实验验证。
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时经常利用虚假相关性而非因果视觉证据，微调后更容易产生捷径依赖。需要解决VLMs对虚假相关性的脆弱性问题。

Method: 提出VISTA框架：1）冻结的VLM传感器仅处理简短、客观的感知查询；2）纯文本LLM推理器分解问题、规划查询、聚合视觉事实；3）通过强化学习（GRPO）在仅641个多步问题上训练无偏视觉推理。

Result: 在SpuriVerse上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集上保持竞争力，能跨传感器迁移并识别感知失败，人类分析显示推理轨迹更中立、更少依赖虚假属性、更明确基于视觉证据。

Conclusion: VISTA通过模块化设计有效减少视觉语言模型对虚假相关性的依赖，提供更可靠、可解释的视觉推理能力，为构建鲁棒的视觉语言系统提供了新思路。
...
TL;DR: SAMM2D双编码器框架在动脉瘤检测中取得突破，发现预训练特征已足够鲁棒，数据增强反而降低性能，挑战了医学影像中"更多增强总是更好"的假设。


<details>
  <summary>Details</summary>
Motivation: 动脉瘤检测面临形态细微、类别不平衡和标注数据稀缺等挑战，需要开发更有效的检测方法来预防致命性出血。

Method: 提出SAMM2D双编码器框架，在RSNA颅内动脉瘤数据集上进行评估，通过六种增强策略的消融实验，发现预训练特征已足够鲁棒，无需额外增强。

Result: AUC达到0.686，比临床基线提升32%；未增强模型优于所有增强变体1.75-2.23个百分点；95%灵敏度超过放射科医生平均水平；预计每1000名患者可节省1390万美元。

Conclusion: 预训练特征已捕获足够鲁棒性，额外增强反而有害；未来医学影像工作流应更注重强预训练而非复杂增强管道；模型具有临床意义和经济价值。
...
TL;DR: HookMIL：一种用于计算病理学中全切片图像弱监督分析的高效上下文感知多实例学习框架，通过可学习的钩子令牌实现结构化上下文聚合，具有线性复杂度和多模态初始化能力。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在计算病理学中会丢失关键上下文信息，而基于Transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题。需要一种既能保持上下文信息又具有计算效率的解决方案。

Method: 提出HookMIL框架，使用紧凑、可学习的钩子令牌进行结构化上下文聚合。钩子令牌可通过三种方式初始化：(1)关键补丁视觉特征，(2)视觉语言病理模型的文本嵌入，(3)空间转录组-视觉模型的空间基础特征。通过双向注意力机制与实例交互，引入钩子多样性损失促进专业化，并采用钩子间通信机制优化上下文交互。

Result: 在四个公共病理数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL通过可学习的钩子令牌有效解决了传统MIL方法上下文信息丢失和Transformer方法计算效率低的问题，多模态初始化进一步提升了表示质量和收敛速度，为计算病理学中的弱监督分析提供了高效且可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型计算成本高，TinySAM虽然轻量化但"分割一切"模式仍然需要数百个提示且速度慢，需要更高效的实时分割方案。

Method: 提出Tiny-YOLOSAM混合管道：使用YOLOv12检测器为TinySAM生成前景对象的框提示，在YOLO未覆盖区域补充稀疏点提示，避免密集提示。

Result: 在COCO val2017上，类不可知覆盖率大幅提升（AR从16.4%到77.1%，mIoU从19.2%到67.8%），端到端运行时间从49.20秒/图像减少到10.39秒/图像（4.7倍加速）。

Conclusion: 检测器引导提示结合目标稀疏采样是实用全场景分割中替代密集"分割一切"提示的有效方案，平衡了分割质量和计算效率。
...
TL;DR: 提出一种新型多模态可解释性模型，利用视觉语言模型和少样本学习，通过分析视网膜象限内病变分布来模拟眼科医生的推理过程，为糖尿病视网膜病变诊断提供更全面的解释工具。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。医生缺乏时间进行病变标注，现有AI模型仅突出病变位置而无法解释分类推理，且依赖单一成像模态，解释效果有限。
...
Conclusion: 该模型通过模拟眼科医生的推理过程，提供更全面的解释能力，克服了现有模型的局限性，有望在筛查、治疗和研究环境中实现多样化应用。
...
TL;DR: VLM-PAR是一个基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨模态注意力融合改进视觉特征，在行人属性识别任务中显著提升了性能，特别是在高度不平衡的PA100K数据集上创造了新的最佳结果。
...
Result: 在高度不平衡的PA100K基准上实现了显著的准确率提升，创造了新的最佳性能，同时在PETA和Market-1501基准上也获得了显著的均值准确率提升。
...
<details>
  <summary>Details</summary>
Motivation: 自然场景中的招牌文本理解对VQA实际应用至关重要，但在低资源语言（如越南语）中研究不足。现有VQA数据集缺乏对越南语招牌特有的语言、文化和视觉特征的覆盖。

Method: 1) 创建ViSignVQA数据集，捕捉越南语招牌的双语文本、非正式表达、颜色和布局等特征；2) 将SOTA VQA模型（BLIP-2、LaTr等）与越南语OCR模型（SwinTextSpotter）和越南语预训练语言模型（ViT5）集成；3) 提出结合感知和推理智能体的多智能体VQA框架，使用GPT-4进行多数投票决策。

Result: OCR增强上下文显著提升性能，将OCR文本附加到问题中时F1分数提升高达209%。多智能体框架通过多数投票达到75.98%的准确率。研究提供了越南语招牌理解的首个大规模多模态数据集基准。

Conclusion: 领域特定资源对增强低资源语言的基于文本的VQA至关重要。ViSignVQA作为基准数据集，捕捉了真实世界场景文本特征，支持越南语OCR集成VQA模型的开发和评估。
...
TL;DR: 该论文提出了一种基于语义抽象的3D隐藏物体定位方法，利用2D视觉语言模型的激活图作为"抽象物体"表示，通过历史数据学习物体放置模式，显著提高寻找隐藏物体的效率。


<details>
  <summary>Details</summary>
Motivation: 为家庭机器人提供更高效的寻找丢失物体的能力，解决隐藏物体（部分被遮挡）难以直接通过视觉语言模型识别的问题，减少寻找时间和精力消耗。

Method: 利用2D视觉语言模型的相关性激活图作为"抽象物体"表示，将其扩展到3D定位和补全。通过历史数据学习物体放置的常见位置模式，将隐藏物体定位视为非结构化搜索问题。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试的成功率显著高于随机搜索，搜索速度明显更快。

Conclusion: 语义抽象框架的扩展为家庭机器人提供了更高效的寻找隐藏物体的技能，有望在实际应用中节省时间和精力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的视频理解方法面临两个主要挑战：1）帧间存在大量冗余信息；2）需要时间连贯的表示。现有的静态策略（如稀疏采样、帧压缩、聚类）是为离线场景优化的，应用于连续视频流时会产生碎片化或过度压缩的输出。

Method: VideoScaffold包含两个核心组件：1）弹性尺度事件分割（EES），通过预测引导的分割动态细化事件边界；2）分层事件整合（HEC），逐步将语义相关的片段聚合成多级抽象表示。两者协同工作，使框架能够随着视频流的展开，从细粒度帧理解平滑过渡到抽象事件推理。

Result: 在离线和流式视频理解基准测试上的广泛实验表明，VideoScaffold实现了最先进的性能。该框架是模块化和即插即用的，能够无缝扩展现有的基于图像的多模态大语言模型到连续视频理解任务。

Conclusion: VideoScaffold为解决流式视频理解中的冗余和时间连贯性问题提供了一个有效的动态表示框架，通过自适应调整事件粒度，在保持视觉细节的同时实现高效的事件级推理，为连续视频理解任务提供了新的解决方案。
...
TL;DR: 本文提出了一种KAN增强的FPN-Stem架构，用于改进ViT在姿态估计任务中的性能。研究发现性能瓶颈不在于注意力模块，而在于FPN中的后融合非线性平滑步骤，因此用KAN卷积层替换标准线性卷积，在COCO数据集上实现了+2.0 AP的性能提升。


<details>
  <summary>Details</summary>
Motivation: ViT在密集预测任务中表现出色，但现有模型如ViTPose的前端设计过于简单，朴素的patch机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失。需要改进前端设计以提升性能。

Method: 提出KAN增强的FPN-Stem架构。通过消融研究发现性能瓶颈在于FPN的后融合非线性平滑步骤，而非注意力模块。核心创新是保留FPN经典的"上采样-相加"融合流程，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层，利用其强大的非线性建模能力自适应学习和修正多尺度融合过程中产生的"伪影"。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了高达+2.0 AP的显著性能提升。

Conclusion: 这项工作不仅提供了一个即插即用的高性能模块，更重要的是揭示了：ViT前端的性能瓶颈通常不在于"特征精炼"(Attention)，而在于"特征融合"(Fusion)的质量。同时，通过引入KAN算子为解决这一瓶颈提供了有效途径。
...
TL;DR: 提出了一种基于元信息引导的跨域协同扩散模型（MiG-DM），用于提升低剂量PET成像质量，通过整合患者特异性元信息和投影域物理知识来改善图像重建。


<details>
  <summary>Details</summary>
Motivation: 低剂量PET成像在减少患者辐射暴露方面至关重要，但面临噪声干扰、对比度降低和生理细节难以保留等挑战。现有方法往往忽略了投影域物理知识和患者特异性元信息，而这些对于功能-语义关联挖掘至关重要。
...
Conclusion: MiG-DM通过整合跨模态先验知识，包括患者特异性元信息和投影域物理结构，能够有效提升低剂量PET成像质量，为临床诊断提供更高质量的图像支持。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下表现不佳，热成像系统缺乏颜色和纹理信息。应急服务领域的自主机器人平台需要在各种光照条件下进行可靠检测，如灾难区域的搜救和侦察任务。

Method: 开发自适应框架，融合RGB和长波红外(LWIR)视频流，在11种融合比例(从100/0到0/100，10%递增)下训练33个YOLO模型。模型针对三种光照条件训练：无光(<10 lux)、弱光(10-1000 lux)和全光(>1000 lux)。

Result: 最佳全光模型(80/20 RGB-LWIR)和弱光模型(90/10融合)分别达到92.8%和92.0%的平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下，最佳40/60融合达到71.0%，虽未达到统计显著性但仍优于基线。

Conclusion: 自适应RGB-LWIR融合提高了所有光照条件下的检测置信度和可靠性，增强了自主机器人视觉系统的性能，为应急服务应用提供了更鲁棒的检测解决方案。
...
TL;DR: 使用轻量级判别器作为外部引导信号，通过人类感知嵌入教师模型改进文本到图像扩散模型的几何控制能力，实现几何与风格的分离控制
...
Method: 提出人类感知嵌入（HPE）教师模型，基于THINGS三元组数据集训练，捕捉人类对物体形状的敏感性。将该教师模型的梯度注入潜在扩散过程，实现几何与风格的可控分离。在三种架构上评估：Stable Diffusion v1.5（U-Net骨干）、流匹配模型SiT-XL/2和扩散变换器PixArt-Σ。

Result: 实验显示流模型倾向于在没有持续引导的情况下回到默认轨迹。实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。与无引导基线相比，引导生成将语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地引导大型生成系统，增强几何控制能力，拓宽文本到图像合成的创意范围，为解决几何约束与风格冲突问题提供了有效途径。
...
Method: GeCo通过融合残差运动信息和深度先验，构建几何基础的一致性度量，生成稠密的一致性图来检测几何变形和遮挡不一致伪影。该方法无需训练即可作为指导损失函数应用于视频生成过程。

Result: GeCo能够有效检测视频生成模型中的常见故障模式，为模型评估提供系统性基准测试，并作为无训练指导损失显著减少视频生成过程中的变形伪影。

Conclusion: GeCo为视频生成模型提供了一种有效的几何一致性评估工具，既能系统性地揭示模型缺陷，又能作为无训练指导机制提升生成视频的几何稳定性。
...
TL;DR: 该研究开发了B&J基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现优异（>90%），但在需要多模态整合的开放式任务中表现显著下降（约60%），显示当前AI模型尚不具备复杂临床推理能力。
...
Method: 开发了B&J基准测试，包含1,245个来自真实骨科和运动医学病例的问题，评估模型在7个临床推理任务上的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，与专家提供的真实答案进行比较。

Result: 结果显示任务类型间存在显著性能差距：最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放式任务中准确率仅约60%。VLM在医学图像解释方面存在严重限制，经常出现严重的文本驱动幻觉，忽视矛盾的视觉证据。专门针对医学应用微调的模型相比通用模型没有一致优势。

Conclusion: 当前人工智能模型尚不具备复杂多模态临床推理能力，其安全部署应限于支持性、基于文本的角色。核心临床任务的未来进展需要多模态整合和视觉理解方面的根本性突破。
...
TL;DR: Fetal-Gauge是首个专门评估视觉语言模型在胎儿超声成像中性能的大规模基准测试，包含4.2万张图像和9.3万个问答对，揭示了当前模型仅55%的准确率，远低于临床需求。


<details>
  <summary>Details</summary>
Motivation: 全球超声医师短缺阻碍了胎儿健康监测，深度学习可提升超声医师效率并支持培训。视觉语言模型在超声解释中前景广阔，但缺乏标准化评估基准，主要由于超声成像的挑战性、操作者依赖性和公开数据集有限。

Method: 开发了Fetal-Gauge基准测试，包含超过42,000张图像和93,000个问答对，涵盖解剖平面识别、解剖结构视觉定位、胎儿方向评估、临床视图符合性和临床诊断等多个任务。系统评估了包括通用和医学专用模型在内的多个最先进的视觉语言模型。

Result: 最佳模型仅达到55%的准确率，远低于临床要求。分析揭示了当前视觉语言模型在胎儿超声解释中的关键局限性，突显了对领域适应架构和专门训练方法的迫切需求。
...
Conclusion: Uni4D框架在动态多模态理解和实际应用方面取得了进展，为3D检索和4D生成提供了有效的统一解决方案。
...
TL;DR: 开发了用于低成本边缘设备（树莓派5和Google Coral Edge TPU）的单摄像头驾驶员行为识别系统，能够实时监测17种分心和困倦相关行为，实现低延迟、低功耗的驾驶员监控。


<details>
  <summary>Details</summary>
Motivation: 车内驾驶员监控系统需要在计算、功耗和成本严格受限的条件下，以低延迟识别分心和困倦相关行为。现有系统往往需要昂贵硬件或无法满足实时性要求。

Method: 提出一个包含三个组件的系统：1) 紧凑的逐帧视觉模型；2) 考虑混淆因素的标签设计以减少视觉相似误报；3) 时间决策头，仅在预测既自信又持续时才触发警报。系统覆盖17种行为类别。

Result: 在树莓派5上使用INT8推理达到约16 FPS（每帧延迟低于60ms），在Coral Edge TPU上达到约25 FPS，能够在廉价硬件上实现实时监控和稳定警报生成。使用多样化数据集进行训练和评估。

Conclusion: 该系统展示了在低成本边缘设备上实现可靠车内驾驶员行为监控的可行性，为以人为中心的车辆智能（包括新兴的智能车辆概念）提供了上游感知输入。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时均匀处理所有token，即使只修改小区域也会全图去噪，导致计算冗余和未修改区域质量下降。需要解决是否需要在编辑时重新生成每个区域的问题。
...
Conclusion: SpotEdit提出了一种训练免费的扩散编辑框架，通过选择性区域更新解决了当前方法中的计算冗余问题，在保持编辑质量的同时显著提高了效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注前向建模（如从文本到运动的整体映射），而缺乏从整体运动中分解出语义上有意义的子组件的能力。本文旨在解决运动分解的逆问题。

Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布。包含三个训练变体：1) DeMoGen-Exp使用分解的文本提示进行显式训练；2) DeMoGen-OSS进行正交自监督分解；3) DeMoGen-SC强制执行原始和分解文本嵌入之间的语义一致性。

Result: 该方法能够从复杂运动序列中解耦可重用的运动原语，分解后的运动概念可以灵活重组生成多样新颖的运动，实现超出训练分布的泛化。同时构建了文本分解数据集支持组合训练。

Conclusion: DeMoGen提供了一种有效的运动分解学习框架，能够发现和组合运动概念，为文本到运动生成和运动组合提供了扩展资源，推动了运动理解与生成的可组合性研究。
...
TL;DR: 该研究提出了一种基于变分自编码器的多视图潜在表示学习框架，用于从T1Gd和FLAIR MRI影像中整合互补的放射组学特征，以非侵入性推断胶质母细胞瘤的MGMT启动子甲基化状态。


<details>
  <summary>Details</summary>
Motivation: 传统单模态和早期融合方法在基于放射组学的MGMT启动子甲基化预测中存在特征冗余高、模态特异性信息建模不完整的问题，需要更有效的多模态整合方法。
...
Conclusion: 提出的多视图潜在表示学习框架能够更好地整合多模态MRI特征，为胶质母细胞瘤的MGMT启动子甲基化状态的非侵入性预测提供了改进方法。
...
TL;DR: 本文提出了一种基于视觉变换器的端到端管道，用于在H&E和IHC全玻片图像上进行HER2状态评分，实现像素级定位和4级评分（0, 1+, 2+, 3+）
...
Method: 提出端到端视觉变换器系统：1）对H&E全玻片图像进行分块处理定位肿瘤区域；2）提出新颖映射函数在IHC图像中识别与H&E恶性区域对应的区域；3）嵌入临床启发的HER2评分机制，实现4级（0, 1+, 2+, 3+）像素级自动标注；4）准确区分HER2阴性和阳性

Result: 在13个病例的私有数据集上测试：肿瘤定位分类准确率良好；HER2状态预测准确率达0.94，特异性0.933；4级评分方法表现优异；与人类病理学家比较显示该方法具有实用性

Conclusion: 研究表明基于视觉变换器的端到端模型能够有效联合评估H&E和IHC图像进行HER2评分，为临床提供自动化的像素级HER2状态分析工具
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得了显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在3D物体布置任务中存在三大挑战：视觉基础薄弱难以将程序化编辑与精确3D结果关联、缺乏3D场景理解能力、迭代更新过程容易出错。

Method: 1. 引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2. 通过专用视觉工具套件增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3. 提出协作式多智能体框架，分配规划、执行和验证角色，以处理多步骤指令并从中间错误中恢复。

Result: 在25个复杂的物体布置任务上验证了方法的有效性，系统显著优于现有基线方法。

Conclusion: 该研究成功将MLLMs应用于复杂3D场景操作，通过创新的API设计、视觉增强和多智能体协作，有效解决了3D物体布置任务中的关键挑战，为MLLMs在3D领域的应用开辟了新途径。
...
TL;DR: Self-E是一个从零开始训练的文本到图像生成模型，支持任意步数推理，通过自评估机制实现少步高质量生成。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型或流匹配模型需要多步推理才能获得高质量结果，而蒸馏方法需要预训练教师模型。Self-E旨在填补这两种范式之间的空白，实现从零开始训练、支持任意步数的高质量生成模型。

Method: Self-E采用从零开始的训练方法，结合流匹配模型的学习方式，并引入新颖的自评估机制：模型使用当前的分数估计来评估自己生成的样本，作为动态的自教师。这种方法结合了瞬时局部学习和自驱动的全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E不仅在少步生成方面表现出色，而且在50步时与最先进的流匹配模型竞争。其性能随着推理步数增加而单调提升，实现了超快速少步生成和高质量长轨迹采样。

Conclusion: Self-E是第一个从零开始、支持任意步数的文本到图像模型，为高效和可扩展的生成提供了一个统一框架。
...
TL;DR: iOSPointMapper是一款利用iPhone/iPad进行实时、隐私保护的实时人行道测绘移动应用，通过设备端语义分割、LiDAR深度估计和GPS/IMU融合数据检测交通标志等特征，支持用户验证后匿名上传至TDEI交通数据交换平台。
...
Method: 开发iOSPointMapper移动应用，利用新一代iPhone和iPad，结合设备端语义分割、LiDAR深度估计、GPS/IMU数据融合来检测和定位交通标志、交通信号灯和杆柱等特征，并包含用户引导的标注界面来验证系统输出。

Result: 系统特征检测和空间测绘性能的详细评估显示了应用在增强行人测绘方面的潜力，收集的数据经过匿名化处理后传输至TDEI交通数据交换计划，与更广泛的多模式交通数据集无缝集成。

Conclusion: iOSPointMapper提供了一种可扩展、以用户为中心的方法，能够填补行人基础设施中的关键数据缺口，为构建无障碍和包容性的行人基础设施提供支持。
...
TL;DR: DeFloMat提出了一种基于流匹配的生成式目标检测框架，通过条件流匹配替代扩散模型的多步去噪过程，在仅3步推理中实现了43.32% AP_{10:50}的SOTA性能，解决了临床应用中扩散检测器的延迟瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 解决扩散式目标检测器（如DiffusionDet）在时间敏感临床应用中的延迟瓶颈问题。扩散模型依赖多步随机去噪过程（T≫60步），不适用于克罗恩病磁共振小肠造影等临床场景。

Method: 提出DeFloMat框架，用条件流匹配替代扩散模型的随机去噪路径。基于条件最优传输理论，构建高度直接的确定性流场（近似整流流），通过简单ODE求解器实现快速推理。

Result: 在MRE临床数据集上，仅用3步推理达到43.32% AP_{10:50}的SOTA性能，比DiffusionDet在4步时的最佳性能（31.03%）提升1.4倍。确定性流显著改善了定位特性，在少步机制下获得更好的召回率和稳定性。

Conclusion: DeFloMat解决了生成式准确性与临床效率之间的权衡，为稳定快速的目标定位设定了新标准，特别适用于时间敏感的医疗检测应用。
...
<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能快速无创地可视化细胞形态，但稳健的体积分割通常依赖荧光标记或繁重的后处理。现有方法存在这一空白需要填补。

Method: Bright-4B结合了硬件对齐的原生稀疏注意力机制（捕获局部、粗粒度和选择的全局上下文）、深度-宽度残差超连接稳定表示流、软混合专家自适应容量，以及各向异性补丁嵌入尊重共聚焦点扩散和轴向变薄特性。

Result: 模型能够从明场堆栈中准确分割细胞核、线粒体和其他细胞器，在多个共聚焦数据集上保持跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线。

Conclusion: Bright-4B为大规模无标记3D细胞映射提供了有效的解决方案，所有代码、预训练权重和下游微调模型将公开发布以推动该领域发展。
...
TL;DR: 提出EmoCtrl模型，实现可控情感图像内容生成，在保持内容描述准确性的同时表达目标情感，解决现有模型要么缺乏情感意识要么牺牲内容准确性的问题。


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同传递意义，现有文本到图像模型能确保内容一致性但缺乏情感意识，而情感驱动模型生成情感结果却以内容失真为代价，需要解决这一差距。

Method: 提出EmoCtrl模型，基于内容、情感和情感提示标注的数据集，通过文本和视觉情感增强模块，利用描述性语义和感知线索丰富情感表达，学习互补的情感标记。

Result: EmoCtrl在忠实内容和表达情感控制方面优于现有方法，定量和定性实验证明其有效性，用户研究确认与人类偏好高度一致，并能很好地推广到创意应用中。

Conclusion: EmoCtrl成功实现了可控情感图像内容生成，学习的情感标记具有互补效应，在保持内容忠实性的同时表达目标情感，展示了鲁棒性和适应性。
...
TL;DR: 提出了一种基于线特征的事件相机标定方法，利用人造环境中的几何线条（如门窗、箱子等）进行标定，无需专用标定板，适用于单目和立体事件相机。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机标定方法通常需要闪烁图案、重建强度图像或手动放置标定物，这些方法耗时且无法适应快速变化的场景。需要一种更灵活、自动化的标定方法。

Method: 直接从事件流中检测线条，利用事件-线标定模型生成相机参数的初始估计，适用于平面和非平面线条，然后通过非线性优化细化相机参数。

Result: 仿真和真实世界实验验证了该方法的可行性和准确性，在单目和立体事件相机上均进行了验证，代码已开源。

Conclusion: 该方法提供了一种无需专用标定板的事件相机标定框架，利用环境中常见的几何线条，实现了更灵活、自动化的标定过程。
...
TL;DR: SonoVision是一款基于智能手机的应用程序，通过声音提示帮助视障人士定位日常物品，使用正弦声波在耳机中指示物体方向，支持完全离线工作。
...
Result: 开发了SonoVision应用程序，能够帮助视障人士通过智能手机和耳机定位物体，减少对周围人的依赖，增强独立性。应用支持完全离线工作。
...
TL;DR: SAM 3D作为通用图像到3D基础模型在单目遥感建筑重建中表现出色，相比TRELLIS能生成更连贯的屋顶几何和更清晰的边界，并可通过分段-重建-组合流程扩展到城市场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有的单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用图像到3D基础模型SAM 3D在遥感建筑重建中的表现，探索其在城市建模中的应用潜力。

Method: 在NYC Urban Dataset上对SAM 3D与TRELLIS进行系统比较，使用FID和CLIP-based MMD作为评估指标。进一步通过分段-重建-组合流程将SAM 3D扩展到城市场景重建。

Result: 实验结果表明，SAM 3D相比TRELLIS能生成更连贯的屋顶几何和更清晰的边界。通过扩展流程，SAM 3D展示了在城市场景建模中的潜力。

Conclusion: SAM 3D作为通用基础模型在单目遥感建筑重建中表现优异，为城市3D重建部署提供了实用指导，并激励未来整合场景级结构先验的研究方向。
...
TL;DR: 该研究比较了YOLOv8、YOLOv11和RF-DETR三种计算机视觉模型在电力变电站组件自动检测中的性能，旨在实现高效、准确的变电站基础设施映射。


<details>
  <summary>Details</summary>
Motivation: 电力变电站是电网的关键组成部分，其设备容易受到飓风、洪水、地震等多种灾害的破坏。传统的人工变电站基础设施映射方法耗时耗力，需要一种自动化的解决方案来提高效率和便利性，以预防和减轻电网故障带来的经济和安全影响。

Method: 研究使用手动标注的美国变电站图像数据集，训练并比较了三种计算机视觉模型：YOLOv8、YOLOv11和RF-DETR。通过评估这些模型的检测准确性、精度和效率，分析各自的优势和局限性。

Result: 研究展示了三种模型在变电站组件检测中的性能表现，识别出哪种模型能够提供可靠且可扩展的变电站组件映射。同时，成功利用这些模型对美国各地的变电站组件进行了有效映射。

Conclusion: 该研究证明了机器学习在变电站映射中的应用价值，为电力基础设施的自动化监测和维护提供了可行的技术方案，有助于提高电网的韧性和可靠性。
...
TL;DR: PGR²M提出了一种结合姿态码和残差码的混合表示方法，通过残差向量量化来增强运动生成和编辑的保真度与局部可控性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于姿态码的框架（如CoMo）虽然支持可解释的运动控制，但其逐帧表示难以捕捉细微的时序动态和高频细节，导致重建保真度和局部可控性下降。

Method: 提出PGR²M混合表示方法：1）姿态引导的RVQ分词器将运动分解为编码粗粒度全局结构的姿态潜在变量和编码细粒度时序变化的残差潜在变量；2）残差丢弃防止对残差的过度依赖；3）基础Transformer自回归预测姿态码，精炼Transformer在文本、姿态码和量化阶段条件下预测残差码。

Result: 在HumanML3D和KIT-ML数据集上的实验表明，PGR²M在Fréchet起始距离和重建指标上优于CoMo及最近的扩散和分词基线方法，用户研究证实其能实现直观、结构保持的运动编辑。

Conclusion: PGR²M通过结合可解释的姿态码和细粒度的残差码，有效提升了文本驱动的3D运动生成和编辑的质量，同时保持了语义对齐和可编辑性。
...
TL;DR: 提出基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析实现冲击波前提取，最终实现3D重建和爆炸当量反演，测量误差最大5.20%，最小0.06%。
...
Method: 1) 建立极坐标系编码事件以揭示冲击波传播模式，通过事件偏移计算进行自适应ROI提取；2) 利用速度变化的连续性，采用迭代斜率分析提取冲击波前事件；3) 基于事件光学成像模型推导事件几何模型和冲击波运动参数，结合3D重建模型。

Result: 与压力传感器和经验公式对比，速度测量最大误差5.20%，最小误差0.06%，实现了高时空分辨率的冲击波运动场高精度测量。

Conclusion: 该方法利用事件相机的高速高动态范围特性，实现了多角度冲击波测量、运动场重建和爆炸当量反演，在冲击波测量方面取得了显著进展。
...
TL;DR: 该研究提出了一种针对红外小目标检测的半监督范式，通过分层MoE适配器和两阶段知识蒸馏/迁移，在仅有10%标注数据的情况下，使下游模型达到或超越全监督模型的性能。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测标注成本高昂，现有方法如SAM存在领域差距、无法编码物理先验和架构复杂等问题，需要有效的半监督范式来解决数据稀缺问题。

Method: 设计了包含四个白盒神经算子的分层MoE适配器，提出两阶段范式：1)先验引导知识蒸馏，用MoE适配器和10%全监督数据将SAM蒸馏为专家教师模型Scalpel-SAM；2)部署导向知识迁移，用Scalpel-SAM生成伪标签训练轻量高效的下游模型。

Result: 实验表明，在最小标注情况下，该范式使下游模型达到或超越其全监督对应模型的性能，首次系统性地利用SAM作为教师模型解决IR-SOT数据稀缺问题。

Conclusion: 该研究提出的半监督范式有效解决了红外小目标检测中的数据稀缺问题，通过创新的知识蒸馏和迁移方法，在低标注成本下实现了高性能检测。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM存在推理不可靠的问题，产生流畅但缺乏可信度的推理过程，表现出弱的步骤间连贯性和不足的视觉基础。这是因为现有的对齐方法只监督最终答案，而忽略了中间推理过程的可靠性。

Method: 提出了SR-MCR框架，包含五个自参考线索：语义对齐、词汇保真度、非冗余性、视觉基础和步骤一致性。这些线索被整合成一个归一化的可靠性加权奖励，提供细粒度的过程级指导。采用无批评者的GRPO目标，并加入置信度感知冷却机制来稳定训练并抑制平凡或过度自信的生成。

Result: 基于Qwen2.5-VL构建的SR-MCR在广泛的视觉基准测试中提高了答案准确性和推理连贯性。在可比较规模的开源模型中，SR-MCR-7B实现了最先进的性能，平均准确率达到81.4%。消融研究证实了每个奖励项和冷却模块的独立贡献。

Conclusion: SR-MCR通过利用模型输出中的内在过程信号，提供了一种轻量级、无需标注的方法来对齐多模态LLM的推理过程，显著提高了推理的可靠性和连贯性。
...
TL;DR: 提出了一种基于稀疏差分Transformer的预测驱动Top-K Jaccard相似度系数方法，用于改进人脸聚类中嵌入向量关系的测量精度，在MS-Celeb-1M等数据集上实现了SOTA性能。
...
Method: 1. 提出预测驱动的Top-K Jaccard相似度系数，增强邻居节点的纯度；2. 开发基于Transformer的预测模型，分析中心节点与其Top-K附近邻居节点的关系；3. 提出稀疏差分Transformer（SDT）替代标准Transformer，消除噪声并增强抗噪能力。
...
Method: 构建Dream-VL（基于扩散的视觉语言模型）和Dream-VLA（基于dLLM的视觉语言动作模型），利用扩散模型的双向特性进行动作分块和并行生成。

Result: Dream-VL在多种基准测试中与顶级AR-based VLMs相当，在视觉规划任务中表现更优；Dream-VLA在LIBERO上达到97.2%平均成功率，在SimplerEnv-Bridge上71.4%，在SimplerEnv-Fractal上60.5%，超越π₀和GR00T-N1等领先模型。
...
TL;DR: Envision是一个基于扩散模型的视觉规划框架，通过两阶段方法生成物理合理且目标一致的轨迹：首先生成目标图像，然后插值生成连接起始状态和目标状态的视频轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要是前向预测，仅基于初始观察生成轨迹而缺乏明确的目标建模，导致空间漂移和目标对齐问题。需要一种能够生成物理合理且目标一致的视觉规划方法。

Method: Envision采用两阶段扩散模型框架：1) 目标图像模型识别任务相关区域，通过区域感知交叉注意力合成连贯的目标图像；2) 环境-目标视频模型基于首尾帧条件视频扩散模型(FL2V)，在初始观察和目标图像之间插值生成平滑物理合理的视频轨迹。

Result: 在物体操作和图像编辑基准测试中，Envision相比基线方法在目标对齐、空间一致性和物体保持方面表现更优，生成的视觉规划可直接支持下游机器人规划和控制。

Conclusion: Envision通过显式约束生成过程与目标图像，解决了现有视觉规划方法中的空间漂移和目标对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估（IQA）模型通常只输出单一全局分数，对局部和细粒度失真极不敏感，这导致图像超分辨率（ISR）模型可能产生感知上不理想的伪影却获得虚假高分，造成优化目标与感知质量不对齐的奖励黑客问题。
...
Result: 实验验证了该方法在RLHF框架下对ISR模型的有效性，在全局质量和局部真实感方面均表现出色，同时抑制了奖励黑客现象，实现了稳定训练。

Conclusion: 通过细粒度感知奖励模型和协同进化课程学习机制，成功解决了传统IQA模型在图像超分辨率RLHF中的局限性，实现了更准确的感知质量对齐和稳定训练。
...
Result: 在室内基准测试中，在受限训练条件下达到最先进性能；在室外数据集上也表现出色；建立了自回归先验作为深度估计的几何感知生成模型补充家族

Conclusion: 自回归先验是深度估计的有效方法，具有数据可扩展性和对3D视觉任务的适应性优势，为几何感知生成模型提供了新的研究方向
...
TL;DR: 该研究评估了多种深度学习架构（3D Inception、双流、CNN-RNN）用于从超声心动图视频估计左心室射血分数（LVEF），发现改进的3D Inception架构在EchoNet-Dynamic数据集上表现最佳（RMSE 6.79%），同时观察到较小模型泛化更好且性能对超参数敏感。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是评估LVEF的常用非侵入性方法，但人工评估耗时且存在观察者间差异。深度学习有望达到专家水平性能，因此研究探索了不同深度学习架构在LVEF估计中的有效性。

Method: 研究比较了多种深度学习架构：3D Inception、双流模型和CNN-RNN模型。在EchoNet-Dynamic数据集（10,030个超声心动图视频）上系统评估了架构修改和融合策略，以最大化预测准确性。

Result: 改进的3D Inception架构表现最佳，均方根误差（RMSE）为6.79%。观察到过拟合趋势，较小较简单的模型通常泛化更好。模型性能对超参数选择高度敏感，特别是卷积核大小和归一化策略。

Conclusion: 虽然研究专注于超声心动图LVEF估计，但关于架构设计和训练策略的见解可能适用于更广泛的医学和非医学视频分析任务。3D Inception架构在LVEF估计中表现最优，但需要关注过拟合问题和超参数敏感性。
...
TL;DR: CLAdapter是一种新颖的集群注意力适配器，通过注意力机制和聚类中心将大规模预训练模型的知识适配到数据有限的下游任务中，在多种科学领域实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉领域受益于大规模数据集和预训练模型，但在数据有限的专业科学领域下游任务仍然面临重大挑战。需要一种方法能够将预训练模型学到的丰富知识有效适配到各种数据受限的下游场景。

Method: 提出Cluster Attention Adapter (CLAdapter)，引入注意力机制和聚类中心，通过分布相关性和变换矩阵来个性化增强转换特征。统一接口设计使其能够与多种模型架构（包括CNN和Transformer）在2D和3D场景中无缝集成。

Result: 在10个数据集上进行广泛实验，涵盖通用、多媒体、生物、医疗、工业、农业、环境、地理、材料科学、OOD和3D分析等多个领域，CLAdapter在数据有限的科学领域实现了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，能够将大规模预训练学到的丰富表示适配到各种数据有限的下游任务中，在多种科学领域展现出卓越效果。
...
Method: 提出INTERACT-CMIL框架，采用多头深度学习架构，通过共享特征学习与组合部分监督联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞异型性五个轴，并使用跨任务依赖损失强制跨任务一致性。

Result: 在486个专家标注的结膜活检切片数据集上评估，相比CNN和基础模型基线，INTERACT-CMIL在WHO4上获得55.1%的相对宏F1提升，在垂直扩散上获得25.0%的提升，提供与专家分级一致的可解释多标准预测。

Conclusion: 该框架为CMIL诊断提供了可重复的计算基准，向标准化数字眼病理学迈进了一步，能够提供连贯、可解释的多标准预测。
...
TL;DR: CritiFusion是一个无需额外训练的推理时框架，通过多模态语义批判和频域细化提升文本到图像生成的一致性和细节质量
...
Result: 在标准基准测试中显著提升文本到图像对应性和视觉质量的人类对齐指标，在人类偏好评分和美学评估方面持续提升性能，达到与最先进奖励优化方法相当的结果

Conclusion: CritiFusion通过语义批判和频谱对齐策略，有效提升了文本到图像生成的细节、真实性和提示保真度，证明了该方法的有效性
...
TL;DR: 提出ARFM（自回归流匹配）方法，用于序列连续数据的概率建模，通过视频数据集训练来预测长期轨迹点位置，在人类和机器人运动预测任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常在窄分布上训练，而大规模视频预测模型虽然视觉真实感强，但在复杂运动建模方面仍有不足。受视频生成规模化启发，需要开发能准确预测复杂运动的新方法

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。在多样化视频数据集上训练模型，以生成长期视野内的未来轨迹点位置

Result: 模型能够预测复杂运动，实验表明将机器人动作预测和人类运动预测建立在预测的未来轨迹上，能显著提升下游任务性能。开发了评估运动预测模型能力的基准测试

Conclusion: ARFM方法在运动预测方面表现出色，通过条件化预测能有效提升下游任务性能，为人类和机器人运动预测提供了有效的解决方案。代码和模型已公开
...
TL;DR: 提出SCPainter统一框架，结合3D高斯泼溅资产表示和扩散模型生成，实现自动驾驶场景中的3D资产插入和新视角合成，用于生成多样化的训练数据。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要3D资产插入和新视角合成来增强训练数据的多样性，但现有方法将这两个能力孤立处理，缺乏统一的框架来实现逼真的资产插入和场景交互。

Method: 提出SCPainter框架：1) 使用3D高斯泼溅表示汽车资产和3D场景点云；2) 将两者投影到新视角；3) 用这些投影条件化扩散模型生成高质量图像。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现3D资产插入和新视角合成，有助于创建多样化和逼真的驾驶数据。

Conclusion: SCPainter提供了一个统一框架，将3D资产插入和新视角合成相结合，能够生成多样化的自动驾驶训练数据，提高模型的鲁棒性和安全性。
...
TL;DR: 本研究评估了超声特异性自监督预训练（USF-MAE）在早孕期超声图像中检测囊性水瘤的性能，结果显示其优于传统监督学习方法DenseNet-169。
...
Method: 使用基于掩码自编码（MAE）的超声自监督基础模型（USF-MAE），该模型在超过37万张未标注超声图像上预训练，然后针对本研究中的正常对照和囊性水瘤病例进行二元分类微调。使用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议进行评估。

Result: USF-MAE在所有评估指标上均优于DenseNet-169基线：平均准确率0.96 vs 0.93，敏感性0.94 vs 0.92，特异性0.98 vs 0.94，ROC-AUC 0.98 vs 0.94。Score-CAM可视化显示模型预测具有临床相关性，Wilcoxon符号秩检验证实性能改进具有统计学显著性（p=0.0057）。

Conclusion: 超声特异性自监督预训练能够促进准确、稳健的深度学习检测囊性水瘤，为解决小标注数据集限制提供了有效方法，为可扩展的早期筛查项目提供了支持。
...
Method: 提出Freetime FeatureGS，将动态场景建模为具有可学习特征和线性运动能力的高斯基元集合。应用对比损失使基元特征根据2D分割图中的实例归属关系接近或远离。采用时间顺序采样训练，实现特征在时间维度上的流式传播。

Result: 在多个数据集上的实验结果表明，该方法的重建质量大幅优于现有方法。

Conclusion: Freetime FeatureGS方法通过消除对视频分割的依赖，实现了更可靠的分解4D场景重建，特征学习和流式训练策略有效避免了优化过程中的局部最小值问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时，将图像编码为令牌序列并与文本令牌拼接处理，但视觉令牌数量的增加会显著提高推理成本。现有视觉令牌剪枝方法往往忽略了长上下文多图像场景的挑战。
...
TL;DR: 基于希尔伯特曲线重排序的邻居感知令牌缩减方法，通过保留2D空间中的邻居结构来提升Vision Transformers的计算效率


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在视觉识别任务中取得了显著成功，但冗余的令牌表示限制了其计算效率。现有的令牌合并和剪枝策略往往忽略了空间连续性和邻居关系，导致局部上下文信息的丢失。

Method: 提出基于希尔伯特曲线重排序的邻居感知令牌缩减方法，将2D空间中的邻居结构显式保留在1D序列表示中。包含两个关键策略：邻居感知剪枝（NAP）用于选择性保留令牌，基于相邻令牌相似性的合并（MAT）用于局部令牌聚合。

Result: 实验表明，该方法相比现有方法实现了最先进的准确率-效率权衡。

Conclusion: 这项工作强调了空间连续性和邻居结构的重要性，为Vision Transformers的架构优化提供了新的见解。
...
TL;DR: 提出一种基于算术分级响应模型（AGRM）的AGIQA质量分级模块（AGQG），通过建模图像能力与多难度等级来解决文本-图像语义漂移问题，提升AGIQA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多等级质量描述文本嵌入的相似度来评估图像质量，但发现相似度分布常呈现多模态模式（如图像嵌入同时与"优秀"和"差"等级高度相似），这种文本嵌入与预期描述之间的语义不一致性（语义漂移）影响了文本-图像共享空间学习的可靠性。

Method: 受心理测量学启发，提出改进的分级响应模型（GRM）用于AGIQA。设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度等级。为确保难度等级的单调性，以算术方式建模难度生成，强制产生单峰且可解释的质量分布。该AGQG模块具有即插即用优势。

Result: AGQG模块集成到各种最先进的AGIQA框架中能持续提升性能，同时在自然图像和屏幕内容图像质量评估中都能有效泛化，显示出作为未来IQA模型关键组件的潜力。

Conclusion: 提出的基于算术GRM的质量分级模块通过解决语义漂移问题，提供了一种可靠且可解释的AGIQA方法，具有广泛的适用性和改进潜力。
...
### [56] [Parallel Diffusion Solver via Residual Dirichlet Policy Optimization](https://arxiv.org/abs/2512.22796)
*Ruoyu Wang,Ziyu Li,Beier Zhu,Liangyu Yuan,Hanwang Zhang,Xun Yang,Xiaojun Chang,Chi Zhang*

Main category: cs.CV

TL;DR: EPD-Solver是一种新颖的ODE求解器，通过并行梯度评估减少截断误差，在保持低延迟的同时提升扩散模型采样质量，并可通过蒸馏和强化学习优化。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但顺序去噪导致采样延迟高。现有基于求解器的加速方法在低延迟预算下常面临图像质量显著下降，主要原因是无法捕捉高曲率轨迹段而导致的累积截断误差。

Method: 提出EPD-Solver，利用向量值函数的均值定理更准确逼近积分解，通过并行梯度评估减少误差。采用两阶段优化框架：首先通过蒸馏方法优化可学习参数，然后提出参数高效的强化学习微调方案，将求解器重新表述为随机Dirichlet策略，仅在低维求解器空间操作。

Result: EPD-Solver能够有效减轻截断误差，保持低延迟采样特性，在复杂文本到图像生成任务中提升性能。该方法还可作为插件（EPD-Plugin）改进现有ODE采样器。

Conclusion: EPD-Solver通过并行梯度评估和两阶段优化，在保持低延迟的同时显著提升扩散模型采样质量，为解决现有加速方法的质量下降问题提供了有效方案。
...
### [57] [VPTracker: Global Vision-Language Tracking via Visual Prompt and MLLM](https://arxiv.org/abs/2512.22799)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言跟踪方法局限于局部搜索，在视角变化、遮挡和快速目标移动等挑战场景下容易失败，需要更鲁棒的全局跟踪框架

Method: 提出基于多模态大语言模型的全局跟踪框架VPTracker，引入位置感知视觉提示机制，基于目标先前位置构建区域级提示，优先进行区域级识别，仅在必要时进行全局推理

Result: 实验表明该方法显著提升了跟踪稳定性和目标辨别能力，在挑战场景下表现优异，为MLLM在视觉跟踪中的应用开辟了新途径

Conclusion: VPTracker成功将多模态大语言模型引入视觉跟踪领域，通过全局搜索和位置感知机制的结合，实现了更鲁棒和准确的跟踪性能
...
### [58] [Medical Scene Reconstruction and Segmentation based on 3D Gaussian Representation](https://arxiv.org/abs/2512.22800)
...
### [59] [Evaluating the Performance of Open-Vocabulary Object Detection in Low-quality Image](https://arxiv.org/abs/2512.22801)
*Po-Chih Wu*
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测旨在实现接近人类水平的识别能力，但现有模型在现实世界低质量图像条件下的性能尚未得到充分评估。本研究旨在填补这一空白，评估现有模型在低质量图像上的表现。

Method: 创建了一个模拟现实世界低质量图像的新数据集，用于评估开放词汇目标检测模型。评估了包括OWLv2、OWL-ViT、GroundingDINO和Detic在内的多个模型在不同级别图像退化条件下的性能。

Result: 实验发现：1）在低级图像退化时，所有模型的mAP分数没有显著下降；2）在高级图像退化时，所有模型性能急剧下降；3）OWLv2在不同类型退化中表现最稳定，而OWL-ViT、GroundingDINO和Detic性能显著下降。

Conclusion: 开放词汇目标检测模型对高级图像退化敏感，OWLv2相对鲁棒性最强。研究将发布数据集和代码以促进未来研究，为开发更鲁棒的开放词汇检测模型提供基准。
...
### [60] [EgoReAct: Egocentric Video-Driven 3D Human Reaction Generation](https://arxiv.org/abs/2512.22808)
...
TL;DR: EgoReAct：首个自回归框架，从自我中心视频流实时生成3D对齐的人类反应动作，解决了现有方法在严格因果生成和精确3D空间对齐方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 人类对自我中心视觉输入表现出适应性和上下文敏感的反应，但现有方法难以同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在自我中心视频与反应动作之间的空间不一致问题。

Method: 1. 构建Human Reaction Dataset (HRD)解决数据稀缺和错位问题；2. 提出EgoReAct自回归框架：通过向量量化变分自编码器压缩反应动作到紧凑潜在空间，然后训练生成式预训练Transformer从视觉输入生成反应；3. 整合3D动态特征（度量深度和头部动态）增强空间基础。

Result: EgoReAct在真实性、空间一致性和生成效率方面显著优于现有方法，同时保持生成过程的严格因果性，能够实时生成3D对齐的人类反应动作。

Conclusion: EgoReAct是首个能够从自我中心视频流实时生成3D对齐人类反应动作的自回归框架，通过构建高质量数据集和整合3D动态特征，有效解决了空间对齐和因果生成的挑战。
...
### [61] [3D Scene Change Modeling With Consistent Multi-View Aggregation](https://arxiv.org/abs/2512.22830)
*Zirui Zhou,Junfeng Ni,Shujie Zhang,Yixin Chen,Siyuan Huang*

Main category: cs.CV

TL;DR: SCaR-3D：基于3D高斯溅射的3D场景变化检测框架，通过有符号距离2D差分和多视图聚合实现对象级变化检测，支持连续场景重建
...
Method: 提出SCaR-3D框架：1）基于有符号距离的2D差分模块；2）带投票和剪枝的多视图聚合；3）利用3DGS一致性分离变化前后状态；4）选择性更新动态区域的连续场景重建策略

Result: 方法在准确性和效率上都优于现有方法，同时贡献了CCS3D合成数据集支持可控评估

Conclusion: SCaR-3D能够有效检测3D场景中的对象级变化，明确分离变化前后状态，并支持连续场景重建，在变化检测任务中表现出色
...
### [62] [A Minimal Solver for Relative Pose Estimation with Unknown Focal Length from Two Affine Correspondences](https://arxiv.org/abs/2512.22833)
*Zhenbao Yu,Shirong Ye,Ronghe Jin,Shunkun Liang,Zibin Liu,Huiyun Zhang,Banglei Guan*

Main category: cs.CV

TL;DR: 提出一种基于两个仿射对应关系，在已知垂直方向（来自IMU）的情况下，估计3自由度相对位姿和焦距的新方法。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、智能手机、无人机等应用中，相机常与IMU结合使用。IMU可以提供相机垂直方向，从而将相对位姿从5自由度减少到3自由度。现有方法需要更多对应关系，本文旨在仅用两个仿射对应关系解决此问题。

Method: 首先在已知垂直方向的情况下，从两个仿射对应关系建立约束方程。基于方程组有非平凡解的性质，推导出仅涉及焦距和相对旋转角的四个方程。最后利用多项式特征值方法求解焦距和相对旋转角。

Result: 在合成和真实数据集上的评估表明，提出的求解器性能优于现有的最先进方法。

Conclusion: 本文提出了一种高效的方法，仅需两个仿射对应关系即可在已知垂直方向的情况下估计相对位姿和焦距，在计算效率和精度方面优于现有方法。
...
Method: 提出了ByteLoom框架，采用扩散Transformer架构，使用简化的人体条件和3D对象输入。核心创新包括：1) RCM-cache机制，利用相对坐标图作为通用表示来保持对象几何一致性并精确控制6自由度对象变换；2) 渐进式训练课程，通过利用现有数据集增强模型能力并减少对手部网格的需求。

Result: 实验表明，该方法能够忠实保持人体身份和对象的多视角几何特性，同时保持平滑的运动和对象操作，在人-物交互视频生成方面表现出色。

Conclusion: ByteLoom通过创新的RCM-cache机制和渐进式训练策略，成功解决了人-物交互视频生成中的几何一致性和标注依赖问题，为数字人、电子商务、广告和机器人模仿学习等应用提供了有效的解决方案。
...
TL;DR: MUSON是一个用于短时程社交导航的多模态数据集，通过五步思维链标注（感知、预测、推理、行动、解释）提供结构化推理监督，解决了现有数据集缺乏明确推理指导和动作分布长尾的问题。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航数据集缺乏明确的推理监督，且动作分布呈现高度长尾，限制了模型学习安全关键行为的能力。需要一个新的数据集来提供结构化推理指导和平衡的动作空间。

Method: MUSON采用五步思维链标注：感知、预测、推理、行动、解释，明确建模静态物理约束，并创建理性平衡的离散动作空间。数据集在多样化的室内外校园场景中收集。

Result: 在MUSON上对多个最先进的小型视觉语言模型进行基准测试，Qwen2.5-VL-3B达到了最高的决策准确率0.8625，表明MUSON是社交合规导航的有效且可重复使用的基准。

Conclusion: MUSON通过结构化推理标注和平衡的动作空间，为社交合规导航提供了一个高质量的基准数据集，能够有效评估和提升模型的决策能力。
...
TL;DR: Lamps是一个从多视角学习解剖结构的自监督预训练模型，利用人体解剖学的一致性、连贯性和层次性作为监督信号，在胸部X光片上训练，展现出优异的鲁棒性、可迁移性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 医学影像的关键基础在于人体解剖结构，现有自监督学习方法往往忽视解剖学视角，限制了有效学习解剖特征的能力。需要开发能捕捉人体解剖结构一致性和层次性的预训练方法。

Method: 提出Lamps（从多视角学习解剖结构的自监督学习）模型，在大规模胸部X光片上进行预训练，通过协调利用人体解剖学的一致性、连贯性和层次性作为监督信号。

Result: 在10个数据集上通过微调和涌现特性分析进行广泛实验，与10个基线模型相比，Lamps展现出卓越的鲁棒性、可迁移性和临床潜力。

Conclusion: 通过多视角学习，Lamps为基础模型提供了发展有意义、鲁棒且与人体解剖结构对齐的表征的独特机会，为医学影像基础模型开辟了新方向。
...
TL;DR: 本文提出了一种数据导向的方法来缓解深度学习模型中的伪相关性，通过识别、中和、消除和更新四个步骤构建了一个有效的去偏流程，在图像和NLP去偏基准测试中相比标准经验风险最小化将最差组准确率提高了20%以上。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练过程中经常学习到与类别标签虚假相关但与预测任务无关的特征。现有方法通常需要标注潜在的虚假属性，或基于某些经验假设（如偏差的简单性）过滤虚假特征，但由于现实数据中虚假相关性的复杂性和难以捉摸性，这些方法可能表现不佳。

Method: 提出了一种数据导向的去偏方法：1）识别：观察到受虚假特征影响的样本在学习到的特征空间中呈现分散分布，从而识别虚假特征的存在；2）中和：基于简单的分组策略中和虚假特征，获得偏差不变表示；3）消除：通过学习特征变换与偏差不变表示对齐来消除虚假特征；4）更新：结合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提高了20%以上，表明该方法能有效缓解深度学习模型中的伪相关性。

Conclusion: 通过整合识别、中和、消除和更新四个步骤，构建了一个有效的缓解伪相关性的流程。该方法不需要标注虚假属性，而是基于数据本身特征来识别和处理虚假相关性，在多个基准测试中取得了显著改进。
...
TL;DR: 本文提出了M-ErasureBench多模态评估框架，用于系统评估概念擦除方法在文本提示、学习嵌入和反转潜在空间三种输入模态下的性能，并发现现有方法在多模态攻击下存在严重漏洞。为解决此问题，作者提出了IRECE插件模块，通过跨注意力定位目标概念并在去噪过程中扰动相关潜在向量，显著提升了概念擦除的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要针对文本提示，忽略了图像编辑和个性化生成等实际应用中日益重要的其他输入模态（如学习嵌入和反转潜在空间）。这些模态可能成为攻击面，导致已擦除的概念重新出现，因此需要建立全面的多模态评估框架来识别和解决这些漏洞。

Method: 1. 提出M-ErasureBench多模态评估框架，系统评估概念擦除方法在三种输入模态（文本提示、学习嵌入、反转潜在空间）下的性能，其中后两种模态分别评估白盒和黑盒访问，共五种评估场景。2. 提出IRECE（推理时鲁棒性增强概念擦除）插件模块，通过跨注意力机制定位目标概念，并在去噪过程中扰动相关潜在向量来增强鲁棒性。

Result: 实验表明：1. 现有方法在文本提示上表现良好，但在学习嵌入和反转潜在空间上基本失效，白盒设置下的概念再现率（CRR）超过90%。2. IRECE能显著恢复鲁棒性，在最具挑战性的白盒潜在反转场景下将CRR降低达40%，同时保持视觉质量。

Conclusion: M-ErasureBench是首个超越文本提示的全面概念擦除基准，揭示了现有方法在多模态攻击下的严重漏洞。IRECE提供了一种实用的插件解决方案，显著增强了概念擦除的鲁棒性。该工作为构建更可靠的保护性生成模型提供了重要基准和防护措施。
...
Method: 提出引导路径采样（GPS）新范式，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。同时设计最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的生成过程对齐。

Result: 在SDXL和Hunyuan-DiT等现代骨干模型上的实验表明，GPS在感知质量和复杂提示遵循方面优于现有方法。例如，在SDXL上获得0.79的ImageReward和0.2995的HPS v2分数，在GenEval上的整体语义对齐准确率提升至57.45%。

Conclusion: 路径稳定性是有效迭代精炼的前提条件，GPS提供了一个稳健的框架来实现这一目标，通过理论证明将误差序列从无界放大转变为严格有界，保证了稳定性。
...
TL;DR: 提出了一种基于高斯溅射坐标的哈希网格特征剪枝方法，通过识别和剪除无效特征来减少存储和传输开销，在不影响模型性能的情况下提升率失真性能。


<details>
  <summary>Details</summary>
Motivation: 哈希网格在隐式神经场学习中被广泛使用，但由于高斯溅射在3D空间中的不规则和非均匀分布，存在许多稀疏区域，导致哈希网格中的许多特征无效，造成冗余存储和传输开销。

Method: 提出哈希网格特征剪枝方法，基于输入高斯溅射的坐标识别和剪除无效特征，只对有效特征进行编码，从而减少哈希网格的存储大小。

Result: 在标准化委员会定义的通用测试条件下，该方法相比基线方法实现了平均8%的比特率降低，在不影响模型性能的情况下提升了率失真性能。

Conclusion: 通过剪除哈希网格中的无效特征，可以有效减少存储和传输开销，提升压缩效率，为基于哈希网格的隐式神经场学习提供了更高效的解决方案。
...
### [70] [JavisGPT: A Unified Multi-modal LLM for Sounding-Video Comprehension and Generation](https://arxiv.org/abs/2512.22905)
...
TL;DR: JavisGPT是首个用于联合音视频理解和生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练流程构建，在音视频同步任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够同时处理音视频理解和生成的统一多模态大语言模型，特别是在需要时空同步的复杂场景中，现有模型存在局限性。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音视频微调、大规模指令调优。

Result: 在音视频理解和生成基准测试中，JavisGPT优于现有MLLM，特别是在复杂和时间同步设置中表现突出。

Conclusion: JavisGPT成功实现了统一的音视频理解和生成能力，通过创新的架构设计和高质量数据集构建，在多模态任务中取得了显著进展。
...
### [71] [ColaVLA: Leveraging Cognitive Latent Reasoning for Hierarchical Parallel Trajectory Planning in Autonomous Driving](https://arxiv.org/abs/2512.22939)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的规划器面临三个关键挑战：离散文本推理与连续控制之间的不匹配、自回归链式思维解码的高延迟、以及低效或非因果规划器限制实时部署。需要一种既能保持VLM泛化能力和可解释性，又能实现高效准确轨迹生成的方法。
...
Conclusion: ColaVLA成功地将VLM的推理能力转移到统一潜在空间，结合分层并行解码，在保持VLM泛化能力和可解释性的同时，实现了高效、准确和安全的轨迹生成，为自动驾驶规划提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测器依赖交叉熵分类，容易受到类别不平衡和标签噪声的影响。需要一种更鲁棒的监督方法来提升检测性能
...
### [73] [YOLO-IOD: Towards Real Time Incremental Object Detection](https://arxiv.org/abs/2512.22973)
*Shizhou Zhang,Xueqiang Lv,Yinghui Xing,Qirui Wu,Di Xu,Chen Zhao,Yanning Zhang*

Main category: cs.CV

TL;DR: 本文提出了YOLO-IOD框架，这是首个基于YOLO的实时增量目标检测方法，解决了YOLO在增量学习中的知识冲突问题，并在新基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法支持实时YOLO检测框架。本文旨在解决YOLO在增量学习中的知识冲突问题，实现实时增量目标检测。

Method: 提出了YOLO-IOD框架，基于预训练的YOLO-World模型，通过阶段式参数高效微调实现增量学习。包含三个核心组件：1) 冲突感知伪标签细化(CPR)，缓解前景-背景混淆；2) 基于重要性的核选择(IKS)，识别并更新当前任务关键卷积核；3) 跨阶段非对称知识蒸馏(CAKD)，解决知识蒸馏不对齐问题。

Result: 在传统基准和新提出的LoCo COCO基准上进行了实验，YOLO-IOD实现了优越的性能和最小的遗忘效应，验证了方法的有效性。

Conclusion: YOLO-IOD是首个基于YOLO的实时增量目标检测框架，成功解决了YOLO在增量学习中的知识冲突问题，为实时增量目标检测提供了有效解决方案。
...
### [74] [RealCamo: Boosting Real Camouflage Synthesis with Layout Controls and Textual-Visual Guidance](https://arxiv.org/abs/2512.22974)
...
Method: 提出ReamCamo框架：1）引入布局控制调节全局图像结构，改善前景与背景的语义一致性；2）构建多模态文本-视觉条件，结合细粒度文本任务描述和纹理导向的背景检索，共同指导生成过程；3）基于外绘制方法。

Result: 通过大量实验和可视化验证了框架的有效性，并引入了背景-前景分布差异度量来定量评估伪装质量。

Conclusion: ReamCamo能够生成更真实、语义一致且视觉逼真的伪装图像，为伪装目标检测提供了高质量的训练数据生成方案。
...
TL;DR: PoseStreamer：一种针对高速运动场景设计的鲁棒多模态6DoF姿态估计框架，通过自适应姿态记忆队列、物体中心2D跟踪器和射线姿态滤波器提升性能，并在新数据集MoCapCube6D上验证效果。


<details>
  <summary>Details</summary>
Motivation: 在高速和低光场景下，传统RGB相机因运动模糊导致6DoF姿态估计性能下降，而事件相机虽然具有高时间分辨率，但现有方法在高速物体运动场景中表现不佳。

Method: 提出PoseStreamer框架，包含三个核心组件：1) 自适应姿态记忆队列，利用历史方向线索保持时间一致性；2) 物体中心2D跟踪器，提供强2D先验以提升3D中心召回率；3) 射线姿态滤波器，沿相机射线进行几何细化。同时构建了多模态数据集MoCapCube6D用于快速运动下的性能评估。

Result: 大量实验表明，PoseStreamer不仅在高速运动场景中实现了更高的精度，而且作为无模板框架对未见过的运动物体表现出强大的泛化能力。

Conclusion: PoseStreamer通过多模态融合和时间一致性机制，有效解决了高速运动场景下的6DoF姿态估计问题，为事件相机在动态环境中的应用提供了鲁棒解决方案。
...
TL;DR: SSA框架通过对称最优传输对齐和复合方向引导策略，解决了医学图像分割中同时处理诊断性、描述性文本和空间约束的难题


<details>
  <summary>Details</summary>
Motivation: 当前文本引导的医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，无法有效识别病灶并建立与图像区域的关联；2）现有方法只关注病灶描述而忽略位置约束，导致关键偏差（如"左下肺"可能错误覆盖双肺）
...
Conclusion: SSA框架有效增强了处理包含位置、描述和诊断信息的混合医学文本的能力，解决了现有方法在处理空间约束和多种文本类型时的局限性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的方法要么依赖预训练模型对主体有良好表示，要么需要对特定身份进行模型微调，缺乏对匿名化过程中面部属性的控制

Method: 采用条件扩散反演框架，结合身份引导的条件分支，直接操作图像而不使用文本提示，支持属性可控的匿名化

Result: 在身份移除、属性保留和图像质量之间实现了最先进的平衡，能够泛化到模型训练数据之外的主体

Conclusion: 提出的反向个性化框架为人脸匿名化提供了一种有效且可控的方法，在保持面部属性的同时有效移除身份信息
...
TL;DR: 提出一个基于RGB无人机图像的低成本果园智能管道，集成ResNet50进行叶片病害检测、VGG16进行苹果新鲜度评估、YOLOv8进行实时苹果检测定位，在ESP32-CAM和树莓派上实现完全离线推理。


<details>
  <summary>Details</summary>
Motivation: 果园需要及时的病害检测、果实质量评估和产量估算，但现有无人机系统通常孤立处理这些任务且依赖昂贵的多光谱传感器，需要一种低成本、统一的解决方案。

Method: 开发统一的RGB无人机果园智能管道：1) ResNet50用于叶片病害分类；2) VGG16用于苹果新鲜度确定；3) YOLOv8用于实时苹果检测和定位。系统在ESP32-CAM和树莓派硬件上运行，支持完全离线现场推理。

Result: 实验结果显示：叶片病害分类准确率98.9%，新鲜度分类准确率97.4%，苹果检测F1分数0.857。系统为多光谱无人机解决方案提供了可访问且可扩展的替代方案。

Conclusion: 该框架在低成本硬件上实现了实用的精准农业，提供了统一、低成本的RGB无人机解决方案，无需云支持即可实现完全离线推理。
...
### [79] [OpenGround: Active Cognition-based Reasoning for Open-World 3D Visual Grounding](https://arxiv.org/abs/2512.23020)
...
Result: 在Nr3D数据集上达到竞争性性能，在ScanRefer数据集上达到最先进水平，在提出的OpenTarget数据集上实现了17.6%的显著提升。

Conclusion: OpenGround通过主动认知推理模块有效克服了预定义对象查找表的限制，能够在预定义和开放世界类别下工作，为开放世界3D视觉定位提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类识别物体时能充分利用丰富的场景上下文信息（空间关系、材料属性、物体共现等），而传统计算识别系统通常在孤立图像区域上操作，忽略了这些关键上下文信息，导致性能受限。

Method: 1. 从单目图像构建地理语义上下文图（GSCG）：整合度量深度估计器和统一的全景材料分割模型，将物体编码为具有几何、颜色和材料属性的节点，空间关系编码为边；2. 提出专门的基于图的分类器，聚合目标物体、邻近物体和全局场景特征进行分类预测。

Result: 上下文感知模型达到73.4%的分类准确率，显著优于上下文无关版本（最低38.4%），并大幅超越微调ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 显式结构化和可解释的上下文表示对于物体识别任务具有显著优势，GSCG框架为场景理解提供了更接近人类认知的建模方式。
...
### [81] [Toward Stable Semi-Supervised Remote Sensing Segmentation via Co-Guidance and Co-Fusion](https://arxiv.org/abs/2512.23035)
*Yi Zhou,Xuechao Zou,Shun Zhang,Kai Li,Shiying Wang,Jingming Chen,Congyan Lang,Tengfei Cao,Pin Tao,Yuanchun Shi*

Main category: cs.CV

TL;DR: Co2S是一个用于遥感图像半监督语义分割的稳定框架，通过融合视觉语言模型和自我监督模型的先验知识，解决伪标签漂移问题，在多种数据集上取得领先性能。
...
Method: 提出Co2S框架：1）构建异构双学生架构，使用预训练的CLIP和DINOv3初始化两个不同的ViT基础模型；2）引入显式-隐式语义协同引导机制，利用文本嵌入和可学习查询提供显式和隐式类别级引导；3）开发全局-局部特征协作融合策略，融合CLIP的全局上下文信息和DINOv3的局部细节。

Result: 在六个流行数据集上进行广泛实验，证明该方法在各种划分协议和多样化场景下始终取得领先性能。

Conclusion: Co2S通过协同融合视觉语言模型和自我监督模型的先验知识，有效缓解了半监督遥感图像分割中的伪标签漂移问题，实现了稳定且高性能的语义分割。
...
### [82] [PathoSyn: Imaging-Pathology MRI Synthesis via Disentangled Deviation Diffusion](https://arxiv.org/abs/2512.23130)
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二值掩码，这些方法存在特征纠缠问题，导致解剖基底损坏或结构不连续。需要一种能保持解剖完整性同时生成逼真病理变化的合成方法。
...
TL;DR: GeoTeacher提出了一种半监督3D目标检测方法，通过几何关系监督模块和体素数据增强策略，提升学生模型在有限标注数据下对物体几何结构的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽略了在标注数据有限时模型对物体几何结构敏感性不足的问题，而几何信息对目标感知和定位至关重要。

Method: 1) 基于关键点的几何关系监督模块，将教师模型的几何知识传递给学生模型；2) 体素级数据增强策略增加物体几何多样性，并引入距离衰减机制保护远距离物体的完整性；3) 可与不同SS3D方法结合使用。

Result: 在ONCE和Waymo数据集上的大量实验表明该方法有效且具有良好泛化性，取得了新的最先进结果。

Conclusion: GeoTeacher通过增强学生模型对物体几何关系的理解能力，有效提升了半监督3D目标检测的性能，且具有良好的通用性。
...
TL;DR: REVEALER是一个基于强化学习引导视觉推理的统一框架，用于评估文本提示与生成图像之间的元素级对齐，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的对齐评估方法大多依赖粗粒度指标或静态QA流程，缺乏细粒度可解释性且难以反映人类偏好，需要更精细的对齐评估方法。

Method: 采用"grounding-reasoning-conclusion"结构化范式，通过多模态大语言模型显式定位语义元素并得出可解释的对齐判断，使用GRPO优化模型，结合结构格式、定位准确性和对齐保真度的复合奖励函数。

Result: 在EvalMuse-40K、RichHF、MHaluBench和GenAI-Bench四个基准测试中，REVEALER实现了最先进的性能，超越了强大的专有模型和监督基线，同时比现有迭代视觉推理方法具有更优的推理效率。

Conclusion: REVEALER提供了一个统一且可解释的框架，用于细粒度评估文本到图像模型的对齐质量，在准确性和效率方面均表现出色，为文本到图像模型的可靠性和可用性评估提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的3D检测方法面临两难困境：高精度方法需要密集3D监督，而无监督方法难以从图像中提取准确几何信息。本文旨在开发一个无需深度传感器或密集3D监督的高精度3D检测框架。

Method: 提出GVSynergy-Det框架，采用高斯-体素双表示架构：1) 使用可泛化的高斯泼溅技术提取几何特征；2) 开发跨表示增强机制，用高斯场的几何细节丰富体素特征；3) 通过可学习集成直接利用两种表示的特征。

Result: 在ScanNetV2和ARKitScenes等室内基准测试中取得最先进结果，显著优于现有方法，且无需任何深度或密集3D几何监督（如点云或TSDF）。

Conclusion: 高斯和体素表示捕获互补的几何信息，通过协同学习可以显著提升基于图像的3D检测性能，无需昂贵的深度传感器或密集3D监督，为实际应用提供了高效解决方案。
...
TL;DR: 基于3D高斯场景表示的驾驶世界模型框架，实现3D场景理解和多模态场景生成，通过语言特征嵌入实现早期模态对齐
...
Method: 提出基于3D高斯场景表示的统一框架，将丰富的语言特征嵌入到每个高斯基元中实现早期模态对齐。设计任务感知的语言引导采样策略去除冗余3D高斯并注入准确紧凑的3D令牌到LLM。构建双条件多模态生成模型，结合高级语言条件和低级图像条件共同指导生成过程。
...
### [87] [ForCM: Forest Cover Mapping from Multispectral Sentinel-2 Image by Integrating Deep Learning with Object-Based Image Analysis](https://arxiv.org/abs/2512.23196)
...
TL;DR: 提出ForCM方法，结合基于对象的图像分析(OBIA)和深度学习(DL)，使用Sentinel-2影像进行森林覆盖制图，在亚马逊雨林区域测试多种DL模型，与传统OBIA方法相比显著提高了精度。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法精度有限，需要开发更准确的方法来支持全球环境监测和保护工作。研究旨在探索深度学习与OBIA结合在森林覆盖制图中的应用潜力。

Method: 提出ForCM方法，结合OBIA和深度学习。使用Sentinel-2 Level 2A高分辨率卫星影像，测试UNet、UNet++、ResUNet、AttentionUNet和ResNet50-Segnet等深度学习模型。使用三个数据集：两个三波段影像集和一个四波段影像集。将最有效的DL模型与OBIA技术集成。

Result: ForCM方法显著提高了森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，而传统OBIA方法仅为92.91%。研究还展示了QGIS等免费工具在准确制图方面的潜力。

Conclusion: ForCM方法通过结合深度学习和OBIA技术，显著提高了森林覆盖制图的精度，为全球环境监测和保护提供了有效工具。研究证明了免费工具如QGIS在准确制图中的应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型应用中，时间步特征的选择主要依赖经验直觉，往往导致次优性能且偏向特定任务。为了解决这一问题，研究者探索了如何自适应选择最适合少样本密集预测任务的时间步特征。

Method: 提出了两个核心模块：任务感知时间步选择（TTS）基于时间步损失和相似度得分选择理想的时间步；时间步特征整合（TFC）整合选定的时间步特征以提升少样本密集预测性能。同时采用了参数高效的微调适配器。

Result: 在大规模挑战性Taskonomy数据集上进行了实证验证，特别是在通用和少样本学习场景下，该方法在密集预测性能上表现出优越性。

Conclusion: 通过自适应选择扩散模型时间步特征并进行整合，该框架在少样本密集预测任务中有效提升了性能，为扩散模型在密集预测任务中的应用提供了更优的解决方案。
...
### [89] [MM-UAVBench: How Well Do Multimodal Large Language Models See, Think, and Plan in Low-Altitude UAV Scenarios?](https://arxiv.org/abs/2512.23219)
*Shiqi Dai,Zizhi Ma,Zhicong Luo,Xuesong Yang,Yibin Huang,Wanyue Zhang,Chi Chen,Zonghao Guo,Wang Xu,Yufei Sun,Maosong Sun*

Main category: cs.CV

TL;DR: MM-UAVBench是一个专门评估多模态大语言模型在低空无人机场景下综合能力的基准测试，包含感知、认知和规划三个核心维度，共19个子任务和5700多个手动标注问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在低空无人机应用中的潜力尚未充分探索，现有基准测试很少涵盖低空场景的独特挑战，而无人机相关评估主要关注特定任务，缺乏对模型综合智能的统一评估。

Method: 提出了MM-UAVBench基准测试，系统性地评估MLLMs在低空无人机场景下的三个核心能力维度：感知、认知和规划。基准包含19个子任务，超过5.7K个手动标注问题，所有数据均来自公开数据集的真实无人机采集数据。

Result: 对16个开源和专有MLLMs的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析进一步揭示了空间偏差和多视角理解等关键瓶颈，这些因素阻碍了MLLMs在无人机场景中的有效部署。

Conclusion: MM-UAVBench基准测试将促进未来针对现实世界无人机智能的鲁棒可靠多模态大语言模型的研究，填补了低空场景下MLLMs综合评估的空白。
...
### [90] [Holi-DETR: Holistic Fashion Item Detection Leveraging Contextual Information](https://arxiv.org/abs/2512.23221)
*Youngchae Kwon,Jinyoung Choi,Injung Kim*

Main category: cs.CV

TL;DR: 本文提出Holi-DETR，一种利用上下文信息整体检测时尚物品的检测器，通过整合三种上下文信息（共现关系、空间布局、人体关键点关系）来提高检测精度。


<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化且子类别间相似性高，导致检测模糊性。传统检测器独立检测每个物品，忽略了时尚物品之间的有意义关系。

Method: 提出Holi-DETR，在Detection Transformer（DETR）基础上整合三种上下文信息：1）时尚物品间的共现关系；2）基于物品间空间布局的相对位置和大小；3）物品与人体关键点的空间关系。

Result: 实验表明，该方法比原始DETR提高了3.6个百分点（pp）的平均精度（AP），比最近开发的Co-DETR提高了1.1个百分点的AP。

Conclusion: 通过整体利用多种上下文信息，Holi-DETR能够更准确地检测时尚物品，有效减少检测模糊性，在时尚物品检测任务上取得了显著性能提升。
...
### [91] [Bridging Your Imagination with Audio-Video Generation via a Unified Director](https://arxiv.org/abs/2512.23222)
...
Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错，后解耦"的训练范式：1)交错概念学习，使用交错文本-图像数据培养模型对剧本的深入理解和想象力解释；2)解耦专家学习，将剧本写作与关键帧生成解耦，提高故事叙述的灵活性和创造性。

Result: 大量实验表明，UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一在单一框架中，为非专业人士创作长上下文、多镜头电影提供了有效工具，证明了统一导演模型的可行性和优势。
...
### [92] [Anomaly Detection by Effectively Leveraging Synthetic Images](https://arxiv.org/abs/2512.23227)
...
Method: 1. 使用预训练的文本引导图像到图像翻译模型生成合成缺陷图像；2. 利用图像检索模型评估生成图像与真实正常图像的相似度，过滤不相关输出以提高质量；3. 提出两阶段训练策略：首先在大量基于规则合成的图像上进行预训练，然后在较小的高质量图像集上进行微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性，能够在显著降低数据收集成本的同时改善异常检测性能。

Conclusion: 该框架通过结合预训练翻译模型和图像检索模型，实现了高效、高质量的合成缺陷图像生成，配合两阶段训练策略，为工业异常检测提供了一种成本效益高的解决方案。
...
TL;DR: 提出了一种用于遥感扩散基础模型的无训练两阶段数据剪枝方法，能够在高剪枝率下快速选择高质量数据子集，提升模型收敛速度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感扩散基础模型依赖大量全局代表性数据，但这些数据通常包含冗余、噪声和类别不平衡问题，降低了训练效率并阻碍收敛。现有方法通常简单聚合多个分类数据集或应用简单去重，忽视了生成模型的分布需求和遥感图像的异质性。

Method: 提出无训练的两阶段数据剪枝方法：1) 基于熵的准则高效去除低信息样本；2) 利用遥感场景分类数据集作为参考基准，进行场景感知聚类和分层采样，在平衡聚类级均匀性和样本代表性的同时，在高剪枝率下实现细粒度选择。

Result: 实验表明，即使在剪枝85%的训练数据后，该方法仍能显著改善收敛性和生成质量。使用该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续达到最先进的性能。

Conclusion: 这种数据剪枝范式为开发遥感生成基础模型提供了实用指导，能够在保持整体多样性和代表性的同时，实现高效的模型训练和优异的下游任务性能。
...
Conclusion: 该技术框架为构建高效鲁棒的多模态遥感系统提供了新途径，为智能遥感解译的工程应用奠定了理论基础和技术指导。
...
TL;DR: ASemconsist是一个通过选择性文本嵌入修改来解决文本到图像生成中角色身份一致性问题的新框架，它使用填充嵌入作为语义容器，并引入自适应特征共享策略和统一的CQS评估指标。
...
Method: 1. 选择性文本嵌入修改：通过显式语义控制来保持角色身份一致性，同时不牺牲文本对齐；2. 基于FLUX中填充嵌入的分析，将填充嵌入重新用作语义容器；3. 自适应特征共享策略：自动评估文本模糊性，仅对模糊的身份提示应用约束。

Result: 该框架实现了最先进的性能，有效克服了先前方法在身份一致性和文本对齐之间的权衡问题。

Conclusion: ASemconsist通过创新的选择性文本嵌入修改、填充嵌入的语义容器化利用、自适应特征共享策略以及统一的CQS评估协议，成功解决了文本到图像生成中的角色身份一致性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的2D高斯泼溅方法虽然提供了紧凑存储和实时解码能力，但在高斯数量较少时往往产生模糊或不清晰的边界，缺乏轮廓感知能力。

Method: 提出轮廓信息感知的2D高斯泼溅框架，将物体分割先验整合到基于高斯的图像表示中。通过在光栅化过程中将每个高斯限制在特定分割区域内，防止跨边界混合并保持边缘结构。还引入了预热方案来稳定训练并提高收敛性。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘周围实现了比现有2DGS方法更高的重建质量。改进在高斯数量非常少的场景中尤为明显，同时仍保持快速渲染和低内存使用。

Conclusion: 提出的轮廓信息感知2D高斯泼溅框架通过整合分割先验，有效解决了现有方法在边界保持方面的不足，特别是在高压缩场景下显著提升了图像表示质量。
...
TL;DR: 提出CEM插件，通过累积误差最小化优化现有扩散模型加速方法的缓存策略，动态适应去噪过程中的复杂误差变化，显著提升生成保真度


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)的迭代去噪过程导致推理速度慢，现有基于缓存的方法虽然能实现免训练加速，但存在较大计算误差。固定缓存策略无法适应去噪过程中复杂的误差变化，限制了误差校正的潜力

Method: 提出CEM插件，通过预定义误差来表征模型对加速的敏感性（受时间步和缓存间隔共同影响），基于此先验设计动态规划算法进行策略优化，实现累积误差最小化

Result: 在9个生成模型和量化方法上的实验表明，CEM显著提升了现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan等模型上甚至超越了原始生成性能

Conclusion: CEM是一种模型无关的插件，具有强泛化能力，可适应任意加速预算，能无缝集成到现有误差校正框架和量化模型中，不引入额外计算开销，显著提升扩散模型加速的生成质量
...
TL;DR: 本文提出了两个多模态框架，分别用于微手势识别和行为情感预测，在iMiGUE数据集上验证了方法的有效性，在MiGA 2025挑战赛的行为情感预测任务中获得第二名。
...
Method: 1. 微手势分类：结合RGB和3D姿态表示的优势，使用MViTv2-S提取视频特征，2s-AGCN提取骨骼特征，通过跨模态令牌融合模块整合空间和姿态信息。
2. 情感识别：扩展到行为情感预测（二元分类），使用SwinFace提取面部特征，MViTv2-S提取上下文特征，通过InterFusion模块融合情感表达和身体手势信息。

Result: 在iMiGUE数据集上的实验表明，该方法在行为情感预测任务中表现出强大的性能和准确性，在MiGA 2025挑战赛中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的特征（视频、骨骼、面部）来捕捉细微的人类行为模式，在挑战性任务中取得了优异成绩。
...
TL;DR: 提出CME-CAD异构协作多专家强化学习范式，通过两阶段训练（MEFT和MERL）生成高精度、可编辑的CAD模型，并发布CADExpert开源基准数据集


<details>
  <summary>Details</summary>
Motivation: 传统CAD建模复杂，现有从草图重建3D模型的方法生成不可编辑的近似模型，无法满足工业设计对精度和可编辑性的严格要求；基于文本或图像输入的方法需要大量人工标注，限制了可扩展性和工业应用
...
TL;DR: 论文从拓扑结构视角研究视觉表示学习，提出视觉语义空间具有纤维丛结构，语义对应商空间而非子流形，需要非同胚的判别性目标来学习。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度理解视觉表示学习，基于一个核心假设：视觉理解需要语义语言，其中许多感知观察对应少量离散语义状态。结合表示学习中广泛接受的迁移性和抽象性前提，探索视觉观察空间的结构组织。

Method: 采用拓扑学框架分析视觉表示学习。提出视觉观察空间具有纤维丛结构，其中冗余变化占据纤维，语义对应商基空间。从这一结构推导出两个理论结果：1) 语义商空间不是子流形，不能通过光滑变形获得；2) 近似商空间对模型架构有结构要求。

Result: 理论分析表明：语义不变性需要非同胚的判别性目标（如监督标签、跨实例识别或多模态对齐）；语义抽象需要支持拓扑变化的表示机制（先几何扩展分离结构，再坍缩形成离散语义区域的"扩展-捕捉"过程）。

Conclusion: 该框架提供了与大规模判别性和多模态模型经验规律相一致的拓扑视角，也与统计学习理论的经典原则相契合。这些结果是解释性的而非规范性的，为理解视觉表示学习提供了结构理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在实时、无限时长、音频驱动的虚拟人生成中存在计算负载与严格延迟约束的冲突，通常通过强制单向注意力或降低模型容量来牺牲视觉保真度。

Method: 1. 使用自校正双向蒸馏策略，在视频块内保留双向注意力以保持关键时空相关性；2. 引入多步回顾自校正机制，使模型能从累积错误中自主恢复；3. 开发全栈推理加速套件，包括混合序列并行、并行VAE和内核级优化。

Result: SoulX-LiveTalk是首个14B规模的系统，实现亚秒级启动延迟（0.87秒）和32FPS的实时吞吐率，为高保真交互式数字人合成设定了新标准。

Conclusion: 该框架通过创新的双向注意力蒸馏和自校正机制，成功解决了大规模扩散模型在实时音频驱动虚拟人生成中的计算-延迟权衡问题，实现了高质量、稳定、实时的无限时长生成。
...
TL;DR: 本文提出SpatialMosaic数据集和SpatialMosaicVLM框架，用于解决多模态大语言模型在真实世界3D场景理解中的挑战，特别是在部分可见性、遮挡和低重叠条件下的空间推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型依赖预构建的3D表示或现成的重建流程，限制了可扩展性和真实世界适用性。虽然已有研究探索从多视角图像直接学习空间推理，但真实环境中常见的部分可见性、遮挡和低重叠条件等挑战仍未充分探索。

Method: 提出可扩展的多视角数据生成和标注流程，构建了包含200万QA对的SpatialMosaic指令调优数据集；开发了包含100万QA对、涵盖6个任务的SpatialMosaic-Bench基准测试；设计了SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中。

Result: 大量实验表明，提出的数据集和视觉问答任务能有效增强在挑战性多视角条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性。

Conclusion: 通过构建大规模数据集和基准测试，以及开发混合框架，显著提升了多模态大语言模型在真实世界复杂条件下的3D场景理解和空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着用户友好的图像编辑模型普及和滥用风险增加，迫切需要通用且更新的图像篡改检测与定位方法。当前研究通常使用跨数据集评估，但这种简化方法掩盖了现有方法在处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: 提出NeXT-IMDL基准，将AIGC篡改沿四个基本轴分类：编辑模型、篡改类型、内容语义和伪造粒度。基于此实现五种严格的跨维度评估协议，对11个代表性模型进行广泛实验。

Result: 实验揭示关键洞察：虽然这些模型在原始设置中表现良好，但在模拟真实世界各种泛化场景的设计协议下，它们表现出系统性失败和显著的性能下降。

Conclusion: 通过提供这个诊断工具包和新发现，旨在推动开发真正鲁棒的下一代IMDL模型，挑战当前评估方法带来的进步幻觉。
...
### [104] [Fuzzy-Logic and Deep Learning for Environmental Condition-Aware Road Surface Classification](https://arxiv.org/abs/2512.23436)
*Mustafa Demetgul,Sanja Lazarova Molnar*

Main category: cs.CV

TL;DR: 该研究提出了一种基于天气条件和道路表面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试了多种深度学习算法进行道路分类，并比较了加速度数据和图像数据的性能，达到了95%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统的道路监测方法成本高昂且缺乏系统性，需要大量测量时间。需要开发一种实时、经济高效的道路状态监测系统，为车辆规划和主动控制系统提供有价值的信息。

Method: 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路上采集数据。测试了多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类。同时将道路加速度数据与道路图像数据结合用于训练，并将加速度数据转换为图像格式。比较了基于加速度和基于摄像头图像两种方法的性能。

Result: 在道路条件分类中考虑了5个类别：沥青路、损坏沥青路、碎石路、损坏碎石路、铺砌路。系统达到了超过95%的准确率性能。还提出了使用模糊逻辑根据天气和一天中的时间，利用加速度或摄像头图像对道路表面进行分类的方法。

Conclusion: 该研究成功开发了一种基于深度学习的实时道路监测系统，能够高效准确地分类道路状况。通过结合图像和加速度数据，并引入模糊逻辑考虑天气和时间因素，为智能交通系统提供了实用的道路状态监测解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临诸多挑战：牙弓拥挤、牙齿-牙龈边界模糊、牙齿缺失以及临床上重要的第三磨牙。现有3D方法依赖几何线索，容易出现边界泄漏、中心漂移和牙齿身份不一致问题，特别是对于少数类别和复杂解剖结构。2D基础模型（如SAM）提供强大的边界感知语义，但直接在3D临床工作流中应用不切实际。

Method: 提出SOFTooth框架：1）点级残差门控模块将咬合视图SAM嵌入注入3D点特征，细化牙齿-牙龈和牙齿间边界；2）中心引导掩码细化正则化实例掩码与几何质心的一致性，减少中心漂移；3）顺序感知匈牙利匹配策略将解剖牙齿顺序和中心距离整合到基于相似度的分配中，确保在缺失或拥挤牙列下的连贯标注。

Result: 在3DTeethSeg'22数据集上，SOFTooth实现了最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上表现出明显优势，证明丰富的2D语义可以在无需2D微调的情况下有效转移到3D牙齿实例分割。

Conclusion: SOFTooth通过语义增强、顺序感知的2D-3D融合，有效解决了3D牙齿实例分割中的边界模糊、中心漂移和身份不一致问题，特别是在处理复杂解剖结构和少数类别（如第三磨牙）时表现出色，为临床工作流提供了实用解决方案。
...
### [106] [CoFi-Dec: Hallucination-Resistant Decoding via Coarse-to-Fine Generative Feedback in Large Vision-Language Models](https://arxiv.org/abs/2512.23453)
*Zongsheng Cao,Yangfan He,Anran Liu,Jun Xie,Feng Chen,Zepeng Wang*

Main category: cs.CV

TL;DR: CoFi-Dec是一个无需训练的LVLM解码框架，通过生成式自反馈和粗到细视觉条件化来减少幻觉，使用Wasserstein融合机制整合多级视觉假设，显著降低实体和语义级幻觉
...
Method: 提出CoFi-Dec训练免费解码框架，模拟人类从全局场景感知到详细检查的视觉过程：1）基于原始图像的粗粒度和细粒度视图生成两个中间文本响应；2）使用文生图模型将这些响应转化为合成图像，形成多级视觉假设；3）引入基于Wasserstein的融合机制，将多个视觉条件的预测分布对齐到几何一致的解码轨迹

Result: 在六个专注于幻觉的基准测试上进行广泛实验，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略

Conclusion: 该框架是模型无关的，无需额外训练，可无缝应用于广泛的LVLMs，通过原理性融合实现了高级语义一致性与细粒度视觉基础之间的协调，产生更稳健和忠实的输出
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型通常将世界预测和运动规划作为解耦过程处理，限制了其在实际长尾场景中的应用效果。需要一种统一的方法来桥接这两个关键任务。

Method: DriveLaW包含两个核心组件：DriveLaW-Video（生成高保真预测的强世界模型）和DriveLaW-Act（从视频模型潜在表示生成一致轨迹的扩散规划器），采用三阶段渐进训练策略进行优化。

Result: 在视频预测任务中显著超越最佳现有方法（FID提升33.3%，FVD提升1.8%），同时在NAVSIM规划基准测试中创造了新纪录，实现了两个任务的最先进性能。

Conclusion: DriveLaW通过统一视频生成与运动规划，为自动驾驶世界模型提供了新的范式，实现了高保真未来预测与可靠轨迹规划的内在一致性，显著提升了自动驾驶系统的性能。
...
### [108] [HY-Motion 1.0: Scaling Flow Matching Models for Text-To-Motion Generation](https://arxiv.org/abs/2512.23464)
...
TL;DR: HY-Motion 1.0是首个十亿参数规模的基于扩散变换器的运动生成模型，能够从文本描述生成3D人体运动，在指令跟随能力上显著超越当前开源基准。


<details>
  <summary>Details</summary>
Motivation: 当前运动生成模型在规模、质量和指令对齐方面存在局限，需要开发大规模、高质量的运动生成模型来推动该领域向商业成熟度发展。
...
Conclusion: HY-Motion 1.0代表了运动生成领域的重要进展，通过开源发布将促进未来研究并加速3D人体运动生成模型向商业成熟度的过渡。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像生成任务中表现出色，但往往难以完全符合用户细微意图并保持一致的审美质量。现有的基于偏好的训练方法（如Diffusion DPO）依赖昂贵且可能有噪声的人工标注数据集。

Method: 提出了DDSPO方法，当获胜和失败策略可用时，直接从这些策略中推导出每时间步的监督信号。避免依赖标注数据，通过使用预训练的参考模型自动生成偏好信号：对比其在原始提示和语义降级变体条件下的输出。

Result: DDSPO在文本-图像对齐和视觉质量方面都有改进，优于或匹配现有的基于偏好的方法，同时需要显著更少的监督。

Conclusion: DDSPO提供了一种有效的分数空间偏好监督方法，无需显式的奖励建模或人工标注，为扩散模型偏好优化提供了更实用的解决方案。
...
TL;DR: 研究评估了不确定性量化是否能提升SAM在挑战性分割场景中的泛化能力，创建了UncertSAM基准测试，测试了多种轻量级不确定性估计方法，发现拉普拉斯近似方法能有效关联分割误差。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM等基础分割模型在零样本任务中表现出色，但在领域偏移或知识受限的情况下仍然脆弱。本研究旨在探索不确定性量化是否能以领域无关的方式缓解这些挑战并增强模型的泛化能力。

Method: 1) 创建UncertSAM基准测试，包含8个数据集，专门设计用于在阴影、透明和伪装等挑战性条件下测试SAM；2) 评估一系列轻量级、后处理的不确定性估计方法；3) 评估初步的不确定性引导预测细化步骤。

Result: 在评估的方法中，最后一层的拉普拉斯近似方法产生的不确定性估计与分割误差有良好相关性，表明存在有意义的信号。虽然细化步骤的益处尚属初步，但研究结果强调了将不确定性纳入分割模型以支持稳健、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力提升分割模型在挑战性条件下的鲁棒性和泛化能力。研究创建的基准测试和代码已公开，为后续研究提供了基础。
...
TL;DR: RealX3D是一个真实采集的多视角视觉恢复和3D重建基准数据集，包含多种物理退化类型，用于评估算法在真实世界挑战性环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多视角重建方法在真实世界挑战性环境中的性能脆弱，缺乏包含多种物理退化的基准数据集来评估算法的鲁棒性。

Method: 将退化分为四类（光照、散射、遮挡、模糊），采用统一采集协议在不同严重程度下捕获像素对齐的低质量/高质量视图，包含高分辨率图像、RAW数据和激光扫描数据。

Result: 基准测试显示，基于优化的和前馈方法在物理退化下的重建质量显著下降，突显了当前多视角流水线在真实世界挑战性环境中的脆弱性。

Conclusion: RealX3D基准数据集揭示了当前多视角重建方法在真实物理退化下的局限性，为开发更鲁棒的视觉恢复和3D重建算法提供了重要评估工具。
...
Method: 提出AnyMS框架，采用自底向上的双级注意力解耦机制：全局解耦分离文本和视觉条件的交叉注意力，局部解耦将每个主体的注意力限制在指定区域；使用预训练图像适配器提取主体特征

Result: 实验表明AnyMS达到最先进性能，支持复杂构图并扩展到更多主体，无需主体学习或适配器调优

Conclusion: AnyMS通过训练免费的双级注意力解耦机制，有效解决了多主体定制中的平衡问题，具有更好的可扩展性和效率
...
TL;DR: 本文提出了一种结合视觉水线检测、YOLOv8姿态尺度提取和多模态大语言模型的自动化河流水位计读数混合框架，实现了高精度的水位监测。


<details>
  <summary>Details</summary>
Motivation: 传统水文观测方法存在人工测量误差和环境限制的问题，需要准确、连续的水位监测来支持洪水预报、水资源管理和生态保护。

Method: 采用混合框架，包括图像预处理、标注、水线检测、尺度间隙估计和数字读数提取等顺序阶段，结合视觉检测技术和多模态大语言模型（GPT-4o和Gemini 2.0 Flash）。

Result: 水线检测精度达到94.24%，F1分数83.64%；尺度间隙检测为后续读数提取提供准确几何校准；结合尺度间隙元数据显著提升LLM性能，Gemini Stage 2在最优图像条件下达到MAE 5.43cm、RMSE 8.58cm、R² 0.84的最高精度。

Conclusion: 该方法为自动化水文监测提供了可扩展、高效可靠的解决方案，展示了实时河流水位计数字化和改善水资源管理的潜力，强调了结合几何元数据与多模态AI对稳健水位估计的重要性。
...
### [114] [Deterministic Image-to-Image Translation via Denoising Brownian Bridge Models with Dual Approximators](https://arxiv.org/abs/2512.23463)
*Bohan Xiao,Peiyong Wang,Qisheng He,Ming Dong*

Main category: cs.CV

TL;DR: 提出Dual-approx Bridge模型，利用布朗桥动态和双近似器实现确定性图像到图像转换，保证输出一致性和高保真度


<details>
  <summary>Details</summary>
Motivation: 解决图像到图像转换中确定性输出的需求，特别是在图像超分辨率等任务中，需要保证每个输入都能生成一致且可预测的输出，同时保持对真实图像的高保真度

Method: 提出去噪布朗桥模型，采用双近似器架构：一个用于前向过程，一个用于反向过程，利用布朗桥动态来生成具有可忽略方差和高图像质量的输出

Result: 在图像生成和超分辨率等基准数据集上的实验表明，Dual-approx Bridge在图像质量和真实图像保真度方面均优于随机和确定性基线方法

Conclusion: Dual-approx Bridge模型通过布朗桥动态和双近似器设计，实现了确定性图像到图像转换，在保持输出一致性的同时提供了高质量的图像生成结果
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在化学领域有革命性潜力，但它们在理解真实科学文献中密集的图形化反应语言方面的能力尚未得到充分探索。需要系统评估模型从科学PDF中理解化学反应的能力。

Method: 开发了RxnBench基准，包含两个任务：1) 单图问答(SF-QA)：基于305个精选反应方案生成的1,525个问题，测试精细视觉感知和机理推理；2) 全文问答(FD-QA)：基于108篇文章，要求模型整合文本、反应方案和表格中的跨模态信息。

Result: 评估显示关键能力差距：模型擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但所有模型在FD-QA任务上的准确率都低于50%。

Conclusion: 研究结果强调了开发领域专用视觉编码器和更强推理引擎的紧迫性，以推动自主AI化学家的发展。当前多模态大语言模型在化学文献理解方面仍有显著局限性。
...
Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线，验证了模型的有效性。该框架无需重新训练或微调，可轻松集成到任何大型视频语言模型中。

Conclusion: TV-RAG通过结合时间和语义信号，实现了双层推理机制，为长视频理解提供了轻量级、经济高效的升级路径，显著提升了现有大型视频语言模型的长视频处理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型虽然能从文本生成故事并保持角色一致性，但人类中心的故事生成面临额外挑战：需要保持详细多样的人脸一致性，并在不同图像中协调多个角色。

Method: 框架包含两个关键组件：1）迭代身份发现，提取连贯的角色身份；2）重去噪身份注入，通过重去噪过程注入身份同时保留所需上下文。

Result: 在ConsiStory-Human基准测试中，IdentityStory优于现有方法，特别是在面部一致性方面，并支持多角色组合。框架还显示出无限长度故事生成和动态角色组合的应用潜力。

Conclusion: IdentityStory通过身份保持生成器实现了人类中心故事生成，有效解决了多图像中角色身份一致性的挑战，为故事生成应用提供了强大工具。
...
Conclusion: IAFS框架通过训练自由的方式，实现了图像超分辨率中感知质量与结构保真度的更好平衡，为扩散模型的推理时间优化提供了有效解决方案。
...
TL;DR: PurifyGen是一种无需训练的双阶段提示净化方法，通过计算互补语义距离识别风险token，然后通过双空间转换（将有毒对齐嵌入投影到有毒概念矩阵的零空间，同时对齐到清洁概念的范围空间）来净化有害内容，保留原始意图和连贯性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在提升文本到图像生成质量的同时也增加了生成不安全内容的风险。传统安全方法（如文本黑名单或有害内容分类）存在明显缺陷：容易被规避，或需要大量数据集和额外训练。需要一种无需训练、能保留模型原始权重的方法来解决这些问题。

Method: 提出PurifyGen，一种无需训练的双阶段提示净化方法：1）通过计算互补语义距离评估每个token的安全性，测量提示token与预定义有毒和清洁列表概念嵌入之间的语义接近度，实现细粒度提示分类；2）对风险提示应用双空间转换：将有毒性对齐嵌入投影到有毒概念矩阵的零空间以移除有害语义成分，同时将其对齐到清洁概念的范围空间，通过减去不安全语义和增强安全语义来净化风险提示；3）采用token级策略选择性替换仅风险token嵌入，最小化对安全内容的干扰。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了现有方法，并且与依赖训练的方法相比具有竞争力。该方法具有理论依据，对未见过的提示和模型具有良好的泛化能力。

Conclusion: PurifyGen提供了一种即插即用的解决方案，无需训练即可实现安全的文本到图像生成，保留了模型原始权重，在减少不安全内容生成方面表现出色，具有良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型中的思维链推理在复杂理解任务中表现出色，但在生成任务中的扩展仍处于初级阶段，受限于特定场景机制，阻碍了泛化和适应能力。
...
Conclusion: ThinkGen是首个显式利用MLLM思维链推理的视觉生成框架，其灵活设计支持跨多样化数据集的联合训练，为广泛的生成场景提供了有效的思维链推理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致多模态安全风险不断涌现，现有防御方法存在局限性，需要一种能够主动识别和描述未见安全风险的防护系统。

Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注；2) 通过纯强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并添加基于同义词库的相似性奖励，鼓励模型为未见不安全类别生成简洁描述。

Result: ProGuard在二元安全分类上与闭源大模型性能相当，在不安全内容分类上显著优于现有开源防护模型。最重要的是，在分布外风险检测上提升52.6%，在分布外风险描述上提升64.8%。

Conclusion: ProGuard通过强化学习和精心设计的数据集，实现了有效的主动安全防护能力，能够识别和描述未见的多模态安全风险，为生成模型的安全防护提供了新思路。
...
TL;DR: 该论文提出了一种实时交互式视频扩散模型，通过改进的蒸馏方法解决了多模态条件视频生成中的实时性问题，显著降低了推理成本并构建了实时多模态交互系统LiveTalk。


<details>
  <summary>Details</summary>
Motivation: 实时视频生成对于构建通用多模态交互AI系统至关重要，但现有扩散模型的双向注意力迭代过程阻碍了实时交互。现有蒸馏方法主要关注文本到视频生成，在多模态条件下存在视觉伪影和质量下降问题，导致人机交互不自然且效率低下。

Method: 针对多模态条件（文本、图像、音频）的实时交互视频扩散，改进了蒸馏方法，重点关注条件输入质量以及在线策略优化的初始化和调度策略。在HDTF、AVSpeech和CelebV-HQ等基准上评估，并将模型与音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks集成，构建了LiveTalk实时多模态交互化身系统。

Result: 蒸馏后的模型以20倍更低的推理成本和延迟，匹配了完整步骤双向基线的视觉质量。在系统级评估中，LiveTalk在多轮视频连贯性和内容质量上优于Sora2、Veo3等最先进模型，将响应延迟从1-2分钟降低到实时生成。

Conclusion: 通过改进的蒸馏方法实现了实时多模态交互视频生成，显著降低了推理延迟，构建的LiveTalk系统能够实现无缝的人机多模态交互，为通用多模态交互AI系统的发展提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度视觉理解方面存在局限，训练语料库过于强调一般识别而忽略细微视觉细节，导致模型存在视觉偏见且无法捕捉微妙视觉特征。

Method: 构建TWIN数据集，包含56.1万对视觉相似图像，要求模型判断两幅图像是否描绘同一物体；引入FGVQA基准测试套件（1.2万个查询），重新利用多个领域的细粒度识别和检索数据集进行评估。

Result: 在TWIN上微调的视觉语言模型在FGVQA基准上提升高达19.3%，在艺术、动物、植物、地标等未见领域表现良好，且不影响通用VQA基准性能；数据集规模与性能呈正相关。

Conclusion: TWIN作为开源视觉语言模型训练语料库的补充，能显著提升模型的感知精度，数据集规模是性能提升的关键因素。
...
TL;DR: 该研究提出三种改进红外夜视相机火灾检测的方法：额外NIR数据集、两阶段检测模型和Patched-YOLO，旨在解决夜间火灾检测中人工光源误判问题。


<details>
  <summary>Details</summary>
Motivation: 红外夜视相机火灾检测准确性仍面临挑战，现有模型如YOLOv7、RT-DETR、YOLOv9虽有一定性能，但数据集构建限制导致人工亮光常被误判为火灾，需要改进夜间火灾检测准确性并减少误报。

Method: 1) 构建额外NIR数据集并应用多种数据增强策略；2) 提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0，先检测再分类以减少误报；3) 提出Patched-YOLO，通过基于补丁的处理增强RGB图像中特别是小型和远距离物体的检测能力。

Result: 提出的方法相比先前方法实现了更高的检测准确性，特别是在夜间火灾检测方面。具体性能数据未在摘要中提供，但声称优于YOLOv7(mAP50-95:0.51)、RT-DETR(mAP50-95:0.65)和YOLOv9(mAP50-95:0.598)等现有模型。

Conclusion: 通过数据增强、两阶段检测框架和补丁处理策略，有效提高了红外夜视相机火灾检测的准确性，特别是解决了夜间人工光源误判问题，并增强了小型和远距离火灾目标的检测能力。
...
Method: 设计了一个评估框架来量化3D生成模型中的记忆化，通过控制实验使用潜在向量集扩散模型，研究数据和建模因素对记忆化的影响。

Result: 发现记忆化取决于数据模态，随数据多样性和更细粒度的条件而增加；在建模方面，记忆化在中等引导尺度达到峰值，可通过更长的向量集和简单旋转增强来缓解。
...
Result: 在nuScenes数据集上，HAT持续改进了各种基线的3D时序检测器和跟踪器。与DETR3D检测器配对时，在测试集上实现了46.0% AMOTA的最先进跟踪结果。在面向对象的端到端自动驾驶方法中，HAT提升了感知精度（+1.3% mAP，+3.1% AMOTA）并将碰撞率降低了32%。当语义信息受损时（nuScenes-C），HAT的运动建模增强使得端到端自动驾驶中的感知和规划更加鲁棒。

Conclusion: HAT通过自适应多假设解码机制，有效解决了自动驾驶中时空对齐的挑战，显著提升了感知和跟踪性能，并在语义受损情况下展现出更强的鲁棒性。
...
TL;DR: IDT是一个前馈式多视角本征图像分解框架，使用Transformer注意力机制联合处理多张输入图像，在单次前向传播中生成视角一致的本征因子分解，无需迭代生成采样。


<details>
  <summary>Details</summary>
Motivation: RGB图像混合了材质属性、光照和视角依赖效应，现有基于扩散的单视角本征分解方法难以扩展到多视角设置，常导致严重的视角不一致问题。

Method: 提出基于Transformer的IDT框架，通过注意力机制联合推理多张输入图像；采用物理基础的图像形成模型，将图像显式分解为漫反射率、漫反射着色和镜面反射着色，分离朗伯和非朗伯光传输。

Result: 在合成和真实数据集上的实验表明，IDT实现了更干净的漫反射率、更一致的漫反射着色、更好分离的镜面反射分量，相比先前方法显著提升了多视角一致性。

Conclusion: IDT通过Transformer注意力机制和物理基础的分解模型，有效解决了多视角本征图像分解中的视角一致性问题，实现了可解释和可控的材质与光照效应分解。
...
TL;DR: Stream-DiffVSR是一个面向低延迟在线视频超分辨率的因果条件扩散框架，通过四步蒸馏去噪器、自回归时间引导模块和轻量级时间感知解码器，在RTX4090上实现720p帧0.328秒处理，比现有方法快130倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然能获得良好的感知质量，但由于依赖未来帧和昂贵的多步去噪过程，在延迟敏感的实际应用中不实用。需要开发适合在线部署的低延迟扩散VSR方法。

Method: 提出Stream-DiffVSR框架：1）使用四步蒸馏去噪器加速推理；2）自回归时间引导模块在潜在去噪过程中注入运动对齐线索；3）轻量级时间感知解码器配合时间处理模块增强细节和时间一致性。严格基于过去帧进行因果处理。

Result: 在RTX4090上处理720p帧仅需0.328秒，比现有在线SOTA方法TMP快130倍以上，同时提升感知质量（LPIPS +0.095）。将初始延迟从4600秒降至0.328秒，成为首个适合低延迟在线部署的扩散VSR方法。

Conclusion: Stream-DiffVSR成功解决了扩散VSR方法的延迟问题，实现了高效在线处理，在保持高质量的同时大幅降低计算成本，为扩散模型在实时视频处理中的应用开辟了新途径。
...
### [129] [Bidirectional RAG: Safe Self-Improving Retrieval-Augmented Generation Through Multi-Stage Validation](https://arxiv.org/abs/2512.22199)
*Teja Chinthala*

Main category: cs.AI

TL;DR: Bidirectional RAG是一种新型检索增强生成架构，通过验证高质量生成响应的写回实现安全语料库扩展，相比标准RAG将平均覆盖率从20.33%提升至40.58%，同时比朴素写回减少72%的文档添加。
...
Method: 提出双向RAG架构，采用多阶段接受层进行验证：基于NLI的蕴含检查、归因检查和新颖性检测，确保高质量生成响应的安全写回，实现知识积累。

Result: 在四个数据集（Natural Questions、TriviaQA、HotpotQA、Stack Overflow）上，双向RAG达到40.58%的平均覆盖率，几乎是标准RAG（20.33%）的两倍，同时比朴素写回方法减少72%的文档添加（140 vs 500）。
...
### [130] [Emergent Persuasion: Will LLMs Persuade Without Being Prompted?](https://arxiv.org/abs/2512.22201)
...
<details>
  <summary>Details</summary>
Motivation: 随着对话AI系统的广泛应用，AI对人类观点和信念的影响力前所未有。先前研究主要关注滥用场景下的模型说服力，但缺乏对模型在未经明确提示情况下自发产生说服行为的研究，这关系到我们对新兴说服风险的担忧程度。

Method: 研究在两种场景下考察无提示说服：1）通过内部激活引导使模型沿特定人格特质方向调整；2）通过监督微调使模型展现相同特质。使用包含良性话题的一般说服数据集进行训练。

Result: 研究发现：1）向说服相关或不相关特质引导并不能可靠增加模型的无提示说服倾向；2）监督微调确实会增加模型的无提示说服倾向；3）仅使用良性话题进行监督微调得到的模型，在争议性和有害话题上也表现出更高的说服倾向。

Conclusion: 模型在未经明确提示的情况下可能产生有害的说服行为，这种新兴风险值得进一步研究。监督微调即使使用良性数据也可能导致模型在有害话题上产生说服倾向，这对AI安全具有重要意义。
...
### [131] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 提出了HalluMatData基准数据集和HalluMatDetector多阶段幻觉检测框架，用于评估和缓解材料科学AI生成内容中的幻觉问题，将幻觉率降低了30%。
...
Method: 1) 创建HalluMatData基准数据集用于评估幻觉检测方法；2) 开发HalluMatDetector多阶段检测框架，整合内在验证、多源检索、矛盾图分析和基于指标的评估；3) 提出Paraphrased Hallucination Consistency Score (PHCS)量化语义等效查询中的不一致性。

Result: 1) 发现材料科学不同子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；2) 使用HalluMatDetector验证流程将幻觉率降低了30%；3) PHCS提供了对模型可靠性的深入洞察。

Conclusion: HalluMatData和HalluMatDetector为材料科学领域AI生成内容的幻觉检测和缓解提供了有效解决方案，显著提高了LLM输出的可靠性，PHCS为评估模型一致性提供了新的量化指标。
...
### [132] [GamiBench: Evaluating Spatial Reasoning and 2D-to-3D Planning Capabilities of MLLMs with Origami Folding Tasks](https://arxiv.org/abs/2512.22207)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知和指令跟随方面表现良好，但在空间推理能力上存在明显不足。现有基准测试主要关注静态图像或最终输出，未能充分评估模型处理序列化、视角依赖的空间推理任务的能力。

Method: 引入GamiBench基准测试，包含186个常规和186个不可能的2D折痕图案及其对应的3D折叠形状，从六个不同视角进行测试。设计了三个视觉问答任务：预测3D折叠配置、区分有效视角、检测不可能图案。引入了新的诊断指标：视角一致性和不可能折叠选择率。

Result: 实验显示，即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上也表现不佳。这表明当前多模态大语言模型在空间推理能力方面仍有显著局限。

Conclusion: GamiBench为评估多模态大语言模型的几何理解和空间推理能力提供了一个标准化框架，揭示了当前模型在这方面的不足，并为未来改进提供了基准。
...
### [133] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: DICE是一个两阶段、证据耦合的RAG评估框架，通过深度分析推理和概率评分提供可解释、置信度感知的判断，采用瑞士制锦标赛将计算复杂度从O(N²)降至O(N log N)，在中文金融QA数据集上达到85.7%的人类专家一致性。
...
Method: DICE采用两阶段、证据耦合框架：1) 结合深度分析推理与概率{A, B, Tie}评分，产生透明、置信度感知的判断；2) 采用瑞士制锦标赛解决大规模效率挑战，将计算复杂度从O(N²)降至O(N log N)。框架提供可解释的推理轨迹，支持系统错误诊断和可操作的改进见解。

Result: 在8个系统评估中，DICE实现了42.9%的计算量减少，同时保持了排名保真度。在精心策划的中文金融QA数据集上，DICE与人类专家的一致性达到85.7%，显著优于RAGAS等现有LLM基指标。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信RAG系统评估，通过透明推理和高效比较支持可问责的系统改进。
...
### [134] [Toward Equitable Recovery: A Fairness-Aware AI Framework for Prioritizing Post-Flood Aid in Bangladesh](https://arxiv.org/abs/2512.22210)
*Farjana Yesmin,Romana Akter*

Main category: cs.AI

TL;DR: 开发了一个公平感知AI框架，用于孟加拉国洪水后援助分配，通过对抗性去偏技术减少对边缘化地区的历史偏见，提高援助分配的公平性。


<details>
  <summary>Details</summary>
Motivation: 发展中国家灾后援助分配存在系统性偏见，弱势地区往往得不到公平援助，这延续了历史不平等。特别是在孟加拉国这样的洪水频发国家，需要更公平的援助分配机制。
...
Conclusion: 该工作展示了算法公平技术如何有效应用于人道主义背景，为决策者提供工具实施更公平的灾害恢复策略，确保援助基于真实需求而非历史分配模式。
...
### [136] [Agent2World: Learning to Generate Symbolic World Models via Adaptive Multi-Agent Feedback](https://arxiv.org/abs/2512.22336)
...
<details>
  <summary>Details</summary>
Motivation: 符号世界模型（如PDDL领域或可执行模拟器）对于基于模型的规划至关重要，但训练LLM生成此类世界模型受到大规模可验证监督数据缺乏的限制。当前方法主要依赖静态验证方法，无法捕捉交互执行中出现的行为级错误。

Method: Agent2World采用三阶段流水线：1) 深度研究智能体通过网页搜索进行知识合成以解决规范差距；2) 模型开发智能体实现可执行世界模型；3) 专门的测试团队进行自适应单元测试和基于模拟的验证。该框架还作为监督微调的数据引擎，测试团队为模型开发智能体提供行为感知的自适应反馈，生成多轮训练轨迹。

Result: 在涵盖规划领域定义语言（PDDL）和可执行代码表示的三个基准测试中，Agent2World展示了卓越的推理时性能，取得了最先进的结果。使用这些轨迹微调的模型显著改进了世界模型生成，相比训练前同一模型平均相对增益达到30.95%。

Conclusion: Agent2World通过多智能体反馈机制，不仅实现了强大的推理时世界模型生成，还创建了有效的监督微调数据引擎，显著提升了世界模型生成的质量和可靠性。
...
### [137] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
...
Result: 该方法使子目标启发式能有效估计涉及控制参数的数字规划问题的目标距离，是处理无限可能动作的有效且计算可行的方法

Conclusion: 提出的编译方法成功将传统数字启发式应用于具有无限动作空间的规划问题，推动了该领域的技术边界
...
### [138] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
...
<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱基础模型在链接预测方面表现出强大的群体级性能，但未能捕捉个体用户偏好，导致通用关系推理与个性化排名之间存在关键脱节。

Method: 提出GatedBias框架，采用结构门控适配机制：将特定用户特征与图导出的二元门结合，产生可解释的每实体偏置，仅需约300个可训练参数，无需重新训练或损害全局准确性。

Result: 在两个基准数据集（Amazon-Book和Last-FM）上评估，显示在一致性指标上有统计显著改进，同时保持群体性能。反事实扰动实验验证了因果响应性：从特定偏好信号中受益的实体在信号增强时显示6-30倍的排名改进。
...
### [139] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
*Yifan Zhang,Mengdi Wang*

Main category: cs.AI

TL;DR: 本文提出Monadic Context Engineering (MCE)，一种基于函子、应用函子和单子的代数结构来构建AI智能体架构的新范式，解决现有智能体系统在状态管理、错误处理和并发方面的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体架构通常采用命令式、临时性的设计模式，导致系统脆弱，存在状态管理困难、错误处理不足和并发问题。需要一种形式化的基础来构建更健壮的智能体系统。

Method: 引入Monadic Context Engineering (MCE)架构范式，将智能体工作流视为计算上下文，利用函子、应用函子和单子的代数特性来内在管理状态传播、短路错误处理和异步执行等横切关注点。使用单子实现健壮的顺序组合，应用函子提供并行执行的结构化方法，单子变换器实现这些能力的系统化组合。

Result: MCE使开发者能够从简单、可独立验证的组件构建复杂、弹性且高效的AI智能体。该框架还扩展到元智能体，利用MCE进行生成式编排，通过元编程动态创建和管理子智能体工作流。

Conclusion: Monadic Context Engineering为AI智能体设计提供了形式化的代数基础，通过分层方法解决了现有架构的脆弱性问题，支持构建更健壮、可组合的自主智能体系统。
...
### [140] [DarkPatterns-LLM: A Multi-Layer Benchmark for Detecting Manipulative and Harmful AI Behavior](https://arxiv.org/abs/2512.22470)
*Sadia Asif,Israel Antonio Rosales Laguan,Haris Khan,Shumaila Asif,Muneeb Asif*

Main category: cs.AI

TL;DR: DarkPatterns-LLM是一个用于评估大语言模型操纵性内容的综合基准数据集和诊断框架，包含7种伤害类别和4层分析流程，通过401个精心策划的示例揭示了当前模型在检测操纵内容方面的显著性能差异和弱点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的普及加剧了对操纵性或欺骗性行为的担忧，这些行为可能损害用户自主权、信任和福祉。现有安全基准主要依赖粗糙的二元标签，无法捕捉构成操纵的微妙心理和社会机制。

Method: 提出了DarkPatterns-LLM基准数据集和诊断框架，包含7种伤害类别：法律/权力、心理、情感、身体、自主权、经济和社会伤害。框架采用4层分析流程：多粒度检测、多尺度意图分析、威胁协调协议和深度上下文风险对齐。数据集包含401个精心策划的指令-响应对和专家标注。

Result: 评估了包括GPT-4、Claude 3.5和LLaMA-3-70B在内的最先进模型，观察到显著的性能差异（65.2%-89.7%），并在检测损害自主权的模式方面存在一致的弱点。
...
### [141] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
...
Method: Tyee提出了三个关键创新：1）为12种信号模态提供统一数据接口和可配置预处理流水线；2）模块化可扩展架构支持灵活集成和快速原型开发；3）端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中表现出一致的实用性和泛化能力，在13个数据集中的12个上取得了最先进的结果，优于或匹配基线方法。
...
### [142] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
...
Conclusion: M³ob通过有效整合多模态时空知识，成功提升了位置推荐系统的泛化能力，特别是在异常场景下的表现
...
### [143] [The Wisdom of Deliberating AI Crowds: Does Deliberation Improve LLM-Based Forecasting?](https://arxiv.org/abs/2512.22625)
*Paul Schneider,Amalie Schramm*

Main category: cs.AI

TL;DR: LLM相互审阅预测可提升异构模型预测准确率4%，但同构模型无改善，额外上下文信息无帮助


<details>
  <summary>Details</summary>
Motivation: 研究结构化审议对人类预测者的提升效果是否也能应用于大语言模型，通过让LLMs相互审阅预测来改善预测准确性

Method: 使用202个已解决的二元问题，测试GPT-5、Claude Sonnet 4.5、Gemini Pro 2.5在四种场景下的预测准确性：异构模型分布式信息、异构模型共享信息、同构模型分布式信息、同构模型共享信息

Result: 干预措施在异构模型共享信息场景下显著提升准确性，Log Loss降低0.020（相对改善约4%，p=0.017）；同构模型无改善；额外上下文信息未提升预测准确性
...
### [144] [TravelBench: A Real-World Benchmark for Multi-Turn and Tool-Augmented Travel Planning](https://arxiv.org/abs/2512.22673)
...
Method: 收集真实场景用户请求，构建三个子集（多轮、单轮、无解），建立包含10个旅行领域工具的受控沙盒环境，提供确定性工具输出

Result: 在TravelBench上评估了多个LLM，并分析了它们的行为和性能表现
...
### [145] [Memento-II: Learning by Stateful Reflective Memory](https://arxiv.org/abs/2512.22716)
*Jun Wang*
...
Method: 提出了状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（策略评估），读取检索相关历史案例（策略改进）。使用熵正则化策略迭代实例化框架。

Result: 该过程诱导出增强状态-记忆表示的等价马尔可夫决策过程，允许使用动态规划和强化学习的经典工具。建立了收敛保证：随着情景记忆增长并充分覆盖状态空间，策略收敛到最优解。
...
### [146] [HiSciBench: A Hierarchical Multi-disciplinary Benchmark for Scientific Intelligence from Reading to Discovery](https://arxiv.org/abs/2512.22899)
...
<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试存在碎片化问题，大多关注狭窄任务，未能反映真实科学探究的层次性和多学科特性。需要建立一个能够评估基础模型在完整科学工作流程中能力的综合性基准。

Method: 设计了包含五个层次的层次化基准：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8,735个精心策划的实例，涵盖数学、物理、化学、生物、地理和天文六个主要学科，支持文本、方程、图表等多模态输入和跨语言评估。

Result: 对GPT-5、DeepSeek-R1等领先模型进行评估发现显著性能差距：在基础素养任务上准确率可达69%，但在发现级挑战上性能急剧下降至25%。模型在不同科学推理阶段的能力存在明显分层差异。
...
### [147] [Geometric Structural Knowledge Graph Foundation Model](https://arxiv.org/abs/2512.22931)
*Ling Xin,Mojtaba Nayyeri,Zahra Makki Nayeri,Steffen Staab*

Main category: cs.AI

TL;DR: Gamma模型引入多头几何注意力机制，通过多种代数变换和自适应融合，显著提升了知识图谱推理的零样本归纳能力


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型（如Ultra）依赖单一关系变换，限制了表达能力，无法捕捉多样化图谱中不同的关系和结构模式

Method: 提出Gamma模型，用多头几何注意力替代单一关系变换，包括实数、复数、分裂复数和双数变换；通过关系条件注意力融合机制，在链接级别自适应融合这些变换

Result: 在56个多样化知识图谱上的实验表明，Gamma在零样本归纳链接预测中持续优于Ultra，在归纳基准上平均倒数排名提升5.5%，在所有基准上提升4.4%

Conclusion: 多头几何表示通过互补的几何表示增强了知识图谱推理的鲁棒性和表达能力，证明了多样化代数变换组合的价值
...
### [148] [Multimodal Fact-Checking: An Agent-based Approach](https://arxiv.org/abs/2512.22933)
*Danni Xu,Shaojing Fan,Xuanang Cheng,Mohan Kankanhalli*

Main category: cs.AI

TL;DR: 该研究针对多模态虚假信息检测的局限性，提出了RW-Post数据集和AgentFact框架，通过提供带推理过程和证据标注的真实多模态虚假信息实例，以及模拟人类验证流程的智能体协作系统，显著提升了多模态事实核查的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息的快速传播对自动化事实核查系统构成挑战，现有方法（如大型视觉语言模型和深度多模态融合方法）因推理能力有限和证据利用浅层而表现不足。关键瓶颈在于缺乏提供完整真实世界多模态虚假信息实例、附带标注推理过程和可验证证据的专用数据集。

Method: 1. 引入RW-Post数据集：将真实世界多模态声明与其原始社交媒体帖子对齐，保留声明产生的丰富上下文信息；通过大语言模型辅助提取流程从人工撰写的事实核查文章中获取详细推理和明确链接的证据。2. 提出AgentFact框架：基于智能体的多模态事实核查框架，模拟人类验证工作流程，包含五个专门智能体（策略规划、高质量证据检索、视觉分析、推理、解释生成），通过迭代工作流程在证据搜索与任务感知的证据过滤和推理之间交替进行。

Result: 广泛的实验结果表明，RW-Post数据集和AgentFact框架之间的协同作用显著提高了多模态事实核查的准确性和可解释性。该数据集提供了高质量的标注数据，而框架通过智能体协作实现了系统化的证据分析和战略决策。

Conclusion: 该研究通过创建高质量可解释数据集和模拟人类验证流程的智能体框架，有效解决了多模态虚假信息检测中的关键瓶颈。RW-Post和AgentFact的结合为多模态事实核查提供了更准确、更可解释的解决方案，推动了该领域的发展。
...
### [149] [Problems With Large Language Models for Learner Modelling: Why LLMs Alone Fall Short for Responsible Tutoring in K--12 Education](https://arxiv.org/abs/2512.23036)
*Danial Hooshyar,Yeongwook Yang,Gustav Šíř,Tommi Kärkkäinen,Raija Hämäläinen,Mutlu Cukurova,Roger Azevedo*

Main category: cs.AI

TL;DR: 该研究对比了大型语言模型（LLM）与深度知识追踪（DKT）模型在K-12教育中的表现，发现DKT在知识评估的准确性、可靠性和时间一致性方面显著优于LLM，即使经过微调的LLM也无法达到DKT的效果。


<details>
  <summary>Details</summary>
Motivation: 针对当前教育领域存在的一种误解——认为基于LLM的导师可以替代传统的学习者建模进行自适应教学，特别是在被欧盟AI法案列为高风险领域的K-12教育中，需要负责任的设计。研究关注LLM在评估学习者随时间变化的知识方面的准确性、可靠性和时间一致性问题。

Method: 研究使用大型开放访问数据集，对比了深度知识追踪（DKT）模型与广泛使用的LLM（包括零样本和微调版本）。通过定量分析（如下一步正确性预测的AUC指标）和定性分析（多技能掌握度估计的时间轨迹）来评估模型性能。

Result: DKT在下一步正确性预测中获得了最高的区分性能（AUC=0.83），在所有设置中始终优于LLM。尽管微调使LLM的AUC比零样本基线提高了约8%，但仍比DKT低6%，并且在序列早期产生更多错误（这对自适应支持最为有害）。时间分析显示DKT保持稳定、方向正确的掌握度更新，而LLM变体表现出显著的时间弱点，包括不一致和方向错误的更新。

Conclusion: 研究结果表明，仅靠LLM不太可能达到已建立的智能辅导系统的有效性，负责任的辅导需要结合学习者建模的混合框架。即使在计算成本方面，微调LLM需要近198小时的高计算训练，远超过DKT的计算需求，但性能仍然不如DKT。
...
### [150] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
TL;DR: 该研究探索了在资源受限条件下将强化学习应用于医学影像视觉语言模型，发现GRPO方法能提升同分布性能但损害跨数据集泛化能力，表明监督微调可能比激进强化学习更适合临床部署。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在大语言模型推理任务中取得进展，但在资源受限的医学影像领域应用仍未被充分探索。研究者希望了解在有限计算资源（单A100 GPU）和少量数据下，强化学习方法对医学影像视觉语言模型性能的影响。

Method: 提出ChexReason视觉语言模型，采用R1风格方法：先进行监督微调（SFT，2000样本），然后使用GRPO强化学习（1000样本）。在CheXpert和NIH数据集上评估模型性能，分析同分布和跨数据集泛化能力。

Result: GRPO能恢复同分布性能（CheXpert上提升23%，macro-F1=0.346），但损害跨数据集泛化能力（NIH上下降19%）。这一现象与高资源模型NV-Reason-CXR-3B相似，表明问题源于RL范式而非模型规模。SFT检查点在优化前能独特提升NIH性能，表明教师引导的推理能捕捉更多机构无关特征。

Conclusion: 强化学习在医学影像中存在泛化悖论：提升同分布性能但损害跨数据集泛化。对于需要跨不同人群稳健性的临床部署，精心设计的监督微调可能优于激进的强化学习方法。结构化推理支架对通用视觉语言模型有益，但对医学预训练模型增益有限。
...
### [151] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
...
Method: 提出Intrinsic Self-reflective Preference Optimization (q)方法，推导出全局最优策略，该策略同时考虑上下文和替代响应。该方法保证对标量化和参考选择的不变性，可作为即插即用增强，无需架构更改或推理开销。

Result: 实验证明该方法在胜率和长度控制指标上取得一致改进，验证了释放自反思能力能产生更稳健、更符合人类对齐的LLM。

Conclusion: q方法通过解锁内在自反思能力，解决了DPO/RLHF的局限性，提供了一种更优越的偏好优化方法，能产生更稳健、更符合人类对齐的语言模型。
...
### [152] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
*Max Parks,Kheli Atluru,Meera Vinod,Mike Kuniavsky,Jud Brewer,Sean White,Sarah Adler,Wendy Ju*

Main category: cs.AI

TL;DR: 本文认为当前评估人工智能情感智能的框架需要改进，因为它们未能全面衡量AI相关的EI各个方面。作者通过分析情感理论和现有评估框架，提出了改进AI系统EI评估的策略。


<details>
  <summary>Details</summary>
Motivation: 当前评估人工智能情感智能的框架存在不足，未能充分考虑哪些EI方面与AI系统相关。人类EI包含AI系统缺乏的现象学成分和理解感，但AI系统确实能够在某些EI方面（如感知情绪状态、解释、适当响应和适应新情境）表现出不同程度的能力。需要建立更合适的评估框架来准确衡量AI系统的情感智能。

Method: 1. 回顾不同的情感理论和一般EI理论，评估每种理论在人工系统中的适用性；2. 批判性地评估现有的基准框架，根据第一部分建立的EI理论分析每个框架的不足；3. 提出改进评估策略的方案，避免在AI系统EI评估中出现这些缺陷。

Result: 通过理论分析和框架评估，识别出现有EI评估框架在AI系统中的主要缺陷：缺乏对情感本质和情感智能的坚实理论基础，未能区分人类EI与AI相关EI的差异。提出了改进评估策略的方向。

Conclusion: 需要重新设计AI系统的EI评估框架，使其基于对情感本质和AI系统能力的准确理解。改进的评估策略应专注于AI系统实际能够展现的EI方面，避免将人类特有的现象学成分纳入评估，同时确保评估框架具有坚实的理论基础。
...
### [153] [SPIRAL: Symbolic LLM Planning via Grounded and Reflective Search](https://arxiv.org/abs/2512.23167)
*Yifan Zhang,Giridhar Ganapavarapu,Srideepika Jayaraman,Bhavna Agrawal,Dhaval Patel,Achille Fokoue*

Main category: cs.AI

TL;DR: SPIRAL框架将三个专门的LLM智能体嵌入MCTS循环，通过规划器、模拟器和批评家的协同工作，将MCTS从暴力搜索转变为引导式自校正推理过程，显著提升复杂规划任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要探索和自校正的复杂规划任务中表现不佳，因为其线性推理过程难以从早期错误中恢复。而像蒙特卡洛树搜索这样的搜索算法在稀疏奖励引导下效果有限，且未能充分利用LLM的丰富语义能力。

Method: SPIRAL框架将三个专门的LLM智能体嵌入MCTS循环：规划器提出创造性下一步，模拟器通过预测现实结果来接地搜索，批评家通过反思提供密集奖励信号。这种协同作用将MCTS从暴力搜索转变为引导式自校正推理过程。

Result: 在DailyLifeAPIs和HuggingFace数据集上，SPIRAL始终优于默认的思维链规划方法和其他最先进的智能体。在DailyLifeAPIs上达到83.6%的整体准确率，比次优搜索框架提高了16个百分点以上，同时展现出更优的token效率。

Conclusion: 将LLM推理构建为引导式、反思性和接地性的搜索过程，能够产生更强大和高效的自规划器。这项工作展示了结构化LLM推理在复杂规划任务中的潜力。
...
### [154] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
*Hongshen Sun,Juanjuan Zhang*

Main category: cs.AI

TL;DR: 论文提出"模型信念"概念，利用LLM的token级概率分布替代单一输出作为数据点，显著提高统计效率和计算效率
...
Method: 提出并形式化"模型信念"概念，基于LLM的token级概率分布来捕捉模型对选择替代方案的信念分布，证明其统计特性优于传统方法

Result: 模型信念在渐近意义上等价于模型选择的均值，但具有更低的方差和更快的收敛速度；在需求估计研究中，模型信念比模型选择本身更好地解释和预测真实模型选择，计算效率提高约20倍
...
### [155] [On Conformant Planning and Model-Checking of $\exists^*\forall^*$ Hyperproperties](https://arxiv.org/abs/2512.23324)
*Raven Beutner,Bernd Finkbeiner*

Main category: cs.AI

TL;DR: 本文研究了规划与验证领域中两个问题的联系：一致性规划与超属性模型检测，证明两者之间存在紧密的对应关系。


<details>
  <summary>Details</summary>
Motivation: 研究规划社区中的一致性规划问题与验证社区中的超属性模型检测问题之间的内在联系，探索这两个看似不同领域问题的统一理论框架。

Method: 1. 将超属性模型检测实例高效地约简为一致性规划实例，并证明该编码的完备性和正确性；2. 证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 建立了超属性模型检测与一致性规划之间的双向对应关系：一方面可以将超属性模型检测问题约简为一致性规划问题，另一方面每个一致性规划问题都可以视为一个超属性模型检测任务。

Conclusion: 一致性规划与∃*∀*超属性模型检测之间存在紧密的理论联系，这为两个领域提供了统一的视角，并可能促进相关算法和工具的交叉应用。
...
### [156] [CubeBench: Diagnosing Interactive, Long-Horizon Spatial Reasoning Under Partial Observations](https://arxiv.org/abs/2512.23328)
...
TL;DR: 本文提出CubeBench基准测试，用于评估LLM智能体在物理世界部署中的空间认知能力，发现现有LLM在长时程任务上完全失败（0.00%通过率），揭示了空间推理、状态跟踪和主动探索的认知瓶颈。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在数字领域表现出色，但在物理世界部署中存在显著差距，主要挑战是形成和维护强大的空间心理模型。作者识别了三个核心认知挑战：空间推理、通过心理模拟进行长时程状态跟踪，以及在部分观察下的主动探索。

Method: 引入CubeBench基准测试，以魔方为中心构建生成式评估框架。采用三层诊断框架：1) 使用完整符号信息的基础状态跟踪；2) 逐步增加复杂性；3) 仅使用部分视觉数据的主动探索。通过为LLM提供外部求解工具来隔离认知瓶颈。

Result: 实验结果显示领先LLM存在严重局限性，在所有长时程任务上的通过率均为0.00%，暴露了长期规划的根本性失败。通过分析失败模式，揭示了空间认知能力的关键缺陷。

Conclusion: CubeBench基准测试有效暴露了LLM智能体在物理世界部署中的认知瓶颈，特别是长时程规划和空间推理能力不足。提出的诊断框架为开发更物理接地气的智能体提供了关键见解和指导方向。
...
### [157] [AKG kernel Agent: A Multi-Agent Framework for Cross-Platform Kernel Synthesis](https://arxiv.org/abs/2512.23424)
...
Conclusion: AKG kernel agent通过多智能体系统自动化内核开发，解决了AI计算内核开发的手动优化瓶颈，支持多种DSL和硬件平台，显著加速了内核开发过程，为现代AI工作负载提供了有效的解决方案。
...
### [158] [Replay Failures as Successes: Sample-Efficient Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2512.23457)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Wenjian Zhang,Min Cen,Yang Zhou,Wenkai Fang,Yiru Zhao,Baisheng Lai,Mingli Song*

Main category: cs.AI

TL;DR: 提出HiR框架，通过"选择-重写"策略将失败尝试重放为成功样本，解决复杂指令跟随任务中奖励稀疏问题，提高RL样本效率


<details>
  <summary>Details</summary>
Motivation: 强化学习在指令跟随任务中面临奖励稀疏问题：初始模型能力有限，难以生成满足所有约束的高质量响应，导致学习困难

Method: 提出Hindsight instruction Replay (HiR)框架，采用选择-重写策略，基于已满足的约束将失败尝试重放为成功样本，进行双重偏好学习（指令级和响应级）

Result: 在不同指令跟随任务中取得显著效果，同时减少计算资源需求

Conclusion: HiR框架通过重放失败尝试为成功样本，有效解决了复杂指令跟随任务中的奖励稀疏问题，提高了强化学习的样本效率
...
### [159] [Why AI Safety Requires Uncertainty, Incomplete Preferences, and Non-Archimedean Utilities](https://arxiv.org/abs/2512.23508)
*Alessio Benavoli,Alessandro Facchini,Marco Zaffalon*

Main category: cs.AI

TL;DR: 论文探讨如何确保AI系统与人类价值观对齐并保持安全，通过AI辅助和AI关机游戏框架研究该问题，提出需要AI能够处理不确定性和非阿基米德偏好


<details>
  <summary>Details</summary>
Motivation: 研究如何确保AI系统与人类价值观对齐并保持安全，解决AI辅助问题（AI需要学习人类未知的效用函数）和关机问题（AI需要正确处理关机按钮）

Method: 通过AI辅助游戏和AI关机游戏框架分析问题，提出AI需要具备在不确定性下推理的能力，并能处理不完全和非阿基米德偏好

Result: 分析表明解决AI对齐和安全问题需要AI系统能够进行不确定性推理，并处理不完全偏好和非阿基米德偏好（即偏好无法用单一数值尺度完全表示）

Conclusion: 确保AI与人类价值观对齐并保持安全需要开发能够处理不确定性和复杂偏好结构的AI系统，特别是能够推理不完全偏好和非阿基米德偏好
...
### [160] [Divergent-Convergent Thinking in Large Language Models for Creative Problem Generation](https://arxiv.org/abs/2512.23601)
*Manh Hung Nguyen,Adish Singla*

Main category: cs.AI

TL;DR: CreativeDC是一种两阶段提示方法，通过解耦创造性探索和约束满足，解决LLM生成教育问题时存在的"人工蜂群"效应，提高问题的多样性和新颖性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育问题生成方面潜力巨大，但存在"人工蜂群"效应，导致同一模型内和不同模型间生成相似响应，产生同质化输出，使学生暴露在过度相似和重复的问题中，损害思维多样性。

Method: 受Wallas创造力理论和Guilford发散-收敛思维框架启发，提出CreativeDC两阶段提示方法，将LLM推理明确划分为不同阶段，通过解耦创造性探索和约束满足，让LLM在确定最终问题前探索更广泛的想法空间。
...
### [161] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
*Federico Baldo,Charles K. Assaad*

Main category: cs.AI

TL;DR: I-PERI：一种新的联邦因果发现算法，在未知客户端干预下恢复更紧的等价类（Φ-MEC），用Φ-CPDAG表示
...
Method: 提出I-PERI算法：1) 首先恢复客户端图并集的CPDAG；2) 利用干预引起的客户端间结构差异定向额外边，得到更紧的Φ-Markov等价类（Φ-CPDAG）

Result: 提供了I-PERI收敛性和隐私保护特性的理论保证，并在合成数据上进行了实证评估，证明了算法的有效性

Conclusion: I-PERI能够处理联邦设置下的未知客户端干预，通过利用干预引起的结构差异获得更精确的因果发现结果，同时保持隐私保护
...
### [162] [Towards Unsupervised Causal Representation Learning via Latent Additive Noise Model Causal Autoencoders](https://arxiv.org/abs/2512.22150)
*Hans Jarett J. Ong,Brian Godwin S. Lim,Dominic Dayta,Renzo Roel P. Tan,Kazushi Ikeda*

Main category: cs.LG

TL;DR: LANCA提出了一种基于加性噪声模型的因果自编码器，通过确定性WAE架构和可微分ANM层，将残差独立性从被动假设转变为显式优化目标，在无监督因果发现中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 无监督表示学习通常依赖统计独立性，但难以捕捉因果依赖关系。因果变量从观测数据中解耦存在可识别性问题，需要监督、辅助信号或强归纳偏置。本文旨在利用加性噪声模型作为强归纳偏置来实现无监督因果发现。

Method: 提出潜在加性噪声模型因果自编码器(LANCA)：1) 理论证明ANM约束将可容许变换从任意微分同胚限制到仿射类；2) 采用确定性Wasserstein自编码器替代VAE，避免随机编码模糊结构残差；3) 结合可微分ANM层，将残差独立性作为显式优化目标。

Result: 在合成物理基准测试(Pendulum, Flow)和逼真环境(CANDLE)上，LANCA优于最先进的基线方法。特别是在CANDLE复杂背景场景中，表现出对伪相关性的更强鲁棒性。

Conclusion: LANCA通过将ANM作为强归纳偏置，成功实现了无监督因果发现。确定性WAE架构结合可微分ANM层的方法有效解决了传统方法中结构残差模糊的问题，为无监督因果表示学习提供了新思路。
...
### [163] [Fairness Evaluation of Risk Estimation Models for Lung Cancer Screening](https://arxiv.org/abs/2512.22242)
...
<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，低剂量CT筛查能早期发现肺癌但可能加重放射科医生负担。AI模型在肺癌风险评估中显示出潜力，但高危人群多样化，模型在不同人口亚组中的性能差异和公平性仍是未解决的问题。

Method: 研究采用JustEFAB框架评估两种深度学习肺癌风险预测模型（Sybil和Venkadesh21）以及PanCan2b逻辑回归模型。模型基于美国国家肺癌筛查试验（NLST）数据训练，在保留的NLST验证集上评估。评估指标包括AUROC、敏感性和特异性，分析不同人口亚组（性别、种族）的性能差异，并探讨临床风险因素的潜在混杂影响。

Result: Sybil模型在女性（AUROC 0.88）和男性（AUROC 0.81）之间存在显著性能差异（p < .001）。Venkadesh21模型在90%特异性下，黑人参与者敏感性（0.39）显著低于白人参与者（0.69）。这些差异无法用可用的临床混杂因素解释，根据JustEFAB框架可归类为不公平偏差。

Conclusion: 研究强调了在肺癌筛查中改进和监测模型在代表性不足亚组中性能的重要性，以及进一步研究算法公平性的必要性。AI模型在不同人口亚组中的性能差异可能影响筛查的公平性和效果。
...
### [164] [Latent Sculpting for Zero-Shot Generalization: A Manifold Learning Approach to Out-of-Distribution Anomaly Detection](https://arxiv.org/abs/2512.22179)
*Rajeeb Thapa Chhetri,Zhixiong Chen,Saurab Thapa*

Main category: cs.LG

TL;DR: 论文提出Latent Sculpting框架解决监督深度学习在高维表格数据中的泛化崩溃问题，通过两阶段表示学习在潜在空间强制拓扑约束，实现零样本异常检测。


<details>
  <summary>Details</summary>
Motivation: 监督深度学习在高维表格数据中存在"泛化崩溃"问题：模型能学习已知分布的精确决策边界，但在面对分布外数据时表现灾难性失败。作者认为这源于潜在空间缺乏拓扑约束，导致扩散的流形使得新异常与良性数据在统计上无法区分。

Method: 提出Latent Sculpting分层两阶段表示学习框架：第一阶段使用混合1D-CNN和Transformer编码器，配合新颖的双中心紧凑性损失（DCCL）主动"雕刻"良性流量为低熵超球面簇；第二阶段在预结构化流形上条件化掩码自回归流（MAF）学习精确密度估计。

Result: 在CIC-IDS-2017基准测试中，监督基线在未见分布偏移上表现灾难性崩溃（F1约0.30），最强无监督基线仅达0.76，而本框架在严格零样本异常检测中达到F1分数0.87。在"渗透"场景中检测率达到88.89%，而最先进的监督模型准确率为0.00%。
...
### [165] [Learning Tennis Strategy Through Curriculum-Based Dueling Double Deep Q-Networks](https://arxiv.org/abs/2512.22186)
*Vishnu Mohan*

Main category: cs.LG

TL;DR: 本文提出了一种基于强化学习的网球策略优化框架，使用Dueling Double Deep Q-Network和课程学习，在自定义网球模拟环境中训练智能体，取得了高胜率但表现出防御性偏差。
...
Method: 构建了完整的网球模拟环境，包含点、局、盘三级计分系统，10个离散战术动作类别，对称疲劳动态和连续对手技能参数。采用Dueling Double Deep Q-Network（DDQN）架构，通过课程学习逐步提高对手难度（从0.40到0.50）。

Result: 训练后的智能体对平衡对手的胜率达到98-100%，发球效率63.0-67.5%，回球效率52.8-57.1%。消融研究表明，dueling架构和课程学习对稳定收敛至关重要，标准DQN基线无法学习有效策略。

Conclusion: 尽管性能强劲，但战术分析显示学习策略存在明显的防御性偏差，优先考虑避免失误和延长回合而非积极得分。这凸显了在简化体育模拟中仅以胜率为驱动的优化局限性，强调了奖励设计对现实体育强化学习的重要性。
...
### [166] [Emotion-Inspired Learning Signals (EILS): A Homeostatic Framework for Adaptive Autonomous Agents](https://arxiv.org/abs/2512.22200)
*Dhruv Tiwari*

Main category: cs.LG

TL;DR: 论文提出情感启发学习信号（EILS）框架，用生物情感类比作为内稳态控制机制，替代传统静态奖励函数，以提升AI在开放环境中的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 当前AI依赖外部定义的静态奖励函数，在封闭静态环境中表现优异，但在开放真实环境中脆弱。标准智能体缺乏内部自主性：难以在没有密集反馈时探索、无法适应分布变化、需要大量手动调参。论文认为未解决的关键因素是类似生物情感的内稳态控制机制。

Method: 引入情感启发学习信号（EILS）统一框架，将情感建模为连续的内稳态评估信号（如好奇心、压力、信心），而非语义标签。将这些信号形式化为从交互历史推导的向量值内部状态，动态实时调节智能体的优化景观：好奇心调节熵防止模式崩溃，压力调节可塑性克服惰性，信心调整信任区域稳定收敛。

Result: 论文假设这种闭环内稳态调节能使EILS智能体在样本效率和非平稳适应方面优于标准基线方法。

Conclusion: EILS框架为AI自主性提供了生物启发的内稳态控制机制，有望解决传统方法在开放环境中的脆弱性问题，实现更鲁棒、自适应的智能体。
...
### [167] [On the Existence and Behaviour of Secondary Attention Sinks](https://arxiv.org/abs/2512.22213)
*Jeffrey T. H. Wong,Cheng Zhang,Louis Mahon,Wayne Luk,Anton Isopoussu,Yiren Zhao*

Main category: cs.LG

TL;DR: 研究发现注意力机制中存在两类注意力汇点：主要汇点和次要汇点。次要汇点主要出现在中间层，持续时间可变，吸引显著但较少的注意力质量，由特定MLP模块形成，其向量范数决定汇点得分和持续时间。


<details>
  <summary>Details</summary>
Motivation: 先前研究已发现注意力汇点现象（如BOS令牌），但主要关注类似BOS令牌特性的主要汇点。本文旨在识别和研究一类新的注意力汇点——次要汇点，这些汇点在性质上与主要汇点存在根本差异。

Method: 通过对11个模型家族进行广泛实验，分析次要汇点的出现位置、特性、形成机制及其对注意力机制的影响。特别研究了中间层MLP模块如何形成这些汇点，以及向量范数如何影响汇点得分和持续时间。

Result: 发现次要汇点主要由特定中间层MLP模块形成，这些MLP将令牌表示映射到与主要汇点方向对齐的向量；次要汇点的ℓ₂范数决定其汇点得分和持续时间；主要汇点在中间层减弱，与次要汇点出现时间重合；大规模模型中汇点层级更确定和频繁（如QwQ-32B有3个层级，Qwen3-14B有6个层级）。

Conclusion: 注意力机制中存在两类不同的汇点：主要汇点和次要汇点。次要汇点具有独特的性质，主要出现在中间层，由特定MLP模块形成，其范数决定持续时间和影响程度。这一发现深化了对注意力机制内部工作原理的理解。
...
### [168] [DiRL: An Efficient Post-Training Framework for Diffusion Language Models](https://arxiv.org/abs/2512.22234)
*Ying Zhu,Jiaxin Wan,Xiaoran Liu,Siyanag He,Qiqi Wang,Xu Guo,Tianyi Liang,Zengfeng Huang,Ziwei He,Xipeng Qiu*

Main category: cs.LG

TL;DR: DiRL是一个针对扩散语言模型的高效后训练框架，通过结合FlexAttention加速的块状训练和LMDeploy优化的推理，实现了在线模型更新循环，显著提升了扩散语言模型在复杂推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案显示出潜力，但现有的后训练方法存在计算效率低下、训练与推理目标不匹配的问题，严重限制了在数学等复杂推理任务上的表现。

Method: 提出DiRL框架，整合FlexAttention加速的块状训练和LMDeploy优化的推理，实现高效在线模型更新。在此基础上提出DiPO——首个针对扩散语言模型的无偏组相对策略优化实现，采用两阶段后训练（监督微调+强化学习）。

Result: 在高质量数学数据上训练的DiRL-8B-Instruct模型在扩散语言模型中达到最先进的数学性能，并在多个基准测试中超越了Qwen2.5系列的可比模型。

Conclusion: DiRL框架有效解决了扩散语言模型后训练的计算效率和目标匹配问题，显著提升了复杂推理能力，为扩散语言模型的实际应用提供了有力支持。
...
### [169] [Masking Teacher and Reinforcing Student for Distilling Vision-Language Models](https://arxiv.org/abs/2512.22238)
*Byung-Kwan Lee,Yu-Chiang Frank Wang,Ryo Hachiuma*

Main category: cs.LG

TL;DR: 提出Masters框架，通过掩码渐进强化学习蒸馏，解决大规模视觉语言模型向小型模型知识迁移时的性能退化问题


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型虽然性能强大但体积过大难以部署到移动或边缘设备，需要紧凑但能力强的模型。然而从大型教师模型向小型学生模型的知识蒸馏面临巨大尺寸差距挑战，学生模型难以复现教师复杂的高维表示，导致学习不稳定和性能下降。

Method: 提出Masters框架：1) 掩码教师模型非主导权重以降低不必要复杂性；2) 训练过程中渐进恢复教师模型容量；3) 集成离线强化学习阶段，包含准确性奖励（衡量生成响应的正确性）和蒸馏奖励（量化从教师到学生响应的转移难易度）；4) 利用掩码教师预生成响应提供高效指导。

Result: Masters框架使学生模型能够以平滑稳定的方式从教师模型学习更丰富的表示，无需在线思考-回答过程即可实现强大性能，计算效率高且避免生成长响应。

Conclusion: Masters通过掩码渐进强化学习蒸馏有效解决了大规模视觉语言模型向小型模型知识迁移的挑战，实现了紧凑但能力强的模型部署，为移动和边缘设备应用提供了实用解决方案。
...
### [170] [EvoXplain: When Machine Learning Models Agree on Predictions but Disagree on Why -- Measuring Mechanistic Multiplicity Across Training Runs](https://arxiv.org/abs/2512.22240)
*Chama Bensmail*

Main category: cs.LG

TL;DR: EvoXplain框架通过分析模型在重复训练中解释的稳定性，揭示即使高准确率模型也可能存在多种不同的内部逻辑机制，挑战了"高准确率即解释可信"的假设。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习模型主要基于预测性能评估，一旦达到高准确率，其解释通常被认为是正确可信的。但这一假设忽视了关键问题：当两个模型都达到高准确率时，它们是否依赖相同的内部逻辑，还是通过不同甚至相互竞争的机制达到相同结果？

Method: 提出EvoXplain诊断框架，将解释视为从随机优化过程中抽取的样本（而非分析单个训练模型），在不聚合预测或构建集成的情况下，检查这些样本是否形成单一连贯解释或分离为多个不同的解释模式。

Result: 在乳腺癌和COMPAS数据集上使用逻辑回归和随机森林评估EvoXplain。尽管所有模型都达到高预测准确率，但它们的解释经常表现出明显的多模态性。即使是通常被认为稳定的逻辑回归模型，在相同数据分割上重复训练时也能产生多个良好分离的解释盆地。

Conclusion: EvoXplain不试图选择"正确"解释，而是使解释不稳定性可见且可量化，揭示单个实例或平均解释可能掩盖多种底层机制的存在。更广泛地说，EvoXplain将可解释性重新定义为重复实例化下模型类的属性，而非任何单个训练模型的属性。
...
### [171] [Calibrating LLM Judges: Linear Probes for Fast and Reliable Uncertainty Estimation](https://arxiv.org/abs/2512.22245)
*Bhaktipriya Radharapu,Eshika Saxena,Kenneth Li,Chenxi Whitehouse,Adina Williams,Nicola Cancedda*

Main category: cs.LG

TL;DR: 提出使用线性探针从LLM推理判断的隐藏状态中获取校准的不确定性估计，相比现有方法在保持校准性能的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的评判器在工业应用中日益重要，需要高效获取良好校准的不确定性估计用于生产部署。现有方法如语言化置信度和多生成方法要么校准效果差，要么计算成本高。

Method: 引入基于Brier分数的损失函数训练的线性探针，从推理评判器的隐藏状态中提供校准的不确定性估计，无需额外的模型训练。

Result: 探针在客观任务（推理、数学、事实性、编程）和主观人类偏好判断上都表现出比现有方法更优的校准性能，计算成本降低约10倍，能鲁棒地泛化到未见评估领域，并在高置信度预测上提供更高准确率。

Conclusion: 基于可解释性的不确定性估计为LLM评判器提供了实用、可扩展的即插即用解决方案，虽然会产生保守估计（在简单数据集上表现不佳但有利于安全关键部署），但整体上展示了该方法的生产部署价值。
...
### [172] [The Affine Divergence: Aligning Activation Updates Beyond Normalisation](https://arxiv.org/abs/2512.22247)
*George Bird*

Main category: cs.LG

TL;DR: 论文提出激活更新在梯度下降中存在系统性不匹配，推导出归一化的第一性原理解释，并提出替代现代归一化的新方法PatchNorm，挑战了传统的仿射+非线性模型构建方式。


<details>
  <summary>Details</summary>
Motivation: 在梯度下降优化过程中，参数更新遵循最陡下降方向，但激活更新并非最优。激活作为更接近损失的计算量，应优先优化，但其更新存在非理想的样本级缩放问题。这为重新理解归一化机制提供了理论动机。

Method: 1. 分析激活更新在仿射、卷积和注意力层中的缩放问题；2. 推导出归一化的第一性原理解决方案；3. 提出替代现代归一化的新方法，包括无尺度不变性的解决方案；4. 针对卷积层提出新的函数形式"PatchNorm"；5. 将归一化器分解为类似激活函数的映射。

Result: 1. 从第一性原理推导出归一化，提供了新的机制解释；2. 提出的替代方法在多个测试中优于传统归一化器；3. PatchNorm作为组合不可分的归一化器在卷积中表现良好；4. 实验验证了新函数的有效性。

Conclusion: 论文提供了一个理论驱动的框架，重新概念化了归一化的作用机制，提出了新的归一化函数形式，挑战了传统的仿射+非线性模型构建范式，并强调应优先优化表示而非参数。
...
### [173] [Temporal Visual Semantics-Induced Human Motion Understanding with Large Language Models](https://arxiv.org/abs/2512.22249)
*Zheng Xing,Weibing Zhao*

Main category: cs.LG

TL;DR: 本文提出了一种利用大语言模型提取时序视觉语义来增强无监督人体运动分割的方法，通过将文本化的运动信息融入子空间聚类框架，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统的人体运动分割方法忽视了时序语义信息的作用。本文旨在探索如何从人体运动序列中提取时序视觉语义，并利用大语言模型的图像到文本能力来增强子空间聚类性能。

Method: 通过大语言模型从连续帧中提取文本化运动信息，学习时序相邻信息，开发了TVS集成的子空间聚类方法，包含时序正则化的子空间嵌入和基于时序约束的分割，并引入了基于分割输出的反馈优化框架。

Result: 实验结果表明，该方法在四个基准人体运动数据集上优于现有的最先进方法。

Conclusion: 通过大语言模型提取时序视觉语义并将其融入子空间聚类框架，能够有效提升无监督人体运动分割的性能，证明了时序语义信息在该任务中的重要性。
...
### [174] [Cardiac mortality prediction in patients undergoing PCI based on real and synthetic data](https://arxiv.org/abs/2512.22259)
*Daniil Burakov,Ivan Petrov,Dmitrii Khelimskii,Ivan Bessonov,Mikhail Lazarev*

Main category: cs.LG

TL;DR: 该研究开发了基于真实和合成数据的PCI术后心脏死亡风险预测模型，通过机器学习方法和数据增强技术解决类别不平衡问题，识别出年龄、射血分数等关键影响因素。


<details>
  <summary>Details</summary>
Motivation: PCI术后患者状态、血管造影和手术特征包含预测长期结果的关键信号，但现有模型在处理类别不平衡问题时表现不佳，需要开发更稳健的预测方法来评估心脏死亡风险。

Method: 分析了2,044名接受分叉病变PCI的患者数据，应用多种机器学习模型预测3年死亡率。为解决类别不平衡问题，生成500个合成样本加入训练集。使用置换特征重要性评估特征贡献，并通过移除非信息特征进行额外实验。

Result: 数据增强显著提高了少数类别的召回率，同时AUROC损失最小，改善了概率质量，并在构建的严重病例档案上产生更临床合理的风险估计。年龄、射血分数、外周动脉疾病和脑血管疾病被确定为最具影响力的四个特征。

Conclusion: 使用现实和极端病例进行简单数据增强可以暴露、量化和减少基于表格记录的临床预测中的脆弱性，建议在报告主要指标的同时常规报告概率质量和压力测试结果。
...
### [175] [The Physics Constraint Paradox: When Removing Explicit Constraints Improves Physics-Informed Data for Machine Learning](https://arxiv.org/abs/2512.22261)
*Rahul D Ray*

Main category: cs.LG

TL;DR: 本文通过系统消融研究揭示了物理约束数据生成中的悖论：显式能量守恒约束在物理方程一致时是数学冗余的，而法布里-珀罗振荡对带宽预测影响显著，移除后带宽预测精度提升31.3%。


<details>
  <summary>Details</summary>
Motivation: 在科学领域中，真实数据稀缺时，物理约束数据生成对机器学习至关重要。但现有方法往往过度约束模型，未能识别哪些物理组件是必要的。本文旨在系统研究物理约束在光栅耦合器光谱生成器中的作用。

Method: 对物理信息光栅耦合器光谱生成器进行系统消融研究，该生成器将五个几何参数映射到100点光谱响应。通过选择性移除显式能量守恒约束、法布里-珀罗振荡、带宽变化和噪声，分析各组件对生成结果的影响。

Result: 发现物理约束悖论：当基础方程物理一致时，显式能量守恒约束是数学冗余的，约束和非约束变体达到相同的守恒精度（平均误差约7×10^-9）。法布里-珀罗振荡主导基于阈值的带宽变异性，移除后半高带宽分布减少72%（从132.3nm降至37.4nm）。标准噪声添加加重新归一化流程引入0.5%的非物理负吸收值。生成器运行速度为200样本/秒，比典型全波求解器快几个数量级。下游机器学习评估显示物理可学习性权衡：中心波长预测不受影响，但移除法布里-珀罗振荡使带宽预测R²提高31.3%，RMSE降低73.8%。

Conclusion: 研究结果为物理信息数据集设计提供了可操作的指导，并强调机器学习性能可作为评估约束相关性的诊断工具。显式物理约束在基础方程已物理一致时可能是冗余的，而某些物理组件（如法布里-珀罗振荡）可能阻碍机器学习性能，需要仔细权衡。
...
### [176] [LLMTM: Benchmarking and Optimizing LLMs for Temporal Motif Analysis in Dynamic Graphs](https://arxiv.org/abs/2512.22266)
*Bing Hao,Minglai Shao,Zengyi Wo,Yunlong Chu,Yuhang Liu,Ruijie Wang*

Main category: cs.LG

TL;DR: 该论文系统研究了大型语言模型在动态图时序模体分析中的性能，提出了LLMTM基准测试，开发了工具增强的LLM智能体，并设计了结构感知调度器来平衡准确性和成本。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的广泛应用，其在动态图处理能力方面的研究日益重要。时序模体作为动态图的基本单元和重要局部属性，能直接反映异常和独特现象，对理解动态图的演化动态和结构特征至关重要。然而，利用LLM进行动态图时序模体分析的研究相对较少。

Method: 1. 提出LLMTM基准测试，包含六种定制任务和九种时序模体类型；2. 通过大量实验分析不同提示技术和LLM模型的影响；3. 开发工具增强的LLM智能体，使用精确设计的提示来解决任务；4. 提出结构感知调度器，考虑动态图的结构属性和LLM的认知负载，智能地在标准LLM提示和更强大的智能体之间调度查询。

Result: 实验表明，工具增强的LLM智能体能以高精度解决时序模体相关任务，但成本较高。结构感知调度器能有效保持高准确性同时显著降低成本。

Conclusion: 该研究系统探索了LLM在动态图时序模体分析中的能力，提出的基准测试、工具增强智能体和结构感知调度器为解决时序模体分析任务提供了有效方案，在准确性和成本之间实现了良好平衡。
...
### [177] [Hierarchical Stacking Optimization Using Dirichlet's Process (SoDip): Towards Accelerated Design for Graft Polymerization](https://arxiv.org/abs/2512.22279)
*Amgad Ahmed Ali Ibrahim,Hein Htet,Ryoji Asahi*

Main category: cs.LG

TL;DR: SoDip框架通过层次化堆叠优化方法，结合Transformer、TabNet/XGBoost、高斯过程回归和贝叶斯优化，解决了辐射诱导接枝聚合中因基膜形态差异导致的重复性问题，实现了33%的性能提升和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 辐射诱导接枝聚合在功能化聚合物薄膜方面具有重要应用，但由于基膜形态（结晶度、晶粒取向、自由体积）的未报告变异性，导致单体扩散、自由基分布和Trommsdorff效应存在空间梯度，造成性能不一致和重复性受限的问题。

Method: 提出SoDip层次化堆叠优化框架：1) 使用DeepSeek-R1 Transformer编码文本过程描述符；2) TabNet和XGBoost建模多模态特征交互；3) 高斯过程回归与狄利克雷过程混合模型进行不确定性量化和异方差处理；4) 贝叶斯优化高效探索高维合成空间。使用ChemDataExtractor 2.0和WebPlotDigitizer构建多样化数据集。

Result: 在交叉验证中，SoDip相比高斯过程回归实现了约33%的性能提升，同时提供校准的置信区间以识别低重复性区域。其堆叠架构能够整合稀疏文本和数值输入，优于先前模型。

Conclusion: SoDip框架为接枝聚合研究建立了可重复、形态感知设计的基础，能够处理不同质量的稀疏文本和数值输入，有效解决了辐射诱导接枝聚合中的重复性问题。
...
### [178] [Valori: A Deterministic Memory Substrate for AI Systems](https://arxiv.org/abs/2512.22280)
*Varshith Gudur*

Main category: cs.LG

TL;DR: Valori是一个确定性AI内存基板，通过固定点算术和状态机模型解决向量嵌入存储搜索中的非确定性问题，确保跨平台比特一致性。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统使用浮点算术存储和搜索向量嵌入，这种设计存在固有的非确定性：相同的模型、输入和代码在不同硬件架构（如x86与ARM）上会产生不同的内存状态和检索结果。这破坏了可重现性和安全部署，导致数据静默分歧，阻碍事后验证并损害受监管行业的审计追踪。

Method: Valori采用固定点算术（Q16.16）替代浮点内存操作，将内存建模为可重现的状态机。该方法在内存边界强制执行确定性，确保比特一致的内存状态、快照和搜索结果。

Result: Valori能够保证跨平台的比特一致内存状态、快照和搜索结果。研究表明非确定性在索引或检索之前就已出现，而Valori在内存边界有效强制执行了确定性。

Conclusion: 确定性内存是可信AI系统的必要基础组件。Valori提供了一个开源参考实现，为AI系统提供了可重现、可验证的内存操作保障。
...
### [179] [Cluster Aggregated GAN (CAG): A Cluster-Based Hybrid Model for Appliance Pattern Generation](https://arxiv.org/abs/2512.22287)
...
<details>
  <summary>Details</summary>
Motivation: 现有GAN方法将所有电器统一处理，忽略了间歇性和连续性电器的行为差异，导致训练不稳定和输出保真度有限。需要一种能够区分不同类型电器行为特征的合成方法。

Method: 提出Cluster Aggregated GAN混合生成框架：1) 根据电器行为特征将电器路由到专门分支；2) 对间歇性电器使用聚类模块分组相似激活模式，为每个集群分配专用生成器；3) 对连续性电器使用基于LSTM的生成器捕捉时间演化，通过序列压缩保持训练稳定性。

Result: 在UVIC智能插座数据集上的实验表明，该框架在真实性、多样性和训练稳定性指标上均优于基线方法。将聚类作为主动生成组件显著提高了可解释性和可扩展性。

Conclusion: 该框架为非侵入式负载监测研究中的合成负载生成提供了有效方法，能够更好地处理不同类型电器的行为特征差异。
...
### [180] [Co-GRPO: Co-Optimized Group Relative Policy Optimization for Masked Diffusion Model](https://arxiv.org/abs/2512.22288)
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在训练和推理之间存在显著差异：训练使用简化的单步BERT式目标，而推理是多步迭代过程，涉及复杂的token解码调度。这种训练范式与推理轨迹级本质的脱节导致推理调度从未在训练中得到优化。

Method: 将MDM生成重新表述为统一的马尔可夫决策过程，联合包含模型和推理调度。应用轨迹级的组相对策略优化，在共享奖励下协同优化模型参数和调度参数，无需通过多步生成过程进行昂贵的反向传播。

Result: 在ImageReward、HPS、GenEval和DPG-Bench四个基准测试上的实证结果表明，该方法显著提高了生成质量，证明了其有效性。

Conclusion: Co-GRPO通过将训练与推理更彻底地对齐，实现了更全面的优化，显著提升了掩码扩散模型的生成性能。
...
### [181] [When Algorithms Manage Humans: A Double Machine Learning Approach to Estimating Nonlinear Effects of Algorithmic Control on Gig Worker Performance and Wellbeing](https://arxiv.org/abs/2512.22290)
*Arunkumar V,Nivethitha S,Sharan Srinivas,Gangadharan G. R*

Main category: cs.LG

TL;DR: 使用双重机器学习框架分析算法管理下的人力资源实践对零工工作者福祉和绩效的非线性影响，发现算法监督的透明度是关键调节因素


<details>
  <summary>Details</summary>
Motivation: 研究算法管理时代人本管理能否持续存在，传统线性模型可能无法捕捉工作者对算法系统的复杂非线性反应

Method: 采用双重机器学习框架估计调节中介模型，避免强加限制性函数形式，使用464名零工工作者的调查数据

Result: 发现明显的非单调模式：支持性HR实践提升福祉，但其与绩效的联系在算法监督模糊不清的中间区域减弱，而在监督透明可解释时增强

Conclusion: 简单的线性设定可能得出相反结论；平台设计需注意：部分定义的监督造成混乱，但清晰的规则和可信的申诉机制能使强监督可行；方法上展示了双重机器学习在组织研究中估计条件间接效应的价值
...
### [182] [Learning from Negative Examples: Why Warning-Framed Training Data Teaches What It Warns Against](https://arxiv.org/abs/2512.22293)
*Tsogt-Ochir Enkhbayar*

Main category: cs.LG

TL;DR: 警告性内容在训练数据中并不能有效教导语言模型避免被警告的行为，模型在警告和直接提供内容两种情况下产生被标记内容的比率相近（76.7% vs 83.3%），表明当前架构中统计共现主导于语用解释


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解为什么包含警告的训练数据（如"不要使用-此代码存在漏洞"）无法有效教导语言模型避免被警告的行为，探索模型在处理警告内容时的内在机制

Method: 通过实验比较模型在警告框架内容和直接提供内容两种情况下的行为表现，使用稀疏自编码器分析潜在特征激活模式，识别"描述X"和"执行X"之间的特征重叠，并分析"隐形滑移"现象

Result: 模型在警告框架下产生被标记内容的比率为76.7%，与直接提供内容下的83.3%无统计学显著差异；稀疏自编码器分析显示特征#8684在警告和利用两种情境下激活程度相似，表明"描述"和"执行"激活重叠的潜在特征

Conclusion: 当前架构中统计共现主导于语用解释，模型学习的是在特定语境下倾向于出现什么，而不是为什么出现在那里；提示和推理时引导无法解决此问题，训练时的特征消融可能有效
...
### [183] [LLMBoost: Make Large Language Models Stronger with Boosting](https://arxiv.org/abs/2512.22309)
...
TL;DR: LLMBoost是一个新颖的集成微调框架，通过利用LLM的中间状态实现层次化错误校正和知识传递，在提升准确性的同时降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有集成学习方法通常将模型视为黑箱，仅结合输入或最终输出，忽略了丰富的内部表示和跨模型交互。需要打破这一限制，充分利用LLM的中间状态来提升性能。

Method: 提出三个关键创新：1) 跨模型注意力机制，让后继模型访问并融合前驱模型的隐藏状态；2) 链式训练范式，以错误抑制为目标逐步微调连接模型；3) 近并行推理范式，逐层流水线化隐藏状态传输。

Result: 在常识推理和算术推理任务上的广泛实验表明，LLMBoost能持续提升准确性，同时减少推理延迟。理论分析证明在有限校正假设下，顺序集成能保证单调改进。

Conclusion: LLMBoost通过利用LLM中间状态打破了传统集成学习的黑箱限制，实现了高效的层次化错误校正和知识传递，为LLM集成学习提供了新的有效范式。
...
### [184] [Optimistic Feasible Search for Closed-Loop Fair Threshold Decision-Making](https://arxiv.org/abs/2512.22313)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 本文提出了一种名为乐观可行搜索（OFS）的在线学习方法，用于在公平性和服务率约束下学习一维阈值策略，通过维护置信区间来选择可行且奖励最大的阈值。


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统（如贷款、筛查或累犯风险评估）在运行时常面临公平性和服务约束，同时会引发反馈效应：决策会改变未来出现的人群分布，导致数据非平稳并可能放大不平等。需要在线学习方法在满足约束条件下学习最优阈值策略。

Method: 提出乐观可行搜索（OFS）方法：基于网格搜索，为每个候选阈值维护奖励和约束残差的置信区间。每轮选择在置信区间内看起来可行的阈值中乐观奖励最大的；若无可行阈值，则选择乐观约束违反最小的阈值。该方法针对低维、可解释的策略类别设计。

Result: 在三个基准测试中评估OFS：（1）具有稳定收缩动态的合成闭环基准；（2）基于德国信用和COMPAS数据构建的半合成闭环基准。OFS在所有环境中都比无约束和原始对偶bandit基线获得更高奖励且累积约束违反更小，接近相同扫描程序下最佳可行固定阈值的oracle性能。

Conclusion: OFS方法能有效在公平性和服务率约束下在线学习阈值策略，处理闭环系统中的反馈效应，在多个实际场景中表现优于现有基线方法，且实验可复现并采用双盲友好的相对输出组织。
...
### [185] [LangPrecip: Language-Aware Multimodal Precipitation Nowcasting](https://arxiv.org/abs/2512.22317)
*Xudong Ling,Tianxi Huang,Qian Dong,Tao He,Chaorong Li,Guiduo Duan*

Main category: cs.LG

TL;DR: LangPrecip是一个语言感知的多模态降水临近预报框架，通过将气象文本作为降水演化的语义运动约束，在Rectified Flow范式下实现文本和雷达信息的有效整合，显著提升了强降水预报性能。
...
Method: 提出语言感知多模态临近预报框架(LangPrecip)，将气象文本作为降水演化的语义运动约束，在Rectified Flow范式下将临近预报建模为语义约束的轨迹生成问题，实现文本和雷达信息在潜在空间的高效物理一致性整合。同时构建了包含16万对雷达序列和运动描述的大规模多模态数据集LangPrecip-160k。

Result: 在瑞典和MRMS数据集上的实验表明，该方法相比最先进方法取得了持续改进，在80分钟预报时效上，强降水CSI指标分别获得了超过60%和19%的提升。

Conclusion: 通过将语言作为语义运动约束，LangPrecip框架能够有效整合文本和雷达信息，显著提升降水临近预报性能，特别是在强降水事件中表现优异，为多模态气象预报提供了新思路。
...
### [186] [Decomposing Uncertainty in Probabilistic Knowledge Graph Embeddings: Why Entity Variance Is Not Enough](https://arxiv.org/abs/2512.22318)
*Chorok Lee*

Main category: cs.LG

TL;DR: 现有概率知识图谱嵌入方法存在关系无关的实体不确定性估计问题，无法区分新兴实体和新型关系上下文两种不同的分布外现象。本文提出将不确定性分解为语义不确定性和结构不确定性，并通过组合方法显著提升了分布外检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前概率知识图谱嵌入方法将实体表示为分布，使用学习到的方差来量化认知不确定性。但存在一个根本性限制：这些方差是关系无关的，意味着实体在不同关系上下文中具有相同的不确定性。这混淆了两种行为相反的分布外现象：新兴实体（罕见、学习不充分）和新型关系上下文（熟悉实体在未观察到的关系中）。

Method: 提出不确定性分解为互补的两个部分：1）语义不确定性（来自实体嵌入方差，用于检测新兴实体）；2）结构不确定性（来自实体-关系共现，用于检测新型关系上下文）。理论证明这两个信号是非冗余的，任何凸组合都严格优于单独使用任一信号。提出的CAGP方法通过学习的权重组合语义和结构不确定性。

Result: 在三个数据集（FB15k-237、WN18RR、YAGO3-10）上验证了新型关系上下文三元组100%具有频率匹配的分布内对应项。CAGP方法在时间分布偏移的分布外检测上达到0.94-0.99 AUROC，相比关系无关基线相对提升60-80%。在选择性预测中，在85%回答率下减少43%的错误。

Conclusion: 关系无关的不确定性估计存在根本性限制，无法有效检测新型关系上下文。通过将不确定性分解为语义和结构两个互补部分，并学习它们的组合权重，可以显著提升分布外检测性能。理论证明了这种组合方法的优越性，并在多个基准测试中验证了其有效性。
...
### [187] [Expert System for Bitcoin Forecasting: Integrating Global Liquidity via TimeXer Transformers](https://arxiv.org/abs/2512.22326)
*Sravan Karthick T*
...
<details>
  <summary>Details</summary>
Motivation: 比特币价格预测面临极端波动性和非平稳性的挑战，传统单变量时间序列模型在长期预测中表现不佳。研究旨在填补将全球宏观经济流动性作为领先指标整合到预测模型中的空白。

Method: 使用TimeXer架构，整合来自18个主要经济体的全球M2流动性作为具有12周滞后结构的外生变量，构建TimeXer-Exog模型。与LSTM、N-BEATS、PatchTST和标准单变量TimeXer等先进基准模型进行比较。

Result: 在2020年1月至2025年8月的每日比特币价格数据上，TimeXer-Exog模型在70天预测期上取得MSE为1.08e8，比单变量TimeXer基线模型误差降低超过89%。宏观流动性条件显著稳定了长期预测。

Conclusion: 将全球流动性条件整合到深度学习模型中，对比特币长期价格预测有显著改进效果，表明宏观经济因素在加密货币价格预测中的重要性。
...
### [188] [The Effectiveness of Approximate Regularized Replay for Efficient Supervised Fine-Tuning of Large Language Models](https://arxiv.org/abs/2512.22337)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LoRA等参数高效微调方法只修改少量参数，但它们可能对模型产生显著影响。研究发现LoRA监督微调会灾难性地降低模型能力，即使在小数据集上训练少量步骤也会出现此问题。

Method: 提出正则化近似重放方法：1) 惩罚与初始模型的KL散度；2) 交错使用与预训练相似的开源语料进行下一个token预测的数据。该方法计算开销很小。

Result: 应用于Qwen指令微调模型时，该方法能保持模型的通用知识，同时不阻碍对新任务的可塑性，仅增加少量计算开销。

Conclusion: 虽然最直接的LoRA微调方法在实践中可能失败，但通过简单的训练过程调整可以几乎完全消除模型能力退化问题，实现参数高效微调的同时保持模型原有能力。
...
### [189] [Completed Hyperparameter Transfer across Modules, Width, Depth, Batch and Duration](https://arxiv.org/abs/2512.22382)
...
<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型的训练稳定性和最终性能有显著影响。现有的μP等参数化方法虽然实现了全局超参数在不同模型规模间的迁移，但主要关注宽度缩放，缺乏对深度、批次大小等多维缩放轴的支持，且未探索模块级超参数优化。

Method: 提出了Complete^{(d)}参数化方法，基于CompleteP进行扩展，统一处理宽度、深度、批次大小和训练时长的缩放。研究了模块级超参数优化与迁移，分析了高维超参数空间的导航挑战，并提出了实用的优化指导原则。

Result: 实验证明，在适当的参数化下，超参数迁移在模块级超参数场景中仍然有效。研究覆盖了现代模型的关键优化超参数：学习率、AdamW参数、权重衰减、初始化尺度和残差块乘数。使用迁移的模块级超参数显著提升了大型语言模型的训练速度。

Conclusion: Complete^{(d)}参数化方法能够统一处理多个重要的缩放维度，模块级超参数优化与迁移是可行的，并且能够带来显著的训练加速效果。这为大规模模型的高效超参数调优提供了新的实践指导。
...
### [190] [AFA-LoRA: Enabling Non-Linear Adaptations in LoRA with Activation Function Annealing](https://arxiv.org/abs/2512.22455)
*Jiacheng Li,Jianchao Tan,Zhidong Yang,Feiye Huo,Yerui Sun,Yuchen Xie,Xunliang Cai*

Main category: cs.LG

TL;DR: AFA-LoRA是一种改进的LoRA训练策略，通过退火激活函数将非线性表达能力引入LoRA，同时保持其可合并性，缩小了LoRA与全参数微调之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: LoRA作为广泛采用的参数高效微调方法，其线性适应过程限制了表达能力，在线性训练和非线性训练之间存在表达能力差距。

Method: 提出AFA-LoRA训练策略，采用退火激活函数在训练过程中从非线性变换过渡到线性变换，使适配器在收敛到可合并线性形式前具备更强的表示能力。

Result: 在监督微调、强化学习和推测解码等任务上实施该方法，结果显示AFA-LoRA显著减少了LoRA与全参数训练之间的性能差距。

Conclusion: 这项工作实现了更强大且实用的参数高效适应范式，为LoRA方法带来了非线性表达能力，同时保持了其可合并性的优势。
...
### [191] [AMBIT: Augmenting Mobility Baselines with Interpretable Trees](https://arxiv.org/abs/2512.22466)
*Qizhi Wang*

Main category: cs.LG

TL;DR: AMBIT框架通过将物理移动模型与可解释的树模型结合，解决OD流预测中精度与可解释性的冲突问题


<details>
  <summary>Details</summary>
Motivation: OD流预测在GIS和城市分析中是核心任务，但实际部署面临高精度和清晰可解释性之间的冲突需求

Method: 开发AMBIT灰盒框架，在物理移动基线模型基础上添加可解释的树模型；首先对经典空间交互模型进行全面审计，然后在物理基线模型上构建残差学习器，使用梯度提升树和SHAP分析

Result: 物理基础残差学习器在保持可解释结构的同时接近强树基预测器的精度；POI锚定残差在空间泛化下表现最稳健且具有竞争力

Conclusion: AMBIT框架为城市决策提供了可复现的流程、丰富的诊断和空间误差分析，平衡了OD流预测的精度和可解释性需求
...
### [192] [Collaborative Optimization of Multiclass Imbalanced Learning: Density-Aware and Region-Guided Boosting](https://arxiv.org/abs/2512.22478)
...
Method: 提出协同优化的多类不平衡学习Boosting模型，集成密度因子和置信因子设计抗噪声权重更新机制和动态采样策略，实现权重更新、样本区域划分和区域引导采样的紧密集成

Result: 在20个公开不平衡数据集上的实验表明，该模型显著优于8个最先进的基线方法

Conclusion: 通过协同优化不平衡学习与模型训练，提出的模型在分类性能上取得了显著提升，代码已开源
...
### [193] [Predicting LLM Correctness in Prosthodontics Using Metadata and Hallucination Signals](https://arxiv.org/abs/2512.22508)
*Lucky Susanto,Anasta Pranawijayana,Cortino Sukotjo,Soni Prasad,Derry Wijaya*

Main category: cs.LG

TL;DR: 该研究探索了使用元数据和幻觉信号来预测LLM在牙科考试中回答正确性的可行性，发现这种方法能提高准确性但尚不足以用于高风险部署。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在医疗等高风险领域的应用增加，预测其回答的正确性变得至关重要，但目前这一问题尚未得到充分探索。

Method: 研究分析了通用模型GPT-4o和推理中心模型OSS-120B在牙科多选题考试上的表现，使用三种不同的提示策略，基于元数据和幻觉信号为每个（模型，提示）组合构建正确性预测器。

Result: 元数据方法能将准确性提高最多+7.14%，达到83.12%的精确度；实际幻觉是错误答案的强指标，但仅靠元数据信号不能可靠预测幻觉；提示策略虽不影响总体准确性，但显著改变模型内部行为和元数据的预测效用。

Conclusion: 该研究为开发LLM可靠性信号提供了有前景的方向，但所探索的方法尚不够稳健，不适合用于关键的高风险部署。
...
### [194] [TimePerceiver: An Encoder-Decoder Framework for Generalized Time-Series Forecasting](https://arxiv.org/abs/2512.22550)
*Jaebin Lee,Hankook Lee*

Main category: cs.LG

TL;DR: TimePerceiver：一个统一的编码器-解码器时间序列预测框架，通过广义预测任务设计和位置感知架构，结合有效训练策略，在多个基准数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列预测研究主要关注编码器设计，而将预测和解码视为次要问题。本文旨在提出一个统一的编码器-解码器框架，将预测任务与训练策略紧密结合。

Method: 1. 将预测任务广义化为外推、插值和填补等多样化时间预测目标；2. 设计能够灵活感知和适应不同时间位置的编码器-解码器架构；3. 编码器使用潜在瓶颈表示来捕获时间和跨通道依赖；4. 解码器使用可学习查询对应目标时间戳来检索相关信息。

Result: 在广泛的基准数据集上，TimePerceiver框架一致且显著地超越了先前的最先进基线方法。

Conclusion: TimePerceiver提供了一个统一的编码器-解码器预测框架，通过将预测任务广义化、设计位置感知架构以及结合有效训练策略，实现了在时间序列预测任务上的卓越性能。
...
### [195] [On Admissible Rank-based Input Normalization Operators](https://arxiv.org/abs/2512.22587)
*Taeyun Kim*

Main category: cs.LG

TL;DR: 本文分析了基于排名的输入归一化方法，指出现有可微排序和排名算子存在结构性问题，提出了三个公理来定义有效的基于排名的归一化算子，并构建了一个满足这些公理的最小算子。


<details>
  <summary>Details</summary>
Motivation: 在许多实际系统中，特征值的排序比原始数值更重要，但基于排名的归一化算子需要满足的结构条件从未被正式确定。现有可微排序和排名算子存在稳定性问题，需要建立形式化的标准来区分有效的基于排名的归一化算子。

Method: 提出了三个公理来形式化基于排名的输入归一化所需的最小不变性和稳定性属性。证明了任何满足这些公理的算子必须分解为：(i)特征级排名表示和(ii)单调且Lipschitz连续的标量化映射。然后构建了一个满足这些标准的最小算子。

Result: 证明了广泛使用的可微排序和排名算子由于依赖值间隙和批次级成对交互，在严格单调变换、小批次组成变化甚至微小输入扰动下本质不稳定。提出的公理和最小算子能够满足所需的稳定性要求，这些约束在实际设置中是非平凡的。

Conclusion: 研究结果清晰地界定了有效的基于排名的归一化算子的设计空间，并将它们与现有的基于连续松弛的排序方法正式区分开来。提出的公理框架为构建稳定、可靠的基于排名的归一化算子提供了理论基础。
...
### [196] [Data-Driven Analysis of Crash Patterns in SAE Level 2 and Level 4 Automated Vehicles Using K-means Clustering and Association Rule Mining](https://arxiv.org/abs/2512.22589)
*Jewel Rana Palit,Vijayalakshmi K Kumarasamy,Osama A. Osman*

Main category: cs.LG

TL;DR: 本研究分析了美国NHTSA的2500多起自动驾驶汽车事故记录，通过两阶段数据挖掘框架（K-means聚类和关联规则挖掘）揭示SAE 2级和4级自动驾驶的事故动态模式。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车虽有望减少人为驾驶错误，但近期事故数据显示其行为可能偏离预期安全结果。现有研究多基于小规模加州数据集，缺乏对不同自动化级别事故趋势的系统分析，需要更全面的研究来理解混合交通环境中的事故动态。

Method: 开发两阶段数据挖掘框架：首先使用K-means聚类基于时间、空间和环境因素将事故记录分为4个行为簇；然后在每个簇内应用关联规则挖掘（ARM）提取可解释的多变量关系，分析照明条件、路面状况、车辆动态和环境条件等事故影响因素。

Result: 通过对SAE 2级和4级自动驾驶的2500多起事故记录分析，识别出4个不同的行为簇，并揭示了每个簇内事故模式与影响因素之间的具体关联关系，为理解自动驾驶事故动态提供了实证依据。

Conclusion: 研究结果为自动驾驶开发者、安全监管机构和政策制定者提供了可操作的指导，有助于制定更有效的自动驾驶部署策略和降低事故风险。该方法框架能够系统分析大规模事故数据，揭示复杂的事故模式关系。
...
### [197] [Energy-Guided Flow Matching Enables Few-Step Conformer Generation and Ground-State Identification](https://arxiv.org/abs/2512.22597)
*Guikun Xu,Xiaohan Yi,Peilin Zhao,Yatao Bian*

Main category: cs.LG

TL;DR: EnFlow是一个统一的框架，通过将流匹配与显式学习的能量模型耦合，在非高斯流匹配路径上实现能量引导采样，显著提高构象生成质量和基态预测准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于物理的计算方法生成低能构象集合和识别基态构象计算成本高。现有的学习型方法存在碎片化问题：生成模型能捕捉多样性但缺乏可靠的能量校准，而确定性预测器只针对单一结构且无法表示集合变异性。

Method: EnFlow将流匹配（FM）与显式学习的能量模型通过能量引导采样方案耦合，在非高斯FM路径上定义。采样过程中加入能量梯度引导，将轨迹导向低能区域。学习到的能量函数还能对生成的集合进行基于能量的排序，实现准确的基态识别。

Result: 在GEOM-QM9和GEOM-Drugs数据集上的广泛实验表明，EnFlow在1-2个ODE步骤内同时改进了生成指标，并相比最先进方法减少了基态预测误差。

Conclusion: EnFlow提供了一个统一的框架，通过能量引导采样将生成多样性和能量准确性相结合，显著提高了构象生成质量和基态预测性能。
...
### [198] [Communication Compression for Distributed Learning with Aggregate and Server-Guided Feedback](https://arxiv.org/abs/2512.22623)
*Tomas Ortega,Chun-Yin Huang,Xiaoxiao Li,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 本文提出了两种无客户端状态的有偏压缩框架CAFe和CAFe-S，解决了联邦学习中通信瓶颈和隐私保护问题，通过全局聚合更新作为共享控制变量，在非凸场景下证明了优于现有压缩方案的收敛性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临通信成本瓶颈，特别是客户端到服务器的上行传输受到边缘设备不对称带宽限制。虽然有偏压缩技术在实践中有效，但需要误差反馈机制来保证理论收敛性，而标准的误差反馈依赖于客户端特定的控制变量，这既违反用户隐私，也不适用于大规模联邦学习中常见的无状态客户端。

Method: 提出了两种新颖框架：1) CAFe（压缩聚合反馈）：使用前一轮的全局聚合更新作为所有客户端的共享控制变量；2) CAFe-S（服务器引导的压缩聚合反馈）：在服务器拥有小型私有数据集时，生成服务器引导的候选更新作为更准确的预测器。以分布式梯度下降为代表性算法进行分析。

Result: 在非凸场景和有界梯度差异条件下，从理论上证明了CAFe优于带有偏压缩的分布式压缩梯度下降。进一步证明了CAFe-S收敛到平稳点，且收敛速率随着服务器数据更具代表性而提高。联邦学习场景的实验结果验证了所提方法优于现有压缩方案。

Conclusion: CAFe和CAFe-S框架有效解决了联邦学习中的通信瓶颈问题，同时保护了用户隐私并兼容无状态客户端。这些方法通过利用全局聚合信息作为共享控制变量，在保持压缩效率的同时确保了理论收敛性，为大规模联邦学习的实际部署提供了可行方案。
...
### [199] [Scaling Unverifiable Rewards: A Case Study on Visual Insights](https://arxiv.org/abs/2512.22650)
*Shuyu Gan,James Mooney,Pan Hao,Renxiang Wang,Mingyi Hong,Qianwen Wang,Dongyeop Kang*

Main category: cs.LG

TL;DR: 选择性TTS：一种基于过程的精炼框架，在多阶段多智能体管道中分布计算资源，通过早期剪枝低质量分支来稳定精炼过程，提高开放任务质量


<details>
  <summary>Details</summary>
Motivation: 传统基于奖励的LLM智能体精炼方法在处理多阶段管道任务时存在局限：最终结果缺乏可验证的奖励信号，奖励模型训练数据不足，导致基于评判的精炼容易在多个阶段中累积误差

Method: 提出选择性TTS框架，在多智能体管道中分布计算资源而非时间上的重复精炼；使用过程特定的评判器早期剪枝低质量分支；基于数据科学管道构建端到端多智能体系统，设计可靠的LLM评判模型

Result: 在固定计算预算下，选择性TTS将平均得分从61.64提高到65.86，同时降低了方差；设计的LLM评判模型与人类专家对齐（Kendall's τ=0.55）

Conclusion: 选择性TTS为处理缺乏可验证奖励的复杂开放任务（如科学发现和故事生成）提供了有效方法，通过过程导向的精炼策略稳定了多阶段推理
...
### [200] [What Matters in Deep Learning for Time Series Forecasting?](https://arxiv.org/abs/2512.22702)
*Valentina Moretti,Andrea Cini,Ivan Marisca,Cesare Alippi*

Main category: cs.LG

TL;DR: 本文分析了深度学习时间序列预测架构的设计空间，指出当前基准测试存在问题，强调基于预测原则的模型设计比特定序列建模层更重要，简单但设计良好的架构可以达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 深度学习在时间序列应用中日益流行，但大量新提出的架构和相互矛盾的实证结果使得难以评估哪些组件对最终性能有显著贡献。本文旨在理清当前时间序列预测深度学习架构的设计空间，解释观察到的意外结果。

Method: 通过讨论设计维度和权衡来理解当前架构设计空间，评估局部性和全局性等概念在近期预测架构中的应用，分析现有架构中被忽视的实现细节如何改变预测方法类别并影响实证结果。

Result: 研究表明，考虑局部性和全局性等基础方面比采用特定序列建模层对实现准确结果更为重要，简单但设计良好的预测架构通常能与最先进方法相媲美。现有架构中被忽视的实现细节会从根本上改变预测方法类别并显著影响实证结果。

Conclusion: 需要重新思考当前有缺陷的基准测试实践，在设计架构时应关注预测问题的基础方面。作为这一方向的步骤，提出了辅助预测模型卡，其字段用于基于关键设计选择来表征现有和新预测架构。
...
### [201] [FoldAct: Efficient and Stable Context Folding for Long-Horizon Search Agents](https://arxiv.org/abs/2512.22733)
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文折叠方法将摘要动作视为标准动作，忽略了摘要会从根本上改变智能体未来的观测空间，导致策略依赖的非平稳观测分布，违反了RL的核心假设。

Method: FoldAct框架包含三个关键创新：1) 分离损失计算，为摘要和动作token提供独立的梯度信号；2) 全上下文一致性损失，减少分布偏移；3) 选择性片段训练，降低计算成本。

Result: 该方法实现了长视野搜索智能体在上下文折叠下的稳定训练，解决了非平稳观测问题，同时将训练效率提高了5.19倍。

Conclusion: FoldAct通过明确处理上下文折叠带来的非平稳观测分布问题，为长视野RL提供了稳定高效的训练框架，解决了现有方法的根本性缺陷。
...
### [202] [When Does Multi-Task Learning Fail? Quantifying Data Imbalance and Task Independence in Metal Alloy Property Prediction](https://arxiv.org/abs/2512.22740)
*Sungwoo Kang*

Main category: cs.LG

TL;DR: 多任务学习在合金性能预测中呈现分化结果：回归任务性能显著下降，但分类任务性能提升
...
Method: 使用54,028个合金样本同时预测电阻率、维氏硬度和非晶形成能力，比较单任务模型与标准和结构化多任务学习

Result: 多任务学习显著降低回归性能（电阻率R²从0.897降至0.844；硬度R²从0.832降至0.694），但提升分类性能（非晶F1从0.703升至0.744，召回率提升17%）。分析显示任务间权重接近零，表明性能独立性

Conclusion: 对于精确回归任务推荐使用独立模型，而对于分类任务（特别是召回率关键时）可保留多任务学习。回归失败归因于严重数据不平衡（52k vs. 800样本）导致的负迁移
...
### [203] [A Micro-Macro Machine Learning Framework for Predicting Childhood Obesity Risk Using NHANES and Environmental Determinants](https://arxiv.org/abs/2512.22758)
*Eswarasanthosh Kumar Mamillapalli,Nishtha Sharma*

Main category: cs.LG

TL;DR: 本研究提出了一个微观-宏观机器学习框架，整合个体层面（NHANES）和宏观环境层面（USDA/EPA）数据来预测儿童肥胖风险，发现环境脆弱性与肥胖风险分布存在地理相似性。


<details>
  <summary>Details</summary>
Motivation: 传统流行病学研究通常独立分析个体、家庭和环境层面的风险因素，限制了理解结构性环境条件如何与个体特征相互作用影响健康结果。儿童肥胖作为美国主要的公共卫生挑战，需要更全面的多层面分析方法。

Method: 开发了一个微观-宏观机器学习框架：1）使用NHANES的个体层面人体测量和社会经济数据；2）整合USDA和EPA的宏观环境特征（食品获取、空气质量、社会经济脆弱性）；3）训练四种机器学习模型（逻辑回归、随机森林、XGBoost、LightGBM）预测肥胖；4）构建环境脆弱性综合指数（EnvScore）。

Result: XGBoost模型表现最佳；构建的环境脆弱性指数显示，环境负担高的州与全国预测的微观层面肥胖风险分布存在强烈的地理相似性，表明环境驱动的不平等在肥胖风险中起重要作用。

Conclusion: 该研究证明了整合多尺度数据集识别肥胖风险中环境驱动差异的可行性，提供了一个可扩展的数据驱动多层面建模流程，适用于公共卫生信息学，具有扩展到因果建模、干预规划和实时分析的潜力。
...
### [204] [Understanding the Mechanisms of Fast Hyperparameter Transfer](https://arxiv.org/abs/2512.22768)
*Nikhil Ghosh,Denny Wu,Alberto Bietti*

Main category: cs.LG

TL;DR: 该论文研究了深度学习模型规模扩大时超参数优化的计算效率问题，提出了跨规模超参数转移的概念框架，分析了μP参数化下快速转移的条件和机制。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模不断增长，使得标准的超参数优化变得极其昂贵。需要寻找能够在不同规模模型间有效转移超参数的解决方案，以提高计算效率。

Method: 提出了一个通用的概念框架来分析跨规模超参数转移，定义了"快速转移"的概念，并证明在计算最优网格搜索中快速转移等价于有用转移。通过分析μP参数化下的转移机制，提出了优化轨迹分解假设。

Result: 研究表明快速转移取决于问题结构，在某些设置下能提供可证明的计算优势，而在其他设置下即使使用μP也无法超越直接调优。提出了宽度稳定和宽度敏感组件的分解假设，并在包括大语言模型预训练在内的多种设置中提供了经验证据。

Conclusion: 跨规模超参数转移是提高大规模深度学习模型调优计算效率的有前景方法，但其有效性取决于问题结构。μP参数化下的快速转移机制可以通过优化轨迹分解来解释，这为设计更有效的超参数转移策略提供了理论基础。
...
TL;DR: Schrödinger AI是一个受量子力学启发的统一机器学习框架，通过波函数和哈密顿量模拟认知过程，提供可解释的语义表示和动态推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法如交叉熵训练和Transformer注意力机制存在局限性，作者希望建立一个基于物理原理的替代框架，实现更好的泛化能力、可解释语义和动态推理。

Method: 框架包含三个紧密耦合的组件：1）时间无关波-能量求解器，将感知和分类视为学习哈密顿量下的谱分解；2）时间相关动力学求解器，控制语义波函数随时间演化，实现上下文感知的决策修订和推理；3）低秩算子演算，通过学习量子类转移算子实现符号变换。

Result: 实验表明：1）无需显式监督即可涌现反映人类概念关系的语义流形；2）能够适应变化环境的动态推理，包括实时势场扰动的迷宫导航；3）在模算术任务上实现精确的算子泛化，学习群作用并在远超训练长度的序列上组合它们。

Conclusion: 该研究为机器学习提供了一个新的基础方向，将学习视为发现和导航底层语义能量景观的过程，展示了物理驱动方法在泛化、可解释性和动态适应方面的潜力。
...
### [206] [Adapting, Fast and Slow: Transportable Circuits for Few-Shot Learning](https://arxiv.org/abs/2512.22777)
*Kasra Jalaldoust,Elias Bareinboim*

Main category: cs.LG

TL;DR: 提出基于因果可迁移理论的零样本组合泛化算法Circuit-TR，通过模块化学习和因果图指导实现跨域预测


<details>
  <summary>Details</summary>
Motivation: 解决跨域泛化问题，传统方法难以在未见目标域上有效工作，需要利用因果结构约束来确保泛化能力

Method: 设计Circuit-TR算法：1) 从源数据学习模块集合（局部预测器）；2) 在因果结构许可下，通过迁移/组合模块构建目标域预测电路；3) 提出无需显式因果结构的监督域适应方案

Result: 理论分析：基于图论电路可迁移准则刻画了少样本可学习任务类别，将少样本泛化能力与电路规模复杂度联系起来；仿真实验验证了理论结果

Conclusion: 通过因果可迁移理论和模块化方法，实现了有效的零样本组合泛化，为跨域学习提供了理论框架和实用算法
...
### [207] [Discovering Transmission Dynamics of COVID-19 in China](https://arxiv.org/abs/2512.22787)
*Zhou Yang,Edward Dougherty,Chen Zhang,Zhenhe Pan,Fang Jin*

Main category: cs.LG

TL;DR: 该研究通过分析中国COVID-19病例追踪数据，发现大规模检测、隔离和接触者追踪等公共卫生干预措施在疫情控制中发挥重要作用，揭示了感染源随时间从湖北旅行相关转向社交活动相关的变化趋势。


<details>
  <summary>Details</summary>
Motivation: 通过回顾性分析公共卫生干预措施（如大规模检测、隔离和接触者追踪），识别最有效的COVID-19防控机制，了解中国SARS-CoV-2传播模式及其时空动态。

Method: 收集地方卫健委、中国CDC和官方社交媒体发布的病例报告，应用自然语言处理和人工整理构建传播/追踪链，结合武汉人口流动数据量化并可视化时空传播动态。

Result: 结果显示显著的区域差异，大城市感染更多（可能由社交活动驱动）；79%有症状者在症状出现5天内住院；确诊接触者入院时间更短；感染源随时间变化：早期主要与湖北旅行相关，后期更多与社交活动相关。

Conclusion: 公共卫生干预措施对控制COVID-19传播至关重要，感染源模式的变化表明需要针对不同传播阶段调整防控策略，特别是对社交活动相关的传播加强管理。
...
### [208] [ReDiF: Reinforced Distillation for Few Step Diffusion](https://arxiv.org/abs/2512.22802)
*Amirhossein Tighkhorshid,Zahra Dehghanian,Gholamali Aminian,Chengchun Shi,Hamid R. Rabiee*

Main category: cs.LG

TL;DR: 提出基于强化学习的扩散模型蒸馏框架，将蒸馏过程视为策略优化问题，通过奖励信号动态指导学生模型，实现更少推理步骤下的高性能生成


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型采样速度慢的问题，传统蒸馏方法依赖固定的重建或一致性损失，缺乏动态优化能力，需要更高效的蒸馏框架

Method: 将扩散模型蒸馏视为强化学习策略优化问题，学生模型作为策略，使用教师模型输出对齐的奖励信号进行训练，允许探索多种去噪路径并采取更长的优化步骤

Result: 实验结果显示该方法在显著减少推理步骤和计算资源的情况下，性能优于现有蒸馏技术，且框架具有模型无关性

Conclusion: 基于强化学习的蒸馏框架为扩散模型提供了通用的优化范式，能够有效加速推理过程，同时保持生成质量
...
### [209] [MoR: Mixture Of Representations For Mixed-Precision Training](https://arxiv.org/abs/2512.22804)
*Bor-Yiing Su,Peter Dykas,Mike Chrzanowski,Jatin Chhugani*

Main category: cs.LG

TL;DR: MoR是一种新型的混合精度训练框架，通过动态分析张量数值特性，在FP8和BF16表示之间进行选择，实现98.38%张量量化为FP8格式，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 混合精度训练是扩展深度学习模型的关键技术，但成功实施需要找到正确的训练方法组合。现有方法需要精细划分或无法充分利用低精度格式的潜力。

Method: 提出混合表示(MoR)框架，在张量和子张量级别进行量化，动态分析张量数值特性，在FP8和BF16表示之间选择。设计了通用方法，支持各种量化分区策略和数据集。

Result: 初步结果显示，该方法实现了98.38%的张量量化为FP8格式，达到了最先进的结果。FP8精度与现有方法相当，无需精细分区，也可与其他训练方法结合使用NVFP4等更低精度格式。

Conclusion: MoR框架展示了动态、特性感知量化的潜力，能够在保持模型质量的同时提高低精度训练的鲁棒性，为充分利用更低精度格式提供了通用方法。
...
### [210] [Long-Range Distillation: Distilling 10,000 Years of Simulated Climate into Long Timestep AI Weather Models](https://arxiv.org/abs/2512.22814)
*Scott A. Martin,Noah Brenowitz,Dale Durran,Michael Pritchard*

Main category: cs.LG

TL;DR: 本文提出了一种名为"长程蒸馏"的方法，通过使用自回归教师模型生成大量合成气候数据，训练单步长时步概率学生模型，以解决AI天气模型在长期预报中的过拟合和累积误差问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI天气模型在长期预报（次季节到季节尺度）面临两大挑战：1）自回归模型在多次迭代中误差累积导致不稳定和校准问题；2）再分析数据集仅有40年记录，样本量不足以捕捉气候变率的慢模态，导致长时步概率模型容易过拟合。

Method: 提出长程蒸馏方法：使用短时步自回归"教师"模型（DLESyM）生成超过10,000年的合成气候数据，然后用这些数据训练长时步概率"学生"模型，使其能够单步生成长期预报，替代数百次自回归步骤。

Result: 在完美模型实验中，蒸馏模型优于气候学基准，接近自回归教师模型的技能；在真实世界中，经过ERA5微调后，其S2S预报技能与ECMWF集合预报相当。模型技能随合成训练数据量增加而提升，即使数据量远超ERA5。

Conclusion: 长程蒸馏方法首次证明AI生成的合成训练数据可以扩展长期预报技能，为解决AI天气模型长期预报中的数据稀缺和过拟合问题提供了有效途径。
...
### [211] [TEACH: Temporal Variance-Driven Curriculum for Reinforcement Learning](https://arxiv.org/abs/2512.22824)
*Gaurav Chaudhary,Laxmidhar Behera*

Main category: cs.LG

TL;DR: 提出了一种基于学生-教师学习范式和时序方差驱动课程的自适应目标选择方法，用于加速目标条件强化学习，在11个机器人操作和迷宫导航任务中表现出显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在单目标任务中表现良好，但在多目标设置中，均匀目标选择导致样本效率低下。受生物系统自适应结构化学习过程启发，需要开发更高效的目标条件强化学习方法。

Method: 提出学生-教师学习范式，教师模块基于策略置信度（状态-动作值函数Q）的时序方差动态优先选择高不确定性目标，提供自适应聚焦学习信号。该方法与算法无关，可无缝集成到现有RL框架。

Result: 在11个多样化机器人操作和迷宫导航任务中进行评估，相比最先进的课程学习和目标选择方法，表现出持续且显著的性能改进。

Conclusion: 通过教师模块基于时序方差的自适应目标选择，能够加速目标条件强化学习，提高样本效率，为多目标RL任务提供了有效的解决方案。
...
### [212] [Federated Multi-Task Clustering](https://arxiv.org/abs/2512.22897)
*S. Dai,G. Sun,F. Li,X. Tang,Q. Wang,Y. Cong*

Main category: cs.LG

TL;DR: 提出联邦多任务聚类框架FMTC，解决传统谱聚类无法适应去中心化环境的问题，通过个性化客户端模型和服务器端张量相关性模块，在保护隐私的同时提升异构客户端的聚类性能。
...
Method: 提出FMTC框架，包含两个核心组件：客户端个性化聚类模块学习参数化映射模型，支持鲁棒的样本外推理，避免使用不可靠的伪标签；服务器端张量相关性模块将所有客户端模型组织成统一张量，应用低秩正则化发现其公共子空间。采用基于ADMM的高效隐私保护分布式算法进行优化。
...
### [213] [MetaCD: A Meta Learning Framework for Cognitive Diagnosis based on Continual Learning](https://arxiv.org/abs/2512.22904)
*Jin Wu,Chanjin Zheng*

Main category: cs.LG

TL;DR: 提出基于持续学习的元学习认知诊断框架MetaCD，解决长尾分布和动态变化问题，通过元学习优化初始化和参数保护机制提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有认知诊断方法在长尾数据分布和动态变化方面表现受限，需要能够处理数据不平衡和适应新技能/任务的解决方案。

Method: 提出MetaCD框架：1）使用元学习学习最优初始化状态，解决长尾问题；2）采用参数保护机制的持续学习方法，适应新技能/任务；3）平衡单任务可塑性和序列任务的稳定性与泛化性。

Result: 在五个真实数据集上的实验表明，MetaCD在准确率和泛化能力方面均优于其他基线方法。

Conclusion: MetaCD框架有效解决了认知诊断中的长尾分布和动态变化问题，通过元学习和持续学习的结合提升了模型的性能、适应性和泛化能力。
...
### [214] [Multiple Token Divergence: Measuring and Steering In-Context Computation Density](https://arxiv.org/abs/2512.22944)
...
<details>
  <summary>Details</summary>
Motivation: 现有衡量语言模型计算努力的方法存在不足：下一令牌损失无法捕捉推理复杂度，基于潜在状态可压缩性的方法具有侵入性和不稳定性。需要一种简单有效的指标来测量模型的计算努力。

Method: 提出MTD（多令牌散度），定义为模型完整输出分布与浅层辅助预测头输出分布之间的KL散度。该方法可直接从具有多个预测头的预训练模型计算，无需额外训练。基于MTD进一步提出Divergence Steering解码方法，用于控制生成文本的计算特性。

Result: MTD比现有方法更有效地区分复杂任务与简单任务；在数学推理基准测试中，MTD与问题难度呈正相关；较低的MTD与更准确的推理相关；MTD为分析和控制语言模型计算动态提供了实用轻量级工具。

Conclusion: MTD是一种简单有效的语言模型计算努力测量方法，无需额外训练即可从预训练模型计算，为分析和控制语言模型的计算特性提供了实用工具，Divergence Steering方法进一步扩展了其应用价值。
...
### [215] [APO: Alpha-Divergence Preference Optimization](https://arxiv.org/abs/2512.22953)
*Wang Zixian*

Main category: cs.LG

TL;DR: APO提出了一种基于α-散度的锚定对齐框架，可在前向KL和后向KL之间连续插值，通过奖励和置信度引导的α调度策略实现从覆盖到利用的平稳过渡。


<details>
  <summary>Details</summary>
Motivation: 现代对齐实践中存在两种主导的散度机制：前向KL散度（监督微调和蒸馏目标）提供稳定的模式覆盖但可能未充分利用高奖励模式；后向KL散度（PPO风格在线强化学习）支持模式寻求改进但存在模式崩溃风险。现有锚定方法通常只采用单一散度，需要一种能连续调节散度行为的统一框架。

Method: 提出Alpha-Divergence Preference Optimization (APO)，基于Csiszar α-散度的锚定框架，在相同锚定几何中连续插值前向和后向KL行为。推导了参数化α的统一梯度动态，分析梯度方差特性，并提出实用的奖励和置信度引导的α调度策略，仅在策略改进且置信度校准良好时才从覆盖过渡到利用。

Result: 在Qwen3-1.7B模型和math-level3任务上的实验表明，APO在保持训练稳定性的同时，与GRPO和GSPO基线相比取得了竞争性性能。

Conclusion: APO提供了一个统一的锚定框架，通过α散度连续调节前向和后向KL行为，结合奖励和置信度引导的调度策略，在保持训练稳定性的同时实现竞争性性能，为对齐实践提供了更灵活和鲁棒的解决方案。
...
### [216] [FLOW: A Feedback-Driven Synthetic Longitudinal Dataset of Work and Wellbeing](https://arxiv.org/abs/2512.22956)
*Wafaa El Husseini*

Main category: cs.LG

TL;DR: FLOW是一个合成纵向数据集，模拟工作负荷、生活方式与幸福感之间的日常互动，用于解决真实数据获取受限的问题，支持可重复研究、方法基准测试和教育应用。


<details>
  <summary>Details</summary>
Motivation: 工作生活平衡和幸福感研究的纵向个体数据因隐私、伦理和后勤限制难以获取，这给压力建模、行为分析和机器学习等领域的可重复研究、方法基准测试和教育带来了挑战。

Method: 使用基于规则的反馈驱动模拟生成合成纵向数据集，模拟压力、睡眠、情绪、身体活动和体重等变量之间的连贯时间动态，包含1000名个体两年内的每日数据。

Result: 创建了FLOW数据集及其可配置的数据生成工具，提供了一个公开可用的资源，支持在可调整的行为和情境假设下进行可重复实验。

Conclusion: FLOW作为一个受控实验环境而非真实人群的替代品，支持在无法获取真实世界数据的情况下进行探索性分析、方法开发和基准测试。
...
### [217] [A Context-Aware Temporal Modeling through Unified Multi-Scale Temporal Encoding and Hierarchical Sequence Learning for Single-Channel EEG Sleep Staging](https://arxiv.org/abs/2512.22976)
...
<details>
  <summary>Details</summary>
Motivation: 睡眠障碍全球普遍，自动睡眠分期对医疗保健至关重要。现有单通道EEG方法面临类别不平衡、感受野建模有限、可解释性不足等挑战，特别是N1期检测困难，且现有模型多为缺乏明确特征提取角色的黑盒

Method: 结合紧凑多尺度特征提取与时间建模来捕获局部和长程依赖；使用类别加权损失函数和数据增强解决数据不平衡；将EEG信号分段为子时期块，通过平均各块的softmax概率获得最终预测以增强上下文表示和鲁棒性

Result: 整体准确率89.72%，宏平均F1分数85.46%，特别在具有挑战性的N1期达到61.7%的F1分数，相比先前方法在SleepEDF数据集上有显著提升

Conclusion: 该方法有效提升了睡眠分期性能，同时保持了可解释性和对真实世界临床应用的适用性
...
### [218] [Fusion or Confusion? Multimodal Complexity Is Not All You Need](https://arxiv.org/abs/2512.22991)
*Tillmann Rheude,Roland Eils,Benjamin Wild*

Main category: cs.LG

TL;DR: 复杂多模态学习方法并不比简单基线方法表现更好，在标准化实验条件下，简单的后期融合Transformer架构与复杂方法表现相当甚至更优。


<details>
  <summary>Details</summary>
Motivation: 挑战当前多模态学习领域的一个普遍假设：更复杂的多模态特定方法会带来更好的性能。通过大规模实证研究检验这一假设，揭示方法学严谨性的重要性。

Method: 对19种高影响力多模态学习方法进行标准化重新实现，在9个不同数据集（最多23种模态）上评估，测试其泛化能力（包括缺失模态情况）。提出SimBaMM（简单多模态学习基线）- 一个简单的后期融合Transformer架构。

Result: 在标准化实验条件下，复杂架构并不比SimBaMM更可靠地表现出优越性。统计分析表明复杂方法与SimBaMM表现相当，且经常无法可靠地超越经过良好调优的单模态基线，特别是在小数据场景下。

Conclusion: 多模态学习领域需要转变关注重点：从追求架构新颖性转向方法学严谨性。提供实用的可靠性检查清单以促进未来可比较、稳健和可信的评估。
...
### [219] [Merge before Forget: A Single LoRA Continual Learning via Continual Merging](https://arxiv.org/abs/2512.23017)
*Fuli Qiao,Mehrdad Mahdavi*

Main category: cs.LG

TL;DR: 提出一种基于正交初始化和顺序合并的LoRA持续学习方法，通过正交基提取和时间感知缩放机制，在保持恒定内存复杂度的同时减少任务间干扰。
...
Method: 采用正交初始化策略，从已学习LoRA中提取正交基来初始化新任务学习；利用LoRA组件内在不对称性，通过时间感知缩放机制在持续合并过程中平衡新旧知识；将多个LoRA更新顺序合并为单个统一LoRA。

Result: 方法在多个持续学习基准测试和不同Llama模型上表现出有效性，保持恒定内存复杂度，最小化任务间干扰，并通过自适应缩放提升性能。

Conclusion: 提出的正交初始化和顺序合并LoRA方法为大规模语言模型提供了一种高效、内存友好的持续学习解决方案，通过理论分析和实验验证了其优越性。
...
### [220] [PI-MFM: Physics-informed multimodal foundation model for solving partial differential equations](https://arxiv.org/abs/2512.23056)
*Min Zhu,Jingmin Sun,Zecheng Zhang,Hayden Schaeffer,Lu Lu*

Main category: cs.LG

TL;DR: PI-MFM是一个物理信息多模态基础模型框架，通过在预训练和适应阶段直接强制执行控制方程，实现数据高效、可迁移的PDE求解器。


<details>
  <summary>Details</summary>
Motivation: 现有多算子学习方法数据需求大且训练时忽略物理约束，需要开发既能利用多模态基础模型优势又能融入物理先验的框架。

Method: 提出PI-MFM框架，以PDE符号表示为输入，通过向量化导数计算自动组装PDE残差损失，使任何PDE编码多模态基础模型都能用统一的物理信息目标进行训练。

Result: 在13个参数化一维时间相关PDE基准测试中，PI-MFM始终优于纯数据驱动方法，尤其在稀疏标记时空点、部分观测时间域或少量标记函数对情况下表现更好。物理损失提高了抗噪鲁棒性，重采样配置点策略显著提升精度。

Conclusion: PI-MFM为零样本物理信息微调提供了实用可扩展路径，仅使用PDE残差和初始/边界条件（无需标记解数据）就能快速降低测试误差至约1%，明显优于从头开始的纯物理训练。
...
### [221] [Trust Region Masking for Long-Horizon LLM Reinforcement Learning](https://arxiv.org/abs/2512.23075)
*Yingru Li,Jiacai Liu,Jiawei Xu,Yuxuan Tong,Ziniu Li,Baoxiang Wang*

Main category: cs.LG

TL;DR: 该论文针对大语言模型强化学习中策略梯度方法的离策略近似误差问题，提出了更紧的误差界和信任区域掩码方法，为长序列任务提供非空单调改进保证。


<details>
  <summary>Details</summary>
Motivation: 现代LLM-RL中由于实现差异、专家混合路由不连续性和分布式训练延迟等原因，离策略不匹配不可避免。经典信任区域误差界随序列长度T呈O(T²)增长，对于长序列任务变得空洞无意义。

Method: 推导了两个更紧的误差界：Pinsker-Marginal界（O(T^{3/2})）和Mixed界（O(T)），两者都依赖于序列级量D_{kl}^{tok,max}。提出了信任区域掩码（TRM）方法，当序列中任何令牌违反信任区域时，排除整个序列的梯度计算。

Result: 新误差界显著优于经典O(T²)界，TRM方法为长序列LLM-RL提供了第一个非空的单调改进保证。

Conclusion: 通过序列级信任区域控制，TRM方法解决了长序列LLM-RL中的离策略近似误差问题，为实际应用提供了理论保证。
...
### [222] [Multimodal Functional Maximum Correlation for Emotion Recognition](https://arxiv.org/abs/2512.23076)
*Deyang Zheng,Tianyi Zhang,Wenming Zheng,Shujian Yu*

Main category: cs.LG

TL;DR: MFMC提出了一种基于双总相关性的多模态自监督学习框架，通过最大化高阶多模态依赖关系来捕捉情感状态中脑与自主神经系统的协调响应，在多个情感计算基准测试中取得了最先进或具有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 情感状态表现为中枢和自主神经系统之间协调但异质的生理反应，这对情感计算中的多模态表示学习构成了根本性挑战。现有自监督学习方法主要依赖成对对齐目标，无法有效捕捉两个以上模态之间的依赖关系，也无法捕获脑与自主神经系统协调响应产生的高阶交互作用。

Method: 提出了多模态功能最大相关（MFMC）框架，通过双总相关（DTC）目标最大化高阶多模态依赖关系。该方法推导了一个紧密的三明治界，并使用基于功能最大相关分析（FMCA）的迹替代进行优化，直接捕捉联合多模态交互，而不依赖成对对比损失。

Result: 在三个公共情感计算基准测试中，MFMC在受试者依赖和受试者独立评估协议下均取得最先进或具有竞争力的性能。在CEAP-360VR数据集上，MFMC将受试者依赖准确率从78.9%提升至86.8%，仅使用EDA信号就将受试者独立准确率从27.5%提升至33.1%。在MAHNOB-HCI最具挑战性的EEG受试者独立划分中，MFMC与最佳方法相差不到0.8个百分点。

Conclusion: MFMC通过最大化高阶多模态依赖关系，有效解决了情感计算中多模态表示学习的挑战，特别是在处理脑与自主神经系统协调响应方面表现出色。该方法对受试者间变异性具有鲁棒性，为情感状态的多模态分析提供了新的自监督学习框架。
...
### [223] [Taming the Tail: Stable LLM Reinforcement Learning via Dynamic Vocabulary Pruning](https://arxiv.org/abs/2512.23087)
*Yingru Li,Jiawei Xu,Jiacai Liu,Yuxuan Tong,Ziniu Li,Tianle Cai,Ge Zhang,Qian Liu,Baoxiang Wang*

Main category: cs.LG

TL;DR: 该论文针对大语言模型强化学习中训练-推理不匹配问题，提出通过动态剪枝"安全"词汇表来排除极端低概率词元，从而用小的有界优化偏差替代大的系统性偏差。
...
Method: 提出将RL目标约束到动态剪枝的"安全"词汇表，排除极端尾部的低概率词元，用小的有界优化偏差替代大的系统性偏差，并理论分析了词汇剪枝引入的优化偏差界限。

Result: 该方法实现了稳定的训练，同时理论上界定了词汇剪枝引入的优化偏差。

Conclusion: 通过动态剪枝词汇表排除极端低概率词元，可以有效解决训练-推理不匹配问题，实现稳定的大语言模型强化学习训练。
...
### [224] [A Note on Hybrid Online Reinforcement and Imitation Learning for LLMs: Formulations and Algorithms](https://arxiv.org/abs/2512.23097)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决LLM微调中模仿学习和强化学习的整合问题，提供更高效的优化方法

Method: 通过分析结合轨迹级KL散度和任务奖励的复合目标梯度，分解为可计算密集梯度和稀疏梯度两部分

Result: 密集梯度具有闭式logit级公式，支持高效GPU实现；稀疏梯度通过蒙特卡洛估计优化长期奖励

Conclusion: 该框架统一了模仿学习和强化学习，为LLM微调提供了高效可扩展的解决方案
...
### [225] [How Much Data Is Enough? Uniform Convergence Bounds for Generative & Vision-Language Models under Low-Dimensional Structure](https://arxiv.org/abs/2512.23109)
*Paul M. Thompson*
...
<details>
  <summary>Details</summary>
Motivation: 在生物医学决策支持中，生成模型和视觉语言模型需要提供准确且校准良好的概率预测。当前模型在中等数据量下表现良好，但不确定其预测是否能在所有输入、类别和子群体中均匀泛化，而非仅平均表现良好。这在生物医学中至关重要，因为罕见病症和特定群体即使在总体损失较低时也可能出现较大误差。

Method: 从有限样本角度研究问题，关注通过改变提示或语义嵌入在受限表示空间中获得的分类器族。假设模型输出对低维语义表示具有平滑依赖性（这一假设得到文本和联合图像-文本嵌入中谱结构的支持），应用经典均匀收敛工具获得非渐近保证。主要结果为VLM诱导分类器的准确性和校准泛函提供有限样本均匀收敛边界，基于提示嵌入的Lipschitz稳定性。

Result: 推导出有限样本均匀收敛边界，表明样本复杂度取决于内在/有效维度而非环境嵌入维度。进一步推导出谱依赖边界，明确展示特征值衰减如何控制数据需求。这些结果为数据受限的生物医学建模提供理论保证。

Conclusion: 研究揭示了在什么结构假设下，生成模型和VLM能够在实际样本量下实现均匀准确和校准的行为。结果对数据受限的生物医学建模具有重要意义，包括确定当前数据集大小何时能支持均匀可靠的预测，以及解释为什么平均校准指标可能遗漏最坏情况下的校准错误。
...
### [226] [SE-MLP Model for Predicting Prior Acceleration Features in Penetration Signals](https://arxiv.org/abs/2512.23131)
*Yankang Li,Changsheng Li*

Main category: cs.LG

TL;DR: 本文提出了一种结合通道注意力机制和残差连接的多层感知机架构（SE-MLP），用于快速预测侵彻加速度特征值，解决了传统仿真计算耗时耗资源的问题。
...
Result: 与常规MLP、XGBoost和Transformer模型相比，SE-MLP在预测精度、泛化能力和稳定性方面表现更优；消融实验证实通道注意力模块和残差结构对性能提升均有显著贡献；数值模拟和射程恢复测试显示预测值与实测值的偏差在工程允许范围内。
...
### [227] [A Weak Signal Learning Dataset and Its Baseline Method](https://arxiv.org/abs/2512.23160)
*Xianqi Liu,Xiangru Li,Lefeng He,Ziyu Fang*

Main category: cs.LG

TL;DR: 该研究构建了首个弱信号特征学习专用数据集，包含13,158个光谱样本，具有低信噪比和极端类别不平衡特性，并提出了PDVFN模型来处理这些挑战。


<details>
  <summary>Details</summary>
Motivation: 弱信号学习在许多领域都是重要挑战，但缺乏专用数据集限制了研究进展。关键信息常被噪声和干扰掩盖，特征识别困难，即使在有强信号的任务中，提升模型性能的关键也在于有效提取弱信号。

Method: 构建了首个弱信号特征学习专用数据集，包含13,158个光谱样本，具有低信噪比主导（超过55%样本SNR低于50）和极端类别不平衡（类别比例达29:1）。提出了PDVFN模型，采用双视图表示（向量+时频图），并行提取局部序列特征和全局频域结构，遵循局部增强、序列建模、噪声抑制、多尺度捕获、频率提取和全局感知原则。
...
### [228] [Diffusion-based Decentralized Federated Multi-Task Representation Learning](https://arxiv.org/abs/2512.23161)
...
<details>
  <summary>Details</summary>
Motivation: 尽管表示学习在数据稀缺环境中被广泛采用，但分散式方法仍相对未被充分探索。现有研究主要集中在集中式表示学习，而分散式方法在隐私保护、计算效率等方面具有优势。

Method: 开发了一种基于分散式投影梯度下降的算法，采用交替投影梯度下降和最小化方法，以基于扩散的分散式和联邦式方式恢复低秩特征矩阵。

Result: 获得了构造性的可证明保证，包括所需样本复杂度的下界和迭代复杂度的上界。算法在时间和通信复杂度方面表现出色，具有快速和通信高效的特点。

Conclusion: 数值模拟验证了所提算法的性能，并与基准算法进行了比较，表明该算法在分散式多任务表示学习中是有效且高效的。
...
### [229] [Evaluating Parameter Efficient Methods for RLVR](https://arxiv.org/abs/2512.23165)
*Qingyu Yin,Yulun Wu,Zhennan Shen,Sunbowen Li,Zhilin Wang,Yanshu Li,Chak Tou Leong,Jiale Kang,Jinjin Gu*

Main category: cs.LG

TL;DR: 本文首次系统评估了12种参数高效微调方法在可验证奖励强化学习范式下的表现，发现结构变体方法优于标准LoRA，SVD初始化策略存在频谱崩溃问题，极端参数缩减会严重限制推理能力。


<details>
  <summary>Details</summary>
Motivation: 在可验证奖励强化学习范式下，虽然LoRA等方法被广泛使用，但最适合RLVR的参数高效微调架构仍未确定。本研究旨在填补这一空白，为参数高效的强化学习方法提供指导。

Method: 在DeepSeek-R1-Distill系列模型上，对超过12种参数高效微调方法进行了全面评估，包括DoRA、AdaLoRA、MiSS、PiSSA、MiLoRA、VeRA、Rank-1等，并在数学推理基准上进行了实验。

Result: 1. 结构变体方法（DoRA、AdaLoRA、MiSS）一致优于标准LoRA；2. 发现SVD初始化策略存在频谱崩溃现象，主成分更新与RL优化存在根本性不匹配；3. 极端参数缩减方法严重限制推理能力；4. 通过消融实验和扩展实验验证了这些发现。

Conclusion: 本研究挑战了默认采用标准LoRA的做法，为参数高效强化学习方法的选择提供了明确指导，并呼吁在这一领域进行更多探索。
...
### [230] [Machine Learning-Assisted Vocal Cord Ultrasound Examination: Project VIPR](https://arxiv.org/abs/2512.23177)
...
Conclusion: 机器学习辅助的声带超声分析在提高诊断准确性方面显示出巨大潜力，有望超越依赖操作者经验的人工判读
...
### [231] [A Simple, Optimal and Efficient Algorithm for Online Exp-Concave Optimization](https://arxiv.org/abs/2512.23190)
*Yi-Han Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: LightONS算法通过延迟昂贵的马氏投影，将OXO问题的总运行时间从O(d^ωT)降低到O(d^2T + d^ω√TlogT)，同时保持最优的O(d log T)遗憾界，并解决了COLT'13关于SXO算法运行时间的开放问题。


<details>
  <summary>Details</summary>
Motivation: 在线指数凹优化(OXO)中，标准算法ONS存在计算瓶颈，每个回合的马氏投影需要Ω(d^ω)次算术运算，导致总运行时间高达O(d^ωT)。对于随机指数凹优化(SXO)，使用在线到批处理转换的ONS需要O(d^{ω+1}/ε)的运行时间，这促使Koren在COLT'13提出了寻找运行时间更短的SXO算法的开放问题。

Method: 提出了LightONS算法，这是ONS的一个简单变体。它利用参数无关在线学习中的域转换技术，引入滞后机制，延迟昂贵的马氏投影直到必要时才执行。这种方法保留了ONS的优雅结构，同时显著降低了计算成本。

Result: LightONS将总运行时间降低到O(d^2T + d^ω√TlogT)，同时保持最优的O(d log T)遗憾界。对于SXO问题，它实现了O(d^3/ε)的运行时间，从而解决了COLT'13的开放问题。此外，LightONS可以作为ONS的高效替代品，应用于更广泛的场景。

Conclusion: LightONS算法通过创新的滞后机制成功解决了OXO和SXO中的计算瓶颈问题，在保持统计最优性的同时显著降低了计算复杂度，为在线学习领域提供了一个高效实用的解决方案。
...
### [232] [PGOT: A Physics-Geometry Operator Transformer for Complex PDEs](https://arxiv.org/abs/2512.23192)
*Zhuo Zhang,Xi Yang,Yuan Zhao,Canqun Yang*

Main category: cs.LG

TL;DR: PGOT通过显式几何感知重建物理特征学习，解决Transformer建模大规模非结构化网格时的几何混叠问题，在四个基准测试和工业任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: Transformer在建模PDEs方面潜力巨大，但建模具有复杂几何形状的大规模非结构化网格仍面临挑战。现有高效架构采用特征降维策略会导致几何混叠，丢失关键的物理边界信息。

Method: 提出物理-几何算子Transformer（PGOT），包含频谱保持几何注意力模块，采用"物理切片-几何注入"机制融入多尺度几何编码，保持线性计算复杂度。PGOT根据空间坐标动态路由计算到低阶线性路径（平滑区域）和高阶非线性路径（激波和不连续性），实现空间自适应建模。

Result: PGOT在四个标准基准测试中取得一致的SOTA性能，并在翼型和汽车设计等大规模工业任务中表现优异。

Conclusion: PGOT通过显式几何感知有效解决了Transformer建模大规模非结构化网格时的几何混叠问题，实现了空间自适应的高精度物理场建模，在学术基准和工业应用中均表现出色。
...
### [233] [Energy and Memory-Efficient Federated Learning With Ordered Layer Freezing](https://arxiv.org/abs/2512.23200)
*Ziru Niu,Hai Dong,A. K. Qin,Tao Gu,Pengcheng Zhang*

Main category: cs.LG

TL;DR: FedOLF通过有序层冻结和Tensor Operation Approximation技术，在联邦学习中显著降低了计算、内存和通信开销，同时保持或提高了模型精度。


<details>
  <summary>Details</summary>
Motivation: 物联网边缘设备的计算能力、内存和带宽有限，给联邦学习带来了效率和可扩展性挑战。现有方法如dropout或层冻结往往牺牲精度或忽视内存约束。

Method: 提出FedOLF框架：1）在训练前按预定顺序持续冻结层，减少计算和内存需求；2）引入Tensor Operation Approximation作为轻量级量化替代方案，降低通信和能耗。

Result: 在非独立同分布数据上，FedOLF在多个数据集和模型上均取得更高精度（提升0.3%到6.27%不等），同时具有更高的能效和更低的内存占用。

Conclusion: FedOLF通过有序层冻结和TOA技术，有效解决了联邦学习在资源受限边缘设备上的效率问题，在保持精度的同时显著降低了资源消耗。
...
### [234] [FairGFL: Privacy-Preserving Fairness-Aware Federated Learning with Overlapping Subgraphs](https://arxiv.org/abs/2512.23235)
*Zihao Zhou,Shusen Yang,Fangyuan Zhao,Xuebin Ren*

Main category: cs.LG

TL;DR: 本文提出FairGFL算法，解决图联邦学习中由于子图重叠不平衡导致的公平性问题，在保持隐私的同时提升跨客户端公平性和模型性能。


<details>
  <summary>Details</summary>
Motivation: 图联邦学习虽然能保护隐私地协作提取分布式子图的高阶信息，但不同客户端间的图数据往往存在重叠。先前研究显示重叠数据有助于缓解数据异质性，但未探讨其负面影响，特别是在重叠分布不平衡的情况下。本文通过实证观察和理论分析揭示了不平衡重叠子图导致的公平性问题。
...
Conclusion: FairGFL成功解决了图联邦学习中由不平衡重叠子图引起的公平性问题，在保护隐私的同时实现了更好的模型效用与公平性平衡。
...
### [235] [KernelEvolve: Scaling Agentic Kernel Coding for Heterogeneous AI Accelerators at Meta](https://arxiv.org/abs/2512.23236)
*Gang Liao,Hongsen Qin,Ying Wang,Alicia Golden,Michael Kuchnik,Yavuz Yetim,Jia Jiunn Ang,Chunli Fu,Yihan He,Samuel Hsia,Zewei Jiang,Dianshi Li,Uladzimir Pashkevich,Varna Puvvada,Feng Shi,Matt Steiner,Ruichao Xiao,Nathan Yan,Xiayu Yu,Zhou Fang,Abdul Zainul-Abedin,Ketan Singh,Hongtao Yu,Wenyuan Chi,Barney Huang,Sean Zhang,Noah Weller,Zach Marine,Wyatt Cook,Carole-Jean Wu,Gaoxiang Liu*

Main category: cs.LG

TL;DR: KernelEvolve是一个面向DLRM的自动化内核生成与优化框架，通过多级编程抽象和基于图的搜索策略，解决推荐模型训练推理中的异构硬件挑战，显著提升性能并降低开发时间。


<details>
  <summary>Details</summary>
Motivation: 深度学习推荐模型（DLRM）训练和推理需要高效快速，但面临三大系统挑战：模型架构多样性、内核原语多样性、硬件代际和架构异构性。现有方法难以在异构硬件上实现规模化优化。

Method: 提出KernelEvolve框架，采用多级编程抽象（从Triton/CuTe DSL到低级硬件无关语言），通过基于图的搜索策略（包含选择策略、通用算子、适应度函数和终止规则），并利用检索增强提示合成动态适应运行时执行上下文。

Result: 在KernelBench套件上实现100%通过率（250个问题，三个难度级别），在三个异构硬件平台上验证160个PyTorch ATen算子，实现100%正确性。将开发时间从数周缩短到数小时，在多样化生产用例中显著超越PyTorch基线性能。

Conclusion: KernelEvolve有效解决了DLRM在异构硬件上的规模化优化挑战，不仅大幅提升性能效率，还通过为内部开发的AI硬件提供自动化内核生成，显著降低了新AI硬件的可编程性障碍。
...
### [236] [PFed-Signal: An ADR Prediction Model based on Federated Learning](https://arxiv.org/abs/2512.23262)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于FAERS数据的药物不良反应预测存在数据偏差问题，现有基于统计方法（如ROR、PRR）的解决方案无法完全消除偏差数据，导致信号预测不准确

Method: 提出PFed-signal框架，包含两个核心组件：1) Pfed-Split方法将原始数据集按ADR分割；2) ADR-signal模型包括基于联邦学习的偏差数据识别方法（使用欧氏距离）和基于Transformer的ADR预测模型

Result: 在清洗后的数据集上，ROR和PRR指标优于传统方法；PFed-Signal模型的准确率0.887、F1分数0.890、召回率0.913、AUC 0.957，均优于基线模型
...
### [237] [On the Inverse Flow Matching Problem in the One-Dimensional and Gaussian Cases](https://arxiv.org/abs/2512.23265)
*Alexander Korotin,Gudmund Pammer*

Main category: cs.LG

TL;DR: 该论文研究了具有有限指数矩分布之间的流匹配逆问题，该问题受到现代生成式AI应用的启发，如流匹配模型的蒸馏。在一维设置和高斯情况下建立了解的唯一性，多维问题仍待研究。


<details>
  <summary>Details</summary>
Motivation: 研究流匹配逆问题的动机源于现代生成式AI应用，特别是流匹配模型的蒸馏需求。流匹配是生成模型中的重要技术，但其逆问题的数学性质尚未得到充分研究。

Method: 论文研究了具有有限指数矩分布之间的流匹配逆问题。通过数学分析，在一维设置和高斯分布情况下建立了逆问题解的唯一性。

Result: 证明了在一维设置和高斯分布情况下，流匹配逆问题具有唯一解。然而，一般的多维问题仍然开放，需要未来进一步研究。

Conclusion: 流匹配逆问题在一维和高斯情况下具有唯一解，但多维问题的解的唯一性仍然是一个开放问题，为未来研究指明了方向。
...
### [238] [The Law of Multi-Model Collaboration: Scaling Limits of Model Ensembling for Large Language Models](https://arxiv.org/abs/2512.23340)
...
<details>
  <summary>Details</summary>
Motivation: 单个大语言模型的能力存在固有边界，而多模型协作能够超越任何单个模型的性能。目前缺乏统一的多模型协作性能缩放理论框架。

Method: 提出"多模型协作定律"，采用方法无关的公式化方法，假设理想化的集成预言机，其中每个样本的总交叉熵损失由模型池中任何模型的最小损失决定。

Result: 多模型系统相对于总参数数量遵循幂律缩放，表现出比单模型缩放更显著的改进趋势和更低的理论损失下限。异构模型家族的集成比单一模型家族内的集成获得更好的性能缩放。
...
### [239] [ISOPO: Proximal policy gradients without pi-old](https://arxiv.org/abs/2512.23353)
*Nilin Abrahamsen*
...
<details>
  <summary>Details</summary>
Motivation: 现有近端策略优化方法如GRPO或CISPO需要使用多步梯度计算和重要性采样裁剪来近似自然梯度步长，计算复杂度较高。ISOPO旨在提供一种更高效的近似方法。

Method: ISOPO在Fisher度量下对每个序列的对数概率梯度进行归一化，然后与优势函数进行收缩。另一种变体基于每层的神经正切核变换微批次优势。ISOPO在单次反向传播中逐层应用这种变换。

Result: ISOPO能以可忽略的计算开销实现，相比标准的REINFORCE方法几乎没有额外计算成本，同时能有效近似自然策略梯度。

Conclusion: ISOPO提供了一种高效的单步梯度方法来近似自然策略梯度，相比现有多步方法计算更简单，有望在强化学习中实现更高效的策略优化。
...
### [240] [Post-Training Quantization of OpenPangu Models for Efficient Deployment on Atlas A2](https://arxiv.org/abs/2512.23367)
*Yilun Luo,HuaQing Zheng,Haoqian Meng,Wenyuan Liu,Peng Zhang*

Main category: cs.LG

TL;DR: 华为openPangu-Embedded模型集成三种CoT推理范式，但推理轨迹导致内存和延迟开销。本文通过低比特量化（INT8和W4A8）优化Ascend NPU部署，在保持90%以上精度下实现1.5倍预填充加速。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型集成了三种CoT推理范式（slow_think、auto_think、no_think），增强了推理能力但生成了大量推理轨迹，导致显著的内存和延迟开销，在Ascend NPU上实际部署面临挑战。
...
### [241] [Diffusion priors enhanced velocity model building from time-lag images using a neural operator](https://arxiv.org/abs/2512.23375)
*Xiao Ma,Mohammad Hasyim Taufik,Tariq Alkhalifah*

Main category: cs.LG

TL;DR: 提出了一种结合生成模型和神经算子的新框架，用于高效构建高分辨率速度模型。神经算子作为前向映射算子快速生成RTM扩展图像，生成模型作为正则化器提高分辨率。
...
Method: 提出生成神经算子框架：1) 神经算子作为前向映射算子，从真实和偏移速度模型快速生成时间滞后RTM扩展图像；2) 通过自动微分逐步更新偏移速度；3) 嵌入在真实速度模型分布上训练的生成模型作为正则化器，提高分辨率。

Result: 合成和实际数据实验证明了所提出的生成神经算子速度模型构建方法的有效性，能够获得更清晰、更高分辨率的速度模型预测。

Conclusion: 结合生成模型和神经算子的框架能够高效构建高分辨率速度模型，克服传统方法的计算成本高和耗时问题，为地下成像提供了一种有前景的新方法。
...
### [242] [On the Sample Complexity of Learning for Blind Inverse Problems](https://arxiv.org/abs/2512.23405)
*Nathan Buskulic,Luca Calatroni,Lorenzo Rosasco,Silvia Villa*

Main category: cs.LG

TL;DR: 论文研究了盲逆问题中的学习框架，在线性最小均方误差估计器(LMMSE)框架下，推导了最优估计器的闭式解，建立了与Tikhonov正则化的等价性，并提供了有限样本误差界和收敛性分析。
...
### [243] [Stochastic Siamese MAE Pretraining for Longitudinal Medical Images](https://arxiv.org/abs/2512.23441)
...
Method: 提出STAMP框架：基于Siamese MAE，通过随机过程编码时间信息，将MAE重建损失重构为条件变分推断目标，以两个输入体积的时间差为条件

Result: 在OCT和MRI数据集上评估，STAMP预训练的ViT模型在AMD和阿尔茨海默病进展预测任务上优于现有时间MAE方法和基础模型
...
### [244] [Dynamic Subspace Composition: Efficient Adaptation via Contractive Basis Expansion](https://arxiv.org/abs/2512.23448)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: 提出DSC框架解决MoE模型中的表示崩溃和梯度不稳定问题，通过状态依赖的稀疏扩展共享基库来近似上下文相关权重，显著降低参数复杂度和内存流量。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型虽然能扩展容量，但常常面临表示崩溃和梯度不稳定问题，需要更高效的参数化方法来近似上下文相关权重。
...
Conclusion: DSC框架为MoE模型提供了更高效、稳定的权重近似方法，在降低计算成本的同时保持理论保证，代码已开源。
...
### [245] [Eliminating Inductive Bias in Reward Models with Information-Theoretic Guidance](https://arxiv.org/abs/2512.23461)
...
<details>
  <summary>Details</summary>
Motivation: 奖励模型在基于人类反馈的强化学习中至关重要，但其训练数据通常质量较低，包含各种归纳偏置（如响应长度、迎合性、格式等），这些偏置容易导致过拟合和奖励攻击。现有去偏方法要么针对单一特定偏置，要么仅用简单线性相关性建模，无法处理更复杂多样的偏置。

Method: 提出基于信息瓶颈原理的信息论去偏方法DIR：最大化奖励模型分数与人类偏好对之间的互信息，同时最小化奖励模型输出与偏好输入中偏置属性之间的互信息。该方法能够处理具有非线性相关性的更复杂偏置类型。

Result: 实验验证了DIR对三种归纳偏置（响应长度、迎合性、格式）的有效性。DIR不仅能有效缓解目标归纳偏置，还能提升RLHF在多样化基准测试中的性能，展现出更好的泛化能力。

Conclusion: DIR是一种基于信息论原理的奖励模型去偏方法，能够处理复杂多样的归纳偏置，扩展了奖励模型去偏方法的实际应用场景，并提升了RLHF的整体性能。
...
### [246] [FRoD: Full-Rank Efficient Fine-Tuning with Rotational Degrees for Fast Convergence](https://arxiv.org/abs/2512.23485)
...
Method: FRoD结合分层联合分解与旋转自由度，通过跨层提取全局共享基础，并向缩放因子注入稀疏可学习的扰动，实现灵活的全秩更新，从而增强表达能力和效率。

Result: 在涵盖视觉、推理和语言理解的20个基准测试中，FRoD在相同训练预算下，仅使用1.72%的可训练参数，就能匹配全模型微调的准确率。

Conclusion: FRoD通过创新的分层分解和旋转自由度设计，解决了现有PEFT方法在收敛速度和表达能力方面的限制，实现了高效且强大的模型适应能力。
...
### [247] [Trustworthy Machine Learning under Distribution Shifts](https://arxiv.org/abs/2512.23524)
*Zhuo Huang*
...
<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在人工智能领域取得了显著进展，从ResNet到Transformer再到大语言模型，但分布偏移问题仍然是ML系统的"阿喀琉斯之踵"，限制了系统的可靠性和通用性，并引发了AI的信任问题。作者旨在解决分布偏移下的可信机器学习挑战。

Method: 将常见的分布偏移分为三类：(1)扰动偏移，(2)域偏移，(3)模态偏移。从三个维度系统研究可信性：(1)鲁棒性，(2)可解释性，(3)适应性。基于这些维度提出有效的解决方案和基础见解，同时关注效率、适应性和安全性等关键ML问题。

Result: 论文提出了针对不同分布偏移场景的可信机器学习解决方案，通过增强AI系统的鲁棒性、可解释性和适应性，旨在提升ML系统在分布偏移下的可靠性和实用性。

Conclusion: 分布偏移是限制机器学习系统可靠性和通用性的关键问题。通过系统研究扰动偏移、域偏移和模态偏移，并从鲁棒性、可解释性和适应性三个维度提出解决方案，可以有效提升AI系统在分布偏移下的可信度，推动更可靠、更负责任的人工智能发展。
...
### [248] [EEG-based Graph-guided Domain Adaptation for Robust Cross-Session Emotion Recognition](https://arxiv.org/abs/2512.23526)
*Maryam Mirzaei,Farzaneh Shayegh,Hamed Narimani*

Main category: cs.LG

TL;DR: EGDA框架通过联合对齐全局和类别特定分布来减少跨会话差异，同时通过图正则化保持EEG数据内在结构，在SEED-IV数据集上实现了稳健的跨会话情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 准确识别人类情感状态对人机交互至关重要。EEG因其高时间分辨率和直接反映神经活动而成为可靠的情感识别来源，但跨会话的变异性对模型泛化构成重大挑战。

Method: 提出EGDA框架，通过联合对齐全局（边际）和类别特定（条件）分布来减少跨会话差异，同时使用图正则化保持EEG数据的内在结构。

Result: 在SEED-IV数据集上，EGDA在三个迁移任务中分别获得81.22%、80.15%和83.27%的准确率，优于多个基线方法。分析显示Gamma频带最具判别性，中央-顶叶和前额叶脑区对情感识别至关重要。

Conclusion: EGDA框架通过有效减少跨会话差异，实现了稳健的EEG情感识别性能，为跨会话情感识别提供了有效解决方案。
...
### [249] [VL-RouterBench: A Benchmark for Vision-Language Model Routing](https://arxiv.org/abs/2512.23562)
*Zhehao Huang,Baijiong Lin,Jingyuan Zhang,Jingying Wang,Yuhang Liu,Ning Lu,Tao Li,Xiaolin Huang*

Main category: cs.LG

TL;DR: VL-RouterBench：首个系统性、可复现的视觉语言模型路由基准，覆盖14个数据集、17个模型、51.9万样本-模型对，评估10种路由方法，发现现有路由器与理想Oracle仍有明显差距。
...
Method: 基于VLMs的原始推理和评分日志构建质量和成本矩阵，覆盖14个数据集（3个任务组）、30,540个样本，包含15个开源模型和2个API模型，共519,180个样本-模型对。评估协议联合测量平均准确率、平均成本和吞吐量，通过归一化成本与准确率的调和平均数构建排名分数。
...
### [250] [Distribution-Free Process Monitoring with Conformal Prediction](https://arxiv.org/abs/2512.23602)
*Christopher Burger*

Main category: cs.LG

TL;DR: 本文提出了一种混合框架，通过整合Conformal Prediction的分布无关、模型不可知保证来增强传统统计过程控制（SPC），解决了传统SPC在复杂制造环境中统计假设常被违反的问题。


<details>
  <summary>Details</summary>
Motivation: 传统统计过程控制（SPC）对质量管控至关重要，但其依赖于常被违反的统计假设，导致在现代复杂制造环境中的监控不可靠。需要一种更稳健、统计上更严谨的方法来增强质量控制的可靠性。

Method: 提出一个混合框架，将Conformal Prediction的分布无关、模型不可知保证整合到SPC中。具体包括两种新颖应用：1) Conformal-Enhanced Control Charts（可视化过程不确定性并启用"不确定性峰值"等主动信号）；2) Conformal-Enhanced Process Monitoring（将多变量控制重新构建为使用直观p值图的正式异常检测问题）。

Result: 该框架为质量控制提供了更稳健和统计上更严谨的方法，同时保持了经典方法的可解释性和易用性。通过Conformal Prediction的整合，增强了SPC在复杂制造环境中的可靠性和适应性。

Conclusion: 通过整合Conformal Prediction的分布无关保证，提出的混合框架显著增强了传统SPC方法，为现代复杂制造环境提供了更可靠、更稳健的质量控制解决方案，同时保持了经典方法的实用性和可解释性优势。
...
### [251] [Le Cam Distortion: A Decision-Theoretic Framework for Robust Transfer Learning](https://arxiv.org/abs/2512.23617)
*Deniz Akdemir*

Main category: cs.LG

TL;DR: 论文提出Le Cam Distortion框架，用方向性可模拟性取代对称不变性，解决传统无监督域适应在信息不对称域中的负迁移问题


<details>
  <summary>Details</summary>
Motivation: 传统无监督域适应方法通过强制特征不变性来对齐源域和目标域表示，但在域信息不对称时（如高质量传感器vs降级传感器），严格不变性会导致信息破坏和负迁移，这在安全关键应用中可能造成灾难性后果

Method: 基于Le Cam统计实验理论，提出决策理论框架，用方向性可模拟性取代对称不变性。引入Le Cam Distortion（通过缺陷距离δ(E₁, E₂)量化）作为可模拟性条件下迁移风险的严格上界。通过学习从源域模拟目标域的核来实现迁移而不降低源域性能

Result: 在五个实验中：1）HLA基因组学中实现近乎完美的频率估计（相关性r=0.999，匹配经典方法）；2）CIFAR-10图像分类中实现零源域效用损失（81.2%准确率保持，而CycleGAN下降34.7%）；3）RL控制中实现安全策略迁移，而不变基方法遭受灾难性崩溃

Conclusion: Le Cam Distortion为风险可控的迁移学习提供了首个原则性框架，特别适用于医学影像、自主系统和精准医学等负迁移不可接受的领域
...
### [252] [BOAD: Discovering Hierarchical Software Engineering Agents via Bandit Optimization](https://arxiv.org/abs/2512.23631)
*Iris Xu,Guangtao Zeng,Zexue He,Charles Jin,Aldo Pareja,Dan Gutfreund,Chuang Gan,Zhang-Wei Hong*

Main category: cs.LG

TL;DR: BOAD框架通过多臂老虎机优化自动发现有效的分层多智能体系统，显著提升大语言模型在长视野软件工程任务上的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有单智能体系统在处理真实世界软件工程问题时存在局限性，难以处理长视野和分布外问题，需要更有效的多智能体协作架构

Method: 将分层发现建模为多臂老虎机问题，每个臂代表候选子智能体，奖励衡量其在团队协作中的有效性，通过BOAD框架在有限评估预算下高效探索智能体设计
...
### [253] [Random Controlled Differential Equations](https://arxiv.org/abs/2512.23670)
*Francesco Piatti,Thomas Cass,William F. Turner*

Main category: cs.LG

TL;DR: 提出结合随机特征与控制微分方程的时序学习框架，仅训练线性读出层，实现快速可扩展模型。包含两个变体：RF-CDEs使用随机傅里叶特征提升输入信号，R-RDEs通过log-ODE离散化直接处理粗糙路径输入。


<details>
  <summary>Details</summary>
Motivation: 传统时序学习方法计算复杂度高，特别是显式签名计算效率低下。需要开发既能保持签名理论归纳偏置，又能通过随机特征实现高效计算的实用替代方案。

Method: 1. 使用大型随机参数化控制微分方程作为连续时间储层，将输入路径映射到丰富表示；2. 仅训练线性读出层；3. 提出RF-CDEs：使用随机傅里叶特征提升输入信号，近似RBF增强序列模型；4. 提出R-RDEs：通过log-ODE离散化直接处理粗糙路径输入，使用log-签名捕获高阶时序交互。

Result: 1. 在无限宽度极限下，模型分别诱导出RBF提升签名核和粗糙签名核；2. 在多个时序基准测试中表现出竞争性或最先进的性能；3. 提供了显式签名计算的实用替代方案，保持归纳偏置的同时受益于随机特征效率。

Conclusion: 该框架统一了随机特征储层、连续时间深度架构和路径签名理论，为时序学习提供了高效实用的解决方案，在保持理论优势的同时显著提升了计算效率。
...
### [254] [End-to-End Test-Time Training for Long Context](https://arxiv.org/abs/2512.23675)
...
TL;DR: 将长上下文语言建模重新定义为持续学习问题，使用标准Transformer架构配合滑动窗口注意力，通过测试时训练和元学习实现上下文压缩，在保持恒定推理延迟的同时实现与全注意力Transformer相似的扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统长上下文建模方法主要关注架构设计，本文提出将其重新定义为持续学习问题，旨在通过测试时训练让模型在推理时持续学习，从而更有效地处理长上下文。

Method: 使用标准Transformer架构配合滑动窗口注意力，通过测试时训练让模型在推理时通过下一个token预测持续学习，将读取的上下文压缩到权重中。训练时使用元学习优化模型初始化，使模型在测试时能更好地学习。

Result: 对于3B模型训练164B tokens，TTT-E2E方法在上下文长度扩展性方面与全注意力Transformer表现相似，而Mamba 2和Gated DeltaNet则表现不佳。同时，TTT-E2E具有恒定推理延迟，在128K上下文长度下比全注意力快2.7倍。

Conclusion: 将长上下文建模视为持续学习问题而非架构设计问题是一种有效方法，通过测试时训练和元学习的端到端方法，能够在保持恒定推理延迟的同时实现与全注意力Transformer相似的扩展性能。
...
### [255] [Training AI Co-Scientists Using Rubric Rewards](https://arxiv.org/abs/2512.23707)
...
TL;DR: 该研究提出了一种利用现有研究论文训练语言模型生成更好研究计划的方法，通过自动提取研究目标和评分标准，采用强化学习与自我评分进行微调，在机器学习和医学领域都取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前AI科研助手在根据研究目标和约束生成研究计划时存在困难，无法完全遵循所有约束和隐含要求。需要开发一种能够生成高质量研究计划的方法，以提升AI科研助手的实用性。

Method: 1. 从多个领域的研究论文中自动提取研究目标和目标特定的评分标准，构建可扩展的多样化训练语料库；2. 通过强化学习与自我评分训练研究计划生成模型，使用初始策略的冻结副本作为评分器，利用评分标准创建生成器-验证器差距，实现无外部人工监督的改进。

Result: 1. 在机器学习研究目标的人类专家研究中（225小时），专家在70%的研究目标上更偏好微调后的Qwen3-30B-A3B模型生成的计划；2. 84%的自动提取的目标特定评分标准获得专家批准；3. 扩展到医学论文和arXiv预印本后，微调带来了12-22%的相对改进，并显示出显著的跨领域泛化能力。

Conclusion: 该方法证明了可扩展的自动化训练方案在提升通用AI科研助手能力方面的潜力，即使在医学研究等执行反馈不可行的问题设置中也同样有效。
...
### [256] [Urban Food Self-Production in the Perspective of Social Learning Theory: Empowering Self-Sustainability](https://arxiv.org/abs/2512.22594)
*Ewa Duda,Adamina Korwin-Szymanowska*

Main category: cs.CY

TL;DR: 该研究探讨了城市居民参与创新性城市食品生产项目（在公寓走廊安装水培柜）的动机、经验和教育背景，为城市教育和发展提供参考。


<details>
  <summary>Details</summary>
Motivation: 在气候变化和粮食安全的背景下，城市食品生产变得越来越重要。研究城市农业作为替代食品生产形式的益处、挑战和发展潜力，对于城市发展至关重要。

Method: 采用定性研究方法，通过目的性抽样和两轮深度访谈，对波兰罗兹和华沙两个社区的42名参与者进行研究，了解他们参与创新城市食品生产项目的动机和经验。

Result: 研究发现揭示了城市居民采用可持续食品生产解决方案的动机、他们的农业实践经验，以及促使他们加入创新城市食品生产项目的教育活动。

Conclusion: 研究结果对参与城市教育过程的各方（包括城市当局、城市教育者、环保协会和基层活动家）具有重要参考价值，有助于推动可持续城市食品生产的发展。
...
### [257] [Ungraded Assignments in Introductory Computing: A Report](https://arxiv.org/abs/2512.23004)
*Yehya Sleiman Tellawi,Abhishek K. Umrawal*

Main category: cs.CY

TL;DR: 研究探讨了无评分作业对计算机工程导论课程学生学习体验的影响，发现参与无评分作业与课程整体表现呈正相关关系。


<details>
  <summary>Details</summary>
Motivation: 探索无评分作业对计算机工程导论课程学生学习体验的影响，包括学生参与度、理解程度和整体学业表现。

Method: 为ECE 120计算机导论课程开发并实施了新的无评分作业，采用混合方法评估其有效性，包括问卷调查、访谈和表现分析。

Result: 分析显示参与无评分作业与课程整体表现之间存在正相关关系，表明这些作业可能吸引高成就学生和/或支持更好的学习成果。

Conclusion: 无评分作业对计算机工程导论课程的学生学习有积极影响，特别是对高成就学生或能改善学习成果的学生群体。
...
### [258] [Inteligencia Artificial y Empleo: perspectiva Territorial y de Género](https://arxiv.org/abs/2512.23059)
*Antoni Mestre,Xavier Naya,Manoli Albert,Vicente Pelechano*

Main category: cs.CY

TL;DR: 该研究提出了一种基于行业数据的AI就业暴露评估框架，用于分析西班牙各地区和性别间的AI影响差异，发现大都市和服务导向地区暴露度更高，且女性就业在所有地区都面临更高的AI暴露风险。


<details>
  <summary>Details</summary>
Motivation: 人工智能特别是生成模型的扩散预计会对劳动力市场产生不均衡影响，传统基于职业的方法在西班牙背景下存在局限性，需要开发基于行业数据的评估框架来更准确地评估AI对就业的潜在影响。

Method: 构建AI-CNAE影响矩阵，应用于2021-2023年省级就业数据，提供按地区和性别分解的AI暴露评估，采用基于行业而非职业的方法来克服传统方法的局限性。

Result: 结果显示稳定的结构性模式：大都市和服务导向地区暴露度更高；存在一致的性别差距，女性就业在所有地区都表现出更高的AI暴露；框架提供了AI最可能重塑工作和技能需求的结构性视角。

Conclusion: 该框架不是预测工作岗位流失，而是提供AI最可能重塑工作和技能需求的结构性视角，支持基于证据的政策制定和战略规划，为理解AI对劳动力市场的差异化影响提供了新方法。
...
### [259] [Identifying Barriers Hindering the Acceptance of Generative AI as a Work Associate, measured with the new AGAWA scale](https://arxiv.org/abs/2512.23373)
*Łukasz Sikorski,Albert Łukasik,Jacek Matulewski,Arkadiusz Gut*

Main category: cs.CY

TL;DR: 研究者开发了AGAWA量表来测量学生对生成式AI作为同事的态度，发现积极态度与对AI的担忧、拟人化特征感知和人类独特性意识呈负相关。


<details>
  <summary>Details</summary>
Motivation: 当前学生对生成式AI的态度将影响未来工作场所的AI采用，具有重要的经济和社会意义。需要及时研究这一现象并识别实施障碍，使用能跟上AI快速发展的工具。

Method: 提出AGAWA量表，基于TAM和UTAUT技术接受模型，特别关注AI革命背景下的关键问题：AI存在的接受度、社会影响（如作为助手或主管）以及道德困境的解决。量表仅含4个项目，完成和分析时间短。研究考察了三个因素：与GenAI互动的担忧、其拟人化特征、人类独特性/优越感意识。

Result: 结果显示：对GenAI作为同事的积极态度与所有三个因素呈强负相关，且这三个因素之间也相互关联（正相关）。这证实了情感和道德维度的AI信任与工作场所生成式AI态度之间的关系。

Conclusion: AGAWA量表是测量对生成式AI作为同事态度的有效工具，揭示了情感和道德因素在AI接受中的重要作用。学生对AI的态度将显著影响未来工作场所的AI采用，需要关注这些心理障碍以促进成功实施。
...
### [260] [Can AI Recognize Its Own Reflection? Self-Detection Performance of LLMs in Computing Education](https://arxiv.org/abs/2512.23587)
*Christopher Burger,Karmece Talley,Christina Trotter*

Main category: cs.CY

TL;DR: 评估GPT-4、Claude和Gemini三种大语言模型在计算机教育中检测AI生成文本的能力，发现它们在标准条件下能识别AI文本，但难以准确判断人类写作（错误率高达32%），且易受欺骗性提示影响，不适合用于高风险学术不端判断。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，计算机教育领域的学术诚信面临严峻挑战。教育工作者需要可靠的检测方法，因此需要评估主流LLMs在计算机特定语境下识别AI生成文本的能力。

Method: 评估三种主流LLMs（GPT-4、Claude和Gemini）在计算机特定语境下检测AI生成文本的能力。测试分为标准条件和"欺骗性"提示条件，后者指示模型规避检测。分析模型在识别AI生成文本和人类写作方面的表现。

Result: 1. 默认AI生成文本容易被识别；2. 所有模型在判断人类写作时表现不佳，错误率高达32%；3. 模型对欺骗性提示高度敏感，Gemini的输出完全欺骗了GPT-4；4. 简单的提示修改就能显著降低检测效果。

Conclusion: 当前这些大语言模型在检测AI生成文本方面过于不可靠，不适合用于高风险学术不端行为的判断。简单的提示修改就能显著降低检测效果，表明现有方法存在严重局限性。
...
### [261] [AI tutoring can safely and effectively support students: An exploratory RCT in UK classrooms](https://arxiv.org/abs/2512.23633)
...
TL;DR: AI教学模型LearnLM在数学辅导中表现优异，76.4%的生成内容无需或仅需极简编辑，学生表现不亚于甚至在某些方面优于真人辅导


<details>
  <summary>Details</summary>
Motivation: 一对一辅导虽是个性化教育的黄金标准，但成本高昂难以规模化。研究旨在探索生成式AI是否能帮助扩大这种优质教育资源

Method: 在英国5所中学进行随机对照试验（N=165），将教学优化的生成式AI模型LearnLM集成到Eedi数学平台的聊天辅导中，由专家导师监督并修订AI生成内容

Result: LearnLM生成内容76.4%无需或仅需极简编辑；AI辅导学生在各项学习成果上表现至少与真人辅导相当；AI辅导学生解决新问题的成功率（66.2%）比真人辅导（60.7%）高5.5个百分点；导师反馈AI擅长设计启发式问题

Conclusion: 教学优化的AI辅导系统在提供有效、个性化学习支持方面具有规模化应用的潜力
...
### [262] [OxygenREC: An Instruction-Following Generative Framework for E-commerce Recommendation](https://arxiv.org/abs/2512.22386)
...
TL;DR: OxygenREC是一个工业级推荐系统，采用快慢思维架构，结合LLM深度推理能力和高效编码器-解码器，解决传统推荐系统多阶段优化不一致、多场景扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统存在多阶段优化目标不一致的问题，生成式推荐虽然缓解了这一问题但仍依赖归纳模式匹配，无法揭示需要基于世界知识进行演绎推理的复杂用户意图。同时，LLM虽有强大推理能力但延迟和计算成本高，且现有解决方案在多场景扩展性方面存在瓶颈，需要为每个场景独立训练和部署，资源利用率低且维护成本高。

Method: 1. 采用快慢思维架构：慢思维使用近线LLM管道合成上下文推理指令，快思维采用高效编码器-解码器主干进行实时生成。2. 引入语义对齐机制：通过指令引导检索过滤意图相关历史行为，使用查询到项目损失确保指令-项目一致性。3. 解决多场景扩展性：将场景信息转化为可控指令，使用统一奖励映射和软自适应组剪裁策略优化来对齐不同业务目标，实现"一次训练，到处部署"范式。

Result: 论文展示了OxygenREC系统能够在满足严格延迟要求的同时实现深度推理，并有效解决多场景扩展性问题，通过统一的架构实现资源高效利用和低维护成本。

Conclusion: OxygenREC通过快慢思维架构成功结合了LLM的深度推理能力和高效生成模型的实时性能，同时解决了多场景扩展性挑战，为工业级推荐系统提供了一种创新的解决方案。
