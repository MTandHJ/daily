...
Motivation: 当前缺乏真实世界和细粒度的用户偏好标注，阻碍了理解个人偏好的生成模型的发展。

Method: 收集真实用户交互数据构建ImageGem数据集，训练偏好对齐模型，研究检索模型和视觉语言模型在个性化图像检索和生成模型推荐中的表现，提出在潜在权重空间中编辑定制扩散模型的端到端框架。

Result: 利用用户偏好标注训练了更好的偏好对齐模型，并成功展示了在个性化图像检索和生成模型推荐中的应用。

Conclusion: ImageGem数据集首次实现了生成模型个性化的新范式，为理解细粒度用户偏好的生成模型研究提供了重要基础。
...
Result: 在Pascal VOC上mAP达97.4（优于PAT-T的96.2），COCO上mAP达92.8（优于HSQ-CvN的92.0），VG-256上mAP达60.9，并实现了加速收敛和跨域泛化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的数据选择方法存在两个关键缺陷：处理整个数据集的计算开销过高，以及重要性评估和多样性选择分离导致次优数据选择。

Method: CoIDO采用轻量级插件评分器，仅需在小规模随机数据样本上训练来学习候选集分布，通过同方差不确定性公式有效平衡重要性和多样性。

Result: 实验中使用20%随机数据训练CoIDO评分器，然后在完整数据集上选择20%子集进行指令调优，在LLaVA-1.5-7B模型上达到全数据微调性能的98.2%。

Conclusion: CoIDO框架显著降低了计算需求，同时保持了接近全数据训练的性能，为大规模多模态模型的高效训练提供了可行方案。
...
Method: 使用潜在扩散模型，结合预治疗MRI和肿瘤定位进行条件生成，采用无分类器引导利用生存信息提升生成质量，实现切片到切片的翻译。

Result: 在包含140名胶质母细胞瘤患者的本地数据集上训练和测试，数据集包括预治疗和T1-Gd MRI、专家手动勾画的肿瘤定位以及生存信息。

Conclusion: 该方法能够早期预测胶质母细胞瘤的治疗反应，为个性化医疗提供支持，通过生成治疗后MRI来反映肿瘤演变过程。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI和大型语言模型的快速发展使得能够生成高度逼真和上下文相关的数字内容，这给数字内容认证带来了挑战，需要验证数字数据的完整性和来源。

Method: 利用五个基准嵌入模型处理多样化的AI和人类生成图像数据集，基于AI生成图像在嵌入空间中更接近其他AI生成内容、而人类创作图像在其领域内聚集的假设。

Result: 实验证明该方法具有鲁棒性，中等到高度扰动对嵌入签名影响最小，扰动图像与其原始版本保持高度相似性匹配。
...
Method: 提出级联多尺度个体标准化网络(CMIS-Net)，在帧级和序列级去除个体特定中性基线，提取个体标准化特征，并引入隐式数据增强模块

Result: CMIS-Net有效处理个体差异和数据不平衡，在反馈信号一致性检测中达到最先进性能

Conclusion: 多尺度个体标准化方法能有效处理反馈信号检测中的个体差异问题，为开发更自然的人机交互系统提供重要技术支撑
...
<details>
  <summary>Details</summary>
Motivation: 现有的流匹配快捷方法需要专门的步长嵌入，与现有模型不兼容，重新训练成本几乎与预训练相当。本文旨在为标准的流匹配模型（如Flux）提供更激进的快捷机制。

Method: 基于速度场而非样本空间进行自蒸馏，采用在线学习方式，无需步长嵌入。该方法也可整合到预训练阶段，使模型天生学习高效的少步流。

Result: 该方法训练效率极高，例如在不到一个A100天的时间内生成3步Flux模型。还能实现首个针对数十亿参数扩散模型的少样本蒸馏方法（如10个文本-图像对），以几乎零成本提供最先进性能。

Conclusion: 该方法成功实现了对大规模预训练流匹配模型的高效快捷化，无需重新训练，并在少步采样和少样本蒸馏方面取得了突破性成果。
...
<details>
  <summary>Details</summary>
Motivation: 传统的人类活动识别方法在水下环境中效果不佳，因为无线信号在水中会衰减，导致潜水员佩戴的传感器与自主水下航行器(AUV)之间的通信受阻。需要一种能够有效监测潜水员安全的新方法。

Method: 利用计算机视觉从3D人体关节点流生成高保真运动数据，创建"伪IMU"，通过分类器识别异常潜水员行为，并将系统集成到AUV上进行实验验证。

Result: 通过在模拟紧急场景下进行实验，证明了该方法在机器人监测和潜水员安全方面的实用性和有效性。

Conclusion: 该混合方法成功解决了水下环境中传统传感器的通信问题，为潜水员安全监测提供了一种有效的技术方案，能够识别如心脏骤停等医疗紧急情况的异常行为。
...
<details>
  <summary>Details</summary>
Motivation: RGB和X射线成像融合在医疗诊断、文化遗产修复和制造等领域具有重要价值，但现有方法面临数据表示差异和配对数据有限的挑战
...
<details>
  <summary>Details</summary>
Motivation: 解决零样本2D目标检测和分割问题，无需额外训练即可处理未见过的3D物体，提高模型的通用性和适应性。

Method: 使用3D未见物体的2D多视图模板和查询图像中的2D目标提议；整合类别和补丁嵌入，用GeM池化归一化；采用结合绝对和相对相似度的联合相似度度量；通过不确定性感知目标先验优化相似度得分。

Result: 在BOP Challenge 2025的Classic Core、H3和Industrial赛道上均排名第一，实现了最先进的性能。

Conclusion: MUSE提供了一个强大且通用的零样本2D目标检测和分割框架，无需训练即可在多个挑战性场景中取得优异表现。
...
<details>
  <summary>Details</summary>
Motivation: 光学音乐识别领域因真实标注数据稀缺而受限，特别是在处理手写历史乐谱时。类似手写文本识别领域已证明合成数据能提升识别性能。

Method: 实现音乐符号级别的生成对抗网络(GAN)生成手写风格符号，然后使用Smashcima排版软件将这些符号组装成完整乐谱。

Result: 系统评估了生成样本的视觉保真度，发现生成的符号具有高度真实感，在合成乐谱生成方面取得显著进展。

Conclusion: 该方法能有效生成逼真的手写风格乐谱，为光学音乐识别训练提供有价值的合成数据源。
...
<details>
  <summary>Details</summary>
Motivation: 性别分类系统经常继承并放大训练数据中的人口统计不平衡问题，导致模型存在偏见，特别是在女性和少数族裔群体上的分类表现较差。

Method: 首先审计五个常用数据集，然后在两个相对平衡的数据集上训练分类器评估偏见。最后通过混合多个数据集构建BalancedFace，确保189个人口统计交叉组别都有均衡的代表性。

Result: 使用BalancedFace训练的分类器将种族子组间的最大真阳性率差距减少了50%以上，平均差异影响得分比次优数据集更接近理想值1.0（改善63%），同时整体准确率损失很小。

Conclusion: 数据为中心的干预措施对改善性别分类系统的公平性具有重要价值，BalancedFace为公平性别分类研究提供了公开可用的资源。
...
Method: 设计了类感知标签精炼模块和几何感知标签精炼组件，结合自训练策略迭代提升伪标签质量并扩展标签覆盖范围。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习分割算法在医学图像分析中取得进展，但数据内在偏见导致基于种族和性别等敏感属性的性能差异问题日益突出，需要评估分割模型的公平性。

Method: 使用包含四个人口亚组的数据集，评估UNesT、nnU-Net、CoTr和ANTs模型在伏隔核分割中的表现，采用公平性度量指标和线性混合模型分析人口变量对分割准确性和体积的影响。

Result: 在种族匹配训练测试条件下，ANTs和UNesT分割准确性显著提高，而nnU-Net表现稳健不受人口匹配影响；手动分割观察到的性别效应在偏置模型中也能重现，但种族效应除一个模型外均消失。

Conclusion: 分割模型存在基于种族的性能差异，种族匹配训练能改善某些模型的公平性，而性别效应比种族效应更稳定地体现在分割结果中，需要开发更公平的分割方法。
...
Method: 收集241名参与者观看10个专业漫才表演的多模态数据，使用k-means聚类识别观众类型，分析观看顺序效应，进行自动化幽默分类和观众反应建模。

Result: 识别出三种观众类型：高稳定欣赏者（72.8%）、低可变下降者（13.2%）和可变改善者（14.0%）；发现积极观看顺序效应；未发现类型间显著差异。

Conclusion: 该数据集支持文化敏感情感AI开发，挑战了疲劳假说，为个性化娱乐系统提供了非西方背景的研究基础。
...
<details>
  <summary>Details</summary>
Motivation: 在线学习环境中学生参与度检测对于改善学习成果和个性化教学至关重要，需要更准确高效的检测方法。

Method: 使用双流架构，通过EfficientNetV2分别提取面部裁剪和完整视频帧的空间特征，然后采用LSTM和Transformer两种时序建模策略分析时序特征，并应用针对性数据增强处理样本不平衡问题。

Result: 在DAiSEE数据集上，ViBED-Net结合LSTM的变体达到73.43%的准确率，超越了现有的最先进方法。

Conclusion: 结合面部感知和场景感知的时空线索能显著提高参与度检测准确率，模块化设计使其可灵活应用于教育、用户体验研究和内容个性化等领域。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统对罕见、分布外场景中的语义异常仍然极度脆弱，而现有的视觉语言模型提示方法性能不可靠且依赖昂贵的专有模型，限制了实际部署。

Method: 采用结构化推理框架，通过分层场景描述提取和多模态评估的两阶段流程，在四个语义层（街道、基础设施、可移动物体、环境）进行系统分析。

Result: 在真实驾驶场景中达到89.6%召回率和88.0%准确率，显著优于非结构化基线。更重要的是，微调的7B参数开源模型达到90.8%召回率和93.8%准确率，超越所有评估模型。
...
<details>
  <summary>Details</summary>
Motivation: 红棕螨侵染已成为严重问题，特别是在棕榈种植区，导致生产力下降和经济损失。准确早期识别受侵染植物对有效管理至关重要。

Method: 使用TriggerNet框架集成Grad-CAM、RISE、FullGrad和TCAV生成可视化解释；采用CNN、EfficientNet、MobileNet、ViT、ResNet50、InceptionV3等深度学习模型以及随机森林、SVM、KNN等机器学习分类器；使用Snorkel通过启发式规则高效标注疾病类别。

Result: 在11种植物物种（槟榔、枣椰树、天堂鸟、椰子树、生姜、柑橘树、油棕、兰花、香蕉树、鳄梨树、铁树）上进行了训练和评估，将植物分为健康、黄斑、红褐色青铜化和丝网四个疾病类别。

Conclusion: 该研究为红棕螨侵染检测提供了一种可解释的AI解决方案，通过自动化标注和多种模型比较，提高了植物疾病分类的准确性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在几何推理方面存在困难，需要开发能够同时处理3D相机轨迹生成和空间描述的方法，以实现自动化专业视频制作。

Method: 使用扩散过程生成平滑相机轨迹，结合已知相机位姿约束，将3D信息整合到视觉语言模型中，最后用3D高斯泼溅渲染新视角视频。

Result: 实验表明，将3D相机轨迹整合到文本生成过程中，性能优于独立处理任务的方法，并提出了新的联合评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有个性化图像生成模型缺乏对多源图像特定部分组合的显式控制能力，需要用户提供掩码或标注。

Method: 构建基于464个独特(部分，主体)对的语义原子数据集，生成37k提示并使用高保真文本到图像模型合成图像。训练具有部分条件引导的自定义扩散先验模型，通过部分条件引导控制图像条件特征以强制执行语义身份和空间布局。

Result: 人类评估和提出的PartEval指标显示，Chimera在部分对齐和组合准确性上优于其他基线14%，在视觉质量上优于21%。
...
<details>
  <summary>Details</summary>
Motivation: 微塑料污染对生态系统和人类健康构成威胁，但传统检测方法需要人工分析，难以在大规模筛查中高效应用。

Method: 使用扫描电子显微镜(SEM)成像结合基于YOLO的机器学习目标检测方法，专注于具有对称重复图案背景的过滤场景。

Result: 研究发现不同YOLO模型在微塑料检测任务中表现存在差异，预处理优化对检测质量有重要影响。

Conclusion: 机器学习在微塑料检测中具有潜力，但需要解决专家标注数据有限的问题，并优化预处理流程以提高检测可靠性。
...
Method: 提出自适应patch Transformer(APT)，根据图像内容复杂度动态调整patch大小，在均匀区域使用大patch减少token数量，在复杂区域使用小patch保留细节。

Result: 在ViT-L和ViT-H上分别实现40%和50%的吞吐量提升，下游任务性能保持不变；在视觉问答、目标检测和语义分割等密集视觉任务中，训练和推理速度提升达30%。

Conclusion: APT方法有效解决了ViT在高分辨率图像处理中的计算效率问题，显著加速推理和训练过程，同时保持模型性能，且可快速适配已微调的ViT模型。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS技术正在彻底改变从姿态图像进行3D重建的方法，但存在内存占用高、光照效果被直接烘焙到表示中以及次生射线效果支持有限等问题，需要系统性地梳理和总结。

Method: 从3DGS的泼溅公式出发，系统探索解决其局限性的主要方法，并调查其在表面重建、虚拟化身建模、动画和内容生成等领域的应用。

Result: 3DGS通过将场景显式建模为3D高斯集合，实现了高效的体积泼溅光栅化，能够与常见图形管线无缝集成，支持实时渲染新视角合成。

Conclusion: 3DGS凭借其高效渲染能力和适用于前馈管线的特性，在多个3D重建相关领域展现出巨大应用潜力，但需要继续解决其内存占用和表示限制等问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在低资源、预算受限环境中的部署问题。大模型性能强但不实用，小模型高效但需要昂贵的微调才能缩小与大型模型在部署领域的性能差距。

Method: 基于上下文学习框架，提出在线上下文蒸馏方法。包括跨模态演示选择策略、教师测试时缩放以减少噪声、学生不确定性条件化来动态填充演示池并最小化教师查询。

Result: ICD方法显著提升了小模型性能(最高33%)，仅使用稀缺的教师标注(低至4%)，就能与教师的零样本性能相竞争。

Conclusion: 该方法为在计算预算受限情况下部署高效视觉语言模型提供了可行方案，通过上下文蒸馏实现了小模型与强教师模型的协作学习。
...
Method: 开发了全面的攻击策略分类，包括连接中断、中继/重放干扰、内容欺骗和多连接伪造。提出了SafeCoop防御框架，集成语义防火墙、语言-感知一致性检查和多源共识，通过代理转换函数实现跨帧空间对齐。

Result: 在CARLA仿真中对32个关键场景进行系统评估，在恶意攻击下实现69.15%的驾驶分数提升，恶意检测F1分数最高达67.32%。

Conclusion: 这项研究为推进交通系统中安全、可靠的语言驱动协作研究提供了指导，展示了SafeCoop框架在提升协同驾驶系统安全性方面的有效性。
...
Method: 开发World-in-World开放平台，提供统一在线规划策略和标准化动作API，在四个闭环环境中评估多样化世界模型，以任务成功率为主要指标。

Result: 发现三个意外结果：视觉质量不保证任务成功，可控性更重要；用动作-观察数据进行后训练扩展比升级预训练视频生成器更有效；增加推理时间计算可显著提升闭环性能。

Conclusion: 世界模型在具身决策中的效用需要闭环评估，可控性比视觉质量更重要，数据扩展和推理计算是提升性能的关键因素。
...
<details>
  <summary>Details</summary>
Motivation: 现有立体视觉重建方法在月球表面重建中表现不佳，因为月球缺乏纹理、光照变化复杂且轨道轨迹特殊，而现有深度学习模型在行星图像上测试不足且无法直接迁移到月球条件。

Method: 使用光线追踪基于高分辨率地形和反射率模型创建了LunarStereo数据集，然后通过在该数据集上微调MASt3R模型来适应月球领域。

Result: 在合成和真实月球数据上的广泛实验验证了该方法，相比零样本基线有显著改进，为地外环境中的跨尺度泛化铺平了道路。

Conclusion: LunarStereo数据集和微调方法有效解决了月球3D重建的挑战，展示了在行星探索中稳健的跨尺度泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 拥挤场景中的异常检测面临严重的人际遮挡和高度动态、上下文相关的运动模式挑战，现有方法难以适应不同人群密度且缺乏可解释的异常指标。

Method: 使用双管道框架结合头部检测和密集光流提取个体速度，通过分层聚类将速度分为停止、慢速、正常和快速四类，采用百分位异常评分系统衡量与学习到的正常模式的偏差。

Result: 实验证明该框架在密集拥挤环境中能够实时检测各种异常运动模式。

Conclusion: VelocityNet框架有效解决了拥挤场景中异常检测的挑战，能够适应不同人群密度并提供可解释的异常指标。
...
Result: RadDiagSeg-M在所有多目标文本和掩码生成任务组件中表现出强劲性能，建立了稳健且具有竞争力的基准。

Conclusion: RadDiagSeg-M能够提供高度信息丰富且临床有用的输出，有效满足辅助诊断中丰富上下文信息的需求。
...
Result: 在PTMC-RFA数据集上，maxDice从0.82提升到0.86，maxIoU从0.72提升到0.76，假阳性减少29%。在外部基准测试中，Dice得分比SAM-2提高2-5个百分点，同时仅增加<0.1%的FLOPs，保持约30FPS的实时吞吐量。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全评估方法将视觉和语言输入分开处理，忽略了组合解释中良性内容可能变得有害的风险，且无法清晰区分明显不安全内容和边界案例。

Method: 使用多阶段流程构建包含8,187个样本的大规模基准测试，涵盖17种安全模式和15种危害类别，评估11个最先进模型。

Result: 模型在清晰单模态安全信号上准确率超过90%，但在需要联合图像-文本推理时性能降至20-55%；34%的联合分类错误发生在单模态分类正确的情况下。

Conclusion: 当前模型在联合图像-文本理解和安全对齐方面存在显著弱点，VLSU框架为研究稳健视觉语言安全提供了关键测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有去偏方法受限于样本表示多样性，简单生成增强往往保留原有偏见，且罕见类单纯增加数据量效果不佳，因为实例频率不能完全反映模型真实需求，且当前布局到图像合成缺乏保真度和控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究长上下文压缩的可行性，解决高分辨率输入下激活值保持低水平同时实现高压缩比的问题，为历史长上下文压缩和LLM记忆遗忘机制等研究领域提供支持。

Method: 采用DeepEncoder作为核心引擎，配合DeepSeek3B-MoE-A570M解码器，通过光学2D映射技术实现视觉标记的高效压缩，在保持低激活值的同时获得高压缩比。

Result: 10倍压缩比下OCR精度达97%，20倍压缩比下精度约60%；在OmniDocBench上超越GOT-OCR2.0和MinerU2.0；单张A100-40G可日处理20万+页面生成LLM/VLM训练数据。

Conclusion: DeepSeek-OCR在长上下文压缩方面展现出巨大潜力，具有高实用价值，为相关研究领域提供了有前景的解决方案，代码和模型权重已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D物体分类中合成训练数据与真实稀疏、噪声LiDAR扫描之间的域差距问题，现有方法要么无法泛化到室外场景，要么缺乏语义多样性来识别罕见或未见物体。

Method: 提出生成大规模对象级三元组数据集（点云、图像、文本描述），并采用课程式数据混合策略，先在语义丰富的合成CAD数据上训练，然后逐步适应真实世界扫描特性。

Result: 仅需在每批次中引入1.5%的真实世界样本，就能在nuScenes基准上提升零样本准确率27%，在nuScenes和TruckScenes数据集上达到最先进性能，比先前最佳方法提升19.3%。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语义高斯溅射方法存在两个主要问题：(1) 预处理阶段单个掩码缺乏足够的上下文线索；(2) 融合多视图特征时存在不一致性和细节缺失。

Method: 包含两个模块：上下文感知特征提取（增强每个掩码的语义上下文）和注意力驱动的特征聚合（选择性融合多视图特征以减少对齐错误和不完整性）。

Result: 在基准数据集上的广泛实验表明，OpenInsGaussian在开放词汇3D高斯分割方面达到了最先进的性能，大幅优于现有基线方法。

Conclusion: 该方法展现了强大的鲁棒性和泛化能力，标志着3D场景理解及其在实际部署中的重大进展。
...
Method: 设计时间运动先验提取模块，分别从3D姿态序列和图像特征序列提取时间运动特征并组合；提出双曲空间优化学习策略，利用时间运动先验在双曲空间中分别优化学习3D姿态和姿态运动信息；设计双曲网格优化损失确保学习稳定有效。

Result: 在大型公开数据集上的广泛实验结果表明，该方法相比现有最先进方法具有优越性。

Conclusion: 通过结合时间运动先验和双曲空间学习，能够更准确地恢复具有层次结构的3D人体网格，产生更准确平滑的重建结果。
...
Method: 构建了包含15,003张高分辨率水下图像的UWBench数据集，涵盖海洋、珊瑚礁和深海栖息地等多种水生环境。每张图像都包含人工验证的标注，包括15,281个物体指代表达式和124,983个问答对，覆盖从物体识别到生态关系理解的各种推理能力。

Result: 基于UWBench建立了三个综合基准：详细图像描述、视觉定位和视觉问答。在最先进的视觉语言模型上的广泛实验表明，水下理解仍然具有挑战性，有显著的改进空间。
...
Method: 提出两种策略：1）视觉原型方法 - 为每个视觉类别学习一个视觉原型，用原型特征代替离散的视觉特征；2）中间嵌入空间优化 - 使用多层感知机框架学习共同的中间嵌入空间，使视觉数据结构更具区分性。

Result: 在四个基准数据集上的广泛实验评估表明，优化视觉空间对零样本学习有益，提出的基于原型的方法实现了新的最先进性能。

Conclusion: 优化视觉空间对零样本学习至关重要，视觉原型方法能够有效提升零样本学习的性能，达到了新的最优水平。
...
Method: 两阶段网络：第一阶段从图像特征的低频和高频分量中挖掘全局和局部信息，聚合为混合潜在频域特征；第二阶段利用混合潜在特征建模粗略3D人体网格模板与3D姿态的交互学习，通过降维和并行优化设计低维网格姿态交互方法。

Result: 在大型公开数据集上的广泛实验结果表明，该方法在重建精度上优于最先进的方法，同时显著降低了计算成本。

Conclusion: 提出的两阶段网络通过有效提取潜在信息和设计低维交互方法，成功解决了现有方法在复杂场景下的肢体错位和细节不足问题，并在保持高精度的同时大幅降低了计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 流式视频视觉语言模型面临两个基本约束：因果性（无法访问未来帧）和累积性（令牌无限增长导致效率瓶颈）。现有方法仅调控后LLM的kv缓存，而忽略了昂贵的预LLM预填充阶段。

Method: 提出两阶段框架：1）因果时间缩减：基于相邻帧变化和令牌显著性选择令牌，每帧只处理紧凑的视觉令牌子集；2）在线量化内存：以4位格式存储令牌，按需检索相关组并反量化，保持活动kv缓存有界。

Result: 实验显示该方法实现15.7倍kv缓存压缩，峰值内存降低1.2倍，TTFT加速2倍。在无需训练的方法中保持最先进精度，离线基准平均63.8%，RVS上55.8%/3.7。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D人脸身份保持编辑的挑战，包括多视角一致性、高分辨率编辑的灵活性，以及减少对大规模属性标记数据集的依赖。

Method: 利用3D感知深度生成模型和2D肖像编辑技术，通过少量标记图像估计潜在空间编辑方向，并使用现有面部数据集生成合成图像。

Result: 实验结果表明，仅需少量标记图像即可实现3D一致的身份保持面部编辑，包括连续风格流形探索和顺序编辑。

Conclusion: 该方法有效缓解了3D人脸编辑的约束，实现了高效的少样本身份保持属性编辑，展示了编辑方向的线性和连续性。
...
Conclusion: 提出的免训练解决方案能够无缝集成到现有DB-MDE框架中，有效解决了度量深度估计的尺度模糊问题，在各种环境中都表现出良好的泛化能力。
...
TL;DR: 提出了首个多模态医疗推理-检索框架Med-RwR，通过主动检索外部知识来增强多模态大语言模型的推理能力，在医疗基准测试中表现显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有医疗MLLMs仅依赖内部知识进行推理，当遇到超出训练范围的病例时会产生幻觉推理和事实错误。虽然最近出现了代理检索增强生成方法，但仅限于单模态LLMs，忽略了推理和检索过程中的关键视觉信息。

Method: 设计了两阶段强化学习策略，通过定制奖励刺激模型利用视觉诊断发现和文本临床信息进行有效检索。进一步提出了置信度驱动的图像重新检索方法，在检测到低预测置信度时进行测试时扩展。

Result: 在各种公共医疗基准测试中，Med-RwR相比基线模型有显著改进，证明了通过外部知识整合增强推理能力的有效性。在不熟悉领域表现出显著泛化能力，在提出的超声心动图基准上性能提升8.8%。

Conclusion: Med-RwR框架通过主动检索外部知识有效增强了多模态医疗大语言模型的推理能力，在医疗诊断任务中表现出优越性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学成像技术需要分析高分辨率图像，但现有MLLM大多针对通用数据集低分辨率图像设计，可能导致关键信息丢失，因此需要研究分辨率对生物医学MLLM性能的影响。
...
TL;DR: OmniNWM是一个全景导航世界模型，通过统一框架解决了自动驾驶世界模型在状态、动作和奖励三个维度的限制，实现了多模态全景视频生成、精确动作控制和基于3D占据的奖励定义。
...
Method: 提出OmniNWM模型：1）状态方面联合生成RGB、语义、度量深度和3D占据的全景视频；2）动作方面引入归一化全景Plucker射线图表示；3）奖励方面利用生成的3D占据直接定义基于规则的密集奖励。
...
TL;DR: 提出了自适应令牌集成解码（ATED），一种无需训练、基于令牌级的集成框架，通过在推理过程中聚合多个大型视觉语言模型的预测来减轻对象幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图像描述和视觉问答等任务中表现出色，但仍容易产生对象幻觉——描述不存在或错误识别的对象。现有方法在可扩展性、适应性和模型独立性方面存在挑战。

Method: ATED在推理过程中动态计算每个模型的不确定性权重，反映它们在每个解码步骤中的可靠性，并集成不同的解码路径以提高上下文基础和语义一致性。
...
<details>
  <summary>Details</summary>
Motivation: 灾难场景下的视觉识别面临数据稀缺、类内变化大和类间相似性高的挑战，现有Few-Shot学习方法在灾难遥感图像上效果有限。

Method: 使用Bhattacharyya系数增强类间可分性，Hellinger距离正则化同类对齐，提出基于概率分布的特征聚合网络和对比损失函数。

Result: 在四个FSL基准和两个灾难图像数据集上的实验表明，ATTBHFA-Net相比现有方法具有更好的有效性和泛化能力。

Conclusion: ATTBHFA-Net通过概率分布层面的特征聚合和对比学习，显著提升了灾难图像Few-Shot分类性能。
...
TL;DR: 本文提出了一个四阶段管道，用于解决自动驾驶中街景外推的挑战，在ICCV 2025 RealADSim Workshop NVS赛道中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 当前的新视角合成方法在超出原始轨迹时经常产生扭曲和不一致的图像，这对于自动驾驶闭环模拟中的真实视角外推至关重要。

Method: 采用四阶段管道：1) 数据驱动的初始化策略生成伪LiDAR点云；2) 使用降维SDF（2D-SDF）建模道路表面；3) 利用生成先验为外推视角创建伪真值；4) 数据驱动的适应网络去除时间特定伪影。

Result: 在RealADSim-NVS基准测试中获得了0.441的最终分数，在所有参与者中排名第一。

Conclusion: 该方法通过结合几何先验和生成先验，有效解决了街景外推的挑战，在基准测试中表现优异。
...
TL;DR: 提出了一种基于大规模网络数据的生成式预训练模型，用于面部知识学习，通过自监督任务训练，在多种面部下游任务中达到SOTA性能，并适用于广泛的面部编辑任务。


<details>
  <summary>Details</summary>
Motivation: 相比自然图像理解中预训练模型的繁荣，面部知识学习的大规模预训练研究仍有限。现有方法依赖人工标注数据集，标注成本高且模型可扩展性有限。

Method: 使用从互联网爬取的包含人脸的文本和图像，进行自监督预训练，包括掩码图像/语言建模和图文匹配。在生成阶段利用图文匹配损失将生成分布拉向控制信号。

Result: 实验结果表明，在属性分类、表情识别等面部下游任务中达到与SOTA预训练模型相当的性能，并适用于面部属性编辑、表情操控、口罩移除和照片修复等编辑任务。

Conclusion: 该方法成功利用大规模网络数据训练生成式预训练模型，在面部知识学习和编辑任务中表现出色，解决了人工标注数据集的局限性问题。
...
Method: 引入动态自适应焦点采样机制逐步聚焦与问题最相关的音视频片段；提出偏好感知策略独立建模每个模态的贡献；使用双路径对比损失强化时间和模态维度的一致性和互补性。

Result: 在四个大规模基准测试上的实验表明，AV-Master显著优于现有方法，尤其在复杂推理任务中表现突出。
...
TL;DR: 提出了Diffusion-DRO框架，通过逆强化学习将偏好学习转化为排序问题，解决了DPO方法在图像概率估计和离线数据多样性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的DPO方法虽然避免了REINFORCE算法提高了训练稳定性，但仍面临sigmoid函数的非线性导致图像概率估计不准确，以及离线数据集多样性有限的问题。

Method: 基于逆强化学习，将偏好学习转化为排序问题，消除了对奖励模型的依赖；结合离线专家演示和在线策略生成的负样本，解决了离线数据的局限性。

Result: 在多个具有挑战性和未见过的提示上实现了更好的生成质量，在定量指标和用户研究中均优于现有最优基线方法。

Conclusion: Diffusion-DRO通过简化的去噪公式和混合数据策略，有效克服了先前方法的非线性估计问题，能够更好地捕捉人类偏好。
...
TL;DR: GroupHOI是一个从群体视角重新思考关系建模的框架，通过几何邻近性和语义相似性来传播上下文信息，解决了现有方法主要关注成对关系而忽略集体行为的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互检测方法主要通过自注意力机制传播信息或使用二分图建立人-物之间的消息传递，但主要关注成对关系，忽略了现实场景中交互往往来自集体行为（多个人和物体参与联合活动）。

Method: 1. 使用基于边界框空间特征的可学习邻近性估计器将人和物体分组为不同的簇；2. 在每个组内通过自注意力计算软对应关系来聚合和分发上下文线索；3. 用HO对特征中的局部上下文线索增强基于Transformer的交互解码器以融入语义相似性。

Result: 在HICO-DET和V-COCO基准测试上的广泛实验证明了GroupHOI相对于最先进方法的优越性。在更具挑战性的非语言交互检测（NVI-DET）任务上也表现出领先性能。

Conclusion: GroupHOI通过从群体视角建模关系，有效处理了集体行为中的交互检测问题，在多个基准测试中取得了最先进的性能。
...
TL;DR: FeatureFool是一种针对视频模型的零查询黑盒攻击方法，通过利用DNN提取的特征信息直接改变干净视频的特征空间，无需多次查询交互。
...
Result: 实验显示FeatureFool对传统视频分类器的攻击成功率超过70%且无需任何查询。利用特征图的可迁移性，还能制作有害内容并绕过Video-LLM识别。生成的对抗视频在SSIM、PSNR和时间一致性方面质量高，攻击几乎不可察觉。

Conclusion: FeatureFool提供了一种高效、隐蔽的视频域零查询黑盒攻击方法，在保持高攻击成功率的同时实现了良好的视觉质量。
...
TL;DR: 提出了一种新颖的图像复杂度评估方法CM-SSA，通过跨模态场景语义对齐来提升ICA性能，使复杂度预测更符合人类主观感知。


<details>
  <summary>Details</summary>
Motivation: 现有ICA方法主要依赖单视觉模态的手工或浅层CNN特征，无法充分捕捉与图像复杂度相关的感知表示。跨模态场景语义信息在感知理解任务中起关键作用，但在ICA领域尚未被探索。

Method: 提出CM-SSA框架，包含复杂度回归分支和场景语义对齐分支。回归分支在语义对齐分支的指导下估计图像复杂度，语义对齐分支通过成对学习将图像与传达丰富场景语义信息的文本提示对齐。
...
<details>
  <summary>Details</summary>
Motivation: 传统对抗性剪枝方法在分数空间优化中容易陷入尖锐局部最小值，导致掩码选择不稳定，降低模型鲁棒性
...
Conclusion: S2AP方法通过分数空间锐度最小化成功解决了对抗性剪枝中的掩码选择不稳定问题，显著提升了剪枝模型的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病与显著的皮质萎缩相关，几何表面模型对于分析解剖结构的3D形状至关重要，需要开发新的局部表面表示方法来进行准确的自动化诊断。

Method: 使用ADNI的160名参与者T1加权MRI数据，通过Freesurfer重建皮质表面模型。计算面积畸变、共形因子和高斯曲率等几何属性，应用香农熵创建紧凑的特征向量，并用多种分类器进行训练评估。

Result: 方法在区分AD患者和健康对照方面非常有效，多层感知机和逻辑回归分类器表现最佳，准确率和F1分数均达到98.62%。

Conclusion: 共形几何特征的熵为皮质形态测量提供了强大而稳健的度量，高分类准确率表明该方法有潜力增强阿尔茨海默病的研究和诊断，为临床研究应用提供简单而强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于张量分解的融合方法存在数据向量化破坏空间-光谱结构、因子张量排列约束严格、需要大量人工参数调优、对噪声和空间退化鲁棒性差等问题。

Method: 提出贝叶斯FCTN方法，使用层次稀疏先验连接因子张量，显式建模空间结构、光谱特征和局部场景同质性的内在物理耦合，基于变分贝叶斯推理和EM算法进行参数估计。

Result: 大量实验表明BFCTN实现了最先进的融合精度和强鲁棒性，在复杂实际场景中具有实用适用性。

Conclusion: BFCTN方法有效解决了现有张量分解融合方法的局限性，在保持空间-光谱结构的同时显著减少了人工参数调优需求，并在实际应用中表现出色。
...
TL;DR: 基于YOLOv8和OCR的板球视频分析系统，能自动检测三柱门投球、识别板球并建模轨迹，为教练决策提供数据支持。


<details>
  <summary>Details</summary>
Motivation: 传统板球视频分析依赖人工，效率低且主观性强。需要自动化系统来提取关键投球信息、检测球体并分析轨迹，为战术分析提供客观数据。

Method: 使用YOLOv8架构进行球场和球体检测，结合OCR技术从视频帧中提取记分卡信息识别三柱门时刻。采用灰度变换、幂变换和形态学操作等图像预处理技术。

Result: 球场检测模型达到99.5% mAP50精度和0.999精确率；球体检测模型通过迁移学习达到99.18% mAP50、0.968精确率和0.978召回率。系统能在检测到的球场上进行轨迹建模。

Conclusion: 该系统在多个板球比赛视频中验证有效，为自动化板球分析提供了可行方案，在教练指导和战略决策方面具有重要应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模ViT模型训练计算成本高的问题，提供一种基于预训练模型的高效扩展方案。

Method: 在预训练ViT中插入附加层，采用层间权重共享机制，并通过并行适配器模块引入调整参数来优化共享权重性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的伪装目标检测方法主要依赖图像级建模或基于标注的优化，难以利用数据集级别的上下文信息，且依赖费时的人工标注。

Method: 提出RISE方法：1) 使用无标注训练图像构建环境和伪装对象的原型库；2) 引入聚类-检索策略生成高质量原型；3) 采用多视图KNN检索生成鲁棒的伪掩码。

Result: 大量实验表明，RISE在无监督和基于提示的方法中表现优于现有最优方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成高质量3D胸部CT扫描的计算成本过高，需要开发更高效的方法来支持AI模型训练和医疗专业人员培训。

Method: 使用潜在扩散模型，以3D解剖掩码（肺部和结节区域）为条件，在单个中端GPU上生成256x256x256体积的图像。

Result: 实验表明仅使用结节掩码会导致解剖结构错误，必须结合全局肺结构才能实现准确的合成。模型能够生成具有不同属性结节或无结节的多样化CT体积。

Conclusion: 该方法为训练AI模型和医疗专业人员提供了有价值的工具，通过降低计算成本实现了高质量的3D胸部CT合成。
...
TL;DR: 提出VFM-VAE方法，通过直接集成视觉基础模型来改进潜在扩散模型的视觉分词器，避免了蒸馏方法的语义偏差问题，实现了更高质量和效率的图像生成。
...
Result: 在80个周期内达到gFID 2.20（10倍加速），640个周期后达到gFID 1.62，显著优于现有方法。

Conclusion: 直接VFM集成是潜在扩散模型的更优范式，提供了更高质量和效率的图像生成能力。
...
TL;DR: Mono4DGS-HDR是首个从无位姿单目LDR视频重建可渲染4D HDR场景的系统，采用两阶段高斯溅射优化方法，无需相机位姿即可实现HDR视频重建。
...
Method: 基于高斯溅射的两阶段优化框架：第一阶段在正交相机坐标系学习视频HDR高斯表示；第二阶段将视频高斯变换到世界坐标系，联合优化世界高斯和相机位姿，并引入时间亮度正则化策略增强HDR外观的时间一致性。

Result: 在构建的新评估基准上，Mono4DGS-HDR在渲染质量和速度上显著优于从最先进方法改编的替代方案。

Conclusion: 提出的Mono4DGS-HDR系统成功解决了从无位姿单目LDR视频重建4D HDR场景的挑战，为这一新任务提供了有效的解决方案。
...
TL;DR: 提出了一种结合视觉语言模型和检索增强生成的方法，实现零样本车辆品牌型号识别，无需大规模重新训练即可适应新车型
...
Result: 相比CLIP基线方法，识别性能提升了近20%，展示了RAG增强语言模型推理在可扩展车辆识别中的潜力

Conclusion: 该方法避免了大规模重新训练，通过添加新车型的文本描述即可快速更新，适用于智慧城市应用中的可扩展车辆识别
...
TL;DR: 开发了DWaste平台，在资源受限设备上实现实时垃圾分类，比较了多种模型性能，发现轻量级目标检测模型在速度、能耗和模型大小方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 便利包装导致大量垃圾产生，需要高效垃圾分类系统来支持可持续垃圾管理。

Method: 开发DWaste计算机视觉平台，测试多种图像分类和目标检测模型，使用自定义标注工具处理垃圾数据集，并进行模型量化优化。

Result: EfficientNetV2S分类器准确率96%但延迟高，YOLO目标检测模型mAP达77%且推理速度快(0.03s)，模型量化可减少75%模型大小和VRAM使用。

Conclusion: 轻量级目标检测模型结合量化技术适合边缘设备的实时可持续垃圾分类，实现了"更环保AI"模型的实际应用。
...
TL;DR: 将基于模板的物体姿态估计重新表述为射线对齐问题，使用扩散变换器架构对齐查询图像与姿态模板，通过粗到细训练策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统模板方法在检索错误模板时会导致姿态预测不准确，需要更鲁棒的方法来处理模板匹配失败的情况。

Method: 将物体姿态估计重新定义为射线对齐问题，使用扩散变换器架构，重新参数化物体旋转为物体中心相机射线，扩展尺度不变平移估计为密集平移偏移，采用粗到细训练策略。

Result: 在多个基准数据集上的广泛实验表明，该方法在未见物体姿态估计方面与最先进方法相比具有竞争力。

Conclusion: 通过射线对齐的重新表述和扩散变换器架构，实现了鲁棒的物体姿态估计，在模板匹配失败时仍能保持准确预测。
...
Method: 采用GBlobs作为网络输入特征，这是一种专门设计的局部点云特征描述符，能够有效规避几何捷径，迫使网络学习鲁棒的对象中心表示。

Result: 该方法显著提升了模型的泛化能力，在挑战中表现出色，实现了最先进的性能。

Conclusion: GBlobs特征描述符通过避免几何捷径，使3D目标检测模型能够更好地适应不同的LiDAR传感器配置，具有重要的实用价值。
...
TL;DR: 提出了Occluded nuScenes数据集，这是nuScenes基准测试的扩展，提供可控、参数化的多传感器遮挡数据，用于评估自动驾驶感知系统在恶劣条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集虽然包含传感器噪声和环境变化，但缺乏可控、参数化且可重复的多模态传感器退化数据，限制了系统评估感知和融合架构在明确定义恶劣条件下性能的能力。

Method: 为相机模态提供完整版和迷你版数据集，包含四种遮挡类型（两种来自公共实现，两种新设计）；为雷达和LiDAR提供参数化遮挡脚本，每种实现三种退化类型，支持灵活可重复生成遮挡数据。
...
TL;DR: Kaleido是一个主题到视频生成框架，通过专门的数据构建管道和参考旋转位置编码，解决了多主题一致性和背景分离问题，显著提升了参考保真度和语义一致性。
...
Method: 提出专门的数据构建管道，包含低质量样本过滤和多样化数据合成，以生成保持一致性的训练数据；引入参考旋转位置编码来处理参考图像，实现稳定精确的多图像集成。

Result: 在多个基准测试上的广泛实验表明，Kaleido在一致性、保真度和泛化能力方面显著优于先前的方法。

Conclusion: Kaleido在主题到视频生成方面取得了重要进展，通过改进的数据构建和参考图像处理机制，有效解决了多主题一致性和背景分离的挑战。
...
TL;DR: 提出CovMatch框架用于多模态数据集蒸馏，通过跨协方差对齐和特征分布正则化，实现图像和文本编码器的联合优化，显著提升跨模态对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法为降低计算成本而冻结文本编码器，但这严重限制了语义对齐能力，成为性能扩展的瓶颈。

Method: CovMatch框架通过对齐真实和合成特征的跨协方差，并在每个模态内正则化特征分布，实现图像和文本编码器的联合优化。

Result: 在Flickr30K和COCO数据集上，CovMatch优于现有多模态蒸馏方法，仅使用500个合成对就能在检索准确率上获得最高6.8%的绝对提升。

Conclusion: CovMatch通过跨协方差对齐和联合优化策略，有效解决了多模态数据集蒸馏中的跨模态对齐挑战，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在理解3D空间关系方面存在局限，特别是从有限视角理解3D空间时，纯文本或2D视觉线索的表示能力不足。

Method: 采用两阶段训练：首先通过监督训练将VLM生成的3D潜在表示与3D基础模型对齐，然后基于结果信号优化整个推理轨迹来改进3D心理建模。

Result: 在多个基准测试中，3DThinker始终优于强基线方法，为将3D表示统一到多模态推理提供了新视角。

Conclusion: 3DThinker框架首次实现了无需3D先验输入的3D心理建模推理，有效解决了3D空间想象力任务中的挑战。
...
TL;DR: 提出一种基于可解释深度学习的新型一次性剪枝框架，通过因果感知方法在训练后直接剪枝，无需微调即可显著减小模型尺寸且保持性能。
...
Method: 采用因果感知剪枝方法，利用模型预测与结构间的因果关系进行渐进式剪枝，确保移除的结构不影响模型性能。
...
TL;DR: ε-Seg是一种基于分层变分自编码器的语义分割方法，通过中心区域掩码、稀疏标签对比学习、高斯混合模型先验和无聚类标签预测，在仅有少量标注数据（0.05%或更少）的情况下实现生物显微镜图像的准确分割。


<details>
  <summary>Details</summary>
Motivation: 生物样本的电子显微镜图像语义分割在生命科学中仍具挑战性，数据复杂度高，即使人类观察者也难以处理。需要开发能够在极稀疏标注下有效学习的方法。

Method: 使用分层变分自编码器，结合中心区域掩码和修复损失学习鲁棒嵌入，通过对比学习和高斯混合模型先验塑造潜在空间，最后用MLP分割头直接预测类别标签而非聚类。

Result: 在2个密集电子显微镜数据集和荧光显微镜数据上，ε-Seg在稀疏监督下实现了具有竞争力的分割结果，即使训练标签数量非常有限。

Conclusion: ε-Seg方法能够在极稀疏标注条件下有效分割复杂的生物图像数据，为生命科学研究提供了实用的语义分割工具。
...
TL;DR: 提出了一种新的矩阵量化方法BQQ，使用二进制二次表达式进行矩阵近似，在矩阵压缩和神经网络后训练量化中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统的一阶量化方法使用二进制基的线性组合来近似实值矩阵，表达能力有限。BQQ旨在利用二进制二次表达式的更强表达能力，同时保持紧凑的数据格式。

Method: BQQ方法采用二进制二次表达式进行矩阵量化，不同于传统的均匀量化和二进制编码量化。该方法不依赖PTQ特定的二进制矩阵优化，而是通过二进制二次表达实现高效矩阵近似。

Result: 在矩阵压缩基准测试中，BQQ在内存效率和重构误差之间实现了更好的权衡。在PTQ实验中，在ImageNet数据集上，2位量化下比最先进方法提升2.2%（基于校准）和59.1%（无数据场景）。

Conclusion: 二进制二次表达式在高效矩阵近似和神经网络压缩方面具有显著效果，BQQ方法在压缩性能和量化精度方面均优于传统方法。
...
Method: 使用可逆网络将非线性输入空间映射到潜在空间进行线性增强，然后映射回输入空间，结合问答模型迭代更新变化检测模型。

Result: 实验结果表明该方法在性能上优于相关工作。

Conclusion: 提出的可逆网络增强方法有效提升了主动学习在卫星图像变化检测中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 历史笔迹识别因笔迹风格多样、文档退化及标注样本有限而具有挑战性。传统方法依赖手工特征，而端到端方法直接从图像学习特征，但在实际文档级场景中泛化能力不足。

Method: 探索了不同的预处理方法、骨干架构和后处理策略组合，包括文本分割、补丁采样和特征聚合。

Result: 大多数配置因低层视觉特征捕获弱、补丁表示不一致和对内容噪声敏感而表现不佳，但发现一种端到端设置能达到顶尖系统性能。

Conclusion: 研究指出了构建鲁棒端到端系统的关键挑战，并为历史文档笔迹识别的设计选择提供了见解。
...
TL;DR: 提出Mixture-of-Groups Attention (MoGA)稀疏注意力机制，通过轻量级可学习token路由器精确匹配token，解决了DiT长视频生成中注意力二次复杂度的问题，实现了分钟级480p视频生成。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在长视频生成中面临注意力机制二次复杂度瓶颈，现有稀疏方法依赖块级粗粒度估计，精度-效率权衡受限于块大小。

Method: 提出MoGA稀疏注意力机制，使用轻量级可学习token路由器进行语义感知路由，无需块级估计即可精确匹配token，支持FlashAttention和序列并行化。

Result: 基于MoGA开发了高效长视频生成模型，端到端生成长度约580k上下文、分钟级、多镜头、480p、24fps的视频，在各种视频生成任务上验证了有效性。

Conclusion: MoGA通过语义感知路由实现了有效的长距离交互，解决了DiT长视频生成的注意力瓶颈，为高效长视频生成提供了可行方案。
...
TL;DR: 提出了UniGenBench++，一个统一的文本到图像生成语义评估基准，包含600个分层组织的提示，涵盖5个主要主题和20个子主题，评估10个主要维度和27个子维度，支持中英文双语和长短提示形式。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏提示场景多样性和多语言支持，评估维度粗糙且覆盖范围有限，无法进行细粒度的子维度评估。

Method: 使用分层组织的600个提示，涵盖多样化的真实场景，通过闭源多模态大语言模型Gemini-2.5-Pro构建可靠基准和简化模型评估流程，并训练离线评估模型。

Result: 通过对开源和闭源T2I模型进行全面基准测试，系统揭示了它们在各个方面的优势和弱点。

Conclusion: UniGenBench++提供了一个全面、细粒度的T2I生成语义评估基准，解决了现有基准的局限性，促进了T2I模型的可靠评估。
...
TL;DR: VC2L是一个统一的视觉中心对比学习框架，将所有输入（文本、图像及其组合）都渲染为图像，在像素空间中进行处理，无需OCR、文本标记化或模态融合策略。


<details>
  <summary>Details</summary>
Motivation: 解决现有对比视觉语言模型在处理复杂网页文档时的局限性，特别是当文本和图像交错排列、松散对齐或以视觉形式嵌入时。
...
TL;DR: PLANA3R是一个无需相机位姿的度量3D重建框架，从无位姿的双视角图像重建室内场景的平面3D结构，无需3D平面标注即可学习平面几何结构。


<details>
  <summary>Details</summary>
Motivation: 利用室内场景固有的几何规律性，通过紧凑的平面3D基元表示来改进度量3D重建，避免对3D平面标注的依赖。

Method: 使用Vision Transformers提取稀疏平面基元，估计相对相机位姿，通过平面splatting进行几何学习监督，梯度通过高分辨率渲染的深度和法线图传播。

Result: 在多个室内场景数据集上验证了方法的有效性，展示了在域外室内环境中的强泛化能力，支持3D表面重建、深度估计和相对位姿估计等任务。

Conclusion: PLANA3R通过平面3D表示实现了无需平面监督的度量重建，具有准确的平面分割能力，可在大型立体数据集上进行可扩展训练。
...
TL;DR: 提出了一种针对自回归图像生成模型的KV缓存压缩框架，通过识别空间局部性和语义汇注意力现象，自适应地将注意力头分为两种类型进行压缩，显著降低内存使用并提升处理速度。
...
Method: 识别空间局部性和语义汇注意力现象，将注意力头自适应分为两类：对空间局部性头维护短近期标记窗口，对语义汇头策略性保留高度关注标记的紧凑集合。
...
TL;DR: 提出了IF-VidCap基准，用于评估可控视频字幕生成，包含1400个高质量样本，评估字幕的格式正确性和内容正确性。评估了20多个模型，发现专有模型仍占主导但开源模型差距在缩小，密集字幕专用模型在复杂指令下表现不如通用MLLMs。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估描述性全面性，而忽略了指令跟随能力。实际应用需要遵循特定用户指令的字幕，而非生成详尽无约束的描述。

Method: 引入IF-VidCap基准，包含系统化框架，从格式正确性和内容正确性两个维度评估字幕。对20多个知名模型进行全面评估。

Result: 专有模型仍占主导地位，但性能差距正在缩小，顶级开源解决方案已接近同等水平。密集字幕专用模型在复杂指令下表现不如通用MLLMs。
...
TL;DR: ColIAGS是一个改进的3D高斯泼溅框架，专门针对结肠镜检查场景，通过改进的外观建模和几何建模来解决动态光照下的渲染问题，在保持几何精度的同时适应光度变化。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS假设静态光照，而结肠镜场景存在动态光源/相机引起的光度变化，导致需要引入违反结构的雾状高斯斑点来补偿光照衰减，从而降低3D重建质量。

Method: 提出改进的外观建模，包含两种光照衰减因子；改进的几何建模使用高维视图嵌入增强高斯几何属性预测；利用余弦嵌入隐式生成光照衰减解决方案。

Result: 在标准基准测试中，ColIAGS实现了新颖视图合成和准确几何重建的双重能力，在渲染保真度方面显著优于其他最先进方法，同时大幅降低深度MSE。

Conclusion: ColIAGS通过物理准确的光照建模和几何增强，有效解决了结肠镜场景中的动态光照问题，为虚拟结肠镜和病变跟踪等应用提供了高质量的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有广义类别发现方法依赖单层语义或人工设计的抽象层次结构，限制了泛化性和可扩展性，需要更自然和可扩展的层次学习方法。
...
Conclusion: SEAL通过利用自然层次结构和改进的对比学习方法，有效解决了广义类别发现问题，在多个数据集上表现出色。
...
TL;DR: 该研究使用地理空间基础模型预测城市热岛效应，通过微调模型预测未来气候情景下的地表温度，并展示其在数据稀缺地区评估缓解策略的实用价值。


<details>
  <summary>Details</summary>
Motivation: 随着城市化和气候变化加剧，城市热岛效应日益严重。传统机器学习模型在数据有限时预测不准确，特别是在服务不足地区。地理空间基础模型提供了一种有前景的替代方案。

Method: 建立城市热模式的实证地面真实数据，量化绿地的降温效应并与模型预测进行基准测试。对基础模型进行微调以预测未来气候情景下的地表温度，并通过模拟修复展示其实用价值。

Result: 结果表明基础模型能够准确预测城市热岛效应，在数据稀缺地区为评估缓解策略提供了强大工具。

Conclusion: 地理空间基础模型为评估城市热岛缓解策略提供了有效方法，有助于建设更具气候韧性的城市。
...
Method: 采用分层双分支注意力架构，将完整注意力解耦为局部注意力分支（用于高保真区域内容）和全局注意力分支（用于整体语义一致性），并提出空间压缩全局建模策略和分层跨窗口局部注意力机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能实现通用部署，但在光谱和空间线索的整合方面仍有改进空间。传统师生范式强调模仿，而本文旨在让空间分支学习教师无法捕获的互补空间模式。

Method: 采用两阶段学习策略：1)通过反向蒸馏训练光谱增强网络获得稳健背景光谱表示；2)使用去相关损失优化空间网络（叛逆学生），强制特征正交性同时保持重建保真度。

Result: 在HAD100基准测试中相比多个基线方法有显著提升，计算开销极小，验证了互补学习范式的有效性和通用性。

Conclusion: 提出的叛逆学生框架成功实现了光谱和空间特征的互补学习，为通用高光谱异常检测提供了高效且鲁棒的解决方案。
...
TL;DR: ProCLIP是一个基于课程学习的渐进式视觉-语言对齐框架，旨在将CLIP图像编码器与基于LLM的嵌入器有效对齐，解决CLIP文本编码器在长文本处理、多语言理解和细粒度语义理解方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 原始CLIP文本编码器存在77个token的最大输入长度限制，无法有效处理长文本和进行细粒度语义理解，且不支持多语言输入。虽然已有研究尝试用基于LLM的嵌入器替换CLIP文本编码器，但由于LLM表示空间与CLIP视觉-语言空间独立预训练缺乏对齐先验，直接对齐会破坏CLIP图像编码器的内在视觉-语言对齐。

Method: ProCLIP采用课程学习策略：首先从CLIP文本编码器向LLM嵌入器蒸馏知识，建立初始对齐；然后通过图像-文本对比调优进一步对齐CLIP图像编码器与LLM嵌入器，使用自蒸馏正则化防止过拟合；在表示继承和对比调优阶段采用实例语义对齐损失和嵌入结构对齐损失。

Result: 该方法有效解决了LLM与CLIP表示空间对齐的挑战，充分利用了CLIP预训练知识，避免了视觉-语言对齐的破坏。

Conclusion: ProCLIP框架成功实现了CLIP图像编码器与LLM嵌入器的渐进式对齐，提升了模型在长文本处理、多语言理解和细粒度语义理解方面的能力。
...
TL;DR: 本文提供了一种基于几何论证和模式匹配基本原理的直观推导方法，用于构建d维可操纵卷积神经网络，并提出了使用插值核的新方法来改进现有实现。
...
Method: 基于几何论证和模式匹配基本原理推导可操纵CNN；使用插值核构造可操纵卷积层，提高对噪声数据的鲁棒性。

Result: 提出了更直观的推导方法，解释了群论方法中的数学结构；新实现的卷积层在噪声数据上表现更稳健。

Conclusion: 几何论证为可操纵CNN提供了更直观的理解框架，插值核方法改进了实现效果，特别是在处理噪声数据时表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 结核病是全球健康问题，尤其在资源有限地区。早期检测至关重要，但缺乏专业放射科医生，需要AI驱动的筛查工具。开发可靠AI模型面临高质量数据集获取成本高的挑战。

Method: 采用教师-学生框架，集成两个监督头（用于疾病分类和症状检测）和一个自监督头，通过多任务学习增强模型性能。

Result: 模型在COVID-19、结核病和正常病例分类中达到98.85%准确率，在症状检测中达到90.09%的宏F1分数，显著优于基线方法。可解释性评估显示模型基于相关解剖特征进行预测。
...
TL;DR: SAM 2++是一个统一的多粒度视频跟踪模型，支持掩码、边界框和点级别的跟踪，通过任务特定提示、统一解码器和任务自适应内存机制实现跨任务的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪器通常针对单一任务设计，依赖定制化模块，导致泛化能力有限、模型冗余。需要开发能够处理不同粒度目标状态的统一跟踪框架。

Method: 1. 设计任务特定提示将不同任务输入编码为通用提示嵌入；2. 使用统一解码器将多样化任务结果统一为预输出形式；3. 引入任务自适应内存机制统一不同粒度的内存匹配；4. 构建Tracking-Any-Granularity数据集支持多粒度跟踪训练。

Result: 在多个基准测试上的综合实验表明，SAM 2++在不同粒度的多样化跟踪任务中均达到了新的最先进水平。

Conclusion: SAM 2++建立了一个统一且鲁棒的跟踪框架，能够在不同粒度上实现高性能的视频跟踪，解决了现有方法的泛化限制和冗余问题。
...
TL;DR: 提出了统一的层次化掩码框架，揭示模型架构与注意力掩码构建的等价性，并基于此设计了M3Dphormer图变换器，通过多级掩码和双注意力计算实现高效可扩展的图表示学习。
...
Method: 提出层次化掩码框架，将模型架构等价于注意力掩码构建。设计M3Dphormer，包含三种理论驱动的层次掩码、双层专家路由机制和基于局部掩码稀疏度的双注意力计算方案。

Result: 在多个基准测试中达到最先进性能，验证了统一框架和模型设计的有效性。

Conclusion: 层次化掩码框架为图变换器提供了统一建模范式，M3Dphormer通过自适应集成多级交互信息实现了高效可扩展的图表示学习。
...
Method: 1) 使用语义和领域转换网络解耦图像中的语义和领域特定特征；2) 引入全局语义提示和局部领域提示的双提示设计；3) 在两个学习到的变换下对齐文本和视觉表示以保持语义和领域一致性。

Result: 在四个数据集上的理论分析和广泛实验证明了该方法在增强CLIP模型在多领域联邦图像识别中的泛化能力的有效性。

Conclusion: FedDEAP框架通过特征解耦、双提示设计和表示对齐，成功提升了CLIP模型在联邦多领域场景下的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 挑战传统子词分词范式，因为该方法在处理低资源语言时会过度分割文本，产生冗长且无语言学意义的序列，增加计算负担。

Method: 将文本渲染为图像（视觉文本），利用预训练的多模态LLM的OCR和文本-视觉对齐能力来解读这些图像。

Result: 在三个不同语言任务中，SeeTok匹配或超越了子词分词器，同时减少了4.43倍的token数量和70.5%的FLOPs，在跨语言泛化、抗噪能力和语言层次结构方面表现更优。

Conclusion: SeeTok标志着从符号化分词向人类式视觉阅读的转变，朝着更自然、认知启发的语言模型迈出了一步。
...
TL;DR: DP²O-SR是一个用于真实图像超分辨率的感知偏好优化框架，利用T2I模型的随机性生成多样化输出，通过混合质量评估奖励信号实现无需人工标注的感知偏好对齐。


<details>
  <summary>Details</summary>
Motivation: T2I扩散模型的随机性虽然导致输出质量波动，但也提供了更广的感知质量范围，可以利用这种多样性来提升真实图像超分辨率性能。

Method: 构建结合全参考和无参考图像质量评估的混合奖励信号，从同一模型输出构建多组偏好对，提出分层偏好优化方法，根据模型容量自适应调整选择策略。

Result: 在扩散和流式T2I骨干网络上的大量实验表明，DP²O-SR显著提升了感知质量，并在真实世界基准上表现出良好的泛化能力。

Conclusion: 通过利用T2I模型的感知多样性并设计有效的偏好优化策略，可以在无需人工标注的情况下显著提升真实图像超分辨率的感知质量。
...
Method: 构建DSI-Bench基准，包含九个解耦的运动模式，采用空间和时间对称设计减少偏见，系统评估模型对自身运动和物体运动的推理能力。

Result: 评估了14个VLMs和专家模型，发现主要局限：模型经常混淆观察者和物体运动，存在语义偏见，在动态场景中无法准确推断相对关系。
...
TL;DR: GAR是一个用于区域级视觉理解的多模态大语言模型，通过RoI对齐特征重放技术，能够利用全局上下文进行精确感知，建模多个提示间的交互，实现高级组合推理。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在理解复杂场景时存在困难，而区域级MLLMs通常孤立理解给定区域，忽略了关键的全局上下文信息。

Method: 引入RoI对齐的特征重放技术，支持利用必要的全局上下文进行精确感知，并建模多个提示之间的交互关系。

Result: GAR-1B在DLC-Bench上超越DAM-3B +4.5分，在GAR-Bench-VQA上甚至超过InternVL3-78B。零样本GAR-8B在VideoRefer-BenchQ上优于领域内VideoRefer-7B。

Conclusion: GAR通过结合全局上下文和区域交互建模，实现了从被动描述到主动对话的范式转变，展现出强大的区域理解和组合推理能力，并能轻松迁移到视频领域。
...
### [100] [Does GenAI Rewrite How We Write? An Empirical Study on Two-Million Preprints](https://arxiv.org/abs/2510.17882)
*Minfeng Qi,Zhongmin Cao,Qin Wang,Ningran Li,Tianqing Zhu*

Main category: cs.CY

TL;DR: 本文通过分析210万篇预印本，发现LLMs加速了提交和修订周期，略微增加了语言复杂性，并显著扩展了AI相关主题，计算密集型领域受益更多。LLMs更多是选择性催化剂而非普遍颠覆者。


<details>
  <summary>Details</summary>
Motivation: 预印本库已成为学术交流的核心基础设施，生成式大语言模型可能改变论文写作方式，但关于LLMs如何重塑科学出版的系统性证据有限。

Method: 使用多级分析框架，整合中断时间序列模型、协作和生产力指标、语言特征分析和主题建模，分析2016-2025年四个主要预印本库的210万篇预印本。

Result: LLMs加速了提交和修订周期，略微增加了语言复杂性，不成比例地扩展了AI相关主题，计算密集型领域受益更多。

Conclusion: LLMs更多是选择性催化剂而非普遍颠覆者，放大了现有优势并扩大了学科鸿沟，需要治理框架来维护AI赋能研究生态系统中的信任、公平和问责。
...
### [101] [Are LLMs Court-Ready? Evaluating Frontier Models on Indian Legal Reasoning](https://arxiv.org/abs/2510.17900)
*Kush Juvekar,Arghya Bhattacharya,Sai Khadloya,Utkarsh Saxena*

Main category: cs.CY

TL;DR: 该研究创建了印度首个基于法律考试的LLM评估框架，使用公开法律考试作为基准，评估LLM在印度法律环境中的能力表现。


<details>
  <summary>Details</summary>
Motivation: 缺乏针对特定司法管辖区的LLM法律能力评估框架，需要建立印度本土化的评估标准来衡量LLM在法律工作流程中的基础能力。

Method: 使用印度公开法律考试作为透明代理，构建多年基准测试，包括客观题和律师评分的长篇答案评估，在真实考试条件下测试前沿LLM。

Result: 前沿系统在客观考试中能通过历史分数线并匹配或超过近期高分考生，但在长篇推理方面无人超越人类顶尖考生。发现三个可靠性失败模式：程序/格式合规性、权威/引用规范、法庭适当语气和结构。

Conclusion: LLM可在检查、跨法规一致性、法规和判例查找方面提供协助，但在法庭特定起草和提交、程序和救济策略、协调权威和例外、以及伦理和问责判断方面仍需人类主导。
...
### [102] [Interpretability Framework for LLMs in Undergraduate Calculus](https://arxiv.org/abs/2510.17910)
*Sagnik Dakshit,Sushmita Sinha Roy*

Main category: cs.CY

TL;DR: 提出了一种用于分析LLM在数学教育中推理过程的解释性框架，通过微积分问题评估模型解决方案的质量、可靠性和教学有效性。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法主要关注最终答案准确性，而忽视了推理过程的质量、可靠性和教学有效性，特别是在需要多步逻辑、符号推理和概念清晰度的数学领域。

Method: 结合推理流程提取、语义标记操作分解和提示消融分析，使用推理复杂性、短语敏感性和鲁棒性等结构化指标评估模型行为。
...
### [103] [Attracting Commercial Artificial Intelligence Firms to Support National Security through Collaborative Contracts](https://arxiv.org/abs/2510.17931)
*Andrew Bowne*

Main category: cs.CY

TL;DR: 商业AI公司对国防部作为客户感兴趣，但传统合同法和采购框架是主要障碍。研究提出最优买家理论框架，建议利用其他交易授权等现有合同法律来匹配商业偏好和机器学习开发周期。
...
Method: 基于社会交换理论提出最优买家理论框架，通过对参与者的访谈来理解AI行业对合同和国防部作为客户的看法、意见和偏好。

Result: 商业AI公司被符合其业务和技术考量的合同所吸引，国防部被视为有吸引力的客户，但传统合同实践构成障碍。

Conclusion: 需要利用现有合同法律（主要是其他交易授权）来调整合同实践，使其与商业偏好和机器学习开发部署周期保持一致。
...
### [104] [The Integration of Artificial Intelligence in Undergraduate Medical Education in Spain: Descriptive Analysis and International Perspectives](https://arxiv.org/abs/2510.17938)
...
TL;DR: 西班牙医学院AI课程整合现状调查显示，仅有19.2%的大学开设AI相关课程，69.2%的大学无相关内容，课程多为选修且学分占比低（平均1.17%），存在明显的地区差异。


<details>
  <summary>Details</summary>
Motivation: 评估西班牙医学院课程中AI内容的整合情况，了解未来医疗专业人员在AI时代所需能力的培养现状。

Method: 横断面研究，分析西班牙52所医学院2025-2026学年的课程设置和机构文件，使用描述性统计方法。

Result: 52所大学中仅10所（19.2%）开设AI课程，36所（69.2%）无相关内容；课程多为选修（仅哈恩大学有必修课），学分占比低；安达卢西亚地区整合率最高（55.5%），部分地区完全缺失。

Conclusion: 西班牙医学院AI整合处于起步阶段，零散且不均衡，有限的培训内容和选修性质限制了未来医生在AI主导的医疗环境中的准备，建议建立最低标准和全国监测指标。
...
### [105] [Trust in foundation models and GenAI: A geographic perspective](https://arxiv.org/abs/2510.17942)
*Grant McKenzie,Krzysztof Janowicz,Carsten Kessler*

Main category: cs.CY

TL;DR: 该章节探讨了地理人工智能中信任的多维概念，将其分为三种类型：对训练数据的认知信任、对模型功能的操作信任以及对开发人员的人际信任，并讨论了地理背景下的信任挑战和解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着预训练模型在地理领域的广泛应用，信任成为关键问题。由于这些模型被用于关键决策，需要系统性地理解信任的不同维度及其在地理应用中的影响。

Method: 通过概念分析框架，将信任分为三种类型：认知信任（训练数据）、操作信任（模型功能）和人际信任（开发人员），并在地理背景下分析每种信任类型的独特含义。

Result: 识别了地理应用中信任面临的独特挑战，包括文化背景、数据异质性和空间关系等地理科学基础要素对信任建立的影响。

Conclusion: 呼吁地理信息科学家进一步推动透明度、偏见缓解和区域知情政策，为研究人员、从业者和政策制定者提供理解生成式GeoAI中信任的概念起点。
...
### [106] [Integrating Generative AI into LMS: Reshaping Learning and Instructional Design](https://arxiv.org/abs/2510.18026)
*Xinran Zhu,Liam Magee,Peg Mischler*

Main category: cs.CY

TL;DR: 本文探讨了在生成式AI时代教育系统的转型需求，提出学习管理系统应从静态内容存储库演变为支持高阶思维和有意义人机交互的动态生态系统。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI重塑专业实践，教育者需要重新思考如何培养学生适应人类与机器共同构建知识的未来，而当前的教育系统尚未充分利用AI的教育潜力。

Method: 提出两个指导原则：从内容传递转向培养高阶思维，以及实现与AI的有意义交互；通过CheckIT Learning案例研究展示这些原则的实际应用。

Result: 展示了如何将生成式AI有意义地整合到学习管理系统中，支持探究、协作和反思性知识构建，培养批判性、有意图和社会中介的AI参与。

Conclusion: 在AI驱动的世界中，教育技术合作至关重要，需要研究人员、教育工作者和技术专家持续合作，确保教育中的AI整合具有伦理基础、教学基础和认知基础。
...
### [107] [Prompt-to-Primal Teaching](https://arxiv.org/abs/2510.18050)
*Euzeli dos Santos*

Main category: cs.CY

TL;DR: 提出Prompt-to-Primal (P2P)教学方法，将AI提示驱动探索与第一性原理推理相结合，由教师在课堂中引导验证AI输出，提升AI素养和工程推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对已有基础知识的学习者很有效，但可能误导缺乏背景知识的学生，需要一种教学框架来平衡AI工具使用与基础概念学习。

Method: P2P教学法：学生生成AI提示作为课堂讨论起点，教师引导学习者通过基础物理和数学定律验证、挑战和重构AI响应，促进自我反思和批判性评估。

Result: 两个学期不同学生群体的结果表明，P2P教学框架在提升AI素养和工程推理方面具有教学有效性。

Conclusion: P2P教学成功地将AI工具整合到工程教育中，通过教师引导的验证过程，既利用AI优势又避免其潜在误导，促进概念性基础知识的掌握。
...
### [108] [The Cost-Benefit of Interdisciplinarity in AI for Mental Health](https://arxiv.org/abs/2510.18581)
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数AI心理健康聊天机器人仅依赖有限的专业知识输入，未能整合整个生命周期中的跨学科专业知识，这可能导致价值不对齐和合规风险。

Method: 通过分析AI心理健康聊天机器人的生命周期阶段，提出需要整合技术、医疗、伦理和法律等不同领域专家的协作框架。

Result: 识别了跨学科合作在确保AI心理健康聊天机器人价值对齐和合规性方面的关键作用，并提供了平衡挑战与收益的实用建议。

Conclusion: 跨学科协作对于开发符合AI法案高风险要求的AI心理健康聊天机器人至关重要，需要在整个生命周期中整合多领域专业知识。
...
### [109] [Integrating Large Language Models and Evaluating Student Outcomes in an Introductory Computer Science Course](https://arxiv.org/abs/2510.18806)
...
TL;DR: 这篇论文研究了在CS1课程中整合LLM作为学习工具的设计和评估，发现学生考试成绩与传统课程相似，开放式项目在LLM环境下特别有价值，学生普遍认为LLM工具有帮助但担心过度依赖。


<details>
  <summary>Details</summary>
Motivation: 由于生成式AI模型在编程作业中表现出色，传统的CS1课程需要改变教学和评估方式，但LLM对学生学习成果的影响仍需深入研究。

Method: 在一所大型研究型大学设计并实施了新的CS1-LLM课程，整合LLM作为学习工具，通过评估分数和调查问卷评价学生成果和感知。

Result: 1) 学生考试成绩与传统课程相似，包括不同人口群体的差异；2) 大型开放式项目在LLM环境下特别有价值；3) 学生普遍认为LLM工具有帮助，但担心过度依赖。

Conclusion: 在CS1课程中整合LLM作为学习工具是可行的，能保持学生表现的同时提供新的学习体验，但需要平衡工具使用与独立学习能力培养。
...
### [110] [MoveOD: Synthesizing Origin-Destination Commute Distribution from U.S. Census Data](https://arxiv.org/abs/2510.18858)
*Rishav Sen,Abhishek Dubey,Ayan Mukhopadhyay,Samitha Samaranayake,Aron Laszka*

Main category: cs.CY

TL;DR: MOVEOD是一个开源管道，利用五种公开数据源合成美国任何县的高分辨率通勤起讫点流量数据，包含精细的空间和时间维度信息。


<details>
  <summary>Details</summary>
Motivation: 高分辨率起讫点表对交通应用至关重要，但在大多数城市中难以获得。MOVEOD旨在解决这一数据稀缺问题。

Method: 结合美国社区调查、纵向雇主-家庭动态数据、县几何、OpenStreetMap道路网络和建筑足迹五种数据源，采用约束采样和整数规划方法进行数据协调。

Result: 在田纳西州汉密尔顿县成功生成了约15万条合成行程，可用于经典和基于学习的车辆路径算法基准测试。

Conclusion: MOVEOD是一个端到端的自动化系统，可轻松应用于美国各地，并能适应其他具有类似人口普查数据的国家。
...
### [111] [From Noise to Laws: Regularized Time-Series Forecasting via Denoised Dynamic Graphs](https://arxiv.org/abs/2510.17817)
*Hongwei Ma,Junbin Gao,Minh-ngoc Tran*

Main category: cs.LG

TL;DR: PRISM模型结合了基于分数的扩散预处理器、动态相关阈值图编码器和物理正则化预测头，用于长时域多变量时间序列预测，在六个标准基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决长时域多变量时间序列预测中的三个关键挑战：对异质信号去噪、跟踪时变跨序列依赖关系、在长预测时域中保持稳定性和物理合理性。

Method: 耦合基于分数的扩散预处理器、动态相关阈值图编码器以及通过通用物理惩罚正则化的预测头。

Result: 在六个标准基准测试中实现了持续的最先进性能，在MSE和MAE指标上取得了显著提升。

Conclusion: PRISM模型通过理论证明在温和条件下诱导的时域动态具有收缩性，并为图块推导了Lipschitz边界，解释了模型的鲁棒性。
...
### [112] [GRETEL: A Goal-driven Retrieval and Execution-based Trial Framework for LLM Tool Selection Enhancing](https://arxiv.org/abs/2510.17843)
*Zongze Wu,Yani Guo,Churong Liang,Runnan Li*

Main category: cs.LG

TL;DR: GRETEL通过执行验证解决工具检索中的语义-功能差距，显著提升工具检索性能
...
Conclusion: 基于执行的验证比单纯语义相似度提供了更可靠的工具选择基础
...
### [113] [CARLE: A Hybrid Deep-Shallow Learning Framework for Robust and Explainable RUL Estimation of Rolling Element Bearings](https://arxiv.org/abs/2510.17846)
...
Result: 在XJTU-SY和PRONOSTIA轴承数据集上的实验表明，CARLE优于多种先进方法，特别是在动态条件下表现更佳。

Conclusion: CARLE框架在剩余使用寿命预测任务中表现出优越的性能、鲁棒性和可解释性，为设备健康管理提供了可靠解决方案。
...
### [114] [Shock-Aware Physics-Guided Fusion-DeepONet Operator for Rarefied Micro-Nozzle Flows](https://arxiv.org/abs/2510.17887)
...
<details>
  <summary>Details</summary>
Motivation: 开发能够捕捉参数依赖关系、嵌入激波对齐坐标系，并强调高梯度区域的物理引导特征空间的深度学习框架，以构建稀薄、含激波微喷嘴流动的快速准确代理模型。

Method: 框架集成三个关键组件：用于捕捉参数依赖关系的融合DeepONet算子学习架构、嵌入激波对齐坐标系的物理引导特征空间，以及强调高梯度区域的两阶段课程策略。

Result: 首先在典型的粘性Burgers方程上验证了该框架的通用性和归纳偏差，该方程表现出对流陡峭和激波样梯度。

Conclusion: 提出的物理感知深度学习框架能够有效构建稀薄、含激波微喷嘴流动的快速准确代理模型，并在典型问题上验证了其有效性。
...
### [115] [MIN-Merging: Merge the Important Neurons for Model Merging](https://arxiv.org/abs/2510.17890)
*Yunfei Liang*

Main category: cs.LG

TL;DR: MIN-Merging是一个基于路由器的模型融合框架，通过选择性融合最重要神经元来减少参数冲突，在CV和NLP基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 深度学习开源模型激增，模型融合虽能结合各模型优势，但现有方法存在参数冲突问题，会降低领域特定任务的性能。

Method: 提出MIN-Merging框架，使用基于路由器的方法选择性融合最重要的神经元，减少参数冲突。
...
### [116] [Hierarchical Federated Unlearning for Large Language Models](https://arxiv.org/abs/2510.17895)
*Yisheng Zhong,Zhengbang Yang,Zhuangdi Zhu*

Main category: cs.LG

TL;DR: 提出了一种用于大语言模型的联邦遗忘方法，通过任务特定适配器学习和分层合并策略来解决异构遗忘需求，在保持模型效用的同时实现隐私保护的遗忘。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在现实应用中面临隐私和安全问题，需要移除不良知识。机器遗忘是解决方案，但面临连续异构遗忘需求和去中心化敏感数据的挑战，导致域间和域内干扰。

Method: 使用任务特定适配器学习将遗忘和保留解耦，采用分层合并策略来缓解目标冲突，实现鲁棒且自适应的遗忘更新。

Result: 在WMDP、MUSE和TOFU基准测试上的综合实验表明，该方法能有效处理异构遗忘请求，与基线方法相比保持强大的LLM效用。
...
### [117] [Long-Context Attention Benchmark: From Kernel Efficiency to Distributed Context Parallelism](https://arxiv.org/abs/2510.17896)
*Tao Bu,Qiangang Wang,Bowen Zeng,Hanwen Sun,Yunpeng Huang,Chun Cao,Jingwei Xu*

Main category: cs.LG

TL;DR: 提出了一个统一的基准测试框架，用于系统评估长上下文LLM训练中的注意力机制优化方法，包括内核级优化和模块级策略。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer注意力机制存在二次计算和内存成本问题，限制了长上下文训练。现有研究缺乏系统性评估，操作级比较不完整，上下文并行策略框架特定且性能分析不清晰。

Method: 构建统一基准测试框架，集成代表性注意力内核和上下文并行机制，提供模块化可扩展接口，从注意力掩码模式和序列长度/分布式规模两个维度进行评估。

Result: 在最多96个GPU集群上进行全面实验，实现了可复现的比较，揭示了方法特定的权衡，为长上下文LLM训练中注意力机制的设计和部署提供了实用指导。

Conclusion: 该基准测试框架填补了系统性评估的空白，能够帮助研究人员和从业者更好地理解和选择适合长上下文训练的注意力优化方法。
...
### [118] [L-MoE: End-to-End Training of a Lightweight Mixture of Low-Rank Adaptation Experts](https://arxiv.org/abs/2510.17898)
*Shihao Ji,Zihui Song*

Main category: cs.LG

TL;DR: L-MoE将混合专家架构与LoRA技术结合，创建了轻量级可训练的LoRA专家混合框架，通过动态组合低秩适配器实现参数高效的专业化语言模型。


<details>
  <summary>Details</summary>
Motivation: 结合MoE架构的可扩展性和LoRA的参数高效微调优势，构建端到端可训练的轻量级专家系统，实现动态技能组合和专业化语言模型。

Method: 将MoE专家重新定义为任务专业化的低秩适配器集合，通过轻量级门控网络动态计算每个输入token的LoRA参数加权平均，实现完全可微分的路由机制。

Result: 提出了L-MoE的正式数学框架，包括可微分路由机制和联合优化目标，为构建更高效、可扩展和专业化语言模型提供了新路径。

Conclusion: L-MoE创建了高度参数高效的MoE模型，具有模块化设计、动态技能组合和端到端可训练的特点，推动了高效专业化语言模型的发展。
...
### [119] [Automated Algorithm Design for Auto-Tuning Optimizers](https://arxiv.org/abs/2510.17899)
*Floris-Jan Willemsen,Niki van Stein,Ben van Werkhoven*

Main category: cs.LG

TL;DR: 使用大型语言模型自动生成针对自动调优问题的优化算法，在真实应用中相比现有最优算法平均提升72.4%性能
...
Result: 在4个真实自动调优应用和6个硬件平台上评估，相比两个当代自动调优框架的最优算法，平均性能提升72.4%

Conclusion: LLM生成的优化器能够媲美甚至超越人工设计的算法，为自动调优提供了新的有效范式
...
### [120] [Fostering the Ecosystem of AI for Social Impact Requires Expanding and Strengthening Evaluation Standards](https://arxiv.org/abs/2510.18238)
*Bryan Wilder,Angela Zhou*

Main category: cs.LG

TL;DR: 论文指出当前AI/ML社会影响研究的评审标准过度强调同时实现部署和方法创新，这不利于更广泛的社会影响研究生态系统，建议采用更广泛的社会影响定义和更严格的部署系统评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI/ML社会影响研究的评审标准倾向于同时认可部署和方法创新，但这种双重标准可能削弱更广泛社会影响研究生态系统的可持续性，因为许多项目可能更适合专注于应用或方法单一方面。

Method: 采用立场分析的方法，论证当前评审标准的问题，并提出两个关键建议：更广泛的社会影响定义和更严格的部署系统评估。

Result: 识别出当前评审标准可能产生的负面激励，这些激励可能阻碍研究人员根据项目合作伙伴需求做出最适合的贡献选择。

Conclusion: 建议机器学习社会影响领域的研究人员和评审者应采纳更广泛的社会影响概念，并对部署系统的影响进行更严格的评估，以支持更可持续的研究生态系统。
...
### [121] [The Sherpa.ai Blind Vertical Federated Learning Paradigm to Minimize the Number of Communications](https://arxiv.org/abs/2510.17901)
*Alex Acero,Daniel M. Jimenez-Gutierrez,Dario Pighin,Enrique Zuazua,Joaquin Del Rio,Xabi Uribe-Etxebarria*

Main category: cs.LG

TL;DR: 提出了一种名为Sherpa.ai Blind Vertical Federated Learning (SBVFL)的新范式，通过分布式训练机制显著减少节点与服务器之间的通信，相比标准VFL减少约99%的通信量，同时保持准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 垂直联邦学习(VFL)虽然适用于节点持有互补特征的场景，但存在通信量过大的问题，这会损害隐私和安全性，导致高能耗，甚至在某些情况下使模型训练不可行。
...
### [122] [RAISE: A Unified Framework for Responsible AI Scoring and Evaluation](https://arxiv.org/abs/2510.18559)
*Loc Phuc Truong Nguyen,Hung Thanh Do*

Main category: cs.LG

TL;DR: 提出了RAISE框架，从可解释性、公平性、鲁棒性和可持续性四个维度综合评估AI模型，并将这些指标聚合成一个整体责任分数。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统进入高风险领域，评估需要超越预测准确性，包含可解释性、公平性、鲁棒性和可持续性等维度。

Method: 引入RAISE（负责任AI评分与评估）统一框架，在金融、医疗和社会经济等结构化数据集上评估了多层感知器、表格ResNet和特征标记化Transformer三种深度学习模型。

Result: 发现关键权衡：MLP在可持续性和鲁棒性方面表现强劲，Transformer在可解释性和公平性方面表现出色但环境成本极高，表格ResNet提供了平衡的配置。

Conclusion: 没有一个模型在所有责任标准上都占主导地位，强调了多维评估对于负责任模型选择的必要性。
...
### [123] [NeuCo-Bench: A Novel Benchmark Framework for Neural Embeddings in Earth Observation](https://arxiv.org/abs/2510.17914)
...
<details>
  <summary>Details</summary>
Motivation: 为解决地球观测领域缺乏标准化神经嵌入评估框架的问题，促进社区驱动的神经压缩和表示学习研究。
...
Conclusion: NeuCo-Bench为地球观测及更广泛领域的神经嵌入标准化评估迈出了第一步，支持社区驱动的基准测试。
...
### [124] [Uncertainty-Aware Post-Hoc Calibration: Mitigating Confidently Incorrect Predictions Beyond Calibration Metrics](https://arxiv.org/abs/2510.17915)
...
<details>
  <summary>Details</summary>
Motivation: 现有校准方法通常应用全局变换，对所有预测一视同仁，忽视了单个预测的异质可靠性。此外，改进的校准与有效的不确定性感知决策之间的关系也未被充分探索。

Method: 使用基于邻近性的共形预测将校准样本按特征空间中的语义相似度分层为推定正确和推定错误组。对推定正确组应用标准等渗回归校准置信度，对推定错误组应用欠置信正则化等渗回归将置信度降低至均匀分布，便于识别需要进一步调查的预测。

Result: 在CIFAR-10和CIFAR-100数据集上使用BiT和CoAtNet骨干网络的实验表明，该方法实现了更低的自信错误预测，与等渗回归和焦点损失基线相比具有竞争力的预期校准误差。

Conclusion: 这项工作通过实例级自适应连接了校准和不确定性量化，提供了一个无需模型重新训练的后处理解决方案，同时改善了概率对齐和不确定性感知决策。
...
### [125] [Data Unlearning Beyond Uniform Forgetting via Diffusion Time and Frequency Selection](https://arxiv.org/abs/2510.17917)
*Jinseong Park,Mijung Park*

Main category: cs.LG

TL;DR: 本文提出了一种针对扩散模型的数据遗忘方法，通过选择性关注特定时间-频率范围来改善遗忘效果和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型数据遗忘方法在所有扩散时间步上平等处理，导致生成质量下降或遗忘不完整。作者观察到遗忘在不同时间和频率上是不均衡的，需要针对性处理。

Method: 采用时间-频率选择性方法，在训练过程中选择性关注特定时间-频率范围，可应用于基于梯度的和偏好优化的目标，以及图像级和文本到图像任务。

Result: 该方法能够生成具有更高美学质量和更低噪声的样本，并在多种设置下验证了改进效果。

Conclusion: 通过时间-频率选择性方法，为扩散模型数据遗忘提供了更清晰的理解和实用策略，同时改进了评估和遗忘性能。
...
### [126] [Rewarding the Journey, Not Just the Destination: A Composite Path and Answer Self-Scoring Reward Mechanism for Test-Time Reinforcement Learning](https://arxiv.org/abs/2510.17923)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法依赖人工标注的偏好数据或标记数据集，存在可扩展性瓶颈，需要探索在无标签数据上的自主学习方法

Method: 提出COMPASS框架，包含双校准答案奖励(DCAR)和决定性路径奖励(DPR)，通过置信度和可信度校准建立可靠的伪标签，并直接优化推理过程质量

Result: 实验表明COMPASS在多样化推理任务和模型架构上实现了显著且一致的性能提升
...
### [127] [EvoSyn: Generalizable Evolutionary Data Synthesis for Verifiable Learning](https://arxiv.org/abs/2510.17928)
...
<details>
  <summary>Details</summary>
Motivation: 可靠的验证数据是现代语言模型能力提升的关键驱动力，但构建可泛化的合成验证数据仍然困难，现有方法依赖特定任务启发式或后处理过滤器，缺乏原则性的通用可验证性评估器。

Method: 采用进化式、任务无关、策略引导的可执行检查数据合成框架，从最小种子监督出发，联合合成问题、多样化候选解决方案和验证工件，通过一致性评估器迭代发现策略。

Result: 实验表明该方法在RLVR和模型蒸馏训练范式下均有效，在LiveCodeBench和AgentBench-OS任务上取得显著改进，展现了框架的强泛化能力。

Conclusion: 该框架将过滤升级为原则性合成，能够可靠地组装连贯、可验证的训练实例，无需领域特定规则即可泛化，为语言模型训练提供了更有效的数据合成方法。
...
### [128] [From Observations to Parameters: Detecting Changepoint in Nonlinear Dynamics with Simulation-based Inference](https://arxiv.org/abs/2510.17933)
...
<details>
  <summary>Details</summary>
Motivation: 在混沌时间序列中检测体制转换很困难，因为观测空间信号与内在变异性纠缠在一起。

Method: 两阶段框架：首先用基于模拟推理训练的神经后验估计器进行贝叶斯参数推断，然后对生成的参数轨迹应用标准变点检测算法。

Result: 在Lorenz-63系统中，Param-CPD相比观测空间基线提高了F1分数，减少了定位误差，降低了误报率。
...
### [129] [UniRL-Zero: Reinforcement Learning on Unified Models with Joint Language Model and Diffusion Model Experts](https://arxiv.org/abs/2510.17937)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态理解和生成模型通常独立开发，缺乏统一的强化学习框架来协同提升它们的性能，并促进理解与生成能力之间的有益交互。

Method: 提出了UniRL-Zero统一强化学习框架，定义了六种统一模型强化学习场景，为统一理解和生成模型的强化学习提供系统基准。

Result: 开发了统一的代码实现，可在https://github.com/G-U-N/UniRL获取。

Conclusion: UniRL-Zero为多模态理解和生成模型的协同强化学习提供了系统化的框架和基准，促进了统一模型的发展。
...
### [130] [Demystifying Transition Matching: When and Why It Can Beat Flow Matching](https://arxiv.org/abs/2510.17991)
*Jaihoon Kim,Rajarshi Saha,Minhyuk Sung,Youngsuk Park*

Main category: cs.LG

TL;DR: 本文分析了Transition Matching(TM)何时及为何优于Flow Matching(FM)。在单峰高斯分布中，TM通过随机差异隐变量更新实现了更低的KL散度。在多峰高斯混合分布中，当模式分离良好且方差不可忽略时，TM表现更优。


<details>
  <summary>Details</summary>
Motivation: 尽管Flow Matching支撑了许多先进的生成模型，但最近结果表明Transition Matching能以更少采样步骤获得更高质量。本文旨在明确TM何时及为何优于FM。

Method: 首先在单峰高斯分布中证明TM获得更低的KL散度，分析收敛率。然后扩展到高斯混合分布，识别局部单峰性机制，研究模式分离距离和方差对性能的影响。

Result: TM在单峰高斯分布中收敛更快，在模式分离良好的高斯混合分布中表现更优。当目标方差趋近于零时，TM优势减弱。

Conclusion: TM在目标分布具有良好分离模式且方差不可忽略时优于FM，这一理论结果在图像和视频生成等实际应用中得到了验证。
...
### [131] [Attention-Guided Deep Adversarial Temporal Subspace Clustering (A-DATSC) Model for multivariate spatiotemporal data](https://arxiv.org/abs/2510.18004)
*Francis Ndikum Nji,Vandana Janeja,Jianwu Wang*

Main category: cs.LG

TL;DR: 提出了A-DATSC模型，通过结合深度子空间聚类生成器和质量验证判别器，解决了现有方法在4D时空数据聚类中的局限性，在多个真实数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度子空间聚类方法存在多个问题：使用浅层自编码器忽略聚类误差、强调全局特征而忽视局部结构、无法建模长程依赖和位置信息、很少应用于4D时空数据。

Method: 结合深度子空间聚类生成器和质量验证判别器，生成器采用U-Net架构，通过堆叠TimeDistributed ConvLSTM2D层保持时空完整性，使用图注意力变换器自表达网络捕获局部空间关系、全局依赖和长短程相关性。

Result: 在三个真实世界多元时空数据集上的实验表明，A-DATSC相比最先进的深度子空间聚类模型实现了显著优越的聚类性能。

Conclusion: A-DATSC模型有效解决了现有深度子空间聚类方法在4D时空数据分析中的关键限制，为复杂时空数据聚类提供了更强大的解决方案。
...
### [132] [Benchmarking Probabilistic Time Series Forecasting Models on Neural Activity](https://arxiv.org/abs/2510.18037)
*Ziyu Lu,Anna J. Li,Alexander E. Ladd,Pascha Matveev,Aditya Deole,Eric Shea-Brown,J. Nathan Kutz,Nicholas A. Steinmetz*

Main category: cs.LG

TL;DR: 系统评估了8种概率深度学习模型在神经活动预测中的表现，发现多个深度学习模型优于传统方法，最佳模型可预测1.5秒内的神经活动。
...
Result: 在多个预测时间尺度上，多个深度学习模型持续优于传统方法，最佳模型能够对未来1.5秒内的神经活动产生信息性预测。
...
### [133] [Cross-Domain Long-Term Forecasting: Radiation Dose from Sparse Neutron Sensor via Spatio-Temporal Operator Network](https://arxiv.org/abs/2510.18041)
*Jay Phil Yoo,Kazuma Kobayashi,Souvik Chakraborty,Syed Bahauddin Alam*

Main category: cs.LG

TL;DR: STONe是一种非自回归神经算子，能够在异构域之间学习稳定的功能映射，直接从稀疏地面中子测量推断高海拔辐射剂量场，实现长期稳定预测。


<details>
  <summary>Details</summary>
Motivation: 解决科学机器学习中从稀疏、跨域传感器数据预测不可观测物理量的核心问题，现有方法依赖密集共置输入输出场和短时间上下文，无法处理真实系统中传感和预测发生在不同物理流形和长时间尺度的情况。
...
### [134] [Measure-Theoretic Anti-Causal Representation Learning](https://arxiv.org/abs/2510.18052)
*Arman Behnam,Binghui Wang*

Main category: cs.LG

TL;DR: 提出了ACIA框架，一种用于反因果表示学习的测度论方法，通过双层设计在反因果设置中学习稳定表示，支持完美和不完美干预，无需显式因果结构，在合成和真实医疗数据上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 反因果设置（标签导致特征而非相反）在因果表示学习中面临独特挑战，需要专门方法来解决现有方法在干预处理、因果结构依赖、高维数据处理和理论保证方面的局限性。

Method: ACIA采用双层设计：低层表示捕捉标签如何生成观测，高层表示学习跨环境特定变化的稳定因果模式。使用干预核处理完美和不完美干预，无需显式因果结构，有效处理高维数据。

Result: 在合成和真实医疗数据集上的实验表明，ACIA在准确性和不变性指标上持续优于最先进方法。理论结果建立了训练环境和未见环境之间性能差距的紧界。

Conclusion: ACIA为稳健的反因果学习提供了有效框架，通过理论保证和实证验证证实了其在分布外泛化方面的有效性。
...
### [135] [Adaptive Divergence Regularized Policy Optimization for Fine-tuning Generative Models](https://arxiv.org/abs/2510.18053)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定发散正则化存在固有困境：强正则化保留模型能力但限制奖励优化，弱正则化可实现更好对齐但存在不稳定或奖励黑客风险。

Method: 基于优势估计自动调整正则化强度：对高价值样本减少正则化，对差样本应用更强正则化，使策略根据数据质量在探索和激进利用之间导航。

Result: 在文本到图像生成中，ADRPO在语义对齐和多样性方面优于DPO和ORW-CFM-W2等方法；2B参数的SD3模型在属性绑定、语义一致性等方面超越4.8B和12B模型；在LLM和多模态推理模型中也表现出色。
...
### [136] [SPACeR: Self-Play Anchoring with Centralized Reference Models](https://arxiv.org/abs/2510.18060)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在计算成本高、推理速度慢、难以适应闭环场景的问题，而自强化学习虽然可扩展但容易偏离人类驾驶规范。

Method: 使用预训练tokenized自回归运动模型作为中心参考策略，指导分散式自强化学习，通过似然奖励和KL散度将策略锚定在人类驾驶分布上。

Result: 在Waymo仿真智能体挑战中表现优异，推理速度快10倍，参数规模小50倍，在闭环规划评估中能有效测试自动驾驶策略质量。
...
### [137] [Fine-tuning Flow Matching Generative Models with Intermediate Feedback](https://arxiv.org/abs/2510.18072)
*Jiajun Fan,Chaoran Cheng,Shuaike Shen,Xiangxin Zhou,Ge Liu*

Main category: cs.LG

TL;DR: AC-Flow是一个用于流匹配模型的演员-评论家框架，通过奖励塑形、双稳定性机制和可扩展的评论家加权方案，解决了文本到图像生成中基于反馈的微调挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的流匹配模型在基于中间反馈进行微调时面临困难，大多数方法仅从结果奖励中学习，难以解决信用分配问题，而直接回归累积奖励的方法在在线设置中容易遇到训练不稳定和模型崩溃。

Method: 提出AC-Flow框架，包含三个关键创新：1）奖励塑形提供归一化学习信号；2）双稳定性机制结合优势裁剪和预热阶段；3）可扩展的广义评论家加权方案，通过Wasserstein正则化保持模型多样性。

Result: 在Stable Diffusion 3上的实验表明，AC-Flow在文本到图像对齐任务中达到最先进性能，并能泛化到未见的人类偏好模型，即使使用计算高效的评论家模型也能稳健微调流模型。

Conclusion: AC-Flow能够在不损害生成质量、多样性或稳定性的情况下，稳健地微调流匹配模型，解决了现有方法在信用分配和训练稳定性方面的挑战。
...
### [138] [R2L: Reliable Reinforcement Learning: Guaranteed Return & Reliable Policies in Reinforcement Learning](https://arxiv.org/abs/2510.18074)
*Nadir Farhi*

Main category: cs.LG

TL;DR: 提出了一种可靠强化学习框架，旨在最大化累积回报超过给定阈值的概率，而非传统RL的期望回报最大化。通过状态增强表示，将可靠RL问题转化为标准RL问题，可直接使用现有算法。


<details>
  <summary>Details</summary>
Motivation: 现实应用如路由、资源分配等需要确保高成功概率的策略，而不仅仅是高平均性能。传统RL算法无法提供性能保证。

Method: 将可靠RL问题重新表述为最大化累积回报超过阈值的概率，通过状态增强表示将其转化为标准RL问题，可使用Q-learning、Dueling Double DQN等现有算法。

Result: 理论证明两种表述的等价性，数值实验显示该方法能产生有效平衡效率和可靠性的策略，在可靠路由问题中显著提高按时到达目的地的概率。

Conclusion: 可靠RL框架为随机和安全关键环境中的应用提供了有前景的解决方案，无需开发全新算法框架即可实现性能保证。
...
### [139] [Batch Distillation Data for Developing Machine Learning Anomaly Detection Methods](https://arxiv.org/abs/2510.18075)
...
TL;DR: 该研究创建了一个用于机器学习异常检测的实验室规模批量蒸馏实验数据库，包含119个实验的时间序列数据、传感器读数、NMR谱图、音视频记录等，为开发先进的ML异常检测方法提供数据支持。


<details>
  <summary>Details</summary>
Motivation: 机器学习在化工过程异常检测中具有巨大潜力，但缺乏公开可用的实验数据阻碍了ML方法的发展。为解决这一数据缺口，研究者建立了实验室规模的批量蒸馏装置来生成实验数据库。

Method: 建立实验室规模批量蒸馏装置，进行119个实验，涵盖正常操作和故意引入异常的工况。收集多种数据源：传感器和执行器的时间序列数据、在线NMR谱图浓度分布、音视频记录，并开发异常本体论进行专家标注。

Result: 创建了包含119个实验的全面数据库，涵盖广泛操作条件和混合物。大多数异常实验都有对应的正常实验作为对照。数据包括时间序列、测量不确定性估计、非常规数据源（NMR、音视频）以及基于本体论的专家标注。

Conclusion: 该数据库为开发先进的ML异常检测方法铺平了道路，由于包含异常原因信息，还能支持可解释和可解释的ML方法以及异常缓解方法的开发。
...
### [140] [MEG-GPT: A transformer-based foundation model for magnetoencephalography data](https://arxiv.org/abs/2510.18080)
*Rukuang Huang,Sungjun Cho,Chetan Gohil,Oiwi Parker Jones,Mark Woolrich*

Main category: cs.LG

TL;DR: 提出了MEG-GPT，一种基于Transformer的基础模型，用于建模大规模脑动力学，通过时间注意力和下一时间点预测来处理MEG数据，在解码任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统方法无法捕捉脑磁图等模态中复杂的时空模式，而深度学习在其他领域的成功表明基础模型具有潜力。

Method: 使用基于Transformer的架构，结合时间注意力和下一时间点预测，并开发了新的数据驱动分词器来保持MEG信号的高时间分辨率。

Result: 模型能生成具有现实时空频谱特性的数据，在下游解码任务中表现优异，跨会话准确率从0.54提升至0.59，跨被试准确率从0.41提升至0.49。
...
### [141] [Any-Depth Alignment: Unlocking Innate Safety Alignment of LLMs to Any-Depth](https://arxiv.org/abs/2510.18081)
...
<details>
  <summary>Details</summary>
Motivation: LLMs表现出强但浅层的对齐：在助手回合开始时直接拒绝有害查询，但一旦有害内容开始生成，这种保护就会崩溃。需要解决在任意生成深度确保安全性的问题。

Method: 提出Any-Depth Alignment (ADA)，基于观察到对齐集中在助手头部令牌中，通过在中途重新引入这些令牌，诱导模型重新评估有害性并在生成过程中恢复拒绝。

Result: 在多个开源模型家族上，ADA实现了接近100%的拒绝率对抗对抗性预填充攻击，将主要对抗提示攻击的平均成功率降至3%以下，同时保持良性任务的实用性。

Conclusion: ADA提供了一种有效的推理时防御方法，能够在任意生成深度确保LLMs的安全性，且对基础模型的后续指令调优保持韧性。
...
### [142] [Provably Optimal Reinforcement Learning under Safety Filtering](https://arxiv.org/abs/2510.18082)
*Donggeon David Oh,Duy P. Nguyen,Haimin Hu,Jaime F. Fisac*

Main category: cs.LG

TL;DR: 本文证明了在强化学习中，使用足够宽松的安全过滤器不会降低渐近性能，从而实现了安全性和性能的完全分离。


<details>
  <summary>Details</summary>
Motivation: 强化学习在复杂任务中的应用受到缺乏正式安全保证的限制。现有的安全过滤方法通常被认为会牺牲性能和阻碍学习过程。
...
### [143] [Enhancing mortality prediction in cardiac arrest ICU patients through meta-modeling of structured clinical data from MIMIC-IV](https://arxiv.org/abs/2510.18103)
*Nursultan Mamatov,Philipp Kellmeyer*

Main category: cs.LG

TL;DR: 该研究开发了结合结构化临床数据和非结构化文本信息的机器学习模型，用于ICU住院死亡率的早期预测，结果显示结合文本特征显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: ICU住院死亡率的准确早期预测对于及时临床干预和高效资源分配至关重要，现有模型主要依赖结构化数据，忽略了非结构化文本信息的价值。

Method: 使用LASSO和XGBoost进行特征选择，然后基于两种模型识别的重要特征训练多元逻辑回归模型，结合TF-IDF和BERT嵌入处理文本特征。

Result: 结合结构化数据和文本输入的逻辑回归模型AUC达到0.918，相比仅使用结构化数据的0.753，相对提升22%，决策曲线分析显示在广泛阈值概率范围内具有优越的标准化净效益。
...
### [144] [Latent Discrete Diffusion Models](https://arxiv.org/abs/2510.18114)
...
Method: 提出了两种LDDMs变体：FUJI-LDDMs（完全联合去噪）和SEQ-LDDMs（顺序解析潜在和离散链），并推导了ELBO风格的目标函数。

Result: 在无条件生成指标上相比最先进的掩码离散扩散基线有所改进，在较低采样预算下特别有效。

Conclusion: LDDMs通过潜在通道提供更柔和的信号和跨标记依赖关系，有助于解决歧义，在少步生成中表现优越。
...
### [145] [Gradient Variance Reveals Failure Modes in Flow-Based Generative Models](https://arxiv.org/abs/2510.18118)
...
<details>
  <summary>Details</summary>
Motivation: 分析Rectified Flows在确定性训练下隐藏的基本失效模式，揭示低梯度方差导致任意训练配对记忆化的问题

Method: 通过高斯到高斯传输研究，使用损失梯度方差分析随机和确定性机制下优化的向量场偏好，并在CelebA数据集上进行实证验证

Result: 在插值线相交的情况下，Rectified Flow在推理时产生与训练时相同的特定配对；确定性积分重现精确的训练配对，而小噪声注入可以恢复泛化

Conclusion: Rectified Flows的直线路径目标会收敛到未定义场，导致记忆化问题，需要噪声注入来确保泛化能力
...
### [146] [Efficient Long-context Language Model Training by Core Attention Disaggregation](https://arxiv.org/abs/2510.18121)
*Yonghao Zhuang,Junda Chen,Bo Pang,Yi Gu,Yibo Zhu,Yimin Jiang,Ion Stoica,Eric Xing,Hao Zhang*

Main category: cs.LG

TL;DR: 提出核心注意力解耦(CAD)技术，通过将核心注意力计算与其他模型层分离并在专用设备池上执行，解决长上下文LLM训练中的负载不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 在长上下文长度下，核心注意力的二次计算增长与其他组件的近线性增长导致数据并行和流水线并行组中的负载不平衡和拖尾问题。

Method: 将核心注意力分解为token级任务，分派到专用注意力服务器，动态重新批处理任务以均衡计算而不牺牲内核效率，使用乒乓执行方案完全重叠通信与计算。
...
### [147] [HyperDiffusionFields (HyDiF): Diffusion-Guided Hypernetworks for Learning Implicit Molecular Neural Fields](https://arxiv.org/abs/2510.18122)
*Sudarshan Babu,Phillip Lo,Xiao Zhang,Aadi Srivastava,Ali Davariashtiyani,Jason Perera,Michael Maire,Aly A. Khan*

Main category: cs.LG

TL;DR: HyperDiffusionFields (HyDiF) 是一个将3D分子构象建模为连续场的框架，使用分子方向场(MDF)表示分子结构，通过超网络生成分子神经场(MNFs)，并基于扩散模型实现分子生成和条件生成任务。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用离散原子坐标或图结构表示分子，难以处理空间细粒度特征提取和条件生成任务。HyDiF旨在通过连续场表示克服这些限制，支持更灵活的分子建模和生成。

Method: 使用分子方向场(MDF)将空间点映射到最近原子的方向；采用超网络生成分子特定的神经隐式场(MNFs)；基于去噪扩散模型训练超网络，实现函数空间的分子采样；支持掩码扩散机制进行条件生成。

Result: 该方法能够实现分子生成、条件生成（如分子修复），支持空间细粒度特征提取用于分子性质预测，并能扩展到更大的生物分子。

Conclusion: HyDiF展示了基于场的分子建模的潜力，为分子表示和生成提供了连续、灵活的新方法，特别适用于需要空间细粒度分析的应用场景。
...
### [148] [Rethinking PCA Through Duality](https://arxiv.org/abs/2510.18130)
*Jan Quan,Johan Suykens,Panagiotis Patrinos*

Main category: cs.LG

TL;DR: 本文重新审视PCA基础，通过DC框架提出新公式和理论见解，展示了PCA类问题的核化能力和样本外适用性，揭示了同时迭代与DCA的联系，并提出了新的PCA算法和鲁棒核化PCA变体。


<details>
  <summary>Details</summary>
Motivation: 受自注意力机制与核主成分分析之间联系的启发，重新审视PCA的基本原理，探索更优化的数学框架和算法。

Method: 使用差异凸(DC)优化框架，提出了多个新的PCA公式，包括核化版本和鲁棒变体，并建立了同时迭代与DC算法之间的联系。

Result: 展示了PCA类问题的核化能力和样本外适用性，揭示了经典QR算法与DC算法的等价关系，提出了新的PCA算法并在实验中与现有方法进行比较。

Conclusion: DC框架为PCA提供了新的理论视角和优化方法，不仅统一了经典算法，还启发了新的核化和鲁棒PCA变体的开发。
...
### [149] [Nash Policy Gradient: A Policy Gradient Method with Iteratively Refined Regularization for Finding Nash Equilibria](https://arxiv.org/abs/2510.18183)
...
<details>
  <summary>Details</summary>
Motivation: 传统正则化方法需要将正则化强度趋近于零来逼近纳什均衡，这在实际中常导致学习不稳定。本文旨在开发更稳定的收敛方法。

Method: 固定正则化强度以保证鲁棒性，通过迭代优化参考策略实现收敛。开发NashPG算法，结合策略梯度方法的通用性，仅依赖当前策略和参考策略。

Result: 理论证明在双人零和博弈中保证严格单调改进并收敛到精确纳什均衡。实验显示在经典基准游戏中达到可比或更低的可剥削性，在大型领域如Battleship和No-Limit Texas Hold'em中持续获得更高Elo评分。

Conclusion: NashPG提供了一种稳定收敛到纳什均衡的新方法，无需减小正则化强度，在理论和实践中均表现出色。
...
### [150] [ActivationReasoning: Logical Reasoning in Latent Activation Spaces](https://arxiv.org/abs/2510.18184)
...
Method: 三阶段框架：(1)通过SAE等方法识别潜在概念表示并组织成字典；(2)在推理时检测激活概念并映射为逻辑命题；(3)应用逻辑规则进行推理，推断高阶结构、组合新概念并引导模型行为。
...
### [151] [Ensemble based Closed-Loop Optimal Control using Physics-Informed Neural Networks](https://arxiv.org/abs/2510.18195)
*Jostein Barry-Straume,Adwait D. Verulkar,Arash Sarshar,Andrey A. Popov,Adrian Sandu*

Main category: cs.LG

TL;DR: 提出一种多阶段集成框架，通过HJB方程学习最优成本函数和相应最优控制信号，无需使用稳定项，可处理噪声和变化初始条件。


<details>
  <summary>Details</summary>
Motivation: HJB方程数值求解计算量大，解析解常不可得，需要知识引导的机器学习方法来缓解这一困难。

Method: 多阶段集成框架，通过物理信息神经网络学习最优成本函数和最优控制信号，支持单一控制信号或集成控制策略。

Result: 成功实现了对稳态时不变两状态连续非线性系统的闭环控制，能够处理噪声扰动和变化初始条件。

Conclusion: 该框架为非线性系统控制提供了一种有效的替代方法，避免了传统HJB求解的计算复杂性。
...
### [152] [Joint Optimization of Cooperation Efficiency and Communication Covertness for Target Detection with AUVs](https://arxiv.org/abs/2510.18225)
...
<details>
  <summary>Details</summary>
Motivation: 解决自主水下航行器在协同目标检测中合作效率与通信隐蔽性之间的关键权衡问题。

Method: 采用分层动作管理框架：宏观层面使用PPO算法进行战略任务分配，微观层面使用多智能体PPO算法基于局部观测动态调整轨迹和传输功率。

Result: 提出的目标检测框架能够在满足能量和机动性约束的同时实现自适应隐蔽合作。
...
### [153] [Towards Fast LLM Fine-tuning through Zeroth-Order Optimization with Projected Gradient-Aligned Perturbations](https://arxiv.org/abs/2510.18228)
...
<details>
  <summary>Details</summary>
Motivation: 现有零阶优化方法在LLM微调中存在梯度估计方差高、收敛慢的问题，需要更高效的优化方法
...
### [154] [ACTG-ARL: Differentially Private Conditional Text Generation with RL-Boosted Control](https://arxiv.org/abs/2510.18232)
...
Method: 采用分层框架：特征学习+条件文本生成，结合DP表格合成器和DP微调条件生成器，并使用锚定强化学习提升控制能力

Result: 在强隐私保证下，DP合成文本质量提升20% MAUVE，条件生成器控制能力显著增强

Conclusion: ACTG-ARL框架在差分隐私文本生成的质量和控制方面均优于现有方法
...
### [155] [Learning with Dual-level Noisy Correspondence for Multi-modal Entity Alignment](https://arxiv.org/abs/2510.18240)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设实体内部和跨图对应关系都是完美的，但在真实世界的多模态知识图谱中，由于依赖专家标注，这些对应关系往往存在噪声。本文揭示并研究了多模态实体对齐中一个实际但未被充分探索的问题——双重噪声对应。

Method: RULE框架首先通过双重原则估计实体内部和跨图对应关系的可靠性，然后利用估计的可靠性在属性融合时减轻实体内部噪声的负面影响，在消除跨图差异时防止对噪声对应关系的过拟合。此外，还包含一个对应关系推理模块，揭示跨图的属性-属性连接。

Result: 在五个基准测试上的广泛实验验证了该方法在双重噪声对应问题上的有效性，与七种最先进方法相比表现优异。

Conclusion: RULE框架能够有效处理多模态实体对齐中的双重噪声对应问题，通过可靠性估计和对应关系推理提高了实体对齐的准确性。
...
### [156] [Scaling Laws Meet Model Architecture: Toward Inference-Efficient LLMs](https://arxiv.org/abs/2510.18245)
*Song Bian,Tao Yu,Shivaram Venkataraman,Youngsuk Park*

Main category: cs.LG

TL;DR: 该研究提出了条件缩放定律和搜索框架，用于在保持准确性的同时优化大语言模型的推理效率，通过调整隐藏层大小、MLP与注意力参数分配比例和分组查询注意力等架构因素。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型参数规模和训练数据量的增长，推理成本已成为重要问题，但模型准确性与推理效率之间的权衡关系尚未得到充分探索。

Method: 引入条件缩放定律扩展Chinchilla框架，包含架构信息；开发搜索框架识别同时具备推理效率和准确性的架构；训练超过200个模型（80M-3B参数，8B-100B训练tokens）来拟合缩放定律。

Result: 条件缩放定律能可靠预测最优架构选择，优化后的模型在相同训练预算下比LLaMA-3.2准确率提高2.1%，推理吞吐量提升42%。

Conclusion: 通过条件缩放定律和架构优化，可以在保持模型性能的同时显著提升推理效率，为构建更高效的大语言模型提供了有效方法。
...
### [157] [NTKMTL: Mitigating Task Imbalance in Multi-Task Learning from Neural Tangent Kernel Perspective](https://arxiv.org/abs/2510.18258)
*Xiaohan Qin,Xiaoxing Wang,Ning Liao,Junchi Yan*

Main category: cs.LG

TL;DR: 提出了基于神经正切核(NTK)理论的多任务学习方法NTKMTL，通过谱分析平衡多任务收敛速度，解决任务不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中任务不平衡是主要挑战，需要准确表征复杂多任务系统中的训练动态和收敛速度。

Method: 引入扩展的NTK矩阵进行谱分析，平衡多任务收敛速度；基于共享表示近似提出NTKMTL-SR提升训练效率。

Result: 在多个基准测试中实现了最先进的性能，包括多任务监督学习和多任务强化学习。

Conclusion: 基于NTK理论的方法能有效解决多任务学习中的任务不平衡问题，在保持竞争力的同时提升训练效率。
...
### [158] [From Competition to Synergy: Unlocking Reinforcement Learning for Subject-Driven Image Generation](https://arxiv.org/abs/2510.18263)
...
<details>
  <summary>Details</summary>
Motivation: 传统GRPO方法在主题驱动图像生成中存在竞争退化问题，线性聚合奖励会导致冲突梯度信号和与扩散过程时间动态不匹配。

Method: 提出Customized-GRPO框架，包含：1) Synergy-Aware Reward Shaping - 非线性机制惩罚冲突奖励信号并放大协同信号；2) Time-Aware Dynamic Weighting - 动态权重调整，早期优先提示遵循，后期优先身份保持。

Result: 实验表明该方法显著优于传统GRPO基线，成功缓解竞争退化，在身份特征保持和复杂文本提示遵循之间实现更好平衡。

Conclusion: Customized-GRPO通过奖励塑形和动态权重调整有效解决了主题驱动图像生成中的竞争退化问题，实现了更好的身份保持与提示遵循平衡。
...
### [159] [Online Time Series Forecasting with Theoretical Guarantees](https://arxiv.org/abs/2510.18281)
*Zijian Li,Changze Zhou,Minghao Fu,Sanjay Manjunath,Fan Feng,Guangyi Chen,Yingyao Hu,Ruichu Cai,Kun Zhang*

Main category: cs.LG

TL;DR: 提出了一个在线时间序列预测的理论框架TOT，通过引入潜在变量来应对未知的分布漂移，并提供了理论保证。该方法使用时间解码器和两个独立的噪声估计器来建模潜在变量的因果推理和观测变量的混合过程。
...
Method: 提出TOT理论框架，通过时间解码器匹配观测变量分布，使用两个独立的噪声估计器分别建模潜在变量的因果推理和观测变量的混合过程。

Result: 在合成数据上验证了理论主张，基于多个基线的插件实现在多个基准测试中均取得了普遍改进。

Conclusion: 该方法能有效应对在线时间序列预测中的分布漂移问题，在真实世界应用中表现出有效性。
...
### [160] [Physics-Informed Parametric Bandits for Beam Alignment in mmWave Communications](https://arxiv.org/abs/2510.18299)
...
<details>
  <summary>Details</summary>
Motivation: 传统bandit算法在大波束空间下收敛慢，现有方法依赖的奖励函数单峰/多峰假设在实践中往往不成立，导致选择次优波束

Method: 基于相位恢复bandit问题，将每条路径参数视为黑盒，基于历史采样奖励维护最优估计。pretc从随机探索开始后提交给最优波束，prgreedy在线估计并选择当前最优波束

Result: 在DeepMIMO合成数据集和DeepSense6G真实数据集上的实验表明，两种算法在多样化信道环境中均优于现有方法

Conclusion: 提出的算法具有普适性和鲁棒性，能够有效解决毫米波通信中的波束对准和跟踪问题
...
### [161] [Towards Identifiability of Hierarchical Temporal Causal Representation Learning](https://arxiv.org/abs/2510.18310)
...
Conclusion: CHiLD框架能够唯一确定分层潜在变量的联合分布，成功解决了分层潜在动态建模的挑战，为时间序列数据分析提供了新方法。
...
### [162] [Higher Embedding Dimension Creates a Stronger World Model for a Simple Sorting Task](https://arxiv.org/abs/2510.18315)
*Brady Bhalla,Honglu Fan,Nancy Chen,Tony Yue YU*

Main category: cs.LG

TL;DR: 研究嵌入维度如何影响transformer在强化学习排序任务中内部世界模型的涌现，发现更大嵌入维度产生更忠实、一致和鲁棒的内部表示，提升可解释性。


<details>
  <summary>Details</summary>
Motivation: 探究transformer在算法任务中如何构建内部世界模型，以及模型大小如何影响表示质量。

Method: 使用强化学习训练transformer执行冒泡排序式相邻交换，系统改变嵌入维度进行数百次实验，分析注意力权重和内部表示。

Result: 发现两个一致机制：注意力权重矩阵最后一行单调编码token全局排序；选择的交换与这些编码值的最大相邻差异对齐。高嵌入维度强化结构化内部表示的形成。

Conclusion: transformer构建结构化内部世界模型，模型大小不仅提升最终性能，还改善表示质量。发布的指标和分析可用于探索类似算法任务。
...
### [163] [Uncertainty Estimation by Flexible Evidential Deep Learning](https://arxiv.org/abs/2510.18322)
*Taeseong Yoon,Heeyoung Kim*

Main category: cs.LG

TL;DR: 提出灵活证据深度学习（F-EDL），通过预测灵活的Dirichlet分布来扩展传统EDL，提升不确定性量化的泛化能力和可靠性。
...
Method: F-EDL扩展EDL，预测灵活的Dirichlet分布（Dirichlet分布的泛化形式）来建模类别概率，提供更具表达性和适应性的不确定性表示。

Result: 在经典、长尾和噪声分布内场景等多种评估设置中，F-EDL展现出最先进的不确定性量化性能。

Conclusion: F-EDL通过灵活的Dirichlet分布显著增强了不确定性量化的泛化能力和可靠性，在挑战性场景下表现优异。
...
### [164] [Scalable, Explainable and Provably Robust Anomaly Detection with One-Step Flow Matching](https://arxiv.org/abs/2510.18328)
*Zhong Li,Qi Huang,Yuxuan Zhu,Lincen Yang,Mohammad Mohammadi Amiri,Niki van Stein,Matthijs van Leeuwen*

Main category: cs.LG

TL;DR: TCCM是一种基于流匹配思想的半监督表格数据异常检测方法，通过预测时间条件收缩向量来简化训练和推理过程，在检测精度和推理成本之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的异常检测方法（如DTE）虽然准确率高，但推理成本过高，需要解决常微分方程，限制了实际应用。

Method: 基于流匹配核心思想，但简化为预测时间条件收缩向量，直接学习从数据分布到原点收缩的向量场，无需训练时求解ODE。

Result: 在ADBench基准测试中，TCCM在检测精度和推理成本之间取得良好平衡，特别是在高维和大规模数据集上优于现有方法。

Conclusion: TCCM通过简化流匹配框架，提供了轻量级、可扩展的异常检测解决方案，具有理论鲁棒性和可解释性优势。
...
### [165] [Why Policy Gradient Algorithms Work for Undiscounted Total-Reward MDPs](https://arxiv.org/abs/2510.18340)
*Jongmin Lee,Ernest K. Ryu*

Main category: cs.LG

TL;DR: 本文分析了无折扣总奖励无限时域MDP中的策略梯度方法，通过引入瞬态访问度量和利用MDP状态的分类不变性，为γ=1的情况提供了理论分析。


<details>
  <summary>Details</summary>
Motivation: 现代基于策略的强化学习算法（特别是大语言模型中的策略优化）常使用无折扣总奖励设置（γ=1），但现有理论大多假设γ<1，因此需要为γ=1的情况建立理论基础。

Method: 基于两个关键洞察：(1) MDP状态在严格正概率策略下的循环和瞬态分类不变性；(2) 用瞬态访问度量替代经典的状态访问度量。

Result: 为无折扣总奖励无限时域MDP中的策略梯度方法提供了严格的理论分析框架。

Conclusion: 提出的瞬态访问度量和状态分类不变性分析为γ=1的策略梯度方法建立了理论基础，填补了现有理论的空白。
...
### [166] [Computable universal online learning](https://arxiv.org/abs/2510.18352)
*Dariusz Kalociński,Tomasz Steifer*

Main category: cs.LG

TL;DR: 本文研究了可计算通用在线学习的问题，探讨了在理论机器学习中学习算法何时能够被实现为计算机程序。研究发现通用在线学习并不一定意味着可计算的通用在线学习，并对可计算通用在线学习的类别进行了精确刻画。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习理论中的学习特性通常作为抽象数学对象来研究，而忽略了学习算法能否被实现为计算机程序这一关键问题。本文旨在填补这一空白，为通用在线学习提供更现实的视角。

Method: 研究通用在线学习模型，其中对手可以改变主意，只要保持与给定假设类的局部一致性。要求学习者在有限错误次数内使用可计算策略。分析了可计算通用在线学习、不可知变体和适当变体的学习能力。

Result: 证明了通用在线学习并不蕴含可计算的通用在线学习，即使假设类在可计算性理论上相对简单。对可计算通用在线学习的不可知变体给出了精确刻画，并确定了适当变体何时可能。

Conclusion: 研究结果为在线二分类和相关归纳推理问题提供了更现实的理论视角，揭示了理论学习特性与实际可计算实现之间的重要差异。
...
### [167] [Ensembling Pruned Attention Heads For Uncertainty-Aware Efficient Transformers](https://arxiv.org/abs/2510.18358)
...
<details>
  <summary>Details</summary>
Motivation: Deep Ensembles等方法虽然不确定性量化性能强，但计算和内存成本高，难以扩展到大型模型。需要开发高效的集成方法来平衡性能和效率。

Method: 提出Hydra Ensembles：1）剪枝注意力头创建多样化集成成员；2）通过新的多头注意力机制和分组全连接层合并成员；3）形成紧凑模型，推理速度接近单网络。

Result: 在图像和文本分类任务上，各种架构下均一致优于Deep Ensembles。在ImageNet-1k零样本分类中，无需额外训练即超越现有最优方法。

Conclusion: Hydra Ensembles提供了一种高效的不确定性量化方法，在保持高性能的同时显著降低了计算和内存开销，为安全关键应用中的深度神经网络部署提供了可行方案。
...
### [168] [Learning to Flow from Generative Pretext Tasks for Neural Architecture Encoding](https://arxiv.org/abs/2510.18360)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于信息流的神经网络架构编码器虽然性能优越，但模型结构复杂导致处理速度慢，存在实际应用挑战。
...
### [169] [Towards Unsupervised Open-Set Graph Domain Adaptation via Dual Reprogramming](https://arxiv.org/abs/2510.18363)
*Zhen Zhang,Bingsheng He*

Main category: cs.LG

TL;DR: 提出GraphRTA框架解决无监督开放集图域自适应问题，通过图重编程和模型重编程来识别已知类和未知类，无需手动设定阈值。


<details>
  <summary>Details</summary>
Motivation: 现实场景中目标域可能包含源域中不存在的类别，现有图域自适应模型主要关注闭集设置，无法处理开放集情况。

Method: GraphRTA框架在图上进行重编程（修改图结构和节点特征）以更好分离已知和未知类，同时进行模型重编程（剪枝领域特定参数）以减少源图偏见，并扩展分类器维度以自动识别未知类。

Result: 在多个公共数据集上的实验表明，该模型相比现有最先进基线方法取得了满意性能。

Conclusion: GraphRTA框架有效解决了无监督开放集图域自适应问题，能够同时正确分类已知类并识别未知类。
...
### [170] [Training Diverse Graph Experts for Ensembles: A Systematic Empirical Study](https://arxiv.org/abs/2510.18370)
...
<details>
  <summary>Details</summary>
Motivation: 单个GNN的性能受限于现实图数据的异质性，而集成具有不同泛化模式的多样化GNN可以显著提升性能。
...
### [171] [Approximation Rates of Shallow Neural Networks: Barron Spaces, Activation Functions and Optimality Analysis](https://arxiv.org/abs/2510.18388)
*Jian Lu,Xiaohuang Huang*

Main category: cs.LG

TL;DR: 本文研究了具有指数函数幂次激活函数的浅层神经网络的逼近特性，重点关注Barron函数空间中逼近率对维度和函数光滑度的依赖性。


<details>
  <summary>Details</summary>
Motivation: 探讨浅层神经网络在Barron函数空间中的逼近能力，分析激活函数选择和网络结构对逼近性能的影响，特别是维度灾难问题。

Method: 研究ReLU^k激活函数的逼近率，分析在ℓ^1有界系数或光滑度不足条件下无法达到最优逼近率的情况。

Result: 在Barron空间和Sobolev空间的各种范数下建立了最优逼近率，证实了维度灾难的存在。

Conclusion: 阐明了浅层神经网络逼近能力的局限性，为激活函数选择和网络结构设计提供了理论指导。
...
### [172] [Learning from N-Tuple Data with M Positive Instances: Unbiased Risk Estimation and Theoretical Guarantees](https://arxiv.org/abs/2510.18406)
*Miao Zhang,Junpeng Li,ChangChun HUa,Yana Yang*

Main category: cs.LG

TL;DR: 提出一种基于计数监督的弱监督学习方法，通过N元组中正例数量信息来训练模型，推导出可训练的无偏风险估计器，并在多个基准测试中优于现有弱监督基线方法。


<details>
  <summary>Details</summary>
Motivation: 弱监督学习通常只有粗粒度的聚合信号而非实例标签。本文研究每个训练示例是包含恰好m个正例的n元组，但只观察到每个元组的计数m的监督设置，这种NTMP监督出现在图像分类的区域提议和多实例测量等场景中。

Method: 通过将元组生成过程与潜在实例边际联系起来，推导出闭式无偏风险估计器(URE)，并将其扩展到可变元组大小、可变计数及其组合。引入简单的ReLU修正来改善有限样本稳定性，同时保持渐近正确性。

Result: 在转换为NTMP任务的基准测试中，该方法持续优于代表性的弱监督基线方法，产生有利的精确率-召回率和F1权衡。在类别先验不平衡和不同元组配置下保持鲁棒性。

Conclusion: 计数监督可以通过理论上有基础且实际稳定的目标有效利用，证明了仅基于计数的监督在弱监督学习中的有效性。
...
### [173] [Provable Generalization Bounds for Deep Neural Networks with Adaptive Regularization](https://arxiv.org/abs/2510.18410)
*Adeel Safder*
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易因高容量而出现过拟合问题，需要更有效的正则化方法来提升泛化性能。

Method: MAGDrop方法动态调整激活层的dropout率，基于当前梯度和累积动量，在非凸优化中增强稳定性。

Result: 在MNIST和CIFAR-10上分别达到99.52%和90.63%的测试准确率，比标准dropout和自适应梯度正则化方法提升1-2%。

Conclusion: MAGDrop通过理论分析和实验验证，为增强DNN泛化提供了鲁棒框架，适用于高风险应用场景。
...
### [174] [Learning Boltzmann Generators via Constrained Mass Transport](https://arxiv.org/abs/2510.18460)
*Christopher von Klitzing,Denis Blessing,Henrik Schopmans,Pascal Friederich,Gerhard Neumann*

Main category: cs.LG

TL;DR: 提出了一种名为约束质量传输(CMT)的变分框架，用于改进玻尔兹曼生成器在高维多模态分布中的采样效率，避免模式崩溃和质量传输问题。


<details>
  <summary>Details</summary>
Motivation: 传统变分方法最小化反向KL散度容易导致模式崩溃，而基于退火的方法使用几何调度可能遭受质量传输问题且严重依赖调度调优。

Method: 引入约束质量传输(CMT)框架，在连续步骤间对KL散度和熵衰减施加约束，增强分布重叠，减轻质量传输，防止过早收敛。

Result: 在标准BG基准测试和新引入的ELIL四肽系统上，CMT始终优于最先进的变分方法，获得超过2.5倍的有效样本量，同时避免模式崩溃。

Conclusion: CMT框架通过约束KL散度和熵衰减，显著提高了玻尔兹曼生成器的采样效率，解决了传统方法中的关键问题。
...
### [175] [Simple and Efficient Heterogeneous Temporal Graph Neural Network](https://arxiv.org/abs/2510.18467)
*Yili Wang,Tairan Huang,Changlong He,Qiutong Li,Jianliang Gao*

Main category: cs.LG

TL;DR: 提出了SE-HTGNN模型，通过动态注意力机制将时间建模整合到空间学习中，并利用大语言模型作为先验知识，在异构时序图上实现了10倍加速和最佳预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用解耦的时空学习范式，削弱了时空信息交互并导致高模型复杂度，需要新的学习范式来改善异构时序图的表示学习。

Method: 创新性地通过动态注意力机制将时间建模整合到空间学习中，保留历史图快照的注意力信息来指导后续计算；同时利用大语言模型捕捉节点类型的隐含属性作为先验知识。

Result: 在广泛实验中，SE-HTGNN相比最先进的基线方法实现了高达10倍的加速，同时保持了最佳预测精度。

Conclusion: SE-HTGNN通过整合时空学习和利用大语言模型先验知识，为异构时序图提供了一种简单高效的表示学习范式，在性能和效率上都显著优于现有方法。
...
### [176] [Benchmarking Fairness-aware Graph Neural Networks in Knowledge Graphs](https://arxiv.org/abs/2510.18473)
*Yuya Sasaki*

Main category: cs.LG

TL;DR: 该论文对知识图谱上的公平感知图神经网络进行了基准测试，发现在知识图谱上存在更明显的准确性与公平性权衡，性能受多种因素影响，预处理方法通常改善公平性指标，而处理方法改善预测准确性。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在处理图结构数据时存在对敏感属性的预测偏见，但之前的研究未在知识图谱上评估公平感知GNN，而知识图谱是许多应用中的重要图结构。

Method: 从YAGO、DBpedia和Wikidata三个知识图谱生成新图，基准测试了不同GNN骨干网络和早停条件下的处理和预处理方法。

Result: 发现知识图谱与其他图数据集相比显示出不同趋势：在公平感知GNN中准确性与公平性指标之间存在更清晰的权衡；性能不仅受公平感知GNN方法影响，还受GNN骨干网络和早停条件影响；预处理方法通常改善公平性指标，而处理方法改善预测准确性。

Conclusion: 知识图谱上的公平感知GNN研究具有重要意义，需要综合考虑方法类型、骨干网络和训练策略等多个因素来平衡准确性与公平性。
...
### [177] [Safe But Not Sorry: Reducing Over-Conservatism in Safety Critics via Uncertainty-Aware Modulation](https://arxiv.org/abs/2510.18478)
*Daniel Bethell,Simos Gerasimou,Radu Calinescu,Calum Imrie*

Main category: cs.LG

TL;DR: 提出不确定安全评论家(USC)方法，在强化学习中平衡安全性和任务性能，减少约40%的安全违规，同时保持或提高奖励。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法在强化学习安全探索中平衡安全约束和任务性能——严格的安全方法会削弱任务表现，而优先奖励的方法则频繁违反安全约束。

Method: 引入不确定安全评论家(USC)，将不确定性感知的调制和精炼集成到评论家训练中，在不确定和高成本区域集中保守性，同时在安全区域保持锐利梯度。

Result: USC减少约40%的安全违规，保持竞争性或更高的奖励，并将预测与真实成本梯度之间的误差减少约83%。
...
### [178] [Learning to Navigate Under Imperfect Perception: Conformalised Segmentation for Safe Reinforcement Learning](https://arxiv.org/abs/2510.18485)
*Daniel Bethell,Simos Gerasimou,Radu Calinescu,Calum Imrie*

Main category: cs.LG

TL;DR: COPPOL是一种基于共形预测的感知到策略学习方法，为语义分割提供有限样本安全保证，生成带有严格漏检界限的校准危险地图，用于下游RL规划。
...
Method: 集成分布无关的有限样本安全保证到语义分割中，生成带有严格漏检界限的校准危险地图，这些地图为下游RL规划诱导风险感知成本场。

Result: 在两个卫星衍生基准测试中，COPPOL相比基线方法将危险覆盖率提高了6倍，实现了近乎完全的不安全区域检测，同时将导航过程中的危险违规减少了约50%。
...
### [179] [Alibaba International E-commerce Product Search Competition DILAB Team Technical Report](https://arxiv.org/abs/2510.18499)
*Hyewon Lee,Junghyun Oh,Minkyung Song,Soyoung Park,Seunghoon Han*

Main category: cs.LG

TL;DR: DILAB团队开发的多语言电商搜索系统在最终排行榜获得第5名，整体得分0.8819，通过多阶段流水线解决了多语言查询-商品理解挑战。
...
Method: 采用多阶段流水线，包括数据精炼、轻量级预处理和自适应建模。数据精炼阶段增强数据集一致性和类别覆盖，建模阶段探索多种架构和微调策略，并使用精选验证集优化超参数。

Result: 系统在最终排行榜获得第5名，整体得分0.8819，在查询-类别和查询-商品任务中均表现出稳定且高性能的结果。

Conclusion: 该框架在多语言和跨领域场景中展现出鲁棒性和适应性，突显了系统性数据管理和迭代评估对多语言搜索系统的有效性。
...
### [180] [Partial VOROS: A Cost-aware Performance Metric for Binary Classifiers with Precision and Capacity Constraints](https://arxiv.org/abs/2510.18520)
*Christopher Ratigan,Kyle Heuton,Carissa Wang,Lenore Cowen,Michael C. Hughes*

Main category: cs.LG

TL;DR: 提出了一种新的ROC分析方法，通过考虑精度约束、容量限制和不对称成本，定义了部分VOROS指标来评估医院警报系统等应用中的分类器性能。


<details>
  <summary>Details</summary>
Motivation: 传统ROC分析无法捕捉医院患者监测等应用中的关键部署因素，如最小精度约束（避免误报疲劳）和预测阳性数量上限（代表医院人员容量），且AUC指标不能反映假阳性和假阴性的不对称成本。

Method: 展示了满足给定精度和容量约束的分类器子集如何在ROC空间中表示为可行区域，建立了该区域的几何特性，定义了部分较差分类器面积这一性能指标，该指标与成本单调相关且仅考虑ROC空间的可行部分。

Result: 在MIMIC-IV数据集上使用生命体征历史预测死亡风险的实验中，证明这种成本感知指标在医院警报应用中比替代方法更适合对分类器进行排序。

Conclusion: 提出的部分VOROS指标能够更好地评估在具有精度和容量约束的实际应用中分类器的性能，特别适用于医院警报系统等场景。
...
### [181] [Pay Attention to the Triggers: Constructing Backdoors That Survive Distillation](https://arxiv.org/abs/2510.18541)
*Giovanni De Muri,Mark Vero,Robin Staab,Martin Vechev*

Main category: cs.LG

TL;DR: 本文研究了从后门教师模型进行知识蒸馏的安全风险，提出了一种新的后门技术T-MTB，能够构建可转移的后门，并在越狱和内容调控两种攻击场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: LLMs常被用作教师模型进行知识蒸馏，但教师模型可能来自不可信方，蒸馏过程会带来安全风险。现有LLM后门方法选择的触发词在正常上下文中很少出现，导致后门难以转移到学生模型，这低估了知识蒸馏的安全风险。

Method: 提出T-MTB后门技术，构建由多个特定词组成的复合后门触发器，这些词在预期的蒸馏数据集中经常单独出现。这样教师模型保持隐蔽，而在蒸馏过程中这些词的单独存在为后门转移到学生模型提供了足够信号。

Result: 使用T-MTB在四个LLM模型族中验证了可转移后门的安全风险，涵盖了越狱和内容调控两种攻击场景，证明后门可以成功从教师模型转移到学生模型。

Conclusion: 知识蒸馏过程存在严重的安全风险，可转移的后门能够从教师模型传播到学生模型，需要重新评估知识蒸馏的安全假设并开发相应的防御机制。
...
<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法容易过早收敛，无法捕捉复杂特征关系，特别是在高维数据集中。

Method: 使用遗传算法，结合偏置初始化方案和比率引导变异机制，通过帕累托多目标优化同时最大化预测准确性和特征互补性。

Result: 在18个基准数据集上的实验表明，HeFS能一致识别被忽略的信息特征，在胃癌分类、药物毒性预测等挑战性领域表现优于现有方法。
...
### [183] [Robustness Verification of Graph Neural Networks Via Lightweight Satisfiability Testing](https://arxiv.org/abs/2510.18591)
*Chia-Hsuan Lu,Tony Tan,Michael Benedikt*

Main category: cs.LG

TL;DR: 提出RobLight工具，使用高效的部分求解器替代传统强大求解器来改进图神经网络的结构鲁棒性验证，在多项式时间内运行但可能不完整


<details>
  <summary>Details</summary>
Motivation: 图神经网络面临对抗攻击问题，现有技术通过约束求解来验证鲁棒性，但依赖强大的求解器效率有限

Method: 使用高效的部分求解器替代传统混合整数规划等强大求解器，这些部分求解器在多项式时间内运行但可能不完整

Result: 在多种GNN变体和数据集上评估RobLight工具，相比现有技术有所改进

Conclusion: 通过使用高效的部分求解器可以改进图神经网络结构鲁棒性验证的现有技术水平
...
### [184] [Unrolled-SINDy: A Stable Explicit Method for Non linear PDE Discovery from Sparsely Sampled Data](https://arxiv.org/abs/2510.18611)
*Fayad Ali Banna,Antoine Caradot,Eduardo Brandao,Jean-Philippe Colombier,Rémi Emonet,Marc Sebban*

Main category: cs.LG

TL;DR: 提出了Unrolled-SINDy方法，通过解耦数值时间步长与数据采样率，解决了稀疏时间采样数据下的PDE发现稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统SINDy方法在处理时间稀疏采样的真实世界问题时失效，因为局部截断误差过大导致无法恢复正确的方程参数。

Method: 利用展开方案改进显式方法的稳定性，通过迭代闭式方法或梯度下降方案实现，支持不同数值格式（欧拉、RK4）。

Result: 实验表明该方法通用性强，在传统SINDy和先进的iNeuralSINDy上都能解决非展开方法无法处理的问题。

Conclusion: Unrolled-SINDy通过解耦数值步长与采样率，有效提升了稀疏采样数据下物理动力学微分方程发现的稳定性。
...
### [185] [A Rectification-Based Approach for Distilling Boosted Trees into Decision Trees](https://arxiv.org/abs/2510.18615)
*Gilles Audemard,Sylvie Coste-Marquis,Pierre Marquis,Mehdi Sabiri,Nicolas Szczepanski*

Main category: cs.LG

TL;DR: 提出了一种将提升树蒸馏为决策树的新方法，旨在在预测性能和可解释性之间取得平衡，通过校正方法实现蒸馏过程。


<details>
  <summary>Details</summary>
Motivation: 为了生成在预测性能和可解释性之间达到可接受折衷的机器学习模型，需要将复杂的提升树模型蒸馏为更易解释的决策树。

Method: 使用称为校正的方法来实现蒸馏过程，将提升树转化为决策树。

Result: 实证研究表明，与通过重新训练模型实现的蒸馏方法相比，该方法提供了有前景的结果。

Conclusion: 通过校正方法将提升树蒸馏为决策树是一种有效的方法，能够在保持预测性能的同时提高模型的可解释性。
...
### [186] [Hardness of Learning Regular Languages in the Next Symbol Prediction Setting](https://arxiv.org/abs/2510.18634)
*Satwik Bhattamishra,Phil Blunsom,Varun Kanade*

Main category: cs.LG

TL;DR: 该论文研究了在下一个符号预测(NSP)设置下语言的可学习性，其中学习者接收正例以及每个前缀的额外信息。尽管NSP提供了比传统分类更丰富的标签，但学习DFA和布尔公式等概念类仍然是计算困难的。
...
Method: 通过构造使几乎所有额外标签变得无信息，将传统学习问题归约到具有NSP标签的学习问题，从而证明计算困难性。

Result: 在密码学假设下，学习DFA在NSP设置下是计算困难的，即使提供了更丰富的标签信息。

Conclusion: 尽管NSP设置提供了比传统分类更丰富的监督信号，但对于某些概念类(如DFA和布尔公式)的学习仍然保持计算困难性。
...
### [187] [Optimality and NP-Hardness of Transformers in Learning Markovian Dynamical Functions](https://arxiv.org/abs/2510.18638)
*Yanna Ding,Songtao Lu,Yingdong Lu,Tomasz Nowicki,Jianxi Gao*

Main category: cs.LG

TL;DR: 本文研究了Transformer在马尔可夫函数学习中的上下文学习能力，揭示了单层线性自注意力模型在表示结构化动态函数时的基本限制，并提供了多层模型的新解释。
...
Method: 通过结构化上下文学习设置，分析损失景观以揭示优化行为，包括推导单层线性自注意力模型的全局最小化器闭式解，证明恢复最优参数的NP难度，以及将多层模型解释为执行预处理梯度下降。

Result: 证明了单层线性自注意力模型在表示结构化动态函数时存在基本限制，恢复最优参数是NP难的，同时提供了多层模型优化多个目标的新解释。

Conclusion: Transformer在动态函数学习中的上下文学习能力受到单层模型的表示限制，但多层模型可以通过预处理梯度下降优化更广泛的目标，这些理论结果通过简化Transformer得到数值验证。
...
### [188] [Informed Learning for Estimating Drought Stress at Fine-Scale Resolution Enables Accurate Yield Prediction](https://arxiv.org/abs/2510.18648)
*Miro Miranda,Marcela Charfuelan,Matias Valdenegro Toro,Andreas Dengel*

Main category: cs.LG

TL;DR: 该研究提出了一种结合作物模拟模型物理可解释性和机器学习模型预测能力的混合方法，通过将作物产量建模为时间性水资源短缺的函数，并引入物理约束的损失函数，实现了高精度且可解释的产量预测。


<details>
  <summary>Details</summary>
Motivation: 传统作物模拟模型具有物理可解释性但性能较差，而机器学习模型预测能力强但缺乏物理一致性。本研究旨在结合两者的优势，解决农业产量预测中可解释性与准确性之间的矛盾。

Method: 将作物产量建模为时间性水资源短缺的函数，预测作物干旱胁迫和对水资源短缺的敏感性；使用多光谱卫星影像、气象数据和精细尺度产量数据；提出物理约束的损失函数；采用深度集成方法处理模型不确定性。

Result: 该方法在作物产量预测中超越了LSTM和Transformer等最先进模型，决定系数(R²)高达0.82，同时保持了高可解释性。

Conclusion: 该方法为行业、政策制定者和农民在气候变化背景下建设更具韧性的农业提供了决策支持，成功实现了物理一致性与预测准确性的平衡。
...
### [189] [Learning Time-Varying Turn-Taking Behavior in Group Conversations](https://arxiv.org/abs/2510.18649)
*Madeline Navarro,Lisa O'Bryan,Santiago Segarra*

Main category: cs.LG

TL;DR: 提出基于个体特征和过往发言行为的灵活概率模型，用于预测群体对话中的发言轮次模式，能够跨群体泛化并提供个性化分析。


<details>
  <summary>Details</summary>
Motivation: 现有对话动态模型通常无法超越单一群体进行泛化，且试图用统一公式描述发言行为可能不适合所有群体。需要开发能够基于个体特征预测任何群体中个体发言轮次的模型。

Method: 开发了先前对话模型的泛化版本，基于个体特征（人格特质）和过往发言行为预测群体中的发言轮次，能够学习个体发言倾向如何根据上次发言时间而变化。

Result: 在合成和真实对话数据上验证了该方法，结果表明先前行为模型可能不总是现实的，证明了这种数据驱动但理论基础的方法的必要性。

Conclusion: 该模型能够有效预测群体对话中的发言模式，提供了超越单一群体的泛化能力，并通过数据驱动方法揭示了现实对话动态的复杂性。
...
TL;DR: 提出一种用于心电和心音信号分类的轻量级卷积神经网络，在医疗边缘设备上实现高效推理，比现有方法减少三个数量级的内存和计算成本。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病早期检测很重要，但传统传感器数据分析需要高精度且低功耗的自动化方法，特别是在资源受限的边缘设备上。

Method: 使用早期融合的卷积神经网络对同步的心电和心音信号进行二分类，在Physionet Challenge 2016数据集上训练验证。

Result: 相比现有技术，内存占用和计算成本降低三个数量级，同时保持竞争力的准确率，在微控制器上验证了能效优势。

Conclusion: 提出的轻量级模型适合医疗边缘设备部署，设备端推理比持续数据流传输更节能。
...
### [191] [Reasoning Language Model Inference Serving Unveiled: An Empirical Study](https://arxiv.org/abs/2510.18672)
*Qi Li,Junpan Wu,Xiang Liu,Yuxin Wang,Zeyu Li,Zhenheng Tang,Yuhan Chen,Shaohuai Shi,Xiaowen Chu*

Main category: cs.LG

TL;DR: 本文对推理大语言模型(RLLM)的服务性能进行了全面研究，揭示了其在内存使用、请求处理、运行时间和领域偏好方面的独特行为特征，并评估了现有推理优化技术在RLLM上的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管RLLM在解决复杂推理任务方面表现出色，但其服务性能和行为特征尚未被充分探索，这阻碍了RLLM在实际场景中的部署和应用。

Method: 首先进行试点研究比较RLLM与传统LLM的服务性能，然后调查现有推理优化技术对RLLM的有效性，最后使用Gamma分布建模的真实工作负载进行评估。

Result: 发现RLLM具有显著的内存使用和波动、拖尾请求、自适应运行时间和领域偏好等独特行为；模型量化和推测解码能提升服务效率且对准确性影响小，而前缀缓存和KV缓存量化可能降低小RLLM的准确性或服务性能。
...
### [192] [Learning Task-Agnostic Representations through Multi-Teacher Distillation](https://arxiv.org/abs/2510.18680)
...
TL;DR: 提出了一个基于“多数投票”目标函数的任务无关蒸馏框架，通过最大化学生与教师嵌入之间的互信息来利用教师多样性，无需任务特定标签或先验知识。


<details>
  <summary>Details</summary>
Motivation: 现有多教师蒸馏方法通常针对特定任务，无法充分利用不同架构、损失函数和数据集产生的多样化嵌入表示。

Method: 使用“多数投票”目标函数，该函数受学生与教师嵌入间互信息的上界约束，从而推导出任务无关的蒸馏损失函数。

Result: 在文本、视觉模型和分子建模等多个领域的评估表明，该方法能有效利用教师多样性，在下游分类、聚类和回归任务中表现优异。

Conclusion: 该方法成功训练并发布了最先进的嵌入模型，在各种模态的下游任务中提升了性能，证明了任务无关蒸馏的有效性。
...
### [193] [Reinforcement Learning with Imperfect Transition Predictions: A Bellman-Jensen Approach](https://arxiv.org/abs/2510.18687)
*Chenbei Lu,Zaiwei Chen,Tongxin Li,Chenye Wu,Adam Wierman*

Main category: cs.LG

TL;DR: 提出贝叶斯价值函数和BOLA算法，解决传统RL在多步预测场景下的维度灾难问题，并证明在不完美预测下仍保持样本效率


<details>
  <summary>Details</summary>
Motivation: 传统RL基于单步转移模型，而现实应用中多步预测能提供决策优势，但直接嵌入会导致状态空间指数爆炸和维度灾难，现有RL理论无法分析带误差的多步预测

Method: 1. 提出贝叶斯价值函数表征最优预测感知策略；2. 开发Bellman-Jensen Gap分析；3. 提出BOLA两阶段算法：离线贝叶斯价值学习+轻量级在线适应

Result: 理论证明BOLA在不完美预测下仍保持样本效率，在合成MDP和真实风能存储控制问题上验证了算法有效性

Conclusion: 通过贝叶斯价值函数和BOLA算法，成功解决了多步预测RL的维度灾难问题，为预测增强的MDP提供了理论分析和实用算法
...
### [194] [OmniCast: A Masked Latent Diffusion Model for Weather Forecasting Across Time Scales](https://arxiv.org/abs/2510.18707)
*Tung Nguyen,Tuan Pham,Troy Arcomano,Veerabhadra Kotamarthi,Ian Foster,Sandeep Madireddy,Aditya Grover*

Main category: cs.LG

TL;DR: OmniCast是一个统一天气预测的时间尺度的概率模型，使用VAE编码和基于扩散的Transformer生成未来潜在标记，避免了自回归方法的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有的数据驱动方法在中短期天气预报中表现良好，但在更长的次季节到季节(S2S)时间尺度上由于自回归方法的误差累积而表现不佳。

Method: OmniCast包含两个组件：VAE模型将原始天气数据编码到低维潜在空间，基于扩散的Transformer模型生成未来潜在标记序列。训练时随机掩码未来标记，推理时通过迭代解掩码生成完整序列。

Result: 在中短期时间尺度上，OmniCast与领先的概率方法竞争性表现，同时快10-20倍；在次季节到季节尺度上，在准确性、基于物理和概率指标方面达到最先进水平，并能生成长达100年的稳定预测。

Conclusion: OmniCast通过联合空间和时间采样有效缓解了自回归方法的误差累积问题，低维潜在空间使得能够建模超越初始条件的天气动力学，实现了跨时间尺度的统一天气预测。
...
### [195] [Preference-based Reinforcement Learning beyond Pairwise Comparisons: Benefits of Multiple Options](https://arxiv.org/abs/2510.18713)
*Joongkyu Lee,Seouh-won Yi,Min-hwan Oh*

Main category: cs.LG

TL;DR: 本文提出M-AUPO算法，用于基于偏好的在线强化学习中的排序反馈问题，通过最大化提供子集内的平均不确定性来选择多个动作，实现了随子集大小增加而提高的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于偏好的强化学习理论研究大多只关注成对比较，少数探索多比较和排序反馈的工作在反馈长度增加时性能保证未能改善甚至恶化，尽管可获得更丰富的信息。

Method: 采用Plackett-Luce模型处理动作子集上的排序反馈，提出M-AUPO算法，通过最大化提供子集内的平均不确定性来选择多个动作。
...
Conclusion: 这是第一个在基于偏好的强化学习中明确显示样本效率随子集大小增加而改善的理论结果，建立了近乎匹配的下界$\Omega \left( \frac{d}{K \sqrt{T}} \right)$。
...
### [196] [Improving the Generation and Evaluation of Synthetic Data for Downstream Medical Causal Inference](https://arxiv.org/abs/2510.18768)
*Harry Amad,Zhaozhi Qian,Dennis Frauen,Julianna Piskorz,Stefan Feuerriegel,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: 提出STEAM方法，用于生成医学治疗效应分析所需的合成数据，满足下游因果推断任务的特定需求


<details>
  <summary>Details</summary>
Motivation: 真实医疗数据因监管障碍难以获取，而现有生成方法未考虑因果推断任务（特别是治疗效应分析）的特殊挑战

Method: 基于三个核心要求（保持协变量分布、治疗分配机制和结果生成机制），提出STEAM方法，模拟包含治疗的数据生成过程

Result: STEAM在提出的评估指标上达到最先进性能，特别是在真实数据生成过程复杂度增加时表现更优

Conclusion: STEAM方法能有效生成满足因果推断需求的合成医疗数据，为治疗效应分析提供有价值的替代数据源
...
### [197] [Enhancing Fractional Gradient Descent with Learned Optimizers](https://arxiv.org/abs/2510.18783)
*Jan Sobotka,Petr Šimánek,Pavel Kordík*

Main category: cs.LG

TL;DR: 提出了L2O-CFGD方法，通过元学习动态调整Caputo分数梯度下降的超参数，解决了分数梯度下降在收敛行为和超参数选择方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 分数梯度下降虽然能加速优化，但在非凸设置中面临收敛行为不稳定和超参数选择困难的问题，特别是其超参数的影响尚未完全理解且调度困难。

Method: 使用元学习方法动态调整Caputo分数梯度下降的超参数，通过元学习生成的调度策略优于静态超参数搜索。

Result: L2O-CFGD在多个任务中表现优于静态超参数的CFGD，在某些任务中甚至达到了与完全黑盒元学习优化器相当的性能。
...
### [198] [CAGE: Curvature-Aware Gradient Estimation For Accurate Quantization-Aware Training](https://arxiv.org/abs/2510.18784)
...
<details>
  <summary>Details</summary>
Motivation: 现有的低比特量化感知训练与原生训练之间存在较大的精度差距，需要新的方法来缩小这一差距。

Method: CAGE通过增强直通估计器梯度，加入曲率感知校正项来抵消量化引起的损失增加，该方法基于多目标优化视角，平衡损失最小化和量化约束。

Result: 在预训练800M参数的Llama风格模型时，CAGE在W4A4量化下比异常值处理方法多恢复了超过10%的量化损失。

Conclusion: 曲率感知梯度校正能够超越当前异常值处理方法，弥补剩余的性能差距。
...
### [199] [Stick-Breaking Embedded Topic Model with Continuous Optimal Transport for Online Analysis of Document Streams](https://arxiv.org/abs/2510.18786)
...
<details>
  <summary>Details</summary>
Motivation: 在线主题模型能够处理持续演化的数据流，与现实场景自然契合，但由于面临额外挑战，相比离线模型受到的关注较少。

Method: 扩展嵌入式主题模型（ETM），通过截断stick-breaking构造处理文档-主题分布，自动推断活动主题数量；引入基于连续最优传输的主题嵌入合并策略，适应高维潜在主题空间。

Result: 数值实验表明SB-SETM在模拟场景中优于基线方法，在2022-2023年俄乌战争新闻语料上进行了广泛测试。

Conclusion: SB-SETM成功解决了在线主题建模的挑战，在真实世界数据流中表现出色。
...
### [200] [On Biologically Plausible Learning in Continuous Time](https://arxiv.org/abs/2510.18808)
*Marc Gong Bacvanski,Liu Ziyin,Tomaso Poggio*

Main category: cs.LG

TL;DR: 该论文提出了一个连续时间神经网络模型，统一了多种生物可塑性学习算法，消除了阶段分离需求，并揭示了学习依赖于输入和误差信号的时间重叠。


<details>
  <summary>Details</summary>
Motivation: 生物学习是连续时间过程，但大多数算法模型依赖离散更新和分离的推理与学习阶段，需要开发更符合生物学习过程的连续时间模型。

Method: 开发连续时间神经网络模型，通过动态分析统一了SGD、FA、DFA和KP等学习规则，并进行模拟验证。

Result: 连续时间网络在生物时间尺度上稳定学习，学习强度随输入与误差信号时间延迟线性下降，稳健学习需要突触可塑性时间尺度超过刺激持续时间1-2个数量级。

Conclusion: 对于典型皮层刺激（数十毫秒），功能性可塑性窗口应在秒级范围内，这确定了秒级资格迹是生物电路中误差驱动学习的必要条件。
...
### [201] [When LRP Diverges from Leave-One-Out in Transformers](https://arxiv.org/abs/2510.18810)
*Weiqiu You,Siqi Zeng,Yao-Hung Hubert Tsai,Makoto Yamada,Han Zhao*

Main category: cs.LG

TL;DR: 研究发现AttnLRP中的双线性传播规则违反了实现不变性公理，并且绕过softmax层的相关性传播能显著提高与LOO的对齐度。


<details>
  <summary>Details</summary>
Motivation: LOO提供直观的特征重要性度量但计算昂贵，LRP作为高效替代方案在现代Transformer中的公理合理性尚未充分研究。

Method: 分析AttnLRP双线性传播规则的公理违反，并重新评估CP-LRP作为诊断基线，比较绕过softmax传播与LOO的对齐度。

Result: 双线性传播规则在理论分析和线性注意力层实证中都违反了实现不变性公理；绕过softmax传播在Transformer中后层显著改善了与LOO的对齐。
...
### [202] [A Unified Perspective on Optimization in Machine Learning and Neuroscience: From Gradient Descent to Neural Adaptation](https://arxiv.org/abs/2510.18812)
*Jesús García Fernández,Nasir Ahmad,Marcel van Gerven*

Main category: cs.LG

TL;DR: 该综述提供了一个关于迭代优化的统一视角，连接经典理论与神经网络训练和生物学习。重点讨论了零阶优化方法在神经网络模型中的有效性及其与生物学习机制的相似性。


<details>
  <summary>Details</summary>
Motivation: 尽管基于梯度的方法在机器学习中占主导地位，但其计算需求在高维环境中可能阻碍可扩展性。零阶优化方法计算更轻量，且其核心原则与生物学习机制相似，为理解大脑学习提供了数学原理视角。

Method: 首先根据使用的导数信息阶数对优化方法进行分类，从一阶、二阶、高阶梯度方法到零阶方法。然后探讨这些方法如何适应神经网络训练的独特挑战和学习动态。

Result: 现代零阶方法能够有效近似梯度，在神经网络模型中实现与反向传播相竞争的性能。零阶范式将大脑固有噪声视为计算资源，为生物学习提供了优化视角。

Conclusion: 零阶优化框架不仅加深了对自然智能的理解，还对神经形态硬件具有重要影响，有助于设计利用硬件固有噪声的快速、节能AI系统。
...
### [203] [Online SFT for LLM Reasoning: Surprising Effectiveness of Self-Tuning without Rewards](https://arxiv.org/abs/2510.18814)
*Mengqi Li,Lei Zhao,Anthony Man-Cho So,Ruoyu Sun,Xiao Li*

Main category: cs.LG

TL;DR: 提出了一种简单、自监督的在线监督微调(OSFT)方法，用于提升LLM的推理能力。该方法让模型生成自己的响应并立即在这些自生成数据上进行微调，无需奖励机制，仅需单次默认展开即可实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 旨在为LLM推理提供一种高效且无需复杂奖励机制的替代训练范式，简化传统基于奖励的训练方法。

Method: 采用在线监督微调(OSFT)范式，模型生成响应后立即在自生成数据上进行微调，无需外部奖励信号，仅需单次默认展开。

Result: 在具有挑战性的数学推理任务上，OSFT实现了与GRPO等强强化学习方法相当的下游性能，消融研究进一步证明了其效率和鲁棒性。

Conclusion: OSFT通过促进模型从预训练中学到的潜在偏好知识，有效提升了推理能力，为复杂奖励训练范式提供了高效且有前景的替代方案。
...
### [204] [Search Self-play: Pushing the Frontier of Agent Capability without Supervision](https://arxiv.org/abs/2510.18821)
...
TL;DR: 提出了一种搜索自博弈（SSP）方法，通过让LLM同时扮演任务提出者和问题解决者角色，在无需人工监督的情况下实现深度搜索代理的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR方法依赖人工构建的任务查询和真实答案，需要大量人工投入且难以控制生成任务的难度，限制了在代理场景下的可扩展性。

Method: 采用自博弈训练框架，LLM同时作为任务提出者生成具有明确定义答案且难度递增的搜索查询，以及作为问题解决者处理这些查询。通过检索增强生成确保每个查询都有准确的真实答案。

Result: SSP在各种基准测试中显著提升了搜索代理的性能，无需任何监督，在从头开始和持续RL训练设置下均表现优异。

Conclusion: 搜索自博弈方法能够有效实现代理RLVR的高可扩展性，通过竞争与合作共同进化代理能力。
...
### [205] [BO4Mob: Bayesian Optimization Benchmarks for High-Dimensional Urban Mobility Problem](https://arxiv.org/abs/2510.18824)
*Seunghee Ryu,Donghoon Kwon,Seongjin Choi,Aryan Deshwal,Seungmo Kang,Carolina Osorio*

Main category: cs.LG

TL;DR: BO4Mob是一个用于高维贝叶斯优化的新基准框架，专注于大型城市道路网络中的起点-终点出行需求估计问题，包含基于真实圣何塞道路网络的五个场景，输入维度高达10,100维。


<details>
  <summary>Details</summary>
Motivation: 解决从有限交通传感器数据估计起点-终点出行需求这一困难的反向优化问题，特别是在现实世界大规模交通网络中，该问题涉及在高维连续空间中进行优化，且目标评估计算昂贵、随机且不可微分。

Method: 构建基于真实圣何塞道路网络的五个场景，利用高分辨率开源交通模拟器，包含真实的非线性和随机动态。评估了三种最先进的贝叶斯优化算法和两种非贝叶斯优化基线方法。

Result: 开发了一个实用的基准框架，支持可扩展优化算法的开发及其在数据驱动的城市移动模型设计中的应用，包括大都市道路网络的高分辨率数字孪生。

Conclusion: BO4Mob基准框架为高维贝叶斯优化提供了有价值的测试平台，特别适用于大规模城市交通网络的复杂优化问题，代码和文档已开源。
...
### [206] [Actor-Free Continuous Control via Structurally Maximizable Q-Functions](https://arxiv.org/abs/2510.18828)
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于值的算法只能用于离散动作空间，在连续动作空间中通常使用actor-critic方法，但这些方法训练不稳定。本文旨在开发一种纯值函数的连续控制方法

Method: 采用结构最大化Q函数的框架，引入关键的架构和算法选择，实现无actor的Q学习

Result: 在标准仿真任务中表现出与最先进基线相当的性能和样本效率，在约束动作空间环境中尤其优于传统的actor-critic方法

Conclusion: 纯值函数方法在连续控制中是可行的，结构最大化在处理非平滑值函数时比基于梯度的最大化方法更有效
...
### [207] [A Hybrid Enumeration Framework for Optimal Counterfactual Generation in Post-Acute COVID-19 Heart Failure](https://arxiv.org/abs/2510.18841)
*Jingya Cheng,Alaleh Azhir,Jiazi Tian,Hossein Estiri*

Main category: cs.LG

TL;DR: 提出了一个反事实推理框架，用于个性化风险评估和干预分析，应用于COVID-19后遗症（PASC）在心力衰竭患者中的研究。


<details>
  <summary>Details</summary>
Motivation: 反事实推理为在替代干预下假设结果提供了数学框架，连接因果推理和预测建模，旨在识别可操作路径来改善患者预后。

Method: 结合正则化预测建模与反事实搜索，使用精确枚举和优化方法（NICE和MOC算法）探索高维干预空间。

Result: 在2700多名确诊SARS-CoV-2感染且有心衰病史的患者中，模型表现出强区分性能（AUROC: 0.88），并生成可解释的患者特异性反事实。
...
### [208] [Retaining by Doing: The Role of On-Policy Data in Mitigating Forgetting](https://arxiv.org/abs/2510.18874)
...
<details>
  <summary>Details</summary>
Motivation: 语言模型通过后训练适应新任务时会出现灾难性遗忘现象，需要找到缓解这一问题的指导原则。

Method: 在Llama和Qwen等模型家族上，系统比较SFT和RL在指令遵循、通用知识和算术推理任务中的遗忘模式，并通过简化设置分析RL模式寻求特性。

Result: RL在所有实验中都表现出比SFT更少的遗忘，同时目标任务性能相当或更好。RL使用在线策略数据的模式寻求特性是其抗遗忘的关键原因。

Conclusion: RL比SFT更能缓解灾难性遗忘，在线策略数据的使用是RL抗遗忘的核心机制，近似在线策略数据可有效缓解遗忘问题。
...
### [209] [From AutoRecSys to AutoRecLab: A Call to Build, Evaluate, and Govern Autonomous Recommender-Systems Research Labs](https://arxiv.org/abs/2510.18104)
*Joeran Beel,Bela Gipp,Tobias Vente,Moritz Baumgart,Philipp Meister*

Main category: cs.IR

TL;DR: 提出从狭隘的AutoRecSys工具转向端到端自动化的自主推荐系统研究实验室（AutoRecLab），包括问题构思、文献分析、实验设计执行、结果解释、论文草拟和溯源记录等全流程自动化。


<details>
  <summary>Details</summary>
Motivation: 推荐系统研究在模型和评估方面取得了进展，但忽视了研究过程本身的自动化。当前工具主要关注算法选择和超参数调优，缺乏端到端的自动化研究能力。

Method: 借鉴自动化科学的最新进展（如多智能体AI科学家系统），提出构建结合LLM驱动构思和报告与自动化实验的AutoRecLab原型，并建立评估标准、竞赛机制和评审渠道。

Result: 提出了AutoRecLab的详细议程，包括构建原型、建立基准、创建评审渠道、定义标准和促进跨学科对话等五个方面的具体行动建议。

Conclusion: 推进这一议程可以提高研究吞吐量，发现非显而易见的见解，使推荐系统为新兴的人工研究智能做出贡献，并呼吁组织社区研讨会来协调后续步骤。
...
### [210] [LIME: Link-based user-item Interaction Modeling with decoupled xor attention for Efficient test time scaling](https://arxiv.org/abs/2510.18239)
*Yunjiang Jiang,Ayush Agarwal,Yang Liu,Bi Xue*

Main category: cs.IR

TL;DR: LIME是一种新颖的推荐系统架构，通过低秩链接嵌入和线性注意力机制，显著降低了计算复杂度，实现了与最先进transformer模型相当的性能，但推理速度提升10倍。


<details>
  <summary>Details</summary>
Motivation: 现有transformer推荐系统的计算成本随用户序列长度呈二次方增长，随候选集大小呈线性增长，这限制了处理更长用户历史和更大候选集的能力。

Method: 1. 使用低秩链接嵌入预计算注意力权重，解耦用户和候选交互，使推理成本几乎与候选集大小无关；2. 提出LIME-XOR线性注意力机制，将用户序列长度的复杂度从O(N²)降低到O(N)。

Result: 在公共和工业数据集上的实验表明，LIME实现了与最先进transformer模型相近的性能，但在大候选集或长序列长度下推理速度提升10倍。在主要推荐平台测试中，LIME提高了用户参与度，同时保持较低的推理成本。

Conclusion: LIME为高效且表达能力强的推荐系统建立了新范式，解决了transformer在推荐系统中的计算复杂度问题。
...
### [211] [Enhancing Hotel Recommendations with AI: LLM-Based Review Summarization and Query-Driven Insights](https://arxiv.org/abs/2510.18277)
*Nikolaos Belibasakis,Anastasios Giannaros,Ioanna Giannoukou,Spyros Sioutas*

Main category: cs.IR

TL;DR: 开发了一个名为instaGuide的Web应用，利用大型语言模型(LLMs)自动总结Booking.com上的用户评论，帮助用户更高效地筛选短租公寓。


<details>
  <summary>Details</summary>
Motivation: 随着预订平台数据量激增，用户难以高效浏览住宿选项和分析评论。虽然平台提供了基于星级、设施、价格等的过滤功能，但最有价值的见解来自非结构化的文本评论，而逐条阅读这些评论非常耗时。

Method: 开发instaGuide Web应用，从Booking.com平台提取物业的文本评论，使用LLM模型进行评论总结，并允许用户查询物业的特定方面以获取个性化反馈。评估了多个LLM模型在准确性、成本和响应质量方面的表现。

Result: LLM驱动的总结显著减少了用户寻找合适短租公寓所需的时间，改善了整体决策过程。

Conclusion: 基于LLM的评论总结和挖掘能够有效提升短租公寓推荐系统的效率，为用户提供更有价值的见解。
...
### [212] [Evaluating LLM-Based Mobile App Recommendations: An Empirical Study](https://arxiv.org/abs/2510.18364)
*Quim Motger,Xavier Franch,Vincenzo Gervasi,Jordi Marco*

Main category: cs.IR

TL;DR: 该论文对大型语言模型在移动应用推荐中的表现进行了实证分析，揭示了其推荐标准与传统应用商店优化指标的部分不一致性，以及推荐一致性和对明确排名指令敏感度的变化。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地通过自然语言提示来推荐移动应用，这些推荐背后的推理过程不透明，引发了对其一致性、可解释性以及与标准ASO指标对齐性的疑问。

Method: 开发了一个系统评估框架来分析推荐一致性和对明确排名指令的响应性，并构建了包含16个通用排名标准的分类法。

Result: 研究发现LLMs依赖广泛但分散的排名标准，仅部分与标准ASO指标对齐。排名靠前的应用在不同运行中较为一致，但随着排名深度和搜索特定性的增加，变异性增加。LLMs对明确排名指令的敏感度各不相同。

Conclusion: 研究结果旨在帮助终端用户、应用开发者和推荐系统研究者在对话式应用发现的新兴领域中导航，支持AI推荐系统的可重复性和未来研究。
...
### [213] [LLMs as Sparse Retrievers:A Framework for First-Stage Product Search](https://arxiv.org/abs/2510.18527)
...
<details>
  <summary>Details</summary>
Motivation: 产品搜索中稀疏检索方法存在严重的词汇不匹配问题，导致性能不佳。大语言模型具有语义分析潜力，但直接应用于稀疏检索面临幻觉问题和训练初始化困难两大挑战。

Method: 提出PROSPER框架：1) 字面残差网络通过残差补偿机制强化被低估的字面术语；2) 词汇聚焦窗口采用从粗到细的稀疏化策略实现有效训练初始化。

Result: 离线和在线实验表明，PROSPER显著优于稀疏基线方法，召回性能可与先进的密集检索器相媲美，并在在线环境中实现了收入增长。

Conclusion: PROSPER成功解决了LLM在稀疏检索中的幻觉和训练初始化问题，为产品搜索提供了一种既高效又高性能的解决方案。
...
### [214] [Activation Manifold Projection: Liberating Task-Specific Behaviors from LLM Architectures](https://arxiv.org/abs/2510.17902)
*Al Kari*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过权重空间对齐进行模型迁移，这种方法脆弱且依赖参数几何的间接关联，无法有效解决LLM微调行为在不同架构间的移植问题。

Method: CAST学习源模型和目标模型激活流形之间的非线性映射，将预训练LoRA作为冻结的行为核，通过轻量级双向投影头在激活空间中进行转换。

Result: 在Llama-2和Mistral等异构模型家族间的迁移实验中，CAST转换的适配器能达到在目标模型上完全重新训练LoRA的85-95%性能。

Conclusion: CAST建立了一种新的模型互操作性标准，能够实现真正的零样本LoRA适配器翻译，显著优于现有的权重空间迁移技术。
...
### [215] [Beyond More Context: Retrieval Diversity Boosts Multi-Turn Intent Understanding](https://arxiv.org/abs/2510.17940)
*Zhiming Lin*
...
<details>
  <summary>Details</summary>
Motivation: 现实部署中的任务导向聊天机器人面临严格的token预算和噪声上下文，现有检索流程过于强调相关性而忽略了集合层面的多样性和混杂因素（如更多上下文或示例顺序）。

Method: 开发了多样性感知检索框架，选择上下文示例以平衡意图覆盖和语言多样性，并将此选择与标准LLM解码器集成；评估采用预算匹配提示和随机位置，并对示例数量、多样性强度和骨干模型大小进行敏感性分析。

Result: 在MultiWOZ 2.4和SGD数据集上，该方法在相同token预算下实现了联合目标准确率的显著提升，超越了强大的LLM/DST基线，在K从4到7的范围内保持一致的改进，且延迟适中。

Conclusion: 该研究分离并验证了检索中内容多样性的影响，为构建准确、预算受限的多轮意图系统提供了一个简单可部署的选择原则。
...
### [216] [FABRIC: Framework for Agent-Based Realistic Intelligence Creation](https://arxiv.org/abs/2510.17995)
...
<details>
  <summary>Details</summary>
Motivation: 收集智能体数据从人工标注者那里成本高、耗时长且难以扩展，需要一种可扩展的替代方案来支持智能体LLM的开发。

Method: 采用模块化流水线生成符合严格语法和语义约束的交互记录，包括约束生成格式、JSON模式验证和基于评判的过滤机制。
...
### [217] [OPTAGENT: Optimizing Multi-Agent LLM Interactions Through Verbal Reinforcement Learning for Enhanced Reasoning](https://arxiv.org/abs/2510.18032)
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统要么采用预定义结构，要么依赖多数投票或圆桌辩论，这会压制正确但非主导的智能体贡献。现有图网络方法只优化智能体性能，忽略了交互质量。

Method: 提出多智能体口头强化学习算法，定义动作空间和反馈机制来评估辩论过程中的沟通鲁棒性和连贯性，通过多数投票达成最终决策。
...
### [218] [Subject-Event Ontology Without Global Time: Foundations and Execution Semantics](https://arxiv.org/abs/2510.18040)
*Alexander Boldachev*

Main category: cs.AI

TL;DR: 提出了一种基于主体-事件的本体论形式化方法，用于在没有全局时间的条件下建模复杂动态系统。核心原则包括事件作为固定行为、因果顺序通过happens-before关系、可执行本体论、模型作为认知过滤器以及真实性假设。


<details>
  <summary>Details</summary>
Motivation: 传统系统依赖全局时间，但在分布式环境中难以实现。需要一种不依赖全局时间的方法来建模复杂动态系统，特别是处理分布式系统、微服务架构和多视角场景中的冲突事实。

Method: 提出九条公理(A1-A9)确保可执行本体论的正确性，包括历史单调性、因果无环性和可追溯性。采用基于模型的方法进行事件验证、参与者授权和自动构建因果链。在boldsea系统中实现，使用BSL语言。

Result: 开发了boldsea系统作为可执行本体论的工作流引擎，理论构念在BSL中实现。该方法适用于分布式系统、微服务架构、DLT平台和多视角场景。

Conclusion: 该形式化方法为没有全局时间的复杂动态系统建模提供了理论基础和实用工具，特别适用于处理分布式环境中的多视角和冲突事实问题。
...
### [219] [CompactPrompt: A Unified Pipeline for Prompt Data Compression in LLM Workflows](https://arxiv.org/abs/2510.18043)
...
Method: 使用自信息评分和基于依赖的短语分组从提示中修剪低信息令牌，同时对附加文档应用n-gram缩写和数值列的均匀量化。

Result: 在TAT-QA和FinQA基准数据集上，总令牌使用量和推理成本减少高达60%，同时保持输出质量（Claude-3.5-Sonnet和GPT-4.1-Mini的准确率下降小于5%）。
...
### [220] [Planned Diffusion](https://arxiv.org/abs/2510.18087)
*Daniel Israel,Tian Jin,Ellie Cheng,Guy Van den Broeck,Aditya Grover,Suvinay Subramanian,Michael Carbin*

Main category: cs.AI

TL;DR: 提出了一种结合自回归和扩散模型优势的混合方法——计划扩散，通过两阶段生成实现更快的文本生成速度，同时保持高质量输出。
...
Method: 两阶段方法：首先自回归生成一个短计划，将输出分解为更小的独立片段；然后使用扩散模型同时生成这些片段。

Result: 在AlpacaEval基准测试中，计划扩散在质量和延迟之间实现了帕累托最优权衡，相比自回归生成获得1.27x到1.81x的加速，仅损失0.87%到5.4%的胜率。

Conclusion: 计划扩散扩展了速度-质量的帕累托前沿，为更快、高质量的文本生成提供了实用路径，其计划机制简单可靠且具有灵活的质量-延迟权衡控制。
...
### [221] [SMaRT: Select, Mix, and ReinvenT -- A Strategy Fusion Framework for LLM-Driven Reasoning and Planning](https://arxiv.org/abs/2510.18095)
*Nikhil Verma,Manasa Bharadwaj,Wonjun Jang,Harmanpreet Singh,Yixiao Wang,Homa Fashandi,Chul Lee*

Main category: cs.AI

TL;DR: SMaRT框架通过融合多种推理策略来提升LLM性能，超越单一策略方法，在推理、规划和决策任务中表现优异。
...
Conclusion: 这项工作通过开创跨策略校准新范式，重新定义了LLM驱动的决策制定，为推理系统解锁了卓越结果并推进了自优化方法学的边界。
...
### [222] [Annotating the Chain-of-Thought: A Behavior-Labeled Dataset for AI Safety](https://arxiv.org/abs/2510.18154)
*Antonio-Gabriel Chacón Menke,Phan Xuan Tan,Eiji Kamioka*

Main category: cs.AI

TL;DR: 提出了一个句子级标注数据集，用于在LLM推理过程中基于激活的安全行为监控，填补了现有数据集只对推理进行整体标注的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本推理步骤的安全监控方法可能遗漏细微的有害模式，且可能被隐藏不安全推理的模型规避，需要更精确的激活层面监控。

Method: 构建包含推理序列的句子级标注数据集，标注安全行为如安全担忧表达或用户意图推测，并提取用于检测和影响这些行为的引导向量。

Result: 展示了该数据集的实用性，通过提取的表征能够在模型激活中检测和引导安全行为。

Conclusion: 激活层面的技术有潜力改进推理过程中的安全监督，为AI安全研究提供了重要工具。
...
### [223] [Measuring Reasoning in LLMs: a New Dialectical Angle](https://arxiv.org/abs/2510.18134)
*Soheil Abbasloo*

Main category: cs.AI

TL;DR: 提出了SIEV框架，基于辩证法思想评估语言模型的推理过程，而不仅仅是最终答案的正确性，揭示了现有模型在推理能力上的显著缺陷


<details>
  <summary>Details</summary>
Motivation: 当前评估方法只关注模型的最终答案正确性，但无法揭示产生答案的推理过程。推理应该是动态的、思想碰撞和演化的过程

Method: 基于辩证法（正题、反题、合题）构建SIEV框架，评估模型在推理过程中解决矛盾、整合不同观点和进行高阶综合的能力

Result: 在GSM和MMLU等饱和基准测试中，SIEV揭示了最先进模型（如GPT-5-chat）存在显著推理缺陷，在GSM上得分下降超过40分

Conclusion: 采用基于过程、哲学基础的方法能够对语言模型推理进行更深入、更严谨和更具区分度的评估
...
### [224] [Learning from Generalization Patterns: An Evaluation-Driven Approach to Enhanced Data Augmentation for Fine-Tuning Small Language Models](https://arxiv.org/abs/2510.18143)
...
TL;DR: PaDA-Agent是一种评估驱动的数据增强方法，通过协调操作简化SLMs的数据增强过程，发现验证数据中的失败模式并制定针对性策略来直接减少泛化差距。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型(SLMs)在部署成本和延迟方面具有优势，但在复杂领域特定任务上的准确性往往落后于大型模型。虽然有监督微调可以缩小性能差距，但需要大量手动数据准备和迭代优化工作。

Method: PaDA-Agent通过评估发现验证数据中的失败模式，制定针对性的数据增强策略，旨在直接减少泛化差距，而不是仅关注模型训练错误。

Result: 实验结果显示，在Llama 3.2 1B Instruct模型微调中，该方法相比最先进的基于LLM的数据增强方法有显著改进。

Conclusion: PaDA-Agent提供了一种有效的评估驱动数据增强方法，能够显著提升小型语言模型在复杂任务上的性能，同时减少手动数据准备的工作量。
...
### [225] [LLM-Based Multi-Agent System for Simulating and Analyzing Marketing and Consumer Behavior](https://arxiv.org/abs/2510.18155)
*Man-Lin Chu,Lucian Terhorst,Kadin Reed,Tom Ni,Weiwei Chen,Rongyu Lin*

Main category: cs.AI

TL;DR: 提出了一个基于大语言模型的多智能体模拟框架，用于模拟消费者决策和社交动态，为营销策略提供低成本、低风险的预实施测试工具。


<details>
  <summary>Details</summary>
Motivation: 传统的事后分析和基于规则的智能体模型难以捕捉人类行为和社交互动的复杂性，需要更有效的营销策略测试方法。

Method: 构建LLM驱动的多智能体模拟框架，让生成式智能体在沙盒环境中互动、表达内部推理、形成习惯并做出购买决策，无需预定义规则。
...
### [226] [Saber: An Efficient Sampling with Adaptive Acceleration and Backtracking Enhanced Remasking for Diffusion Language Model](https://arxiv.org/abs/2510.18165)
...
TL;DR: Saber是一种无需训练的DLM采样算法，通过自适应加速和回溯增强重掩码技术，在代码生成任务中同时提升推理速度和输出质量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在代码生成任务中面临推理速度与输出质量的关键权衡问题，减少采样步骤会导致性能灾难性下降。
...
### [227] [AgentChangeBench: A Multi-Dimensional Evaluation Framework for Goal-Shift Robustness in Conversational AI](https://arxiv.org/abs/2510.18170)
*Manik Rana,Calissa Man,Anotida Expected Msiiwa,Jeffrey Paine,Kevin Zhu,Sunishchal Dev,Vasu Sharma,Ahan M R*

Main category: cs.AI

TL;DR: 提出了AgentChangeBench基准，专门评估工具增强语言模型代理在对话中适应目标变化的能力，包含四个互补指标来衡量效果、可靠性、效率和适应延迟。
...
Method: 构建包含2,835个任务序列和五个用户角色的基准框架，通过四个指标（任务成功率、工具使用效率、工具调用冗余率、目标转移恢复时间）进行系统评估。

Result: 评估显示前沿模型在动态目标下表现差异显著：GPT-4o在机票预订任务中恢复率达92.2%，而Gemini仅为48.6%；零售任务参数有效性接近完美但冗余率超过80%。

Conclusion: 高原始准确率并不代表在动态目标下的鲁棒性，明确测量恢复时间和冗余率对于诊断和改进代理在现实企业环境中的韧性至关重要。
...
### [228] [Local Coherence or Global Validity? Investigating RLVR Traces in Math Domains](https://arxiv.org/abs/2510.18176)
*Soumya Rani Samineni,Durgesh Kalwar,Vardaan Gangal,Siddhant Bhambri,Subbarao Kambhampati*

Main category: cs.AI

TL;DR: 本文研究了RL后训练对LLM推理轨迹的影响，发现RL虽然提高了推理步骤的局部一致性（轨迹连贯性），但这并不保证最终答案的正确性或数学证明的有效性。
...
Result: RL后训练整体提高了轨迹连贯性，在基础模型失败但RL模型成功的问题上提升最显著。令人惊讶的是，RL增强了局部连贯性但未必产生有效或正确的解决方案。

Conclusion: 基于RL改善推理的主张必须谨慎审视，因为这种改善可能基于提高的轨迹连贯性，而这不一定转化为完全有效的数学证明。局部推理步骤连贯性的提高不能保证最终答案正确性。
...
### [229] [FST.ai 2.0: An Explainable AI Ecosystem for Fair, Fast, and Inclusive Decision-Making in Olympic and Paralympic Taekwondo](https://arxiv.org/abs/2510.18193)
*Keivan Shariatmadar,Ahmad Osman,Ramin Ray,Usman Dildar,Kisam Kim*

Main category: cs.AI

TL;DR: FST.ai 2.0是一个可解释的AI生态系统，通过姿态识别、不确定性建模和可视化解释，为跆拳道比赛和训练提供实时决策支持，显著减少决策审查时间并提高裁判信任度。
...
Method: 集成图卷积网络的姿态动作识别、通过可信集进行认知不确定性建模、可视化解释叠加层以及交互式仪表板，实现人机协作。

Result: 实验验证显示决策审查时间减少85%，AI辅助决策的裁判信任度达到93%。
...
### [230] [A Definition of AGI](https://arxiv.org/abs/2510.18212)
...
TL;DR: 本文提出了一个可量化的AGI评估框架，基于Cattell-Horn-Carroll人类认知理论，将通用智能分解为10个核心认知领域，并应用人类心理测量工具评估AI系统。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对AGI的具体定义，模糊了专业AI与人类水平认知之间的差距，需要建立可量化的评估标准。

Method: 基于Cattell-Horn-Carroll理论，将通用智能分解为10个认知领域，并采用成熟的人类心理测量工具来评估AI系统的认知能力。

Result: 应用该框架发现当代AI模型具有高度"锯齿状"的认知特征，在知识密集型领域表现出色，但在长期记忆存储等基础认知机制上存在严重缺陷。GPT-4得分为27%，GPT-5为58%。

Conclusion: 该框架量化了AI向AGI发展的进展和差距，显示虽然进步迅速，但距离真正的AGI仍有显著距离。
...
### [231] [ssToken: Self-modulated and Semantic-aware Token Selection for LLM Fine-tuning](https://arxiv.org/abs/2510.18250)
...
Method: 使用历史模型计算token损失差异作为自调制信号，结合注意力机制的语义重要性评估，实现自适应token选择

Result: 实验表明自调制选择和语义感知选择单独使用均优于全数据微调，两者结合进一步超越现有token级选择方法

Conclusion: ssToken通过自调制和语义感知的token选择策略，在无需额外模型的情况下实现了更好的SFT性能
...
### [232] [Illusions of reflection: open-ended task reveals systematic failures in Large Language Models' reflective reasoning](https://arxiv.org/abs/2510.18254)
*Sion Weatherhead,Flora Salim,Aaron Belbasis*

Main category: cs.AI

TL;DR: 当前大语言模型的'反思'功能与人类反思推理在功能上不等价，在开放式任务中表现有限，无法有效检测错误并进行约束敏感的修复。


<details>
  <summary>Details</summary>
Motivation: 研究当前大语言模型的反思能力是否真正等同于人类的反思推理，特别是在开放式但规则约束的任务中。

Method: 在八个前沿模型上测试生成有效科学测试项目的任务，要求模型先产生项目，然后基于自身批评进行修订。

Result: 首次尝试表现差（平均约1个有效项目），反思后只有轻微改进（同样约1个）。模型经常重复相同的约束违反，表明'修正收益'主要来自偶然产生有效项目而非错误检测和原则性修复。
...
### [233] [Genesis: Evolving Attack Strategies for LLM Web Agent Red-Teaming](https://arxiv.org/abs/2510.18314)
...
Conclusion: Genesis框架能够有效应对Web代理的安全风险，通过动态策略发现和持续优化机制，显著提升了攻击效果和泛化能力。
...
### [234] [Earth AI: Unlocking Geospatial Insights with Foundation Models and Cross-Modal Reasoning](https://arxiv.org/abs/2510.18318)
...
TL;DR: Earth AI是一个地理空间AI模型家族，通过多领域基础模型和Gemini驱动的智能推理引擎，解决地理空间数据分析的挑战。


<details>
  <summary>Details</summary>
Motivation: 地理空间数据具有巨大潜力，但其海量、多样性和不同的分辨率、时间尺度及稀疏性给全面分析和解释带来重大挑战。

Method: 构建了三个关键领域的基础模型（行星尺度影像、人口、环境）和Gemini驱动的智能推理引擎，开发了能够联合推理多个基础模型、大型地理空间数据源和工具的Gemini驱动代理。

Result: 基准测试展示了基础模型的能力，验证了联合使用时能提供互补价值并解锁卓越的预测能力。在真实危机场景基准测试中，代理能够提供关键及时的洞察。

Conclusion: Earth AI有效弥合了原始地理空间数据与可操作理解之间的差距，为地球洞察提供了重要进展。
...
### [235] [ShortcutBreaker: Low-Rank Noisy Bottleneck with Global Perturbation Attention for Multi-Class Unsupervised Anomaly Detection](https://arxiv.org/abs/2510.18342)
...
TL;DR: 提出ShortcutBreaker框架解决多类无监督异常检测中的身份捷径问题，通过低秩噪声瓶颈和全局扰动注意力机制防止输入直接复制到输出，在多个基准数据集上取得优异性能。
...
Result: 在四个基准数据集（三个工业数据集MVTec-AD、ViSA、Real-IAD和一个医学数据集Universal Medical）上分别达到99.8%、98.9%、90.6%和87.8%的图像级AUROC，显著优于现有方法。

Conclusion: ShortcutBreaker通过解决身份捷径问题，在多类无监督异常检测任务中取得了最先进的性能，证明了所提框架的有效性和通用性。
...
### [236] [Memory-Augmented State Machine Prompting: A Novel LLM Agent Framework for Real-Time Strategy Games](https://arxiv.org/abs/2510.18395)
*Runnan Qi,Yanan Ni,Lumin Jiang,Zongyuan Li,Kuihua Huang,Xian Guo*

Main category: cs.AI

TL;DR: 提出了Memory-Augmented State Machine Prompting (MASMP)框架，用于实时策略游戏中的LLM智能体，通过状态机提示和记忆机制解决幻觉和决策碎片化问题，在星际争霸II中达到60%胜率。
...
Result: 在星际争霸II中对战最强内置AI（Lv7）达到60%胜率，远超基线方法（0%），案例研究显示该方法保持了LLM的语义理解能力，同时通过严格状态-动作映射解决了"知行差距"问题。
...
### [237] [Heterogeneous Adversarial Play in Interactive Environments](https://arxiv.org/abs/2510.18407)
...
<details>
  <summary>Details</summary>
Motivation: 传统自博弈框架依赖智能体对称性，在零和竞争环境中有效，但无法适应非对称的开放学习场景。人类教学系统展示了非对称教学框架的优势，但如何在人工智能系统中实现这种自适应教学机制是主要挑战。
...
Conclusion: HAP框架成功实现了非对称自适应教学机制，为开放学习场景提供了有效的自动课程学习方法。
...
### [238] [Deep Learning-Based Control Optimization for Glass Bottle Forming](https://arxiv.org/abs/2510.18412)
...
<details>
  <summary>Details</summary>
Motivation: 在玻璃瓶制造中，精确控制成型机器对于确保质量和最小化缺陷至关重要，需要优化实时生产环境中的成型过程。

Method: 使用来自活跃制造工厂的真实操作数据，神经网络基于当前生产设置预测参数变化的影响，通过专门设计的反演机制识别实现所需玻璃料滴特性所需的最佳机器设置。
...
### [239] [Med-VRAgent: A Framework for Medical Visual Reasoning-Enhanced Agents](https://arxiv.org/abs/2510.18424)
*Guangfu Guo,Xiaoqian Lu,Yue Feng*

Main category: cs.AI

TL;DR: 提出Med-VRAgent框架，通过视觉引导和自我奖励结合MCTS树搜索，提升VLMs在医学视觉推理中的表现，并使用PPO微调进一步优化性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学推理中表现良好但存在幻觉、模糊描述、逻辑不一致和定位差等问题，需要改进。

Method: 基于视觉引导和自我奖励范式，结合蒙特卡洛树搜索，使用Med-VRAgent收集轨迹作为反馈，通过PPO目标微调VLMs。

Result: 在多个医学VQA基准测试中，该方法优于现有方法。

Conclusion: Med-VRAgent框架能有效提升VLMs的医学视觉推理能力，通过视觉引导和强化学习优化模型性能。
...
### [240] [Automated urban waterlogging assessment and early warning through a mixture of foundation models](https://arxiv.org/abs/2510.18425)
*Chenxu Zhang,Fuxiang Huang,Lei Zhang*

Main category: cs.AI

TL;DR: UWAssess是一个基于基础模型的框架，自动从监控图像中识别积水区域并生成结构化评估报告，解决了传统人工监测方法的不足。
...
Method: 采用半监督微调策略和思维链提示策略，利用基础模型解决标注数据稀缺问题，实现积水区域的自动识别和报告生成。

Result: 在视觉基准测试中感知性能显著提升，GPT评估确认能够生成准确描述积水范围、深度、风险和影响的可靠文本报告。

Conclusion: 该框架将内涝监测从感知转向生成，多基础模型协作架构为智能可扩展系统奠定了基础，支持城市管理、灾害响应和气候韧性。
...
### [241] [AlphaOPT: Formulating Optimization Programs with Self-Improving LLM Experience Library](https://arxiv.org/abs/2510.18428)
...
TL;DR: AlphaOPT是一个自改进的经验库系统，让LLM能够从有限的演示和求解器反馈中学习优化建模，无需标注推理轨迹或参数更新。


<details>
  <summary>Details</summary>
Motivation: 优化建模在各行业决策中至关重要，但难以自动化：需要将非正式语言映射为精确的数学公式和可执行的求解器代码。现有LLM方法要么依赖脆弱的提示，要么需要昂贵的重新训练且泛化能力有限。

Method: 采用持续的两阶段循环：(1) 库学习阶段：反思失败尝试，提取求解器验证的结构化洞察；(2) 库演化阶段：诊断检索偏差，细化存储洞察的适用条件，提高跨任务迁移能力。

Result: 实验显示AlphaOPT随数据量增加而稳步改进（从100到300训练项，准确率从65%提升到72%），在仅使用答案训练时，在分布外OptiBench数据集上比最强基线高出7.7%。

Conclusion: 该方法能高效从有限演示中学习，无需精心设计的推理过程；通过更新库而非模型权重实现持续扩展，无需昂贵的重新训练；使知识显式且可解释，便于人类检查和干预。
...
### [242] [PlanU: Large Language Model Decision Making through Planning under Uncertainty](https://arxiv.org/abs/2510.18442)
*Ziwei Deng,Mian Deng,Chenjing Liang,Zeming Gao,Chennan Ma,Chenxing Lin,Haipeng Zhang,Songzhu Mei,Cheng Wang,Siqi Shen*

Main category: cs.AI

TL;DR: PlanU是一种基于LLM的规划方法，通过蒙特卡洛树搜索(MCTS)中的分位数分布建模来捕捉不确定性，使用UCC评分平衡探索与利用，有效解决LLM决策中的不确定性挑战。


<details>
  <summary>Details</summary>
Motivation: LLM在不确定性环境中的决策能力不足，现有方法要么只处理LLM不确定性而忽略环境不确定性，要么无法处理多步决策任务。需要一种能同时处理LLM不确定性和环境不确定性的方法。

Method: PlanU将MCTS中每个节点的回报建模为分位数分布，使用一组分位数表示回报分布。引入UCC评分来估计MCTS节点的不确定性，平衡探索与利用。

Result: 通过广泛实验证明，PlanU在基于LLM的不确定性决策任务中表现出色。

Conclusion: PlanU通过将不确定性建模融入MCTS框架，有效提升了LLM在不确定性环境中的决策能力，为LLM决策提供了新的解决方案。
...
### [243] [CircuitSeer: Mining High-Quality Data by Probing Mathematical Reasoning Circuits in LLMs](https://arxiv.org/abs/2510.18470)
*Shaobo Wang,Yongliang Miao,Yuancheng Liu,and Qianli Ma,Ning Liao,Linfeng Zhang*

Main category: cs.AI

TL;DR: CircuitSeer：一种基于注意力头激活的数据选择方法，通过识别核心推理电路来高效选择训练数据，在仅使用10%数据的情况下实现优于全数据集训练的效果。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖昂贵的外部模型或不透明的启发式方法，而本文转向模型内部机制，发现复杂推理任务会激活稀疏的专门注意力头，形成核心推理电路。

Method: 提出CircuitSeer方法，通过测量数据对关键推理电路的影响来量化推理复杂度，从而选择高质量训练数据。

Result: 在4个模型和9个数据集上的实验显示，CircuitSeer仅使用10%数据就能让Qwen2.5-Math-7B在平均Pass@1上获得1.4分的提升，优于全数据集训练。

Conclusion: CircuitSeer通过利用模型内部推理电路实现了高效的数据选择，显著降低了训练成本同时提升了性能。
...
### [244] [Probabilistic Modeling of Intentions in Socially Intelligent LLM Agents](https://arxiv.org/abs/2510.18476)
*Feifan Xia,Yuyang Fang,Defang Li,Yantong Xie,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: 提出了一种用于多轮社交对话中LLM代理的概率意图建模框架，通过维护对合作伙伴潜在意图的信念分布，实现自适应对话策略。


<details>
  <summary>Details</summary>
Motivation: 在社交对话中，准确理解对话伙伴的意图对于LLM代理的社会智能至关重要，但意图通常是隐性的，需要动态建模。

Method: 框架维护对合作伙伴潜在意图的信念分布，从上下文先验初始化，并在每次话语后通过似然估计动态更新，为策略提供额外上下文基础。
...
### [245] [LAFA: Agentic LLM-Driven Federated Analytics over Decentralized Data Sources](https://arxiv.org/abs/2510.18477)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM代理的分析框架假设集中式数据访问，缺乏隐私保护；而联邦分析支持隐私保护但需要结构化查询，不支持自然语言输入。需要结合两者的优势。
...
### [246] [StarBench: A Turn-Based RPG Benchmark for Agentic Multimodal Decision-Making and Information Seeking](https://arxiv.org/abs/2510.18483)
...
Method: StarBench标准化评估八个战斗任务和两种模式：直接控制（仅接收截图并发出低级原语）和工具辅助控制（高级意图可通过检测器映射到原语）。还包括询问或行动诊断，测量代理是否以及何时选择在继续之前请求简短指导。

Result: 结果显示在直接控制模式下感知到控制的保真度存在显著差距，而明智的信息寻求与改进的成功率相关。

Conclusion: StarBench为真实客户端游戏中的主动信息寻求和多模态决策建立了可复现的衡量标准。
...
### [247] [AndroidControl-Curated: Revealing the True Potential of GUI Agents through Benchmark Purification](https://arxiv.org/abs/2510.18488)
*Ho Fai Leung,Xiaoyan Xi,Fei Zuo*

Main category: cs.AI

TL;DR: 论文发现现有GUI代理基准测试AndroidControl存在缺陷，通过改进创建了AndroidControl-Curated基准，使SOTA模型在复杂任务上的成功率从60%提升到75%。同时开发了Magma-R1-3B模型，仅用2.4k样本训练就达到与Qwen3-VL-235B相当的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有设备虚拟助手依赖刚性API的问题，以及GUI代理因基准测试缺陷而被低估性能的现状，推动更实用的设备端虚拟助手发展。

Method: 识别AndroidControl基准的模糊性和事实错误，通过严格净化流程改进为AndroidControl-Curated基准；使用2.4k精选样本在H20 GPU上训练60小时开发Magma-R1-3B模型。

Result: 在改进后的基准上，SOTA模型复杂任务成功率从约60%提升到近75%；Magma-R1-3B模型参数仅为Qwen3-VL-235B的1/200，但性能相当。

Conclusion: 设备端GUI代理的实际能力被低估，通过改进基准测试和高效模型训练，证明其已接近实际部署水平，为开发稳健的设备端虚拟助手提供了新路径。
...
### [248] [Crucible: Quantifying the Potential of Control Algorithms through LLM Agents](https://arxiv.org/abs/2510.18491)
...
Result: 在从经典控制任务到复杂计算机系统的广泛案例研究中验证了Crucible的有效性，并在实际部署中确认了其发现。实验结果显示Crucible能够系统量化不同算法的可调优空间。

Conclusion: Crucible为算法分析和设计提供了新的维度，最终带来性能提升，为算法评估开辟了新方向。
...
### [249] [Counterfactual Reasoning for Steerable Pluralistic Value Alignment of Large Language Models](https://arxiv.org/abs/2510.18526)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多元文化应用中的普及，需要超越平均原则（如HHH）来对齐多元人类价值观。现有方法难以处理价值观的相互依赖性和优先级控制。

Method: 提出COUPLE框架，使用结构因果模型（SCM）建模价值观间的复杂依赖关系和优先级，并通过反事实推理生成符合特定价值目标的输出。

Result: 在两个不同价值体系的数据集上评估，COUPLE在多种价值目标类型上优于其他基线方法。

Conclusion: COUPLE通过显式因果建模有效解决了价值观对齐中的复杂性和可操控性问题，并提供了更好的可解释性。
...
### [250] [Physics-guided Emulators Reveal Resilience and Fragility under Operational Latencies and Outages](https://arxiv.org/abs/2510.18535)
*Sarth Dubey,Subimal Ghosh,Udit Bhatia*

Main category: cs.AI

TL;DR: 开发了一个操作就绪的全球洪水意识系统模拟器，结合长短期记忆网络和松弛水平衡约束，在数据延迟、缺失或不一致时保持稳定性能。
...
Method: 结合长短期记忆网络与松弛水平衡约束，设计五种架构覆盖从完整历史预报到数据延迟和中断的不同信息可用性场景，在美国和印度5000多个流域进行训练和测试。

Result: 模拟器成功复现了GloFAS的水文核心，在信息质量下降时性能平稳退化，跨不同水文气候和管理体制的迁移产生物理一致但性能降低的结果。
...
### [251] [SOCIA-Nabla: Textual Gradient Meets Multi-Agent Orchestration for Automated Simulator Generation](https://arxiv.org/abs/2510.18551)
...
<details>
  <summary>Details</summary>
Motivation: 将脆弱的提示管道转换为可重复、约束感知的模拟器代码生成，实现跨领域和模拟粒度的扩展，同时最小化专家工作量。

Method: 使用文本梯度下降(TGD)优化器，在文本计算图中嵌入专门的LLM驱动智能体作为节点，执行损失驱动的循环工作流：代码合成→执行→评估→代码修复。

Result: 在三个CPS任务（用户建模、口罩采用和个人移动性）中实现了最先进的整体准确率。

Conclusion: 通过统一多智能体编排与损失对齐的优化视角，SOCIA-Nabla成功将脆弱的提示管道转换为可重复、约束感知的模拟器代码生成，能够跨领域和模拟粒度扩展。
...
### [252] [Extracting alignment data in open models](https://arxiv.org/abs/2510.18554)
...
TL;DR: 研究表明可以从后训练模型中提取大量对齐训练数据，这些数据可用于改进模型的特定能力，如长上下文推理、安全性、指令遵循和数学能力。


<details>
  <summary>Details</summary>
Motivation: 揭示模型在训练过程中可能被忽视的数据提取风险，特别是对齐训练数据的泄露问题，以及蒸馏实践可能带来的间接训练效应。

Method: 使用高质量的嵌入模型来识别训练数据提取，通过语义相似性而非传统的字符串匹配方法，更准确地评估数据提取量。

Result: 研究发现模型容易重现后训练阶段（如SFT或RL）使用的训练数据，这些数据可用于训练基础模型，恢复相当程度的原始性能。近似字符串匹配会严重低估可提取的数据量（约10倍）。

Conclusion: 这项工作暴露了对齐数据提取的潜在风险，并引发了对蒸馏实践下游效应的讨论：由于模型会重现其训练集的某些方面，蒸馏可被视为间接在模型原始数据集上进行训练。
...
### [253] [QuantEvolve: Automating Quantitative Strategy Discovery through Multi-Agent Evolutionary Framework](https://arxiv.org/abs/2510.18569)
...
Method: 使用与投资者偏好对齐的特征图来维护多样化有效策略，整合假设驱动的多智能体系统通过迭代生成和评估来系统探索策略空间。
...
### [254] [VAR: Visual Attention Reasoning via Structured Search and Backtracking](https://arxiv.org/abs/2510.18619)
*Wei Cai,Jian Zhao,Yuchen Yuan,Tianle Zhang,Ming Zhu,Haichuan Tang,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: VAR是一个新的多模态大语言模型框架，通过结构化搜索解决幻觉问题，将推理分解为可追踪的证据基础和基于搜索的思维链生成，包含回溯机制进行自我修正。
...
Method: 将基础推理重新构建为在推理轨迹空间上的结构化搜索，包含两个关键阶段：可追踪的证据基础和基于搜索的思维链生成，采用多面奖励函数进行语义和几何自我验证。

Result: VAR-7B模型在幻觉和安全基准测试中创下新的最先进水平，显著优于现有开源模型，并与领先的专有系统表现相当。

Conclusion: VAR框架通过结构化搜索和自我修正机制，有效解决了多模态大语言模型的幻觉问题，在推理任务中表现出色。
...
### [255] [Leveraging Association Rules for Better Predictions and Better Explanations](https://arxiv.org/abs/2510.18628)
*Gilles Audemard,Sylvie Coste-Marquis,Pierre Marquis,Mehdi Sabiri,Nicolas Szczepanski*

Main category: cs.AI

TL;DR: 提出一种结合数据和知识的分类方法，通过数据挖掘获取关联规则，提升树模型预测性能并改进解释能力。


<details>
  <summary>Details</summary>
Motivation: 传统分类方法主要依赖数据，缺乏知识整合。本文旨在结合数据挖掘得到的关联规则与树模型，提升分类性能和解释能力。

Method: 从数据中挖掘关联规则（可能包含否定），将这些规则与决策树和随机森林结合，用于分类任务和生成更一般的溯因解释。

Result: 实验表明该方法能提高树模型的预测性能，并生成更简洁的解释。

Conclusion: 结合数据和知识的分类方法在预测性能和解释能力方面均有优势，为树模型提供了新的改进方向。
...
### [256] [Comparative Expressivity for Structured Argumentation Frameworks with Uncertain Rules and Premises](https://arxiv.org/abs/2510.18631)
*Carlo Proietti,Antonio Yuste-Ginel*

Main category: cs.AI

TL;DR: 该论文研究了形式论证中定性不确定性的建模，提出了一个能够处理抽象和结构化形式主义的表达性概念，并比较了抽象和结构化不确定性论证模型的表达性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注论证不确定性的抽象模型，但缺乏对这些抽象模型的合理实例化研究。论文旨在填补这一空白，将论证的不确定性基于其组成部分（规则和前提）进行结构化建模。

Method: 通过将论证的不确定性基于其组成部分（规则和前提）进行结构化建模，引入了一个能够处理抽象和结构化形式主义的表达性概念，并比较了抽象和结构化模型的表达性。

Result: 提出了负面和正面的表达性结果，比较了抽象和结构化不确定性论证模型的表达性，涉及不完全抽象论证框架及其依赖扩展（抽象侧）和ASPIC+（结构化侧）。

Conclusion: 该研究为形式论证中定性不确定性的建模提供了理论基础，通过比较抽象和结构化模型的表达性，为实际应用和理论理解提供了重要见解。
...
### [257] [Query Decomposition for RAG: Balancing Exploration-Exploitation](https://arxiv.org/abs/2510.18633)
*Roxana Petcu,Kenton Murray,Daniel Khashabi,Evangelos Kanoulas,Maarten de Rijke,Dawn Lawrie,Kevin Duh*

Main category: cs.AI

TL;DR: 本文提出了一种基于多臂老虎机框架的检索增强生成系统，通过动态选择信息最丰富的子查询来平衡检索的广度和精度。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中检索效率的权衡问题：既要广泛检索以捕捉所有相关材料，又要限制检索以避免过多噪声和计算成本。

Method: 将查询分解和文档检索建模为利用-探索设置，使用各种老虎机学习方法动态选择最有信息的子查询，并利用排名信息和人工判断估计文档相关性。

Result: 使用排名信息和人工判断估计文档相关性，在文档级精度上获得35%的提升，α-nDCG提高15%，并在长文本生成的下游任务中表现更好。

Conclusion: 基于老虎机学习的方法能有效平衡RAG系统中的检索广度和精度，显著提升检索质量和下游生成任务的性能。
...
### [258] [Sherlock Your Queries: Learning to Ask the Right Questions for Dialogue-Based Retrieval](https://arxiv.org/abs/2510.18659)
...
<details>
  <summary>Details</summary>
Motivation: 用户查询在信息检索中常常存在歧义，现有对话式交互检索系统缺乏明确的策略来提出最具信息量的问题，导致效率低下。

Method: 使用强化学习训练智能体生成二进制问题序列，通过最优提问策略来缩小搜索空间。

Result: 在结构化任务中性能与强基线相当，接近二分搜索的理论最优；在非结构化任务中显著优于基线方法。
...
### [259] [Seg the HAB: Language-Guided Geospatial Algae Bloom Reasoning and Segmentation](https://arxiv.org/abs/2510.18751)
...
Method: 整合GeoSAM辅助人工评估进行高质量分割掩码整理，并在NASA的CAML数据集上微调视觉语言模型进行严重程度预测。
...
### [260] [Decoding Funded Research: Comparative Analysis of Topic Models and Uncovering the Effect of Gender and Geographic Location](https://arxiv.org/abs/2510.18803)
*Shirin Tavakoli Kafiabad,Andrea Schiffauerova,Ashkan Ebadi*

Main category: cs.AI

TL;DR: 该研究比较了三种主题建模方法(LDA、STM、BERTopic)在分析加拿大NSERC 18年研究提案中的应用，并开发了COFFEE算法来增强BERTopic的协变量分析能力。


<details>
  <summary>Details</summary>
Motivation: 优化国家科学投资需要了解研究趋势演变及人口地理因素影响，特别是在促进公平、多元和包容的背景下。

Method: 分析2005-2022年NSERC资助的研究提案，比较LDA、STM和BERTopic三种主题建模方法，并开发COFFEE算法用于BERTopic的协变量效应估计。

Result: BERTopic在识别更精细、连贯和新兴主题方面表现最佳，如人工智能的快速扩张。协变量分析揭示了省级研究专业化和跨学科一致的性别主题模式。
