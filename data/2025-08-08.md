...
Conclusion: RetinexDual通过独特的分支设计和组件组合，有效解决了超高清图像恢复中的关键问题，展示了其优越性能。
...
<details>
  <summary>Details</summary>
Motivation: ENT内窥镜分析的自动化因设备、操作者差异及细微局部发现等问题而受限，现有公共基准很少支持类似案例的检索需求。

Method: 提出ENTRep数据集，包含专家标注的图像和解剖区域标签，并定义三个基准任务，标准化提交协议，通过服务器端评分评估性能。

Result: 报告了表现最佳团队的结果，并提供了深入讨论。

Conclusion: ENTRep为ENT内窥镜分析提供了一个综合性的基准，支持分类和检索任务，填补了现有公共基准的不足。
...
Method: CoMAD通过非对称掩码、线性适配器和联合共识门控，从多个ViT-Base教师模型中蒸馏知识。

Result: 在ImageNet-1K上，ViT-Tiny达到75.4% Top-1准确率；在密集预测任务中，性能优于现有方法。

Conclusion: CoMAD为紧凑自监督蒸馏设定了新标准，展示了多教师模型整合的有效性。
...
Conclusion: RADAR通过无重建方法解决了传统扩散模型的局限性，为实时异常检测提供了高效解决方案。
...
TL;DR: 研究利用事件相机和深度学习技术，开发了一种低成本且高精度的眼球追踪算法，用于预测用户注意力，提升VR/AR设备的用户体验。


<details>
  <summary>Details</summary>
Motivation: 眼球快速运动（可达300度/秒）的精确追踪通常需要昂贵的高速相机，研究旨在通过事件相机和深度学习降低成本并提高实用性。

Method: 采用CNN_LSTM模型，结合事件相机的输入数据，定位眼球中心位置（x, y）。

Result: 模型达到约81%的准确率，证明了其有效性。

Conclusion: 未来计划通过Layer-wise Relevance Propagation（LRP）进一步提升模型的解释性和预测性能。
...
Method: 使用离散小波变换编码时间信息，空间投影层捕捉关节依赖关系，KAN层参数化Lucas多项式进行高效函数逼近，最后通过逆小波变换重建运动序列。

Result: 在三个基准数据集上表现优于基线模型，兼具定量和定性优势，且计算效率高。

Conclusion: LuKAN模型在3D人体运动预测中实现了高精度和高效计算的平衡。
...
TL;DR: 论文提出VER-Bench框架，用于评估多模态大语言模型（MLLMs）在识别细粒度视觉线索和结合世界知识进行复杂推理的能力。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试在评估MLLMs视觉能力时存在不足，无法全面测试模型对细微视觉线索的识别和复杂推理能力。

Method: 引入VER-Bench框架，包含374个精心设计的问题，覆盖多种推理类型，并提供结构化证据支持。

Result: VER-Bench揭示了当前模型在提取细微视觉证据和构建基于证据的推理方面的局限性。

Conclusion: 研究强调需要提升模型在细粒度视觉证据提取、整合和推理方面的能力，以实现真正的视觉理解和类人分析。
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based检测器在遮挡、细粒度定位和计算效率方面存在问题，DAMM旨在解决这些问题。

Method: DAMM引入三种查询（视觉语言模型的外观查询、多边形嵌入的位置查询和随机学习查询）和双流交叉注意力模块。

Result: 在四个基准测试中，DAMM在平均精度和召回率上达到SOTA性能。

Conclusion: 多模态查询适应和双流注意力有效提升了检测器的性能。
...
TL;DR: 论文研究了多模态仇恨视频中标签噪声的影响，通过精细时间标注分析仇恨与非仇恨内容，发现粗粒度标注会混淆模型决策边界，提出需要时间感知模型以提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在线多媒体内容的快速增长加剧了仇恨言论的传播，现有方法依赖粗粒度视频级标注，忽略了仇恨内容的时间粒度，导致标签噪声问题。

Method: 从HateMM和MultiHateClip数据集中提取时间标注的仇恨片段，分析其分布与特征，并通过实验验证标签噪声对模型的影响。

Result: 实验表明，时间标注噪声会改变模型决策边界并降低分类置信度，凸显仇恨言论的上下文依赖性和时间连续性。
...
Method: 提出Highlight-TTA框架，结合主任务（高光检测）和辅助任务（跨模态幻觉），采用元辅助训练方案，测试时通过辅助任务自适应调整模型。
...
Method: 引入校准令牌调整潜在嵌入分布，自监督学习，利用公开透视图像数据集生成鱼眼图像。

Result: 在室内外场景中，方法优于现有技术，仅需一组令牌即可适配多种FMDEs。

Conclusion: 通过潜在嵌入调制，避免了传统方法中的伪影和损失，实现了高效适配。
...
Result: 模型在10个批次分区中平均完美分类285.9个标签，但未达到100%准确率，推测因数据集存在双标签问题。
...
Method: ProMIM采用掩码策略生成实例条件提示，增强现有方法（如CoOp和CoCoOp）的鲁棒性，同时计算成本极低。
...
TL;DR: 论文提出了一种名为TRKT的方法，用于解决弱监督动态场景图生成（WS-DSGG）中外部目标检测器性能不足的问题。TRKT通过关系感知知识挖掘和双流融合模块，提升了动态场景中的目标检测和关系预测能力。
...
Method: TRKT包含两个关键组件：1）关系感知知识挖掘，通过类别特定注意力图和帧间注意力增强策略提取关系与运动感知知识；2）双流融合模块，将注意力图与外部检测结果结合以优化目标定位和置信度。

Result: 在Action Genome数据集上的实验表明，TRKT达到了最先进的性能。
...
TL;DR: AdvDINO是一种自监督学习框架，通过对抗性训练解决领域偏移问题，提高了生物医学图像中特征学习的鲁棒性和生物学意义。


<details>
  <summary>Details</summary>
Motivation: 标准自监督学习方法在领域偏移（如生物医学图像中的批次效应）下的鲁棒性不确定，影响模型泛化和可解释性。

Method: 在DINOv2架构中引入梯度反转层，实现对抗性训练以学习领域不变特征。

Result: 在非小细胞肺癌患者的mIF图像数据中，AdvDINO减少了批次偏差，发现了具有不同蛋白质组和预后意义的表型簇，并提高了生存预测性能。

Conclusion: AdvDINO不仅适用于生物医学图像，还可推广到其他存在领域偏移和标注数据有限的领域。
...
TL;DR: HOW-Seg是一种基于人类反馈的开环点云语义分割框架，通过稀疏标注和层次原型消歧机制，实现对基类和新类的高质量分割。
...
Method: 构建查询数据的类原型，利用稀疏人类标注指导分割，引入层次原型消歧机制优化模糊原型，并使用密集CRF优化标签分配。

Result: 在稀疏标注（如每新类一次点击）下，HOW-Seg性能与5-shot GFS-Seg相当或更优；使用高级骨干和密集标注时，mIoU显著提升。

Conclusion: HOW-Seg通过人类反馈动态优化分割结果，为基类和新类提供高质量分割，显著优于现有方法。
...
TL;DR: 论文提出了一种基于自适应高斯权重和不确定性学习的方法，提升了稀疏视角下3D高斯渲染的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯渲染方法在稀疏视角下容易过拟合，导致渲染质量下降。

Method: 通过学习的自适应高斯权重和不确定性，优化高斯不透明度更新，并引入软可微丢弃正则化。

Result: 在多个数据集上表现优于现有稀疏视角方法，如PSNR提升3.27%。

Conclusion: 自适应权重和不确定性学习显著提升了稀疏视角下的渲染质量，减少了高斯数量需求。
...
TL;DR: 本文提出了一种改进的基于画布的注意力调度方法，支持可变大小的画布帧和可选的帧率，提升了边缘平台上实时目标检测的性能。


<details>
  <summary>Details</summary>
Motivation: 边缘平台在有限计算资源下实现高分辨率目标检测面临延迟约束的挑战，需要优化资源利用。

Method: 扩展了基于画布的注意力调度，引入可变画布帧大小和可选帧率，并在NVIDIA Jetson Orin Nano上运行YOLOv11进行验证。

Result: 实验结果表明，该方法在Waymo Open Dataset上实现了更高的mAP和召回率，优化了质量与成本的权衡。

Conclusion: 改进的注意力调度方法显著提升了边缘平台上实时目标检测的性能，优于现有技术。
...
TL;DR: SODEC是一种单步扩散图像压缩模型，解决了传统扩散方法解码延迟高和保真度低的问题，通过单步解码和保真度引导模块显著提升了性能和速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度补全模型在分布外（OOD）场景中性能显著下降，而基础模型在大规模训练中表现出鲁棒性，因此利用基础模型提升深度补全的鲁棒性。

Method: 结合深度基础模型提取RGB图像的环境线索，设计双空间传播方法（3D和2D），并引入可学习的校正模块逐步调整深度预测。

Result: 在16个数据集上广泛评估，模型在OOD场景中表现优异，优于现有最先进方法。

Conclusion: 该框架通过利用基础模型和双空间传播，显著提升了深度补全的鲁棒性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLMs）在无监督域适应（UDA）中存在模态差距问题，导致仅传递模态不变知识，性能受限。

Method: 提出模态分离框架，分离VLM特征中的模态特定和不变组件，设计模态差异度量分类样本，并通过提示调优技术优化。

Result: 实验表明，方法在多种设置下性能提升达9%，计算效率提高9倍。

Conclusion: 模态分离框架有效解决了模态差距问题，显著提升了UDA性能。
...
TL;DR: 研究表明，早期视觉皮层能快速学习全局图像上下文，通过稀疏化群体响应和降低平均活动实现。研究使用ViT自编码器探索快速权重的作用，发现LoRA能增强全局敏感性。


<details>
  <summary>Details</summary>
Motivation: 探索快速学习如何通过快速权重（如LoRA）在深度神经网络中引入全局上下文敏感性，模拟大脑的快速学习机制。

Method: 采用ViT自编码器，结合LoRA实现快速权重，分析自注意力机制和潜在表征的变化。

Result: ViT自编码器模拟了神经回路的流形变换，早期层与顶层表征对齐，自注意力范围扩大，LoRA显著增强效果。

Conclusion: 快速权重架构为研究大脑快速全局上下文学习提供了可行计算模型，早期层通过训练获得全局敏感性。
...
Conclusion: AG-ReID有效解决了遮挡和属性差异问题，同时保持标准场景的竞争力。
...
TL;DR: 论文提出了一种名为CRAM的持续学习方法，通过压缩视频数据并定期刷新视频编码来解决视频持续学习中的高内存需求和灾难性遗忘问题。
...
Method: 使用压缩视觉技术存储视频编码而非原始数据，并通过定期刷新编码来应对灾难性遗忘。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像数据的高变异性与复杂性导致现有视觉语言模型（如CLIP）在医学影像任务中泛化能力不足。

Method: MCDRL分两步：1）利用CLIP的跨模态能力识别病变区域并构建领域特定变异的字典；2）训练因果干预网络消除这些变异的影响，同时保留关键解剖结构信息。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏专门用于评估AI-UGC感知质量的模型，限制了用户体验和增强方法的发展。

Method: 构建了包含4,800张AI-UGC图像的AU-IQA数据集，并评估了多种现有质量评估模型的性能。

Result: 论文提供了对现有方法在评估AI-UGC感知质量方面的全面分析。

Conclusion: AU-IQA数据集的建立为AI-UGC质量评估提供了重要基准，推动了该领域的进一步发展。
...
TL;DR: 皮肤癌是全球最常见的癌症，每年医疗支出超过80亿美元。早期诊断和及时治疗对提高患者生存率至关重要。临床中，医生使用SOAP笔记记录患者就诊情况，但手动生成这些笔记耗时且易导致医生疲劳。本文提出skin-SOAP框架，通过弱监督多模态方法从有限输入（如病变图像和稀疏临床文本）生成结构化SOAP笔记，减少对人工标注的依赖，同时减轻医生负担。该方法在关键临床指标上表现与GPT-4o、Claude和DeepSeek Janus Pro相当。
...
Result: 在临床相关指标上表现与先进模型（如GPT-4o）相当，并引入MedConceptEval和CCS两项新评估指标。

Conclusion: skin-SOAP框架能有效生成临床相关的SOAP笔记，减轻医生负担，且无需大量标注数据。
...
TL;DR: 论文提出了一种新的图像质量评估方法SCSSIM，专注于评估生成AI模型输出的场景构图结构（SCS）完整性，解决了传统方法的不足。


<details>
  <summary>Details</summary>
Motivation: 传统图像相似性度量方法无法有效评估SCS完整性，像素级方法对视觉噪声敏感，而感知度量则偏重美学，均无法准确捕捉结构保真度。

Method: 提出SCSSIM，一种无需训练的度量方法，利用图像的立方体层次分区统计量来量化SCS保真度。

Result: 实验表明SCSSIM对非构图失真具有高不变性，对构图失真则表现出单调递减特性，优于现有度量方法。

Conclusion: SCSSIM是一种高效且准确的SCS评估工具，有助于生成模型的开发和评估。
...
Conclusion: HAMoBE通过分层和自适应特征整合，有效提升了视频行人重识别的鲁棒性和准确性。
...
Method: 采用双路径设计：一条路径优化反照率，另一条路径专注于金属和粗糙度估计；通过特征蒸馏确保一致性，并使用整流流提高效率。
...
Conclusion: DecoupleCSS通过解耦设计有效平衡了保留与适应性，为CSS提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 图像着色是一个高度不适定问题，但场景语义和纹理提供了重要线索，现有大量数据可用于学习先验知识。

Method: 采用分类和对抗学习方法，基于先前工作构建模型并进行改进和比较。

Result: 未明确提及具体结果，但强调了方法的改进和比较。

Conclusion: 分类和对抗学习是解决图像着色多模态问题的有效方法。
...
TL;DR: FLUX-Makeup是一种无需辅助人脸控制组件的高保真、身份一致且鲁棒的化妆迁移框架，通过直接利用源-参考图像对实现卓越性能。
...
Conclusion: FLUX-Makeup无需辅助组件，实现了高保真化妆迁移，为实际应用提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 病理基础模型（PFMs）的预训练背景多样且不透明，导致潜在偏差，影响下游应用的泛化性和透明度。
...
TL;DR: PoseGen是一个新颖的框架，通过单张参考图像和驱动姿态序列生成任意长度的视频，解决了扩散模型在身份漂移和视频长度限制上的问题。
...
Method: 采用in-context LoRA微调策略注入主题外观，并通过通道级姿态信息控制运动；提出交错片段生成方法实现视频无缝拼接。

Result: PoseGen在身份保真度、姿态准确性和生成无限长度视频方面显著优于现有方法。

Conclusion: PoseGen通过创新的微调和生成策略，实现了高质量、长时间一致的视频生成。
...
TL;DR: SMP是一种新型的FSCIL方法，通过在不同阶段策略性地整合边缘惩罚，解决了基类可区分性和新类泛化之间的平衡问题，并在实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现实应用中数据隐私和获取成本高，导致增量任务中训练数据不足，性能下降。现有方法难以平衡基类可区分性和新类泛化。

Method: 提出SMP方法，包括MIAM机制（基任务学习）和MPCC策略（增量任务），通过边缘惩罚优化分类器。

Result: 在CIFAR100、ImageNet-R和CUB200上，SMP实现了最先进的性能，并更好地平衡了基类和新类。

Conclusion: SMP通过边缘惩罚和参数高效微调，显著提升了FSCIL的性能和平衡性。
...
TL;DR: AHDMIL框架通过两步训练过程（自蒸馏和非对称蒸馏）减少无关图像块处理，提高病理图像分类的速度和准确性。
...
Method: 提出AHDMIL框架，包含动态多实例网络（DMIN）和双分支轻量级实例预筛选网络（DB-LIPN），通过自蒸馏和非对称蒸馏优化分类。

Result: 在四个公开数据集上表现优于现有方法，分类性能和推理速度均有提升，例如在Camelyon16数据集上准确率提高5.3%，推理速度提升1.2倍。

Conclusion: AHDMIL在病理图像分类中实现了高效和准确的平衡，代码已公开。
...
TL;DR: 提出了一种新的视觉定位框架，通过多潜在表达增强文本输入，结合视觉细节，提升目标对象定位的准确性。
...
Method: 引入主题分发器和视觉概念注入器模块，嵌入共享主题和独特属性概念，采用正边际对比学习策略对齐潜在表达与原始文本。
...
TL;DR: FedGIN是一种联邦学习框架，通过轻量级GIN增强模块解决多模态医学图像分割中的数据稀缺和隐私问题，显著提升了跨模态泛化性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割在多模态数据中面临数据稀缺、领域偏移和隐私限制的挑战，需要一种无需共享原始数据的统一模型。

Method: 提出FedGIN框架，结合GIN增强模块协调多模态强度分布，并在有限和完整数据集上评估其性能。

Result: 在有限数据场景下，FedGIN在MRI测试中Dice分数提升12-18%；在完整数据场景下，性能接近集中式训练，跨模态泛化能力显著。

Conclusion: FedGIN在多模态医学图像分割中表现出色，解决了隐私和数据共享问题，具有实际应用潜力。
...
TL;DR: 该研究提出了一种自动发现与慢性疼痛相关行为特征的框架，无需依赖人工标注，显著提高了分类准确性。
...
Result: 在15类疼痛分类任务中达到48.41%的准确率，显著优于人类专家（21.33%）和B-SOiD方法（30.52%）；在三分类任务中准确率为73.1%，同样优于人类（48%）和B-SOiD（58.43%）。
...
TL;DR: 本文提出了一种旋转等变的任意尺度图像超分辨率（ASISR）方法，通过重新设计编码器和隐式神经表示（INR）模块，实现了从输入到输出的端到端旋转等变性，解决了低分辨率图像中几何图案变形的问题。


<details>
  <summary>Details</summary>
Motivation: 低分辨率图像中的几何图案（如重复纹理、边缘或形状）在超分辨率恢复时容易产生变形和伪影，嵌入旋转等变性可以保持原始方向和结构完整性。

Method: 重新设计了编码器和INR模块的基本架构，使其具备旋转等变能力，并通过理论分析评估了其内在等变误差。

Result: 实验表明，该方法在模拟和真实数据集上均表现出色，并能以即插即用的方式提升现有ASISR方法的性能。
...
Conclusion: X-MoGen首次实现了跨物种文本驱动运动生成的统一框架，解决了形态差异问题，并在实验中验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗补丁方法主要针对目标检测模型，在MLLM系统中表现不佳，需解决其复杂架构和推理能力的挑战。

Method: PhysPatch联合优化补丁位置、形状和内容，引入语义掩码初始化、SVD局部对齐损失和势场掩码细化方法。

Result: PhysPatch在多个MLLM系统中显著优于现有方法，并确保补丁在物理可行区域放置。
...
TL;DR: 论文提出了一种名为MTT的多目标跟踪方法，通过增强轨迹片段生成和关联框架，解决现有方法在未知类别目标跟踪中的低置信度检测、弱运动约束和长期遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中，未知类别目标的跟踪因低置信度检测、弱运动和外观约束以及长期遮挡等问题，对现有方法构成挑战。

Method: 提出MTT框架，通过自适应聚类检测结果生成鲁棒轨迹片段，并利用位置和外观等多线索估计最佳轨迹片段划分，以减少长期关联中的错误传播。

Result: 在通用多目标跟踪基准测试中，MTT框架表现出竞争力。

Conclusion: MTT框架通过增强轨迹片段生成和关联，有效提升了多目标跟踪的性能，尤其在未知类别目标跟踪中表现优异。
...
TL;DR: SPA++是一个图谱对齐框架，通过结合粗粒度图对齐和细粒度邻居感知传播机制，提升域适应的可区分性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有域适应方法主要关注域间可转移性，但忽视了域内结构，导致可区分性下降。

Method: 1) 将域适应问题转化为图原语，结合谱正则化对齐域图；2) 提出邻居感知传播机制增强目标域可区分性；3) 结合数据增强和一致性正则化适应复杂场景。

Result: 在基准数据集上，SPA++表现优于现有方法，具有更强的鲁棒性和适应性。

Conclusion: SPA++通过谱对齐和一致性正则化，有效解决了域适应中的可区分性与可转移性权衡问题。
...
TL;DR: 论文提出了一种名为SPEX的多模态大语言模型，用于光谱遥感图像中的地物提取，通过结合光谱信息和文本属性，显著提升了性能。
...
Result: 在五个公共多光谱数据集上，SPEX在植被、建筑和水体等地物提取任务中优于现有方法，并能生成预测的文本解释。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜图像中的通用密集特征匹配对机器人辅助任务至关重要，但由于视觉条件困难（如弱纹理、大视角变化）和标注数据稀缺，仍具挑战性。

Method: EndoMatcher采用双分支Vision Transformer提取多尺度特征，并通过双交互块增强对应学习；构建Endo-Mix6多域数据集，采用渐进多目标训练策略。

Result: 在零样本匹配实验中，EndoMatcher在Hamlyn和Bladder数据集上的内点匹配数分别提升140.69%和201.43%，在Gastro-Matching数据集上MDPA提升9.40%。

Conclusion: EndoMatcher通过多域预训练和渐进学习策略，实现了在挑战性内窥镜条件下的通用密集匹配，性能显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型（LMMs）在视觉语言任务中表现优异，但过多的视觉令牌导致高计算成本。现有修剪方法简单且性能下降明显，需改进。

Method: 提出VFlowOpt框架，包括基于注意力相关性和信息熵的重要性映射、渐进修剪模块及回收机制，并通过视觉信息流优化策略。

Result: 实验显示VFlowOpt可修剪90%的视觉令牌，性能相当，减少89%的KV-Cache内存，推理速度提升3.8倍。

Conclusion: VFlowOpt通过优化修剪策略，显著降低计算成本，同时保持模型性能，适用于不同LMMs。
...
Method: 通过任务特定注意力适配器（TSAA）和视觉-视觉嵌入对齐（VVEA）、文本-视觉嵌入对齐（TVEA）模块实现跨模态适应。
...
TL;DR: 本文提出了一种基于现代视觉基础模型的方法，通过微调Segment Anything Model (SAM v2.1)，仅需100张标注图像即可准确分割多相流中高度不规则的气泡结构。


<details>
  <summary>Details</summary>
Motivation: 多相流中气泡的分割是一个关键但未解决的挑战，传统方法假设气泡形状接近球形，无法有效处理变形、合并或破裂的气泡。

Method: 将任务视为迁移学习问题，微调SAM v2.1模型，利用少量标注图像（100张）进行训练。

Result: 微调后的SAM v2.1能够准确分割高度不规则和非凸的气泡结构。

Conclusion: 该方法为解决多相流中气泡分割问题提供了高效且准确的解决方案。
...
Result: Arbiviewgen首次实现多车辆配置下可控任意视角图像生成。

Conclusion: 该框架无需额外传感器或深度图，为自动驾驶提供高效解决方案。
...
TL;DR: 该报告总结了八篇关于视觉语言模型（如CLIP）零样本对抗鲁棒性的论文，探讨了增强对抗鲁棒性与保持零样本泛化能力之间的权衡，并分析了两种防御范式及其演变。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决视觉语言模型在零样本对抗鲁棒性中的核心挑战，即如何在增强对抗鲁棒性的同时不损害其零样本泛化能力。

Method: 方法包括对抗微调（AFT）和训练无关/测试时防御，并追踪了从对齐保留方法到嵌入空间重构、输入启发式到潜在空间净化的演变。

Result: 研究结果展示了不同防御方法的优缺点，并提出了未来研究方向。

Conclusion: 结论指出未来需探索混合防御策略和对抗预训练等方向。
...
TL;DR: RegionMed-CLIP是一种区域感知的多模态对比学习框架，通过结合局部病理信号和全局语义表征，解决了医学图像理解中的标注数据不足和全局特征依赖问题。
...
Method: 提出RegionMed-CLIP框架，包含自适应整合局部与全局特征的ROI处理器，并采用渐进式训练策略增强多模态对齐。构建MedRegion-500k数据集支持区域级表征学习。
...
TL;DR: 该论文综述了性别分类的多种方法，重点介绍了基于面部和虹膜特征的技术，并分析了现有方法的优缺点，提出了未来研究方向。
...
Result: 总结了现有方法的优缺点，并指出虹膜特征因其稳定性和非侵入性在性别分类中的潜力。

Conclusion: 论文为研究者提供了性别分类领域的全面分析，指出了当前挑战和未来改进方向。
...
Method: 采用多视角2D特征与预训练高斯的快速加权融合，训练高斯级自编码器，并引入自适应稀疏化方法。
...
Result: 方法在117个目标点上实现了25.4 fps的实时跟踪，误差为5.00 +/- 0.22 px，优于深度学习和传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法因缺乏深层语义理解，导致关键目标丢失，且融合过程可能引入伪影和细节损失。

Method: SGDFuse采用两阶段方法：先初步融合多模态特征，再利用SAM生成的语义掩码与初步融合图像作为条件，驱动扩散模型进行粗到细的去噪生成。
...
<details>
  <summary>Details</summary>
Motivation: 动态户外环境需要捕捉复杂物体交互及其随时间的变化，但4D LiDAR在MLLMs中未充分探索，因缺乏高质量标注和适配架构。

Method: 引入B4DL基准，提出可扩展的数据生成流程和直接处理原始4D LiDAR的MLLM模型。

Result: 模型结合数据集和基准，为动态户外环境提供了统一的时空推理解决方案。

Conclusion: B4DL基准和模型填补了4D LiDAR在MLLMs中的空白，推动了动态环境理解的研究。
...
Method: 1. 使用离散小波变换（DWT）将图像分解为高频和低频成分；2. 高频分支设计双频特征增强（DFFE）模块和频域交互差异（FDID）模块；3. 低频分支利用Transformer捕获全局语义关系，并采用渐进上下文差异模块（PCDM）细化变化区域；4. 融合高频和低频特征。

Result: 在多个遥感数据集上的实验表明，WGDF显著减轻了边缘模糊问题，检测精度和鲁棒性优于现有方法。

Conclusion: WGDF通过频域建模有效提升了遥感图像变化检测的性能，尤其是在边缘和细微变化区域的检测上表现突出。
...
Method: 提出VS-LLM方法，结合视觉和语义分析，优化PPAT草图的抑郁评估。

Result: 实验显示，VS-LLM比心理学家评估方法提升17.6%的准确率。

Conclusion: 该方法为基于PPAT草图的心理状态评估研究提供了有效工具，数据集和代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有高效小模型在分布外（OOD）数据上性能下降显著，表明泛化能力不足。论文旨在通过改进架构设计提升小模型的鲁棒性。

Method: 提出CoCA机制，结合动态、领域感知的全局令牌，增强局部-全局特征建模，并引入CoCAViT作为新型视觉主干网络。

Result: CoCAViT-28M在ImageNet-1K上达到84.0% top-1准确率，在多个OOD基准测试中表现优异，同时在COCO和ADE20K任务中保持高效。

Conclusion: CoCAViT通过改进架构设计显著提升了小模型的泛化能力和效率，适用于实时视觉任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于RAG的VQA方法因依赖非结构化文档和忽视知识元素间的结构关系，常引入无关或误导性内容，影响答案质量。

Method: 利用多模态知识图谱（KGs）构建结构化知识表示，结合MLLM驱动的关键词提取和视觉-文本匹配，并采用双阶段检索策略优化检索效率与精度。

Result: 实验表明，该方法显著优于现有方法，为知识密集型VQA任务设立了新的性能标杆。

Conclusion: 通过引入结构化多模态知识，mKG-RAG框架有效解决了传统RAG方法的局限性，提升了VQA任务的性能。
...
TL;DR: 论文提出了一种基于文本反转（TI）的方法，用于开放词汇目标检测，通过少量样本学习新词汇，同时保留预训练模型的零样本能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型（VLMs）在目标检测任务中表现优异，但在特定任务上仍需微调，而微调通常会丧失原始的自然语言查询和零样本能力。

Method: 采用类似文本反转（TI）的方法，通过学习新词汇或改进现有词汇，仅需少量样本即可检测新对象或细粒度对象，同时保持原始模型权重不变。

Result: 该方法在多种定量和定性实验中表现优异，避免了基线方法的遗忘问题，且计算成本显著低于全模型微调。

Conclusion: 提出的方法在保留预训练模型能力的同时，有效扩展了其词汇表，适用于开放词汇目标检测任务。
...
TL;DR: 3D Gabor Splatting (3DGabSplat) 提出了一种基于3D Gabor的基元，用于提升3D场景高频细节的表示能力，同时减少冗余基元和内存开销，显著优于3DGS。


<details>
  <summary>Details</summary>
Motivation: 3DGS因使用高斯函数而无法有效表示高频细节，且存在冗余基元和效率问题。
...
TL;DR: PriorRG是一个新颖的胸部X光报告生成框架，通过两阶段训练流程模拟真实临床工作流，利用患者特定先验知识提升报告质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法多基于单张图像生成报告，忽略了临床背景和先验图像信息，导致无法捕捉诊断意图或疾病进展。

Method: PriorRG采用两阶段训练：先验引导对比预训练和先验感知的粗到细解码，逐步整合先验知识与视觉编码器隐藏状态。

Result: 在MIMIC-CXR和MIMIC-ABN数据集上，PriorRG显著优于现有方法，BLEU-4和F1分数分别提升3.6%和3.8%。

Conclusion: PriorRG通过有效利用先验知识，显著提升了报告的临床准确性和流畅性。
...
TL;DR: Slice-Loc是一种两阶段方法，通过分割查询图像为子图像并估计每个子图像的3自由度位姿，生成冗余观测，再通过几何刚性公式过滤错误位姿，最终合并内点生成相机位姿。该方法显著提高了定位精度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角定位方法通常仅输出单一相机位姿，缺乏冗余观测，难以评估定位可靠性。Slice-Loc旨在通过生成冗余观测和验证机制解决这一问题。

Method: 1. 将查询图像分割为子图像，为每个子图像估计3自由度位姿；2. 提出几何刚性公式过滤错误位姿；3. 通过估计虚假警报数量（NFA）量化定位可靠性。

Result: Slice-Loc将定位误差超过10米的比例降至3%以下，在DReSS数据集上，平均定位误差从4.47米降至1.86米，平均方向误差从3.42°降至1.24°。
...
TL;DR: CT-GRAPH是一种基于图注意力网络的模型，通过显式建模解剖区域的层次关系，结合预训练的3D医学特征编码器和大语言模型，显著提升了CT报告生成的准确性。


<details>
  <summary>Details</summary>
Motivation: 医学影像在诊断中至关重要，但现有方法仅依赖全局特征，忽略了细粒度器官关系，影响了报告的准确性。

Method: 提出CT-GRAPH，利用解剖掩码获取全局和器官级特征，通过图注意力网络建模层次关系，并结合大语言模型生成报告。

Result: 在CT-RATE数据集上，F1分数比现有方法提高了7.9%。

Conclusion: CT-GRAPH通过显式建模解剖层次关系，显著提升了报告生成的性能，代码已开源。
...
Conclusion: 可变形注意力机制能有效捕捉复杂空间结构，提升病理图像分析能力。
...
TL;DR: 论文提出了一种名为UNCAGE的无训练方法，通过注意力图优先解掩码清晰表示对象的标记，提升了文本到图像生成的组合保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御机制在识别和缓解被投毒图像时面临挑战，可能影响数据效用，系统可靠性不足。

Method: 利用多个先进视觉语言模型进行多数投票，检测投毒图像，并通过校准噪声修正。
...
<details>
  <summary>Details</summary>
Motivation: 物理对抗伪装对自动驾驶等安全关键领域构成威胁，现有方法在多变环境中效果不佳。

Method: 引入梯度校准策略和梯度去相关方法，优化多角度和距离下的梯度更新。

Result: 攻击成功率平均提升13.46%（距离）和11.03%（角度），优于现有技术。

Conclusion: 该方法有效解决了梯度优化中的局部连续性和稳定性问题，但现实场景仍需更鲁棒的系统设计。
...
TL;DR: SmoothSA改进Slot Attention（SA）在图像和视频中的对象中心学习，通过预热冷启动查询和区分帧间变换，提升聚合精度。
...
Method: 提出SmoothSA：（1）预热冷启动查询，通过自蒸馏模块丰富输入特征信息；（2）区分首帧和非首帧的变换，分别使用完整和单次迭代。
...
TL;DR: 论文提出了一种名为FIxLIP的方法，用于分解视觉语言编码器中的相似性，解决了现有显著性图仅捕获一阶归因的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如显著性图）仅能捕捉一阶归因，无法全面反映跨模态交互的复杂性。

Method: 基于博弈论，使用加权Banzhaf交互指数，提出FIxLIP方法，并扩展了评估指标（如pointing game和插入/删除曲线面积）。

Result: 在MS COCO和ImageNet-1k基准测试中，二阶方法FIxLIP表现优于一阶归因方法。

Conclusion: FIxLIP不仅能提供高质量的解释，还能用于比较不同模型（如CLIP与SigLIP-2）。
...
TL;DR: 提出了一种基于Flow Matching（FM）的无监督异常检测与定位新范式，解决了传统流方法的模型表达能力限制。通过时间反转FM（rFM）和Worst Transport（WT）插值路径，实现了对异常样本的理论分离，并在MVTec数据集上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统流方法在模型表达能力上存在局限性，无法有效处理无监督异常检测任务。本文旨在通过FM及其改进方法解决这一问题。

Method: 提出时间反转FM（rFM）作为向量场回归方法，将未知数据分布转换为标准高斯分布。进一步引入Worst Transport（WT）插值路径，增强对样本轨迹的动态控制。

Result: 在MVTec数据集上实现了单尺度下的最优性能，验证了方法的有效性。

Conclusion: FM为无监督异常检测提供了理论支持，并通过WT-Flow实现了对异常样本的有效分离，具有计算高效性和可扩展性。
...
TL;DR: 论文提出了一种名为F2PASeg的方法，用于垂体解剖结构分割，通过特征融合模块提升手术中的鲁棒性，并发布了一个新的数据集PAS。


<details>
  <summary>Details</summary>
Motivation: 垂体肿瘤常导致邻近重要结构的变形或包裹，解剖结构分割可为外科医生提供手术风险区域的早期预警，从而提高手术安全性。但现有的像素级标注视频数据集稀缺。

Method: 引入PAS数据集（7,845张图像），采用数据增强技术缓解类别不平衡问题，并提出F2PASeg方法，结合高分辨率图像特征和深度语义嵌入，通过特征融合模块提升分割效果。

Result: 实验表明，F2PASeg能实时稳定地分割关键解剖结构，为术中手术规划提供可靠解决方案。

Conclusion: F2PASeg通过特征融合模块有效解决了术中特征不一致性问题，提升了垂体手术的安全性。
...
TL;DR: 研究发现，通过高保真图像重建的压缩模型能显著抵抗对抗攻击，而低保真模型易被攻破，揭示了图像真实性与防御能力的关系。


<details>
  <summary>Details</summary>
Motivation: 以往研究缺乏对图像压缩预处理防御对抗攻击的全面评估，本文旨在填补这一空白。

Method: 构建强白盒和自适应攻击，评估多种压缩模型，分析攻击难度与图像真实性的关系。

Result: 高保真重建的压缩模型对攻击具有显著抵抗力，低保真模型则易被攻破，且与梯度掩蔽无关。

Conclusion: 图像真实性与防御能力密切相关，未来对抗攻击需克服高真实性带来的挑战。
...
TL;DR: 论文提出了一种基于声明性知识的提示方法（OND），用于改进预训练基础模型在历史地图分割中的性能，并展示了其优于UNet基线的效果。


<details>
  <summary>Details</summary>
Motivation: 历史地图缺乏现代图像中的一致性模式，预训练基础模型难以直接应用，需要一种方法引导模型识别不同形状和风格的相似概念。

Method: 提出OND知识提示机制，替换SAM模型的提示编码器，并在历史地图上微调，形成SMOL-MapSeg模型。

Result: SMOL-MapSeg能准确分割OND定义的类别，并通过少量样本适应新类别，平均分割性能优于UNet基线。

Conclusion: OND提示机制有效解决了历史地图分割的挑战，SMOL-MapSeg展示了灵活性和性能优势。
...
Method: 采用双源策略收集数据，结合原生网络替代文本（文化）和MLLM生成标题（语言），构建MELLA数据集。
...
<details>
  <summary>Details</summary>
Motivation: 工业异常检测（IAD）对制造质量控制至关重要，但传统方法需要大量人工干预，AutoIAD旨在解决这一问题。

Method: AutoIAD采用Manager-Driven中心智能体协调多个子智能体（如数据准备、模型设计等），并结合领域知识库，实现从原始图像到训练模型的自动化流程。
...
TL;DR: 提出了一种无监督的手性特征提取方法，用于增强形状顶点的手性信息，解决了现有形状描述符无法区分左右对称部分的问题。
...
Result: 通过定量和定性实验验证了手性特征的有效性，在下游任务（如左右分离、形状匹配和部分分割）中表现优异。
...
Conclusion: MagicHOI通过结合新视角合成模型和接触约束，提升了手-物体重建的准确性，尤其在视角受限场景中表现优异。
...
Method: 框架分为三个阶段：差异引导掩码建模、固定骨干特征转换和聚焦对比学习，分别用于提取信息、保留表示和提升语义区分。

Result: 实验表明，该框架在物体识别、语义分割和光流估计等任务中优于现有方法。

Conclusion: 提出的自监督预训练框架能有效提取事件数据中的信息，并在多种任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中，行人跟踪常因遮挡导致目标特征丢失，传统方法在严重遮挡场景下表现不佳。

Method: 结合目标检测器的回归和分类分支特征，引入头部关键点检测模型，并提出迭代卡尔曼滤波方法。

Result: 新方法在遮挡严重的拥挤环境中提供了更鲁棒的多目标跟踪解决方案。

Conclusion: 通过改进特征表示和运动建模，该方法显著提升了遮挡场景下的跟踪性能。
...
TL;DR: 提出了一种基于特征空间随机平滑的图像质量评估（IQA）模型认证防御方法，相比直接在输入空间添加噪声的方法，该方法在保持图像质量的同时提供鲁棒性保证。


<details>
  <summary>Details</summary>
Motivation: 现有方法在输入图像中直接添加高斯噪声会降低视觉质量，而该方法旨在在保持图像质量的同时提供鲁棒性。

Method: 在特征空间而非输入空间应用噪声，通过分析主干网络Jacobian的最大奇异值，将特征空间噪声与输入空间扰动关联。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验数据清理是药物开发中的瓶颈，手动处理难以应对数据量和复杂性的增长。
...
Result: 在多样基准数据集上，SSTGNN在域内和跨域设置中表现优异，且参数减少42.4倍。

Conclusion: SSTGNN轻量高效，适用于实际部署，对未知篡改具有强鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI定位方法依赖昂贵标注数据，而模型预测的空间重叠模式隐含置信信号，可用于提升定位精度。
...
Conclusion: 测试时扩展和强化学习为GUI定位提供了数据高效的新路径。
...
TL;DR: 论文提出了一种名为Action Discovery的新方法，用于解决时间动作分割中模糊动作和部分标注数据集的挑战，通过已知动作引导未知动作的分割和分类。


<details>
  <summary>Details</summary>
Motivation: 在神经科学等领域，明确的行为（如行走、进食）与容易被忽略的细微或不常见动作共存，且数据集常因模糊或缺失标签而部分标注。
...
Method: 通过MLLM-Collector收集资产及其描述，构建3D布局，利用Vision-Language Models（VLMs）进行语义优化，并通过MLLM-Planner生成时间连贯的未来帧。
...
Method: 采用LLaVA架构，结合任务指令和多模态上下文样本，构建二进制相关性评估模型LLaVA-RE。
...
TL;DR: Uni-CoT是一个统一的链式思维框架，通过结合图像理解和生成能力，实现多模态推理，并在计算效率和性能上取得突破。
...
Method: 采用两级推理范式：宏观级CoT用于任务规划，微观级CoT用于子任务执行，结合图像-文本监督和多任务目标训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D内容生成质量评估方法主要依赖基于图像的指标，且仅针对对象级别，难以捕捉空间一致性、材料真实性和高保真局部细节。

Method: 提出Hi3DEval框架，结合对象级和部分级评估，扩展纹理评估至材料真实性；构建Hi3DBench数据集和多智能体标注流程；设计基于混合3D表示的自动化评分系统。
...
TL;DR: MOSEv2是一个更具挑战性的视频对象分割数据集，旨在推动真实场景下的研究，包含复杂场景、多样挑战和大量高质量数据。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要包含显著、孤立的对象，限制了真实场景的泛化能力，因此需要更复杂的数据集。

Method: 通过构建MOSEv2数据集，包含5,024个视频、701,976个高质量掩码，涵盖200个类别，引入复杂场景和新挑战。
...
TL;DR: GAP提出了一种将无色3D点云转化为高保真3D高斯分布的方法，利用文本引导和多视角优化框架，结合深度感知扩散模型和表面锚定机制，解决了点云到高斯的生成难题。


<details>
  <summary>Details</summary>
Motivation: 点云是广泛使用的3D表示形式，但如何将其转化为高斯分布仍是一个未解决的挑战。GAP旨在填补这一空白，实现高质量的点云到高斯的转换。

Method: GAP采用多视角优化框架，利用深度感知图像扩散模型合成一致的外观，并通过表面锚定机制约束高斯分布。此外，还引入了基于扩散的修复策略处理难以观察的区域。

Result: GAP在从合成点云到真实扫描的不同复杂度任务中表现优异，能够生成高保真的3D高斯分布。

Conclusion: GAP为点云到高斯的生成提供了一种高效且高质量的解决方案，适用于多种场景。
...
TL;DR: FaceAnonyMixer是一种可取消的人脸生成框架，通过混合真实人脸图像的潜在代码与可撤销密钥生成的合成代码，保护隐私同时保持识别效果。


<details>
  <summary>Details</summary>
Motivation: 人脸识别技术发展引发隐私担忧，现有匿名化方法无法满足生物特征模板保护的需求（如可撤销性、不可链接性和不可逆性）。

Method: 利用预训练生成模型的潜在空间合成隐私保护人脸图像，通过多目标损失函数优化混合潜在代码。

Result: 实验表明，FaceAnonyMixer在商业API上比现有方法提升11%的识别准确率，同时提供更强的隐私保护。

Conclusion: FaceAnonyMixer是一种高效且隐私保护的人脸匿名化方法，适用于现有识别系统。
...
### [102] [Augmented Question-guided Retrieval (AQgR) of Indian Case Law with LLM, RAG, and Structured Summaries](https://arxiv.org/abs/2508.04710)
*Vishnuprabha V,Daleesha M Viswanathan,Rajesh R,Aneesh V Pillai*

Main category: cs.IR

TL;DR: 该论文提出了一种基于大型语言模型（LLMs）的方法，结合检索增强生成（RAG）和结构化摘要，用于改进法律案例检索，生成解释并识别核心法律问题。


<details>
  <summary>Details</summary>
Motivation: 当前法律案例检索方法过于依赖事实相似性，缺乏对法律问题的关注和解释，导致检索结果不够相关和透明。

Method: 采用检索增强生成（RAG）和结构化摘要，结合增强问题引导检索（AQgR）框架，生成针对性法律问题以提高检索效果。

Result: 在FIRE 2019数据集上测试，平均精度（MAP）为0.36，平均召回率（MAR）为0.67，显著优于现有基准。

Conclusion: 该方法从基于事实的检索转向基于法律问题的检索，提供了更符合法律专业人士需求的上下文相关结果。
...
### [103] [Scaling Generative Recommendations with Context Parallelism on Hierarchical Sequential Transducers](https://arxiv.org/abs/2508.04711)
...
<details>
  <summary>Details</summary>
Motivation: 大规模推荐系统需要处理高基数和非平稳数据，但扩展序列长度会导致内存问题，需要并行化解决方案。

Method: 引入支持不规则张量的上下文并行技术，结合分布式数据并行，扩展HSTU注意力模型的序列长度。

Result: 实现了5.3倍的序列长度扩展，结合DDP时达到1.55倍的扩展因子。

Conclusion: 该方法为推荐系统中长序列建模提供了有效的并行化解决方案。
...
### [104] [A Metric for MLLM Alignment in Large-scale Recommendation](https://arxiv.org/abs/2508.04963)
*Yubin Zhang,Yanhua Huang,Haiming Xu,Mingliang Qi,Chang Wang,Jiarui Jin,Xiangyuan Ren,Xiaodan Wang,Ruiwen Xu*

Main category: cs.IR

TL;DR: 论文提出了一种名为Leakage Impact Score（LIS）的新指标，用于解决多模态推荐系统中评估MLLMs对齐的挑战，并在实际场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 多模态推荐系统需要适应MLLMs的内容表示，但现有评估方法存在静态基准不准确、在线评估成本高和传统指标缺乏可操作性等问题。

Method: 提出LIS指标，间接测量偏好数据的上限，并结合实际部署MLLMs的实践。

Result: 在Xiaohongshu的Explore Feed生产环境中进行A/B测试，显著提升了用户停留时间和广告价值。

Conclusion: LIS是一种高效且实用的多模态推荐评估方法，能够有效解决现有挑战并提升系统性能。
...
### [105] [Align-for-Fusion: Harmonizing Triple Preferences via Dual-oriented Diffusion for Cross-domain Sequential Recommendation](https://arxiv.org/abs/2508.05074)
*Yongfu Zha,Xinxin Dong,Haokai Ma,Yonghui Yang,Xiaodong Wang*

Main category: cs.IR

TL;DR: 论文提出了一种名为HorizonRec的跨域序列推荐框架，通过双导向扩散模型解决现有方法在细粒度偏好融合上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统跨域序列推荐方法通常采用对齐后融合范式，忽略了域间偏好的细粒度融合，导致推荐效果受限。

Method: 提出基于扩散模型的align-for-fusion框架，引入混合条件分布检索策略和双导向偏好扩散方法，以抑制噪声并增强目标相关兴趣。

Result: 在四个跨域序列推荐数据集上的实验表明，HorizonRec在细粒度三域偏好融合中表现出色。

Conclusion: HorizonRec通过扩散模型实现了跨域偏好的高效融合，显著提升了推荐性能。
...
### [106] [An End-to-End Multi-objective Ensemble Ranking Framework for Video Recommendation](https://arxiv.org/abs/2508.05093)
*Tiantian He,Minzhi Xie,Runtong Li,Xiaoxiao Xu,Jiaqi Yu,Zixiu Wang,Lantao Hu,Han Li,Kun Gai*

Main category: cs.IR

TL;DR: 提出了一种端到端多目标集成排序框架（EMER），用于短视频推荐系统中的多目标集成排序模块，通过端到端建模增强个性化，并引入新的损失函数、样本组织方法和网络架构。
...
Method: 设计了端到端建模框架，包括新的损失函数、样本组织方法和基于Transformer的网络架构，以及离线-在线一致评估系统。

Result: 在快手平台上部署后，整体App停留时间提升1.39%，7天用户生命周期提升0.196%。

Conclusion: EMER框架在多目标排序任务中表现出色，显著提升了推荐系统的效果。
...
### [107] [Navigating Through Paper Flood: Advancing LLM-based Paper Evaluation through Domain-Aware Retrieval and Latent Reasoning](https://arxiv.org/abs/2508.05129)
*Wuqiang Zheng,Yiyan Xu,Xinyu Lin,Chongming Gao,Wenjie Wang,Fuli Feng*

Main category: cs.IR

TL;DR: PaperEval是一个基于LLM的框架，通过领域感知检索和潜在推理机制提升论文评估的准确性和可靠性，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 学术出版物快速增长，识别高质量研究成为挑战，现有LLM方法受限于过时知识和有限推理能力。

Method: PaperEval结合领域感知检索模块和潜在推理机制，采用渐进排名优化策略迭代优化预测。

Result: 实验显示PaperEval在学术影响和论文质量评估上优于现有方法，实际应用中表现优异。

Conclusion: PaperEval通过创新方法解决了LLM在论文评估中的局限性，展示了实际应用价值。
...
### [108] [Tool Graph Retriever: Exploring Dependency Graph-based Tool Retrieval for Large Language Models](https://arxiv.org/abs/2508.05152)
*Linfeng Gao,Yaoxiang Wang,Minlong Peng,Jialong Tang,Yuzhe Shang,Mingming Sun,Jinsong Su*

Main category: cs.IR

TL;DR: 论文提出Tool Graph Retriever (TGR)，通过利用工具间的依赖关系改进工具检索性能，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有工具检索方法仅基于语义相似性，忽略工具间的依赖关系，可能导致任务执行失败。

Method: 构建TDI300K数据集训练依赖判别器，将工具表示为依赖图，通过图卷积整合依赖关系。

Result: 实验表明TGR显著提升性能，达到SOTA水平。

Conclusion: 工具依赖关系对检索至关重要，TGR有效解决了现有方法的缺陷。
...
### [109] [Balancing Accuracy and Novelty with Sub-Item Popularity](https://arxiv.org/abs/2508.05198)
...
TL;DR: 论文提出了一种基于子ID级别的个性化流行度评分方法（sPPS），通过更细粒度的建模重复收听模式，在保持推荐准确性的同时显著提升个性化新颖性。


<details>
  <summary>Details</summary>
Motivation: 音乐推荐中，个性化流行度评分（PPS）虽然能增强相关性，但容易过度推荐已知内容，限制了新颖性和偶然性。

Method: 在RecJPQ框架中引入子ID级别的PPS（sPPS），通过子嵌入建模共享重复模式，实现准确性与新颖性的平衡。

Result: sPPS在保持推荐准确性的同时，显著提升了个性化新颖性，优于传统的项目级PPS。

Conclusion: 子ID级别的PPS为音乐推荐系统提供了一种有效平衡准确性与新颖性的方法。
...
### [110] [FIRE: Faithful Interpretable Recommendation Explanations](https://arxiv.org/abs/2508.05225)
*S. M. F. Sani,Asal Meskin,Mohammad Amanlou,Hamid R. Rabiee*

Main category: cs.IR

TL;DR: 论文探讨了推荐系统中自然语言解释的问题，提出了一种新框架FIRE，通过结合SHAP特征归因和结构化提示生成忠实、多样化的解释。


<details>
  <summary>Details</summary>
Motivation: 现有方法将用户评论作为解释依据，导致解释与模型逻辑不一致，无法透明传达推荐原因。

Method: 提出FIRE框架，结合SHAP特征归因和结构化提示生成语言解释。

Result: FIRE在推荐准确性和解释质量（如对齐性、结构性和忠实性）上表现优异。

Conclusion: 需超越评论即解释的范式，开发更可解释和可问责的解释方法。
...
### [111] [Difference Views for Visual Graph Query Building](https://arxiv.org/abs/2508.05314)
*Benedikt Kantz,Stefan Lengauer,Peter Waldert,Tobias Schreck*

Main category: cs.IR

TL;DR: 论文提出了一种可视化查询界面，通过图形差异对比查询构建过程中的变化，并集成了自然语言接口，以支持用户动态调整信息需求。


<details>
  <summary>Details</summary>
Motivation: 知识图谱（KGs）中的查询构建是一个迭代且视觉化的过程，用户的需求可能在过程中动态变化，尤其是探索性搜索时。现有工具未充分支持这种动态变化。

Method: 设计了可视化查询界面，通过图形差异展示查询构建的迭代变化，并集成自然语言接口。同时，在结果视图中对比结果分布和实例差异。

Result: 通过不同本体和使用场景的案例研究，验证了系统在支持数据探索和领域特定图谱分析方面的有效性。

Conclusion: 该系统通过动态展示查询变化和结果差异，有效支持了用户的探索性搜索和数据分析需求。
...
### [112] [Multi-Modal Multi-Behavior Sequential Recommendation with Conditional Diffusion-Based Feature Denoising](https://arxiv.org/abs/2508.05352)
*Xiaoxi Cui,Weihai Lu,Yu Tong,Yiheng Li,Zhejun Zhao*

Main category: cs.IR

TL;DR: 提出了一种多模态多行为序列推荐模型（M³BSR），通过模态去噪和行为去噪提升推荐准确性，显著优于现有方法。
...
Method: 采用条件扩散模态去噪层去除模态噪声，利用深度行为信息指导浅层行为去噪，并通过多专家兴趣提取层建模共同和特定兴趣。

Result: 在基准数据集上显著优于现有最优方法。

Conclusion: M³BSR有效解决了多模态多行为序列推荐中的挑战，提升了推荐性能。
...
### [113] [Does Multimodality Improve Recommender Systems as Expected? A Critical Analysis and Future Directions](https://arxiv.org/abs/2508.05377)
*Hongyu Zhou,Yinan Zhang,Aixin Sun,Zhiqi Shen*

Main category: cs.IR

TL;DR: 本文提出了一种结构化评估框架，系统地评估多模态推荐系统在四个维度上的表现，发现多模态数据在稀疏交互和召回阶段特别有效，且不同模态的重要性因任务而异。


<details>
  <summary>Details</summary>
Motivation: 多模态推荐系统虽然潜力巨大，但其实际效益尚不明确，因此需要系统评估其在不同场景下的表现。

Method: 提出一个结构化评估框架，对多模态推荐系统在四个维度（比较效率、推荐任务、推荐阶段、多模态数据整合）上进行评估，并对比传统基线模型。

Result: 多模态数据在稀疏交互和召回阶段效果显著；文本特征在电商中更有效，视觉特征在短视频推荐中更优；集成学习优于融合学习，大模型未必更好。

Conclusion: 研究为构建高效多模态推荐系统提供了实用见解，强调需谨慎选择模态、整合策略和模型设计。
...
### [114] [On the Reliability of Sampling Strategies in Offline Recommender Evaluation](https://arxiv.org/abs/2508.05398)
*Bruno L. Pereira,Alan Said,Rodrygo L. T. Santos*

Main category: cs.IR

TL;DR: 论文研究了离线评估推荐系统时的偏差问题，重点分析了曝光偏差和采样偏差对评估可靠性的影响，并提出了评估采样策略的四个维度。


<details>
  <summary>Details</summary>
Motivation: 离线评估在推荐系统基准测试中至关重要，但存在曝光偏差和采样偏差问题，现有方法未充分评估其在不同曝光条件下的可靠性。

Method: 通过完全观测数据集模拟多样化的曝光偏差，评估常见采样策略在分辨率、保真度、鲁棒性和预测能力四个维度的表现。

Result: 研究发现采样策略会扭曲评估结果，并提供了选择可靠且鲁棒的离线评估策略的实用指南。

Conclusion: 论文揭示了采样偏差对离线评估的影响，为选择更可靠的评估策略提供了依据。
...
### [115] [RankArena: A Unified Platform for Evaluating Retrieval, Reranking and RAG with Human and LLM Feedback](https://arxiv.org/abs/2508.05512)
*Abdelrahman Abdallah,Mahmoud Abdalla,Bhawna Piryani,Jamshid Mozafari,Mohammed Ali,Adam Jatowt*

Main category: cs.IR

TL;DR: RankArena是一个统一的平台，用于评估和比较检索增强生成（RAG）和文档重排系统的性能，支持多种评估模式和结构化反馈。


<details>
  <summary>Details</summary>
Motivation: 现有工具缺乏可扩展性、用户中心化和多视角评估能力，因此需要开发一个更全面的评估平台。

Method: RankArena提供多种评估模式，包括直接重排可视化、盲对比投票、手动标注和端到端RAG质量评估，并结合人类和LLM反馈。

Result: 平台支持细粒度反馈和多模态评估，生成结构化数据集，可用于训练模型或优化检索策略。

Conclusion: RankArena为RAG和重排系统提供了全面的评估工具，公开可用，并支持进一步的研究和应用。
...
### [116] [KuaiLive: A Real-time Interactive Dataset for Live Streaming Recommendation](https://arxiv.org/abs/2508.05633)
*Changle Qu,Sunhao Dai,Ke Guo,Liqin Zhao,Yanan Niu,Xiao Zhang,Jun Xu*

Main category: cs.IR

TL;DR: KuaiLive是一个实时交互的直播数据集，填补了学术界缺乏公开动态直播数据的空白，支持多种推荐任务研究。


<details>
  <summary>Details</summary>
Motivation: 直播平台的动态性和实时互动特性为推荐系统带来新挑战，但缺乏公开数据集阻碍了研究进展。

Method: 从快手平台收集了23,772用户和452,621主播的21天交互日志，包含精确时间戳、多种互动类型和丰富的侧信息。

Result: KuaiLive支持动态候选项目模拟和用户行为建模，为推荐方法提供了基准。

Conclusion: KuaiLive为直播领域的多任务研究提供了数据支持，并公开了数据集以促进未来研究。
...
### [117] [Prescriptive Agents based on Rag for Automated Maintenance (PARAM)](https://arxiv.org/abs/2508.04714)
...
### [118] [GeoFlow: Agentic Workflow Automation for Geospatial Tasks](https://arxiv.org/abs/2508.04719)
...
Result: 相比现有方法，GeoFlow将代理成功率提高6.8%，token使用减少至四分之一。

Conclusion: GeoFlow通过明确工具调用目标，显著提升了地理空间任务代理的效率和性能。
...
### [119] [Who is a Better Player: LLM against LLM](https://arxiv.org/abs/2508.04720)
...
<details>
  <summary>Details</summary>
Motivation: 棋盘游戏作为战略推理和智能的典型领域，是评估AI系统的理想基准。现有问答基准依赖数据，缺乏对抗性环境的全面评估。

Method: 开发了Qi Town平台，支持5种游戏和20个LLM玩家，使用Elo评分系统和性能循环图（PLG）定量评估技术能力，并通过积极情感分数（PSS）评估心理适应性。

Result: 实验表明，尽管技术差异大，多数LLMs在对抗环境中表现乐观且适应性强，但PLG揭示了技能发挥的不稳定性。

Conclusion: 对抗性棋盘游戏是评估LLMs的有效方法，但需进一步研究其技能不稳定的原因。
...
### [120] [Fine-Tuning Small Language Models (SLMs) for Autonomous Web-based Geographical Information Systems (AWebGIS)](https://arxiv.org/abs/2508.04846)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于云的大型语言模型（LLM）在隐私、可扩展性和网络依赖性上的问题。

Method: 比较了三种方法：在线云LLM、半离线经典机器学习、离线客户端SLM（T5-small）。

Result: 客户端SLM方法在准确性（0.93精确匹配）和效率上最优，且减少服务器负担。

Conclusion: 客户端SLM方法可行，为AWebGIS提供了隐私友好且高效的解决方案。
...
### [121] [The Term 'Agent' Has Been Diluted Beyond Utility and Requires Redefinition](https://arxiv.org/abs/2508.05338)
*Brinnae Bent*
...
<details>
  <summary>Details</summary>
Motivation: 由于AI领域对'agent'一词的多重解释导致研究交流、系统评估和政策制定中的模糊性，需重新定义以解决这些问题。

Method: 通过历史分析和当代使用模式，提出一个多维框架，定义'agent'的最低要求，并描述系统在环境交互、学习适应、自主性、目标复杂性和时间一致性等方面的特征。

Result: 提出的框架为系统描述提供了精确词汇，同时保留了术语的多面性，并建议了术语标准化和框架采纳的具体措施。

Conclusion: 该框架有助于提升研究清晰度和可重复性，同时支持更有效的政策制定。
...
### [122] [Large Language Models Reasoning Abilities Under Non-Ideal Conditions After RL-Fine-Tuning](https://arxiv.org/abs/2508.04848)
*Chang Tian,Matthew B. Blaschko,Mingzhe Xing,Xiuxing Li,Yinliang Yue,Marie-Francine Moens*

Main category: cs.AI

TL;DR: 论文研究了强化学习（RL）在提升大语言模型（LLMs）推理能力中的作用，发现现有基准测试忽略了非理想场景下的表现。作者定义了三种非理想场景，并通过实验表明RL微调在这些场景下效果显著下降，暴露了当前方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估理想化场景下的大语言模型推理能力，而忽视了现实中的非理想场景，如摘要推理、细粒度噪声抑制和上下文过滤。

Method: 作者使用代表性策略梯度算法对三种LLMs和一种大型视觉语言模型（LVLM）进行RL微调，并在八个公共数据集上测试其性能。

Result: 结果显示，RL微调在理想化场景下提升了推理能力，但在三种非理想场景中表现显著下降。

Conclusion: 当前方法在非理想场景下的推理能力存在明显不足，强调了评估模型在非理想场景下的重要性。
...
### [123] [Whose Truth? Pluralistic Geo-Alignment for (Agentic) AI](https://arxiv.org/abs/2508.05432)
*Krzysztof Janowicz,Zilong Liu,Gengchen Mai,Zhangyu Wang,Ivan Majic,Alexandra Fortacz,Grant McKenzie,Song Gao*

Main category: cs.AI

TL;DR: 论文探讨了AI对齐中的地理差异性挑战，强调需根据文化和政治背景调整AI行为，并提出未来研究方向和方法。


<details>
  <summary>Details</summary>
Motivation: AI对齐需考虑地理和文化差异，避免一刀切方法，以适应不同地区的规范和法律。

Method: 回顾关键地理研究问题，提出未来工作方向，并概述评估对齐敏感性的方法。

Result: 指出AI对齐需结合时空背景，确保模型输出符合地区特定需求。

Conclusion: 呼吁开发更具地理敏感性的AI对齐方法，以适应全球多样性。
...
### [124] [ConfAgents: A Conformal-Guided Multi-Agent Framework for Cost-Efficient Medical Diagnosis](https://arxiv.org/abs/2508.04915)
...
Method: 引入HealthFlow，通过元级进化机制自主提炼成功与失败经验，形成战略知识库，并使用EHRFlowBench进行评测。
...
### [125] [The Docking Game: Loop Self-Play for Fast, Dynamic, and Accurate Prediction of Flexible Protein--Ligand Binding](https://arxiv.org/abs/2508.05006)
*Youzhi Zhang,Yufei Li,Gaofeng Meng,Hongbin Liu,Jiebo Luo*

Main category: cs.AI

TL;DR: 提出了一种基于博弈论的新型分子对接框架（Docking Game），通过LoopPlay算法显著提升了配体与蛋白质口袋的对接准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在配体对接中表现不佳，主要由于配体和蛋白质结构复杂性的差异。

Method: 将蛋白质-配体相互作用建模为双玩家博弈，提出LoopPlay算法，通过内外循环交替训练玩家。

Result: 在公开数据集上，LoopPlay比现有方法提升了约10%的对接准确性。

Conclusion: LoopPlay为药物发现中的分子对接提供了更准确的预测方法。
...
### [126] [Can Large Language Models Integrate Spatial Data? Empirical Insights into Reasoning Strengths and Computational Weaknesses](https://arxiv.org/abs/2508.05009)
...
<details>
  <summary>Details</summary>
Motivation: 传统规则方法和机器学习方法在整合异构、噪声空间数据时存在不足，LLMs被探索为一种灵活替代方案。
...
### [127] [Cognitive Duality for Adaptive Web Agents](https://arxiv.org/abs/2508.05081)
*Jiarun Liu,Chunhong Zhang,Zheng Hu*

Main category: cs.AI

TL;DR: 论文提出了一种结合离线模仿学习和在线探索的Web导航智能体框架CogniWeb，基于人类认知的双系统理论，实现了高效且性能优越的导航能力。
...
Method: 基于双系统认知理论，将智能体分解为快速直觉（System 1）和慢速推理（System 2）模块，设计了CogniWeb框架，动态切换两种模式。
...
### [128] [MedMKEB: A Comprehensive Knowledge Editing Benchmark for Medical Multimodal Large Language Models](https://arxiv.org/abs/2508.05083)
...
### [129] [EasySize: Elastic Analog Circuit Sizing via LLM-Guided Heuristic Search](https://arxiv.org/abs/2508.05113)
*Xinyue Wu,Fan Hu,Shaik Jani Babu,Yi Zhao,Xinfei Guo*

Main category: cs.AI

TL;DR: EasySize是一个基于Qwen3-8B模型的轻量级门尺寸优化框架，通过动态构建任务特定损失函数和结合全局与局部优化方法，实现了跨工艺节点和电路拓扑的通用性。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计中门尺寸优化耗时且依赖经验，现有AI方法通用性不足且计算资源消耗大。

Method: 结合微调的Qwen3-8B模型，动态构建损失函数，并采用差分进化和粒子群优化进行启发式搜索。

Result: 在多个工艺节点上表现优异，优于AutoCkt框架，显著减少仿真资源需求。

Conclusion: EasySize减少了对人力和计算资源的依赖，有望加速模拟电路设计流程。
...
### [130] [Beyond Automation: Socratic AI, Epistemic Agency, and the Implications of the Emergence of Orchestrated Multi-Agent Learning Architectures](https://arxiv.org/abs/2508.05116)
*Peer-Benedikt Degen,Igor Asanov*

Main category: cs.AI

TL;DR: 论文探讨生成式AI在高等教育中的核心作用，通过实验验证苏格拉底AI导师对学生批判性思维的促进作用，并提出多代理系统（MAS）的教育应用框架。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正重塑知识生成与验证方式，研究旨在探索其如何支持学生批判性思维，并推动教育系统的变革。

Method: 在德国65名师范生中进行对照实验，比较苏格拉底AI导师与普通AI聊天机器人的效果。

Result: 使用苏格拉底AI导师的学生在批判性、独立性和反思性思维方面表现显著更优。

Conclusion: 研究提出多代理系统（MAS）的教育应用框架，强调人机协作与教学对齐，为高等教育提供可扩展的解决方案。
...
### [131] [Graph-based Event Log Repair](https://arxiv.org/abs/2508.05145)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界的事件日志常因数据采集问题导致信息缺失，现有方法需依赖过程模型或机器学习模型，但效果有限。
...
TL;DR: QA-Dragon是一种查询感知的动态RAG系统，通过结合文本和图像检索策略，显著提升了知识密集型VQA任务的性能。
...
Method: 引入领域路由器和搜索路由器，动态选择检索策略，结合文本和图像搜索代理，支持多模态、多轮和多跳推理。

Result: 在Meta CRAG-MM挑战中，QA-Dragon显著提升了基础模型的推理性能，单源任务提升5.06%，多源任务提升6.35%，多轮任务提升5.03%。
...
### [133] [An Explainable Natural Language Framework for Identifying and Notifying Target Audiences In Enterprise Communication](https://arxiv.org/abs/2508.05267)
...
### [134] [A Novel Architecture for Symbolic Reasoning with Decision Trees and LLM Agents](https://arxiv.org/abs/2508.05311)
*Andrew Kiruluta*

Main category: cs.AI

TL;DR: 提出了一种结合决策树符号推理与大型语言模型（LLM）生成能力的混合架构，通过多智能体框架实现协调推理，提升了逻辑一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法松散结合符号与神经模块，缺乏统一性和可解释性，本设计旨在通过紧密集成符号推理与LLM，提升推理性能和可解释性。

Method: 将决策树和随机森林作为可调用模块嵌入统一推理系统，LLM处理归纳推理和规划，中央协调器维护状态一致性并协调多智能体通信。

Result: 在ProofWriter、GSM8k和ARC等基准测试中，逻辑一致性和准确性显著提升（分别提高7.2%、5.3%和6.0%），并在临床和科学领域展示了实际应用价值。

Conclusion: 该架构为通用神经符号推理提供了鲁棒、可解释且可扩展的解决方案，结合了符号规则与LLM的上下文推理能力。
...
### [135] [NomicLaw: Emergent Trust and Strategic Argumentation in LLMs During Collaborative Law-Making](https://arxiv.org/abs/2508.05344)
...
Result: 实验显示LLM能自发形成联盟、背叛信任，并调整语言策略影响集体决策，展示了其社会推理和说服能力。

Conclusion: 研究揭示了LLM在自主协商、协调和立法起草中的潜力，为未来AI系统设计提供了参考。
...
### [136] [Minimal Model Reasoning in Description Logics: Don't Try This at Home!](https://arxiv.org/abs/2508.05350)
*Federica Di Stefano,Quentin Manière,Magdalena Ortiz,Mantas Šimkus*

Main category: cs.AI

TL;DR: 研究描述逻辑中纯最小模型的问题，发现即使在简单逻辑（如$\mathcal{EL}$）中概念可满足性也是不可判定的，并提出通过限制TBox的循环性来恢复可判定性。


<details>
  <summary>Details</summary>
Motivation: 理解描述逻辑中纯最小模型的推理问题，填补现有研究的空白。

Method: 分析纯最小模型在描述逻辑中的表现，提出限制TBox循环性的方法。

Result: $\mathcal{EL}$中的概念可满足性不可判定；通过限制TBox可恢复可判定性，但复杂度高。

Conclusion: 纯最小模型在描述逻辑中具有挑战性，需进一步研究限制条件以平衡复杂性和表达能力。
...
### [137] [StructVRM: Aligning Multimodal Reasoning with Structured and Verifiable Reward Models](https://arxiv.org/abs/2508.05383)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在多问题推理任务中表现不佳，传统二元奖励机制无法提供细粒度指导。

Method: 提出StructVRM，利用模型验证器提供子问题级别的反馈，评估语义和数学等价性。

Result: Seed-StructVRM在12个多模态基准中的6个及新STEM-Bench上达到最佳性能。
...
### [138] [An Explainable Machine Learning Framework for Railway Predictive Maintenance using Data Streams from the Metro Operator of Portugal](https://arxiv.org/abs/2508.05388)
*Silvia García-Méndez,Francisco de Arriba-Pérez,Fátima Leal,Bruno Veloso,Benedita Malheiro,Juan Carlos Burguillo-Rial*

Main category: cs.AI

TL;DR: 本文提出了一种实时数据驱动的智能交通系统预测性维护方法，结合了样本预处理、增量分类和结果解释模块，实验结果显示高准确性和F1值。


<details>
  <summary>Details</summary>
Motivation: 为智能交通系统提供实时、高精度的故障预测，以支持主动维护决策，减少误报并提高服务可用性。

Method: 采用包含样本预处理、增量分类和解释性模块的在线处理流程，特别关注统计和频率特征的动态生成。

Result: 在Porto地铁数据集上，F1值超过98%，准确率达99%，且在类不平衡和噪声下表现稳定。
...
### [139] [DeepPHY: Benchmarking Agentic VLMs on Physical Reasoning](https://arxiv.org/abs/2508.05405)
...
<details>
  <summary>Details</summary>
Motivation: VLMs在复杂动态环境中缺乏细节关注和精确行动规划能力，而现实任务需要高级空间推理和长期规划，因此需要评估其对物理规则的理解。

Method: 提出DeepPHY框架，通过一系列模拟环境系统评估VLMs的物理推理能力，包含多难度环境和细粒度指标。
...
### [140] [Large Language Models Transform Organic Synthesis From Reaction Prediction to Automation](https://arxiv.org/abs/2508.05427)
*Kartar Kumar Lohana Tharwani,Rajesh Kumar,Sumita,Numan Ahmed,Yong Tang*

Main category: cs.AI

TL;DR: LLMs正改变有机合成方式，结合其他技术加速绿色化学发展，但仍需解决数据偏见等问题。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs如何从理论工具转变为实验室实用助手，推动分子创新。

Method: 结合图神经网络、量子计算和实时光谱技术，优化合成路线预测与实验执行。

Result: 缩短发现周期，支持数据驱动的绿色化学，但仍存在数据集偏见等问题。

Conclusion: 通过开放基准和可解释界面，实现快速、可靠且包容的分子创新。
...
### [141] [Bench-2-CoP: Can We Trust Benchmarking for EU AI Compliance?](https://arxiv.org/abs/2508.05464)
*Matteo Prandi,Vincenzo Suriani,Federico Pierucci,Marcello Galisai,Daniele Nardi,Piercosma Bisconti*

Main category: cs.AI

TL;DR: 论文提出了Bench-2-CoP框架，用于量化AI评估基准与新法规（如欧盟AI法案）之间的差距，发现现有评估工具严重忽视系统风险。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估工具未能覆盖新法规关注的系统风险，亟需量化这一差距以改进评估框架。

Method: 使用LLM-as-judge分析方法，将194,955个基准问题映射到欧盟AI法案的能力分类中。

Result: 发现评估工具过度关注行为倾向（如幻觉倾向和歧视性偏见），而关键功能能力（如失控场景）几乎未被覆盖。
...
### [142] [Can Large Language Models Generate Effective Datasets for Emotion Recognition in Conversations?](https://arxiv.org/abs/2508.05474)
*Burak Can Kaplan,Hugo Cesar De Castro Carneiro,Stefan Wermter*

Main category: cs.AI

TL;DR: 论文提出了一种使用小型通用LLM生成多样化ERC数据集的方法，以解决现有数据稀缺和偏差问题，并验证了生成数据对ERC分类和标签不平衡分析的实用性。


<details>
  <summary>Details</summary>
Motivation: ERC数据稀缺且存在偏差，现有LLM成本高且应用有限，需一种资源高效的方法生成多样化数据。

Method: 使用小型通用LLM生成六个新数据集，补充三个主流ERC基准，并评估其在分类和标签不平衡分析中的效果。

Result: 生成的数据集显著提升了ERC分类器的鲁棒性，并在现有基准上实现了统计显著的性能改进。

Conclusion: 小型LLM生成的多样化数据集能有效补充ERC任务，提升模型性能并解决数据稀缺问题。
...
### [143] [InfiAlign: A Scalable and Sample-Efficient Framework for Aligning LLMs to Enhance Reasoning Capabilities](https://arxiv.org/abs/2508.05496)
*Shuo Cai,Su Lu,Qi Zhou,Kejing Yang,Zhijie Sang,Congkai Xie,Hongxia Yang*

Main category: cs.AI

TL;DR: InfiAlign是一个可扩展且样本高效的后训练框架，结合监督微调（SFT）和直接偏好优化（DPO），通过多维质量指标自动筛选高质量数据，显著提升LLM的推理能力，同时减少数据需求。


<details>
  <summary>Details</summary>
Motivation: 提升大型语言模型（LLM）的推理能力通常需要大量数据和计算资源，现有方法依赖启发式或任务特定策略，难以扩展。

Method: 提出InfiAlign框架，集成SFT和DPO，通过自动数据选择管道从开源推理数据集中筛选高质量对齐数据。
...
### [144] [GRAIL:Learning to Interact with Large Knowledge Graphs for Retrieval Augmented Reasoning](https://arxiv.org/abs/2508.05498)
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在处理结构化知识（如知识图谱）时能力有限，且图检索方法难以捕获整体结构并控制精度。
...
### [145] [Auto-Eval Judge: Towards a General Agentic Framework for Task Completion Evaluation](https://arxiv.org/abs/2508.05508)
...
TL;DR: 提出了一种通用、模块化的框架，用于评估代理任务完成情况，通过分解任务并验证每个步骤，提高与人类评估的一致性。
...
Method: 设计了一个模块化框架，将任务分解为子任务，逐步验证，并汇总模块输出以生成最终评估结果。
...
### [146] [Streamlining Admission with LOR Insights: AI-Based Leadership Assessment in Online Master's Program](https://arxiv.org/abs/2508.05513)
*Meryem Yilmaz Soylu,Adrian Gallard,Jeonghyun Lee,Gayane Grigoryan,Rushil Desai,Stephen Harmon*

Main category: cs.AI

TL;DR: 论文介绍了一种名为LORI的AI工具，用于从推荐信中自动检测领导力技能，以简化招生流程并提升评估的全面性。


<details>
  <summary>Details</summary>
Motivation: 推荐信（LORs）能提供候选人能力和经验的额外信息，但人工审查耗时费力。因此，研究旨在通过AI工具LORI解决这一问题，支持招生委员会高效评估申请者的领导力。

Method: 研究采用自然语言处理技术，结合RoBERTa和LLAMA等大型语言模型，识别推荐信中的领导力属性（如团队合作、沟通和创新）。

Result: RoBERTa模型在测试数据中表现优异，加权F1分数为91.6%，精确率为92.4%，召回率为91.6%。

Conclusion: LORI工具在STEM领域招生中具有重要价值，能自动化评估领导力技能，提升招生效率和评估的全面性。
...
### [147] [MV-Debate: Multi-view Agent Debate with Dynamic Reflection Gating for Multimodal Harmful Content Detection in Social Media](https://arxiv.org/abs/2508.05557)
*Rui Lu,Jinhe Bi,Yunpu Ma,Feng Xiao,Yuntao Du,Yijun Tian*

Main category: cs.AI

TL;DR: MV-Debate是一种多视图代理辩论框架，用于统一多模态有害内容检测，通过动态反思门控提高准确性。
...
Conclusion: 多代理辩论框架在安全关键的在线环境中具有提升社会意图检测可靠性的潜力。
...
### [148] [The Missing Reward: Active Inference in the Era of Experience](https://arxiv.org/abs/2508.05619)
*Bo Wen*
...
Method: 结合主动推理（AIF）与大型语言模型，利用内在驱动力和贝叶斯目标实现自主学习和决策。

Result: 提出了一种能高效学习并保持与人类价值观一致的自主AI代理框架。

Conclusion: AIF为解决自主AI的"接地代理差距"提供了可行路径，同时满足计算和物理约束。
...
### [149] [Simulating Human-Like Learning Dynamics with LLM-Empowered Agents](https://arxiv.org/abs/2508.05622)
*Yu Yuan,Lili Zhao,Wei Chen,Guangting Zheng,Kai Zhang,Mengdi Zhang,Qi Liu*

Main category: cs.AI

TL;DR: LearnerAgent是一个基于大型语言模型的多智能体框架，用于模拟真实教学环境，研究人类学习行为。通过心理特征分类和学习动态跟踪，揭示了不同类型学习者的认知和行为模式。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉学习动态或提供可解释性，因此需要一种新框架来模拟真实学习环境并分析学习行为。

Method: 构建具有心理特征的学习者（如深层、表层和懒惰学习者），通过知识获取、策略选择、测试和互动，跟踪其全年学习动态。

Result: 研究发现：1）深层学习者持续认知增长；2）学习者行为与心理特征一致；3）自我效能感演变现实；4）基础LLM默认行为类似勤奋但脆弱的表层学习者。

Conclusion: LearnerAgent能有效模拟真实学习场景，为LLM行为提供深入见解。
...
### [150] [Agency, Affordances, and Enculturation of Augmentation Technologies](https://arxiv.org/abs/2508.04725)
*Ann Hill Duin,Isabel Pedersen*

Main category: cs.CY

TL;DR: 论文探讨了增强技术的文化适应过程，重点关注AI技术的影响，批判性地分析了其对生活、文化和社会的影响，并探讨了非人类代理在技术发展中的作用。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于揭示AI技术如何通过营销传播等方式被文化适应，并探讨其对人类社会和商业数字景观（如元宇宙）的影响。

Method: 方法包括分析WIPO的AI技术分类方案，结合媒体与传播研究，探讨代理、权力等概念，以及非人类代理在技术发展中的作用。

Result: 研究发现，增强技术通过营销传播被文化适应，推动了商业数字景观（如元宇宙）的发展，但也引发了对其影响的批判性讨论。

Conclusion: 结论指出，尽管增强技术有潜力改善社会，但其文化适应过程和商业驱动需被批判性审视，尤其是在元宇宙和增强现实领域。
...
### [151] [Resistance Technologies: Moving Beyond Alternative Designs](https://arxiv.org/abs/2508.05223)
*Iness Ben Guirat,Jan Tobias Mühlberg*

Main category: cs.CY

TL;DR: 本文提出“抵抗技术”概念，强调技术设计需关注环境破坏后果（如战争、殖民），而非仅优化资源或替代服务。反监控技术是主权的核心组成部分，应纳入可持续发展讨论。


<details>
  <summary>Details</summary>
Motivation: 当前可持续技术讨论多聚焦资源优化或替代服务，忽视了环境破坏引发的严重后果（如战争、殖民）。本文旨在重新定义技术设计的核心价值。

Method: 通过理论分析，提出“抵抗技术”概念，并论证反监控技术作为主权的组成部分，呼吁与Computing-within-Limits社区展开对话。

Result: 明确了“抵抗技术”的定义及其在可持续发展中的重要性，强调反监控技术的主权意义。

Conclusion: 未来技术设计需纳入“抵抗技术”理念，尤其是反监控技术，以应对环境破坏的连锁反应。
...
### [152] [Everything You Need to Know About CS Education: Open Results from a Survey of More Than 18,000 Participants](https://arxiv.org/abs/2508.05286)
...
TL;DR: 本文通过一项覆盖18,032名学习者的全球调查，更新了计算机科学教育领域的综合研究，探讨了多种主题，并提供了开放数据集以支持未来研究。


<details>
  <summary>Details</summary>
Motivation: 计算机科学教育领域的新趋势（如AI）和学习形式（如IDE内学习）的出现，以及学习者多样性的增加，需要更新的全面研究。

Method: 对来自173个国家的18,032名学习者进行了大规模调查，涵盖正式教育、学习形式、AI使用、挑战和动机等多个主题。

Result: 提供了开放的调查数据集，并展示了三个潜在研究方向：学习挑战、新兴学习形式和IDE内学习形式的见解。

Conclusion: 数据集旨在支持计算机教育领域的进一步研究，推动该领域的进步。
...
### [153] [Building Effective Safety Guardrails in AI Education Tools](https://arxiv.org/abs/2508.05360)
*Hannah-Beth Clark,Laura Benton,Emma Searle,Margaux Dowland,Matthew Gregory,Will Gayne,John Roberts*

Main category: cs.CY

TL;DR: 本文探讨了Oak National Academy如何通过四项安全防护措施（提示工程、输入威胁检测、独立内容审核代理和人工审核）确保生成式AI教育工具Aila的安全性和适龄性，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育领域的快速发展和教师采用率的提高，AI生成内容的安全性和适龄性成为关注焦点。

Method: 采用四项安全防护措施：提示工程、输入威胁检测、独立内容审核代理（IACMA）和人工审核。

Result: 通过评估发现实施安全防护措施时的挑战和机遇，并提出了改进建议。

Conclusion: 建议通过持续迭代、跨部门协作和开源共享来构建更有效的生成式AI教育工具安全防护措施。
...
### [154] [NAEx: A Plug-and-Play Framework for Explaining Network Alignment](https://arxiv.org/abs/2508.04731)
...
### [155] [Provable Post-Training Quantization: Theoretical Analysis of OPTQ and Qronos](https://arxiv.org/abs/2508.04853)
*Haoyu Zhang,Shihao Zhang,Ian Colbert,Rayan Saab*

Main category: cs.LG

TL;DR: 本文首次为OPTQ（GPTQ）及其随机变体以及Qronos算法提供了严格的量化误差界限，分析了OPTQ迭代过程如何引入量化误差，并提出了非渐近2-范数误差界限。


<details>
  <summary>Details</summary>
Motivation: 尽管OPTQ在实践中有广泛应用，但缺乏严格的理论保证。本文旨在填补这一空白，为OPTQ及其变体提供理论支持。

Method: 通过分析OPTQ的迭代过程，推导出非渐近2-范数误差界限，并扩展到随机变体和Qronos算法。

Result: 提出了明确的误差界限，解释了实践中常用的启发式方法（如按范数排序特征）的理论依据，并为参数选择提供了指导。

Conclusion: 本文为OPTQ和Qronos提供了理论支持，解释了其优势，并为实际应用中的设计选择提供了依据。
...
### [156] [LumiGen: An LVLM-Enhanced Iterative Framework for Fine-Grained Text-to-Image Generation](https://arxiv.org/abs/2508.04732)
*Xiaoqi Dong,Xiangyu Zhou,Nicholas Evans,Yujia Lin*

Main category: cs.LG

TL;DR: LumiGen是一个基于视觉语言模型（LVLM）的迭代框架，通过闭环反馈机制提升文本到图像（T2I）生成的性能，尤其在细粒度控制和语义一致性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前T2I生成模型在处理复杂指令、细粒度内容控制和语义一致性方面存在不足，而LVLM在跨模态理解和指令跟随方面表现出色。

Method: LumiGen包含智能提示解析与增强（IPPA）模块和迭代视觉反馈与优化（IVFR）模块，通过LVLM驱动的闭环反馈机制优化图像生成。

Result: 在LongBench-T2I基准测试中，LumiGen平均得分为3.08，优于现有方法，尤其在文本渲染和姿势表达方面表现突出。
...
### [157] [Deep Neural Networks with General Activations: Super-Convergence in Sobolev Norms](https://arxiv.org/abs/2508.05141)
...
Method: 使用常用且通用的激活函数，在Sobolev空间$W^{n,\infty}$中分析深度神经网络的逼近能力，误差以$W^{m,p}$-范数衡量。

Result: 深度神经网络在逼近PDE弱解时表现出超收敛现象，精度超越有限元和谱方法等传统数值方法。

Conclusion: 研究为神经网络在科学计算中的应用提供了理论支持，展示了其优于传统方法的潜力。
...
### [158] [MissMecha: An All-in-One Python Package for Studying Missing Data Mechanisms](https://arxiv.org/abs/2508.04740)
*Youran Zhou,Mohamed Reda Bouadjenek,Sunil Aryal*

Main category: cs.LG

TL;DR: MissMecha是一个开源Python工具包，用于模拟、可视化和评估MCAR、MAR和MNAR假设下的缺失数据，支持数值和分类特征，为不完整数据研究提供统一平台。


<details>
  <summary>Details</summary>
Motivation: 现实数据中缺失数据问题普遍存在，但现有工具分散且仅关注数值变量，无法满足混合类型表格数据的复杂需求。

Method: 开发MissMecha工具包，支持MCAR、MAR和MNAR假设下的缺失数据模拟，提供可视化诊断、MCAR测试工具和类型感知的插补评估指标。

Result: MissMecha为研究人员和从业者提供了一个统一的平台，支持混合类型数据的机制感知研究和数据质量分析。

Conclusion: MissMecha填补了现有工具的不足，为缺失数据研究、基准测试和教育提供了全面支持。
...
### [159] [Edge-Assisted Collaborative Fine-Tuning for Multi-User Personalized Artificial Intelligence Generated Content (AIGC)](https://arxiv.org/abs/2508.04745)
*Nan Li,Wanting Yang,Marie Siew,Zehui Xiong,Binbin Chen,Shiwen Mao,Kwok-Yan Lam*

Main category: cs.LG

TL;DR: 本文提出了一种基于集群感知的分层联邦聚合框架，用于解决边缘设备上扩散模型的计算效率和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在高质量内容生成方面表现优异，但其计算需求高，边缘设备难以满足，且云解决方案存在隐私、个性化和通信成本问题。

Method: 采用基于LoRA的参数高效本地微调，通过客户端任务需求相似性聚类，并进行集群内聚合和集群间知识交互。

Result: 框架在边缘约束下实现了加速收敛，同时支持可扩展的多用户个性化AIGC服务。

Conclusion: 该框架有效平衡了计算效率、隐私保护和个性化需求，适用于边缘设备上的多用户场景。
...
### [160] [A Foundational Multi-Modal Model for Few-Shot Learning](https://arxiv.org/abs/2508.04746)
*Pengtao Dang,Tingbo Guo,Sha Cao,Chi Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种基于大型多模态模型（LMMM）的小样本学习方法（FSL），通过多模态数据集M3FD和框架M3F，显著提升了FSL模型的泛化能力。
...
### [161] [AttriLens-Mol: Attribute Guided Reinforcement Learning for Molecular Property Prediction with Large Language Models](https://arxiv.org/abs/2508.04748)
...
Result: 在实验中对7B规模的模型进行训练，性能显著优于监督微调模型和先进模型，同时提取的属性在决策树模型中表现更优。
...
### [162] [PA-RNet: Perturbation-Aware Reasoning Network for Multimodal Time Series Forecasting](https://arxiv.org/abs/2508.04750)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视文本数据中的固有扰动，导致模型性能下降，尤其是在噪声强度变化或结构不一致时。

Method: 提出PA-RNet，包含扰动感知投影模块和跨模态注意力机制，理论证明其Lipschitz连续性和预测误差降低能力。

Result: 实验表明PA-RNet在多种领域和时间设置下优于现有基线。
...
### [163] [InfoQ: Mixed-Precision Quantization via Global Information Flow](https://arxiv.org/abs/2508.04753)
*Mehmet Emre Akbulut,Hazem Hesham Yousef Shalby,Fabrizio Pittorino,Manuel Roveri*

Main category: cs.LG

TL;DR: InfoQ是一种无需训练的混合精度量化框架，通过单次前向传播评估层敏感度，优化比特分配，显著提升搜索效率和模型精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖计算昂贵的搜索或局部启发式代理（如Hessian），无法捕捉量化误差的全局级联效应，需要更高效的量化敏感度评估方法。

Method: 提出InfoQ框架，通过量化各层并测量后续层互信息变化来评估敏感度，将比特分配建模为整数线性规划问题。

Result: 在ImageNet上，MobileNetV2和ResNet18的精度提升达1%，压缩率分别为14X和10.66X，搜索效率提高两个数量级。

Conclusion: InfoQ通过全局信息流评估量化敏感度，实现了高效且高精度的混合精度量化。
...
### [164] [Are Large Language Models Dynamic Treatment Planners? An In Silico Study from a Prior Knowledge Injection Angle](https://arxiv.org/abs/2508.04755)
*Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: 研究表明，通过精心设计的零样本提示，小型LLM（如Qwen2.5-7B）在动态胰岛素剂量任务中表现优于或接近经过训练的RL代理，但在复杂生理动态推理中存在局限性。


<details>
  <summary>Details</summary>
Motivation: 解决RL在动态治疗中的工程复杂性，探索LLM作为替代方案的潜力。

Method: 在模拟的1型糖尿病环境中，比较零样本LLM与训练过的RL代理的性能，分析其临床推理能力。

Result: LLM在稳定患者群体中表现优异，但在复杂推理中易出现剂量过激等问题。

Conclusion: 建议谨慎整合LLM到临床工作流，需结合提示工程和生理建模以确保安全性和有效性。
...
### [165] [Uncertainty-aware Predict-Then-Optimize Framework for Equitable Post-Disaster Power Restoration](https://arxiv.org/abs/2508.04780)
...
<details>
  <summary>Details</summary>
Motivation: 极端天气事件频发，现有电力恢复策略因请求提交量的差异导致不公平，弱势社区更容易遭受长时间停电。

Method: 设计了EPOPR框架，包含Equity-Conformalized Quantile Regression和Spatial-Temporal Attentional RL两部分，分别用于预测修复时间和实现公平决策。

Result: 实验表明，EPOPR将平均停电时间减少3.60%，社区间不公平性降低14.19%。
...
### [166] [Federated Continual Recommendation](https://arxiv.org/abs/2508.04792)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦推荐方法无法处理动态数据流，而持续学习方法又需要集中式数据访问，因此需要一种既能保护隐私又能适应动态数据的解决方案。

Method: 提出了F3CRec框架，包括客户端的自适应回放记忆和服务端的项目级时间均值，以平衡知识保留和适应能力。

Result: 实验表明，F3CRec在联邦环境下长期保持推荐质量方面优于现有方法。

Conclusion: F3CRec成功解决了联邦持续推荐任务中的隐私保护和动态适应问题，为未来研究提供了新方向。
...
### [167] [HCRide: Harmonizing Passenger Fairness and Driver Preference for Human-Centered Ride-Hailing](https://arxiv.org/abs/2508.04811)
...
<details>
  <summary>Details</summary>
Motivation: 现有网约车系统过于关注运营商收入，忽视了乘客和司机的体验，因此需要设计一个兼顾公平性和偏好的系统。
...
### [168] [Unified Flow Matching for Long Horizon Event Forecasting](https://arxiv.org/abs/2508.04843)
*Xiao Shou*
...
<details>
  <summary>Details</summary>
Motivation: 现有自回归模型在长时标事件序列预测中效率低且误差累积，需要更高效的建模方法。
...
### [169] [Multi-Stage Knowledge-Distilled VGAE and GAT for Robust Controller-Area-Network Intrusion Detection](https://arxiv.org/abs/2508.04845)
...
Result: 在六个公开CAN数据集上表现优异，F1分数平均提升16.2%，参数量减少96%。

Conclusion: 该方法在检测精度和效率上优于现有方法，尤其适用于类别不平衡的数据集。
...
### [170] [Bidding-Aware Retrieval for Multi-Stage Consistency in Online Advertising](https://arxiv.org/abs/2508.05206)
*Bin Liu,Yunfei Liu,Ziru Xu,Zhaoyu Zhou,Zhi Kou,Yeqiu Yang,Han Zhu,Jian Xu,Bo Zheng*

Main category: cs.LG

TL;DR: 论文提出了一种名为Bidding-Aware Retrieval (BAR)的模型框架，通过将广告出价信息整合到检索评分函数中，解决了在线广告系统中检索阶段与排序阶段的不一致问题，从而提升了平台收入和广告效果。


<details>
  <summary>Details</summary>
Motivation: 随着自动出价策略的普及，广告系统中检索阶段无法实时获取精确出价信息，导致与排序阶段的不一致，进而影响平台收入和广告效果。

Method: BAR框架通过Bidding-Aware Modeling（单调性约束学习和多任务蒸馏）整合出价信号，并利用Asynchronous Near-Line Inference实现实时更新，同时通过Task-Attentive Refinement模块增强特征交互。

Result: 在阿里巴巴展示广告平台上的离线实验和全规模部署显示，BAR实现了4.32%的平台收入增长和22.2%的正向操作广告展示量提升。
...
### [171] [Agnostics: Learning to Code in Any Programming Language via Reinforcement with a Universal Learning Environment](https://arxiv.org/abs/2508.04865)
*Aleksander Boruch-Gruszecki,Yangtian Zi,Zixuan Wu,Tejas Oberoi,Carolyn Jane Anderson,Joydeep Biswas,Arjun Guha*

Main category: cs.LG

TL;DR: Agnostics是一种语言无关的后训练管道，通过观察代码行为而非语言特性，简化了对低资源语言的支持，提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在低资源语言上的表现不足问题，避免为每种语言单独开发数据集和测试工具。

Method: 将单元测试数据集转换为I/O格式，提供配置以编译运行目标语言，使用RLVR在代码执行环境中训练。

Result: 在五种低资源语言上显著提升模型性能，部分模型达到或超越更大规模模型的表现。

Conclusion: Agnostics简化了多语言支持，为低资源语言的代码生成提供了高效解决方案。
...
### [172] [Hilbert Neural Operator: Operator Learning in the Analytic Signal Domain](https://arxiv.org/abs/2508.04882)
*Saman Pordanesh,Pejman Shahsavari,Hossein Ghadjari*

Main category: cs.LG

TL;DR: 论文提出了一种新的神经算子架构Hilbert Neural Operator (HNO)，通过结合希尔伯特变换的强归纳偏置，改进了现有Fourier Neural Operator (FNO)在非周期性、因果性和相位敏感系统建模中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有FNO方法在频域中进行卷积，虽然有效，但受限于傅里叶变换的周期性假设，且缺乏对信号瞬时振幅和相位信息的显式利用。HNO旨在通过希尔伯特变换提供更丰富的信号特征，以更好地建模复杂系统。

Method: HNO首先通过希尔伯特变换将输入信号映射为解析表示，显式提取瞬时振幅和相位信息，随后在希尔伯特变换后的表示上应用可学习的谱卷积操作。

Result: HNO在理论上能够更有效地建模因果性、相位敏感性和非平稳系统的算子。

Conclusion: HNO通过引入希尔伯特变换的归纳偏置，为神经算子提供了一种新的架构，有望在复杂系统建模中取得更好的效果。
...
### [173] [Gaussian mixture layers for neural networks](https://arxiv.org/abs/2508.04883)
*Sinho Chewi,Philippe Rigollet,Yuling Yan*

Main category: cs.LG

TL;DR: 该论文研究了在概率测度上直接实现动态的可能性，提出了一种新型的高斯混合（GM）层，并通过实验验证其性能与传统两层全连接网络相当。
...
Method: 使用高斯混合模型作为参数化分布族，结合Wasserstein梯度流理论，推导出训练动态，并引入GM层。
...
### [174] [Uncertainty Quantification for Surface Ozone Emulators using Deep Learning](https://arxiv.org/abs/2508.04885)
...
<details>
  <summary>Details</summary>
Motivation: 地表臭氧（O3）是重要污染物，传统物理模型难以准确模拟其趋势，且现有深度学习模型缺乏可解释性，无法支持政策决策。

Method: 采用不确定性感知的U-Net架构，结合贝叶斯和分位数回归方法，预测MOMO-Chem模型的地表臭氧偏差。

Result: 在北美和欧洲的2019年6月数据中验证了方法的有效性，比较了两种不确定性量化方法，并评估了土地利用信息的影响。

Conclusion: 该方法为地表臭氧偏差建模提供了可解释且实用的工具，支持政策制定和公共健康措施。
...
### [175] [Leveraging Deep Learning for Physical Model Bias of Global Air Quality Estimates](https://arxiv.org/abs/2508.04886)
...
Result: 该方法在北美和欧洲表现出优于传统机器学习方法的能力，并能更好地捕捉物理模型残差。

Conclusion: 研究结果有助于提高对城市尺度臭氧偏差影响因素的科学理解，从而改进环境政策。
...
### [176] [Retrieval-Augmented Water Level Forecasting for Everglades](https://arxiv.org/abs/2508.04888)
*Rahuul Rangaraj,Jimeng Shi,Rajendra Paudel,Giri Narasimhan,Yanzhao Wu*

Main category: cs.LG

TL;DR: 该论文提出了一种名为检索增强预测（RAF）的框架，用于提高水文领域的水位预测准确性，通过检索历史类似水文事件来增强模型输入，无需任务特定的重新训练。


<details>
  <summary>Details</summary>
Motivation: 准确的水位预测对生态系统管理至关重要，但现有深度学习模型在水文领域的应用不足，且难以泛化到未见过的数据集和领域。

Method: 提出RAF框架，通过检索历史类似水文事件并融入模型输入，探索了基于相似性和互信息的两种RAF方法。
...
### [177] [Honest and Reliable Evaluation and Expert Equivalence Testing of Automated Neonatal Seizure Detection](https://arxiv.org/abs/2508.04899)
*Jovana Kljajic,John M. O'Toole,Robert Hogan,Tamara Skoric*

Main category: cs.LG

TL;DR: 该研究系统评估了新生儿癫痫检测中机器学习模型的性能指标，提出了针对类不平衡、评分者间一致性和评分者数量的最佳实践，并推荐了具体的报告标准。


<details>
  <summary>Details</summary>
Motivation: 当前新生儿癫痫检测模型的评估方法存在不一致性和偏差，影响了模型的可比性和可靠性，亟需系统化的评估和改进。

Method: 使用真实和合成的癫痫标注数据，评估了标准性能指标、共识策略和人类专家水平等价测试，分析了类不平衡、评分者间一致性和评分者数量的影响。

Result: Matthews和Pearson相关系数在类不平衡下表现优于AUC；共识类型对评分者数量和一致性敏感；多评分者图灵测试（Fleiss k）最能反映专家级AI性能。

Conclusion: 建议报告至少一种平衡指标、敏感性和特异性等指标、多评分者图灵测试结果，并在验证集上全面评估，为临床验证提供可靠基础。
...
### [178] [Sensitivity of Stability: Theoretical & Empirical Analysis of Replicability for Adaptive Data Selection in Transfer Learning](https://arxiv.org/abs/2508.04901)
*Prabhav Singh,Jessica Sorrell*

Main category: cs.LG

TL;DR: 论文研究了迁移学习中自适应数据选择策略的可靠性，提出了选择敏感性（$\Delta_Q$）的数学框架，揭示了性能与结果一致性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 迁移学习的广泛应用使得预训练模型能够高效适应新领域，但其可靠性，尤其是动态优先选择训练数据的策略，尚未得到充分理解。

Method: 论文通过理论和实证分析，引入选择敏感性（$\Delta_Q$）来衡量自适应策略对数据扰动的响应，并研究了样本量对结果一致性的影响。

Result: 实验表明，高适应性策略（如基于梯度的选择）性能优越但复制失败率高，而低适应性策略失败率低于7%。预训练可显著降低失败率。

Conclusion: 研究为迁移学习中的性能与复制性权衡提供了指导，并强调了现代迁移学习系统设计中复制性意识的重要性。
...
### [179] [Advancing Hate Speech Detection with Transformers: Insights from the MetaHate](https://arxiv.org/abs/2508.04913)
...
<details>
  <summary>Details</summary>
Motivation: 仇恨言论在社交媒体上广泛存在，对社会和个人造成严重伤害，需要开发高效的自动化检测方法。

Method: 研究采用Transformer模型（如BERT、RoBERTa、GPT-2和ELECTRA）对MetaHate数据集（包含120万个社交媒体样本）进行仇恨言论检测。

Result: 微调后的ELECTRA模型表现最佳，F1分数达0.8980。研究还发现分类错误与讽刺、编码语言和标签噪声相关。

Conclusion: Transformer模型在仇恨言论检测中表现出色，但需进一步解决讽刺和编码语言等挑战。
...
### [180] [ALScope: A Unified Toolkit for Deep Active Learning](https://arxiv.org/abs/2508.04937)
*Chenkai Wu,Yuanyuan Qi,Xiaohao Yang,Jueqing Lu,Gang Liu,Wray Buntine,Lan Du*

Main category: cs.LG

TL;DR: ALScope是一个新的深度主动学习平台，整合了10个数据集和21种算法，支持灵活配置实验条件，用于全面评估算法性能。实验发现算法性能因领域和任务设置而异，非标准场景下算法仍有改进空间，部分算法性能好但耗时较长。


<details>
  <summary>Details</summary>
Motivation: 由于现实应用中分布偏移和数据不平衡问题日益突出，缺乏统一的平台阻碍了对深度主动学习算法的公平和系统性评估。

Method: 开发了ALScope平台，整合了CV和NLP领域的10个数据集及21种代表性算法，支持灵活配置实验条件（如OOD样本比例、类别不平衡比例）。

Result: 实验表明：(1)算法性能因领域和任务设置差异显著；(2)非标准场景下算法表现有待改进；(3)部分算法性能优异但选择时间较长。

Conclusion: ALScope为深度主动学习研究提供了统一评估平台，揭示了算法在不同场景下的表现差异和改进需求。
...
### [181] [REINA: Regularized Entropy Information-Based Loss for Efficient Simultaneous Speech Translation](https://arxiv.org/abs/2508.04946)
*Nameer Hirschkind,Joseph Liu,Mahesh Kumar Nandwana,Xiao Yu*

Main category: cs.LG

TL;DR: 论文提出了一种名为REINA的新损失函数，用于优化同步语音翻译（SimulST）系统的延迟与翻译质量之间的权衡。
...
Method: 基于信息论原则，提出REINA损失函数，通过训练自适应策略来优化延迟与质量的权衡。

Result: 在法语、西班牙语和德语的翻译任务中，REINA显著提升了延迟与质量的权衡，比现有方法提高了21%。

Conclusion: REINA为同步语音翻译系统提供了一种高效的优化方法，显著提升了性能。
...
### [182] [Self-Error Adjustment: Theory and Practice of Balancing Individual Performance and Diversity in Ensemble Learning](https://arxiv.org/abs/2508.04948)
*Rui Zou*

Main category: cs.LG

TL;DR: 提出了一种名为SEA的新框架，通过分解集成误差为个体性能和多样性两部分，实现对集成性能的精细调控，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统集成学习方法（如Bagging和Boosting）在平衡个体学习器准确性与多样性方面缺乏精确控制，NCL方法虽有改进但理论界限松散且调整范围有限。

Method: SEA框架将集成误差分解为个体性能项和多样性项，通过引入可调参数实现对两者的精确控制。

Result: SEA在多个公开数据集上表现优于基线方法，提供了更灵活的调整能力和更一致的多样性变化。

Conclusion: SEA通过更精细的误差分解和参数调整，显著提升了集成学习的性能，并提供了更严格的理论界限。
...
### [183] [Compressed Decentralized Momentum Stochastic Gradient Methods for Nonconvex Optimization](https://arxiv.org/abs/2508.04950)
*Wei Liu,Anweshit Panda,Ujwal Pandey,Christopher Brissette,Yikang Shen,George M. Slota,Naigang Wang,Jie Chen,Yangyang Xu*

Main category: cs.LG

TL;DR: 本文设计了两种压缩去中心化算法，分别用于解决非凸随机优化问题，结合动量技术和消息压缩技术以提高收敛速度和降低通信成本。


<details>
  <summary>Details</summary>
Motivation: 研究如何在去中心化算法中同时利用动量加速和压缩通信的优势，并理论证明其有效性。
...
Conclusion: 算法在理论和实验上均优于现有方法，为去中心化非凸优化提供了高效解决方案。
...
### [184] [MENDR: Manifold Explainable Neural Data Representations](https://arxiv.org/abs/2508.04956)
...
TL;DR: MENDR是一种基于滤波器组和黎曼流形变换器的脑电图（EEG）基础模型，通过离散小波包变换学习对称正定矩阵嵌入，提高了EEG信号的可解释性和重建能力。


<details>
  <summary>Details</summary>
Motivation: 当前EEG基础模型缺乏预训练动态的透明性，且对EEG信息的保留情况不明确，限制了临床应用的可靠性。

Method: MENDR采用滤波器组和黎曼流形变换器架构，通过离散小波包变换分解EEG信号，学习对称正定矩阵嵌入。

Result: MENDR在多项临床EEG任务中接近最先进性能，且参数更少，支持信号重建和几何可视化。

Conclusion: MENDR为高效、可解释且适用于临床的EEG分析提供了潜在解决方案。
...
### [185] [RCUKF: Data-Driven Modeling Meets Bayesian Estimation](https://arxiv.org/abs/2508.04985)
...
### [186] [Disentangling Bias by Modeling Intra- and Inter-modal Causal Attention for Multimodal Sentiment Analysis](https://arxiv.org/abs/2508.04999)
*Menghua Jiang,Yuxia Lin,Baoliang Chen,Haifeng Hu,Yuncheng Jiang,Sijie Mai*

Main category: cs.LG

TL;DR: 论文提出了一种多模态因果干预模型（MMCI），通过因果理论中的后门调整解决多模态情感分析中的虚假相关性，提升模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多模态情感分析中存在虚假相关性，导致模型依赖统计捷径而非真实因果关系，影响泛化性能。

Method: 提出MMCI模型，将多模态输入建模为多关系图，利用注意力机制分离因果特征与捷径特征，并通过后门调整动态结合这些特征。

Result: 在多个标准数据集和OOD测试集上验证了MMCI能有效抑制偏差并提升性能。

Conclusion: MMCI通过因果干预显著改善了多模态情感分析的泛化能力。
...
### [187] [R-Zero: Self-Evolving Reasoning LLM from Zero Data](https://arxiv.org/abs/2508.05004)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM训练方法依赖人工标注任务，限制了AI向超人类智能的发展，R-Zero旨在解决这一瓶颈。
...
Conclusion: R-Zero为LLM的自我进化提供了一种无需人工干预的有效方法，推动了AI向超人类智能的发展。
...
### [188] [SPaRFT: Self-Paced Reinforcement Fine-Tuning for Large Language Models](https://arxiv.org/abs/2508.05015)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的LLM训练方法需要大量数据和计算资源，限制了小模型的实用性，且现有课程学习或数据选择方法缺乏效率和可扩展性。

Method: SPaRFT结合了基于聚类的数据缩减和多臂老虎机算法，动态选择训练数据以匹配模型当前能力。

Result: 实验表明，SPaRFT在多个推理基准测试中仅需最多100倍更少的样本即可达到或超越现有方法的准确率。

Conclusion: 结果表明，精心设计的性能驱动训练课程可以以最小资源解锁LLMs的强大推理能力。
...
### [189] [Will You Be Aware? Eye Tracking-Based Modeling of Situational Awareness in Augmented Reality](https://arxiv.org/abs/2508.05025)
*Zhehan Qu,Tianyi Hu,Christian Fronk,Maria Gorlatova*

Main category: cs.LG

TL;DR: 论文研究了AR引导的心肺复苏（CPR）中情境意识（SA）的问题，开发了AR应用并通过眼动追踪分析SA，提出了一种基于图神经网络的SA预测模型。


<details>
  <summary>Details</summary>
Motivation: AR系统在提升任务表现的同时可能引发认知隧道效应，影响安全关键场景中的情境意识（SA）。

Method: 开发了AR应用（Magic Leap 2），结合眼动追踪和用户研究，提出FixGraphPool模型预测SA。

Result: 高SA水平与眼动特征相关，FixGraphPool模型在SA预测上表现优异（准确率83.0%）。

Conclusion: 眼动追踪可用于AR中的SA建模，有助于设计兼顾用户安全和情境意识的AR系统。
...
### [190] [Learning from Oblivion: Predicting Knowledge Overflowed Weights via Retrodiction of Forgetting](https://arxiv.org/abs/2508.05059)
...
<details>
  <summary>Details</summary>
Motivation: 预训练权重在现代深度学习中至关重要，但如何获得包含更多知识的权重仍是一个核心问题。
...
### [191] [TANGO: Graph Neural Dynamics via Learned Energy and Tangential Flows](https://arxiv.org/abs/2508.05070)
*Moshe Eliasof,Eldad Haber,Carola-Bibiane Schönlieb*

Main category: cs.LG

TL;DR: TANGO是一个基于动力系统的图表示学习框架，通过学习的能量景观及其下降动态控制节点特征的演化。


<details>
  <summary>Details</summary>
Motivation: 解决图学习中常见的能量区域平坦或病态问题，同时避免过度压缩（oversquashing）。

Method: 结合可学习的Lyapunov函数和切向分量，通过消息传递实现特征演化，同时保持能量值不变。

Result: 在多种节点和图分类、回归基准测试中表现优异。

Conclusion: TANGO通过联合学习能量函数和切向流，为图神经网络提供了有效的动态表示方法。
...
### [192] [ULU: A Unified Activation Function](https://arxiv.org/abs/2508.05073)
*Simin Huo*

Main category: cs.LG

TL;DR: 论文提出了一种新的非单调分段激活函数ULU及其自适应变体AULU，在图像分类和目标检测任务中显著优于ReLU和Mish。


<details>
  <summary>Details</summary>
Motivation: 现有激活函数（如ReLU和Mish）在处理正负输入时表现不足，需要一种更灵活且高效的分段激活函数。

Method: ULU通过分段定义处理正负输入，AULU进一步引入可学习参数自适应调整响应。此外，提出LIB指标量化模型的归纳偏置。
...
### [193] [Analyzing the Impact of Multimodal Perception on Sample Complexity and Optimization Landscapes in Imitation Learning](https://arxiv.org/abs/2508.05077)
*Luai Abuelsamen,Temitope Lukman Adebanjo*

Main category: cs.LG

TL;DR: 论文通过统计学习理论分析多模态模仿学习的理论基础，探讨多模态感知如何影响样本复杂性和优化景观，并证明多模态策略在泛化边界和优化景观上优于单模态策略。


<details>
  <summary>Details</summary>
Motivation: 研究多模态感知（如RGB-D、本体感觉和语言）对模仿学习的影响，以理解多模态策略为何在性能上优于单模态策略。

Method: 基于多模态学习理论的最新进展，分析多模态策略的泛化边界和优化景观，并结合Rademacher复杂度、PAC学习和信息理论进行理论框架综述。

Result: 研究表明，适当整合的多模态策略可以实现更紧的泛化边界和更优的优化景观。

Conclusion: 多模态架构（如PerAct和CLIPort）的优越性能可通过统计学习理论的基本概念得到解释。
...
### [194] [Integrated Influence: Data Attribution with Baseline](https://arxiv.org/abs/2508.05089)
*Linxiao Yang,Xinyu Gu,Liang Sun*

Main category: cs.LG

TL;DR: 提出了一种新的数据归因方法Integrated Influence，通过引入基线数据集和退化过程，解决了现有LOO方法的局部解释问题，并提供了更灵活的解释能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据归因方法（如LOO）仅关注单个样本的影响，忽视集体作用，且缺乏基线导致解释灵活性不足。

Method: 提出Integrated Influence方法，定义基线数据集，通过数据退化过程累积样本影响，理论框架涵盖现有方法（如影响函数）。

Result: 实验表明，Integrated Influence在数据归因和错误标注样本识别任务中优于现有方法。

Conclusion: Integrated Influence通过引入基线和退化过程，提供了更可靠的数据归因解释，增强了模型透明度。
...
### [195] [Cold Start Active Preference Learning in Socio-Economic Domains](https://arxiv.org/abs/2508.05090)
...
<details>
  <summary>Details</summary>
Motivation: 冷启动问题在计算社会系统和经济分析中尤为突出，标记数据稀缺且昂贵，现有方法性能下降显著。
...
### [196] [Learning from Similarity-Confidence and Confidence-Difference](https://arxiv.org/abs/2508.05108)
*Tomoya Tate,Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: 提出了一种新的弱监督学习框架，通过整合多种弱监督信号（相似性-置信度和置信度-差异）来提升有限标注数据下的模型性能。
...
Method: 提出SconfConfDiff分类方法，整合两种弱标签形式，并设计两种无偏风险估计器（一种基于现有估计器的凸组合，另一种通过建模弱标签间的交互）。

Result: 实验证明，该方法在多种设置下均优于现有基线，且理论分析表明其具有鲁棒性。

Conclusion: 该框架为有限标注数据下的弱监督学习提供了有效解决方案，具有理论和实践意义。
...
### [197] [Exploring Superior Function Calls via Reinforcement Learning](https://arxiv.org/abs/2508.05118)
*Bingguang Hao,Maolin Wang,Zengzhuang Xu,Yicheng Chen,Cunyin Peng,Jinjie GU,Chenyi Zhuang*

Main category: cs.LG

TL;DR: 提出了一种新的强化学习框架，通过基于熵的探索策略优化函数调用任务，解决了探索不足、推理结构缺失和参数验证不足的问题，实验表明其性能优于现有方法。
...
Method: 采用两阶段数据准备流程，结合LLM评估和抽象语法树验证，提出基于熵的探索策略优化函数调用。

Result: 在Berkeley函数调用排行榜上达到86.02%准确率，比标准GRPO提升6%，尤其在代码预训练模型上表现突出。

Conclusion: 该方法为函数调用任务提供了有效的强化学习框架，代码和数据集将开源以促进社区发展。
...
### [198] [HFedATM: Hierarchical Federated Domain Generalization via Optimal Transport and Regularized Mean Aggregation](https://arxiv.org/abs/2508.05135)
...
<details>
  <summary>Details</summary>
Motivation: 传统分层联邦学习（HFL）虽然解决了单点故障和可扩展性问题，但仍面临域偏移的挑战，导致模型在未见目标域上性能下降。

Method: 提出HFedATM方法，包括滤波器最优传输对齐和收缩感知正则化均值聚合，以解决域偏移问题。

Result: 实验表明HFedATM显著提升了现有FedDG基线的性能，同时保持了计算和通信效率。理论分析显示其具有更紧的泛化误差界。
...
### [199] [PSEO: Optimizing Post-hoc Stacking Ensemble Through Hyperparameter Tuning](https://arxiv.org/abs/2508.05144)
*Beicheng Xu,Wei Liu,Keyao Ding,Yupeng Lu,Bin Cui*

Main category: cs.LG

TL;DR: PSEO框架通过结合多样性与性能的模型选择和优化多层堆叠机制，显著提升了AutoML中后验集成方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有AutoML系统在后验集成阶段通常采用固定策略，未能适应任务特性，限制了集成潜力。

Method: PSEO通过二元二次规划选择基础模型，引入多层堆叠机制，并构建超参数空间优化集成策略。
...
### [200] [Domain-driven Metrics for Reinforcement Learning: A Case Study on Epidemic Control using Agent-based Simulation](https://arxiv.org/abs/2508.05154)
*Rishabh Gaur,Gaurav Deshkar,Jayanta Kshirsagar,Harshal Hayatnagarkar,Janani Venugopalan*

Main category: cs.LG

TL;DR: 该研究提出了一种基于领域驱动的强化学习（RL）指标，用于评估和优化基于代理的模型（ABMs）和理性代理模型（RABMs），并通过疾病建模案例展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 由于ABMs和RABMs的复杂性和随机性，以及缺乏标准化的RL算法评估指标，研究旨在开发领域驱动的RL指标以填补这一空白。

Method: 研究结合了领域驱动的奖励与现有先进指标，通过政策优化在理性ABM疾病建模案例中验证了其方法。

Result: 结果表明，领域驱动的奖励与传统及先进指标结合，在不同模拟场景（如口罩供应差异）中表现良好。

Conclusion: 领域驱动的RL指标为ABMs和RABMs的性能评估提供了新的有效工具。
...
### [201] [pFedDSH: Enabling Knowledge Transfer in Personalized Federated Learning through Data-free Sub-Hypernetwork](https://arxiv.org/abs/2508.05157)
*Thinh Nguyen,Le Huy Khiem,Van-Tuan Tran,Khoa D Doan,Nitesh V Chawla,Kok-Seng Wong*

Main category: cs.LG

TL;DR: 论文提出了一种名为pFedDSH的新框架，用于解决动态客户端加入的个性化联邦学习问题，通过超网络和掩码技术保持知识稳定性，并通过数据回放策略提升性能。
...
Method: 提出pFedDSH框架，基于中央超网络生成个性化模型，使用批特定掩码保持知识稳定性，并引入数据回放策略促进反向知识迁移。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet上的实验表明，pFedDSH优于现有方法，实现了对现有客户端的性能稳定性和新客户端的适应性。

Conclusion: pFedDSH在动态客户端加入场景下表现出色，平衡了性能稳定性和资源效率。
...
### [202] [S$^2$M-Former: Spiking Symmetric Mixing Branchformer for Brain Auditory Attention Detection](https://arxiv.org/abs/2508.05164)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于EEG的AAD方法缺乏能够充分利用互补EEG特征且满足能效限制的协同框架。

Method: 提出S$^2$M-Former，采用尖峰驱动的对称架构和轻量级1D令牌序列，减少参数和能耗。

Result: 在三个AAD基准测试中，S$^2$M-Former实现了与SOTA相当的解码精度，同时能耗降低5.8倍。
...
### [203] [Aligning LLMs on a Budget: Inference-Time Alignment with Heuristic Reward Models](https://arxiv.org/abs/2508.05165)
*Mason Nakamura,Saaduddin Mahmud,Kyle H. Wray,Hamed Zamani,Shlomo Zilberstein*

Main category: cs.LG

TL;DR: HIA（启发式引导的推理时对齐）是一种无需调优、兼容黑盒的方法，通过轻量级提示优化器、启发式奖励模型和两阶段过滤，减少推理调用同时保持对齐质量。
...
### [204] [Near Optimal Inference for the Best-Performing Algorithm](https://arxiv.org/abs/2508.05173)
*Amichai Painsky*

Main category: cs.LG

TL;DR: 论文提出了一种新框架，用于从竞争算法中选择性能最优的子集，改进了现有方法，并提供了理论下界验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 在多个数据集上评估算法性能时，如何选择未来表现最好的算法是一个挑战，尤其是性能差异微小的情况下。

Method: 将问题建模为多类别分布的子集选择问题，提出渐近和有限样本方案。

Result: 新方法显著优于现有方法，并通过理论下界验证了其性能优势。

Conclusion: 提出的框架为算法选择提供了高效且可靠的解决方案。
...
### [205] [Human Activity Recognition from Smartphone Sensor Data for Clinical Trials](https://arxiv.org/abs/2508.05175)
...
TL;DR: 该研究开发了一种基于ResNet的人类活动识别（HAR）模型，用于检测步态与非步态活动及日常活动，并在多种智能手机佩戴位置下表现出高准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一种低开销的HAR模型，以准确识别步态与非步态活动及日常活动，并验证其在健康人群和多发性硬化症患者中的实用性。

Method: 使用ResNet架构，结合智能手机传感器数据（来自健康对照组和多发性硬化症患者）进行训练和评估。
...
### [206] [Physics-Informed Time-Integrated DeepONet: Temporal Tangent Space Operator Learning for High-Accuracy Inference](https://arxiv.org/abs/2508.05190)
*Luis Mandl,Dibyajyoti Nayak,Tim Ricken,Somdatta Goswami*

Main category: cs.LG

TL;DR: 论文提出了一种名为PITI-DeepONet的双输出架构，用于解决传统方法在长时间依赖的偏微分方程建模中的不足，显著提升了长期预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如全滚动和自回归）在长时间依赖的偏微分方程建模中存在因果依赖捕捉不足和误差累积的问题，限制了长期预测的准确性和可靠性。

Method: PITI-DeepONet通过双输出架构学习时间导数算子，并结合经典时间步进方案进行时间推进，同时利用残差监测来评估预测质量。

Result: 在多个基准问题中，PITI-DeepONet显著优于传统方法，误差分别减少了84%（vs. FR）和79%（vs. AR）等。
...
### [207] [FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in finance](https://arxiv.org/abs/2508.05201)
*Mengao Zhang,Jiayu Fu,Tanya Warrier,Yuwen Wang,Tianhui Tan,Ke-wei Huang*

Main category: cs.LG

TL;DR: 论文提出了一个评估金融领域大型语言模型（LLMs）内在幻觉的框架，通过上下文感知的掩码预测任务，利用S&P 500年报数据生成评估数据集，并分析了现有LLMs在金融表格数据上的幻觉模式。


<details>
  <summary>Details</summary>
Motivation: 金融领域对LLMs的准确性和可靠性要求极高，但现有幻觉评测基准难以捕捉金融数据的独特需求（如上下文依赖、数值精确性），因此需要专门的方法来评估和改进金融LLMs。

Method: 开发了一个基于上下文感知掩码预测任务的框架，通过自动化掩码策略生成数据集，并利用S&P 500年报数据构建评测集，对主流LLMs进行内在幻觉分析。

Result: 提出了一个可扩展的评测框架和新的幻觉评估数据集，揭示了现有LLMs在金融表格数据上的内在幻觉模式。

Conclusion: 该研究为金融LLMs的评估提供了可靠方法，是构建更可信赖的金融生成AI系统的重要一步。
...
### [208] [Advanced Hybrid Transformer LSTM Technique with Attention and TS Mixer for Drilling Rate of Penetration Prediction](https://arxiv.org/abs/2508.05210)
*Saddam Hussain Khan*

Main category: cs.LG

TL;DR: 提出了一种混合深度学习架构，结合LSTM、Transformer、TS-Mixer和注意力机制，显著提升了钻井穿透率（ROP）的预测精度。
...
### [209] [DFW: A Novel Weighting Scheme for Covariate Balancing and Treatment Effect Estimation](https://arxiv.org/abs/2508.05215)
*Ahmad Saeed Khan,Erik Schaffernicht,Johannes Andreas Stork*

Main category: cs.LG

TL;DR: 论文提出了一种新的基于倾向得分的权重方法DFW，通过去混淆因子构建稳定有效的样本权重，解决了传统方法（如IPW）因倾向得分方差大导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 观测数据中的选择偏差导致处理组间协变量分布不平衡，传统倾向得分方法（如IPW）因倾向得分方差大而效果受限。

Method: 提出Deconfounding Factor Weighting (DFW)，利用去混淆因子构建稳定权重，优先选择低混淆样本，减少高混淆样本的影响。

Result: DFW在真实和合成数据集上优于IPW和CBPS，实现了更好的协变量平衡和因果效应估计。

Conclusion: DFW通过去混淆因子解决了传统倾向得分方法的局限性，适用于二元和多处理场景，提升了因果效应估计的稳定性和准确性。
...
### [210] [ML-based Short Physical Performance Battery future score prediction based on questionnaire data](https://arxiv.org/abs/2508.05222)
*Marcin Kolakowski,Seif Ben Bader*

Main category: cs.LG

TL;DR: 该研究探讨了基于问卷数据预测老年人四年后SPPB评分的可能性，XGBoost表现最佳（MAE为0.79），并通过特征选择优化模型（MAE为0.82）。


<details>
  <summary>Details</summary>
Motivation: 早期干预可减缓老年人身体机能衰退，研究旨在通过问卷数据预测未来SPPB评分。

Method: 使用多种ML算法（如XGBoost、随机森林等）预测SPPB评分，并通过Shapley值分析进行特征选择。

Result: XGBoost表现最优（MAE为0.79），特征选择后模型性能略有下降（MAE为0.82）。

Conclusion: 问卷数据可用于预测老年人未来身体机能，XGBoost是有效工具，特征选择可简化模型。
...
### [211] [Don't Reach for the Stars: Rethinking Topology for Resilient Federated Learning](https://arxiv.org/abs/2508.05224)
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习的集中式架构存在单点故障、个性化不足和鲁棒性差等问题，且更新选择不可靠。

Method: 采用P2P拓扑结构，利用本地验证集计算一致性分数，选择个性化更新集合并引入正则化。
...
### [212] [Cross-LoRA: A Data-Free LoRA Transfer Framework across Heterogeneous LLMs](https://arxiv.org/abs/2508.05232)
...
Method: 提出Cross-LoRA框架，包含LoRA-Align（通过SVD和线性变换对齐子空间）和LoRA-Shift（将权重更新投影到目标模型空间）。

Result: 在ARCs、OBOA和HellaSwag等任务上，Cross-LoRA相对基础模型提升达5.26%，性能接近直接训练的LoRA适配器。

Conclusion: Cross-LoRA是一种高效、轻量级的跨模型迁移方法，无需额外数据或训练，适用性广泛。
...
### [213] [MoBE: Mixture-of-Basis-Experts for Compressing MoE-based LLMs](https://arxiv.org/abs/2508.05257)
*Xiaodong Chen,Mingming Ha,Zhenzhong Lan,Jing Zhang,Jianguo Li*

Main category: cs.LG

TL;DR: 本文提出了一种新的Mixture-of-Basis-Experts (MoBE)方法，用于压缩大型MoE模型，显著减少了参数数量且仅带来微小的准确率下降。


<details>
  <summary>Details</summary>
Motivation: 尽管MoE架构在扩展大型语言模型方面表现出色，但其部署时的高内存需求成为主要挑战。现有压缩方法通常导致较大的准确率下降。

Method: MoBE通过将专家矩阵分解为独特矩阵A和共享基础矩阵B的线性组合，并通过最小化重构误差学习分解。

Result: 实验表明，MoBE在压缩参数24%-30%的同时，仅带来1%-2%的准确率下降，优于现有方法。

Conclusion: MoBE是一种高效且低损失的MoE模型压缩方法，为大规模模型部署提供了可行解决方案。
...
### [214] [Marine Chlorophyll Prediction and Driver Analysis based on LSTM-RF Hybrid Models](https://arxiv.org/abs/2508.05260)
...
TL;DR: 提出了一种结合LSTM和RF的混合模型LSTM-RF，用于预测海洋叶绿素浓度，解决了单一模型在时间序列建模和非线性特征刻画上的不足。
...
Method: 采用LSTM-RF混合模型，结合多源海洋数据（温度、盐度、溶解氧等）进行训练，并引入标准化处理和滑动窗口方法。
...
### [215] [FlowState: Sampling Rate Invariant Time Series Forecasting](https://arxiv.org/abs/2508.05287)
*Lars Graf,Thomas Ortner,Stanisław Woźniak,Angeliki Pantazi*

Main category: cs.LG

TL;DR: FlowState是一种新型时间序列基础模型（TSFM），通过状态空间模型编码器和函数基解码器解决现有TSFM在泛化性、采样率适应性和计算效率上的不足，实现了跨时间分辨率的动态调整和小模型高效表现。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型（TSFM）在泛化性、采样率适应性和计算效率方面表现不佳，限制了其在实际应用中的潜力。

Method: FlowState采用状态空间模型（SSM）编码器和函数基解码器，支持连续时间建模和动态时间尺度调整，并通过高效预训练策略提升鲁棒性和训练速度。

Result: FlowState在GIFT-ZS和Chronos-ZS基准测试中表现最优，且模型规模最小，同时具备在线适应不同输入采样率的独特能力。

Conclusion: FlowState通过创新的架构设计解决了现有TSFM的关键问题，为时间序列预测领域提供了更高效、适应性更强的解决方案。
...
### [216] [RLHF Fine-Tuning of LLMs for Alignment with Implicit User Feedback in Conversational Recommenders](https://arxiv.org/abs/2508.05289)
...
<details>
  <summary>Details</summary>
Motivation: 传统监督微调无法捕捉用户隐式反馈信号（如停留时间、情感极性等），需要更高效的方法来优化基于大语言模型（LLM）的CRS。

Method: 使用RLHF方法，通过奖励模型$R_{\phi}$学习弱标记的参与信息，并采用近端策略优化（PPO）优化LLM $M_{\theta}$，建模对话状态转移$s_t \\to a_t \\to s_{t+1}$。

Result: 在合成和真实数据集（如REDIAL、OpenDialKG）上的评估表明，RLHF微调模型在top-$k$推荐准确性、连贯性和用户满意度上表现更优。
...
### [217] [Optimal Growth Schedules for Batch Size and Learning Rate in SGD that Reduce SFO Complexity](https://arxiv.org/abs/2508.05297)
*Hikaru Umeda,Hideaki Iiduka*

Main category: cs.LG

TL;DR: 论文研究了深度学习中批量大小和学习率的调度问题，提出了基于随机一阶预言机（SFO）复杂度的最优增长调度策略，以提高训练效率和收敛性。
...
Method: 基于随机一阶预言机（SFO）复杂度，理论推导了批量大小和学习率的最优增长调度策略，并通过实验验证。

Result: 提出的调度策略显著降低了SFO复杂度，为大规模批量训练提供了理论和实践指导。

Conclusion: 研究为深度学习中高效且可扩展的大批量训练提供了理论支持和实用建议。
...
### [218] [Adaptive Batch Size and Learning Rate Scheduler for Stochastic Gradient Descent Based on Minimization of Stochastic First-order Oracle Complexity](https://arxiv.org/abs/2508.05302)
*Hikaru Umeda,Hideaki Iiduka*
...
Conclusion: 通过动态调整批量大小和学习率，可以显著提高SGD的收敛效率。
...
### [219] [ASkDAgger: Active Skill-level Data Aggregation for Interactive Imitation Learning](https://arxiv.org/abs/2508.05310)
*Jelle Luijkx,Zlatan Ajanović,Laura Ferranti,Jens Kober*

Main category: cs.LG

TL;DR: ASkDAgger框架通过利用新手计划中的信息，减少教师反馈需求，提高模仿学习的效率。
...
Method: 提出ASkDAgger框架，包含SAG、FIER和PIER三个组件，调整反馈机制并优化数据利用。

Result: 在模拟和真实环境中验证了ASkDAgger的有效性，减少了标注需求并提高了适应性。

Conclusion: ASkDAgger通过优化反馈和数据利用，显著提升了交互式模仿学习的效率和适应性。
...
### [220] [Divide-and-Conquer for Enhancing Unlabeled Learning, Stability, and Plasticity in Semi-supervised Continual Learning](https://arxiv.org/abs/2508.05316)
...
### [221] [Optimal Corpus Aware Training for Neural Machine Translation](https://arxiv.org/abs/2508.05364)
*Yi-Hsiu Liao,Cheng Shen,Brenda,Yang*

Main category: cs.LG

TL;DR: OCAT是一种轻量级、抗过拟合且能提升模型准确性的方法，通过微调CAT预训练模型中的少量参数，显著提升了翻译任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统CAT方法在训练前需预定义高质量数据组，容易出错且效率低，因此提出了OCAT以优化这一过程。

Method: OCAT通过冻结CAT预训练模型的大部分参数，仅微调少量与语料相关的参数，实现轻量化和高效性。
...
### [222] [Latent Preference Bandits](https://arxiv.org/abs/2508.05367)
*Newton Mwai,Emil Carlsson,Fredrik D. Johansson*

Main category: cs.LG

TL;DR: 论文提出了一种放宽潜在老虎机假设的方法，仅需动作在潜在状态中的偏好排序模型，而非完整的奖励分布，从而适应奖励尺度不同的情况。


<details>
  <summary>Details</summary>
Motivation: 个性化任务中，单个个体决策点少，传统潜在老虎机需要完整奖励分布模型，难以满足实际需求。

Method: 提出仅需动作偏好排序模型的潜在老虎机算法，并给出后验采样算法。

Result: 算法在奖励分布正确时与传统潜在老虎机性能相当，在奖励尺度不同时表现更优。

Conclusion: 放宽假设的潜在老虎机算法更具实用性，适用于奖励尺度变化的情况。
...
### [223] [Echo: Decoupling Inference and Training for Large-Scale RL Alignment on Heterogeneous Swarms](https://arxiv.org/abs/2508.05387)
*Jie Xiao,Shaoduo Gan,Changyuan Fan,Qingnan Ren,Alfred Long,Yuchen Zhang,Rymon Yu,Eric Yang,Lynn Ai*

Main category: cs.LG

TL;DR: Echo是一个RL系统，通过将轨迹采样和策略优化解耦到异构的“推理”和“训练”集群中，解决了现代RL后训练中的上下文切换问题，同时保持了统计效率。
...
Method: Echo通过两种轻量级同步协议（顺序拉取模式和异步推拉模式）将采样和训练解耦到异构集群中。

Result: 在分布式集群上训练多个RL工作负载时，Echo在收敛速度和最终奖励上与完全共置的基线相当，同时将轨迹生成卸载到边缘硬件。
...
### [224] [NT-ML: Backdoor Defense via Non-target Label Training and Mutual Learning](https://arxiv.org/abs/2508.05404)
...
Method: 采用非目标标签训练（NT）和互学习（ML），通过教师模型和学生模型的协作学习净化模型。

Result: NT-ML能有效抵御6种后门攻击，仅需少量干净样本，优于5种现有防御方法。
...
### [225] [Cumulative Learning Rate Adaptation: Revisiting Path-Based Schedules for SGD and Adam](https://arxiv.org/abs/2508.05408)
*Asma Atamna,Tom Maus,Fabian Kievelitz,Tobias Glasmachers*

Main category: cs.LG

TL;DR: 本文研究了深度学习中的自适应学习率机制，提出了一种修正的Adam优化器变体，并评估了其实际效果。


<details>
  <summary>Details</summary>
Motivation: 学习率是深度学习中关键的超参数，其理想值可能随训练过程变化。本文旨在探讨动态调整学习率的实际效用。

Method: 重新审视了一种基于累积路径的自适应学习率机制，并提出了一种修正的Adam变体以解决其概念不一致问题。

Result: 通过对比实验，评估了SGD和Adam（带或不带累积自适应）以及另一种方法的性能。

Conclusion: 研究结果旨在阐明自适应学习率策略的实际优势及其适用场景。
...
### [226] [MolSnap: Snap-Fast Molecular Generation with Latent Variational Mean Flow](https://arxiv.org/abs/2508.05411)
*Md Atik Ahamed,Qiang Ye,Qiang Cheng*

Main category: cs.LG

TL;DR: 提出了一种新颖的因果感知框架，结合Causality-Aware Transformer和Variational Mean Flow，解决了分子生成中的质量、多样性和效率问题。
...
Method: 1. 使用Causality-Aware Transformer（CAT）联合编码分子图标记和文本指令，并在生成过程中强制因果依赖；2. 开发Variational Mean Flow（VMF）框架，通过将潜在空间建模为高斯混合模型，增强表达能力。

Result: 在四个标准分子基准测试中，模型表现优于现有方法，实现了更高的新颖性（74.5%）、多样性（70.3%）和100%的有效性。VMF在条件生成中仅需一次函数评估（NFE），无条件生成最多五次NFE，计算效率显著优于基于扩散的方法。

Conclusion: 提出的因果感知框架在分子生成任务中显著提升了生成质量、多样性和计算效率。
...
### [227] [Echo State Networks for Bitcoin Time Series Prediction](https://arxiv.org/abs/2508.05416)
*Mansi Sharma,Enrico Sartor,Marc Cavazza,Helmut Prendinger*

Main category: cs.LG

TL;DR: 本文探讨了使用回声状态网络（ESN）预测加密货币价格，特别是在极端波动时期的表现，并通过李雅普诺夫指数进行混沌分析，证明其优于现有机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 由于股票和加密货币价格的高波动性和非平稳性，传统预测方法效果有限。ESN在短期股票市场预测中表现优异，但尚未广泛应用于加密货币领域。

Method: 采用回声状态网络（ESN）进行预测，并结合李雅普诺夫指数分析混沌时期的动态特性。

Result: ESN在极端波动时期表现优于Boosting和Naïve方法，且与李雅普诺夫指数分析结果一致。

Conclusion: ESN在加密货币价格预测中具有鲁棒性，尤其在混沌时期表现优异，为相关研究提供了新思路。
...
### [228] [Negative Binomial Variational Autoencoders for Overdispersed Latent Modeling](https://arxiv.org/abs/2508.05423)
*Yixuan Zhang,Wenxin Zhang,Hua Jiang,Quyu Kong,Feng Zhou*

Main category: cs.LG

TL;DR: NegBio-VAE扩展了VAE框架，使用负二项分布建模神经元放电计数，解决了Poisson-VAE中均值与方差相等的刚性约束问题，显著提升了重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统VAE无法准确模拟神经元放电的变异性，而Poisson-VAE虽引入泊松分布，但其均值与方差相等的约束不符合神经活动的真实随机性。

Method: 提出NegBio-VAE，采用负二项分布建模放电计数，开发了两种ELBO优化方案和两种可微分重参数化策略。

Result: 通过引入一个额外的离散参数，NegBio-VAE在重建精度上取得显著提升。

Conclusion: 显式建模过度离散对模拟神经元放电活动至关重要，NegBio-VAE为此提供了更准确的框架。
...
### [229] [Federated Multi-Objective Learning with Controlled Pareto Frontiers](https://arxiv.org/abs/2508.05424)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中FedAvg忽视少数客户端公平性，现有方法如FMOL仅实现任务级帕累托最优，未解决客户端公平问题。

Method: 提出CR-FMOL框架，结合FMGDA/FSMGDA本地优化和服务器端的锥约束帕累托子问题求解，确保客户端级帕累托最优。

Result: 在非独立同分布数据上，CR-FMOL提升客户端公平性，初期性能略逊FedAvg，但长期可达到相近精度。

Conclusion: CR-FMOL首次实现客户端级帕累托最优，为联邦学习中的公平性问题提供了有效解决方案。
...
### [230] [Group Causal Policy Optimization for Post-Training Large Language Models](https://arxiv.org/abs/2508.05428)
*Ziyin Gu,Jingyao Wang,Ran Zuo,Chuxiong Sun,Zeen Song,Changwen Zheng,Wenwen Qiang*

Main category: cs.LG

TL;DR: 论文提出了一种新的方法GCPO，通过引入因果结构改进GRPO，解决了候选响应间语义交互被忽视的问题，并在实验中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管GRPO在特定领域表现高效，但其将候选响应视为独立，忽略了语义交互（如互补性和矛盾性），因此需要改进。

Method: 提出了一种结构因果模型（SCM）揭示候选响应间的隐藏依赖关系，并基于此设计了GCPO方法，包含因果奖励调整和KL正则化项。

Result: GCPO在多个推理基准测试中表现优于GRPO等现有方法。

Conclusion: GCPO通过整合因果结构，显著提升了模型性能，为特定领域任务提供了更优的解决方案。
...
### [231] [Discovering Interpretable Programmatic Policies via Multimodal LLM-assisted Evolutionary Search](https://arxiv.org/abs/2508.05433)
*Qinglong Hu,Xialiang Tong,Mingxuan Yuan,Fei Liu,Zhichao Lu,Qingfu Zhang*

Main category: cs.LG

TL;DR: MLES结合多模态大语言模型与进化搜索，提出了一种可解释的高性能控制策略发现方法，性能媲美PPO，同时提供透明逻辑。
...
Conclusion: MLES是一种可扩展、可解释的策略发现方法，有望成为下一代控制策略设计的领先方案。
...
### [232] [Competing Risks: Impact on Risk Estimation and Algorithmic Fairness](https://arxiv.org/abs/2508.05435)
*Vincent Jeanselme,Brian Tom,Jessica Barrett*

Main category: cs.LG

TL;DR: 论文指出，将竞争风险视为截尾会显著高估生存风险并加剧不公平性，提出了量化误差的框架，并强调在生存模型中考虑竞争风险的重要性。


<details>
  <summary>Details</summary>
Motivation: 准确预测事件发生时间对决策至关重要，但现有方法常将竞争风险错误地视为截尾，导致生存估计偏差和公平性问题。

Method: 通过理论分析量化将竞争风险视为截尾的误差，并开发框架评估其对预测性能和算法公平性的影响。

Result: 研究表明，忽视竞争风险会系统性高估风险，并加剧不同人口群体间的差异，尤其是在心血管管理中的实证分析中表现明显。

Conclusion: 生存模型必须考虑竞争风险以提高准确性、减少评估偏差，并更好地支持下游决策。
...
### [233] [Tail-Risk-Safe Monte Carlo Tree Search under PAC-Level Guarantees](https://arxiv.org/abs/2508.05441)
...
Method: 1. CVaR-MCTS：嵌入条件风险价值（CVaR）以控制最坏情况下的损失。2. W-MCTS：引入Wasserstein模糊集以减少尾部风险估计偏差。

Result: 实验表明，两种方法在模拟环境中优于基线，实现了稳健的尾部风险控制和更高的奖励。
...
### [234] [EnergyPatchTST: Multi-scale Time Series Transformers with Uncertainty Estimation for Energy Forecasting](https://arxiv.org/abs/2508.05454)
...
Method: 提出EnergyPatchTST，包含多尺度特征提取、蒙特卡洛概率预测、未来已知变量集成路径和预训练微调策略。

Result: 在多个能源数据集上，预测误差降低7-12%，并提供可靠的不确定性估计。
...
### [235] [Task complexity shapes internal representations and robustness in neural networks](https://arxiv.org/abs/2508.05463)
*Robert Jankowski,Filippo Radicchi,M. Ángeles Serrano,Marián Boguñá,Santo Fortunato*

Main category: cs.LG

TL;DR: 论文通过五种数据无关的探针方法，研究了任务难度对多层感知机（MLPs）内部表示拓扑和鲁棒性的影响，揭示了符号二分图拓扑在表示学习中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 神经网络虽然表现出色，但其内部表示的形成机制仍不透明。本文旨在量化任务难度如何影响MLPs的表示拓扑和鲁棒性。

Method: 采用五种数据无关的探针（剪枝、二值化、噪声注入、符号翻转和二分图随机化），在MNIST和Fashion-MNIST数据集上对比简单和困难分类任务。

Result: 困难任务模型在二值化后准确率骤降至随机水平，而简单任务模型仍保持鲁棒性。此外，噪声注入和符号翻转能提升性能，保留符号结构即可维持高准确率。

Conclusion: 符号二分图拓扑在表示学习中至关重要，研究结果为模型压缩和可解释性提供了实用策略，并与任务复杂度对齐。
...
### [236] [Let's Measure Information Step-by-Step: LLM-Based Evaluation Beyond Vibes](https://arxiv.org/abs/2508.05469)
*Zachary Robertson,Sanmi Koyejo*

Main category: cs.LG

TL;DR: 论文提出了一种无需真实标签评估AI系统的方法，利用游戏抵抗性与输出质量的关系，证明了f-互信息是唯一满足自然条件的游戏抵抗机制，并在多个领域验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统评估常依赖真实标签，但真实标签难以获取或不可靠，因此需要一种无需真实标签的评估机制。

Method: 利用数据处理的不可逆性，提出f-互信息作为游戏抵抗机制，并通过理论和实验验证其有效性。

Result: 在翻译、同行评审等十个领域中，信息论机制能完美区分忠实与策略性代理，且对抗操纵的鲁棒性比现有方法高10-100倍。

Conclusion: f-互信息是一种高效的游戏抵抗评估机制，尤其在信息多样性最优的压缩比（10:1）下表现最佳。
...
### [237] [Prediction of Survival Outcomes under Clinical Presence Shift: A Joint Neural Network Architecture](https://arxiv.org/abs/2508.05472)
*Vincent Jeanselme,Glen Martin,Matthew Sperrin,Niels Peek,Brian Tom,Jessica Barrett*

Main category: cs.LG

TL;DR: 论文提出了一种多任务循环神经网络模型，联合建模电子健康记录中的观察间隔和缺失过程，以提高临床预测模型的性能和可移植性。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的临床存在（clinical presence）通常被忽视，但会显著影响预测模型的性能和可移植性。

Method: 采用多任务循环神经网络，同时建模观察间隔、缺失过程和生存结局。

Result: 在MIMIC-III数据集上的死亡率预测任务中，模型性能优于现有方法，且更具可移植性。

Conclusion: 利用临床存在信息可显著提升预测模型的性能和可移植性。
...
### [238] [MoMA: A Mixture-of-Multimodal-Agents Architecture for Enhancing Clinical Prediction Modelling](https://arxiv.org/abs/2508.05492)
...
Method: MoMA通过专用LLM代理将非文本模态转换为结构化文本摘要，再通过聚合代理生成统一多模态摘要，最后由预测代理生成预测。

Result: 在三种预测任务中，MoMA表现优于现有方法，展示了其高准确性和灵活性。
...
### [239] [Parameter-free entropy-regularized multi-view clustering with hierarchical feature selection](https://arxiv.org/abs/2508.05504)
...
<details>
  <summary>Details</summary>
Motivation: 多视图聚类面临高维特征和异构数据整合的挑战，传统方法依赖手动调参且缺乏跨视图整合机制。

Method: 引入熵正则化替代模糊化参数，采用信噪比正则化进行特征加权，并利用双级熵项平衡视图和特征贡献。AAMVFCM-U进一步通过自适应阈值实现分层降维。

Result: 在五个基准测试中优于15种先进方法，AAMVFCM-U计算效率提升97%，维度降至原始0.45%，并能自动识别关键视图组合。

Conclusion: 提出的无参数框架在多视图聚类中表现出色，解决了传统方法的局限性，为模式发现提供了高效且自适应的解决方案。
...
### [240] [Tractable Sharpness-Aware Learning of Probabilistic Circuits](https://arxiv.org/abs/2508.05537)
...
<details>
  <summary>Details</summary>
Motivation: 随着PCs的表达能力增强，过拟合问题在数据有限时变得显著，论文从对数似然景观角度分析了这一问题，发现尖锐的优化点导致泛化能力差。

Method: 提出了一种Hessian-based正则化器，通过高效计算对数似然的Hessian迹（尖锐度代理），并结合梯度范数正则化，适用于EM和梯度学习方法。
...
### [241] [Adapting Vision-Language Models Without Labels: A Comprehensive Survey](https://arxiv.org/abs/2508.05547)
...
Result: 系统分析了每种范式的核心方法，并总结了代表性基准和未来研究方向。
...
TL;DR: LLMs能够生成可解释的可计算表型（CPs），并通过迭代学习提高准确性，接近最先进ML方法的性能，同时需要更少的训练样本。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在生成可计算表型（CPs）方面的潜力，以支持高血压患者的临床决策。

Method: 提出并测试了一种“合成、执行、调试、指导”策略，利用LLMs生成并通过数据驱动反馈迭代优化CPs。

Result: LLMs结合迭代学习可以生成可解释且准确性合理的程序，性能接近最先进ML方法，且训练样本需求显著减少。

Conclusion: LLMs在生成可计算表型方面具有潜力，能够为临床决策提供可扩展的支持。
...
### [243] [X-VFL: A New Vertical Federated Learning Framework with Cross Completion and Decision Subspace Alignment](https://arxiv.org/abs/2508.05568)
*Qinghua Yao,Xiangrui Xu,Zhize Li*

Main category: cs.LG

TL;DR: X-VFL是一种新的垂直联邦学习框架，解决了数据样本不对齐和缺失特征的问题，并支持本地独立推理。通过XCom和DS-Align模块，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统垂直联邦学习需要数据样本完全对齐且不支持本地独立推理，限制了其应用。

Method: 提出X-VFL框架，包含XCom模块完成缺失特征，DS-Align模块对齐决策子空间。

Result: 在CIFAR-10和MIMIC-III数据集上分别提升15%和43%的准确率。

Conclusion: X-VFL在部分缺失特征和本地推理场景中具有显著优势。
...
### [244] [Shuffle-R1: Efficient RL framework for Multimodal Large Language Models via Data-centric Dynamic Shuffle](https://arxiv.org/abs/2508.05612)
*Linghao Zhu,Yiran Guan,Dingkang Liang,Jianzhong Ju,Zhenbo Luo,Bin Qin,Jian Luan,Yuliang Liu,Xiang Bai*

Main category: cs.LG

TL;DR: 论文提出Shuffle-R1框架，通过动态重组轨迹采样和批次组成，解决强化学习中优势崩溃和轨迹沉默问题，提升多模态大语言模型的训练效率。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习（RL）训练中存在优势崩溃和轨迹沉默问题，导致梯度更新不理想，影响长期学习效率。

Method: 提出Shuffle-R1框架，包括成对轨迹采样（选择高对比度轨迹）和基于优势的轨迹重组（增加有价值轨迹的曝光）。

Result: 在多个推理基准测试中，Shuffle-R1表现优于现有RL基线方法，且开销极小。

Conclusion: 数据为中心的调整对提升多模态大语言模型的RL训练效率至关重要。
...
TL;DR: Fairy±i是一种新型的2位量化框架，通过提升全精度模型的性能并利用复数域的优势，首次突破了量化误差的上限。


<details>
  <summary>Details</summary>
Motivation: 现有QAT研究仅关注最小化全精度模型的量化误差，未尝试突破其性能上限。本文旨在打破这一限制。

Method: 利用复数域的表示优势，将权重映射到单位四根{±1, ±i}，实现对称且信息最优的2位表示，支持无乘法推理。
...
### [246] [TrajEvo: Trajectory Prediction Heuristics Design via LLM-driven Evolution](https://arxiv.org/abs/2508.05616)
*Zhikai Zhao,Chuanbo Hua,Federico Berto,Kanghoon Lee,Zihan Ma,Jiachen Li,Jinkyoo Park*

Main category: cs.LG

TL;DR: TrajEvo利用大型语言模型和进化算法自动设计轨迹预测启发式方法，显著优于传统启发式和深度学习方法，尤其在分布外场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统启发式方法准确性差且泛化能力不足，深度学习方法计算成本高且解释性差，TrajEvo旨在解决这些问题。

Method: 结合大型语言模型和进化算法，提出跨代精英采样和统计反馈循环两大创新。

Result: 在多个真实数据集上优于现有方法，尤其在分布外场景中表现突出。

Conclusion: TrajEvo为快速、可解释且泛化能力强的轨迹预测启发式方法设计提供了新方向。
...
### [247] [Enhancing PyKEEN with Multiple Negative Sampling Solutions for Knowledge Graph Embedding Models](https://arxiv.org/abs/2508.05587)
...
Result: 扩展不仅增强了PyKEEN功能，还通过实证研究展示了其对链接预测任务性能的积极影响。

Conclusion: 该扩展为知识图谱嵌入方法的设计和定制提供了更灵活的工具，并揭示了更有效策略的设计方向。
...
### [248] [Optimizing IoT Threat Detection with Kolmogorov-Arnold Networks (KANs)](https://arxiv.org/abs/2508.05591)
...
<details>
  <summary>Details</summary>
Motivation: 物联网的快速发展带来了严重的安全问题，亟需高效的入侵检测方法。

Method: 采用KANs模型，利用可学习的激活函数进行入侵检测。

Result: KANs在准确率上优于传统MLPs，与随机森林和XGBoost相当，且更具可解释性。
...
### [249] [Non-omniscient backdoor injection with a single poison sample: Proving the one-poison hypothesis for linear regression and linear classification](https://arxiv.org/abs/2508.05600)
*Thorsten Peinemann,Paula Arnold,Sebastian Berndt,Thomas Eisenbarth,Esfandiar Mohammadi*

Main category: cs.LG

TL;DR: 论文提出“一毒假设”，证明仅需一个毒样本即可成功注入后门，且不影响正常学习任务性能。


<details>
  <summary>Details</summary>
Motivation: 研究后门注入攻击所需的最小毒数据量，解决现有攻击方法需大量毒样本或过多背景信息的问题。

Method: 提出“一毒假设”，并通过理论证明其在线性回归和线性分类中的有效性。

Result: 实验验证表明，仅需一个毒样本即可实现零后门误差，且对正常任务影响有限。

Conclusion: 研究为后门攻击的最小毒数据需求提供了理论支持，并揭示了攻击的潜在威胁。
...
### [250] [On the Generalization of SFT: A Reinforcement Learning Perspective with Reward Rectification](https://arxiv.org/abs/2508.05629)
