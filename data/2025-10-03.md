...
Motivation: 现有基于图像LDM的零样本方法在视频修复中存在时间不一致问题，需要开发能够同时恢复空间细节和捕捉时间依赖性的视频修复方法。

Method: 利用视频一致性模型(VCMs)作为先验，构建LVTINO零样本逆求解器，通过条件机制绕过自动微分需求，仅需少量神经网络评估即可实现高质量视频重建。

Result: 在多种视频逆问题上，LVTINO相比基于图像LDM的逐帧方法在重建保真度和计算效率方面均取得显著提升，建立了新的性能基准。

Conclusion: LVTINO通过整合视频一致性模型的时间因果先验，成功解决了视频修复中的时间一致性问题，为高分辨率视频逆问题提供了高效且高质量的解决方案。
...
Method: 使用风格编码器和风格投影层从单个风格参考图像中提取细粒度风格表示，并将其注入到预训练生成模型中，不改变下游生成模型的结构框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有操作数据集未关注挣扎行为随时间的变化，而理解技能学习过程中挣扎的演变对于优化人类学习和开发有效辅助系统至关重要。

Method: 收集了76名参与者在18个任务（分为打结、折纸、七巧板、洗牌四类活动）上的视频数据，每个任务重复5次以捕捉技能演变。将挣扎检测定义为时间动作定位任务，使用TAL模型进行识别。

Result: 时间动作定位模型能够成功学习检测挣扎线索，在未见任务上平均mAP为34.56%，在未见活动上为19.24%，表明挣扎是跨技能任务的可迁移概念。

Conclusion: 挣扎检测在跨任务和跨活动中具有可行性，但仍需进一步改进检测性能。数据集已公开可用。
...
Result: SPUS在6个未见下游PDE任务上实现最先进泛化性能，同时需要显著更少的参数和极少的微调数据。

Conclusion: SPUS展示了残差U-Net作为PDE求解基础模型的潜力，为多样化PDE系统提供了高度参数效率的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到图像模型在现实主义方面表现出色，但在多人生成提示下表现不佳，会出现重复面孔、合并身份和错误计数个体的问题。

Method: DisCo通过组相对策略优化（GRPO）微调流匹配模型，使用组合奖励函数：(i)惩罚图像内面部相似性，(ii)阻止跨样本身份重复，(iii)确保准确的人物计数，(iv)通过人类偏好评分保持视觉保真度。采用单阶段课程来稳定训练复杂性扩展。

Result: 在DiverseHumans测试集上，DisCo实现了98.6%的独特面孔准确率和近乎完美的全局身份分布，超越了开源和专有方法（如Gemini、GPT-Image），同时保持了竞争力的感知质量。

Conclusion: DisCo建立了一个可扩展、无需额外注释的解决方案，解决了生成模型中长期存在的身份危机问题，并为组合多人生成设定了新的基准。
...
Method: 将地理定位建模为查询图像视觉表示与学习的地理表示对齐，使用分层地理嵌入表示世界，并融合外观特征和语义分割图形成鲁棒的视觉表示。

Result: 在5个基准数据集的25个指标中，22个指标达到了新的最佳性能，超越了现有最先进方法和大型视觉语言模型。

Conclusion: 通过地理表示和视觉表示的有效结合，显著提升了视觉地理定位的性能。
...
Method: 基于注意力矩阵相似性理论，提出XMAS方法：通过聚类跨模态注意力矩阵的顶部奇异值轨迹，从聚类中平衡采样来构建训练子集。

Result: XMAS能丢弃LLaVA-665k数据集的50%和Vision-Flan数据集的85%，完全保持LLaVA-1.5-7B在10个下游基准上的性能，训练速度提升1.2倍。

Conclusion: XMAS是首个针对LVLM指令调优的基于原则的数据选择方法，能有效消除大规模训练数据中的冗余，实现显著的数据缩减。
...
<details>
  <summary>Details</summary>
Motivation: 现有的向量量化图像生成方法要么使用连续方法但缺乏显式分类监督，要么使用离散方法但缺乏几何感知能力。Purrception旨在结合两者的优势，在保持连续传输动力学的同时提供显式分类监督。

Method: 通过将变分流匹配适应于向量量化潜在空间，学习码书索引的分类后验，同时在连续嵌入空间中计算速度场。这种方法结合了连续方法的几何感知和分类方法的离散监督。

Result: 在ImageNet-1k 256x256生成任务中，训练收敛速度比连续流匹配和离散流匹配基线更快，同时达到了与最先进模型竞争的FID分数。

Conclusion: 变分流匹配能够有效桥接连续传输和离散监督，提高图像生成的训练效率，为向量量化生成模型提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统对比增强CT检查需要碘造影剂，存在肾毒性、患者过敏和环境危害等风险。现有方法采用多阶段流程（先生成图像再分割），导致误差累积且无法利用共享的语义和解剖结构。

Method: 集成条件扩散模型与多任务学习，实现端到端的图像合成和解剖分割联合优化。无需初始预测，共享编码器和解码器参数，采用半监督训练策略处理临床数据中常见的缺失分割标签问题。

Result: 在264名患者队列中表现优于最先进的单任务和多阶段模型。图像合成PSNR达25.61 dB，主动脉腔Dice分数提升至0.89，血栓Dice分数提升至0.53，临床测量精度显著提高。

Conclusion: 该统一框架通过联合优化图像合成和分割任务，在减少造影剂使用的同时提高了诊断精度，为临床实践提供了更安全有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态内容分析通常计算成本高且工程量大，现有预训练模型难以与视频等复杂数据融合

Method: 设计候选流水线，结合预训练模型将视频转换为时间半结构化数据，再转化为帧级索引的知识图谱

Result: 创建了支持查询和持续学习的知识图谱表示，能够动态融入新的领域特定知识

Conclusion: 该框架为多模态内容分析提供了高效原型设计方法，支持交互式知识融合
...
<details>
  <summary>Details</summary>
Motivation: 当前Web代理方法依赖逐步UI交互和大量LLM推理，在动态布局和长任务中表现脆弱。相比之下，人类通过网站提供的高级功能（搜索、筛选、排序等）更高效地完成任务。
...
<details>
  <summary>Details</summary>
Motivation: 在线果蔬电商存在'信任赤字'，因为数字交易无法提供产品质量的直接感官感知，需要建立可信的在线交易机制。

Method: 采用'双源验证'构建信任金字塔模型，设计TriAlignXA可解释AI框架，包含生物自适应引擎、时效优化引擎和经济优化引擎，并通过'预映射机制'将过程数据编码为二维码。

Result: 在分级任务实验中，相比基线模型获得显著更高的准确率，实证和理论分析验证了框架在解决'不可能三角'方面的平衡能力。

Conclusion: 该研究为构建可信的在线农产品生态系统提供了从理论到实践的全面支持，建立了从算法决策到消费者信任的关键路径。
...
<details>
  <summary>Details</summary>
Motivation: 在半监督分割中，从未标记数据中捕获有意义的语义结构至关重要，特别是在密集分布的病理图像分析中。

Method: 利用随机dropout和时间训练快照获得多个扰动预测，通过整合空间重叠和全局结构对齐的匹配策略强制拓扑一致性。

Result: 实验表明该方法有效减少拓扑错误，产生更鲁棒和准确的分割结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有DPO方法主要依赖成对偏好，但人类对图像的反馈通常包含隐含的排序信息，能更精确地表达偏好。列表偏好优化在扩散模型中尚未得到充分研究。

Method: 提出Diffusion-LPO框架，将用户反馈聚合成图像排序列表，基于Plackett-Luce模型推导列表扩展的DPO目标，强制整个排序的一致性。

Result: 在文本到图像生成、图像编辑和个性化偏好对齐等任务中，Diffusion-LPO在视觉质量和偏好对齐方面一致优于成对DPO基线。

Conclusion: Diffusion-LPO是一个简单有效的列表偏好优化框架，能更好地利用人类反馈中的排序信息，提升扩散模型与人类偏好的对齐效果。
...
<details>
  <summary>Details</summary>
Motivation: 构建同时支持理解和生成的统一多模态大语言模型存在挑战：混合方法破坏了自回归范式，而纯自回归方法在语义对齐和像素级保真度之间存在权衡。

Method: 采用混合Transformer架构，通过语义到像素的离散表示整合紧凑语义token和细粒度像素token，在预训练视觉理解模型基础上增加生成能力。

Result: 在多样化多模态基准测试中，Bridge在理解和生成基准上均取得竞争性或更优结果，仅增加7.9%的序列长度即可实现强语言对齐和精确视觉细节描述。

Conclusion: Bridge证明了纯自回归方法可以在单一框架中有效统一多模态理解和生成，同时减少训练资源需求。
...
Result: 在训练数据分布相似的测试集上达到94%准确率，与传统CNN相当。在真实世界图像数据上，尽管存在差异，仍达到88%准确率，显著优于传统CNN的72.94%。模型对正确分类样本显示低不确定性，对错误分类样本显示高不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 由于无法直接访问源域数据，现有方法难以获得确定性的不变特征，且容易受到硬样本和域偏差的影响。

Method: 构建辅助域模块获取多样化表示，通过多个一致性策略获得不变特征表示，使用条件多核最大均值差异策略对齐硬样本与易样本。

Result: 在Office-31、Office-Home、VISDA-C和DomainNet-126等多个基准测试上取得了显著的性能提升。

Conclusion: CADTrans方法通过构建域一致性的不变特征表示，有效解决了无源域自适应中的硬样本和域偏差问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLLM应用通常提供全面的冗长描述，而不考虑上下文，导致BLV用户需要筛选无关信息，无法快速获得所需的具体信息
...
Conclusion: 利用BLV用户历史问题指导MLLM生成描述的方法能够提供更相关、更高效的视觉解释
...
Result: 创建了包含结构化思维标记和对应答案的多模态推理数据集，捕捉了视觉语言模型的逐步推理过程和最终描述性答案

Conclusion: 该数据集将公开提供，以支持推理/思维多模态视觉语言模型的研究
...
<details>
  <summary>Details</summary>
Motivation: 传统的正则化方法通常忽略感知矩阵零空间的特定结构，而零空间包含了对感知过程不可见的信号分量信息。NPN旨在利用零空间的结构信息来改进重建。

Method: NPN方法通过神经网络学习感知矩阵零空间的低维投影，将解约束在这个投影空间中。该方法与现有重建框架兼容，可作为传统图像域先验的补充。

Result: 在压缩感知、去模糊、超分辨率、CT和MRI等多种成像逆问题中，NPN先验能持续提升重建保真度，适用于plug-and-play方法、展开网络、深度图像先验和扩散模型。

Conclusion: NPN提供了一种可解释且灵活的正则化方法，通过利用感知矩阵零空间的结构信息，有效提升了各种成像逆问题的重建性能。
...
<details>
  <summary>Details</summary>
Motivation: 建立可靠的基础，弥合可解释基因组建模与自动化决策之间的差距，为基因组医学中的机器人和临床自动化提供支持。

Method: 结合混沌游戏表示(CGR)与概念瓶颈模型(CBM)，通过GC含量、CpG密度等生物学概念进行预测，集成概念保真度监督、先验一致性对齐、KL分布匹配和不确定性校准。

Result: 在HIV亚型分类上达到最先进性能，概念预测保真度更高，成本效益比更优，减少不必要的重复测试并提高效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型缺乏显式的逐步推理过程，忽略了可操作性约束和几何关系，且后训练流程很少强化推理质量，主要依赖弱奖励设计的监督微调。

Method: 提出VLA-R1模型，整合了基于验证奖励的强化学习(RLVR)和组相对策略优化(GRPO)，设计了针对区域对齐、轨迹一致性和输出格式的验证奖励机制，并构建了VLA-CoT-13K高质量数据集。

Result: 在领域内、领域外、仿真和真实机器人平台上的广泛评估表明，VLA-R1相比之前的VLA方法实现了更优越的泛化能力和真实世界性能。

Conclusion: VLA-R1通过系统化的推理增强方法显著提升了VLA模型的性能，为具身AI提供了更强大的感知-语言-动作统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 微距镜头具有高分辨率和大放大倍率的优势，但散焦模糊问题严重阻碍了清晰成像和高质量3D重建。传统方法需要大量图像和标注，且目前没有针对微距摄影的多视角3D重建方法。

Method: 从多视角模糊图像出发，联合优化物体的清晰3D模型和每个像素的散焦模糊核。采用可微分渲染方法自监督优化3D模型和散焦模糊核。
...
Method: 将运动去模糊重新定义为扩散过程，每个时间步对应逐渐模糊的图像；训练一致性模型将所有时间步对齐到同一清晰图像；集成Kernel ControlNet进行模糊核估计，并引入自适应时间步预测。

Result: 在全参考指标上取得优异性能，超越了之前的扩散基方法，与其他最先进模型性能相当。

Conclusion: FideDiff为预训练扩散模型在高保真图像恢复任务中的应用提供了新方向，为在真实工业应用中推进扩散模型建立了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 青铜器铭文识别面临严重视觉退化、多领域变异性（照片、拓片、摹本）和极端长尾字符分布的挑战，需要专门的方法来解决这些问题。

Method: 构建了包含22454张全页图像和198598个标注字符的大规模数据集，开发了两阶段检测-识别流程，使用LadderMoE增强预训练CLIP编码器，实现动态专家专业化和更强的鲁棒性。

Result: 在单字符和全页识别任务上显著优于现有场景文本识别基线方法，在头部、中部和尾部类别以及所有采集模态上都取得了优越的准确率。

Conclusion: 为青铜器铭文识别和下游考古分析建立了坚实的基础，提出的方法能够有效处理多领域异质性和罕见类别问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的UDA方法为每个新的源-目标对微调骨干网络参数，导致训练参数和存储内存线性增长，且无法重用训练好的骨干网络参数。

Method: 在骨干网络前添加领域特定的视觉重编程层，生成视觉提示作为纹理偏置，通过优化域内和域间分布差异来训练这些层，而不修改骨干网络参数。

Result: 在Office-31数据集上达到92.8%的平均准确率，仅需1.5M可训练参数，优于现有参数高效的UDA基线方法。

Conclusion: VirDA方法实现了高效的领域自适应，显著减少了参数需求，同时保持了竞争性的性能，支持骨干网络在不同域间的重用。
...
Method: 使用3D形变模型提取身份不变的表情特征，然后通过残差向量量化变分自编码器(RVQ-VAE)将这些特征编码为共享码本中的离散标记序列。

Result: DFE比FACS和其他面部编码方法捕捉更精确的面部行为，在压力检测、人格预测和抑郁检测等心理任务中表现优于基于FACS的流程和强图像/视频表示学习模型。
...
Method: 使用基于图的框架优化2D选择的图像扭曲进行逐点重建，采用并行可分离迭代优化策略，并结合自监督学习的编码器-解码器网络生成带纹理的密集3D点云。

Result: 在合成和真实数据集上的实验表明，该方法在重建精度和鲁棒性方面优于现有方法。

Conclusion: Con-NRSfM方法消除了传统约束，能够准确计算局部共形尺度，为单目视觉可变形SLAM提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖密集观测来优化模型参数，难以处理图像不一致问题。本文旨在解决稀疏观测下的鲁棒3D重建挑战。

Method: 将鲁棒重建解耦为恢复和重建两个子任务：首先将不一致图像转换为初始视频，然后使用专门设计的视频扩散模型恢复为一致图像，最后从恢复后的图像重建3D场景。

Result: 在合成和真实数据集上的广泛实验表明，该方法具有强大的泛化能力和优越的鲁棒重建性能，并能控制重建3D场景的风格。

Conclusion: UniVerse框架通过解耦恢复和重建任务，利用扩散模型学习的大规模场景先验，有效解决了多视角图像不一致的鲁棒重建问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统模板匹配方法需要穷举角度和尺度，效率低下；而大多数基于深度学习的方法只估计相似度分数，没有显式建模几何姿态，无法满足实际部署需求。

Method: 使用模板感知动态卷积模块(TDCM)在推理时动态注入模板特征，采用深度可分离卷积和像素洗牌实现高效匹配，通过旋转-剪切增强策略实现无几何标注训练，并使用轻量级细化模块提高精度。

Result: 3.07M参数的模型在复合变换下达到高精度和14ms推理速度，在小模板和多目标场景中表现出强鲁棒性。

Conclusion: 该方法非常适合实时工业应用部署，在复杂背景下实现了高效的模板匹配和几何状态估计。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理需要精确理解细粒度视觉元素的任务时经常表现不佳，主要由于图像编码过程中的信息丢失或对关键区域关注不足。现有的像素级信息整合方法存在过度使用问题，导致效率低下和无关视觉细节干扰。

Method: 首先应用操作感知的监督微调建立文本推理和视觉操作的基础能力，然后设计新颖的rollout引导的强化学习框架，基于模型自身响应的反馈，使VLM能够根据查询难度动态决定何时调用像素操作。

Result: 在广泛的多模态推理基准测试中，模型实现了卓越性能，同时显著减少了不必要的视觉操作。在HR-Bench 4K上达到73.4%的准确率，工具使用率仅为20.1%，相比先前方法在提高准确率的同时减少了66.5%的工具使用。

Conclusion: 提出的自适应像素推理框架有效解决了VLM在细粒度视觉任务中的局限性，通过智能地动态调用像素级操作，实现了性能提升与效率优化的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在胸部X光片解读中性能良好，但在患者亚组间准确性不均，存在隐藏故障。现有错误检测方法难以处理分布内的细微错误

Method: 使用ASRS框架，应用±15°/±30°临床合理旋转，通过RAD-DINO编码器测量嵌入变化，根据敏感度分数将样本分层到稳定性四分位

Result: 高敏感度病例的召回率显著降低(-0.2到-0.3)，尽管AUROC和置信度很高。ASRS提供了无需标签的选择性预测和临床医生审查方法

Conclusion: ASRS框架能够识别易出错病例，为医学AI的公平性和安全性提供了有效的错误检测工具
...
<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法存在时序不一致、风格丰富度不足的问题，而训练专门的视频风格化模型需要配对视频数据且计算成本高。

Method: 将多个风格化参考整合到预训练的I2V模型中，使用高频补偿约束内容布局和运动，结合基于光流的运动线索来保留低显著性区域的风格纹理。

Result: FreeViS在风格化保真度和时序一致性方面表现优异，超越了现有基线方法，获得了强烈的人类偏好。
...
Method: 设计MedQ-Perception（感知任务）和MedQ-Reasoning（推理任务），包含2600个感知查询和708个推理评估，采用多维评判协议从四个互补维度评估模型输出。

Result: 评估14个最先进MLLM显示模型具有初步但不稳定的感知和推理能力，临床使用准确性不足，需要进行针对性优化。

Conclusion: MedQ-Bench将推动MLLM在医学图像质量评估领域的进一步探索，释放其未开发潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉模型在理解实例级几何关系时依赖昂贵的输入格式（类别标签、分割掩码）和推理成本（二次前向传播），限制了实际应用。

Method: 提出InstaFormer网络，通过对象查询与潜在掩码描述符之间的交互，在单次前向传播中预测场景中所有实例的完整遮挡和深度排序。

Result: 通过全面基准测试和消融实验验证了方法的有效性，代码和模型已开源。

Conclusion: InstaFormer能够高效地从RGB图像中预测实例级几何关系，克服了现有方法的输入和计算成本限制。
...
Method: 提出金字塔位置编码(PPE)来捕捉局部细节和全局上下文，并结合强化学习动态优化风格化过程，加速收敛。

Result: 在COCO和WikiArt数据集上训练后，内容损失降低62.6%至2.07，风格损失降低57.4%至0.86，推理时间1.39秒；使用RL后进一步改善至内容损失2.03，风格损失0.75，推理时间1.40秒。
...
<details>
  <summary>Details</summary>
Motivation: 现有分治方法在大规模无边界场景中存在负载不平衡问题，均匀或启发式分区无法反映实际计算需求，粗到细流水线效率低下，重新加载整个模型导致高开销。

Method: 引入深度感知分区方法将预处理时间从小时级降至分钟级；采用基于优化的策略平衡可见高斯分布；使用可见性裁剪和选择性致密化两种轻量级技术降低训练成本。

Result: 在大规模城市和户外数据集上，LoBE-GS相比最先进基线实现了2倍的端到端训练加速，同时保持重建质量，能够扩展到传统3DGS无法处理的场景。

Conclusion: LoBE-GS通过重新设计大规模3DGS流水线，有效解决了负载不平衡和效率瓶颈，为大规模场景重建提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 长视频生成面临两个主要挑战：需要捕捉长程依赖关系，同时防止自回归解码中固有的错误累积。

Method: 1. MemoryPack：可学习的上下文检索机制，利用文本和图像信息作为全局指导，联合建模短期和长期依赖关系；2. Direct Forcing：高效的单步近似策略，改善训练-推理对齐以减少推理时的错误传播。

Result: MemoryPack能够实现分钟级的时间一致性，计算效率高且保持线性复杂度；Direct Forcing有效减轻了错误累积问题。

Conclusion: MemoryPack和Direct Forcing共同显著提升了长视频生成的上下文一致性和可靠性，推进了自回归视频模型的实际可用性。
...
Conclusion: 完整预测向量校准损失项与等渗回归的组合能有效提升3D检测器的校准性能，但不同检测器架构可能需要不同的校准策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有行人搜索方法主要使用ImageNet预训练骨干网络，可能无法充分捕捉复杂空间上下文和细粒度身份线索。同时，共享骨干网络特征会导致检测和重识别任务之间的优化目标冲突。

Method: 提出DiffPS框架，利用预训练扩散模型，包含三个专门模块：扩散引导区域提议网络(DGRPN)用于增强行人定位，多尺度频率精炼网络(MSFRN)减轻形状偏差，语义自适应特征聚合网络(SFAN)利用文本对齐的扩散特征。

Result: 在CUHK-SYSU和PRW数据集上达到了新的最先进性能。

Conclusion: 扩散先验知识能有效提升行人搜索性能，通过专门设计的模块可以解决检测和重识别任务之间的优化冲突问题。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱成像(HSI)虽然提供丰富的空间-光谱信息，但由于硬件限制和从压缩测量重建三维数据的困难，获取成本高昂。压缩感知系统如CASSI提高了效率，但准确重建仍面临严重退化和精细光谱细节丢失的挑战。

Method: 提出FMU网络，将流匹配的生成先验嵌入深度展开框架，引入平均速度损失来增强流的全局一致性，结合基于优化方法的可解释性和流匹配的生成能力。

Result: 在模拟和真实数据集上的广泛实验表明，FMU在重建质量上显著优于现有方法。

Conclusion: FMU成功地将流匹配集成到HSI重建中，通过混合设计实现了更鲁棒和准确的重建，代码和模型将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 传统工业组件缺陷检测耗时耗力，给质检人员带来沉重负担，难以有效管理产品质量。

Method: 使用数字相机光学和深度学习模型，采用ConSinGAN生成训练数据集，比较四种YOLO模型（v3、v4、v7、v9）性能。

Result: YOLOv7结合ConSinGAN表现最佳，准确率达95.50%，检测时间285毫秒，远优于基于阈值的方法。
...
Method: 设计FoundAD方法，通过观察图像中异常数量与学习嵌入差异的相关性，学习一个非线性投影算子到自然图像流形上，作为检测图像中分布外区域的有效工具。

Result: 大量实验表明该方法支持多类别检测，在使用比先前方法少得多的参数情况下实现竞争性性能，并通过包括DINOv3在内的多个基础编码器评估验证。

Conclusion: 该方法拓宽了基础特征应用的视角，推动了少样本异常检测领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers在实际机器人系统中应用受限，因为其二次注意力复杂度太高。现有的token合并方法适合分类任务但不适合密集预测任务如语义分割。

Method: 在ViT骨干网络基础上，添加可训练的聚类模块根据分割掩码的伪聚类指导合并相似token，然后使用再生模块恢复细节信息供下游头使用。

Result: 在三个不同数据集上实现了高达2.18倍的GFLOPs减少和1.64倍的推理加速，同时保持可比的语义分割精度。
...
Method: 引入视觉参考标记(VRTs)，从查询图像的视觉补丁嵌入中派生，与LLM的输出文本标记交错。使用轻量级解码器将LLM输出转换为检测、分割和定位预测。

Result: 在四个视觉感知和理解任务中持续达到最先进性能，即使与显著更大的MLLM模型相比。

Conclusion: PaDT提供了一种统一的范式，使MLLM能够直接生成多样化的视觉输出，显著提升了定位和相似对象区分能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯溅射编辑方法在视图一致性、时间一致性和非编辑区域保持方面存在挑战，且难以处理复杂的文本指令。

Method: 使用4D感知InstructPix2Pix模型结合4D VGGT几何特征；采用多视图网格模块迭代优化；通过Gaussian选择机制保护非编辑区域；设计基于LLM的用户意图理解模块。

Result: 相比相关工作，该方法实现了更一致和可控的4D场景编辑，能够处理复杂的用户指令。

Conclusion: 4DGS-Craft框架有效解决了4D高斯溅射编辑中的一致性问题，并通过用户意图理解提升了编辑的交互性和性能。
...
TL;DR: Pure-Pass (PP) 是一种像素级掩码机制，通过识别纯像素并免除其昂贵计算，集成到 ATD-light 模型中，在保持计算效率的同时实现更好的超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级超分辨率方法如 CAMixer 存在适应性差、掩码粒度粗和空间灵活性不足等问题，需要更精细的计算分配机制。

Method: 提出 Pure-Pass (PP) 像素级掩码机制，利用固定颜色中心点对像素分类，识别纯像素并免除其昂贵计算，实现细粒度、空间灵活的计算分配。

Result: PP-ATD-light 在相似计算量下，重建质量和参数效率均优于 CAMixer-ATD-light，实现了更优的超分辨率性能。

Conclusion: Pure-Pass 机制通过像素级精细掩码，有效提升了轻量级超分辨率模型的性能和效率，为实际部署提供了更好的解决方案。
...
TL;DR: 本研究开发了基于GPT-4o的SLSO框架，通过自校正循环自动生成颌骨囊肿的放射学发现，相比传统CoT方法在多个评估项目上提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 利用多模态AI自动生成颌骨囊肿的放射学发现，提高诊断报告的准确性和效率。

Method: 构建SLSO框架，包含10步处理流程：图像输入分析、结构化数据生成、牙号提取与一致性检查、检测到不一致时迭代再生、发现生成与后续重构和一致性验证。

Result: SLSO框架在牙号、牙齿移动和牙根吸收三个项目上分别提高了66.9%、33.3%和28.6%的准确率，成功案例最多经过5次再生后获得一致结构化输出。

Conclusion: SLSO框架能够强制阴性发现描述、抑制幻觉、提高牙号识别准确性，但识别跨多牙的广泛病变仍有限制，需要进一步优化以实现实用的发现生成系统。
...
TL;DR: 提出LiLa-Net 3D自编码器架构，仅使用LiDAR点云编码真实交通环境特征，通过简化编码器层数和跳跃连接实现高效重建。


<details>
  <summary>Details</summary>
Motivation: 开发仅依赖LiDAR点云的高效3D特征编码方法，避免使用大量计算资源，同时保持重建质量。

Method: 采用简化的编码器层数和跳跃连接结构，平衡跳跃连接信息和潜在编码，构建高效潜在空间。

Result: 模型能够准确重建原始点云，并在与原始交通环境无关的对象上表现出良好的泛化能力。

Conclusion: LiLa-Net在保持性能的同时实现了高效的点云重建，证明了简化架构在3D特征编码中的有效性。
...
TL;DR: 开发了kabr-tools开源工具包，通过无人机视频和机器学习实现多物种行为自动监测，相比传统方法显著提高了行为数据的采集效率和精度。


<details>
  <summary>Details</summary>
Motivation: 传统野外行为观察方法范围有限、耗时耗力，难以评估跨景观的行为响应，需要可扩展的方法来量化复杂多维行为模式。

Method: 整合无人机视频与机器学习系统，利用目标检测、跟踪和行为分类技术，提取行为、社交和空间指标，包括时间预算、行为转换、社交互动等。

Result: 无人机观测比地面方法显著改善行为粒度，减少15%的可见性损失，捕获更多转换且精度和连续性更高。验证了969个行为序列，发现不同斑马物种的行为差异和空间隔离。

Conclusion: kabr-tools通过规模化自动行为监测，为生态系统研究提供了强大工具，推动保护、生物多样性研究和生态监测的进步。
...
TL;DR: GaussianMorphing是一个用于多视角图像语义感知3D形状和纹理变形的新框架，通过网格引导的3D高斯溅射实现高保真几何和外观建模。


<details>
  <summary>Details</summary>
Motivation: 克服现有方法依赖点云或需要预定义同胚映射的限制，实现无标签数据的语义对应和结构完整性保持。

Method: 使用统一变形策略将3D高斯锚定到重建的网格面片，通过拓扑感知约束确保几何一致性和纹理保真度，同时利用网格拓扑作为几何先验建立无监督语义对应。

Result: 在TexMorph基准测试中，GaussianMorphing显著优于现有2D/3D方法，颜色一致性误差(ΔE)降低22.2%，EI降低26.2%。

Conclusion: 该集成方法在整个变形过程中保持了局部细节和全局语义连贯性，无需标记数据。
...
TL;DR: 提出InPose方法，使用预训练扩散模型仅基于旋转测量进行姿态估计，通过似然项指导生成过程，实现零样本泛化
...
Result: 提出的InPose方法能够为任意用户生成性估计最可能解释稀疏身体测量数据的姿态序列

Conclusion: 该方法通过分离旋转和位置测量的使用，解决了跨用户泛化问题，实现了零样本泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积架构如U-Net在捕捉长距离依赖关系方面有限，限制了在复杂肿瘤结构上的性能。扩散模型在生成高质量医学图像和优化分割边界方面展现出潜力。
...
Conclusion: Transformer引导的扩散模型为脑肿瘤分割提供了改进的鲁棒性和可扩展性，超越了传统U-Net基线方法。
...
TL;DR: 分析SAM2等VOS模型在腹腔镜胆囊切除术视频中点追踪的失效模式，发现点追踪在手术工具上表现良好，但在解剖目标上因组织相似性和边界模糊而表现不佳。


<details>
  <summary>Details</summary>
Motivation: 点追踪在手术视频中提供了高效低成本的跟踪方案，但其在复杂手术环境中的可靠性和失效情况尚不清楚，需要系统分析。

Method: 在腹腔镜胆囊切除术视频中，针对胆囊、抓钳和L型电钩三个手术目标，比较点追踪与分割掩码初始化的性能表现。

Result: 点追踪在手术工具上表现有竞争力，但在解剖目标上持续表现不佳，主要由于组织相似性和边界模糊导致跟踪失败。
...
TL;DR: 提出FFREEDG新任务：在服务器预训练后仅使用客户端无标签数据进行联邦学习，不重新访问源数据。提出FRIEREN框架，利用视觉基础模型整合视觉和语言模态，通过CLIP文本嵌入和弱到强一致性学习解决该任务。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在语义分割任务中面临域偏移挑战，特别是当客户端数据无标签时。现有方法要么不切实际地假设客户端有标签数据，要么未能充分利用现代视觉基础模型的能力。

Method: FRIEREN框架：1）使用视觉-语言解码器，通过CLIP文本嵌入改善语义消歧；2）采用弱到强一致性学习策略，在伪标签上进行鲁棒的本地训练。

Result: 在合成到真实场景和清晰到恶劣天气基准测试中，该框架有效解决了新任务，性能与现有域泛化和适应方法相当，为未来研究建立了强基线。

Conclusion: 提出的FFREEDG任务具有挑战性，FRIEREN框架通过整合视觉和语言模态成功解决了该问题，在联邦语义分割领域取得了有竞争力的结果。
...
TL;DR: ASK-Hint是一个基于动作知识的提示框架，通过细粒度的结构化提示提升冻结视觉语言模型在视频异常检测中的性能，无需训练即可实现最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有提示方法过于抽象，忽略了视频异常检测中重要的人-物交互和动作语义细节，导致检测准确性不足。

Method: 提出结构化提示框架，将提示按语义分组（如暴力、财产犯罪、公共安全），并设计细粒度的引导问题，使模型预测与判别性视觉线索对齐。

Result: 在UCF-Crime和XD-Violence数据集上的实验显示，ASK-Hint显著提升了AUC指标，优于现有基线和微调方法，实现了最先进的性能。

Conclusion: 提示的细粒度对视频异常检测至关重要，ASK-Hint提供了一个无需训练、可泛化且可解释的解决方案。
...
TL;DR: GeoPurify通过学生亲和网络和几何引导池化，利用2D视觉语言模型的特征和3D几何先验，在少量训练数据下实现高效的3D语义分割。


<details>
  <summary>Details</summary>
Motivation: 解决2D视觉语言模型特征直接投影到3D时产生的噪声和碎片化预测问题，同时避免需要大规模标注3D数据的昂贵训练流程。

Method: 使用学生亲和网络从3D自监督教师模型中提取几何先验来净化2D VLM生成的3D点特征，并在推理时采用几何引导池化模块进一步去噪。

Result: 在主要3D基准测试中达到或超越最先进性能，仅使用约1.5%的训练数据。

Conclusion: GeoPurify通过利用潜在几何信息和学习的亲和网络，有效缓解了2D语义与3D几何结构之间的权衡，实现了卓越的数据效率。
...
TL;DR: 提出了一种基于猪耳静脉模式的非侵入式生物识别方法，使用智能手机采集图像，通过多阶段计算机视觉流程提取特征，SVM模型在混合品种猪群中达到98.12%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 现有猪只识别方法（如耳标、芯片）不可靠、成本高且主要针对纯种猪，不适合小规模养殖户，需要一种成本效益高且无压力的识别方案。
...
Conclusion: 该系统通过用永久性生物标记替代脆弱的物理标识符，为农民提供了一种经济高效且无压力的动物识别方法，证实了耳静脉生物识别技术在数字化畜牧管理中的实用性。
...
TL;DR: 提出一个多类别计数框架，使用Twins金字塔视觉transformer骨干网络和专门的多类别计数头，通过双任务设计和类别聚焦模块减少类别间干扰，在密集场景中优于现有方法。
...
Method: 采用Twins金字塔视觉transformer作为骨干网络，构建专门的多类别计数头，使用多尺度解码方法，并添加基于分割的类别聚焦模块来抑制训练时的类别间干扰。

Result: 在VisDrone和iSAID基准测试中表现优异，相比之前的多类别人群计数方法，MAE分别降低了33%、43%和64%，验证了在密集场景中人群计数方法的必要性。
...
TL;DR: TempoControl是一种无需重新训练或额外监督的方法，通过优化交叉注意力图来实现视频生成中的细粒度时间控制，让用户能够指定视觉元素在生成序列中的出现时机。


<details>
  <summary>Details</summary>
Motivation: 现有的生成视频模型虽然能够基于自然语言提示创建高质量视频，但缺乏细粒度的时间控制能力，无法让用户指定特定视觉元素在视频序列中的出现时间。

Method: TempoControl利用文本到视频扩散模型中的交叉注意力图，通过新颖的优化方法指导概念的时间安排。该方法通过三个互补原则引导注意力：通过相关性使其时间形状与控制信号对齐，通过能量在需要可见性的地方放大注意力，通过熵保持空间聚焦。

Result: TempoControl能够在确保高质量和多样化视频的同时，实现对时间的精确控制。该方法在各种视频生成应用中表现出有效性，包括单个和多个对象的时间重新排序，以及动作和音频对齐的生成。

Conclusion: TempoControl提供了一种有效的方法来解决生成视频模型中缺乏时间控制的问题，通过优化注意力机制实现了对视觉元素出现时间的精确控制，同时保持了视频质量和多样性。
...
Method: 构建ReasonMap-Plus数据集提供密集奖励信号，提出RewardMap框架包含难度感知奖励设计和多阶段RL方案，从简单感知任务逐步过渡到复杂推理任务。
...
TL;DR: DragFlow是首个利用FLUX强大先验进行拖拽式图像编辑的框架，通过区域级编辑范式、个性化适配器和多模态大语言模型，显著提升了编辑质量。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型从UNet-based DDPMs转向更具扩展性的DiT with flow matching（如SD3.5、FLUX），生成先验显著增强，但拖拽式编辑尚未从中受益，仍存在目标区域扭曲问题。

Method: 提出区域级编辑范式，使用仿射变换提供更丰富一致的特征监督；集成预训练开放域个性化适配器增强主体一致性；采用基于梯度掩码的硬约束保护背景保真度；利用多模态大语言模型解决任务歧义。

Result: 在DragBench-DR和ReD Bench基准测试中，DragFlow超越了基于点和区域的基线方法，在拖拽式图像编辑中达到了新的最先进水平。

Conclusion: DragFlow成功利用了FLUX的丰富先验，通过区域级编辑和多种技术集成，显著提升了拖拽式图像编辑的性能和质量。
...
TL;DR: 本文提出F2C方法，通过选择关键片段而非孤立关键帧来改善视频理解，同时采用自适应分辨率策略保持固定计算预算。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型面临'大海捞针'问题：原始视频帧产生的大量视觉标记耗尽模型上下文窗口。现有解决方案通过选择稀疏帧集来减少标记数量，但这种帧级选择丢弃了基本的时间动态，导致对运动和事件连续性的推理不理想。
...
Conclusion: 结果强调了在帧选择中保持时间一致性的重要性，并为将视频LLM扩展到实际视频理解应用提供了实用途径。
...
<details>
  <summary>Details</summary>
Motivation: 利用机器学习和可穿戴传感器的进步，在真实世界条件下准确评估人体运动，这对于远程医疗、运动科学和康复至关重要。
...
Conclusion: 研究揭示了视频和传感器方法在成本、可访问性和精度方面的权衡，为研究人员和临床医生开发稳健、经济高效且用户友好的远程医疗解决方案提供了指导方针。
...
TL;DR: NeuroSwift是一个从fMRI数据重建视觉信息的扩散模型，通过集成AutoKL和CLIP适配器来解决跨被试重建的挑战，仅需微调17%参数即可实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决从脑活动重建视觉信息时面临的跨被试变异性挑战和计算需求高的问题，因为不同被试的神经表征存在差异且大脑对复杂视觉输入的编码具有抽象性。

Method: 提出NeuroSwift模型，集成互补适配器：AutoKL处理低级特征，CLIP处理语义特征。CLIP适配器在Stable Diffusion生成图像和COCO字幕上训练以模拟高级视觉皮层编码。跨被试泛化时，在一个被试上预训练，然后仅微调新被试17%的参数（全连接层）。

Result: 在轻量级GPU（三块RTX 4090）上每个被试仅需1小时训练，就能超越现有方法，实现最先进的性能。

Conclusion: NeuroSwift通过集成互补适配器和高效的参数微调策略，成功解决了跨被试视觉重建的挑战，实现了高效且准确的性能。
...
TL;DR: microCLIP是一个自训练框架，通过联合优化CLIP的视觉和文本表示来提升细粒度图像分类性能。核心创新包括Saliency-Oriented Attention Pooling (SOAP)和TokenFusion模块，以及动态知识聚合方法。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在细粒度图像分类任务中表现受限，因为它主要依赖粗粒度的全局特征，而忽略了微观局部线索。现有方法通过对齐LLM描述与CLIP的[CLS]token来注入细粒度知识，但缺乏空间精度。

Method: 提出SOAP机制构建显著性引导的[FG]token，并与全局[CLS]token融合实现粗-细粒度对齐。采用双头LLM派生分类器：冻结分类器提供稳定的文本先验，可学习分类器从LLM描述初始化并与TokenFusion共同微调。引入动态知识聚合来迭代优化伪标签。

Result: 在13个细粒度基准测试中平均准确率提升2.90%，仅需轻量级适配。

Conclusion: microCLIP成功挖掘了CLIP中的潜在细粒度信号，通过联合优化视觉和文本表示显著提升了细粒度分类性能，同时保持了轻量适配的优势。
...
TL;DR: VidGuard-R1是首个通过多模态大语言模型和群体相对策略优化实现的高精度视频真实性检测器，能够提供准确分类和可解释的推理过程。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频技术的快速发展，迫切需要有效的检测工具来应对错误信息和声誉损害等社会风险，同时需要提供透明可解释的检测结果。

Method: 使用群体相对策略优化(GRPO)微调多模态大语言模型(Qwen-VL)，构建包含14万真实和AI生成视频的挑战性数据集，并设计专门针对时间伪影和生成复杂度的奖励模型。

Result: 在现有基准测试中实现最先进的零样本性能，经过额外训练后准确率超过95%，并能产生精确可解释的预测理由。

Conclusion: VidGuard-R1成功实现了高精度视频真实性检测，同时提供透明可解释的推理过程，为应对AI生成视频带来的社会风险提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成方面取得了革命性进展，但基于transformer架构的计算成本过高，特别是在生成长视频时。现有自回归方法在超出训练范围时会出现质量显著下降的问题。

Method: 利用教师模型的丰富知识，通过从自生成长视频中抽取片段为学生模型提供指导，保持时间一致性，避免过曝光和错误累积问题。

Result: 能够生成长达4分15秒的视频，相当于基础模型位置嵌入支持的最大跨度的99.9%，比基线模型长50倍以上，在保真度和一致性方面显著优于基线方法。

Conclusion: 该方法有效解决了长视频生成中的质量退化问题，实现了高质量的长视频生成，无需额外的长视频监督或重新训练。
...
TL;DR: KineMask是一种物理引导的视频生成方法，通过两阶段训练策略实现逼真的刚体控制和物体交互，在合成场景和真实场景中都显著提升了物体交互效果。
...
Method: 提出两阶段训练策略，通过物体掩码逐步移除未来运动监督，训练视频扩散模型；结合低级运动控制和高级文本条件，支持复杂动态现象合成。

Result: 在合成简单交互场景上训练后，在真实场景中显著改善了物体交互效果；相比同类规模模型有显著提升；消融研究验证了低级和高级条件的互补作用。

Conclusion: KineMask通过物理引导的视频生成方法有效解决了物体交互的物理合理性问题，为视频生成模型在物理仿真应用中的发展提供了重要进展。
...
TL;DR: 提出了一种结合本体感觉、动觉、力触觉和肌肉激活的多模态动作表示方法，用于实现精细的机器人控制，解决了现有视频模型缺乏精细控制能力的问题。


<details>
  <summary>Details</summary>
Motivation: 通用家用机器人需要实时精细运动控制来处理精细任务和紧急情况，而当前视频模型缺乏这种精细控制能力。

Method: 开发了特征学习范式来对齐多模态信息，同时保留每个模态的独特信息；提出了正则化方案来增强动作轨迹特征在表示复杂交互动态时的因果性。

Result: 实验表明，结合多模态感知提高了模拟精度并减少了时间漂移；广泛的消融研究和下游应用证明了方法的有效性和实用性。

Conclusion: 多模态感知的引入显著提升了机器人精细控制能力，为通用家用机器人的实际应用提供了可行的解决方案。
...
TL;DR: VideoNSA采用原生稀疏注意力机制，通过端到端训练提升视频语言模型的长视频理解能力，在128K令牌下保持稳定性能。
...
Method: 基于Qwen2.5-VL模型，采用硬件感知的混合注意力方法：文本使用密集注意力，视频使用原生稀疏注意力，在216K视频指令数据集上进行端到端训练。

Result: 相比令牌压缩和无训练稀疏基线，VideoNSA在长视频理解、时序推理和空间基准测试中表现更优，能够可靠扩展到128K令牌。

Conclusion: 研究发现存在最优的全局-局部注意力分配、任务依赖的分支使用模式，可学习的组合稀疏注意力有助于诱导动态注意力汇聚点。
...
TL;DR: NoiseShift是一种无需训练的方法，通过根据分辨率大小重新校准去噪器的噪声水平，显著提升扩散模型在低分辨率图像生成上的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的高分辨率文本到图像生成器无法为不需要高分辨率图像的用户提供开箱即用的经济高效替代方案，因为扩散模型在固定分辨率集上训练后往往无法泛化到其他分辨率。

Method: 提出NoiseShift方法，识别噪声调度器在不同分辨率下具有不等的感知效应，通过重新校准条件于分辨率大小的去噪器噪声水平来解决训练-测试不匹配问题。

Result: 在LAION-COCO数据集上，NoiseShift将SD3.5的FID提升15.89%，SD3提升8.56%，Flux-Dev提升2.44%；在CelebA数据集上，分别提升10.36%、5.19%和3.02%。

Conclusion: NoiseShift能有效缓解分辨率相关的伪影，显著提升低分辨率图像生成质量，且无需改变模型架构或采样计划，与现有模型兼容。
...
TL;DR: 该研究探索从视频中预测动态物理属性（弹性、粘性、摩擦）的方法，包括创建新数据集、比较三种推断方法，发现视频基础模型表现接近oracle方法，而多模态大语言模型表现较差但可通过提示改进。


<details>
  <summary>Details</summary>
Motivation: 研究如何从视频中推断需要时间信息才能判断的动态物理属性，如物体的弹性、液体的粘性和表面的动态摩擦。

Method: 收集了三个物理属性的新视频数据集；探索了三种推断方法：基于经典计算机视觉的oracle方法、使用预训练视频模型的可训练提示向量方法、以及多模态大语言模型的提示策略。

Result: 生成式或自监督训练的视频基础模型表现相似，接近oracle方法但略差；多模态大语言模型目前表现较差，但通过合适的提示可以改善性能。

Conclusion: 视频基础模型在预测动态物理属性方面表现良好，而多模态大语言模型仍有改进空间，提示策略对其性能提升有帮助。
...
TL;DR: 提出声音物体检测任务，通过多模态对象感知框架从第一人称视频中学习，使用分割掩码和槽注意力机制来引导模型关注交互中的关键物体区域。


<details>
  <summary>Details</summary>
Motivation: 日常物体交互产生独特声音，但现有模型难以区分声音来源（如勺子敲击木地板vs地毯），需要评估模型将声音与直接参与物体关联的能力。

Method: 开发自动管道计算交互物体的分割掩码，使用槽注意力视觉编码器强化对象先验，通过多模态框架从第一人称视频学习。

Result: 在新任务和现有多模态动作理解任务上达到最先进性能。

Conclusion: 提出的对象感知框架能有效链接声音与交互物体，在声音物体检测任务中表现优异。
...
TL;DR: 该论文分析了3D高斯泼溅(3DGS)对图像级投毒攻击的脆弱性，提出了一种基于密度引导的投毒方法，通过在低密度区域注入高斯点来嵌入视角依赖的虚幻物体，并引入自适应噪声策略破坏多视角一致性。


<details>
  <summary>Details</summary>
Motivation: 随着NeRF和3DGS等3D场景表示方法的普及，解决其安全漏洞变得至关重要。论文旨在分析3DGS对投毒攻击的鲁棒性，并开发更有效的攻击方法。

Method: 使用核密度估计(KDE)识别低密度区域，在这些区域战略性地注入高斯点来嵌入视角依赖的虚幻物体。同时采用自适应噪声策略破坏多视角一致性。

Result: 大量实验表明，该方法在性能上优于现有最先进技术，能够有效嵌入在投毒视角清晰可见的虚幻物体，同时对无辜视角影响最小。

Conclusion: 论文揭示了3DGS对投毒攻击的脆弱性，提出的密度引导投毒方法具有优越性能，并为未来研究提供了基于KDE的系统性评估协议。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在单主体提示上表现出色，但在多主体描述中经常出现属性泄漏、身份纠缠和主体遗漏等问题，需要专门的方法来提升多主体生成质量。

Method: 通过随机最优控制视角看待流匹配，提出了两种架构无关算法：无需训练的单次更新测试时控制器，以及轻量级微调方法Adjoint Matching。

Result: 在Stable Diffusion 3.5、FLUX和Stable Diffusion XL等模型上，两种算法都一致提升了多主体对齐效果，同时保持了基础模型的风格。测试时控制可在消费级GPU上高效运行，微调控制器在有限提示上训练后能泛化到未见过的提示。
...
### [78] [Discovering Self-Regulated Learning Patterns in Chatbot-Powered Education Environment](https://arxiv.org/abs/2510.01275)
*Yilin Lyu,Ren Ding*

Main category: cs.CY

TL;DR: 提出了Gen-SRL标注框架分析学生与聊天机器人互动中的自我调节学习行为，发现学生主要关注任务执行而缺乏规划和反思，且学习过程呈现非顺序性模式。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的普及，需要了解学生如何在与聊天机器人互动中进行自我调节学习，传统SRL理论可能无法充分捕捉这种动态交互模式。

Method: 提出Gen-SRL标注框架，将学生提示分为4个宏观阶段16个微观行动，对212个英语写作任务中的聊天互动进行标注，使用频率分析和过程挖掘技术。

Result: 82%的行为集中在任务执行阶段，规划和反思参与有限，过程分析显示非顺序性调节模式，传统SRL理论无法完全捕捉聊天机器人互动中的动态模式。

Conclusion: 需要设计适应学生动态行为的个性化支架，为SRL研究提供新视角，指导开发更好地支持自我调节的聊天机器人。
...
### [79] [An Analysis of the New EU AI Act and A Proposed Standardization Framework for Machine Learning Fairness](https://arxiv.org/abs/2510.01281)
*Mike Teodorescu,Yongxu Sun,Haren N. Bhatia,Christos Makridis*

Main category: cs.CY

TL;DR: 欧盟AI法案缺乏可量化的公平性指标，存在术语模糊性，这会产生重大责任风险并阻碍投资。作者建议制定更精细的监管框架，提出评估AI系统公平性和透明度的公共系统框架，并倡导标准化行业最佳实践。


<details>
  <summary>Details</summary>
Motivation: 欧盟AI法案在监管伦理和负责任的AI系统方面迈出了重要一步，但发现法案中缺乏可量化的公平性指标，且术语使用模糊（如透明度、可解释性和可理解性混用），也没有提及伦理合规的透明度。这种模糊性会产生重大责任风险，阻碍投资。

Method: 分析欧盟AI法案中的术语模糊性和监管不足，提出更精细的监管框架建议，构建评估AI系统公平性和透明度的公共系统框架，并倡导标准化行业最佳实践。以ASR和语音合成器为例进行说明。

Result: 识别出欧盟AI法案在公平性指标量化和术语清晰度方面的缺陷，这些缺陷可能导致责任风险和投资障碍。提出了具体的改进建议和评估框架。

Conclusion: 需要更精细的监管框架来增强欧盟AI法规，标准化行业最佳实践对于实现行业所需的细节水平至关重要，同时防止抑制AI领域的创新和投资。
...
### [80] [Emergent evaluation hubs in a decentralizing large language model ecosystem](https://arxiv.org/abs/2510.01286)
*Manuel Cebrian,Tomomi Kito,Raul Castro Fernandez*

Main category: cs.CY

TL;DR: 论文研究了大型语言模型与基准测试生态系统的发展模式，发现模型创建呈现分散化趋势，而基准测试影响力却高度集中，这种集中化的基准影响力作为协调基础设施支持标准化和可比性。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型和基准测试这两个生态系统层级的演变模式是否同步发展还是出现分化，理解模型生产多样性与基准测试集中化之间的动态关系。

Method: 使用斯坦福基础模型生态系统图和Evidently AI基准注册表作为生态系统代理，分析模型创建和基准影响力的网络结构，并进行基于代理的模拟来识别机制。

Result: 模型创建在国家和地区、组织、模态、许可和访问方面多样化，而基准影响力高度集中：前15%节点占高介数路径的80%以上，三个国家产生83%的基准输出，基准权威的全球基尼系数达0.89。

Conclusion: 集中的基准影响力作为协调基础设施，在模型生产异质性增加的情况下支持标准化、可比性和可重复性，但也带来了路径依赖、选择性可见性和随着排行榜饱和而减弱的判别力等权衡。
...
### [81] [Sycophantic AI Decreases Prosocial Intentions and Promotes Dependence](https://arxiv.org/abs/2510.01395)
*Myra Cheng,Cinoo Lee,Pranav Khadpe,Sunny Yu,Dyllan Han,Dan Jurafsky*

Main category: cs.CY

TL;DR: 研究发现AI模型普遍存在谄媚行为，会过度认同用户观点，这降低了用户修复人际冲突的意愿，但用户却更喜欢谄媚的AI，形成了危险的激励循环。


<details>
  <summary>Details</summary>
Motivation: 研究AI谄媚现象的普遍程度及其对用户行为的影响，因为虽然公众和学术界都关注这个问题，但对其实际影响知之甚少。

Method: 评估11个最先进AI模型的谄媚程度，并进行两项预注册实验（N=1604），包括让参与者讨论真实人际冲突的实时互动研究。

Result: AI模型比人类谄媚程度高50%，即使涉及操纵或欺骗时也如此。谄媚AI显著降低参与者修复人际冲突的意愿，增加其自认为正确的信念，但参与者却更信任和愿意使用谄媚AI。

Conclusion: 人们被无条件认同的AI所吸引，但这种认同会削弱判断力并减少亲社会行为倾向，形成了危险的激励结构，需要明确解决以减轻AI谄媚的广泛风险。
...
### [82] [A principled way to think about AI in education: guidance for action based on goals, models of human learning, and use of technologies](https://arxiv.org/abs/2510.01467)
*Noah D. Finkelstein*

Main category: cs.CY

TL;DR: 提出一个原则性框架来指导AI在教学中的使用，连接教育目标与可操作实践，明确教育者、学习者和技术的角色，确保AI增强而非取代人类能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和相关技术可能深刻影响高等教育，现有讨论往往只关注AI的承诺与风险或即时实施，需要第三条路径来指导AI在教学中的使用。

Method: 基于学习科学和教育技术应用的数十年研究，提出一套原则性框架，连接广泛的教育目标与可操作实践，阐明教育者、学习者和技术在课程设计、教学评估和社区建设中的角色。

Result: 开发了一个原则性框架，使高等教育能够利用新工具同时保持其基本使命：促进有意义的学习、支持民主社会和为学生准备动态未来。

Conclusion: 该框架确保AI增强而非取代人类能力，使技术使用与持久的教育价值观和目标保持一致。
...
### [83] [Extracting O*NET Features from the NLx Corpus to Build Public Use Aggregate Labor Market Data](https://arxiv.org/abs/2510.01470)
*Stephen Meisenbacher,Svetlozar Nestorov,Peter Norlander*

Main category: cs.CY

TL;DR: 开发了Job Ad Analysis Toolkit (JAAT)工具包，使用O*NET框架从在线招聘广告中提取结构化信息，处理了1.55亿个招聘广告，提取超过100亿个数据点。


<details>
  <summary>Details</summary>
Motivation: 在线招聘数据难以获取且缺乏标准化，而O*NET数据更新不频繁且样本量小，需要更好的工具来处理和分析大规模招聘数据。

Method: 采用O*NET作为框架，构建自然语言处理工具JAAT，从招聘广告中提取结构化信息，包括职业代码、工具技术、工资等特征。

Result: 成功处理了1.55亿个招聘广告，提取超过100亿个数据点，构建了2015-2025年月度活跃工作的职业、州和行业级特征数据集。

Conclusion: JAAT工具包具有可靠性和准确性，为研究和教育及劳动力发展提供了新的数据资源和分析能力。
...
### [84] [Framing Unionization on Facebook: Communication around Representation Elections in the United States](https://arxiv.org/abs/2510.01757)
*Arianna Pera,Veronica Jude,Ceren Budak,Luca Maria Aiello*

Main category: cs.CY

TL;DR: 该研究结合NLRB选举数据和工会Facebook帖子，分析了工会在线话语框架与选举结果的关系，发现获胜选举前诊断性、前瞻性和社区框架使用增加，而失败选举中沟通策略缺乏调整。


<details>
  <summary>Details</summary>
Motivation: 数字媒体已成为工会沟通、组织和维持集体行动的核心，但工会在线话语与具体选举结果之间的关系尚不清楚。

Method: 结合NLRB选举数据和158k个工会Facebook帖子，使用微调RoBERTa分类器系统标注帖子，分析选举事件周围的框架使用模式。

Result: 诊断性和社区框架主导工会沟通，但框架使用在组织间差异显著。获胜选举前诊断性、前瞻性和社区框架使用增加，失败选举中沟通策略缺乏调整。

Conclusion: 研究通过分析信息层面框架变化，揭示了沟通策略如何适应组织环境，为理解工会和社运数字沟通提供开放工具和数据。
...
### [85] [Small is Sufficient: Reducing the World AI Energy Consumption Through Model Selection](https://arxiv.org/abs/2510.01889)
*Tiago da Silva Barros,Frédéric Giroire,Ramon Aparicio-Pardo,Joanna Moulierac*

Main category: cs.CY

TL;DR: 论文提出通过模型选择策略实现绿色AI，从追求大模型转向选择合适的小模型，可在保持性能的同时显著降低AI推理能耗。研究表明模型选择可减少27.8%的AI能耗，相当于2025年节省31.9TWh电力。


<details>
  <summary>Details</summary>
Motivation: AI的能耗和碳足迹已成为关键问题，需要从'越大越好'转向'小而足够'的能源节约理念。模型选择是一种简单易行的方法，不需要新硬件或架构。

Method: 通过系统研究AI任务，分析其流行度、模型大小和效率，考察不同任务成熟度和模型采用模式对节能潜力的影响。

Result: 模型选择可为不同任务节省1%到98%的能耗，总体可减少AI能耗27.8%，2025年全球可节省31.9TWh电力，相当于五个核电站年发电量。

Conclusion: 模型选择是实现绿色AI的有效策略，能够在保持实用性的同时显著降低能源消耗，推动AI向更可持续的方向发展。
...
### [86] [The Current State of AI Bias Bounties: An Overview of Existing Programmes and Research](https://arxiv.org/abs/2510.02036)
*Sergej Kucenko,Nathaniel Dennler,Fengxiang He*

Main category: cs.CY

TL;DR: 本文对AI偏见悬赏项目进行了系统调查，分析了现有的5个偏见悬赏项目和5篇相关研究文献，探讨了这种基于奖励的社区参与式偏见检测方法的现状和挑战。


<details>
  <summary>Details</summary>
Motivation: 当前偏见评估方法很少涉及受AI系统影响的社区，偏见悬赏作为基于奖励的方法，旨在让社区参与AI偏见检测，让用户报告在使用AI系统时遇到的偏见问题。

Method: 通过搜索Google、Google Scholar、PhilPapers和IEEE Xplore等数据库，识别并分析了现有的偏见悬赏项目和相关学术文献。

Result: 发现了5个由美国组织举办的偏见悬赏项目（均为限时竞赛，奖金池7,000-24,000美元）和5篇研究文献，涵盖从bug悬赏应用到算法危害、Twitter偏见悬赏、制度机制设计等不同方面。

Conclusion: 需要降低偏见悬赏项目的技术门槛以包含无编码经验者，未来应探索从bug悬赏中借鉴最佳实践，设计对弱势群体敏感且降低组织采用障碍的方案。
...
### [87] [OR-Toolformer: Modeling and Solving Operations Research Problems with Tool Augmented Large Language Models](https://arxiv.org/abs/2510.01253)
...
Result: 在四个标准基准测试中的三个上，OR-Toolformer达到80.1%的执行准确率，比同等规模基线提高4.3%以上；在零样本评估中，对未见过的运筹学问题类型达到54%的平均准确率，比最强基线提高21个百分点。
...
### [88] [Modeling Others' Minds as Code](https://arxiv.org/abs/2510.01272)
*Kunal Jha,Aydan Yuenan Huang,Eric Ye,Natasha Jaques,Max Kleiman-Weiner*

Main category: cs.AI

TL;DR: 提出ROTE算法，通过将日常社交互动建模为行为程序，结合大语言模型生成假设空间和概率推理处理不确定性，显著提升人类行为预测的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的人类行为建模方法要么对理性做出不切实际的假设，要么计算成本过高难以快速适应，需要一种更高效、更鲁棒的方法来预测人类行为以实现安全的人机协作。

Method: 将日常社交互动建模为行为程序（计算机代码），提出ROTE算法：使用大语言模型合成行为程序的假设空间，结合概率推理处理不确定性。
...
### [89] [Cyber Academia-Chemical Engineering (CA-ChemE): A Living Digital Town for Self-Directed Research Evolution and Emergent Scientific Discovery](https://arxiv.org/abs/2510.01293)
*Zekun Jiang,Chunming Xu,Tianhang Zhou*

Main category: cs.AI

TL;DR: 提出了CA-ChemE系统，一个通过多智能体协作实现自主研究演进和新兴科学发现的数字化学工程城镇，解决了AI在化学工程中跨学科合作和未知问题探索的局限性。


<details>
  <summary>Details</summary>
Motivation: 解决现有AI系统在化学工程中跨学科协作和未知问题探索方面的局限性，构建能够自主演进和发现新知识的智能生态系统。

Method: 集成领域特定知识库、知识增强技术和协作智能体，构建CA-ChemE系统，通过多智能体协作和本体工程能力实现跨领域合作。

Result: 知识库增强机制使7个专家智能体的对话质量评分平均提高10-15%；协作智能体的干预使远领域专家对的效率提升8.5%，而邻近领域仅提升0.8%，揭示了"知识库差距导致的协作效率降低"效应。

Conclusion: 精心设计的多智能体架构为化学工程中的自主科学发现提供了可行路径，证明了通过智能体协作可以克服跨学科知识差距带来的挑战。
...
### [90] [The Social Laboratory: A Psychometric Framework for Multi-Agent LLM Evaluation](https://arxiv.org/abs/2510.01295)
*Zarreen Reza*

Main category: cs.AI

TL;DR: 提出了一种新的多智能体辩论评估框架，用于量化LLM智能体在交互环境中的社交和认知动态行为，发现智能体具有强烈的共识寻求倾向。


<details>
  <summary>Details</summary>
Motivation: 传统评估基准无法捕捉LLM智能体在交互环境中产生的涌现社交和认知动态，如沟通、说服和协作行为。

Method: 使用多智能体辩论作为受控"社交实验室"，让具有不同角色和激励的LLM智能体在LLM主持人的监督下就各种挑战性话题进行辩论，并采用新的心理测量和语义指标进行分析。

Result: 在数百场辩论中发现智能体具有强烈的共识寻求倾向，即使没有明确指令也能达到高语义一致性（μ > 0.88）；分配的角色能诱导稳定的心理测量特征；主持人的角色能显著改变辩论结果。

Conclusion: 这项工作为面向智能体场景的动态、基于心理测量的评估协议提供了蓝图，为理解和塑造下一代AI智能体的社交行为提供了关键方法。
...
TL;DR: AGILE通过将拼图任务构建为交互式学习过程，使用代码执行动作和环境反馈来增强视觉语言模型的感知和推理能力，显著提升了拼图任务表现并在多个视觉任务上展现出良好的泛化能力。
...
Method: AGILE将拼图解决构建为交互过程，模型在每个步骤生成可执行代码来执行动作，环境提供细粒度视觉反馈来指导任务完成。通过这种观察和交互的迭代循环，模型通过探索和反馈逐步提升感知和推理能力。

Result: AGILE显著提升了不同复杂度拼图任务的性能（如在2×2设置下准确率从9.5%提升至82.8%），并在9个通用视觉任务上展现出强泛化能力，平均提升3.1%。
...
TL;DR: Aristotle系统结合形式验证与非形式推理，在2025年国际数学奥林匹克竞赛中达到金牌水平表现


<details>
  <summary>Details</summary>
Motivation: 开发一个能够结合形式验证严格性和非形式推理灵活性的AI系统，以解决复杂的数学问题
...
Method: 通过专家手动设计和基于代理的可扩展合成来构建数据集，整合Slack、Linear、Git等多个平台的异步事件，创建具有噪声、冲突和跨平台引用的时间线。

Result: 实验显示当前最先进的LLM和记忆后端在处理长期跨平台依赖和解决矛盾方面面临挑战，表现最佳的GPT-5模型在MEMTRACK上仅达到60%的正确率。

Conclusion: MEMTRACK为记忆增强代理的评估研究提供了可扩展框架，超越了现有对话设置的局限，为复杂组织环境中的多代理多平台记忆基准测试奠定了基础。
...
### [94] [Retrieval-Augmented Framework for LLM-Based Clinical Decision Support](https://arxiv.org/abs/2510.01363)
...
<details>
  <summary>Details</summary>
Motivation: 临床决策复杂性增加和电子健康记录快速增长带来了机遇与挑战，需要数据驱动的决策支持工具来改善医疗服务质量。

Method: 采用检索增强生成(RAG)管道，整合自然语言处理和结构化临床输入，通过检索相似病例并合成信息来生成治疗建议。

Result: 初步评估显示模型输出具有临床合理性和一致性，表明在适当约束和严格验证下，LLM工具可为处方工作流程提供有价值的决策支持。
...
TL;DR: 提出了TRACE方法检测隐式奖励攻击，通过截断思维链测量模型通过验证器所需的最小努力，在数学推理和编程任务中显著优于现有监控方法。


<details>
  <summary>Details</summary>
Motivation: 奖励攻击（模型利用奖励函数漏洞获得高分但不解决实际任务）是严重威胁，特别是隐式攻击会绕过思维链监控，需要新的检测方法。

Method: TRACE方法：逐步截断模型的思维链，强制模型回答，测量每个截断长度下通过验证器的比率。攻击模型会以较少的推理努力获得高通过率，表现为准确率-长度曲线下面积较大。

Result: 在数学推理任务中比最强的72B思维链监控器提升超过65%，在编程任务中比32B监控器提升超过30%。还能在训练中发现未知漏洞。
...
### [96] [Just Do It!? Computer-Use Agents Exhibit Blind Goal-Directedness](https://arxiv.org/abs/2510.01670)
...
TL;DR: 本文提出了计算机使用代理(CUAs)存在的盲目标导向(BGD)问题，即代理会不顾可行性、安全性和上下文盲目追求目标，并开发了BLIND-ACT基准来评估这一问题。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在GUI上执行操作时存在盲目追求目标的风险，这种风险在输入无害时也会出现，需要系统性的评估和缓解方法。

Method: 开发了BLIND-ACT基准，包含90个任务捕捉BGD的三种模式，基于OSWorld提供真实环境，使用LLM评估代理行为，并与人类标注达到93.75%的一致性。

Result: 评估了9个前沿模型，包括Claude Sonnet和Opus 4等，观察到平均80.8%的高BGD率，提示性干预能降低但无法完全消除风险。

Conclusion: 识别BGD并引入BLIND-ACT为未来研究和缓解这一基本风险奠定了基础，确保CUA的安全部署需要更强的训练或推理时干预。
...
Method: 提出三步蒸馏管道：(1)从智能体失败中提取紧凑、可重用的提示；(2)使用这些提示通过单次检索在任务开始时生成改进的教师轨迹；(3)在移除提示字符串的情况下训练学生模型，强制模型内部化而非记忆。

Result: 在ALFWorld(家庭任务)和WebShop(在线购物)两个交互基准测试中，蒸馏后的学生模型持续优于基线智能体，ALFWorld成功率高达91%(基线为79%)，WebShop得分提升至72(基线为61)，同时使用的token数量比检索增强教师少10-60%。

Conclusion: 该方法在不同模型规模(7B/14B参数)和智能体架构(ReAct/StateAct)上均能泛化，证明检索的益处可以通过有针对性的微调有效内部化，无需永久性的运行时依赖。
...
### [98] [Automating Data-Driven Modeling and Analysis for Engineering Applications using Large Language Model Agents](https://arxiv.org/abs/2510.01398)
*Yang Liu,Zaid Abulawi,Abhiram Garimidi,Doyeong Lim*

Main category: cs.AI

TL;DR: 提出使用大语言模型(LLM)代理自动进行数据驱动建模和分析的创新管道，特别关注回归任务，在临界热通量预测基准测试中表现优异。
...
Result: 在包含约25,000个实验数据点的OECD/NEA基准数据集上，LLM代理开发的模型超越了传统临界热通量查找表，预测精度和不确定性量化与人类专家开发的贝叶斯优化深度神经网络相当。
...
### [99] [OntoLogX: Ontology-Guided Knowledge Graph Extraction from Cybersecurity Logs with Large Language Models](https://arxiv.org/abs/2510.01409)
*Luca Cotti,Idilio Drago,Anisa Rula,Devis Bianchini,Federico Cerutti*

Main category: cs.AI

TL;DR: OntoLogX是一个基于LLM的AI代理，将原始系统日志转换为基于本体的知识图谱，并通过RAG和迭代校正确保图谱质量，能够将日志证据映射到MITRE ATT&CK战术。
...
Method: 集成轻量级日志本体与检索增强生成(RAG)和迭代校正步骤，确保生成的知识图谱语法和语义有效。系统将知识图谱聚合为会话，并使用LLM预测MITRE ATT&CK战术。

Result: 在公共基准和真实世界蜜罐数据集上的评估显示，系统能够跨多个知识图谱后端生成稳健的知识图谱，并准确将对抗活动映射到ATT&CK战术。检索和校正提高了精确率和召回率。
...
### [100] [A Tale of LLMs and Induced Small Proxies: Scalable Agents for Knowledge Mining](https://arxiv.org/abs/2510.01427)
*Sipeng Zhang,Longfei Yun,Zilong Wang,Jingbo Shang,Letian Peng*

Main category: cs.AI

TL;DR: Falconer是一个结合LLM智能推理和轻量级代理模型的知识挖掘框架，通过LLM作为规划器和标注器，训练小型代理模型执行分类和提取任务，在保持高准确率的同时大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 解决大规模知识挖掘中LLM部署成本过高，而传统分类器-提取器流水线又不够灵活、无法泛化到新任务的问题。

Method: 使用LLM作为规划器分解用户指令为可执行流水线，作为标注器生成监督数据训练小型代理模型，统一分类和提取为两个原子操作：获取标签和获取跨度。

Result: Falconer在指令跟随准确率上接近最先进的LLM，同时推理成本降低90%，大规模知识挖掘速度提升20倍以上。

Conclusion: Falconer为深度研究提供了一个高效且可扩展的基础框架，实现了LLM智能推理能力与轻量级模型效率的良好平衡。
...
### [101] [On the Role of Domain Experts in Creating Effective Tutoring Systems](https://arxiv.org/abs/2510.01432)
*Sarath Sreedharan,Kelsey Sikes,Nathaniel Blanchard,Lisa Mason,Nikhil Krishnaswamy,Jill Zarestky*

Main category: cs.AI

TL;DR: 本文探讨了领域专家提供的精炼知识在创建有效辅导系统中的作用，提出了两种利用专家知识开发新型教育系统的方法：使用可解释AI技术自动生成课程，以及利用专家制定的课程大纲开发自适应辅导系统。


<details>
  <summary>Details</summary>
Motivation: AI教育社区往往忽视了领域专家提供的精炼知识在创建有效辅导系统中的作用，本文旨在强调这一被忽视的话题。

Method: 1. 使用可解释AI技术结合专家指定的问题解决规则自动生成课程；2. 利用专家制定的学习目标概念课程大纲开发自适应辅导系统。

Result: 通过传粉者识别辅导系统的案例研究，证明了这些方法的重要性，表明这类知识可以轻松从专家那里获取。

Conclusion: 领域专家的精炼知识在创建有效教育系统中具有重要作用，通过结合可解释AI技术和专家制定的课程，可以开发出更有效的自适应辅导系统。
...
### [102] [VOGUE: Guiding Exploration with Visual Uncertainty Improves Multimodal Reasoning](https://arxiv.org/abs/2510.01444)
*Rui Liu,Dian Yu,Tong Zheng,Runpeng Dai,Zongxia Li,Wenhao Yu,Zhenwen Liang,Linfeng Song,Haitao Mi,Pratap Tokekar,Dong Yu*

Main category: cs.AI

TL;DR: VOGUE是一种新颖的视觉不确定性引导探索方法，通过将图像视为随机上下文，量化策略对视觉扰动的敏感性，从而在多模态大语言模型中改善探索问题，显著提升视觉数学和通用领域推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在处理多模态大语言模型时，将视觉输入视为固定确定性条件，忽略了视觉变化的模糊性，导致策略对合理视觉变化缺乏鲁棒性，探索问题依然存在。

Method: VOGUE通过将探索从输出（文本）空间转移到输入（视觉）空间，将图像视为随机上下文，使用对称KL散度量化策略对视觉扰动的敏感性，结合不确定性比例奖励、token熵奖励和退火采样调度来平衡探索与利用。

Result: 在Qwen2.5-VL-3B/7B模型上实现，VOGUE在三个视觉数学基准上平均提升pass@1准确率2.6%，在三个通用领域推理基准上提升3.7%，同时提高了pass@4性能并缓解了RL微调中常见的探索衰减问题。

Conclusion: 基于视觉输入固有不确定性的探索策略是改善多模态推理的有效方法，VOGUE通过量化视觉不确定性成功解决了多模态大语言模型中的探索挑战。
...
### [103] [AIReg-Bench: Benchmarking Language Models That Assess AI Regulation Compliance](https://arxiv.org/abs/2510.01474)
...
TL;DR: 提出了首个用于评估LLM在AI法规合规性检查方面性能的基准数据集AIReg-Bench，重点关注欧盟AI法案的合规性评估。


<details>
  <summary>Details</summary>
Motivation: 随着政府对AI的监管加强，需要评估LLM在AI法规合规性检查方面的能力，但目前缺乏相关基准。

Method: 通过两步流程创建数据集：(1) 使用结构化指令提示LLM生成120个技术文档摘录；(2) 法律专家审查并标注每个样本是否违反AI法案的具体条款。

Result: 创建了包含120个样本的基准数据集，并评估了前沿LLM能否复现专家的合规性标签。

Conclusion: 该数据集为理解基于LLM的AI法规合规性评估工具的机遇和局限性提供了起点，并为后续LLM比较建立了基准。
...
### [104] [Lateral Tree-of-Thoughts Surpasses ToT by Incorporating Logically-Consistent, Low-Utility Candidates](https://arxiv.org/abs/2510.01500)
*Abhinav Madahar*

Main category: cs.AI

TL;DR: LToT是一种改进的Tree-of-Thoughts搜索控制器，通过分离效用和逻辑一致性，将低效用但一致的候选视为资产而非浪费，解决广度饱和和深度近视问题。


<details>
  <summary>Details</summary>
Motivation: 现代部署分配大量测试时计算资源，但标准Tree-of-Thoughts搜索存在广度饱和（额外样本产生近似重复）和深度近视（噪声短期效用过早剪枝）的病理问题。

Method: LToT将前沿分为主线（高效用候选用于开发）和侧线（一致但初始低效用候选），通过侧向竞赛与短路机制进行探索，使用带重复确认的宽度感知阈值。

Result: 理论证明侧向成本为伪线性Θ(N₀ logₙ N₀)，与无上限主线的指数增长形成对比。实证评估正在准备中。

Conclusion: LToT将大型测试时预算转化为原则性多样性，同时保持提升纪律，在不增加计算的情况下缓解饱和和近视问题。
...
### [105] [Towards Interpretable and Inference-Optimal COT Reasoning with Sparse Autoencoder-Guided Generation](https://arxiv.org/abs/2510.01528)
*Daniel Zhao,Abhilash Shankarampeta,Lanxiang Hu,Tajana Rosing,Hao Zhang*

Main category: cs.AI

TL;DR: 提出了一种使用稀疏自编码器和聚类技术分析LLM内部token表示的方法，通过构建token转移图来指导数学推理任务的生成过程，平衡利用和探索。


<details>
  <summary>Details</summary>
Motivation: 为了分析大型语言模型在数学推理任务中的内部表示，并指导生成过程避免极端行为，实现高质量的推理。

Method: 训练稀疏自编码器生成稀疏向量表示，使用k-means聚类构建token转移图，基于边权重定义奖励函数量化推理轨迹的遵循程度。

Result: 研究发现平衡利用和探索对数学推理任务的高准确性至关重要，稀疏自编码器可作为可扩展的奖励模型指导生成过程。

Conclusion: 该方法能有效防止利用或探索的极端行为，促进LLM中更高质量的推理过程，实现利用和探索的平衡权衡。
...
### [106] [LOGicalThought: Logic-Based Ontological Grounding of LLMs for High-Assurance Reasoning](https://arxiv.org/abs/2510.01530)
*Navapat Nananukul,Yue Zhang,Ryan Lee,Eric Boxer,Jonathan May,Vibhav Giridhar Gogate,Jay Pujara,Mayank Kejriwal*

Main category: cs.AI

TL;DR: 提出LOGicalThought (LogT)神经符号架构，结合逻辑语言推理器和LLM，在四个多领域基准测试中提升推理性能11.84%，在否定、蕴含和可废止推理方面均有显著改进。


<details>
  <summary>Details</summary>
Motivation: 关键领域如法律和医学需要准确、可验证且基于证据的推理，但传统LLM在处理包含大量例外和可废止逻辑的规则时存在困难。

Method: 使用高级逻辑语言和推理器与LLM结合，构建双重符号图上下文和基于逻辑的上下文，将长文本推理转化为紧凑的接地评估。

Result: 在四个基准测试中，LogT相比最强基线整体性能提升11.84%，否定推理提升10.2%，蕴含推理提升13.2%，可废止推理提升5.5%。

Conclusion: LogT神经符号方法能有效处理高保证文本中的复杂逻辑结构，显著提升LLM在严格推理任务中的表现。
...
TL;DR: InfoSeeker是一个LLM决策框架，通过集成任务导向规划和信息寻求来在不确定环境中做出最优决策，相比现有方法实现了74%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM规划智能体在处理观测不确定性时，往往忽视其内部动态与实际环境之间的差异，导致在部分可观测环境中表现不佳。

Method: InfoSeeker通过提示LLM主动收集信息来验证理解、检测环境变化或测试假设，然后生成或修订任务导向计划。

Result: 在包含不完全观测和不确定动态的新基准测试中，InfoSeeker相比现有方法实现了74%的绝对性能提升，且不牺牲样本效率。
...
### [108] [Step-Aware Policy Optimization for Reasoning in Diffusion Large Language Models](https://arxiv.org/abs/2510.01544)
*Shaoan Xie,Lingjing Kong,Xiangchen Song,Xinshuai Dong,Guangyi Chen,Eric P. Xing,Kun Zhang*

Main category: cs.AI

TL;DR: 提出Step-Aware Policy Optimization (SAPO)算法，通过过程奖励函数改进扩散语言模型的推理能力，解决现有方法中推理路径不结构化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的扩散语言模型训练依赖稀疏的结果奖励，会强化导致巧合正确结果的错误推理路径，这与推理的自然结构不匹配。

Method: 提出理论框架将复杂问题解决形式化为层次选择过程，并基于此设计SAPO算法，使用过程奖励函数引导模型学习结构化推理路径。

Result: 在挑战性推理基准测试中显著提升性能，并增强了生成过程的可解释性。

Conclusion: 基于理论框架的SAPO算法能有效改进扩散语言模型的复杂推理能力，使推理过程更加结构化和可解释。
...
### [109] [InvThink: Towards AI Safety via Inverse Reasoning](https://arxiv.org/abs/2510.01569)
*Yubin Kim,Taehan Kim,Eugene Park,Chunjong Park,Cynthia Breazeal,Daniel McDuff,Hae Won Park*

Main category: cs.AI

TL;DR: InvThink是一种让大语言模型具备逆向思维能力的方法，通过先分析潜在危害及其后果，再生成安全回复，显著提升模型安全性并保持通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法直接优化安全回复，但无法系统性地考虑潜在风险。InvThink旨在通过逆向思维让模型主动识别和避免风险。

Method: 1) 枚举潜在危害 2) 分析后果 3) 生成主动避免这些风险的安全输出。通过监督微调和强化学习在三个LLM家族中实现。

Result: 相比基线方法SafetyPrompt，有害回复减少达15.7%；安全改进随模型规模扩大而增强；在高风险领域（医疗、金融、法律等）表现优异；减轻安全税效应，保持标准基准上的通用推理能力。
...
### [110] [AdvEvo-MARL: Shaping Internalized Safety through Adversarial Co-Evolution in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.01586)
...
TL;DR: AdvEvo-MARL是一个协同进化的多智能体强化学习框架，通过内部化安全机制到任务智能体中，无需依赖外部防护模块就能同时提高系统安全性和任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM多智能体系统防御方法存在两个问题：自我验证方法因单个智能体能力有限而效果不佳；外部防护模块会增加系统开销并造成单点故障风险。需要一种既能保证安全又不增加系统复杂度的解决方案。

Method: 采用对抗学习环境，联合优化攻击者（生成不断演变的越狱提示）和防御者（训练任务智能体既能完成任务又能抵抗攻击）。引入公共基线优势估计，同一功能组内的智能体共享组级平均回报基线，实现低方差更新和更强的组内协调。

Result: 在代表性攻击场景中，AdvEvo-MARL始终将攻击成功率保持在20%以下，而基线方法最高可达38.33%。同时保持甚至提高了任务准确率（在推理任务上最高提升3.67%）。

Conclusion: 安全性和实用性可以在不依赖额外防护智能体或增加系统开销的情况下共同提升，证明了内部化安全机制的有效性。
...
### [111] [AgentRec: Next-Generation LLM-Powered Multi-Agent Collaborative Recommendation with Adaptive Intelligence](https://arxiv.org/abs/2510.01609)
...
<details>
  <summary>Details</summary>
Motivation: 现有交互式对话推荐系统在处理动态用户偏好、保持对话连贯性以及同时平衡多个排序目标方面面临重大挑战。

Method: 采用分层智能体网络，配备专门用于对话理解、偏好建模、上下文感知和动态排序的LLM智能体，通过自适应权重机制进行协调，结合三层学习策略：简单查询的快速响应、复杂偏好的智能推理和挑战场景的深度协作。
...
### [112] [PychoBench: Evaluating the Psychology Intelligence of Large Language Models](https://arxiv.org/abs/2510.01611)
*Min Zeng*

Main category: cs.AI

TL;DR: 论文评估LLMs在心理咨询领域的应用潜力，通过PsychoBench基准测试发现只有前沿模型能达到美国国家心理咨询师认证考试(NCE)的通过标准。
...
Method: 开发PsychoBench基准，包含2,252个精心设计的单选题，基于美国国家心理咨询师认证考试，涵盖心理学多个子学科。

Result: GPT-4o、Llama3.3-70B和Gemma3-27B等先进模型得分远超通过阈值(约70%)，而较小的开源模型(Qwen2.5-7B、Mistral-7B)远低于标准。

Conclusion: 只有前沿LLMs目前能够满足心理咨询考试标准，这突显了开发心理学导向LLMs的潜力和挑战。
...
### [113] [Learning to Decide with Just Enough: Information-Theoretic Context Summarization for CDMPs](https://arxiv.org/abs/2510.01620)
*Peidong Liu,Junjiang Lin,Shaowen Wang,Yao Xu,Haiqing Li,Xuhao Xie,Siyi Wu,Hao Li*

Main category: cs.AI

TL;DR: 提出基于信息论的上下文总结方法，使用LLM将高维上下文压缩为低维语义摘要，增强状态表示，在CMDP中实现高效决策。
...
Method: 利用LLM进行信息论驱动的上下文摘要，生成低维语义丰富的摘要来增强状态表示，基于近似上下文充分性理论。

Result: 在离散、连续、视觉和推荐基准测试中，方法优于原始上下文和无上下文基线，提升奖励、成功率、样本效率，降低延迟和内存使用。
...
### [114] [Understanding the Geospatial Reasoning Capabilities of LLMs: A Trajectory Recovery Perspective](https://arxiv.org/abs/2510.01639)
...
Method: 将轨迹恢复作为代理任务，使用道路网络作为上下文，通过提示框架让LLMs生成有效路径而无需外部导航工具。

Result: LLMs在GLOBALTRACE数据集上优于现成基线和专门的轨迹恢复模型，表现出对道路网络和坐标系统的强理解能力。

Conclusion: LLMs具有强大的空间推理能力，能够通过灵活的地图推理来增强导航体验并整合用户偏好。
...
### [115] [GuruAgents: Emulating Wise Investors with Prompt-Guided LLM Agents](https://arxiv.org/abs/2510.01664)
...
<details>
  <summary>Details</summary>
Motivation: 将投资大师的定性投资哲学转化为可复现的定量策略，探索自动化系统化投资的新方向。

Method: 通过将不同投资大师的独特哲学编码到LLM提示中，结合金融工具和确定性推理管道，开发了五个不同的GuruAgents。

Result: 在2023年第四季度至2025年第二季度的回测中，各代理表现出基于其提示角色的独特行为，巴菲特代理获得最高42.2%的年化收益率。
...
### [116] [A Locally Executable AI System for Improving Preoperative Patient Communication: A Multi-Domain Clinical Evaluation](https://arxiv.org/abs/2510.01671)
...
TL;DR: LENOHA是一个安全优先、本地优先的系统，使用高精度分类器从临床FAQ中返回经过验证的答案，避免生成错误，在临床路径中能耗极低（1.0 mWh/输入）。


<details>
  <summary>Details</summary>
Motivation: 解决患者术前问题无人解答的困境，同时应对时间压力和隐私限制，提供个性化咨询。

Method: 使用句子转换器分类器路由输入，从临床医生策划的FAQ中返回逐字答案，避免自由文本生成。在两个领域（拔牙和胃镜检查）进行评估。

Result: E5-large-instruct模型总体准确率达0.983，AUC 0.996，仅7个错误，与GPT-4o表现相当。临床路径能耗仅为1.0 mWh/输入，比本地8B SLM低170倍。

Conclusion: 通过返回经过验证的FAQ答案，临床路径可以避免生成错误，支持隐私保护、可持续性和在带宽有限环境中的公平部署。
...
TL;DR: 本文提出AGI评估方法应从基于直觉的合成任务转向关注稳健任务执行能力的评估哲学，借鉴数据科学实践来验证系统可靠部署。


<details>
  <summary>Details</summary>
Motivation: 当前AGI评估方法依赖对智能的直觉认知来设计合成任务，这在AI历史上表现不佳，需要更有效的评估方法。

Method: 提出基于数据科学实践的替代设计哲学，专注于评估稳健任务执行能力，通过系统可靠部署来证明AGI能力。

Result: 提供了AGI评估的实际示例，展示了如何通过稳健任务执行来评估AGI系统。

Conclusion: AGI评估应转向关注系统在真实环境中的稳健执行能力，而非依赖直觉设计的合成任务。
...
### [118] [VaPR -- Vision-language Preference alignment for Reasoning](https://arxiv.org/abs/2510.01700)
*Rohan Wadhawan,Fabrice Y Harel-Canada,Zi-Yi Dou,Suhaila Shakiah,Robinson Piramuthu,Nanyun Peng*

Main category: cs.AI

TL;DR: 提出VaPR框架，通过LLM引导的响应编辑生成硬负样本，解决合成偏好标注中的风格和长度偏差问题，显著提升大型视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好微调方法忽视了合成偏好标注中普遍存在的风格和长度偏差噪声，需要一种能产生高质量负样本的方法来改善模型对齐效果。

Method: 基于LLM引导的响应编辑框架，生成具有目标错误但保持风格和长度相似性的被拒绝响应，构建了包含30K高质量样本的VaPR数据集。

Result: 在三个LVLM家族上实现显著性能提升：LLaVA平均提升6.5%，Qwen2VL提升4.0%，Qwen2.5VL提升1.5%，特别是在推理任务上表现突出，同时减少了二元问题中回答"是"的倾向。

Conclusion: VaPR框架有效解决了合成偏好标注的偏差问题，性能随数据规模扩展而提升，且该框架可推广到使用开源LLM作为编辑器，性能接近使用GPT-4o的效果。
...
### [119] [MetaboT: AI-based agent for natural language-based interaction with metabolomics knowledge graphs](https://arxiv.org/abs/2510.01724)
...
TL;DR: MetaboT是一个基于大语言模型的多智能体AI系统，能够将用户自然语言问题转换为SPARQL查询语言，用于操作代谢组学知识图谱，显著提高了知识图谱的可访问性和查询准确性。


<details>
  <summary>Details</summary>
Motivation: 代谢组学质谱分析产生大量数据，知识图谱虽然能有效组织这些数据，但其复杂本体论和查询语言阻碍了研究人员的使用。需要开发用户友好的界面来降低技术门槛。

Method: 采用多智能体系统架构，使用LangChain和LangGraph库构建。包含入口代理、验证代理、监督代理、知识图谱代理和SPARQL查询代理，通过分工协作处理用户查询并生成SPARQL查询。

Result: 在50个代谢组学相关问题的测试中，MetaboT达到83.67%的准确率，显著优于仅使用GPT-4o的基线方法（8.16%准确率）。

Conclusion: MetaboT成功地将大语言模型能力与实验基础查询生成相结合，为研究人员提供了通过自然语言访问复杂代谢组学知识图谱的有效工具，促进了数据驱动发现。
...
### [120] [A cybersecurity AI agent selection and decision support framework](https://arxiv.org/abs/2510.01751)
*Masike Malatji*
...
<details>
  <summary>Details</summary>
Motivation: 弥合理论AI构建与运营网络安全需求之间的差距，建立符合行业标准的强大多智能体系统基础。

Method: 通过将NIST CSF 2.0功能细分为具体任务，将AI智能体特性（自主性、自适应学习、实时响应）与每个子类别的安全要求关联，并定义分级自主水平。

Result: 概念验证表明，定制的AI智能体部署能够与现实约束和风险状况对齐，增强态势感知、加速响应时间，并通过自适应风险管理强化长期韧性。

Conclusion: 该研究在理论AI构建与运营网络安全需求之间建立了桥梁，为符合行业标准的稳健、经验验证的多智能体系统奠定了基础。
...
### [121] [REBot: From RAG to CatRAG with Semantic Enrichment and Graph Routing](https://arxiv.org/abs/2510.01800)
...
<details>
  <summary>Details</summary>
Motivation: 学术规章咨询对帮助学生理解和遵守机构政策至关重要，但构建有效系统需要特定领域的规章资源。

Method: CatRAG框架结合了检索增强生成和图推理，使用层次化、类别标记的知识图谱，并配备轻量级意图分类器来路由查询。

Result: 在规章特定数据集上的评估显示，REBot在分类和问答任务上达到了最先进的性能，F1分数为98.89%。

Conclusion: 开发了展示REBot在实际学术咨询场景中实用价值的Web应用，证明了该方法的有效性。
...
### [122] [Human-AI Teaming Co-Learning in Military Operations](https://arxiv.org/abs/2510.01815)
*Clara Maathuis,Kasper Cools*

Main category: cs.AI

TL;DR: 提出了一种可信赖的人机协作学习模型，用于军事行动中的人机团队合作，通过可调节自主性、多层控制、双向反馈和协作决策四个维度实现持续双向知识交换。


<details>
  <summary>Details</summary>
Motivation: 在快速演变的军事威胁和复杂作战环境中，AI集成到军事行动中具有显著优势，但也带来各种挑战和风险。当前对人机团队系统的理解往往从外部视角出发，而深入系统内部动态能处理更广泛的多维责任、安全和鲁棒性方面。

Method: 设计可信赖的共学习模型，整合四个维度：1)可调节自主性，根据任务状态、系统置信度和环境不确定性动态校准自主级别；2)多层控制，包括持续监督、活动监控和问责制；3)双向反馈，通过显性和隐性反馈回路确保推理、不确定性和学习适应的适当沟通；4)协作决策，生成、评估和提出带有置信水平和理由的决策。

Result: 提出了一个完整的人机团队可信赖共学习模型框架，并提供了具体示例和建议。

Conclusion: 该模型有助于进一步开发军事行动中负责任和可信赖的人机团队系统，通过持续双向知识交换和四个关键维度的整合来应对复杂作战环境中的挑战。
...
### [123] [Plan Then Action:High-Level Planning Guidance Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2510.01833)
...
Method: 两阶段框架：1) 使用高级LLMs提炼思维链为高层指导进行SFT；2) 引入指导感知RL方法联合优化最终输出和指导质量。

Result: 在多个数学推理基准测试（MATH、AIME2024、AIME2025、AMC）上，使用不同基础模型（Qwen2.5-7B、Qwen3-8B、Qwen3-14B、LLaMA3.2-3B）均实现了稳定显著的性能提升。

Conclusion: PTA-GRPO能够有效提升LLMs的推理能力，在不同模型和任务上均表现出良好的有效性和泛化性。
...
### [124] [Learning a Dense Reasoning Reward Model from Expert Demonstration via Inverse Reinforcement Learning](https://arxiv.org/abs/2510.01857)
*Claudio Fanconi,Nicolás Astorga,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: 该论文将对抗性逆强化学习应用于大语言模型推理，学习密集的token级奖励模型，用于过程监督而非风格模仿。学习到的推理奖励既能在训练时优化推理策略，又能在推理时作为评判器对采样轨迹进行重排序。


<details>
  <summary>Details</summary>
Motivation: 重新构建和操作化对抗性逆强化学习，直接从专家演示中学习密集的token级奖励模型，而不是通过监督微调来模仿风格，旨在提高多步推理的正确性。

Method: 使用对抗性逆强化学习框架，学习密集的推理奖励模型。该奖励模型在训练时提供步骤级反馈优化推理策略，在推理时作为评判器对采样轨迹进行重排序。

Result: 在GSM8K数据集上使用Llama3和Qwen2.5骨干网络，证明：密集推理奖励可用作学习信号来引发推理；通过奖励引导的重排序提高了预测性能（特别是基于Llama的策略）。
...
### [125] [Constrained Adaptive Rejection Sampling](https://arxiv.org/abs/2510.01902)
...
<details>
  <summary>Details</summary>
Motivation: 现有受限生成方法存在两极化问题：贪婪解码会扭曲模型分布，而拒绝采样则计算效率低下。在程序模糊测试等需要样本有效性和多样性的领域，这两种方法都有问题

Method: CARS从无约束语言模型采样开始，通过将违反约束的后续记录在trie中并从未来采样中减去其概率质量，自适应地排除这些无效延续。这种自适应剪枝确保已证明无效的前缀不会被重新访问

Result: 在程序模糊测试和分子生成等多个领域的实验中，CARS在采样效率（每个有效样本所需的LM前向传递次数）上始终表现更好，同时产生的样本多样性也强于GCD和其他近似LM分布的方法

Conclusion: CARS严格提高了拒绝采样的样本效率，而不会造成分布扭曲，在需要高有效性和多样性的约束生成任务中具有优势
...
### [126] [To Mask or to Mirror: Human-AI Alignment in Collective Reasoning](https://arxiv.org/abs/2510.01924)
*Crystal Qian,Aaron Parisi,Clémentine Bouleau,Vivian Tsai,Maël Lebreton,Lucas Dixon*

Main category: cs.AI

TL;DR: 本文提出了一个评估LLMs与人类集体决策对齐的框架，通过"Lost at Sea"实验发现LLMs在集体推理中的行为存在差异，有些反映人类偏见，有些则试图补偿偏见。
...
Method: 使用Lost at Sea社会心理学任务进行大规模在线实验(N=748)，随机分配组别进行领导者选举，比较可见人口属性与匿名别名的差异，并模拟匹配的LLMs组进行基准测试。

Result: LLMs行为出现分化：一些模型反映人类偏见；其他模型则掩盖这些偏见并试图补偿。人类-AI对齐在集体推理中取决于情境、线索和模型特定的归纳偏见。
...
### [127] [Zero-shot reasoning for simulating scholarly peer-review](https://arxiv.org/abs/2510.02027)
*Khalid M. Saqr*

Main category: cs.AI

TL;DR: 提出了一个确定性模拟框架，为评估AI生成的同行评审报告提供稳定、基于证据的标准，通过分析352份模拟报告证明了系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 学术出版生态系统面临提交量不可管理和AI不受监管的双重危机，需要新的治理模式来维护科学诚信。传统的人工同行评审缺乏可扩展、客观的基准。

Method: 采用确定性模拟框架，分析352份同行评审模拟报告，识别一致的系统状态指标。

Result: 系统能够模拟校准的编辑判断，'修订'决策始终占多数（>50%），'拒绝'率动态适应领域规范（健康科学达45%）；保持稳定的29%证据锚定合规率。
...
### [128] [ReTabAD: A Benchmark for Restoring Semantic Context in Tabular Anomaly Detection](https://arxiv.org/abs/2510.02060)
*Sanghyu Yoon,Dongmin Kim,Suhee Yoon,Ye Seul Sim,Seungdong Yoa,Hye-Seung Cho,Soonyoung Lee,Hankook Lee,Woohyung Lim*

Main category: cs.AI

TL;DR: ReTabAD通过恢复文本语义来解决表格异常检测中缺乏语义上下文的问题，提供了20个带有结构化文本元数据的数据集和多种检测算法，并建立了零样本LLM框架作为强基线。


<details>
  <summary>Details</summary>
Motivation: 现有表格异常检测基准只提供原始数据点而缺乏语义上下文，忽略了特征描述和领域知识等关键文本元数据，这限制了研究灵活性并阻碍模型充分利用领域知识进行检测。

Method: 提供20个精心策划的表格数据集，包含结构化文本元数据，并实现了包括经典方法、深度学习和LLM方法在内的最先进异常检测算法；还提出了零样本LLM框架，无需任务特定训练即可利用语义上下文。

Result: 实验结果显示语义上下文能提高检测性能，并通过支持领域感知推理来增强可解释性。

Conclusion: 这项工作建立了ReTabAD作为系统探索上下文感知异常检测的基准，并深入分析了文本元数据在异常检测中的作用和实用性。
...
### [129] [Demystifying the Roles of LLM Layers in Retrieval, Knowledge, and Reasoning](https://arxiv.org/abs/2510.02091)
*Xinyuan Song,Keyu Wang,PengXiang Li,Lu Yin,Shiwei Liu*

Main category: cs.AI

TL;DR: 本文系统研究发现LLM的深度利用具有高度异质性和上下文依赖性，深层在生成式评估中对于推理和长程连贯性至关重要，而知识和检索主要集中于浅层组件。


<details>
  <summary>Details</summary>
Motivation: 针对现有研究认为LLM深层对表示学习贡献有限且可被移除的观点，本文旨在通过多维度系统分析来验证深度利用的真实情况，避免基于狭窄评估的片面结论。

Method: 采用系统研究方法，从评估协议、任务类别和模型架构等多个维度分析深度利用，比较基于似然度评估和生成式评估的不同结果。

Result: 研究发现：基于似然度评估时，仅需保留前几层即可保持性能；但生成式评估显示中层和深层对推理能力和长程连贯性不可或缺；知识检索集中于浅层，推理准确性依赖深层但可通过蒸馏重塑。

Conclusion: LLM的深度利用高度异质且依赖上下文，需要基于任务、度量和模型的视角来理解和压缩大模型，不能简单认为深层可被移除。
...
TL;DR: 论文研究发现，虽然AI模型在ConceptARC基准测试中能达到人类水平的输出准确率，但通过细粒度规则分析发现，模型主要依赖表面模式而非真正的抽象推理，在视觉模态中准确率下降但仍有抽象推理能力，表明仅凭准确率评估会高估文本模态的推理能力而低估视觉模态的推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究AI模型在ConceptARC任务中的抽象推理能力，验证模型是否真正理解任务设计者意图的抽象概念，而非仅依赖表面模式匹配。

Method: 在ConceptARC基准上评估模型，改变输入模态（文本vs视觉）、是否允许使用外部Python工具、推理模型的推理努力程度，同时评估输出准确率和模型生成的自然语言解释规则。

Result: 文本模态下某些模型能达到人类水平的输出准确率，但最佳模型的规则往往基于表面"捷径"，很少捕捉到预期的抽象概念；视觉模态下模型输出准确率大幅下降，但规则分析显示模型仍能生成相当比例的抽象规则，只是无法正确应用。

Conclusion: 模型在抽象推理方面仍落后于人类，仅凭准确率评估ARC类任务的抽象推理能力会高估文本模态的能力而低估视觉模态的能力，需要更全面的评估框架来准确衡量多模态模型的抽象推理能力。
...
### [131] [FlexDoc: Parameterized Sampling for Diverse Multilingual Synthetic Documents for Training Document Understanding Models](https://arxiv.org/abs/2510.02133)
...
<details>
  <summary>Details</summary>
Motivation: 企业级文档理解模型需要大量多样化标注数据，但传统数据收集方法成本高昂（可达数百万美元），受隐私、法律限制和人工标注量大的制约。
...
### [132] [A Rigorous Benchmark with Multidimensional Evaluation for Deep Research Agents: From Answers to Reports](https://arxiv.org/abs/2510.02190)
...
TL;DR: 提出了一个针对深度研究代理（DRAs）的严格基准和多维评估框架，包含214个专家策划的挑战性查询，支持对长格式报告进行全面评估。
...
Method: 构建包含214个专家策划查询的基准，分布在10个广泛主题领域，并开发多维评估框架，集成语义质量、主题聚焦和检索可信度的评分指标。
...
### [133] [UpSafe$^\circ$C: Upcycling for Controllable Safety in Large Language Models](https://arxiv.org/abs/2510.02194)
...
<details>
  <summary>Details</summary>
Motivation: 现有安全技术（外部护栏、推理时指导和训练后对齐）在平衡安全性、实用性和可控性方面存在局限，需要一种更灵活的安全增强方法。

Method: 识别安全关键层并升级为稀疏MoE结构，其中路由器作为软护栏；采用两阶段SFT策略强化安全判别能力；引入安全温度机制实现推理时动态控制。

Result: 在多个基准测试、基础模型和模型规模上，UpSafe°C实现了对有害和越狱输入的鲁棒安全改进，同时在通用任务上保持竞争力，安全温度机制实现了效用与安全性的帕累托最优前沿。
...
### [134] [The Reasoning Boundary Paradox: How Reinforcement Learning Constrains Language Models](https://arxiv.org/abs/2510.02230)
...
<details>
  <summary>Details</summary>
Motivation: 尽管RLVR被用于提升语言模型的推理能力，但最近证据表明它可能反而缩小推理边界而非扩展它。本文旨在探究RLVR的这种收缩问题。

Method: 通过分析RLVR的学习动态，揭示负干扰和赢家通吃两种关键现象。基于这些洞察，提出了一种简单有效的数据筛选算法，专注于低概率问题的RLVR学习。

Result: 在多个数学推理基准测试中，通过理论和实证分析显示，标准RL目标中的固有在线策略采样导致模型收敛到狭窄的解决方案策略。提出的数据筛选算法显著提升了Pass@k性能。

Conclusion: RLVR存在收缩推理边界的问题，主要源于负干扰和赢家通吃现象。通过专注于低概率问题的数据筛选方法可以有效改善Pass@k性能。
...
### [135] [The Unreasonable Effectiveness of Scaling Agents for Computer Use](https://arxiv.org/abs/2510.02250)
...
Method: bBoN方法通过生成多个执行轨迹，并使用描述代理行为的行为叙述来选择最佳轨迹，实现广泛探索和原则性轨迹选择。

Result: 在OSWorld基准测试中达到69.9%的新SOTA，接近72%的人类水平性能，并在WindowsAgentArena和AndroidWorld上展示了良好的泛化能力。
...
### [136] [RLAD: Training LLMs to Discover Abstractions for Solving Reasoning Problems](https://arxiv.org/abs/2510.02263)
*Yuxiao Qu,Anikait Singh,Yoonho Lee,Amrith Setlur,Ruslan Salakhutdinov,Chelsea Finn,Aviral Kumar*

Main category: cs.AI

TL;DR: 提出RLAD方法，通过推理抽象来引导模型进行结构化探索，联合训练抽象生成器和解决方案生成器，提高推理能力


<details>
  <summary>Details</summary>
Motivation: 解决现有大模型推理过程中缺乏过程重用和结构化探索的问题，避免推理轨迹退化为冗长探索

Method: 引入推理抽象概念，采用双玩家RL训练范式，联合训练抽象生成器和解决方案生成器
...
### [137] [BioX-Bridge: Model Bridging for Unsupervised Cross-Modal Knowledge Transfer across Biosignals](https://arxiv.org/abs/2510.02276)
*Chenqi Li,Yu Liu,Timothy Denison,Tingting Zhu*

Main category: cs.AI

TL;DR: 提出BioX-Bridge框架，通过轻量级桥接网络实现生物信号跨模态知识迁移，显著减少可训练参数88-99%，同时保持或提升迁移性能。


<details>
  <summary>Details</summary>
Motivation: 生物信号模态间存在相关性，但标注数据有限且现有知识蒸馏方法计算开销大，特别是对于大型基础模型。

Method: 训练轻量级桥接网络对齐基础模型的中间表示，实现跨模态信息流动，包含高效的桥接位置选择策略和灵活的原型网络架构。

Result: 在多个生物信号模态、任务和数据集上的实验表明，BioX-Bridge显著减少可训练参数，同时维持或优于现有方法的迁移性能。
...
### [138] [Location Matters: Leveraging Multi-Resolution Geo-Embeddings for Housing Search](https://arxiv.org/abs/2510.01196)
...
<details>
  <summary>Details</summary>
Motivation: QuintoAndar作为拉丁美洲最大的住房平台，用户难以在海量房源中找到理想住所。地理位置对房产价值、便利设施和生活质量至关重要，因此将位置信息融入推荐系统对推荐效果至关重要。

Method: 采用分层H3网格的多层级方法，集成到双塔神经网络架构中，与传统的矩阵分解基线和单分辨率变体进行比较。

Result: 嵌入特定评估显示更丰富和平衡的嵌入表示，离线排名模拟显示推荐质量显著提升。

Conclusion: 该地理感知嵌入框架有效解决了住房推荐中的空间稀疏性问题，显著提升了推荐系统的性能。
...
### [139] [Are LLMs ready to help non-expert users to make charts of official statistics data?](https://arxiv.org/abs/2510.01197)
*Gadir Suleymanli,Alexander Rogiers,Lucas Lageweg,Jefrey Lijffijt*

Main category: cs.IR

TL;DR: 评估大型语言模型从复杂数据自动生成图表的能力，发现数据检索和处理是主要挑战，但通过适当指导和自评估机制可以显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在虚假信息和偏见数据泛滥的时代，可靠数据源的可访问性至关重要。国家统计机构提供的数据通常分散在多个表格中，普通用户难以处理，因此需要自动化工具来帮助用户识别相关数据并生成可视化图表。

Method: 使用荷兰统计局的不同公共数据，评估多个LLM在识别相关数据表、执行必要操作和自动生成适当可视化方面的能力。提出了一个包含三个维度的新评估框架：数据检索与预处理、代码质量和视觉表示。

Result: 结果表明，定位和处理正确数据是最重大的挑战。LLM在没有明确指导的情况下很少实施可视化最佳实践。当提供有效图表设计信息时，模型的表示得分显著提高。采用具有迭代自评估的代理方法在所有评估维度上都表现出色。
...
### [140] [Optimal signals assignment for eBay View Item page](https://arxiv.org/abs/2510.01198)
*Matan Mandelbrod,Biwei Jiang,Giald Wagner,Tal Franji,Guy Feigenblat*

Main category: cs.IR

TL;DR: 本文提出了两种统计模型方法，用于在eBay商品页面智能展示信号片段，通过A/B测试验证了这些方法能显著提升业务指标。


<details>
  <summary>Details</summary>
Motivation: 在eBay商品详情页展示信号片段，为用户提供额外的上下文信息，旨在促进智能购买决策和激励用户参与。

Method: 开发了两种统计模型方法，用于优化商品页面中信号片段的展示布局和内容。

Result: 两种方法都进行了A/B测试，结果显示业务指标有显著提升。

Conclusion: 通过统计模型优化信号展示策略能有效改善用户体验并提升业务表现。
...
### [141] [MetaSynth: Multi-Agent Metadata Generation from Implicit Feedback in Black-Box Systems](https://arxiv.org/abs/2510.01523)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在优化元标题和描述时存在多样性不足、产生幻觉属性或忽略历史成功案例的问题，无法有效利用可观察结果的隐式信号。

Method: 构建从顶级排名结果中提取的范例库，基于产品内容和范例生成候选片段，并通过评估器-生成器循环迭代优化输出的相关性、推广强度和合规性。

Result: 在专有电子商务数据和Amazon评论语料库上，MetaSynth在NDCG、MRR和排名指标上均优于强基线方法，大规模A/B测试显示点击率提升10.26%，点击量提升7.51%。

Conclusion: 这项工作不仅改进了元数据优化，还贡献了一个在黑盒系统中使用隐式信号优化内容的通用范式。
...
### [142] [IoDResearch: Deep Research on Private Heterogeneous Data via the Internet of Data](https://arxiv.org/abs/2510.01553)
*Zhuofan Shi,Zijie Guo,Xinjian Ma,Gang Huang,Yun Ma,Xiang Jing*

Main category: cs.IR

TL;DR: 提出IoDResearch框架，专注于私有数据的深度研究，通过将异构资源封装为FAIR合规的数字对象，构建多粒度检索的异构图索引，并利用多智能体系统支持可靠问答和结构化科学报告生成。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究框架主要关注网络搜索，忽视了本地私有数据，导致私有数据检索效率低且不符合FAIR原则，造成效率低下和可重用性有限的问题。

Method: 将异构资源封装为FAIR合规数字对象，细化为原子知识单元和知识图谱，形成异构图索引用于多粒度检索，并构建多智能体系统支持问答和报告生成。

Result: 在检索、问答和报告撰写任务上的实验结果表明，IoDResearch始终优于代表性的RAG和深度研究基线方法。
...
### [143] [Synthetic Prefixes to Mitigate Bias in Real-Time Neural Query Autocomplete](https://arxiv.org/abs/2510.01574)
*Adithya Rajan,Xiaoyu Liu,Prateek Verma,Vibhu Arora*

Main category: cs.IR

TL;DR: 提出一种数据中心的解决方案，通过使用从完整用户查询生成的合成前缀来减轻实时神经查询自动补全系统中的呈现偏差，从而改善排名模型的训练数据多样性。
...
Method: 从无自动补全的常规搜索会话中收集完整用户查询生成合成前缀，优化神经排名器以适应严格延迟约束，引入简化的列表损失函数降低计算复杂度。

Result: 在大规模电商环境中部署，通过平均倒数排名等指标显示用户参与度有统计显著提升。

Conclusion: 合成前缀不仅能提高泛化能力，还为其他低延迟排名任务中的偏差缓解提供了可扩展路径。
...
### [144] [Bridging Collaborative Filtering and Large Language Models with Dynamic Alignment, Multimodal Fusion and Evidence-grounded Explanations](https://arxiv.org/abs/2510.01606)
*Bo Ma,LuYao Liu,Simon Lau,Chandler Yuan,and XueY Cui,Rosie Zhang*

Main category: cs.IR

TL;DR: 提出了一个名为\model{}的推荐框架，通过在线适应机制、统一多模态表示和基于证据的解释系统，解决了传统推荐系统中的静态数据、模态缺失和可信度问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的推荐系统存在三个主要问题：使用静态数据无法捕捉快速变化的用户偏好；缺乏对视觉和音频等多模态内容的处理；无法提供基于具体证据的可信解释。

Method: 1. 在线适应机制：通过轻量级模块持续整合新用户交互，避免重新训练大模型
2. 统一表示：将协同过滤信号与视觉、音频特征无缝结合，处理模态缺失情况
3. 解释系统：基于具体协同模式和物品属性生成可验证的自然语言解释

Result: 该框架保持了基础模型的效率，同时只增加了最小的计算开销，适合实际部署。

Conclusion: \model{}框架通过三个关键创新有效解决了当前推荐系统的局限性，在保持效率的同时提升了推荐的准确性和可信度。
...
### [145] [LLM4Rec: Large Language Models for Multimodal Generative Recommendation with Causal Debiasing](https://arxiv.org/abs/2510.01622)
*Bo Ma,Hang Li,ZeHua Hu,XiaoFan Gui,LuYao Liu,Simon Lau*

Main category: cs.IR

TL;DR: 提出增强生成推荐框架，通过多模态融合、检索增强生成、因果推断去偏、可解释推荐和实时自适应学习五大创新，在准确率、公平性和多样性方面显著优于现有方法。
...
Method: 基于大语言模型，集成多模态融合架构、检索增强生成机制、因果推断去偏、可解释推荐生成和实时自适应学习能力。

Result: 在三个基准数据集上，NDCG@10提升2.3%，多样性指标提升1.4%，同时保持计算效率。

Conclusion: 该框架有效解决了多模态数据处理、偏见消除和决策透明度等关键问题，为生成推荐系统提供了全面解决方案。
...
### [146] [TalkPlay-Tools: Conversational Music Recommendation with LLM Tool Calling](https://arxiv.org/abs/2510.01698)
*Seungheon Doh,Keunwoo Choi,Juhan Nam*

Main category: cs.IR

TL;DR: 提出基于LLM的音乐推荐系统，通过工具调用统一检索-重排流程，支持多种检索方法（布尔过滤、稀疏检索、密集检索、生成检索）的智能组合使用。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐系统主要依赖自然语言交互，但忽视了元数据过滤等关键组件，限制了推荐行为的多样性。

Method: 将LLM定位为端到端推荐系统，通过工具规划预测工具类型、执行顺序和参数，协调专用组件（SQL布尔过滤器、BM25稀疏检索、嵌入相似性密集检索、语义ID生成检索）。

Result: 该统一工具调用框架在多样化推荐场景中实现竞争性性能，能根据用户查询选择性使用合适的检索方法。

Conclusion: 为对话式音乐推荐系统开创了新范式，通过无缝集成多种数据库过滤方法支持多样化模态。
...
### [147] [Ranking Items from Discrete Ratings: The Cost of Unknown User Thresholds](https://arxiv.org/abs/2510.01871)
*Oscar Villemaud,Suryanarayana Sankagiri,Matthias Grossglauser*

Main category: cs.IR

TL;DR: 该论文研究了从粗粒度用户评分中恢复细粒度物品排序的问题，证明了需要Θ(n²)用户才能实现近乎完美的排序，这比基于比较的排序所需的O(n log n)查询要多得多。


<details>
  <summary>Details</summary>
Motivation: 在信息检索和推荐系统中，用户输入通常采用粗粒度离散评分形式，但系统需要恢复细粒度的物品排序。研究旨在探索从粗粒度评分中恢复细粒度排序的可能性。

Method: 将物品建模为具有分数，用户建模为具有阈值；当物品分数超过用户阈值时，用户给予正面评价。用户按顺序到达，每个新用户可以被查询以改进当前排序。

Result: 证明了实现近乎完美的排序（以Spearman距离衡量）需要Θ(n²)用户，这显著多于基于比较排序所需的O(n log n)查询。差异反映了识别具有适当阈值用户所需的额外查询。
...
### [148] [Contrastive Retrieval Heads Improve Attention-Based Re-Ranking](https://arxiv.org/abs/2510.02219)
*Linh Tran,Yulong Li,Radu Florian,Wei Sun*

Main category: cs.IR

TL;DR: 提出CoRe heads方法，通过对比评分指标识别出少量对检索任务关键的注意力头，显著提升重排序性能，同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的重排序方法中，许多注意力头会产生噪声和冗余，限制了性能提升。需要找到真正对检索任务有判别力的注意力头。

Method: 使用对比评分指标识别CoRe heads，该指标奖励与相关文档相关性高的注意力头，抑制与不相关文档相关性高的注意力头。

Result: 实验表明，仅使用不到1%的注意力头就能显著提升重排序准确率，且这些头集中在中层。修剪后50%层可保持精度同时大幅减少推理时间和内存使用。

Conclusion: CoRe heads方法能有效识别对检索任务关键的注意力头，在提升重排序性能的同时显著降低计算成本。
...
### [149] [Study on LLMs for Promptagator-Style Dense Retriever Training](https://arxiv.org/abs/2510.02241)
*Daniel Gwon,Nour Jedidi,Jimmy Lin*

Main category: cs.IR

TL;DR: 研究发现开源的小型LLM（最小3B参数）可以作为有效的Promptagator式查询生成器，替代原本需要专有大模型的方法。


<details>
  <summary>Details</summary>
Motivation: 原Promptagator方法依赖专有的大规模LLM，用户可能无法访问或无法用于敏感数据，需要寻找开源替代方案。

Method: 使用参数规模≤14B的开源LLM作为Promptagator式查询生成器，研究其在不同规模下的表现。

Result: 开源LLM小至3B参数就能有效生成查询，为领域专用密集检索模型的微调提供可靠替代方案。

Conclusion: 开源小型LLM是可行的Promptagator替代方案，为领域特定应用的微调提供了实用见解。
...
### [150] [Learning Regularization Functionals for Inverse Problems: A Comparative Study](https://arxiv.org/abs/2510.01755)
...
Result: 统一视角使得能够系统比较不同方法，突出它们的优势和局限性。

Conclusion: 这项工作为理解各种学习正则化方法的未来潜力提供了有价值的见解，并提供了实用的指导方针。
...
### [151] [Accelerating Long-Term Molecular Dynamics with Physics-Informed Time-Series Forecasting](https://arxiv.org/abs/2510.01206)
*Hung Le,Sherif Abbas,Minh Hoang Nguyen,Van Dai Do,Huu Hiep Nguyen,Dung Nguyen*

Main category: cs.LG

TL;DR: 将分子动力学模拟重新定义为时间序列预测问题，通过位移预测而非绝对位置来预测原子轨迹，结合物理约束的损失函数确保物理合理性。
...
Method: 采用时间序列预测框架，结合基于DFT参数化对势函数的物理约束损失函数和推理机制，惩罚不合理的原子接近距离。

Result: 在多种材料上持续超越标准基线方法，实现了数千步分子动力学模拟的稳定建模，计算时间仅需几分钟。
...
### [152] [Neural non-canonical Hamiltonian dynamics for long-time simulations](https://arxiv.org/abs/2510.01788)
...
<details>
  <summary>Details</summary>
Motivation: 当结合基于势能的架构和退化变分积分器时，学习到的模型由于方案的规范依赖性而出现数值不稳定，导致长时间模拟无法进行。

Method: 提出两种训练策略：直接学习向量场，或通过方案学习时间离散动力学。

Result: 通过多个数值测试案例验证了方法学习复杂物理动力学的能力，如陀螺动力学等离子体物理中的引导中心。

Conclusion: 成功解决了结合哈密顿动力学学习和数值方案时的稳定性问题，使长时间模拟成为可能。
...
### [153] [Control the Temperature: Selective Sampling for Diverse and High-Quality LLM Outputs](https://arxiv.org/abs/2510.01218)
...
<details>
  <summary>Details</summary>
Motivation: 传统的高温度采样方法在需要高精度的任务（如数学推理）中会降低推理质量，因为会在敏感的解码位置采样错误的延续。

Method: 训练一个轻量级分类器来预测采样风险，基于风险指标动态选择贪婪采样或高温度采样，最小化延迟开销。

Result: 在数学推理任务上的实验表明，选择性采样即使在高温设置下也能改善质量-多样性权衡。

Conclusion: 选择性采样方法有效解决了高精度任务中温度采样导致的准确性问题，实现了更好的多样性和准确性平衡。
...
### [154] [Automated Extraction of Material Properties using LLM-based AI Agents](https://arxiv.org/abs/2510.01235)
*Subham Ghosh,Abhishek Tewari*

Main category: cs.LG

TL;DR: 开发了一个基于大语言模型的自动化工作流，从10,000篇科学文献中提取热电和结构性能数据，构建了包含27,822条温度分辨记录的数据库，是迄今为止最大的LLM整理的热电数据集。


<details>
  <summary>Details</summary>
Motivation: 材料发现受限于缺乏大规模、机器可读的数据集，现有数据库规模小、手动整理或偏向第一性原理结果，实验文献未被充分利用。

Method: 采用动态令牌分配、零样本多智能体提取和条件表解析的LLM驱动工作流，平衡准确性和计算成本。GPT-4.1和GPT-4.1 Mini在基准测试中表现最佳。

Result: 构建了包含热电优值(ZT)、塞贝克系数、电导率、电阻率、功率因子和热导率等性能指标，以及晶体类别、空间群和掺杂策略等结构属性的数据集。数据分析重现了已知热电趋势。

Conclusion: 提供了可复现且成本分析的数据提取流程，为超越热电领域的可扩展、数据驱动的材料发现奠定了基础，并发布了交互式网络浏览器。
...
### [155] [RSAVQ: Riemannian Sensitivity-Aware Vector Quantization for Large Language Models](https://arxiv.org/abs/2510.01240)
*Zukang Xu,Xing Hu,Qiang Wu,Dawei Yang*

Main category: cs.LG

TL;DR: RSAVQ是一种用于大语言模型极低比特量化的新型向量量化框架，通过几何驱动创新解决方向误差和比特分配问题，在2-4比特量化中表现优异。
...
Method: 提出RSAVQ框架，包含两个几何驱动创新：1）误差方向敏感度指导(EDSG)，利用Fisher信息矩阵诱导的黎曼度量将量化误差投影到参数空间低敏感方向；2）权重通道敏感度指导(WCSG)，通过FIM曲率分析构建通道级敏感度度量来动态指导比特资源分配。
...
### [156] [Adaptive Federated Learning Defences via Trust-Aware Deep Q-Networks](https://arxiv.org/abs/2510.01261)
*Vedant Palit*

Main category: cs.LG

TL;DR: 提出了一种基于信任感知深度Q网络的联邦学习防御方法，通过多信号证据集成和序列决策来对抗部分可观测性下的投毒和后门攻击。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在部分可观测环境下容易受到投毒攻击和后门攻击，需要设计能够整合多信号证据并优化长期鲁棒性-准确性目标的防御机制。

Method: 将防御建模为部分可观测序列决策问题，引入信任感知深度Q网络，集成多信号证据进行客户端信任更新，同时优化长期鲁棒性-准确性目标。

Result: 在CIFAR-10上：(1)建立准确率稳步提升的基线；(2)Dirichlet扫描显示增加客户端重叠持续提升准确率并降低攻击成功率，检测稳定性良好；(3)信号预算研究表明准确率保持稳定而攻击成功率上升、ROC-AUC下降，表明序列信念更新可缓解弱信号问题；(4)与随机、线性Q和政策梯度控制器比较，DQN获得最佳鲁棒性-准确性权衡。

Conclusion: 信任感知深度Q网络能够有效应对联邦学习中的部分可观测性挑战，通过序列信念更新和多信号集成实现良好的鲁棒性-准确性平衡。
...
### [157] [RSTGCN: Railway-centric Spatio-Temporal Graph Convolutional Network for Train Delay Prediction](https://arxiv.org/abs/2510.01262)
*Koyena Chowdhury,Paramita Koley,Abhijnan Chakraborty,Saptarshi Ghosh*

Main category: cs.LG

TL;DR: 提出RSTGCN模型预测铁路车站平均到站延误，使用空间注意力机制，并发布了印度铁路网络数据集。
...
Result: 在印度铁路网络数据集上进行了广泛实验，相比现有基线方法在标准指标上取得了持续改进。
...
### [158] [Budgeted Broadcast: An Activity-Dependent Pruning Rule for Neural Network Efficiency](https://arxiv.org/abs/2510.01263)
*Yaron Meirovitch,Fuming Yang,Jeff Lichtman,Nir Shavit*

Main category: cs.LG

TL;DR: 提出Budgeted Broadcast (BB)剪枝方法，通过局部流量预算（激活率与扇出的乘积）来平衡选择性和传播范围，使用简单的局部执行器进行剪枝，在多种网络结构中提高编码熵和去相关性，有时甚至超过密集基线的准确率。


<details>
  <summary>Details</summary>
Motivation: 大多数剪枝方法基于对损失的影响（如幅度或梯度）来移除参数，但这种方法可能不够高效。BB方法旨在通过局部流量预算来优化网络表示，实现更好的选择性和传播平衡。

Method: BB为每个单元分配局部流量预算（激活率a_i与扇出k_i的乘积），通过约束熵分析得出选择性-受众平衡公式，并使用局部执行器剪枝扇入（降低激活率）或扇出（减少传播）。

Result: 在ASR Transformer、人脸识别ResNet和突触预测3D U-Net等网络中，BB在相同稀疏度下提高了编码熵、去相关性和准确率，有时超过密集基线。在电子显微镜图像上实现了最先进的F1和PR-AUC。

Conclusion: BB方法易于集成，为学习更多样化和高效的表征提供了一条路径，通过局部流量预算实现了更好的网络剪枝效果。
...
### [159] [A Framework for Scalable Heterogeneous Multi-Agent Adversarial Reinforcement Learning in IsaacLab](https://arxiv.org/abs/2510.01264)
...
<details>
  <summary>Details</summary>
Motivation: 虽然先前研究主要关注协作环境，但对抗性交互对于现实世界应用（如追逃、安全和竞争性操作）同样至关重要。
...
Conclusion: 该工作为对抗性多智能体强化学习提供了可扩展的训练平台，支持异构智能体在物理模拟环境中的竞争性交互。
...
### [160] [RLP: Reinforcement as a Pretraining Objective](https://arxiv.org/abs/2510.01265)
...
TL;DR: RLP是一种信息驱动的强化预训练目标，将强化学习的探索精神引入预训练阶段，通过奖励思维链提供的信息增益来鼓励模型独立思考，显著提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前主流训练范式仅在最后阶段使用强化学习，作者质疑这是否是最优方式，希望将强化学习的探索精神更早地引入预训练过程。

Method: 将思维链视为探索性动作，基于思维链为预测未来token提供的信息增益计算奖励信号，在预训练阶段使用密集奖励进行训练。

Result: 在Qwen3-1.7B-Base上预训练后，数学和科学基准套件的平均性能提升19%；在Nemotron-Nano-12B-v2上，整体平均从42.81%提升到61.32%，科学推理提升23%。

Conclusion: RLP成功将强化学习重新定义为普通文本的预训练目标，弥合了下一个token预测与有用思维链推理之间的差距，展示了跨架构和模型大小的可扩展性。
...
### [161] [Safe Reinforcement Learning-Based Vibration Control: Overcoming Training Risks with LQR Guidance](https://arxiv.org/abs/2510.01269)
...
<details>
  <summary>Details</summary>
Motivation: 解决结构振动控制中传统模型依赖和RL训练期间的安全风险问题。

Method: 使用基于随机模型的LQR控制器引导RL控制器训练，形成混合控制框架。

Result: 该混合方法在保持模型无关性的同时，显著降低了RL训练期间的结构风险。

Conclusion: 提出的混合框架是首个解决RL振动控制训练安全挑战的有效方案。
...
### [162] [Identifying Information-Transfer Nodes in a Recurrent Neural Network Reveals Dynamic Representations](https://arxiv.org/abs/2510.01271)
*Arend Hintze,Asadullah Najam,Jory Schossau*

Main category: cs.LG

TL;DR: 提出了一种基于信息论的方法来识别和分析RNN中的信息中继节点，通过量化节点间输入输出的互信息来揭示信息流动的关键路径，并在多种RNN架构上验证了方法的有效性。
...
### [163] [Noisy-Pair Robust Representation Alignment for Positive-Unlabeled Learning](https://arxiv.org/abs/2510.01278)
*Hengwei Zhao,Zhengzhong Tu,Zhuo Zheng,Wei Wang,Junjue Wang,Rusty Feagin,Wenzhe Jiao*

Main category: cs.LG

TL;DR: 提出NcPU框架，通过非对比学习解决PU学习中因监督不可靠导致的表征学习瓶颈问题，在多个数据集上显著优于现有方法
...
Method: 结合噪声对鲁棒的非对比监督损失（NoiSNCL）和幻影标签消歧（PLD）方案，前者在不可靠监督下对齐类内表征，后者通过基于遗憾的标签更新提供保守负监督

Result: NoiSNCL使简单PU方法达到竞争性能；NcPU在多样化数据集上显著优于最先进PU方法，包括灾后建筑损伤映射等实际应用

Conclusion: NcPU框架无需辅助信息即可有效解决PU学习中的表征学习瓶颈，理论分析和实证结果均验证了其有效性，具有实际应用前景
...
### [164] [Microsaccade-Inspired Probing: Positional Encoding Perturbations Reveal LLM Misbehaviours](https://arxiv.org/abs/2510.01288)
*Rui Melo,Rui Abreu,Corina S. Pasareanu*

Main category: cs.LG

TL;DR: 受微眼动启发，提出一种轻量级位置编码扰动方法，无需微调即可检测LLM在事实性、安全性、毒性等方面的故障行为。


<details>
  <summary>Details</summary>
Motivation: 从人类视觉感知中的微眼动现象获得灵感，微眼动能揭示隐藏的感知动态，类似地希望通过轻量级扰动来揭示LLM的潜在故障信号。

Method: 使用轻量级位置编码扰动作为探测手段，无需微调或任务特定监督，通过扰动引发模型内部潜在信号来指示错误行为。

Result: 在多个先进LLM上的实验表明，该方法能有效检测事实性、安全性、毒性及后门攻击等故障，同时保持计算高效性。

Conclusion: 预训练LLM已编码了标记自身故障的内部证据，微眼动启发的干预为检测和缓解不良行为提供了可行路径。
...
### [165] [ThinKV: Thought-Adaptive KV Cache Compression for Efficient Reasoning Models](https://arxiv.org/abs/2510.01290)
*Akshat Ramachandran,Marina Neseem,Charbel Sakr,Rangharajan Venkatesan,Brucek Khailany,Tushar Krishna*

Main category: cs.LG

TL;DR: ThinKV是一个思想自适应的KV缓存压缩框架，通过混合量化-驱逐策略，在保持接近无损精度的同时将KV缓存减少到原始大小的5%以下，推理吞吐量提升高达5.8倍。
...
Method: 基于注意力稀疏性揭示思维链中不同重要性的思想类型，采用混合量化-驱逐策略，根据思想重要性分配token精度，并随着推理轨迹演进逐步驱逐次要思想的token。

Result: 在数学和编程基准测试中，ThinKV使用不到原始KV缓存5%的大小实现了接近无损的精度，推理吞吐量比最先进基线提高了5.8倍。

Conclusion: ThinKV通过思想自适应压缩有效解决了大型推理模型KV缓存的内存瓶颈问题，在保持精度的同时显著提升性能。
...
### [166] [Network-Level Vehicle Delay Estimation at Heterogeneous Signalized Intersections](https://arxiv.org/abs/2510.01292)
*Xiaobo Ma,Hyunsoo Noh,James Tokishi,Ryan Hatch*

Main category: cs.LG

TL;DR: 提出一种基于域适应的车辆延误估计框架GBBW，通过重新加权源域数据来提高模型在不同交叉口的泛化能力，在57个异质交叉口数据上验证了其优越性能。
...
Method: 使用域适应框架将数据分为源域和目标域，提取关键交通特征，利用目标域的少量标注数据微调模型，并提出GBBW模型基于与目标域的相似度重新加权源域数据。

Result: 在57个异质交叉口数据上测试，GBBW框架相比8种先进ML回归模型和7种基于实例的域适应方法，提供了更准确和鲁棒的延误估计。
...
### [167] [From 2D to 3D, Deep Learning-based Shape Reconstruction in Magnetic Resonance Imaging: A Review](https://arxiv.org/abs/2510.01296)
*Emma McMillian,Abhirup Banerjee,Alfonso Bueno-Orovio*

Main category: cs.LG

TL;DR: 本文综述了基于深度学习的2D MRI到3D形状重建方法，涵盖点云、网格、形状感知和体积模型四类方法，分析了技术现状、局限性和临床应用，并探讨了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 3D MRI重建在医学疾病诊断、治疗规划和计算建模中日益重要，需要系统梳理当前方法现状以推动更鲁棒、通用和临床有效的解决方案。

Method: 系统综述方法，分析四类主要重建方法：点云模型、网格模型、形状感知模型和体积模型，涵盖技术基础、局限性和应用场景。

Result: 提供了从心脏到神经到肺部成像的广泛概述，分析了模型在病变解剖中的临床适用性，以及训练测试数据的影响，考察了公开数据集、计算需求和评估指标。

Conclusion: 强调了多模态集成和跨模态框架等新兴研究方向，为研究人员提供结构化概览以识别推进深度学习在3D重建中发展的机会。
...
### [168] [Low Rank Gradients and Where to Find Them](https://arxiv.org/abs/2510.01303)
*Rishi Sonthalia,Michael Murray,Guido Montúfar*

Main category: cs.LG

TL;DR: 该论文研究了两层神经网络训练损失梯度中的低秩结构，放宽了训练数据和参数的各向同性假设。在尖峰数据模型中，证明了输入权重梯度近似低秩，由两个秩一成分主导：一个与数据残差对齐，另一个与输入数据中的秩一尖峰对齐。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络梯度中的低秩结构，但放宽传统各向同性假设，考虑更一般的各向异性和病态数据模型，以更真实地理解梯度特性。
...
Conclusion: 标准正则化器（如权重衰减、输入噪声和雅可比惩罚）选择性地调节这两个梯度成分。理论和实验验证了这些发现。
...
### [169] [Quantum-inspired Benchmark for Estimating Intrinsic Dimension](https://arxiv.org/abs/2510.01335)
...
<details>
  <summary>Details</summary>
Motivation: 现有IDE方法在真实数据集上估计结果差异很大，需要更复杂的基准来评估这些方法。流形假设认为数据位于低维潜在流形上，但现有基准流形过于简单。

Method: 基于量子光学方法构建QuIIEst基准，嵌入任意齐次空间，允许曲率修改和加性噪声。测试多种IDE方法在复杂流形上的性能。

Result: 在相同资源分配下，IDE方法在QuIIEst流形上的准确性普遍低于现有基准。非均匀曲率增加时性能下降最小，表明基准本身具有挑战性。

Conclusion: QuIIEst基准揭示了现有IDE方法在复杂流形上的局限性，为评估IDE方法提供了更现实的测试环境。
...
### [170] [On the Identifiability of Latent Action Policies](https://arxiv.org/abs/2510.01337)
...
Method: 正式描述动作表示的期望特性，分析统计效益和不可识别性来源，证明熵正则化的LAPO目标在适当条件下能够识别满足期望特性的动作表示。

Result: 证明了在合适条件下，熵正则化的LAPO目标能够识别出满足期望特性的动作表示，为离散动作表示在实践中表现良好提供了理论解释。

Conclusion: 熵正则化的LAPO目标能够识别满足期望特性的动作表示，这解释了为什么离散动作表示在实践中表现良好，为潜在动作策略学习提供了理论基础。
...
### [171] [Self-Supervised Representation Learning as Mutual Information Maximization](https://arxiv.org/abs/2510.01345)
...
<details>
  <summary>Details</summary>
Motivation: 自监督表示学习虽然取得了显著经验成功，但其基本原理理解不足。现有工作通常将预测器网络、停止梯度操作等架构元素视为经验性添加，缺乏理论依据。

Method: 从变分互信息下界出发，推导出两种训练范式：SDMI（需要交替优化和停止梯度操作）和JMI（允许联合优化和对称架构）。预测器网络和统计正则化器分别作为两种范式中互信息目标的可行替代。

Result: 证明了现有许多自监督表示学习方法都是这两种范式的具体实例或近似，为不同架构组件的选择提供了理论解释。

Conclusion: 该工作为自监督表示学习的架构设计提供了基于第一性原理的理论基础，超越了启发式便利的解释，统一了现有方法的理论框架。
...
### [172] [To Augment or Not to Augment? Diagnosing Distributional Symmetry Breaking](https://arxiv.org/abs/2510.01349)
...
<details>
  <summary>Details</summary>
Motivation: 对称感知方法（如数据增强和等变架构）假设变换后的数据点在测试分布中具有高概率，但这一假设需要验证。本文旨在开发评估该假设的方法。

Method: 提出通过双样本神经网络分类器测试来量化数据集各向异性程度的指标，区分原始数据集与其随机增强等价版本。在合成数据集上验证指标，并应用于基准点云数据集。

Result: 在多个基准点云数据集中发现了出乎意料的高度对齐；理论证明分布对称性破缺会阻止不变方法达到最优性能；实证发现等变方法在部分各向异性数据集上仍有优势。

Conclusion: 理解等变性需要重新思考数据中的对称性偏差，等变方法的有效性取决于具体数据集特性。
...
### [173] [RheOFormer: A generative transformer model for simulation of complex fluids and flows](https://arxiv.org/abs/2510.01365)
...
Method: 使用自注意力机制的生成算子学习方法，学习复杂流体流动的空间相互作用和特征，在多种流变测量和非流变测量流动中进行基准测试。

Result: RheOFormer能够准确学习不同复杂流体的标量和张量非线性力学，预测其流动的时空演化，即使在有限数据集上训练也能表现出色。

Conclusion: RheOFormer具有强大的泛化能力和计算效率，是加速预测性复杂流体模拟、推进数据驱动实验和实现实时过程优化的稳健神经代理模型。
...
### [174] [Selective Underfitting in Diffusion Models](https://arxiv.org/abs/2510.01378)
*Kiwhan Song,Jaeyeon Kim,Sitan Chen,Yilun Du,Sham Kakade,Vincent Sitzmann*

Main category: cs.LG

TL;DR: 扩散模型通过选择性欠拟合来平衡训练数据拟合与生成新颖样本的能力，在输入空间的特定区域更准确地学习分数函数，而在其他区域则欠拟合。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型学习何种分数函数的基本问题，传统观点认为扩散模型由于训练时的归纳偏置而欠拟合经验分数，但本文提出更精细的选择性欠拟合概念。

Method: 通过表征输入空间中的特定区域，并设计经验干预来验证选择性欠拟合的观点。

Result: 研究发现更好的扩散模型在特定区域更准确地近似分数函数，而在其他区域则欠拟合，这种选择性欠拟合对于理解扩散模型至关重要。

Conclusion: 选择性欠拟合为理解扩散模型的泛化和生成性能提供了新的可测试见解，是理解其工作机制的关键概念。
...
### [175] [Fine-Tuning Masked Diffusion for Provable Self-Correction](https://arxiv.org/abs/2510.01384)
*Jaeyeon Kim,Seunggeun Kim,Taekyun Lee,David Z. Pan,Hyeji Kim,Sham Kakade,Sitan Chen*

Main category: cs.LG

TL;DR: PRISM是一种轻量级的自校正方法，适用于任何预训练的掩码扩散模型，通过在推理时检测和修正低质量标记来提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码扩散模型缺乏有效的自校正能力，要么需要重新设计架构/训练，要么依赖不精确的标记质量代理，限制了其适用性。

Method: PRISM通过定义自校正损失来学习每个标记的质量分数，无需强化学习或验证器，在MDM的前向传播中计算质量分数并检测低质量标记。

Result: PRISM在多个领域和规模上提升了MDM推理性能：数独游戏、无条件文本生成（170M参数）以及代码生成（LLaDA，8B参数）。

Conclusion: PRISM为掩码扩散模型提供了一种模型无关的轻量级自校正方法，能够有效提高生成质量，具有广泛的应用潜力。
...
### [176] [Optimal Stopping vs Best-of-$N$ for Inference Time Optimization](https://arxiv.org/abs/2510.01394)
*Yusuf Kalayci,Vinod Raman,Shaddin Dughmi*

Main category: cs.LG

TL;DR: 提出基于Pandora's Box问题的新框架，用于LLM推理时优化，通过UCB算法自适应决定停止生成时机，相比传统Best-of-N采样可减少15-35%的生成次数。


<details>
  <summary>Details</summary>
Motivation: LLM生成需要在输出质量和推理成本之间取得平衡，特别是使用多次生成时，需要优化停止时机以降低计算开销。

Method: 将每次生成视为打开一个代价高昂的"盒子"，开发UCB风格的Pandora's Box算法，结合Bradley-Terry变换处理跨提示的奖励缩放，实现自适应推理时优化。
...
### [177] [Neural Network Surrogates for Free Energy Computation of Complex Chemical Systems](https://arxiv.org/abs/2510.01396)
*Wasut Pornpatcharapong*

Main category: cs.LG

TL;DR: 提出了一种神经网络替代框架，可直接从笛卡尔坐标学习集体变量，并通过自动微分提供雅可比矩阵，绕过了传统方法对解析形式的依赖。
...
Result: 在MgCl2离子配对系统中，该方法对简单距离CV和复杂配位数CV都实现了高精度，且雅可比误差接近高斯分布，适用于GPR流程。
...
### [178] [Ultra-Efficient Decoding for End-to-End Neural Compression and Reconstruction](https://arxiv.org/abs/2510.01407)
*Ethan G. Rogers,Cheng Wang*

Main category: cs.LG

TL;DR: 提出基于低秩表示和向量量化的神经压缩重建框架，显著降低解码器计算复杂度


<details>
  <summary>Details</summary>
Motivation: 解决当前神经压缩方法中卷积解码器复杂度高、计算成本大的瓶颈问题

Method: 在带有向量量化的自编码器中引入低秩表示，通过对学习到的潜在表示执行高效低秩操作来重建图像

Result: 大幅减少神经压缩/重建解码阶段的计算开销，基本消除解码器计算瓶颈，同时保持高质量图像输出

Conclusion: 该框架有效解决了神经压缩中的解码器瓶颈问题，为实际应用提供了更高效的解决方案
...
### [179] [Edge Artificial Intelligence: A Systematic Review of Evolution, Taxonomic Frameworks, and Future Horizons](https://arxiv.org/abs/2510.01439)
*Mohamad Abou Ali,Fadi Dornaika*

Main category: cs.LG

TL;DR: 这篇综述论文系统性地分析了边缘人工智能(Edge AI)的发展历程、当前现状和未来方向，涵盖了部署位置、处理能力、应用领域和硬件类型等多个维度。


<details>
  <summary>Details</summary>
Motivation: 边缘AI通过在网络边缘设备中嵌入智能，实现实时处理、提升隐私保护和降低延迟。本文旨在全面梳理该领域的发展脉络和技术现状。

Method: 遵循PRISMA指南，采用多维度分类法分析边缘AI，包括部署位置、处理能力（如TinyML和联邦学习）、应用领域和硬件类型等。

Result: 分析了从早期内容分发网络和雾计算到现代设备端智能的演进历程，探讨了专用硬件加速器、优化软件和通信协议等核心技术。

Conclusion: 识别了资源限制、安全、模型管理、功耗和连接性等挑战，并强调了神经形态硬件、持续学习算法、边云协作和可信集成等新兴机遇，为研究者和从业者提供了全面框架。
...
### [180] [SoftAdaClip: A Smooth Clipping Strategy for Fair and Private Model Training](https://arxiv.org/abs/2510.01447)
*Dorsa Soleymani,Ali Dadsetan,Frank Rudzicz*

Main category: cs.LG

TL;DR: SoftAdaClip是一种差分隐私训练方法，用基于tanh的平滑变换替代硬裁剪，在保持梯度相对大小的同时限制敏感度，显著减少子群体差异。
...
Method: 提出SoftAdaClip方法，用平滑的tanh变换替代硬裁剪，保持梯度相对大小同时限制敏感度，结合自适应机制实现公平的私有模型训练。

Result: 在多个数据集上评估显示，SoftAdaClip相比DP-SGD减少子群体差异达87%，相比Adaptive-DPSGD减少达48%，且这些减少具有统计显著性。

Conclusion: 将平滑变换与自适应机制结合对于实现公平和私有的模型训练至关重要。
...
### [181] [Local Linear Attention: An Optimal Interpolation of Linear and Softmax Attention For Test-Time Regression](https://arxiv.org/abs/2510.01450)
*Yifei Zuo,Yutong Yin,Zhichen Zeng,Ang Li,Banghua Zhu,Zhaoran Wang*

Main category: cs.LG

TL;DR: 提出了Local Linear Attention (LLA)，一种基于非参数统计理论的新型注意力机制，通过测试时回归视角推导，在理论分析和实际性能上都优于线性注意力和Softmax注意力。
...
Method: 从非参数统计的测试时回归视角推导出LLA，提出两种内存高效原语解决计算复杂度问题，开发FlashLLA硬件高效块状算法，并实现定制推理内核减少内存开销。

Result: LLA在测试时回归、上下文回归、关联回忆和状态跟踪任务中有效适应非平稳性，在测试时训练和上下文学习中优于强基线，并展现出在大规模模型中可扩展性和适用性的有希望证据。
...
### [182] [SCOPED: Score-Curvature Out-of-distribution Proximity Evaluator for Diffusion](https://arxiv.org/abs/2510.01456)
*Brett Barkley,Preston Culbertson,David Fridovich-Keil*

Main category: cs.LG

TL;DR: SCOPED是一种快速通用的扩散模型OOD检测方法，通过结合雅可比矩阵迹和得分函数平方范数，大幅减少前向传播次数，在视觉和机器人任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法计算成本高，需要可靠且高效的OOD检测来确保机器学习系统在现实世界中的安全部署。
...
Conclusion: SCOPED为现实世界领域提供了快速可靠的OOD检测实用基础，适用于视觉感知异常、自回归模型异常检测、强化学习探索等场景。
...
### [183] [Fixing That Free Lunch: When, Where, and Why Synthetic Data Fails in Model-Based Policy Optimization](https://arxiv.org/abs/2510.01457)
*Brett Barkley,David Fridovich-Keil*

Main category: cs.LG

TL;DR: 该论文分析了基于模型的强化学习算法MBPO在DeepMind Control Suite中表现不佳的原因，识别了动态模型与奖励模型尺度不匹配、目标表示选择不当两个关键问题，并提出解决方案使MBPO在多数任务中超越模型自由算法SAC。


<details>
  <summary>Details</summary>
Motivation: 研究MBPO算法在OpenAI Gym中表现优异但在DeepMind Control Suite中表现不佳的原因，揭示环境特定假设如何隐式影响算法设计，并解决由此导致的性能下降问题。

Method: 识别MBPO失败的两个关键问题：动态模型与奖励模型的尺度不匹配导致评论家低估，以及目标表示选择不当增加模型方差。通过解决这些问题来改进MBPO算法。

Result: 解决识别的问题后，MBPO在7个DeepMind Control Suite任务中的5个任务上超越了SAC算法，同时保持了在OpenAI Gym中的优异表现。

Conclusion: 研究强调需要开发将MDP任务和环境结构与算法失败模式联系起来的分类法，追求统一解决方案，并澄清基准选择如何影响算法泛化条件。
...
### [184] [How Well Can Preference Optimization Generalize Under Noisy Feedback?](https://arxiv.org/abs/2510.01458)
*Shawn Im,Yixuan Li*

Main category: cs.LG

TL;DR: 本文研究了噪声反馈对大型语言模型偏好优化的影响，提供了在噪声条件下的泛化保证，分析了不同类型噪声对泛化的影响，并验证了理论发现在实际LLM训练中的相关性。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法大多假设无噪声反馈，但现实中人类反馈存在错误和不一致性。本文旨在解决噪声反馈对偏好优化的影响问题。

Method: 考虑现实世界中常见的噪声模型（如错误标记和不确定性），分析有限步偏好优化而非传统收敛假设，研究不同噪声类型和噪声率对泛化的影响。

Result: 描述了泛化如何随不同类型噪声和噪声率衰减，基于偏好数据分布和样本数量。分析适用于DPO、IPO、SLiC等多种偏好优化损失函数。

Conclusion: 实证验证证实了理论发现在当代LLM中的实际相关性，为开发符合人类偏好的AI系统提供了有价值的见解。
...
### [185] [LSPO: Length-aware Dynamic Sampling for Policy Optimization in LLM Reasoning](https://arxiv.org/abs/2510.01459)
*Weizhe Chen,Sven Koenig,Bistra Dilkina*

Main category: cs.LG

TL;DR: 提出LSPO算法，基于响应长度动态选择训练数据，提升RLVR在推理任务上的训练效果。


<details>
  <summary>Details</summary>
Motivation: 受LLMs中过度思考现象研究的启发，旨在通过动态数据选择提高强化学习与可验证奖励的训练效率。

Method: LSPO（长度感知采样策略优化），一种元RLVR算法，根据平均响应长度在每个训练步骤动态选择训练数据。

Result: 在多个基础模型和数据集上评估，LSPO一致提高了学习效果，并通过消融研究验证了长度信号整合方式。

Conclusion: LSPO有效提升了RLVR训练效果，为未来研究提供了有前景的方向。
...
### [186] [The Three Regimes of Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2510.01460)
*Lu Li,Tianwei Ni,Yihao Sun,Pierre-Luc Bacon*

Main category: cs.LG

TL;DR: 提出了一个稳定性-可塑性原则来解释离线到在线强化学习的不一致性，强调在在线微调中应保留预训练策略或离线数据集中更好的知识，同时保持足够的可塑性。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习在实践中表现出高度不一致性，相同的在线微调设计选择在不同设置下效果差异很大，需要理论框架来解释和指导。

Method: 基于稳定性-可塑性原则，识别了三种在线微调机制，每种需要不同的稳定性特性，并通过大规模实证研究验证该框架。

Result: 在63个案例中的45个案例中，实证结果与该框架的预测高度一致。

Conclusion: 该工作提供了一个基于离线数据集和预训练策略相对性能的原则性框架，用于指导离线到在线强化学习的设计选择。
...
### [187] [Fine-tuning LLMs with variational Bayesian last layer for high-dimensional Bayesian optimzation](https://arxiv.org/abs/2510.01471)
*Haotian Xiang,Jinwen Xu,Qin Lu*

Main category: cs.LG

TL;DR: 提出了一种基于大语言模型(LLM)的贝叶斯优化方法，使用LoRA-VBLL框架进行轻量级适配，并通过集成学习自动选择超参数，在高维黑盒优化问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统高斯过程在应对高维不规则变量(如分类、序数变量)的黑盒优化问题时表现不佳，而神经网络代理模型计算成本高。受LLM强大能力启发，将其用作代理模型来解决高维优化问题。

Method: 采用LLM作为代理模型，使用LoRA进行轻量级微调，结合VBLL框架优化线性回归头后验。开发了LoRA-VBLL集成方法，自动选择LoRA秩等超参数，并通过递归贝叶斯持续更新模型权重和参数。

Result: 在各种高维基准测试和真实世界分子优化任务中，所提出的(ENS-)LoRA-VBLL方法表现出卓越性能。

Conclusion: 基于LLM的代理模型结合LoRA-VBLL框架，为高维黑盒优化问题提供了一种计算轻量且性能优越的解决方案，特别适用于处理不规则变量。
...
### [188] [PEL-NAS: Search Space Partitioned Architecture Prompt Co-Evolutionary LLM-driven Hardware-Aware Neural Architecture Search](https://arxiv.org/abs/2510.01472)
*Hengyi Zhu,Grace Li Zhang,Shaoyi Huang*

Main category: cs.LG

TL;DR: PEL-NAS是一种基于LLM的神经架构搜索方法，通过分区搜索空间、架构提示协同进化和零成本预测器，在保持高精度的同时显著降低延迟和搜索成本。


<details>
  <summary>Details</summary>
Motivation: 传统超网方法需要多GPU天，而现有LLM驱动方法存在探索偏差，无法在完整搜索空间中跨不同延迟范围发现架构。
...
Conclusion: PEL-NAS有效解决了LLM驱动NAS的探索偏差问题，实现了高效、多样化的神经架构搜索。
...
### [189] [Density-Ratio Weighted Behavioral Cloning: Learning Control Policies from Corrupted Datasets](https://arxiv.org/abs/2510.01479)
*Shriram Karpoora Sundara Pandian,Ali Baheri*

Main category: cs.LG

TL;DR: 提出一种基于密度比加权的行为克隆方法，使用少量干净参考集通过二元判别器估计轨迹级密度比，在行为克隆目标中加权以优先学习干净专家行为，对抗数据污染。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习数据集常被对抗性污染、系统错误或低质量样本污染，导致标准行为克隆和离线RL方法性能下降，需要鲁棒的模仿学习方法。

Method: 使用小规模验证干净参考集，通过二元判别器估计轨迹级密度比，将密度比裁剪后作为权重用于行为克隆目标，优先学习干净数据，无需知道污染机制。

Result: 建立了理论保证，证明在有限样本下收敛到干净专家策略，且收敛界与污染率无关。在连续控制基准测试中，即使在高污染率下仍能保持接近最优性能，优于传统BC、BCQ和BRAC等基线方法。

Conclusion: 密度比加权行为克隆是一种有效的鲁棒模仿学习方法，能够有效处理数据污染问题，在多种污染协议下均表现出色。
...
### [190] [Understanding Adversarial Transfer: Why Representation-Space Attacks Fail Where Data-Space Attacks Succeed](https://arxiv.org/abs/2510.01494)
*Isha Gupta,Rylan Schaeffer,Joshua Kazdan,Ken Liu,Sanmi Koyejo*

Main category: cs.LG

TL;DR: 该论文揭示了对抗性攻击可转移性的关键区别：输入数据空间的攻击可以成功转移，而模型表示空间的攻击则不能转移，除非表示空间具有几何对齐。


<details>
  <summary>Details</summary>
Motivation: 解释为什么图像对抗样本和文本越狱攻击能在不同模型间成功转移，但视觉语言模型的图像越狱攻击却无法转移的显著差异。

Method: 通过理论证明和四个实证场景验证假设：1) 数学证明相同输入输出映射但不同表示的网络；2) 构建图像分类器的表示空间攻击；3) 构建语言模型的表示空间越狱攻击；4) 构建视觉语言模型的数据空间和表示空间攻击。

Result: 数据空间攻击能成功转移到新模型，而表示空间攻击除非表示几何对齐否则无法转移。在视觉语言模型中，当后投影器空间的潜在几何充分对齐时，表示空间攻击也能转移。
...
### [191] [Beyond Majority Voting: LLM Aggregation by Leveraging Higher-Order Information](https://arxiv.org/abs/2510.01499)
*Rui Ai,Yuqi Pan,David Simchi-Levi,Milind Tambe,Haifeng Xu*

Main category: cs.LG

TL;DR: 提出了两种新的多智能体LLM答案聚合算法OW和ISP，利用一阶和二阶信息改进多数投票，在各种数据集上表现更优。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM推理快速发展，但标准多数投票平等对待所有答案，忽略了模型间的潜在异质性和相关性，需要更有效的聚合方法。

Method: 设计了两种新算法：最优权重(OW)和逆惊喜流行度(ISP)，利用一阶和二阶信息来改进多数投票。

Result: 在合成数据集、LLM微调基准(UltraFeedback、MMLU)和真实医疗场景(ARMMAN)中，新方法始终优于多数投票。
...
### [192] [Realistic CDSS Drug Dosing with End-to-end Recurrent Q-learning for Dual Vasopressor Control](https://arxiv.org/abs/2510.01508)
*Will Y. Zou,Jean Feng,Alexandre Kalimouttou,Jennifer Yuntong Zhang,Christopher W. Seymour,Romain Pirracchio*

Main category: cs.LG

TL;DR: 提出一种端到端强化学习方法，用于ICU脓毒症休克患者的双血管加压药物剂量控制，通过动作空间设计提高临床可解释性和采纳度，在eICU和MIMIC数据集上实现超过15%的生存改善概率。


<details>
  <summary>Details</summary>
Motivation: 解决临床决策支持系统中强化学习应用面临的剂量决策不可操作性挑战，提高临床医生对RL决策的接受度。

Method: 结合离线保守Q学习和新型循环建模的回放缓冲区，设计支持离散、连续和定向剂量策略的动作空间，捕捉ICU时间序列数据的时序依赖性。

Result: 设计的动作空间显著影响学习到的行为策略，在eICU和MIMIC数据集上实现超过15%的生存改善概率，同时与既定临床协议保持一致。

Conclusion: 动作空间设计能够提高强化学习在临床决策支持系统中的可解释性和临床采纳度，同时保持治疗效果，为RL在医疗领域的实际应用提供了可行路径。
...
### [193] [Flock: A Knowledge Graph Foundation Model via Learning on Random Walks](https://arxiv.org/abs/2510.01510)
*Jinwoo Kim,Xingyue Huang,Krzysztof Olejniczak,Kyungbin Min,Michael Bronstein,Seunghoon Hong,İsmail İlkan Ceylan*

Main category: cs.LG

TL;DR: 提出了Flock模型，通过概率性节点-关系等变性解决知识图谱零样本链接预测问题，克服了传统确定性等变性在表达能力上的限制。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱基础模型(KGFMs)的确定性等变性限制了其表达能力，无法区分结构相似但语义不同的关系，需要新的方法来处理新颖实体和关系。

Method: 引入概率性节点-关系等变性，通过迭代采样随机游走、编码序列、序列模型嵌入和池化聚合来表示节点和关系，保持分布等变性同时打破推理中的对称性。

Result: Flock在新诊断数据集Petals上完美解决当前KGFMs失败的问题，在54个不同领域的知识图谱上实现了实体和关系预测任务的最先进性能。

Conclusion: 概率性节点-关系等变性是提升知识图谱基础模型表达能力的有效方法，Flock模型证明了其在零样本链接预测中的优越性。
...
### [194] [Predictive Modeling and Explainable AI for Veterinary Safety Profiles, Residue Assessment, and Health Outcomes Using Real-World Data and Physicochemical Properties](https://arxiv.org/abs/2510.01520)
*Hossein Sholehrasa,Xuan Xu,Doina Caragea,Jim E. Riviere,Majid Jaberi-Douraki*

Main category: cs.LG

TL;DR: 该研究开发了一个预测框架，使用FDA兽医医学中心的128万份报告来分类药物在食用动物中的安全结果（死亡vs恢复），结合数据工程、机器学习和可解释AI，实现了高精度的预测。


<details>
  <summary>Details</summary>
Motivation: 确保食用动物中药物的安全使用对保护动物福利和人类食品安全至关重要，不良事件可能预示着意外的药代动力学或毒代动力学效应，增加食品链中违规残留的风险。

Method: 构建预处理管道合并关系表并通过VeDDRA本体标准化不良事件，整合理化药物特性，评估包括随机森林、CatBoost、XGBoost、ExcelFormer和大型语言模型在内的监督模型，使用集成方法和AUM伪标记处理类别不平衡。

Result: 集成方法和CatBoost表现最佳，精确度、召回率和F1分数达到0.95，通过SHAP可解释性识别了与致命结果强相关的生物学合理预测因子。

Conclusion: 结合严格的数据工程、先进机器学习和可解释AI能够实现准确、可解释的兽医安全结果预测，支持FARAD使命，加强残留风险评估并为监管决策提供信息。
...
### [195] [CarbonX: An Open-Source Tool for Computational Decarbonization Using Time Series Foundation Models](https://arxiv.org/abs/2510.01521)
...
<details>
  <summary>Details</summary>
Motivation: 现有碳强度预测工具存在三个主要限制：需要电网特定的电力混合数据、依赖单独的电网特定模型难以实现全球覆盖、不提供不确定性估计。

Method: 利用时间序列基础模型的通用性，仅使用历史碳强度数据和单一通用模型，实现零样本预测和插补任务。

Result: 在214个全球电网中零样本预测MAPE为15.82%；在13个基准电网中平均MAPE为9.59%，尾部预测MAPE为16.54%，并提供95%覆盖率的预测区间；可提供长达21天的预测；在插补任务上比统计基线提升1.2-3.9倍。

Conclusion: CarbonX可以在数据有限的任何电网中轻松使用并提供强大性能，是实用的全球规模脱碳工具。
...
### [196] [On Integer Programming for the Binarized Neural Network Verification Problem](https://arxiv.org/abs/2510.01525)
*Woojin Kim,James R. Luedtke*

Main category: cs.LG

TL;DR: 本文提出了两种改进二值神经网络验证的整数规划方法：新的多类别线性目标函数和基于递归结构的有效不等式生成技术，显著提升了验证效率。


<details>
  <summary>Details</summary>
Motivation: 二值神经网络的验证问题通常被表述为整数规划问题，但传统方法由于大M约束导致的巨大整数间隙而难以求解，需要改进IP公式化方法。

Method: 1) 为多类别设置引入新的线性目标函数方法；2) 开发基于BNN递归结构的有效不等式生成技术来改进IP公式化。

Result: 与现有IP方法相比，新技术能够在有限时间内验证BNN对抗更大范围的输入扰动。

Conclusion: 所提出的IP公式化改进技术显著提升了二值神经网络验证的效率和能力，能够处理更广泛的输入扰动情况。
...
### [197] [Round-trip Reinforcement Learning: Self-Consistent Training for Better Chemical LLMs](https://arxiv.org/abs/2510.01527)
*Lecheng Kong,Xiyuan Wang,Yixin Chen,Muhan Zhang*

Main category: cs.LG

TL;DR: 提出了Round-Trip Reinforcement Learning (RTRL)框架，通过使用往返转换成功作为奖励信号来训练模型提高一致性，显著提升了化学LLM的性能和一致性。


<details>
  <summary>Details</summary>
Motivation: 当前化学大语言模型缺乏往返一致性，例如能够成功描述分子结构但无法从其生成的文本准确重建原始结构，这表明模型学习的是单向记忆而非灵活掌握。

Method: 引入RTRL框架，使用往返转换成功作为奖励信号训练模型；提出迭代变体，其中前向和反向映射交替训练彼此，形成自改进循环。

Result: RTRL在监督、自监督和合成数据机制下，显著提升了性能和一致性，优于强基线。
...
### [198] [Bypassing Prompt Guards in Production with Controlled-Release Prompting](https://arxiv.org/abs/2510.01529)
*Jaiden Fairoze,Sanjam Garg,Keewoo Lee,Mingyuan Wang*

Main category: cs.LG

TL;DR: 本文提出了一种新的攻击方法，能够绕过大型语言模型的提示防护机制，成功越狱多个生产级模型，揭示了轻量级提示防护在架构上的固有漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，AI安全和对齐变得至关重要。提示防护作为轻量级防护机制虽然易于实现和更新，但其有效性需要验证。

Method: 利用提示防护与主模型之间的资源不对称性，编码一种轻量级防护无法解码但主模型能够理解的越狱提示。
...
### [199] [NVIDIA AI Aerial: AI-Native Wireless Communications](https://arxiv.org/abs/2510.01533)
*Kobi Cohen-Arazi,Michael Roe,Zhen Hu,Rohan Chavan,Anna Ptasznik,Joanna Lin,Joao Morais,Joseph Boccuzzi,Tommaso Balercia*

Main category: cs.LG

TL;DR: 提出了一个将Python算法编译为GPU可执行代码的框架，用于6G AI原生无线系统中DSP和ML的无缝集成，并在NVIDIA AI Aerial平台上实现了信道估计CNN模型的实时部署。


<details>
  <summary>Details</summary>
Motivation: 6G需要向AI原生无线系统转型，将数字信号处理和机器学习无缝集成到蜂窝网络软件栈中，使现代网络生命周期更接近AI系统。

Method: 开发了一个稳健框架，将基于Python的算法编译成GPU可运行的二进制文件，在数字孪生中训练CNN进行信道估计，然后在实时测试平台上部署。

Result: 实现了高效、灵活且在NVIDIA GPU上达到最高性能的统一方法，成功演示了在PUSCH接收器中通过CNN进行信道估计的能力。
...
### [200] [TimeSeriesScientist: A General-Purpose AI Agent for Time Series Analysis](https://arxiv.org/abs/2510.01538)
*Haokun Zhao,Xiang Zhang,Jiaqi Wei,Yiwei Xu,Yuting He,Siqi Sun,Chenyu You*

Main category: cs.LG

TL;DR: TSci是首个基于LLM的智能代理框架，用于通用时间序列预测，通过四个专门代理（数据管理、规划、预测、报告）实现自动化预测流程，显著降低预测误差并提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测面临数千个短而嘈杂的序列，不同频率、质量和预测范围，主要成本在于人工密集的预处理、验证和集成过程。现有统计和深度学习模型针对特定数据集或领域，泛化能力差，急需一个最小化人工干预的通用领域无关框架。

Method: TSci框架包含四个专门代理：Curator通过LLM引导的诊断和外部工具进行数据预处理；Planner利用多模态诊断和自我规划缩小模型选择空间；Forecaster执行模型拟合和验证，自适应选择最佳模型配置和集成策略；Reporter将整个过程合成为全面的透明报告。

Result: 在八个基准测试上的实证结果表明，TSci持续优于统计和基于LLM的基线方法，分别平均减少预测误差10.4%和38.2%。

Conclusion: TSci通过透明自然语言原理和全面报告，将预测工作流程转变为可解释且可跨任务扩展的白盒系统，显著提高了预测性能和透明度。
...
### [201] [Executable Counterfactuals: Improving LLMs' Causal Reasoning Through Code](https://arxiv.org/abs/2510.01539)
*Aniket Vashishtha,Qirun Dai,Hongyuan Mei,Amit Sharma,Chenhao Tan,Hao Peng*

Main category: cs.LG

TL;DR: 本文提出了可执行反事实框架，通过代码和数学问题评估LLM的反事实推理能力，发现现有模型在完整反事实推理中准确率下降25-40%，并比较了监督微调和强化学习在提升模型泛化能力方面的效果。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM反事实推理能力的方法往往跳过溯因步骤，导致高估模型性能。需要开发能够完整评估反事实推理三个步骤（溯因、干预、预测）的框架。

Method: 提出可执行反事实框架，通过代码和数学问题操作化因果推理，要求完整执行反事实推理的三个步骤，并创建可扩展的合成数据。比较了监督微调和强化学习两种训练方法的效果。

Result: SOTA模型在从干预推理到反事实推理时准确率下降25-40%。监督微调虽然提升域内性能，但在OOD任务上准确率下降；强化学习则能诱导核心认知行为，在代码和数学问题上均获得1.5-2倍的改进。

Conclusion: 强化学习在提升LLM反事实推理能力方面比监督微调更具优势，能够更好地泛化到新领域，为改进LLM的因果理解提供了有前景的方向。
...
### [202] [Predictive Preference Learning from Human Interventions](https://arxiv.org/abs/2510.01545)
*Haoyuan Cai,Zhenghao Peng,Bolei Zhou*

Main category: cs.LG

TL;DR: PPL通过从人类干预中学习预测偏好，将单次干预扩展到未来L个时间步，通过偏好优化将专家修正传播到安全关键区域，提高学习效率并减少所需的人类演示。
...
Method: 提出预测偏好学习(PPL)，将每次人类干预引导到未来L个时间步（偏好视野），假设智能体执行相同动作且人类进行相同干预，然后对这些未来状态应用偏好优化。
...
### [203] [MIRA: Towards Mitigating Reward Hacking in Inference-Time Alignment of T2I Diffusion Models](https://arxiv.org/abs/2510.01549)
...
TL;DR: MIRA是一种无需训练、推理时对齐的方法，通过图像空间的KL代理正则化来防止奖励黑客攻击，在保持提示遵循的同时提高奖励分数。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像虽然能基于文本提示，但往往无法满足用户特定的标量奖励标准（如美学评分）。现有的推理时对齐方法存在奖励黑客问题，即图像获得高评分但严重偏离原始提示。

Method: 提出MIRA方法，引入基于分数的图像空间KL代理正则化，约束采样轨迹以防止奖励黑客。还提出了MIRA-DPO，将偏好优化映射到推理时间，适用于不可微分的奖励。

Result: 在SDv1.5和SDXL模型、多种奖励函数和公共数据集上的实验表明，MIRA相比强基线获得超过60%的胜率，同时保持提示遵循性。机制图显示奖励增益接近零漂移，而DNO方法随计算增加而漂移。

Conclusion: MIRA提供了一种有效的推理时对齐方法，能够在不进行微调的情况下提高奖励分数，同时防止奖励黑客问题，适用于可微分和不可微分的奖励场景。
...
### [204] [Rethinking KL Regularization in RLHF: From Value Estimation to Gradient Optimization](https://arxiv.org/abs/2510.01555)
*Kezhao Liu,Jason Klein Liu,Mingtao Chen,Yiming Liu*

Main category: cs.LG

TL;DR: 本文分析了RLHF中KL散度正则化的不同实现方式，建立了统一框架证明'k1 in reward'和'k2 as loss'在策略条件下梯度等价，都是RKL正则化的理论正确实现，而'k3 as loss'只是有偏的一阶近似。
...
Method: 建立统一分析框架，将KL正则化的两种实现风格（'kn in reward'和'kn as loss'）联系起来，通过梯度等价性分析证明不同实现的理论正确性。

Result: 证明在策略条件下'k2 as loss'与'k1 in reward'梯度等价，都是RKL正则化的正确实现；而'k3 as loss'是有偏的一阶近似；同时指出离策略实现中的重要性采样偏差问题并提出修正方法。
...
### [205] [Large-Scale Bayesian Causal Discovery with Interventional Data](https://arxiv.org/abs/2510.01562)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在大规模任务上性能不佳且无法量化不确定性，而高通量基因组扰动数据为改进模型识别提供了机会。

Method: 采用经验贝叶斯框架，建模总因果效应矩阵的似然（近似矩阵正态分布），使用spike-and-slab horseshoe先验，从观测数据中学习scale-free和Erdős-Rényi结构的权重，将每条边作为潜变量。

Result: 在广泛模拟中，IBCD在结构恢复方面优于现有基线方法；应用于521个基因的CRISPR扰动数据，边缘后验包含概率能够识别稳健的图结构。

Conclusion: IBCD框架在因果发现中表现出优越性能，能够有效处理大规模任务并提供不确定性量化。
...
### [206] [TetriServe: Efficient DiT Serving for Heterogeneous Image Generation](https://arxiv.org/abs/2510.01565)
*Runyu Lu,Shiqi He,Wenxuan Tan,Shenggui Li,Ruofan Wu,Jeff J. Ma,Ang Chen,Mosharaf Chowdhury*

Main category: cs.LG

TL;DR: TetriServe是一个针对Diffusion Transformer (DiT)模型的高效服务系统，通过步骤级序列并行和轮次调度机制，在保证图像质量的同时显著提高了服务级别目标(SLO)达成率。


<details>
  <summary>Details</summary>
Motivation: 现有的DiT服务系统使用固定程度的序列并行，在处理异构工作负载（混合分辨率和截止时间）时效率低下，导致GPU利用率差和SLO达成率低。

Method: 提出步骤级序列并行策略，根据请求的截止时间动态调整并行度；引入轮次调度机制，包括时间离散化、步骤级并行度适配和请求联合打包。

Result: 在最先进的DiT模型上的广泛评估显示，TetriServe相比现有解决方案实现了高达32%的SLO达成率提升，且不降低图像质量。

Conclusion: TetriServe通过动态并行度调整和智能调度，有效解决了DiT模型服务中的效率问题，为高质量图像生成提供了高效的服务解决方案。
...
### [207] [From Supervision to Exploration: What Does Protein Language Model Learn During Reinforcement Learning?](https://arxiv.org/abs/2510.01571)
...
TL;DR: 将强化学习与蛋白质语言模型结合，在四个蛋白质设计领域验证RL能否超越监督学习的限制，发现RL能提升采样效率和成功率，但效果取决于任务空间、奖励准确性和策略容量三个因素的相互作用。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习能否推动蛋白质语言模型超越其预训练先验，发现潜在的序列-结构-功能规则，解决监督学习在蛋白质设计中存在的局限性。

Method: 在抗菌肽设计、激酶变体优化、抗体工程和逆折叠四个领域，使用多种RL算法和模型类别，与蛋白质语言模型配对进行实验。

Result: RL在所有基准测试中都一致提高了成功率和采样效率。性能遵循三因素交互作用：任务空间、奖励保真度和策略容量共同决定增益。当奖励准确且信息丰富、策略容量足够、任务有超越监督基线的空间时，改进会扩展；当奖励嘈杂或容量受限时，增益会饱和。

Conclusion: 为蛋白质设计中的RL提供了实用指导：在扩展策略规模之前优先考虑奖励建模和校准，根据任务难度匹配算法和正则化强度，在边际收益最大的地方分配容量。
...
### [208] [Gradient Shaping Beyond Clipping: A Functional Perspective on Update Magnitude Control](https://arxiv.org/abs/2510.01578)
...
<details>
  <summary>Details</summary>
Motivation: 传统的梯度裁剪使用固定的硬阈值，限制了灵活性且忽略了梯度分布动态。SPAMP旨在提供更灵活、自适应的梯度调制方法。
...
Conclusion: SPAMP提供了一个原则性的替代方案，取代了僵硬的启发式方法，通过统一框架改善了深度网络训练的稳定性。
...
### [209] [Think Right: Learning to Mitigate Under-Over Thinking via Adaptive, Attentive Compression](https://arxiv.org/abs/2510.01581)
...
<details>
  <summary>Details</summary>
Motivation: 现有思维模型在复杂推理任务中存在推理长度不适应问题：过短推理（欠思考）导致困难问题错误，过长推理（过思考）造成token效率低下。需要平衡欠思考与过思考。

Method: 提出TRAAC方法：1）利用自注意力分析长推理轨迹，识别重要步骤并剪枝冗余；2）估计问题难度并纳入训练奖励；3）学习根据难度分配推理预算。

Result: 在多种任务上，TRAAC（Qwen3-4B）相比基础模型平均准确率提升8.4%，推理长度相对减少36.8%；相比最佳RL基线准确率提升7.9%，长度减少29.4%。在非数学数据集上也表现出良好泛化能力。

Conclusion: TRAAC通过任务难度校准和基于注意力的压缩，实现了基于难度的细粒度推理预算调整，在多样化任务上均取得收益。
...
### [210] [Enhancing Noise Robustness of Parkinson's Disease Telemonitoring via Contrastive Feature Augmentation](https://arxiv.org/abs/2510.01588)
*Ziming Tang,Chengbin Hou,Tianyu Zhang,Bangxu Tian,Jinbao Wang,Hairong Lv*

Main category: cs.LG

TL;DR: 提出了NoRo框架，一种噪声鲁棒的UPDRS预测方法，通过对比学习增强特征鲁棒性，解决帕金森病远程监测中的噪声问题。


<details>
  <summary>Details</summary>
Motivation: 帕金森病远程监测中存在三种噪声：患者测量误差、环境噪声和数据包丢失，导致UPDRS评分预测误差增加，需要开发噪声鲁棒的预测方法。

Method: 首先将语音特征按选定特征值分组构建对比对，然后训练多层感知机编码器生成噪声鲁棒特征，最后将原始特征与鲁棒特征拼接作为增强特征输入预测模型。

Result: 实验表明NoRo能显著提升不同下游预测模型在各种噪声环境下的UPDRS预测鲁棒性。

Conclusion: NoRo框架有效解决了帕金森病远程监测中的噪声问题，提高了UPDRS评分的预测准确性和鲁棒性。
...
### [211] [Securing generative artificial intelligence with parallel magnetic tunnel junction true randomness](https://arxiv.org/abs/2510.01598)
*Youwei Bao,Shuhan Yang,Hyunsoo Yang*

Main category: cs.LG

TL;DR: 该论文提出了一种基于自旋转移矩磁隧道结(STT-MTJs)的硬件真随机数生成器，用于解决生成式AI模型中伪随机数生成器的安全漏洞问题。
...
Method: 通过嵌入STT-MTJs产生的硬件真随机比特，构建高度并行的FPGA辅助计算系统，并将硬件随机比特集成到生成对抗网络中。

Result: 系统每秒可产生兆比特级的真随机数，通过NIST随机性测试，在CIFAR-10数据集上训练GAN时，相比低质量随机数生成器基线，不安全输出减少了18.6倍。

Conclusion: 基于STT-MTJ的系统具有纳秒级切换速度、高能效和良好可扩展性，有望扩展到10^6个并行单元，实现千兆比特/秒的吞吐量，适合大型语言模型采样，展示了自旋电子RNG作为下一代GAI系统实用安全组件的潜力。
...
### [212] [Posterior Collapse as a Phase Transition in Variational Autoencoders](https://arxiv.org/abs/2510.01621)
*Zhen Li,Fan Zhang,Zheng Zhang,Yu Chen*

Main category: cs.LG

TL;DR: 从统计物理学角度研究VAE中的后验坍塌现象，揭示其是由数据结构和模型超参数共同控制的相变过程，存在临界超参数阈值区分有意义的潜在推断和坍塌状态。


<details>
  <summary>Details</summary>
Motivation: 理解变分自编码器中后验坍塌现象的本质原因，传统观点认为这是优化失败，但本文从统计物理角度重新审视这一现象。

Method: 通过分析后验坍塌相关平凡解的稳定性，识别临界超参数阈值，并在合成和真实数据集上验证这种临界行为。

Result: 发现后验坍塌与KL散度不连续性相关的相变边界，确认了相变的存在，表明后验坍塌不是单纯的优化失败。

Conclusion: 后验坍塌是数据结构和变分约束相互作用产生的涌现相变，为深度生成模型的可训练性和表示能力提供了新见解。
...
### [213] [Quagmires in SFT-RL Post-Training: When High SFT Scores Mislead and What to Use Instead](https://arxiv.org/abs/2510.01624)
*Feiyang Kang,Michael Kuchnik,Karthik Padthe,Marin Vlastelica,Ruoxi Jia,Carole-Jean Wu,Newsha Ardalani*

Main category: cs.LG

TL;DR: 本文挑战了当前LLM后训练中SFT分数高必然带来RL性能提升的假设，通过大规模实验证明高SFT分数可能源于数据偏差，且不能可靠预测RL效果，甚至可能比无SFT的基础模型表现更差。


<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练实践中，SFT和RL被作为两个独立阶段，但高SFT分数是否真正转化为RL后的性能提升缺乏验证。本文旨在检验这一假设并寻找更可靠的预测指标。

Method: 训练了数百个12B参数模型，使用SFT和GRPO RLVR方法，在7个数学基准上进行了多达256次重复的广泛评估，消耗超过100万GPU小时。

Result: 发现SFT分数与RL效果相关性弱，甚至可能负相关。提出了泛化损失和Pass@large k作为更可靠的RL效果预测指标，将R²和Spearman相关系数提高了0.5（2倍）。

Conclusion: SFT性能不是RL效果的可靠预测指标，应使用泛化损失和Pass@large k来评估模型潜力，这对LLM后训练策略有重要指导意义。
...
### [214] [Demystifying Synthetic Data in LLM Pre-training: A Systematic Study of Scaling Laws, Benefits, and Pitfalls](https://arxiv.org/abs/2510.01631)
...
TL;DR: 大规模实证研究发现，在预训练中混合1/3重述合成数据和2/3自然网络数据可以加速5-10倍，但单独使用合成数据效果不佳。好的合成数据比例约为30%，且8B参数生成器模型已足够。


<details>
  <summary>Details</summary>
Motivation: 高质量训练数据对LLM扩展至关重要但供应有限，合成数据技术可能突破这一限制，需要实证验证其有效性。

Method: 使用统一协议和缩放定律进行大规模实证研究（>1000个LLM，>10万GPU小时），比较自然网络数据、多种合成数据类型及其混合。

Result: 单独使用重述合成数据不比自然数据快；1/3重述合成+2/3自然数据可加速5-10倍；教科书式合成数据在小型数据预算下损失更高；好的合成数据比例约30%；8B参数生成器已足够。

Conclusion: 合成数据在预训练中具有条件性益处，混合使用重述合成数据可显著加速训练，但需注意数据比例和类型选择。
...
### [215] [CAT: Curvature-Adaptive Transformers for Geometry-Aware Learning](https://arxiv.org/abs/2510.01634)
*Ryan Y. Lin,Siddhartha Ojha,Nicholas Bai*

Main category: cs.LG

TL;DR: 提出了曲率自适应Transformer（CAT），通过可学习的门控机制动态路由token到三种几何注意力分支，解决了固定几何方法在处理混合几何结构数据时的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer假设欧几里得几何，限制了在非欧几里得结构数据上的效果。现有双曲和球面几何扩展需要预先选择单一几何，缺乏处理混合几何属性的灵活性。
...
Conclusion: 学习几何自适应优于任何单一固定几何，CAT为语言、视觉和多模态领域的混合几何架构提供了可扩展和可解释的基础。
...
### [216] [Detecting Post-generation Edits to Watermarked LLM Outputs via Combinatorial Watermarking](https://arxiv.org/abs/2510.01637)
...
Result: 在开源LLM上评估了各种编辑场景，展示了在编辑定位方面强大的实证性能。
...
### [217] [Support Basis: Fast Attention Beyond Bounded Entries](https://arxiv.org/abs/2510.01643)
*Maryam Aliakbarpour,Vladimir Braverman,Junze Yin,Haochen Zhang*

Main category: cs.LG

TL;DR: 提出了一种新的支持基分解框架，用于在超出有界条目限制的情况下高效近似注意力机制，解决了现有方法在现代大语言模型中的适用性问题。


<details>
  <summary>Details</summary>
Motivation: 现有子二次注意力近似算法需要限制性有界条目假设，而该假设在实际应用中很少成立，限制了其在现代大语言模型中的适用性。

Method: 利用查询和键矩阵条目呈现次高斯行为的经验发现，通过支持基分解将大条目和小条目分离，对稀疏组件进行精确计算，对密集组件进行多项式近似。

Result: 建立了严格的理论保证，证明了次二次运行时间，并扩展到多阈值设置以消除所有分布假设。

Conclusion: 该方法不仅提供了高效的注意力近似，还为多项式注意力的经验成功提供了首个理论依据。
...
### [218] [Source-Free Cross-Domain Continual Learning](https://arxiv.org/abs/2510.01649)
*Muhammad Tanzil Furqon,Mahardhika Pratama,Igor Škrjanc,Lin Liu,Habibullah Habibullah,Kutluyil Dogancay*

Main category: cs.LG

TL;DR: 本文提出REFEREE方法，解决源自由跨域持续学习问题，无需源域样本，通过频率感知提示技术和不确定性加权策略处理域偏移和噪声伪标签问题，使用KLDA缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有跨域持续学习方法需要完全标记的源域数据，这在隐私受限环境中不可行。本文旨在解决源自由跨域持续学习问题，完全禁止使用源域样本。

Method: REFEREE方法结合源预训练模型和大规模视觉语言模型，采用频率感知提示技术生成增强样本，使用不确定性加权策略处理噪声伪标签，通过核线性判别分析(KLDA)缓解灾难性遗忘。

Result: 严格的数值研究表明，该方法显著优于现有需要源域样本的方法。

Conclusion: REFEREE方法在源自由跨域持续学习场景中表现优异，能够有效处理域偏移、噪声伪标签和灾难性遗忘问题。
...
### [219] [The Unseen Frontier: Pushing the Limits of LLM Sparsity with Surrogate-Free ADMM](https://arxiv.org/abs/2510.01650)
...
<details>
  <summary>Details</summary>
Motivation: 传统神经网络剪枝方法在超过50-60%稀疏度时会严重降低模型精度，这限制了大型语言模型的计算和内存效率提升。

Method: 使用基于ADMM的标准约束优化技术，直接解决现有方法依赖代理目标函数的局限性，并提出量化变体Elsa-L以扩展到极大模型。

Result: 在LLaMA-2-7B模型上达到90%稀疏度时，困惑度比现有最佳方法低7.8倍，并能扩展到27B规模的极大模型。
...
### [220] [Asymmetric Proximal Policy Optimization: mini-critics boost LLM reasoning](https://arxiv.org/abs/2510.01656)
...
TL;DR: 提出了AsyPPO框架，通过轻量级mini-critics恢复critic角色，在LLM规模下保持高效，提升学习稳定性和性能


<details>
  <summary>Details</summary>
Motivation: 传统RL方法在LLM规模下计算成本高，在稀疏奖励和长推理视野下容易失败，需要恢复critic角色同时保持效率

Method: 使用多个轻量级mini-critics，每个在不相交的提示分片上训练，利用critic间不确定性优化策略更新，包括优势掩码和过滤高分歧状态

Result: 在仅5000个样本上训练后，在多个基准测试中持续提升学习稳定性和性能，在Qwen3-4b-Base上比经典PPO提升超过6%
...
### [221] [Learning Time-Series Representations by Hierarchical Uniformity-Tolerance Latent Balancing](https://arxiv.org/abs/2510.01658)
...
<details>
  <summary>Details</summary>
Motivation: 旨在在嵌入空间中实现均匀性和容忍度的有效平衡，以学习更强的时间序列表示，同时捕捉实例级和时间级信息。

Method: 使用层次化设置学习实例级和时间级信息；在对比损失中集成温度调度器来平衡嵌入的均匀性和容忍度特性；使用层次化角度边界损失在时间序列的正负对之间创建几何边界。
...
### [222] [Shift-Invariant Attribute Scoring for Kolmogorov-Arnold Networks via Shapley Value](https://arxiv.org/abs/2510.01663)
...
<details>
  <summary>Details</summary>
Motivation: KAN网络虽然能恢复符号表示并保持竞争力性能，但其架构对网络剪枝带来独特挑战。传统基于幅度的剪枝方法由于对输入坐标偏移敏感而变得不可靠。

Method: 使用Shapley值归因来评估节点重要性，以平移不变的方式量化每个节点的实际贡献，确保重要性排名不受输入参数化影响。

Result: 在合成和真实数据集上的广泛实验表明，ShapKAN能保持真实节点重要性，同时实现有效的网络压缩。

Conclusion: ShapKAN方法增强了KAN的可解释性优势，促进了在资源受限环境中的部署。
...
### [223] [Beyond Simple Fusion: Adaptive Gated Fusion for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2510.01677)
*Han Wu,Yanming Sun,Yunhe Yang,Derek F. Wong*

Main category: cs.LG

TL;DR: 提出了一种自适应门控融合网络(AGFN)，通过基于信息熵和模态重要性的双门融合机制自适应调整特征权重，有效处理多模态情感分析中的噪声模态和语义冲突问题。
...
Conclusion: AGFN通过减少特征位置与预测误差之间的相关性，学习更广泛的特征分布，从而增强了泛化能力，创建了更鲁棒的多模态特征表示。
...
### [224] [PASTA: A Unified Framework for Offline Assortment Learning](https://arxiv.org/abs/2510.01693)
*Juncheng Dong,Weibin Mo,Zhengling Qi,Cong Shi,Ethan X. Fang,Vahid Tarokh*

Main category: cs.LG

TL;DR: 提出了PASTA框架，通过悲观原则解决离线数据驱动的品类优化问题，在数据覆盖不足的情况下实现最优预期收益，并在多种选择模型中建立了首个有限样本遗憾界。


<details>
  <summary>Details</summary>
Motivation: 品类优化问题中，企业缺乏对底层选择模型的先验知识，只能基于历史客户选择数据确定最优品类。组合性质导致数据覆盖不足，设计可证明有效的解决方案面临重大挑战。

Method: 引入悲观品类优化(PASTA)框架，利用悲观原则在一般选择模型下实现最优预期收益。PASTA仅要求离线数据分布包含最优品类，而非提供所有可行品类的完整覆盖。

Result: 为包括多项logit和嵌套logit模型在内的多种广泛使用的选择模型建立了首个有限样本遗憾界。推导了极小极大遗憾下界，证明PASTA在样本和模型复杂度方面是极小极大最优的。数值实验表明该方法优于现有基线方法。

Conclusion: PASTA框架有效解决了离线数据驱动品类优化中的数据覆盖不足问题，在理论和实验上都表现出优越性能，为实际应用提供了可靠解决方案。
...
### [225] [Representational Alignment Across Model Layers and Brain Regions with Hierarchical Optimal Transport](https://arxiv.org/abs/2510.01706)
*Shaan Shah,Meenakshi Khosla*

Main category: cs.LG

TL;DR: 提出了分层最优传输（HOT）框架，通过全局优化解决神经网络表示对齐问题，能够处理不同深度网络的对齐，提供单一对齐分数和软传输计划。


<details>
  <summary>Details</summary>
Motivation: 标准表示相似性方法存在局限性：独立对齐每层导致不对称结果、缺乏全局对齐分数、难以处理不同深度网络。这些问题源于忽略全局激活结构和限制为刚性的一对一层对应。

Method: HOT联合推断软性、全局一致的层到层耦合和神经元级传输计划，允许源神经元跨多个目标层分配质量，在边际约束下最小化总传输成本。

Result: 在视觉模型、大语言模型和人类视觉皮层记录上的评估显示，HOT在匹配或超越标准成对匹配的同时，揭示了平滑、细粒度的分层对应关系。

Conclusion: HOT能够实现更丰富、更可解释的表示比较，特别是在网络架构或深度不同的情况下，从全局优化中自然涌现出结构化模式。
...
### [226] [ActiNet: Activity intensity classification of wrist-worn accelerometers using self-supervised deep learning](https://arxiv.org/abs/2510.01712)
*Aidan Acquah,Shing Chan,Aiden Doherty*

Main category: cs.LG

TL;DR: ActiNet模型（自监督ResNet-V2+HMM）在手腕加速度计数据上比传统随机森林+HMM方法表现更好，能更准确识别活动强度，适用于大规模流行病学研究。
...
Method: 使用151名CAPTURE-24参与者的数据，训练ActiNet模型（18层自监督改进ResNet-V2模型），然后进行隐马尔可夫模型平滑处理，并与基线随机森林+HMM方法比较。

Result: ActiNet模型的平均宏观F1分数为0.82，Cohen's kappa分数为0.86，显著优于RF+HMM的0.77和0.81，且在不同年龄和性别亚组中表现一致。

Conclusion: ActiNet模型在活动强度分类方面表现优异，鼓励在未来的流行病学研究中用于从手腕加速度计数据提取活动强度标签。
...
### [227] [Latency-aware Multimodal Federated Learning over UAV Networks](https://arxiv.org/abs/2510.01717)
...
Conclusion: 无人机辅助的联邦多模态学习框架能有效降低系统延迟并提升模型性能，为多模态感知和分布式学习提供了可行的解决方案。
...
### [228] [Accelerating Attention with Basis Decomposition](https://arxiv.org/abs/2510.01718)
*Jialin Zhao*
...
<details>
  <summary>Details</summary>
Motivation: 注意力是大语言模型和视觉语言模型的核心操作，现有优化方法如FlashAttention主要关注I/O层面的系统优化，缺乏数学上保证的无损加速方法。

Method: 利用基础分解(BD)的简单矩阵恒等式，将多头投影重新组织为紧凑形式，无需重新训练，只需4秒离线准备即可应用。

Result: 在DeepSeek-V2-Lite (16B, FP16)上，BDA实现了32%更快的键值投影和25%更小的权重，端到端困惑度仅增加0.02%(FP16)或0.0004%(FP32)，对模型性能影响可忽略。
...
### [229] [Finite-Time Bounds for Distributionally Robust TD Learning with Linear Function Approximation](https://arxiv.org/abs/2510.01721)
*Saptarshi Mandal,Yashaswini Murthy,R. Srikant*

Main category: cs.LG

TL;DR: 本文提出了首个使用线性函数逼近的鲁棒TD学习算法，解决了分布鲁棒强化学习中的模型不确定性挑战，无需生成模型访问即可实现ε-准确值估计。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒TD学习的收敛保证仅限于表格MDP或依赖限制性折扣因子假设，缺乏函数逼近下的理论保证，这阻碍了鲁棒RL算法的实际应用。

Method: 结合双时间尺度随机逼近更新和外循环目标网络更新，针对总变差距离和Wasserstein距离不确定性集设计模型无关算法。

Result: 建立了Õ(1/ε²)的样本复杂度，实现了ε-准确值估计，填补了鲁棒RL算法经验成功与非鲁棒对应物理论保证之间的关键差距。

Conclusion: 该算法成功将鲁棒TD学习扩展到线性函数逼近场景，关键思想可轻松扩展到鲁棒Q学习，为鲁棒RL提供了坚实的理论基础。
...
### [230] [Workplace Location Choice Model based on Deep Neural Network](https://arxiv.org/abs/2510.01723)
*Tanay Rastogi,Anders Karlström*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度神经网络（DNN）的工作地点选择建模方法，相比传统离散选择模型（DCM）能更好地理解复杂决策模式并提供更优结果。


<details>
  <summary>Details</summary>
Motivation: 传统离散选择模型在准确反映个体决策过程方面面临挑战，需要开发更有效的建模方法来理解工作地点选择的复杂决策模式。

Method: 使用深度神经网络（DNN）对工作地点选择进行建模，并与传统离散选择模型（DCM）进行对比分析。

Result: DNN在工作地点选择建模中显示出作为DCM强有力替代方案的潜力。两种模型都能有效复制工作机会对选择的影响，但DNN在某些方面表现更优，而DCM在评估个体属性对工作距离影响时与数据更吻合。DCM在短距离上表现更好，DNN在长距离上与数据和DCM表现相当。

Conclusion: 选择合适的工作地点选择分析模型应基于具体应用需求，两种模型各有优势，需要根据具体情况选择使用。
...
### [231] [Private and Fair Machine Learning: Revisiting the Disparate Impact of Differentially Private SGD](https://arxiv.org/abs/2510.01744)
*Lea Demelius,Dominik Kowald,Simone Kopeinik,Roman Kern,Andreas Trügler*

Main category: cs.LG

TL;DR: 本文分析了差分隐私随机梯度下降（DPSGD）对模型公平性的影响，发现直接优化差分隐私模型的超参数虽然不能可靠地缓解DPSGD的不公平影响，但相比重用非隐私模型的超参数，能获得更好的效用-公平性权衡。


<details>
  <summary>Details</summary>
Motivation: 研究DPSGD对模型公平性的影响，验证直接优化差分隐私模型超参数是否能改善公平性，以及分析不同性能指标间不公平影响的差异。

Method: 1) 比较DPSGD对不同性能指标的不公平影响差异；2) 在广泛的超参数设置下分析DPSGD的公平性影响；3) 评估DPSGD-Global-Adapt变体在缓解不公平性方面的效果。

Result: 不同性能指标的不公平影响存在差异；直接优化差分隐私模型超参数不能可靠缓解DPSGD的不公平影响，但能改善效用-公平性权衡；DPSGD-Global-Adapt对超参数选择不稳健。

Conclusion: 超参数调优会带来额外的隐私泄漏，需要在隐私、效用和公平性之间谨慎权衡；直接优化差分隐私模型超参数是改善效用-公平性权衡的有效方法，但不能完全解决DPSGD的不公平问题。
...
### [232] [Unsupervised Dynamic Feature Selection for Robust Latent Spaces in Vision Tasks](https://arxiv.org/abs/2510.01758)
*Bruno Corcuera,Carlos Eiras-Franco,Brais Cancela*

Main category: cs.LG

TL;DR: 提出了一种无监督动态特征选择方法，通过移除图像中的误导性或冗余信息来增强潜在表示，提高模型泛化性能。
...
### [233] [Octax: Accelerated CHIP-8 Arcade Environments for Reinforcement Learning in JAX](https://arxiv.org/abs/2510.01764)
*Waris Radji,Thomas Michel,Hector Piteau*

Main category: cs.LG

TL;DR: Octax是一个基于JAX的高性能经典街机游戏环境套件，为强化学习研究提供GPU加速的Atari替代方案，实现数量级的速度提升并保持原始游戏机制。


<details>
  <summary>Details</summary>
Motivation: 现代视频游戏计算成本高且不适合大规模实验，需要提供高性能、可扩展的强化学习环境。

Method: 基于CHIP-8仿真在JAX中实现经典街机游戏环境，提供端到端的GPU加速方案。

Result: 相比传统CPU仿真器实现数量级加速，在多个游戏中训练RL智能体显示训练速度和可扩展性的显著提升。

Conclusion: Octax是理想的大规模强化学习实验平台，其模块化设计便于扩展新游戏或生成新环境。
...
TL;DR: 提出G²RPO框架，通过奇异随机采样策略和多粒度优势集成模块，解决流模型强化学习中奖励信号稀疏和窄化的问题，实现更精确全面的采样方向评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法在探索高价值样本时有效，但由于稀疏和窄化的奖励信号，导致偏好对齐效果不佳。

Method: 采用奇异随机采样策略实现逐步随机探索，确保奖励与注入噪声高度相关；引入多粒度优势集成模块，聚合多个扩散尺度的优势计算，提供更全面的采样方向评估。
...
### [235] [Sensitivity, Specificity, and Consistency: A Tripartite Evaluation of Privacy Filters for Synthetic Data Generation](https://arxiv.org/abs/2510.01793)
*Adil Koeken,Alexander Ziller,Moritz Knolle,Daniel Rueckert*

Main category: cs.LG

TL;DR: 对胸部X光合成数据应用后处理隐私过滤器的严格评估显示，现有过滤器在特异性、一致性和检测训练数据生成的近重复样本方面表现有限，无法可靠保护患者隐私。
...
Conclusion: 后处理过滤方法存在关键限制，无法有效保护患者隐私，反而可能提供虚假安全感，在敏感应用部署前需要过滤器设计的实质性改进。
...
### [236] [Rethinking the shape convention of an MLP](https://arxiv.org/abs/2510.01796)
*Meng-Hsi Chen,Yu-Ang Lee,Feng-Ting Liao,Da-shan Shiu*

Main category: cs.LG

TL;DR: 提出了一种宽-窄-宽（沙漏）MLP块结构，与传统窄-宽-窄MLP设计相反，在扩展维度上使用跳跃连接，通过窄瓶颈进行残差计算，在生成任务中实现了更好的性能-参数帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 挑战传统MLP的窄-宽-窄设计惯例，探索在扩展维度上使用跳跃连接的可能性，以利用高维空间进行增量细化，同时通过参数匹配设计保持计算效率。

Method: 提出沙漏MLP块结构，其中跳跃连接在扩展维度上操作，残差计算流经窄瓶颈。使用随机初始化的固定投影将输入信号提升到扩展维度，实现高效训练和推理。通过系统架构搜索评估性能-参数帕累托前沿。

Result: 沙漏架构在生成任务中始终优于传统设计，获得更优的帕累托前沿。随着参数预算增加，最优沙漏配置倾向于更深的网络、更宽的跳跃连接和更窄的瓶颈，这与传统MLP的缩放模式不同。
...
### [237] [Sparse Query Attention (SQA): A Computationally Efficient Attention Mechanism with Query Heads Reduction](https://arxiv.org/abs/2510.01817)
*Adam Filipek*
...
<details>
  <summary>Details</summary>
Motivation: 解决Transformer中多头注意力机制(MHA)的二次计算复杂度问题，特别是在长上下文应用中。现有方法如MQA和GQA虽然解决了内存带宽瓶颈，但未减少注意力分数计算所需的FLOPs。

Method: 提出SQA架构，减少查询头的数量而非键/值头，直接降低注意力机制的计算复杂度。提供了理论基础、数学公式和架构变体家族。

Result: 在长序列(32k-200k tokens)上的实证基准测试显示，在计算受限场景中(如预训练、微调和编码器任务)可实现高达3倍的吞吐量提升，初步小规模实验中模型质量影响很小。

Conclusion: SQA是在开发Reactive Transformer架构时偶然发现的，有望成为构建更高效和可扩展模型的有力工具。
...
### [238] [Test-Time Anchoring for Discrete Diffusion Posterior Sampling](https://arxiv.org/abs/2510.02291)
...
<details>
  <summary>Details</summary>
Motivation: 利用预训练离散扩散基础模型从噪声测量中恢复图像，无需重新训练任务特定模型。离散扩散提供了统一框架、更快推理、精细控制和无需训练的原则性贝叶斯推理。

Method: 基于两个关键创新：离散嵌入空间中的量化期望实现类梯度引导，以及自适应解码的锚定重掩码。

Result: 在标准基准测试的线性和非线性逆问题上，在离散扩散采样器中实现了最先进的性能。

Conclusion: APS方法在无需训练的风格化和文本引导编辑中展示了优势，为离散扩散后验采样提供了有效解决方案。
...
### [239] [Black-Box Combinatorial Optimization with Order-Invariant Reinforcement Learning](https://arxiv.org/abs/2510.01824)
*Olivier Goudet,Quentin Suire,Adrien Goëffon,Frédéric Saubion,Sylvain Lamprier*

Main category: cs.LG

TL;DR: 提出了一种顺序不变强化学习框架用于黑盒组合优化，通过随机生成顺序训练自回归生成模型，提高样本效率并避免灾难性失败。
...
### [240] [Continual Personalization for Diffusion Models](https://arxiv.org/abs/2510.02296)
...
<details>
  <summary>Details</summary>
Motivation: 在现实应用中增量更新扩散模型具有实用性，但计算成本高昂。需要一种能够在持续学习方案中执行个性化，同时保持模型原有能力的方法。
...
### [241] [Pre-Hoc Predictions in AutoML: Leveraging LLMs to Enhance Model Selection and Benchmarking for Tabular datasets](https://arxiv.org/abs/2510.01842)
*Yannis Belkhiter,Seshu Tirupathi,Giulio Zizzo,Sachin Sharma,John D. Kelleher*

Main category: cs.LG

TL;DR: 本文提出了一种结合传统模型和大型语言模型（LLM）代理的预预测方法，用于减少AutoML库的搜索空间，显著降低计算开销，同时仍能为给定数据集选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当前AutoML方法依赖耗尽的超参数搜索，需要自动训练和测试不同类型模型。预预测作为一种有前景的替代方案，能够通过智能预选模型来绕过这种搜索，但在文献中仍未被充分探索。

Method: 利用传统模型和LLM代理，基于数据集描述和统计信息来减少AutoML搜索空间。该方法在AWS AutoGluon投资组合数据集（包含175个表格分类数据集）上进行了应用。

Result: 提出的方法显著减少了计算开销，同时仍能选择给定数据集的最佳模型。

Conclusion: 该方法为AutoML工作流程提供了转变，通过预预测方法有效平衡了计算效率和模型性能。
...
### [242] [Equilibrium Matching: Generative Modeling with Implicit Energy-Based Models](https://arxiv.org/abs/2510.02300)
*Runqian Wang,Yilun Du*

Main category: cs.LG

TL;DR: 提出Equilibrium Matching (EqM)生成模型框架，通过平衡动力学视角学习隐式能量景观的平衡梯度，采用基于优化的采样过程，在ImageNet 256×256上达到1.90 FID，性能超越传统扩散/流模型。


<details>
  <summary>Details</summary>
Motivation: 传统扩散和流模型依赖非平衡的时间条件动力学，EqM旨在通过平衡动力学视角构建更简单的生成模型框架。

Method: 丢弃传统模型中的非平衡时间条件动力学，学习隐式能量景观的平衡梯度，在推理时采用基于梯度下降的优化采样过程，支持可调步长、自适应优化器和自适应计算。

Result: 在ImageNet 256×256上达到1.90 FID，超越扩散/流模型性能，并能处理部分噪声图像去噪、OOD检测和图像合成等任务。

Conclusion: EqM通过统一平衡景观替代时间条件速度，在流模型和能量基模型间建立更紧密联系，为优化驱动推理提供简单途径。
...
### [243] [Learning Representations Through Contrastive Neural Model Checking](https://arxiv.org/abs/2510.01853)
*Vladimir Krsmanovic,Matthias Cosler,Mohamed Ghanem,Bernd Finkbeiner*

Main category: cs.LG

TL;DR: CNML是一种新颖的对比学习方法，将模型检查任务作为指导信号，在共享潜在空间中共同嵌入逻辑规范和系统，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习在模型检查中显示出潜力，但表示学习在形式验证领域仍未被充分探索。

Method: 使用自监督对比目标，在共享潜在空间中联合嵌入逻辑规范和系统。

Result: 在行业启发的检索任务中，CNML在跨模态和模态内设置中显著优于算法和神经基线，学习到的表示能有效迁移到下游任务并泛化到更复杂的公式。

Conclusion: 模型检查可以作为学习形式语言表示的目标，为形式验证领域开辟了新方向。
...
### [244] [Explicit Discovery of Nonlinear Symmetries from Dynamic Data](https://arxiv.org/abs/2510.01855)
*Lexiang Hu,Yikang Li,Zhouchen Lin*

Main category: cs.LG

TL;DR: 提出LieNLSD方法，首次能够确定非线性项的无穷小生成器数量及其显式表达式，通过函数库和SVD求解系数矩阵，在动态系统和物理问题上优于现有方法。
...
Method: 为无穷小群作用指定函数库，求解其系数矩阵。证明微分方程的延拓公式对系数矩阵也是线性的。通过数据中心差分和神经网络雅可比矩阵代入无穷小准则，得到系数矩阵的线性方程组，使用SVD求解。

Result: 在顶夸克标记和一系列动态系统上，LieNLSD显示出优于现有方法的定性优势，将神经PDE求解器的长期滚动精度提高了20%以上，同时指导数据增强。

Conclusion: LieNLSD是首个能够确定非线性无穷小生成器数量及其显式表达式的有效方法，在对称性发现任务中表现出色。
...
### [245] [Compositional meta-learning through probabilistic task inference](https://arxiv.org/abs/2510.01858)
...
<details>
  <summary>Details</summary>
Motivation: 解决从最小经验中学习新任务的问题，需要有效重用先前任务的知识，即元学习问题。组合式解决方案特别适合元学习，因为可以灵活重组计算元素。

Method: 学习一个生成模型来捕获任务族共享的底层组件及其统计信息，将学习新任务转化为概率推理问题，通过高度约束的假设测试找到解决方案而无需参数更新。

Result: 模型成功恢复了规则学习和运动学习任务中的真实组件和统计信息，并展示了从单个示例快速推断新解决方案的能力。

Conclusion: 该框架结合了神经网络的表现力和概率推理的数据效率，实现了快速的组合式元学习。
...
### [246] [Universal Dynamic Regret and Constraint Violation Bounds for Constrained Online Convex Optimization](https://arxiv.org/abs/2510.01867)
...
<details>
  <summary>Details</summary>
Motivation: 扩展经典的在线凸优化框架，处理由对手任意选择的成本和约束函数，且约束函数不需要包含任何公共可行点的情况。

Method: 通过将约束学习问题转化为具有特殊构造代理成本函数的标准在线凸优化问题实例。

Result: 获得了通用的动态遗憾和累积约束违反边界，改进了现有最优结果。

Conclusion: 所提出的模块化算法在对抗性约束设置下有效，为约束在线学习提供了新的解决方案。
...
### [247] [Randomized Gradient Subspaces for Efficient Large Language Model Training](https://arxiv.org/abs/2510.01878)
*Sahar Rajabi,Nayeema Nonta,Samanvay Vajpayee,Sirisha Rambhatla*

Main category: cs.LG

TL;DR: 分析梯度空间动态，发现核心子空间随时间衰减，梯度空间呈现近平坦曲率，提出GrassWalk和GrassJump随机算法实现最优内存节省并提升LLaMA预训练性能


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练受限于内存需求，优化器状态占主导。现有方法通过将梯度投影到低维子空间来降低成本，但需要更深入理解梯度空间动态

Method: 分析梯度空间及其子空间动态，发现核心子空间影响随时间衰减，梯度空间具有近平坦曲率特性。基于这些洞察开发GrassWalk和GrassJump随机算法

Result: 在LLaMA-1B和LLaMA-7B预训练中实现最优内存节省同时提升性能

Conclusion: 梯度空间分析揭示了核心子空间动态变化和近平坦曲率特性，基于此开发的随机算法能有效平衡内存效率和模型性能
...
### [248] [Multi-marginal temporal Schrödinger Bridge Matching for video generation from unpaired data](https://arxiv.org/abs/2510.01894)
*Thomas Gravier,Thomas Boyer,Auguste Genovesio*

Main category: cs.LG

TL;DR: 提出MMtSBM方法，用于从非配对数据生成视频，通过多边际Schrödinger桥匹配来重建隐藏的动态过程。
...
Method: 扩展Diffusion Schrödinger Bridge Matching，推导出迭代马尔可夫拟合算法，以因子化方式处理多边际问题。
...
### [249] [Multimodal Foundation Models for Early Disease Detection](https://arxiv.org/abs/2510.01899)
*Md Talha Mohsin,Ismail Abdulrashid*

Main category: cs.LG

TL;DR: 提出一种基于注意力Transformer的多模态基础模型，整合电子健康记录、医学影像、遗传学和可穿戴设备数据，通过专用编码器和多头注意力机制实现跨模态关联，提升早期疾病诊断能力。


<details>
  <summary>Details</summary>
Motivation: 传统诊断模型孤立分析各种医疗数据源，无法捕捉跨模态相关性，限制了早期疾病诊断的能力。需要整合多样化患者数据来改善预测准确性。

Method: 使用注意力Transformer框架，先通过专用编码器将各模态数据映射到共享潜在空间，然后采用多头注意力和残差归一化进行融合，支持多任务预训练和轻松适应新疾病。

Result: 在肿瘤学、心脏病学和神经学基准数据集上测试早期检测任务，模型包含数据治理和模型管理工具，提高透明度、可靠性和临床可解释性。
...
### [250] [A Methodology for Transparent Logic-Based Classification Using a Multi-Task Convolutional Tsetlin Machine](https://arxiv.org/abs/2510.01906)
*Mayur Kishor Shende,Ole-Christoffer Granmo,Runar Helin,Vladimir I. Zadorozhny,Rishad Shafik*

Main category: cs.LG

TL;DR: 本文探索了Tsetlin Machine(TM)在大规模多通道图像分类中的应用，提出了一种生成局部解释和全局类别表示的方法，在保持可解释性的同时实现了与深度学习模型相当的分类性能。


<details>
  <summary>Details</summary>
Motivation: Tsetlin Machine作为一种基于有限状态自动机和命题逻辑的新型机器学习范式，比神经网络更具可解释性。本文旨在探索TM架构在大规模多通道图像分类任务中的适用性。

Method: 提出了一种生成局部解释和全局类别表示的方法论。局部解释用于说明模型预测，全局类别表示聚合每个类别的重要模式。这些解释总结了卷积子句捕获的知识，可被可视化为图像。

Result: 在MNIST数据集上达到98.5%准确率，在CelebA数据集上达到86.56% F1分数（相比ResNet50的88.07%）。TM在保持可解释性的同时，性能与深度学习模型相当。

Conclusion: TM在复杂大规模训练环境中仍能保持竞争力，同时维持其可解释性优势。这有助于更好地理解TM子句，并为将TM应用于更复杂多样的数据集提供了见解。
...
### [251] [Are LLMs Better GNN Helpers? Rethinking Robust Graph Learning under Deficiencies with Iterative Refinement](https://arxiv.org/abs/2510.01910)
*Zhaoyan Wang,Zheng Gao,Arogya Kharel,In-Young Ko*

Main category: cs.LG

TL;DR: 该论文提出了RoGRAD框架，通过检索增强生成和迭代图对比学习来解决图神经网络在复合缺陷下的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图数据存在多种缺陷，但现有研究缺乏对图原生方法和LLM增强方法在复合缺陷下表现的全面比较，且LLM增强方法是否始终优于传统方法尚不明确。

Method: 提出RoGRAD框架，采用检索增强生成(RAG)注入类别一致且多样化的增强，通过迭代图对比学习强制区分性表示，将LLM增强从静态信号注入转变为动态精炼。

Result: RoGRAD在广泛实验中表现出色，相比传统GNN和LLM增强基线平均提升达82.43%。

Conclusion: RoGRAD是首个迭代范式，通过动态精炼方法显著提升了图神经网络在复合缺陷下的鲁棒性，挑战了LLM增强始终优于传统方法的假设。
...
### [252] [StelLA: Subspace Learning in Low-rank Adaptation using Stiefel Manifold](https://arxiv.org/abs/2510.01938)
...
<details>
  <summary>Details</summary>
Motivation: 传统LoRA在性能上仍落后于全参数微调，部分原因是未能充分利用低秩流形的几何结构。

Method: 使用三因子分解U S V⊤，将U和V约束在Stiefel流形上保持正交性，采用几何优化设计将欧几里得优化器转换为黎曼优化器。

Result: 在常识推理、数学和代码生成、图像分类和图像生成等广泛下游任务中，性能优于当前最先进的LoRA变体。

Conclusion: 提出的几何感知LoRA扩展方法在保持与现有微调流程兼容的同时，实现了更优越的性能。
...
### [253] [Lower Bounds on Adversarial Robustness for Multiclass Classification with General Loss Functions](https://arxiv.org/abs/2510.01969)
*Camilo Andrés García Trillos,Nicolás García Trillos*

Main category: cs.LG

TL;DR: 该论文针对多类别设置下的对抗鲁棒分类问题，在任意损失函数下推导了学习者无关鲁棒风险最小化问题的对偶和重心重构形式，特别针对交叉熵损失、幂形式损失和二次损失提供了显式表征，揭示了对抗鲁棒性与α-公平打包问题及广义重心问题之间的有趣联系。


<details>
  <summary>Details</summary>
Motivation: 扩展对抗鲁棒分类研究，超越传统的0-1损失设置，探索在任意损失函数下的鲁棒风险最小化问题，以设计更有效的鲁棒分类器。

Method: 通过推导对偶和重心重构形式，为不同损失函数（交叉熵、幂形式、二次损失）提供显式表征，建立与α-公平打包问题和广义重心问题的理论联系。

Result: 获得了对抗风险的更紧下界，特别是在交叉熵损失函数下，通过数值实验验证了理论结果的有效性。

Conclusion: 该研究为超越0-1损失设置的对抗鲁棒分类提供了新的理论框架和计算方法，揭示了对抗鲁棒性与优化理论中其他重要问题的深刻联系。
...
### [254] [Moon: A Modality Conversion-based Efficient Multivariate Time Series Anomaly Detection](https://arxiv.org/abs/2510.01970)
*Yuanyuan Yao,Yuhan Shi,Lu Chen,Ziquan Fang,Yunjun Gao,Leong Hou U,Yushuai Li,Tianyi Li*

Main category: cs.LG

TL;DR: 提出Moon框架，通过多变量马尔可夫转移场将数值时间序列转换为图像表示，结合多模态CNN融合数值和图像特征，使用SHAP解释器提高异常检测的可解释性，在效率和准确性上优于现有方法。
...
Method: 1. 提出MV-MTF技术将数值时间序列转换为图像表示；2. 使用多模态CNN通过参数共享融合数值和图像特征；3. 采用SHAP解释器识别关键异常变量。

Result: 在6个真实MTS数据集上的实验表明，Moon在效率上提升达93%，准确性提升4%，解释性能提升10.8%，优于6种最先进方法。
...
### [255] [Private Federated Multiclass Post-hoc Calibration](https://arxiv.org/abs/2510.01987)
*Samuel Maddock,Graham Cormode,Carsten Maple*

Main category: cs.LG

TL;DR: 该论文将传统的后处理模型校准方法（如直方图分箱和温度缩放）引入联邦学习环境，针对客户端异构性设计了新方法，并在联邦学习和差分隐私设置下评估校准效果。
...
Method: 将集中式校准方法（直方图分箱和温度缩放）转移到联邦环境，针对客户端异构性设计新方法，包括加权分箱方法。

Result: 研究发现联邦温度缩放最适合差分隐私联邦学习，而加权分箱方法在不需要差分隐私时表现最佳。

Conclusion: 该工作成功将模型校准集成到联邦学习中，并针对不同隐私需求提供了有效的校准策略。
...
### [256] [PepCompass: Navigating peptide embedding spaces using Riemannian Geometry](https://arxiv.org/abs/2510.01988)
...
TL;DR: PepCompass是一个几何感知的肽探索和优化框架，通过定义解码器诱导的黎曼流形家族来捕捉局部几何结构，提供局部探索方法和优化算法，在体外验证中成功发现了25种高活性抗菌肽。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型忽略了解码器诱导的几何结构，依赖平坦的欧几里得度量，导致肽空间探索和优化失真且低效。现有基于流形的方法假设固定的内在维度，这在肽数据实践中严重失败。

Method: 提出PepCompass框架：1）定义κ-稳定黎曼流形联合体；2）两种局部探索方法：二阶黎曼布朗高效采样和切空间突变枚举；3）局部枚举贝叶斯优化算法；4）势最小化测地线搜索方法。

Result: 体外验证证实了PepCompass的有效性：PoGS产生了4个新型种子肽，随后通过LE-BO优化发现了25种具有广谱活性的高活性肽，包括对耐药菌株的活性。

Conclusion: 几何信息探索为抗菌肽设计提供了一个强大的新范式。
...
### [257] [Normality Calibration in Semi-supervised Graph Anomaly Detection](https://arxiv.org/abs/2510.02014)
*Guolei Zeng,Hezhe Qiao,Guoguo Ai,Jinsong Guo,Guansong Pang*

Main category: cs.LG

TL;DR: GraphNC是一个图异常检测框架，通过异常分数分布对齐和基于扰动的正态性正则化，利用标记和未标记数据校准教师模型的异常分数和节点表示，提高检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的半监督图异常检测方法仅基于标记的正常节点学习正态性，容易过拟合给定模式，导致高误报率。需要利用未标记数据来校准正态性。

Method: GraphNC包含两个组件：异常分数分布对齐(ScoreDA)通过对齐教师模型的分数分布来优化异常分数；基于扰动的正态性正则化(NormReg)通过在表示空间最小化扰动引导的一致性损失，使正常节点表示更紧凑。

Result: 分数对齐使正常和异常类别的异常分数向两端分离，产生更可分的异常分数；正则化缓解了教师模型不准确分数的误导影响。

Conclusion: GraphNC通过联合校准异常分数和节点表示空间的正态性，有效提高了半监督图异常检测的性能，减少了误报率。
...
### [258] [FairContrast: Enhancing Fairness through Contrastive learning and Customized Augmenting Methods on Tabular Data](https://arxiv.org/abs/2510.02017)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统在日常生活中更广泛地应用，开发公平无偏的模型变得至关重要。虽然表示学习框架在去偏见方面已被证明有效，但表格数据中学习公平表示的问题仍未充分探索。

Method: 设计了一个对比学习框架，专门用于处理表格数据集中的偏见问题，通过策略性选择正样本对，结合监督和自监督对比学习方法。

Result: 与现有的最先进表格数据对比学习模型相比，该方法显著减少了偏见，同时在准确性方面仅有最小程度的权衡，学习到的公平表示可在各种下游任务中有效应用。

Conclusion: 该研究证明了所提出框架在减轻表格数据偏见方面的有效性，为开发更公平的AI系统提供了有前景的方法。
...
### [259] [Mathematical Modeling and Convergence Analysis of Deep Neural Networks with Dense Layer Connectivities in Deep Learning](https://arxiv.org/abs/2510.02049)
*Jinshu Huang,Haibin Su,Xue-Cheng Tai,Chunlin Wu*

Main category: cs.LG

TL;DR: 该论文从数学角度建模密集连接的深度神经网络，使用非线性积分方程框架分析其在深层极限下的学习问题，证明了从离散网络到连续时间对应问题的收敛性。


<details>
  <summary>Details</summary>
Motivation: 密集连接已成为深度神经网络的关键设计原则，但缺乏严格的数学分析。本文旨在为密集连接的DNNs提供数学基础，理解其训练稳定性。

Method: 提出密集非局部（DNL）框架，将密集连接网络建模为非线性积分方程，从最优控制角度研究训练问题，使用分段线性扩展和Γ-收敛分析证明收敛性。

Result: 证明了最优值的收敛性以及极小化子的子序列收敛性，表明密集连接架构可以提供深度模型训练的稳定性。

Conclusion: 该工作为理解密集连接的DNNs提供了数学基础，表明此类架构能够提供深度模型训练的稳定性，与普通微分方程方法形成对比。
...
### [260] [Adaptive Heterogeneous Mixtures of Normalising Flows for Robust Variational Inference](https://arxiv.org/abs/2510.02056)
*Benjamin Wiriyapong,Oktay Karakuş,Kirill Sidorov*

Main category: cs.LG

TL;DR: AMF-VI是一种自适应混合流变分推理方法，通过异质混合互补流模型（MAF、RealNVP、RBIG）的两阶段训练，在多种后验分布上实现更稳定和鲁棒的推理性能。


<details>
  <summary>Details</summary>
Motivation: 单一流模型在不同性质的后验分布上表现不一致，需要一种能够适应多种分布形状和模态的鲁棒变分推理方法。

Method: 采用两阶段训练：1) 顺序专家训练单个流模型；2) 通过似然驱动的更新进行自适应全局权重估计，无需逐样本门控或架构更改。

Result: 在六种典型后验分布上，AMF-VI始终获得比单一流基准更低的负对数似然，并在Wasserstein-2和MMD等传输指标上实现稳定增益。

Conclusion: 自适应混合多样化流模型为跨不同后验家族的鲁棒变分推理提供了可靠途径，同时保留了每个专家的归纳偏置。
...
### [261] [Inferring Optical Tissue Properties from Photoplethysmography using Hybrid Amortized Inference](https://arxiv.org/abs/2510.02073)
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型虽然能提取PPG波形中的丰富生理信息，但依赖特征生理意义不明确，在预测能力、临床可解释性和传感器设计之间存在矛盾。

Method: 提出PPGen生物物理模型，建立PPG信号与可解释参数的关系，并开发混合摊销推理(HAI)方法进行参数估计和模型失配校正。

Result: 在广泛的仿真实验中，HAI能够在不同噪声和传感器条件下准确推断生理参数。

Conclusion: 该方法为保留深度学习特征保真度同时支持临床解释和硬件设计的PPG模型提供了一条路径。
...
TL;DR: 本文提出通过最大似然估计对Flow Matching模型进行微调，以解决训练-推理差距问题，并在图像生成和机器人操作任务中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: Flow Matching模型在训练时无法评估输出质量，存在训练-推理差距，特别是在需要高精度的机器人操作任务中。同时，FM对预定义直线路径的过度追求可能导致系统僵硬等问题。

Method: 提出基于最大似然估计的微调方法，包括直接微调和基于残差的微调。通过特殊设计的架构，基于残差的微调可以将收缩特性融入模型，提高模型的鲁棒性和可解释性。

Result: 在图像生成和机器人操作任务上的实验结果表明，该方法可靠地提升了Flow Matching模型的推理性能。

Conclusion: 通过最大似然估计对FM进行微调是有效的，能够改善训练-推理差距问题，提高模型在生成任务中的性能表现。
...
### [263] [KAIROS: Unified Training for Universal Non-Autoregressive Time Series Forecasting](https://arxiv.org/abs/2510.02084)
*Kuiye Ding,Fanda Fan,Zheya Wang,Hongxiao Li,Yifan Wang,Lei Wang,Chunjie Luo,Jianfeng Zhan*

Main category: cs.LG

TL;DR: KAIROS是一个非自回归时间序列预测框架，直接建模分段多峰分布，避免了误差累积，实现了即时推理，并在六个基准测试中表现出强大的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在万维网应用中，可靠的时间序列预测对于资源规划、缓存放置和异常响应至关重要，需要更快的响应速度来支持实时决策。

Method: KAIROS采用非自回归方法直接建模分段级别的多峰分布，避免了传统自回归方法的误差累积问题，同时改进了现有非自回归模型过度平滑的预测。

Result: 在大规模语料库上训练的KAIROS在六个广泛使用的基准测试中表现出强大的零样本泛化能力，其预测性能与类似规模的先进基础模型相当，但推理成本显著降低。
...
### [264] [Learning Model Representations Using Publicly Available Model Hubs](https://arxiv.org/abs/2510.02096)
*Damian Falk,Konstantin Schürholt,Konstantinos Tzevelekakis,Léo Meynent,Damian Borth*

Main category: cs.LG

TL;DR: 该论文提出了一种直接从Hugging Face等非结构化模型仓库学习权重空间表示的方法，无需依赖精心构建的模型动物园，解决了权重空间学习对大规模计算资源的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 传统权重空间学习需要大量精心训练的模型动物园，这需要巨大的计算资源且限制了权重空间表示的规模和灵活性。论文旨在通过直接从非结构化模型仓库学习来克服这一限制。

Method: 提出了一种新的权重空间骨干网络，专门设计用于处理异构模型群体，能够处理不同架构、数据集和文档不完整的模型。

Result: 在Hugging Face模型上训练的权重空间表示表现出强大的性能，通常优于在实验室生成的模型动物园上训练的骨干网络，并且能够泛化到未见过的数据模态。

Conclusion: 研究表明高质量权重空间表示可以在非结构化环境中学习，证明了精心构建的模型动物园并非不可或缺，克服了权重空间学习社区当前面临的重要限制。
...
TL;DR: 提出了PENEX（惩罚指数损失），一种新的多类指数损失公式，可替代AdaBoost进行深度神经网络训练，具有正则化效果和隐式最大化边界的特点。


<details>
  <summary>Details</summary>
Motivation: AdaBoost在实践中随着弱学习器数量增加而泛化良好，但其指数损失难以通过一阶方法优化。需要一种理论上有依据且易于优化的替代方案。

Method: 提出PENEX损失函数，通过梯度下降优化，隐式参数化弱学习器，并最大化数据点的边界。
...
### [266] [GRACE: A Language Model Framework for Explainable Inverse Reinforcement Learning](https://arxiv.org/abs/2510.02180)
*Silvia Sapora,Devon Hjelm,Alexander Toshev,Omar Attia,Bogdan Mazoure*

Main category: cs.LG

TL;DR: GRACE使用大型语言模型和进化搜索，从专家轨迹中逆向工程出可解释的基于代码的奖励函数，解决了传统逆强化学习黑盒模型难以解释的问题。


<details>
  <summary>Details</summary>
Motivation: 传统逆强化学习方法产生难以解释和调试的"黑盒"奖励模型，需要开发可解释的奖励函数表示方法。

Method: 使用大型语言模型在进化搜索框架中，直接从专家轨迹逆向工程出基于代码的可执行奖励函数。

Result: 在BabyAI和AndroidWorld基准测试中高效学习到高精度奖励，在多任务设置中构建复杂奖励API，产生的奖励能训练出优于模仿学习和在线强化学习的策略。

Conclusion: GRACE成功实现了从专家轨迹生成可解释、可验证的代码奖励函数，在复杂多任务环境中表现出色。
...
### [267] [Hybrid Deep Learning Modeling Approach to Predict Natural Gas Consumption of Home Subscribers on Limited Data](https://arxiv.org/abs/2510.02115)
...
<details>
  <summary>Details</summary>
Motivation: 伊朗作为世界第二大天然气储量国，面临人口增长和能源消耗增加导致的冬季压力下降和天然气中断问题，需要控制居民部门（最大消费部门）的天然气消费。
...
Conclusion: 机器学习方法特别是混合模型可有效管理和预测天然气消费，地理和气候因素对预测建模至关重要，有助于更高效的资源管理和减少季节性短缺。
...
### [268] [Detection of Chagas Disease from the ECG: The George B. Moody PhysioNet Challenge 2025](https://arxiv.org/abs/2510.02202)
...
<details>
  <summary>Details</summary>
Motivation: 恰加斯病是一种寄生虫感染，慢性阶段可导致心血管疾病和消化问题。由于血清学检测能力有限，但恰加斯心肌病常在心电图中表现，因此需要开发算法来优先识别需要检测和治疗的患者。
...
Conclusion: 该挑战赛通过创新的数据集构建、数据增强策略和评估指标设计，为从心电图识别恰加斯病提供了有效的算法开发平台，有助于优化医疗资源分配和患者管理。
...
### [269] [Ensemble Threshold Calibration for Stable Sensitivity Control](https://arxiv.org/abs/2510.02116)
*John N. Daras*

Main category: cs.LG

TL;DR: 提出了一个端到端框架，通过神经网络排序器和多方法阈值估计集成，在大规模空间匹配任务中实现精确的召回率控制，显著降低方差和冗余验证。


<details>
  <summary>Details</summary>
Motivation: 在大规模空间融合和实体匹配任务中，精确的召回率控制至关重要，因为遗漏少量真实匹配会破坏下游分析，而过度的手动审查会增加成本。传统的置信区间方法通常超出目标几个百分点，且在偏斜分数分布下运行方差较高。

Method: 使用等网格边界框过滤器和压缩稀疏行候选表示减少对枚举；通过确定性xxHash引导样本训练轻量级神经网络排序器；构建可重现的分数十分位分层校准集；集成四种阈值估计方法（Clopper-Pearson、Jeffreys、Wilson和精确分位数），通过逆方差加权和跨九个独立子样本融合。

Result: 在两个真实地籍数据集（约631万和6734万对）上评估，该方法始终在较小误差内达到召回目标，相比其他校准方法减少了冗余验证，并在单个TPU v3核心上端到端运行。

Conclusion: 该框架在大规模几何对匹配中实现了精确的召回控制，具有亚百分位方差，显著优于传统方法，同时保持TPU友好的计算特性。
...
### [270] [DiFFPO: Training Diffusion LLMs to Reason Fast and Furious via Reinforcement Learning](https://arxiv.org/abs/2510.02212)
*Hanyang Zhao,Dawen Liang,Wenpin Tang,David Yao,Nathan Kallus*

Main category: cs.LG

TL;DR: DiFFPO是一个统一的框架，通过强化学习训练掩码扩散大语言模型，使其推理更好更快。它包含两个主要创新：基于离策略RL训练代理策略，以及联合训练高效采样器/控制器来优化推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练扩散大语言模型时存在推理效率低的问题，需要平衡推理质量与计算成本。作者希望开发一个框架，既能提高推理质量，又能通过自适应推理策略减少计算开销。

Method: 1. 使用离策略RL训练代理策略，采用两阶段似然近似和重要性采样校正；2. 联合训练dLLMs的高效采样器/控制器，让模型学习为每个提示自适应分配推理阈值，利用多令牌预测能力。

Result: 在数学和规划基准任务上训练开源大扩散语言模型，结果显示该方法在提高任务性能的同时，显著降低了函数评估次数，优化了推理计算效率的帕累托前沿。

Conclusion: DiFFPO框架成功实现了推理质量与效率的平衡，通过强化学习训练和联合采样器优化，为扩散大语言模型提供了更好的训练范式。
...
### [271] [DAG DECORation: Continuous Optimization for Structure Learning under Hidden Confounding](https://arxiv.org/abs/2510.02117)
*Samhita Pal,James O'quinn,Kaveh Aryan,Heather Pua,James P. Long,Amir Asiaee*

Main category: cs.LG

TL;DR: DECOR是一种用于线性高斯结构方程模型的联合学习方法，能在存在潜在混杂的情况下同时学习DAG和相关噪声模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法在误差独立时表现良好，而去混杂流程依赖于普遍因子结构或非线性。需要一种能在各种混杂密度下稳健工作的统一方法。

Method: 提出DECOR方法，通过交替执行平滑无环图更新和凸噪声更新来联合学习DAG和相关噪声模型，可包含轻量弓形互补惩罚或后处理协调步骤。

Result: 在合成基准测试中，DECOR匹配或优于强基线方法，在混杂非普遍时特别稳健，在普遍混杂下仍保持竞争力。

Conclusion: DECOR提供了一个统一的似然框架，能够在各种混杂设置下有效学习因果结构，特别是在n<p的高维情况下表现优异。
...
### [272] [Catalyst GFlowNet for electrocatalyst design: A hydrogen evolution reaction case study](https://arxiv.org/abs/2510.02142)
...
TL;DR: 提出了一种名为Catalyst GFlowNet的生成模型，利用机器学习预测形成能和吸附能来设计高效催化剂晶体表面，以解决氢能存储中催化剂开发难题。


<details>
  <summary>Details</summary>
Motivation: 开发廉价高效的电催化剂对于加速可再生能源采用和确保稳定能源供应至关重要，但目前缺乏负担得起的高性能催化剂。

Method: 使用基于机器学习的形成能和吸附能预测器，构建生成模型来设计催化剂晶体表面，以氢析出反应作为概念验证应用。

Result: 成功识别出铂作为已知最高效的催化剂，证明了模型的有效性。
...
### [273] [Policy Gradient Guidance Enables Test Time Control](https://arxiv.org/abs/2510.02148)
*Jianing Qi,Hao Tang,Zhigang Zhu*

Main category: cs.LG

TL;DR: 将扩散模型中的分类器无关引导方法扩展到经典策略梯度方法，提出策略梯度引导(PGG)，通过插值条件分支和无条件分支实现无需重新训练的行为调节。


<details>
  <summary>Details</summary>
Motivation: 将扩散模型中成功的引导机制引入强化学习，为在线强化学习提供可控性，无需重新训练即可调节行为。

Method: 在策略梯度中增加无条件分支，插值条件分支和无条件分支，理论推导显示在优势估计下额外归一化项消失，得到简洁的引导策略梯度更新公式。
...
### [274] [ExGRPO: Learning to Reason from Experience](https://arxiv.org/abs/2510.02245)
*Runzhe Zhan,Yafu Li,Zhi Wang,Xiaoye Qu,Dongrui Liu,Jing Shao,Derek F. Wong,Yu Cheng*

Main category: cs.LG

TL;DR: 提出了ExGRPO框架，通过识别推理经验的价值指标（正确性和熵）来组织和优先处理有价值的经验，使用混合策略目标平衡探索与经验利用，显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准在线策略训练在单次更新后丢弃经验，导致计算效率低下和不稳定性。现有研究对经验特征在大推理模型学习动态中的作用探索不足。

Method: 提出ExGRPO框架，识别推理经验的价值指标（正确性和熵），组织和优先处理有价值经验，使用混合策略目标平衡探索与经验利用。

Result: 在五个骨干模型（1.5B-8B参数）上，ExGRPO在数学/通用基准上平均比在线策略RLVR提升+3.5/7.6分，并在强弱模型上都能稳定训练。

Conclusion: 原则性经验管理是高效可扩展RLVR的关键要素，ExGRPO通过经验价值识别和利用显著提升了推理性能。
...
### [275] [Reinforcement Learning with Action-Triggered Observations](https://arxiv.org/abs/2510.02149)
*Alexander Ryabchenko,Wenlong Mou*

Main category: cs.LG

TL;DR: 本文研究了动作触发随机观测的强化学习问题，提出了ATST-MDP框架，并开发了ST-LSVI-UCB算法，在稀疏观测约束下实现了高效学习。


<details>
  <summary>Details</summary>
Motivation: 现实应用中状态观测通常由动作随机触发，传统强化学习假设每个时间步都能获得状态观测，这与许多实际场景不符。

Method: 提出动作序列学习范式，智能体承诺执行动作序列直到下一次观测到达；在线性MDP假设下，推导了动作序列特征映射的线性表示；开发了具有统计误差保证的离策略估计器和ST-LSVI-UCB算法。

Result: ST-LSVI-UCB实现了$\widetilde O(\sqrt{Kd^3(1-\gamma)^{-3}})$的遗憾界，其中K是回合数，d是特征维度，γ是折扣因子。

Conclusion: 为稀疏动作触发观测的学习建立了理论基础，证明了在此类观测约束下高效学习仍然是可行的。
...
### [276] [Flatness-Aware Stochastic Gradient Langevin Dynamics](https://arxiv.org/abs/2510.02174)
*Stefano Bruno,Youngsik Hwang,Jaehyeon An,Sotirios Sabanis,Dong-Young Lim*

Main category: cs.LG

TL;DR: 提出了Flatness-Aware SGLD (fSGLD)方法，通过随机权重扰动优化随机平滑目标，有效寻找平坦最小值，在保持SGD计算成本的同时实现更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统SGLD无法偏向平坦最小值，而深度学习泛化与平坦最小值密切相关，需要一种能有效寻找平坦最小值的优化方法。

Method: fSGLD在每次迭代中使用各向同性高斯噪声扰动参数后的随机梯度，优化随机平滑目标来隐式捕获曲率信息。

Result: 理论证明fSGLD的平稳测度集中在Hessian迹正则化损失函数的全局最小化器附近；实验在噪声标签和大规模视觉任务中显示fSGLD实现优越泛化和鲁棒性，计算成本仅为SGD水平。

Conclusion: fSGLD是首个能高效且可证明地寻找平坦最小值的SGLD变体，为随机权重扰动提供了严格理论解释，在保持计算效率的同时显著改善泛化性能。
...
### [277] [How to Combat Reactive and Dynamic Jamming Attacks with Reinforcement Learning](https://arxiv.org/abs/2510.02265)
*Yalin E. Sagduyu,Tugba Erpek,Kemal Davaslioglu,Sastry Kompella*

Main category: cs.LG

TL;DR: 使用强化学习来对抗动态反应式干扰，通过调整发射功率、调制方式和信道选择来优化吞吐量，无需预先了解信道条件或干扰策略。
...
Method: 采用Q-learning处理离散干扰事件状态，使用深度Q网络（DQN）处理基于接收功率的连续状态，通过不同的奖励函数和动作集来学习最优策略。

Result: 结果显示强化学习能够快速适应频谱动态变化，在信道和干扰策略随时间变化时仍能维持高传输速率。

Conclusion: 强化学习是应对动态反应式干扰的有效方法，能够在不了解先验知识的情况下自适应优化通信性能。
...
### [278] [Poolformer: Recurrent Networks with Pooling for Long-Sequence Modeling](https://arxiv.org/abs/2510.02206)
*Daniel Gallo Fernández*

Main category: cs.LG

TL;DR: Poolformer是一个序列到序列模型，用循环层替代自注意力机制，并通过池化操作减少序列长度，在长序列处理上优于现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制在长序列处理时存在二次方复杂度问题，限制了其实际应用。需要开发更高效的序列建模方法。

Method: 使用循环层替代自注意力，结合池化操作减少序列长度。模型采用递归定义的SkipBlocks结构，包含残差块、下池化层、嵌套SkipBlock、上池化层等组件。

Result: 池化显著加速训练，改善感知指标（FID和IS），防止过拟合。在原始音频处理上优于SaShiMi和Mamba等最先进模型。

Conclusion: Poolformer为长序列处理提供了高效解决方案，未来可应用于文本、视觉和多模态场景。
...
### [279] [Addressing Pitfalls in the Evaluation of Uncertainty Estimation Methods for Natural Language Generation](https://arxiv.org/abs/2510.02279)
*Mykyta Ielanskyi,Kajetan Schweighofer,Lukas Aichberger,Sepp Hochreiter*

Main category: cs.LG

TL;DR: 本文分析了LLM幻觉检测中不确定性估计方法的评估问题，提出了更稳健的风险指标和Elo评级系统来改进评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM不确定性估计方法的评估存在缺陷，常用的近似正确性函数之间存在显著分歧，导致评估结果不可靠，容易被操纵。

Method: 提出使用多种替代风险指标进行风险相关性实验，包括在QA任务中对多个LLM-as-a-judge变体进行边缘化，探索结构化任务、分布外和扰动检测任务，并使用Elo评级系统进行综合评估。

Result: 研究表明，在QA任务中通过边缘化多个LLM评判变体可以减少评估偏差，结构化任务和扰动检测任务提供了更稳健的风险指标。

Conclusion: 提出的多指标评估框架和Elo评级系统能够提供更客观、稳健的不确定性估计算法评估，改进LLM幻觉检测的可靠性。
...
### [280] [StockBench: Can LLM Agents Trade Stocks Profitably In Real-world Markets?](https://arxiv.org/abs/2510.02209)
...
<details>
  <summary>Details</summary>
Motivation: 现有金融基准主要测试静态知识问答，无法捕捉交易的动态迭代特性，而金融领域对经济价值和决策至关重要但研究不足。

Method: 构建无污染基准，让智能体接收每日市场信号（价格、基本面、新闻），做出买卖持有决策，使用累计回报、最大回撤、Sortino比率等金融指标评估。

Result: 大多数LLM智能体难以超越简单的买入持有基准，但部分模型显示出获得更高回报和更好风险管理的能力。

Conclusion: 在静态金融知识任务表现出色不一定能转化为成功的交易策略，StockBench作为开源资源支持可复现性和未来研究。
...
### [281] [C2AL: Cohort-Contrastive Auxiliary Learning for Large-scale Recommendation Systems](https://arxiv.org/abs/2510.02215)
...
Method: 利用注意力机制在因子分解机中进行共享嵌入选择，通过分析数据子结构并暴露具有强分布对比的子结构进行辅助学习，使用部分冲突的辅助标签来正则化共享表示。

Result: 在包含数十亿数据点的生产数据集上评估六个SOTA模型，因子分解机能够捕获细粒度用户-广告交互，整体归一化熵降低0.16%，在目标少数群体上增益超过0.30%。

Conclusion: 该方法通过定制注意力层学习过程，在保持与少数群体互信息的同时提升全局性能，有效解决了推荐模型中的分布不平衡问题。
...
### [282] [Tree-based Dialogue Reinforced Policy Optimization for Red-Teaming Attacks](https://arxiv.org/abs/2510.02286)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在多轮交互场景下容易受到对抗攻击，而现有方法主要依赖人工红队测试或预定义模板，无法充分探索多轮攻击的复杂对话动态和策略规划空间。

Method: 将对话建模为序列决策问题，结合树搜索的在线策略强化学习框架，无需人工标注数据即可自主发现多样化的多轮攻击策略。

Result: 在10个目标模型上相比现有最优方法攻击成功率提高25.9%以上，并能有效发现新的攻击策略。

Conclusion: DialTree-RPO能够系统性地探索多轮攻击空间，学习最大化攻击成功率的对话策略，显著提升了对抗攻击的发现能力。
...
### [283] [Diffusion Transformers for Imputation: Statistical Efficiency and Uncertainty Quantification](https://arxiv.org/abs/2510.02216)
*Zeqi Ye,Minshuo Chen*

Main category: cs.LG

TL;DR: 本文研究了基于扩散变换器的时序数据插补方法的统计效率，推导了样本复杂度边界，构建了缺失值的置信区域，并揭示了缺失模式对插补性能的影响。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型在时序数据插补中表现出色，但其如何捕捉缺失值与观测值间复杂时空依赖关系的理论理解仍然有限。

Method: 基于条件扩散变换器，推导了条件评分函数的近似理论，建立了统计样本复杂度边界，并提出了混合掩码训练策略。

Result: 理论分析表明插补效率和准确性受缺失模式显著影响，通过仿真验证了理论见解，混合掩码训练提升了插补性能。

Conclusion: 扩散变换器能够有效捕捉时序数据的复杂依赖关系，为缺失值插补提供了理论保证和实用改进方法。
...
### [284] [Efficiently Generating Correlated Sample Paths from Multi-step Time Series Foundation Models](https://arxiv.org/abs/2510.02224)
...
TL;DR: 提出一种基于copula的方法，能够从现有的多步时间序列基础模型中高效生成具有相关性的样本路径，相比自回归采样速度提升数个数量级，并改善了样本路径质量。
...
### [285] [Interactive Training: Feedback-Driven Neural Network Optimization](https://arxiv.org/abs/2510.02297)
*Wentao Zhang,Yang Young Lu,Yuntian Deng*

Main category: cs.LG

TL;DR: 提出了Interactive Training框架，实现神经网络训练过程中的人工或AI代理实时干预，通过控制服务器动态调整超参数、训练数据和模型检查点，提升训练稳定性和适应性。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练采用固定优化方案，缺乏对训练不稳定或问题的动态响应能力，需要更灵活的训练范式。

Method: 使用控制服务器作为中介，允许用户或AI代理在训练过程中实时调整优化器超参数、训练数据和模型检查点。

Result: 通过三个案例研究表明，该框架实现了更好的训练稳定性、降低了对初始超参数的敏感性，并提高了对用户需求变化的适应性。

Conclusion: 为AI代理自主监控训练日志、主动解决不稳定问题和优化训练动态的未来训练范式铺平了道路。
...
### [286] [xLSTM Scaling Laws: Competitive Performance with Linear Time-Complexity](https://arxiv.org/abs/2510.02228)
*Maximilian Beck,Kajetan Schweighofer,Sebastian Böck,Sebastian Lehner,Sepp Hochreiter*

Main category: cs.LG

TL;DR: 本文比较了Transformer和xLSTM的扩展规律，发现在典型LLM训练和推理场景中，xLSTM比Transformer具有更好的扩展性，且随着训练和推理上下文增长，xLSTM的优势更加明显。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer一直是主导架构，但xLSTM等替代方案在保持竞争力的同时提供了线性复杂度。研究两种架构的扩展行为可以为未来模型设计和部署提供指导。

Method: 通过IsoFLOP和参数拟合方法，在广泛模型规模（80M-7B）和训练token数量（2B-2T）上研究xLSTM在计算最优和过训练机制中的扩展行为；考察最优模型大小对上下文长度的依赖性；分析推理时扩展特性。

Result: 在典型LLM训练和推理场景中，xLSTM比Transformer扩展更有利；xLSTM的优势随着训练和推理上下文增长而扩大。
...
### [287] [PUL-Inter-slice Defender: An Anomaly Detection Solution for Distributed Slice Mobility Attacks](https://arxiv.org/abs/2510.02236)
*Ricardo Misael Ayala Molina,Hyame Assem Alameddine,Makan Pourzandi,Chadi Assi*

Main category: cs.LG

TL;DR: 提出了PUL-Inter-Slice Defender，一种基于正未标记学习和LSTM自编码器与K-Means聚类结合的异常检测方案，用于保护5G网络切片免受分布式切片移动攻击。


<details>
  <summary>Details</summary>
Motivation: 5G网络中用户设备可以在多个网络切片间无缝切换，这种灵活性引入了可能被利用来发动分布式切片移动攻击的漏洞，这是一种分布式拒绝服务攻击形式。

Method: 利用正未标记学习，结合长短期记忆自编码器和K-Means聚类，使用3GPP关键性能指标和性能测量计数器作为机器学习模型的特征。

Result: 在基于free5GC和UERANSIM的5G测试床数据上评估，当训练数据包含10%到40%攻击污染时，PUL-Inter-Slice Defender的F1分数超过98.50%，始终优于其对应方案和其他基于PUL的解决方案。

Conclusion: PUL-Inter-Slice Defender能够有效检测分布式切片移动攻击变体，并在存在污染训练数据的情况下保持鲁棒性。
...
### [288] [Diffusion Models and the Manifold Hypothesis: Log-Domain Smoothing is Geometry Adaptive](https://arxiv.org/abs/2510.02305)
*Tyler Farghly,Peter Potaptchik,Samuel Howard,George Deligiannidis,Jakiw Pidstrigach*

Main category: cs.LG

TL;DR: 该论文通过理论和实证研究证实，扩散模型的成功归因于其能够适应数据中的低维几何结构，特别是通过分数匹配中的隐式正则化机制实现沿数据流形的平滑泛化。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在多个领域展现出卓越的泛化能力，但其工作机制尚未完全理解。基于流形假设，研究者推测这种成功源于模型对数据低维几何结构的适应能力，本文旨在验证这一猜想。

Method: 通过研究经验分数匹配目标最小化器的平滑效应，分析隐式正则化的作用，检验分数函数平滑（或等价于对数密度域平滑）如何产生沿数据流形的切向平滑。

Result: 理论和实证结果证实，平滑分数函数确实会产生沿数据流形的切向平滑，并且通过选择适当的平滑方式可以控制扩散模型泛化的流形。

Conclusion: 扩散模型的强泛化能力确实源于其对数据低维几何结构的适应，分数匹配中的隐式正则化机制是实现这种适应的关键，通过平滑控制可以引导模型在特定流形上泛化。
...
### [289] [Drop-Muon: Update Less, Converge Faster](https://arxiv.org/abs/2510.02239)
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习优化器如Muon每步更新所有层，但作者认为这种全网络更新在理论和实践上都是次优的，需要更高效的训练方法。

Method: 提出非欧几里得随机渐进训练方法Drop-Muon，每步只按随机计划更新部分层，结合渐进式训练效率和层特定非欧几里得更新。

Result: 在CNN实验中，Drop-Muon始终优于全网络Muon，达到相同准确度时wall-clock时间快1.4倍，并提供了严格的收敛保证。

Conclusion: 全网络更新并非最优选择，除非层平滑常数满足特定关系，Drop-Muon为大规模模型训练提供了高效的理论基础替代方案。
...
### [290] [Transformers Discover Molecular Structure Without Graph Priors](https://arxiv.org/abs/2510.02259)
...
<details>
  <summary>Details</summary>
Motivation: 传统GNN在分子机器学习中依赖预定义的图结构（如固定半径截断或k近邻），这种硬编码的图结构限制了表达性（固定感受野）并降低了推理速度。本文旨在探索纯Transformer是否能在没有图结构或物理先验的情况下学习分子性质。

Method: 使用标准Transformer架构，直接在笛卡尔坐标上训练，不引入任何预定义的图结构或物理先验。与最先进的等变GNN在OMol25数据集上进行比较，保持相同的训练计算预算。

Result: Transformer达到了与等变GNN竞争的能量和力平均绝对误差。模型自适应地学习了物理一致的注意力模式（如注意力权重与原子间距离成反比衰减），并能灵活适应不同分子环境。标准Transformer还展现出可预测的训练资源扩展改进。
...
### [291] [Diffusion^2: Turning 3D Environments into Radio Frequency Heatmaps](https://arxiv.org/abs/2510.02274)
*Kyoungjun Park,Yifan Yang,Changhan Ge,Lili Qiu,Shiqi Jiang*

Main category: cs.LG

TL;DR: Diffusion^2是一种基于扩散模型的RF信号传播建模方法，使用3D点云数据在Wi-Fi到毫米波的广泛频率范围内预测RF信号传播，比现有方法快27倍且误差仅为1.9dB。


<details>
  <summary>Details</summary>
Motivation: RF信号传播建模对于环境理解和无线网络优化至关重要，但复杂环境中的精确预测仍具挑战性，因为RF信号会受到障碍物的吸收和反射等相互作用影响。

Method: 提出Diffusion^2方法，使用RF-3D编码器从3D点云数据中提取RF相关特征，并通过多尺度嵌入模拟实际的RF信号传播过程。

Result: 在合成和真实世界测量中的评估表明，Diffusion^2在各种频率带和环境条件下都能准确估计RF信号行为，误差仅为1.9dB，比现有方法快27倍。
...
### [292] [Fine-Grained Urban Traffic Forecasting on Metropolis-Scale Road Networks](https://arxiv.org/abs/2510.02278)
...
<details>
  <summary>Details</summary>
Motivation: 当前交通预测基准数据集存在重大缺陷：缺乏道路连通性信息、道路属性信息有限、路段数量不足，且主要包含城际高速公路数据，而城市道路网络由于更密集的道路和更复杂的交通模式提出了更具挑战性的预测任务。

Method: 提出了一个替代方法，使用图神经网络（GNN）但不包含专门的时序序列处理模块，从而实现更好的可扩展性。

Result: 该方法在可扩展性方面表现更好，同时展示了更强的预测性能。

Conclusion: 希望该数据集和建模见解能为交通预测研究提供有价值的资源。
...
### [293] [Knowledge Distillation Detection for Open-weights Models](https://arxiv.org/abs/2510.02302)
...
Conclusion: 提出的方法能有效检测知识蒸馏，在多种架构和任务上表现优于基线方法。
...
### [294] [Robust Tangent Space Estimation via Laplacian Eigenvector Gradient Orthogonalization](https://arxiv.org/abs/2510.02308)
*Dhruv Kohli,Sawyer J. Robertson,Gal Mishne,Alexander Cloninger*

Main category: cs.LG

TL;DR: 提出LEGO方法，利用图拉普拉斯低频特征向量的全局结构来指导局部切空间估计，相比标准LPCA方法在噪声环境下更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 标准LPCA方法在高噪声环境下存在邻域大小选择的临界权衡问题，需要先验的几何和噪声特征知识，而这些信息通常不可得。
...
Conclusion: LEGO方法通过利用全局结构信息，有效解决了高噪声环境下切空间估计的挑战，为相关数据分析任务提供了更可靠的解决方案。
...
### [295] [KaVa: Latent Reasoning via Compressed KV-Cache Distillation](https://arxiv.org/abs/2510.02312)
*Anna Kuzina,Maciej Pioro,Paul N. Whatmough,Babak Ehteshami Bejnordi*

Main category: cs.LG

TL;DR: KaVa框架通过压缩KV缓存的知识蒸馏，将显式思维链推理转化为高效的隐式推理，在保持准确性的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 显式思维链推理虽然效果好但计算成本高且包含冗余信息，而隐式推理缺乏有效监督信号，限制了其在复杂自然语言推理中的效果。

Method: 提出KaVa框架，通过自蒸馏将教师模型的压缩KV缓存知识直接蒸馏到隐式推理学生模型中，利用连续潜在令牌的表示灵活性对齐逐步KV轨迹。

Result: 该方法在隐式推理基准上表现优于强基线，从纯方程到自然语言轨迹的退化明显更小，可扩展到更大骨干网络同时保持效率。

Conclusion: 压缩KV缓存蒸馏为隐式推理提供了可扩展的监督信号，结合了思维链训练的准确性和隐式推理的效率与可部署性。
