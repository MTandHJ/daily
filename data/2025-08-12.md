...
Motivation: 现有VQA模型在复杂领域（如医疗）中缺乏详细精确性，需改进以支持特定领域应用。

Method: BIND通过密集查询编码优化嵌入空间；Med-GRIM结合图检索和提示工程，使用小语言模型实现高效模块化工作流。

Result: Med-GRIM以低计算成本实现高性能，并发布DermaGraph数据集支持多模态研究。

Conclusion: BIND和Med-GRIM为医疗VQA提供了高效精确的解决方案，DermaGraph推动了零样本多模态研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的肖像动画方法主要关注唇同步或静态情感转换，忽视了动态风格（如头部运动），且依赖双U-Net架构导致计算开销大。

Method: 提出DiTalker框架，包含风格-情感编码模块（分离提取身份特定风格和身份无关情感特征）和音频-风格融合模块（通过并行交叉注意力层解耦音频与说话风格）。采用两种优化约束提升唇同步和细节保留。

Result: 实验表明DiTalker在唇同步和说话风格可控性上优于现有方法。

Conclusion: DiTalker通过模块化设计和优化约束，实现了高效且高质量的肖像动画，解决了动态风格和计算效率问题。
...
Method: 结合水平集表示与配准框架，对变形水平集函数施加约束，支持单中心或多中心的星形分割，并通过ADMM方法求解。

Result: 在合成和真实图像上的数值实验证明了该模型在实现准确星形分割方面的有效性。

Conclusion: 提出的模型能够有效处理星形分割问题，支持多种分割需求，并通过实验验证了其性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有半监督学习方法在息肉分割中因忽略息肉特异性结构而导致的泛化能力不足问题。

Method: 提出FPGM框架，分两阶段：首先从标记息肉边缘学习域不变频率先验，然后对未标记图像进行频谱扰动，对齐振幅谱并保留相位信息。

Result: 在六个公共数据集上验证，FPGM性能优于十种竞争方法，零样本泛化能力突出，数据稀缺场景下Dice分数提升超过10%。

Conclusion: FPGM通过增强跨域鲁棒性，为临床可部署的息肉分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究LMMs在视觉任务中的表现和可解释性，探索其如何通过视觉反射提升性能。

Method: 通过提示LMM验证专用视觉模型预测，分析视觉反射行为，并测试训练无关的连接器。

Result: LMMs通过视觉反射提升准确性，依赖紧凑的文本表示而非原始视觉特征，训练无关连接器可增强性能。

Conclusion: 视觉反射是实现稳健且可解释视觉识别的有前景策略。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要高精度和实时性的全景驾驶感知，现有方法在多任务处理中存在负迁移问题和标签不一致性。

Method: 提出轻量级门控适配器模块自适应融合共享和任务特定特征，设计自适应分割解码器自动学习多尺度特征权重，解决车道线分割标签不一致问题。

Result: 在BDD100K数据集上，目标检测mAP50达84.9%，召回率95.4%；可行驶区域分割mIoU达92.6%；车道线分割IoU达56.8%，准确率84.7%；推理速度32.6 FPS。

Conclusion: RMT-PPAD在性能和效率上均达到先进水平，实际场景中表现稳定，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有POPE基准在评估LVLMs的对象幻觉问题时效果逐渐减弱，因其忽略了图像特定信息且仅限制于负面对象类别。

Method: HOPE基准利用CLIP选择高预测可能性的负面对象作为干扰项，并通过真实对象与虚假描述配对构建误导性干扰项。

Result: HOPE导致多种LVLMs的精度下降9%至23%，显著优于POPE。

Conclusion: HOPE为评估LVLMs的幻觉问题提供了更严格的基准，揭示了其潜在漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 现有目标检测数据集（如ImageNet、COCO）缺乏非商业领域的信号稀疏性，MobilTelesco填补了这一空白。

Method: 使用MobilTelesco数据集对多种目标检测模型进行基准测试。

Result: 揭示了在特征稀疏条件下目标检测模型面临的挑战。

Conclusion: MobilTelesco为稀疏特征条件下的目标检测研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂多实例场景（如人-人遮挡、人-物体纠缠和背景干扰）下表现不佳，主要由于数据集限制和缺乏空间解耦能力。
...
<details>
  <summary>Details</summary>
Motivation: 在缺乏3D真实标注的情况下，探索仅利用多视角RGB图像进行3D语义场景图估计，并解决伪点云几何和多视角图像特征中的噪声问题。

Method: 通过语义掩码引导特征聚合以过滤背景噪声，设计新方法利用邻域节点信息增强特征，并利用统计先验优化节点和边预测。

Result: 实验表明，该方法在仅使用多视角图像作为输入的情况下优于现有方法。

Conclusion: 提出的方法在无3D真实标注的情况下，通过特征增强和统计先验优化，实现了更准确的3D语义场景图估计。
...
<details>
  <summary>Details</summary>
Motivation: 训练深度神经网络资源消耗大，传统方法需为不同需求训练多个模型，效率低且难以维护。

Method: 提出NNObfuscator机制，允许单一模型实时调整性能，支持分层访问控制。

Result: 实验验证了NNObfuscator的有效性，单一模型能适应多任务需求。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测中的数据集存在年龄偏见，影响模型的公平性和准确性。

Method: 通过整合现有数据集（Celeb-DF、FaceForensics++、UTKFace）并生成合成数据，构建了一个年龄多样化的深度伪造数据集。使用XceptionNet、EfficientNet和LipForensics三种模型评估其效果。

Result: 模型在年龄多样化数据集上表现出更高的公平性、整体准确性和跨数据集泛化能力。

Conclusion: 研究提供了一个可复现的、注重公平性的数据集和模型流程，为未来公平深度伪造检测研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前具身智能的评估方法依赖交互式模拟或真实环境，成本高且难以扩展，需要一种更高效的解决方案。

Method: 提出StaticEmbodiedBench，利用静态场景表示，通过简单接口实现统一评估，并测试了19个VLMs和11个VLAs模型。

Result: 建立了首个统一的静态具身智能排行榜，并发布了200个样本以加速研究。
...
Method: StyleTailor采用迭代视觉细化范式，由多级负面反馈驱动，核心包括Designer（个性化服装选择）和Consultant（虚拟试穿）两个代理，通过分层视觉语言模型反馈逐步优化。

Result: 实验表明，StyleTailor在个性化设计和推荐方面表现优异，优于无负面反馈的基线方法，为智能时尚系统设立了新基准。

Conclusion: StyleTailor通过闭环机制和全面评估套件，显著提升了时尚推荐的精准性和用户体验。
...
<details>
  <summary>Details</summary>
Motivation: 在临床实践中部署深度学习模型需要多模态数据，但推理时并非所有模态都可用。

Method: 使用基于文本和表格元数据的教师模型（MIMIC-CXR和CBIS-DDSM）向视觉Transformer学生模型蒸馏知识。

Result: MMPKD提升了注意力图在输入图像中定位ROI的零样本能力，但效果不跨域。

Conclusion: MMPKD有效但领域特异性强，与先前研究结论不同。
...
Method: 将脑子网络视为成像数据的标记，与临床报告中的单词标记对齐，构建共享潜在空间。

Result: 在轻度认知障碍（MCI）上应用，取得了最先进的预测性能，并识别出有临床意义的连接组-文本对。
...
<details>
  <summary>Details</summary>
Motivation: 表面材料识别是机器人感知和物理交互的关键，尤其是在结合视觉和触觉输入时。现有深度学习模型在视觉任务中表现优异，但触觉和视觉多模态融合的研究仍有探索空间。

Method: 提出Surformer v1，结合结构化触觉特征和PCA降维的视觉嵌入（通过ResNet-50提取），使用模态特定编码器和跨模态注意力层。实验包括触觉单独分类和多模态融合对比。

Result: Surformer v1在触觉单独分类中表现最佳，多模态融合中达到99.4%准确率，推理时间0.77毫秒，优于多模态CNN。

Conclusion: Surformer v1在准确性、效率和计算成本之间提供了理想平衡，适用于实时表面材料识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究多集中于文本和图像的仇恨言论检测，视频领域的隐式仇恨言论检测尚未充分探索。
...
Conclusion: ImpliHateVid数据集和提出的方法为视频隐式仇恨言论检测提供了重要工具和框架。
...
<details>
  <summary>Details</summary>
Motivation: 数字新闻媒体的普及需要验证内容真实性，尤其是视觉与文本信息的深层一致性，传统方法在细粒度跨模态上下文一致性（FCCC）问题上表现不足。

Method: 基于高级视觉语言大模型（LVLMs），提出ContextGuard-LVLM框架，结合多阶段上下文推理机制，并通过强化或对抗学习增强模型能力。

Result: ContextGuard-LVLM在细粒度一致性任务中显著优于现有零样本基线模型（如InstructBLIP和LLaVA 1.5），在复杂逻辑推理和上下文理解方面表现突出。

Conclusion: ContextGuard-LVLM在检测上下文一致性方面表现出色，对细微扰动具有鲁棒性，并与人类专家判断高度一致。
...
Method: 采用两阶段框架：多模态概念感知模块（识别并描述视觉特征）和可解释疾病推理模块（结合概念与视觉信息进行诊断）。

Result: 在 Derm7pt 数据集上，VL-MedGuide 在疾病诊断（83.55% BACC）和概念检测（76.10% BACC）上表现优异，且生成的解释清晰可信。

Conclusion: VL-MedGuide 通过提供可解释的诊断结果，缩小了 AI 性能与临床实用性之间的差距。
...
Method: 提出联合学习框架，利用扩散模型提取图像成分表示干净信号，并引入时间依赖翻译网络学习复杂映射。

Result: 在RGB↔RGB及多种跨模态翻译任务中表现优于现有方法。

Conclusion: 联合学习框架显著提升翻译的全局最优性、保真度和结构一致性。
...
Method: 通过动态系数分解将复杂外观分解为静态神经基和动态系数，结合动态辐射积分器合成最终辐射。
...
Result: CausalNet在不同噪声水平下表现稳健，并在标准基准测试中超越现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索自回归（AR）图像模型中生成内水印的应用，填补现有研究空白，并解决直接迁移水印方案在图像扰动下检测性下降的问题。

Method: 方法包括两种基于视觉令牌聚类的水印技术：一种是无训练方法，依赖聚类查找表；另一种是微调VAE编码器，直接从扰动图像预测令牌聚类。

Result: 实验表明，聚类级水印提高了对扰动和再生攻击的鲁棒性，同时保持图像质量。聚类分类进一步提升了水印检测性，优于基线方法。

Conclusion: 结论是提出的水印方法在鲁棒性、检测性和运行效率上表现优异，适用于自回归图像模型。
...
Method: 提出使用线描作为预训练模态，并通过实验验证其在分类、检测和分割任务中的效果。此外，还提出了一种无监督学习方法“learning to draw”。
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症早期检测困难，基于社交媒体内容的研究成为重要方向，但多模态信息融合和时空特征提取是挑战。
...
<details>
  <summary>Details</summary>
Motivation: 计算机生成全息术（CGH）是一种有前景的方法，但需要高效且快速的合成框架来优化性能。

Method: 将点云和MRI数据重建为体积对象，输入非凸傅里叶光学优化算法（交替投影、SGD、拟牛顿法）生成全息图，并与HoloNet深度学习CGH对比。

Result: 通过2D中值滤波去除优化过程中的伪影和噪声，性能指标（MSE、RMSE、PSNR）得到提升。

Conclusion: 提出的框架在CGH合成中表现优异，优化算法结合滤波技术显著提升了全息图质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像和图像到视频生成模型难以捕捉物理真实性和运动动态，而真实视频能提供物理基础的几何和运动线索。论文探索如何利用真实视频的运动先验生成物理一致的4D内容。

Method: 提出Restage4D，一种基于视频条件的4D重演方法，采用视频重放训练策略、遮挡感知刚性损失和遮挡回溯机制，以改进运动质量和几何一致性。

Result: 在DAVIS和PointOdyssey数据集上验证，Restage4D在几何一致性、运动质量和3D跟踪性能上表现优异，能纠正生成模型引入的错误。

Conclusion: Restage4D展示了视频先验在4D重演任务中的潜力，能保持可变形结构并提升合成运动的真实性。
...
<details>
  <summary>Details</summary>
Motivation: 传统侵入性方法无法捕捉肿瘤空间异质性，且现有深度学习模型因标注数据稀缺而受限。

Method: 采用SWIN-UNETR架构，结合肿瘤感知特征编码（TAFE）和跨模态差异（CMD）模块，从多中心数据中训练模型。
...
Method: 利用计算机图形渲染技术构建VOccl3D数据集，包含多样化的真实遮挡场景，并对HPS方法（CLIFF和BEDLAM-CLIFF）进行微调。

Result: 实验表明，微调后的方法在多个公共数据集及VOccl3D测试集上均取得显著提升，同时增强了遮挡下的人体检测性能。

Conclusion: VOccl3D数据集为未来遮挡处理研究提供了更真实的基准资源。
...
Method: 提出SafePLUG框架，支持任意形状的视觉提示用于区域感知问答，以及基于语言指令的像素级分割和时间锚定事件识别。

Result: 实验结果表明，SafePLUG在区域问答、像素级分割、时间事件定位和事故事件理解等任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 术中超声成像的噪声、伪影以及与术前MRI/CT扫描的对齐问题增加了其解读难度，DiffUS旨在解决这些问题。

Method: DiffUS通过机器学习将MRI 3D扫描转换为声阻抗体积，利用射线追踪和反射-传输方程模拟超声束传播，并通过稀疏线性系统捕捉多次内部反射，最终生成B模式图像。

Result: 在ReMIND数据集上的评估表明，DiffUS能够从脑MRI数据生成解剖学上准确的超声图像。

Conclusion: DiffUS作为一种可微分渲染器，为下游应用（如切片到体积配准和体积重建）提供了基于梯度的优化能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像中器官边界的精确定位对分割、手术规划和放疗等任务至关重要，现有卷积网络在边缘检测中定位精度不足。

Method: 采用自上而下的反向细化架构，融合高层次语义特征与低层次细节，并扩展至各向异性体积处理。

Result: 在CT和MRI数据集上，边界定位性能显著提升，下游任务（如分割和配准）效果改善。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤镜图像中黑色素瘤的精确分割对自动化癌症筛查至关重要，但现有方法难以处理纹理、颜色变化及常见成像伪影。

Method: 采用ResNet启发的双分辨率架构，结合边界感知残差连接和通道注意力模块，并引入轻量级伪影抑制块和多任务训练目标。

Result: 在公开皮肤镜基准测试中，该方法显著改善了边界贴合度和临床相关分割指标。

Conclusion: 该方法无需复杂后处理或预训练，为自动化黑色素瘤评估系统提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床图像中皮下血管分割因标注稀缺、成本高和图像低对比度而困难，需开发高效方法。

Method: 通过可微随机游走标签传播模型扩展稀疏标注为密集监督，结合CNN分割预测器和拓扑感知正则化器。

Result: 在临床数据集上优于传统方法，生成更完整的血管图并提供校准的不确定性。

Conclusion: 该方法显著降低标注负担，同时保持血管拓扑结构，具有临床实用性。
...
Method: 使用源自由UDA训练低秩适配器（LoRA），并通过轻量级门控网络动态分配合并权重，实现跨域知识迁移。

Result: 在Market-1501、DukeMTMC-reID和MSMT17基准测试中表现优于现有方法，且计算高效。

Conclusion: SAGE-reID是一种高效、无源的MSDA方法，显著提升了性能和计算效率。
...
Result: 系统预测精度达到0.012毫米（95%置信水平），比传统统计过程控制方法提高了73%，并揭示了制造参数与几何偏差之间的隐藏相关性。
...
Result: AGIC在Flickr8k和Flickr30k上匹配或超越多个先进模型，推理速度更快，且在多指标上表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 多视图聚类旨在利用不同视图的一致性和互补信息进行样本分组，而子空间聚类作为其基础技术，需要更高效的方法。

Method: 使用基数约束替代图拉普拉斯正则化，提取局部和全局结构信息，并通过低秩约束揭示全局一致结构。采用交替二次惩罚方法解决非凸非光滑模型的收敛问题。

Result: 在六个标准数据集上的实验表明，该模型和方法优于八种最先进算法。

Conclusion: 所提出的模型和方法在多视图聚类中表现出优越性能，解决了传统方法的收敛问题。
...
<details>
  <summary>Details</summary>
Motivation: 长视频理解因数据规模庞大而具有挑战性，现有方法因多模态对齐不足和复杂时间语义信息捕捉不准确而效果受限。
...
Conclusion: VSI在长视频任务中表现出鲁棒性和泛化能力，成为中长视频问答任务的SOTA方法。
...
Method: 结合LFP模块抑制高频噪声和SFS模块螺旋采样融合目标相关特征，设计轻量高效的NS-FPN。

Result: 在公开数据集上显著减少误报，性能优于现有方法。

Conclusion: NS-FPN为红外小目标检测提供了一种噪声抑制的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型在视觉理解方面存在视觉嵌入与文本模态之间的差距，缺乏直接的视觉监督，限制了视觉嵌入的精细对齐。

Method: BASIC方法利用LLM浅层中精炼的视觉嵌入作为监督，从嵌入方向和语义匹配两个角度直接指导视觉投影器生成初始视觉嵌入。

Result: BASIC显著提升了MLLMs在多个基准测试中的性能，无需额外的监督模型或人工标注。

Conclusion: 直接视觉监督的有效性得到了验证，为多模态模型的视觉理解提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 汉字字体生成是字体设计师和排版师关注的重要课题，但如何提高生成质量仍是一个难题。本文旨在全面调查基于深度学习的汉字字体生成方法。

Method: 通过文献选择与分析，分类现有方法为多参考样本和少参考样本生成，并总结代表性方法的优缺点。

Result: 综述了深度学习架构、字体表示格式、数据集和评估指标，并详细讨论了不同方法的优缺点。

Conclusion: 总结了当前挑战和未来研究方向，为相关领域研究者提供有价值的参考。
...
TL;DR: 提出了一种无需训练的实例感知提示框架（IAPF），通过细化实例级掩码显著提升伪装目标分割（COS）性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的COS方法在标注稀疏时性能下降，而训练免费方法因语义级提示导致掩码粗糙，无法处理多实例场景。

Method: IAPF包括文本提示生成器、实例掩码生成器和自一致性投票，通过细化实例级提示生成精细掩码。
...
Method: 引入MultiRef-bench评估框架，包含合成和真实样本，并构建数据集MultiRef。

Result: 实验显示现有模型在多参考条件下表现有限，最佳模型OmniGen在合成和真实样本中分别达到66.6%和79.0%。
...
TL;DR: 论文提出MMReID-Bench，首个针对行人重识别的多任务多模态基准，展示了多模态大语言模型在行人重识别中的潜力与局限性。


<details>
  <summary>Details</summary>
Motivation: 传统行人重识别模型在多模态数据中泛化能力差，而现有方法未充分利用多模态大语言模型的推理和跨模态理解能力。

Method: 引入MMReID-Bench基准，包含20,710个多模态查询和库图像，覆盖10种行人重识别任务。

Result: 实验证明多模态大语言模型在行人重识别中表现优异，但在处理热成像和红外数据时存在局限性。

Conclusion: MMReID-Bench有助于开发更鲁棒和通用的多模态基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成任务多关注单轮场景，难以处理多轮创意任务，而对话系统因单代理、顺序模式导致意图漂移和不连贯编辑。
...
<details>
  <summary>Details</summary>
Motivation: 受强化学习在大型语言模型优化中的成功启发，探索其在自回归图像生成模型中的应用。

Method: 采用Group Relative Policy Optimization (GRPO)算法，设计多维度奖励函数评估生成图像质量。

Result: 在类和文本条件图像生成任务中，RL增强框架显著优于标准自回归基线。

Conclusion: 证实了基于RL的优化在自回归图像生成中的可行性，为高质量图像合成开辟了新途径。
...
Conclusion: CannyEdit通过创新方法显著提升了图像编辑的质量和自然度，为训练自由的区域编辑提供了有效解决方案。
...
TL;DR: 论文介绍了首个手语生成挑战赛，旨在通过标准化评估指标比较不同系统的性能，并发布了评估网络以推动领域发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有频率域方法基于固定基函数，无法动态适应图像内容和不同图像的判别需求。
...
TL;DR: 本文提出了首个针对文本到视频检索（T2VR）的攻击方法ViPro，旨在通过对抗性攻击提升视频排名，并提出了Modal Refinement（MoRe）以增强黑盒迁移性。实验表明ViPro在多种设置下显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有T2VR攻击主要关注降低视频排名，而提升视频排名的攻击尚未被充分研究，这种攻击可能带来更大的经济利益和传播（错误）信息的风险。

Method: 提出Video Promotion攻击（ViPro）和Modal Refinement（MoRe）方法，通过精细化的跨模态交互增强攻击效果。

Result: ViPro在白盒、灰盒和黑盒设置下分别平均超过基线方法30%、10%和4%。

Conclusion: ViPro揭示了T2VR的潜在漏洞，并提供了攻击上下界的定性分析，为可能的防御措施提供了参考。
...
TL;DR: 论文首次评估了基于鱼眼镜头的3D高斯泼溅方法Fisheye-GS和3DGUT在超过180度视场的真实图像上的表现，分析了极端畸变下的处理能力，并提出了一种基于深度的初始化策略。


<details>
  <summary>Details</summary>
Motivation: 研究鱼眼镜头在3D重建中的实际应用，解决传统SfM初始化在强畸变下失效的问题。

Method: 通过不同视场（200度、160度、120度）评估Fisheye-GS和3DGUT的性能，并提出基于UniK3D预测的深度初始化策略。

Result: Fisheye-GS在160度视场表现最佳，3DGUT在200度视场下保持稳定；UniK3D策略在困难场景中表现优异。
...
Method: 提出WeatherDiffusion框架，结合文本描述引导的预测本征图，并引入MAA机制实现高质量逆向渲染。同时构建了两个数据集（WeatherSynthetic和WeatherReal）。

Result: 实验表明，WeatherDiffusion在多个基准测试中优于现有方法，并在下游任务（如目标检测和图像分割）中显著提升鲁棒性。
...
Method: 将任务重新建模为动态过程，提出TADoc框架，并设计DLS评价指标。
...
TL;DR: OctreeNCA提出了一种基于八叉树数据结构的改进NCA模型，用于高效分割高分辨率医学图像和视频，显著降低VRAM消耗并提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 医学图像和视频分割需要全局上下文，但现有模型如UNet和Vision Transformers因VRAM消耗大而无法直接处理大输入，导致分割效果和速度受限。

Method: 通过八叉树数据结构扩展NCA的邻域定义，实现全局知识的高效传递，并开发了CUDA实现的NCA推理函数以优化VRAM使用和推理速度。

Result: OctreeNCA在高分辨率图像和视频分割中VRAM消耗比UNet减少90%，能够一次性处理184兆像素病理切片或1分钟手术视频。

Conclusion: OctreeNCA为医学大输入分割提供了一种高效、低资源消耗的解决方案，显著优于传统方法。
...
Conclusion: S2-UniSeg在效率和性能上均优于现有方法，适用于大规模数据。
...
Method: 引入HiMat框架和CrossStitch模块，通过轻量级卷积操作捕获多图依赖关系，不显著改变DiT主干。

Result: 实验证明HiMat能生成具有结构一致性和高频细节的4K SVBRDF，并可推广到其他任务如本征分解。
...
TL;DR: TerraMAE是一种针对高光谱图像（HSI）的新型自监督编码框架，通过自适应通道分组和增强重建损失函数，有效捕捉空间-光谱信息，并在多个地理空间任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 高光谱卫星图像具有数百个连续光谱波段，但现有自监督方法难以利用其复杂的空间-光谱相关性。

Method: 提出TerraMAE框架，包括基于统计反射特性的自适应通道分组和结合空间-光谱质量指标的增强重建损失函数。

Result: TerraMAE在图像重建中表现出卓越的空间-光谱信息保留能力，并在作物识别、土地覆盖分类和土壤纹理预测任务中验证了其有效性。

Conclusion: TerraMAE为高光谱图像分析提供了一种高效的自监督学习框架，适用于多种地理空间任务。
...
Method: DocRefine采用六种专业协作智能体（如布局分析、内容理解、指令分解等）和闭环反馈架构，结合先进的多模态语言模型（如GPT-4o）。

Result: 在DocEditBench数据集上，DocRefine在语义一致性（86.7%）、布局保真度（93.9%）和指令遵循率（85.0%）上表现优异。

Conclusion: DocRefine在复杂多模态文档编辑中表现出色，保持了语义完整性和视觉一致性，是科学文档处理的重要进展。
...
TL;DR: FOCUS-Med提出了一种结合空间和结构图的注意力感知内窥镜息肉分割方法，通过双图卷积网络和自注意力机制提升分割性能，并在公开基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 内窥镜息肉分割对早期结直肠癌检测至关重要，但由于低对比度、高光反射和模糊边界等问题，该任务具有挑战性。
...
TL;DR: 提出了一种新的3D表示方法TeSO，结合八叉树结构和纹理贴图，提高了渲染质量并降低了比特率。
...
Method: 基于点云构建TeSO，将3D场景表示为八叉树组织的带纹理贴图的立方体边界surfel。

Result: TeSO在低比特率下实现了比现有方法更高的渲染质量。
...
TL;DR: ForeSight是一种新型的联合检测与预测框架，用于自动驾驶中的3D视觉感知，通过多任务流式学习和双向学习提升性能。
...
Method: 采用多任务流式学习和双向学习，结合检测与预测的查询记忆，并引入预测感知检测变换器和流式预测变换器。

Result: 在nuScenes数据集上，ForeSight以54.9%的EPA表现超越前方法9.3%，并在mAP和minADE上表现最佳。

Conclusion: ForeSight通过联合检测与预测，显著提升性能，同时避免了显式目标关联的需求。
...
TL;DR: 提出了一种名为HyComm的混合协作3D检测系统，通过自适应整合两种通信消息（感知输出和原始观测）来优化检测性能与通信带宽的权衡。
...
TL;DR: AugLift通过增强2D关键点输入（加入置信度和深度估计），显著提升3D人体姿态估计的泛化能力，无需额外数据或传感器。


<details>
  <summary>Details</summary>
Motivation: 基于提升的3D姿态估计方法在新数据集和真实场景中泛化能力差，AugLift旨在解决这一问题。

Method: AugLift在标准2D关键点坐标基础上，稀疏地加入置信度得分和深度估计，利用预训练模型计算这些信号。
...
TL;DR: 生成图像模型在医学影像中显示出潜力，但合成图像的真实性和临床实用性仍存疑。本研究评估了GAN和扩散模型在合成胸片中的表现，发现扩散模型视觉更真实，而GAN在某些条件下更准确。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像数据稀缺问题，尤其是低发病率异常，同时评估合成图像的真实性和临床实用性。

Method: 使用GAN和扩散模型合成胸片，基于MIMIC-CXR数据集进行读者研究，由三位放射科医生评估合成图像的真实性和一致性。

Result: 扩散模型生成的图像视觉更真实，而GAN在特定条件下（如ECS缺失）更准确。研究还揭示了放射科医生识别合成图像的视觉线索。
...
Method: CMAMRNet包含掩码感知上下采样器（MAUDS）和共特征聚合器（CFA），通过多尺度特征提取和掩码引导提升修复效果。
...
TL;DR: 论文提出了一种名为DPAL的蒸馏预训练框架，通过动态模式对齐学习，使轻量级HVM能够从大型HVM中学习并实现强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有HVM依赖大型神经架构和受限的预训练数据，限制了实际应用。DPAL旨在解决这一问题。

Method: 设计了动态模式解码器（D-PaDe）和三层次对齐目标，分别提取典型视觉模式并最小化泛化差距。

Result: 实验表明，DPAL在15个数据集上表现优异，轻量级模型（5M参数）性能接近大型HVM（84M/307M）。

Conclusion: DPAL为轻量级HVM提供了一种高效预训练方法，显著提升了泛化能力。
...
Method: 使用残差极坐标表示建模短期意图，基于可学习的端点预测器估计长期意图，并结合自适应引导和残差噪声预测器优化扩散过程。
...
TL;DR: 论文提出了一种名为SketchAnimator的模型，能够通过输入草图和参考视频，为静态草图添加动态效果，简化了专业动画制作过程。


<details>
  <summary>Details</summary>
Motivation: 草图动画通常需要专业技能且耗时，对业余用户不友好。本文旨在通过自动化工具降低这一门槛。

Method: 模型分为三个阶段：外观学习、运动学习和视频先验蒸馏，利用LoRA和SDS技术整合草图和视频的动态信息。

Result: 模型成功生成保留草图外观并模仿参考视频动态的动画，在一键运动定制任务中表现优于其他方法。

Conclusion: SketchAnimator为草图动画提供了一种高效且用户友好的解决方案，适用于非专业用户。
...
TL;DR: CoopDiff提出了一种基于接触一致性的解耦扩散框架，分别建模人和物体的运动，通过共享接触点实现一致性预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽略人和物体运动的物理差异，CoopDiff旨在通过解耦建模提升预测准确性。

Method: 使用两个分支分别预测人和物体的运动，通过共享接触点及一致性约束实现协同预测，并引入人驱动交互模块。

Result: 在BEHAVE和HOI数据集上表现优于现有方法。

Conclusion: CoopDiff通过解耦建模和接触一致性约束，显著提升了3D人-物交互预测的性能。
...
Method: 采用深度可分离膨胀卷积构建LMF层，并集成多个LMF层形成LMFNet。

Result: 在五个基准数据集上达到或接近最先进水平，仅需0.81M参数。
...
TL;DR: EventRR框架通过解耦RVOS任务为对象摘要和引用推理两部分，利用REG图和TCRR方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有RVOS方法忽视表达式的语义结构，且视频引用表达式比图像更复杂，需新方法处理事件属性和时序关系。

Method: EventRR分为对象摘要和引用推理两部分，使用REG图和TCRR方法进行结构化推理。
...
TL;DR: 论文提出了一种深度引导网络（DGN）用于图像修复，结合了深度估计和图像修复的双分支结构，并引入了一个新的大规模高分辨率数据集。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法常忽略深度信息，导致相似性匹配不佳、浅景深场景注意力分散或深景深场景背景过度增强。

Method: DGN包含两个交互分支：深度估计分支提供结构指导，图像修复分支通过渐进窗口自注意力和稀疏非局部注意力实现修复。

Result: 实验表明，该方法在多个标准基准上达到最先进性能，并能泛化到未见过的植物图像。

Conclusion: 深度引导网络有效提升了图像修复质量，同时深度估计与修复任务相互促进。
...
TL;DR: 提出了一种基于整流流的无监督真实世界超分辨率方法，通过RFDM和FGDM模块捕捉真实退化，提升SR性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界超分辨率面临复杂未知的退化分布，现有方法难以从合成数据泛化到真实数据。

Method: 使用RFDM和FGDM模块连续建模退化轨迹，生成真实退化的LR-HR训练对。

Result: 在真实数据集上显著提升了现有SR方法的性能。

Conclusion: 该方法有效解决了真实世界SR的退化建模问题，提升了实用性。
...
TL;DR: 提出了一种基于认知的多模态边界保持网络（CMB-Net），通过结合视觉和文本信息提升图像篡改定位的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有图像篡改定位模型依赖视觉线索，忽略了内容特征的语义逻辑关系，而篡改技术通常会破坏这些关系，留下语义线索。

Method: 利用大语言模型（LLMs）分析篡改区域并生成文本信息，设计了图像-文本中心模糊模块（ITCAM）和图像-文本交互模块（ITIM），并提出恢复边缘解码器（RED）以保留边界信息。

Result: 实验表明CMB-Net优于现有大多数图像篡改定位模型。

Conclusion: CMB-Net通过结合视觉和文本信息，有效提升了图像篡改定位的准确性和边界保持能力。
...
TL;DR: 论文提出了一种结合通用和参数化模型的混合校准方法，解决了通用校准中的姿态模糊问题，并通过实验验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 离线相机校准中参数化模型依赖用户经验，通用模型复杂且无法提供传统内参，且存在姿态模糊问题，影响后续姿态估计。

Method: 提出线性求解器和非线性优化解决姿态模糊问题，并引入全局优化的混合校准方法，结合通用和参数化模型。
...
TL;DR: 提出了一种基于面部标志的视觉特征提取器，结合多图卷积网络和多级唇动态融合框架，以有限数据提升视觉语音识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法易受视觉干扰影响且需大量数据和计算资源，本文旨在减少用户特定特征的影响并提升小数据下的性能。

Method: 使用面部标志辅助训练视觉特征提取器，设计时空多图卷积网络提取标志特征，并引入多级唇动态融合框架结合原始视频帧特征。

Result: 实验表明该方法在有限数据下表现良好，并提高了对未见说话者的识别准确率。

Conclusion: 该方法有效减少了用户特定特征的干扰，提升了小数据场景下的视觉语音识别性能。
...
TL;DR: ASM-UNet是一种基于Mamba的新型架构，通过动态调整扫描顺序来解决细粒度分割（FGS）中的个体差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有粗粒度分割方法在细粒度分割中表现不佳，且传统Mamba模型依赖固定扫描顺序，无法适应个体差异。

Method: 提出ASM-UNet，利用自适应扫描分数动态调整扫描顺序，结合群体共性和个体差异。

Result: 在ACDC、Synapse和BTMS数据集上表现优异，优于现有方法。

Conclusion: ASM-UNet在细粒度和粗粒度分割任务中均表现出色，解决了传统方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注空间交互而忽略光谱交互，导致在复杂背景和小目标跟踪中表现不佳。

Method: 利用Transformer建立波段间长程空间关系，并通过集合论的包含-排除原理建模光谱交互，同时引入光谱损失优化训练。

Result: 实验表明，该方法实现了最先进的跟踪性能。

Conclusion: 通过结合空间和光谱交互，显著提升了高光谱视频跟踪的准确性和鲁棒性。
...
Result: 在EgoDynamic4D上的实验表明，该方法显著优于基线，验证了多模态时间建模对动态场景理解的有效性。

Conclusion: EgoDynamic4D填补了动态场景理解的数据集空白，提出的框架为复杂时空推理任务提供了有效解决方案。
...
TL;DR: 论文提出了一种名为Small-Large Collaboration (SLC)的协作框架，通过小型VLM生成个性化信息，大型VLM整合信息以提供准确响应，解决了大型VLM难以直接个性化和小型VLM推理能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（VLMs）虽然具备强大的多模态理解能力，但训练成本高且难以直接个性化；小型VLMs易于个性化但推理能力不足。因此，需要一种高效的方法结合两者的优势。

Method: 提出SLC框架，小型VLM负责生成个性化信息，大型VLM整合信息并避免小型VLM的幻觉问题。采用测试时反射策略，仅需训练一个元个性化小型VLM。

Result: 实验证明SLC框架在多种基准测试和大型VLM上均有效，支持开源和闭源大型VLM的个性化应用。

Conclusion: SLC是首个训练高效的框架，能够支持广泛的实际个性化应用，为视觉语言模型的个性化提供了可行方案。
...
TL;DR: OpenHAIV框架整合了OOD检测、新类发现和增量持续微调，以解决开放世界识别中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如OOD检测和增量学习）在开放世界场景中存在局限性，无法满足模型自主更新知识的需求。
...
TL;DR: 提出统一特征可视化框架，适用于CNN和ViT，扩展至中间层，并探索其在生成对抗样本中的应用。


<details>
  <summary>Details</summary>
Motivation: 理解DNN内部特征表示是模型可解释性的关键步骤，受神经科学启发，研究如何通过激活最大化方法揭示人工神经元的响应模式。

Method: 采用激活最大化（AM）方法，扩展特征可视化至中间层，并研究其在生成对抗样本中的应用。

Result: 实验证明该方法在CNN和ViT中均有效，具有普适性和解释价值。

Conclusion: 统一框架为DNN特征可视化提供新视角，同时揭示模型潜在脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中标签稀缺问题限制了深度学习性能，现有方法因伪标签与图像不一致而效果不佳。

Method: SynMatch利用分割模型提取的纹理和形状特征合成图像，生成与伪标签高度一致的图像对。

Result: 在多种监督设置下表现优异，尤其在BSL设置中，性能显著优于现有方法。

Conclusion: SynMatch为医学图像分割提供了一种高效解决方案，尤其在标签稀缺场景下表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 解决实时语义分割中高效架构设计和大感受野捕获的矛盾，同时提升细节轮廓的精确性。

Method: 采用稀疏分解大可分核注意力（SDLSKA）和深度大核金字塔池化模块（DLKPPM），结合双边架构和边界引导自适应融合（BGAF）模块。

Result: BEVANet在未预训练和ImageNet预训练后分别达到79.3%和81.0%的mIoU，实时分割速度为33 FPS。

Conclusion: BEVANet通过创新的注意力机制和动态感受野调整，实现了实时语义分割的先进性能。
...
TL;DR: DragonFruitQualityNet是一种轻量级CNN，用于火龙果实时质量检测，准确率达93.98%，并嵌入移动应用，支持农民进行实时检查。
...
Result: 模型准确率达93.98%，优于现有方法，移动应用便于农民使用。
...
TL;DR: MCITlib是一个用于多模态大型语言模型持续指令调优的代码库，旨在解决多模态持续学习中的挑战，如灾难性遗忘和跨模态协调。


<details>
  <summary>Details</summary>
Motivation: 多模态持续学习任务（如视觉和语言）需要模型不仅避免遗忘旧知识，还需处理跨模态交互的挑战，传统单模态方法无法满足需求。

Method: 开发了MCITlib代码库，实现了8种多模态持续指令调优算法，并在2个基准上进行了系统评估。

Result: MCITlib为多模态持续学习研究提供了工具支持，并将持续更新以反映领域进展。

Conclusion: MCITlib的发布推动了多模态持续学习的研究，代码库开源以促进社区发展。
...
TL;DR: MobileViCLIP是一种高效的视频-文本模型，通过时间结构重参数化技术优化，适用于移动设备，具有快速的推理速度和强大的零样本分类与检索能力。
...
TL;DR: DocR1是一种多模态大语言模型（MLLM），通过新型强化学习框架EviGRPO训练，用于多页文档理解，结合证据感知奖励机制和课程学习策略，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 多页文档理解对MLLM提出了挑战，需要细粒度视觉理解和跨页多跳推理，而现有强化学习方法在此领域研究不足。

Method: 提出EviGRPO框架，采用证据感知奖励机制，分两阶段标注和课程学习策略，构建EviBench和ArxivFullQA数据集。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像描述方法依赖对象检测器或结合GCN，存在冗余信息、构建复杂和高成本的问题，需要一种更高效的替代方案。

Method: RORPCap通过对象和关系提取模型获取语义信息，结合预定义提示模板和Mamba映射网络，生成文本增强的特征嵌入，最终用GPT-2生成描述。

Result: 在MS-COCO数据集上，RORPCap仅需2.6小时训练，达到120.5% CIDEr和22.0% SPICE分数，性能与检测器和GCN模型相当。

Conclusion: RORPCap是一种高效且性能优异的图像描述生成替代方案。
...
TL;DR: Planner-Refiner框架通过迭代优化视觉元素的时空表示，减少语言与视觉间的语义鸿沟，提升视频-语言对齐任务的表现。
...
Method: Planner模块将复杂语言提示分解为短句链，Refiner模块通过名词-动词对引导视觉标记的自注意力，实现高效单步优化。

Result: 在Referring Video Object Segmentation和Temporal Grounding任务中表现优异，尤其在复杂提示下优于现有方法。

Conclusion: Planner-Refiner框架在复杂语言提示下展现出潜力，并通过新基准MeViS-X验证了其能力。
...
TL;DR: CoAR是一种新型框架，通过冻结预训练参数，仅调整少量参数实现定制化图像生成，解决了现有方法成本高、易过拟合的问题。


<details>
  <summary>Details</summary>
Motivation: 探索统一自回归模型在定制化图像生成中的潜力，解决现有方法依赖全微调或适配器导致的高成本和过拟合问题。

Method: 采用层间多模态上下文学习策略，学习有效的特定主题表示，并通过正则化防止过拟合和语言漂移。

Result: CoAR在主题和风格个性化任务中表现优异，计算和内存效率显著提升，仅调整0.05%参数。

Conclusion: CoAR是一种高效、低成本的定制化图像生成方法，具有广泛应用潜力。
...
Conclusion: SODiff通过语义导向和自适应优化，有效解决了JPEG伪影问题。
...
TL;DR: GS4Buildings是一种基于语义3D建筑模型的高斯泼溅方法，用于大规模复杂城市场景的建筑物表面重建，显著提高了重建完整性和几何精度。


<details>
  <summary>Details</summary>
Motivation: 传统2D高斯泼溅方法在大规模和复杂城市场景中表现不佳，导致建筑物重建不完整。GS4Buildings通过利用语义3D建筑模型提供几何先验，解决了这一问题。

Method: GS4Buildings直接从低细节语义3D建筑模型初始化高斯分布，并利用平面几何生成的深度和法线图优化重建过程。还引入了建筑物聚焦模式以减少高斯基元数量。

Result: 实验表明，GS4Buildings在城市场景中提高了20.5%的重建完整性和32.8%的几何精度，同时减少了71.8%的高斯基元。

Conclusion: GS4Buildings展示了语义建筑模型在高斯泼溅重建中的潜力，为智慧城市和数字孪生等应用提供了高效解决方案。
...
TL;DR: 提出了一种模块化分解和特征裁剪方法，显著提升了跨传感器泛化能力，同时大幅降低了训练和推理时间。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习全色锐化模型在跨传感器数据上泛化能力差的问题，避免耗时或需要额外数据的现有方法。

Method: 通过模块化分解模型，在关键接口集成特征裁剪器，利用物理感知无监督损失高效训练，并采用分块并行推理。

Result: 在跨传感器情况下性能显著提升，训练和推理时间极短（如512x512x8图像仅需0.2秒）。

Conclusion: 该方法在质量和效率上均达到最优，适用于实际应用中的跨传感器全色锐化任务。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在多视图重建中表现优异，但在稀疏视图下效果不佳，因此提出DIP-GS以改进稀疏视图重建。

Method: 利用Deep Image Prior的内部结构和模式，以粗到细的方式增强3DGS，适用于稀疏视图场景。

Result: DIP-GS在稀疏视图重建任务中达到SOTA水平，表现优于传统3DGS。

Conclusion: DIP-GS是一种无需预训练模型的有效方法，显著提升了稀疏视图重建的性能。
...
TL;DR: LET-US 是一个用于长事件流-文本理解的框架，通过自适应压缩机制减少输入事件量，同时保留关键视觉细节，提升了跨模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLMs 在处理事件流时效果不佳或仅能处理短序列，因此需要一种能够有效理解长事件流并与文本对齐的方法。

Method: 采用两阶段优化范式，结合文本引导的跨模态查询、分层聚类和相似性计算，减少事件流中的冗余信息，并通过大规模事件-文本对齐数据集训练模型。

Result: LET-US 在长事件流的描述准确性和语义理解上优于现有 MLLMs，并在多个任务（如推理、分类、时间定位等）上表现优异。

Conclusion: LET-US 为长事件流与文本的跨模态理解开辟了新方向，所有数据集、代码和模型将公开。
...
TL;DR: ForensicsSAM是一个统一的图像伪造检测与定位（IFDL）框架，通过注入伪造专家和对抗专家，增强对抗攻击的鲁棒性，同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于参数高效微调（PEFT）的方法忽视了其对对抗攻击的脆弱性，导致性能下降。

Method: 1. 注入伪造专家增强图像编码器的伪造检测能力；2. 设计轻量级对抗检测器；3. 注入对抗专家修正对抗噪声引起的特征偏移。

Result: ForensicsSAM在多个基准测试中表现出对多种对抗攻击方法的强鲁棒性，并在伪造检测和定位任务中达到最优性能。

Conclusion: ForensicsSAM通过创新的设计解决了PEFT方法的对抗攻击问题，为IFDL任务提供了高效且鲁棒的解决方案。
...
TL;DR: CharacterShot是一个可控且一致的4D角色动画框架，通过单张参考图像和2D姿势序列生成动态3D角色。结合DiT模型、双注意力模块和4D高斯溅射优化，实现了高质量4D角色动画。


<details>
  <summary>Details</summary>
Motivation: 解决从单张图像和2D姿势序列生成高质量4D角色动画的挑战，提升角色动画的灵活性和一致性。

Method: 1. 基于DiT模型预训练2D动画模型；2. 通过双注意力模块和相机先验将2D动画提升到3D；3. 使用4D高斯溅射优化生成稳定4D角色。
...
<details>
  <summary>Details</summary>
Motivation: 图像编辑工具和生成式AI的普及导致数字媒体伪造问题严重，需要高效定位伪造区域的方法。

Method: 结合SD3的Rectified Flow机制和LoRA调整，注入噪声放大伪造痕迹，并利用SAM的语义上下文增强边界检测。
...
TL;DR: 论文研究了视觉语言模型中的性别偏见，提出了基于反事实数据增强和任务向量方法的去偏方法，并引入新指标和去偏方法DAUDoS。实验表明，方法有效减少了性别差距，并识别了偏见的来源。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在训练数据中可能继承性别偏见，本研究旨在分析视觉和文本模态对偏见的贡献，并提出高效的去偏方法。

Method: 采用反事实数据增强（CDA）和任务向量方法，提出新指标Degree of Stereotypicality及去偏方法DAUDoS。

Result: CDA减少性别差距6%，DAUDoS减少3%且仅需三分之一数据。两种方法均提升性别识别准确率3%。CLIP视觉编码器偏见更显著，PaliGemma2文本编码器偏见更显著。

Conclusion: 通过识别偏见的来源，研究为未来多模态系统提供了更精准的去偏策略。
...
TL;DR: 本文提出了一种两阶段框架，用于解决完全无监督图像异常检测（FUIAD）中训练数据可能包含未标记异常的问题，通过利用模型的学习偏差来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法假设训练数据无异常，但实际数据可能被污染，导致模型将异常误认为正常，从而降低检测性能。

Method: 提出两阶段框架：第一阶段利用学习偏差（统计优势和特征空间差异）筛选纯净数据集，第二阶段在纯净数据上训练最终检测器。

Result: 在Real-IAD基准测试中表现出优异的异常检测和定位性能，且对噪声具有鲁棒性。

Conclusion: 该框架兼容多种无监督骨干网络，为实际场景中不完美训练数据提供了实用解决方案。
...
TL;DR: 该研究评估了多种医疗废物分类模型，发现YOLOv5-s在准确率上表现最佳（95.06%），但YOLOv8-n在推理速度上略胜一筹。EfficientNet-B0准确率为93.22%，但推理时间最长。最终部署了YOLOv5-s模型，并结合尼泊尔的医疗废物管理标准。


<details>
  <summary>Details</summary>
Motivation: 尼泊尔医疗设施增加导致医疗废物管理挑战加剧，不正确的分类和处置可能引发污染和传染病传播，危害废物处理人员。

Method: 研究采用分层K折技术（5折）评估了ResNeXt-50、EfficientNet-B0、MobileNetV3-S、YOLOv8-n和YOLOv5-s模型，并通过重复ANOVA验证统计显著性。

Result: YOLOv5-s准确率最高（95.06%），但YOLOv8-n推理速度更快；EfficientNet-B0准确率为93.22%，但推理时间最长。

Conclusion: 研究成功部署了YOLOv5-s模型，并建议进一步优化数据及结合本地化背景。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅关注最终答案准确性，忽略了推理过程，无法区分真正理解与错误推理或幻觉导致的正确答案。

Method: AURA包含六个认知领域的问题，强制模型基于音频和视频构建逻辑路径，并提出AuraScore评估推理保真度。

Result: SOTA模型在AURA上显示高准确率（92%），但推理保真度低于45%，表明模型常通过错误逻辑得出正确答案。
...
TL;DR: 论文比较了摄影测量和高斯泼溅技术在3D模型重建和视图合成中的表现，提出了一种改进的高斯泼溅方法，并展示了其在提升摄影测量重建质量上的潜力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在比较摄影测量与高斯泼溅技术在3D重建和视图合成中的性能差异，探索高斯泼溅在生成高质量新视图方面的潜力及其对摄影测量的改进作用。

Method: 通过创建真实场景的图像数据集，分别使用摄影测量和高斯泼溅技术构建3D模型，并采用SSIM、PSNR、LPIPS和lp/mm分辨率等指标进行评估。改进高斯泼溅方法以支持Blender环境中的新视图合成。

Result: 改进的高斯泼溅方法能够生成高质量的新视图，并提升摄影测量重建的效果。比较分析揭示了两种方法的优缺点。

Conclusion: 高斯泼溅技术在视图合成和3D重建中具有显著潜力，可应用于扩展现实（XR）、摄影测量和自动驾驶模拟等领域。
...
Result: MLLMs表现优于基于文本和多模态编码器模型，但在结构化表格和低资源语言上仍有挑战。

Conclusion: VisR-Bench填补了多语言长文档多模态检索的空白，并揭示了模型在复杂场景中的局限性。
...
Method: 利用视觉语言模型（VLMs）分析用户动作，通过网页界面展示功能，并在1700个专家标注的视频对上测试。
...
TL;DR: 提出了一种结合自监督视觉模型和植物分类学的改进分割模型，显著提升了物种识别和损害分类的准确率，并在跨设备评估中表现出强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统手动视觉评估耗时、费力且主观，自动化物种和损害识别能显著提升效率和一致性。

Method: 结合通用自监督视觉模型与植物分类学的分层推理，使用多国多年数据集训练，并在跨设备数据上测试。

Result: 物种识别（F1-score: 0.52到0.85）和损害分类（F1-score: 0.28到0.44）显著提升，跨设备测试中表现稳健。

Conclusion: 模型鲁棒性强，已部署于实际应用中，支持大规模自动化作物和杂草监测。
...
TL;DR: 论文分析了基于Transformer的扩散模型MM-DiT的注意力机制，提出了一种适用于其的基于提示的图像编辑方法。


<details>
  <summary>Details</summary>
Motivation: 传统U-Net架构已被Transformer扩散模型取代，MM-DiT作为主流方法，其双向注意力机制对现有编辑技术提出了挑战。

Method: 通过分解注意力矩阵为四个块，分析其特性，提出了一种支持全局到局部编辑的提示方法。

Result: 提出了一种适用于MM-DiT变体的鲁棒图像编辑方法，包括少步模型。

Conclusion: 研究填补了U-Net方法与新兴架构之间的空白，为MM-DiT的行为模式提供了深入见解。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像诊断的准确性至关重要，但噪声标签和类别模糊实例会干扰top-rank学习的目标，导致诊断错误。

Method: 提出了一种新方法，通过集成拒绝模块与top-rank损失共同优化，识别并减少异常值的影响。拒绝模块通过测量实例与标准的偏差来评估其是否异常。

Result: 在医学数据集上的实验验证表明，该方法能有效检测和减少异常值，提升诊断的可靠性和准确性。

Conclusion: 结合拒绝模块的top-rank学习方法显著改善了医学图像诊断的性能，为处理噪声和模糊实例提供了有效解决方案。
...
TL;DR: 提出了一种基于结构先验的高质量中文文本图像超分辨率框架，通过结合StyleGAN和代码本机制，实现了对低分辨率中文字符的精确笔画恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注英文文本，对更复杂的中文脚本关注不足，且缺乏结构级别的指导。

Method: 提出了一种结构先验，结合StyleGAN和代码本机制，生成高分辨率结构先验以指导超分辨率任务。

Result: 实验表明，该方法能够准确恢复退化字符的清晰笔画，适用于不规则布局的真实低分辨率中文文本。

Conclusion: 该框架通过结构先验和生成模型的协同作用，显著提升了中文文本图像的超分辨率效果。
...
TL;DR: 该论文探讨了医学图像去标识化的重要性，介绍了MIDI-B挑战赛及其方法，并展示了团队算法的优异表现和未来改进方向。
...
Method: 采用像素掩码、日期偏移、哈希、文本识别与替换等方法，严格遵循标准处理数据集。

Result: 团队算法在MIDI-B挑战赛中正确执行99.92%的操作，排名第二。

Conclusion: 当前方法存在局限性，未来需进一步优化以提升去标识化效果。
...
TL;DR: 提出一种针对病理图像中域偏移的新方法，专注于全切片图像（WSI）内的偏移，而非医院间的偏移。通过聚类非肿瘤区域的WSI级特征作为域，并采用两阶段对比学习减少域间特征差距。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖多医院数据，但数据收集困难。因此，研究专注于利用医院内部的域偏移，提出更实用的解决方案。

Method: 通过聚类WSI级特征定义域，并采用两阶段对比学习（WSI级和patch级）减少域间特征差距。

Result: 提出的方法有效减少了域偏移，提高了模型的泛化能力。

Conclusion: 该方法为病理图像中的域偏移问题提供了一种新颖且实用的解决方案，尤其适用于数据收集受限的场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到姿态生成模型依赖低级别详细提示，而人类倾向于使用高级别抽象语言，导致实际应用中的挑战。

Method: 引入CoT推理框架，将抽象提示转化为准确3D姿态，并提出自动生成训练数据三元组（抽象提示、详细提示、对应姿态）的合成流程。

Result: 实验表明CoT-Pose能从抽象文本生成语义对齐的合理姿态。

Conclusion: 研究强调了高级别理解在姿态生成中的重要性，为推理增强方法开辟了新方向。
...
Method: 基于MatchVoice模型，采用粗粒度和细粒度对齐技术，使用GOAL数据集进行训练和评估。

Result: 实验表明MatchVoice具有较好的泛化能力，但需结合更广泛的视频-语言技术以进一步提升性能。
...
Method: 引入PU学习，仅使用正样本和未标记数据进行二分类，为每个未标记图像选择伪标签。

Result: 实验证明了该方法的有效性。

Conclusion: 该方法能够有效选择伪标签，适用于医学图像分割任务。
...
TL;DR: 该论文提出了一种基于特征图收敛分数（FMCS）的独立评估方法，用于端到端自动驾驶模型中的功能模块特征图质量评估，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 端到端模型在自动驾驶感知和规划中成为主流，但缺乏对中间功能模块的显式监督信号，导致机制不透明且可解释性有限。

Method: 构建了双粒度动态加权评分系统（DG-DWSS），并提出基于CLIP的特征图质量评估网络（CLIP-FMQE-Net），实现特征图质量的实时分析。
...
Method: 采用多视图渲染、不一致性识别、视频扩散模型和非对称U-Net进行优化。

Result: 在公开基准测试中表现优异，支持多种应用（如文本/图像条件生成、4D人体生成和文本引导编辑）。

Conclusion: Splat4D在4D内容生成中表现出色，具有高效性和多功能性。
...
TL;DR: 论文提出了自适应缓存增强（ACE）框架，通过动态调整缓存和决策边界，解决了视觉语言模型在分布偏移下的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本泛化中表现优异，但在分布偏移时性能下降，尤其是在缺乏标注数据的情况下。现有缓存方法因不可靠的置信度指标和刚性决策边界而受限。

Method: ACE框架通过动态、类别特定的阈值选择高置信度或低熵图像嵌入，并使用指数移动平均和探索增强更新来优化缓存和决策边界。

Result: 在15个基准数据集上的实验表明，ACE在分布偏移场景下实现了最先进的性能，优于现有测试时适应方法。
...
TL;DR: 论文研究了LayerNorm在Vision Transformers（ViTs）中的微调动态，提出了一种基于Fine-tuning Shift Ratio（FSR）的标量λ重新缩放机制，以优化LayerNorm的微调效果。


<details>
  <summary>Details</summary>
Motivation: 探索LayerNorm在数据稀缺和领域转移下的微调动态，揭示其参数变化对源域和目标域过渡的指示作用。

Method: 提出FSR量化目标训练样本的代表性，并设计标量λ和循环框架来优化LayerNorm微调。

Result: 实验验证了框架在自然和病理图像中的有效性，OOD任务通常需要更高的λ和更低的FSR。
...
TL;DR: GAPNet是一种轻量级网络，基于粒度感知范式，用于图像和视频显著性目标检测（SOD），通过多尺度监督和高效特征融合，实现了高性能且低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前SOD模型依赖重型主干网络，计算成本高，限制了在边缘设备等实际场景中的应用。GAPNet旨在解决这一问题。

Method: 采用粒度感知范式，通过多尺度解码器监督（粗粒度目标位置和细粒度边界），设计粒度金字塔卷积（GPC）和跨尺度注意力（CSA）模块进行特征融合，并在编码器上加入自注意力模块。

Result: 在轻量级图像和视频SOD模型中达到了新的最佳性能。

Conclusion: GAPNet通过优化特征利用和语义解释，实现了高效且高性能的显著性目标检测。
...
TL;DR: 论文提出了一种基于机器学习的非侵入性方法，利用声学特征和RNN模型检测语音病理，旨在替代传统侵入性诊断方法。


<details>
  <summary>Details</summary>
Motivation: 语音障碍严重影响交流和生活质量，传统诊断方法（如喉镜检查）具有侵入性、主观性和不易获取性。

Method: 使用MFCC、chroma特征和Mel频谱图分析语音数据，结合LSTM和注意力机制的RNN进行分类，并通过数据增强和预处理提高模型泛化能力。

Result: 提出的框架为非侵入性、自动化的语音病理诊断工具，支持AI驱动的医疗保健。

Conclusion: 该方法为早期语音病理检测提供了有效解决方案，有望改善患者治疗效果。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造技术对数字完整性构成严重威胁，现有检测系统缺乏透明度和可解释性，限制了其实际应用。

Method: DF-P2E包含三个模块：基于Grad-CAM的深度伪造分类器、视觉描述生成模块和基于LLM的叙述优化模块。

Result: 在DF40数据集上，系统实现了竞争性的检测性能，并提供高质量的解释。
...
TL;DR: ShoulderShot框架通过双镜头生成和循环视频技术，解决了对话视频生成中的角色一致性和空间连续性问题，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 对话视频在影视和广告中很重要，但现有研究对其生成挑战（如角色一致性和空间连续性）探索不足。

Method: 结合双镜头生成与循环视频技术，生成长对话视频。

Result: 在镜头切换布局、空间连续性和对话长度灵活性上优于现有方法。

Conclusion: 为实际对话视频生成提供了新可能性。
...
TL;DR: LaVieID是一个新颖的局部自回归视频扩散框架，专注于解决身份保持的文本到视频任务，通过空间和时间视角优化身份信息保留。
...
Method: 引入局部路由器显式表示细粒度面部结构，并集成时序自回归模块利用长程依赖优化潜在令牌。
...
TL;DR: 论文提出了X2Edit数据集和任务感知的MoE-LoRA训练方法，解决了开源图像编辑数据集的不足和生成模型兼容性问题。
...
Method: 构建X2Edit数据集（14种任务，370万高质量数据），设计任务感知MoE-LoRA训练方法，结合对比学习优化性能。

Result: 模型编辑性能优异，数据集优于现有开源资源。

Conclusion: X2Edit数据集和方法显著提升了图像编辑任务的多样性和兼容性。
...
Method: 第一阶段使用INR生成覆盖患者头部的先验图像，第二阶段利用校正后的投影数据进行迭代重建。

Result: 数值结果表明，该方法有效抑制了截断伪影，提升了CBCT图像质量。

Conclusion: 两阶段方法在减少截断伪影方面具有潜力，适用于牙科CBCT系统。
...
TL;DR: SOFA是一种深度学习框架，通过模拟消融策略和优化手术参数，预测房颤复发风险并降低22.18%的复发风险。


<details>
  <summary>Details</summary>
Motivation: 房颤消融手术效果差异大，需改进方法以预测复发并优化手术参数。

Method: SOFA利用多模态多视角生成器模拟消融后图像，预测复发风险，并通过优化手术参数降低风险。
...
TL;DR: 提出了一种基于图神经网络的后期处理流程，通过建模物体间的空间关系来纠正检测异常，显著提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 静态环境中物体的空间布局通常一致，但现有目标检测模型未能充分利用这一先验，导致检测不一致或错误。

Method: 使用图神经网络（GNN）建模物体间的空间关系，纠正检测异常，并作为标准检测器的后期处理模块。
...
TL;DR: 提出了一种基于不确定性估计的可信情感识别方法（TER），通过结合多模态数据的置信度输出可信预测，并引入新的评估标准验证模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别方法虽性能优越但模型复杂，且对噪声和异常数据缺乏可靠性，因此需要一种能提供可信预测的方法。

Method: TER利用不确定性估计计算预测置信度，结合多模态数据输出可信预测，并引入可信精度和召回率等新评估标准。

Result: TER在Music-video数据集上达到82.40%准确率，在IEMOCAP和Music-video上的可信F1分数分别为0.7511和0.9035，表现优异。

Conclusion: TER通过置信度模块提升了模型的可靠性和鲁棒性，实验验证了其在情感识别中的有效性。
...
TL;DR: AR-VRM通过显式模仿人类手部关键点，提升视觉机器人操纵的泛化能力，尤其在数据稀缺时表现优异。
...
Conclusion: 显式模仿人类动作能有效提升机器人操纵性能，尤其在数据不足时。
...
Conclusion: 该方法无需重新训练，实现了精确遮挡控制，并扩展了多种视觉效果的应用。
...
TL;DR: 提出了一种结合散射和深度特征的协作学习方法（CLSDF），用于解决SAR自动目标识别中噪声标签的问题，通过多模型特征融合和半监督学习实现了高性能。


<details>
  <summary>Details</summary>
Motivation: 由于SAR数据标注需要专业知识，噪声标签不可避免，导致性能下降。现有方法主要针对图像数据，而SAR数据的非直观特性使其难以实现噪声鲁棒学习。

Method: 设计了多模型特征融合框架，将散射和深度特征结合；利用动态图结构数据表示散射中心；通过GMM模型区分干净和噪声标签样本；采用半监督学习和联合分布对齐策略。

Result: 在MSTAR数据集上的实验表明，该方法在不同标签噪声条件下均能达到最先进的性能。

Conclusion: CLSDF方法通过结合物理特征和深度学习，有效提升了SAR自动目标识别在噪声标签下的鲁棒性和性能。
...
TL;DR: 论文提出了一种无需训练的虚拟试穿框架UR-VTON，通过“脱衣-穿衣”机制解决长袖转短袖试穿中皮肤还原不准确的问题，并结合动态指导和结构细化提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法在长袖转短袖场景中因皮肤还原不准确导致输出不真实，需改进。

Method: 提出UR-VTON框架，采用“脱衣-穿衣”两步机制，结合动态分类器自由指导和结构细化器。

Result: UR-VTON在细节保留和图像质量上优于现有方法，并提出了新基准LS-TON。

Conclusion: UR-VTON为虚拟试穿提供了一种高效且无需训练的解决方案，显著提升了长袖转短袖场景的效果。
...
TL;DR: 论文提出TAR-TVG框架，通过引入时间戳锚点强化推理过程，提升视频片段定位的准确性，并通过三阶段训练策略优化模型表现。
...
Result: 实验显示模型达到最先进性能，生成可解释、可验证的推理链，时间估计逐步细化。

Conclusion: TAR-TVG通过显式监督和渐进优化，显著提升了视频片段定位的准确性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 用户对个性化3D内容的需求增加，但现有编辑工具多为2D领域，直接应用于3D生成会导致信息丢失和质量下降。

Method: 使用几何保留模块和注入切换器，通过原始输入的法线潜在变量指导编辑的多视图生成，并控制原始法线的监督程度。

Result: 实验表明，该方法显著提升了编辑3D资产的多视图一致性和网格质量。

Conclusion: 该方法在多种多视图扩散模型和编辑方法的组合中均表现优异，有效解决了编辑与几何对齐的问题。
...
TL;DR: 3D高斯泼溅（3DGS）在表面重建中表现优异，但单视角投影平面内的高斯法向量对齐可能导致多视角偏差。本文提出多视角法向量和距离引导的高斯泼溅方法，通过约束邻近深度图和对齐3D法向量，实现几何深度统一和高精度重建。


<details>
  <summary>Details</summary>
Motivation: 解决3DGS在多视角场景中因单视角法向量对齐导致的几何偏差问题，提升表面重建的准确性和一致性。

Method: 设计多视角距离重投影正则化模块和多视角法向量增强模块，通过计算邻近视图间的距离损失和法向量匹配损失，实现多视角高斯对齐和一致性。

Result: 实验结果表明，该方法在定量和定性评估中均优于基线，显著提升了3DGS的表面重建能力。

Conclusion: 多视角法向量和距离引导的高斯泼溅方法有效解决了3DGS在多视角场景中的几何偏差问题，实现了高精度和一致性的表面重建。
...
TL;DR: 提出了一种半自动化流程，结合目标检测模型和大语言模型（LLM）构建多类别门检测数据集，显著降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集稀缺，而精准检测和分类门类型对建筑合规检查和室内场景理解至关重要。

Method: 使用深度学习目标检测模型统一检测门，再通过LLM基于视觉和上下文特征分类，最后人工验证确保质量。

Result: 成功构建高质量数据集，适用于神经网络在平面图分析中的基准测试。

Conclusion: 结合深度学习和多模态推理，展示了在复杂现实领域中高效构建数据集的潜力。
...
TL;DR: 论文提出TriReWeight方法，通过三重连接样本重加权增强生成数据增强，解决生成图像噪声问题，理论分析和实验验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 解决生成模型在数据增强中因不可控生成和自然语言模糊性导致的噪声图像问题。

Method: 基于理论分析开发TriReWeight方法，三重连接样本重加权，兼容任何生成数据增强方法。

Result: 实验证明TriReWeight优于现有方法，在自然图像和医学数据集上分别平均提升7.9%和3.4%。
...
TL;DR: 提出了一种名为GSD的无训练加速方法，显著提升了自回归图像模型的推理速度，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 自回归图像模型推理速度慢，限制了其实际应用，现有加速方法效果有限或需额外训练。

Method: 通过动态分组推测解码（GSD），评估视觉有效令牌簇而非单一目标令牌，减少假阴性拒绝。

Result: GSD平均加速3.7倍，且无需额外训练。

Conclusion: GSD是一种高效且实用的自回归图像模型加速方法。
...
<details>
  <summary>Details</summary>
Motivation: 定制化图像生成需求增加，现有方法依赖额外指导（如文本提示或空间掩码），可能导致辅助特征分离不完全，影响生成质量。

Method: 提出对比反转方法，通过对比学习训练目标标记和图像辅助文本标记，应用解耦交叉注意力微调以提高概念保真度。

Result: 实验证明，该方法在概念表示和编辑方面均表现优异，优于现有技术。

Conclusion: 对比反转方法能够有效提取目标概念，无需额外信息，生成和编辑效果更优。
...
Method: CAV-SAM包含DBST模块（扩散模型构建语义转换序列）和TTGA模块（测试时几何对齐），将图像对视为伪视频。

Result: 在多个数据集上性能提升超过5%，优于现有SOTA方法。

Conclusion: CAV-SAM为轻量级参考分割提供了高效解决方案，显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统的发展，SVG的理解和生成需求日益迫切，但现有方法面临高精度和多模态处理的挑战。

Method: 提出UniSVG数据集，包含525k数据项，支持文本提示和图像输入的SVG生成，以及SVG的多维度理解。

Result: 基于UniSVG训练的MLLMs在SVG任务中表现优异，甚至超过GPT-4V等闭源模型。

Conclusion: UniSVG为SVG理解和生成提供了统一的数据支持，推动了MLLMs在该领域的应用。
...
Method: 采用两阶段架构：首先通过少样本学习预测相机轨迹，然后通过姿态条件扩散过程生成几何一致的多视角序列，最终转换为持久4D表示。

Result: Dream4D在质量指标（如mPSNR、mSSIM）上优于现有方法。

Conclusion: 该框架首次结合了视频扩散模型的时间先验和重建模型的几何感知能力，显著推动了4D生成的发展。
...
Method: 提出CLZSL框架，包含PCL模块（通过课程学习减少实例级不匹配）和PUP模块（动态更新语义原型以减少类级不精确）。
...
TL;DR: 论文提出了一种基于动作片段的步态识别方法，通过多尺度时间上下文学习更全面的步态特征。
...
Method: 将步态视为个性化动作的集合，通过随机选取连续片段（称为片段）表示动作，并提出了片段采样和片段建模的关键组件。

Result: 在四个广泛使用的步态数据集上验证了方法的有效性，例如在Gait3D和GREW上分别达到了77.5%和81.7%的rank-1准确率。

Conclusion: 该方法通过片段实现了多尺度时间上下文的学习，展示了片段在步态识别中的潜力。
...
### [156] [Architectural Co-Design for Zero-Shot Anomaly Detection: Decoupling Representation and Dynamically Fusing Features in CLIP](https://arxiv.org/abs/2508.07819)
*Ke Ma,Jun Long,Hongxiao Fei,Liujie Hua,Yueyi Luo*

Main category: cs.CV

TL;DR: 提出了一种架构协同设计框架，通过卷积低秩适配器和动态融合网关解决视觉语言模型在零样本异常检测中的适应性问题。
...
Method: 采用卷积低秩适配器（Conv-LoRA）注入局部归纳偏置，并引入动态融合网关（DFG）自适应调制文本提示。

Result: 在工业和医学基准测试中表现出更高的准确性和鲁棒性。

Conclusion: 协同设计框架为密集感知任务提供了强大的适应能力。
...
### [157] [Deep Space Weather Model: Long-Range Solar Flare Prediction from Multi-Wavelength Images](https://arxiv.org/abs/2508.07847)
*Shunya Nagashima,Komei Sugiura*

Main category: cs.CV

TL;DR: 提出了一种基于多深度状态空间模型的Deep SWM方法，用于处理太阳图像和长时空依赖性，并通过新的预训练策略和公开基准FlareBench验证其性能。


<details>
  <summary>Details</summary>
Motivation: 准确的太阳耀斑预测对减少关键基础设施的潜在破坏至关重要，但现有方法在表示学习和长时空依赖性建模方面存在不足。

Method: 提出Deep SWM，结合多深度状态空间模型和稀疏掩码自编码器，采用两阶段掩码策略保留关键区域并压缩空间信息。

Result: Deep SWM在性能和可靠性上优于基线方法及人类专家，并通过FlareBench验证。

Conclusion: Deep SWM为太阳耀斑预测提供了高效可靠的解决方案，并公开了基准数据集。
...
TL;DR: 论文提出了一种两阶段防御框架（TSDF），通过双重功能对抗扰动来持久防御深度伪造技术，防止攻击者通过重新训练模型绕过防御。
...
Method: 提出TSDF框架，利用强度分离机制设计双重功能对抗扰动，既能直接扭曲伪造结果，又能通过数据中毒破坏攻击者的重新训练流程。

Result: 实验表明，传统防御方法在对抗重新训练时性能急剧下降，而TSDF展现出强大的双重防御能力，提升了防御的持久性。

Conclusion: TSDF通过双重功能对抗扰动实现了持久防御，解决了现有防御策略的不足。
...
TL;DR: 论文提出了一种新的任务——动力电池检测（PBD），并构建了首个大规模基准数据集PBD5K，包含5000张X射线图像。作者开发了智能标注流程，并提出MDCNeXt模型，结合多维结构线索和状态空间模块，解决了传统视觉算法在密集板片、低对比度等问题上的不足。


<details>
  <summary>Details</summary>
Motivation: 动力电池内部结构缺陷可能引发严重安全隐患，传统人工检测效率低且易出错，现有视觉算法难以应对密集板片、低对比度等挑战。

Method: 提出PBD5K数据集和智能标注流程；将PBD任务建模为点级分割问题，设计MDCNeXt模型，整合点、线、计数等多维结构线索，并引入两个状态空间模块（提示过滤模块和密度感知重排序模块）。

Result: MDCNeXt模型在动力电池检测任务中表现优异，能够有效区分板片并抑制视觉干扰。

Conclusion: PBD5K数据集和MDCNeXt模型为动力电池检测任务提供了有效解决方案，未来将公开源代码和数据集。
...
### [160] [Selective Contrastive Learning for Weakly Supervised Affordance Grounding](https://arxiv.org/abs/2508.07877)
*WonJun Moon,Hyun Seok Seong,Jae-Pil Heo*

Main category: cs.CV

TL;DR: 论文提出了一种选择性原型和像素对比目标的方法，通过多视角学习来提升弱监督功能接地（WSAG）的性能，解决了传统方法依赖分类而忽略功能相关部分的问题。


<details>
  <summary>Details</summary>
Motivation: 人类能够从第三人称演示中直观地理解功能部分，而无需像素级标注。现有方法主要依赖分类，容易忽略功能相关部分，因此需要一种更有效的方法来学习功能接地。

Method: 通过选择性原型和像素对比目标，结合CLIP模型，从第一人称和第三人称图像中挖掘功能相关部分，并通过对比学习区分功能相关与无关区域。

Result: 实验结果表明，该方法能够有效将激活从无关区域转移到功能相关线索上，提升了功能接地的准确性。

Conclusion: 该方法通过多视角学习和对比目标，显著提升了弱监督功能接地的性能，代码已开源。
...
### [161] [MambaTrans: Multimodal Fusion Image Translation via Large Language Model Priors for Downstream Visual Tasks](https://arxiv.org/abs/2508.07803)
*Yushen Xu,Xiaosong Li,Zhenyu Kuang,Xiaoqi Cheng,Haishu Tan,Huafeng Li*

Main category: cs.CV

TL;DR: 本文提出MambaTrans，一种多模态融合图像模态翻译器，旨在解决多模态融合图像与可见光图像之间的像素分布差异问题，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 多模态融合图像与可见光图像的像素分布差异导致下游任务性能下降，甚至低于仅使用可见光图像的情况。本文探索如何将多模态融合图像适配于基于可见光图像训练的模型。

Method: 提出MambaTrans，利用多模态大语言模型的描述和语义分割模型的掩码作为输入，通过Multi-Model State Space Block结合掩码-图像-文本交叉注意力和3D-Selective Scan模块，增强视觉能力并最小化检测损失。

Result: 在公开数据集上的实验表明，MambaTrans有效提升了多模态图像在下游任务中的性能。

Conclusion: MambaTrans在不调整预训练模型参数的情况下，成功改善了多模态融合图像在下游任务中的表现。
...
### [162] [Pose-RFT: Enhancing MLLMs for 3D Pose Generation via Hybrid Action Reinforcement Fine-Tuning](https://arxiv.org/abs/2508.07804)
*Bao Li,Xiaomei Zhang,Miao Xu,Zhaoxin Fan,Xiangyu Zhu,Zhen Lei*

Main category: cs.CV

TL;DR: Pose-RFT提出了一种基于强化学习的微调框架，用于多模态大语言模型（MLLMs）中的3D人体姿态生成，通过混合动作强化学习优化语言预测和姿态生成。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在3D姿态生成任务中因监督目标（如SMPL参数回归或令牌级预测）难以建模模糊性和任务对齐，导致性能受限。

Method: 提出Pose-RFT框架，采用混合动作强化学习（HyGRPO算法），结合离散语言预测和连续姿态生成的联合优化，并引入任务特定奖励函数。

Result: 在多个姿态生成基准测试中，Pose-RFT显著优于现有MLLMs，验证了混合动作强化微调的有效性。

Conclusion: Pose-RFT通过强化学习优化多模态输入下的3D姿态生成，解决了现有方法的模糊性和对齐问题，提升了性能。
...
### [163] [NeeCo: Image Synthesis of Novel Instrument States Based on Dynamic and Deformable 3D Gaussian Reconstruction](https://arxiv.org/abs/2508.07897)
*Tianle Zeng,Junlei Hu,Gerardo Loza Galindo,Sharib Ali,Duygu Sarikaya,Pietro Valdastri,Dominic Jones*

Main category: cs.CV

TL;DR: 论文提出了一种动态高斯Splatting技术，用于解决手术图像数据集稀缺问题，通过动态高斯模型生成逼真的合成图像，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动方法需要大量高质量标注数据，限制了手术数据科学的应用。本文旨在解决手术图像数据集稀缺问题。

Method: 提出动态高斯Splatting技术，利用动态高斯模型表示手术场景，结合动态训练调整策略和自动标注方法生成合成数据。

Result: 实验结果显示，该方法生成的合成图像质量高（PSNR 29.87），且使用合成数据训练的模型性能优于标准数据增强方法10%。

Conclusion: 动态高斯Splatting技术有效解决了数据稀缺问题，显著提升了模型性能，为手术自动化提供了新思路。
...
### [164] [DiTVR: Zero-Shot Diffusion Transformer for Video Restoration](https://arxiv.org/abs/2508.07811)
...
Method: 使用扩散变换器与轨迹感知注意力机制，结合光流轨迹对齐令牌，动态选择相关令牌，并通过小波引导的流一致采样器注入数据一致性。

Result: DiTVR在视频修复基准测试中达到零样本最优性能，时间一致性和细节保留表现优异，且对光流噪声和遮挡具有鲁棒性。
...
### [165] [Semi-supervised Multiscale Matching for SAR-Optical Image](https://arxiv.org/abs/2508.07812)
*Jingze Gai,Changchun Li*

Main category: cs.CV

TL;DR: 提出了一种半监督SAR-光学图像匹配方法S2M2-SAR，结合伪标签和无监督特征增强，显著减少对标注数据的依赖，性能优于现有半监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有SAR-光学图像匹配方法依赖大量标注数据，标注成本高且难以获取。

Method: 设计半监督匹配流程，利用伪标签和跨模态特征增强模块，结合深度和浅层匹配结果生成伪标签。

Result: S2M2-SAR性能优于半监督方法，与全监督方法相当。

Conclusion: S2M2-SAR高效实用，减少了对标注数据的依赖。
...
### [166] [Segmenting and Understanding: Region-aware Semantic Attention for Fine-grained Image Quality Assessment with Large Language Models](https://arxiv.org/abs/2508.07818)
*Chenyue Song,Chen Hui,Haiqi Zhu,Feng Jiang,Yachun Mi,Wei Zhang,Shaohui Liu*

Main category: cs.CV

TL;DR: RSFIQA是一种细粒度图像质量评估模型，通过动态分割图像并结合多模态大语言模型感知局部失真，利用区域感知语义注意力机制提升全局注意力，性能优于现有方法。
...
Method: 利用SAM动态分割图像，结合MLLM提取局部描述和失真信息，引入RSA机制聚合局部特征生成全局注意力。

Result: 在多个基准数据集上表现出鲁棒性和有效性，预测性能优于现有方法。

Conclusion: RSFIQA通过细粒度区域分析和注意力机制，显著提升了图像质量评估的准确性。
...
### [167] [Omni-Effects: Unified and Spatially-Controllable Visual Effects Generation](https://arxiv.org/abs/2508.07981)
*Fangyuan Mao,Aiming Hao,Jintao Chen,Dongxia Liu,Xiaokun Feng,Jiashu Zhu,Meiqi Wu,Chubin Chen,Jiahong Wu,Xiangxiang Chu*

Main category: cs.CV

TL;DR: Omni-Effects是一个统一的框架，通过LoRA-MoE和SAP技术实现多效果的空间可控生成，解决了现有方法的单效果限制和空间不可控问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在视觉特效（VFX）生产中受限于单效果训练，无法实现多效果的空间可控生成。

Method: 提出LoRA-MoE（专家LoRA组）和SAP（空间感知提示）技术，结合IIF模块，实现多效果集成和精确空间控制。

Result: Omni-Effects能够生成多样化的效果并精确控制其空间位置，实验验证了其有效性。

Conclusion: Omni-Effects为多效果的空间可控生成提供了高效解决方案，推动了VFX生产的发展。
...
### [168] [MIMIC: Multimodal Inversion for Model Interpretation and Conceptualization](https://arxiv.org/abs/2508.07833)
*Animesh Jain,Alexandros Stergiou*

Main category: cs.CV

TL;DR: MIMIC框架通过视觉概念合成可视化VLM内部表示，提升透明度和信任。
...
Method: 提出MIMIC框架，结合VLM反演和特征对齐目标，使用三种正则化器。

Result: 定量和定性评估显示MIMIC能有效反演视觉概念，并达到高质量和语义准确性。

Conclusion: MIMIC是首个针对VLM视觉解释的反演方法，具有创新性和实用性。
...
### [169] [Effortless Vision-Language Model Specialization in Histopathology without Annotation](https://arxiv.org/abs/2508.07835)
...
<details>
  <summary>Details</summary>
Motivation: 尽管通用视觉语言模型（如CONCH和QuiltNet）在组织病理学中表现出色，但其通用设计可能导致在特定下游任务中性能不佳。传统的有监督微调需要手动标注样本，而本文探索无标注的适应方法。

Method: 通过从现有数据库中提取领域和任务相关的图像-标题对，对VLMs进行继续预训练。实验在CONCH和QuiltNet上进行，覆盖三个下游任务。

Result: 实验表明，这种无标注的继续预训练显著提升了零样本和小样本性能，且在更大训练规模下可与小样本方法媲美，同时无需手动标注。

Conclusion: 无标注的继续预训练是一种有效的、任务无关的VLM适应方法，为组织病理学任务提供了一种有前景的解决方案。
...
### [170] [CBDES MoE: Hierarchically Decoupled Mixture-of-Experts for Functional Modules in Autonomous Driving](https://arxiv.org/abs/2508.07838)
*Qi Xiang,Kunsong Shi,Zhigui Lin,Lei He*

Main category: cs.CV

TL;DR: 提出了一种名为CBDES MoE的分层解耦Mixture-of-Experts架构，用于解决多模态BEV感知系统的输入适应性、建模能力和泛化性问题。


<details>
  <summary>Details</summary>
Motivation: 现有BEV方法在多模态输入适应性、建模能力和泛化性方面存在不足。

Method: 采用分层解耦的Mixture-of-Experts架构，结合轻量级自注意力路由机制，实现动态专家路径选择和高效推理。

Result: 在nuScenes数据集上，CBDES MoE在3D目标检测中显著优于单专家模型，mAP提升1.6点，NDS提升4.1点。

Conclusion: CBDES MoE是一种高效且实用的多模态BEV感知解决方案。
...
### [171] [Morphological Analysis of Semiconductor Microstructures using Skeleton Graphs](https://arxiv.org/abs/2508.07850)
*Noriko Nitta,Rei Miyata,Naoto Oishi*

Main category: cs.CV

TL;DR: 通过电子显微镜图像提取拓扑特征，利用图卷积网络嵌入，并用主成分分析评估簇分离性，发现辐照角度对Ge表面形态的影响大于辐照通量。
...
Method: 处理电子显微镜图像提取拓扑特征，使用图卷积网络嵌入，通过主成分分析和Davies-Bouldin指数评估簇分离性。

Result: 辐照角度的变化对Ge表面形态的影响比辐照通量的变化更显著。
...
### [172] [Tracking Any Point Methods for Markerless 3D Tissue Tracking in Endoscopic Stereo Images](https://arxiv.org/abs/2508.07851)
...
<details>
  <summary>Details</summary>
Motivation: 微创手术中存在动态组织运动和视野受限的问题，准确的追踪可提升手术安全性并支持机器人辅助。
...
### [173] [Investigating the Design Space of Visual Grounding in Multimodal Large Language Model](https://arxiv.org/abs/2508.08066)
*Weitai Kang,Weiming Zhuang,Zhizhong Li,Yan Yan,Lingjuan Lyu*

Main category: cs.CV

TL;DR: 本文系统研究了多模态大语言模型（MLLMs）在视觉接地（VG）任务中的设计选择，通过分析LLaVA-1.5模型，提出了优化方案，性能提升了5.6%-7.0%。


<details>
  <summary>Details</summary>
Motivation: 现有方法在MLLMs的视觉接地任务中设计选择分散且缺乏系统性验证，本文旨在填补这一空白。

Method: 使用LLaVA-1.5模型，分析不同视觉接地范式及接地数据设计，进行消融研究。

Result: 优化后的MLLM在RefCOCO/+/g数据集上性能提升了5.6%-7.0%。

Conclusion: 本文的研究为MLLMs在视觉接地任务中的设计提供了系统性指导，显著提升了性能。
...
### [174] [Being-M0.5: A Real-Time Controllable Vision-Language-Motion Model](https://arxiv.org/abs/2508.07863)
*Bin Cao,Sipeng Zheng,Ye Wang,Lujie Xia,Qianshan Wei,Qin Jin,Jing Liu,Zongqing Lu*

Main category: cs.CV

TL;DR: 论文提出了Being-M0.5，一种实时可控的视觉-语言-运动模型（VLMM），解决了现有模型在可控性方面的五大限制，并在多个运动生成任务中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-运动模型（VLMMs）在可控性方面存在显著限制，阻碍了其实际应用。论文旨在解决这些限制，提升运动生成技术的实用性。

Method: 基于HuMo100M数据集（包含500万自收集运动序列和1亿多任务指令实例），提出了一种新颖的部分感知残差量化技术，实现了对单个身体部位的精细控制。

Result: Being-M0.5在多个运动生成任务中表现出色，并通过效率分析验证了其实时能力。

Conclusion: HuMo100M和Being-M0.5为运动生成技术的实际应用提供了重要进展，未来有望加速该技术在现实场景中的采用。
...
### [175] [CATP: Contextually Adaptive Token Pruning for Efficient and Enhanced Multimodal In-Context Learning](https://arxiv.org/abs/2508.07871)
*Yanshu Li,Jianjiang Yang,Zhennan Shen,Ligong Han,Haoyan Xu,Ruixiang Tang*

Main category: cs.CV

TL;DR: 论文提出了一种针对多模态上下文学习（ICL）的上下文自适应令牌修剪方法（CATP），通过两阶段渐进修剪减少图像令牌冗余，提升效率且性能略有提升。


<details>
  <summary>Details</summary>
Motivation: 现有图像令牌修剪方法仅适用于单图像任务，忽略了多模态ICL中更大的冗余和效率需求，导致性能下降。

Method: 提出CATP方法，通过两阶段渐进修剪考虑输入序列中的跨模态交互，减少冗余图像令牌。

Result: 修剪77.8%的图像令牌后，CATP在四个LVLM和八个基准测试中平均性能提升0.6%，推理延迟平均降低10.78%。
...
### [176] [Hyperspectral Imaging](https://arxiv.org/abs/2508.08107)
...
TL;DR: 这篇论文综述了高光谱成像（HSI）的技术原理、数据处理方法、应用领域及未来发展方向，强调了其在多学科中的潜力。


<details>
  <summary>Details</summary>
Motivation: HSI作为一种先进的传感技术，能够同时捕捉空间和光谱信息，为材料、化学和生物分析提供非侵入性、无标记的解决方案。本文旨在全面介绍HSI的技术基础、数据处理方法及其广泛应用。

Method: 论文从HSI的物理原理和传感器架构出发，详细描述了数据采集、校准和校正的关键步骤，并总结了常见的数据结构和分析方法，包括降维、分类、光谱解混和基于深度学习的AI技术。

Result: HSI在多个领域（如地球观测、精准农业、生物医学等）展示了其揭示亚视觉特征的能力，支持高级监测、诊断和决策。论文还探讨了当前的技术挑战和新兴解决方案。

Conclusion: HSI有望成为跨学科的通用平台，未来发展方向包括实时嵌入式系统、传感器小型化和自监督学习等，其在科学、技术和社会中的应用前景广阔。
...
### [177] [TAP: Parameter-efficient Task-Aware Prompting for Adverse Weather Removal](https://arxiv.org/abs/2508.07878)
...
Method: 采用两阶段训练范式（预训练和提示调优），利用任务感知增强提示和低秩分解捕捉任务共性与特性。
...
### [178] [Safeguarding Generative AI Applications in Preclinical Imaging through Hybrid Anomaly Detection](https://arxiv.org/abs/2508.07923)
*Jakub Binda,Valentina Paneta,Vasileios Eleftheriadis,Hongkyou Chung,Panagiotis Papadimitroulas,Neo Christopher Chung*

Main category: cs.CV

TL;DR: 论文提出了一种混合异常检测框架，用于增强生成式AI在核医学数据合成中的可靠性和安全性，并通过两个应用案例验证了其有效性。
...
Method: 开发并实施了一种混合异常检测框架，应用于Pose2Xray和DosimetrEYE两个系统，以检测和管理模型异常行为。

Result: 该框架提高了系统的可靠性，减少了人工监督需求，并支持实时质量控制，增强了生成式AI在临床前环境中的工业可行性。

Conclusion: 混合异常检测框架为生成式AI在核医学中的应用提供了更高的稳健性、可扩展性和法规合规性。
...
### [179] [GRASPTrack: Geometry-Reasoned Association via Segmentation and Projection for Multi-Object Tracking](https://arxiv.org/abs/2508.08117)
...
<details>
  <summary>Details</summary>
Motivation: 传统跟踪方法因缺乏几何意识，难以解决遮挡和深度模糊问题，GRASPTrack旨在通过3D几何推理提升跟踪性能。

Method: 集成单目深度估计和实例分割，生成3D点云并体素化，结合深度感知自适应噪声补偿和深度增强的运动一致性。

Result: 在MOT17、MOT20和DanceTrack基准测试中表现优异，显著提升了复杂场景下的跟踪鲁棒性。

Conclusion: GRASPTrack通过3D几何推理和深度感知技术，有效提升了多目标跟踪在复杂场景中的性能。
...
TL;DR: 提出了一种轻量级、即插即用的身份保留视频生成框架Stand-In，通过条件图像分支和受限自注意力实现高效身份控制，仅需少量训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖过多训练参数且与其他AIGC工具不兼容，需改进身份保留视频生成的效率和兼容性。

Method: 在预训练视频生成模型中引入条件图像分支，通过受限自注意力和条件位置映射实现身份控制，仅需2000对训练数据。

Result: 仅增加约1%参数，视频质量和身份保留效果优于全参数训练方法，且可无缝集成其他任务。

Conclusion: Stand-In框架高效、轻量且兼容性强，为身份保留视频生成提供了实用解决方案。
...
### [181] [PrIINeR: Towards Prior-Informed Implicit Neural Representations for Accelerated MRI](https://arxiv.org/abs/2508.08058)
...
Method: PrIINeR通过整合预训练模型的先验知识，结合实例优化和双重数据一致性，实现k空间数据与先验重建的对齐。

Result: 在NYU fastMRI数据集上，PrIINeR优于现有INR和部分学习型方法，显著提升结构保留和图像保真度。
...
<details>
  <summary>Details</summary>
Motivation: 历史文档手写文本识别面临手写变异性、文档质量下降及标注不足的挑战，尤其是标注错误（如连字符问题）影响识别效果。

Method: 采用基于CTC对齐算法的自训练方法，通过动态规划和模型输出概率匹配完整转录与文本行图像，迭代训练提升性能。

Result: 方法提高了识别性能（如CER提升1.1个百分点）和对齐准确性，发现较弱模型能提供更准确的对齐结果。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频抠图方法因缺乏高质量真实数据而受限，现有数据集仅提供人工标注的不完美数据，导致模型在真实场景中泛化能力差。

Method: 1. 通过大规模合成和伪标注数据集进行预训练；2. 开发可扩展的合成数据生成流程；3. 提出基于视频扩散模型的新架构，利用强先验知识并确保时间一致性。

Result: 在三个基准数据集上表现优异，定性结果显示在多样真实场景中具有强泛化能力。

Conclusion: 新方法通过合成数据和视频扩散模型先验，显著提升了视频抠图的性能和时间一致性，代码已开源。
...
### [184] [MDD-Net: Multimodal Depression Detection through Mutual Transformer](https://arxiv.org/abs/2508.08093)
*Md Rezwanul Haque,Md. Milon Islam,S M Taslim Uddin Raju,Hamdi Altaheri,Lobna Nassar,Fakhri Karray*

Main category: cs.CV

TL;DR: 提出了一种多模态抑郁症检测网络（MDD-Net），通过声学和视觉数据高效融合特征，显著提升了抑郁症检测性能。
...
Method: MDD-Net包含声学和视觉特征提取模块、互相关变换器及检测层，利用多模态数据检测抑郁症。
...
### [185] [MedReasoner: Reinforcement Learning Drives Reasoning Grounding from Clinical Thought to Pixel-Level Precision](https://arxiv.org/abs/2508.08177)
*Zhonghao Yan,Muxi Diao,Yuxuan Yang,Jiayuan Xu,Kaizhou Zhang,Ruoyan Jing,Lele Yang,Yanxi Liu,Kongming Liang,Zhanyu Ma*

Main category: cs.CV

TL;DR: 论文提出了一种名为UMRG的新任务，结合临床推理和像素级定位，并发布了包含14K样本的数据集U-MRG-14K。同时，提出了MedReasoner框架，通过强化学习优化推理模块，实现了在医疗定位任务中的领先性能。


<details>
  <summary>Details</summary>
Motivation: 当前医疗定位方法依赖显式空间提示，难以处理临床实践中的隐式查询，因此需要一种结合临床推理和像素级定位的新方法。

Method: 提出了UMRG任务和U-MRG-14K数据集，并设计了MedReasoner框架，通过强化学习优化推理模块，分离推理与分割任务。

Result: MedReasoner在U-MRG-14K上实现了最先进的性能，并能泛化到未见过的临床查询。

Conclusion: 强化学习在可解释的医疗定位任务中展现出显著潜力。
...
Method: 设计了Transient Dynamics Memory（TDM）和Persistent Structure Memory（PSM）双内存架构，分别处理动态内容和静态结构。

Result: 在多个基准测试中表现优异，实现了高保真动态重建和全局一致的静态结构。
...
### [187] [Integrating Task-Specific and Universal Adapters for Pre-Trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2508.08165)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练模型的CIL方法通常冻结预训练网络并使用轻量级模块（如适配器）适应增量任务，但模块选择错误和忽视共享知识会导致性能下降。

Method: 训练任务特定适配器捕获关键特征，引入熵选择机制；通过适配器融合构建通用适配器，结合两者预测以利用专业和通用知识。

Result: 在多个基准数据集上展示了最先进的性能。

Conclusion: TUNA方法有效解决了CIL中的模块选择和知识共享问题，显著提升了性能。
...
### [188] [RSVLM-QA: A Benchmark Dataset for Remote Sensing Vision Language Model-based Question Answering](https://arxiv.org/abs/2508.07918)
...
Method: 通过整合多个遥感分割和检测数据集，采用双轨标注生成流程，结合GPT-4.1自动生成详细标注和复杂问答对，并开发专门的对象计数自动化流程。

Result: RSVLM-QA包含13,820张图像和162,373个问答对，标注丰富且问题类型多样，实验证明其能有效评估主流视觉语言模型的推理能力。
...
TL;DR: 论文提出了一种名为TAG的零样本视频时间定位方法，通过时间池化、时间相干性聚类和相似性调整，解决了现有方法的语义碎片化和相似性分布偏差问题。
...
Result: 在Charades-STA和ActivityNet Captions数据集上取得最佳性能，且不依赖LLMs。
...
### [190] [OMGSR: You Only Need One Mid-timestep Guidance for Real-World Image Super-Resolution](https://arxiv.org/abs/2508.08227)
*Zhiqiang Wu,Zhaomang Sun,Tong Zhou,Bingtao Fu,Ji Cong,Yitong Dong,Huaqi Zhang,Xuan Tang,Mingsong Chen,Xian Wei*

Main category: cs.CV

TL;DR: OMGSR提出了一种基于DDPM/FM的单步真实图像超分辨率框架，通过在中时间步注入低质量图像潜在分布，优化生成效果。
...
Method: OMGSR在中时间步注入低质量图像潜在分布，并提出潜在分布细化损失和重叠分块LPIPS/GAN损失以优化生成效果。

Result: OMGSR-S/F在512分辨率下表现优异，OMGSR-F在所有参考指标中占据主导地位，1k分辨率下细节生成效果出色。

Conclusion: OMGSR框架有效解决了潜在分布差距问题，显著提升了真实图像超分辨率的生成质量。
...
### [191] [VOIDFace: A Privacy-Preserving Multi-Network Face Recognition With Enhanced Security](https://arxiv.org/abs/2508.07960)
*Ajnas Muhammed,Iurri Medvedev,Nuno Gonçalves*

Main category: cs.CV

TL;DR: VOIDFace提出了一种新型人脸识别框架，通过视觉秘密共享避免数据复制，并采用基于补丁的多训练网络，提升隐私保护和数据控制。


<details>
  <summary>Details</summary>
Motivation: 解决人脸识别中数据复制带来的隐私和伦理问题，增强用户对个人数据的控制。

Method: 使用视觉秘密共享技术存储训练数据，提出基于补丁的多训练网络。

Result: 在VGGFace2数据集上验证了VOIDFace在隐私保护、数据控制和性能上的优势。

Conclusion: VOIDFace在保护隐私和数据控制的同时，保持了竞争性的人脸识别性能。
...
### [192] [Cut2Next: Generating Next Shot via In-Context Tuning](https://arxiv.org/abs/2508.08244)
*Jingwen He,Hongbo Liu,Jiajun Li,Ziqi Huang,Yu Qiao,Wanli Ouyang,Ziwei Liu*

Main category: cs.CV

TL;DR: 论文提出Next Shot Generation (NSG)框架Cut2Next，通过Diffusion Transformer和分层多提示策略生成符合专业编辑模式和电影连续性的高质量镜头。


<details>
  <summary>Details</summary>
Motivation: 当前方法多注重基本视觉一致性，忽略了推动叙事的编辑模式（如正反打镜头、插入镜头），导致输出缺乏叙事复杂性和电影完整性。

Method: 采用Diffusion Transformer (DiT)，结合Hierarchical Multi-Prompting策略（Relational Prompts和Individual Prompts）以及Context-Aware Condition Injection (CACI)和Hierarchical Attention Mask (HAM)技术。
...
### [193] [TrackOR: Towards Personalized Intelligent Operating Rooms Through Robust Tracking](https://arxiv.org/abs/2508.07968)
*Tony Danjun Wang,Christian Heiliger,Nassir Navab,Lennart Bastian*

Main category: cs.CV

TL;DR: TrackOR框架通过3D几何特征实现手术室中多人员长期跟踪与重识别，提升关联准确率11%，支持离线恢复轨迹，为个性化智能系统提供基础。
...
Conclusion: 3D几何信息使手术室人员跟踪成为可能，为团队效率与安全提供新应用。
...
### [194] [The Escalator Problem: Identifying Implicit Motion Blindness in AI for Accessibility](https://arxiv.org/abs/2508.07989)
*Xiantao Zhang*

Main category: cs.CV

TL;DR: 论文指出多模态大语言模型（MLLMs）在盲人和视障人士（BVI）辅助技术中存在信任问题，提出“自动扶梯问题”作为典型案例，揭示模型对连续低信号运动的感知缺陷，呼吁转向物理感知和以人为本的基准开发。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在辅助盲人和视障人士方面潜力巨大，但在实际应用中存在信任问题，尤其是对连续运动的感知不足。

Method: 通过分析“自动扶梯问题”这一典型案例，揭示模型对连续低信号运动的感知缺陷，并提出其根源在于视频理解的帧采样范式。

Result: 模型在感知连续运动时存在“隐性运动盲点”，影响用户信任。

Conclusion: 呼吁从语义识别转向物理感知，开发以人为本的基准，以提升安全性和可靠性。
...
### [195] [Prompt-Guided Relational Reasoning for Social Behavior Understanding with Vision Foundation Models](https://arxiv.org/abs/2508.07996)
*Thinesh Thiyakesan Ponbagavathi,Chengzheng Yang,Alina Roitberg*

Main category: cs.CV

TL;DR: ProGraD是一种通过可学习组提示和轻量级GroupContext Transformer改进Vision Foundation Models（VFMs）在群体活动检测（GAD）中表现的方法，尤其在多群体场景中效果显著。


<details>
  <summary>Details</summary>
Motivation: 现有VFMs（如DinoV2）虽提供优秀特征，但因其基于对象中心数据预训练，在群体动态建模中表现不足，直接替换CNN骨干网络效果有限。

Method: ProGraD通过1）可学习组提示引导VFM关注社交配置，2）轻量级GroupContext Transformer推断参与者-群体关联和集体行为。

Result: 在Cafe和Social-CAD基准测试中超越现有方法，多群体场景下提升6.5%（Group mAP@1.0）和8.2%（Group mAP@0.5），且仅需10M可训练参数。

Conclusion: ProGraD不仅性能优越，还提供可解释的注意力图，为群体活动检测提供了新思路。
...
### [196] [Sample-aware RandAugment: Search-free Automatic Data Augmentation for Effective Image Recognition](https://arxiv.org/abs/2508.08004)
*Anqi Xiao,Weichen Yu,Hongyuan Yu*

Main category: cs.CV

TL;DR: Sample-aware RandAugment (SRA) 是一种无需搜索的自动数据增强方法，通过动态调整增强策略和启发式评分模块，显著提升了性能，并在 ImageNet 上达到 78.31% 的 Top-1 准确率。


<details>
  <summary>Details</summary>
Motivation: 主流自动数据增强方法存在搜索过程耗时或性能不足的问题，SRA 旨在解决这些问题，提供更简单有效的解决方案。

Method: SRA 采用启发式评分模块评估数据复杂性，并应用非对称增强策略，动态调整增强策略。

Result: 在 ImageNet 上达到 78.31% 的 Top-1 准确率，且无需超参数调优，兼容现有增强流程。

Conclusion: SRA 是一种简单、高效且实用的自动数据增强方法，适用于多种任务。
...
### [197] [Mitigating Biases in Surgical Operating Rooms with Geometry](https://arxiv.org/abs/2508.08028)
*Tony Danjun Wang,Tobias Czempiel,Nassir Navab,Lennart Bastian*

Main category: cs.CV

TL;DR: 论文探讨了深度学习模型在手术室（OR）中因学习虚假相关性而导致的偏差问题，提出了一种基于3D点云序列的方法来避免这种偏差。


<details>
  <summary>Details</summary>
Motivation: 手术室中标准化服装（如手术服）掩盖了识别特征，导致模型依赖无关视觉线索（如鞋子或眼镜），而非有意义的生物特征。这影响了智能辅助系统的准确性。

Method: 通过梯度显著性分析揭示CNN模型的偏差，并提出使用3D点云序列编码人员信息，分离形状和运动模式与外观干扰。

Result: 实验显示，RGB模型在临床真实场景中性能下降12%，而几何方法表现更稳定，表明几何表示能捕捉更有意义的生物特征。

Conclusion: 几何表示为解决手术室中人员建模的偏差问题提供了更稳健的途径。
...
### [198] [TRIDE: A Text-assisted Radar-Image weather-aware fusion network for Depth Estimation](https://arxiv.org/abs/2508.08038)
...
Conclusion: TRIDE通过多模态融合和天气适应机制，显著提升了深度估计的精度和鲁棒性。
...
### [199] [S^2VG: 3D Stereoscopic and Spatial Video Generation via Denoising Frame Matrix](https://arxiv.org/abs/2508.08048)
*Peng Dai,Feitong Tan,Qiangeng Xu,Yihua Huang,David Futschik,Ruofei Du,Sean Fanello,Yinda Zhang,Xiaojuan Qi*

Main category: cs.CV

TL;DR: 提出了一种无需姿态估计和额外训练的3D视频生成方法，利用单目视频生成模型和深度信息，通过帧矩阵修复框架和双更新方案实现高质量多视角视频合成。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在单目视频上表现优异，但3D立体和空间视频生成仍是一个未充分探索的挑战。

Method: 通过深度信息将单目视频映射到预定义视角，采用帧矩阵修复框架和双更新方案合成多视角内容，最终生成立体或4D高斯优化的空间视频。
...
### [200] [Information Bottleneck-based Causal Attention for Multi-label Medical Image Recognition](https://arxiv.org/abs/2508.08069)
*Xiaoxiao Cui,Yiran Li,Kai He,Shanzhi Jiang,Mengli Xue,Wentao Li,Junhong Leng,Zhi Liu,Lizhen Cui,Shuo Li*

Main category: cs.CV

TL;DR: 论文提出了一种基于信息瓶颈的因果注意力模型（IBCA），用于医学图像的多标签分类，通过高斯混合多标签空间注意力过滤无关信息，并通过对比增强因果干预减少虚假注意力。


<details>
  <summary>Details</summary>
Motivation: 当前方法在医学图像多标签分类中难以区分因果与虚假注意力，导致诊断准确性受限。

Method: 提出结构因果模型（SCM）和IBCA，结合高斯混合多标签空间注意力与对比增强因果干预。

Result: 在Endo和MuReD数据集上，IBCA在多项指标上显著优于其他方法。

Conclusion: IBCA有效提升了医学图像多标签分类的准确性和可解释性。
...
### [201] [ME-TST+: Micro-expression Analysis via Temporal State Transition with ROI Relationship Awareness](https://arxiv.org/abs/2508.08082)
*Zizheng Guo,Bochao Zou,Junbao Zhuo,Huimin Ma*

Main category: cs.CV

TL;DR: 论文提出ME-TST和ME-TST+两种基于状态空间模型的架构，通过时间状态转移机制改进微表情（MEs）的检测与识别，实现视频级回归和多粒度建模，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在微表情分析中存在固定窗口长度和硬分类的局限性，且将检测与识别视为独立任务，忽略了其内在联系。

Method: 提出ME-TST和ME-TST+架构，利用时间状态转移机制替代传统窗口分类，支持视频级回归和多粒度ROI建模，结合慢快Mamba框架减少信息损失。

Result: 实验表明，所提方法在性能上达到最先进水平。

Conclusion: 通过状态空间模型和协同策略，论文有效解决了微表情分析中的关键问题，为相关研究提供了新思路。
...
### [202] [Matrix-3D: Omnidirectional Explorable 3D World Generation](https://arxiv.org/abs/2508.08086)
...
TL;DR: Matrix-3D框架通过全景表示和视频扩散模型实现高质量3D世界生成，结合了条件视频生成和全景3D重建。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在生成场景范围上的局限性，提升3D世界生成的广泛性和通用性。

Method: 1. 训练轨迹引导的全景视频扩散模型；2. 提出两种3D重建方法：前馈式大范围重建和优化式精细重建；3. 引入Matrix-Pano数据集支持训练。

Result: 在全景视频生成和3D世界生成任务中达到最先进性能。

Conclusion: Matrix-3D框架通过全景表示和高质量数据集，显著提升了3D世界生成的覆盖范围和细节表现。
...
### [203] [3D Plant Root Skeleton Detection and Extraction](https://arxiv.org/abs/2508.08094)
*Jiakai Lin,Jinchang Zhang,Ge Jin,Wenzhan Song,Tianming Liu,Guoyu Lu*

Main category: cs.CV

TL;DR: 提出了一种从少量图像中高效提取植物根系3D骨架的方法，解决了根系复杂结构和缺乏纹理信息导致的建模难题。


<details>
  <summary>Details</summary>
Motivation: 植物根系结构复杂且缺乏纹理信息，传统2D研究无法满足3D空间生长的根系研究需求，3D表型信息对遗传性状研究至关重要。

Method: 包括侧根检测与匹配、三角测量提取侧根骨架结构、以及侧根与主根整合。

Result: 在复杂根系数据集上测试，提取的3D骨架与真实结构高度相似，验证了模型有效性。

Conclusion: 该方法可应用于自动化育种机器人，通过精确分析3D根系结构提升育种效率，推动现代农业智能化。
...
### [204] [TBAC-UniImage: Unified Understanding and Generation by Ladder-Side Diffusion Tuning](https://arxiv.org/abs/2508.08098)
*Junzhe Xu,Yuyang Yin,Xi Chen*

Main category: cs.CV

TL;DR: TBAC-UniImage通过将预训练的扩散模型与多模态大语言模型深度集成，解决了现有统一模型的浅层连接和高计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散统一模型要么生成条件单一（仅用MLLM的最终隐藏状态），要么从头训练成本高昂。

Method: 利用MLLM多个层次的表示作为扩散模型的生成条件，实现更深层次的理解与生成统一。

Result: TBAC-UniImage实现了更精细的理解与生成统一。
...
### [205] [Follow-Your-Shape: Shape-Aware Image Editing via Trajectory-Guided Region Control](https://arxiv.org/abs/2508.08134)
*Zeqian Long,Mingzhe Zheng,Kunyu Feng,Xinhua Zhang,Hongyu Liu,Harry Yang,Linfeng Zhang,Qifeng Chen,Yue Ma*

Main category: cs.CV

TL;DR: 提出了一种无需训练和掩码的图像编辑框架Follow-Your-Shape，通过轨迹差异图（TDM）和KV注入机制实现精确的形状编辑，同时保持非目标区域不变。
...
Method: 通过计算反转和去噪路径的轨迹差异图（TDM）定位可编辑区域，并采用KV注入机制确保编辑稳定性。
...
### [206] [FantasyStyle: Controllable Stylized Distillation for 3D Gaussian Splatting](https://arxiv.org/abs/2508.08136)
*Yitong Yang,Yinglin Wang,Changshuo Wang,Huajie Wang,Shuting He*

Main category: cs.CV

TL;DR: FantasyStyle提出了一种基于3DGS的风格迁移框架，通过扩散模型蒸馏解决多视角不一致和VGG特征依赖问题。
...
Method: 采用多视角频率一致性和可控风格蒸馏，利用负引导优化3D高斯分布。

Result: 实验表明，该方法在多种场景和风格下均优于现有技术，实现了更高的风格化质量和视觉真实感。

Conclusion: FantasyStyle通过创新方法有效解决了3D风格迁移中的关键挑战，显著提升了性能。
...
### [207] [Pindrop it! Audio and Visual Deepfake Countermeasures for Robust Detection and Fine Grained-Localization](https://arxiv.org/abs/2508.08141)
...
Result: 在时间定位任务中表现最佳，在分类任务的TestA子集中排名前四。

Conclusion: 该方法在深度伪造检测领域具有竞争力，尤其在处理局部修改时表现突出。
...
TL;DR: ReconDreamer-RL框架通过结合视频扩散先验和物理建模，缩小自动驾驶训练中的模拟与现实差距，并引入动态对抗代理和轨迹生成器以覆盖更多极端场景。


<details>
  <summary>Details</summary>
Motivation: 现有模拟环境与现实条件差异大，且受限于训练数据分布，难以生成高质量传感器数据。

Method: 提出ReconDreamer-RL框架，结合视频扩散先验和物理建模，引入动态对抗代理和轨迹生成器。

Result: 实验显示，ReconDreamer-RL优于模仿学习方法，碰撞率降低5倍。

Conclusion: 该框架有效提升端到端自动驾驶训练性能，缩小模拟与现实差距。
...
Method: CD-TVD结合对比学习编码器和改进的扩散超分辨率模型，通过预训练学习降级模式和高分辨率细节，仅需一个新生成的高分辨率时间步进行微调。
...
TL;DR: 提出了一种利用RGBD相机进行3D人体网格估计的方法M$^3$，通过模拟深度数据并训练掩码自编码器解决数据稀缺问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: RGBD相机虽普及但未充分利用，现有数据集小且多样性不足，需解决数据稀缺问题以提升3D人体网格估计精度。

Method: 利用MoCap数据集生成虚拟单视角深度数据，训练掩码自编码器补全部分网格，推理时匹配深度数据生成完整网格。

Result: 在SURREAL、CAPE和BEHAVE数据集上分别达到16.8 mm、22.0 mm和70.9 mm的PVE，优于现有方法。

Conclusion: M$^3$方法有效利用深度数据，显著提升3D人体网格估计性能，代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 人体运动生成在多个领域有广泛应用，但现有评估方法在物理可行性与人类感知之间存在差距，且缺乏细粒度的客观标注。

Method: 提出物理标注方法，计算运动与物理定律的最小修改量；基于此标注，设计PP-Motion指标，结合皮尔逊相关损失和人类感知损失训练模型。

Result: 实验表明PP-Motion在物理和感知保真度上优于先前方法。

Conclusion: PP-Motion为运动生成提供了更全面的评估标准，同时兼顾物理可行性和人类感知。
...
TL;DR: 论文提出了一种名为THAT的Transformer框架，通过改进高频特征表示和令牌选择，解决了现有方法在保留高频细节和冗余令牌方面的不足，提升了高光谱图像融合的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的方法在高光谱图像融合中存在高频细节丢失和冗余令牌的问题，未能充分利用光谱和空间先验信息。
...
Result: 实验表明，THAT在标准基准测试中实现了最先进的性能，显著提升了重建质量和效率。

Conclusion: THAT通过改进高频特征表示和令牌选择，有效解决了Transformer在高光谱图像融合中的局限性，为未来研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习方法参数过多、不适用于实时检查系统的问题，同时处理缺陷外观多变、成像条件恶劣和类别不平衡的挑战。

Method: 引入KARMA框架，包含TiKAN模块、优化的特征金字塔结构和静态-动态原型机制，减少参数并提升性能。

Result: 在基准数据集上表现优异，参数减少97%（0.959M vs. 31.04M），推理速度适合实时部署。

Conclusion: KARMA为基础设施检查提供了一种高效、实用的自动化解决方案，兼具准确性和实时性。
...
Result: 总结了视觉RL的关键趋势（如课程驱动训练、偏好对齐扩散）和开放挑战（如样本效率、泛化能力）。
...
TL;DR: 提出了一种名为Spatial-ORMLLM的新型大视觉语言模型，仅使用RGB模态进行3D空间推理，解决了手术室中多模态数据难以获取的问题。


<details>
  <summary>Details</summary>
Motivation: 手术室中精确的空间建模对临床任务至关重要，但现有方法依赖多模态数据且难以获取3D信息，导致细节捕捉不足。

Method: 通过Spatial-Enhanced Feature Fusion Block将2D输入与3D空间知识结合，采用端到端框架实现3D场景推理。

Result: 在多个临床数据集上表现优异，泛化能力强。

Conclusion: Spatial-ORMLLM为手术室3D空间建模提供了高效解决方案，无需额外传感器或标注。
...
TL;DR: SAGOnline是一个轻量级、零样本的实时3D分割框架，通过解耦策略和GPU加速算法，解决了现有方法的高计算成本和空间推理限制，实现了高效的多目标跟踪和分割。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯点云分割方法存在高计算成本、空间推理能力有限以及无法同时跟踪多目标的问题，需要一种更高效的解决方案。

Method: SAGOnline结合视频基础模型（如SAM2）进行视图一致的2D掩码传播，并使用GPU加速的3D掩码生成和高斯级实例标注算法。

Result: 在NVOS和Spin-NeRF基准测试中表现优异（92.7%和95.2% mIoU），推理速度比现有方法快15-1500倍（27毫秒/帧）。

Conclusion: SAGOnline为实时3D场景理解和AR/VR应用提供了高效解决方案，通过2D模型在3D领域的有效适配，实现了多目标分割和跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖通用人类偏好或静态用户画像，忽视了个体差异和动态多面的个人品味。

Method: 使用多模态大语言模型，引入对比偏好损失和偏好标记，从历史交互中学习个性化偏好。

Result: 模型在偏好预测准确性上优于其他方法，能有效识别相似审美倾向的用户，并为生成符合个人品味的图像提供更精准指导。

Conclusion: 该方法通过动态捕捉用户偏好，显著提升了预测精度和个性化推荐效果。
...
Method: StableAvatar通过时间步感知音频适配器和音频原生引导机制解决音频建模问题，并采用动态加权滑动窗口策略增强视频平滑性。
...
<details>
  <summary>Details</summary>
Motivation: 推动具身AI发展，解决3D场景中基于语言描述的目标分割问题。

Method: 提出ReferSplat框架，显式建模3D高斯点与自然语言表达的空间关系。

Result: 在R3DGS任务和3D开放词汇分割基准上达到最优性能。
...
TL;DR: 论文提出了一种基于物理约束的神经网络方法，从单张图像生成4D场景（包含3D几何和运动），解决了现有方法中运动预测违背物理原理的问题。


<details>
  <summary>Details</summary>
Motivation: 人类能够从单张静态图像想象4D场景，但现有神经网络方法生成的动画常违背物理原理，导致不真实。本文旨在通过物理约束改进这一能力。

Method: 使用物理约束的神经网络预测表面点运动，结合Navier-Stokes方程等物理原理设计损失函数，并通过3D高斯特征捕捉外观并渲染动画。

Result: 实验表明，该方法能生成物理一致的动画，性能显著优于现有方法。

Conclusion: 提出的方法成功实现了从单张图像生成物理合理的4D场景，为未来研究提供了新思路。
...
### [221] [BiXSE: Improving Dense Retrieval via Probabilistic Graded Relevance Distillation](https://arxiv.org/abs/2508.06781)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的相关性通常是连续的，而传统方法仅依赖二元标签。LLM的进步使得生成细粒度标签成为可能。

Method: 提出BiXSE方法，将LLM生成的分级相关性分数解释为概率目标，利用批次内负样本进行训练。

Result: 在多个基准测试中，BiXSE优于基于softmax的对比学习方法，并与强排序基线相当或更优。

Conclusion: BiXSE为密集检索模型提供了一种高效、可扩展的训练方案，适用于日益普及的分级相关性监督。
...
### [222] [CLAP: Coreference-Linked Augmentation for Passage Retrieval](https://arxiv.org/abs/2508.06941)
*Huanwei Xu,Lin Xu,Liang Yuan*

Main category: cs.IR

TL;DR: CLAP是一种基于LLM的轻量级段落扩展框架，通过解决指代连续性和生成局部伪查询，显著提升了密集检索器的性能，尤其在跨域场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有LLM扩展方法在密集检索器中表现不佳，主要由于语义漂移和与预训练语义空间的不对齐，且段落中仅部分内容与查询相关，其余部分引入噪声。

Method: CLAP将段落分割为连贯块，解决指代链，并生成与密集检索器表示对齐的局部伪查询，结合全局和细粒度信号实现鲁棒性能。

Result: CLAP显著提升了密集检索器的性能，在跨域场景中表现尤为突出，nDCG@10绝对提升达20.68%。

Conclusion: CLAP通过逻辑中心化流程实现了鲁棒且领域无关的泛化能力，为密集检索器提供了高效扩展方案。
...
### [223] [Blending Sequential Embeddings, Graphs, and Engineered Features: 4th Place Solution in RecSys Challenge 2025](https://arxiv.org/abs/2508.06970)
*Sergei Makeev,Alexandr Andreev,Vladimir Baikalov,Vladislav Tytskiy,Aleksei Krasilnikov,Kirill Khrylchenko*

Main category: cs.IR

TL;DR: 本文介绍了团队ambitious在RecSys Challenge 2025中的第四名解决方案，专注于通用行为建模，通过整合多种技术生成适用于六种下游任务的用户嵌入。


<details>
  <summary>Details</summary>
Motivation: 挑战的目标是生成适用于多种下游任务的用户嵌入，团队希望通过综合多种技术提升模型的通用性和性能。

Method: 解决方案包括：(1) 顺序编码器捕捉用户兴趣的时间演化，(2) 图神经网络增强泛化能力，(3) 深度交叉网络建模高阶特征交互，(4) 关键特征工程。

Result: 该方案在挑战中获得了第四名的成绩。

Conclusion: 通过整合多种技术，团队成功生成了适用于多样化任务的用户嵌入，验证了方法的有效性。
...
### [224] [ReasonRank: Empowering Passage Ranking with Strong Reasoning Ability](https://arxiv.org/abs/2508.07050)
*Wenhan Liu,Xinyu Ma,Weiwei Sun,Yutao Zhu,Yuchen Li,Dawei Yin,Zhicheng Dou*

Main category: cs.IR

TL;DR: 论文提出了一种自动化生成推理密集型训练数据的框架，并设计了两阶段后训练方法，显著提升了列表排序性能。
...
Method: 提出自动化数据合成框架和两阶段训练方法（SFT和RL），并设计多视图排序奖励。

Result: ReasonRank显著优于基线，并在BRIGHT榜单上达到SOTA性能40.6。

Conclusion: 该方法有效提升了推理密集型排序能力，且延迟更低。
...
### [225] [Uncertainty-Aware Semantic Decoding for LLM-Based Sequential Recommendation](https://arxiv.org/abs/2508.07210)
*Chenke Yin,Li Fan,Jia Wang,Dongxiao Hu,Haichao Zhang,Chong Zhang,Yang Xiang*

Main category: cs.IR

TL;DR: 论文提出了一种不确定性感知语义解码（USD）框架，通过结合基于logit的聚类和自适应评分，改进序列推荐中的下一项预测。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在序列推荐任务中广泛应用，但其解码策略仍基于自然语言处理目标，与推荐任务目标不匹配。

Method: 提出USD框架，将具有相似logit向量的项目聚类为语义等价组，重新分配概率质量，并通过熵控制评分和采样温度。

Result: 在Amazon Product数据集上，HR@3提升18.5%，NDCG@3提升11.9%，MRR@3提升10.8%。

Conclusion: 结合语义聚类和不确定性评估，能提供更可靠和准确的推荐。
...
### [226] [Selection and Exploitation of High-Quality Knowledge from Large Language Models for Recommendation](https://arxiv.org/abs/2508.07223)
*Guanchen Wang,Mingming Ha,Tianbao Ma,Linxun Chen,Zhaojie Liu,Guorui Zhou,Kun Gai*

Main category: cs.IR

TL;DR: 论文提出KSER框架，通过知识过滤和嵌入对齐模块，从LLMs中提取高质量知识以提升推荐系统性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLMs的世界知识和推理能力增强推荐系统，但直接使用其生成内容会导致性能下降，需解决知识幻觉、冗余和同质化问题。

Method: KSER框架包含知识过滤模块（ESFNet）和嵌入对齐模块，采用两种训练策略（全参数训练和仅提取器训练）。

Result: 实验验证了知识过滤和对齐模块的必要性，以及仅提取器训练策略的高效性。

Conclusion: KSER框架能有效选择和利用LLMs的高质量知识，提升推荐系统性能。
...
### [227] [SocRipple: A Two-Stage Framework for Cold-Start Video Recommendations](https://arxiv.org/abs/2508.07241)
*Amit Jaspal,Kapil Dalwani,Ajantha Ramineni*

Main category: cs.IR

TL;DR: SocRipple是一个针对社交图谱平台中冷启动项目分发的两阶段检索框架，通过社交连接和KNN搜索提升分发效果。


<details>
  <summary>Details</summary>
Motivation: 解决冷启动项目中新物品因缺乏交互历史而难以个性化分发的问题。

Method: 两阶段框架：第一阶段利用创作者社交连接进行初始曝光，第二阶段基于早期交互信号和用户嵌入进行KNN搜索。

Result: 在大型视频平台上，SocRipple将冷启动项目分发提升了36%，同时保持用户参与率。
...
### [228] [PrLM: Learning Explicit Reasoning for Personalized RAG via Contrastive Reward Optimization](https://arxiv.org/abs/2508.07342)
*Kepu Zhang,Teng Shi,Weijie Yu,Jun Xu*

Main category: cs.IR

TL;DR: PrLM是一个强化学习框架，通过显式推理用户档案提升个性化检索增强生成（RAG）的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大型语言模型（LLMs）隐式整合检索内容，易受检索质量影响且可能偏离用户偏好。

Method: PrLM利用对比训练的个人化奖励模型，指导LLMs显式推理用户档案，无需标注推理路径。

Result: 在三个个性化文本生成数据集上，PrLM表现优于现有方法，且对不同检索器和档案数量具有鲁棒性。

Conclusion: PrLM通过强化学习显式推理用户档案，有效提升了个性化RAG的性能和鲁棒性。
...
### [229] [Are Multimodal Embeddings Truly Beneficial for Recommendation? A Deep Dive into Whole vs. Individual Modalities](https://arxiv.org/abs/2508.07399)
*Yu Ye,Junchen Fu,Yu Song,Kaiwen Zheng,Joemon M. Jose*

Main category: cs.IR

TL;DR: 论文通过大规模实证研究验证了多模态嵌入对推荐系统的实际效果，发现文本模态单独使用效果接近多模态，而图像模态单独使用效果较差。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态推荐（MMRec）已成为主流范式，但其多模态嵌入是否真正提升推荐性能缺乏实证验证。

Method: 采用模态剔除策略，通过将文本或视觉嵌入设为常量或随机噪声，评估14种先进MMRec模型。

Result: 多模态嵌入整体提升推荐性能，尤其是通过复杂图融合模型；文本模态单独使用效果接近多模态，图像模态单独使用效果较差。

Conclusion: 研究为MMRec社区提供了基础性见解和实践指导，并计划公开代码和数据集。
...
### [230] [Orthogonal Low Rank Embedding Stabilization](https://arxiv.org/abs/2508.07574)
...
<details>
  <summary>Details</summary>
Motivation: 解决模型重训练周期中嵌入空间不稳定性对下游应用的影响。

Method: 结合低秩奇异值分解和正交Procrustes变换，将嵌入映射到标准化空间。

Result: 计算高效、无损且轻量，保持点积和推理质量，减少操作负担。

Conclusion: 方法不改变训练目标或嵌入结构，可与其他稳定技术无缝集成。
...
### [231] [Towards Comprehensible Recommendation with Large Language Model Fine-tuning](https://arxiv.org/abs/2508.07595)
*Yunze Luo,Yinjie Jiang,Gaode Chen,Xinghua Zhang,Jun Zhang,Jian Liang,Kaigui Bian*

Main category: cs.IR

TL;DR: 论文提出了一种名为CURec的新框架，通过结合LLM和传统推荐方法，生成协作对齐的内容特征以提升推荐系统的语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 传统推荐方法在捕捉用户偏好语义方面存在不足，LLM方法则面临如何确保其推理能力与推荐目标对齐的挑战。

Method: CURec框架通过预训练对齐LLM与推荐目标，设计奖励模型评估推荐理由质量，并通过RL微调LLM生成准确理由，最终整合到下游推荐模型中。

Result: 在公开基准测试中，CURec表现优于现有方法。

Conclusion: CURec通过协作对齐的内容特征生成，显著提升了推荐系统的语义理解能力和推荐性能。
...
### [232] [UMRE: A Unified Monotonic Transformation for Ranking Ensemble in Recommender Systems](https://arxiv.org/abs/2508.07613)
*Zhengrui Xu,Zhe Yang,Zhengxiao Guo,Shukai Liu,Luocheng Lin,Xiaoyan Liu,Yongqi Liu,Han Li*

Main category: cs.IR

TL;DR: 论文提出了一种名为UMRE的新框架，用于改进工业推荐系统中的集成排序方法，通过无约束单调神经网络和轻量级排序模型实现自动化和个性化。


<details>
  <summary>Details</summary>
Motivation: 传统集成排序方法依赖手工设计的非线性变换和手动调整的融合权重，效率低下且难以达到帕累托最优。

Method: UMRE使用无约束单调神经网络（UMNN）替代手工变换，并通过轻量级排序模型融合预测分数，同时引入帕累托最优策略动态调整任务权重。

Result: 在两个公开推荐数据集（Kuairand和Tenrec）及在线A/B测试中，UMRE表现出优异的性能和泛化能力。

Conclusion: UMRE无需手动调整，保持排序一致性，并实现细粒度个性化，显著优于传统方法。
...
### [233] [Encode Me If You Can: Learning Universal User Representations via Event Sequence Autoencoding](https://arxiv.org/abs/2508.07748)
*Anton Klenitskiy,Artem Fatkulin,Daria Denisova,Anton Pembek,Alexey Vasilev*

Main category: cs.IR

TL;DR: 论文提出了一种基于GRU自编码器的通用用户行为表示方法，通过重构用户历史行为序列生成固定大小的潜在向量，并结合多种嵌入方法提升泛化能力，在RecSys Challenge 2025中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 构建任务无关的通用用户表示，减少任务特定特征工程和模型重新训练的需求，提升机器学习管道的可扩展性和效率。

Method: 将用户历史行为转化为单一时间序列，使用GRU自编码器重构序列并生成固定大小的潜在向量；结合多种嵌入方法生成统一表示。

Result: 提出的方法在RecSys Challenge 2025中表现优异，团队获得第二名。

Conclusion: 通过自编码器和嵌入方法结合的策略，能够有效捕捉用户行为模式并提升下游任务的泛化能力。
...
### [234] [Recommendation Is a Dish Better Served Warm](https://arxiv.org/abs/2508.07856)
*Danil Gusak,Nikita Sukhorukov,Evgeny Frolov*

Main category: cs.IR

TL;DR: 论文研究了推荐系统中冷启动阈值的设定问题，发现不一致的阈值选择会影响数据质量和结果可比性。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统通常基于最小交互阈值过滤冷用户和物品，但这些阈值选择随意且不一致，影响了评估结果的可靠性和可比性。

Method: 通过实验逐步调整训练中物品的交互数量和推理中用户交互历史的长度，研究不同数据集和基线推荐系统中的冷启动阈值。

Result: 研究发现不一致的冷启动阈值选择会导致有价值数据被不必要地移除，或将冷实例误分类为温实例，增加系统噪声。

Conclusion: 冷启动阈值的设定需要更系统化，以提高推荐系统的评估一致性和可靠性。
...
### [235] [Careful Queries, Credible Results: Teaching RAG Models Advanced Web Search Tools with Reinforcement Learning](https://arxiv.org/abs/2508.07956)
...
TL;DR: WebFilter是一个新的RAG框架，通过生成受限查询和过滤不可靠内容，解决了网络环境中的错误信息和工具利用不足问题，显著提升了检索精度和答案质量。


<details>
  <summary>Details</summary>
Motivation: 网络环境中普遍存在错误信息，且工具利用不足，导致检索精度下降，影响了RAG系统的性能。

Method: 提出WebFilter框架，结合检索过滤机制和行为驱动的奖励策略，优化查询生成和检索结果。

Result: 实验表明，WebFilter在领域内和领域外基准测试中均优于现有RAG方法，提升了答案质量和检索精度。

Conclusion: WebFilter通过过滤不可靠内容和优化查询，有效解决了RAG系统在网络环境中的挑战。
...
### [236] [Improving Document Retrieval Coherence for Semantically Equivalent Queries](https://arxiv.org/abs/2508.07975)
*Stefano Campese,Alessandro Moschitti,Ivano Lauriola*

Main category: cs.IR

TL;DR: 提出一种改进的多负排名损失方法，用于训练密集检索模型，以提高模型在语义相似查询下检索文档的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有密集检索模型对查询和文档词汇敏感，微小变化可能导致检索结果显著差异。

Method: 提出一种多负排名损失的变体，惩罚语义等价查询在检索结果中的不一致性。

Result: 实验表明，优化后的模型对词汇变化敏感性降低，且准确率提高。

Conclusion: 改进的损失函数能提升密集检索模型的鲁棒性和准确性。
...
### [237] [DIVER: A Multi-Stage Approach for Reasoning-intensive Information Retrieval](https://arxiv.org/abs/2508.07995)
*Meixiu Long,Duolin Sun,Dan Yang,Junjie Wang,Yue Shen,Jian Wang,Peng Wei,Jinjie Gu,Jiahai Wang*

Main category: cs.IR

TL;DR: DIVER是一个针对推理密集型信息检索的检索管道，通过改进输入质量、查询扩展、增强检索器和重排序，显著提升了复杂查询的性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法在处理需要抽象推理、类比思维或多步推理的查询时表现不佳，DIVER旨在解决这一问题。

Method: DIVER包含四个组件：文档处理、LLM驱动的查询扩展、推理增强的检索器和结合LLM评分的重排序器。

Result: 在BRIGHT基准测试中，DIVER取得了41.6和28.9的nDCG@10分数，优于现有模型。
...
### [238] [Multi-modal Adaptive Mixture of Experts for Cold-start Recommendation](https://arxiv.org/abs/2508.08042)
*Van-Khang Nguyen,Duc-Hoang Pham,Huy-Son Nguyen,Cam-Van Thi Nguyen,Hoang-Quynh Le,Duc-Trong Le*

Main category: cs.IR

TL;DR: 论文提出了一种名为MAMEX的新型混合专家（MoE）框架，用于解决冷启动推荐系统中多模态数据的动态整合问题。
...
Method: MAMEX采用模态特定专家网络和可学习的门控机制，动态加权各模态贡献，适应不同模态的信息量和缺失情况。

Result: 在基准数据集上的实验表明，MAMEX在冷启动场景中优于现有方法，具有更高的准确性和适应性。

Conclusion: MAMEX通过动态整合多模态数据，显著提升了冷启动推荐的效果，代码已开源。
...
### [239] [HierSearch: A Hierarchical Enterprise Deep Search Framework Integrating Local and Web Searches](https://arxiv.org/abs/2508.08088)
*Jiejun Tan,Zhicheng Dou,Yan Yu,Jiehan Cheng,Qiang Ju,Jian Xie,Ji-Rong Wen*

Main category: cs.IR

TL;DR: HierSearch框架通过分层强化学习解决了多源深度搜索问题，提升了数据效率和工具掌握能力。


<details>
  <summary>Details</summary>
Motivation: 企业需要能够同时利用本地和网络资源的私有深度搜索系统，现有方法局限于单一知识源且效率低。

Method: 提出分层强化学习框架HierSearch，包含本地和网络搜索代理，以及协调它们的规划代理和知识精炼器。

Result: 在六个基准测试中优于现有方法，涵盖通用、金融和医疗领域。

Conclusion: HierSearch有效解决了多源深度搜索的挑战，性能优于传统方法。
...
### [240] [Solving Pasur Using GPU-Accelerated Counterfactual Regret Minimization](https://arxiv.org/abs/2508.06559)
*Sina Baghal*
...
<details>
  <summary>Details</summary>
Motivation: Pasur游戏规则复杂且游戏树庞大，需要高效的计算框架来解决这些问题。

Method: 使用PyTorch CUDA张量处理规则复杂性，将游戏树分解为实际状态和继承分数，采用轮次反向训练策略。

Result: 构建了包含超过10^9个节点的完整游戏树，并训练了基于树的模型预测策略。

Conclusion: 该框架可扩展至其他多轮次强化学习场景，如回合制策略游戏或金融市场决策。
...
### [241] [Operationalizing Serendipity: Multi-Agent AI Workflows for Enhanced Materials Characterization with Theory-in-the-Loop](https://arxiv.org/abs/2508.06569)
...
<details>
  <summary>Details</summary>
Motivation: 现代自主实验室虽然能高效验证假设，但可能忽略意外发现。SciLink旨在填补这一空白，通过AI框架系统性分析观察结果，促进科学探索中的意外发现。

Method: 采用混合AI策略，结合机器学习模型进行实验数据的定量分析，以及大语言模型处理高级推理。框架能自主将原始数据转化为可验证的科学主张，并根据文献评估其新颖性。

Result: SciLink展示了在多种研究场景中的适用性，包括原子分辨率和超光谱数据的分析，整合实时专家指导，并提出针对性后续实验。

Conclusion: SciLink不仅提高了研究效率，还通过系统性分析和情境化观察，为AI驱动的材料研究提供了一个实用框架，促进了意外发现与开放科学探索的结合。
...
### [242] [IRL-VLA: Training an Vision-Language-Action Policy via Reward World Model](https://arxiv.org/abs/2508.06571)
...
TL;DR: 论文提出IRL-VLA框架，通过逆强化学习构建轻量级奖励世界模型，结合VLA架构和PPO强化学习，解决自动驾驶中闭环训练的挑战，并在NAVSIM v2和CVPR2025竞赛中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在自动驾驶中存在模仿学习的局限性（开环设置）和闭环训练对高保真模拟的依赖问题，需改进性能。

Method: 三阶段方法：1) 预训练VLA策略；2) 通过逆强化学习构建轻量级奖励世界模型；3) 使用PPO优化规划性能。

Result: 在NAVSIM v2端到端驾驶基准和CVPR2025竞赛中表现优异。
...
### [243] [CountQA: How Well Do MLLMs Count in the Wild?](https://arxiv.org/abs/2508.06585)
*Jayant Sravan Tamarapalli,Rynaa Grover,Nilay Pande,Sahiti Yerramilli*

Main category: cs.AI

TL;DR: 论文介绍了CountQA，一个针对多模态大语言模型（MLLMs）在物体计数能力上的新基准测试，揭示了现有模型的不足。


<details>
  <summary>Details</summary>
Motivation: MLLMs在视觉场景理解上表现出色，但在物体计数这一基础认知技能上存在严重缺陷，限制了其实际应用的可靠性。

Method: 提出CountQA基准测试，包含1,500多个问答对，涵盖高密度、杂乱和遮挡的真实世界图像，评估了15种主流MLLMs的性能。

Result: 表现最佳的模型准确率仅为42.9%，且随着物体数量增加性能下降。

Conclusion: CountQA为诊断和解决MLLMs的核心弱点提供了工具，推动新一代模型的开发，使其不仅描述流畅，还能在数值和空间上更准确。
...
### [244] [Formal Concept Analysis: a Structural Framework for Variability Extraction and Analysis](https://arxiv.org/abs/2508.06668)
*Jessie Galasso*
...
<details>
  <summary>Details</summary>
Motivation: FCA作为一种数学框架，能够对对象进行层次聚类并提取共性，但其在变异性任务中的应用潜力尚未被充分挖掘。本文旨在填补这一空白。

Method: 通过筛选FCA框架中与变异性分析相关的关键属性，并探讨如何利用这些属性解释变异性信息。

Result: 明确了FCA中可用于变异性分析的关键属性及其应用方式。

Conclusion: FCA在变异性分析中具有潜力，本文为其应用提供了理论支持和方法指导。
...
### [245] [Zero-Shot Cellular Trajectory Map Matching](https://arxiv.org/abs/2508.06674)
*Weijie Shi,Yue Cui,Hao Chen,Jiaming Li,Mengze Li,Jia Zhu,Jiajie Xu,Xiaofang Zhou*

Main category: cs.AI

TL;DR: 提出了一种基于像素的轨迹校准辅助方法，用于零样本CTMM，通过可转移的地理空间知识校准轨迹，并在道路网络层面指导路径查找。


<details>
  <summary>Details</summary>
Motivation: 当前CTMM方法依赖ID特征和区域特定数据，适应性差，无法应对未探索区域。零样本CTMM需要提取区域自适应特征、序列和位置不确定性以提高准确性。

Method: 结合高斯混合模型与VAE，设计空间-时间感知模块，使用约束路径查找算法重构道路ID序列。

Result: 实验表明，该方法在零样本CTMM中优于现有方法16.8%。

Conclusion: 该方法通过知识共享和误差缓解，显著提升了零样本CTMM的性能。
...
### [246] [Probabilistic Circuits for Knowledge Graph Completion with Reduced Rule Sets](https://arxiv.org/abs/2508.06706)
*Jaikrishna Manojkumar Patil,Nathaniel Lee,Al Mehdi Saadat Chowdhury,YooJung Choi,Paulo Shakarian*

Main category: cs.AI

TL;DR: 论文提出了一种基于规则上下文和概率分布的知识图谱补全方法，显著减少了规则数量，同时保持高性能。
...
### [247] [GLIDR: Graph-Like Inductive Logic Programming with Differentiable Reasoning](https://arxiv.org/abs/2508.06716)
*Blair Johnson,Clayton Kerce,Faramarz Fekri*

Main category: cs.AI

TL;DR: GLIDR是一种可微分的规则学习方法，通过更丰富的语法结构（如分支和循环）改进知识图谱任务中的规则学习和推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有链式规则结构的假设限制了性能和可解释性，需要更灵活的规则学习方法。

Method: GLIDR使用可微分消息传递推理算法，支持分支和循环等复杂规则结构，并允许从模型权重中提取显式逻辑规则。

Result: GLIDR在知识图谱补全任务中显著优于现有规则学习方法，甚至可与嵌入方法竞争，且对训练数据噪声具有高鲁棒性。

Conclusion: GLIDR是一种高效、灵活的规则学习方法，适用于多模态数据，并能与深度神经网络结合进行端到端优化。
...
### [248] [ParBalans: Parallel Multi-Armed Bandits-based Adaptive Large Neighborhood Search](https://arxiv.org/abs/2508.06736)
*Alican Yilmaz,Junyang Cai,Serdar Kadioglu,Bistra Dilkina*

Main category: cs.AI

TL;DR: 论文研究了Balans的并行化能力，提出ParBalans以提升MIP问题的求解效率，实验表明其性能优于Gurobi。


<details>
  <summary>Details</summary>
Motivation: 由于MIP问题的组合性质，需要大量计算资源，并行化是加速求解的关键策略。

Method: 扩展Balans为ParBalans，利用求解器和算法级并行性。

Result: ParBalans在复杂MIP实例上表现优于Gurobi。
...
### [249] [Topology Generation of UAV Covert Communication Networks: A Graph Diffusion Approach with Incentive Mechanism](https://arxiv.org/abs/2508.06746)
...
Conclusion: 提出的框架能有效解决无人机网络的动态移动性和隐蔽通信问题。
...
### [250] [Pushing the Envelope of LLM Inference on AI-PC](https://arxiv.org/abs/2508.06753)
...
<details>
  <summary>Details</summary>
Motivation: 探索超低比特LLM模型在资源受限环境（如边缘设备和AI PC）中的高效部署，弥补现有推理运行时（如bitnet.cpp）在计算效率上的不足。

Method: 设计并实现针对现代CPU优化的1位和2位微内核，集成到PyTorch-TPP框架中，进行端到端推理测试。

Result: 2位模型推理性能比bitnet.cpp提升2.2倍，比16位模型快7倍。
...
### [251] [A Fuzzy Logic Prompting Framework for Large Language Models in Adaptive and Uncertain Tasks](https://arxiv.org/abs/2508.06754)
*Vanessa Figueiredo*

Main category: cs.AI

TL;DR: 提出了一种模块化提示框架，支持在大语言模型（LLMs）中更安全、自适应地应用于动态用户任务。
...
Conclusion: 该框架不仅适用于教育领域，还可推广至其他交互密集型场景，为不确定或动态环境中的LLM行为提供可重用方法。
...
### [252] [Natural Language-Driven Viewpoint Navigation for Volume Exploration via Semantic Block Representation](https://arxiv.org/abs/2508.06823)
*Xuan Zhao,Jun Tao*

Main category: cs.AI

TL;DR: 提出一种基于自然语言交互的框架，通过CLIP Score和强化学习优化体数据导航的视点选择。
...
### [253] [Remote Sensing Image Intelligent Interpretation with the Language-Centered Perspective: Principles, Methods and Challenges](https://arxiv.org/abs/2508.06832)
*Haifeng Li,Wang Guo,Haiyang Wu,Mengwei Wu,Jipeng Zhang,Qing Zhu,Yu Liu,Xin Huang,Chao Tao*

Main category: cs.AI

TL;DR: 论文主张从视觉为中心的遥感图像解释转向语言为中心的范式，提出基于全局工作空间理论（GWT）的语言框架，利用大语言模型（LLMs）作为认知中心，整合感知、任务、知识和行动空间，实现统一理解、推理和决策。
...
Method: 提出语言为中心的框架，将LLMs作为认知中心，整合多模态表示、知识关联及推理决策，构建全局工作空间驱动的解释机制。

Result: 总结了语言为中心解决方案如何应对核心挑战，并提出了未来研究方向。
...
### [254] [Multi-level Advantage Credit Assignment for Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2508.06836)
*Xutong Zhao,Yaqi Xie*

Main category: cs.AI

TL;DR: 论文提出了一种多级优势信用分配方法（MACA），用于解决多智能体强化学习中的信用分配问题，通过显式反事实推理和注意力机制捕捉不同层次的智能体贡献。
...
Method: MACA通过多级优势公式进行显式反事实推理，结合注意力框架识别智能体间的相关性，并构建多级优势函数（个体、联合和相关动作）指导策略学习。

Result: 在Starcraft v1&v2任务上的实验表明，MACA在复杂信用分配场景中表现优异。

Conclusion: MACA通过多级优势信用分配方法有效解决了MARL中的信用分配问题，适用于多种协调层次的任务。
...
### [255] [MDK12-Bench: A Comprehensive Evaluation of Multimodal Large Language Models on Multidisciplinary Exams](https://arxiv.org/abs/2508.06851)
...
TL;DR: MDK12-Bench是一个多学科大规模基准测试，用于评估多模态大语言模型（MLLMs）的智能表现，覆盖六个学科的141K实例和6,225个知识点。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs的评测基准存在规模小、覆盖窄、知识结构单一的问题，无法全面评估模型能力。

Method: 提出MDK12-Bench基准和动态评估框架，引入视觉、文本和问题形式的变化，并采用知识参考增强生成（KP-RAG）方法。

Result: 发现当前MLLMs在多个维度上存在局限性，为提升模型鲁棒性和可解释性提供了指导。

Conclusion: MDK12-Bench为MLLMs的全面评估和改进提供了重要工具，尤其在AI辅助教育领域具有潜力。
...
### [256] [MeteorPred: A Meteorological Multimodal Large Model and Dataset for Severe Weather Event Prediction](https://arxiv.org/abs/2508.06859)
*Shuo Tang,Jian Xu,Jiadong Zhang,Yi Chen,Qizhao Jin,Lingdong Shen,Chenglin Liu,Shiming Xiang*

Main category: cs.AI

TL;DR: 论文提出了一种基于AI的端到端天气预警系统，通过构建大规模多模态数据集MP-Bench和开发气象多模态大模型MMLM，解决了现有系统依赖人工、数据对齐不足等问题。


<details>
  <summary>Details</summary>
Motivation: 当前天气预警系统依赖专家手动解释，存在主观性和操作负担。AI技术的发展为自动化天气预测提供了新机会，但面临数据稀缺、高维数据对齐困难等挑战。

Method: 构建MP-Bench数据集（421,363对气象数据与文本），开发MMLM模型，支持4D气象数据输入，并设计自适应融合模块处理时间、垂直压力和空间维度特征。

Result: MMLM在MP-Bench上表现优异，验证了其在天气理解和自动化预测中的有效性。

Conclusion: MMLM为AI驱动的自动化天气预警系统迈出了关键一步，代码和数据集将公开。
...
### [257] [Designing a Feedback-Driven Decision Support System for Dynamic Student Intervention](https://arxiv.org/abs/2508.07107)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型多为静态，无法适应新数据（如干预后结果），限制了预测的时效性和准确性。

Method: 系统结合LightGBM回归器和增量训练，支持实时更新模型；通过Flask接口和SHAP提升交互性和可解释性。

Result: 实验显示，模型重训练后RMSE降低10.7%，干预学生预测分数持续提升。

Conclusion: 该框架将静态预测转化为自优化系统，推动教育分析向以人为本、数据驱动和响应式AI发展。
...
TL;DR: 本文介绍了推下奖励机（pdRMs），它是基于确定性下推自动机的奖励机扩展，能够识别和奖励确定性上下文无关语言中的行为。


<details>
  <summary>Details</summary>
Motivation: 奖励机（RMs）在强化学习中表现出色，但其表达能力有限，仅能处理正则语言。为了扩展其表达能力，作者提出了pdRMs，以支持更复杂的确定性上下文无关语言。

Method: 提出了两种基于pdRM的策略：一种可以访问整个堆栈，另一种仅能访问堆栈顶部的k个符号。并提供了检查两种策略是否达到相同最优预期奖励的方法。

Result: 理论分析表明pdRMs具有更强的表达能力，实验结果显示代理可以通过pdRMs训练完成确定性上下文无关语言的任务。

Conclusion: pdRMs扩展了奖励机的表达能力，适用于更复杂的任务，并通过理论和实验验证了其有效性。
...
### [259] [Democratizing Diplomacy: A Harness for Evaluating Any Large Language Model on Full-Press Diplomacy](https://arxiv.org/abs/2508.07485)
*Alexander Duffy,Samuel J Paech,Ishana Shastri,Elizabeth Karpinski,Baptiste Alloui-Cros,Tyler Marques,Matthew Lyle Olson*

Main category: cs.AI

TL;DR: 提出了一种无需微调或专门训练的评估工具，使本地大型语言模型（LLMs）能够玩完整的《外交》游戏。


<details>
  <summary>Details</summary>
Motivation: 解决《外交》游戏因高复杂性和信息密度而难以研究的问题，降低研究门槛。

Method: 通过数据驱动的迭代优化文本游戏状态表示，开发工具支持假设测试和统计分析。

Result: 较大模型表现最佳，但较小模型也能胜任；引入关键状态分析协议。

Conclusion: 该工具无需微调即可评估LLMs的战略推理能力，揭示了这些能力的自然涌现。
...
### [260] [GDBA Revisited: Unleashing the Power of Guided Local Search for Distributed Constraint Optimization](https://arxiv.org/abs/2508.06899)
*Yanchen Deng,Xinrun Wang,Bo An*

Main category: cs.AI

TL;DR: 论文提出了一种新的分布式引导局部搜索（DGLS）方法，解决了GDBA在分布式约束优化问题（DCOPs）中的性能不足问题。


<details>
  <summary>Details</summary>
Motivation: GDBA在解决DCOPs时容易陷入局部最优，且性能提升有限。论文旨在通过改进约束违反条件、惩罚机制和更新协调性来提升性能。

Method: 提出DGLS框架，包括自适应违反条件、惩罚蒸发机制和同步更新方案。

Result: DGLS在标准基准测试中显著优于现有方法，特别是在结构化问题上表现突出。

Conclusion: DGLS是一种高效的DCOPs解决方法，具有理论保证和实际优势。
...
### [261] [Automated Formalization via Conceptual Retrieval-Augmented LLMs](https://arxiv.org/abs/2508.06931)
...
<details>
  <summary>Details</summary>
Motivation: 交互式定理证明器需要手动形式化，耗时且依赖专家知识。自动形式化面临模型幻觉和语义鸿沟的挑战。

Method: 提出CRAMF框架，基于检索增强生成（RAG），从Mathlib4构建知识库，引入上下文查询增强和双通道混合检索策略。

Result: 在多个基准测试中，CRAMF显著提升了翻译准确性，最高相对提升62.1%，平均29.9%。

Conclusion: CRAMF为自动形式化提供了有效解决方案，尤其在处理数学概念时表现出色。
...
### [262] [EMPATHIA: Multi-Faceted Human-AI Collaboration for Refugee Integration](https://arxiv.org/abs/2508.07671)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI方法在难民整合中过于关注就业等狭窄目标，忽视了文化、情感和伦理等关键维度。

Method: EMPATHIA基于Kegan的理论，分为SEED、RISE和THRIVE三个模块，采用多智能体架构进行透明决策。

Result: 在UN Kakuma数据集上，EMPATHIA实现了87.4%的验证收敛，并在五个接收国中提供了可解释的评估。

Conclusion: EMPATHIA通过平衡多种价值体系，为AI驱动的分配任务提供了通用框架，同时支持人机协作。
...
### [263] [Intrinsic Explainability of Multimodal Learning for Crop Yield Prediction](https://arxiv.org/abs/2508.06939)
*Hiba Najjar,Deepak Pathak,Marlon Nuske,Andreas Dengel*

Main category: cs.AI

TL;DR: 论文提出基于Transformer的多模态学习方法，用于作物产量预测，并通过自注意力机制解释模型，比较了不同特征和模态归因方法的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态学习在农业中具有潜力，但模型可解释性常被忽视。本研究旨在利用Transformer的固有可解释性，解释多模态网络在作物产量预测任务中的表现。
...
Result: Transformer模型优于卷积和循环网络，R2分数更高；Attention Rollout在时间归因上更可靠；模态归因方法显示不同模式。

Conclusion: Transformer在多模态学习中表现优异，自注意力机制为模型解释提供了有效工具，有助于结合农业知识理解预测结果。
...
### [264] [Large Language Models Do Not Simulate Human Psychology](https://arxiv.org/abs/2508.06950)
*Sarah Schröder,Thekla Morgenroth,Ulrike Kuhl,Valerie Vaquet,Benjamin Paaßen*

Main category: cs.AI

TL;DR: 论文警告不要用大语言模型（LLM）替代人类参与者进行心理学研究，指出LLM无法模拟人类心理，并通过实验证明其不可靠性。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM（如ChatGPT）是否能够模拟人类心理学并替代人类参与者，以解决心理学研究中可能存在的局限性。

Method: 通过概念论证和实证研究，分析LLM对语义细微变化的反应差异，并比较不同LLM在新项目中的表现。

Result: 实验显示LLM对语义变化的反应与人类存在显著差异，且不同LLM之间表现不一致，证明其不可靠。

Conclusion: LLM无法模拟人类心理学，心理学研究者应将其视为需要验证的工具，而非替代人类参与者的可靠方法。
...
### [265] [DatasetResearch: Benchmarking Agent Systems for Demand-Driven Dataset Discovery](https://arxiv.org/abs/2508.06960)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，数据可用性成为AI开发的瓶颈，论文旨在探索AI代理是否能自主发现满足特定需求的数据集。

Method: 通过208个真实需求构建DatasetResearch基准，采用三维评估框架测试AI代理在知识和推理任务中的表现。

Result: 结果显示，即使先进系统在挑战性子集上仅得22分，搜索代理擅长知识任务，合成代理擅长推理任务，但两者在极端案例中表现不佳。
...
### [266] [MASteer: Multi-Agent Adaptive Steer Strategy for End-to-End LLM Trustworthiness Repair](https://arxiv.org/abs/2508.06963)
*Changqing Li,Tianlin Li,Xiaohan Zhang,Aishan Liu,Li Pan*

Main category: cs.AI

TL;DR: MASteer是一个基于表示工程的端到端框架，用于修复大语言模型（LLMs）的可信度问题，通过自动生成样本和自适应策略选择，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有修复方法（如SFT和RLHF）成本高且速度慢，提示工程缺乏鲁棒性和可扩展性，而表示工程虽轻量但依赖人工样本和固定策略，限制了自动化和适应性。

Method: MASteer结合AutoTester（多智能体生成样本）和AutoRepairer（自适应策略选择），实现了自动化、上下文感知的可信度修复。

Result: 在标准及定制任务中，MASteer性能优于基线，LLaMA-3.1-8B-Chat和Qwen-3-8B-Chat分别提升15.36%和4.21%，同时保持模型通用能力。

Conclusion: MASteer展示了强大的鲁棒性、泛化能力和实用价值，为高效、可扩展的可信度修复提供了解决方案。
...
### [267] [DSperse: A Framework for Targeted Verification in Zero-Knowledge Machine Learning](https://arxiv.org/abs/2508.06972)
...
<details>
  <summary>Details</summary>
Motivation: 解决分布式零知识机器学习中全模型电路化的高成本和僵化问题，提供更具针对性的验证策略。

Method: 通过选择性地验证关键子计算（“切片”），结合审计、复制或经济激励确保全局一致性。

Result: 实验评估了多种证明系统，展示了切片与非切片配置下的内存使用、运行时间和电路行为。
...
### [268] [Simulating Biological Intelligence: Active Inference with Experiment-Informed Generative Model](https://arxiv.org/abs/2508.06980)
*Aswin Paul,Moein Khajehnejad,Forough Habibollahi,Brett J. Kagan,Adeel Razi*

Main category: cs.AI

TL;DR: 论文提出了一种基于主动推理的框架，用于模拟具身代理的决策过程，结合生物神经元网络的实验数据，展示了记忆学习和预测规划在智能决策中的作用。
...
Method: 采用主动推理理论构建框架，结合实验数据生成模型，模拟游戏环境中的决策过程。
...
### [269] [Efficient and Reliable Hitting-Set Computations for the Implicit Hitting Set Approach](https://arxiv.org/abs/2508.07015)
*Hannes Ihalainen,Dieter Vandesande,André Schidler,Jeremias Berg,Bart Bogaerts,Matti Järvisalo*

Main category: cs.AI

TL;DR: 本文探讨了隐式命中集（IHS）框架中替代优化技术，比较了伪布尔推理和随机局部搜索的效果，发现商业整数规划求解器效率高但存在数值不稳定性问题，而基于伪布尔推理的精确计算更具竞争力且可提供正确性证明。


<details>
  <summary>Details</summary>
Motivation: 研究隐式命中集（IHS）框架中替代优化技术的可行性，解决商业整数规划求解器在效率和数值稳定性上的问题。

Method: 采用伪布尔（PB）推理和随机局部搜索作为替代优化技术，并在伪布尔优化背景下进行广泛评估。

Result: 商业整数规划求解器效率最高但存在数值不稳定性；基于伪布尔推理的精确计算在效率和可靠性上更具竞争力，并能提供正确性证明。

Conclusion: 伪布尔推理可作为隐式命中集计算的替代方案，在效率和可靠性上表现优异，同时支持正确性证明。
...
### [270] [MultiMedEdit: A Scenario-Aware Benchmark for Evaluating Knowledge Editing in Medical VQA](https://arxiv.org/abs/2508.07022)
*Shengtao Wen,Haodong Chen,Yadong Wang,Zhongying Pan,Xiang Chen,Yu Tian,Bo Qian,Dong Liang,Sheng-Jun Huang*

Main category: cs.AI

TL;DR: MultiMedEdit是首个针对临床多模态任务的知识编辑（KE）基准，揭示了当前方法在复杂临床工作流中的局限性，并为未来开发更稳健的KE技术提供了基础。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑研究多集中于文本领域，而多模态医疗场景下的KE研究不足，需要结合视觉推理以支持安全且可解释的临床决策。

Method: 提出MultiMedEdit基准，涵盖理解和推理任务类型，定义可靠性、通用性和局部性三维指标，支持跨范式比较，并进行单次编辑和终身编辑实验。

Result: 当前方法在泛化和长尾推理方面表现不佳，尤其在复杂临床工作流中。效率分析揭示了实际部署中的权衡问题。
...
### [271] [K-Dense Analyst: Towards Fully Automated Scientific Analysis](https://arxiv.org/abs/2508.07043)
*Orion Li,Vinayak Agarwal,Summer Zhou,Ashwin Gopinath,Timothy Kassis*

Main category: cs.AI

TL;DR: K-Dense Analyst是一个分层多智能体系统，通过双循环架构实现自主生物信息学分析，性能超越现有最佳语言模型。
...
Conclusion: 自主科学推理需要专门构建的系统，而不仅仅是增强的语言模型，K-Dense Analyst为实现完全自主的计算生物学家迈出重要一步。
...
### [272] [Towards Safer AI Moderation: Evaluating LLM Moderators Through a Unified Benchmark Dataset and Advocating a Human-First Approach](https://arxiv.org/abs/2508.07063)
*Naseem Machlovi,Maryam Saleki,Innocent Ababio,Ruhul Amin*

Main category: cs.AI

TL;DR: 论文探讨了LLMs在内容审核中的局限性，提出了一个评估框架和优化模型SafePhi，其在性能上优于现有基准模型。
...
Method: 开发了一个基于SOTA模型的实验框架，评估人类情感和攻击性行为，并引入统一基准数据集和优化模型SafePhi。

Result: SafePhi在Macro F1得分上达到0.89，优于OpenAI Moderator（0.77）和Llama Guard（0.74）。

Conclusion: 研究强调了LLMs在特定领域的不足，需结合更多异构数据和人类参与以提高模型的鲁棒性和可解释性。
...
### [273] [Multi-Dimensional Summarization Agents with Context-Aware Reasoning over Enterprise Tables](https://arxiv.org/abs/2508.07186)
*Amit Dhanda*

Main category: cs.AI

TL;DR: 提出了一种基于大型语言模型（LLM）的多代理框架，用于跨维度总结结构化企业数据，显著优于传统方法。
...
Method: 采用多代理流程，包括数据切片、方差检测、上下文构建和基于LLM的生成。

Result: 框架在数据忠实度（83%）、显著变化覆盖率和决策关键见解相关性（4.4/5）上表现优异。

Conclusion: 该框架在复杂商业场景中（如价格与销量权衡）表现突出，显著提升了总结质量。
...
### [274] [EndoAgent: A Memory-Guided Reflective Agent for Intelligent Endoscopic Vision-to-Decision Reasoning](https://arxiv.org/abs/2508.07292)
*Yi Tang,Kaini Wang,Yang Chen,Guangquan Zhou*

Main category: cs.AI

TL;DR: EndoAgent是一种基于双记忆设计的AI代理，用于内窥镜图像诊断，通过迭代推理和自适应工具选择提升临床决策能力。
...
Method: 提出EndoAgent，采用双记忆设计（短期行动跟踪和长期经验学习），集成专家工具于统一推理循环中。

Result: 在EndoAgentBench基准测试中，EndoAgent表现优于通用和医学多模态模型。

Conclusion: EndoAgent展示了强大的灵活性和推理能力，适用于复杂的内窥镜诊断任务。
...
### [275] [Hallucination as a Computational Boundary: A Hierarchy of Inevitability and the Oracle Escape](https://arxiv.org/abs/2508.07334)
...
Method: 构建“计算必要性层次结构”，证明幻觉的不可避免性，并提出RAG和持续学习作为逃逸路径。

Result: 证明了幻觉的不可避免性，并提供了RAG和持续学习的理论支持。

Conclusion: 通过形式化方法和两种逃逸路径，为大语言模型的可靠部署提供了理论基础。
...
### [276] [Rethinking Domain-Specific LLM Benchmark Construction: A Comprehensiveness-Compactness Approach](https://arxiv.org/abs/2508.07353)
*Rubing Chen,Jiaxin Wu,Jian Wang,Xulu Zhang,Wenqi Fan,Chenghua Lin,Xiao-Yong Wei,Qing Li*

Main category: cs.AI

TL;DR: 论文提出Comp-Comp框架，基于全面性-紧凑性原则构建领域特定基准，挑战了传统依赖规模扩展的方法。


<details>
  <summary>Details</summary>
Motivation: 现有领域特定基准主要依赖规模扩展，但语料和QA设计对模型精确率和召回率的影响未被探索。

Method: 提出Comp-Comp框架，通过全面性确保语义召回，紧凑性提升精确率，指导语料和QA集构建。

Result: 案例研究中构建了XUBench，验证了框架有效性，并展示了其跨领域扩展潜力。

Conclusion: Comp-Comp框架为领域特定基准构建提供了新思路，适用于学术及其他领域。
...
### [277] [Pentest-R1: Towards Autonomous Penetration Testing Reasoning Optimized via Two-Stage Reinforcement Learning](https://arxiv.org/abs/2508.07382)
...
Method: Pentest-R1采用两阶段强化学习：离线RL学习基础攻击逻辑，在线RL在CTF环境中微调，学习自我纠错和自适应策略。

Result: 在AutoPenBench上成功率为24.2%，仅次于Gemini 2.5 Flash；在Cybench上成功率为15.0%，达到开源LLM的新标杆。

Conclusion: 两阶段训练的结合是Pentest-R1成功的关键，证明了其在渗透测试中的高效性。
...
### [278] [Invert4TVG: A Temporal Video Grounding Framework with Inversion Tasks for Enhanced Action Understanding](https://arxiv.org/abs/2508.07388)
*Zhaoyu Chen,Hongnan Lin,Yongwei Nie,Fei Ma,Xuemiao Xu,Fei Yu,Chengjiang Long*

Main category: cs.AI

TL;DR: 论文提出Invert4TVG框架，通过三个反转任务增强视频片段定位和动作理解，优化了语义和定位的平衡，实验显示性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖时间IoU指标，忽略了语义动作理解，影响了TVG的鲁棒性。

Method: 提出三个反转任务（动词补全、动作识别、视频描述），结合强化学习框架优化定位和语义。

Result: 在Charades-STA数据集上，R1@0.7指标提升7.1%，优于现有方法。

Conclusion: 通过反转任务增强语义理解，显著提高了定位精度上限。
...
### [279] [Generative AI for Strategic Plan Development](https://arxiv.org/abs/2508.07405)
*Jesse Ponnock*

Main category: cs.AI

TL;DR: 论文提出了一种利用生成式人工智能（GAI）为大型政府组织制定战略计划的模块化模型，并评估了BERTopic和非负矩阵分解（NMF）在主题建模中的表现。结果显示，BERTopic表现更优。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用GAI和LLMs自动化专业服务，特别是为政府组织制定战略计划，以应对监管需求和公共利益。

Method: 使用BERTopic和NMF模型对政府问责办公室（GAO）的大量报告进行主题建模，生成与战略计划中“愿景要素”相似的主题，并进行相似性评分比较。

Result: 两种技术均能生成与所有被评估要素相似的主题，其中BERTopic表现更优，超过一半的主题达到“中等”或“强”相关性。

Conclusion: GAI在战略计划制定中具有潜力，未来工作将聚焦于模型的操作化及剩余模块的可行性验证。
...
### [280] [A Comprehensive Survey of Self-Evolving AI Agents: A New Paradigm Bridging Foundation Models and Lifelong Agentic Systems](https://arxiv.org/abs/2508.07407)
...
TL;DR: 本文综述了自进化AI代理系统的现有技术，提出了统一的概念框架，并探讨了领域特定策略及伦理考量。
...
Method: 提出包含系统输入、代理系统、环境和优化器的统一框架，系统回顾自进化技术及领域特定策略。

Result: 总结了自进化代理系统的关键技术、领域应用及评估、安全与伦理问题。

Conclusion: 为开发更自适应、自主和终身代理系统奠定基础，促进未来研究与实践。
...
### [281] [Grounding Natural Language for Multi-agent Decision-Making with Multi-agentic LLMs](https://arxiv.org/abs/2508.07466)
*Dom Huh,Prasant Mohapatra*

Main category: cs.AI

TL;DR: 该论文提出了一种将大型语言模型（LLMs）与多智能体决策算法结合的系统框架，以提升协作与推理能力。


<details>
  <summary>Details</summary>
Motivation: 语言是协作与推理的基础，通过建立共同语言可以促进智能体间的清晰沟通与协调。

Method: 提出了一种多智能体LLMs的设计框架，包括高级提示工程、记忆架构、多模态信息处理和微调算法。
...
### [282] [CP-Agent: Agentic Constraint Programming](https://arxiv.org/abs/2508.07468)
*Stefan Szeider*

Main category: cs.AI

TL;DR: 论文提出了一种基于纯代理策略的新方法，通过通用的Python编码代理和精心设计的提示，成功解决了约束编程中的自然语言问题描述转换为形式化模型的挑战。


<details>
  <summary>Details</summary>
Motivation: 将自然语言问题描述自动转换为形式化约束模型是一个长期挑战，现有固定流程方法在大量基准问题上失败。

Method: 采用基于ReAct原则的通用Python编码代理，利用持久IPython内核进行状态化代码执行和迭代开发，通过提示注入领域知识。

Result: 该方法成功解决了CP-Bench约束编程基准集中的所有101个问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏全面数据集或基准来评估LLMs和AI代理使用MCP工具的能力，且MCP工具响应格式多样，增加了评估难度。

Method: 构建MCPToolBench++基准，基于来自MCP市场和GitHub社区的4k多个MCP服务器，涵盖40多个类别，包含单步和多步工具调用。

Result: 评估了具有代理能力的SOTA LLMs，并报告了结果。

Conclusion: MCPToolBench++为解决LLMs调用MCP工具的评估挑战提供了有效工具。
...
Method: 部署友好干扰器干扰攻击者，服务器优化时间槽选择和传输功率，提出优先采样辅助的双延迟深度确定性策略梯度算法。

Result: 仿真结果显示，与传统强化学习方法相比，隐私和语义信息传输质量分别提升77.8%和14.3%。

Conclusion: 该算法有效提升了语义通信的隐私性和传输质量，无需服务器与干扰器之间的通信。
...
TL;DR: HGMF是一种概率剪枝方法，通过分层高斯混合模型提高大规模工具库中工具选择的准确性和效率。
...
Method: HGMF将查询和工具描述映射到统一语义空间，分两阶段聚类和过滤：先对服务器聚类，再对工具聚类，生成高相关性候选集。

Result: 实验表明HGMF显著提高工具选择准确性并降低推理延迟。

Conclusion: HGMF在大规模工具库中具有可扩展性和有效性。
...
TL;DR: 论文提出了ThinkTuning方法，通过教师模型的反馈指导学生模型，提升其推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如RL）仅能激发基础模型已有的推理行为，而无法让不具备此类行为的模型发展出推理能力。

Method: 采用GRPO-based交互训练方法，教师模型通过反馈指导学生模型的推理过程。

Result: 在多个基准测试中，ThinkTuning平均提升3.85%，在MATH-500、AIME和GPQA-Diamond上分别提升2.08%、2.23%和3.99%。

Conclusion: ThinkTuning通过教师模型的隐式监督，有效提升了学生模型的推理能力。
...
TL;DR: 论文提出利用多模态AI技术改进家禽福利监测，通过特征级融合策略实现高效、可扩展的解决方案，并引入新评估工具和部署框架以应对实际挑战。


<details>
  <summary>Details</summary>
Motivation: 传统家禽福利监测方法受限于主观性和单传感器数据，无法全面反映现代农场中蛋鸡的多维度福利需求。

Method: 采用多模态AI整合视觉、声学、环境和生理数据，提出特征级融合策略，并引入域转移评分（DTS）和数据可靠性指数（DRI）作为评估工具。

Result: 特征级融合策略在鲁棒性和性能间取得最佳平衡，新工具和框架提升了模型适应性和数据质量。

Conclusion: 多模态AI为家禽福利监测提供了从被动单模态转向主动精准系统的路径，结合生产效率和科学伦理。
...
TL;DR: SkillNav是一个模块化框架，通过将导航任务分解为可解释的原子技能，并利用VLM路由动态选择最佳技能代理，显著提升了Vision-and-Language Navigation的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前VLN方法在复杂空间和时间推理场景中泛化能力不足，尤其是在未见过的环境中。

Method: 提出SkillNav框架，将导航任务分解为原子技能（如垂直移动、区域识别等），每个技能由专门代理处理，并引入VLM路由动态选择代理。

Result: 在R2R和GSA-R2R基准测试中达到新最优性能，展示了强大的泛化能力。
...
### [289] [Disentangling Multiplex Spatial-Temporal Transition Graph Representation Learning for Socially Enhanced POI Recommendation](https://arxiv.org/abs/2508.07649)
*Jie Li,Haoye Dong,Zhengyang Wu,Zetao Zheng,Mingrong Lin*

Main category: cs.AI

TL;DR: DiMuST是一种基于解缠表示学习的POI推荐模型，通过多路时空转换图和社会关系提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法将时空转换分开建模，导致节点表示不一致，引入冗余信息，降低模型性能和可解释性。

Method: 提出DiMuST模型，使用解缠变分多路图自编码器（DAE）分离共享和私有分布，通过PoE机制融合共享特征，并利用对比约束去噪私有特征。
...
### [290] [1-2-3 Check: Enhancing Contextual Privacy in LLM via Multi-Agent Reasoning](https://arxiv.org/abs/2508.07667)
*Wenkai Li,Liwen Sun,Zhenxiang Guan,Xuhui Zhou,Maarten Sap*

Main category: cs.AI

TL;DR: 论文提出了一种多智能体框架，通过分解隐私推理任务来减少单个智能体的信息负载，从而更可靠地遵守上下文隐私规范。实验表明，该方法显著降低了隐私信息泄露。


<details>
  <summary>Details</summary>
Motivation: 解决在交互式环境中处理多源信息时的上下文隐私问题，尤其是LLMs在混合公私信息场景下的隐私保护挑战。

Method: 引入多智能体框架，将隐私推理分解为专门子任务（提取、分类），并通过迭代验证减少隐私错误。

Result: 在ConfAIde和PrivacyLens基准测试中，最佳多智能体配置显著减少隐私泄露（GPT-4o下分别降低18%和19%），同时保持公共内容的保真度。

Conclusion: 多智能体系统的信息流设计在LLMs的上下文隐私保护中具有潜力，优于单智能体基线。
...
### [291] [Ethics2vec: aligning automatic agents and human preferences](https://arxiv.org/abs/2508.07673)
*Gianluca Bontempi*

Main category: cs.AI

TL;DR: 论文提出Ethics2Vec方法，将伦理价值映射为向量表示，以解决AI与人类价值观对齐的难题。


<details>
  <summary>Details</summary>
Motivation: AI行为中隐含的伦理价值难以量化，导致与人类价值观对齐困难，尤其是在涉及不可比较的伦理考量时。

Method: 扩展Anything2vec方法，将代理决策策略映射为多元向量表示，用于评估与人类价值观的对齐性。

Result: 提出Ethics2Vec方法，并在二元决策和自动驾驶控制场景中验证其可行性。

Conclusion: Ethics2Vec为AI伦理对齐提供了一种可量化的解决方案，适用于复杂决策场景。
...
### [292] [Symmetry-Aware Transformer Training for Automated Planning](https://arxiv.org/abs/2508.07743)
*Markus Fritzsche,Elliot Gestrin,Jendrik Seipp*

Main category: cs.AI

TL;DR: 本文提出了一种新的对比学习目标，使Transformer能够感知对称性，从而弥补其在自动规划领域的不足，并展示了其在计划生成和启发式预测中的高效训练效果。


<details>
  <summary>Details</summary>
Motivation: Transformer在自动规划领域的应用受限，尤其是PlanGPT在处理从简单到复杂规划问题的外推时表现不佳，主要由于问题对称性导致的组合爆炸。

Method: 提出了一种新的对比学习目标，结合架构改进，使Transformer能够感知对称性。

Result: 实验结果表明，对称感知训练有效且高效地解决了PlanGPT的局限性。

Conclusion: 通过对称感知训练，Transformer在自动规划领域表现出色，能够高效处理计划生成和启发式预测任务。
...
### [293] [Best-Effort Policies for Robust Markov Decision Processes](https://arxiv.org/abs/2508.07790)
...
<details>
  <summary>Details</summary>
Motivation: 现有的RMDPs方法虽然能高效计算最优鲁棒策略，但可能存在多个等效策略，这些策略在非对抗性转移概率下表现不同。因此，需要一种更精细的策略选择标准。

Method: 提出ORBE策略，结合博弈论中的支配和最佳努力概念，要求策略不仅在最坏情况下最优，还需在其他转移概率下表现最佳。给出了ORBE策略的存在性证明、结构特征及计算算法。

Result: ORBE策略始终存在，且计算开销仅略高于标准鲁棒值迭代。数值实验验证了方法的可行性。

Conclusion: ORBE策略为RMDPs提供了一种更优的策略选择标准，解决了等效策略的区分问题，具有实际应用潜力。
...
### [294] [KIRETT: Knowledge-Graph-Based Smart Treatment Assistant for Intelligent Rescue Operations](https://arxiv.org/abs/2508.07834)
...
<details>
  <summary>Details</summary>
Motivation: 全球救援需求增加，急救人员需快速评估患者状况并提供优化治疗，但时间压力限制了其决策能力。

Method: 采用知识图谱作为核心知识表示，结合人工智能技术进行情境预识别，生成智能治疗建议。

Result: 系统能够为急救人员提供实时、智能化的治疗推荐，提升急救效率。

Conclusion: 知识图谱与AI结合的方法为急救场景提供了有效的决策支持，有望改善急救效果。
...
### [295] [\(X\)-evolve: Solution space evolution powered by large language models](https://arxiv.org/abs/2508.07932)
...
<details>
  <summary>Details</summary>
Motivation: 当前结合LLM和EA的方法通常演化单个解，导致LLM调用成本高昂，限制了解决复杂优化问题的效率。

Method: X-evolve通过生成可调程序定义解空间，利用基于分数的搜索算法高效探索参数化空间，减少LLM调用。

Result: 在三个优化问题上取得突破：发现更大的部分可容许集（cap set问题）、更大的独立集（信息论问题）以及更优的启发式算法（在线装箱问题）。
...
### [296] [Deep Reinforcement Learning with anticipatory reward in LSTM for Collision Avoidance of Mobile Robots](https://arxiv.org/abs/2508.07941)
...
### [297] [FEAT: A Multi-Agent Forensic AI System with Domain-Adapted Large Language Model for Automated Cause-of-Death Analysis](https://arxiv.org/abs/2508.07950)
...
<details>
  <summary>Details</summary>
Motivation: 解决法医死因鉴定中的劳动力短缺和诊断不一致问题，特别是在中国等高负荷法医系统中。

Method: FEAT采用多智能体架构，包括任务分解的中央规划器、证据分析的专业本地求解器、迭代优化的记忆与反思模块，以及结论合成的全局求解器。结合工具增强推理、分层检索增强生成和法医调优的LLM。

Result: FEAT在多样化的中国案例中表现优于现有AI系统，具有强大的泛化能力和高专家一致性，能够检测细微证据。
...
### [298] [Interpreting Fedspeak with Confidence: A LLM-Based Uncertainty-Aware Framework Guided by Monetary Policy Transmission Paths](https://arxiv.org/abs/2508.08001)
...
### [299] [Fitting Description Logic Ontologies to ABox and Query Examples](https://arxiv.org/abs/2508.08007)
*Maurice Funk,Marvin Grosser,Carsten Lutz*

Main category: cs.AI

TL;DR: 论文研究了基于本体介导查询的拟合问题，通过正负例拟合本体，分析了不同查询语言的复杂性和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过正负例拟合本体，以支持本体介导查询的自动化构建。

Method: 使用描述逻辑ALC和ALCI作为本体语言，分析原子查询（AQs）、联合查询（CQs）及其并集（UCQs）的拟合问题。

Result: 拟合问题对AQs和完整CQs是CO-NP复杂度，对CQs和UCQs是2EXPTIME完全。

Conclusion: 研究结果为不同查询语言的拟合问题提供了理论支持，展示了其计算复杂度。
...
### [300] [AdaptFlow: Adaptive Workflow Optimization via Meta-Learning](https://arxiv.org/abs/2508.08053)
...
TL;DR: AdaptFlow是一个基于自然语言的元学习框架，通过学习通用工作流初始化实现快速子任务适应，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态模板或手动设计的工作流方法适应性差且难以扩展，需改进。

Method: 采用双层优化方案：内循环通过LLM反馈优化子任务工作流，外循环更新共享初始化以实现跨任务泛化。

Result: 在问答、代码生成和数学推理任务中，AdaptFlow表现优于基线方法，达到SOTA。

Conclusion: AdaptFlow通过语言引导的修改实现高效泛化，适用于多种任务和模型。
...
### [301] [FNBT: Full Negation Belief Transformation for Open-World Information Fusion Based on Dempster-Shafer Theory of Evidence](https://arxiv.org/abs/2508.08075)
*Meishen He,Wenjun Ma,Jiao Wang,Huijun Yue,Xiaoma Fan*

Main category: cs.AI

TL;DR: 该论文提出了一种基于Dempster-Shafer理论的开放世界信息融合方法FNBT，用于处理异构框架下的证据融合问题。
...
Method: 提出FNBT方法，通过扩展框架和全否定机制处理异构框架，并满足三个理论性质。

Result: FNBT在真实数据集上表现优异，成功解决了Zadeh反例。

Conclusion: FNBT方法在理论和实践中均表现出色，适用于开放世界信息融合。
...
### [302] [TeamMedAgents: Enhancing Medical Decision-Making of LLMs Through Structured Teamwork](https://arxiv.org/abs/2508.08115)
*Pranav Pushkar Mishra,Mohammad Arvan,Mohan Zalake*

Main category: cs.AI

TL;DR: TeamMedAgents将人类团队合作的心理学模型应用于多智能体医疗决策系统，通过六个核心团队协作组件提升LLM在医疗任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 将人类团队协作的理论应用于AI多智能体系统，以提升医疗决策的准确性和协作效率。

Method: 基于Salas的“Big Five”模型，实现六个团队协作组件，并通过模块化架构评估不同任务和领域下的协作效果。

Result: 在8个医疗基准测试中，7个数据集表现提升，并通过消融实验揭示了不同任务的最优协作配置。

Conclusion: TeamMedAgents为关键决策领域的多智能体系统设计提供了基于证据的理论基础。
...
### [303] [BlindGuard: Safeguarding LLM-based Multi-Agent Systems under Unknown Attacks](https://arxiv.org/abs/2508.08127)
*Rui Miao,Yixin Liu,Yili Wang,Xu Shen,Yue Tan,Yiwei Dai,Shirui Pan,Xin Wang*

Main category: cs.AI

TL;DR: BlindGuard是一种无监督防御方法，无需攻击标签即可检测多智能体系统中的恶意代理。


<details>
  <summary>Details</summary>
Motivation: 现有监督防御方法依赖标记数据，不适用于实际场景，需开发更通用的防御方法。

Method: BlindGuard通过分层代理编码器和基于噪声注入的对比学习，无监督检测恶意代理。
...
### [304] [From Natural Language to Solver-Ready Power System Optimization: An LLM-Assisted, Validation-in-the-Loop Framework](https://arxiv.org/abs/2508.08147)
*Yunkai Hu,Tianqiao Zhao,Meng Yue*

Main category: cs.AI

TL;DR: 论文提出了一种基于大型语言模型（LLM）的代理，将电力系统优化的自然语言描述自动转换为紧凑、可求解的数学公式，并生成解决方案。该方法通过结合领域知识提示和验证机制，显著提高了解决方案的可靠性。


<details>
  <summary>Details</summary>
Motivation: 直接使用LLM生成解决方案常导致不可行或次优结果，因为LLM缺乏数值精度和约束处理能力。本文旨在通过结合LLM与优化求解器，解决这一问题。

Method: 提出了一种结合LLM与优化求解器的管道，包括领域感知提示、系统验证和迭代修复，生成可求解的数学公式和解决方案。

Result: 以机组组合问题为例，代理生成了最优或接近最优的调度方案及目标成本，验证了方法的有效性。

Conclusion: 结合AI与传统优化框架，能够高效地将高层问题描述转化为可执行数学模型，提升能源系统决策效率。
...
### [305] [Transfer Learning with EfficientNet for Accurate Leukemia Cell Classification](https://arxiv.org/abs/2508.06535)
*Faisal Ahmed*

Main category: eess.IV

TL;DR: 该研究使用预训练的卷积神经网络（CNN）和数据增强技术，通过迁移学习提高急性淋巴细胞白血病（ALL）的分类准确性。EfficientNet-B3表现最佳，F1分数为94.30%，准确率为92.02%，AUC为94.79%。


<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病（ALL）的准确分类对早期诊断和治疗计划至关重要，但现有方法在性能上仍有提升空间。

Method: 采用迁移学习和预训练的CNN（如ResNet50、ResNet101和EfficientNet变体），结合数据增强技术处理类别不平衡问题。

Result: EfficientNet-B3表现最佳，F1分数为94.30%，准确率为92.02%，AUC为94.79%，优于C-NMC挑战中的先前方法。

Conclusion: 结合数据增强和先进迁移学习模型（如EfficientNet-B3）可开发出准确且稳健的血液恶性肿瘤诊断工具。
...
### [306] [LWT-ARTERY-LABEL: A Lightweight Framework for Automated Coronary Artery Identification](https://arxiv.org/abs/2508.06874)
*Shisheng Zhang,Ramtin Gharleghi,Sonit Singh,Daniel Moses,Dona Adikari,Arcot Sowmya,Susann Beier*

Main category: eess.IV

TL;DR: 提出一种结合解剖学知识和规则拓扑约束的轻量级方法，用于冠状动脉自动标注，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球主要死因，CTCA是关键诊断工具，但人工分析耗时且劳动密集。现有自动标注方法在利用数据驱动或临床知识方面存在不足。

Method: 提出一种轻量级方法，整合解剖学知识和规则拓扑约束，用于冠状动脉自动标注。
...
### [307] [Fusion-Based Brain Tumor Classification Using Deep Learning and Explainable AI, and Rule-Based Reasoning](https://arxiv.org/abs/2508.06891)
*Melika Filvantorkaman,Mohsen Piri,Maral Filvan Torkaman,Ashkan Zabihi,Hamidreza Moradi*

Main category: eess.IV

TL;DR: 该研究提出了一种基于MobileNetV2和DenseNet121的集成深度学习框架，用于MRI脑肿瘤分类，结合可解释AI模块和临床决策规则，取得了高准确性和临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 准确且可解释的脑肿瘤分类对诊断和治疗至关重要，但现有方法在临床信任和透明度上存在不足。

Method: 采用MobileNetV2和DenseNet121的集成模型，结合软投票策略，并引入Grad-CAM++和临床决策规则增强可解释性。

Result: 集成模型表现优于单一模型，准确率达91.7%，Grad-CAM++可视化与专家标注区域高度一致，临床医生评估认可其解释性。

Conclusion: 该框架为脑肿瘤分类提供了强大且可解释的解决方案，推动了深度学习在临床神经诊断中的应用。
...
### [308] [Spatio-Temporal Conditional Diffusion Models for Forecasting Future Multiple Sclerosis Lesion Masks Conditioned on Treatments](https://arxiv.org/abs/2508.07006)
*Gian Mario Favero,Ge Ya Luo,Nima Fathi,Justin Szeto,Douglas L. Arnold,Brennan Nichyporuk,Chris Pal,Tal Arbel*

Main category: eess.IV

TL;DR: 提出首个治疗感知的时空扩散模型，用于预测多发性硬化症（MS）病灶的未来演化，结合多模态患者数据，在临床实验中验证了其准确性和潜在应用价值。


<details>
  <summary>Details</summary>
Motivation: 个性化医疗在异质性疾病如MS中潜力巨大，但缺乏能够预测病灶演变的模型。

Method: 采用基于体素空间的方法，整合MRI和治疗信息，生成未来病灶掩膜。

Result: 在2131例患者数据上验证，模型能准确预测六种治疗下的病灶演化，并展示临床下游任务潜力。
...
### [309] [Trustworthy Medical Imaging with Large Language Models: A Study of Hallucinations Across Modalities](https://arxiv.org/abs/2508.07031)
*Anindya Bijoy Das,Shahnewaz Karim Sakib,Shibbir Ahmed*

Main category: eess.IV

TL;DR: 研究探讨了大型语言模型（LLMs）在医学影像任务中的幻觉问题，分析了图像到文本和文本到图像两种方向的错误模式及其对临床可靠性的影响。


<details>
  <summary>Details</summary>
Motivation: LLMs在医学影像任务中常产生幻觉（自信但错误的输出），可能误导临床决策，因此需要系统研究其错误模式和改进方法。

Method: 研究从图像到文本（生成报告）和文本到图像（生成图像）两个方向分析幻觉问题，使用专家标准评估输出的准确性和一致性。
...
### [310] [3DGS-VBench: A Comprehensive Video Quality Evaluation Benchmark for 3DGS Compression](https://arxiv.org/abs/2508.07038)
*Yuke Xing,William Gordon,Qi Yang,Kaifa Yang,Jiarui Wang,Yiling Xu*

Main category: eess.IV

TL;DR: 3DGS-VBench是一个用于评估3D高斯泼溅（3DGS）压缩算法视觉质量的大规模数据集和基准测试，包含660个压缩模型和视频序列，并提供了MOS分数和15种质量评估指标的对比。


<details>
  <summary>Details</summary>
Motivation: 3DGS的高存储需求限制了实际应用，而现有压缩方法缺乏系统性的质量评估研究。

Method: 建立了3DGS-VBench数据集，包含11个场景的660个压缩3DGS模型和视频序列，由50名参与者标注MOS分数，并验证了数据集可靠性。

Result: 评估了6种3DGS压缩算法的存储效率和视觉质量，并对比了15种质量评估指标。

Conclusion: 该工作为3DGS压缩和质量评估研究提供了专门的数据集和基准测试，推动了相关领域的发展。
...
### [311] [SAGCNet: Spatial-Aware Graph Completion Network for Missing Slice Imputation in Population CMR Imaging](https://arxiv.org/abs/2508.07041)
*Junkai Liu,Nay Aung,Theodoros N. Arvanitis,Stefan K. Piechnik,Joao A C Lima,Steffen E. Petersen,Le Zhang*

Main category: eess.IV

TL;DR: 论文提出了一种名为SAGCNet的方法，用于解决MRI中缺失切片的问题，通过图结构和空间适配器模块提升3D空间信息的利用。


<details>
  <summary>Details</summary>
Motivation: MRI中缺失或不可用的切片影响了诊断准确性，现有方法难以建模3D数据的局部和全局依赖关系。

Method: SAGCNet包含两个创新模块：1) 切片图补全模块，建模切片间关系；2) 空间适配器模块，捕获3D空间上下文。

Result: 在心脏MRI数据集上，SAGCNet在定量和定性上均优于现有方法，且在数据有限时仍表现优异。

Conclusion: SAGCNet通过结合图结构和空间信息，有效解决了MRI缺失切片的合成问题，具有实际应用潜力。
...
### [312] [Large-scale Multi-sequence Pretraining for Generalizable MRI Analysis in Versatile Clinical Applications](https://arxiv.org/abs/2508.07165)
...
TL;DR: PRISM是一个基于大规模多序列MRI预训练的基础模型，旨在解决MRI序列异质性对深度学习模型泛化能力的挑战，并在44个下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: MRI序列的异质性限制了深度学习模型的泛化能力，影响其在临床中的实用性。

Method: 收集64个数据集（34个用于预训练），提出一种新的预训练范式，分离解剖不变特征与序列特异性变化。
...
### [313] [HaDM-ST: Histology-Assisted Differential Modeling for Spatial Transcriptomics Generation](https://arxiv.org/abs/2508.07225)
*Xuepeng Liu,Zheng Jiang,Pinan Zhu,Hanyu Liu,Chao Li*

Main category: eess.IV

TL;DR: HaDM-ST是一种基于H&E图像和低分辨率空间转录组数据的高分辨率空间转录组生成框架，解决了现有方法在特征提取、多模态对齐和基因特异性建模方面的挑战。
...
Method: HaDM-ST包含三个模块：(1)语义蒸馏网络提取H&E图像的预测线索；(2)空间对齐模块实现像素级对应；(3)通道感知对抗学习器进行基因级建模。

Result: 在200个基因的实验中，HaDM-ST在空间保真度和基因级一致性上优于现有方法。

Conclusion: HaDM-ST为高分辨率空间转录组预测提供了一种有效解决方案，显著提升了性能。
...
### [314] [DiffVC-OSD: One-Step Diffusion-based Perceptual Neural Video Compression Framework](https://arxiv.org/abs/2508.07682)
*Wenzhuo Ma,Zhenzhong Chen*

Main category: eess.IV

TL;DR: DiffVC-OSD是一种基于单步扩散的感知神经视频压缩框架，通过直接输入重建的潜在表示到单步扩散模型中，提升了感知质量，并利用时间上下文适配器优化时间依赖性。


<details>
  <summary>Details</summary>
Motivation: 传统多步扩散方法效率较低，DiffVC-OSD旨在通过单步扩散提升压缩性能和速度。

Method: 采用单步扩散模型，设计时间上下文适配器编码条件输入，并使用端到端微调策略。

Result: DiffVC-OSD在感知压缩性能上达到最优，解码速度提升20倍，比特率降低86.92%。

Conclusion: DiffVC-OSD通过单步扩散和时间上下文适配器显著提升了视频压缩的效率和性能。
...
### [315] [Anatomy-Aware Low-Dose CT Denoising via Pretrained Vision Models and Semantic-Guided Contrastive Learning](https://arxiv.org/abs/2508.07788)
...
<details>
  <summary>Details</summary>
Motivation: 现有低剂量CT去噪方法忽略了人体组织的解剖语义，可能导致去噪效果不佳。ALDEN旨在通过引入解剖感知的判别器和语义引导的对比学习模块来解决这一问题。

Method: ALDEN结合了预训练视觉模型的语义特征，采用解剖感知判别器动态融合参考正常剂量CT的层次语义特征，并通过语义引导的对比学习模块增强解剖一致性。

Result: 在两个低剂量CT去噪数据集上的实验表明，ALDEN实现了最先进的性能，显著减少了过平滑问题并保留了解剖结构。下游多器官分割任务进一步验证了其解剖感知能力。

Conclusion: ALDEN通过结合解剖语义特征和先进学习策略，显著提升了低剂量CT去噪的效果，为医学影像处理提供了新的解决方案。
...
### [316] [Towards Human-AI Collaboration System for the Detection of Invasive Ductal Carcinoma in Histopathology Images](https://arxiv.org/abs/2508.07875)
*Shuo Han,Ahmed Karam Eldaly,Solomon Sunday Oyelere*

Main category: eess.IV

TL;DR: 提出了一种人机协作的深度学习系统，结合EfficientNetV2S模型和专家反馈，用于提高浸润性导管癌（IDC）检测的准确性和效率。
...
Method: 采用人机协作（HITL）系统，AI模型（EfficientNetV2S）提供初始诊断，专家修正结果并反馈给模型，形成迭代优化。
...
### [317] [Diffusing the Blind Spot: Uterine MRI Synthesis with Diffusion Models](https://arxiv.org/abs/2508.07903)
*Johanna P. Müller,Anika Knupfer,Pedro Blöss,Edoardo Berardi Vittur,Bernhard Kainz,Jana Hutter*

Main category: eess.IV

TL;DR: 提出了一种新的扩散模型框架，用于生成解剖学精确的女性盆腔MRI图像，解决了现有模型在妇科影像中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成女性盆腔图像时解剖学精度不足，且妇科影像数据稀缺和患者隐私问题严重，限制了其应用。
...
### [318] [RedDino: A foundation model for red blood cell analysis](https://arxiv.org/abs/2508.08180)
...
Result: RedDino在红细胞形状分类上表现优异，特征表示和泛化能力强。
...
### [319] [A Physics-Driven Neural Network with Parameter Embedding for Generating Quantitative MR Maps from Weighted Images](https://arxiv.org/abs/2508.08123)
...
<details>
  <summary>Details</summary>
Motivation: 提高临床加权MRI定量图像合成的准确性和泛化性。
...
Conclusion: 通过参数嵌入显著提升定量MRI合成的性能和可靠性，具有加速qMRI和提升临床实用性的潜力。
...
### [320] [Assessing Engineering Student Perceptions of Introductory CS Courses in an Indian Context](https://arxiv.org/abs/2508.06563)
...
TL;DR: 研究探讨了印度工学院学生对计算机科学课程评估的看法，发现实验作业被视为有效学习活动，考试和项目被认为真实且提升技能。学生表现和评估感知因背景差异显著。


<details>
  <summary>Details</summary>
Motivation: 了解学生对评估的看法对设计包容有效的学习环境至关重要，尤其是在技术教育中。

Method: 对318名大一工科生进行为期九周的Likert量表调查，使用描述性统计和非参数检验分析数据。

Result: 学生认为实验作业有效，考试和项目真实且提升技能。表现和评估感知因背景差异显著，数据不符合高斯分布。

Conclusion: 研究为编程教育中设计包容公平的评估策略提供了宝贵见解，并揭示了与欧洲学生的异同。
...
### [321] [Teaching Introduction to Programming in the times of AI: A case study of a course re-design](https://arxiv.org/abs/2508.06572)
*Nikolaos Avouris,Kyriakos Sgarbas,George Caridakis,Christos Sintoris*

Main category: cs.CY

TL;DR: 本文综述了AI工具在编程教育中的应用，探讨了其在课程设计、学习目标、评估等方面的挑战，并提出了应对策略。


<details>
  <summary>Details</summary>
Motivation: 研究AI工具在编程教育中的普及及其带来的变革，旨在优化教学效果并应对相关挑战。

Method: 通过综述现有AI工具及其在编程教学中的应用，分析课程设计、评估和学生使用中的问题。

Result: 提出了重新设计课程和教学方法的策略，以最大化AI工具的益处并解决相关问题。

Conclusion: 本文为教育机构和教师提供了应对AI工具挑战的指南，以优化编程教学。
...
### [322] [Leveraging LLMs for Privacy-Aware Predictions in Participatory Budgeting](https://arxiv.org/abs/2508.06577)
*Juan Zambrano,Clément Contet,Jairo Gudiño,Felipe Garrido-Lucero,Umberto Grandi,Cesar A Hidalgo*

Main category: cs.CY

TL;DR: 论文提出了一种隐私保护方法，利用文本描述和历史投票记录预测参与式预算提案的资助可能性，评估了GPT 4 Turbo的性能，发现需结合历史数据才能准确预测。


<details>
  <summary>Details</summary>
Motivation: 参与式预算（PB）的民主潜力常因低参与率受限，影响了透明度和合法性。研究旨在通过支持提案者和组织者提升PB选举的效果。

Method: 采用隐私保护方法，仅基于提案文本描述和匿名历史投票记录预测资助可能性，评估了GPT 4 Turbo在不同情境下的表现。

Result: 研究发现，GPT 4 Turbo需结合历史投票数据才能准确预测提案结果，反映真实投票行为。

Conclusion: AI工具可提升PB过程的透明度、规划效率和公民参与度，具有实际应用潜力。
...
### [323] [Towards Integrated Alignment](https://arxiv.org/abs/2508.06592)
...
<details>
  <summary>Details</summary>
Motivation: AI对齐领域因行为与表征方法的分歧而面临脆弱性，需整合以应对日益复杂的错位威胁。

Method: 提出集成对齐框架设计原则，结合多样对齐方法，强调战略多样性和自适应协同进化。

Result: 框架通过整合互补方法提升对齐效果，避免同质化导致的脆弱性。

Conclusion: 建议通过跨领域合作和资源共享，推动AI对齐研究的统一与进步。
...
### [324] [Understanding Privacy Norms Around LLM-Based Chatbots: A Contextual Integrity Perspective](https://arxiv.org/abs/2508.06760)
...
TL;DR: 研究发现，ChatGPT用户对隐私高度敏感，但行为与担忧脱节，仅传输因素（如知情同意和数据匿名化）显著影响数据共享的适当性感知。


<details>
  <summary>Details</summary>
Motivation: 了解用户对LLM驱动聊天机器人数据共享的隐私期望演变。

Method: 对300名美国ChatGPT用户进行问卷调查和因子实验，分析隐私态度和行为。

Result: 82%用户认为聊天内容敏感，但近半数讨论健康或财务；仅传输因素显著影响隐私感知。

Conclusion: 用户对聊天数据隐私有基线期望，重视程序保障而非接收方可信度，对AI系统整合个人数据有启示。
...
### [325] [Towards Experience-Centered AI: A Framework for Integrating Lived Experience in Design and Development](https://arxiv.org/abs/2508.06849)
*Sanjana Gautam,Mohit Chandra,Ankolika De,Tatiana Chakravorti,Girik Malik,Munmun De Choudhury*

Main category: cs.CY

TL;DR: 提出一个框架，将生活经验融入AI系统的设计和评估中，以提升安全性、信任和可用性。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对人类生活经验的系统理解，未能将其有效嵌入AI开发生命周期。

Method: 综合跨学科文献，提出生活经验的分类法，并在教育、医疗和文化对齐三个领域验证框架。

Result: 框架能更准确地反映人类认知的回顾性、情感性和情境性维度。

Conclusion: 建议开发以经验为中心的AI系统，兼具技术稳健性和与人类现实的契合性。
...
### [326] [Making Effective Decisions: Machine Learning and the Ecogame in 1970](https://arxiv.org/abs/2508.07027)
*Catherine Mason*

Main category: cs.CY

TL;DR: Ecogame是1970年的创新艺术项目，结合视觉艺术与控制论概念，通过模拟和早期机器学习技术，提出行为对整体系统的影响，为当代以人为中心的AI艺术提供历史先例。


<details>
  <summary>Details</summary>
Motivation: 探索技术未来的积极愿景，通过数字手段实现更民主化的参与，基于控制论的理解。

Method: 结合视觉艺术与控制论概念（如适应、反馈和控制），利用模拟和早期机器学习技术，通过实时网络实现。

Result: 提出行为对整体系统的影响，为当代AI艺术提供历史参考。

Conclusion: Ecogame为现代以人为中心的AI艺术实践提供了早期范例。
...
### [327] ["Draw me a curator" Examining the visual stereotyping of a cultural services profession by generative AI](https://arxiv.org/abs/2508.07132)
*Dirk HR Spennemann*

Main category: cs.CY

TL;DR: 论文通过分析230张AI生成的图像，发现ChatGPT4o对博物馆策展人的描绘存在性别、年龄和种族的严重偏差，且带有刻板印象。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示生成式AI模型在描绘博物馆策展人时是否存在偏见，以及这些偏见与现实世界数据的对比。

Method: 基于230张由ChatGPT4o生成的图像，分析其对博物馆策展人的描绘特征，并与现实世界的人口统计数据对比。

Result: AI生成的图像严重低估了女性（3.5% vs 49%-72%）和非白人族群（0% vs 18%-36%），同时过度代表年轻人（79% vs 约27%），并带有刻板印象。

Conclusion: 研究揭示了生成式AI图像数据集的偏见，可能导致对博物馆专业人士的不准确描绘。
...
### [328] [Shaping a Profession, Building a Community: A Practitioner-Led Investigation of Public Interest Technologists in Civil Society](https://arxiv.org/abs/2508.07230)
*Mallory Knodel,Mallika Balakrishnan,Lauren M. Chambers*

Main category: cs.CY

TL;DR: 本文研究了“公共利益技术”（PIT）在民间社会中的实践，发现其定义模糊且缺乏专业化和社区支持。


<details>
  <summary>Details</summary>
Motivation: 探索PIT在民间社会、公民权利和倡导组织中的实践，填补相关研究的空白。

Method: 采用混合方法研究，包括访谈和圆桌讨论，分析PIT从业者的定位和需求。

Result: PIT的定义过于宽泛，从业者需要更强的专业化和社区支持。

Conclusion: PIT是技术与政策的新兴交叉领域，对研究计算与社会关系的变化具有重要意义。
...
### [329] [Enhancing Systematic Interoperability: Convergences and Mismatches between Web 3.0 and the EU Data Act](https://arxiv.org/abs/2508.07356)
*Linyi Xu,Zihao Li*

Main category: cs.CY

TL;DR: 本文探讨了Web 3.0的技术互操作性与欧盟《数据法案》法律互操作性框架的异同，旨在通过跨学科建议提升下一代网络的系统性互操作性。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注技术或法律互操作性，缺乏对其交叉领域的探索。本文旨在填补这一空白，促进技术与法律互操作性的协调。

Method: 比较Web 3.0的技术互操作性与《数据法案》的法律框架，分析其一致性与不匹配之处。

Result: 发现Web 3.0的互操作性涵盖数据、系统和应用，而《数据法案》仅关注数据，可能导致生态系统碎片化。法律条款可能限制技术互操作性的实现。

Conclusion: 建议《数据法案》扩展互操作性范围至系统和应用层，并通过软法机制引入标准化协议，以弥补法律不足并适应技术进步。
...
### [330] [An Empirical Inquiry into Surveillance Capitalism: Web Tracking](https://arxiv.org/abs/2508.07454)
*Nils Bonfils*

Main category: cs.CY

TL;DR: 该研究通过实证方法分析了GAFAM公司的网络跟踪行为及其与财务绩效的关系，揭示了谷歌的主导地位、跟踪技术的演变及其社会环境影响。


<details>
  <summary>Details</summary>
Motivation: 探讨监控资本主义在现代网络中的普遍性及其对经济和社会的深远影响。

Method: 使用WhoTracks.Me的纵向数据和SEC公开文件，分析网络跟踪模式与财务指标的关系。

Result: 发现谷歌在网络跟踪中的主导地位，GAFAM公司分层的跟踪策略，以及跟踪技术的隐蔽化趋势。

Conclusion: 研究强调监控资本主义的负面影响，并探讨替代技术（如Gemini协议）的潜力以挑战其逻辑。
...
### [331] [Intersectoral Knowledge in AI and Urban Studies: A Framework for Transdisciplinary Research](https://arxiv.org/abs/2508.07507)
*Rashid Mushkani*

Main category: cs.CY

TL;DR: 本文提出一个六维框架，用于评估和增强AI与城市研究中的跨学科知识有效性，基于对高引用研究的分析。


<details>
  <summary>Details</summary>
Motivation: 解决跨学科知识整合中的验证难题，特别是在AI和城市研究领域。

Method: 分析2014-2024年高引用研究，提出基于本体论、认识论、方法论、目的论、价值论和增值维度的框架。

Result: 发现研究多倾向于批判现实主义、实证主义、分析方法等，但也探讨了其他较少见视角的潜力。

Conclusion: 框架可帮助早期研究人员和团队协调学科分歧，促进社会责任感强的成果。
...
### [332] [Unequal Uncertainty: Rethinking Algorithmic Interventions for Mitigating Discrimination from AI](https://arxiv.org/abs/2508.07872)
*Holli Sargeant,Mackenzie Jorgensen,Arina Shah,Adrian Weller,Umang Bhatt*

Main category: cs.CY

TL;DR: 论文探讨了AI预测中的不确定性对法律和伦理的挑战，分析了两种算法干预措施（选择性弃权和选择性摩擦）及其潜在歧视风险。


<details>
  <summary>Details</summary>
Motivation: 解决AI辅助决策中不确定性带来的法律和伦理问题，避免加剧社会不平等。

Method: 通过两个案例研究（消费者信贷决策和内容审核），分析基于不确定性的算法干预的歧视性影响。

Result: 选择性摩擦比选择性弃权更能促进公平和透明的AI决策。

Conclusion: 选择性摩擦是一种更公平且合法的干预方式，能减少歧视风险并提升决策透明度。
...
### [333] [Advancing Knowledge Tracing by Exploring Follow-up Performance Trends](https://arxiv.org/abs/2508.08019)
*Hengyu Liu,Yushuai Li,Minghe Yu,Tiancheng Zhang,Ge Yu,Torben Bach Pedersen,Kristian Torp,Christian S. Jensen,Tianyi Li*

Main category: cs.CY

TL;DR: 论文提出了一种名为FINER的知识追踪方法，通过结合历史学习序列和后续表现趋势（FPTs），显著提升了学生未来表现的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有知识追踪方法在分析历史学习序列与未来表现关系时存在相关性冲突，需要一种更有效的方法来解决这一问题。

Method: FINER方法提取FPTs并结合历史学习序列，采用线性时间检索FPTs、相似性感知注意力机制以及FPTs与历史序列的融合策略。

Result: 在六个真实数据集上，FINER优于十种先进方法，准确率提升8.74%至84.85%。

Conclusion: FINER通过整合FPTs和历史学习序列，显著提升了知识追踪的准确性，为智能辅导系统提供了更可靠的预测工具。
...
### [334] [$100,000 or the Robot Gets it! Tech Workers' Resistance Guide: Tech Worker Actions, History, Risks, Impacts, and the Case for a Radical Flank](https://arxiv.org/abs/2508.08084)
*Mohamed Abdalla*

Main category: cs.CY

TL;DR: 论文分析了科技巨头工人运动的现状与挑战，提出通过系统性研究和历史比较，探讨工人行动的策略与效果，并建议建立激进派以提高运动成效。


<details>
  <summary>Details</summary>
Motivation: 科技公司对工人运动的策略调整导致工人行动的成功率下降，亟需更新工人策略以影响企业行为。

Method: 通过分析Google和微软的工人行动报道，结合劳工运动历史比较，并与现任和前任科技工人活动家直接交流，评估行动的风险与效果。

Result: 研究发现工人行动策略存在显著差异，当代科技工人运动相对温和，建立激进派可能提升运动效果。

Conclusion: 建议通过建立激进派增强工人运动的有效性，同时发布了《科技工人抵抗指南》以支持行动。
...
### [335] [AI Gossip](https://arxiv.org/abs/2508.08143)
*Joel Krueger,Lucy Osler*

Main category: cs.CY

TL;DR: 论文探讨了生成式AI聊天机器人（如ChatGPT和Gemini）的“幻觉”现象，认为这些输出更像是“胡说八道”，并进一步分析AI是否能“八卦”，以及由此带来的“技术社会危害”。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解AI聊天机器人在生成内容时缺乏真实性（即“幻觉”或“胡说”）的现象，并探讨其是否具备“八卦”能力及其潜在危害。

Method: 通过定义和场景设定，结合近期案例分析AI“八卦”的表现形式。

Result: 研究发现AI确实能够“八卦”，并揭示了由此产生的“技术社会危害”。

Conclusion: 结论指出AI的“八卦”行为可能带来独特的社会危害，需要进一步研究和关注。
...
### [336] [Street-Level AI: Are Large Language Models Ready for Real-World Judgments?](https://arxiv.org/abs/2508.08193)
*Gaurab Pokharel,Shafkat Farabi,Patrick J. Fowler,Sanmay Das*

Main category: cs.CY

TL;DR: 论文探讨了AI模型在道德判断中的伦理和社会影响，尤其是与人类判断和社会公平评分系统的对齐问题，发现当前LLM在优先级分配上存在不一致性。


<details>
  <summary>Details</summary>
Motivation: 研究AI模型在替代基层官僚分配社会资源时的伦理和社会影响，尤其是其判断与人类和社会公平评分系统的一致性。

Method: 使用真实数据（保持严格保密）分析LLM的判断与人类和社会公平评分系统的一致性，包括内部、跨模型及与评分系统的比较。

Result: LLM的优先级分配在多次运行、不同模型之间以及与评分系统之间均表现出不一致性，但与人类判断在成对测试中具有定性一致性。

Conclusion: 当前AI系统尚未准备好直接用于高风险社会决策，需进一步改进其一致性和可靠性。
...
### [337] [A Moral Agency Framework for Legitimate Integration of AI in Bureaucracies](https://arxiv.org/abs/2508.08231)
*Chris Schmitz,Joanna Bryson*

Main category: cs.CY

TL;DR: 论文探讨了公共部门官僚机构在使用AI时的问责和透明度问题，提出了避免道德责任模糊的三点框架。


<details>
  <summary>Details</summary>
Motivation: 公共部门希望通过AI提高效率，但担心其对官僚机构合法性和稳定性的威胁，尤其是道德责任的模糊化。

Method: 通过将AI伦理中的道德代理概念应用于韦伯式官僚机构，提出了三点道德代理框架。

Result: 研究表明，在明确人类问责、确保系统可验证性和不损害官僚机构目标的前提下，AI可以提高透明度和合法性。

Conclusion: AI在合理框架下不仅能提升效率，还能增强官僚机构的透明度和合法性。
...
### [338] [Fractal Language Modelling by Universal Sequence Maps (USM)](https://arxiv.org/abs/2508.06641)
...
<details>
  <summary>Details</summary>
Motivation: 随着基于Transformer的语言模型（如ChatGPT）的兴起，研究者对多尺度和多维度编码符号序列的方法重新产生兴趣。编码的挑战在于需要保留符号序列的上下文信息，以便通过非线性模型（如神经网络）进行建模。

Method: 通用序列映射（USM）是一种双射编码方法，通过正向和反向的混沌游戏表示（CGR）将符号序列映射到数值空间，并投影到频域（FCGR）。USM坐标可用于计算切比雪夫距离和k-mer频率，无需重新计算数值坐标。

Result: 研究解决了USM迭代过程中的种子偏差问题，实现了序列身份与数值定位的完全一致，并发现USM是一种高效数值过程，能够收敛到稳态序列嵌入解。

Conclusion: USM方法不仅适用于基因组序列（4个核苷酸），还可轻松扩展到任意基数的字母表，展示了其广泛的适用性。
...
### [339] [Self-Organizing Survival Manifolds: A Theory for Unsupervised Discovery of Prognostic Structures in Biological Systems](https://arxiv.org/abs/2508.06539)
*Atahan Karagoz*

Main category: cs.LG

TL;DR: 论文提出了一种新的生存模型理论，将生存视为生物状态空间中几何特性的结果，而非传统的外部标注目标。


<details>
  <summary>Details</summary>
Motivation: 传统生存模型依赖标注数据和固定协变量，忽略了生存作为生物系统内在属性的可能性。

Method: 提出了自组织生存流形（SOSM）理论，基于低曲率测地流和生物约束构建生存相关的几何结构。

Result: 证明了在生物合理条件下，生存对齐的轨迹会自然涌现并收敛。

Conclusion: 该理论为生存建模提供了无需标注的通用框架，将健康、疾病和死亡视为流形结构的几何相变。
...
### [340] [Structure-Preserving Digital Twins via Conditional Neural Whitney Forms](https://arxiv.org/abs/2508.06981)
...
Method: 结合条件注意力机制和有限元外微积分（FEEC），学习降阶基和非线性守恒律，非侵入式集成传统有限元技术。

Result: 在复杂几何和稀疏数据（25次LES模拟）下实现准确预测，实时推理速度提升3.1x10^8倍，开源实现可用。

Conclusion: 该框架在复杂问题和实时应用中表现出色，为数字孪生提供了高效、准确的解决方案。
...
### [341] [Semi-Supervised Supply Chain Fraud Detection with Unsupervised Pre-Filtering](https://arxiv.org/abs/2508.06574)
*Fatemeh Moradi,Mehran Tarif,Mohammadhossein Homaei*

Main category: cs.LG

TL;DR: 提出了一种结合无监督和半监督学习的两阶段框架，用于供应链欺诈检测，效果显著。
...
Method: 第一阶段使用Isolation Forest进行无监督异常检测，第二阶段用自训练SVM结合标签和高置信度伪标签样本进行半监督学习。

Result: 在DataCo数据集上F1-score达0.817，假阳性率低于3.0%。

Conclusion: 该方法有效结合无监督和半监督学习，但需进一步解决概念漂移和与深度学习的对比。
...
### [342] [SGD Convergence under Stepsize Shrinkage in Low-Precision Training](https://arxiv.org/abs/2508.07142)
*Vincent-Daniel Yun*

Main category: cs.LG

TL;DR: 论文研究了低精度训练中梯度收缩和量化噪声对SGD收敛的影响，证明其仍能收敛但速度减慢，并分析了数值精度降低对训练的减速作用。


<details>
  <summary>Details</summary>
Motivation: 低精度训练虽降低计算和内存成本，但梯度量化的收缩和噪声可能改变SGD的收敛行为，需深入研究其影响。

Method: 通过梯度收缩模型分析SGD收敛性，量化噪声为零均值，将名义步长替换为有效步长。

Result: 低精度SGD仍收敛，但速度因梯度收缩减慢，且量化噪声增加了渐近误差。

Conclusion: 数值精度降低通过梯度收缩和噪声减缓训练速度，但SGD仍能收敛，需权衡精度与效率。
...
### [343] [GFlowNets for Learning Better Drug-Drug Interaction Representations](https://arxiv.org/abs/2508.06576)
*Azmine Toushik Wasi*

Main category: cs.LG

TL;DR: 提出了一种结合生成流网络（GFlowNet）和变分图自编码器（VGAE）的框架，用于生成稀有药物相互作用样本，以解决数据不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 药物相互作用（DDI）预测中，稀有但关键的相互作用样本不足，导致模型性能不佳。现有方法多为二分类问题，忽略了类别特异性。

Method: 结合GFlowNet和VGAE生成稀有类别的合成样本，以平衡数据集并提高预测性能。

Result: 该方法显著提升了稀有类别的预测性能，增强了临床可靠性。

Conclusion: 提出的框架有效解决了DDI预测中的数据不平衡问题，为临床提供了更可靠的预测结果。
...
### [344] [Hypergraph Neural Network with State Space Models for Node Classification](https://arxiv.org/abs/2508.06587)
*A. Quadir,M. Tanveer*
...
<details>
  <summary>Details</summary>
Motivation: 传统GNN主要关注节点间的邻接关系，忽略了角色特征的重要性，且现有角色特征捕捉方法多为无监督，性能不佳。

Method: HGMN通过超图构建技术建模高阶关系，结合角色和邻接表示，并引入残差网络解决过平滑问题。

Result: 在多个数据集上，HGMN显著优于现有GNN方法，性能提升明显。
...
### [345] [Graph is a Natural Regularization: Revisiting Vector Quantization for Graph Representation Learning](https://arxiv.org/abs/2508.06588)
...
<details>
  <summary>Details</summary>
Motivation: 图数据中VQ的代码本崩溃问题限制了图令牌的表达能力和泛化性，需要解决。

Method: 提出RGVQ框架，结合图拓扑和特征相似性作为正则化信号，使用Gumbel-Softmax重参数化实现软分配，并引入结构感知对比正则化。

Result: RGVQ显著提高了代码本利用率，并在多个下游任务中提升了图VQ骨干网络的性能。

Conclusion: RGVQ通过增强代码本利用率和令牌多样性，为图数据提供了更具表达力和可迁移性的令牌表示。
...
### [346] [A Federated Learning Framework for Handling Subtype Confounding and Heterogeneity in Large-Scale Neuroimaging Diagnosis](https://arxiv.org/abs/2508.06589)
...
<details>
  <summary>Details</summary>
Motivation: 小样本研究可重复性低，大规模数据集因亚型混淆引入异质性，需要一种新方法解决这些问题。

Method: 采用动态导航模块将样本路由到最适合的本地模型，并通过元整合模块将异构本地模型的预测结果统一输出。

Result: 在1300多名MDD患者和1100名健康对照的fMRI数据上测试，平均准确率达74.06%，优于传统方法。

Conclusion: 该框架通过解决数据异质性和亚型混淆，提升了神经影像CAD系统的可靠性和可重复性，对个性化医疗和临床决策有重要意义。
...
### [347] [Generative Artificial Intelligence Extracts Structure-Function Relationships from Plants for New Materials](https://arxiv.org/abs/2508.06591)
*Rachel K. Luu,Jingyu Deng,Mohammed Shahrudin Ibrahim,Nam-Joon Cho,Ming Dao,Subra Suresh,Markus J. Buehler*

Main category: cs.LG

TL;DR: 论文提出了一种结合生成式AI与多领域文献的框架，用于设计新型生物启发材料，并通过实验验证了AI生成的材料设计和预测。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型（LLMs）在知识检索和创意生成方面有广泛应用，但在多学科领域（如材料科学）的实验科学中应用有限。本文旨在填补这一空白。

Method: 通过整合生成式AI、检索增强生成（RAG）、代理系统和分层采样策略，从植物科学、仿生学和材料工程等未连接领域提取知识，设计实验并验证假设。

Result: 成功开发了一种新型花粉基粘合剂，具有可调形态和实测剪切强度，验证了AI辅助设计的可行性。

Conclusion: AI辅助创意生成能够推动现实世界的材料设计，并实现有效的人机协作。
...
### [348] [Deep Ignorance: Filtering Pretraining Data Builds Tamper-Resistant Safeguards into Open-Weight LLMs](https://arxiv.org/abs/2508.06601)
...
TL;DR: 研究探讨通过过滤训练数据中的双用途主题文本，防止开放权重AI系统被篡改攻击，并提出一种多阶段数据过滤方法。


<details>
  <summary>Details</summary>
Motivation: 开放权重AI系统易受篡改攻击，现有安全微调方法难以抵抗大量对抗性微调步骤，需探索更有效的防护措施。

Method: 引入多阶段数据过滤管道，预训练多个6.9B参数模型，测试其对生物威胁相关文本的抵抗能力。

Result: 过滤模型在对抗性微调攻击中表现优异，抵抗能力远超现有基线，但需结合上下文防御措施。

Conclusion: 预训练数据过滤是开放权重AI系统防御的有效层，但仍需多层次防护策略。
...
### [349] [Local Diffusion Models and Phases of Data Distributions](https://arxiv.org/abs/2508.06614)
*Fangjun Hu,Guangkuo Liu,Yifan Zhang,Xun Gao*

Main category: cs.LG

TL;DR: 扩散模型在生成复杂数据分布方面表现优异，但传统方法忽略数据的局部结构，计算成本高。本文提出数据分布相的新视角，通过局部去噪器降低计算成本，并证明在相变点附近需全局网络，其他时段可用小型局部网络。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型忽略数据的局部结构，导致计算成本高。本文旨在通过数据分布相的概念，设计更高效的局部去噪器。

Method: 定义数据分布相，证明去噪过程包含早期平凡相和晚期数据相，相变点附近需全局网络。提出基于条件互信息的信息论界限诊断相变。

Result: 实验表明，远离相变点时可用小型局部网络计算分数函数，仅在相变点附近需全局网络。

Conclusion: 本文为扩散模型设计提供了更高效架构，并开辟了研究数据分布相和生成人工智能的新方向。
...
### [350] [Generalizing Scaling Laws for Dense and Sparse Large Language Models](https://arxiv.org/abs/2508.06617)
*Md Arafat Hossain,Xingfu Wu,Valerie Taylor,Ali Jannesari*

Main category: cs.LG

TL;DR: 本文提出了一种适用于密集和稀疏大型语言模型的通用扩展法则，以解决现有扩展法则的架构特定性问题。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型规模的指数级增长和训练成本的增加，研究如何高效训练模型变得至关重要，但现有扩展法则多为架构特定，缺乏统一框架。

Method: 重新审视现有扩展法则，提出一种通用扩展法则，适用于密集和稀疏模型，并通过实验评估其有效性。

Result: 提出的通用扩展法则在实验中表现出色，优于现有的架构特定扩展法则。

Conclusion: 通用扩展法则为大型语言模型的训练提供了更灵活和高效的框架，适用于多种架构。
...
### [351] [Learning to Forget with Information Divergence Reweighted Objectives for Noisy Labels](https://arxiv.org/abs/2508.06622)
*Jeremiah Birrell,Reza Ebrahimi*

Main category: cs.LG

TL;DR: ANTIDOTE是一种针对噪声标签学习的新目标函数，通过信息散度邻域的松弛定义，并利用凸对偶性转化为对抗训练方法，计算成本接近标准交叉熵损失。


<details>
  <summary>Details</summary>
Motivation: 解决训练数据中固有或由对手引入的噪声标签问题，减少噪声样本对学习的影响。

Method: 通过信息散度邻域的松弛定义目标函数，利用凸对偶性转化为对抗训练方法。

Result: 在不同类型噪声标签（对称、非对称、人工标注和真实噪声）下表现优于同类方法，计算成本接近标准交叉熵损失。

Conclusion: ANTIDOTE能有效减少噪声标签的影响，适用于实际噪声环境，计算效率高。
...
### [352] [Early Detection of Pancreatic Cancer Using Multimodal Learning on Electronic Health Record](https://arxiv.org/abs/2508.06627)
...
Method: 结合神经控制微分方程、预训练语言模型、循环网络和交叉注意力机制，建模不规则时间序列和多模态交互。

Result: 在4700名患者数据集上，AUC提升6.5%-15.5%，并识别出新的风险标志物。

Conclusion: 该方法显著优于现有技术，为PDAC早期检测提供了新工具。
...
### [353] [Using Imperfect Synthetic Data in Downstream Inference Tasks](https://arxiv.org/abs/2508.06635)
*Yewon Byun,Shantanu Gupta,Zachary C. Lipton,Rachel Leah Childers,Bryan Wilder*

Main category: cs.LG

TL;DR: 论文提出了一种基于广义矩估计的新估计器，用于结合大语言模型生成的合成数据与真实数据，以在计算社会科学中实现统计有效的结论。


<details>
  <summary>Details</summary>
Motivation: 解决如何在计算社会科学和人类研究中结合合成数据与真实数据，并确保统计结论的有效性。

Method: 引入基于广义矩估计的新估计器，无需超参数调整，具有理论保证。

Result: 发现合成数据与真实数据的矩残差交互可以改进目标参数的估计，实证验证了该估计器在计算社会科学任务中的优越性能。

Conclusion: 提出的估计器在有限样本条件下表现优异，为合成数据与真实数据的结合提供了有效解决方案。
...
### [354] [Segmented Confidence Sequences and Multi-Scale Adaptive Confidence Segments for Anomaly Detection in Nonstationary Time Series](https://arxiv.org/abs/2508.06638)
...
Method: 提出了Segmented Confidence Sequences (SCS)和Multi-Scale Adaptive Confidence Segments (MACS)两种框架，结合统计在线学习和分段原理。
...
### [355] [Privacy-Preserving Tabular Synthetic Data Generation Using TabularARGN](https://arxiv.org/abs/2508.06647)
*Andrey Sidorenko,Paul Tiwald*

Main category: cs.LG

TL;DR: TabularARGN是一种用于生成高质量合成表格数据的神经网络架构，具有高数据保真度和计算效率，并在隐私保护方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统匿名化技术在保护隐私方面不足，需要一种更有效的方法生成合成数据。

Method: 采用基于离散化的自回归方法设计TabularARGN。

Result: TabularARGN在统计相似性、机器学习实用性和检测鲁棒性上表现优异，隐私评估显示其具有有效的隐私-效用平衡。

Conclusion: TabularARGN是一种高效且隐私保护良好的合成数据生成方法。
...
### [356] [In-Context Reinforcement Learning via Communicative World Models](https://arxiv.org/abs/2508.06659)
*Fernando Martinez-Lopez,Tao Li,Yingdong Lu,Juntao Chen*

Main category: cs.LG

TL;DR: 论文提出CORAL框架，通过将潜在表示学习与控制解耦，提升强化学习代理的上下文适应能力。


<details>
  <summary>Details</summary>
Motivation: 强化学习代理在新任务和上下文中的泛化能力不足，因为其表示和策略过度拟合训练环境。

Method: 将上下文强化学习建模为双代理通信问题，引入CORAL框架，预训练信息代理（IA）作为世界模型，并通过因果影响损失优化通信协议。控制代理（CA）利用IA提供的上下文学习任务。

Result: 实验表明，CORAL显著提高了样本效率，并在未见过的稀疏奖励环境中实现零样本适应。

Conclusion: 学习可迁移的通信表示有效提升了强化学习代理的上下文适应能力。
...
### [357] [Transferring Social Network Knowledge from Multiple GNN Teachers to Kolmogorov-Arnold Networks](https://arxiv.org/abs/2508.06663)
...
Result: 实验表明新模型提高了节点分类准确性，知识融合显著提升学生模型性能。

Conclusion: KANs能增强GNN表达能力，并支持高效的图无关推理。
...
### [358] [Watermarking Kolmogorov-Arnold Networks for Emerging Networked Applications via Activation Perturbation](https://arxiv.org/abs/2508.06676)
...
Method: 利用KAN的可学习激活函数，通过离散余弦变换（DCT）在激活输出中嵌入水印。
...
### [359] [Uncertainty-Driven Reliability: Selective Prediction and Trustworthy Deployment in Modern Machine Learning](https://arxiv.org/abs/2508.07556)
*Stephan Rabanser*

Main category: cs.LG

TL;DR: 该论文研究了如何通过不确定性估计提升机器学习系统的安全性和可信度，重点探讨了选择性预测方法，并提出了一种轻量级的后验弃权方法。


<details>
  <summary>Details</summary>
Motivation: 在机器学习系统应用于高风险领域时，可靠性和不确定性估计至关重要。论文旨在通过选择性预测提升模型的安全性和可信度。

Method: 通过利用模型训练轨迹中的不确定性信号，提出了一种无需改变架构或损失的轻量级后验弃权方法，并研究了差分隐私对不确定性的影响。

Result: 提出的方法在选择性预测任务中表现优异，且在差分隐私下保持鲁棒性。论文还分解了选择性分类差距，并设计了防御对抗性攻击的方法。

Conclusion: 论文通过改进、评估和保护不确定性估计，推动了可靠机器学习的发展，使模型不仅能准确预测，还能在不确定时拒绝预测。
...
### [360] [Stabilizing Federated Learning under Extreme Heterogeneity with HeteRo-Select](https://arxiv.org/abs/2508.06692)
*Md. Akmol Masud,Md Abrar Jahin,Mahmud Hasan*

Main category: cs.LG

TL;DR: 论文提出了HeteRo-Select框架，用于解决联邦学习中因客户端数据多样性导致的训练不稳定问题，通过智能选择客户端子集提升性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习常因客户端数据多样性导致训练不稳定，现有方法（如Oort）在后期训练中准确性下降显著，需要更稳定的解决方案。

Method: 提出HeteRo-Select框架，基于客户端有用性、公平性、更新速度和数据多样性设计评分系统，理论分析支持其在高度异构数据下的高效通信。

Result: 在CIFAR-10数据集上，HeteRo-Select在峰值准确性（74.75%）、最终准确性（72.76%）和稳定性（仅下降1.99%）上优于Oort。

Conclusion: HeteRo-Select为异构联邦学习问题提供了理论和实证支持的高效解决方案。
...
### [361] [TLCCSP: A Scalable Framework for Enhancing Time Series Forecasting with Time-Lagged Cross-Correlations](https://arxiv.org/abs/2508.07016)
*Jianfei Wu,Wenmian Yang,Bingning Liu,Weijia Jia*

Main category: cs.LG

TL;DR: 提出了TLCCSP框架，通过整合时间滞后交叉相关序列提升预测精度，使用SSDTW算法和对比学习编码器，显著降低MSE并提高计算效率。
...
Method: 提出TLCCSP框架，结合SSDTW算法捕捉滞后相关性，并使用对比学习编码器高效近似SSDTW距离。

Result: 在天气、金融和房地产数据集上，SSDTW和对比学习编码器显著降低MSE，计算时间减少99%。

Conclusion: TLCCSP框架有效提升预测精度和计算效率，适用于多领域时序预测任务。
...
### [362] [CISO: Species Distribution Modeling Conditioned on Incomplete Species Observations](https://arxiv.org/abs/2508.06704)
...
<details>
  <summary>Details</summary>
Motivation: 传统物种分布模型（SDMs）常忽略物种间的生物相互作用，且现有方法对数据要求严格，难以处理不完整的观察数据。

Method: 提出CISO方法，利用深度学习灵活处理不完整的物种观察数据，并结合环境变量进行建模。

Result: CISO在三个数据集（植物、鸟类、蝴蝶）上表现优于其他方法，尤其在结合多数据集时性能进一步提升。

Conclusion: CISO是一种有潜力的生态工具，能够整合不完整的生物信息并识别跨类群物种间的潜在相互作用。
...
### [363] [Analysis of Schedule-Free Nonconvex Optimization](https://arxiv.org/abs/2508.06743)
*Connor Brown*

Main category: cs.LG

TL;DR: 论文提出了一种鲁棒的Lyapunov框架，用于分析Schedule-Free（SF）方法在非凸优化中的性能，证明了其无需依赖总步数T的超参数，并提供了多种收敛速率。


<details>
  <summary>Details</summary>
Motivation: 经典一阶方法依赖于预先知道总步数T的步长调度，而SF方法通过结合Polyak-Ruppert平均和动量，避免了这一限制，但其在非凸优化中的分析尚不完善。

Method: 引入Lyapunov框架，仅需L-平滑性和下界假设，将SF分析简化为单步下降不等式，推导出多种收敛速率。

Result: 在非凸设置下，证明了SF方法的收敛速率：常数步长+PR平均为O(1/log T)，线性增长步长为O(log T/T)，多项式平均为O(T^{-(1-\alpha)})。数值实验支持这些理论结果。

Conclusion: 研究将SF方法的无T依赖保证扩展到非凸优化，并为进一步优化非凸收敛速率提供了方向。
...
### [364] [Fed MobiLLM: Efficient Federated LLM Fine-Tuning over Heterogeneous Mobile Devices via Server Assisted Side-Tuning](https://arxiv.org/abs/2508.06765)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统联邦学习方法在移动设备上计算和内存负担过重，以及同步模型聚合协议导致的延迟问题。

Method: 采用服务器辅助的联邦侧调优范式，移动设备仅执行轻量级前向传播计算，服务器独立训练共享侧网络，并引入自适应分层特征对齐方法。
...
### [365] [PANAMA: A Network-Aware MARL Framework for Multi-Agent Path Finding in Digital Twin Ecosystems](https://arxiv.org/abs/2508.06767)
*Arman Dogru,R. Irem Bor-Yaliniz,Nimal Gamini Senarath*

Main category: cs.LG

TL;DR: 论文探讨了数字孪生（DTs）在下一代技术中的关键作用，提出了一种名为PANAMA的新型算法，用于网络感知的多智能体强化学习（MARL）路径规划，展示了其在准确性、速度和可扩展性上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 随着机器人和自动化系统的扩展，高效的数据共享框架和鲁棒算法变得至关重要。论文旨在解决数字孪生生态系统中应用和网络提供商（AP/NP）之间的动态数据处理的挑战。

Method: 论文提出了PANAMA算法，采用集中训练与分散执行（CTDE）框架和异步行动者-学习者架构，优化多智能体路径规划（MAPF）任务。

Result: PANAMA在路径规划中表现出更高的准确性、速度和可扩展性，并通过仿真验证了其在复杂现实环境中的优化数据共享策略。

Conclusion: PANAMA填补了网络感知决策与鲁棒多智能体协调之间的空白，推动了数字孪生、无线网络和AI驱动自动化之间的协同发展。
...
### [366] [Zero-Direction Probing: A Linear-Algebraic Framework for Deep Analysis of Large-Language-Model Drift](https://arxiv.org/abs/2508.06776)
*Amit Pandey*

Main category: cs.LG

TL;DR: Zero-Direction Probing (ZDP) 是一种无需任务标签或输出评估、通过变压器激活的零方向检测模型漂移的理论框架。
...
Method: 基于假设A1-A6，提出Variance-Leak定理、Fisher Null-Conservation、Rank-Leak边界和在线零空间跟踪器的对数遗憾保证。

Result: 推导出Spectral Null-Leakage (SNL) 度量，具有非渐近尾部边界和集中不等式，为高斯零模型下的漂移提供先验阈值。
...
### [367] [PROPS: Progressively Private Self-alignment of Large Language Models](https://arxiv.org/abs/2508.06783)
*Noel Teku,Fengwei Tian,Payel Bhattacharjee,Souradip Chakraborty,Amrit Singh Bedi,Ravi Tandon*

Main category: cs.LG

TL;DR: 论文提出了PROPS框架，用于在LLM对齐中保护人类偏好标签的隐私，相比现有方法（如DP-SGD和RR）在相同隐私预算下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 依赖人类反馈的LLM对齐可能泄露标注者的隐私信息，现有隐私保护方法（如DP-SGD）可能过度保护隐私并降低模型性能。

Method: 提出PROPS框架，通过多阶段自对齐逐步补充训练数据，同时保护偏好标签的隐私。

Result: 在相同隐私预算下，PROPS比DP-SGD和RR分别提高3倍和2.5倍的胜率。

Conclusion: PROPS在保护隐私的同时显著提升了LLM对齐的效用，为隐私保护对齐提供了新思路。
...
### [368] [Mode-Aware Non-Linear Tucker Autoencoder for Tensor-based Unsupervised Learning](https://arxiv.org/abs/2508.06784)
*Junjing Zheng,Chengliang Song,Weidong Jiang,Xinyu Zhang*

Main category: cs.LG

TL;DR: MA-NTAE是一种新型的非线性Tucker自编码器，通过改进传统Tucker分解方法，解决了高维张量数据在自监督学习中的计算和优化问题。


<details>
  <summary>Details</summary>
Motivation: 高维张量数据在自监督学习中面临维度灾难、计算开销大和优化困难等问题，现有方法难以有效学习非线性关系。

Method: 提出MA-NTAE，结合Pick-and-Unfold策略，通过递归展开-编码-折叠操作，实现高维张量的灵活编码，并整合张量结构先验。

Result: MA-NTAE在压缩和聚类任务中优于标准自编码器和现有张量网络，尤其对高阶高维张量表现更优。

Conclusion: MA-NTAE通过非线性Tucker分解和灵活编码策略，显著提升了高维张量数据的处理效率和性能。
...
### [369] [Hardness-Aware Dynamic Curriculum Learning for Robust Multimodal Emotion Recognition with Missing Modalities](https://arxiv.org/abs/2508.06800)
*Rui Liu,Haolin Zuo,Zheng Lian,Hongyu Yuan,Qi Fan*

Main category: cs.LG

TL;DR: 论文提出了一种名为HARDY-MER的动态课程学习框架，通过评估样本的难度并动态调整训练策略，以提升多模态情感识别中缺失模态的处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在缺失模态重建中未考虑不同样本的难度差异，导致对困难样本处理效果不佳。
...
### [370] [Offline-to-Online Reinforcement Learning with Classifier-Free Diffusion Generation](https://arxiv.org/abs/2508.06806)
*Xiao Huang,Xu Liu,Enze Zhang,Tong Yu,Shuai Li*

Main category: cs.LG

TL;DR: 论文提出了一种名为CFDG的新数据增强方法，用于离线到在线强化学习（O2O RL），通过无分类器扩散生成技术提升数据质量，并在实验中显著优于现有方法。
...
Method: 提出CFDG方法，利用无分类器扩散生成技术增强数据质量，并结合重加权方法使生成数据更符合在线数据分布。

Result: 在D4RL基准测试中，CFDG平均提升15%的性能。

Conclusion: CFDG是一种通用性强的方法，可与现有O2O RL算法结合，显著提升性能。
...
### [371] [Technical Report: Full-Stack Fine-Tuning for the Q Programming Language](https://arxiv.org/abs/2508.06813)
*Brendan R. Hogan,Will Brown,Adel Boyarsky,Anderson Schneider,Yuriy Nevmyvaka*

Main category: cs.LG

TL;DR: 论文提出了一种开源方法，将大语言模型（LLMs）适配到Q编程语言，解决了小众语言任务中LLMs表现不佳的问题。通过预训练、监督微调和强化学习，训练了一系列模型，并在Q语言基准测试中显著优于主流模型。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在小众编程语言（如Q语言）任务中表现不佳的问题，特别是在金融量化领域。

Method: 构建了Q语言的Leetcode风格评估数据集，对前沿模型进行基准测试，并通过预训练、监督微调和强化学习训练了不同规模的模型。

Result: 最佳模型在Q语言基准测试中达到59%的pass@1准确率，优于Claude Opus-4和GPT-4.1。

Conclusion: 提出的方法不仅适用于Q语言，还可扩展到其他任务，为小众领域LLMs适配提供了详细蓝图。
...
### [372] [Who's the Evil Twin? Differential Auditing for Undesired Behavior](https://arxiv.org/abs/2508.06827)
*Ishwar Balappanawar,Venkata Hasith Vattikuti,Greta Kintzley,Ronan Azimi-Mancel,Satvik Golechha*

Main category: cs.LG

TL;DR: 论文探讨了神经网络中隐藏行为的检测问题，通过红蓝对抗游戏框架，比较了不同检测方法的性能，发现基于对抗攻击的方法效果最佳。


<details>
  <summary>Details</summary>
Motivation: 由于先验知识有限和潜在的对抗性混淆，检测神经网络中的隐藏行为具有挑战性。

Method: 红队训练两个相似模型（一个仅使用良性数据，另一个包含隐藏有害行为），蓝队在有限信息下尝试识别被篡改的模型。实验使用了CNN，并测试了多种蓝队策略。

Result: 基于对抗攻击的方法在提供提示的情况下准确率最高（100%），其他方法表现不一。LLM审计方法需要关于不良分布的提示。

Conclusion: 研究结果为设计更好的审计方法提供了参考，并开源了相关模型和数据。
...
### [373] [Sparsity-Driven Plasticity in Multi-Task Reinforcement Learning](https://arxiv.org/abs/2508.06871)
*Aleksandar Todorov,Juan Cardenas-Cartagena,Rafael F. Cunha,Marco Zullich,Matthia Sabatelli*

Main category: cs.LG

TL;DR: 论文探讨了深度强化学习中可塑性丧失的问题，研究了稀疏化方法（GMP和SET）如何提升多任务强化学习（MTRL）中的可塑性和性能。


<details>
  <summary>Details</summary>
Motivation: 解决MTRL中因任务多样性导致的可塑性丧失问题，探索稀疏化方法的作用。

Method: 采用Gradual Magnitude Pruning (GMP)和Sparse Evolutionary Training (SET)方法，评估其在多种MTRL架构中的表现。

Result: GMP和SET有效缓解了可塑性退化，提升了多任务性能，稀疏模型常优于密集模型。
...
### [374] [Conformal Prediction and Trustworthy AI](https://arxiv.org/abs/2508.06885)
*Anthony Bellotti,Xindi Zhao*

Main category: cs.LG

TL;DR: 本文回顾了保形预测在可信AI中的潜力，探讨了其超越边际有效性的应用，如泛化风险和AI治理，并通过实验展示了其作为校准预测器和偏差识别工具的实用性。


<details>
  <summary>Details</summary>
Motivation: 保形预测因其提供具有置信度保证的集合预测能力而受到关注，尤其在可信AI领域具有潜力。本文旨在探讨其在泛化风险和AI治理等更广泛问题中的应用。

Method: 通过文献回顾和实验分析，展示了保形预测作为校准预测器和偏差识别工具的实际应用。

Result: 保形预测不仅提供了校准的预测结果，还能有效识别和缓解偏差，为可信AI的发展提供了支持。

Conclusion: 保形预测在可信AI中具有广泛潜力，尤其是在泛化风险和AI治理方面，未来研究可进一步探索其应用。
...
### [375] [QuiZSF: An efficient data-model interaction framework for zero-shot time-series forecasting](https://arxiv.org/abs/2508.06915)
*Shichao Ma,Zhengyang Zhou,Qihe Huang,Binwu Wang,Kuo Yang,Huan Li,Yang Wang*

Main category: cs.LG

TL;DR: QuiZSF是一个轻量级模块化框架，结合检索增强生成（RAG）和时间序列预训练模型（TSPMs），提升零样本时间序列预测（ZSF）性能。
...
Method: 提出QuiZSF框架，包括ChronoRAG Base（CRB）用于存储和检索，Multi-grained Series Interaction Learner（MSIL）提取特征，以及Model Cooperation Coherer（MCC）对齐知识。

Result: QuiZSF在75%和87.5%的预测场景中排名Top1，同时保持高效内存和推理时间。

Conclusion: QuiZSF成功结合RAG与TSPMs，显著提升零样本时间序列预测性能。
...
### [376] [Class Unbiasing for Generalization in Medical Diagnosis](https://arxiv.org/abs/2508.06943)
...
<details>
  <summary>Details</summary>
Motivation: 医学诊断模型可能因类别特征偏差和类别不平衡导致性能偏差和泛化能力差，需同时解决这两问题。

Method: 提出类别不平等损失和类别加权分布鲁棒优化目标，平衡正负类样本的贡献并加权表现不佳的类别。

Result: 实验证明类别特征偏差会损害模型性能，所提方法能有效缓解偏差和不平衡，提升泛化能力。
...
### [377] [AMFT: Aligning LLM Reasoners by Meta-Learning the Optimal Imitation-Exploration Balance](https://arxiv.org/abs/2508.06944)
...
<details>
  <summary>Details</summary>
Motivation: 传统两阶段方法（SFT+RL）存在灾难性遗忘和探索-模仿权衡问题，而现有的单阶段方法缺乏动态平衡机制。

Method: 提出AMFT算法，通过元梯度自适应权重控制器动态优化SFT和RL的平衡，并结合策略熵正则化以提高稳定性。

Result: 在数学推理、抽象视觉推理和视觉语言导航等任务上，AMFT均取得了最先进的性能，并在分布外任务上表现出更强的泛化能力。

Conclusion: AMFT通过元学习控制器实现了更稳定、高效的训练，为LLM对齐提供了更原则性和有效的方法。
...
### [378] [BoRA: Towards More Expressive Low-Rank Adaptation with Block Diversity](https://arxiv.org/abs/2508.06953)
*Shiwei Li,Xiandi Luo,Haozhao Wang,Xing Tang,Ziqiang Cui,Dugang Liu,Yuhua Li,Xiuqiang He,Ruixuan Li*

Main category: cs.LG

TL;DR: BoRA是一种改进的低秩适应方法，通过块矩阵乘法和块间对角矩阵提升LoRA权重矩阵的秩，同时仅需少量额外参数。


<details>
  <summary>Details</summary>
Motivation: LoRA方法在大型语言模型微调中表现良好，但增加秩会显著增加可训练参数数量，BoRA旨在提升性能的同时减少参数增长。

Method: BoRA将LoRA的权重矩阵分解为块矩阵，并引入块间对角矩阵，通过块乘积提升秩，仅需少量额外参数。

Result: 实验证明BoRA在多个数据集和模型上优于LoRA，且消融研究验证了其可扩展性。

Conclusion: BoRA通过块多样化的低秩适应，显著提升了LoRA的性能和效率。
...
### [379] [Can Multitask Learning Enhance Model Explainability?](https://arxiv.org/abs/2508.06966)
*Hiba Najjar,Bushra Alshbib,Andreas Dengel*

Main category: cs.LG

TL;DR: 通过多任务学习利用遥感数据的多模态性，提升模型性能并增强可解释性，同时避免额外数据收集需求。


<details>
  <summary>Details</summary>
Motivation: 遥感数据多模态性丰富，但多模态学习网络的复杂性降低了模型的可解释性。本研究旨在通过多任务学习利用这些模态，以内在方式解释模型行为。

Method: 将某些模态作为附加目标与主任务一起预测，而非额外输入。利用卫星数据的丰富信息作为输入模态。

Result: 方法在三个数据集上验证，结果显示：(1) 部署时无需收集附加模态数据，(2) 性能与多模态基线相当或更优，(3) 主任务预测误差可通过辅助任务行为解释。

Conclusion: 多任务学习能有效利用遥感数据的多模态性，提升模型性能和可解释性，同时减少数据需求。
...
### [380] [Discovery Learning accelerates battery design evaluation](https://arxiv.org/abs/2508.06985)
*Jiawei Zhang,Yifei Zhang,Baozhao Yi,Yao Ren,Qi Jiao,Hanyu Bai,Weiran Jiang,Ziyou Song*

Main category: cs.LG

TL;DR: 论文提出了一种名为Discovery Learning（DL）的科学机器学习范式，通过结合主动学习、物理引导学习和零样本学习，显著减少了电池设计的原型测试需求，实现了高效寿命评估。


<details>
  <summary>Details</summary>
Motivation: 电池研发因高时间和能源成本的限制而进展缓慢，现有数据驱动方法依赖目标设计的标记数据，无法在原型设计前提供可靠预测。

Method: DL整合了主动学习、物理引导学习和零样本学习，利用历史电池设计数据，减少原型测试需求。

Result: 在123个工业级大尺寸锂离子电池上测试，DL仅使用公开小容量圆柱电池数据集，实现了7.2%的测试误差，节省了98%的时间和95%的能源。
...
### [381] [UniMove: A Unified Model for Multi-city Human Mobility Prediction](https://arxiv.org/abs/2508.06986)
*Chonghua Han,Yuan Yuan,Yukun Liu,Jingtao Ding,Jie Feng,Yong Li*

Main category: cs.LG

TL;DR: UniMove是一个统一的多城市人类移动预测模型，通过通用空间表示和轨迹-位置双塔架构解决城市间异质性问题，显著提升预测准确性。
...
Result: 在多城市数据集上，UniMove通过联合训练提升预测准确性超过10.2%。

Conclusion: UniMove是实现人类移动预测统一基础模型的关键进展。
...
### [382] [A Comparative Study of Feature Selection in Tsetlin Machines](https://arxiv.org/abs/2508.06991)
*Vojtech Halenka,Ole-Christoffer Granmo,Lei Jiao,Per-Arne Andersen*

Main category: cs.LG

TL;DR: 本文评估了多种特征选择（FS）方法在Tsetlin机器（TM）中的应用，包括经典过滤和嵌入方法，以及后解释方法（如SHAP和LIME），并提出了一种基于TM子句权重和Tsetlin自动机状态的新型嵌入式评分器。


<details>
  <summary>Details</summary>
Motivation: TM缺乏估计特征重要性的工具，而FS对提高模型可解释性、降低复杂性和增强准确性至关重要。

Method: 采用多种FS技术，包括经典方法和后解释方法，并开发了新型TM嵌入式评分器。通过12个数据集进行基准测试，使用ROAR和ROAD等评估协议。

Result: TM内部评分器表现优异，利用子句可解释性揭示特征交互模式。简单的TM专用评分器能以较低计算成本实现类似准确性。

Conclusion: 本研究为TM中的FS建立了首个全面基线，并为开发专用TM可解释性技术铺平了道路。
...
### [383] [Conformal Set-based Human-AI Complementarity with Multiple Experts](https://arxiv.org/abs/2508.06997)
*Helbert Paat,Guohao Shen*

Main category: cs.LG

TL;DR: 论文研究了在多专家场景下，如何利用保形预测集选择实例特定的专家子集，提出了一种贪心算法，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注单专家场景，而多专家协作可能进一步提升分类性能，因此需要探索如何选择实例相关的专家子集。

Method: 提出了一种贪心算法，利用保形预测集选择专家子集，对比了传统方法。

Result: 在CIFAR-10H和ImageNet-16H数据集上的模拟实验表明，贪心算法能选择接近最优的子集，提升分类性能。

Conclusion: 多专家协作中，实例特定的专家子集选择能显著提升分类效果，贪心算法是一种有效方法。
...
### [384] [From Imitation to Optimization: A Comparative Study of Offline Learning for Autonomous Driving](https://arxiv.org/abs/2508.07029)
*Antonio Guillen-Perez*

Main category: cs.LG

TL;DR: 论文提出了一种通过离线强化学习（CQL）从静态专家数据中学习稳健驾驶策略的方法，显著优于行为克隆（BC）基线。


<details>
  <summary>Details</summary>
Motivation: 在线数据收集在自动驾驶中不切实际且不安全，而行为克隆策略在闭环执行中容易出错，因此需要更稳健的学习方法。

Method: 开发了基于Transformer的行为克隆基线，并应用保守Q学习（CQL）算法，结合精心设计的奖励函数。

Result: 在Waymo Open Motion数据集上，CQL策略的成功率比BC基线高3.2倍，碰撞率低7.4倍。

Conclusion: 离线强化学习是从静态专家数据中学习长期稳健驾驶策略的关键。
...
### [385] [A Stage-Aware Mixture of Experts Framework for Neurodegenerative Disease Progression Modelling](https://arxiv.org/abs/2508.07032)
*Tiantian He,Keyue Jiang,An Zhao,Anna Schroder,Elinor Thompson,Sonja Soskic,Frederik Barkhof,Daniel C. Alexander*

Main category: cs.LG

TL;DR: 提出了一种新型阶段感知的Mixture of Experts框架（IGND-MoE），用于建模神经退行性疾病的动态进展机制。


<details>
  <summary>Details</summary>
Motivation: 解决传统模型无法捕捉疾病不同阶段的动态机制及缺乏纵向数据的问题。

Method: 结合时间依赖的专家权重、非均匀图神经扩散模型（IGND）和局部神经反应模块。

Result: 模型揭示了早期图相关机制主导，后期其他物理机制主导的进展模式。

Conclusion: IGND-MoE为理解疾病阶段特异性机制提供了新方法。
...
### [386] [Differentiable Adaptive Kalman Filtering via Optimal Transport](https://arxiv.org/abs/2508.07037)
*Yangguang He,Wenhao Li,Minzhe Li,Juan Zhang,Xiangfeng Wang,Bo Jin*

Main category: cs.LG

TL;DR: OTAKNet是一种在线解决方案，用于解决学习型自适应卡尔曼滤波中的噪声统计漂移问题，通过最优传输实现无需地面真值或重新训练的在线适应。


<details>
  <summary>Details</summary>
Motivation: 现实环境中，环境因素（如风况变化或电磁干扰）会导致未观测到的噪声统计漂移，从而显著降低学习型方法的性能。

Method: OTAKNet通过一步预测测量似然建立状态估计与漂移之间的联系，并利用最优传输的几何感知成本和稳定梯度实现完全在线适应。

Result: 在合成和真实NCLT数据集上，OTAKNet表现优于经典模型自适应卡尔曼滤波和离线学习型滤波，尤其在训练数据有限时。

Conclusion: OTAKNet是首个针对噪声统计漂移的在线学习型解决方案，无需地面真值或重新训练即可实现高效适应。
...
### [387] [Membership and Memorization in LLM Knowledge Distillation](https://arxiv.org/abs/2508.07054)
*Ziqi Zhang,Ali Shahin Shamsabadi,Hanxiao Lu,Yifeng Cai,Hamed Haddadi*

Main category: cs.LG

TL;DR: 本文系统研究了六种大型语言模型（LLM）知识蒸馏（KD）技术中的隐私风险，发现所有方法均存在成员和记忆隐私风险，但程度因技术而异。
...
Method: 使用七种NLP任务、三种教师模型家族（GPT-2、LLAMA-2、OPT）和不同规模的学生模型，分析六种KD技术的隐私风险。

Result: 所有KD技术均存在隐私风险，但程度不同；记忆与成员隐私风险之间存在显著不一致；不同模块的隐私风险差异显著。

Conclusion: KD技术的隐私风险需进一步研究，尤其是模块间差异和风险不一致性。
...
### [388] [Surgical Knowledge Rewrite in Compact LLMs: An 'Unlearn-then-Learn' Strategy with ($IA^3$) for Localized Factual Modulation and Catastrophic Forgetting Mitigation](https://arxiv.org/abs/2508.07075)
*Stanley Ngugi*

Main category: cs.LG

TL;DR: 论文提出了一种“先遗忘再学习”策略，结合参数高效微调技术（$IA^3$），用于解决大语言模型（LLMs）动态知识更新中的冲突问题。该方法通过定位并修改特定内部组件，显著提高了新知识的准确性和旧知识的遗忘率，同时减少了灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在动态知识更新时面临新知识与旧知识冲突的问题，导致模型难以接受新知识或遗忘无关知识。本文旨在解决这一问题。

Method: 采用“先遗忘再学习”策略，结合参数高效微调技术（$IA^3$），并通过电路定位阶段识别和修改冲突知识编码的内部组件。

Result: 实验表明，该方法在新知识准确率（98.50%）和旧知识遗忘率（96.00%）上表现优异，同时显著减少了灾难性遗忘（72.00% F_control准确率）。

Conclusion: 该方法为大语言模型提供了精确、局部化且安全的知识管理方案，是知识编辑领域的重要进展。
...
### [389] [Improving Real-Time Concept Drift Detection using a Hybrid Transformer-Autoencoder Framework](https://arxiv.org/abs/2508.07085)
*N Harshit,K Mounvik*
...
<details>
  <summary>Details</summary>
Motivation: 概念漂移（数据分布的逐渐或突然变化）会显著降低模型性能，现有检测方法通常是反应式的且对早期检测不敏感。

Method: 采用Transformer和Autoencoder的混合框架，结合Trust Score方法（包括统计和重构指标、预测不确定性、规则违反和分类器误差趋势）。

Result: 在航空乘客数据集上，该模型比基线方法更早、更敏感地检测到漂移，并减少了错误率和逻辑违反。
...
### [390] [Towards High-Order Mean Flow Generative Models: Feasibility, Expressivity, and Provably Efficient Criteria](https://arxiv.org/abs/2508.07102)
...
<details>
  <summary>Details</summary>
Motivation: 生成建模在无模拟范式（如Flow Matching和MeanFlow）中取得了显著进展，但需要进一步扩展以支持更丰富的动力学和高效采样。

Method: 引入Second-Order MeanFlow，将平均加速度场纳入MeanFlow目标，证明其满足广义一致性条件，并通过电路复杂性分析表征其表达能力。

Result: 证明了Second-Order MeanFlow的采样过程可在$\mathsf{TC}^0$类中实现，并推导了基于快速注意力计算的高效实现标准。

Conclusion: 研究为高阶流匹配模型奠定了理论基础，结合了丰富的动力学和实际采样效率。
...
### [391] [BrainATCL: Adaptive Temporal Brain Connectivity Learning for Functional Link Prediction and Age Estimation](https://arxiv.org/abs/2508.07106)
*Yiran Huang,Amirhossein Nouranizadeh,Christine Ahrends,Mengjia Xu*

Main category: cs.LG

TL;DR: 论文提出了一种名为BrainATCL的无监督、非参数框架，用于动态fMRI数据的自适应时间脑连接学习，解决了传统GNN在捕捉长时依赖上的不足。


<details>
  <summary>Details</summary>
Motivation: fMRI信号的功能连接动态可能与行为和神经精神疾病相关，但传统GNN难以捕捉动态fMRI数据中的长时依赖关系。

Method: 提出BrainATCL框架，动态调整每个快照的回溯窗口，并使用GINE-Mamba2骨干网络编码图序列，结合脑结构和功能信息。

Result: 在功能连接预测和年龄估计任务中表现出优越性能，并具有强泛化能力。
...
### [392] [Approaching Maximal Information Extraction in Low-Signal Regimes via Multiple Instance Learning](https://arxiv.org/abs/2508.07114)
*Atakan Azakli,Bernd Stelzer*

Main category: cs.LG

TL;DR: 提出了一种新的机器学习方法，通过多实例学习（MIL）提高假设检验中参数预测的精确度，并在高难度分类任务中增强判别能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有分类器在极端挑战性任务中预测准确性不足的问题，并系统性地降低机器学习模型的预测误差。

Method: 采用多实例学习（MIL）方法，分析其相对于单实例学习的预测优势，并通过实例数量的缩放行为验证理论。

Result: 在标准模型有效场论（SMEFT）的Wilson系数约束中，展示了MIL方法的有效性，并可能提取数据集中的最大Fisher信息。

Conclusion: MIL方法在提高预测精度和判别能力方面具有潜力，尤其在复杂物理问题中表现突出。
...
### [393] [From Nodes to Narratives: Explaining Graph Neural Networks with LLMs and Graph Context](https://arxiv.org/abs/2508.07117)
*Peyman Baghershahi,Gregoire Fournier,Pranav Nyati,Sourav Medya*

Main category: cs.LG

TL;DR: LOGIC是一个轻量级框架，利用大语言模型（LLM）为GNN预测生成忠实且可解释的解释，通过将GNN节点嵌入投影到LLM嵌入空间并构建混合提示，实现自然语言解释和简洁解释子图。


<details>
  <summary>Details</summary>
Motivation: 现有GNN解释方法在生成细粒度、可解释的解释方面存在困难，尤其是当节点属性包含丰富自然语言时。

Method: LOGIC将GNN节点嵌入投影到LLM嵌入空间，构建混合提示（软提示与文本输入交织），使LLM能推理GNN内部表示并生成自然语言解释。

Result: 在四个真实TAG数据集上的实验表明，LOGIC在保真度和稀疏性之间取得良好平衡，并显著提升人类中心指标（如洞察力）。

Conclusion: LOGIC通过将GNN内部与人类推理对齐，为基于LLM的图学习可解释性开辟了新方向。
...
### [394] [Multi-Level Service Performance Forecasting via Spatiotemporal Graph Neural Networks](https://arxiv.org/abs/2508.07122)
...
<details>
  <summary>Details</summary>
Motivation: 分布式后端系统的性能波动预测是一个复杂且具有挑战性的问题，尤其是多级服务调用结构增加了预测难度。

Method: 将系统状态抽象为图结构序列，结合服务节点运行时特征和调用关系，构建统一的时空建模框架。使用图卷积网络提取高阶依赖信息，门控循环网络捕捉动态演化，并引入时间编码机制增强非平稳时间序列表示。

Result: 实验结果表明，该模型在MAE、RMSE和R2等关键指标上优于现有方法，并在不同负载和结构复杂度下表现出强鲁棒性。

Conclusion: 该模型在分布式后端服务性能管理任务中具有实际应用潜力。
...
### [395] [Pref-GUIDE: Continual Policy Learning from Real-Time Human Feedback via Preference-Based Learning](https://arxiv.org/abs/2508.07126)
...
### [396] [How Effectively Can Large Language Models Connect SNP Variants and ECG Phenotypes for Cardiovascular Risk Prediction?](https://arxiv.org/abs/2508.07127)
*Niranjana Arun Menon,Iqra Farooq,Yulong Li,Sara Ahmed,Yutong Xie,Muhammad Awais,Imran Razzak*

Main category: cs.LG

TL;DR: 利用微调的大型语言模型（LLMs）预测心血管疾病（CVD）风险和遗传标记，通过基因组数据学习潜在生物学关系，并探索其在个性化医疗中的应用。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病预测因多因素病因和高维、噪声数据的复杂性而具有挑战性，LLMs可能提供新的解决方案。

Method: 通过微调LLMs，利用高维基因组数据预测CVD风险和SNPs，采用Chain of Thought（CoT）推理任务生成疾病标签和临床推论。

Result: LLMs在早期检测、风险评估和个性化医疗中显示出潜力。

Conclusion: LLMs有望推动心血管疾病的早期预测和个性化医疗的发展。
...
### [397] [A Globally Optimal Analytic Solution for Semi-Nonnegative Matrix Factorization with Nonnegative or Mixed Inputs](https://arxiv.org/abs/2508.07134)
*Lu Chenggang*
...
<details>
  <summary>Details</summary>
Motivation: 现有半NMF算法多为迭代、非凸且易陷入局部最优，本文旨在提出一种全局最优的非迭代方法。

Method: 基于输入数据的散布矩阵进行正交分解，推导出Frobenius范数下的全局最优解。
...
### [398] [A Stable and Principled Loss Function for Direct Language Model Alignment](https://arxiv.org/abs/2508.07137)
*Yuandong Tan*

Main category: cs.LG

TL;DR: 论文提出了一种新的损失函数，解决了DPO方法中因无限最大化对数差而导致的训练不稳定和奖励黑客问题，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: DPO方法在理论上与其推导不一致，可能导致训练不稳定和奖励黑客行为，因此需要改进。

Method: 从RLHF最优性条件直接推导出新损失函数，针对对数差设定特定有限值，而非最大化。

Result: 新方法避免了DPO的大梯度问题，防止奖励黑客，实验显示在Qwen2.5-7B模型上显著优于DPO基线。

Conclusion: 提出的损失函数更稳定且有效，在模型对齐任务中表现优异。
...
### [399] [Strategic Incentivization for Locally Differentially Private Federated Learning](https://arxiv.org/abs/2508.07138)
*Yashwant Krishna Pagoti,Arunesh Sinha,Shamik Sural*

Main category: cs.LG

TL;DR: 论文研究了联邦学习中隐私与准确性的权衡问题，提出了一种基于令牌的激励机制，通过游戏理论分析优化噪声添加策略。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中因添加噪声保护隐私而导致的模型准确性下降问题。

Method: 将隐私-准确性权衡建模为游戏，引入令牌激励机制，客户端通过添加噪声获取令牌以访问更新后的模型。

Result: 实验验证了不同参数对隐私保护和模型准确性的影响。

Conclusion: 提出的激励机制有效平衡了隐私保护与模型准确性，为联邦学习提供了一种可行的解决方案。
...
### [400] [What One Cannot, Two Can: Two-Layer Transformers Provably Represent Induction Heads on Any-Order Markov Chains](https://arxiv.org/abs/2508.07208)
*Chanakya Ekbote,Marco Bondaschi,Nived Rajaraman,Jason D. Lee,Michael Gastpar,Ashok Vardhan Makkuva,Paul Pu Liang*

Main category: cs.LG

TL;DR: 两层的单头Transformer可以表示任何k阶马尔可夫过程，填补了Transformer深度与ICL能力关系的理论空白。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer深度与ICL能力的关系，特别是两层单头Transformer是否能表示高阶马尔可夫过程。

Method: 理论证明两层单头Transformer可以表示任何条件k-gram，并分析其学习动态。

Result: 证明了两层单头Transformer具备表示高阶马尔可夫过程的能力，揭示了ICL能力的深度依赖性。

Conclusion: 浅层Transformer也能在结构化序列建模任务中表现出强大的ICL能力，深化了对Transformer ICL机制的理解。
...
### [401] [Neural Bridge Processes](https://arxiv.org/abs/2508.07220)
*Jian Xu,Yican Liu,Qibin Zhao,John Paisley,Delu Zeng*

Main category: cs.LG

TL;DR: 本文提出了一种名为Neural Bridge Processes (NBPs)的新方法，用于建模随机函数，通过动态锚定输入x来增强扩散轨迹的耦合性和终点一致性。


<details>
  <summary>Details</summary>
Motivation: 传统方法如高斯过程（GPs）在处理大规模数据时存在可扩展性问题，且假设高斯性限制了其适用性。神经过程（NPs）虽更灵活，但难以捕捉复杂多模态分布。神经扩散过程（NDPs）通过扩散过程增强表达能力，但输入耦合弱且终点语义不匹配。

Method: NBP通过将前向核显式依赖于输入x，约束扩散轨迹严格终止于监督目标，从而增强梯度信号并保证终点一致性。

Result: 在合成数据、EEG信号回归和图像回归任务中，NBP显著优于基线方法。

Conclusion: NBP通过DDPM风格的桥采样，在提升性能和理论一致性方面表现出色。
...
### [402] [LLM-based Agents for Automated Confounder Discovery and Subgroup Analysis in Causal Inference](https://arxiv.org/abs/2508.07221)
...
<details>
  <summary>Details</summary>
Motivation: 观测数据中的个体化治疗效果估计面临未测量混杂和结构偏倚的挑战，现有因果机器学习方法在复杂环境中效果有限，且依赖领域专家导致高成本和可扩展性问题。

Method: 通过将LLM代理集成到因果机器学习流程中，模拟领域专业知识，自动化进行亚组识别和混杂结构发现。

Result: 在真实医疗数据集上的实验表明，该方法通过缩小置信区间和揭示未识别的混杂偏倚，增强了治疗效果估计的稳健性。
...
### [403] [EDGE: A Theoretical Framework for Misconception-Aware Adaptive Learning](https://arxiv.org/abs/2508.07224)
*Ananda Prakash Verma*

Main category: cs.LG

TL;DR: EDGE是一个通用的、能识别误解的自适应学习框架，包含评估、诊断、生成和练习四个阶段，结合了心理测量学、认知诊断和对比项生成技术。


<details>
  <summary>Details</summary>
Motivation: 解决传统自适应学习框架无法有效识别和纠正学习者误解的问题。

Method: 通过IRT/Bayesian模型评估能力，从干扰项和反应时间诊断误解，生成对比项纠正误解，并基于索引策略安排练习。

Result: 提出了EdgeScore指标，证明了其单调性和连续性，并推导出近似最优的索引策略。

Conclusion: EDGE框架理论上能更高效地减少误解，未来需实证验证。
...
### [404] [Causal Negative Sampling via Diffusion Model for Out-of-Distribution Recommendation](https://arxiv.org/abs/2508.07243)
*Chu Zhao,Eneng Yang,Yizhou Dang,Jianzhe Zhao,Guibing Guo,Xingwei Wang*

Main category: cs.LG

TL;DR: 论文提出了一种名为CNSDiff的新方法，通过扩散过程在潜在空间中合成负样本，避免预定义候选池的偏差，并通过因果正则化减少环境混杂因素的影响，显著提升了推荐系统的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有启发式负采样方法可能因环境混杂因素（如曝光或流行度偏差）引入虚假硬负样本，导致模型学习虚假相关性，影响泛化能力。

Method: 提出CNSDiff方法，利用条件扩散过程在潜在空间合成负样本，避免候选池偏差，并加入因果正则化减少混杂因素影响。

Result: 在四种代表性分布偏移场景下，CNSDiff平均性能提升13.96%，验证了其在OOD推荐任务中的有效性和鲁棒性。

Conclusion: CNSDiff通过避免候选池偏差和减少混杂因素影响，显著提升了推荐系统的泛化能力，适用于分布偏移场景。
...
### [405] [Policy Newton methods for Distortion Riskmetrics](https://arxiv.org/abs/2508.07249)
*Soumen Pachal,Mizhaan Prajit Maniyar,Prashanth L. A*

Main category: cs.LG

TL;DR: 论文提出了一种基于强化学习的风险敏感控制方法，通过最大化失真风险度量（DRM）来寻找最优策略，并提出了收敛到二阶稳定点的算法。


<details>
  <summary>Details</summary>
Motivation: 现有文献中，风险敏感目标通常仅收敛到一阶稳定点，而风险中性目标可能收敛到二阶稳定点。本文旨在填补这一空白，提出一种能收敛到风险敏感目标二阶稳定点的方法。

Method: 使用似然比方法推导了DRM目标的策略Hessian定理，并提出了基于样本轨迹的Hessian估计器。进一步提出了一种立方正则化的策略牛顿算法，利用DRM梯度和Hessian估计在策略强化学习框架中求解问题。

Result: 算法被证明能收敛到DRM目标的ε-二阶稳定点（ε-SOSP），且样本复杂度为O(ε^-3.5)。实验验证了理论结果。

Conclusion: 本文首次实现了风险敏感目标收敛到二阶稳定点，填补了现有研究的空白，并通过实验验证了算法的有效性。
...
### [406] [PySeizure: A single machine learning classifier framework to detect seizures in diverse datasets](https://arxiv.org/abs/2508.07253)
*Bartlomiej Chybowski,Shima Abdullateef,Hollan Haule,Alfredo Gonzalez-Sulser,Javier Escudero*

Main category: cs.LG

TL;DR: 论文提出了一种开源机器学习框架，用于跨不同临床数据集的可靠癫痫发作检测，通过自动预处理和多模型投票机制提高鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 癫痫发作检测依赖耗时的手动EEG分析，现有机器学习方法因数据集特定优化而难以推广。

Method: 开发了包含自动预处理和多模型投票机制的框架，评估了其在两个不同EEG数据集上的性能。

Result: 模型在数据集内表现优异（AUC 0.904和0.864），跨数据集泛化能力显著（AUC 0.615和0.762），后处理进一步提升了性能。

Conclusion: 该框架为临床可行的癫痫发作检测系统提供了基础，具有广泛应用的潜力。
...
### [407] [Revisiting Data Attribution for Influence Functions](https://arxiv.org/abs/2508.07297)
*Hongbo Zhu,Angelo Cangelosi*

Main category: cs.LG

TL;DR: 本文综述了影响函数在深度学习中的数据归因能力，探讨其理论基础、高效逆Hessian-向量积估计算法，并评估其在数据归因和错误标签检测中的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过影响函数追踪模型预测到训练数据，以识别最具影响力的训练样本，提升机器学习可解释性、数据调试和模型问责。

Method: 利用影响函数的一阶近似方法，估计数据点对模型参数和预测的边际影响，避免昂贵的重新训练。

Result: 影响函数在数据归因和错误标签检测中表现出高效性，但仍面临大规模实际应用中的挑战。

Conclusion: 影响函数在深度学习中有巨大潜力，但需进一步解决实际应用中的技术难题。
...
### [408] [When Is Prior Knowledge Helpful? Exploring the Evaluation and Selection of Unsupervised Pretext Tasks from a Neuro-Symbolic Perspective](https://arxiv.org/abs/2508.07299)
*Lin-Han Jia,Si-Yu Han,Wen-Chao Hu,Jie-Jing Shao,Wen-Da Wei,Zhi Zhou,Lan-Zhe Guo,Yu-Feng Li*

Main category: cs.LG

TL;DR: 论文通过将神经符号学习理论扩展到不可靠知识（假设）场景，统一了半/自监督学习与神经符号学习的理论框架，并提出了预测无监督任务有效性的方法。


<details>
  <summary>Details</summary>
Motivation: 当前无监督任务选择缺乏理论依据，难以提前评估其合理性，论文旨在解决这一问题。

Method: 基于知识可学习性、可靠性和完整性三个因素，提出理论分析框架，并设计预测方法。

Result: 实验验证了预测性能与实际性能高度相关，证实了理论和评估方法的有效性。

Conclusion: 论文为无监督任务选择提供了理论依据，提升了实践中的效率和效果。
...
### [409] [Efficient Edge LLMs Deployment via HessianAware Quantization and CPU GPU Collaborative](https://arxiv.org/abs/2508.07329)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在资源受限的边缘设备上高效部署面临挑战，尤其是MoE架构中的量化精度和内存管理问题。

Method: 采用Hessian感知量化（HAQ）实现8位量化，设计CPU-GPU协同推理机制优化专家模块调度。

Result: 在OPT系列和Mixtral 8*7B等模型上，量化模型精度接近全精度模型，GPU内存减少60%，推理延迟显著降低。

Conclusion: 该方法有效解决了MoE架构在边缘设备部署中的量化精度和内存管理问题，提升了推理效率。
...
### [410] [Finite-Time Convergence Analysis of ODE-based Generative Models for Stochastic Interpolants](https://arxiv.org/abs/2508.07333)
*Yuhao Liu,Rui Hu,Yu Chen,Longbo Huang*

Main category: cs.LG

TL;DR: 本文研究了基于随机插值的ODE数值实现有限时间收敛性，提出了前向欧拉法和Heun法的误差界，并优化了计算效率。


<details>
  <summary>Details</summary>
Motivation: 随机插值在生成建模中潜力巨大，但数值实现的有限时间收敛性尚未充分研究。

Method: 分析了前向欧拉法和Heun法两种数值积分器的有限时间误差界，并优化了迭代复杂度。

Result: 建立了总变差距离下的误差界，并通过实验验证了理论结果。
...
### [411] [ProteoKnight: Convolution-based phage virion protein classification and uncertainty analysis](https://arxiv.org/abs/2508.07345)
*Samiha Afaf Neha,Abir Ahammed Bhuiyan,Md. Ishrak Khan*

Main category: cs.LG

TL;DR: 论文提出了一种基于图像的编码方法ProteoKnight，用于噬菌体病毒蛋白（PVP）分类，结合预训练卷积神经网络和蒙特卡洛Dropout评估预测不确定性。


<details>
  <summary>Details</summary>
Motivation: 噬菌体病毒蛋白（PVP）的准确预测对基因组研究至关重要，现有方法存在空间信息丢失问题。

Method: ProteoKnight改进DNA-Walk算法，通过像素颜色和步长调整编码蛋白质序列，使用预训练CNN分类，并通过蒙特卡洛Dropout评估不确定性。

Result: 二元分类准确率达90.8%，多分类表现一般；不确定性分析显示预测置信度受蛋白类别和序列长度影响。

Conclusion: ProteoKnight优于现有FCGR方法，提供更准确的PVP预测，并能识别低置信度预测。
...
### [412] [Intrinsic training dynamics of deep neural networks](https://arxiv.org/abs/2508.07370)
*Sibylle Marcotte,Gabriel Peyré,Rémi Gribonval*

Main category: cs.LG

TL;DR: 论文研究了高维参数空间中梯度流的低维简化结构，提出了内在动态性质，并应用于ReLU网络和线性网络。
...
Method: 通过研究梯度流在高维变量θ和低维变量z=ϕ(θ)之间的关系，提出内在动态性质，并基于线性映射的核包含条件给出判定准则。

Result: 证明了对于任意初始化的ReLU网络，梯度流可重写为仅依赖于z和初始化的低维动态；推广了线性网络的平衡初始化条件。

Conclusion: 研究为高维梯度流的低维简化提供了理论支持，并扩展了平衡初始化的适用范围。
...
### [413] [Tight Bounds for Schrödinger Potential Estimation in Unpaired Image-to-Image Translation Problems](https://arxiv.org/abs/2508.07392)
...
<details>
  <summary>Details</summary>
Motivation: 解决在仅能获取初始和目标分布独立同分布样本的情况下，如何通过最优控制理论实现生成建模和图像转换的问题。

Method: 采用Ornstein-Uhlenbeck过程作为参考过程，估计Schrödinger势，并通过Kullback-Leibler散度定义风险函数，推导泛化能力界限。

Result: 在包括高斯混合的Schrödinger势类中，几乎实现了快速收敛速率，数值实验验证了方法的性能。

Conclusion: 提出的方法在生成建模和图像转换中表现出色，泛化能力强，收敛速率快。
...
### [414] [Parity Requires Unified Input Dependence and Negative Eigenvalues in SSMs](https://arxiv.org/abs/2508.07395)
*Behnoush Khavari,Mehran Shakerinava,Jayesh Khullar,Jerry Huang,François Rivest,Siamak Ravanbakhsh,Sarath Chandar*

Main category: cs.LG

TL;DR: 研究发现，LRNN模型（如S4D、Mamba和DeltaNet）因时间不变转移矩阵或受限特征值范围而缺乏状态跟踪能力。输入依赖的转移矩阵被提出以提升性能，但多层SSM结合输入独立和非负SSM仍无法解决简单任务（如奇偶校验）。


<details>
  <summary>Details</summary>
Motivation: 探讨多层SSM结合输入独立和非负SSM是否能解决状态跟踪任务，特别是奇偶校验问题。

Method: 研究具有对角转移矩阵的高效SSM，分析其组合是否能解决奇偶校验任务。

Result: 实验表明，即使组合输入独立和非负SSM，仍无法解决奇偶校验任务。

Conclusion: 状态跟踪任务需要输入依赖且包含负特征值的递归层。
...
### [415] [Efficient Reward Identification In Max Entropy Reinforcement Learning with Sparsity and Rank Priors](https://arxiv.org/abs/2508.07400)
...
<details>
  <summary>Details</summary>
Motivation: 奖励函数恢复问题在缺乏额外假设时高度不适定，但实际应用中奖励函数通常具有稀疏性或可表示为少量特征的线性组合。

Method: 1）将奖励函数稀疏化问题转化为带线性约束的稀疏化问题，提出多项式时间精确算法；2）将最小特征数奖励识别问题转化为带线性约束的秩最小化问题，利用凸松弛求解。

Result: 提出的算法能高效恢复奖励函数，并通过示例验证了恢复的准确性和泛化性。

Conclusion: 通过先验假设和优化方法，论文解决了奖励函数恢复问题，并提供了高效算法。
...
### [416] [Lightning Prediction under Uncertainty: DeepLight with Hazy Loss](https://arxiv.org/abs/2508.07428)
...
<details>
  <summary>Details</summary>
Motivation: 闪电对人和经济造成重大风险，现有预测模型在动态空间上下文和不确定性处理上存在不足。

Method: 利用雷达反射率、云属性和历史闪电数据，采用双编码器架构和多分支卷积技术，引入Hazy Loss函数处理不确定性。

Result: 实验表明，DeepLight在公平威胁分数（ETS）上比现有方法提升18%-30%。
...
### [417] [Unsupervised operator learning approach for dissipative equations via Onsager principle](https://arxiv.org/abs/2508.07440)
*Zhipeng Chang,Zhenye Wen,Xiaofei Zhao*

Main category: cs.LG

TL;DR: 提出了一种名为DOOL的无监督学习方法，用于解决耗散方程，避免了高成本的高保真模拟数据需求。


<details>
  <summary>Details</summary>
Motivation: 现有算子学习方法依赖高保真模拟数据，计算成本高，DOOL旨在通过无监督学习降低这一成本。

Method: 基于Onsager变分原理，直接最小化Rayleighian泛函训练深度算子网络，无需标记数据，并通过时空解耦策略提高效率。

Result: 数值实验验证了DOOL的有效性，其性能优于监督学习的DeepONet和MIONet。

Conclusion: DOOL为耗散方程提供了一种高效的无监督学习框架，并可扩展至不直接遵循OVP的二阶波动模型。
...
### [418] [Stackelberg Coupling of Online Representation Learning and Reinforcement Learning](https://arxiv.org/abs/2508.07452)
*Fernando Martinez,Tao Li,Yingdong Lu,Juntao Chen*

Main category: cs.LG

TL;DR: SCORER框架通过Stackelberg博弈结构化感知与控制网络的交互，提升深度强化学习的样本效率和性能。
...
### [419] [Towards Unveiling Predictive Uncertainty Vulnerabilities in the Context of the Right to Be Forgotten](https://arxiv.org/abs/2508.07458)
...
<details>
  <summary>Details</summary>
Motivation: 随着对遗忘权的需求增长，机器遗忘被广泛研究，但预测不确定性在此类攻击中的脆弱性尚未被探索。

Method: 设计了针对预测不确定性的恶意遗忘攻击优化框架，并在黑盒场景下进行了广泛实验。

Result: 实验表明，该攻击在操纵预测不确定性方面比传统标签误分类攻击更有效，且现有防御措施无效。

Conclusion: 本文填补了预测不确定性在恶意遗忘攻击中的研究空白，并展示了其潜在威胁。
...
### [420] [MOTGNN: Interpretable Graph Neural Networks for Multi-Omics Disease Classification](https://arxiv.org/abs/2508.07465)
*Tiantian Yang,Zhiqian Chen*

Main category: cs.LG

TL;DR: MOTGNN是一种新颖的多组学整合框架，用于疾病分类，通过XGBoost和GNN实现高效建模，显著提升预测性能并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 多组学数据整合能全面揭示疾病机制，但其高维性和复杂交互为建模带来挑战。

Method: MOTGNN结合XGBoost构建监督图，使用GNN进行分层表示学习，并通过深度前馈网络整合跨组学数据。

Result: 在三个真实疾病数据集上，MOTGNN在准确率、ROC-AUC和F1分数上优于基线5-10%，且对类别不平衡鲁棒。

Conclusion: MOTGNN在多组学疾病建模中兼具预测准确性和可解释性，具有广泛应用潜力。
...
### [421] [Online Convex Optimization with Heavy Tails: Old Algorithms, New Regrets, and Applications](https://arxiv.org/abs/2508.07473)
*Zijian Liu*

Main category: cs.LG

TL;DR: 本文研究了在线凸优化（OCO）中梯度估计具有重尾分布时的问题，证明了经典算法（如在线梯度下降）无需修改即可实现最优遗憾界，并扩展了其在非光滑非凸优化中的应用。


<details>
  <summary>Details</summary>
Motivation: 在OCO中，梯度估计通常假设为有限方差，但实际中可能存在重尾分布（即梯度仅具有有限p阶中心矩）。本文旨在探索经典算法在此更具挑战性场景下的表现。

Method: 分析了经典OCO算法（如在线梯度下降）在重尾梯度下的性能，未对算法做任何修改，仅基于标准有界域假设。

Result: 证明了这些算法在重尾梯度下仍能实现完全最优的遗憾界，且无需额外操作（如梯度裁剪）。

Conclusion: 研究表明，重尾梯度下的OCO问题可通过经典算法有效解决，且结果适用于更广泛的场景（如非光滑非凸优化和乐观算法）。
...
### [422] [N-BEATS-MOE: N-BEATS with a Mixture-of-Experts Layer for Heterogeneous Time Series Forecasting](https://arxiv.org/abs/2508.07490)
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法（如N-BEATS）在时间序列预测中表现优异，但仍有改进空间，特别是在适应不同时间序列特性和增强可解释性方面。

Method: N-BEATS-MOE在N-BEATS基础上加入Mixture-of-Experts层，利用门控网络动态调整块权重，以适应不同时间序列的特性。

Result: 在12个基准数据集上的实验表明，N-BEATS-MOE在多个数据集上表现优于其他方法，尤其是在异质性时间序列上。

Conclusion: N-BEATS-MOE通过动态门控机制提升了模型的适应性和可解释性，为时间序列预测提供了更优的解决方案。
...
### [423] [Enhancing Privacy in Decentralized Min-Max Optimization: A Differentially Private Approach](https://arxiv.org/abs/2508.07505)
...
<details>
  <summary>Details</summary>
Motivation: 分散式最小-最大优化中，模型更新共享可能导致敏感数据泄露，差分隐私虽能保护隐私，但噪声添加可能影响收敛。

Method: 基于STORM算法，设计DPMixSGD算法，在局部梯度中添加噪声，并通过理论证明其不影响收敛。

Result: 实验验证了DPMixSGD在多种任务和模型中的有效性，同时确保隐私保护。

Conclusion: DPMixSGD成功解决了隐私与收敛性能的权衡问题，适用于非凸分散式最小-最大优化。
...
### [424] [FairDRL-ST: Disentangled Representation Learning for Fair Spatio-Temporal Mobility Prediction](https://arxiv.org/abs/2508.07518)
...
<details>
  <summary>Details</summary>
Motivation: 随着深度时空神经网络在城市计算中的应用增加，其预测偏差可能加剧社会经济不平等。现有方法多关注准确性，而公平性成为新焦点。

Method: 通过对抗学习和解耦表示学习，框架分离敏感信息属性，以无监督方式实现公平性，避免性能损失。

Result: 在真实城市移动数据集上验证，框架能缩小公平性差距，同时保持与最先进公平感知方法相当的预测性能。

Conclusion: FairDRL-ST为时空预测中的公平性问题提供了有效解决方案，平衡了公平性与性能。
...
### [425] [Physics-Informed Multimodal Bearing Fault Classification under Variable Operating Conditions using Transfer Learning](https://arxiv.org/abs/2508.07536)
...
<details>
  <summary>Details</summary>
Motivation: 解决在变工况下轴承故障分类的准确性和可解释性问题，避免领域偏移对模型性能的影响。

Method: 采用多模态CNN架构，融合振动和电流信号，并引入物理特征提取分支和物理信息损失函数。评估了三种迁移学习策略。

Result: 在Paderborn数据集上表现优于基线，最高准确率达98%，迁移学习策略LAS表现最佳。

Conclusion: 物理信息与数据驱动结合可提升故障诊断的鲁棒性和泛化能力，适用于工业应用。
...
### [426] [Multimodal Remote Inference](https://arxiv.org/abs/2508.07555)
...
<details>
  <summary>Details</summary>
Motivation: 由于网络资源有限，实时传递所有模态的特征不可行，需优化调度以最小化推理误差。

Method: 开发索引阈值策略，证明其最优性，适用于非单调、非加性AoI函数和异构传输时间。

Result: 相比轮询和随机策略，新策略减少推理误差高达55%。

Conclusion: 优化面向任务的AoI函数可显著提升远程推理准确性。
...
### [427] [Towards Theoretical Understanding of Transformer Test-Time Computing: Investigation on In-Context Linear Regression](https://arxiv.org/abs/2508.07571)
*Xingwu Chen,Miao Lu,Beining Wu,Difan Zou*

Main category: cs.LG

TL;DR: 论文探讨了在语言模型推理中增加测试时间计算（如生成更多中间思考或采样多个候选答案）的有效性，并通过结合随机性和采样，初步缩小了实际推理与理论分析之间的差距。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于通过结合随机性和采样，填补实际语言模型推理与理论分析之间的空白，特别是在上下文线性回归任务中。

Method: 方法包括通过噪声注入和二元系数采样模拟语言模型解码，并分析广泛采用的推理技术。

Result: 实证结果支持的理论框架和分析为理解实际语言模型中的推理行为提供了新视角。

Conclusion: 结论表明，该框架和分析方法有助于更深入地理解语言模型的推理行为，并为未来研究提供了基础。
...
### [428] [When and how can inexact generative models still sample from the data manifold?](https://arxiv.org/abs/2508.07581)
*Nisha Chandramoorthy,Adriaan de Clercq*

Main category: cs.LG

TL;DR: 论文研究了生成模型中学习误差对数据分布支持的鲁棒性现象，揭示了误差仅影响数据流形上的密度预测，并提出了动态机制和充分条件。


<details>
  <summary>Details</summary>
Motivation: 探索生成模型中学习误差导致样本沿数据分布支持移动而非远离的现象，理解其鲁棒性机制。

Method: 采用动态系统方法，分析概率流的扰动，研究Lyapunov向量与数据流形切空间的对齐。

Result: 发现学习误差仅影响数据流形上的密度预测，并证明了动态机制的对齐条件。
...
### [429] [Klear-Reasoner: Advancing Reasoning Capability via Gradient-Preserving Clipping Policy Optimization](https://arxiv.org/abs/2508.07629)
*Zhenpeng Su,Leiyu Pan,Xue Bai,Dening Liu,Guanting Dong,Jiaming Huang,Wenping Hu,Guorui Zhou*

Main category: cs.LG

TL;DR: Klear-Reasoner是一个具有长推理能力的模型，通过详细的数据准备、长链思维监督微调和强化学习流程，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型的训练细节披露不完整，导致高性能模型难以复现。本文旨在提供完整的训练流程分析和改进方法。

Method: 采用长链思维监督微调（long CoT SFT）和强化学习（RL），并提出梯度保留裁剪策略优化（GPPO）解决RL中的裁剪问题。

Result: Klear-Reasoner在数学和编程任务中表现优异，如AIME 2024（90.5%）和LiveCodeBench V5（66.0%）。

Conclusion: 高质量数据和小样本难例训练更有效，GPPO提升了模型的探索能力和学习效率。
...
### [430] [Efficient Approximate Posterior Sampling with Annealed Langevin Monte Carlo](https://arxiv.org/abs/2508.07631)
*Advait Parulekar,Litu Rout,Karthikeyan Shanmugam,Sanjay Shakkottai*

Main category: cs.LG

TL;DR: 论文研究了基于分数的生成模型中的后验采样问题，提出了一种在多项式时间内近似采样后验分布的方法，确保样本与测量和先验一致。


<details>
  <summary>Details</summary>
Motivation: 尽管后验采样在KL散度下通常是难解的，但实际应用中许多算法仍能成功。本文旨在探索一种更通用的方法，无需严格假设即可实现近似后验采样。

Method: 将问题视为对分布进行“倾斜”调整，提出了一种方法，能够在最小假设下采样一个分布，该分布同时接近噪声先验的KL散度和真实后验的Fisher散度。

Result: 证明了在多项式时间内可以近似采样后验分布，确保样本与测量和先验一致。这是首次在多项式时间内实现近似后验采样的正式结果。

Conclusion: 本文提出了一种通用的后验采样方法，为基于分数的生成模型提供了理论支持，并在实际应用中具有潜力。
...
### [431] [Attribution Explanations for Deep Neural Networks: A Theoretical Perspective](https://arxiv.org/abs/2508.07636)
*Huiqi Deng,Hongbin Pei,Quanshi Zhang,Mengnan Du*

Main category: cs.LG

TL;DR: 论文探讨了深度神经网络（DNN）归因解释方法的可靠性问题，总结了三个核心挑战，并提出了三个关键方向来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 归因解释方法在解释DNN决策时存在可靠性问题，缺乏统一的理论基础和评估标准，影响了其实际应用价值。

Method: 通过理论统一、理论基础和理论评估三个方向，系统地分析和比较现有归因方法。

Result: 总结了现有研究的进展，揭示了归因方法的共性和差异，并提出了解决可靠性问题的理论框架。

Conclusion: 论文为归因方法的理论理解、方法选择和新方法设计提供了见解，并指出了未来研究的开放性问题。
...
### [432] [Extracting Complex Topology from Multivariate Functional Approximation: Contours, Jacobi Sets, and Ridge-Valley Graphs](https://arxiv.org/abs/2508.07637)
...
<details>
  <summary>Details</summary>
Motivation: 连续隐式模型（如MFA）为科学数据提供了连续、高阶和可微的表示方法，但缺乏直接提取拓扑特征的工具。

Method: 基于MFA模型，直接提取轮廓线、雅可比集和脊谷图等拓扑特征，无需离散化。

Result: 实现了从连续隐式模型中直接提取复杂拓扑特征，适用于支持函数值和高阶导数查询的任何连续隐式模型。
...
### [433] [Beyond Single: A Data Selection Principle for LLM Alignment via Fine-Grained Preference Signals](https://arxiv.org/abs/2508.07638)
*Jia Zhang,Yao Liu,Chen-Xi Zhang,Yi Liu,Yi-Xuan Jin,Lan-Zhe Guo,Yu-Feng Li*

Main category: cs.LG

TL;DR: 论文提出了一种名为DMPO的方法，通过量化偏好冲突并选择高共识数据，优化LLM对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如DPO）在处理细粒度偏好数据时存在噪声和冲突问题，需要更有效的数据选择策略。

Method: 提出DMPO目标，引入偏好分歧（PD）项，并基于此设计数据选择原则，选择高共识数据用于训练。

Result: 在UltraFeedback数据集上，该方法相对改进超过10%，同时提升训练效率。
...
### [434] [Multi-Turn Jailbreaks Are Simpler Than They Seem](https://arxiv.org/abs/2508.07646)
*Xiaoxue Yang,Jaeha Lee,Anna-Katharina Dick,Jasper Timm,Fei Xie,Diogo Cruz*

Main category: cs.LG

TL;DR: 多轮越狱攻击的成功率高达70%，研究发现其复杂性被高估，攻击效果与单轮攻击多次重采样相当。


<details>
  <summary>Details</summary>
Motivation: 揭示多轮越狱攻击的实际效果及其对AI安全评估的影响。

Method: 使用StrongREJECT基准对GPT-4、Claude和Gemini等模型进行实证分析。

Result: 多轮攻击成功率与单轮攻击多次重采样相当，攻击效果在相似模型间具有相关性。

Conclusion: 研究结果对AI安全评估和抗越狱系统设计具有重要意义。
...
### [435] [Discovering Spatial Correlations between Earth Observations in Global Atmospheric State Estimation by using Adaptive Graph Structure Learning](https://arxiv.org/abs/2508.07659)
*Hyeon-Ju Jeon,Jeon-Ho Kang,In-Hyuk Kwon,O-Joun Lee*

Main category: cs.LG

TL;DR: 研究旨在通过时空图神经网络（STGNN）和结构学习，动态处理地球观测与大气状态间的空间相关性，提升全球大气状态估计的预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报（NWP）系统在固定网格点上预测大气状态，但地球观测数据位置不固定，导致空间相关性复杂且动态变化。

Method: 采用STGNN结合结构学习，并通过自适应节点度数和空间距离调节边采样，避免结构信息丢失和过平滑问题。

Result: 在东亚真实数据验证中，该方法在高变异性区域优于现有STGNN模型（无论是否使用结构学习）。
...
### [436] [GLiClass: Generalist Lightweight Model for Sequence Classification Tasks](https://arxiv.org/abs/2508.07662)
*Ihor Stepanov,Mykhailo Shtopko,Dmytro Vodianytskyi,Oleksandr Lukashov,Alexander Yavorskyi,Mykyta Yaroshenko*

Main category: cs.LG

TL;DR: GLiClass是一种新型分类方法，结合了高效性和灵活性，适用于零样本和小样本学习场景。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要高效且准确的分类方法，同时适应动态需求，但现有方法如生成式LLM和交叉编码器存在效率或灵活性不足的问题。

Method: GLiClass基于GLiNER架构，适用于序列分类任务，并采用近端策略优化（PPO）进行多标签文本分类训练。

Result: GLiClass在准确性和效率上与基于嵌入的方法相当，同时保持了零样本和小样本学习的灵活性。

Conclusion: GLiClass为动态分类需求提供了一种高效且灵活的解决方案，尤其在数据稀疏或需要人类反馈的场景中表现出色。
...
### [437] [AIS-LLM: A Unified Framework for Maritime Trajectory Prediction, Anomaly Detection, and Collision Risk Assessment with Explainable Forecasting](https://arxiv.org/abs/2508.07668)
*Hyobin Park,Jinwook Jung,Minseok Seo,Hyunsoo Choi,Deukjae Cho,Sekil Park,Dong-Geol Choi*

Main category: cs.LG

TL;DR: 论文提出了一种名为AIS-LLM的新框架，结合时间序列AIS数据与大语言模型（LLM），以同时处理船舶轨迹预测、异常检测和碰撞风险评估三项任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常单独处理海事分析任务，难以全面考虑复杂海事情况，因此需要一种更综合的解决方案。

Method: AIS-LLM框架包括时间序列编码器、基于LLM的提示编码器、跨模态对齐模块和多任务解码器，实现端到端的多任务处理。

Result: 实验表明，AIS-LLM在各项任务上均优于现有方法，并能生成综合情境摘要。
...
### [438] [Semantic Caching for Low-Cost LLM Serving: From Offline Learning to Online Adaptation](https://arxiv.org/abs/2508.07675)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的高推理成本带来了可扩展性和可持续性挑战，传统缓存方法无法有效利用语义相似性，且现有方法缺乏理论基础和适应性。

Method: 提出了一个基于学习的框架，包括离线优化和在线学习两种问题形式，并开发了具有理论保证的高效算法。

Result: 在合成数据集上的评估表明，所提算法性能优于或匹配基线方法。

Conclusion: 该框架为语义缓存问题提供了理论基础和实际解决方案，能够适应未知的查询和成本分布。
...
### [439] [Multi-Hop Privacy Propagation for Differentially Private Federated Learning in Social Networks](https://arxiv.org/abs/2508.07676)
*Chenchen Lin,Xuehe Wang*

Main category: cs.LG

TL;DR: 本文提出了一种社交感知的隐私保护联邦学习机制，通过多跳传播模型量化间接隐私泄露，采用Stackelberg博弈优化激励策略，并通过均值场估计器解决信息不对称问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中社交网络引入的隐私外部性导致客户隐私损失不仅取决于自身策略，还受他人决策影响，需系统性解决方案。

Method: 提出多跳传播模型量化隐私泄露，采用Stackelberg博弈设计激励机制，引入均值场估计器近似外部隐私风险。

Result: 理论证明了均值场估计器的收敛性，并推导了Stackelberg纳什均衡的闭式解，实验显示方法显著提升客户效用并降低服务器成本。

Conclusion: 该机制在保持模型性能的同时优化社会福利，优于社交无关基线和其他考虑社交外部性的方法。
...
### [440] [MORE-CLEAR: Multimodal Offline Reinforcement learning for Clinical notes Leveraged Enhanced State Representation](https://arxiv.org/abs/2508.07681)
*Yooseok Lim,ByoungJun Jeon,Seong-A Park,Jisoo Lee,Sae Won Choi,Chang Wook Jeong,Ho-Geol Ryu,Hongyeol Lee,Hyun-Lim Yang*

Main category: cs.LG

TL;DR: MORE-CLEAR框架利用预训练语言模型和多模态数据提升脓毒症管理的强化学习性能。


<details>
  <summary>Details</summary>
Motivation: 脓毒症的早期检测和优化管理至关重要，现有方法依赖结构化数据且缺乏对患者状态的全面理解。

Method: 结合临床笔记的语义表示和多模态数据，通过门控融合和跨模态注意力动态调整权重。

Result: 在公开和私有数据集上验证，显著提升生存率和策略性能。

Conclusion: 首次将LLM用于多模态离线强化学习，为脓毒症治疗提供更全面的患者状态表示。
...
### [441] [Semantic-Enhanced Time-Series Forecasting via Large Language Models](https://arxiv.org/abs/2508.07697)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注令牌级模态对齐，未能弥合语言知识结构与时间序列数据模式之间的模态差距，限制了语义表示。

Method: 提出SE-LLM，将时间序列的周期性和异常特征嵌入语义空间，增强令牌嵌入；同时设计了一个插件模块，改进LLM对长短期依赖的建模。

Result: 实验表明，SE-LLM在性能上优于现有最先进方法。

Conclusion: SE-LLM通过语义增强和插件模块设计，显著提升了LLM在时间序列分析中的表现和效率。
...
### [442] [Energy Consumption in Parallel Neural Network Training](https://arxiv.org/abs/2508.07706)
...
TL;DR: 论文研究了数据并行训练中并行化参数对能耗的影响，发现能耗与GPU小时数近似线性相关，但具体因素因模型和硬件而异。
...
Method: 通过数据并行训练ResNet50和FourCastNet模型，评估GPU数量、全局批次大小和局部批次大小对性能、训练时间和能耗的影响。

Result: 能耗与GPU小时数近似线性相关，但具体比例因模型和硬件不同，且受每GPU小时的样本和梯度更新数量影响。
...
### [443] [Training-Free ANN-to-SNN Conversion for High-Performance Spiking Transformer](https://arxiv.org/abs/2508.07710)
*Jingya Wang,Xin Deng,Wenjie Wei,Dehao Zhang,Shuai Wang,Qian Sun,Jieyuan Zhang,Hanwen Liu,Ning Xie,Malu Zhang*

Main category: cs.LG

TL;DR: 提出了一种针对Transformer架构的高性能、无需训练的ANN-to-SNN转换框架，通过MBE神经元有效解决非线性操作问题。


<details>
  <summary>Details</summary>
Motivation: 现有ANN-to-SNN转换方法在Transformer架构中处理非线性操作效果不佳，且需额外微调预训练ANN。

Method: 引入多基指数衰减（MBE）神经元，采用指数衰减策略和多基编码方法近似非线性操作，无需修改预训练ANN权重。

Result: 在多种任务和主流Transformer架构上实现近乎无损的转换精度，且延迟显著降低。

Conclusion: 为Spiking Transformer的高效、可扩展部署提供了可行方案。
...
### [444] [Detecting Mislabeled and Corrupted Data via Pointwise Mutual Information](https://arxiv.org/abs/2508.07713)
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易记忆损坏的标签，而现实数据集常受标签噪声和输入噪声的影响，数据质量对模型性能至关重要。

Method: 计算每个样本对整体互信息的点贡献，贡献较低的样本被视为噪声或错误标记。
...
### [445] [Robust Reinforcement Learning over Wireless Networks with Homomorphic State Representations](https://arxiv.org/abs/2508.07722)
*Pietro Talli,Federico Mason,Federico Chiariotti,Andrea Zanella*

Main category: cs.LG

TL;DR: 提出了一种名为HR3L的新架构，用于在非理想无线信道上训练远程强化学习（RL）代理，解决了传统方法中反馈不完美和计算负担重的问题。


<details>
  <summary>Details</summary>
Motivation: 传统RL方法在无线通信网络中因状态更新延迟或丢失而难以实现即时感知，现有解决方案计算负担重且不够通用。

Method: HR3L架构包含编码环境信息的发射器和解码并执行动作的接收器，无需交换梯度信息，降低了通信开销。

Result: 实验表明HR3L在样本效率和适应不同通信场景（如丢包、延迟和容量限制）方面显著优于基线方法。

Conclusion: HR3L为无线通信网络中的RL训练提供了一种高效且适应性强的解决方案。
...
### [446] [Separation and Collaboration: Two-Level Routing Grouped Mixture-of-Experts for Multi-Domain Continual Learning](https://arxiv.org/abs/2508.07738)
...
<details>
  <summary>Details</summary>
Motivation: 多领域持续学习面临任务类别和分布变化的双重异质性挑战，现有方法存在灾难性遗忘和前瞻性遗忘问题。

Method: TRGE方法动态扩展CLIP模型，为每个任务分配专家组，并设计两级路由策略（组内路由和基于任务标识符的组间路由），同时利用MLLMs生成任务描述以识别任务标识符。

Result: 实验表明，TRGE在多种设置下优于其他先进方法，且参数效率更高。

Conclusion: TRGE通过动态扩展和两级路由策略，有效解决了多领域持续学习中的遗忘问题，提升了模型性能。
...
### [447] [A Tutorial: An Intuitive Explanation of Offline Reinforcement Learning Theory](https://arxiv.org/abs/2508.07746)
*Fengdi Che*

Main category: cs.LG

TL;DR: 本文综述了离线强化学习的理论见解及其对算法设计的实际影响，探讨了数据覆盖和函数表示条件，并分析了离线RL的固有挑战和解决途径。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习（RL）旨在通过固定数据集优化回报，但理论见解与实际算法设计之间存在差距。本文旨在弥合这一差距。

Method: 通过分析理论条件（如函数表示和数据覆盖假设）、反例及解决挑战的技术，探讨离线RL的可行性。

Result: 揭示了离线RL的固有局限性，并提出了满足条件时的解决方案。

Conclusion: 理论条件不仅用于证明，还揭示了算法局限性，提醒在条件不满足时寻找新方法。
...
### [448] [Learning to Align, Aligning to Learn: A Unified Approach for Self-Optimized Alignment](https://arxiv.org/abs/2508.07750)
...
TL;DR: GRAO（Group Relative Alignment Optimization）是一种结合SFT和RL优势的统一框架，通过多样本生成、组内相对优势加权和参考感知参数更新，显著提升了语言模型的对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如SFT和RL）在语言模型对齐中存在局限性：SFT受限于离线策略轨迹，RL则样本效率低且依赖高质量基础模型。GRAO旨在解决这些问题。

Method: GRAO提出三项创新：1）多样本生成策略；2）组内相对优势加权的对齐损失；3）基于成对偏好的参考感知参数更新。

Result: GRAO在复杂人类对齐任务中表现优异，相对SFT、DPO、PPO和GRPO分别提升57.70%、17.65%、7.95%和5.18%。
...
### [449] [Sparse Probabilistic Graph Circuits](https://arxiv.org/abs/2508.07763)
*Martin Rektoris,Milan Papež,Václav Šmídl,Tomáš Pevný*

Main category: cs.LG

TL;DR: 论文提出了一种稀疏概率图电路（SPGCs），解决了传统深度生成模型（DGMs）在计算复杂度上的问题，适用于稀疏图表示，显著提升了效率和性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度生成模型（DGMs）虽然表达能力强大，但由于非线性特性导致概率推断不可解，且计算复杂度高（O(n^2)）。稀疏图表示的需求促使研究者提出更高效的解决方案。

Method: 引入稀疏概率图电路（SPGCs），直接操作稀疏图表示，将复杂度降至O(n + m)，适用于边数远小于节点数平方的稀疏图。

Result: 实验证明，SPGCs在药物设计中保留了精确推断能力，提升了内存效率和推断速度，并在关键指标上与不可解DGMs性能相当。

Conclusion: SPGCs为稀疏图的高效生成和推断提供了可行的解决方案，适用于实际应用场景。
...
### [450] [Pareto Multi-Objective Alignment for Language Models](https://arxiv.org/abs/2508.07768)
*Qiang He,Setareh Maghsudi*

Main category: cs.LG

TL;DR: 论文提出了一种名为PAMA的高效多目标对齐算法，解决了大语言模型（LLMs）在多目标优化中的复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于RLHF的对齐方法只能优化单一目标，无法捕捉人类偏好的多样性和复杂性，限制了LLMs在实际场景中的适应性。

Method: PAMA将多目标RLHF转化为凸优化问题，提供闭式解，显著降低了计算复杂度（从O(n^2*d)降至O(n)）。

Result: 实验证明PAMA在125M到7B参数的模型中均能有效实现多目标对齐，且理论保证其收敛到Pareto稳定点。

Conclusion: PAMA为多目标对齐提供了高效且理论可靠的解决方案，推动了LLMs在多样化实际场景中的应用。
...
### [451] [Topological Feature Compression for Molecular Graph Neural Networks](https://arxiv.org/abs/2508.07807)
*Rahul Khorana*
...
<details>
  <summary>Details</summary>
Motivation: 解决分子表示学习中提取化学洞察时平衡预测准确性、可解释性和计算效率的挑战。

Method: 引入结合压缩高阶拓扑信号与标准分子特征的GNN架构，捕捉全局几何信息。

Result: 在多个基准测试中表现优异，准确性和鲁棒性均领先。

Conclusion: 该架构在分子表示学习中具有高效性和可解释性，代码已开源。
...
### [452] [EvoCoT: Overcoming the Exploration Bottleneck in Reinforcement Learning](https://arxiv.org/abs/2508.07809)
*Huanyu Liu,Jia Li,Chang Yu,Taozhi Chen,Yihong Dong,Lecheng Wang,Hu XiaoLong,Ge Li*

Main category: cs.LG

TL;DR: EvoCoT是一种基于两阶段思维链推理优化的自演化课程学习框架，用于解决强化学习中稀疏奖励导致的探索瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，当面对困难问题时，奖励稀疏会限制学习效率和探索能力，现有方法依赖更强的LLM或过滤难题，限制了可扩展性和推理能力的提升。

Method: EvoCoT通过自生成和验证思维链轨迹来约束探索空间，并逐步缩短轨迹以可控方式扩展空间，使LLM能在稀疏奖励下稳定学习。

Result: 实验表明，EvoCoT使LLM能解决之前未解决的问题，无需外部思维链监督即可提升推理能力，并与多种RL微调方法兼容。

Conclusion: EvoCoT为LLM在稀疏奖励环境下的推理能力提升提供了一种有效且可扩展的解决方案。
...
### [453] [Learning Satellite Attitude Dynamics with Physics-Informed Normalising Flow](https://arxiv.org/abs/2508.07841)
*Carlo Cena,Mauro Martini,Marcello Chiaberge*

Main category: cs.LG

TL;DR: 论文研究了将物理信息神经网络（PINNs）引入航天器姿态动力学学习，相比纯数据驱动方法，显著提升了性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 在航天器姿态控制中，物理模型可能不完整或计算成本高，而纯数据驱动方法泛化性和稳定性不足。

Method: 使用Real NVP神经网络架构和自注意力机制，结合Basilisk模拟器生成的数据，比较纯数据驱动和物理信息驱动的训练策略。

Result: 物理信息模型的平均相对误差降低了27.08%，在MPC框架中控制精度和鲁棒性提升高达42.86%。

Conclusion: 物理信息的引入显著提升了航天器姿态控制的性能和稳定性，尤其在MPC框架中表现优越。
...
### [454] [Not Yet AlphaFold for the Mind: Evaluating Centaur as a Synthetic Participant](https://arxiv.org/abs/2508.07887)
*Sabrina Namazova,Alessandra Brondetta,Younes Strittmatter,Matthew Nassar,Sebastian Musslick*

Main category: cs.LG

TL;DR: 论文探讨了模拟器在科学实践中的重要性，尤其是行为科学中的参与者模拟器，并评估了Centaur模型的表现。


<details>
  <summary>Details</summary>
Motivation: 模拟器在自然科学中已取得显著成果，如AlphaFold。行为科学中，可靠的参与者模拟器（如Centaur）可能带来类似变革。

Method: 评估Centaur作为参与者模拟器的核心标准，尤其是其生成行为与人类数据的对比。
...
### [455] [Score Augmentation for Diffusion Models](https://arxiv.org/abs/2508.07926)
*Liang Hou,Yuan Gao,Boyuan Jiang,Xin Tao,Qi Yan,Renjie Liao,Pengfei Wan,Di Zhang,Kun Gai*

Main category: cs.LG

TL;DR: 本文提出了一种名为ScoreAug的新型数据增强框架，专门针对扩散模型中的过拟合问题，通过在噪声数据上应用变换并建立等变学习目标，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成建模中取得了显著成功，但在数据有限的情况下存在过拟合问题。为了解决这一问题，作者提出了ScoreAug框架。

Method: ScoreAug在噪声数据上应用变换，与扩散模型的内在去噪机制对齐，并要求去噪器预测原始目标的增强，从而建立等变学习目标。

Result: 实验在多个基准数据集上验证了ScoreAug的有效性，结果显示其显著优于基线方法，并能有效缓解过拟合问题。

Conclusion: ScoreAug不仅能够与传统数据增强技术结合以进一步提升性能，还能避免某些条件下的数据泄漏问题。
...
### [456] [Adaptive Fine-Tuning via Pattern Specialization for Deep Time Series Forecasting](https://arxiv.org/abs/2508.07927)
*Amal Saadallah,Abdulaziz Al-Ademi*

Main category: cs.LG

TL;DR: 提出了一种通过模型适应和选择提升DNN性能的框架，用于非平稳环境中的时间序列预测。
...
Method: 离线训练基础DNN，分割验证子集以聚类主导模式，为每个聚类微调DNN，并在推理时根据相似度选择模型。
...
### [457] [Shapley-Inspired Feature Weighting in $k$-means with No Additional Hyperparameters](https://arxiv.org/abs/2508.07952)
*Richard J. Fawley,Renato Cordeiro de Amorim*

Main category: cs.LG

TL;DR: SHARK是一种基于Shapley值的特征加权聚类算法，无需额外参数调优，通过分解k-means目标函数为Shapley值，高效计算特征相关性，提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 传统聚类算法假设所有特征对数据结构的贡献均等，但在高维或噪声场景下不成立。现有特征加权方法需额外参数调优，SHARK旨在解决这一问题。

Method: SHARK利用Shapley值量化特征相关性，将k-means目标函数分解为各特征的Shapley值之和，通过迭代重加权特征提升聚类效果。

Result: 实验表明，SHARK在合成和真实数据集上优于现有方法，尤其在噪声场景下表现出更强的鲁棒性和准确性。

Conclusion: SHARK为无监督特征相关性提供了理论基础，并通过高效计算实现了性能提升，适用于复杂数据场景。
...
### [458] [WeChat-YATT: A Simple, Scalable and Balanced RLHF Trainer](https://arxiv.org/abs/2508.07970)
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF训练框架在控制器可扩展性和复杂RLHF流水线编排方面存在局限性，尤其是在动态采样和资源分配场景中。

Method: 提出WeChat-YATT框架，采用并行控制器编程模型和动态资源分配策略，优化RLHF工作流。

Result: 实验表明，WeChat-YATT在吞吐量上显著优于现有RLHF框架，并成功应用于微信产品模型训练。

Conclusion: WeChat-YATT有效解决了RLHF训练中的可扩展性和效率问题，适用于大规模实际应用。
...
### [459] [A Physics-informed Deep Operator for Real-Time Freeway Traffic State Estimation](https://arxiv.org/abs/2508.08002)
*Hongxin Yu,Yibing Wang,Fengyue Jin,Meng Zhang,Anni Chen*

Main category: cs.LG

TL;DR: 本文提出了一种基于物理信息深度算子网络（PI-DeepONet）的实时高速公路交通状态估计方法，通过扩展架构支持2D数据输入、引入非线性扩展层和注意力机制，并在实验中验证了其优于基线方法的高精度性能。


<details>
  <summary>Details</summary>
Motivation: 现有的交通状态估计方法分为模型驱动、数据驱动和模型-数据双驱动三类，但各自存在局限性。本文旨在结合物理模型和数据驱动的优势，提出一种更准确的实时估计方法。

Method: 扩展了PI-DeepONet架构，支持2D数据输入，引入非线性扩展层、注意力机制和多输入多输出（MIMO）机制，并设计了自适应识别交通流模型参数的神经网络。

Result: 在NGSIM短高速公路段和中国大规模城市快速路的实验中，该方法在流量和平均速度的估计精度上优于四种基线方法。

Conclusion: 基于扩展PI-DeepONet架构的交通状态估计方法具有高精度和实时性，为交通管理提供了有效工具。
...
### [460] [Learning to Select MCP Algorithms: From Traditional ML to Dual-Channel GAT-MLP](https://arxiv.org/abs/2508.08005)
*Xiang Li,Shanshan Wang,Chenglong Xiao*

Main category: cs.LG

TL;DR: 提出了一种基于学习的框架，结合传统机器学习和图神经网络，用于解决最大团问题（MCP）的算法选择问题。实验表明随机森林（RF）表现稳定，而双通道模型GAT-MLP在所有指标上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏针对MCP的算法选择方法，且没有单一算法在所有实例中表现最佳，因此需要基于实例特征选择合适算法。

Method: 构建带标签的数据集，提取图的全局统计和结构特征，评估四种传统分类器，并开发双通道模型GAT-MLP。

Result: RF表现稳定，GAT-MLP在所有指标上表现优异，连通性和拓扑结构是算法性能的强预测因子。
...
### [461] [Communication-Efficient Zero-Order and First-Order Federated Learning Methods over Wireless Networks](https://arxiv.org/abs/2508.08013)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在边缘设备协作训练时面临通信开销大的问题，需要高效方法减少信息交换。

Method: 采用零阶优化技术和一阶梯度计算策略，利用信道信息并支持异步设备。

Result: 提供了两种方法的收敛性保证和性能界限。

Conclusion: 提出的方法有效减少了通信开销，同时保持了学习性能。
...
### [462] [Deep Learning-Based Analysis of Power Consumption in Gasoline, Electric, and Hybrid Vehicles](https://arxiv.org/abs/2508.08034)
*Roksana Yahyaabadi,Ghazal Farhani,Taufiq Rahman,Soodeh Nikan,Abdullah Jirjees,Fadi Araji*

Main category: cs.LG

TL;DR: 提出了一种基于数据驱动的方法，结合传统机器学习和深度神经网络，用于预测内燃机、电动车和混合动力车的瞬时和累积功耗。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖专业仪器或物理模型，难以大规模部署，亟需一种更实用的解决方案。

Method: 使用动力系统动态特征集，结合传统机器学习和深度神经网络（如Transformer和LSTM）进行预测。

Result: 内燃机瞬时误差低至$10^{-3}$，累积误差低于3%；电动车和混合动力车累积误差分别低于4.1%和2.1%。

Conclusion: 该方法在不同车型中均表现有效，但电动车和混合动力车数据变异性更高，需更鲁棒的模型。
...
### [463] [BadPromptFL: A Novel Backdoor Threat to Prompt-based Federated Learning in Multimodal Models](https://arxiv.org/abs/2508.08040)
*Maozhen Zhang,Mengnan Zhao,Bo Wang*

Main category: cs.LG

TL;DR: BadPromptFL是一种针对多模态对比模型中基于提示的联邦学习的后门攻击，通过优化本地后门触发器和提示嵌入，将中毒提示注入全局聚合过程。


<details>
  <summary>Details</summary>
Motivation: 探索基于提示的联邦学习在多模态学习中的安全性问题，填补现有研究中未解决的关键攻击面。

Method: 通过受损客户端联合优化本地后门触发器和提示嵌入，将中毒提示注入全局聚合过程，利用CLIP架构的上下文学习行为实现攻击。

Result: 攻击成功率高达90%以上，且具有隐蔽性和有限客户端参与的特点。

Conclusion: BadPromptFL揭示了基于提示的联邦学习在实际部署中的鲁棒性问题，引发对其安全性的关注。
...
### [464] [On Understanding of the Dynamics of Model Capacity in Continual Learning](https://arxiv.org/abs/2508.08052)
*Supriyo Chakraborty,Krishnan Raghavan*

Main category: cs.LG

TL;DR: 论文提出了CL的有效模型容量（CLEMC）概念，用于描述稳定性-可塑性平衡点的动态行为，并通过理论和实验验证了其非平稳性。
...
Method: 提出CLEMC概念，建立差分方程建模神经网络、任务数据和优化过程的交互演化。

Result: 实验表明，无论网络架构或优化方法如何，任务分布变化时网络表示新任务的能力会下降。

Conclusion: CLEMC揭示了稳定性-可塑性平衡点的非平稳性，为持续学习提供了新的理论视角。
...
### [465] [From Source to Target: Leveraging Transfer Learning for Predictive Process Monitoring in Organizations](https://arxiv.org/abs/2508.08061)
*Sven Weinzierl,Sandra Zilker,Annina Liessmann,Martin Käppel,Weixin Wang,Martin Matzner*

Main category: cs.LG

TL;DR: 本文提出了一种基于迁移学习的预测过程监控（PPM）技术，帮助缺乏足够事件数据或相关资源的组织实现有效的决策支持。
...
Method: 提出基于迁移学习的PPM技术，并在两个真实案例中实例化，通过IT服务管理过程的事件日志进行数值实验。

Result: 实验结果表明，可以将一个业务流程的知识迁移到相似业务流程（同一或不同组织）中，实现有效的PPM。

Conclusion: 该技术使组织能够在跨组织环境中受益于迁移学习，转移预训练模型等资源。
...
### [466] [C-MAG: Cascade Multimodal Attributed Graphs for Supply Chain Link Prediction](https://arxiv.org/abs/2508.08071)
...
Method: 提出C-MAG架构，分两阶段对齐多模态属性并通过异构图消息传递增强链接预测。

Result: 构建了PMGraph基准，包含大量制造商和产品数据，C-MAG显著提升了链接预测准确性。

Conclusion: C-MAG为多模态数据融合和供应链链接预测提供了实用解决方案。
...
### [467] [ELF: Efficient Logic Synthesis by Pruning Redundancy in Refactoring](https://arxiv.org/abs/2508.08073)
*Dimitris Tsaras,Xing Li,Lei Chen,Zhiyao Xie,Mingxuan Yuan*

Main category: cs.LG

TL;DR: 提出了一种基于分类器的逻辑优化方法，通过预剪枝无效切割，显著减少了计算需求，相比现有技术提速3.9倍。


<details>
  <summary>Details</summary>
Motivation: 传统逻辑优化操作（如重构）计算成本高，且98%的尝试失败，亟需高效方法减少冗余计算。

Method: 利用分类器预判并剪枝无效切割，避免不必要的重构操作。

Result: 在EPFL基准测试和10个大型工业设计上，平均提速3.9倍。

Conclusion: 该方法显著提升了逻辑优化效率，为电子设计自动化提供了更高效的解决方案。
...
### [468] [Symbolic Quantile Regression for the Interpretable Prediction of Conditional Quantiles](https://arxiv.org/abs/2508.08080)
*Cas Oude Hoekstra,Floris den Hengst*

Main category: cs.LG

TL;DR: Symbolic Quantile Regression (SQR) 是一种基于符号回归的方法，用于预测条件分位数，在保持透明性的同时性能接近黑盒模型。


<details>
  <summary>Details</summary>
Motivation: 传统符号回归（SR）仅能估计目标变量的平均值，无法描述变量在其他分位数（如中位数或极值）上的关系，而这对高风险的领域至关重要。

Method: 提出了 SQR 方法，通过符号回归预测条件分位数。

Result: SQR 在透明模型中表现优异，性能接近黑盒基线模型，并通过航空燃油案例展示了其在解释目标分布差异中的应用。

Conclusion: SQR 适用于预测条件分位数，并能帮助理解不同分位数下特征的影响。
...
### [469] [Fast and Generalizable parameter-embedded Neural Operators for Lithium-Ion Battery Simulation](https://arxiv.org/abs/2508.08087)
*Amir Ali Panahi,Daniel Luder,Billy Wu,Gregory Offer,Dirk Uwe Sauer,Weihan Li*

Main category: cs.LG

TL;DR: 论文提出了一种参数嵌入傅里叶神经算子（PE-FNO），用于锂离子电池的高保真数字孪生模型，在速度和精度上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 实现高物理保真度且速度快的锂离子电池数字孪生模型，以满足实时电池管理和大规模推理的需求。

Method: 比较了DeepONet、FNO和新提出的PE-FNO三种算子学习模型，训练数据涵盖多种电流类型和全范围荷电状态（SOC）。PE-FNO通过参数嵌入实现参数灵活性。

Result: PE-FNO比传统方法快200倍，电压误差低于1.7 mV，并能通过贝叶斯优化在参数估计任务中准确恢复扩散系数。

Conclusion: PE-FNO为高保真、高速度的电池数字孪生提供了实用解决方案，优于传统神经网络模型。
...
### [470] [Grid2Guide: A* Enabled Small Language Model for Indoor Navigation](https://arxiv.org/abs/2508.08100)
*Md. Wasiul Haque,Sagar Dasgupta,Mizanur Rahman*

Main category: cs.LG

TL;DR: Grid2Guide结合A*算法和小型语言模型，提供清晰、可读的室内导航指令。


<details>
  <summary>Details</summary>
Motivation: 复杂环境中缺乏外部定位信号和基础设施，需要可靠的室内导航解决方案。

Method: 使用二进制占用矩阵和A*算法计算最优路径，再通过小型语言模型生成自然语言指令。
...
### [471] [Vision-Based Localization and LLM-based Navigation for Indoor Environments](https://arxiv.org/abs/2508.08120)
...
### [472] [MemoryKT: An Integrative Memory-and-Forgetting Method for Knowledge Tracing](https://arxiv.org/abs/2508.08122)
*Mingrong Lin,Ke Deng,Zhengyang Wu,Zetao Zheng,Jie Li*

Main category: cs.LG

TL;DR: 论文提出memoryKT模型，通过三阶段记忆动态模拟提升知识追踪性能，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有知识追踪模型依赖单一遗忘机制，忽略记忆过程的其他阶段和个性化遗忘模式。

Method: 基于时间变分自编码器，模拟记忆动态三阶段：知识记忆特征分布学习、练习反馈重建、个性化遗忘模块嵌入。

Result: 在四个公开数据集上显著优于现有基线。

Conclusion: memoryKT通过完整编码-存储-检索循环建模，增强了个体差异感知能力。
...
### [473] [NeuroDx-LM: A Clinical Large-Scale Model for EEG-based Neurological Disorder Detection](https://arxiv.org/abs/2508.08124)
*Guanghao Jin,Yuan Liang,Yihan Ma,Jingpei Wu,Guoyang Liu*

Main category: cs.LG

TL;DR: 论文提出NeuroDx-LM，一种用于检测基于EEG的神经系统疾病的大规模模型，解决了标记数据不足和临床性能不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 解决基于EEG的大规模模型在临床应用中面临的标记数据有限和性能不佳的挑战。

Method: 提出选择性时频嵌入机制和渐进特征感知训练策略，分两阶段优化特征表示。

Result: 在CHB-MIT和Schizophrenia数据集上取得最先进的性能。

Conclusion: NeuroDx-LM展示了基于EEG的大规模模型在临床应用中的潜力。
...
### [474] [OFAL: An Oracle-Free Active Learning Framework](https://arxiv.org/abs/2508.08126)
*Hadi Khorsand,Vahid Pourahmadi*

Main category: cs.LG

TL;DR: OFAL是一种无需人工标注的主动学习方法，通过神经网络不确定性生成信息丰富的样本，提升模型准确性。


<details>
  <summary>Details</summary>
Motivation: 主动学习中人工标注成本高，OFAL旨在减少对标注者的依赖，利用模型自身不确定性优化学习过程。

Method: 1. 分离和量化不确定性，使用蒙特卡洛Dropout近似贝叶斯神经网络；2. 通过变分自编码器从置信样本生成不确定样本；3. 与其他主动学习方法对比和整合。

Result: OFAL能够生成信息丰富的样本，提升模型在主动学习中的准确性。

Conclusion: OFAL提供了一种高效的无标注者主动学习方案，具有实际应用潜力。
...
### [475] [MuaLLM: A Multimodal Large Language Model Agent for Circuit Design Assistance with Hybrid Contextual Retrieval-Augmented Generation](https://arxiv.org/abs/2508.08137)
*Pravallika Abbineni,Saoud Aldowaish,Colin Liechty,Soroosh Noorzad,Ali Ghazizadeh,Morteza Fayazi*

Main category: cs.LG

TL;DR: MuaLLM是一个开源的多模态大型语言模型代理，用于电路设计辅助，结合了混合检索增强生成框架和自适应向量数据库，显著提升了电路设计文献分析的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 电路设计文献综述的复杂性、数据不一致性以及优化目标的多样性使得传统方法难以应对，需要一种更高效、全面的解决方案。

Method: MuaLLM采用Reason + Act（ReAct）工作流，结合多模态能力和动态检索工具，支持文本和视觉数据处理，并实现实时数据库更新。

Result: MuaLLM在RAG-250数据集上达到90.1%的召回率，在Reas-100数据集上达到86.8%的准确率，同时成本降低10倍，速度提升1.6倍。
...
### [476] [FairFLRep: Fairness aware fault localization and repair of Deep Neural Networks](https://arxiv.org/abs/2508.08151)
*Moses Openja,Paolo Arcaini,Foutse Khomh,Fuyuki Ishikawa*

Main category: cs.LG

TL;DR: FairFLRep是一种自动化的公平性感知故障定位与修复技术，用于识别和修正DNN分类器中可能引发偏见的神经元。


<details>
  <summary>Details</summary>
Motivation: DNN在高风险决策应用中可能放大数据偏见，导致不公平行为，需要有效方法识别和修正这些偏见。

Method: FairFLRep通过调整与敏感属性（如种族或性别）相关的神经元权重，分析网络的输入输出关系，修正导致预测质量差异的神经元。

Result: 在多个数据集和DNN模型上的实验表明，FairFLRep在提升公平性同时保持准确性方面优于现有方法，且效率更高。
...
### [477] [Federated Learning for Epileptic Seizure Prediction Across Heterogeneous EEG Datasets](https://arxiv.org/abs/2508.08159)
*Cem Ata Baykara,Saurav Raj Pandey,Ali Burak Ünal,Harlin Lee,Mete Akgün*

Main category: cs.LG

TL;DR: 论文研究了使用联邦学习（FL）进行癫痫发作预测，提出了一种随机子集聚合策略，以解决数据异构性和隐私问题，显著提高了模型的泛化能力和公平性。


<details>
  <summary>Details</summary>
Motivation: 由于患者隐私法规和数据异构性（非独立同分布特性），开发跨多个临床站点的癫痫发作预测模型面临挑战。联邦学习提供了一种隐私保护框架，但标准聚合方法在异构环境中可能因主导数据集而产生偏差。

Method: 论文采用单通道脑电图（EEG）数据，在四个公共数据集上实施隐私保护的全局归一化，并提出随机子集聚合策略，确保每个客户端在每轮训练中使用固定大小的随机子集数据，以实现公平聚合。

Result: 结果表明，标准加权联邦平均（FedAvg）在异构数据上表现不均衡（如CHB-MIT准确率为89.0%，而Helsinki和NCH仅为50.8%和50.6%）。随机子集聚合显著提高了代表性不足客户端的性能（Helsinki和NCH准确率分别提升至81.7%和68.7%），并实现了77.1%的宏平均准确率和80.0%的总体准确率。

Conclusion: 论文展示了平衡的联邦学习方法在异构多医院环境中构建有效且泛化的癫痫发作预测系统的潜力，同时尊重数据隐私。
...
### [478] [Neural Logic Networks for Interpretable Classification](https://arxiv.org/abs/2508.08172)
*Vincent Perreault,Katsumi Inoue,Richard Labib,Alain Hertz*

Main category: cs.LG

TL;DR: 论文提出了一种可解释的神经网络结构，结合逻辑运算（AND、OR、NOT）和概率建模，改进了布尔网络的发现方法，并在医疗领域展示了其价值。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络缺乏可解释性，无法验证或提取学习内容，而逻辑神经网络通过学习输入输出的逻辑关系提供了可解释性。本文旨在进一步扩展这种能力。

Method: 通过引入NOT操作和考虑未观测数据的偏置，提出了一种因子化的IF-THEN规则结构，并改进了学习算法。

Result: 方法在布尔网络发现中达到最先进水平，并在表格分类中学习到相关且可解释的规则，特别是在医疗领域表现出实际价值。

Conclusion: 该研究为神经网络的可解释性提供了新思路，尤其在需要透明决策的领域（如医疗）具有重要意义。
...
### [479] [Cross-Subject and Cross-Montage EEG Transfer Learning via Individual Tangent Space Alignment and Spatial-Riemannian Feature Fusion](https://arxiv.org/abs/2508.08216)
*Nicole Lai-Tan,Xiao Gu,Marios G. Philiastides,Fani Deligianni*

Main category: cs.LG

TL;DR: 论文提出了一种名为ITSA的新方法，通过结合个体特异性调整和混合架构，提升脑机接口在跨个体应用中的性能。


<details>
  <summary>Details</summary>
Motivation: 脑机接口在个性化音乐干预中的跨个体应用受到EEG信号变异性和运动伪影的限制，需要一种更通用的校准方法。

Method: 提出ITSA预对齐策略，结合个体特异性调整和混合架构（RCSP与黎曼几何的并行和序列融合）。

Result: ITSA在跨个体验证中表现显著提升，并行融合方法优于序列融合。

Conclusion: ITSA为脑机接口的跨个体通用性提供了有效解决方案，代码将公开。
...
### [480] [Part I: Tricks or Traps? A Deep Dive into RL for LLM Reasoning](https://arxiv.org/abs/2508.08221)
...
TL;DR: 本文系统综述了强化学习在LLM推理中的应用，通过统一框架评估不同技术，提出了选择RL技术的指南，并发现一种简单组合能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在LLM推理领域发展迅速，但缺乏标准化指南和对其机制的深入理解，导致实践中的混乱和结论冲突。

Method: 通过统一的开源框架，对广泛采用的RL技术进行严格复现和孤立评估，分析其内部机制、适用场景和核心原则。

Result: 研究发现，两种技术的简单组合能显著提升性能，优于GRPO和DAPO等策略。

Conclusion: 本文提供了RL技术在LLM领域的选择指南，并揭示了简单组合的潜力，为实践者提供了可靠路线图。
...
### [481] [Multi-head Transformers Provably Learn Symbolic Multi-step Reasoning via Gradient Descent](https://arxiv.org/abs/2508.08222)
...
Method: 分析两种任务：反向推理（从目标节点到根节点的路径）和更复杂的前向推理（两阶段推理），基于梯度下降动态理论分析单层Transformer的能力。

Result: 训练后的单层Transformer可以解决两种任务，并具有对未见树的泛化能力；前向推理中，不同注意力头能自主分工协作。

Conclusion: 研究揭示了Transformer如何实现顺序算法程序，表明任务结构化和中间思维链步骤可以使浅层Transformer有效解决复杂问题。
