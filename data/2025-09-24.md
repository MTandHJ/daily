...
Method: 集成U-Net架构和梯度加权类激活映射(Grad-CAM)，构建可解释的息肉分割框架，在1000张内镜图像数据集上训练评估。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是成年人视力丧失的主要原因，特别是在医疗资源匮乏地区，需要开发高效、可扩展的筛查解决方案。

Method: 使用多个卷积神经网络（ResNet-18、EfficientNet-B0和SqueezeNet）进行开发和评估，寻找准确性和计算效率的最佳平衡点。

Result: 最终模型对疾病严重程度分类的准确率为85.4%，系统具备云可扩展性、安全患者数据管理和多用户框架。
...
Method: SIM框架利用逆映射机制，通过从变换后的输出重构输入来减少信息损失。ρSIM通过补丁级特征采样和基于投影的方法重构潜在特征，降低了计算复杂度。

Result: 在图像分类、少样本提示学习和领域泛化等任务中，ρSIM相比基线方法均取得一致改进。该方法与现有正则化方法正交，能进一步提升其效果，并在密集到密集任务和非视觉领域也表现良好。

Conclusion: SIM是一种简单有效的模型无关正则化器，能无缝集成到不同网络架构和任务中，显著提升表示学习效果，具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决在单轮平均梯度SAG机制下，由于每个样本线索在批次平均梯度中纠缠，导致现有方法难以实现有效图像重建的挑战。

Method: 1）基于组合重缩放的闭式优化边界；2）动量混合全批次和子集损失的鲁棒重建机制；3）通过随机数据子集探测感知潜在图像信号。

Result: 在大批次场景下显著优于现有先进方法，实现高保真多图像重建，计算开销与标准求解器相当。

Conclusion: MAGIA框架在无需辅助信息的情况下，成功解决了单轮平均梯度反演的关键挑战，为隐私保护机器学习提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯文档OCR面临草书字体、多样化字体、变音符号和从右到左书写方向的挑战，现有MLLM在阿拉伯语上的表现有限，需要专门优化的解决方案。

Method: 采用仅解码器微调策略，在结合合成和真实文档的大规模数据集上微调预训练MLLM，同时保留通用视觉特征，并开发了高质量的专家验证基准Misraj-DocOCR。

Result: Baseer显著优于现有开源和商业解决方案，WER达到0.25，在阿拉伯文档OCR领域建立了新的最先进水平。

Conclusion: 研究证明了针对特定领域微调通用MLLM的益处，为形态丰富的语言（如阿拉伯语）的高精度OCR建立了强大基线。
...
Method: 设计并实现了一个混合卷积神经网络和长短期记忆网络（CNN-LSTM）模型，专门用于从生成的数据张量中同时学习空间模式和时间依赖性。

Result: 与LightGBM和LASSO回归等强大机器学习基线相比，所提出的架构提供了显著更准确和空间一致的预测，为该任务建立了新的性能基准。

Conclusion: 研究发现证实了时空深度学习在高分辨率变形预测中的有效性和潜力，可解释性分析显示基线模型往往默认使用简单的持续性模式，突显了集成时空方法的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 需要验证AI模型对基础概念的理解能力，为处理更复杂任务奠定基础。通过生成包含大量问题和语言变体的数据集来系统评估模型性能。

Method: 提出Scrapbook框架，生成针对单个概念的大规模多样化数据集，包含广泛的语言变体，用于系统测试模型的基础概念理解能力。

Result: 当代模型在物体识别方面表现良好，但在位置信息理解和带约束的问题上存在困难。MobileVLM-V2模型出现显著答案分歧，其他模型存在肯定答案偏见，在几何形状和位置信息问题上表现不佳。

Conclusion: Scrapbook框架为生成多样化数据集提供了有效工具，可用于系统评估和提升AI模型性能，揭示了模型在基础概念理解方面的改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态AI系统在创意工作流程中的集成日益增多，理解视觉-语言-视觉管道中的信息损失对于评估系统局限性变得重要，但目前视觉内容通过文本中介时发生的退化尚未得到充分量化。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在处理不同视角的视觉观察时存在视角异构问题，限制了模型的泛化能力。

Method: 提出VLA-LPAF模块，使用单视图图像进行微调，在潜在空间融合多视角观察，有效弥合视角不一致带来的差距。

Result: 在CALVIN、LIBERO和定制仿真基准上分别实现约8%、15%和30%的任务成功率提升，并在真实任务中展示视角自适应特性。

Conclusion: VLA-LPAF能够有效提升VLA模型的视角适应性和泛化性能，为多视角视觉任务提供了高效解决方案。
...
Method: URNet包含局部-全局精化模块来捕捉细粒度局部细节和长距离全局上下文，并引入基于KL散度的不确定性建模方法来增强预测可靠性。

Result: 在DSEC数据集上的大量实验表明，URNet在定性和定量评估中都一致优于现有最优方法。

Conclusion: URNet通过结合局部-全局精化和不确定性建模，显著提升了基于事件相机的立体深度估计性能。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症常导致慢性盆腔疼痛和可能的神经受累，但周围神经成像仍然是一个挑战，需要开发新的方法来改善神经识别。

Method: Visionerves采用两阶段流程：(A)使用深度学习模型自动分割解剖结构，(B)通过符号空间推理进行纤维束追踪和神经识别，利用模糊空间关系编码解剖知识。

Result: 在10名子宫内膜异位症患者的腰骶神经丛应用中，Visionerves相比标准纤维束追踪方法有显著改进，Dice评分提高达25%，空间误差减少到小于5mm。

Conclusion: 这种自动且可重复的方法能够进行详细的神经分析，为非侵入性诊断子宫内膜异位症相关神经病变以及其他涉及神经的疾病开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要来自发达国家，缺乏新兴经济体中观察到的行为多样性，且驾驶员面部记录侵犯隐私保护。巴基斯坦等国家存在异质道路条件、混合交通流和可变驾驶纪律等问题。

Method: 使用定制Android应用收集高频率加速度计、陀螺仪和GPS数据，结合同步的道路视频，在多种道路类型上记录三种目标驾驶行为（正常、激进和危险）。数据分为原始层、处理层和语义层。

Result: 创建了V-SenseDrive数据集，填补了全球驾驶员行为数据集的空白，为驾驶员行为分类、交通安全分析和ADAS开发提供了基础。

Conclusion: V-SenseDrive代表了巴基斯坦真实世界的驾驶情况，为上下文感知智能交通解决方案奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够同时具备强大通用性能和领域特定能力的多模态模型，以满足企业多样化部署需求。通过验证百度昆仑P800芯片在大规模AI基础设施上的训练能力，展示国产芯片训练SOTA级多模态模型的可行性。

Method: 采用多阶段渐进训练策略和高精度数据合成管道。模型系列包含3B到70B不同参数规模，其中8B和70B变体集成了长链思维能力。所有模型均在百度昆仑P800芯片上训练，在5000芯片上实现了超过90%的扩展效率。

Result: 在通用基准测试中与领先开源模型表现相当，在CCBench、SEEDBench IMG、ScienceQA和MMStar等基准上达到最先进性能。在OCR和文档理解领域优势显著，OCRBench得分873，DocVQA得分94.75%。数学推理（MathVista 78.6%）和逻辑推理任务表现优异。

Conclusion: 该工作确立了开发领域增强多模态模型的有效方法学，证明了大规模AI基础设施能够高效训练SOTA级多模态模型，为多样化企业部署场景提供了合适的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏配对的真实世界训练数据，导致域间差距大，难以泛化到真实场景。传统基于大气散射模型的物理方法无法处理真实世界的复杂性和多样化雾霾模式。
...
Conclusion: HazeFlow通过ODE框架和创新的雾霾生成方法，有效解决了真实世界去雾的挑战，在泛化性和性能方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 农业领域部署深度学习模型面临边缘设备资源有限的挑战，需要开发高效的压缩和加速方法。

Method: 采用结构化通道剪枝、量化感知训练(QAT)和NVIDIA TensorRT加速技术，针对包含残差连接、注意力机制、连接操作和CSP块的复杂架构进行优化。

Result: 模型大小减少68.5%，计算量减少3.2 GFLOPs，推理速度达到184 FPS（FP16），比基线快28.7%。在CottonWeedDet12数据集上，39.5%剪枝率的EcoWeedNet达到83.7%精确率、77.5%召回率和85.9% mAP50，优于YOLO11n和YOLO12n。

Conclusion: 该方法证明了在保持高精度的同时实现显著模型压缩和加速的可行性，为精准农业中的高效杂草检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着医学诊断越来越多地使用多模态数据，需要模型能够有效融合异构信息并保持对缺失模态的鲁棒性，同时应对现实世界中的模态不平衡和缺失问题。

Method: 引入了可学习的模态token来改进缺失感知的模态融合，并在传统的单模态对比目标基础上增加了融合的多模态表示对比学习。

Result: 在大规模临床数据集上的实验表明，该方法在疾病检测和预测任务中取得了最先进的性能，特别是在只有单一模态可用的实际场景中表现优异。

Conclusion: 该方法具有高效性、可扩展性和通用性，为多模态学习提供了一个可扩展、低成本的解决方案，在现实临床应用中具有重要潜力。
...
<details>
  <summary>Details</summary>
Motivation: 肺栓塞分割对于临床诊断至关重要，但目前缺乏对不同分割架构在该任务上的系统性评估。本研究旨在填补这一空白，为肺栓塞分割提供可靠的性能基准。

Method: 使用490个CTPA扫描构建密集标注数据集，在统一测试框架下评估9种CNN和ViT分割架构，比较预训练和随机初始化的效果。

Result: 最佳模型达到0.7131的平均Dice分数，在60个测试扫描中检测到181个栓子，有49个假阳性和28个假阴性。3D模型表现优于2D模型，CNN模型优于ViT模型。

Conclusion: 3D U-Net with ResNet encoder是肺栓塞分割的有效架构，远端栓子分割仍具挑战性，分类和分割任务可能依赖不同的判别特征。
...
<details>
  <summary>Details</summary>
Motivation: 可靠的外分布检测对于在异质性图像特征和临床环境中安全部署深度学习模型至关重要。现有研究主要关注不确定性量化方法，而本文研究分类任务本身的影响。

Method: 通过实验，在四个分类任务上评估了八种不确定性量化方法，分析不同任务对OOD检测性能的影响。

Result: OOD检测性能随任务显著变化，最佳任务取决于ID-OOD标准（图像特征偏移或解剖特征偏移）。更好的OOD检测不一定能保证最优的弃权预测。

Conclusion: 在医学图像分析中，需要根据具体的下游应用来调整任务选择和不确定性策略。
...
<details>
  <summary>Details</summary>
Motivation: 在无互联网连接或GPS支持的资源受限场景下，现有基于大型图像数据库或3D模型的方法不实用，而轻量级的正射地理数据作为替代方案尚未得到充分研究。

Method: 构建包含16,425张无人机图像的多模态数据集，采用配对结构分离图像检索和特征匹配评估，并提出AdHoP优化技术可集成到任何特征匹配器中。

Result: AdHoP技术可将匹配性能提升高达95%，平移误差降低高达63%，数据集有效解决了无人机图像与地理数据之间的域偏移问题。

Conclusion: OrthoLoC数据集为视觉定位研究提供了公平的基准测试平台，证明了正射地理数据在资源受限定位场景中的实用性，AdHoP技术显著提升了定位精度。
...
<details>
  <summary>Details</summary>
Motivation: 解决现实场景中训练数据不可用、只有测试图像可用的零样本异常检测问题。

Method: 基于Deep Image Prior思想，设计patch-based训练框架，通过掩码、patch打乱和小高斯噪声避免恒等映射，使用基于内积相似度的感知损失。

Result: 在MVTec-AD数据集上达到0.99 AUROC和0.60 AUPRC，在fabric数据集上达到0.98 AUROC和0.67 AUPRC，优于现有方法。

Conclusion: SSDnet无需外部训练数据、标签或参考，在噪声或像素缺失情况下仍保持鲁棒性，是有效的零样本异常定位方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（如孟加拉语）中视觉-语言模型存在的挑战：配对数据稀缺、翻译对齐断裂、以及英语中心预训练忽略目标语言语义的问题。

Method: 使用LaBSE验证的英-孟配对数据和11万双语提示合成图像训练管道。采用冻结的MaxViT提取视觉特征，孟加拉语原生mBART-50解码，轻量级桥接模块连接模态。核心创新是三重损失函数：PAL对齐真实和合成图像块描述符，InfoNCE强制全局分离，Sinkhorn OT确保细粒度对应。
...
Conclusion: 提出的三重损失函数协同机制有效改善了低资源语言的视觉-语言对齐，减少了虚假匹配，为低资源多模态学习提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有高效相机基线模型功能不完整的问题，将大规模多模态感知规划模型的智能保留到资源受限的部署环境中。

Method: 采用模型无关的多阶段蒸馏策略，结合特征级、输出级和自适应区域感知监督，将高容量多模态知识转移到轻量级BEV表示中。

Result: 在nuScenes数据集上，检测mAP达到39.0，运动预测minADE为1.08，碰撞率0.32，运行速度11 FPS，仅需相机输入。

Conclusion: TinyBEV证明了在资源受限环境下仍能保留完整的驾驶智能，弥合了大规模多模态模型与部署就绪实时自动驾驶系统之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有标注方法将球标记在模糊条纹的前沿，这引入了不对称性并忽略了与速度相关的运动线索，限制了检测系统的性能。

Method: 引入新的标注策略，将球置于模糊条纹中心并标注模糊属性；提出BlurBall模型，通过多帧输入的Squeeze-and-Excitation注意力机制联合估计球位置和运动模糊属性。

Result: 新标注方法在各种模型上一致提升了检测性能；BlurBall模型在球检测任务上达到了最先进的结果。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模开放词汇表检测器在视频每帧上运行计算成本过高的问题，寻求一种无需训练和标注的高效视频检测方案。

Method: 使用固定间隔的关键帧进行OWLv2检测，通过压缩域运动向量（3x3网格聚合）在中间帧传播检测结果，包含面积增长检查和可选单类别切换。

Result: 在ILSVRC2015-VID数据集上达到mAP@0.5=0.609，在宽松IoU阈值下接近逐帧检测性能，优于基于跟踪器的传播方法。

Conclusion: 压缩域传播是减少检测器调用次数的实用方法，能在视频中保持强大的零样本覆盖能力，且无需标注训练。
...
<details>
  <summary>Details</summary>
Motivation: 高动态范围光照估计技术虽然在增强现实应用中展现了潜力，但颜色鲁棒性这一关键因素常被忽视。现有评估往往将颜色与其他光照属性混淆，需要专门研究颜色准确性对视觉真实感的影响。

Method: 使用包含多样化光照颜色的新型HDR数据集，系统评估多种适应策略，重点测试预训练白平衡网络预处理输入图像的效果，并在三种最先进的光照估计方法上进行验证。

Result: 实验结果表明，白平衡预处理策略在所有测试场景中都优于其他方法，显著提高了颜色鲁棒性，且该方法具有通用性，适用于不同的光照估计算法。

Conclusion: 简单的预处理技术（如白平衡）可以有效提升现有HDR光照估计模型的颜色准确性，为增强现实应用提供更真实的视觉渲染效果，且无需复杂的模型重新训练。
...
<details>
  <summary>Details</summary>
Motivation: 支票在金融系统中使用广泛但易受欺诈，传统检测方法需要大量标注数据，而这类数据由于隐私和专有性问题难以获取。

Method: 使用视觉语言模型(VLM)和多模态大语言模型(MLLM)构建免训练框架，通过零样本学习自动识别支票中的签名、MICR线、金额等关键字段。

Result: 在包含110张不同格式支票的手工数据集上评估，模型表现出强大的性能和泛化能力。

Conclusion: 该框架不仅可以直接用于支票字段检测，还能作为生成高质量标注数据的引导机制，为开发专门的实时目标检测模型奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准假设图表清晰且查询基于事实，但现实图表常包含失真并需要复杂推理。需要评估模型在退化条件下的表现，特别是幻觉问题。

Method: 引入CHART NOISe数据集，结合图表腐蚀、遮挡和考试风格多选题，采用提示反向不一致性测试方法，评估ChatGPT 4o、Claude Sonnet 4和Gemini 2.5 Pro等模型。

Result: 模型在失真条件下性能急剧下降，幻觉问题（如数值捏造、趋势误判、实体混淆）更频繁，模型在退化设置中仍过度自信。

Conclusion: 建立了图表理解的严格测试基准，揭示了系统脆弱性，提出了质量过滤和遮挡检测等缓解策略，为提升图表理解的鲁棒性和可靠性奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统的4D-MRI重建方法依赖相位分箱或模板扫描，难以捕捉呼吸运动的时序变化，工作流程复杂且计算负担重。

Method: 采用双网络架构：空间解剖网络（SAN）编码连续3D解剖表示，时序运动网络（TMN）基于Transformer生成的呼吸信号产生时间一致的变形场。

Result: 在19名志愿者的自由呼吸数据集上验证，新方法能准确捕捉规则和不规则呼吸模式，处理时间从5小时缩短到15分钟，单帧3D体积推断时间小于1秒。
...
<details>
  <summary>Details</summary>
Motivation: 快速移动的小物体（如壁球）由于其不可预测的运动模式和小尺寸，是计算机视觉中的挑战性问题。这对于体育机器人应用尤为重要，需要轻量级且准确的跟踪系统来提升机器人感知和规划能力。
...
Result: DeepOCSORT获得最低跟踪误差（平均ADE为31.15像素），而ByteTrack处理速度最快（平均推理时间26.6ms）。但所有跟踪器都表现出显著的跟踪漂移，空间误差在3-11cm之间。

Conclusion: 当前跟踪方法在处理快速移动小物体的不可预测运动模式方面存在根本性局限，误差率比标准物体跟踪基准高3-4倍，需要开发专门的跟踪方法。
...
<details>
  <summary>Details</summary>
Motivation: 为了在压缩域中实现高效实时的视频动作识别，需要一种能够利用视频压缩信息（如运动向量）来减少计算开销的方法。

Method: MoCrop包含三个轻量级组件：去噪与合并（DM）、蒙特卡洛采样（MCS）和通过运动密度子矩阵搜索的自适应裁剪（AC），在推理时对所有I帧应用单一剪辑级裁剪。

Result: 在UCF101数据集上，MoCrop在保持相同FLOPs时提升ResNet-50准确率3.5%，或在减少26.5%FLOPs时提升2.4%准确率。应用于CoViAR时，在原始计算成本下达到89.2%准确率，或在减少计算量（从11.6降至8.5 GFLOPs）时达到88.5%准确率。

Conclusion: MoCrop在多种骨干网络（MobileNet-V3、EfficientNet-B1、Swin-B）上均表现一致，具有强通用性，适合压缩域实时部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在低比特率条件下表现不佳，因为它们保留过多冗余细节或学习过度集中的符号分布，需要一种更有效的特征压缩方法
...
<details>
  <summary>Details</summary>
Motivation: 解决现有医学图像分割方法计算复杂度高、参数量大，难以在资源受限环境中部署的问题

Method: 设计了多核深度卷积块（MKDC）处理多分辨率空间关系，结合通道、空间和分组门控注意力机制突出图像显著特征

Result: 仅需0.316M参数和0.314G FLOPs，在六个医学图像基准测试中超越TransUNet、UNeXt等SOTA方法，DICE分数显著提升

Conclusion: MK-UNet在保持高性能的同时大幅降低计算需求，为资源受限环境下的实时医学诊断提供了无与伦比的解决方案
...
Result: 在猪内脏手术和人体肝脏合成数据上的实验表明，该方法能够在单目RGB数据上实现术前CT的合理变形。
...
<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法通常同时优化整个伪标签集，但由于伪标签集包含大量噪声，这种优化方式效果有限。需要一种能够从少量高质量伪标签开始，逐步生成完整高质量伪标签的方法。

Method: 1）基于置信度过滤和超分辨率增强的伪标签融合方法，获得少量高质量伪标签作为初始种子；2）利用扩散模型对不规则分布的种子伪标签进行传播，生成完整高质量伪标签。

Result: DGLE框架有效避免了直接优化完整伪标签集的困难，显著提高了伪标签质量，从而提升了模型在目标域的性能。

Conclusion: 提出的扩散引导标签富集方法为SFDA中的伪标签优化提供了新思路，通过从高质量种子逐步生成完整标签集，解决了现有方法在噪声伪标签优化方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 双曲空间相比欧几里得空间对分层数据具有更优的表示能力，特别是在粗到细的少样本类增量学习任务中，需要更好地处理层次化类别结构。

Method: 使用庞加莱球模型将特征提取器嵌入双曲空间，引入双曲对比损失和双曲全连接层，并采用双曲空间最大熵分布进行特征增强以缓解少样本过拟合。

Result: 在C2FSCIL基准测试中，该方法有效提升了粗类和细类分类准确率。

Conclusion: 双曲空间嵌入为粗到细的少样本类增量学习提供了有效的解决方案，显著改善了分层数据的表示和学习性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像外观的物体移除方法存在两个主要问题：严格掩码对齐训练无法移除未明确掩码的因果视觉伪影；宽松掩码对齐策略缺乏可控性可能过度擦除其他物体。这些限制源于忽略了物体几何存在与其视觉效应之间的因果关系。

Method: 提出两阶段框架：1）几何移除阶段：使用严格掩码对齐监督直接从几何（如深度）中移除物体，实现具有强几何约束的结构感知编辑；2）外观渲染阶段：基于更新后的几何条件渲染逼真的RGB图像，其中因果视觉效应作为修改3D几何的结果被隐式考虑。在几何移除阶段引入基于正负样本对的偏好驱动目标。

Result: 在两个流行基准测试上的广泛实验表明，该方法在移除物体及其相关伪影方面达到了最先进的性能。

Conclusion: 通过解耦几何移除和外观渲染，GeoRemover能够有效处理物体移除中的因果视觉伪影问题，在保持可控性的同时实现更完整的物体移除效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的NR-IQA模型白盒攻击方法在更现实的黑盒场景中迁移性较差，无法有效攻击未知目标模型

Method: SEGA方法通过应用高斯平滑到源模型并集成其平滑梯度来近似目标模型的梯度，并使用专门设计的扰动滤波掩码确保对抗性扰动的不可感知性
...
<details>
  <summary>Details</summary>
Motivation: 现有的多任务学习框架在结合深度学习架构与手工D-Marker特征时存在计算效率低下的问题，需要辅助任务监督和复杂的损失平衡要求。

Method: 提出HadaSmileNet框架，系统评估了15种融合策略，发现Hadamard乘法融合能够实现直接特征交互同时保持计算效率。

Result: 在四个基准数据集上达到最先进结果：UvA-NEMO（88.7%，+0.8）、MMI（99.7%）、SPOS（98.5%，+0.7）和BBC（100%，+5.0），参数减少26%且训练简化。
...
Method: 使用统一的3D高斯集合，其中可学习的语义属性区分人体和场景高斯。仅人体高斯进行形变动画，场景高斯保持静态。提出事件引导损失函数，匹配连续渲染之间的模拟亮度变化与事件流。

Result: 在两个基准数据集ZJU-MoCap-Blur和MMHPSD-Blur上实现了最先进的人体-场景重建效果，在PSNR/SSIM指标上显著优于强基线，LPIPS降低，特别对高速运动主体效果明显。

Conclusion: 该方法无需外部人体掩码，简化了独立高斯集的管理，有效解决了快速运动下的模糊问题，为动态人体与静态场景的联合重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在美学视觉理解方面存在局限，只能处理基础美学常识，无法满足需要专业摄影技术和图像处理知识的真实场景需求。作者受摄影美学与通用视觉理解差异的启发，旨在填补这一空白。
...
Conclusion: 通过专业数据集、创新模型和评估基准的结合，本研究显著提升了MLLMs的美学理解能力，为专业级视觉美学分析提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 提高MRI引导放疗中肿瘤跟踪的精度，增强癌症治疗的准确性和安全性

Method: 利用XMem模型的内存增强架构，通过内存机制在有限标注数据条件下实现实时肿瘤分割

Result: 实验记录丢失无法提供精确量化结果，但初步印象显示框架具有合理的分割性能并满足临床实时要求

Conclusion: 该工作有助于改善MRI引导放疗中的肿瘤跟踪精度，对提升癌症治疗质量具有重要意义
...
TL;DR: 提出SSCM模型解决多对比度MRI超分辨率问题，通过动态空间扭曲模块、语义感知标记聚合块和空间频率融合块，实现空间语义一致性重建。
...
Method: SSCM模型包含三个核心模块：动态空间扭曲模块用于跨对比度空间对齐，语义感知标记聚合块确保长程语义一致性，空间频率融合块用于精细结构恢复。

Result: 在公开和私有数据集上的实验表明，SSCM以更少的参数实现了最先进的性能，同时确保空间和语义一致性重建。
...
TL;DR: 提出OraPO方法和FactS奖励机制，在有限预算下实现高效放射学报告生成，显著减少训练数据需求并达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决现有放射学报告生成方法对大规模数据和计算资源的依赖问题，在资源受限条件下实现高效准确的报告生成

Method: 结合Oracle-educated GRPO (OraPO)和FactScore-based奖励机制(FactS)，通过单阶段强化学习训练，将失败的探索转化为偏好监督，并基于临床事实提取和验证提供密集奖励

Result: 在CheXpert Plus数据集上达到0.341 F1分数，使用2-3个数量级更少的训练数据和小型基础VLM，在普通硬件上实现SOTA性能

Conclusion: 提出的紧凑框架显著提高了临床挑战性病例的学习效率，为资源受限环境下的放射学报告生成提供了有效解决方案
...
TL;DR: AMSF是一个基于参考图像的无训练框架，能够在扩散模型中实现多风格的可控融合，解决了现有方法只能处理单一风格和缺乏平衡机制的问题。
...
Method: AMSF通过语义标记分解模块编码所有风格图像和文本提示，自适应地注入到冻结扩散模型的每个交叉注意力层中，然后通过相似度感知重加权模块在每个去噪步骤重新校准对每个风格组件的注意力分配。

Result: 定性和定量评估显示，AMSF生成的多风格融合结果始终优于最先进方法，且其融合设计可无缝扩展到两种或更多风格。

Conclusion: AMSF是扩散模型中实现表达性多风格生成的实用步骤。
...
TL;DR: MLF-4DRCNet是一个用于3D目标检测的两阶段框架，通过4D雷达和相机图像的多级融合来解决现有方法中雷达点云稀疏和噪声问题，在VoD和TJ4DRadSet数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达具有成本效益和鲁棒性，但其点云存在显著稀疏性和噪声，限制了在3D目标检测中的独立应用。现有雷达-相机融合方法大多采用为LiDAR设计的BEV融合范式，忽视了雷达点云的稀疏性和不完整几何特性。
...
Result: 在View-of-Delft（VoD）和TJ4DRadSet数据集上的实验结果表明，MLF-4DRCNet实现了最先进的性能，在VoD数据集上达到了与基于LiDAR模型相当的性能。

Conclusion: 该方法通过点级、场景级和提议级的多模态信息融合，实现了全面的特征表示，有效解决了4D雷达点云稀疏和噪声问题，为自动驾驶感知提供了有效的解决方案。
...
TL;DR: PDLS是一个无需训练的双流潜在空间引导框架，通过结构路径和语义路径的互补，解决扩散模型在图像逆变换中的语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于单潜在向量的方法在平衡结构保真度和语义准确性方面存在困难，导致重建图像出现语义漂移（如细节模糊或属性错误）。

Method: 基于Rectified Flow模型的稳定逆变换路径，将逆变换过程分解为结构路径（保持源完整性）和语义路径（由提示词引导），通过最优控制问题构建线性二次调节器进行动态轨迹引导。

Result: 在FFHQ-1K和ImageNet-1K数据集上的多种逆变换任务（高斯去模糊、运动去模糊、超分辨率和自由形式修复）中，PDLS比单潜在基线方法产生更忠实于原始图像且语义信息更一致的重建结果。

Conclusion: PDLS框架通过双流潜在引导有效解决了语义漂移问题，无需昂贵的逐图像优化，在保持细节的同时确保语义准确性。
...
TL;DR: 开发了Prima，首个用于神经影像学的视觉语言模型，能够处理真实临床MRI数据，在52种神经疾病诊断中平均AUC达到92.0，优于现有AI模型。


<details>
  <summary>Details</summary>
Motivation: 全球MRI需求增长给医疗系统带来压力，特别是在资源匮乏地区。需要AI工具来缓解医生负担、缩短诊断时间并减少医疗系统偏见。

Method: 利用大型学术医疗系统作为数据引擎，训练了包含22万例MRI研究的视觉语言模型Prima，采用分层视觉架构提供通用可转移的MRI特征。

Result: 在包含3万例MRI研究的1年系统测试中，Prima在主要神经疾病诊断中表现优异，提供可解释的鉴别诊断、工作列表优先级和临床转诊建议。

Conclusion: Prima展示了健康系统规模视觉语言模型的变革潜力，能够推动AI驱动的医疗保健发展，特别是在缓解医疗系统偏见方面。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Chain-of-Thought方法将理解和生成过程分离，限制了统一模型在解决生成能力不足时的推理指导能力
...
TL;DR: 本文提出了一个用于内窥镜图像深度估计的综合性基准测试，并发布了一个新的合成数据集EndoSynth，通过在该数据集上微调深度基础模型，显著提升了在真实内窥镜图像上的深度估计精度。


<details>
  <summary>Details</summary>
Motivation: 内窥镜图像的深度估计领域缺乏稳健的基准测试和高质量数据集，限制了深度估计模型在临床场景中的应用和评估。

Method: 1）建立内窥镜图像深度估计的基准测试，评估现有最先进的深度估计模型在真实内窥镜图像上的性能；2）创建并发布EndoSynth合成数据集，包含内窥镜手术器械及其对应的真实深度和分割掩码；3）在合成数据集上微调深度基础模型。

Result: 基准测试揭示了现有模型在真实内窥镜图像上的泛化能力不足；使用EndoSynth数据集微调后的深度基础模型在大多数未见过的真实数据上精度显著提升。
...
TL;DR: 本文提出了一种名为LEAF-Mamba的新方法，用于RGB-D显著目标检测，通过结合局部强调状态空间模块和自适应融合模块，在保持线性复杂度的同时提升局部语义建模和跨模态融合能力。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D显著目标检测方法存在局限性：CNN方法受限于局部感受野，而Vision Transformers则面临二次复杂度的高计算成本。状态空间模型（SSM）如Mamba虽然能有效建模长距离依赖关系，但直接应用于RGB-D SOD会导致局部语义不足和跨模态融合不充分的问题。

Method: 提出LEAF-Mamba模型，包含两个核心组件：1）局部强调状态空间模块（LE-SSM）用于捕获多尺度局部依赖关系；2）基于SSM的自适应融合模块（AFM）实现互补的跨模态交互和可靠的跨模态集成。

Result: 在大量实验中，LEAF-Mamba在16个最先进的RGB-D SOD方法中表现出色，在效果和效率方面均优于现有方法。同时，该方法在RGB-T SOD任务上也表现出优异的性能，证明了强大的泛化能力。

Conclusion: LEAF-Mamba通过创新的局部强调和自适应融合机制，成功解决了RGB-D SOD中的局部语义建模和跨模态融合问题，在保持线性复杂度的同时实现了卓越的性能和泛化能力。
...
Method: 集成窗口多头注意力机制(WMHAM)和空间注意力机制(SAM)，WMHAM通过窗口分区捕获局部和全局上下文特征降低计算成本，SAM自适应强调关键空间区域提升特征表示
...
<details>
  <summary>Details</summary>
Motivation: 解决遥感图像中需要精细空间定位和语义开放分类的挑战，实现无需类别监督的新物体检测分割和可解释语义标签分配

Method: 三阶段流程：1) 使用可提示的微调分割模型SAM进行精确发现和掩码提取；2) 通过两阶段微调的多模态大语言模型进行语义归因和上下文描述；3) LLM作为评判者和人工评分进行MLLM评估

Result: 该框架结合像素级精度和高层语义理解，在开放世界遥感解释中解决关键挑战，支持跨多样卫星图像的鲁棒评估

Conclusion: OSDA为动态土地覆盖监测提供了可扩展和可解释的解决方案，在自动制图更新和大规模地球观测分析中显示出强大潜力
...
TL;DR: LifeCLEF 2021植物识别挑战赛旨在评估如何利用植物标本馆收藏改进生物多样性丰富但数据稀缺地区的植物自动识别，基于圭亚那地盾地区的约1000种植物数据集进行跨域分类评估。


<details>
  <summary>Details</summary>
Motivation: 当前自动植物识别主要依赖北美和西欧的野外照片数据，而生物多样性最丰富的热带地区数据稀缺。植物标本馆收藏了大量热带植物标本，可为数据稀缺地区提供补充训练数据。

Method: 采用跨域分类方法，训练集包含数十万植物标本馆标本和数千张野外照片，测试集仅包含野外照片。数据集还包括5种形态和功能性状值作为额外特征。

Result: 挑战赛评估了不同研究团队的系统性能，分析了利用标本馆数据改进热带地区植物识别的效果。

Conclusion: 植物标本馆收藏可以作为数据稀缺地区植物自动识别的重要补充数据源，跨域学习方法有助于弥合标本馆标本与野外照片之间的领域差异。
...
TL;DR: 提出AGSwap方法解决多类别对象融合问题，并构建COF数据集进行评测


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法在跨类别对象融合时存在偏差、视觉混乱和语义不一致问题，且缺乏全面的基准数据集

Method: AGSwap方法包含两个关键组件：组级嵌入交换（通过特征操作融合不同概念的语义属性）和自适应组更新（基于平衡评估分数的动态优化机制）

Result: AGSwap在简单和复杂提示下均优于现有最先进的组合T2I方法，包括GPT-Image-1

Conclusion: AGSwap是一种简单而高效的方法，结合新构建的COF数据集，显著提升了跨类别对象融合的质量和一致性
...
TL;DR: 该论文介绍了LifeCLEF 2019植物识别挑战赛，旨在评估对数据稀缺地区（主要是圭亚那地盾和北亚马逊雨林）植物物种的自动识别能力，并比较系统性能与热带植物专家的表现。


<details>
  <summary>Details</summary>
Motivation: 当前植物自动识别技术主要针对少数几万种物种，而全球有近36.9万种植物。挑战赛旨在解决数据稀缺地区的植物识别问题，特别是生物多样性极高的热带雨林区域。

Method: 基于包含1万种物种的数据集，主要聚焦圭亚那地盾和北亚马逊雨林区域。与往年一样，将系统性能与最佳热带植物专家的表现进行比较评估。

Result: 论文提供了挑战赛的资源、评估结果，总结了参与研究团队采用的方法和系统，并分析了主要成果。

Conclusion: 该挑战赛为评估数据稀缺地区植物自动识别技术提供了重要平台，有助于推动对全球生物多样性热点区域的植物识别研究。
...
TL;DR: RSVG-ZeroOV是一个无需训练的训练框架，利用冻结的通用基础模型实现零样本开放词汇遥感视觉定位，通过三阶段方法（概览、聚焦、演化）解决现有方法依赖封闭词汇表和需要大量标注数据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法受限于封闭词汇表，在开放世界场景中适用性有限。虽然近期尝试利用通用基础模型，但过度依赖高质量数据集和耗时微调。本文旨在探索冻结通用基础模型在零样本开放词汇遥感视觉定位中的潜力。

Method: 提出三阶段框架：1）概览阶段使用视觉语言模型获取文本查询与视觉区域的语义关联；2）聚焦阶段利用扩散模型的细粒度建模能力补充结构信息；3）演化阶段通过注意力演化模块抑制无关激活，生成纯净分割掩码。

Result: 大量实验表明，该框架在无需任务特定训练的情况下，持续优于现有的弱监督和零样本方法。

Conclusion: RSVG-ZeroOV提供了一个高效且可扩展的解决方案，证明了冻结通用基础模型在零样本开放词汇遥感视觉定位中的有效性。
...
TL;DR: 本文提出了一个属性提示组合（APC）框架，利用文本语义联合增强对象重识别（ReID）的判别性和泛化能力。通过属性提示生成器和快慢训练策略，在保持预训练视觉语言模型泛化能力的同时提升ReID特定判别性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有ReID模型在单域和跨域场景中的局限性：单域模型容易过拟合域特定特征，跨域模型可能抑制身份特定的判别线索。

Method: 1. 属性提示生成器（APG）：包含语义属性字典（SAD）和提示组合模块（PCM）；2. 快慢训练策略（FSTS）：快速更新流获取ReID特定知识，慢速更新流保留预训练模型的泛化知识。

Result: 在传统和域泛化ReID数据集上的广泛实验表明，该框架在判别性和泛化性方面均优于最先进方法。

Conclusion: APC框架通过文本语义的有效利用，成功平衡了ReID特定判别性和模型泛化能力，为实际应用提供了更实用的解决方案。
...
TL;DR: 本文提出OTCCLIP框架，使用最优传输理论重建图像-文本对，通过细粒度特征对齐防御CLIP模型的数据投毒攻击。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法仅依赖图像和文本的全局表示，忽略了细粒度特征，可能导致错误的图像-文本配对并损害CLIP预训练效果。

Method: 提出基于最优传输的细粒度视觉和文本特征集距离度量，重新分配文本描述，并采用最优传输目标函数促进模态间和模态内的细粒度对齐。

Result: OTCCLIP成功降低了投毒攻击的成功率，相比之前方法显著提升了CLIP在投毒数据集上的零样本和线性探测性能。
...
TL;DR: 提出LFI框架，通过建模视觉理解作为交互过程，解决视觉基础模型从视觉语言模型知识迁移的局限性，在多个基准测试中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型主要采用结果导向范式，忽视了视觉语言模型中跨模态交互过程，这种表示差异阻碍了有效的知识迁移和跨视觉任务的泛化能力。

Method: 提出LFI框架，核心创新包括交互查询（维护跨网络层的持久关系结构）和基于交互的监督（源自视觉语言模型的跨模态注意力机制）。

Result: 在TinyImageNet分类上提升3.3mAP，COCO检测/分割提升1.6mAP/2.4AP，在PACS和VLCS的跨域设置中分别提升2.4和9.3的零样本性能，参数开销小且收敛更快。

Conclusion: LFI框架通过显式建模交互过程，实现了更忠实和高效的知识迁移，在认知对齐方面优于结果导向方法2.7倍，特别擅长跨域设置。
...
TL;DR: 本文提出HyPSAM方法，利用SAM的零样本泛化能力进行RGB-热成像显著目标检测，通过动态融合网络和即插即用优化网络实现高质量的显著目标检测。


<details>
  <summary>Details</summary>
Motivation: 解决RGB-热成像显著目标检测中存在的特征融合不足和数据稀缺问题，提升边界精度和完整目标检测能力。

Method: 提出动态融合网络(DFNet)生成视觉提示，使用动态卷积和多分支解码实现跨模态交互；设计即插即用优化网络(P2RNet)通过混合提示(文本、掩码、边界框)指导SAM优化显著图。

Result: 在三个公开数据集上的实验表明，该方法达到最先进性能，且具有出色的通用性，可与不同RGB-T SOD方法集成实现显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR点云在自动驾驶和机器人感知中易受噪声、遮挡和对抗性攻击的影响，传统自编码器在真实世界挑战性条件下性能下降。

Method: 提出TriFusion-AE模型，通过多模态交叉注意力机制对齐文本语义线索、图像几何特征和LiDAR空间结构，实现联合表示学习。

Result: 在强对抗攻击和重度噪声条件下，模型显著优于CNN自编码器，在nuScenes-mini数据集上验证了有效性。

Conclusion: 该多模态融合框架具有模型无关性，可与任何基于CNN的点云自编码器无缝集成，提高鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频LLM工具使用方法假设工具库固定，难以适应现实世界中工具数据持续演变的动态环境，存在灾难性遗忘问题。
...
TL;DR: FixingGS是一种无需训练的稀疏视角3D高斯泼溅重建增强方法，通过改进的蒸馏方法提供准确且跨视角一致的扩散先验，结合自适应渐进增强方案来消除伪影和补全缺失内容。


<details>
  <summary>Details</summary>
Motivation: 当前基于生成先验的方法在处理稀疏视角3D重建时存在多视角一致性差的问题，导致结构模糊和细节不真实。

Method: 提出FixingGS方法，核心是改进的蒸馏方法，提供更准确和跨视角一致的扩散先验；同时采用自适应渐进增强方案来细化欠约束区域的重建。

Result: 大量实验表明FixingGS在视觉质量和重建性能上优于现有最先进方法。

Conclusion: FixingGS通过充分利用现有扩散模型的能力，有效解决了稀疏视角3D高斯泼溅重建中的伪影和一致性挑战。
...
TL;DR: Bi-VLM提出了一种基于高斯分位数的非均匀权重分离方法，通过将模型权重分为异常值（显著）和多个正常值（非显著）子集，实现超低比特（≤2位）的视觉语言模型量化，显著提升效率。
...
Method: 提出基于高斯分位数的权重分离方法，将权重分为显著和非显著子集，并使用显著性感知混合量化算法对不同子集施加不同的约束条件进行量化。

Result: 在视觉问答任务上，Bi-VLM在语言模型部分比SOTA方法提升3%-47%，在整个VLM上提升4%-45%。量化模型中的图像令牌冗余度达90%-99%，可进一步剪枝提升效率。

Conclusion: Bi-VLM成功实现了超低比特的视觉语言模型量化，显著提升了模型效率，同时保持了良好的性能，为硬件受限环境下的VLM应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法依赖复杂架构、解剖学先验或精心调优的数据增强，限制了可扩展性和泛化能力，且容易在解剖相似度高、病理细微的模态（如胸片）中出现捷径学习问题。
...
Conclusion: DiSSECT通过离散表示瓶颈有效解决了医学图像自监督学习中的捷径学习问题，提供了高效、可迁移的表示学习方法。
...
TL;DR: 本文提出了一种结合热成像、深度学习和车联网通信的实时鹿检测与驾驶员预警系统，旨在减少美国每年210万起鹿车碰撞事故。系统在12000张热成像鹿图像数据集上训练，达到98.84%的平均精度，并在实地测试中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 鹿车碰撞在美国造成严重安全问题，每年导致440人死亡、5.9万人受伤和100亿美元经济损失，同时导致鹿群数量下降。现有可见光摄像头在恶劣天气条件下检测效果不足60%，需要更可靠的技术解决方案。

Method: 系统集成热成像技术、深度学习算法和C-V2X通信技术。使用在北卡罗来纳州Mars Hill收集的12000张热成像鹿图像训练深度学习模型，当检测到高概率鹿存在时，通过C-V2X设备向周围车辆和路边单元广播预警信息。

Result: 系统性能优异：平均精度98.84%，精确率95.44%，召回率95.96%。热成像在恶劣天气下保持88-92%检测准确率，而可见光摄像头不足60%。端到端延迟始终低于100毫秒。实地测试验证了系统在各种天气条件下的稳健运行。

Conclusion: 该研究通过热成像和车联网技术建立了减少鹿车碰撞的可行技术路径，系统能够为驾驶员提供提前预警，有效降低碰撞风险。
...
TL;DR: SAADi框架通过将扩散模型与下游模型偏好的样本对齐，解决了手术数据稀缺问题，避免了数据记忆化导致的样本不一致问题，显著提升了分类和分割任务的性能。


<details>
  <summary>Details</summary>
Motivation: 手术标注数据稀缺限制了深度学习系统在计算机辅助干预中的发展。现有扩散模型虽然能合成逼真图像，但存在数据记忆化问题，导致样本不一致或缺乏多样性，可能损害下游任务性能。

Method: 提出Surgical Application-Aligned Diffusion (SAADi)框架，构建偏好和非偏好合成图像对，通过轻量级微调扩散模型，使图像生成过程与下游目标明确对齐。

Result: 在三个手术数据集上的实验显示，分类任务提升7-9%，分割任务提升2-10%，特别是对代表性不足的类别有显著改进。迭代细化合成样本可进一步提升性能4-10%。
...
TL;DR: 提出了一种基于模型神经网络的动态PET图像去噪方法KMDS-Net，利用帧间空间相关性和帧内结构一致性建立多维稀疏模型，通过神经网络实现自适应参数优化。


<details>
  <summary>Details</summary>
Motivation: 动态PET中短时间帧的图像质量因统计量有限而难以保证，需要有效的去噪方法来提高时空分辨率。

Method: 建立基于核空间的多维稀疏模型，将参数估计替换为神经网络实现自适应优化，形成端到端的KMDS-Net网络。
...
TL;DR: 提出了一种结合光流标签插值和多任务学习的新框架，解决机器人辅助手术中时空标注不平衡问题，提升手术场景理解能力


<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术需要精确理解视觉数据，但现有方法多为单任务，且像素级分割标注成本高，导致长短期标注存在时空不平衡问题

Method: 使用光流估计从标注关键帧向相邻未标注帧传播标签，结合多任务学习来丰富稀疏的空间监督并平衡时空信息

Result: 该集成方法提高了手术场景理解的准确性和效率

Conclusion: 该框架增强了机器人辅助手术的实用性，为复杂手术场景的全面理解提供了有效解决方案
...
TL;DR: Hyper-Bagel是一个统一的多模态模型加速框架，通过推测解码和多阶段蒸馏技术，同时加速多模态理解和生成任务，实现2倍以上的理解速度提升和16-22倍的生成速度提升。
...
Method: 采用分治策略，使用推测解码进行下一token预测，通过多阶段蒸馏过程加速扩散去噪，结合对抗蒸馏和人类反馈学习开发高效的1-NFE模型。

Result: 多模态理解任务实现2倍以上加速；文本到图像生成实现16.67倍加速；图像编辑实现22倍加速；1-NFE模型支持近实时交互式编辑和生成。
...
TL;DR: 本研究评估了多模态大语言模型和视觉语言模型在基督教图像学单标签分类任务中的能力，发现GPT-4o和Gemini 2.5等模型在复杂文化遗产领域的分类表现优于ResNet50基线模型。
...
Method: 使用ArtDL、ICONCLASS和Wikidata三个数据集，在三种条件下测试模型性能：基于类别标签的分类、基于Iconclass描述的分类、以及包含五个示例的小样本学习。结果与在同一数据集上微调的ResNet50基线进行比较。

Result: Gemini-2.5 Pro和GPT-4o的表现优于ResNet50基线。在Wikidata数据集上准确率显著下降，Siglip获得最高准确率。添加类别描述通常能提高零样本性能，而小样本学习效果较差。
...
TL;DR: 本文提出了一种可学习的重参数化图构建方法（LRGC）用于视觉图神经网络，通过键值注意力机制和软阈值重参数化实现可学习的图构建，无需超参数搜索，在ImageNet-1k上超越了同类ViG模型。


<details>
  <summary>Details</summary>
Motivation: 传统ViG模型依赖非参数化的统计方法构建图结构，无法为每个节点选择最优邻域，且需要超参数搜索。现有方法如k-NN图构建、超图构建等缺乏可学习的超参数无关图构建能力。

Method: LRGC方法在每对节点间应用键值注意力机制，然后使用软阈值重参数化进行边选择，通过可微分数学模型进行训练。可学习参数自动调整每层的阈值，无需手动设置超参数。

Result: 在ImageNet-1k基准数据集上，提出的ViG-LRGC方法在相似模型规模下超越了最先进的ViG模型性能。

Conclusion: LRGC提供了一种可学习的超参数无关图构建方法，通过可训练参数自动优化图结构，消除了传统聚类或阈值方法引入的偏差，显著提升了ViG模型的性能。
...
TL;DR: 本文提出结构化反思方法，将错误到修复的路径转化为明确、可控且可训练的动作，通过Reflect-Call-Final策略优化工具调用，显著提升多轮工具调用的成功率和错误恢复能力。


<details>
  <summary>Details</summary>
Motivation: 当前工具增强的大型语言模型通常使用监督模仿或粗粒度强化学习进行训练，自我反思实践依赖启发式提示或单向推理，在多轮交互中脆弱且容易重复错误。

Method: 提出结构化反思方法，结合DAPO和GSPO目标以及针对工具使用的奖励方案，优化Reflect-Call-Final策略。引入Tool-Reflection-Bench基准程序化检查结构有效性、可执行性、参数正确性和结果一致性。

Result: 在BFCL v3和Tool-Reflection-Bench上的实验显示，多轮工具调用成功率和错误恢复能力大幅提升，冗余调用减少。

Conclusion: 使反思明确化并直接优化可提高工具交互的可靠性，为智能体从失败中学习提供可复现的路径。
...
TL;DR: SmartWilds是一个多模态野生动物监测数据集，包含无人机图像、相机陷阱照片和视频以及生物声学记录，用于支持全面的环境监测AI研究。


<details>
  <summary>Details</summary>
Motivation: 解决濒危物种研究、保护生态学和栖息地管理中对多模态AI监测的迫切需求，建立可复现的多模态野生动物监测协议。

Method: 在俄亥俄州野生动物园220英亩牧场进行为期四天的同步监测，收集三种模态数据（无人机图像、相机陷阱、生物声学记录），涵盖多种物种。

Result: 提供了传感器模态性能的比较分析，展示了不同模态在土地利用模式、物种检测、行为分析和栖息地监测方面的互补优势。

Conclusion: 该工作为多模态野生动物监测建立了可复现的协议，并为保护计算机视觉研究贡献了开放数据集，未来将扩展包含GPS跟踪数据和更长时间跨度的数据。
...
TL;DR: RS3DBench是一个用于遥感图像3D理解的新基准数据集，包含54,951对遥感图像和像素级对齐的深度图，配有文本描述，旨在推动通用大规模3D视觉模型在遥感领域的发展。
...
Method: 提出了RS3DBench数据集，并基于稳定扩散模型开发了遥感深度估计模型，利用其多模态融合能力。

Result: 该模型在RS3DBench数据集上实现了最先进的性能。

Conclusion: 这项工作旨在为遥感领域3D视觉感知模型的发展和地理人工智能的进步做出重要贡献，数据集、模型和代码将公开提供。
...
TL;DR: 提出首个无需运动恢复结构（SfM）的基于事件相机的去模糊3D高斯泼溅方法DeblurSplat，通过利用预训练密集立体模块直接获取初始点云，并引入事件流进行精细监督，实现高效高质量的去模糊3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 解决运动模糊问题，避免传统方法中相机位姿估计误差累积到点云位置的问题，利用事件相机对动态变化的高敏感性提升去模糊效果。

Method: 1. 使用预训练DUSt3R模块直接从模糊图像获取准确初始点云，跳过相机位姿计算步骤；2. 将事件流引入去模糊流程，通过解码事件流和模糊图像获得潜在清晰图像作为精细监督信号。
...
Result: 大量实验表明，MoiréNet在公共和实际使用数据集上实现了最先进的性能，同时具有高参数效率。仅需5.513M参数，相比ESDNet-L减少了48%。
...
TL;DR: 本文提出了一种新颖的频率感知音频-视觉分割框架，通过频率域分解和重组来解决音频和视觉模态在频域上的固有矛盾，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉分割方法忽视了音频和视觉模态在频域上的固有矛盾：音频高频信号普遍存在干扰噪声，而视觉高频信号包含丰富的结构细节。忽略这些差异会导致次优性能。

Method: 提出FAVS框架，包含两个关键模块：频率域增强分解器模块（使用基于残差的迭代频率分解来区分模态特定语义和结构特征）和协同跨模态一致性模块（采用专家混合架构通过动态专家路由增强语义一致性和模态特定特征保留）。

Result: 在三个基准数据集上的大量实验表明，FAVS框架实现了最先进的性能，丰富的定性可视化进一步验证了所提出模块的有效性。

Conclusion: 通过将AVS任务重新定义为频率域分解和重组问题，提出的FAVS框架有效解决了音频-视觉模态的频域矛盾，为多模态机器学习提供了新的视角。
...
Method: 通过文献综述方法，系统分析四种xAI方法：显著性图、概念瓶颈模型、原型方法和混合方法。

Result: 提供了对四种xAI方法的全面比较分析，包括各自的机制、优势和局限性。

Conclusion: 该综述为未来xAI研究和应用提供了指导框架，有助于推动可解释AI在视觉感知领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆依赖LiDAR数据进行环境感知，但真实LiDAR数据采集耗时且易受噪声和稀疏性影响，特别是在恶劣天气条件下。需要生成高质量合成数据来增强模型性能。
...
Conclusion: 提出的改进DDPM方法能够生成高质量的合成LiDAR数据，显著提升自动驾驶感知系统的性能，为解决真实数据采集的局限性提供了有效解决方案。
...
TL;DR: AGSSP是一种新颖的预训练范式，通过异常先验指导表示学习，解决金属表面缺陷检测中预训练-微调范式的领域差距问题。


<details>
  <summary>Details</summary>
Motivation: 解决金属表面缺陷检测中预训练面临的困境：在自然图像数据集上预训练存在领域差距，而在工业数据上进行自监督预训练又难以区分细微缺陷与复杂背景噪声。
...
Conclusion: AGSSP通过异常先验指导的自监督预训练，有效解决了金属表面缺陷检测中的领域适应问题，显著提升了检测性能。
...
TL;DR: 本文提出了首个音频驱动的通用逼真头像合成方法，结合了人物无关的语音模型和新的通用头部头像先验（UHAP），能够同时处理几何变形和外观变化。
...
Method: 使用跨身份多视角视频训练UHAP，通过单目编码器进行轻量级个性化，将原始音频直接映射到UHAP潜在表达空间，该空间编码了几何和外观变化。

Result: 该方法能够生成高度逼真的头像，具有精确的唇部同步和细腻的表情细节（如眉毛移动、视线转移和口腔内部外观），在唇同步准确性、图像质量和感知真实性方面优于现有方法。

Conclusion: 这是首个能够进行详细外观建模和渲染的通用音频驱动头像模型，在多个评估指标上均优于现有的几何方法。
...
TL;DR: 本文提出了一种基于机器学习的模块化管道，用于自动检测、时间跟踪和提取3D+时间显微镜数据中树突棘的特征，解决了大规模分析树突棘结构动力学的挑战。


<details>
  <summary>Details</summary>
Motivation: 树突棘是大脑中兴奋性突触的关键结构组成部分，其大小可作为突触效能的指标。然而，在3D+时间显微镜数据中进行大规模树突棘结构动力学分析仍然具有挑战性且劳动密集。

Method: 该方法结合了基于transformer的检测模块、集成空间特征的深度跟踪组件、利用空间一致性跨时间关联3D脊柱的时间跟踪模块，以及量化生物学相关脊柱特征的特征提取单元。

Result: 该方法在开源标记脊柱数据和两个新发布的注释数据集上进行了验证，这些数据集包括检测和深度跟踪数据以及时间跟踪数据，后者是该领域的首个此类数据。

Conclusion: 研究团队发布了数据、代码和预训练权重，为可扩展的端到端树突棘动力学分析建立了基准，促进了未来研究。
...
TL;DR: 提出了一种结合视觉语言模型和预训练视觉模型的零样本图像分类框架，通过自学习循环实现无需标注数据的分类


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在标注数据稀缺场景下的应用难题，利用视觉语言模型和预训练模型来处理零样本分类问题

Method: 采用基于置信度的伪标注策略，在测试数据上直接训练轻量级分类器，通过VLM识别高置信度样本，预训练视觉模型增强视觉表示，迭代训练实现动态适应

Result: 在十个多样化数据集上的实验表明，该方法优于基线零样本方法

Conclusion: 该方法无需VLM微调和大语言模型，仅依赖视觉模型减少对语义表示的依赖，能够有效捕获互补的语义和视觉线索
...
TL;DR: 本文提出MirrorScene3D数据集和ReflectiveGS方法，用于解决含镜面环境中的3D重建和新视角合成问题，通过利用镜面反射作为补充视角来提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法如NeRF和3DGS在含镜面场景中性能下降，主要关注镜面表面的对称映射而忽略了镜面反射携带的丰富信息。这些反射可以提供补充视角，填补缺失细节并显著提升重建质量。

Method: 提出ReflectiveGS方法，基于3D高斯泼溅技术，将镜面反射作为补充视角而非简单的对称伪影，从而增强场景几何并恢复缺失细节。

Result: 在MirrorScene3D数据集上的实验表明，ReflectiveGS在SSIM、PSNR、LPIPS和训练速度方面均优于现有方法。

Conclusion: ReflectiveGS为含镜面环境中的3D重建设定了新的基准，证明了利用镜面反射作为补充视角的有效性。
...
TL;DR: 该论文提出了一种基于深度学习的胆道定位方法，旨在通过Yolo检测算法和GAN生成合成训练数据来提高腹腔镜胆囊切除术中胆管的可视化，从而降低胆管损伤风险。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜胆囊切除术虽然恢复快、美容效果好，但存在较高的胆管损伤风险，严重影响患者生活质量和生存率。为了解决这一问题，需要改进术中胆管的可视化技术。

Method: 构建并标注图像数据库用于训练Yolo检测算法，除了传统的数据增强技术外，还使用生成对抗网络（GAN）生成部分合成训练数据集。

Result: 论文讨论了实验结果，表明该方法能够有效定位胆道结构。

Conclusion: 该方法通过深度学习技术改善了胆管可视化，有望降低腹腔镜胆囊切除术中的胆管损伤风险，同时论文还考虑了相关的伦理问题。
...
Method: 基于SAM的多任务框架，结合辅助中心点检测任务和提示引导对比学习，支持全点、稀疏点或无点提示配置

Result: 在多个挑战性基准测试中优于现有的UDA、WDA和基于SAM的方法

Conclusion: Prompt-DAS框架在领域自适应分割任务中表现出色，提供了灵活高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基准主要关注室内场景或短距离户外活动，缺乏对长距离旅行挑战的探索，而掌握扩展的地理时空轨迹对于下一代MLLMs在具身AI规划和导航等实际应用中至关重要。

Method: 构建包含200个旅行视频的VIR-Bench基准，将行程重建作为评估任务，并对最先进的MLLMs进行实验评估。

Result: 实验结果显示，包括专有模型在内的最先进MLLMs都难以获得高分，表明处理跨越扩展空间和时间尺度的视频具有很大挑战性。

Conclusion: VIR-Bench不仅能有效评估模型性能，还能转化为用户端应用中的具体性能提升，通过案例研究开发的原型旅行规划代理验证了这一点。
...
TL;DR: 本文提出了一种用于视觉语言模型的链式步骤推理框架，通过细粒度的步骤级推理、过程奖励模型和强化学习训练，解决了现有方法在细粒度结构化推理和中间推理质量评估方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的思维链推理方法在视觉语言推理中通常采用粗粒度的推理链，难以进行细粒度结构化推理，并且难以评估中间推理的质量和奖励。

Method: 提出了一个简单有效的透明框架，包括步骤级推理数据、过程奖励模型（PRM）和强化学习训练，通过细粒度的奖励实现有效的强化学习和推理时扩展。

Result: 所提出的方法在具有挑战性的视觉语言基准测试中建立了强大的基线，并取得了一致的改进。通过详细的实证分析和消融研究，揭示了每个组件的影响和推理时扩展的几个有趣特性。

Conclusion: 本文为视觉语言模型提供了一个基准，并为更复杂的多模态推理提供了见解。提出的框架能够准确评估推理步骤质量，实现有效的强化学习和推理时扩展。
...
<details>
  <summary>Details</summary>
Motivation: 解决食物图像分割需要大量像素级标注的问题，利用弱监督学习和预训练模型的能力来降低标注成本
...
TL;DR: DyL-UNet是一种基于动态学习的时序一致性U-Net分割架构，旨在实现超声心动图分割的时间稳定性和精确性。
...
Method: DyL-UNet通过动态学习构建回声动态图（EDG）提取视频动态信息，采用多个基于Swin-Transformer的编码器-解码器分支处理单帧图像，并在跳跃连接处引入心脏相位动态注意力（CPDA）机制。
...
TL;DR: ColorBlindnessEval是一个新颖的基准测试，用于评估视觉语言模型在受色盲测试启发的视觉对抗场景中的鲁棒性。该数据集包含500张类似石原色盲测试的图像，测试VLMs在复杂视觉模式中识别数字信息的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在复杂视觉环境中的鲁棒性存在不足，特别是在对抗性场景下容易产生幻觉问题。研究旨在评估VLMs在类似色盲测试的视觉挑战中的表现。

Method: 创建包含500张石原色盲测试风格图像的数据集，数字范围0-99，采用不同颜色组合。评估9个VLMs模型，使用是/否和开放式提示，并与人类参与者表现进行对比。

Result: 实验显示VLMs在对抗性环境中解释数字的能力存在明显局限，普遍出现幻觉问题，表现不如人类参与者。

Conclusion: 研究强调了提高VLMs在复杂视觉环境中鲁棒性的必要性，ColorBlindnessEval可作为评估和改进VLMs在实际应用中可靠性的重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在稀疏视图设置下性能急剧下降，现有方法使用扩散模型修复损坏的渲染但计算成本高昂。

Method: 将扩散过程转移到小波域：仅在低分辨率LL子带应用扩散，高频子带使用轻量网络细化；提出在线随机掩码策略替代低效的留一法。

Result: 在Mip-NeRF 360和OmniObject3D数据集上的实验表明，WaveletGaussian在保持竞争力的渲染质量的同时显著减少了训练时间。
...
TL;DR: 提出一种无需训练的零样本方法，将多光谱数据适配到仅训练RGB输入的多模态模型中，使通用大模型能够理解专业的多光谱遥感信号


<details>
  <summary>Details</summary>
Motivation: 多光谱图像在遥感应用中很重要，但现有机器学习模型训练成本高，且无法与强大的通用多模态模型兼容，这些模型虽然能解决多种视觉问题，但无法理解专业的多光谱信号

Method: 采用训练免费的方法，将多光谱数据以零样本方式输入到仅训练RGB的多模态模型中，通过适配输入空间并向模型注入领域特定信息作为指令

Result: 在Gemini2.5模型上验证，在土地覆盖和土地利用分类的遥感基准测试中观察到显著的零样本性能提升
...
TL;DR: Citrus-V是一个多模态医学基础模型，结合图像分析和文本推理，在单一框架中实现病灶定位、分割和诊断推理，超越现有开源医学模型。
...
Method: 提出新颖的多模态训练方法，整合检测、分割和多模态链式推理，实现像素级病灶定位、结构化报告生成和医生级诊断推理。

Result: 在多个基准测试中超越现有开源医学模型和专家级影像系统，支持精确病灶量化、自动报告生成和可靠第二意见。

Conclusion: Citrus-V提供了从视觉定位到临床推理的统一流程，为医学影像分析提供了更通用和强大的解决方案。
...
TL;DR: 本研究评估了多模态大语言模型在交通事故检测中的零样本能力，通过整合视觉分析技术提升模型性能，Pixtral模型表现最佳，F1分数达0.71。


<details>
  <summary>Details</summary>
Motivation: 交通事故检测对公共安全至关重要，基础设施视觉传感器可提供实时监控，但缺乏多样化标注数据限制了传统方法的应用。

Method: 使用CARLA模拟的DeepAccident数据集，评估Gemini、Gemma和Pixtral等MLLMs的零样本能力，并整合YOLO、Deep SORT和SAM等视觉分析技术增强提示效果。

Result: Pixtral模型表现最佳（F1-score 0.71，召回率83%），Gemini模型通过增强提示精度提升至90%但召回率下降，Gemma 3性能最均衡。

Conclusion: MLLMs与先进视觉分析技术结合在自动化交通监控系统中具有巨大应用潜力，可减少对标注数据的依赖。
...
Method: 扩展Track-On模型为Track-On2，采用因果处理框架和内存机制保持时间一致性，通过粗粒度块级分类和细化进行推理，系统研究合成训练对内存行为的影响。

Result: 在五个合成和真实世界基准测试中取得最先进结果，超越现有在线跟踪器甚至利用双向上下文的强离线方法。

Conclusion: 基于因果处理和内存机制的架构，结合纯合成数据训练，为现实世界点跟踪提供了可扩展的有效解决方案。
...
TL;DR: KAMERA是一个用于多摄像头、多光谱同步和实时检测海豹与北极熊的综合系统，相比之前方法可减少80%数据处理时间。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够高效处理航空调查中冰相关海豹检测的系统，提高数据处理的效率和准确性。

Method: 采用严格的校准和硬件同步技术，利用多光谱进行目标检测，所有收集的数据都带有元数据注释，便于后续参考。

Result: 在阿拉斯加白令海、楚科奇海和波弗特海的航空调查中，KAMERA系统实现了高达80%的数据处理时间减少。
...
Result: 数据传输减少高达2048倍，边缘能耗降低超过90%，端到端延迟降低3倍，精度损失小于2%。
...
TL;DR: 提出RoSe方法，通过视觉基础模型的先验知识和场景对应学习，提升自监督立体匹配在恶劣天气条件下的性能
...
Method: 注入视觉基础模型的鲁棒先验到CNN特征提取器，构建场景对应先验作为监督信号，提出鲁棒自监督训练范式包括场景对应学习和恶劣天气蒸馏

Result: 在合成恶劣天气立体数据集上实验表明，该方法优于现有最先进的自监督方法

Conclusion: 通过结合视觉基础模型的先验和场景对应学习，可以有效提升立体匹配在恶劣天气条件下的鲁棒性
...
TL;DR: 本文提出YOLO-LAN，一种基于YOLO的息肉检测管道，通过M2IoU损失、数据增强和负样本训练，在结肠镜检测中实现了高精度的实时息肉检测。


<details>
  <summary>Details</summary>
Motivation: 结直肠癌（CRC）是一种致命疾病，始于结肠内壁异常黏膜细胞增殖形成的息肉。结肠镜检查是检测息肉的标准方法，但手动检测存在不一致性和漏检问题。基于深度学习的对象检测能够提供更准确、实时的诊断解决方案。

Method: 提出YOLO-LAN息肉检测管道，使用M2IoU损失函数、多种数据增强技术和负样本训练，以模拟真实临床情况。该方法基于YOLO架构进行优化。

Result: 在Kvasir-seg和BKAI-IGH NeoPolyp数据集上优于现有方法：YOLOv12在Kvasir-seg上达到mAP50为0.9619，mAP50:95为0.8599；YOLOv8达到mAP50为0.9540，mAP50:95为0.8487。mAP50:95分数的显著提升显示了息肉检测的精确性。
...
TL;DR: 本文分析了MOSEv2赛道中的SeC框架，展示了长期记忆和概念感知记忆在视频对象分割中的优势，最终在LSVOS挑战赛中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 探索复杂半监督视频对象分割（LSVOS）挑战中的MOSEv2赛道，研究如何通过改进的SAM-2框架（SeC）来解决长期遮挡和语义干扰等核心挑战。

Method: 通过分析和改进SeC框架，重点研究其长期记忆机制（保持时间连续性）和概念感知记忆（提供语义先验抑制干扰物）。

Result: 在MOSEv2测试集上获得了39.89%的JF分数，在LSVOS挑战赛的MOSEv2赛道中排名第一。

Conclusion: 长期记忆和概念感知记忆的结合能有效应对视频对象分割中的遮挡重现和语义干扰问题，为复杂半监督视频分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM通过序列化图像处理视觉信息，与人类视觉的并行性存在显著差异，且其不透明的内部机制阻碍了深入理解和架构创新。受人类视觉'what'和'where'双流假说启发，作者希望深入理解VLM的内部工作机制。

Method: 1）将图像转换为文本标记图研究物体识别过程；2）理论推导和实证验证VLM位置表示的几何结构；3）提出基于即插即用视觉解码器的指令无关标记压缩算法；4）引入RoPE缩放技术增强空间推理。

Result: 研究发现VLM对图像内容的感知呈现从浅层到深层的两阶段过程：从属性识别开始，最终实现语义消歧。同时验证了VLM位置表示背后的几何结构。

Conclusion: 本研究验证了对VLM内部机制的分析，为深入理解VLM提供了新视角，并为设计更强大的未来架构提供了明确的设计原则。
...
TL;DR: 本文提出了一种无需视觉编码器的文本到文本检索方法，通过VLLM生成结构化图像描述来替代传统文本到图像检索，解决了多模态模型的语言理解浅层、模态鸿沟和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 传统对比学习的视觉语言模型存在语言理解浅层、模态鸿沟问题，且依赖大量网络数据训练带来计算成本和隐私担忧。

Method: 采用文本到文本检索范式，使用VLLM生成结构化图像描述，仅需少量GPU小时进行校准，无需视觉编码器。

Result: 该方法显著减小模态鸿沟，提升组合性，在短长文本查询上表现更好，在多个检索和组合性基准上达到最先进的零样本性能。

Conclusion: 视觉无关的检索方法在性能和隐私方面优于传统多模态模型，即使使用小规模模型也能取得优异结果。
...
TL;DR: 该论文研究了对比视觉语言模型（VLMs）中组合性与长标题理解之间的双向关系，发现两者可以相互促进，但受数据质量和模型设计的影响。
...
Method: 训练和评估了一系列针对组合性和长标题理解能力的模型，研究两者之间的相互作用。
...
<details>
  <summary>Details</summary>
Motivation: 热图像中植物分割在室外高通量表型分析中面临挑战，主要由于植物与杂草对比度低和频繁遮挡导致性能不佳

Method: 使用1,128张合成图像训练模型生成作物和杂草分割掩码，结合少量真实标注图像，利用CycleGAN-turbo实现RGB到热图像的跨模态对齐

Result: 结合所有合成图像和少量真实标注图像，杂草类和植物类的分割性能相比全真实数据基线分别提升了22%和17%
...
TL;DR: 本文提出了HyKid数据集，这是一个包含48名儿童脑积水患者的开源数据集，包含3D MRI图像和手动校正的脑组织分割，特别是脉络丛的分割，为脑积水评估提供了高质量基准。


<details>
  <summary>Details</summary>
Motivation: 儿童脑积水的评估具有挑战性，相关研究受到缺乏公开可用的专家注释数据集（特别是包含脉络丛分割的数据集）的限制。

Method: 使用切片到体积算法从常规低分辨率图像重建3D MRI，由经验丰富的神经学家手动校正脑组织分割，并使用检索增强生成框架从临床放射学报告中提取结构化数据。

Result: 脉络丛体积与总脑脊液体积之间存在强相关性，这为脑积水评估提供了潜在的生物标志物，预测模型表现出色（AUC = 0.87）。
...
TL;DR: 提出了一种多尺度特征交互网络(MsFIN)用于从行车记录仪视频中早期预测交通事故，通过多尺度特征聚合、时序特征处理和后融合来解决交通参与者特征交互建模和多时序行为线索捕获的挑战


<details>
  <summary>Details</summary>
Motivation: 随着行车记录仪的广泛部署和计算机视觉技术的发展，从行车记录仪视角开发事故预测模型对于主动安全干预变得至关重要。但存在两个关键挑战：交通参与者特征级交互建模（在行车记录仪视图中常被遮挡）和捕获事故前复杂异步多时序行为线索

Method: MsFIN包含三个层次：多尺度特征聚合层使用多尺度模块在短、中、长期时间尺度提取场景表示，利用Transformer架构促进全面特征交互；时序特征处理层在因果约束下捕获场景和对象特征的序列演化；多尺度特征后融合层融合多个时间尺度的场景和对象特征生成综合风险表示

Result: 在DAD和DADA数据集上的实验表明，MsFIN在预测准确性和及时性方面显著优于采用单尺度特征提取的最先进模型。消融研究验证了MsFIN中每个模块的有效性

Conclusion: 该网络通过多尺度特征融合和上下文交互建模实现了优越性能，为行车记录仪视角的事故早期预警提供了有效解决方案
...
TL;DR: 本文提出了一种基于持续学习的面部伪造检测方法，采用发展性专家混合架构，通过Real-LoRA学习真实面部特征，多个Fake-LoRA捕获不同伪造类型信息，有效应对不断演变的伪造技术。


<details>
  <summary>Details</summary>
Motivation: 面对数字面部生成和操纵技术的快速发展，现有检测模型难以跟上新型伪造技术的演变速度。需要让模型能够快速适应新领域，同时避免遗忘已学习的伪造类型。

Method: 采用发展性专家混合架构，使用LoRA模型作为专家。分为Real-LoRA组学习真实面部知识，多个Fake-LoRA组捕获不同伪造类型的增量信息。通过正交梯度和正交损失防止梯度干扰和灾难性遗忘。

Result: 在数据集和操纵类型增量协议下的实验结果表明该方法具有有效性。

Conclusion: 将面部伪造检测构建为持续学习问题，通过提出的架构能够有效应对不断演变的伪造技术，在有限计算和数据下实现快速适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态扩散语言模型如MMaDa和Muddit仅支持简单的图像级理解任务和低分辨率图像生成，缺乏高级功能如物体定位和高质量图像编辑。Lavida-O旨在填补这一空白。
...
Conclusion: Lavida-O是首个统一的掩码扩散模型，成功地将图像理解与生成能力结合，通过创新技术实现了高效训练和采样，为多模态AI应用开辟了新方向。
...
TL;DR: 本文提出了基于概念的视频相似度估计任务（ConViS），通过预定义的关键语义概念计算可解释的相似度分数，使视频相似性评估更接近人类的多维度判断方式。


<details>
  <summary>Details</summary>
Motivation: 现有视频相似度评估方法通常依赖全局相似度分数，无法像人类那样从多个不同方面（如动作、地点等）进行细粒度比较。大型多模态模型的出现为利用自然语言进行视频比较任务提供了新机会。

Method: 提出了ConViS任务框架，通过预定义的关键语义概念对视频对进行相似度评估。同时构建了ConViS-Bench基准数据集，包含精心标注的视频对、概念级相似度分数以及差异和相似性的文本描述。

Result: 在ConViS基准上对多个先进模型进行了测试，结果显示不同模型在概念相似度估计上存在显著性能差异，表明某些概念对视频相似度估计更具挑战性。

Conclusion: ConViS-Bench将成为语言驱动视频理解研究的重要资源，有助于推动视频相似度评估向更细粒度、更符合人类判断的方向发展。
...
TL;DR: 本文提出了一种基于对抗性优化的VQ-GAN框架，通过密集运动标记化技术来压缩时空热图，有效解决人体运动理解中的高维度和冗余问题。
...
Method: 采用对抗性优化的VQ-GAN框架，结合密集运动标记化技术，消除重建过程中的运动模糊和时间不对齐等伪影。

Result: 在CMU Panoptic数据集上，该方法比dVAE基线在SSIM指标上提升9.31%，时间不稳定性降低37.1%。研究发现2D运动可用128个标记的紧凑词汇表表示，而3D运动需要1024个标记的更大码本。
...
<details>
  <summary>Details</summary>
Motivation: 传统放射组学方法在聚合感兴趣区域特征时常常忽略复杂的空间关系，难以可靠区分恶性肿瘤与混淆性病理变化。

Method: GrRAiL方法首先使用逐体素放射组学测量识别亚区域簇，然后计算图论指标来量化簇之间的空间关联，生成加权图编码高阶空间关系。

Result: 在多中心研究中，GrRAiL在三个应用场景中均显著优于现有方法：胶质母细胞瘤中测试准确率达78%（提升>10%），脑转移瘤中达74%（提升>13%），胰腺IPMN风险分层中达75%（提升>10%）。

Conclusion: GrRAiL能够有效捕捉病灶内异质性，为临床MRI中可靠区分恶性肿瘤与混淆性病理提供了有前景的工具。
...
TL;DR: CLOPS是首个仅使用自我中心视觉感知环境并导航的人类化身，通过将低级运动技能学习与高级视觉控制解耦，实现了人类化的人类运动生成。


<details>
  <summary>Details</summary>
Motivation: 当前人类运动生成方法忽视了感知与运动之间的相互依赖关系，使用的感知方式与人类感知差异很大。作者认为生成人类化化身行为需要人类化的感知方式。
...
<details>
  <summary>Details</summary>
Motivation: 当前布局到图像生成方法在处理边界框显著重叠时表现不佳，特别是大重叠区域和语义区分度小的重叠实例。现有基准存在偏向简单案例的偏差，无法有效评估模型在挑战性条件下的性能。

Method: 1）提出OverLayScore指标量化边界框重叠复杂度；2）构建OverLayBench基准，包含高质量标注和平衡的OverLayScore分布；3）开发CreatiLayout-AM模型，在amodal掩码数据集上进行微调。

Result: 通过定性和定量分析证明了重叠问题对生成质量的负面影响，新基准填补了现有评估体系的空白，为更鲁棒的布局到图像生成奠定基础。

Conclusion: 该研究为现实挑战性场景下的布局到图像生成提供了系统评估框架和初步解决方案，推动了该领域的发展。
...
Method: 在RGB解码器基础上增加3DGS解码器，通过RGB解码器的输出进行监督训练。3DGS解码器可完全使用视频扩散模型生成的合成数据进行训练
...
TL;DR: VolSplat提出了一种新的多视图前馈3D高斯溅射方法，用体素对齐的高斯预测替代传统的像素对齐方法，解决了现有方法对输入视图数量的依赖、视角偏差密度分布和对齐误差等问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素对齐的3D高斯预测方法存在多个固有局限性：重建的3D模型严重依赖输入视图数量，导致视角偏差的密度分布，在源视图存在遮挡或低纹理时引入对齐误差。

Method: VolSplat采用体素对齐的高斯预测范式，直接从预测的3D体素网格预测高斯分布，避免了像素对齐方法对容易出错的2D特征匹配的依赖，确保鲁棒的多视图一致性，并能基于3D场景复杂度自适应控制高斯密度。

Result: 在RealEstate10K和ScanNet等基准测试上的实验表明，VolSplat实现了最先进的性能，产生了更合理和视角一致的高斯重建，具有更忠实的高斯点云、改进的几何一致性和增强的新视角渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散和流匹配方法需要同时学习质量传输和条件注入，这给模型带来了较大负担。为了减轻模型的学习需求，作者提出了条件感知重参数化方法。
...
Conclusion: CAR-Flow能有效缩短概率路径，加速训练过程，并在不显著增加参数的情况下显著提升模型性能。
...
TL;DR: 本文提出了一个成本效益分析框架，帮助组织确定在什么情况下本地部署LLM比商业订阅服务更经济可行。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的广泛应用，组织面临选择商业LLM服务还是本地部署的决策。商业云服务虽然方便但存在数据隐私、供应商锁定和长期成本等问题，因此需要分析本地部署的经济可行性。

Method: 通过分析最新开源模型（如Qwen、Llama、Mistral等）的硬件需求、运营成本和性能基准，并与主要云服务提供商的订阅费用进行对比。

Result: 研究提供了基于使用水平和性能需求的盈亏平衡点估计。

Conclusion: 该框架为组织规划LLM战略提供了实用的决策工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有土壤湿度时间序列分析方法要么依赖基于阈值的规则，要么使用数据饥渴的机器学习模型，存在适应性和可解释性限制。

Method: 利用ChatGPT-4.1的高级推理能力，将时间序列数据转换为文本表示，设计领域知识提示模板，识别灌溉事件、估计净灌溉增益、检测和分类异常。

Result: 在真实农田数据上的实验表明，SPADE在异常检测方面优于现有方法，召回率和F1分数更高，灌溉事件检测精度和召回率也很高。
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗AI系统无法以符合临床推理的方式明确量化或传达不确定性，现有可解释AI(XAI)和不确定性估计(UE)技术之间存在脱节，限制了AI在医学领域的应用。

Method: 系统地将医学不确定性映射到AI不确定性概念，识别XUE实施的关键挑战，提出包括多模态不确定性量化、模型无关可视化技术和不确定性感知决策支持系统的技术方向。

Result: 提出了确保有效实现XUE的指导原则，强调需要开发既能生成可靠预测又能以临床有意义的方式表达置信度的AI系统。

Conclusion: 这项工作通过桥接可解释性和不确定性，为开发值得信赖的医疗AI做出贡献，为符合现实世界临床复杂性的AI系统铺平道路。
...
TL;DR: Foam-Agent是一个多智能体框架，通过单一自然语言提示自动化整个OpenFOAM CFD工作流程，包括网格生成、HPC脚本创建和后处理可视化，显著降低了CFD的使用门槛。


<details>
  <summary>Details</summary>
Motivation: 计算流体动力学(CFD)模拟工具学习曲线陡峭且设置复杂，现有系统存在自动化程度不足的问题，需要开发能够端到端自动化的解决方案。

Method: 采用多智能体框架，使用模型上下文协议(MCP)暴露核心功能作为可调用工具，通过分层多索引RAG实现高精度配置生成，包含依赖感知的生成过程确保配置一致性。

Result: 在110个模拟任务基准测试中，Foam-Agent使用Claude 3.5 Sonnet实现了88.2%的成功率，显著优于现有框架(MetaOpenFOAM的55.5%)。

Conclusion: Foam-Agent显著降低了CFD的专业知识门槛，展示了专用多智能体系统如何民主化复杂科学计算，代码已开源。
...
### [129] [From latent factors to language: a user study on LLM-generated explanations for an inherently interpretable matrix-based recommender system](https://arxiv.org/abs/2509.18980)
*Maxime Manderlier,Fabian Lecron,Olivier Vu Thanh,Nicolas Gillis*

Main category: cs.AI

TL;DR: 研究大型语言模型是否能从数学可解释的推荐模型中生成有效的用户导向解释，通过用户研究评估不同解释策略的效果。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI研究多依赖自动评估指标，但这些指标往往无法真实反映用户需求和感知，因此需要采用用户中心的方法来评估解释质量。

Method: 使用基于约束矩阵分解的可解释推荐模型，通过精心设计的LLM提示将模型结构转化为自然语言解释，并在326名参与者中进行用户研究，评估五种关键维度的解释质量。

Result: 所有解释类型都受到良好评价，不同策略之间存在中等统计差异，用户评论提供了定量结果之外的补充见解。
...
### [130] [Large Language Models and Operations Research: A Structured Survey](https://arxiv.org/abs/2509.18180)
*Yang Wang,Kai Li*

Main category: cs.AI

TL;DR: 本文综述了大型语言模型在运筹学中的应用进展，主要涵盖自动建模、辅助优化和直接求解三个方向，并讨论了评估基准、领域应用以及当前面临的挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统运筹学方法依赖专家建模和手动参数调整，难以处理大规模、动态和多约束问题。大型语言模型通过语义理解、结构化生成和推理控制，有望解决这些局限性。

Method: 将LLMs与OR集成的方法分为三类：自动建模（将自然语言描述转换为数学模型或可执行代码）、辅助优化（生成启发式算法和进化算法）以及直接求解优化任务。

Result: LLMs在OR领域显示出处理复杂决策问题的潜力，但目前存在语义到结构映射不稳定、研究进展碎片化、泛化能力有限和评估体系不足等挑战。

Conclusion: 本文系统梳理了LLMs在OR中的应用现状，指出了关键开放问题，并为进一步推进LLMs在OR中的作用提供了可能的研究方向。
...
### [131] [Synthesizing Attitudes, Predicting Actions (SAPA): Behavioral Theory-Guided LLMs for Ridesourcing Mode Choice Modeling](https://arxiv.org/abs/2509.18181)
...
Method: SAPA采用分层方法：1）使用LLM从原始出行调查数据生成定性旅行者画像；2）基于人口统计和行为特征训练倾向得分模型；3）LLM为理论驱动的潜在变量分配定量分数；4）最终分类器整合倾向得分、潜在变量分数和可观察行程属性。

Result: 在大规模多年出行调查上的实验表明，SAPA显著优于最先进的基线方法，在测试集上的PR-AUC指标上提升了高达75.9%。

Conclusion: 该研究为准确预测网约车模式选择提供了强大工具，其方法论可轻松迁移到各种应用中。
...
### [132] [An Outcome-Based Educational Recommender System](https://arxiv.org/abs/2509.18186)
*Nursultan Askarbekuly,Timur Fayzrakhmanov,Sladjan Babarogić,Ivan Luković*

Main category: cs.AI

TL;DR: OBER是一个基于结果的教育推荐系统，通过将学习成果和评估项目直接嵌入数据模式，使任何推荐算法都能根据其促进的掌握程度进行评估。


<details>
  <summary>Details</summary>
Motivation: 大多数教育推荐系统仅基于点击或评分相关性进行调优和评估，无法衡量其真正的教学影响。

Method: OBER采用极简的实体关系模型、基于日志的掌握度公式和插件架构，在非正式学习领域的电子学习系统中进行了为期两周的随机分组测试，比较了固定专家路径、协同过滤和基于知识的过滤三种方法。

Result: 协同过滤方法最大化了用户留存率，但固定路径方法实现了最高的掌握度。

Conclusion: OBER框架允许从业者在没有额外测试开销的情况下，权衡相关性、参与度和结果掌握度，且该方法与具体算法无关，可轻松扩展到未来的自适应或上下文感知推荐系统。
...
### [133] [MMCD: Multi-Modal Collaborative Decision-Making for Connected Autonomy with Knowledge Distillation](https://arxiv.org/abs/2509.18198)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统在传感器故障或协作车辆缺失时性能下降，需要一种能够在测试时应对模态缺失的鲁棒决策框架
...
Conclusion: MMCD框架通过多模态融合和知识蒸馏技术，显著提升了自动驾驶系统在挑战性环境中的鲁棒性和安全性
...
### [134] [Change in Quantitative Bipolar Argumentation: Sufficient, Necessary, and Counterfactual Explanations](https://arxiv.org/abs/2509.18215)
...
<details>
  <summary>Details</summary>
Motivation: 在QBAF中进行推理并更新框架时，论证强度排序可能出现不一致性，需要系统化地解释这些变化的原因。

Method: 定义强度不一致性概念，识别充分、必要和反事实解释，开发基于启发式的解释搜索方法并提供实现。

Result: 证明强度不一致性解释存在的充要条件是更新导致强度不一致性，并提供了有效的解释搜索实现。

Conclusion: 该方法能够系统化地解释QBAF中推理变化的原因，为论证系统的动态分析提供了理论基础和实用工具。
...
### [135] [nDNA -- the Semantic Helix of Artificial Cognition](https://arxiv.org/abs/2509.18216)
*Amitava Das*
...
### [136] [Similarity Field Theory: A Mathematical Framework for Intelligence](https://arxiv.org/abs/2509.18218)
*Kei-Sing Ng*
...
Method: 定义了相似性场S: U×U→[0,1]，满足自反性但允许不对称性和非传递性；系统演化序列Z_p=(X_p,S^(p))；概念纤维F_α(K)；生成算子G。基于此形式化智能的定义。
...
### [137] [Multimodal Health Risk Prediction System for Chronic Diseases via Vision-Language Fusion and Large Language Models](https://arxiv.org/abs/2509.18221)
...
<details>
  <summary>Details</summary>
Motivation: 随着慢性疾病负担增加和多模态临床数据的复杂性，需要统一的AI框架来主动预测健康风险。

Method: 采用分层堆叠的视觉-语言多模态Transformer，具有LLM推理头，包含跨模态预训练、时间融合块和疾病本体图适配器三个创新点。

Result: 在MIMIC-IV纵向队列中，平均AUROC达到0.90，预期校准误差为2.7%。

Conclusion: VL-RiskFormer展示了在多模态临床数据上有效预测健康风险的潜力。
...
### [138] [From "What to Eat?" to Perfect Recipe: ChefMind's Chain-of-Exploration for Ambiguous User Intent in Recipe Recommendation](https://arxiv.org/abs/2509.18226)
...
### [139] [An N-Plus-1 GPT Agency for Critical Solution of Mechanical Engineering Analysis Problems](https://arxiv.org/abs/2509.18229)
*Anthony Patera,Rohan Abeyaratne*

Main category: cs.AI

TL;DR: 本文提出了一种"N-Plus-1"GPT代理机构，通过多个独立求解代理和比较代理来提高机械工程问题分析的可靠性，解决GPT单独使用时的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: GPT在机械工程分析中表现出不稳定性，成功概率仅为85%，这种不可靠性使其不适合直接用于教育或工程实践。需要一种方法来提高解决方案的可靠性。

Method: 采用"N-Plus-1"代理架构：首先启动N个独立的求解代理生成N个解决方案，然后通过比较代理对这些方案进行总结、比较，并推荐最佳解决方案。基于孔多塞陪审团定理，当每个求解的成功概率大于1/2时，多数解决方案很可能是正确的。

Result: 与商业多代理模型Grok Heavy相比，该方法在设计和性能上有相似之处，但更注重透明度和教学价值。通过多代理协作显著提高了问题求解的可靠性。

Conclusion: 提出的N-Plus-1代理架构能够有效解决GPT在机械工程分析中的不可靠性问题，通过多数决策机制提高解决方案的正确概率，特别适合教育应用场景。
...
### [140] [Towards General Computer Control with Hierarchical Agents and Multi-Level Action Spaces](https://arxiv.org/abs/2509.18230)
*Zihan Dong,Xinyu Fan,Zixiang Tang,Yunqing Li*

Main category: cs.AI

TL;DR: 本文提出了一种轻量级分层强化学习框架ComputerAgent，用于桌面应用控制，相比现有多模态大语言模型方法，在保持性能的同时大幅减小模型规模并降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型方法存在推理延迟高、样本效率低、无法在设备上部署等问题，需要更实用的计算机控制解决方案。

Method: 采用分层强化学习框架，将操作系统控制建模为两级选项过程（管理器和子策略），使用三模态状态编码器处理视觉和上下文多样性，集成元动作和早停机制减少无效交互，使用紧凑视觉骨干网络和小型策略网络实现设备端推理。

Result: 在135个真实世界桌面任务测试中，简单任务（<8步）成功率92.1%，困难任务（≥8步）成功率58.8%，性能匹配或超过200B参数MLLM基准，同时模型规模减小4个数量级，推理时间减半。
...
### [141] [The Illusion of Readiness: Stress Testing Large Frontier Models on Multimodal Medical Benchmarks](https://arxiv.org/abs/2509.18234)
...
TL;DR: 论文通过压力测试发现，尽管大型前沿模型在医疗基准测试中得分很高，但它们存在脆弱性和走捷径学习的问题，表明当前基准测试奖励应试技巧而非真正的医学理解。


<details>
  <summary>Details</summary>
Motivation: 揭示医疗AI基准测试的局限性，证明高分并不代表真实世界的医疗准备度，需要更严格的评估标准。

Method: 对六个旗舰模型在六个广泛使用的基准上进行评估，包括移除关键输入、改变提示等压力测试，并通过临床医生指导的评分标准进行评估。

Result: 发现领先系统在关键输入被移除时仍能猜对答案，在简单提示变化下会翻转答案，并生成有说服力但有缺陷的推理，基准测试测量内容差异很大但被互换使用。

Conclusion: 医疗基准测试分数不能直接反映真实世界的准备度，AI在医疗领域赢得信任需要超越排行榜胜利，要求系统具备鲁棒性、合理推理和与真实医疗需求的一致性。
...
### [142] [Evaluating the Safety and Skill Reasoning of Large Reasoning Models Under Compute Constraints](https://arxiv.org/abs/2509.18382)
*Adarsha Balaji,Le Chen,Rajeev Thakur,Franck Cappello,Sandeep Madireddy*

Main category: cs.AI

TL;DR: 本研究探讨了两种计算约束策略（推理长度约束和模型量化）对推理模型计算需求和安全性能的影响，提出了基于强化学习的长度控制策略优化方法和量化方法，并分析了计算效率与安全性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 测试时计算扩展虽然能通过生成长链思维序列提高推理语言模型的性能，但带来了显著的计算成本增加。本研究旨在寻找在保持性能的同时降低计算需求的方法。

Method: 1. 使用基于强化学习的长度控制策略优化方法微调推理模型，以满足用户定义的推理长度；2. 应用量化技术，在用户定义的计算约束下最大化链式思维序列的生成。

Result: 研究结果表明，通过推理长度约束和模型量化可以有效降低推理模型的计算需求，但需要平衡计算效率与模型安全性之间的关系。

Conclusion: 计算约束策略是降低推理模型计算成本的有效方法，但在实际应用中需要仔细权衡计算效率与安全性能之间的平衡关系。
...
### [143] [Gödel Test: Can Large Language Models Solve Easy Conjectures?](https://arxiv.org/abs/2509.18383)
*Moran Feldman,Amin Karbasi*

Main category: cs.AI

TL;DR: 本文提出了Gödel测试，评估大型语言模型能否为简单未解决的数学猜想生成正确证明，并在组合优化领域的五个猜想上测试了GPT-5的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然前沿AI模型在中学和大学数学竞赛中表现出色，但尚不清楚它们是否能在更高级的数学领域解决新的简单猜想。

Method: 设计Gödel测试，选择五个组合优化领域的简单未解决猜想，为每个问题提供1-2篇源论文，然后详细评估GPT-5的推理能力。

Result: 在三个较简单问题上GPT-5产生接近正确的解；问题2中甚至推导出不同近似保证，推翻了原猜想；问题4（需结合两篇论文结果）失败；问题5提出正确算法但分析失败。

Conclusion: GPT-5在常规推理上有显著进步，偶尔展现原创性，但在跨论文综合推理方面仍有明显局限，可能代表前沿模型通过Gödel测试的早期步骤。
...
### [144] [ATLAS: Benchmarking and Adapting LLMs for Global Trade via Harmonized Tariff Code Classification](https://arxiv.org/abs/2509.18400)
*Pritish Yuvraj,Siva Devarakonda*

Main category: cs.AI

TL;DR: 该论文提出了首个基于美国海关在线搜索系统的HTS代码分类基准，并开发了Atlas模型（基于LLaMA-3.3-70B），在10位和6位分类上分别达到40%和57.5%的准确率，相比GPT-5和Gemini-2.5有显著提升，且成本更低、可自托管保证数据隐私。


<details>
  <summary>Details</summary>
Motivation: HTS代码分类是全球贸易中的关键瓶颈，但机器学习社区对此关注不足。错误分类可能导致货物运输完全停止，主要邮政运营商因海关文件不完整而暂停对美送货。

Method: 基于美国海关CROSS系统构建首个HTS代码分类基准，对领先的LLM进行评估，并微调Atlas模型（LLaMA-3.3-70B）进行HTS代码分类。

Result: Atlas模型在10位分类上达到40%完全正确率，6位分类达到57.5%正确率，比GPT-5-Thinking提高15个百分点，比Gemini-2.5-Pro-Thinking提高27.5个百分点。成本约为GPT-5的1/5和Gemini-2.5的1/8，且可自托管保证数据隐私。

Conclusion: Atlas模型为HTS分类设立了强基线，但该基准仍极具挑战性（10位准确率仅40%）。通过发布数据集和模型，旨在将HTS分类定位为新的社区基准任务，并鼓励在检索、推理和对齐方面的未来工作。
...
### [145] [Instruction-Following Evaluation in Function Calling for Large Language Models](https://arxiv.org/abs/2509.18420)
*Nikolai Skripko*

Main category: cs.AI

TL;DR: IFEval-FC是一个新的函数调用基准测试，专门评估大语言模型在函数调用中遵循格式指令的能力，填补了现有基准测试只关注参数正确性而忽略格式要求的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试（如BFCL、tau^2-Bench、ACEBench）只评估参数正确性，不测试格式指令遵循能力，而格式遵循对于现实世界AI代理系统至关重要。

Method: 基于IFEval设计，在JSON schema描述中直接编码可验证的格式要求，包含750个测试用例，每个用例包含带有格式要求的函数和相应用户查询，采用全算法评估确保客观性。

Result: 即使最先进的专有模型（如GPT-5和Claude 4.1 Opus）也经常无法遵循基本格式规则，揭示了现实世界代理系统的实际局限性。

Conclusion: IFEval-FC揭示了当前大语言模型在格式指令遵循方面的显著不足，为改进函数调用能力提供了重要基准，代码和数据已开源。
...
### [146] [Memory-QA: Answering Recall Questions Based on Multimodal Memories](https://arxiv.org/abs/2509.18436)
...
TL;DR: Memory-QA是一个新颖的视觉记忆问答任务，Pensieve方法通过记忆增强、时空感知检索和多记忆QA微调，在真实场景中实现了优于现有方法14%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决从多模态记忆中回答视觉内容回忆问题的现实挑战，包括任务导向记忆创建、时空信息利用以及多记忆推理能力。

Method: 提出Pensieve管道，包含记忆特定增强、时空感知多信号检索和多记忆QA微调三个核心组件。

Result: 在构建的多模态基准测试中，Pensieve相比现有最优方法在问答准确率上提升高达14%。

Conclusion: Memory-QA任务具有重要现实意义，Pensieve方法有效解决了多模态记忆问答的关键挑战，为相关研究提供了新方向。
...
### [147] [FERA: Foil Fencing Referee Assistant Using Pose-Based Multi-Label Move Recognition and Rule Reasoning](https://arxiv.org/abs/2509.18527)
...
<details>
  <summary>Details</summary>
Motivation: 击剑运动在裁判方面面临主观判罚、人为错误、偏见以及训练环境中裁判资源有限等挑战

Method: 系统整合了基于姿态的多标签动作识别和基于规则的推理，从视频中提取2D关节位置，计算101维运动学特征，使用Transformer进行多标签动作分类，并应用语言模型进行优先级和得分判定

Result: 在有限的手动标注数据下，5折交叉验证的平均macro-F1得分为0.549，优于TCN、BiLSTM和普通Transformer等多个基线模型

Conclusion: 虽然尚未达到部署水平，但结果显示了在击剑裁判自动化辅助方面的可行前景，并为AI在击剑领域的教练应用开辟了新机会
...
### [148] [LLMZ+: Contextual Prompt Whitelist Principles for Agentic LLMs](https://arxiv.org/abs/2509.18557)
*Tom Pawelek,Raj Patel,Charlotte Crowell,Noorbakhsh Amiri,Sudip Mittal,Shahram Rahimi,Andy Perkins*

Main category: cs.AI

TL;DR: LLMZ+是一种基于提示白名单的新型防御机制，通过只允许上下文适当的消息与代理式LLM交互，有效防御越狱攻击，同时保证合法业务通信不受影响。


<details>
  <summary>Details</summary>
Motivation: 代理式AI相比传统模型具有更高的安全风险，因为它们拥有对数据源和API工具的特权访问权限，且依赖AI的非确定性行为。现有的基于恶意意图检测的防御机制存在局限性，需要更有效的安全解决方案。

Method: 提出LLMZ+方法，采用提示白名单机制，只允许符合预定义用例和操作边界的消息与代理式LLM交互，通过上下文特异性确保所有外部用户与LLM的交换都符合安全要求。

Result: 实证评估显示LLMZ+对最常见的越狱提示具有强韧性，同时不干扰合法业务通信。在实验环境中，误报率和漏报率均可降至0。
...
### [149] [Solving Math Word Problems Using Estimation Verification and Equation Generation](https://arxiv.org/abs/2509.18565)
*Mitchell Piehl,Dillon Wilson,Ananya Kalita,Jugal Kalita*

Main category: cs.AI

TL;DR: 该论文提出了一种新颖的方法，通过分解数学问题生成方程，使用外部符号方程求解器获得答案，并通过二次估算验证答案准确性，在数学应用题求解上取得了新的最先进结果。
...
Method: 首先提示LLM从问题分解中创建方程，使用外部符号方程求解器生成答案；然后让LLM第二次估算正确答案，通过比较估算值与生成值进行验证；如果验证失败，采用迭代修正过程。

Result: 该方法在数值和代数数学应用题数据集上取得了新的最先进结果，平均比之前最佳结果提高了近2%。在三角函数应用题上也获得了满意结果。

Conclusion: 提出的两步验证方法有效提升了LLM解决数学应用题的能力，并引入了两个新数据集SVAMPClean和Trig300来进一步测试LLM的推理能力。
...
### [150] [Adaptive Learning in Spatial Agent-Based Models for Climate Risk Assessment: A Geospatial Framework with Evolutionary Economic Agents](https://arxiv.org/abs/2509.18633)
*Yara Mohajerani*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的地理空间基于代理的模型，将气候灾害数据与经济代理的进化学习相结合，用于评估复杂的气候风险。


<details>
  <summary>Details</summary>
Motivation: 气候风险评估需要模拟空间异质性灾害与适应性经济系统之间的复杂相互作用，现有模型难以捕捉这种复杂动态。

Method: 结合Mesa空间建模和CLIMADA气候影响评估框架，引入适应性学习行为，让企业通过基于适应度的选择和变异来进化预算分配、定价、工资和风险适应策略。

Result: 使用RCP8.5情景下的河流洪水预测到2100年，显示进化适应使企业能够在经历数十年气候压力干扰后恢复到基线生产水平；未直接暴露于洪水的代理也面临供应链中断影响，世纪末商品平均价格比基线高5.6%。
...
### [151] [TERAG: Token-Efficient Graph-Based Retrieval-Augmented Generation](https://arxiv.org/abs/2509.18667)
*Qiao Xiao,Hong Ting Tsang,Jiaxin Bai*

Main category: cs.AI

TL;DR: TERAG是一个低成本图增强检索生成框架，通过个性化PageRank在检索阶段显著减少LLM令牌使用量，仅需3%-11%的输出令牌即可达到主流图RAG方法80%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有图增强RAG系统在构建图时LLM令牌使用成本过高，阻碍了大规模应用。需要开发低成本的图构建方法。

Method: 受HippoRAG启发，在检索阶段引入个性化PageRank(PPR)算法，构建信息丰富的图结构同时大幅降低令牌消耗。

Result: TERAG仅消耗3%-11%的输出令牌，就能达到广泛使用的图RAG方法至少80%的准确率。

Conclusion: TERAG证明了通过优化检索策略可以显著降低图RAG系统的令牌成本，为大规模应用提供了可行方案。
...
### [152] [Implementation of airborne ML models with semantics preservation](https://arxiv.org/abs/2509.18681)
*Nicolas Valot,Louis Fabre,Benjamin Lesage,Ammar Mechouche,Claire Pagetti*

Main category: cs.AI

TL;DR: 本文探讨了机器学习模型在航空系统中的安全合规性，重点区分了ML模型与其明确描述（MLMD）之间的差异，并提出了语义保持的概念以确保模型的准确复制。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在航空系统中的应用日益增多，确保这些系统的安全性和合规性变得至关重要。EASA和EUROCAE/SAE等机构已发布相关指导文件，但需要更具体的方法来验证ML模型在其目标环境中的性能。

Method: 通过澄清ML模型与MLMD之间的区别，并细化语义保持的概念，作者构建并比较了多个目标模型，应用于工业用例中。

Result: 研究提出了确保ML模型安全合规的具体方法，并通过工业用例验证了其有效性。

Conclusion: 本文为ML模型在航空系统中的安全应用提供了理论框架和实践指导，强调了语义保持在模型复制中的重要性。
...
### [153] [Advances in Large Language Models for Medicine](https://arxiv.org/abs/2509.18690)
...
Result: 提出了医学LLMs的三分类法和评估方法的二分类法，识别了当前领域存在的问题并提出了相应解决方案。

Conclusion: 通过系统综述，强调了开发医学LLMs的必要性，深化了对当前发展状况的理解，为后续研究提供了明确指导。
...
### [154] [Autonomous Data Agents: A New Opportunity for Smart Data](https://arxiv.org/abs/2509.18710)
*Yanjie Fu,Dongjie Wang,Wangyang Ying,Xiangliang Zhang,Huan Liu,Jian Pei*

Main category: cs.AI

TL;DR: 本文提出自主数据代理（DataAgents）的概念，通过整合LLM推理与任务分解、行动推理和工具调用，实现从复杂非结构化数据到可操作知识的自动化转换。
...
Method: DataAgents通过动态规划工作流、调用强大工具和适应多样化数据任务，能够自主解释数据任务描述、分解任务、推理行动、将行动转换为Python代码或工具调用，并执行操作。

Result: DataAgents能够处理数据收集、集成、预处理、选择、转换、重新加权、增强、重编程、修复和检索等任务，将复杂非结构化数据转化为连贯可操作的知识。

Conclusion: DataAgents代表了向自主数据到知识系统的范式转变，需要推动行动工作流优化、建立开放数据集和基准生态系统、保障隐私、平衡效率与可扩展性，并开发可信赖的保护机制。
...
### [155] [Experience Scaling: Post-Deployment Evolution For Large Language Models](https://arxiv.org/abs/2509.18771)
...
<details>
  <summary>Details</summary>
Motivation: 传统通过扩大模型规模、训练数据和计算能力的LLM发展方法已接近饱和，人类生成文本资源枯竭，进一步增益有限

Method: 经验扩展框架：捕获原始交互→提炼为紧凑可重用知识→定期优化存储内容以保持相关性和效率

Result: 在模拟真实场景中验证，包括泛化到未见相关任务、重复查询和知识库过饱和情况，经验扩展提高了准确性、维持长期性能并在新情境中保持增益
...
### [156] [The AGNTCY Agent Directory Service: Architecture and Implementation](https://arxiv.org/abs/2509.18787)
*Luca Muscariello,Vijoy Pandey,Ramiz Polic*

Main category: cs.AI

TL;DR: ADS是一个分布式目录服务，用于发现AI代理的能力、元数据和来源，采用内容寻址存储、分层分类和加密签名实现高效、可验证的多维度发现。
...
Result: 实现了高效、可验证的多维度代理发现，支持LLM提示代理、MCP服务器、A2A组件等新兴代理模式。

Conclusion: ADS为多代理系统提供了标准化的代理注册和发现解决方案，在代理互操作性生态系统中具有重要地位。
...
### [157] [Bounded PCTL Model Checking of Large Language Model Outputs](https://arxiv.org/abs/2509.18836)
*Dennis Gross,Helge Spieker,Arnaud Gotlieb*

Main category: cs.AI

TL;DR: LLMCHECKER是一种基于模型检测的方法，用于验证LLM文本生成过程的概率计算树逻辑（PCTL）属性。该方法通过α-k有界文本生成来限制验证范围，重点关注每个生成步骤中top-k令牌的累积概率。


<details>
  <summary>Details</summary>
Motivation: LLM文本生成过程中通常只选择有限数量的令牌，但这些选择并不总是一致的。需要一种形式化方法来验证LLM生成过程的属性，确保文本质量和减少偏见。

Method: 提出α-k有界文本生成方法，在每个生成步骤中只考虑累积概率超过阈值α的top-k令牌。LLMCHECKER利用模型检测技术来正式验证PCTL属性。

Result: 该方法在多个LLM模型（包括Llama、Gemma、Mistral、Genstruct和BERT）上进行了验证，证明了其适用性。

Conclusion: 这是首次将基于PCTL的模型检测应用于验证LLM文本生成过程的一致性，为LLM的形式化验证提供了新方法。
...
### [158] [Model selection meets clinical semantics: Optimizing ICD-10-CM prediction via LLM-as-Judge evaluation, redundancy-aware sampling, and section-aware fine-tuning](https://arxiv.org/abs/2509.18846)
...
TL;DR: 提出一个模块化框架用于ICD-10-CM编码预测，通过系统化模型选择、冗余感知数据采样和结构化输入设计来解决现有LLM在医疗编码中的挑战。


<details>
  <summary>Details</summary>
Motivation: ICD编码对临床文档、计费和分析至关重要，但目前仍是劳动密集且易出错的任务。现有LLM在基础模型选择、输入上下文化和训练数据冗余方面存在限制。

Method: 使用LLM-as-judge评估协议和Plackett-Luce聚合来评估开源LLM，引入嵌入相似度测量和冗余感知采样策略，利用台湾医院的结构化出院摘要评估上下文效果。

Result: 在两家机构数据集上的实验表明，经过微调的基础模型在内部和外部评估中始终优于基线LLM，包含更多临床部分能持续提升预测性能。

Conclusion: 该框架为ICD-10-CM编码预测提供了实用且系统化的方法，结合了明智的模型选择、高效数据优化和上下文感知提示，为自动化医疗编码系统的实际部署提供了可扩展的机构就绪解决方案。
...
### [159] [MAPO: Mixed Advantage Policy Optimization](https://arxiv.org/abs/2509.18849)
...
<details>
  <summary>Details</summary>
Motivation: 现有的GRPO方法在优势函数分配上存在优势反转和优势镜像问题，这阻碍了不同查询样本间合理的优势分配。
...
### [160] [Conf-Profile: A Confidence-Driven Reasoning Paradigm for Label-Free User Profiling](https://arxiv.org/abs/2509.18864)
*Yingxin Li,Jianbo Zhao,Xueyu Ren,Jie Tang,Wangjie You,Xu Chen,Kan Zhou,Chao Feng,Jiao Ran,Yuan Meng,Zhi Wang*

Main category: cs.AI

TL;DR: 提出ProfileBench基准和Conf-Profile框架，通过置信度驱动的两阶段方法解决用户画像中的标签稀缺和噪声问题


<details>
  <summary>Details</summary>
Motivation: 用户画像作为用户理解的核心技术面临缺乏全面基准、大规模真实标签收集困难以及异构噪声用户信息影响LLM可靠性的挑战

Method: Conf-Profile框架采用两阶段范式：1）利用带置信度提示的先进LLM合成高质量标签，通过置信度加权投票和校准；2）通过置信度引导的无监督强化学习增强推理能力

Result: 实验结果表明Conf-Profile通过两阶段训练显著提升性能，在Qwen3-8B模型上F1分数提高了13.97

Conclusion: 该方法实现了标签无关且可靠的用户画像，为解决用户画像中的关键挑战提供了有效方案
...
### [161] [Memory in Large Language Models: Mechanisms, Evaluation and Evolution](https://arxiv.org/abs/2509.18868)
*Dianxing Zhang,Wendong Li,Kani Song,Jiaye Lu,Gang Li,Liuchun Yang,Sheng Li*

Main category: cs.AI

TL;DR: 本文提出了一个统一的LLM记忆定义和四部分分类法（参数化、上下文、外部、程序/情景记忆），建立了记忆四元组框架，并开发了分层评估协议和可治理的更新/遗忘机制DMM Gov。


<details>
  <summary>Details</summary>
Motivation: 为了解决LLM记忆研究中缺乏统一操作定义、评估标准不一致以及治理机制不完善的问题，需要建立一个可复现、可比较和可治理的坐标系。

Method: 采用三设置协议（仅参数化、离线检索、在线检索）解耦能力与信息可用性，构建分层评估框架，并设计DMM Gov机制协调多种技术实现可审计的记忆更新循环。

Result: 建立了包含记忆定义、分类、评估协议和治理机制的综合框架，提出了四个可测试的命题，为LLM记忆研究提供了系统性的方法论基础。

Conclusion: 该框架为LLM记忆的研究和部署提供了可复现、可比较和可治理的坐标系，有助于推动该领域的标准化发展。
...
### [162] [LongCat-Flash-Thinking Technical Report](https://arxiv.org/abs/2509.18883)
...
TL;DR: LongCat-Flash-Thinking是一个5600亿参数的开放源码MoE推理模型，通过精心设计的训练流程实现高效推理能力，包括长链思维数据冷启动和大规模强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 开发一个高效的开放源码推理模型，解决复杂推理任务中的效率问题，特别是在代理推理中减少token消耗。

Method: 采用冷启动训练策略增强推理潜力，然后通过领域并行训练方案将不同领域的专家模型融合为单一模型，使用DORA系统进行大规模RL训练。

Result: 在复杂推理任务上达到开放源码模型的最先进性能，在AIME-25上平均token消耗减少64.5%，从19,653降至6,965，且不降低任务准确率。

Conclusion: LongCat-Flash-Thinking在推理效率和代理AI研究方面具有显著优势，已开源以促进相关领域进一步发展。
...
### [163] [How Far are VLMs from Visual Spatial Intelligence? A Benchmark-Driven Perspective](https://arxiv.org/abs/2509.18905)
...
TL;DR: 本文系统研究了视觉空间推理在视觉语言模型中的现状，提出了空间智能的三级能力分类，并创建了包含20个开源数据集的SIBench基准测试，发现当前模型在感知任务上表现良好但在理解和规划任务上存在明显差距。
...
Method: 系统回顾现有方法（输入模态、模型架构、训练策略、推理机制），将空间智能分为基础感知、空间理解、空间规划三个能力层级，并构建SIBench基准测试（包含23个任务设置的近20个开源数据集）。

Result: 实验表明最先进的视觉语言模型在感知与推理之间存在显著差距：模型在基础感知任务上有竞争力，但在理解和规划任务上持续表现不佳，特别是在数值估计、多视角推理、时间动态和空间想象方面。
...
### [164] [Data Efficient Adaptation in Large Language Models via Continuous Low-Rank Fine-Tuning](https://arxiv.org/abs/2509.18942)
*Xiao Han,Zimo Zhao,Wanyu Wang,Maolin Wang,Zitao Liu,Yi Chang,Xiangyu Zhao*

Main category: cs.AI

TL;DR: 本文提出DEAL框架，通过将低秩适应（LoRA）与连续微调策略相结合，解决传统微调方法中的灾难性遗忘和数据效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法存在灾难性遗忘和数据效率低下的局限性，限制了其在实际应用中的效果。

Method: DEAL框架整合了LoRA和连续微调策略，包含知识保留和自适应参数更新模块，在隐私保护设置下保持效率。
...
### [165] [LLM-based Agents Suffer from Hallucinations: A Survey of Taxonomy, Methods, and Directions](https://arxiv.org/abs/2509.18970)
...
TL;DR: 本文提出了首个关于基于大语言模型（LLM）的智能体中幻觉问题的全面综述，分析了幻觉在不同阶段的表现形式、触发原因，并总结了缓解和检测方法。


<details>
  <summary>Details</summary>
Motivation: 尽管基于LLM的智能体在认知、推理和交互方面展现出强大能力，但它们仍然容易产生幻觉问题，这会影响任务执行的准确性和系统可靠性。为解决这一关键挑战，需要对相关研究进展进行系统整理。

Method: 通过仔细分析智能体的完整工作流程，提出了新的分类法来识别不同阶段出现的幻觉类型；深入研究了18种导致幻觉的触发原因；通过大量现有研究的详细回顾，总结了幻觉缓解和检测方法。

Result: 建立了基于LLM智能体幻觉问题的系统性分析框架，包括分类体系、触发原因识别以及现有解决方案的总结。

Conclusion: 这项调查为理解和管理基于LLM智能体的幻觉问题提供了重要基础，有望推动开发更稳健可靠的智能体系统，并为未来研究指明有前景的方向。
...
TL;DR: 本文比较了四种剩余时间预测方法在物流公司出库仓库流程中的性能，发现深度学习模型精度最高，但浅层方法如传统提升技术在计算资源需求上更具优势。


<details>
  <summary>Details</summary>
Motivation: 预测性过程监控旨在预测正在进行的流程执行的未来，其中剩余时间预测是一个常见目标。本文旨在在真实的物流公司出库仓库流程中比较不同预测方法的性能。

Method: 在物流公司提供的包含169,523条轨迹的原始事件日志上，比较了四种剩余时间预测方法，包括深度学习模型和浅层方法如传统提升技术。

Result: 深度学习模型达到最高精度，但浅层方法如传统提升技术实现了具有竞争力的精度，且需要显著更少的计算资源。

Conclusion: 虽然深度学习在精度上表现最佳，但浅层方法在精度和计算效率之间提供了更好的平衡，特别是在资源受限的实际应用场景中。
...
TL;DR: 该论文提出了基于玩家视角的Landmarks、Monuments和Beacons概念，用于自动分解和评估程序生成内容，旨在改善算法评估与人类体验的一致性。


<details>
  <summary>Details</summary>
Motivation: 解决程序生成内容算法评估中难以找到与人类体验一致的度量标准的问题，特别是对于复合人工制品。

Method: 引入基于可感知性、唤起性和行动召唤的Landmarks、Monuments和Beacons概念，这些概念可跨游戏类型使用，并可通过现有技术进行自动识别和评估。

Result: 提出了一个连接人文学科和技术游戏研究的框架，为程序生成内容的完全自动化分解和重要子组件评估开辟了路径。

Conclusion: 虽然研究重点在混合主动PCG和组合PCG，但该方法具有更广泛的应用潜力，能够实现更好的计算PCG评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有的因果表示学习方法通常需要已知辅助变量来实现可识别性，但这些方法限制辅助变量必须独立于混合函数。然而在某些情况下，系统驱动的潜在因素可以从数据中轻松观测或提取，这可能有助于识别。

Method: 引入可观测源作为辅助变量的框架，使用保体积编码器实现潜在变量的子空间变换和置换识别；当有多个已知辅助变量时，提供变量选择方案来选择那些能最大化潜在因素可恢复性的变量。

Result: 实验在合成图和图像数据上证明了该框架的有效性，能够成功识别潜在变量。
...
TL;DR: CoPiC提出了一种新的LLM规划方法，通过生成高级规划程序和使用领域自适应评估器来减少查询成本并提高长期奖励对齐。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在顺序决策问题中因通用知识与环境特定需求不匹配导致的规划不准确问题，同时减少频繁查询的高成本。

Method: 使用LLM生成多样化高级规划程序，通过领域自适应评估器评估候选计划并选择最符合长期奖励的方案。

Result: 在ALFWorld、NetHack和StarCraft II Unit Building中，CoPiC相比AdaPlanner和Reflexion基线，平均成功率提升23.33%，查询成本降低91.27%。

Conclusion: CoPiC通过程序化规划和领域自适应评估，有效提升了LLM规划性能并显著降低了查询成本。
...
TL;DR: AgentInit是一种多智能体系统初始化方法，通过优化智能体团队结构和引入自然语言格式化机制，提升系统性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有MAS初始化方法未能充分考虑智能体在后续阶段的协作需求，需要更好的团队组合策略来提升系统效率。

Method: AgentInit结合多轮交互和反思机制，引入自然语言到格式的转换机制确保一致性，并应用帕累托原则进行平衡的团队选择策略。

Result: 实验显示AgentInit在多种框架和任务中优于现有方法，性能提升达1.2-1.6倍，同时显著减少令牌消耗。

Conclusion: AgentInit具有良好的可迁移性和适应性，是一种可靠的MAS初始化方法。
...
TL;DR: 该论文研究了大型语言模型在阿拉伯世界的跨文化常识推理迁移，发现仅需少量文化特定示例就能显著提升模型在其他文化背景下的性能，证明了跨文化对齐的可行性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在西方中心偏见，限制了其在多元文化背景下的有效性。虽然已有研究探索文化对齐，但跨文化迁移潜力（利用一种文化的对齐来改进其他文化性能）仍未充分探索。

Method: 使用覆盖13个阿拉伯国家的文化基础常识推理数据集，评估轻量级对齐方法（上下文学习、演示强化DITTO）以及基线方法（监督微调、直接偏好优化）。

Result: 仅需来自一个国家的12个文化特定示例就能使多语言模型在其他国家的性能平均提升10%。来自印尼和美国等异文化演示在多项选择推理中能达到或超过同文化对齐效果。
...
### [172] [Understand your Users, An Ensemble Learning Framework for Natural Noise Filtering in Recommender Systems](https://arxiv.org/abs/2509.18560)
*Clarita Hawat,Wissam Al Jurdi,Jacques Bou Abdo,Jacques Demerjian,Abdallah Makhoul*

Main category: cs.IR

TL;DR: 该论文提出了一种新的框架来识别推荐系统中的噪声评分，通过区分外部因素、意外偏好和偶然交互三种现象，采用模块化三层结构来提高推荐准确性和用户满意度。


<details>
  <summary>Details</summary>
Motivation: 随着网络内容的指数级增长，推荐系统面临噪声定义的挑战，噪声与人类偏好和行为的变异性密切相关。需要区分不同类型的变化现象来提升推荐质量。

Method: 提出模块化框架，包含三层：已知自然噪声算法进行项目分类、集成学习模型进行项目精炼评估、基于签名的噪声识别。同时提出量化意外性和群体验证的指标。

Result: 该方法能够识别噪声评分，提供更干净的训练数据集，从而提高推荐系统的准确性和鲁棒性。

Conclusion: 所提出的框架能够有效改善推荐系统的训练数据质量，进而提升用户满意度和参与度，为推荐系统的噪声处理提供了新的解决方案。
...
### [173] [The Ranking Blind Spot: Decision Hijacking in LLM-based Text Ranking](https://arxiv.org/abs/2509.18575)
*Yaoyao Qian,Yifan Zeng,Yuchao Jiang,Chelsi Jain,Huazheng Wang*

Main category: cs.IR

TL;DR: 本文研究了LLM在多文档比较任务中的"排序盲点"问题，揭示了恶意内容提供者如何通过决策目标劫持和决策标准劫持来操纵LLM排序系统。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在信息检索任务中表现出色，但研究者发现其在多文档比较任务中存在"排序盲点"，这种特性可能被恶意利用来影响文档排序结果。

Method: 通过两种攻击方法进行研究：决策目标劫持（改变成对排序系统的评估目标）和决策标准劫持（修改不同排序方案的相关性标准），并在多种LLM上进行实证验证。

Result: 实验表明这些攻击在多种LLM上都有效，可以推广到多种排序方案，且更强的LLM更容易受到攻击。攻击在真实场景中也显示出有效性。

Conclusion: LLM排序系统存在安全漏洞，恶意内容提供者可以利用排序盲点来操纵排名结果获得额外曝光，需要加强对LLM排序系统的安全防护。
...
### [174] [Agentic AutoSurvey: Let LLMs Survey LLMs](https://arxiv.org/abs/2509.18661)
*Yixin Liu,Yonghui Wu,Denghui Zhang,Lichao Sun*

Main category: cs.IR

TL;DR: Agentic AutoSurvey是一个多智能体框架，用于自动生成文献综述，通过四个专门化智能体协作，在快速发展的科学领域中显著提升了综述质量。
...
Method: 系统采用四个专门化智能体（论文搜索专家、主题挖掘与聚类、学术综述撰写者、质量评估者）协同工作，处理75-443篇论文，通过智能体编排实现高引用覆盖率。

Result: 在COLM 2024六个代表性LLM研究主题上的实验表明，多智能体方法相比现有基线有显著改进，得分8.18/10 vs AutoSurvey的4.77/10。
...
### [175] [Robust Denoising Neural Reranker for Recommender Systems](https://arxiv.org/abs/2509.18736)
*Wenyu Mao,Shuchang Liu,Hailan Yang,Xiaobei Wang,Xiaoyu Yang,Xu Gao,Xiang Li,Lantao Hu,Han Li,Kun Gai,An Zhang,Xiang Wang*

Main category: cs.IR

TL;DR: 本文提出了一种名为DNR的对抗性框架，用于改进多阶段推荐系统中的重排序任务，将重排序视为从检索器分数中去除噪声的问题。


<details>
  <summary>Details</summary>
Motivation: 在多阶段推荐系统中，检索器分数的重要性被有限探索，尽管这些分数包含有用信息。作者认为重排序本质上是从检索器分数中去除噪声的问题。

Method: 提出了DNR对抗框架，包含去噪重排序器和精心设计的噪声生成模块。扩展了传统的分数误差最小化目标，增加了三个增强目标：去噪目标、对抗性检索器分数生成目标和分布正则化项。

Result: 在三个公共数据集上进行了广泛实验，验证了DNR框架的有效性。

Conclusion: DNR框架通过将重排序建模为去噪问题，有效提升了多阶段推荐系统的性能。
...
### [176] [Single-Branch Network Architectures to Close the Modality Gap in Multimodal Recommendation](https://arxiv.org/abs/2509.18807)
*Christian Ganhör,Marta Moscati,Anna Hausberger,Shah Nawaz,Markus Schedl*

Main category: cs.IR

TL;DR: 该论文提出使用单分支神经网络结合权重共享、模态采样和对比损失的方法，在模态缺失情况下提高混合推荐系统的推荐质量，通过缩小模态差距来实现更准确的推荐。


<details>
  <summary>Details</summary>
Motivation: 传统混合推荐系统在模态缺失时推荐质量会下降，需要一种能够在模态不完整情况下仍能提供准确推荐的方法。

Method: 采用单分支神经网络架构，结合权重共享、模态采样和对比损失技术，通过缩小不同模态之间的差距来提升推荐性能。

Result: 在三个数据集上的实验表明，单分支网络在热启动场景中表现竞争性，在模态缺失设置中显著优于多分支网络，并且能够在嵌入空间中实现更紧密的模态接近度。

Conclusion: 单分支神经网络是处理模态缺失推荐问题的有效方法，能够通过缩小模态差距来提升推荐系统的鲁棒性和准确性。
...
### [177] [RELATE: Relation Extraction in Biomedical Abstracts with LLMs and Ontology Constraints](https://arxiv.org/abs/2509.19057)
*Olawumi Olasunkanmi,Mathew Satursky,Hong Yi,Chris Bizon,Harlin Lee,Stanley Ahalt*

Main category: cs.IR

TL;DR: RELATE是一个三阶段管道，将LLM提取的生物医学关系映射到标准化本体谓词，解决LLM输出缺乏标准化和本体对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 生物医学知识图谱对药物发现和临床决策至关重要，但存在不完整性问题。LLM擅长提取生物医学关系，但其输出缺乏标准化和本体对齐，限制了知识图谱集成。

Method: 三阶段管道：1）本体预处理与谓词嵌入；2）基于相似性的检索（使用SapBERT增强）；3）基于LLM的重排序（处理显式否定）。

Result: 在ChemProt基准测试中，RELATE达到52%的精确匹配和94%的准确率@10；在2,400篇HEAL项目摘要中，有效拒绝无关关联（0.4%）并识别否定断言。
...
### [178] [A Knowledge Graph and a Tripartite Evaluation Framework Make Retrieval-Augmented Generation Scalable and Transparent](https://arxiv.org/abs/2509.19209)
*Olalekan K. Akindele,Bhupesh Kumar Mishra,Kenneth Y. Wertheim*

Main category: cs.IR

TL;DR: 该研究提出了一个结合知识图谱和向量搜索的RAG聊天机器人，并开发了RAG-Eval评估框架，用于在工程邮件等高数据量场景下提高回答的准确性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在对话AI中表现出色，但在特定领域应用中仍面临事实准确性不足和避免事实不一致的挑战，特别是在处理大规模数据集时。

Method: 采用检索增强生成（RAG）方法，结合知识图谱和向量搜索检索技术，减少文档分块需求，并开发了基于思维链的三方评估框架RAG-Eval。

Result: 实验表明RAG-Eval在摘要评估任务中优于BERTScore和G-EVAL，能可靠检测事实差距和查询不匹配问题，提供1-100%的置信度评分。

Conclusion: 该方法为开发准确、用户可验证的聊天机器人提供了可扩展路径，弥合了对话流畅性和事实准确性之间的差距。
...
### [179] [Learning Progression-Guided AI Evaluation of Scientific Models To Support Diverse Multi-Modal Understanding in NGSS Classroom](https://arxiv.org/abs/2509.18157)
*Leonora Kaldaras,Tingting Li,Prudence Djagba,Kevin Haudek,Joseph Krajcik*

Main category: cs.CY

TL;DR: 该论文提出了一种基于学习进展(LP)的多模态科学建模评估方法，利用机器学习自动评分和提供个性化反馈，以支持文化语言多样性学生的科学理解发展。


<details>
  <summary>Details</summary>
Motivation: 支持文化语言多样性学生通过多模态建模来发展科学理解，为公平的科学评估提供替代性沟通方式，解决传统评分耗时的问题。

Method: 基于已验证的NGSS对齐多模态LP框架，使用机器学习评估学生对静电现象的科学建模（绘图）和简短文本解释，并设计基于LP的个性化反馈。

Result: 展示了LP如何指导设计基于学生思维多样性的个性化机器学习驱动反馈，适用于多模态评估模式。

Conclusion: 多模态LP框架结合机器学习评分能够有效支持学生的科学建模能力发展，为公平的科学教育评估提供了可行方案。
...
### [180] [Deleuze's "Postscript on the Societies of Control" Updated for Big Data and Predictive Analytics](https://arxiv.org/abs/2509.18194)
*James Brusseau*

Main category: cs.CY

TL;DR: 本文是对德勒兹1990年《控制社会后记》的30年更新，分析了控制社会从广泛理论构想发展为围绕个人信息、大数据、预测分析和营销的具体经济机制的过程。


<details>
  <summary>Details</summary>
Motivation: 探讨德勒兹的控制社会概念如何在过去30年中从理论构想演变为现实的经济机制，特别是通过个人信息和大数据实现的新型控制形式。

Method: 通过理论分析和历史比较方法，追踪控制社会概念的发展轨迹，分析其从哲学理论到具体经济实践的转变过程。

Result: 发现当代控制社会通过非禁止性的激励手段运作，这些激励令人愉悦甚至兴奋，迫使个体服从于自己的个人信息。

Conclusion: 文章最后提出了两种生活策略，这些策略与控制社会本身一样未被充分探索，因为它们正是由控制这种特定压迫方法所揭示和促成的。
...
### [181] [Algorithmic A-Legality: Shorting the Human Future through AI](https://arxiv.org/abs/2509.18195)
*Scott Veitch*

Main category: cs.CY

TL;DR: 本文纠正了当前法律和政治概念及制度能够有效监管AI技术权力的错误认知，指出AI发展依赖经济与法律权力的结合，但其技术形态已超出最严格法律政治体制的监管能力，导致"非法性"现象出现。


<details>
  <summary>Details</summary>
Motivation: 揭示现有法律政治制度在监管新兴AI技术权力方面的局限性，警示AI技术发展已超越传统监管框架的应对能力。

Method: 采用法理学分析方法，论证AI技术发展如何依赖经济与法律权力的结合，并分析其技术形态超越现有监管能力的机制。

Result: 发现AI技术正在形成"非法性"状态，即AI可能造成的危害无法通过传统法律或政治机构得到有效约束。

Conclusion: 当前法律和政治概念及制度不足以应对AI技术带来的监管挑战，需要新的监管框架来应对AI技术权力的超越性特征。
...
### [182] [Dark and Bright Patterns in Cookie Consent Requests](https://arxiv.org/abs/2509.18210)
*Paul Graßl,Hanna Schraffenberger,Frederik Zuiderveen Borgesius,Moniek Buijzen*

Main category: cs.CY

TL;DR: 该研究通过两个预注册在线实验，探讨了暗模式（默认设置、美学操纵、阻碍）对用户cookie同意决策的影响。实验1发现大多数用户无论暗模式如何都会同意所有请求，阻碍隐私友好选项反而增加了感知控制。实验2将设计引导转向隐私友好选项（亮模式），发现阻碍和默认设置能有效引导用户选择隐私友好选项。


<details>
  <summary>Details</summary>
Motivation: 暗模式（邪恶的设计引导）通过说服性界面设计操纵用户行为，在cookie同意请求中越来越常见，可能破坏欧盟隐私法的原则。研究旨在调查常见设计引导对用户同意决策和感知控制的影响。

Method: 采用两个预注册在线实验：实验1（N=228）探索向隐私不友好选项的设计引导（暗模式）；实验2（N=255）将设计引导方向反转至隐私友好选项（亮模式）。测试了三种常见设计引导：默认设置、美学操纵和阻碍。

Result: 实验1：大多数参与者同意所有请求，不受暗模式影响；阻碍隐私友好选项反而增加了感知控制。实验2：阻碍和默认设置能有效引导用户选择隐私友好选项；关于感知控制的结果与实验1一致。

Conclusion: 当前cookie同意请求的许多实现未能让互联网用户做出有意义的选择，不符合欧盟政策制定者的意图。研究还探讨了政策制定者如何解决这一问题。
...
### [183] [Microtargeted propaganda by foreign actors: An interdisciplinary exploration](https://arxiv.org/abs/2509.18211)
*Ronan Ó Fathaigh,Tom Dobber,Frederik Zuiderveen Borgesius,James Shires*

Main category: cs.CY

TL;DR: 本文探讨了外国行为体进行微观目标化宣传的问题，分析了其与传统外国宣传的区别，并提出了欧洲立法者的应对措施。


<details>
  <summary>Details</summary>
Motivation: 研究动机是填补文献中对微观目标化宣传关注不足的空白，特别是外国行为体利用该技术进行政治干预的风险。

Method: 通过比较分析微观目标化宣传与传统宣传的区别，探讨其独特特征和风险。

Result: 发现微观目标化宣传具有更高的精准性、隐蔽性和影响力，能够针对特定群体进行分化性宣传。

Conclusion: 建议欧洲立法者需要制定专门法规来监管微观目标化政治广告，保护民主进程免受外国干预。
...
### [184] [Personalised Pricing: The Demise of the Fixed Price?](https://arxiv.org/abs/2509.18212)
*Joost Poort,Frederik Zuiderveen Borgesius*

Main category: cs.CY

TL;DR: 本文讨论了在线价格歧视的经济学基础、消费者态度以及GDPR法规的应用，指出消费者普遍反对价格歧视并支持禁令，但行业实践并未遵循GDPR的披露和同意要求。


<details>
  <summary>Details</summary>
Motivation: 研究在线价格歧视对公平性和道德性的影响，探讨消费者对价格歧视的态度以及现有法规（如GDPR）如何适用于这种商业行为。

Method: 通过经济理论分析价格歧视的基础，并进行消费者调查来了解公众对在线价格歧视的接受度和看法。

Result: 调查显示消费者对在线价格歧视持批判和怀疑态度，多数人认为其不可接受且不公平，支持禁止该行为，并认为企业应告知消费者价格歧视的存在。

Conclusion: GDPR适用于最具争议的在线价格歧视形式，要求企业披露价格歧视行为并获得消费者事先同意，但行业实践尚未遵循这些原则。
...
### [185] [Enhanced Interpretable Knowledge Tracing for Students Performance Prediction with Human understandable Feature Space](https://arxiv.org/abs/2509.18231)
*Sein Minn,Roger Nkambou*

Main category: cs.CY

TL;DR: 本文提出了一种增强可解释知识追踪模型的方法，通过引入反映学生学习能力的人类可理解特征，在保持与认知理论一致性的同时提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的知识追踪模型虽然预测准确率高，但其复杂性和不透明性阻碍了提供心理学上有意义的解释，限制了在教育应用中的可信度。

Method: 探索从学生交互数据中提取人类可理解的特征，特别是反映学生学习能力的特征，并将这些特征整合到可解释的知识追踪模型中。

Result: 增强后的方法在保持与认知理论对齐的同时，提高了预测准确性。
...
### [186] [Perceptions of AI Across Sectors: A Comparative Review of Public Attitudes](https://arxiv.org/abs/2509.18233)
*Filip Bialy,Mark Elliot,Robert Meckin*

Main category: cs.CY

TL;DR: 本文对2011-2025年间251项关于公众对AI态度的研究进行了领域介导的比较综述，分析了不同因素如何影响公众对AI的接受或抵制。


<details>
  <summary>Details</summary>
Motivation: 系统梳理不同领域和用例中影响公众AI态度的因素，为制定更有针对性的AI治理策略提供基础。

Method: 采用系统性文献综述方法，分析医疗、教育、安全、公共管理、生成式AI和自动驾驶等领域的251项研究。

Result: 发现公众对AI的认知不仅受技术设计和性能影响，还受领域特定考虑、想象、文化叙事和历史遗产等因素塑造。

Conclusion: 比较方法为制定更贴合情境的负责任AI治理策略奠定了基础，强调了情境敏感性在AI治理中的重要性。
...
### [187] [An Artificial Intelligence Value at Risk Approach: Metrics and Models](https://arxiv.org/abs/2509.18394)
*Luis Enriquez Alvarez*

Main category: cs.CY

TL;DR: 本文分析了人工智能风险管理的多维性，指出当前AI风险管理方法不成熟，缺乏具有实际实施价值的指南，并强调需要针对特定AI风险场景定制风险指标和模型。


<details>
  <summary>Details</summary>
Motivation: 随着新AI法规的出现，AI风险管理现状高度不成熟，各部门对AI系统技术方面了解不足，需要开发适当的指标和风险模型来减少决策不确定性。

Method: 通过分解AI风险到多个维度（数据保护、公平性、准确性、鲁棒性、信息安全），开发定制化的风险指标和模型，使用FAIR模型等方法进行风险管理。

Result: 提出了一个全面的AI风险管理框架，展示了AI风险之间的相互依赖性以及如何在风险场景中共同建模。

Conclusion: AI风险管理需要跨部门协作和定制化方法，本文为AI利益相关者提供了风险管理的基本导向和可发展的简单思路。
...
### [188] [Large-Scale, Longitudinal Study of Large Language Models During the 2024 US Election Season](https://arxiv.org/abs/2509.18446)
*Sarah H. Cen,Andrew Ilyas,Hedi Driss,Charlotte Park,Aspen Hopkins,Chara Podimata,Aleksander Mądry*

Main category: cs.CY

TL;DR: 对12个大语言模型在2024年美国总统选举期间进行大规模纵向研究，分析模型行为随时间变化、对人口统计引导的敏感性、对候选人属性的信念以及选举结果的隐性预测


<details>
  <summary>Details</summary>
Motivation: 2024年美国总统选举是LLMs普及后的首次重大选举，需要了解LLMs如何影响信息生态系统和政治话语，以及平台选举保护措施的实际效果

Method: 使用结构化调查对12个模型进行大规模纵向研究，从2024年7月到11月以近每日频率查询超过12,000个问题，系统变化内容和格式

Result: 创建了丰富的数据集，能够分析模型行为随时间变化、对引导的敏感性、对指令的响应性以及选举相关知识和信念

Conclusion: 研究揭示了LLMs在选举背景下的行为模式，为未来评估LLMs在选举环境中的表现提供了方法论和数据集支持
...
### [189] [Developing a Decolonial Mindset for Indigenising Computing Education (CE)](https://arxiv.org/abs/2509.18509)
*Jianhua Li,Yin Paradies,Trina Myers,Robin Doss,Armita Zarnegar,Jack Reis*

Main category: cs.CY

TL;DR: 本文提出了一个七层框架——去殖民心态堆栈（DMS），用于教育工作者在计算教育中实现去殖民化转型，解决原住民在计算教育中的代表性不足问题。


<details>
  <summary>Details</summary>
Motivation: 计算教育中课程、教学法和数字基础设施中嵌入的殖民遗产导致原住民代表性不足，这反映了系统性排斥而非能力问题。

Method: 基于Freire批判教育学和原住民方法论，DMS框架包含七个层次：认知、反思、重构、再嵌入、互惠、重获和复兴，并与"关于我"、"我们之间"、"由我们"的关系视角对齐。

Result: DMS框架培养了自我反思性、关系责任感和原住民主权，为计算教育的去殖民化提供了理论基础和实践路径。

Conclusion: 去殖民化不是终点，而是对变革性正义和与原住民共同创建计算教育的持续伦理承诺，DMS为此提供了系统化框架。
...
### [190] [Automatic coherence-driven inference on arguments](https://arxiv.org/abs/2509.18523)
*Steve Huntsman*

Main category: cs.CY

TL;DR: 提出一种基于大型语言模型和组合优化的神经符号架构，用于检测法律、行政和司法中的不一致性，通过提取命题并进行一致性驱动推理来支持立法、政策分析和法律推理。


<details>
  <summary>Details</summary>
Motivation: 法律、行政和司法领域中普遍存在不一致性问题，需要技术手段来检测和解决这些不一致性，以支持更有效的立法和政策分析。

Method: 使用大型语言模型从论证中准确提取命题，将其编译为自然数据结构，然后通过组合优化进行一致性驱动推理。这种神经符号架构分离了关注点。

Result: 该方法能够对论证的一致性做出有意义的判断，为立法和政策分析提供信息支持。

Conclusion: 提出的神经符号架构为解决法律和行政领域的不一致性问题提供了有效的技术方案，具有实际应用价值。
...
### [191] [Judging Data: Critical Discourse and the Rise of Data Intellectual Property Rights in Chinese Courts](https://arxiv.org/abs/2509.18605)
*Chanhou Lou*

Main category: cs.CY

TL;DR: 本文运用批判性话语分析（CDA）研究中国司法能动主义如何塑造数据知识产权（DIPR），揭示了地方法院（如浙江高院）的司法延续话语和最高法院（SPC）的司法联动话语之间的双向概念耦合机制。


<details>
  <summary>Details</summary>
Motivation: 研究中国司法系统如何在数据知识产权领域通过司法能动主义平衡统一市场标准与防止平台垄断的需求，探索数字治理中的制度协调机制。

Method: 采用批判性话语分析方法，分析浙江高院和最高法院在数据知识产权案件中的司法话语，特别是通过淘宝诉美景案和反不正当竞争司法解释等典型案例。

Result: 发现地方法院通过司法延续话语将知识产权规范扩展到数据争议，最高法院通过司法联动话语将裁判与国家政策和行政治理对齐，形成双向概念耦合机制。

Conclusion: 数据知识产权成为中国数字治理中法律创新和制度协调的试验场，司法能动主义既合法化又约束法院和政策制定者，实现统一标准与反垄断的平衡。
...
### [192] [Purer than pure: how purity reshapes the upstream materiality of the semiconductor industry](https://arxiv.org/abs/2509.18768)
*Gauthier Roussilhe,Thibault Pirson,David Bol,Srinjoy Mitra*

Main category: cs.CY

TL;DR: 本文探讨数字行业的环境影响，特别关注半导体产业中多种元素及其纯度要求对数字行业物质性的影响。


<details>
  <summary>Details</summary>
Motivation: 数字行业的环境影响日益受到关注，但现有讨论往往局限于少数矿产（如钴、锂）。本文旨在通过分析半导体产业中多种元素及其纯度要求，更全面地揭示数字行业的物质性。

Method: 采用基于元素多样性和纯度要求的方法，研究半导体产业中少数全球公司的供应链，特别关注超纯材料的需求。

Result: 研究发现数字行业对其他工业部门（特别是化学工业）存在强烈依赖，揭示了复杂供应链中的新关键参与者。

Conclusion: 需要更深入地调查数字行业与化学工业的互动，作为对采矿业研究的补充，以全面理解数字行业的物质性。
...
### [193] [The AI Literacy Heptagon: A Structured Approach to AI Literacy in Higher Education](https://arxiv.org/abs/2509.18900)
*Veronika Hackl,Alexandra Mueller,Maximilian Sailer*

Main category: cs.CY

TL;DR: 这篇论文通过整合文献综述，探讨了高等教育中AI素养的概念化和实施，提出了包含七个维度的AI素养七边形模型，旨在弥合理论概念与实际课程实施之间的差距。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决高等教育中AI素养概念定义不清晰、缺乏系统框架的问题，以及如何将科学见解有效转化为教育实践。

Method: 采用整合性文献综述方法，分析2021-2024年的研究文献，探索AI素养的定义、概念化及其与相关素养概念的区分。

Result: 识别出AI素养的七个核心维度：技术、应用、批判性思维、伦理、社会、整合和法律，并综合为AI素养七边形模型。

Conclusion: 研究提出了AI素养七边形模型，深化了对AI素养概念的理解，支持高等教育中有结构地发展AI素养，旨在弥合理论与实践的差距。
...
### [194] [A Mega-Study of Digital Twins Reveals Strengths, Weaknesses and Opportunities for Further Improvement](https://arxiv.org/abs/2509.19088)
...
TL;DR: 该研究通过19项预注册研究比较了基于LLM的数字孪生与真实人类在调查和实验中的回答差异，发现数字孪生与人类回答的相关性仅为0.2左右，且数字孪生的回答变异性较低。


<details>
  <summary>Details</summary>
Motivation: 评估当前基于大型语言模型的数字孪生技术是否能够准确捕捉个体在调查和实验中的真实反应，验证数字孪生在社会科学研究中的实用性。

Method: 在美国全国面板上进行19项预注册研究，基于先前收集的个体数据构建LLM驱动的数字孪生，比较孪生与人类在164个结果上的回答。

Result: 数字孪生与人类回答的相关性中等（平均约0.2），回答变异性低于人类。虽然基于丰富个体数据构建的数字孪生能更好地捕捉参与者间的异质性，但无法显著改善对特定参与者确切回答的预测或总体均值的预测。

Conclusion: 当前数字孪生技术能够捕捉一定程度的相对差异，但在个体层面预测和样本均值方差估计方面不可靠，使用前需要谨慎验证。
...
### [195] [Generative Propaganda](https://arxiv.org/abs/2509.19147)
...
TL;DR: 该研究通过访谈台湾和印度的相关从业者，分析了生成式AI在宣传中的实际应用，发现"深度伪造"概念过度影响了防御者对误用的预期，提出了区分明显vs隐藏、宣传vs贬低使用的分类法，并指出欺骗并非主要驱动因素，而是效率和规避检测。


<details>
  <summary>Details</summary>
Motivation: 研究旨在了解生成式AI在真实世界宣传中的使用情况，特别是在台湾和印度这两个网络宣传高发地区，以纠正对AI宣传威胁的误解。

Method: 通过对台湾的防御者（事实核查员、记者、官员）和创作者（影响者、政治顾问、广告商）以及印度的防御者和创作者进行访谈，分析生成式宣传的实际应用模式。

Result: 研究发现欺骗不是AI使用的主要动机，印度创作者更倾向于说服而非欺骗，台湾防御者将欺骗视为更广泛叙事操纵的一部分。AI主要用于提高跨语言沟通效率和规避检测。

Conclusion: 安全研究人员应重新考虑威胁模型，明确区分深度伪造与宣传性明显使用，补充约束内部行为者的社会因素，并应对全球范围内的效率增益问题。
...
TL;DR: 使用图像机器学习模型分析Ulam螺旋中不同区域的质数分布规律，发现在较大整数区域（约5亿附近）比小整数区域（低于2500万）更容易学习到质数模式，表明高阶质数分布具有更强的规律性。


<details>
  <summary>Details</summary>
Motivation: 质数分布具有确定性定义但表现出类似随机过程的统计行为，研究旨在探索机器学习能否作为数论研究的新实验工具，特别是分析质数在不同数量级区域的分布规律差异。

Method: 采用图像聚焦的机器学习模型，从Ulam螺旋的不同区域提取数据块进行训练，比较模型在低整数区域（<25m）和高整数区域（~500m）的学习效果。

Result: 在500m区域训练的模型准确率优于25m区域，表明高阶质数分布存在更易学习的规律性；精度和召回率分析显示模型在不同区域采用不同分类策略。

Conclusion: 机器学习可作为数论研究的新实验工具，特别适用于研究强质数和弱质数模式，为密码学应用提供新视角。
...
### [197] [Fast Linear Solvers via AI-Tuned Markov Chain Monte Carlo-based Matrix Inversion](https://arxiv.org/abs/2509.18452)
...
TL;DR: 提出AI驱动的框架，通过图神经网络预测MCMC参数对Krylov迭代的加速效果，贝叶斯采集函数选择最优参数，在相同搜索预算下比传统方法减少50%迭代次数


<details>
  <summary>Details</summary>
Motivation: 解决Krylov子空间求解器在病态矩阵上收敛慢的问题，传统MCMC预条件器参数优化成本高且依赖手动搜索

Method: 使用图神经网络构建代理模型预测预条件效果，结合贝叶斯采集函数自动选择最优MCMC参数

Result: 在未见过的病态系统上，用传统方法50%的搜索预算实现更好的预条件效果，收敛迭代次数减少约10%

Conclusion: 该框架为大规模系统中集成MCMC预条件器提供了可行路径，展示了AI驱动参数优化的潜力
...
### [198] [Data Valuation and Selection in a Federated Model Marketplace](https://arxiv.org/abs/2509.18104)
*Wenqian Li,Youjia Yang,Ruoxi Jia,Yan Pang*

Main category: cs.LG

TL;DR: 本文提出了一个基于Wasserstein距离的联邦学习框架，用于解决数据市场中数据估值和选择的挑战，通过隐私保护的距离估计和性能预测来优化模型交易。


<details>
  <summary>Details</summary>
Motivation: 在AI时代，数据市场需要可信的模型交易机制来促进数据共享。联邦学习虽然能保护隐私，但在异构数据源的有效估值和选择方面仍面临挑战。

Method: 提出基于Wasserstein距离的估计器，预测模型在未见数据组合上的性能；开发分布式方法来近似Wasserstein距离而不需要原始数据；利用神经缩放定律外推模型性能。

Result: 在标签偏斜、错误标签和无标签等多种场景下的实验表明，该方法能一致地识别高性能数据组合。
...
### [199] [BULL-ODE: Bullwhip Learning with Neural ODEs and Universal Differential Equations under Stochastic Demand](https://arxiv.org/abs/2509.18105)
*Nachiket N. Naik,Prathamesh Dinesh Joshi,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: cs.LG

TL;DR: 该研究比较了完全学习的神经网络ODE（NODE）与物理信息通用微分方程（UDE）在预测牛鞭效应方面的表现，发现结构约束在不同需求模式下对预测精度有不同影响。


<details>
  <summary>Details</summary>
Motivation: 研究结构约束在连续时间库存动态学习中的作用，量化结构在不同需求模式下对牛鞭效应预测的帮助或阻碍。

Method: 使用单级测试平台，在三种需求模式（AR(1)、i.i.d.高斯、重尾对数正态）下比较NODE和UDE方法，通过不同训练数据比例评估多步预测性能。

Result: 在结构化需求模式下，UDE表现更好（AR(1)下库存RMSE从4.92降至0.26）；在重尾分布下，NODE更灵活。训练数据减少时，NODE出现相位漂移，UDE稳定但低估罕见峰值。

Conclusion: 在噪声轻尾或时间相关时强制执行结构约束，在极端事件主导时放松结构约束。这一发现为科学和工程系统中的混合建模提供了指导。
...
### [200] [Model-Based Transfer Learning for Real-Time Damage Assessment of Bridge Networks](https://arxiv.org/abs/2509.18106)
...
<details>
  <summary>Details</summary>
Motivation: 永久监测系统的广泛使用增加了数据可用性，为结构评估提供了新机会，但在大型桥梁网络中面临可扩展性挑战。管理多个结构需要高效跟踪和比较长期行为，因此相似结构间的知识转移变得至关重要。

Method: 使用神经网络代理模型构建模型驱动的迁移学习方法，将在一个桥梁上训练的模型适配到另一个具有相似特征的桥梁。该方法集成到贝叶斯推理框架中，基于监测数据的模态特征进行连续损伤评估。

Result: 使用两个桥梁的真实数据进行验证，结果显示该方法对损伤位置、严重程度和范围具有高敏感性。

Conclusion: 该方法增强了实时监测能力，实现了跨结构知识转移，促进了智能监测策略的制定，提高了网络层面的韧性。
...
### [201] [Individualized non-uniform quantization for vector search](https://arxiv.org/abs/2509.18471)
*Mariano Tepper,Ted Willke*

Main category: cs.LG

TL;DR: NVQ是一种新的向量压缩技术，通过非均匀向量量化在高保真度下实现计算和空间效率的提升


<details>
  <summary>Details</summary>
Motivation: 高维嵌入向量在存储和检索时存在成本高、效率低的问题，需要一种既能保持语义相似性又能减少向量大小的压缩方法

Method: 使用新颖的简约且计算高效的非线性函数构建非均匀向量量化器，并为每个索引向量单独学习量化器

Result: 实验结果显示NVQ在最小计算成本下相比现有技术表现出更高的准确性

Conclusion: NVQ技术在高保真度向量压缩方面具有显著优势，能够有效解决高维向量存储和检索的成本问题
...
### [202] [AdaMixT: Adaptive Weighted Mixture of Multi-Scale Expert Transformers for Time Series Forecasting](https://arxiv.org/abs/2509.18107)
...
Result: 在8个广泛使用的基准数据集上的综合实验表明，AdaMixT在真实场景中具有有效性

Conclusion: AdaMixT能够有效解决多尺度特征融合问题，提升多元时间序列预测的性能和泛化能力
...
Method: 通过可复现的反馈循环集成多个LLM的互补角色（生成器、分析师、评估器），支持多模型协同工作

Result: 提供了一个透明且可扩展的平台，使研究人员和从业者能够在不同领域共同设计算法和其他生成式解决方案

Conclusion: EASE框架有效降低了使用LLM进行算法设计的门槛，提高了解决方案生成的可控性和可复现性
...
TL;DR: 基于AIS轨迹数据的船舶类型分类方法，使用机器学习模型在丹麦博恩霍尔姆海峡实现92.15%的分类准确率


<details>
  <summary>Details</summary>
Motivation: 准确识别船舶类型对于海上安全监管和打击非法、未报告和无管制捕捞活动至关重要

Method: 使用AIS轨迹数据提取31个特征（运动学、时空、船舶形状等），采用随机森林模型和SMOTE技术进行五类船舶分类

Result: 随机森林模型在测试集上达到92.15%准确率，宏平均F1分数93.27%，ROC-AUC最高达0.9897

Conclusion: 基于AIS轨迹的轻量级特征能够实现海峡区域的实时船舶类型分类，具有实际应用价值
...
TL;DR: 提出基于patch的PCA-Net框架，通过将解场分解为小patch并在每个patch内应用PCA，在降维后的PCA空间中训练神经算子，显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统PCA应用于高维解场时计算开销大，需要更高效的降维方法。

Method: 研究两种patch-based方法：局部到全局patch PCA和局部到局部patch PCA，并探索重叠patch平滑滤波和CNN细化两种改进策略。

Result: 基于patch的PCA将端到端处理时间减少3.7-4倍，同时保持高精度。
...
TL;DR: 提出了一种基于上下文优化(CoOp)的新框架，结合子空间表示学习和提示调优，用于提升视觉语言模型在少样本OOD检测中的性能。
...
Method: 通过将ID特征投影到提示向量张成的子空间，同时将ID无关特征投影到正交零空间，提出端到端学习准则，确保强OOD检测性能和高的ID分类准确率。

Result: 在真实世界数据集上的实验证明了该方法的有效性。

Conclusion: 该方法通过整合子空间表示学习和提示调优，显著提升了ID-OOD的可分离性，为AI系统在开放世界环境中的可靠性提供了有效解决方案。
...
TL;DR: 该研究首次全面比较了最先进的AI方法在产前电子胎心监护分析中的表现，发现微调后的大语言模型优于基础模型和领域特定方法。


<details>
  <summary>Details</summary>
Motivation: 电子胎心监护分析对评估胎儿健康至关重要，但目前主要依赖主观临床解读，导致诊断准确性存在差异。基础模型和大语言模型在医疗领域的潜力尚未在CTG分析中得到充分探索。

Method: 系统比较时间序列基础模型、大语言模型与已建立的CTG特定架构，评估超过500个不同时长的CTG记录，提供不同建模范式的稳健性能基准。

Result: 微调后的大语言模型相比基础模型和领域特定方法表现出更优越的性能，为临床CTG解读提供了有前景的替代途径。
...
### [208] [A Study of Skews, Imbalances, and Pathological Conditions in LLM Inference Deployment on GPU Clusters detectable from DPU](https://arxiv.org/abs/2509.18114)
*Javed I. Khan an Henry Uwabor Moye*

Main category: cs.LG

TL;DR: 本文提出了一种利用BlueField-3 DPU来实时检测和缓解多节点张量并行推理中负载不均衡问题的框架，旨在识别多GPU执行LLM时的负载不均衡问题及其对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 大型transformer语言模型的自回归推理在解码阶段面临GPU分片间负载不均衡的问题，导致吞吐量下降和延迟峰值，需要有效的实时监测和缓解机制。

Method: 利用DPU辅助框架，将监控任务卸载到DPU，通过分析GPU遥测数据和节点间通信模式来检测负载不均衡，并为推理控制器和调度器提供可操作的反馈。

Result: 研究旨在识别多GPU执行LLM张量计算时出现的负载不均衡问题，评估其对计算性能的影响，并判断是否可以通过DPU网络进行跟踪和缓解。

Conclusion: DPU辅助的实时负载监测和缓解框架有望提升大型语言模型推理阶段的运行效率，减少负载不均衡导致的性能下降问题。
...
### [209] [Towards Scalable and Structured Spatiotemporal Forecasting](https://arxiv.org/abs/2509.18115)
*Hongyi Chen,Xiucheng Li,Xinyang Chen,Jing Li,Kehai Chen,Liqiang Nie*

Main category: cs.LG

TL;DR: 提出了一种新颖的空间平衡注意力块用于时空预测，通过将空间图划分为子图，分别使用子图内注意力和子图间注意力来平衡局部空间相关性和全局空间相关性，构建了一个多尺度时空预测模型。


<details>
  <summary>Details</summary>
Motivation: 为了在遵循空间邻近性和捕捉全局相关性之间取得平衡，解决现有方法在处理时空数据时对局部和全局空间关系建模不足的问题。

Method: 将空间图划分为子图，使用子图内注意力学习局部空间相关性，通过子图间注意力实现子图间的消息传递以捕捉全局空间相关性，并构建多尺度模型。

Result: 在真实世界的中大型时空数据集上，相比基线方法性能提升最高达7.7%，且运行成本较低。

Conclusion: 所提出的模型既具有可扩展性，又能产生结构化的空间相关性，且易于实现，在效率和效果上都表现出色。
...
### [210] [Amortized Latent Steering: Low-Cost Alternative to Test-Time Optimization](https://arxiv.org/abs/2509.18116)
*Nathan Egbuna,Saatvik Gaur,Sunishchal Dev,Ashwinee Panda,Maheep Chaudhary*

Main category: cs.LG

TL;DR: 本文提出了摊销潜在引导（ALS）方法，通过离线计算单个向量来替代昂贵的逐查询优化循环，在推理时以恒定成本实现潜在空间优化，显著提升效率。
...
Method: ALS计算成功与不成功生成之间隐藏状态的平均差异，使用这个方向来校准模型的隐藏表示。当解码偏离成功流形时，ALS将激活值推回正确方向。

Result: 在GSM8K和MATH-500基准测试中，ALS比迭代方法快2-5倍，同时达到或超过贪婪思维链和自我一致性基线，效率-准确率权衡提升高达101%。

Conclusion: 研究表明，潜在优化的主要好处可以通过离线方式获得，使得复杂推理技术能够在生产环境中实际部署。
...
### [211] [Robust and continuous machine learning of usage habits to adapt digital interfaces to user needs](https://arxiv.org/abs/2509.18117)
*Eric Petit,Denis Chêne*

Main category: cs.LG

TL;DR: 本文提出了一种基于机器学习的数字界面设计方法，能够动态适应用户和使用策略。算法使用贝叶斯统计建模用户浏览行为，专注于个人习惯而非群体偏好，具有在线增量学习能力。


<details>
  <summary>Details</summary>
Motivation: 设计能够动态适应不同用户和使用策略的自适应系统，通过理解个人浏览习惯来改善用户体验，帮助用户更好地导航和操作界面。

Method: 使用贝叶斯统计方法建模用户浏览行为，采用在线增量学习算法，能够生成任务模型并提供当前用户导航的图形化表示和使用统计。算法能够在学习新任务的同时保留先验知识。

Result: 模拟实验表明该方法在静态和非静态环境中都有效，能够在数据量少和环境变化的情况下进行可靠预测。

Conclusion: 这项研究为开发自适应系统铺平了道路，通过帮助用户更好地导航和操作界面来提升用户体验。
...
### [212] [Decentor-V: Lightweight ML Training on Low-Power RISC-V Edge Devices](https://arxiv.org/abs/2509.18118)
*Marcelo Ribeiro,Diogo Costa,Gonçalo Moreira,Sandro Pinto,Tiago Gomes*

Main category: cs.LG

TL;DR: 本文提出了一种针对RISC-V微控制器的8位量化L-SGD算法，解决了RISC-V平台缺乏FPU导致的训练效率问题，实现了内存使用减少4倍和训练速度提升2.2倍。


<details>
  <summary>Details</summary>
Motivation: 现代IoT设备需要在本地进行机器学习训练，但大多数平台缺乏GPU或专用加速器。联邦学习虽然能实现去中心化训练，但需要高效的优化算法。RISC-V作为新兴开源架构，目前缺乏对设备端训练的稳健支持。

Method: 将L-SGD（轻量级随机梯度下降）扩展到RISC-V平台，开发了8位量化版本的L-SGD算法，以应对RISC-V MCU缺乏浮点运算单元的限制。

Result: 在Arm和RISC-V平台上评估32位浮点运算时，发现RISC-V因缺乏FPU性能受限。8位量化L-SGD在RISC-V上实现了内存使用减少近4倍，训练速度提升2.2倍，且精度损失可忽略不计。

Conclusion: 量化L-SGD算法有效解决了RISC-V MCU在设备端训练中的性能瓶颈，为开源架构上的高效联邦学习提供了可行方案。
...
### [213] [MobileRL: Online Agentic Reinforcement Learning for Mobile GUI Agents](https://arxiv.org/abs/2509.18119)
...
Method: 提出ADAGRPO算法，包含难度自适应正向回放和失败课程过滤策略，以及针对多轮任务的最短路径奖励调整策略。

Result: 在Qwen2.5-VL-7B-Instruct和GLM-4.1V-9B-Base模型上应用，MOBILERL-9B在AndroidWorld上达到75.8%成功率，AndroidLab上达到46.8%成功率。
...
### [214] [A Coopetitive-Compatible Data Generation Framework for Cross-silo Federated Learning](https://arxiv.org/abs/2509.18120)
*Thanh Linh Nguyen,Quoc-Viet Pham*

Main category: cs.LG

TL;DR: CoCoGen是一个基于生成AI和势博弈理论的协作学习框架，用于解决跨机构联邦学习中的统计异质性和经济竞争问题，通过数据生成策略最大化社会福利。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注统计异质性，但忽略了机构间的经济竞争关系。竞争可能导致机构因担心效用损失而不愿参与联合训练，且统计异质性和竞争的共同影响尚未充分研究。

Method: 提出CoCoGen框架，通过学习性能和效用公式刻画竞争与统计异质性，将每轮训练建模为加权势博弈，并推导基于生成AI的数据生成策略来最大化社会福利。

Result: 在Fashion-MNIST数据集上的实验表明，CoCoGen在不同异质性和竞争水平下均优于基线方法，有效提升了社会福利。

Conclusion: CoCoGen成功解决了联邦学习中的竞争与异质性挑战，为协作学习在竞争环境下的优化提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索数据驱动方法补充传统咖啡品鉴专业评估，通过机器学习预测咖啡质量评分。

Method: 使用文本清洗、TF-IDF特征提取和SelectKBest特征选择进行数据预处理，训练并评估六种机器学习模型（决策树、K近邻、多层感知器、随机森林、极端随机树和XGBoost）。

Result: 集成方法（极端随机树、随机森林、XGBoost）和多层感知器在F1分数、G-mean和AUC等指标上表现优于决策树和K近邻。
...
### [216] [NurseSchedRL: Attention-Guided Reinforcement Learning for Nurse-Patient Assignment](https://arxiv.org/abs/2509.18125)
*Harsha Koduri*

Main category: cs.LG

TL;DR: 提出了NurseSchedRL强化学习框架，用于解决护士排班优化问题，通过整合结构化状态编码、约束动作掩码和注意力机制，在模拟环境中实现了比传统方法更高效的排班效果。


<details>
  <summary>Details</summary>
Motivation: 医疗系统面临有限护理资源分配压力，传统优化和启发式方法难以处理动态多约束环境，需要更智能的排班解决方案。

Method: 使用近端策略优化（PPO）算法，结合可行性掩码确保分配符合现实约束，动态适应患者到达和护士可用性变化，采用注意力机制表示技能、疲劳和地理上下文。

Result: 在真实护士和患者数据的模拟中，NurseSchedRL相比基线启发式和无约束RL方法，实现了更高的排班效率、更好的技能与患者需求匹配度以及更低的疲劳度。

Conclusion: 强化学习在复杂高风险医疗人力管理决策支持中具有巨大潜力，能够有效处理动态多约束的排班问题。
...
### [217] [Anomaly Detection in Electric Vehicle Charging Stations Using Federated Learning](https://arxiv.org/abs/2509.18126)
...
<details>
  <summary>Details</summary>
Motivation: 随着电动汽车基础设施的快速扩张，保护基于物联网的充电站免受网络威胁变得至关重要。集中式入侵检测系统存在隐私问题，而现有的联邦学习评估忽略了系统异构性和非IID数据等实际挑战。

Method: 使用FedAvg和FedAvgM两种优化方法，在系统异构性和非IID数据条件下进行实验，评估联邦学习在电动汽车充电站异常检测中的性能。
...
### [218] [Safe-SAIL: Towards a Fine-grained Safety Landscape of Large Language Models via Sparse Autoencoder Interpretation Framework](https://arxiv.org/abs/2509.18127)
*Jiaqi Weng,Han Zheng,Hanyu Zhang,Qinqin He,Jialing Tao,Hui Xue,Zhixuan Chu,Xiting Wang*

Main category: cs.LG

TL;DR: Safe-SAIL是一个用于解释大型语言模型中稀疏自编码器（SAE）特征的框架，旨在提高安全领域的机制理解。


<details>
  <summary>Details</summary>
Motivation: 现有安全研究主要关注评估LLM输出或特定安全任务，无法应对更广泛的未定义风险。SAE虽然有助于解释模型行为，但之前的研究未能将特征与细粒度安全概念关联，无法充分解决安全关键行为。

Method: 提出Safe-SAIL框架，系统识别具有最佳概念特定可解释性的SAE，解释安全相关神经元，并引入高效策略来扩展解释过程。

Result: 将发布包含SAE检查点和人类可读神经元解释的完整工具包，支持安全风险的实证分析。

Conclusion: 该框架有助于促进LLM安全研究，通过解释安全相关特征来更好地理解和防范高风险行为。
...
### [219] [Accounting for Uncertainty in Machine Learning Surrogates: A Gauss-Hermite Quadrature Approach to Reliability Analysis](https://arxiv.org/abs/2509.18128)
*Amirreza Tootchi,Xiaoping Du*

Main category: cs.LG

TL;DR: 本文提出了一种高斯-埃尔米特求积方法，用于解耦机器学习代理模型中的认知不确定性和偶然不确定性，从而提高可靠性分析的准确性。


<details>
  <summary>Details</summary>
Motivation: 机器学习代理模型在物理可靠性分析中广泛应用，但其引入的模型近似误差（认知不确定性）与模型输入的偶然不确定性相互耦合，可能影响可靠性预测的准确性。

Method: 采用高斯-埃尔米特求积方法，首先使用一阶和二阶可靠性方法评估偶然不确定性下的条件失效概率，然后对这些概率在认知不确定性的实现上进行积分。

Result: 三个示例表明，该方法在保持计算效率的同时，比忽略模型不确定性的传统方法产生更可信的预测结果。

Conclusion: 所提出的方法能够有效解耦嵌套不确定性，为基于机器学习的可靠性分析提供了更准确的预测框架。
...
### [220] [Research on Metro Transportation Flow Prediction Based on the STL-GRU Combined Model](https://arxiv.org/abs/2509.18130)
...
<details>
  <summary>Details</summary>
Motivation: 在地铁智能交通系统中，准确的换乘客流量预测是优化运营计划和提升运输效率的关键环节，需要改进现有预测理论来为智能运营决策提供更可靠支持。

Method: 创新性地提出STL-GRU组合预测模型：首先使用深度优先搜索算法处理地铁刷卡数据构建换乘客流时间序列，然后通过STL算法将时间序列分解为趋势、周期和残差分量，用3σ原则处理异常值，最后使用GRU模型完成预测。

Result: 实验结果表明，与LSTM、GRU以及STL-LSTM组合模型相比，STL-GRU模型在工作日（除周五外）、周五和休息日的预测精度均有显著提升，MAPE分别降低了至少2.3、1.36和6.42个百分点。

Conclusion: STL-GRU组合预测模型能够有效提高地铁换乘客流量的预测准确性，为地铁智能运营决策提供了可靠的技术支持。
...
### [221] [Two ways to knowledge?](https://arxiv.org/abs/2509.18131)
*Jean-Michel Tucny,Abhisek Ganguly,Santosh Ansumali,Sauro Succi*

Main category: cs.LG

TL;DR: 该论文研究表明，基于Transformer的机器学习应用在解决物理问题时，其权重矩阵呈现随机特性，与物理问题的数学结构没有直接可识别的联系，这表明机器学习与科学方法可能是两种不同但互补的知识获取路径。


<details>
  <summary>Details</summary>
Motivation: 探讨机器学习（特别是Transformer模型）在解决物理问题时的内在工作机制，以及其权重矩阵是否能够反映物理问题的数学和物理结构。

Method: 分析Transformer模型在解决两个代表性物理应用时的权重矩阵特性，并将其与路径积分技术进行类比。

Result: 发现权重矩阵具有随机特性，与物理问题的结构没有直接对应关系，但可以通过与路径积分技术的类比来解释这种随机性。

Conclusion: 机器学习与科学方法可能是互补的知识获取路径，但直接的可解释性（网络参数与物理结构的直接对应）可能难以实现，同时警告了缺乏洞察力的知识获取风险。
...
### [222] [Self-Evolving LLMs via Continual Instruction Tuning](https://arxiv.org/abs/2509.18133)
*Le Huang,Jiazheng Kang,Cheng Hou,Zhe Zhao,Zhenxiang Yan,Chuan Shi,Ting Bai*

Main category: cs.LG

TL;DR: MoE-CL是一种参数高效的对抗性专家混合框架，用于工业规模的大语言模型持续指令调优，通过双专家设计和任务感知鉴别器来平衡知识保留和跨任务泛化。


<details>
  <summary>Details</summary>
Motivation: 在工业环境中，大语言模型需要持续学习以适应动态变化的任务，但现有持续学习方法存在灾难性遗忘问题，新任务训练会降低对先前任务的性能。
...
Conclusion: MoE-CL适用于大规模工业部署，能够实现持续适应和稳定迁移，具有实际应用价值。
...
### [223] [A Weighted Gradient Tracking Privacy-Preserving Method for Distributed Optimization](https://arxiv.org/abs/2509.18134)
*Furan Xie,Bing Liu,Li Chai*

Main category: cs.LG

TL;DR: 本文提出了一种加权梯度跟踪的分布式隐私保护算法，解决了梯度跟踪技术中的隐私泄露风险，通过衰减权重因子消除隐私泄漏，并在时变异构步长下证明了算法的精确收敛性。
...
Conclusion: 所提出的加权梯度跟踪算法成功解决了梯度跟踪中的隐私泄露问题，同时保持了良好的收敛性能，为隐私保护的分布式优化提供了有效解决方案。
...
### [224] [SDGF: Fusing Static and Multi-Scale Dynamic Correlations for Multivariate Time Series Forecasting](https://arxiv.org/abs/2509.18135)
*Shaoxun Wang,Xingjun Zhang,Qianyang Li,Jiawei Cao,Zhendong Tan*

Main category: cs.LG

TL;DR: 提出了一种静态-动态图融合网络（SDGF），通过双路径图结构学习方法捕捉多尺度时间序列间的相关性，解决了现有方法难以建模复杂多尺度依赖关系的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模多尺度时间序列间相关性方面存在局限，难以捕捉复杂且动态变化的依赖关系，这影响了多元时间序列预测的准确性。

Method: 使用基于先验知识的静态图锚定长期稳定依赖关系，同时采用多级小波分解提取多尺度特征构建自适应学习的动态图，设计了注意力门控模块融合两种信息源，并使用多核扩张卷积网络加深对时间模式的理解。

Result: 在多个广泛使用的真实世界基准数据集上的综合实验证明了所提出模型的有效性。

Conclusion: SDGF模型能够有效捕捉多尺度时间序列间的复杂相关性，显著提升了多元时间序列预测的准确性。
...
### [225] [From Parameters to Performance: A Data-Driven Study on LLM Structure and Development](https://arxiv.org/abs/2509.18136)
*Suqing Wang,Zuchao Li,Luohe Shi,Bo Du,Hai Zhao,Yun Li,Qianren Wang*

Main category: cs.LG

TL;DR: 该论文提出了一个大规模数据集，系统分析了大型语言模型（LLMs）结构配置与性能之间的关系，通过数据挖掘和机制解释技术验证结构选择对性能的影响。
...
Method: 构建包含多样化开源LLM结构及其在多个基准测试中性能的大规模数据集，采用数据挖掘驱动的系统性分析方法，并结合机制解释技术验证发现。

Result: 量化了结构配置与性能之间的关系，为LLM优化提供了数据驱动的见解。

Conclusion: 该研究旨在指导未来模型的针对性开发和应用，数据集将在Hugging Face上公开发布。
...
### [226] [LoRALib: A Standardized Benchmark for Evaluating LoRA-MoE Methods](https://arxiv.org/abs/2509.18137)
*Shaoheng Wang,Yao Lu,Yuqi Li,Yaxin Gao,Jiaqi Nie,Shanqing Yu,Yingli Tian,Qi Xuan*

Main category: cs.LG

TL;DR: 提出了一个名为LoRALib的统一基准，用于标准化LoRA-MoE方法的评估，通过40个下游任务的数据集和17种模型架构获得了680个LoRA模块，并对3种代表性LoRA-MoE方法进行了大规模实验。
...
Method: 将40个下游任务的数据集标准化为统一格式，使用相同超参数进行微调，获得680个LoRA模块，基于OpenCompass测试工具对3种代表性LoRA-MoE方法和不同LoRA选择机制进行大规模实验。
...
### [227] [Rank-Induced PL Mirror Descent: A Rank-Faithful Second-Order Algorithm for Sleeping Experts](https://arxiv.org/abs/2509.18138)
*Tiantian Zhang*

Main category: cs.LG

TL;DR: RIPLM算法利用排名基准与分布基准的结构等价性，直接在排名诱导的Plackett-Luce参数化中更新，确保算法在每一轮都保持排名诱导分布的特性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常基于专家身份进行操作，而RIPLM旨在开发一种既保持排名忠实性又具有方差自适应性的算法，特别是在睡眠专家设置中。

Method: RIPLM通过排名诱导的Plackett-Luce镜像下降算法，直接在排名诱导的PL参数化中进行更新，确保分布始终属于排名诱导分布类。

Result: RIPLM是第一个在睡眠专家设置中同时满足排名忠实性和方差自适应性的算法。

Conclusion: 该算法通过结构等价性保持了与排名基准的等价关系，为排名诱导分布提供了有效的更新机制。
...
### [228] [Comparative Analysis of FOLD-SE vs. FOLD-R++ in Binary Classification and XGBoost in Multi-Category Classification](https://arxiv.org/abs/2509.18139)
*Akshay Murthy,Shawn Sebastian,Manil Shangle,Huaduo Wang,Sopam Dasgupta,Gopal Gupta*

Main category: cs.LG

TL;DR: 本研究比较了规则分类器FOLD-SE与FOLD-R++在二分类任务中的表现，以及FOLD-SE与XGBoost在多分类任务中的性能。结果显示FOLD-SE在保持可解释性的同时，在准确性和效率方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 随着对兼具准确性、效率和可解释性的机器学习模型需求增长，需要开发能够平衡这些特性的算法。传统模型在准确性和可解释性之间存在权衡，因此需要研究像FOLD-SE这样的规则分类器是否能弥合这一差距。
...
Conclusion: FOLD-SE等基于规则的方法能够弥合可解释性与性能之间的差距，是多样化分类任务中黑盒模型的可行替代方案。
...
### [229] [A Machine Learning Framework for Pathway-Driven Therapeutic Target Discovery in Metabolic Disorders](https://arxiv.org/abs/2509.18140)
*Iram Wajahat,Amritpal Singh,Fazel Keshtkar,Syed Ahmad Chan Bukhari*

Main category: cs.LG

TL;DR: 本研究开发了一个结合逻辑回归和主成分分析的机器学习框架，用于预测2型糖尿病风险，并提出基因无关的途径映射方法来生成机制性见解，识别针对高风险人群的新型治疗策略。


<details>
  <summary>Details</summary>
Motivation: 代谢性疾病特别是2型糖尿病是全球重大健康负担，对遗传易感人群如皮马印第安人影响尤为严重。需要开发可解释且可扩展的解决方案来实现早期检测和靶向干预。

Method: 使用皮马印第安人数据集，应用逻辑回归和t检验识别2型糖尿病的关键预测因子，开发途径映射策略将预测因子与关键信号网络（如胰岛素信号、AMPK和PPAR途径）联系起来。

Result: 模型整体准确率达到78.43%，通过途径富集分析验证了包括双重GLP-1/GIP受体激动剂、AMPK激活剂、SIRT1调节剂和植物化学物质在内的治疗策略。

Conclusion: 该框架通过提供可解释和可扩展的解决方案，推进了精准医学在代谢性疾病早期检测和靶向干预方面的应用。
...
### [230] [KM-GPT: An Automated Pipeline for Reconstructing Individual Patient Data from Kaplan-Meier Plots](https://arxiv.org/abs/2509.18141)
*Yao Zhao,Haoyue Sun,Yantian Ding,Yanxun Xu*

Main category: cs.LG

TL;DR: KM-GPT是一个全自动AI驱动的管道，用于从Kaplan-Meier图中高精度重建个体患者数据，解决了传统手动数字化的局限。
...
### [231] [AdaSTI: Conditional Diffusion Models with Adaptive Dependency Modeling for Spatio-Temporal Imputation](https://arxiv.org/abs/2509.18144)
*Yubo Yang,Yichen Zhu,Bo Jiang*

Main category: cs.LG

TL;DR: AdaSTI是一种基于条件扩散模型的时空数据插补方法，通过双向S4模型进行预插补，并利用噪声感知时空网络捕获不同扩散步骤中的依赖关系，在三个真实数据集上实现了高达46.4%的插补误差降低。
...
Method: 提出AdaSTI框架，包含基于双向S4模型的BiS4PI网络进行预插补，设计时空条件化器(STC)网络提取条件信息，以及具有门控注意力机制的噪声感知时空(NAST)网络来捕获不同扩散步骤的变异性依赖。

Result: 在三个真实世界数据集上的实验表明，AdaSTI在所有设置下都优于现有方法，插补误差最多降低了46.4%。

Conclusion: AdaSTI通过自适应捕获不同扩散步骤中的时空依赖关系，显著提升了时空数据插补性能，证明了该方法在处理复杂时空依赖关系方面的有效性。
...
### [232] [Early Prediction of Multi-Label Care Escalation Triggers in the Intensive Care Unit Using Electronic Health Records](https://arxiv.org/abs/2509.18145)
*Syed Ahmad Chan Bukhari,Amritpal Singh,Shifath Hossain,Iram Wajahat*

Main category: cs.LG

TL;DR: 本研究提出了一个多标签分类框架，使用ICU入院前24小时数据预测护理升级触发事件（CETs），包括呼吸衰竭、血流动力学不稳定、肾功能损害和神经功能恶化。基于MIMIC-IV数据库，XGBoost模型在四个CET类别上表现最佳。
...
Method: 使用MIMIC-IV数据库中85,242例ICU住院数据，基于前24小时的生命体征、实验室值和人口统计学特征，训练多标签分类模型预测24-72小时内发生的CETs。CETs通过基于规则的临床标准定义。

Result: XGBoost模型在四个CET类别上的F1分数分别为：呼吸0.66、血流动力学0.72、肾脏0.76、神经0.62。特征分析显示呼吸频率、血压和肌酐等临床相关参数是最具影响力的预测因子。

Conclusion: 该框架展示了早期、可解释的临床预警潜力，无需复杂的时序建模或自然语言处理，与CETs的临床定义一致。
...
### [233] [ConceptFlow: Hierarchical and Fine-grained Concept-Based Explanation for Convolutional Neural Networks](https://arxiv.org/abs/2509.18147)
*Xinyu Mu,Hui Dou,Furao Shen,Jian Zhao*

Main category: cs.LG

TL;DR: 提出了ConceptFlow框架，通过追踪概念在CNN各层中的出现和演化来模拟模型的内部"思考路径"，包含概念注意力和概念通路两个关键组件。


<details>
  <summary>Details</summary>
Motivation: 现有的基于概念的CNN可解释性方法忽视了单个过滤器的语义角色和概念在层间的动态传播，需要更全面地理解模型的内部推理过程。

Method: ConceptFlow框架包括：(i)概念注意力-将每个过滤器与相关高级概念关联；(ii)概念通路-通过概念转移矩阵量化概念在过滤器间的传播和转换。

Result: 实验结果表明ConceptFlow能够产生语义上有意义的模型推理洞察，验证了概念注意力和概念通路在解释决策行为方面的有效性。

Conclusion: 通过建模层次化的概念通路，ConceptFlow为CNN的内部逻辑提供了更深入的洞察，支持生成更忠实且与人类认知对齐的解释。
...
### [234] [Sparse Training Scheme for Multimodal LLM](https://arxiv.org/abs/2509.18150)
*Kean Shi,Liang Chen,Haozhe Zhao,Baobao Chang*

Main category: cs.LG

TL;DR: 提出了一种基于稀疏表示的训练高效框架STS，通过视觉标记压缩器和层动态跳过器来减少计算开销，提高多模态大语言模型的训练效率。
...
Method: STS框架包含两个核心组件：视觉标记压缩器（压缩视觉标记减少信息负载）和层动态跳过器（在前后向传播中动态跳过语言模型的不必要层）。

Result: 在多个基准测试上进行了广泛评估，证明了该方法的有效性和效率。

Conclusion: 该方法适用于多种MLLM架构，能够显著提高训练效率。
...
### [235] [HyperNAS: Enhancing Architecture Representation for NAS Predictor via Hypernetwork](https://arxiv.org/abs/2509.18151)
...
### [236] [WLFM: A Well-Logs Foundation Model for Multi-Task and Cross-Well Geological Interpretation](https://arxiv.org/abs/2509.18152)
*Zhenyu Qi,Qing Yu,Jichen Wang,Yun-Bo Zhao,Zerui Li,Wenjun Lv*

Main category: cs.LG

TL;DR: WLFM是一个用于测井解释的基础模型，通过三阶段训练在1200口井的多曲线测井数据上预训练，在孔隙度估计和岩性分类任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决测井解释中工具响应异质性、噪声信号和标签有限等挑战，为地质AI提供可扩展、可解释和可迁移的骨干网络。

Method: 三阶段方法：1) 将测井数据块标记化为地质标记；2) 使用掩码标记建模和地层感知对比学习进行自监督预训练；3) 通过少样本微调进行多任务适配。

Result: WLFM在孔隙度估计上达到0.0041 MSE，岩性分类准确率74.13%；微调后分别提升至0.0038 MSE和78.10%准确率。模型展现出层感知能力，可学习可重用的地质词汇表。
...
### [237] [A deep reinforcement learning platform for antibiotic discovery](https://arxiv.org/abs/2509.18153)
...
TL;DR: ApexAmphion是一个深度学习框架，通过结合64亿参数蛋白质语言模型和强化学习，用于从头设计抗生素。该框架在体外评估中显示出100%的成功率和纳米级MIC值。
...
Method: 该框架首先在精选的肽数据上进行微调以捕捉抗菌序列规律，然后使用近端策略优化结合MIC分类器预测和可微分物理化学目标进行优化。

Result: 100种设计肽的体外评估显示所有候选物均具有低MIC值（某些情况下为纳摩尔范围），且99/100的化合物对至少两种临床相关细菌表现出广谱抗菌活性。

Conclusion: 通过将生成、评分和多目标优化与深度强化学习统一在单一流程中，该方法能快速产生多样、强效的候选物，为肽抗生素提供了可扩展的途径。
...
### [238] [MiniCPM-V 4.5: Cooking Efficient MLLMs via Architecture, Data, and Training Recipe](https://arxiv.org/abs/2509.18154)
*Tianyu Yu,Zefan Wang,Chongyi Wang,Fuwei Huang,Wenshuo Ma,Zhihui He,Tianchi Cai,Weize Chen,Yuxiang Huang,Yuanqian Zhao,Bokai Xu,Junbo Cui,Yingjing Xu,Liqing Ruan,Luoyuan Zhang,Hanyu Liu,Jingkun Tang,Hongyuan Liu,Qining Guo,Wenhao Hu,Bingxiang He,Jie Zhou,Jie Cai,Ji Qi,Zonghao Guo,Chi Chen,Guoyang Zeng,Yuxuan Li,Ganqu Cui,Ning Ding,Xu Han,Yuan Yao,Zhiyuan Liu,Maosong Sun*

Main category: cs.LG

TL;DR: MiniCPM-V 4.5是一个8B参数的多模态大语言模型，通过架构、数据和训练方法的创新实现了高效性和强性能，在多项评测中超越了GPT-4o-latest和Qwen2.5-VL 72B等模型。
...
Method: 采用统一的3D-Resampler模型架构实现图像和视频的紧凑编码；使用统一学习范式处理文档知识和文本识别；采用混合强化学习策略支持长短推理模式。

Result: 在OpenCompass评测中超越GPT-4o-latest和Qwen2.5-VL 72B等模型；在VideoMME基准测试中，仅使用Qwen2.5-VL 7B 46.7%的GPU内存和8.7%的推理时间就达到30B以下模型的最优性能。

Conclusion: MiniCPM-V 4.5在保持强性能的同时实现了显著的效率提升，为多模态大语言模型的高效发展提供了有效解决方案。
...
### [239] [Developing Training Procedures for Piecewise-linear Spline Activation Functions in Neural Networks](https://arxiv.org/abs/2509.18161)
*William H Patty*

Main category: cs.LG

TL;DR: 该论文研究了通过优化神经网络中激活函数的形状来提高模型参数效率和准确性的方法，比较了9种训练方法，在FNN和CNN中分别实现了94%和51%的错误率降低。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络通常使用静态激活函数如ReLU、tanh或sigmoid，但通过优化激活函数的形状，可以为神经元分配更优化的激活函数，从而训练出更参数高效和准确的模型。

Method: 使用参数化线性B样条激活函数，比较了9种训练方法来探索神经网络中的双优化动态。

Result: 实验结果显示，与传统基于ReLU的模型相比，FNN的最终模型错误率降低了94%，CNN降低了51%。
...
### [240] [A Simple and Reproducible Hybrid Solver for a Truck-Drone VRP with Recharge](https://arxiv.org/abs/2509.18162)
*Meraryslan Meraliyev,Cemil Turan,Shirali Kadyrov*

Main category: cs.LG

TL;DR: 本文提出了一种混合强化学习方法，用于优化卡车和无人机的最后一公里配送问题，通过结合ALNS卡车路径规划和注意力策略调度无人机任务，在电池管理约束下实现了比传统方法更优的配送效率。


<details>
  <summary>Details</summary>
Motivation: 研究最后一公里配送中卡车和无人机的协同作业问题，特别关注电池管理约束（无人机速度是卡车的两倍，每次飞行需满足续航预算，每次交付后需在卡车上充电）。传统方法在平衡卡车等待时间和无人机调度效率方面存在挑战。

Method: 开发了混合强化学习求解器：使用ALNS算法规划卡车路径（包含2/3-opt和Or-opt优化），配合基于指针/注意力机制的小型策略来调度无人机任务。策略通过硬可行性掩码确保续航和充电约束，使用精确时间线模拟器计算实际完成时间。

Result: 在欧几里得实例（N=50，E=0.7，R=0.1）上，该方法平均完成时间为5.203±0.093，相比ALNS的5.349±0.038和NN的5.208±0.124，平均比ALNS优2.73%，与NN相差仅0.10%。学习到的调度器能更好地平衡卡车等待时间。

Conclusion: 混合强化学习方法在卡车-无人机协同配送中表现出色，能够有效管理电池约束并优化整体完成时间。该方法为复杂的物流调度问题提供了新的解决方案，并提供了可复现的实现工具。
...
### [241] [DSFT: Inspiring Diffusion Large Language Models to Comprehend Mathematical and Logical Patterns](https://arxiv.org/abs/2509.18164)
...
Method: 提出DSFT策略，调整掩码策略和损失函数，可与预训练、强化学习等方法灵活结合。

Result: 在LLaDA和Dream系列模型上验证，小规模数据上的DSFT在数学和逻辑问题上分别实现5-10%和约2%的性能提升。

Conclusion: 这种掩码方法为未来学习特定模式提供了见解，可轻松高效地与其他训练方法结合并应用于各种扩散大语言模型。
...
### [242] [MobiGPT: A Foundation Model for Mobile Wireless Networks](https://arxiv.org/abs/2509.18166)
...
<details>
  <summary>Details</summary>
Motivation: 当前移动数据预测方法需要为不同数据类型定制模型，增加了大规模异构网络中的复杂性和部署成本，需要统一的预测解决方案。

Method: 提出MobiGPT基础模型，采用软提示学习方法帮助模型理解不同数据类型的特征，引入时间掩码机制支持短期预测、长期预测和分布生成三种任务。

Result: 在超过10万个真实样本的评估中，MobiGPT相比现有模型预测准确率分别提高了27.37%、20.08%和7.27%，在未见场景中的零/少样本性能提升超过21.51%。

Conclusion: MobiGPT作为基础模型具有强大的泛化能力和迁移能力，能够有效解决移动数据预测中的多类型、多任务需求。
...
### [243] [PiMoE: Token-Level Routing for Integrating High-Precision Computation and Reasoning](https://arxiv.org/abs/2509.18169)
...
Method: 提出PiMoE架构，通过分别训练专家、文本到计算模块和路由器，将计算能力内生于神经网络。在推理时，路由器在token级别指导计算和推理，实现单链思维中的迭代交替。

Result: 在两个推理-计算任务上的评估显示，PiMoE不仅比直接微调LLM精度更高，而且在响应延迟、token使用和GPU能耗方面相比主流多智能体方法有显著改善。
...
### [244] [FedIA: A Plug-and-Play Importance-Aware Gradient Pruning Aggregation Method for Domain-Robust Federated Graph Learning on Node Classification](https://arxiv.org/abs/2509.18171)
*Zhanting Zhou,KaHou Tam,Zeqin Wu,Pengzhao Sun,Jinbo Wang,Fengli Zhang*

Main category: cs.LG

TL;DR: FedIA是一个联邦图学习框架，采用投影优先策略，通过服务器端top-ρ掩码和影响力正则化动量权重来去噪客户端更新，在域偏移场景下实现更稳定收敛和更高准确率。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习在域偏移场景下（如Twitch Gamers和多语言Wikipedia网络）会导致客户端模型表示不兼容，传统聚合方法不稳定且效果差。研究发现问题根源是噪声梯度信号而非权重方案。

Method: 提出投影优先策略，采用两阶段即插即用管道：1）服务器端top-ρ掩码保留约5%最信息丰富的坐标；2）轻量级影响力正则化动量权重抑制异常客户端。该方法不增加上行流量且服务器内存开销可忽略。

Result: 在同构（Twitch Gamers）和异构（Wikipedia）图上，FedIA相比9个强基线方法实现了更平滑稳定的收敛和更高的最终准确率。收敛分析表明动态投影保持了最优的O(σ²/√T)收敛速率。

Conclusion: FedIA通过重要性感知的投影优先策略有效解决了联邦图学习中的域偏移问题，在保持通信效率的同时显著提升了模型性能。
...
### [245] [SBVR: Summation of BitVector Representation for Efficient LLM Quantization](https://arxiv.org/abs/2509.18172)
*Wonjun Bang,Jongseok Park,Hongseung Yu,Kyungmin Bin,Kyunghan Lee*

Main category: cs.LG

TL;DR: SBVR是一种新型的大语言模型量化方法，通过高斯分布感知的码本表示和定制CUDA内核，在4位量化下实现最先进的性能，同时比FP16模型快2.21-3.04倍。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法存在两个主要问题：RTN方法无法处理LLM权重的高斯分布特性，码本方法虽然考虑了分布但存在内存访问模式问题导致推理速度下降。

Method: 提出SBVR方法，将权重值映射到遵循LLM权重实际分布的非均匀表示点，并设计定制CUDA内核直接在SBVR格式下进行矩阵向量乘法而无需解压缩。

Result: 在各种模型上的评估显示，SBVR在4位量化下实现了最先进的困惑度和准确度基准性能，同时端到端token生成速度比朴素FP16模型快2.21-3.04倍。

Conclusion: SBVR通过硬件友好的高斯分布码本表示和高效的计算内核，成功解决了现有量化方法在精度和速度之间的权衡问题。
...
### [246] [TurnBack: A Geospatial Route Cognition Benchmark for Large Language Models through Reverse Route](https://arxiv.org/abs/2509.18173)
...
<details>
  <summary>Details</summary>
Motivation: 人类能够通过自然语言理解地理空间信息，但大语言模型的地理空间认知能力尚未得到充分探索，现有研究受限于非量化指标、有限评估数据集和不清晰的研究层次。

Method: 创建包含全球12个大都市36000条路线的大规模评估数据集，开发PathBuilder工具在自然语言指令和导航路线之间进行转换，提出新的评估框架和指标对11个SOTA LLMs进行路线反转任务评估。

Result: LLMs在路线反转任务中表现有限：大多数反转路线既没有返回起点，也与最优路线不相似。LLMs还面临路线生成鲁棒性低和对错误答案置信度高的问题。

Conclusion: 大语言模型在地理空间路线认知方面存在显著局限性，需要进一步改进其空间推理能力。
...
### [247] [Conversational Orientation Reasoning: Egocentric-to-Allocentric Navigation with Multimodal Chain-of-Thought](https://arxiv.org/abs/2509.18200)
*Yu Ti Huang*

Main category: cs.LG

TL;DR: 本文提出了多模态思维链（MCoT）框架，用于解决对话式导航中的自我中心到非自我中心空间定向问题，在台湾LLM模型上实现了高精度定向推理。


<details>
  <summary>Details</summary>
Motivation: 解决室内或复杂环境中GPS信号弱、缺乏详细地图时，对话代理如何将自我中心表达（如"在我右边"）转换为非自我中心方向（东南西北）的挑战。

Method: 提出多模态思维链框架，整合ASR转录语音和地标坐标，通过三步推理过程：提取空间关系、坐标映射到绝对方向、推断用户朝向，并采用课程学习策略在台湾LLM-13B模型上训练。

Result: MCoT在干净转录本上达到100%定向准确率，在ASR转录本上达到98.1%，显著优于单模态和非结构化基线，在多语言代码切换和噪声条件下表现出强鲁棒性。

Conclusion: 结构化MCoT空间推理为可解释和资源高效的具身导航提供了一条可行路径，展示了在资源受限环境下的应用潜力。
...
### [248] [Variational Task Vector Composition](https://arxiv.org/abs/2509.18208)
*Boyuan Zhang,Yingjun Du,Xiantong Zhen,Ling Shao*

Main category: cs.LG

TL;DR: 本文提出了变分任务向量组合方法，将组合系数作为潜变量在贝叶斯推理框架中进行估计，通过引入Spike-and-Slab先验促进稀疏性，并开发门控采样机制来提高稳定性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有任务向量组合方法在任务级别操作，但观察到任务向量存在结构冗余，需要更精细的样本特定组合方法来选择最信息丰富的组件。

Method: 提出变分任务向量组合框架，使用Spike-and-Slab先验促进稀疏性，开发基于不确定性和重要性的门控采样机制来构建可控后验分布。

Result: 实验结果表明，该方法在所有数据集上一致优于现有方法，通过选择性利用任务向量中最可靠和信息丰富的组件来提高性能。

Conclusion: 该方法为高效有效的任务向量组合建立了新标准，展示了选择性利用任务组件在实际应用中的价值。
...
### [249] [MolPILE - large-scale, diverse dataset for molecular representation learning](https://arxiv.org/abs/2509.18353)
...
Method: 从6个大规模数据库中通过自动化筛选流程构建MolPILE数据集，包含2.22亿个化合物，并对现有预训练数据集进行全面分析。

Result: 在MolPILE上重新训练现有模型能够提高泛化性能，证明了该数据集的有效性。

Conclusion: MolPILE为分子化学领域提供了一个标准化的模型训练资源，解决了该领域对类似ImageNet数据集的需求。
...
### [250] [FastMTP: Accelerating LLM Inference with Enhanced Multi-Token Prediction](https://arxiv.org/abs/2509.18362)
*Yuxuan Cai,Xiaozhuan Liang,Xinghua Wang,Jin Ma,Haijin Liang,Jinwen Luo,Xinyu Zuo,Lisheng Duan,Yuyang Yin,Xi Chen*

Main category: cs.LG

TL;DR: FastMTP是一种通过将多令牌预测训练与其推理模式对齐来改进多步草稿质量的方法，显著提升推测解码性能，在保持无损输出质量的同时实现平均2.03倍加速。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，自回归生成的顺序特性造成了吞吐量瓶颈，限制了实际部署。多令牌预测在训练效率和性能方面表现出色，但其在推理加速方面的潜力尚未充分探索。

Method: 通过位置共享权重在自蒸馏数据上微调单个MTP头，使其能够捕捉连续未来令牌之间的依赖关系，并在多个递归草稿步骤中保持高接受率。同时集成语言感知动态词汇压缩以减少计算开销。

Result: 在七个不同基准测试中，FastMTP相比标准下一令牌预测实现了平均2.03倍加速，输出质量无损，比普通MTP性能提升82%。

Conclusion: FastMTP仅需轻量级训练，可无缝集成到现有推理框架中，为大语言模型推理加速提供了实用且快速部署的解决方案。
...
### [251] [Multi-Worker Selection based Distributed Swarm Learning for Edge IoT with Non-i.i.d. Data](https://arxiv.org/abs/2509.18367)
*Zhuoyu Yao,Yue Wang,Songyang Zhang,Yingshu Li,Zhipeng Cai,Zhi Tian*

Main category: cs.LG

TL;DR: 本文提出了一种新的多工作者选择算法M-DSL，用于解决分布式群学习中的非独立同分布数据挑战，通过引入非i.i.d.程度度量来指导工作者选择，提升学习性能。


<details>
  <summary>Details</summary>
Motivation: 分布式群学习在边缘物联网中具有优势，但非i.i.d.数据会降低学习性能并导致训练行为发散，目前缺乏数据异质性对模型训练精度影响的理论指导。

Method: 首先研究数据异质性，测量非i.i.d.数据集在DSL框架下的影响，然后提出M-DSL算法，引入新的非i.i.d.程度度量来制定局部数据集间的统计差异，指导多工作者选择。

Result: 通过在不同异构数据集和非i.i.d.数据设置下的广泛实验，数值结果验证了M-DSL在性能提升和网络智能增强方面的优势。

Conclusion: M-DSL算法有效解决了分布式异构数据下的学习挑战，提供了理论收敛性分析，并在实验中表现出优于基准的性能。
...
### [252] [GnnXemplar: Exemplars to Explanations - Natural Language Rules for Global GNN Interpretability](https://arxiv.org/abs/2509.18376)
*Burouj Armgaan,Eshan Jain,Harsh Pandey,Mahesh Chandran,Sayan Ranu*

Main category: cs.LG

TL;DR: GnnXemplar是一种新颖的全局解释方法，通过认知科学中的范例理论识别GNN嵌入空间中的代表性节点，并使用LLM生成自然语言规则来解释预测。


<details>
  <summary>Details</summary>
Motivation: 现有全局解释方法依赖小图中的模体发现，在大型真实场景中效果不佳，因为子图重复罕见、节点属性高维且预测来自复杂的结构-属性交互。

Method: 将范例选择建模为反向k近邻覆盖最大化问题，使用贪心近似算法；采用自精炼提示策略利用LLM生成可解释规则。

Result: 在多个基准测试中，GnnXemplar在保真度、可扩展性和人类可解释性方面显著优于现有方法，用户研究验证了其有效性。

Conclusion: GnnXemplar为GNN的全局解释提供了一种有效且可扩展的解决方案，结合了范例理论和LLM的优势。
...
### [253] [Graph Enhanced Trajectory Anomaly Detection](https://arxiv.org/abs/2509.18386)
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹异常检测方法只考虑轨迹的有限方面，在欧几里得空间中分析轨迹，忽略了底层移动网络（如道路网络）的约束和连通性信息

Method: GETAD框架使用图注意力网络学习道路感知嵌入，结合基于图的位置编码，Transformer解码器建模序列移动，多目标损失函数结合自回归预测和监督链接预测

Result: 在真实世界和合成数据集上的实验表明，GETAD相比现有方法取得了持续改进，特别是在检测道路约束环境中的细微异常方面表现优异

Conclusion: 结果表明将图结构和上下文语义整合到轨迹建模中能够实现更精确和上下文感知的异常检测
...
### [254] [Towards Provable Emergence of In-Context Reinforcement Learning](https://arxiv.org/abs/2509.18389)
*Jiuqi Wang,Rohan Chandra,Shangtong Zhang*

Main category: cs.LG

TL;DR: 本文探讨了为什么强化学习预训练算法能够生成支持上下文强化学习的网络参数，并通过案例研究证明，当Transformer用于策略评估预训练时，其损失函数的全局最小值能够实现上下文时序差分学习。


<details>
  <summary>Details</summary>
Motivation: 现代强化学习智能体通过更新神经网络参数来适应任务，但最近发现一些预训练后的智能体能够在参数固定的情况下，仅通过上下文信息解决新的分布外任务。本文旨在解释为什么标准RL预训练算法能够产生支持这种上下文强化学习能力的参数。

Method: 通过理论分析的方法，研究Transformer在策略评估预训练中的损失函数最小化问题。具体证明当Transformer预训练用于策略评估时，其损失函数的某个全局最小值能够实现上下文时序差分学习。

Result: 研究证明，在策略评估预训练中，Transformer网络参数的全局最小值确实能够支持上下文强化学习能力，为假设提供了初步的理论支持。

Conclusion: 本文通过案例研究验证了假设：能够实现上下文强化学习的参数是预训练损失函数的极小值点，这为理解上下文强化学习的理论基础迈出了重要一步。
...
### [255] [Development of Deep Learning Optimizers: Approaches, Concepts, and Update Rules](https://arxiv.org/abs/2509.18396)
*Doğay Altınel*

Main category: cs.LG

TL;DR: 这是一篇关于深度学习优化器的综述论文，系统回顾了从随机梯度下降到最新优化器（如Momentum、AdamW、Sophia、Muon等）的发展历程、技术特点和超参数设置。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习的快速发展，各种优化器不断涌现，但缺乏系统的综述来帮助研究者理解当前优化器的现状和未来发展方向。

Method: 按时间顺序分析各种优化器，详细展示每个优化器的更新规则、相关概念和变量，讨论它们的技术方法、对优化过程的贡献以及默认超参数设置。

Result: 提供了深度学习优化器的全面综述，涵盖了从基础到最新优化器的详细分析，为理解当前优化器状态和识别未来发展领域提供了资源。

Conclusion: 该研究为深度学习优化器领域提供了宝贵的综述资源，有助于研究者更好地理解优化器的发展历程和技术特点，同时指出了优化过程中面临的开放挑战和未来研究方向。
...
### [256] [Explicit Path CGR: Maintaining Sequence Fidelity in Geometric Representations](https://arxiv.org/abs/2509.18408)
*Sarwan Ali*
...
<details>
  <summary>Details</summary>
Motivation: 传统CGR方法在生物序列分析中存在序列信息丢失的问题，这限制了其在需要准确序列恢复的应用中的使用。
...
### [257] [Diffusion Policies with Offline and Inverse Reinforcement Learning for Promoting Physical Activity in Older Adults Using Wearable Sensors](https://arxiv.org/abs/2509.18433)
*Chang Liu,Ladda Thiamwong,Yanjie Fu,Rui Xie*

Main category: cs.LG

TL;DR: 提出了KANDI方法，结合Kolmogorov-Arnold网络和扩散策略，用于解决离线强化学习在医疗健康应用中奖励函数定义困难和策略对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习在医疗健康应用中面临奖励函数定义困难、逆强化学习在复杂环境中难以准确推断奖励函数、以及学习策略与观察到的医疗行为对齐的挑战。

Method: 使用Kolmogorov-Arnold网络进行灵活的函数逼近来估计奖励函数，学习低跌倒风险老年人（专家）的自由生活环境行为；在Actor-Critic框架中使用基于扩散的策略进行动作细化和离线强化学习的效率提升。

Result: KANDI在D4RL基准测试中优于最先进方法，并在PEER研究的临床试验中验证了其在跌倒风险干预项目中促进老年人身体活动的实际应用效果。
...
### [258] [MeshODENet: A Graph-Informed Neural Ordinary Differential Equation Neural Network for Simulating Mesh-Based Physical Systems](https://arxiv.org/abs/2509.18445)
...
Conclusion: 该方法为开发数据驱动代理模型以加速复杂结构系统分析提供了强大且通用的途径
...
### [259] [GluMind: Multimodal Parallel Attention and Knowledge Retention for Robust Cross-Population Blood Glucose Forecasting](https://arxiv.org/abs/2509.18457)
...
<details>
  <summary>Details</summary>
Motivation: 解决血糖预测中多源信号采样率不同、长期时间依赖性建模以及持续学习中的灾难性遗忘问题。

Method: 设计并行工作的交叉注意力和多尺度注意力机制，结合知识保留模块的Transformer架构。

Result: 在AIREADI数据集上，GluMind相比现有最优模型在RMSE和MAE上分别提升约15%和9%。

Conclusion: GluMind框架在血糖预测任务中表现出色，具有良好的性能稳定性和适应性。
...
### [260] [Probabilistic Geometric Principal Component Analysis with application to neural data](https://arxiv.org/abs/2509.18469)
*Han-Lin Hsieh,Maryam M. Shanechi*

Main category: cs.LG

TL;DR: 提出了概率几何主成分分析（PGPCA），这是一种新的降维算法，能够显式地结合非线性流形几何信息，将概率主成分分析（PPCA）推广到非线性流形场景。


<details>
  <summary>Details</summary>
Motivation: 传统PPCA及其扩展主要基于线性模型，只能描述欧几里得坐标系中的数据。然而在神经科学等应用中，数据可能分布在非线性流形周围而非欧几里得空间中，需要能够处理非线性几何的降维方法。

Method: 开发了PGPCA算法，首先从数据中拟合非线性流形，然后推导出几何坐标系来捕捉数据与流形的偏差和噪声。提出了数据驱动的EM算法来学习PGPCA模型参数。
...
### [261] [Discrete-time diffusion-like models for speech synthesis](https://arxiv.org/abs/2509.18470)
*Xiaozhou Tan,Minghui Zhao,Mattias Cross,Anton Ragni*

Main category: cs.LG

TL;DR: 本文探讨了扩散模型的离散时间过程，提出了几种新的变体，包括加性高斯噪声、乘性高斯噪声、模糊噪声以及模糊和高斯噪声的混合。实验结果表明，离散时间过程在主观和客观语音质量上与流行的连续对应物相当，但具有更高效和一致的训练和推理模式。


<details>
  <summary>Details</summary>
Motivation: 传统的扩散模型将语音生成视为连续时间过程，但训练通常限制在加性高斯噪声，这限制了模型的表达能力。此外，推理时的时间离散化导致训练和采样条件不匹配。离散时间过程没有这些限制，可能需要更少的推理步骤，并且在训练和推理条件之间完全一致。

Method: 本文探索了一些类似扩散的离散时间过程，并提出了新的变体。这些过程包括应用加性高斯噪声、乘性高斯噪声、模糊噪声以及模糊和高斯噪声的混合。

Result: 实验结果表明，离散时间过程在主观和客观语音质量上与广泛流行的连续对应物相当，同时具有更高效和一致的训练和推理模式。

Conclusion: 离散时间过程为扩散模型提供了一种有前景的替代方案，克服了连续时间模型的局限性，同时保持了高质量的语音生成能力。
...
TL;DR: SimpleFold是首个基于流匹配的蛋白质折叠模型，仅使用通用Transformer块，挑战了蛋白质折叠领域对复杂领域特定架构设计的依赖。


<details>
  <summary>Details</summary>
Motivation: 质疑蛋白质折叠模型是否需要复杂的领域特定架构设计，探索是否可以使用通用架构实现高性能。

Method: 使用标准Transformer块和自适应层，通过生成流匹配目标训练，结合结构项，在900万蛋白质结构数据上训练30亿参数模型。

Result: 在标准折叠基准测试中达到与最先进基线相竞争的性能，在集成预测方面表现强劲，在消费级硬件上部署和推理效率高。

Conclusion: SimpleFold挑战了蛋白质折叠对复杂领域特定架构的依赖，为未来进展开辟了替代设计空间。
...
<details>
  <summary>Details</summary>
Motivation: 量子系统在高维希尔伯特空间中的演化使得传统数值方法计算成本过高，现有神经网络架构需要大量训练数据且存在虚假振荡问题，影响物理可解释性。

Method: 使用Kolmogorov Arnold Networks（KANs）并结合物理信息损失函数来强制执行Ehrenfest定理，同时引入Chain of KANs架构直接嵌入时间因果关系。

Result: 该方法仅需200个样本（传统Temporal Convolution Networks需要3,700个），准确率显著提升，数据需求减少94.6%。

Conclusion: 物理信息KANs在保持数学严谨性和物理一致性的同时，相比传统黑盒模型具有明显优势，大幅降低了数据需求。
...
TL;DR: 本文提出使用混合数据集（结合合成数据和真实公开特征）来增强反洗钱（AML）模型的实用性，在保护隐私的同时提高模型性能。


<details>
  <summary>Details</summary>
Motivation: 反洗钱是金融机构面临的全球性关键问题，但开发AML模型时缺乏训练数据，因为隐私和保密问题限制了真实数据的访问。纯合成数据集虽然解决了隐私问题，但存在实用性挑战。

Method: 提出使用混合数据集方法，将合成生成的数据与公开可获取的真实世界特征相结合，增强合成数据集的实用性。

Result: 混合数据集不仅能够保护隐私，还能提高模型效用，为金融机构增强AML系统提供了实用途径。

Conclusion: 混合数据集方法为解决AML模型训练中的数据隐私和实用性挑战提供了有效解决方案，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前RL训练中，rollout生成占90%以上运行时间，且长尾响应分布导致GPU空闲浪费，限制了模型的可扩展性。

Method: APRIL在rollout阶段超额配置请求，达到目标响应数后终止，并将未完成的响应回收用于后续步骤继续处理，避免丢弃任何rollout。

Result: 实验显示APRIL将rollout吞吐量最高提升44%，加速收敛，并在多个任务上获得最高8%的最终准确率提升。
...
TL;DR: 本文提出了一种称为反向互补一致性正则化（RCCR）的方法，通过在微调过程中直接惩罚DNA序列与其反向互补序列预测之间的差异，来提高DNA语言模型对反向互补对称性的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: DNA序列的反向互补（RC）通常具有相同的生物学意义，但现有的DNA语言模型往往无法捕捉这种对称性，导致对序列及其RC的预测不一致，降低了模型的可靠性。

Method: RCCR是一种简单且模型无关的微调目标，通过直接惩罚模型在序列上的预测与其反向互补序列上的对齐预测之间的差异来实现。该方法在三种不同的骨干网络（Nucleotide Transformer、HyenaDNA、DNABERT-2）上进行了评估，涵盖了序列分类、标量回归和轮廓预测等多种基因组任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE模型通过复制单一预训练模型的FFN层来构建专家，限制了专家多样性。本文旨在利用多个异构预训练模型来构建更具多样性的MoE模型。
...
Conclusion: Symphony-MoE框架能够有效融合异构预训练模型，构建出性能更优的MoE模型，为MoE模型的构建提供了新的思路。
...
TL;DR: 本文从理论上解释了在Sigmoid损失函数下同步可训练逆温度和偏置的优势，分析了SigLIP和SigLIP2模型中的(𝖒,𝖇𝖗𝖊𝖑)-星座配置，并提出了带有显式相对偏置的Sigmoid损失重参数化方法。


<details>
  <summary>Details</summary>
Motivation: 对比预训练中的表示获取和对齐任务日益重要，需要从理论上理解SigLIP模型中温度和偏置参数的作用机制。

Method: 通过引入(𝖒,𝖇𝖗𝖊𝖑)-星座这一新的组合对象，分析球形码相关配置，理论推导损失函数为零的条件，并提出显式相对偏置的重参数化方法。

Result: 理论证明了SigLIP在检索任务上的成功，解释了模态间隙现象，确定了产生高质量表示所需的维度，并在合成数据实验中改善了训练动态。

Conclusion: 同步可训练温度和偏置在Sigmoid损失下具有理论优势，提出的重参数化方法能有效提升训练效果，为对比学习提供了新的理论框架。
...
TL;DR: 这篇论文是关于可解释图神经网络在痴呆症研究中的首次全面综述，涵盖了阿尔茨海默病、帕金森病、轻度认知障碍等多种痴呆类型的诊断应用，并提出了针对痴呆症任务的可解释性方法分类体系。


<details>
  <summary>Details</summary>
Motivation: 痴呆症具有临床和生物学异质性，使得诊断和亚型区分极具挑战性。传统图神经网络在脑连接建模中表现出潜力，但其有限的鲁棒性、数据稀缺性和缺乏可解释性限制了临床应用。可解释图神经网络旨在解决这些障碍。

Method: 通过系统综述方法，对XGNN在痴呆症研究中的应用进行全面梳理，引入针对痴呆症相关任务的可解释性方法分类体系，并对现有模型在临床场景中进行比较分析。

Result: 总结了XGNN在多种痴呆症诊断中的应用进展，识别了当前面临的挑战，包括有限的泛化能力、未充分探索的领域以及大语言模型在早期检测中的整合问题。

Conclusion: 该综述旨在指导未来工作朝着可信赖、具有临床意义且可扩展的XGNN在痴呆症研究中的应用方向发展，为解决痴呆症诊断挑战提供透明化见解。
...
TL;DR: 提出了Interaction Topological Transformer (ITT)框架，通过多尺度交互拓扑结构来预测多孔材料的吸附、传输和稳定性性能，实现了最先进的预测精度和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 多孔材料在气体存储、分离和催化等领域有重要应用，但由于结构-性能关系的多尺度特性以及标记数据稀疏且分布不均，预测建模面临挑战，需要能够跨材料家族泛化的数据高效框架。

Method: 开发了Interaction Topological Transformer (ITT)框架，利用新颖的交互拓扑结构在多尺度（结构、元素、原子、成对元素组织）上捕获材料信息，通过Transformer架构支持跨尺度联合推理，采用两阶段训练策略：在60万无标签结构上自监督预训练，然后有监督微调。

Result: ITT在多孔材料的吸附、传输和稳定性性能预测方面实现了最先进的准确性和可迁移性。
...
TL;DR: DS-Diffusion是一种基于风格引导核的扩散模型，通过时间信息分层去噪机制减少生成数据与真实数据之间的分布偏差，无需重新训练即可适应特定条件，提高模型的可解释性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在时间序列生成中需要重新训练以适应特定条件指导，存在生成数据与真实数据之间的分布偏差，且推理过程不具可解释性。

Method: 提出数据风格引导扩散模型（DS-Diffusion），包括基于风格引导核的扩散框架和时间信息分层去噪机制（THD）。

Result: 相比ImagenTime等先进模型，预测分数和判别分数分别降低5.56%和61.55%，分布偏差进一步减小，推理过程更可解释。

Conclusion: DS-Diffusion通过避免重新训练增强了模型对特定条件的适应性和灵活性，同时提高了生成质量和可解释性。
...
### [272] [Flow marching for a generative PDE foundation model](https://arxiv.org/abs/2509.18611)
*Zituo Chen,Sili Deng*

Main category: cs.LG

TL;DR: 提出Flow Marching算法，将神经算子学习与流匹配相结合，构建生成式PDE基础模型，通过联合采样噪声水平和物理时间步长来减少长期滚转漂移并实现不确定性感知的集成生成。
...
Method: 1. Flow Marching算法：联合采样噪声水平和物理时间步长，学习统一速度场；2. P2VAE：将物理状态嵌入紧凑潜在空间；3. FMT：结合扩散强制方案和潜在时间金字塔，实现15倍计算效率提升。

Result: 在12个不同PDE家族的约250万轨迹上训练，在未见过的Kolmogorov湍流上表现出良好的少样本适应能力，长期滚转稳定性优于确定性模型，并能提供不确定性分层的集成结果。

Conclusion: 生成式PDE基础模型对于现实世界应用具有重要意义，Flow Marching方法在减少计算成本的同时提高了模型的生成灵活性和不确定性建模能力。
...
### [273] [HyperAdapt: Simple High-Rank Adaptation](https://arxiv.org/abs/2509.18629)
*Abel Gurung,Joseph Campbell*

Main category: cs.LG

TL;DR: HyperAdapt是一种参数高效微调方法，通过行列缩放对角矩阵来适应预训练权重，仅需n+m个可训练参数即可实现高秩更新，在保持性能的同时大幅减少参数数量。
...
Method: HyperAdapt通过应用行列缩放对角矩阵来适应预训练权重矩阵，仅需n+m个可训练参数即可为n×m矩阵实现高秩更新。

Result: 在GLUE、算术推理和常识推理基准测试中，HyperAdapt在模型参数高达140亿的情况下，性能与全微调和最先进PEFT方法相当或接近，同时使用的可训练参数数量少几个数量级。

Conclusion: HyperAdapt是一种高效的参数微调方法，能够在保持模型性能的同时显著减少训练参数，为大规模模型的高效适应提供了可行方案。
...
### [274] [Subspace Clustering of Subspaces: Unifying Canonical Correlation Analysis and Subspace Clustering](https://arxiv.org/abs/2509.18653)
*Paris A. Karakasis,Nicholas D. Sidiropoulos*

Main category: cs.LG

TL;DR: 本文提出了一种新的子空间聚类框架SCoS，直接对矩阵数据进行聚类，通过块项分解构建三阶张量来联合估计聚类成员和部分共享子空间。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类方法假设向量化数据，但实际应用中数据样本往往是矩阵形式。本文旨在直接对矩阵数据进行聚类，捕捉超越单个数据向量的结构信息。

Method: 基于块项分解(BTD)构建三阶张量，联合估计聚类成员和部分共享子空间，并提供可识别性分析和可扩展优化算法。

Result: 在真实高光谱成像数据集上的实验表明，该方法在高噪声和干扰下具有优越的聚类精度和鲁棒性。

Conclusion: 所提出的框架在高维应用中具有重要潜力，能够有效处理存在超越单个数据向量结构的数据。
...
### [275] [Towards Rational Pesticide Design with Graph Machine Learning Models for Ecotoxicology](https://arxiv.org/abs/2509.18703)
*Jakub Adamczyk*

Main category: cs.LG

TL;DR: 该研究利用图机器学习进行理性农药设计，开发更安全环保的农用化学品，创建了最大的蜜蜂毒性数据集ApisTox，评估了多种机器学习模型，发现药物发现中的成功方法在农药领域泛化能力不足。
...
Result: 研究发现药物化学中成功的方法在农用化学品领域往往泛化能力不足，凸显了领域特定模型和基准测试的必要性。
...
### [276] [A Generalized Bisimulation Metric of State Similarity between Markov Decision Processes: From Theoretical Propositions to Applications](https://arxiv.org/abs/2509.18714)
...
Method: 正式建立MDP对之间的广义双模拟度量，严格证明其三个基本性质：对称性、MDP间三角不等式和相同状态空间上的距离界限。

Result: GBSM在策略迁移、状态聚合和基于采样的估计中获得了比标准BSM更严格的理论界限，并提供了闭式样本复杂度估计。

Conclusion: GBSM为多MDP场景提供了理论基础，数值结果验证了理论发现的有效性。
...
### [277] [Theory of periodic convolutional neural network](https://arxiv.org/abs/2509.18744)
*Yuqing Liu*

Main category: cs.LG

TL;DR: 本文提出了一种新型的周期性CNN架构，通过在卷积层中引入周期性边界条件，能够精确逼近高维输入空间中的脊函数，填补了传统CNN在低维脊函数逼近方面的理论空白。


<details>
  <summary>Details</summary>
Motivation: 传统CNN在处理具有周期性结构的数据（如图像分析、物理信息学习和材料科学）时存在局限性，需要开发能够更好处理高维脊函数结构的网络架构。

Method: 设计周期性CNN架构，在卷积层中引入周期性边界条件，并建立严格的逼近理论，证明该架构能够逼近d维输入空间中依赖于d-1个线性变量的脊函数。

Result: 理论证明周期性CNN能够精确逼近高维脊函数，而传统CNN在低维脊函数设置（d-2或更少变量）下无法实现这种逼近，确立了周期性CNN表达能力的严格特征。

Conclusion: 周期性CNN不仅扩展了CNN逼近理论的数学基础，而且为处理具有高内在维度脊状结构的数据问题提供了一类具有实际应用价值的架构。
...
### [278] [MOMEMTO: Patch-based Memory Gate Model in Time Series Foundation Model](https://arxiv.org/abs/2509.18751)
*Samuel Yoon,Jongwon Kim,Juyoung Ha,Young Myoung Ko*

Main category: cs.LG

TL;DR: MOMEMTO是一个用于时间序列异常检测的TFM模型，通过引入基于patch的内存模块来缓解过泛化问题，支持多数据集联合微调。
...
Method: 提出MOMEMTO模型，包含基于patch的内存模块，从预训练编码器初始化内存项，组织为patch级单元，通过注意力机制更新，采用多域训练策略实现单模型跨数据集联合微调。

Result: 在23个单变量基准数据集上评估，MOMEMTO在AUC和VUS指标上优于基线方法，并提升了其骨干TFM的性能，特别是在少样本学习场景中表现更佳。

Conclusion: MOMEMTO通过内存模块有效缓解了过泛化问题，实现了多数据集联合训练，在时间序列异常检测任务中表现出色。
...
### [279] [Diagonal Linear Networks and the Lasso Regularization Path](https://arxiv.org/abs/2509.18766)
*Raphaël Berthier*
...
<details>
  <summary>Details</summary>
Motivation: 对角线性网络因其隐式正则化可被严格分析而具有理论价值，本文旨在深化这种分析，揭示训练轨迹与LASSO路径的关系。

Method: 通过理论分析和数值模拟，研究对角线性网络的训练轨迹，并与LASSO正则化路径进行比较。在LASSO路径单调性假设下建立精确联系，一般情况则展示近似联系。

Result: 发现对角线性网络的训练轨迹与LASSO正则化路径密切相关，训练时间相当于逆正则化参数。在单调性条件下存在精确对应关系。

Conclusion: 对角线性网络的训练过程本质上与LASSO正则化路径相关，这为理解神经网络隐式正则化提供了新的理论视角。
...
### [280] [Probabilistic Machine Learning for Uncertainty-Aware Diagnosis of Industrial Systems](https://arxiv.org/abs/2509.18810)
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在故障诊断中应用广泛，但现有模型在评估自身置信度方面存在困难，这在基于一致性的诊断中尤为重要，因为决策逻辑对误报高度敏感。

Method: 使用集成概率机器学习方法，通过量化预测不确定性来改进数据驱动的基于一致性的诊断框架。

Result: 通过消融分析和对比分析在多个案例研究中评估，结果显示在一系列诊断指标上都有持续改进。

Conclusion: 所提出的集成概率机器学习框架能够有效提升数据驱动故障诊断的性能，特别是在处理预测不确定性方面表现出色。
...
### [281] [Training-Free Data Assimilation with GenCast](https://arxiv.org/abs/2509.18811)
...
<details>
  <summary>Details</summary>
Motivation: 数据同化在气象学、海洋学和机器人学等领域广泛应用，用于从噪声观测中估计动力系统状态，但现有方法往往复杂且需要专门训练

Method: 基于粒子滤波器算法，利用预训练的扩散模型（如GenCast天气预测模型）进行数据同化，无需额外训练

Result: 该方法具有轻量级和通用性特点，能够有效利用预训练扩散模型进行状态估计

Conclusion: 提出的方法为数据同化提供了一种新的有效途径，特别适用于基于扩散模型的动力系统仿真应用
...
### [282] [Graph-based Clustering Revisited: A Relaxation of Kernel $k$-Means Perspective](https://arxiv.org/abs/2509.18826)
*Wenlong Lyu,Yuheng Jia,Hui Liu,Junhui Hou*

Main category: cs.LG

TL;DR: 本文提出了一种新的低秩双随机聚类方法（LoRD），通过减少对正交约束的松弛来改进传统图聚类方法，并引入块对角正则化（B-LoRD）进一步提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有的图聚类方法（如谱聚类、对称非负矩阵分解等）为了数值可行性过度松弛了低秩、非负、双随机和正交约束，这可能限制了它们的聚类效果。

Method: 提出LoRD模型，仅松弛正交约束来获得概率聚类结果；引入块对角正则化（B-LoRD）增强聚类性能；通过引入类概率参数将非凸双随机约束转化为线性凸约束；提出全局收敛的投影梯度下降算法进行优化。

Result: 理论分析证明了正交性与块对角性在双随机约束下的等价性，并证明了LoRD和B-LoRD的梯度Lipschitz连续性。大量实验验证了方法的有效性。

Conclusion: LoRD和B-LoRD通过更合理的约束松弛和块对角正则化，显著提升了聚类性能，为图聚类提供了新的有效解决方案。
...
### [283] [Shared-Weights Extender and Gradient Voting for Neural Network Expansion](https://arxiv.org/abs/2509.18842)
...
Result: 在四个数据集上的广泛基准测试表明，该方法能有效抑制神经元失效，相比其他扩展方法和基线实现了更好的性能。
...
### [284] [NGRPO: Negative-enhanced Group Relative Policy Optimization](https://arxiv.org/abs/2509.18851)
...
TL;DR: NGRPO算法解决了GRPO在组内响应全部正确或全部错误时无法学习的问题，通过优势校准和不对称裁剪机制，将同质错误转化为有效的学习信号，显著提升了数学推理能力。


<details>
  <summary>Details</summary>
Motivation: GRPO算法存在一个关键限制：当组内所有响应要么全部正确要么全部错误时，模型无法从这些同质响应中学习。特别是对于全部错误的组，GRPO的优势函数值为零，导致梯度消失，损失了宝贵的学习信号。

Method: NGRPO包含两个核心机制：1）优势校准：在优势计算中假设存在一个虚拟的最大奖励样本，改变组内奖励的均值和方差，确保同质错误样本的优势不再为零；2）不对称裁剪：放宽正样本的更新幅度，同时对负样本施加更严格的约束，以稳定优势校准引入的探索压力。

Result: 在Qwen2.5-Math-7B上的实验表明，NGRPO在MATH500、AMC23和AIME2025等数学基准测试中显著优于PPO、GRPO、DAPO和PSR-NSR等基线方法。
...
### [285] [Exploring Heterophily in Graph-level Tasks](https://arxiv.org/abs/2509.18893)
*Qinhan Hou,Yilun Zheng,Xichun Zhang,Sitao Luan,Jing Tang*

Main category: cs.LG

TL;DR: 该论文首次分析了异质性在图级学习中的影响，提出了图级标签方案的分类法，重点研究了基于motif的局部结构标签任务。通过能量梯度流分析发现，motif检测需要混合频率动态而非节点级任务中的频率主导机制。


<details>
  <summary>Details</summary>
Motivation: 虽然异质性在节点级任务中已被广泛研究，但其在图级学习中的影响仍不清楚。本文旨在填补这一空白，建立图级学习中异质性的理论理解。

Method: 1) 提出图级标签方案的分类法；2) 聚焦局部结构标签中的motif检测任务；3) 使用基于能量的梯度流分析；4) 在合成数据集和真实分子属性预测数据集上进行实验验证。

Result: 理论分析表明motif目标与全局频率主导机制存在固有错位，需要不同的架构考虑。实验证明频率自适应模型优于频率主导模型。

Conclusion: 这项工作建立了图级学习中异质性的新理论理解，并为设计有效的GNN架构提供了指导。
...
### [286] [Enhancing the Effectiveness and Durability of Backdoor Attacks in Federated Learning through Maximizing Task Distinction](https://arxiv.org/abs/2509.18904)
*Zhaoxin Wang,Handing Wang,Cong Tian,Yaochu Jin*

Main category: cs.LG

TL;DR: 本文提出了一种在联邦学习中通过动态优化后门触发器来解耦主任务和后门任务的新方法，该方法在多种防御算法下都表现出良好的攻击性能。
...
Method: 采用min-max框架动态优化后门触发器：内层最大化中毒样本与良性样本之间的性能差距，确保良性用户更新对后门影响最小；外层将自适应触发器注入本地模型。

Result: 在计算机视觉和自然语言任务上评估，与6种后门攻击方法和6种防御算法比较，实验结果表明该方法攻击性能良好且易于集成到现有后门攻击技术中。

Conclusion: 提出的动态优化后门触发器方法有效解决了联邦学习中后门攻击的持久性问题，能够抵抗多种防御机制。
...
### [287] [Tackling GNARLy Problems: Graph Neural Algorithmic Reasoning Reimagined through Reinforcement Learning](https://arxiv.org/abs/2509.18930)
*Alex Schutz,Victor-Alexandru Darvariu,Efimia Panagiotaki,Bruno Lacerda,Nick Hawes*

Main category: cs.LG

TL;DR: GNARL框架将神经算法推理重构为马尔可夫决策过程，利用模仿学习和强化学习解决传统NAR方法的局限性，在NP难问题和无专家算法场景下表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统神经算法推理存在无法构造有效解、无法处理多个正确解、在NP难问题上表现差、以及无法应用于无已知强算法的问题等局限性。

Method: 将算法轨迹学习问题重构为马尔可夫决策过程，提出GNARL框架，结合模仿学习和强化学习方法，适用于广泛的图基问题。

Result: 在多个CLRS-30问题上获得极高的图精度，在NP难问题上性能匹配或超越传统NAR方法，甚至在缺乏专家算法时仍能应用。

Conclusion: GNARL框架成功解决了传统NAR方法的局限性，为算法学习提供了更强大和通用的解决方案。
...
### [288] [Towards Privacy-Aware Bayesian Networks: A Credal Approach](https://arxiv.org/abs/2509.18949)
*Niccolò Rocchi,Fabio Stella,Cassio de Campos*

Main category: cs.LG

TL;DR: 本文提出使用置信网络（CN）作为贝叶斯网络（BN）的隐私保护替代方案，通过模糊化而非添加噪声的方式在保护隐私的同时保持模型效用。


<details>
  <summary>Details</summary>
Motivation: 随着隐私问题日益严重，公开发布的贝叶斯网络容易受到追踪攻击，现有噪声添加方法虽然能保护隐私但会显著降低模型效用。

Method: 将贝叶斯网络转换为置信网络，通过调整CN超参数来调节隐私保护程度，同时识别并隐藏关键学习信息以防止攻击者恢复原始BN。

Result: 数值实验证实CN能够有效降低追踪攻击的成功概率，同时保持有意义的推理能力，实现隐私与效用的平衡。
...
### [289] [Lift What You Can: Green Online Learning with Heterogeneous Ensembles](https://arxiv.org/abs/2509.18962)
*Kirsten Köbschall,Sebastian Buschjäger,Raphael Fischer,Lisa Hartung,Stefan Kramer*

Main category: cs.LG

TL;DR: 提出HEROS异构在线集成方法，在资源约束下选择模型子集进行训练，平衡预测性能和可持续性，通过ζ策略实现接近最优性能同时减少资源消耗


<details>
  <summary>Details</summary>
Motivation: 现有流式挖掘集成方法过度关注预测能力而忽视计算成本，需要更可持续的绿色在线学习方法

Method: 基于马尔可夫决策过程框架，提出不同模型选择策略，特别是ζ策略专注于以降低的成本训练接近最优的模型

Result: 在11个基准数据集上的实验表明，ζ策略在保持高精度的同时显著减少资源消耗，有时甚至优于竞争对手

Conclusion: HEROS的ζ策略是当前最先进方法的重要贡献，实现了预测性能和可持续性之间的良好平衡
...
### [290] [Central Limit Theorems for Asynchronous Averaged Q-Learning](https://arxiv.org/abs/2509.18964)
*Xingtu Liu*
...
<details>
  <summary>Details</summary>
Motivation: 研究Q-learning算法在异步更新情况下的统计性质，特别是中心极限定理的建立，这对于理解算法的收敛性和统计推断具有重要意义。

Method: 采用Polyak-Ruppert平均技术，分析异步Q-learning的收敛行为，建立非渐近中心极限定理和函数中心极限定理。

Result: 得到了收敛速率在Wasserstein距离下明确依赖于迭代次数、状态-动作空间大小、折扣因子和探索质量的中心极限定理，并证明部分和过程弱收敛于布朗运动。

Conclusion: 该研究为异步Q-learning提供了严格的统计理论基础，揭示了算法收敛的统计性质，对强化学习的理论分析和实际应用具有重要价值。
...
### [291] [Otters: An Energy-Efficient SpikingTransformer via Optical Time-to-First-Spike Encoding](https://arxiv.org/abs/2509.18968)
*Zhanglu Yan,Jiayi Mao,Qianhui Liu,Fanfan Li,Gang Pan,Tao Luo,Bowen Zhu,Weng-Fai Wong*

Main category: cs.LG

TL;DR: 本文提出了一种名为Otters的新型光电突触硬件，利用光电器件的自然信号衰减特性实现时间到首次脉冲编码，消除了传统SNN中昂贵的数字运算，在GLUE基准测试中达到最先进精度，能效比之前领先的SNN提高1.77倍。


<details>
  <summary>Details</summary>
Motivation: 传统SNN中的时间到首次脉冲编码虽然理论上能效高，但实际推理时需要评估时间衰减函数并与突触权重相乘，这些昂贵的数字操作往往无法实现预期的能效优势。

Method: 利用光电器件的自然物理衰减特性直接实现所需的时间函数，将器件的模拟输出视为突触权重和时间衰减的融合乘积；同时开发了量化神经网络到SNN的转换算法，以支持复杂架构如Transformer。

Result: 在七个GLUE基准数据集上达到最先进精度，基于商业22nm工艺的能耗分析显示，能效比之前领先的SNN提高1.77倍。

Conclusion: 建立了一种新的能效SNN范式，将基础器件物理直接转化为强大的计算原语，实现了硬件-软件协同设计的完整解决方案。
...
### [292] [Learning From Simulators: A Theory of Simulation-Grounded Learning](https://arxiv.org/abs/2509.18990)
...
<details>
  <summary>Details</summary>
Motivation: SGNNs在真实标签有限或不可观测的领域取得了最先进性能，但缺乏理论基础，需要建立形式化的理论框架。

Method: 通过理论分析证明SGNNs实现摊销贝叶斯推断，推导模型错误设定下的泛化边界，并通过数值实验验证理论预测。

Result: SGNNs能够恢复潜在参数，在失配情况下保持鲁棒性，在模型选择任务中比AIC错误率降低一半。

Conclusion: SGNNs为数据有限场景下的科学预测提供了一个原则性和实用的框架。
...
### [293] [CR-Net: Scaling Parameter-Efficient Training with Cross-Layer Low-Rank Structure](https://arxiv.org/abs/2509.18993)
*Boao Kong,Junzhu Liang,Yuxi Liu,Renjia Deng,Kun Yuan*

Main category: cs.LG

TL;DR: CR-Net是一种创新的低秩参数高效框架，通过利用层间激活残差的低秩特性，在保持高性能的同时显著降低计算和内存需求
...
Method: 采用双路径架构，通过结合前一层输出与其低秩差异来高效重构层激活，并开发专门的重计算策略降低内存需求

Result: 在60M到7B参数规模的预训练实验中，CR-Net持续优于最先进的低秩框架，同时需要更少的计算资源和内存

Conclusion: CR-Net成功解决了低秩方法的局限性，为高效LLM预训练提供了有效的解决方案
...
### [294] [Theoretical Foundations of Representation Learning using Unlabeled Data: Statistics and Optimization](https://arxiv.org/abs/2509.18997)
*Pascal Esser,Maximilian Fleissner,Debarghya Ghoshdastidar*

Main category: cs.LG

TL;DR: 本文概述了无标签数据表示学习的最新理论进展，特别是针对深度学习模型的新原理与传统统计理论的差异分析。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型使用自监督、去噪/掩码自编码器等新原理进行无监督表示学习，但这些方法难以用经典理论分析，需要结合统计和优化的数学工具来理解其表征特性和性能表现。

Method: 结合统计学和优化的数学工具，分析深度学习模型中无监督表示学习的理论原理，特别是视觉基础模型的自监督和掩码自编码器方法。

Result: 提供了无标签数据表示学习的最新理论进展概述，并介绍了作者在该方向上的贡献。

Conclusion: 需要发展新的理论框架来理解和分析深度学习模型中的无监督表示学习原理，以解释其在不同预测任务中的优异表现和涌现行为。
...
### [295] [Fully Learnable Neural Reward Machines](https://arxiv.org/abs/2509.19017)
*Hazem Dewidar,Elena Umili*

Main category: cs.LG

TL;DR: 本文提出了一种完全可学习的神经奖励机（FLNRM），能够端到端学习符号接地函数和自动机，无需依赖先验知识，在非马尔可夫强化学习任务中优于基于RNN的方法。
...
Method: 提出完全可学习的神经奖励机（FLNRM），通过端到端学习同时获取符号接地函数和自动机表示。

Result: FLNRM方法在非马尔可夫强化学习任务中优于基于循环神经网络（RNN）的先前方法。

Conclusion: FLNRM方法既保持了经典深度强化学习的易用性，又通过有限自动机的紧凑性提供了更好的可解释性，同时实现了更好的性能。
...
### [296] [OmniBridge: Unified Multimodal Understanding, Generation, and Retrieval via Latent Space Alignment](https://arxiv.org/abs/2509.19018)
*Teng Xiao,Zuchao Li,Lefei Zhang*

Main category: cs.LG

TL;DR: OmniBridge是一个统一的多模态框架，支持视觉语言理解、生成和检索任务，采用语言中心设计和轻量级双向潜在对齐模块，通过两阶段解耦训练策略实现多模态统一建模。
...
Method: 采用语言中心设计重用预训练LLM，引入轻量级双向潜在对齐模块，提出两阶段解耦训练策略：监督微调与潜在空间对齐，以及语义引导的扩散训练。
...
### [297] [Improving Credit Card Fraud Detection through Transformer-Enhanced GAN Oversampling](https://arxiv.org/abs/2509.19032)
*Kashaf Ul Emaan*

Main category: cs.LG

TL;DR: 本文提出了一种基于Transformer编码器的GAN混合方法，用于生成高质量的信用卡欺诈交易样本，以解决数据集严重不平衡的问题。
...
Method: 使用带有Transformer编码器块的生成对抗网络（GAN），GAN通过对抗训练生成真实样本，Transformer通过自注意力机制学习丰富的特征交互。

Result: 在公开的信用卡欺诈检测数据集上测试，与多种分类器（LR、RF、XGBoost、SVM）相比，基于Transformer的GAN在Recall、F1-score和AUC指标上均有显著提升。

Conclusion: 该混合策略有效克服了SMOTE、CTGAN和TVAE的局限性，能够生成高质量的少数类样本，显著改善了欺诈检测中的类别不平衡问题。
...
### [298] [Latent Danger Zone: Distilling Unified Attention for Cross-Architecture Black-box Attacks](https://arxiv.org/abs/2509.19044)
*Yang Li,Chenyu Wang,Tingrui Wang,Yongwei Wang,Haonan Li,Zhunga Liu,Quan Pan*

Main category: cs.LG

TL;DR: JAD是一个基于潜在扩散模型的黑盒对抗攻击框架，通过联合注意力蒸馏策略实现跨架构的对抗样本生成，提高了攻击的泛化性和效率。
...
Method: 使用潜在扩散模型，通过从CNN和ViT模型中提取的注意力图作为指导，生成对抗扰动，重点关注不同架构共同敏感的图像区域。
...
### [299] [Beyond Backpropagation: Exploring Innovative Algorithms for Energy-Efficient Deep Neural Network Training](https://arxiv.org/abs/2509.19063)
*Przemysław Spyra*

Main category: cs.LG

TL;DR: 本文系统比较了三种无需反向传播的深度学习训练方法（FF、CaFo、MF），发现MF算法在MLP架构上不仅分类精度超越BP，还能降低41%能耗和34%训练时间，为可持续AI发展提供了数据驱动的路线图。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络的计算和能耗需求不断增长，主要受反向传播驱动，这对可持续AI发展构成挑战。本文旨在研究BP-free训练方法的可行性和效率。

Method: 建立了稳健的比较框架：在原生架构上实现三种算法（FF和MF用MLP，CaFo用CNN），与等效BP模型对比。使用Optuna优化超参数，基于验证性能应用一致的早停标准。

Result: MF在MLP上的分类精度不仅与BP相当，而且持续超越。硬件级测量显示MF降低能耗41%，缩短训练时间34%，收敛到验证损失景观中更有利的最小值。
...
### [300] [Diffusion Bridge Variational Inference for Deep Gaussian Processes](https://arxiv.org/abs/2509.19078)
...
Method: DBVI通过参数化的神经网络从数据依赖的初始分布开始反向扩散过程，使用ELBO目标的梯度逐步调整初始化，减少后验差距并提高样本效率。

Result: 在回归、分类和图像重建任务中，DBVI在预测精度、收敛速度和后验质量方面一致优于DDVI和其他变分基线方法。

Conclusion: DBVI保留了DDVI的数学优雅性，同时通过可学习的初始分布显著提升了深度高斯过程后验推断的效率和性能。
...
### [301] [Graph Neural Networks with Similarity-Navigated Probabilistic Feature Copying](https://arxiv.org/abs/2509.19084)
*Asela Hevapathige*

Main category: cs.LG

TL;DR: AxelGNN是一种新颖的图神经网络架构，基于Axelrod文化传播模型，通过相似性门控概率交互、特征级复制机制和全局极化保持来解决GNN中的特征过度平滑、异构关系处理和特征向量不可分问题。


<details>
  <summary>Details</summary>
Motivation: 解决图神经网络在深层网络中节点表示不可区分、难以有效处理异构关系以及特征向量作为不可分单元处理的局限性。

Method: 提出AxelGNN架构，采用相似性门控概率交互机制自适应促进收敛或发散，实现特征级复制机制进行细粒度特征聚合，并保持全局极化以维护节点在不同表示簇中的区分度。

Result: 在节点分类和影响力估计基准测试中，AxelGNN在不同同质-异质特性的图结构上一致优于或匹配最先进的GNN方法。

Conclusion: AxelGNN的双稳态收敛动力学能够在一个架构内自然处理同质和异质图，为GNN提供了更灵活和有效的解决方案。
...
### [302] [Asymptotically Optimal Problem-Dependent Bandit Policies for Transfer Learning](https://arxiv.org/abs/2509.19098)
*Adrien Prevost,Timothee Mathieu,Odalric-Ambrym Maillard*

Main category: cs.LG

TL;DR: 该论文研究了多臂老虎机问题中的迁移学习设置，提出了KL-UCB-Transfer算法，在源分布和目标分布足够接近时显著优于无先验基线。


<details>
  <summary>Details</summary>
Motivation: 研究如何在多臂老虎机问题中利用来自相关任务的先验知识（源分布样本）来加速学习过程，特别是在源分布和目标分布存在已知距离约束的情况下。

Method: 首先推导了包含迁移参数的问题依赖渐近遗憾下界，然后提出了KL-UCB-Transfer索引策略，该策略在高斯情况下能够匹配这个新的下界。

Result: 理论分析表明KL-UCB-Transfer算法能够达到最优的渐近遗憾界，仿真实验验证了当源分布和目标分布足够接近时，该算法显著优于无先验知识的方法。

Conclusion: 该研究为多臂老虎机中的迁移学习提供了理论框架和实用算法，证明了在适当的迁移条件下，利用先验知识可以显著提高学习效率。
...
### [303] [Algorithms for Adversarially Robust Deep Learning](https://arxiv.org/abs/2509.19100)
*Alexander Robey*
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型在安全关键领域的广泛应用，确保模型决策能够抵御对抗性攻击变得至关重要。论文旨在设计具有理想鲁棒性特性的算法。

Method: 针对计算机视觉中的对抗样本问题，提出了新的技术结果、训练范式和认证算法；针对领域泛化问题，开发了在医学影像、分子识别和图像分类中达到最先进泛化性能的新算法；针对LLM越狱问题，提出了新的攻击和防御方法。

Result: 在多个领域实现了最先进的性能：在医学影像、分子识别和图像分类中取得了优异的泛化结果；在LLM安全方面代表了设计鲁棒语言智能体的前沿进展。

Conclusion: 该论文在深度学习模型鲁棒性方面做出了重要贡献，为安全关键应用中可靠AI系统的开发提供了新的算法基础和技术保障。
...
### [304] [DRO-REBEL: Distributionally Robust Relative-Reward Regression for Fast and Efficient LLM Alignment](https://arxiv.org/abs/2509.19104)
*Sharan Sahu,Martin T. Wells*

Main category: cs.LG

TL;DR: DRO-REBEL是一种鲁棒的离线RLHF方法，通过Wasserstein、KL和χ²模糊集解决现有方法的过优化问题，在保持可扩展性的同时实现最优参数率。
...
Method: 提出DRO-REBEL统一框架，使用Fenchel对偶将更新简化为相对奖励回归，避免了PPO式裁剪和辅助价值网络。推导了三种散度的实用SGD算法。

Result: 在Emotion Alignment、ArmoRM多目标基准和HH-Alignment上表现出强大的最坏情况鲁棒性，χ²-REBEL表现尤为突出。验证了半径-覆盖率的无免费午餐权衡。

Conclusion: DRO-REBEL在理论和实证上都表现出色，为RLHF提供了鲁棒且可扩展的解决方案，同时揭示了覆盖率保证与收敛速率之间的权衡关系。
...
### [305] [Towards Practical Multi-label Causal Discovery in High-Dimensional Event Sequences via One-Shot Graph Aggregation](https://arxiv.org/abs/2509.19112)
...
<details>
  <summary>Details</summary>
Motivation: 理解事件序列中的因果关系对于医疗保健、车辆诊断等领域至关重要，但目前仍是一个未解决的挑战，特别是在处理数千种独特事件类型的高维稀疏序列时。

Method: 使用两个预训练的因果Transformer作为领域特定的基础模型，并行处理每个序列以推断因果图，然后通过自适应频率融合聚合这些图来重建标签的全局马尔可夫边界。

Result: 在具有29,100多个独特事件类型和474个不平衡标签的真实世界汽车故障预测数据集上，CARGO展示了执行结构化推理的能力。
...
### [306] [FedFiTS: Fitness-Selected, Slotted Client Scheduling for Trustworthy Federated Learning in Healthcare AI](https://arxiv.org/abs/2509.19120)
*Ferdinand Kahenga,Antoine Bagula,Sajal K. Das,Patrick Sello*

Main category: cs.LG

TL;DR: FedFiTS是一个结合信任和公平感知的联邦学习框架，通过基于适应度的客户端选举和时隙聚合机制，解决非IID数据、客户端不可靠性和对抗性攻击等挑战。
...
Method: 采用三阶段参与策略（自由训练、自然选择、时隙团队参与），结合动态客户端评分、自适应阈值和基于队列的调度，平衡收敛效率与鲁棒性。

Result: 在医学影像、视觉基准和表格农业数据上的实验表明，FedFiTS在准确性、达到目标时间和对中毒攻击的韧性方面持续优于FedAvg、FedRand和FedPow。

Conclusion: FedFiTS通过集成信任感知聚合和公平导向的客户端选择，推进了可扩展和安全的联邦学习，特别适合现实世界的医疗和跨领域部署。
...
### [307] [Analysis on distribution and clustering of weight](https://arxiv.org/abs/2509.19122)
...
Conclusion: 标准差向量和聚类向量是分析大语言模型权重特征的有效工具，能揭示模型间的关系和微调过程中的变化规律。
...
### [308] [PipelineRL: Faster On-policy Reinforcement Learning for Long Sequence Generatio](https://arxiv.org/abs/2509.19128)
*Alexandre Piché,Ehsan Kamaloo,Rafael Pardinas,Dzmitry Bahdanau*

Main category: cs.LG

TL;DR: PipelineRL是一种用于大规模语言模型强化学习训练的新方法，通过并发异步数据生成和模型训练，结合飞行中权重更新机制，在保持数据新鲜度的同时实现硬件加速器的高效利用。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在增强大语言模型推理能力时面临扩展性挑战，主要问题是在保持高AI加速器利用率的同时避免产生过时的离策略数据，这些数据会损害常见的RL算法性能。

Method: PipelineRL采用并发异步数据生成和模型训练，核心创新是飞行中权重更新机制，允许语言模型生成引擎在生成token序列过程中以最小中断接收更新的模型权重。

Result: 在128个H100 GPU上进行的实验表明，PipelineRL在长形式推理任务中相比传统RL基线实现了约2倍的学习速度提升，同时保持高度在策略的训练数据。
...
### [309] [GSTM-HMU: Generative Spatio-Temporal Modeling for Human Mobility Understanding](https://arxiv.org/abs/2509.19135)
*Wenying Luo,Zhiyuan Lin,Wenhao Xu,Minghao Liu,Zhi Li*

Main category: cs.LG

TL;DR: GSTM-HMU是一个生成式时空框架，通过显式建模人类移动的语义和时间复杂性来推进移动性分析。该框架包含四个关键创新：时空概念编码器、认知轨迹记忆、生活方式概念库和任务导向生成头。在四个真实数据集上的实验显示，在三个基准任务上均取得了显著性能提升。
...
Result: 在Gowalla、WeePlace、Brightkite和FourSquare四个数据集上，对下一个位置预测、轨迹用户识别和时间估计三个任务进行实验，结果显示相比强基线取得了持续且显著的性能改进。
...
### [310] [Efficient Reinforcement Learning by Reducing Forgetting with Elephant Activation Functions](https://arxiv.org/abs/2509.19159)
...
<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘是强化学习中长期存在的挑战，现有研究主要关注算法层面，而对神经网络架构特性如何导致灾难性遗忘的理解不足，特别是激活函数的作用机制。

Method: 通过研究激活函数在神经网络训练动态中的角色，提出新的大象激活函数类别，该函数能同时产生稀疏输出和稀疏梯度，并在基于价值的算法中替换传统激活函数。

Result: 实验表明，使用大象激活函数可以显著提高神经网络对灾难性遗忘的抵抗能力，使强化学习更加样本高效和内存高效。

Conclusion: 激活函数的梯度稀疏性是减少灾难性遗忘的关键因素，大象激活函数为缓解这一问题提供了有效的架构解决方案。
...
### [311] [Unveiling the Role of Learning Rate Schedules via Functional Scaling Laws](https://arxiv.org/abs/2509.19189)
...
Method: 使用师生核回归设置和在线SGD训练，引入内在时间视角和SDE建模，提出FSL框架来分析不同学习率调度下的训练动态。

Result: FSL能够准确捕捉学习率调度的影响，理论验证了高容量模型更高效、学习率衰减能提升效率、WSD调度优于直接衰减等经验实践。
...
### [312] [A Validation Strategy for Deep Learning Models: Evaluating and Enhancing Robustness](https://arxiv.org/abs/2509.19197)
*Abdul-Rauf Nuhu,Parham Kebria,Vahid Hemmati,Benjamin Lartey,Mahmoud Nabil Mahmoud,Abdollah Homaifar,Edward Tunstel*

Main category: cs.LG

TL;DR: 提出了一种基于训练数据中弱鲁棒样本的模型鲁棒性验证方法，通过局部鲁棒性分析识别最易受扰动影响的样本，作为模型脆弱性的早期敏感指标。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒性验证依赖扰动测试数据集，但这种方法可能不够敏感。深度学习分类器在干净数据集上表现良好，但对对抗性和常见失真扰动仍然脆弱，需要更有效的鲁棒性评估方法。

Method: 从训练数据集中通过局部鲁棒性分析提取"弱鲁棒"样本，这些样本对扰动最敏感，作为模型脆弱性的指示器。在CIFAR-10、CIFAR-100和ImageNet数据集上验证方法有效性。

Result: 基于弱鲁棒样本的鲁棒性验证能够更细致地理解模型鲁棒性，并为针对性性能提升提供指导。

Conclusion: 该方法能够有效驱动模型在对抗性和常见失真场景下的可靠性改进，为模型鲁棒性评估提供了新的视角。
...
### [313] [PPG-Distill: Efficient Photoplethysmography Signals Analysis via Foundation Model Distillation](https://arxiv.org/abs/2509.19215)
*Juntong Ni,Saurabh Kataria,Shengpu Tang,Carl Yang,Xiao Hu,Wei Jin*

Main category: cs.LG

TL;DR: PPG-Distill是一个知识蒸馏框架，通过预测、特征和补丁级别的蒸馏，将大型PPG基础模型压缩到资源受限的可穿戴设备上，实现高效PPG分析


<details>
  <summary>Details</summary>
Motivation: 光电容积描记术（PPG）广泛应用于可穿戴健康监测，但大型PPG基础模型难以在资源受限的设备上部署

Method: 采用知识蒸馏框架，结合形态蒸馏保持局部波形模式，节奏蒸馏捕获补丁间时间结构，通过预测级、特征级和补丁级蒸馏传递全局和局部知识

Result: 在心率和房颤检测任务上，PPG-Distill将学生模型性能提升高达21.8%，推理速度提高7倍，内存使用减少19倍

Conclusion: PPG-Distill框架能够在可穿戴设备上实现高效的PPG分析，为资源受限环境下的健康监测提供了可行解决方案
...
### [314] [FedFusion: Federated Learning with Diversity- and Cluster-Aware Encoders for Robust Adaptation under Label Scarcity](https://arxiv.org/abs/2509.19220)
*Ferdinand Kahenga,Antoine Bagula,Patrick Sello,Sajal K. Das*

Main category: cs.LG

TL;DR: FedFusion是一个联邦迁移学习框架，通过统一域适应和节俭标注来解决联邦学习中的异构特征空间、非IID数据和标签稀缺问题。
...
Method: 使用多样性/聚类感知编码器（DivEn系列），通过置信度过滤的伪标签和域自适应迁移，结合相似性加权分类器耦合和选择性微调。

Result: 在表格和图像基准测试中，FedFusion在IID、非IID和标签稀缺情况下，在准确性、鲁棒性和公平性方面均优于现有基线方法。

Conclusion: 协调个性化、域适应和标签效率是在现实约束下实现鲁棒联邦学习的有效方法。
...
TL;DR: 本文对最先进的开源文本到视频生成模型进行了系统性的延迟和能耗研究，开发了计算受限的分析模型来预测缩放规律，并通过实验验证了预测结果。


<details>
  <summary>Details</summary>
Motivation: 文本到视频生成系统虽然能够创建高质量视频，但计算成本高昂且能耗需求未被充分理解，需要系统研究其延迟和能耗特性。

Method: 开发计算受限分析模型预测空间分辨率、时间长度和去噪步骤的缩放规律，在WAN2.1-T2V模型上进行细粒度实验验证，并扩展到六个不同的T2V模型进行比较分析。

Result: 实验验证了预测的缩放规律：空间和时间维度呈二次增长，去噪步骤呈线性缩放，为不同T2V模型提供了运行时和能耗基准。
...
TL;DR: 本文通过消融研究分析了电力系统混合机器学习模型中物理约束集成策略的效果，使用LIPS基准测试框架评估了从简单多层感知器到图神经网络等多种架构在准确性、物理合规性、工业准备度和分布外泛化四个维度上的表现。


<details>
  <summary>Details</summary>
Motivation: 随着能源转型中可再生能源和跨境电力交换的增加，电网面临更大的不确定性和运行风险。传统物理求解器虽然准确但速度慢，无法满足近实时需求，而纯机器学习模型缺乏物理约束保证。需要研究如何有效将物理知识融入机器学习模型以提高性能。

Method: 使用自定义的LIPS基准测试管道，对多种混合模型策略进行消融研究，包括将物理约束作为正则化项或无监督损失的集成方法，以及从简单多层感知器到支持物理方程直接优化的图神经网络等不同架构。

Result: 研究结果揭示了物理知识集成对这些模型在准确性、物理合规性、工业准备度和分布外泛化四个维度上的具体影响，为混合模型设计提供了实证指导。

Conclusion: 通过系统性的消融研究，明确了不同物理约束集成策略和模型架构对混合模型性能的影响，所有实现均可复现，为电力系统快速仿真提供了实用的混合建模指导。
...
### [317] [Stability and Generalization of Adversarial Diffusion Training](https://arxiv.org/abs/2509.19234)
*Hesam Hosseini,Ying Cao,Ali H. Sayed*

Main category: cs.LG

TL;DR: 该论文提出了在去中心化网络中使用扩散策略的对抗训练稳定性分析，推导了泛化误差与对抗扰动强度和训练步数相关的界限。
...
Method: 采用基于稳定性的泛化分析方法，针对凸损失函数在扩散策略下进行理论分析，并通过逻辑回归进行数值实验验证。

Result: 推导出泛化误差随对抗扰动强度和训练步数增长的界限，这一发现在单智能体情况下一致，但在去中心化设置中是新颖的。数值实验验证了理论预测。

Conclusion: 该工作首次在去中心化设置中建立了对抗训练的泛化分析框架，为理解去中心化对抗学习的泛化特性提供了理论依据。
...
### [318] [What Characterizes Effective Reasoning? Revisiting Length, Review, and Structure of CoT](https://arxiv.org/abs/2509.19284)
*Yunzhen Feng,Julia Kempe,Cheng Zhang,Parag Jain,Anthony Hartshorn*

Main category: cs.LG

TL;DR: 该论文研究发现，在大型推理模型中，更长的思维链（CoT）和更多的回顾步骤反而会降低准确性。作者提出了"失败步骤比例（FSF）"这一指标，能更好地预测CoT的有效性，并证明通过移除失败分支可以显著提高推理准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型在测试时需要大量计算资源来生成长思维链，但什么构成有效的思维链仍不清楚。先前研究认为更长的思维链和更多回顾步骤能提高性能，但最近研究表明短思维可能优于长思维，因此需要系统评估。

Method: 在10个大型推理模型上进行数学和科学推理的系统评估；引入思维链的图结构视图来提取结构；提出失败步骤比例（FSF）指标；设计两种干预实验：基于指标排名候选思维链和编辑思维链移除失败分支。

Result: 发现思维链长度增加和回顾比例提高与准确性降低相关；FSF指标在预测正确性方面优于长度和回顾比例；移除失败分支的干预显著提高了准确性；基于FSF的排名在pass@1指标上获得最大增益。

Conclusion: 有效的思维链是那些失败步骤更少的思维链，支持基于结构感知的测试时扩展策略，而不是盲目生成长思维链。失败分支会对后续推理产生偏见，移除它们能改善模型性能。
