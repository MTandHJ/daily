...
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观量化方法。本研究旨在利用数学工具为艺术分析提供结构化视角。

Method: 采用拓扑数据分析中的持久同调（PH）方法，该方法能够捕捉图像中的拓扑特征。

Result: PH方法能够以统计显著性区分不同艺术家、同一艺术流派内的艺术家，并能有效区分艺术家原作与AI生成的模仿风格图像。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的洞察工具，能够超越传统主观判断，实现艺术风格的量化区分。
...
### [2] [A Little More Like This: Text-to-Image Retrieval with Vision-Language Models Using Relevance Feedback](https://arxiv.org/abs/2511.17255)
*Bulat Khaertdinov,Mirela Popa,Nava Tintarev*

Main category: cs.CV

TL;DR: 本文提出了一种基于相关性反馈的机制来改进视觉语言模型的检索性能，包括四种反馈策略：伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型需要微调或扩展到更大模型变体来提升性能，本文旨在通过推理时的相关性反馈机制来改进检索性能，这种模型无关的设计也可与微调后的VLMs配合使用。

Method: 提出四种反馈策略：1）伪相关性反馈（PRF）基于排名靠前结果优化查询嵌入；2）生成式相关性反馈（GRF）使用合成描述进行查询优化；3）注意力反馈总结器（AFS）整合多模态细粒度特征的自定义transformer模型；4）使用真实描述作为显式反馈的上界基准。

Result: 在Flickr30k和COCO数据集上的实验显示，GRF、AFS和显式反馈相比无反馈检索，在较小VLMs上MRR@5提升3-5%，在较大VLMs上提升1-3%。AFS与显式反馈类似，能减轻查询漂移并在多轮迭代检索中比GRF更稳健。

Conclusion: 相关性反馈能持续提升不同VLMs的检索性能，为交互式和自适应视觉搜索开辟了新机会。
...
<details>
  <summary>Details</summary>
Motivation: 由于可用于训练的动物面部图像在数量和变化上都很有限，特别是在不同个体的面部表情方面，因此生成动物面部具有挑战性。本研究聚焦于在系统神经科学和进化研究中广泛研究的猕猴。

Method: 使用基于风格的生成图像模型StyleGAN2，实施：1）通过运动转移使用计算机图形学为静态图像添加动画来合成新的面部表情图像进行数据增强；2）基于初始训练的StyleGAN2模型的潜在表示进行样本选择，确保训练数据集的变异性和均匀采样；3）损失函数优化以确保准确再现细微运动（如眼球运动）。

Result: 所提出的方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型。此外，模型在基于风格的图像编辑方面也很有效，特定风格参数对应不同的面部运动。

Conclusion: 这些发现强调了模型在将运动组件解耦为风格参数方面的潜力，为猕猴面部表情研究提供了有价值的工具。
...
TL;DR: 本文提出了首个大规模双人肖像生成基准数据集PairHuman，包含10万+高质量图像，并开发了DHumanDiff基线方法，在保持面部一致性的同时平衡个性化人物生成和语义驱动场景创建。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制具有重要应用价值，但缺乏基准数据集阻碍了高质量双人生成的发展。现有方法难以同时满足面部一致性和场景语义的要求。

Method: 构建PairHuman数据集（10万+图像，包含丰富元数据），并提出DHumanDiff基线方法，增强面部一致性，平衡个性化人物生成和语义驱动场景创建。

Result: 实验结果表明，该方法能生成高度定制化、视觉质量优越且符合人类偏好的双人肖像。

Conclusion: PairHuman数据集和DHumanDiff方法为双人肖像生成提供了有效解决方案，显著提升了生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通过微调提升视觉语言模型安全性时，往往导致泛化性能显著下降。这种权衡源于僵化的对齐策略，破坏了模型学习的语义结构。
...
<details>
  <summary>Details</summary>
Motivation: 单对象SVG的多视图一致生成尚未充分探索，需要解决几何和颜色一致性问题，支持资产创建和语义向量编辑等应用。

Method: 三阶段方法：1) 将光栅化输入提升到3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立部分级对应关系；3) 在光栅到向量转换中执行路径整合和结构优化。

Result: 生成的SVG在多视图间表现出强烈的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。

Conclusion: 该工作连接了生成建模和结构化向量表示，为单输入、对象级多视图SVG生成提供了可扩展路径，支持资产创建和语义向量编辑应用。
...
Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术来检索、生成和整合网格组件，解耦生成过程的序列依赖性。

Result: Mesh RAG显著提升了网格质量，加速了生成速度（相比顺序部件预测），并实现了增量编辑功能，无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成模型的质量-速度权衡问题，为3D内容创作提供了更高效的解决方案。
...
### [9] [Navigating in the Dark: A Multimodal Framework and Dataset for Nighttime Traffic Sign Recognition](https://arxiv.org/abs/2511.17183)
*Aditya Mishra,Akshay Agarwal,Haroon Lone*

Main category: cs.CV

TL;DR: 本文提出了INTSD夜间交通标志数据集和LENS-Net方法，解决了夜间交通标志识别中光照不足和缺乏多模态线索利用的问题。


<details>
  <summary>Details</summary>
Motivation: 夜间交通标志识别面临视觉噪声和公共夜间数据集稀缺的挑战，现有方法在低光照条件下鲁棒性不足，且未能有效利用互补的多模态线索。

Method: 首先引入INTSD大规模夜间交通标志数据集；其次提出LENS-Net方法，集成自适应图像增强检测器进行联合光照校正和标志定位，然后使用结构化多模态CLIP-GCNN分类器，结合跨模态注意力和基于图的推理。

Result: 该方法超越了现有框架，消融研究证实了其关键组件的有效性。数据集和代码已公开供研究使用。

Conclusion: INTSD数据集和LENS-Net方法为夜间交通标志识别提供了全面的基准和有效的解决方案，显著提升了在低光照条件下的识别性能。
...
Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，采用模块化设计支持对布局、规模和样式的细粒度控制。

Result: 系统能够生成几何一致、视觉丰富且实时渲染效率高的大规模交互式3D世界，可直接在标准游戏引擎中探索或编辑。
...
### [11] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 本文分析了多语言文本到图像模型在跨语言文化一致性方面的问题，发现现有模型往往产生文化中立或英语偏见的结果。通过定位文化敏感神经元，提出了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 多语言文本到图像模型虽然发展迅速，但在跨语言文化一致性方面存在问题。语言承载着文化内涵，但当前模型在生成多语言提示的图像时，往往无法保持文化一致性，产生文化中立或英语偏见的结果。

Method: 1. 提出探测方法定位文化敏感信号到少量固定层的神经元；2. 引入两种互补对齐策略：推理时文化激活（放大已识别神经元）和层定向文化增强（仅更新文化相关层）。

Result: 在CultureBench上的实验表明，相比强基线方法，所提方法在文化一致性方面取得了一致性改进，同时保持了保真度和多样性。

Conclusion: 研究表明，文化一致性问题的根源不是文化知识的缺失，而是文化相关表征激活不足。通过定位和增强文化敏感神经元，可以有效提升多语言文本到图像模型的文化一致性。
...
TL;DR: REO-Instruct是首个用于地球观测中描述和回归任务的统一基准，通过建立森林生态场景的认知逻辑链，连接定性理解和定量预测。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖6个任务，旨在评估和训练视觉语言模型在精细空间理解方面的能力。
...
Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，显著提升了模型在精细物体交互推理方面的能力，并发布了数据集和生成管道。
...
TL;DR: 提出Parts-Mamba模型，结合GCN和Mamba架构，通过部分特定扫描特征和部分-身体融合模块来增强对远距离关节上下文信息的捕捉能力，在骨架动作识别中处理遮挡和不完整骨架问题。
...
Method: 提出混合GCN-Mamba模型，通过部分特定扫描特征捕获部分特定信息，并通过部分-身体融合模块保持非相邻关节上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下评估，准确率提升高达12.9%。

Conclusion: Parts-Mamba模型能有效增强对远距离关节上下文信息的捕捉能力，在骨架不完整情况下显著提升动作识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Gromov-Wasserstein距离仅限于单对象间的成对匹配，无法满足需要多对一或多对多对象匹配的场景和应用需求。

Method: 引入JGW目标，将GW框架扩展到同时匹配对象集合，采用最优传输算法（包括熵正则化）来求解点云对象表示。
...
TL;DR: 该论文提出了一种名为REPA（表示对齐）的方法，通过将扩散或流式生成模型的内部表示与预训练自监督编码器（如DINOv2）对齐，来改进逆问题中的重建质量。该方法在推理时引导重建过程，无需真实信号即可显著提升重建保真度和感知真实性。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，将生成模型的内部表示与预训练自监督编码器对齐可以提供强大的归纳偏置，改善收敛性和样本质量。本研究旨在将此思想扩展到逆问题领域，利用预训练生成模型作为先验，通过表示对齐来指导重建过程。

Method: 提出REPA方法，在推理时将扩散或流式生成模型与预训练自监督视觉编码器（如DINOv2）的表示对齐。虽然逆问题中缺乏真实信号，但通过与近似目标特征对齐来增强重建质量。提供了理论分析，展示REPA正则化与DINOv2嵌入空间中散度度量的关系，以及REPA更新如何引导模型内部表示朝向干净图像表示。

Result: 在超分辨率、框内修复、高斯去模糊和运动去模糊等任务上的广泛实验表明，该方法能一致地提高各任务的重建质量，同时通过减少所需离散化步骤数量提供显著的效率提升，且不影响底层求解器的性能。

Conclusion: REPA方法为逆问题提供了一种有效的表示对齐框架，能够显著提升重建的感知真实性和保真度，同时提高计算效率。该方法具有通用性，可集成到多种最先进的逆问题求解器中。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的内在特性。观察到现实场景中玻璃两侧光照强度差异会导致反射变化，这种反射动态特性可用于更准确的玻璃检测。
...
Conclusion: 利用闪光/无闪光图像中的反射动态特性是玻璃表面检测的有效方法，NFGlassNet通过专门的反射挖掘和特征融合模块实现了优越的检测性能。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视频时空推理任务。R-AVST包含5K+未剪辑视频和27K+对象，覆盖100种音频-视频事件；AVST-Zero采用强化学习方法直接优化行为，无需中间监督。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中进展迅速，但主要关注简单视频场景，无法反映真实世界中复杂多样的音频-视频事件。需要构建更贴近现实的数据集和模型来推动音频-视频时空推理能力的发展。

Method: 1) 构建R-AVST数据集：使用LLM提取关键对象、自动空间标注和人工质量检查的流程；2) 提出AVST-Zero模型：基于强化学习，通过精心设计的多维奖励直接优化行为，避免中间监督。

Result: R-AVST数据集包含5,000+未剪辑视频、27,000+对象和8,000+高质量问答对。AVST-Zero在R-AVST基准测试中表现出与现有模型相当的性能，验证了数据集和方法的有效性。

Conclusion: R-AVST是首个专为真实世界音频-视频时空推理设计的数据集，AVST-Zero为该领域未来挑战提供了新的解决视角，共同推动了音频-视频时空推理研究的发展。
...
TL;DR: 本文提出了Warm Diffusion，一种统一的模糊-噪声混合扩散模型（BNMD），通过联合控制模糊和噪声来整合热扩散和冷扩散的优势，解决了热扩散忽视图像相关性而冷扩散忽视噪声作用的问题。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用高频图像细节与低频结构之间的强相关性，导致生成早期步骤中的随机行为；而冷扩散仅使用模糊而忽视噪声在塑造数据流形中的作用，导致流形外问题和性能下降。需要整合两者的优势。

Method: 提出了Warm Diffusion（BNMD模型），采用分治策略利用图像中的频谱依赖性，通过解耦去噪和去模糊过程来简化分数模型估计。使用频谱分析研究模糊-噪声比（BNR）在模型学习动态和数据流形变化之间的权衡。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成方面的有效性。

Conclusion: Warm Diffusion成功整合了热扩散和冷扩散的优势，通过联合控制模糊和噪声，利用图像频谱依赖性简化模型估计，在图像生成任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统的异常生成方法需要少量异常样本进行微调，这与异常稀缺性的现实相矛盾，且容易过拟合类别先验。本文旨在解决无真实异常样本或训练数据可用的情况。

Method: 提出Delta-Denoising方法，使用最小提示对驱动两个共享调度的扩散分支进行对比，通过累积每步去噪差异生成图像特定的定位图，指导潜在修复过程，同时保留周围上下文并生成真实的局部缺陷。还包含标记级提示优化和空间注意力偏置等技术。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成效果、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效定位和生成局部缺陷，在异常检测任务中表现出色。
...
Conclusion: 特征域比像素域更适合信息补偿，上采样分辨率下的变形能更好保留高频信息，提出的DGAF-VSR方法在视频超分辨率的关键方面都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 在CBCT图像中准确分割牙齿对数字牙科至关重要，但邻牙粘连会导致严重的解剖形状失真，这仍然是一个挑战。

Method: 采用目标牙齿质心提示的多标签学习策略建模牙齿间的语义关系，结合牙齿形状感知学习机制强制执行形态约束，通过多任务学习统一优化分割和形状保持。
...
Method: 提出了OmniGround基准测试，采用前向-后向-精化标注流程；开发了DeepSTG系统评估框架；提出了PG-TAF训练免费两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡物体和复杂空间关系上。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务有效提升了时空视频定位性能，为复杂现实场景中的目标定位提供了有效解决方案。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，评估了50多个基础模型和商业模型。


<details>
  <summary>Details</summary>
Motivation: 当前隐私基准主要评估隐私感知，无法应对视觉语言模型从分布式信息推断和链接构建个体档案的隐私推理风险，存在关键评估缺口。

Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，设计九个挑战性任务评估从属性检测到跨图像重识别和链式推理的完整隐私推理能力。

Result: 评估发现：许多视觉语言模型存在显著的未测量推理隐私风险；感知级指标无法预测推理风险；现有安全对齐措施对此类推理攻击无效且不一致。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预先估计的光学流，但在欠采样情况下不准确且会降低重建质量，需要一种能同时恢复图像和运动场的方法。

Method: 使用两个隐式神经表示(INR)，一个参数化时空图像内容，另一个表示光学流，通过光学流方程进行物理约束，并结合数据一致性损失进行联合优化。
...
TL;DR: 本文提出了FingerCap任务和FiGOP方法，用于生成细粒度手指级别的手部动作文本描述，通过结合RGB帧和手部关键点来捕捉高频细微动作，在FingerCap-40K数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度手部动作对视觉感知、具身智能和多模态通信至关重要，现有视频MLLMs因时间稀疏性难以捕捉细微的手指动态。

Method: 提出FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时序编码器将关键点转换为运动嵌入并与RGB特征融合，在不增加RGB密度的情况下恢复精细时序线索。

Result: 在FingerCap-40K数据集上的实验表明，现有强视频MLLMs在手指级别推理上仍有困难，而FiGOP增强模型在HandJudge评估和人工研究中均取得一致提升。

Conclusion: FiGOP方法能有效解决视频MLLMs在捕捉细粒度手指动作时的时间稀疏性问题，为手部动作理解提供了新的解决方案。
...
TL;DR: 本文提出了一种基于点监督的面部表情检测方法，通过双分支框架解决单时间戳标注下的表情实例识别问题。


<details>
  <summary>Details</summary>
Motivation: 现有的面部表情检测方法主要依赖完全监督学习，需要昂贵的时间边界标注。本文研究点监督面部表情检测，仅需每个实例的单个时间戳标注，以降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块进行软伪标签生成；2）类别感知的顶点分类分支区分宏表情和微表情。同时引入强度感知对比损失来增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 该点监督面部表情检测框架能够有效利用单时间戳标注，通过软伪标签和双分支设计实现可靠的表情检测和分类。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR贴图的相互依赖潜在向量，使用视频扩散架构统一处理多种材料任务。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在向量（RGB外观和四个PBR贴图），将其表述为5帧序列并使用视频扩散架构来捕捉相关性。

Result: 在MatHybrid-410K混合语料库上训练，MatPedia实现了原生1024×1024合成，在质量和多样性上显著超越现有方法。

Conclusion: MatPedia提供了一个统一的框架，能够在单一架构中处理文本到材料生成、图像到材料生成和内在分解等多种材料任务。
...
TL;DR: 提出了一种面向边缘设备的烹饪过程图像生成方法，通过厨师标注的熟度级别和烹饪状态引导，从生食图像合成逼真的熟食图像，并引入领域特定的烹饪图像相似度指标。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从生食图像合成逼真的熟食图像具有挑战性，现有方法要么效果不真实，要么资源消耗过大，无法在边缘设备上部署。

Method: 构建首个基于烤箱的烹饪过程数据集，提出边缘高效的配方和烹饪状态引导生成器，引入领域特定的烹饪图像相似度指标作为训练损失和进度监控信号。

Result: 模型在现有基准上表现优异，FID分数显著降低（在自建数据集上提升30%，在公共数据集上提升60%）。

Conclusion: 该方法能够根据用户偏好的视觉目标而非固定预设生成逼真的烹饪图像，确保了时间一致性和烹饪合理性。
...
TL;DR: 本文提出了SeeCLIP框架来解决开放集域泛化问题，通过语义增强的CLIP模型来平衡已知类别的结构风险和未知类别的开放空间风险，特别针对与已知类别视觉相似但语义不同的'困难未知'类别。


<details>
  <summary>Details</summary>
Motivation: 现有的开放集域泛化方法在处理视觉相似但语义不同的'困难未知'类别时容易产生过度自信问题，无法有效平衡已知类别的结构风险和未知类别的开放空间风险。

Method: 提出了语义增强的CLIP框架，包括语义感知提示增强模块将图像分解为判别性语义标记，双工对比学习保持与已知类别的分离性和语义邻近性，以及语义引导的扩散模块生成伪未知样本。

Result: 在五个基准测试上的广泛实验表明，相比最先进方法，准确率提高了3%，H-score提高了5%。

Conclusion: SeeCLIP框架通过语义增强有效解决了开放集域泛化中的困难未知类别识别问题，在多个基准测试上取得了显著改进。
...
TL;DR: 提出受自然选择启发的3D高斯泼溅修剪框架，通过优化梯度自主决定保留或修剪高斯元素，无需人工干预，在15%预算下获得超过0.6 dB PSNR提升。
...
Method: 提出自然选择启发的修剪框架，将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定高斯元素的保留或修剪。引入带有限不透明度先验的不透明度衰减技术，加速选择过程。

Result: 与3DGS相比，在15%预算下实现超过0.6 dB PSNR增益，为紧凑型3DGS建立了最先进的性能。

Conclusion: 该框架完全可学习且无需人工干预，有效解决了3DGS的存储和计算开销问题，实现了高质量的紧凑表示。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去识别，利用化学中的替换机制概念，在潜在h空间中进行语义位移来编辑身份。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以达到良好平衡，需要开发一种能够有效保护隐私同时保持图像实用性的方法。
...
Conclusion: FLUID框架通过潜在空间的身份替换，为人脸去识别提供了一种有效的解决方案，在隐私保护和图像实用性之间取得了良好平衡。
...
Method: 引入DepthFocus，一个可控制的视觉Transformer，通过标量深度偏好条件化，动态调整计算以聚焦于预期深度。训练主要使用新构建的50万多层合成数据集，捕捉多样透射效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示意图对齐估计，并在未见透射场景中表现出强大的泛化能力。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和传统球位置信息，支持复杂人-物交互研究。
...
Method: 构建包含乒乓球、网球和羽毛球的数据集，提供球拍姿态细粒度标注和球位置信息，设计三个相互关联的任务：细粒度球跟踪、关节式球拍姿态估计和预测性球轨迹预测，并采用CrossAttention机制进行多模态融合。

Result: 评估现有基线发现关键洞察：简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，使轨迹预测结果超越强单模态基线。

Conclusion: RacketVision为动态物体跟踪、条件运动预测和体育多模态分析提供了多功能资源和强大起点，CrossAttention机制在多模态融合中至关重要。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索下稳健性的对抗性测试套件，发现现有模型在简单图像堆叠扰动下准确率大幅下降，揭示了文化概念混合对模型性能的严重影响。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型评估通常只关注单一文化概念，忽视了现实中多个文化线索共存的情况，需要测试模型在多样化文化输入下的稳定性以支持多元文化社会。

Method: 开发ConfusedTourist文化对抗稳健性测试套件，通过图像堆叠扰动和基于图像生成的变体来评估VLMs对地理线索扰动的稳定性，并进行可解释性分析。

Result: 实验显示VLMs在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动效果更差，可解释性分析表明失败源于系统性的注意力偏移到干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进的视觉语言模型，迫切需要开发更具文化稳健性的多模态理解系统。
...
TL;DR: 提出了一个统一的行人跟踪框架OmniPT，能够进行跟踪、基于参考的跟踪以及交互式生成跟踪对象的语义理解，解决了如何将跟踪任务建模为基础模型可执行任务以及如何使模型输出格式化答案的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然LVLMs在图像级任务中表现出色，但在实例级任务（如视觉定位和物体检测）中仍与专家模型存在性能差距。行人跟踪作为经典任务，结合物体跟踪和自然语言的新主题（如Referring MOT、Cross-view Referring MOT和Semantic MOT）要求模型在高级语义层面理解跟踪对象，这正是LVLMs的优势所在。

Method: 实现了一个由RL-Mid Training-SFT-RL组成的训练阶段：基于LVLM预训练权重，首先进行简单的RL阶段使模型输出固定且可监督的边界框格式；随后使用大量行人相关数据集进行中间训练阶段；最后在几个行人跟踪数据集上进行监督微调，然后进行另一个RL阶段以提高模型的跟踪性能并增强其遵循指令的能力。

Result: 在跟踪基准上的实验结果表明，所提出的方法能够比先前的方法表现更好。

Conclusion: 提出的OmniPT框架成功地将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略实现了优于先前方法的性能，特别是在语义理解方面展现了LVLMs的优势。
...
TL;DR: ReBrain是一个基于检索增强扩散的框架，用于从稀疏CT扫描重建完整脑部MRI体积，通过Brownian Bridge扩散模型和检索相似CT切片来指导MRI生成，在稀疏条件下实现最先进的跨模态重建性能。
...
Method: 提出ReBrain框架：使用Brownian Bridge扩散模型在2D维度合成MRI切片；通过微调检索模型从先验数据库中检索结构和病理相似的CT切片；利用ControlNet分支将检索切片作为参考指导中间MRI切片生成；对罕见检索失败情况应用球面线性插值提供补充指导。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下实现了最先进的跨模态重建性能。

Conclusion: ReBrain通过检索增强扩散框架有效解决了从稀疏CT扫描重建脑部MRI的挑战，在稀疏条件下表现出优越的重建性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中使用随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择不灵活。需要一种方法既能保持性能又能支持灵活的序列顺序。

Method: 利用图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的图像部分观测作为序列前缀出现。

Result: STAR通过定制但结构化的随机策略相比随机排列，保持了后缀完成能力，同时维持了采样性能，无需对广泛采用的语言AR模型架构进行重大更改。
...
TL;DR: 本文提出了一个与LVLMs的transformer因果架构对齐的综合干预框架，发现幻觉来源于图像-输入文本、图像-输出文本和文本-文本路径的相互作用，并提出了针对不同问答对齐格式的简单有效方法来识别和干预关键幻觉头。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但仍然容易产生幻觉。本研究旨在深入理解LVLMs中幻觉的因果路径，并开发有效的干预方法。

Method: 提出了一个综合干预框架，分析图像到输入文本、图像到输出文本和文本到文本三种路径对幻觉的影响，并针对判别式和生成式格式设计干预关键幻觉头的方法。

Result: 实验表明，该方法在多个基准测试中能够持续减少不同对齐类型的幻觉。

Conclusion: LVLMs中的幻觉并非来自单一因果路径，而是多种路径相互作用的结果，通过针对不同问答格式干预关键幻觉头可以有效减少幻觉。
...
TL;DR: 提出了一种基于语义掩码引导编辑真实LiDAR扫描数据的新方法，通过范围图像投影和语义掩码条件化实现扩散式生成，用于合成新颖的LiDAR点云数据。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样化的点云数据集，但获取真实世界中的边缘案例场景具有挑战性。现有基于手工3D虚拟环境的方法耗时、计算昂贵且难以完全捕捉真实场景复杂性。

Method: 将点云转换为2D范围视图图像作为中间表示，使用基于凸包的语义掩码进行语义编辑，通过语义掩码条件化实现扩散式生成，确保几何一致性和真实感。
...
TL;DR: REArtGS++提出了一种基于平面高斯抛光和时序几何约束的通用铰接物体重建方法，改进了REArtGS在螺旋关节和多部件物体上的表现，通过解耦螺旋运动和部件运动混合实现更好的部件级表面重建和关节参数估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多部件物体时存在困难，且缺乏对未见状态的几何约束。需要一种更通用的铰接物体重建方法来解决这些问题。

Method: 1) 为每个关节建模解耦螺旋运动，无需类型先验；2) 通过部件运动混合联合优化部件感知高斯和关节参数；3) 引入时序几何约束，鼓励高斯平面化，并通过泰勒一阶展开实现平面法向和深度的时序一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在通用部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时序几何约束和平面高斯抛光，显著提升了铰接物体重建的通用性和准确性，特别是在处理螺旋关节和多部件物体时表现优异。
...
TL;DR: 本文提出MuM模型，通过将掩码自编码扩展到任意多视图来学习3D视觉特征，在多个下游任务中超越了DINOv3和CroCo v2等最先进视觉编码器。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法主要优化语义理解而非几何推理，CroCo是面向3D理解的掩码自编码方法，本文旨在继续这一方向，专注于学习适合3D视觉的特征。

Method: 将掩码自编码扩展到任意多视图，通过统一掩码所有视图并使用带有帧间注意力的轻量级解码器，方法比CroCo更简单且更具可扩展性。

Result: 在包括前馈重建、密集图像匹配和相对姿态估计在内的下游任务中，MuM模型的表现优于最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过将掩码自编码扩展到多视图，MuM模型能够学习到更适合3D视觉的特征，在多个几何推理任务中取得了优越性能。
...
Result: 实验结果表明该方法显著增强了生成多样性，对性能影响可忽略不计。

Conclusion: DiverseVAR有效释放了VAR模型的内在生成潜力，同时保持了高保真度合成。
...
TL;DR: 该论文提出了一种生成多样化合成人脸图像的方法论，并创建了DIF-V数据集（27,780张图像，926个身份），以解决现有面部验证系统中存在的种族和性别偏见问题。


<details>
  <summary>Details</summary>
Motivation: 现有面部图像数据集在种族、性别等人口特征方面存在显著偏见，限制了面部验证系统的有效性和公平性。

Method: 提出了一种综合方法，集成先进的生成模型来创建多样化的高质量合成人脸图像，强调代表各种面部特征，并确保符合身份证照片的允许特征。

Result: 分析发现现有验证模型对某些性别和种族存在偏见，身份风格修改会对模型性能产生负面影响。
...
Method: 提出ReCoVAD框架，包含两个核心通路：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中等规模视觉语言模型生成文本事件描述和精炼异常分数，并集成大语言模型定期审查描述以识别未见异常。
...
TL;DR: 提出分区自适应对比学习(PACL)方法，通过利用预训练文本模型的知识来增强预训练视觉模型的情感感知能力，解决视觉情感识别中的'情感鸿沟'问题。
...
Conclusion: ChainV通过动态整合视觉提示显著提升了多模态推理的准确性和效率，特别是在需要多步符号推理的数学密集型任务中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 由于建模范式限制、严重运动模糊和物理一致性不足，大时空尺度的刚性运动重建仍然具有挑战性。

Method: 提出三重监督方案：通过加速度约束强制物理合理性，利用事件流进行高时间分辨率指导，使用卡尔曼正则器融合多源观测；设计基于实时运动状态的自适应模拟退火训练策略。

Result: 实验表明，与主流动态方法相比，PEGS在大时空尺度运动重建方面表现出优越性能。

Conclusion: PEGS框架成功解决了大时空尺度刚性运动重建的挑战，为相关领域提供了有效解决方案。
...
### [50] [REMSA: An LLM Agent for Foundation Model Selection in Remote Sensing](https://arxiv.org/abs/2511.17442)
*Binger Chen,Tacettin Emre Bök,Behnood Rasti,Volker Markl,Begüm Demir*

Main category: cs.CV

TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个模型，REMSA是基于LLM的智能体，能通过自然语言查询自动选择模型，并在专家评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 遥感领域的基础模型越来越多，但由于文档分散、格式异构和部署约束不同，选择合适的模型变得困难。需要一种系统化的方法来帮助用户选择适合的遥感基础模型。

Method: 构建了RSFM数据库（RS-FMD），包含150多个模型的多模态数据。在此基础上开发了REMSA智能体，使用LLM技术解释用户需求、解决约束缺失问题，通过上下文学习对候选模型进行排序。

Result: 提出了包含75个专家验证查询场景的基准测试，生成900个配置。REMSA在专家中心评估协议中优于多个基线方法，包括朴素智能体、密集检索和非结构化RAG-based LLMs。

Conclusion: REMSA是首个基于LLM的遥感基础模型自动选择智能体，能够有效解决模型选择难题，完全基于公开元数据运行，不访问私有或敏感数据。
...
TL;DR: 本文提出从传统的1D相关色温(CCT)插值转向2D色度空间，并使用轻量级多层感知机(MLP)来提高非普朗克光源(如LED)下的比色映射精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的比色映射方法针对普朗克光源(如CIE A和D65)进行优化，但现代LED光源可能显著偏离普朗克轨迹，暴露了依赖一维CCT进行光照表征的局限性。

Method: 使用基于2D色度特征的轻量级MLP替代传统CCT插值，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多样化LED照明下验证，该方法在LED照明场景中平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景，并可实时部署且计算成本可忽略。

Conclusion: 从1D CCT转向2D色度空间结合MLP方法能显著提高非普朗克光源下的比色精度，同时保持传统方法的优势。
...
TL;DR: SketchVerify是一个无需训练的基于草图验证的运动规划框架，通过测试时采样和验证循环来生成动态连贯的轨迹，提高视频生成的运动质量和物理真实性。
...
Conclusion: SketchVerify通过测试时采样和验证循环，在保持高效率的同时显著提升了视频生成的运动规划质量，为复杂动态场景的视频生成提供了有效解决方案。
...
TL;DR: 本文提出了一个完整的多阶段优化框架，将高性能浮点图像压缩模型转换为高效的硬件友好型整数实现，通过动态范围感知量化、渐进混合精度搜索和通道剪枝等技术，在FPGA上实现了既高效又高质量的图像压缩。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的图像压缩模型在RD性能上达到了最先进水平，但在资源受限的FPGA上部署这些模型仍然是一个重大挑战，需要解决量化引起的性能下降和硬件效率问题。

Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和新型权重正则化方案来抵消极端数据异常值和较大动态范围的影响；2. 渐进混合精度搜索算法：利用FPGA灵活性为每层分配最优的非均匀位宽；3. 针对GDN层的通道剪枝方法：消除非活跃通道以减少模型冗余。

Result: 基础DRAQ方法将基于GDN模型的BD-rate开销从30%降低到6.3%；后续硬件感知优化进一步将计算复杂度降低超过20%，对RD性能影响可忽略，最终模型在效率上达到最先进水平，在质量上优于现有的FPGA图像压缩实现。

Conclusion: 该多阶段优化框架成功地将高性能浮点图像压缩模型转换为高效的硬件友好型整数实现，在FPGA上实现了既高效又高质量的图像压缩部署。
...
### [54] [One-Step Diffusion Transformer for Controllable Real-World Image Super-Resolution](https://arxiv.org/abs/2511.17138)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散基真实世界图像超分辨率方法在保真度和可控性之间存在平衡问题：多步方法存在生成多样性和随机性导致保真度低，而单步方法由于保真度特定微调而失去控制灵活性。
...
### [55] [Learning to Look Closer: A New Instance-Wise Loss for Small Cerebral Lesion Segmentation](https://arxiv.org/abs/2511.17146)
*Luc Bouteille,Alexander Jaus,Jens Kleesiek,Rainer Stiefelhagen,Lukas Heine*

Main category: cs.CV

TL;DR: 提出CC-DiceCE损失函数来解决医学图像分割中传统Dice损失对小病灶分割不足的问题，通过实例级评估提高小病灶检测率。


<details>
  <summary>Details</summary>
Motivation: 传统Dice损失函数在医学图像分割中往往对小病灶分割不足，因为小病灶的相对体积对整体损失贡献很小，需要实例级损失函数来改善小病灶检测性能。

Method: 基于CC-Metrics框架开发CC-DiceCE损失函数，与现有的blob损失进行比较，在nnU-Net框架中进行基准测试，提供标准化实验设置。

Result: CC-DiceCE损失显著提高了病灶检测率（召回率），同时分割性能仅有轻微下降，但会产生稍多的假阳性；在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效提高小病灶检测能力，在保持良好分割性能的同时优于现有实例级损失方法，是医学图像小病灶分割的有效解决方案。
...
Method: 提出了FireScope-Bench数据集和基准，结合Sentinel-2影像和气候数据与专家定义的风险栅格。FireScope框架基于VLM，通过强化学习和视觉监督学习预测风险栅格并生成补充推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升。专家反馈和自动分析确认其推理轨迹是忠实且语义有意义的。

Conclusion: 推理可以为基础栅格预测模型提供支撑，提高泛化能力和可解释性。这是首个证明基于语言的推理可以改善视觉生成泛化能力的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
TL;DR: 本文系统评估了自监督表示在音频-视觉深度伪造检测中的潜力，发现这些特征能捕获相关且互补的信息，但无法可靠跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表示在音频-视觉深度伪造检测中的潜力，现有工作要么孤立使用这些特征，要么将其嵌入复杂架构中。
...
Conclusion: 自监督表示在深度伪造检测中具有潜力，能学习有意义的模式，但实现稳健的跨域性能仍具挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。作者希望通过结合判别式方法提供更好的初始指导来提升扩散模型的轨迹预测性能。

Method: 提出两阶段框架：第一阶段使用基于Transformer的提案解码器从传感器输入回归生成粗略轨迹预测；第二阶段应用扩散精炼器迭代去噪和精炼这些初始预测，并设计了细粒度去噪解码器来增强场景合规性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中都创造了新记录。

Conclusion: DiffRefiner通过结合判别式轨迹提案和生成式精炼，有效提升了扩散模型在轨迹预测中的性能，消融研究验证了各组件有效性。
...
### [59] [UI-Styler: Ultrasound Image Style Transfer with Class-Aware Prompts for Cross-Device Diagnosis Using a Frozen Black-Box Inference Network](https://arxiv.org/abs/2511.17155)
*Nhat-Tuong Do-Tran,Ngoc-Hoang-Lam Le,Ching-Chun Huang*

Main category: cs.CV

TL;DR: UI-Styler是一个针对超声图像的类别感知风格迁移框架，通过模式匹配机制将目标图像的纹理模式转移到源图像上，同时保留源图像的结构内容，并使用类别感知提示策略确保语义对齐。
...
Method: 提出UI-Styler框架：1）使用模式匹配机制将目标图像的纹理模式转移到源图像；2）引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的大量实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上始终优于现有无配对图像翻译方法，达到最先进的性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备域适应问题，在保持结构内容的同时实现准确的语义对齐，显著提升了诊断准确性。
...
### [60] [Non-Parametric Probabilistic Robustness: A Conservative Metric with Optimized Perturbation Distributions](https://arxiv.org/abs/2511.17380)
...
Method: 采用非参数统计建模范式，基于高斯混合模型(GMM)和多层感知机(MLP)头以及双三次上采样开发NPPR估计器，覆盖输入相关和输入无关的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16验证，NPPR比现有方法提供最多40%更保守的概率鲁棒性估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够更好地处理分布不确定性，为深度学习模型的鲁棒性评估提供了更可靠的框架。
...
### [61] [PostCam: Camera-Controllable Novel-View Video Generation with Query-Shared Cross-Attention](https://arxiv.org/abs/2511.17185)
*Yipeng Chen,Zhichao Ye,Zhenzhou Fang,Xinyu Chen,Xiaoyu Zhang,Jialing Liu,Nan Wang,Haomin Liu,Guofeng Zhang*

Main category: cs.CV

TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹。通过查询共享的交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现精确的相机运动控制和高质量视频生成。
...
Method: 引入查询共享的交叉注意力模块，将6-DoF相机位姿和2D渲染视频帧两种控制信号融合到共享特征空间；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息细化运动精度和视觉保真度。
...
### [62] [VLA-4D: Embedding 4D Awareness into Vision-Language-Action Models for SpatioTemporally Coherent Robotic Manipulation](https://arxiv.org/abs/2511.17199)
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在时空一致的操作任务中面临挑战，特别是在实现动作执行的时间一致性方面存在困难。

Method: 提出4D感知的视觉表示，将1D时间嵌入3D位置形成4D嵌入，并通过交叉注意力机制融合；扩展传统空间动作表示加入时间信息，实现时空规划。

Result: 在多种机器人操作任务中验证了方法的优越性，实现了空间平滑和时间一致的操作。

Conclusion: VLA-4D通过统一的4D感知框架，有效解决了机器人操作中的时空一致性问题，为通用机器人任务提供了新的解决方案。
...
### [63] [Continual Alignment for SAM: Rethinking Foundation Models for Medical Image Segmentation in Continual Learning](https://arxiv.org/abs/2511.17201)
*Jiayi Wang,Wei Dai,Haoyu Wang,Sihan Yang,Haixia Bi,Jian Sun*

Main category: cs.CV

TL;DR: 本文提出CA-SAM方法，通过轻量化的Alignment Layer模块来平衡SAM模型的计算效率和性能，并在持续学习场景下自动适配对齐层以缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的隐私政策差异使得联合训练不可行，需要持续学习而不产生灾难性遗忘。SAM模型虽然提供强大的零样本先验，但参数量大、计算开销高，限制了实际部署。

Method: 提出Alignment Layer轻量模块来对齐编码器-解码器特征分布，高效适配SAM到特定医学图像；在此基础上构建CA-SAM持续学习策略，自动适配合适的对齐层。
...
### [64] [SING3R-SLAM: Submap-based Indoor Monocular Gaussian SLAM with 3D Reconstruction Priors](https://arxiv.org/abs/2511.17207)
*Kunyi Li,Michael Niemeyer,Sen Wang,Stefano Gasperini,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: SING3R-SLAM是一个基于高斯表示的密集RGB SLAM框架，通过结合局部一致3D重建与全局高斯表示，解决了SLAM中的漂移和冗余点云问题，实现了高效紧凑的3D建图。


<details>
  <summary>Details</summary>
Motivation: 现有密集3D重建方法虽然能准确捕捉局部几何，但在SLAM中集成时面临漂移和冗余点云问题，限制了效率和下游任务（如新视角合成）的应用。

Method: 首先通过轻量级跟踪和重建模块构建局部一致子图，然后逐步对齐和融合到全局高斯地图中，强制跨视角几何一致性，全局地图反馈纠正局部漂移并增强跟踪鲁棒性。
...
Method: 使用可扩展的3D Vision Transformer架构，结合自监督和视觉语言预训练策略，联合优化局部transformer进行高分辨率特征提取和全局transformer进行全扫描上下文建模，结合DINO自蒸馏和SigLIP视觉语言对齐。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于先前的CT基础模型，证明了无需私有数据即可获得高性能、可泛化的表示。

Conclusion: SPECTRE是一个可扩展、开放且完全基于transformer的3D医学影像基础模型，能够学习几何一致且临床有意义的特征。
...
TL;DR: 提出了一种针对鱼眼图像的失真感知BEV语义分割框架，直接处理多相机高分辨率鱼眼图像，通过几何反投影和逐像素深度分布估计，在BEV空间中生成连续的不确定性感知语义图。


<details>
  <summary>Details</summary>
Motivation: 解决鱼眼图像由于极端非线性失真、遮挡和深度模糊导致的BEV语义分割挑战，避免传统方法中失真校正或透视校正的需求。

Method: 使用几何反投影将每个图像像素通过高斯参数化提升到3D空间，预测空间均值和各向异性协方差来显式建模几何不确定性，然后通过可微分splatting将投影的3D高斯融合到BEV表示中。

Result: 在复杂停车和城市驾驶场景中表现出强大的分割性能，在严重鱼眼失真和多样化环境条件下，可行驶区域的IoU达到87.75%，车辆的IoU达到57.26%。

Conclusion: 该框架能够有效处理鱼眼图像的几何失真，在BEV空间中生成高质量的语义分割结果，无需进行图像失真校正，在自动驾驶应用中具有重要价值。
...
### [67] [QueryOcc: Query-based Self-Supervision for 3D Semantic Occupancy](https://arxiv.org/abs/2511.17221)
*Adam Lilja,Ji Lan,Junsheng Fu,Lars Hammarstrand*

Main category: cs.CV

TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接从图像学习连续的3D语义占据，在自动驾驶场景中实现了高效的几何和语义理解。


<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本高昂，现有方法要么依赖2D渲染一致性导致3D结构隐式学习，要么使用离散体素网格限制空间精度和可扩展性，需要更直接有效的自监督3D学习方法。

Method: 使用独立的4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示在恒定内存下实现长距离推理。

Result: 在自监督Occ3D-nuScenes基准测试中，语义RayIoU比之前的相机方法提升26%，运行速度达到11.6 FPS。

Conclusion: 直接的4D查询监督能够实现强大的自监督占据学习，为自动驾驶等应用提供了高效的3D场景理解方案。
...
TL;DR: 提出了一种基于四元数保真项的颜色图像盲反卷积方法，通过四元数卷积核建模颜色通道间的相互依赖关系，有效提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度或单独处理每个颜色通道，忽略了颜色通道之间的关系，导致去模糊效果不佳。

Method: 设计了一个新颖的四元数保真项，使用四元数卷积核（包含四个核：一个捕获整体模糊的非负核和三个无约束的RGB通道核）来建模颜色通道间的未知相互依赖关系，并使用归一化四元数核保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法作为彩色图像反卷积的强大工具具有潜力，能够有效处理颜色通道间的相互依赖关系。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段训练框架，通过预训练、多粒度微调和强化学习优化，专门解决立体化学信息识别难题，在立体分子识别方面达到最先进性能。
...
Method: 采用三阶段训练范式：1）在大规模但噪声数据集上进行预训练；2）使用具有更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务；3）使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 实验结果表明，即使MolSight参数规模相对紧凑，GRPO算法仍能进一步提升模型在立体分子识别方面的性能，在多样化数据集上实现了最先进的(立体)化学光学结构识别性能。

Conclusion: MolSight框架通过创新的三阶段训练方法，成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学、药物发现和LLM应用提供了强大的自动化工具。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，包括之前无法估计的滚转角，并在预测性能、任务完成效率和用户操作准确性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的指尖姿态估计算法主要依赖电容图像，但这种方法只能估计俯仰角和偏航角，且在处理大角度输入时精度下降。需要一种能够同时准确预测全面指尖姿态信息的方法。

Method: 提出BiFingerPose算法，探索双模态输入，包括来自触摸屏的电容图像和通过屏下指纹传感器获取的指纹补丁。这种双模态方法能够可靠估计滚转角。

Result: 在12人用户研究中，BiFingerPose相比之前的最先进方法在预测性能上提升了21%以上，任务完成效率提高了2.5倍，用户操作准确性提高了23%。

Conclusion: BiFingerPose在增强认证安全性和改善交互体验方面具有实际优势，展示了指尖姿态在交互应用中的潜力。
...
TL;DR: 本文提出SpatialGeo，一种基于几何和语义特征层次融合的新型视觉编码器，通过增强多模态大语言模型的空间感知能力来解决现有模型在三维空间推理方面的局限性。
...
Method: 提出基于几何和语义特征层次融合的视觉编码器，通过层次适配器将自监督学习的几何特征补充到CLIP中，使用预训练的LLaVA模型进行高效训练，并通过随机特征丢弃优化以避免仅依赖CLIP编码器的平凡解。

Result: 实验结果表明，SpatialGeo在空间推理任务中提高了准确性，在SpatialRGPT-Bench上将最先进模型的准确率提高了至少8.0%，同时推理期间内存成本减少了约50%。

Conclusion: SpatialGeo通过增强空间感知能力，有效提升了多模态大语言模型在空间推理任务中的性能，同时降低了计算成本。
...
TL;DR: NoPe-NeRF++是一种无需姿态先验的局部到全局优化算法，通过相对姿态初始化、局部联合优化和全局束调整，显著提升了相机姿态估计精度和NeRF渲染质量。
...
Method: 采用相对姿态初始化与显式特征匹配，然后进行局部联合优化增强姿态估计，最后通过包含特征轨迹的束调整进行全局优化。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视角合成方面优于现有最先进方法，在挑战性场景中表现稳健。

Conclusion: NoPe-NeRF++是首个无缝结合局部和全局线索的NeRF方法，验证了设计选择的正确性，显著提升了无姿态先验的NeRF训练效果。
...
TL;DR: 本文提出了一种生成具有准确折射效果的图像的方法，通过同步物体边界内外的像素，使用斯涅尔折射定律在生成过程的每个步骤中进行像素扭曲和合并，从而生成更符合光学物理约束的图像。


<details>
  <summary>Details</summary>
Motivation: 生成式图像模型在合成透明物体时表现不佳，特别是折射效果，因为折射像素光线经常与图像其他部分观察到的表面相交，对颜色产生约束。现有模型未能充分学习光学定律来准确渲染折射物体。

Method: 在生成轨迹的每个步骤中，使用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并来同步像素。对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图同步来恢复其外观。

Result: 该方法生成的图像在光学上更加合理，能够更好地遵守物理约束。

Conclusion: 提出的方法能够显著改善生成图像中折射效果的物理准确性，通过像素同步和光学定律的应用，生成了更符合真实光学行为的图像。
...
TL;DR: 提出一个统一框架用于多媒体绘画过程生成，通过语义驱动的风格控制机制将多种媒介嵌入扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和过程转换。
...
Conclusion: 提出的感知距离分布曲线能够定量建模创作序列，反映人类艺术创作进程，为绘画过程生成提供了有效的评估和建模方法。
...
TL;DR: DSeq-JEPA提出了一种判别性顺序联合嵌入预测架构，通过识别主要判别区域并按重要性顺序预测后续区域，将JEPA的潜在预测与GPT风格的顺序推理相结合，在多个视觉任务上优于I-JEPA。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，现有I-JEPA方法对所有区域进行统一独立处理，缺乏对预测顺序和位置的显式建模，无法像人类视觉那样从最具信息量的区域到次要区域进行选择性顺序注意。

Method: 首先基于transformer生成的显著性图识别主要判别区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的课程式语义递进，整合了JEPA风格的潜在预测和GPT风格的顺序推理。

Result: 在图像分类、细粒度视觉分类、检测分割和低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体持续关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA通过引入判别性顺序预测，有效结合了预测性和自回归自监督学习的优势，在多个视觉任务上取得了优于现有方法的性能。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占据估计方法，相比传统高斯方法减少了84%的基元数量，实现了75%的内存占用降低、124%的推理速度提升和5.9%的mIoU改进，首次实现了实时推理。
...
Result: 在Occ3D数据集上，相比先前高斯方法实现了75%内存占用减少、124%推理速度提升和5.9% mIoU改进，无需使用时间标签。基元数量减少了84%。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间中计算增强诱导的漂移向量来推断语义恢复方向，并基于这些潜在漂移的角度一致性来校正嵌入，显著提升了CLIP对抗扰动的鲁棒性。
...
Result: 在广泛的基准测试中，ATAC始终实现极高的鲁棒性，平均比之前的最先进方法高出近50%，同时只需要最小的计算开销。在非传统和极端设置下仍保持最先进的鲁棒性，甚至对自适应攻击也实现了非平凡的鲁棒性。

Conclusion: ATAC是在CLIP嵌入空间中测试时对抗防御新范式中的一种高效方法。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏体素虽然具有空间解耦和锐利边界的优势，但在优化过程中容易陷入局部最小值。SDF提供了平滑连续的几何场，但如何在独立参数化的稀疏体素间保持这种平滑性是一个挑战。

Method: 1) 使用视觉几何模型进行鲁棒的几何初始化；2) 通过空间平滑损失在父子体素组和兄弟体素组间强制执行一致关系；3) 集成SDF到稀疏体素栅格化框架中。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon通过促进体素间的连贯平滑结构，成功解决了稀疏体素优化中的局部最小值问题，在保持高重建精度的同时实现了快速收敛。
...
TL;DR: MorphSeek是一个用于医学图像可变形配准的细粒度表示级策略优化框架，通过将配准问题重新定义为潜在特征空间中的空间连续优化过程，解决了高维变形空间和体素级监督稀缺的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像可变形配准中高维变形空间和体素级监督稀缺的根本挑战，现有强化学习框架通常将空间投影到粗糙的低维表示，限制了捕捉空间变异变形的能力。

Method: 提出MorphSeek框架，在编码器顶部引入随机高斯策略头来建模潜在特征分布，通过无监督预热和弱监督微调相结合，使用组相对策略优化进行多轨迹采样以稳定训练。
...
TL;DR: 本文提出了一种名为MCMoE的多模态动作质量评估框架，通过混合专家机制解决推理阶段部分模态缺失的问题，实现了完整和不完整多模态学习的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 多模态动作质量评估在现实中经常面临部分模态不可用的问题，导致现有模型无法运行且性能急剧下降，需要解决模态缺失情况下的鲁棒性评估。

Method: 提出MCMoE框架，包含自适应门控模态生成器动态融合可用信息重构缺失模态，设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示，通过混合专家进一步精化和补充缺失模态。

Result: 在三个公共AQA基准测试中，MCMoE在完整和不完整多模态学习方面均取得了最先进的成果。

Conclusion: MCMoE框架通过单阶段训练统一了单模态和联合表示学习，有效解决了多模态动作质量评估中的模态缺失问题，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗蒸馏方法存在知识多样性有限、收敛慢、难以平衡鲁棒性和准确性的问题，需要更有效的多教师协作框架来提升视觉语言模型的对抗鲁棒性。
...
Conclusion: MMT-ARD框架能有效提升多模态大模型的对抗鲁棒性，具有较好的可扩展性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 传统世界模型只能基于事实观察进行预测，无法回答反事实查询（如"如果移除这个物体会发生什么？"），而这类能力对于评估AI在不同条件下的行为越来越重要。
...
Conclusion: CWMDT框架成功地将标准视频扩散模型转化为有效的反事实世界模型，通过结构化表示和大语言模型推理实现了对特定场景属性的针对性干预。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号的频率与多分辨率形状特征相关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似从粗到细的方式对雷达信号频率进行条件化，扩散到该潜在空间中。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体的形状在商业和航空航天应用中至关重要，但分析复杂。现有深度学习方法难以表示任意形状或处理有限视角收集的真实世界雷达信号，而光学3D重建方法将雷达信号简单视为相机视图时效果不佳。

Method: 提出Radar2Shape去噪扩散模型，采用两阶段方法：1）学习具有层次分辨率形状特征的规范化潜在空间；2）通过以从粗到细的方式对雷达信号频率进行条件化，扩散到该潜在空间中。
...
TL;DR: 提出一种基于计算机视觉的深度学习方法来估计脊柱年龄，使用超过18,000个MRI序列图像，通过脊柱年龄差(SAG)评估脊柱健康状况。


<details>
  <summary>Details</summary>
Motivation: 脊柱是复杂的结构，容易发生与年龄相关的退行性变化，这些变化可以通过MRI识别。需要开发自动评估脊柱健康的方法。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退变聚类，构建深度学习模型估计脊柱年龄，通过脊柱年龄差(SAG)评估临床效用。

Result: SAG与椎间盘突出、骨赘、椎管狭窄、骨折等脊柱退变条件相关，也与吸烟和体力劳动等生活方式因素相关。
...
TL;DR: 本文分析了多模态模型中缩小LLM规模对视觉能力的影响，发现LLM缩小会不成比例地损害视觉能力而非语言能力，并提出了Extract+Think方法来解决这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的扩展，实际应用需要更小、更高效的系统。本文旨在系统分析缩小LLM规模如何影响多模态能力，特别是视觉能力。

Method: 首先分析LLM缩小对视觉能力的影响，然后引入视觉提取调优方法，训练模型提取任务相关的视觉细节，最后应用逐步推理生成答案，形成Extract+Think方法。

Result: 研究发现LLM缩小会显著降低视觉性能，影响程度甚至超过推理能力。提出的Extract+Think方法在该领域实现了效率和性能的新标准。

Conclusion: LLM缩小对多模态模型的视觉能力产生不成比例的负面影响，而Extract+Think方法通过分离视觉提取和推理过程，有效解决了这一瓶颈问题。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍（迭代选择帧、放大信息区域、重新编码像素）来增强文本丰富视频的理解能力，在多个视频QA任务上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，导致幻觉和细粒度证据失败。受人类暂停、放大、重读关键区域启发，需要迭代视觉反刍机制。
...
Result: 标记拼接方法在参数效率上更优且性能更好；广泛评估显示该方法在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。
...
### [88] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [89] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*
...
Method: 使用自然语言解释器文件指定命令语义、内存更新和日志行为，通过心智模型解释器提取陈述性和程序性知识，检测矛盾并在必要时产生解决方案。

Result: 在三个LLM上的基准测试比较显示，所有模型都能执行Cognitive BASIC程序，整体表现强劲但不均匀。

Conclusion: Cognitive BASIC为LLM推理提供了一种结构化的、可解释的方法，能够有效支持知识提取、冲突检测和推理任务。
...
### [90] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出了一种利用响应模式统计分析来系统性修订AI基准测试的框架，通过检测统计异常值来识别无效问题，结合专家评审和LLM初步筛选提高效率。
...
Method: 基于AI评估中常用的核心假设——均值足以总结模型性能，构建单维潜在结构。通过分析各项统计指标的预期范围，检测超出预期范围的异常问题项，结合专家评审和LLM初步筛选。

Result: 在9个广泛使用的基准测试中，该方法指导专家评审识别问题问题的精度高达84%。LLM评审初步筛选进一步减少了人工工作量。

Conclusion: 该框架为系统性基准测试修订提供了高效且可扩展的解决方案，通过统计分析和自动化工具的结合显著提高了基准测试的可靠性。
...
### [91] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
*Ye Han,Lijun Zhang,Dejian Meng,Zhuang Zhang*

Main category: cs.AI

TL;DR: 提出混合差分奖励机制解决多车协同驾驶中传统奖励函数梯度消失问题，通过时间差分奖励和动作梯度奖励提高策略梯度信噪比，显著提升算法收敛速度和策略稳定性。
...
### [92] [MirrorMind: Empowering OmniScientist with the Expert Perspectives and Collective Knowledge of Human Scientists](https://arxiv.org/abs/2511.16997)
*Qingbin Zeng,Bingbing Fan,Zhiyu Chen,Sijian Ren,Zhilun Zhou,Xuhua Zhang,Yuanyi Zhen,Fengli Xu,Yong Li,Tie-Yan Liu*

Main category: cs.AI

TL;DR: 本文提出MirrorMind分层认知架构，通过整合个体认知轨迹和集体学科记忆，实现AI科学家在科学发现中的个性化、结构化推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有AI科学家方法将科学发现视为孤立的优化过程，忽视了知识生产的社会性和历史性本质。人类科学洞察来源于个体认知轨迹和集体学科记忆两个相互关联的来源。

Method: MirrorMind采用三层架构：个体层面构建研究者的认知模型（情景、语义和人格记忆）；领域层面将集体知识映射为结构化学科概念图；跨学科层面作为正交编排引擎。该架构将记忆存储与智能执行分离。

Result: 在四个综合任务评估中，包括作者级认知模拟、互补推理、跨学科协作促进和多智能体科学问题解决，MirrorMind展现出超越简单事实检索的结构化、个性化和洞察生成能力。

Conclusion: 通过整合个体认知深度和集体学科广度，MirrorMind实现了向结构化、个性化和洞察生成型科学推理的转变，超越了现有方法的局限性。
...
### [93] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 论文重新解释了扩散模型在逆问题求解中的作用，提出了DAPS++方法，将扩散阶段与数据驱动细化完全解耦，实现了更高的计算效率和鲁棒的重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要受测量一致性项驱动，推理过程与扩散动力学有效解耦。需要澄清这种结构并改进方法。
...
Conclusion: 通过将扩散阶段与数据驱动细化完全解耦，DAPS++提供了对统一扩散轨迹在实践中保持有效的解释，并显著提高了逆问题求解的效率。
...
### [94] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了一种名为MIR的相互内在奖励方法，用于解决多智能体强化学习中稀疏奖励（特别是情节奖励）的挑战，通过激励智能体探索影响队友的行为来改善团队探索效果。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，情节奖励带来了显著挑战。现有内在奖励方法在单智能体场景中有效，但在MARL中存在问题，主要由于联合动作轨迹的指数稀疏性和现有方法未能考虑影响团队状态的联合动作。

Method: 提出了相互内在奖励（MIR）方法，这是一种简单而有效的增强策略，激励个体智能体探索影响队友的行为，当与原始策略结合时，能有效刺激团队探索并提高算法性能。

Result: 通过扩展代表性单智能体MiniGrid环境创建了MiniGrid-MA系列MARL环境进行评估，实验结果表明该方法在稀疏奖励设置下优于现有最先进方法。

Conclusion: MIR方法能有效解决MARL中稀疏奖励问题，通过促进智能体间的相互影响探索来提升团队性能，在MiniGrid-MA环境中表现出优越性能。
...
### [95] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一种基于层次任务抽象机制(HTAM)的多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务，显著优于现有的单智能体和多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有的通用智能体框架(如ReAct或角色扮演)在需要严格结构化工作流程的专业领域(如遥感)中表现不佳，因为这些领域需要专用工具和多步骤程序。

Method: 引入层次任务抽象机制(HTAM)，将多智能体系统构建为逻辑层次结构，反映特定领域的任务依赖图，将复杂问题分解为顺序层级，每个层级的子智能体基于前一层级的输出进行操作。
...
### [96] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为构建智能AI系统的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，实现透明、协作和可问责的智能系统。


<details>
  <summary>Details</summary>
Motivation: 智能AI需要具备持续自主性、推理和交互能力，这需要明确的认知、合作和治理模型来补充其关于智能体的假设。

Method: 利用AAMAS社区的概念工具，包括BDI架构、通信协议、机制设计和制度建模，将自适应数据驱动方法与结构化推理协调模型对齐。

Result: 提出了一种连接形式理论和实践自主性的智能体视角，为构建不仅能力强、灵活，而且透明、协作和可问责的智能系统指明了路径。

Conclusion: AAMAS社区的概念工具为智能AI系统提供了必要的理论基础，通过结合数据驱动方法和结构化模型，可以实现更可靠和负责任的智能系统。
...
### [97] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成和离策略数据对LLM行为探测泛化能力的影响，发现响应生成策略显著影响探测性能，离策略数据到激励行为的泛化成功可预测同策略泛化，并指出欺骗和伪装探测可能在真实监控场景中泛化失败。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然示例稀少，研究者被迫依赖合成或离策略LLM响应来训练探测，需要系统评估这些数据对探测泛化能力的影响。

Method: 在八个不同的LLM行为上测试线性和注意力探测，比较不同响应生成策略对探测性能的影响，分析离策略数据到同策略数据的泛化能力。

Result: 发现响应生成策略显著影响探测性能，离策略数据到激励行为的泛化成功可预测同策略泛化，欺骗和伪装探测可能在真实监控中泛化失败，训练数据域偏移导致更大性能下降。

Conclusion: 在缺乏同策略数据时，使用同域离策略数据比不同域同策略数据产生更可靠的探测，强调需要能更好处理LLM监控中分布偏移的方法。
...
### [98] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
*Jiaxi Liu,Chengyuan Ma,Hang Zhou,Weizhe Tang,Shixiao Liang,Haoyang Ding,Xiaopeng Li,Bin Ran*

Main category: cs.AI

TL;DR: 提出SRA-CP框架，通过风险感知的选择性协作感知，在保持安全关键目标检测精度的同时，仅使用20%通信带宽。
...
Result: 相比通用协作感知方法，安全关键目标的平均精度损失小于1%，仅使用20%通信带宽。相比现有选择性协作感知方法，感知性能提升15%。
...
### [99] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测通常需要大规模数据集和大量计算资源，而数据集蒸馏提供了一种替代方案。但将数据集蒸馏扩展到时间序列预测面临两个挑战：时间偏差和样本多样性不足。

Method: 基于一阶压缩分解构建DDTime框架，使用频域对齐机制缓解自相关引起的时间偏差，设计基于信息瓶颈原理的样本间正则化来增强样本多样性。

Result: 在20个基准数据集和多种预测架构上的实验表明，DDTime始终优于现有蒸馏方法，相对准确度提升约30%，同时仅引入约2.49%的计算开销。

Conclusion: DDTime是一个高效的时间序列数据集蒸馏框架，通过频域对齐和多样性增强机制，在保持高精度的同时显著降低了计算需求。
...
### [100] [When Structure Doesn't Help: LLMs Do Not Read Text-Attributed Graphs as Effectively as We Expected](https://arxiv.org/abs/2511.16767)
*Haotian Xu,Yuning You,Tengfei Ma*

Main category: cs.LG

TL;DR: 本文研究发现，在文本属性图上，仅使用节点文本描述的LLM已经能取得很强的性能，大多数结构编码策略只带来边际收益甚至负面影响，表明在强大语言模型时代需要重新思考图结构表示方式。
...
Result: 发现仅使用节点文本描述的LLM已能取得强大性能，大多数结构编码策略只带来边际收益或负面影响，显式结构先验在强大语言模型下往往不必要甚至适得其反。

Conclusion: 这标志着与传统图学习范式的重大背离，需要重新思考在LLM时代如何表示和利用图结构，为新的语义驱动图学习方法打开了大门。
...
### [101] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
*Yating Ren,Yikun Ban,Huobin Tan*

Main category: cs.LG

TL;DR: 本文提出GCL-OT框架，通过最优传输解决文本属性图中的多粒度异质性挑战，包括完全异质性、部分异质性和潜在同质性，实现结构与文本的灵活双向对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法在文本属性图上的结构-文本对比学习通常依赖同质性假设和硬优化目标，限制了在异质性图上的应用。虽然可以通过结构调整或邻居聚合缓解异质性，但通常将文本嵌入视为静态目标，导致对齐效果不佳。

Method: 提出GCL-OT框架，采用最优传输方法：1）针对部分异质性，设计RealSoftMax相似度估计器强调关键邻居-词交互；2）针对完全异质性，引入基于提示的过滤器自适应排除无关噪声；3）结合OT引导的软监督发现潜在语义相似邻居。

Result: 理论分析表明GCL-OT能改进互信息边界和贝叶斯误差保证。在9个基准数据集上的广泛实验显示，GCL-OT持续优于最先进方法，验证了其有效性和鲁棒性。

Conclusion: GCL-OT通过最优传输方法有效解决了文本属性图中的多粒度异质性挑战，实现了结构与文本的灵活双向对齐，在异质性图上表现出优越性能。
...
### [102] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache是一个基于频域分析和异常KV感知的多模态KV缓存压缩框架，通过识别和保留关键异常KV对，在保持任务性能的同时显著减少KV缓存内存使用并加速推理。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在显著的推理开销，因为多模态KV缓存随视觉输入长度成比例增长。现有的压缩方法主要依赖注意力分数，与高效注意力内核不兼容且忽略了值向量对注意力输出的贡献。

Method: 从KV矩阵分布角度重新审视压缩问题：1）发现多模态KV矩阵的频域能量集中在低频，通过低通滤波器提取主能量；2）提出异常KV识别模块，在频域建模KV矩阵主成分并优先保留显著偏离的KV对；3）设计动态预算分配模块，自适应确定每层KV缓存大小以保留更多异常KV。

Result: 在多个MLLM和基准测试上的实验表明，FlashCache优于最先进的多模态KV压缩方法，在保持任务性能的同时实现高达1.69倍的解码加速和80%的KV内存使用降低。

Conclusion: FlashCache通过频域引导的异常KV感知压缩框架，有效解决了多模态KV缓存的内存和计算开销问题，为多模态大语言模型的高效推理提供了可行方案。
...
### [103] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
*Ehsan Ahmed Dhrubo,Mohammad Mahmudul Alam,Edward Raff,Tim Oates,James Holt*

Main category: cs.LG

TL;DR: 提出基于向量符号架构(VSA)的多示例学习(MIL)框架，通过高维向量和代数运算直接编码MIL的iff约束，确保模型严格遵循正包当且仅当至少包含一个正实例的逻辑要求。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习MIL方法大多违反MIL的iff约束，导致性能指标虚高和泛化能力差。需要一种能严格遵循MIL逻辑约束的模型结构。

Method: 使用VSA将实例和概念表示为近似正交的高维向量，通过代数运算在分类过程中强制执行iff约束。设计学习编码器将原始数据转换为VSA兼容向量，并开发VSA驱动的MaxNetwork分类器。
...
### [104] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: 本文研究联邦学习算法（FedAvg、FedProx、Scaffold）在非独立同分布数据下的性能，特别关注物联网攻击检测场景中的统计异构性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着物联网设备激增和数据量暴涨，传统机器学习面临资源受限和安全敏感环境的挑战。联邦学习虽能解决隐私和资源问题，但非独立同分布数据的统计异构性严重影响了其效果。现有研究缺乏对联邦学习方法在物联网攻击检测中处理统计异构性的全面比较。

Method: 使用CICIoT2023数据集对大规模物联网攻击进行分类，系统比较FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下的表现。

Result: 通过细致的分析和实验，揭示了这些联邦学习方法在处理统计异构性时的性能差异和特点。

Conclusion: 研究为物联网安全领域的联邦学习应用提供了有价值的见解，帮助研究者和从业者更好地理解和应对统计异构性挑战。
...
### [105] [Energy Scaling Laws for Diffusion Models: Quantifying Compute and Carbon Emissions in Image Generation](https://arxiv.org/abs/2511.17031)
*Aniketh Iyengar,Jiaqi Han,Boris Ruf,Vincent Grari,Marcin Detyniecki,Stefano Ermon*

Main category: cs.LG

TL;DR: 本文提出了基于计算复杂度的扩散模型GPU能耗预测方法，通过改进Kaplan缩放定律来预测不同模型配置和硬件设置下的能耗。
...
Method: 将扩散模型推理分解为文本编码、迭代去噪和解码组件，假设去噪操作因多步重复执行而主导能耗。基于计算复杂度(FLOPs)建立能耗缩放定律。

Result: 在四个先进扩散模型和三个GPU架构上的实验表明，该方法在单个架构内预测准确度高(R平方>0.9)，并具有强大的跨架构泛化能力，能可靠估计未见过的模型-硬件组合能耗。
...
### [106] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
*Joseph Kim,Saahith Potluri*

Main category: cs.LG

TL;DR: 本文提出MOCET指标来评估AI安全等级威胁，特别是针对ASL-3+模型在生物安全领域的风险，该指标具有可解释性和双重可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标如LAB-Bench、BioLP-bench和WMDP能够可靠评估模型提升和领域知识，但缺乏更好量化"现实世界风险"的指标，需要可扩展的开放式指标来跟上LLM的快速发展。

Method: 引入MOCET指标，这是一个可解释且双重可扩展的指标（可自动化和开放式），能够量化现实世界风险。

Result: MOCET指标能够解决现有评估指标的不足，提供更好的现实世界风险量化能力。

Conclusion: MOCET指标填补了现有AI安全评估的空白，为LLM安全案例提供了更有效的风险评估工具。
...
### [107] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
...
### [108] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: 本文系统比较了Transformer、Transformer++和Mamba三种架构在心力衰竭患者临床预测任务中的表现，发现Llama（Transformer++）在预测区分度、校准性和鲁棒性方面表现最佳，Mamba次之，且两者都能用更少的训练数据达到同等性能。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer和Mamba架构在临床预测任务中表现出色，但医学领域缺乏系统分析模型在不同设置下性能和效率的实证研究。本文旨在填补这一空白，为临床预测任务的模型开发提供系统设计指导。

Method: 在瑞典大型心力衰竭队列（N=42820）中，比较了三种架构类别（Transformer、Transformer++、Mamba）共六个序列模型，评估了三个一年期预测任务，并通过消融实验分析了输入序列修改、架构配置和时间预处理技术的影响。

Result: Llama（Transformer++）在所有任务中获得了最高的预测区分度、最佳校准性和鲁棒性，其次是Mamba。两种架构都展示了高效的表征学习能力，小配置模型超越了其他大型Transformer。在同等模型规模下，Llama和Mamba仅需75%的训练数据就能达到同等性能。

Conclusion: 本研究首次提供了针对输入标记化、模型配置和时间数据预处理的系统消融研究，为未来基于EHR的临床预测任务模型开发提供了起点建议。
...
### [109] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
...
<details>
  <summary>Details</summary>
Motivation: 序数分类在医疗影像和诊断等高风险应用中广泛使用，需要可靠的不确定性量化。现有的序数保形预测方法要么是启发式算法，要么要求基础模型预测单峰分布，限制了覆盖效率权衡的洞察力和模型无关性。

Method: 将序数保形预测建模为实例级最小长度覆盖问题，开发了滑动窗口算法，在K个标签候选中具有线性时间复杂度，并提出长度正则化变体以缩小预测集大小同时保持覆盖。
...
### [110] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
TL;DR: 本研究利用欧洲龙虾的生物声学特征（嗡嗡声/甲壳振动），通过深度学习和机器学习模型成功实现了对龙虾年龄（幼体/成体）和性别（雄性/雌性）的非侵入式分类，准确率超过97%。


<details>
  <summary>Details</summary>
Motivation: 监测水生生物（特别是难以观察的龙虾）存在挑战，了解龙虾栖息地、福利、繁殖、性别和年龄对于管理和保护至关重要。本研究旨在开发一种非侵入式的被动声学监测方法。
...
Conclusion: 这项研究展示了监督机器学习和深度学习从龙虾声音中提取年龄和性别相关特征的潜力，为龙虾保护、检测和管理提供了一种有前景的非侵入式被动声学监测方法。
...
### [111] [PepEVOLVE: Position-Aware Dynamic Peptide Optimization via Group-Relative Advantage](https://arxiv.org/abs/2511.16912)
*Trieu Nguyen,Hao-Wei Pang,Shasha Feng*

Main category: cs.LG

TL;DR: PepEVOLVE是一个用于大环肽优化的动态框架，它能够自动学习编辑位置并动态优化肽序列，在Rev结合大环肽基准测试中优于现有方法PepINVENT。
...
Method: PepEVOLVE采用动态掩码和CHUCKLES移位增强预训练，使用上下文无关多臂老虎机路由器发现高奖励残基，结合进化优化算法和组相对优势来稳定强化学习更新。

Result: 在Rev结合大环肽基准测试中，PepEVOLVE达到更高的平均分数（约0.8 vs 0.6），最佳候选分数达0.95（vs 0.87），并在优化渗透性和亲脂性时收敛更快。

Conclusion: PepEVOLVE为肽先导优化提供了一条实用、可重复的路径，能够在最优编辑位置未知时实现更高效的探索和跨多目标的设计质量提升。
...
### [112] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: SCR-MF是一个用于单细胞RNA测序数据缺失值填补的两阶段工作流，结合了scRecover的缺失检测和missForest的非参数填补方法，在保持生物学保真度的同时提供稳健性能。


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA测序技术存在普遍的缺失事件，这些缺失会掩盖生物信号，因此需要开发能够准确填补缺失值的方法来恢复真实的转录组信息。

Method: 采用模块化两阶段工作流：第一阶段使用scRecover进行原理性缺失检测，第二阶段使用missForest进行稳健的非参数填补。

Result: 在公共和模拟数据集上的测试表明，SCR-MF在大多数情况下达到或超过现有填补方法的性能，同时保持生物学保真度和透明度。运行时间分析显示该方法在准确性和计算效率之间达到良好平衡。

Conclusion: SCR-MF为中等规模单细胞数据集提供了一个在准确性和计算效率之间达到竞争性平衡的缺失值填补解决方案。
...
### [113] [CroTad: A Contrastive Reinforcement Learning Framework for Online Trajectory Anomaly Detection](https://arxiv.org/abs/2511.16929)
*Rui Xue,Dan He,Fengmei Jin,Chen Zhang,Xiaofang Zhou*

Main category: cs.LG

TL;DR: CroTad是一个基于对比强化学习的在线轨迹异常检测框架，无需阈值设定，能够处理噪声和不规则采样的轨迹数据，在子轨迹和点级别实现细粒度异常检测。


<details>
  <summary>Details</summary>
Motivation: 解决智能交通系统中轨迹异常检测的关键挑战：子轨迹异常检测研究不足、现有方法依赖阈值调优、不规则采样和训练集噪声影响模型性能。

Method: 提出对比强化学习框架，结合对比学习提取不同行程的正常旅行模式，利用深度强化学习进行在线实时异常评分。
...
### [114] [A novel approach to classification of ECG arrhythmia types with latent ODEs](https://arxiv.org/abs/2511.16933)
*Angelina Yan,Matt L. Sampson,Peter Melchior*

Main category: cs.LG

TL;DR: 提出了一种端到端的心电图分类方法，使用潜在ODE建模连续ECG波形，通过降采样在不同频率下保持高性能，解决了可穿戴设备电池寿命与信号保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统12导联ECG只能短期监测，容易错过间歇性事件；可穿戴ECG虽然能长期监测，但受电池限制采样频率较低，形态分析困难。需要解决信号保真度与电池寿命的权衡问题。

Method: 训练潜在ODE模型来建模连续ECG波形，从高频单通道信号创建鲁棒特征向量。通过将初始360Hz ECG降采样到90Hz和45Hz，为每个波形构建三个潜在向量，然后使用梯度提升树进行分类。

Result: 在不同频率下性能下降极小，宏平均AUC-ROC值在360Hz、90Hz和45Hz下分别为0.984、0.978和0.976，表明该方法能有效绕过信号保真度与电池寿命的权衡。

Conclusion: 该方法使更小的可穿戴设备成为可能，促进了心脏健康的长期监测，为可穿戴ECG设备的实际应用提供了可行解决方案。
...
### [115] [ToC: Tree-of-Claims Search with Multi-Agent Language Models](https://arxiv.org/abs/2511.16972)
...
<details>
  <summary>Details</summary>
Motivation: 手动起草专利权利要求劳动密集、成本高昂且不一致，而传统大语言模型缺乏结构化迭代推理能力，难以精确优化权利要求。

Method: ToC框架整合蒙特卡洛树搜索与多智能体系统，包括基于LLM的编辑智能体提出上下文编辑建议，审查智能体通过结构化思维链分析新颖性和现有技术披露。

Result: 在1145个权利要求基准测试中，ToC在零样本和少样本场景下显著优于标准LLM，平均综合得分提升8%，某些情况下可达9%。

Conclusion: ToC建立了一种透明、可控且可解释的方法论，有效结合了先进LLM推理能力与战略MCTS规划，用于结构化专利权利要求优化。
...
### [116] [FIRM: Federated In-client Regularized Multi-objective Alignment for Large Language Models](https://arxiv.org/abs/2511.16992)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型与人类价值观对齐需要平衡多个冲突目标，但集中化训练存在计算密集和数据隐私问题，现有联邦多目标优化方法面临严重的通信瓶颈。

Method: 提出FIRM算法，每个客户端本地解决正则化多目标优化问题，通过客户端内正则化直接缓解客户端分歧漂移，仅需传输单一参数集。

Result: 证明算法收敛到帕累托稳定点，提供有限时间收敛保证，实验显示训练动态更平滑、客户端分歧漂移减少、奖励权衡改善。

Conclusion: FIRM能够高效实现联邦多目标对齐，支持目标偏好调整，在通信效率和性能权衡方面优于基线方法。
...
### [117] [Mask the Redundancy: Evolving Masking Representation Learning for Multivariate Time-Series Clustering](https://arxiv.org/abs/2511.17008)
*Zexi Tan,Xiaopeng Luo,Yunlin Liu,Yiqun Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种演化掩码的多变量时间序列聚类方法（EMTC），通过重要性感知的变量级掩码和多内生视图表示学习模块，解决了传统方法中冗余信息影响聚类性能的问题。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列包含大量冗余信息（如稳态机器运行记录、太阳能发电的零输出期），这些冗余降低了表示学习中对判别性时间戳的关注，导致聚类性能瓶颈。现有掩码策略多为独立预处理步骤，无法动态适应聚类关键时间戳的重要性变化。

Method: 提出EMTC方法，包含重要性感知变量级掩码（IVM）和多内生视图（MEV）表示学习模块。IVM自适应指导模型学习更具判别性的聚类表示，MEV通过重建和对比学习路径增强泛化能力。

Result: 在15个真实基准数据集上的实验表明，EMTC优于8种最先进方法，平均比最强基线提升了4.85%。

Conclusion: EMTC方法通过演化掩码机制有效解决了多变量时间序列聚类中的冗余问题，显著提升了聚类性能。
...
### [118] [Hash Collisions in Molecular Fingerprints: Effects on Property Prediction and Bayesian Optimization](https://arxiv.org/abs/2511.17078)
*Walter Virany,Austin Tripp*

Main category: cs.LG

TL;DR: 本文研究了在分子性质预测和贝叶斯优化中使用精确指纹与标准压缩指纹的对比，发现精确指纹在预测准确性上有小幅但一致的提升，但在贝叶斯优化中未能带来显著改进。


<details>
  <summary>Details</summary>
Motivation: 分子指纹方法使用哈希函数创建固定长度的分子向量表示，但哈希冲突导致不同子结构具有相同特征，从而在分子相似性计算中产生高估。本文旨在研究使用精确指纹是否能提高准确性。

Method: 在分子性质预测和贝叶斯优化中使用高斯过程作为基础预测模型，比较精确指纹与标准压缩指纹的性能。使用DOCKSTRING数据集中的五个分子性质预测基准进行测试。

Result: 使用精确指纹在五个分子性质预测基准上产生了小幅但一致的预测准确性提升。然而，这些增益并未转化为贝叶斯优化性能的显著改进。
...
### [119] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
*Duo Zhou,Yuji Zhang,Tianxin Wei,Ruizhong Qiu,Ke Yang,Xiao Lin,Cheng Qian,Jingrui He,Hanghang Tong,Heng Ji,Huan Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一种基于几何解缠的机器遗忘方法(GU)，通过将遗忘梯度更新分解为与保留梯度空间正交的分量，在保证遗忘效果的同时最小化对保留知识的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的机器遗忘方法在有效遗忘和保留知识之间存在权衡，缺乏对遗忘更新如何影响保留知识的理论分析，以及能否在理论保证下消除副作用。

Method: 从一阶分析出发，提出几何解缠遗忘(GU)方法，将遗忘梯度更新分解为保留梯度空间的切向和法向分量，只执行法向分量。在信任区域约束下，该投影方向在所有一阶保留不变移动中是最优的。

Result: GU方法在TOFU、MUSE和WMDP三个基准测试中，与现有梯度遗忘方法结合使用时取得了持续改进的效果。

Conclusion: 基于几何解缠的遗忘方法提供了一种理论上可靠且简单的解决方案，能够有效缓解遗忘过程中的副作用，实现更好的遗忘效果与保留知识平衡。
...
### [120] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
*Sonia Dupuis,Nando Metzger,Konrad Schindler,Frank Göttsche,Stefan Wunderle*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度各向异性扩散模型的超分辨率算法，将AVHRR GAC的陆地表面温度数据从粗分辨率降尺度到1公里，生成了42年的泛北极地区LST数据集。


<details>
  <summary>Details</summary>
Motivation: 北极地区快速变暖，但AVHRR GAC数据的粗空间分辨率限制了其在分析细尺度永久冻土动态和其他地表过程方面的应用，需要高分辨率的长期LST观测数据。

Method: 使用基于深度各向异性扩散模型的超分辨率算法，以MODIS LST数据为训练集，结合高分辨率土地覆盖、数字高程和植被高度图作为指导，将AVHRR GAC数据降尺度到1公里分辨率。

Result: 生成了覆盖整个泛北极地区42年、每天两次的1公里分辨率LST观测数据集，显著提高了数据空间分辨率。

Conclusion: 该增强数据集能够改进永久冻土建模、重建近地表气温、评估格陵兰冰盖表面质量平衡，并为前MODIS时代的气候监测提供支持，同时为未来卫星任务提供了可适应的框架。
...
### [121] [Reconstruction of Surface EMG Signal using IMU data for Upper Limb Actions](https://arxiv.org/abs/2511.17200)
*Shubhranil Basak,Mada Hemanth,Madhav Rao*

Main category: cs.LG

TL;DR: 本文研究了使用深度学习从6轴IMU数据合成归一化sEMG信号的方法，通过滑动窗口Wave-Net模型成功预测了肌肉激活的时间和基本形状。


<details>
  <summary>Details</summary>
Motivation: sEMG信号对肌肉功能分析很重要但容易受噪声干扰且采集困难，而IMU提供了更稳健的可穿戴替代方案，因此研究从IMU数据合成sEMG信号的可行性。

Method: 收集了手臂各种运动时同步采样的sEMG和IMU数据（1KHz），使用基于扩张因果卷积的滑动窗口Wave-Net模型将IMU数据映射到sEMG信号。

Result: 模型成功预测了肌肉激活的时间和基本形状，尽管峰值幅度经常被低估，但具有高时间保真度。

Conclusion: 该方法在肌肉意图检测应用中（如假肢和康复生物反馈）具有可行性。
...
### [122] [FlexiFlow: decomposable flow matching for generation of flexible molecular ensemble](https://arxiv.org/abs/2511.17249)
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D从头设计流匹配或扩散模型仅限于生成单一构象，但分子的构象景观决定了其可观测性质和与蛋白质靶点的结合能力。通过生成一组代表性的低能构象体，可以更直接评估这些性质并改进生成具有所需热力学可观测分子的能力。

Method: 提出FlexiFlow架构，扩展流匹配模型，允许联合采样分子及其多个构象，同时保持等变性和置换不变性。

Result: 在QM9和GEOM Drugs数据集上实现最先进的分子生成结果，能够生成有效、无应变、独特且新颖的分子，同时捕捉分子的构象多样性。模型生成的构象集合在推理时间大幅减少的情况下，提供了与最先进物理方法相似的覆盖范围。
...
### [123] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
...
<details>
  <summary>Details</summary>
Motivation: 高端汽车制造商面临产品种类多、变体级数据稀疏和市场波动大的复杂预测挑战，需要解决多层级需求预测问题。

Method: 结合点预测和概率预测，使用LightGBM集成模型、分位数回归和混合整数线性规划协调方法，利用池化训练集。

Result: 时空依赖性和取整偏差显著影响预测精度，整数预测对操作可行性很重要；短期需求受生命周期成熟度、自回归动量和操作信号影响，中期需求受在线参与、规划目标和竞争指标等预期驱动因素影响。

Conclusion: 在线行为数据显著提高了细分层次的预测精度，强调了整数预测的操作重要性以及不同时间尺度需求驱动因素的差异。
...
### [124] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
*Artem Frenk,Roee Shraga*

Main category: cs.LG

TL;DR: SAVeD是一个基于对比学习的框架，用于识别结构化数据集的版本，无需依赖元数据、标签或集成假设。它通过修改的SimCLR流程生成增强表视图，使用自定义transformer编码器进行嵌入，并在潜在空间中进行对比优化语义相似性。
...
Method: 采用对比学习方法，通过随机变换（如行删除、编码扰动）生成增强表视图，使用自定义transformer编码器进行嵌入，在潜在空间中优化语义相似性，最小化同一数据集增强视图间的距离，最大化无关表之间的距离。

Result: 在五个标准数据集上的实验显示，SAVeD在完全未见表上获得显著更高的准确率，分离分数显著提升，能够有效区分语义改变的版本。相比未训练的基线和现有最先进方法如Starmie，取得竞争性或更优的结果。

Conclusion: SAVeD框架能够有效识别结构化数据集的版本，无需依赖元数据，在版本检测任务中表现出色，为解决数据科学中的重复劳动问题提供了有效解决方案。
...
### [125] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
*Israt Jahan Tulin,Sebastian Starke,Dominic Windisch,André Bieberle,Peter Steinbach*

Main category: cs.LG

TL;DR: 本文研究了两种自监督深度学习方法用于超快电子束X射线CT的探测器数据去噪，相比非学习方法取得了更好的效果。
...
### [126] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: 本文提出了一种名为ReBaPL的贝叶斯提示学习方法，通过结合循环步长调度和SGHMC算法，以及基于表示空间概率度量的排斥力，有效探索提示的多模态后验分布，提升泛化能力。
...
Method: 提出ReBaPL方法：1）结合循环步长调度和SGHMC算法，交替进行探索和利用；2）引入基于表示空间概率度量（MMD和Wasserstein距离）的排斥力，防止过早收敛到单一模式。

Result: 在多个基准数据集上验证了ReBaPL的有效性，相比最先进的提示学习方法表现出更优越的性能。

Conclusion: ReBaPL提供了一种模块化的贝叶斯扩展方案，能够更全面地刻画提示后验分布，显著提升泛化能力，可作为现有最大似然估计提示学习方法的即插即用扩展。
...
### [127] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
*Massimiliano Manenti,Andrea Iannelli*

Main category: cs.LG

TL;DR: 本文提出了封建Q学习方案，研究了其收敛和稳定的条件，通过随机逼近理论和ODE方法证明了封建Q学习的收敛性和稳定性，并表明更新收敛到可解释为适当定义博弈的均衡点。


<details>
  <summary>Details</summary>
Motivation: 分层强化学习承诺高效捕捉和利用决策问题的时间结构并增强持续学习能力，但理论保证落后于实践。

Method: 提出封建Q学习方案，利用随机逼近理论和ODE方法分析其收敛性和稳定性。

Result: 证明了封建Q学习的收敛性和稳定性，更新收敛到可解释为博弈均衡的点，实验支持理论预期结果。

Conclusion: 为封建强化学习提供了原则性的收敛和稳定性分析，为分层强化学习的博弈论方法打开了大门。
...
### [128] [R2PS: Worst-Case Robust Real-Time Pursuit Strategies under Partial Observability](https://arxiv.org/abs/2511.17367)
...
<details>
  <summary>Details</summary>
Motivation: 现有的追逃博弈方法在部分可观测性和逃避者预测能力方面存在局限，无法满足实际安全应用中的实时需求。

Method: 首先证明传统动态规划算法在异步移动下保持最优性，然后提出信念保持机制扩展DP策略到部分可观测设置，最后将信念保持嵌入EPG框架进行跨图强化学习。

Result: 强化学习后，策略实现了对未见真实世界图结构的鲁棒零样本泛化，并持续优于现有博弈强化学习方法直接训练的基准策略。

Conclusion: R2PS方法成功解决了部分可观测追逃博弈中的实时鲁棒策略问题，为实际安全应用提供了有效解决方案。
...
### [129] [Stable Coresets via Posterior Sampling: Aligning Induced and Full Loss Landscapes](https://arxiv.org/abs/2511.17399)
*Wei-Kai Chang,Rajiv Khanna*

Main category: cs.LG

TL;DR: 提出了一种基于后验采样的核心集选择新框架，通过连接后验采样与损失景观来提升核心集选择的鲁棒性，使用平滑损失函数增强稳定性，并在多种数据集上实现了更快的训练和更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求激增，需要有效的核心集选择技术来加速训练。基于梯度的方法虽然理论基础扎实，但面临SGD基线过强和代表性随时间退化的问题。

Method: 建立后验采样与损失景观的连接，提出基于模型权重后验采样的平滑损失函数，并进行收敛性分析。

Result: 在多种数据集上的实验表明，该方法比现有技术实现了更快的训练速度和更好的泛化性能。

Conclusion: 所提出的基于后验采样的核心集选择框架能够有效解决现有方法的局限性，在数据预算有限的情况下提供稳定且高效的核心集选择方案。
...
### [130] [DS-Span: Single-Phase Discriminative Subgraph Mining for Efficient Graph Embeddings](https://arxiv.org/abs/2511.17419)
*Yeamin Kaiser,Muhammed Tasnim Bin Anwar,Bholanath Das,Chowdhury Farhan Ahmed,Md. Tanvir Alam*

Main category: cs.LG

TL;DR: DS-Span是一个单阶段判别子图挖掘框架，通过统一模式增长、剪枝和监督驱动评分，在单次搜索空间遍历中生成紧凑且判别性强的子图特征，用于图嵌入和分类。
...
Method: 提出DS-Span框架，引入覆盖上限资格机制动态限制探索，以及信息增益引导的选择机制，在单次搜索空间遍历中统一进行模式增长、剪枝和监督驱动评分。

Result: 在多个基准测试上的实验表明，DS-Span比先前的多阶段方法生成更紧凑和判别性的子图特征，在显著减少运行时间的同时达到更高或相当的准确率。
...
### [131] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
*Etienne Meunier,Said Ouala,Hugo Frezat,Julien Le Sommer,Ronan Fablet*

Main category: cs.LG

TL;DR: 本文提出了VEROS海洋模型的可微分扩展，使其能够通过自动微分框架JAX进行梯度计算，并展示了在海洋状态修正和物理参数校准中的应用。


<details>
  <summary>Details</summary>
Motivation: 传统的海洋模型难以进行梯度计算，限制了基于梯度的优化方法在海洋建模中的应用。本文旨在通过可微分编程技术，实现海洋模型的端到端学习和参数调优。

Method: 对VEROS海洋模型进行关键修改，使其完全兼容JAX自动微分框架，并评估数值一致性。通过梯度优化方法实现初始海洋状态修正和未知物理参数校准。

Result: 成功实现了VEROS模型的可微分扩展，保持了数值一致性。应用案例显示该方法能够有效修正初始海洋状态并校准物理参数，验证了可微分编程在海洋建模中的可行性。

Conclusion: 可微分编程为海洋建模提供了新的技术途径，能够促进端到端学习和参数调优，为海洋科学研究开辟了新的可能性。
...
### [132] [Unmasking Airborne Threats: Guided-Transformers for Portable Aerosol Mass Spectrometry](https://arxiv.org/abs/2511.17446)
*Kyle M. Regan,Michael McLoughlin,Wayne A. Bryden,Gonzalo R. Arce*

Main category: cs.LG

TL;DR: 提出了MS-DGFormer框架，通过直接处理原始质谱数据，使用Transformer架构和字典编码器结合SVD去噪，实现单次光谱检测，解决MALDI-MS在环境监测中的实时分析限制。
...
Method: 开发了MS-DGFormer框架，采用Transformer架构捕捉时间序列光谱的长程依赖，引入基于SVD的字典编码器集成去噪光谱信息，直接从原始质谱数据中提取关键生物分子模式。

Result: 该方法能够从单次光谱中稳健地识别病原体，在气溶胶样品中实现优异的病原体识别性能，支持自主实时分析。
...
### [133] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
*Vinay Kanakeri,Shivam Bajaj,Ashwin Verma,Vijay Gupta,Aritra Mitra*

Main category: cs.LG

TL;DR: 提出一种结合聚类和学习的强化学习算法，用于多智能体线性二次调节器(LQR)控制，通过同时聚类和学习为每个集群输出个性化策略，在保证正确聚类的同时实现样本效率提升。


<details>
  <summary>Details</summary>
Motivation: 强化学习通常需要大量数据，为提高样本效率，可以利用来自'近似相似'过程的数据。但由于过程模型未知，识别相似过程具有挑战性。本文旨在解决多智能体LQR设置中如何识别相似过程并学习个性化策略的问题。

Method: 结合顺序消除和零阶策略优化的思想，提出新算法同时执行聚类和学习。算法将智能体的本地过程基于动态和任务相似性进行聚类，为每个集群输出个性化控制器。

Result: 在适当的集群分离概念下，证明方法能以高概率保证正确聚类。每个集群学习策略的次优性差距与集群大小成反比，且无额外偏差。相比现有协作学习方法，避免了包含不相似过程数据导致的次优性。

Conclusion: 这是首个揭示聚类如何在数据驱动控制中用于学习个性化策略的工作，既能享受协作带来的统计增益，又不会因包含不相似过程数据而遭受次优性。方法在分布式实现上具有吸引力，仅产生轻微的对数通信开销。
...
### [134] [δ-EMG: A Monotonic Graph Index for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.16921)
*Liming Xiang,Jing Feng,Ziqi Yin,Zijian Li,Daihao Xue,Hongchao Qin,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 本文提出了一种误差有界的近似最近邻搜索方法，通过引入δ-EMG图结构确保搜索结果具有可证明的近似保证，相比传统基于召回率的方法提供了更强的理论保证。


<details>
  <summary>Details</summary>
Motivation: 传统近似最近邻搜索方法仅关注召回率，无法保证错误结果的偏差范围。本文旨在开发一种误差有界的ANN方法，确保返回结果是真实值的(1/δ)-近似。

Method: 采用基于图的框架，提出δ-EMG（误差有界单调图），通过在构建图时施加δ-单调几何约束，确保贪心搜索收敛到(1/δ)-近似邻居。进一步设计了误差有界top-k ANN搜索算法，并在查询时自适应控制近似精度。
...
### [135] [RASTP: Representation-Aware Semantic Token Pruning for Generative Recommendation with Semantic Identifiers](https://arxiv.org/abs/2511.16943)
*Tianyu Zhan,Kairui Fu,Zheqi Lv,Shengyu Zhang*

Main category: cs.IR

TL;DR: RASTP是一种用于生成式推荐系统的语义令牌剪枝方法，通过结合语义显著性和注意力中心性来动态剪枝输入序列中信息量较少的令牌，从而减少计算复杂度和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统使用语义标识符(SIDs)表示物品，但多个SIDs会显著增加输入序列长度，导致计算复杂度和内存消耗增加。现有方法主要优化注意力计算和KV缓存，而RASTP直接剪枝输入序列中信息量较少的令牌。

Method: RASTP通过结合语义显著性（通过表示大小衡量）和注意力中心性（来自累积注意力权重）来评估令牌重要性，动态剪枝低信息量或不相关的语义令牌。

Result: 在三个真实世界的Amazon数据集上的实验表明，RASTP减少了26.7%的训练时间，同时保持或略微提高了推荐性能。

Conclusion: RASTP通过直接剪枝输入序列中的低信息量令牌，有效减少了生成式推荐系统的计算开销，同时保持了推荐质量。
...
### [136] [CLLMRec: LLM-powered Cognitive-Aware Concept Recommendation via Semantic Alignment and Prerequisite Knowledge Distillation](https://arxiv.org/abs/2511.17041)
*Xiangrui Xiong,Yichuan Lu,Zifei Pan,Chang Sun*

Main category: cs.IR

TL;DR: CLLMRec是一个基于大语言模型的MOOC概念推荐框架，通过语义对齐和先验知识蒸馏技术，在不依赖结构化知识图谱的情况下实现个性化概念推荐。


<details>
  <summary>Details</summary>
Motivation: 现有MOOC概念推荐方法严重依赖高质量结构化知识图谱，但在真实教育场景中这类图谱往往稀缺，限制了推荐系统的实际应用。

Method: 提出CLLMRec框架，包含两个核心技术支柱：语义对齐组件构建学习者和概念的统一表示空间；先验知识蒸馏采用师生架构，大教师LLM提取概念先决关系并蒸馏到学生排序器中。

Result: 在两个真实MOOC数据集上的广泛实验表明，CLLMRec在多个评估指标上显著优于现有基线方法。

Conclusion: 该框架能够在无需显式结构先验的情况下，生成真正认知感知和个性化的概念推荐，验证了其有效性。
...
### [137] [Parametric Retrieval-Augmented Generation using Latent Routing of LoRA Adapters](https://arxiv.org/abs/2511.17044)
*Zhan Su,Fengran Mo,Jian-yun Nie*

Main category: cs.IR

TL;DR: Poly-PRAG是一种新的参数化检索增强生成方法，通过潜在路由编码过程解决传统PRAG方法中一对一文档编码方案的数据稀缺和推理开销问题。


<details>
  <summary>Details</summary>
Motivation: 传统PRAG方法采用一对一文档编码方案，每个文档使用专用的LoRA适配器，导致数据稀缺和推理时的高计算开销。

Method: 提出多任务学习框架，将文档集编码视为多任务学习过程，使用路由函数和少量潜在LoRA适配器编码整个文档空间，在线推理时根据查询选择性激活专家子集。

Result: 在多个知识密集型NLP任务上的综合评估表明，该方法在四个不同数据集上达到了最先进的结果。

Conclusion: Poly-PRAG通过潜在路由编码有效解决了传统PRAG方法的局限性，实现了更好的性能和效率。
...
### [138] [Generative AI in Sociological Research: State of the Discipline](https://arxiv.org/abs/2511.16884)
*AJ Alvero,Dustin S. Stoltz,Oscar Stuhler,Marshall Taylor*

Main category: cs.CY

TL;DR: 对433名社会学期刊作者调查显示，约三分之一学者每周使用生成式AI，主要用于写作辅助，而非研究规划、数据收集或分析。计算与非计算研究者在使用和态度上差异不大，学者普遍担忧AI的社会和环境后果，对其输出信任度低，但对AI改进持乐观态度。


<details>
  <summary>Details</summary>
Motivation: 在生成式AI引起广泛关注但伴随环境成本、社会风险和质量问题的背景下，需要了解社会学领域学者如何实际使用该技术及其态度。

Method: 对50个社会学期刊近5年发表的433名作者进行问卷调查，了解使用频率、原因、关注度、信任度和乐观度。

Result: 约33%受访者每周使用GenAI，主要用于写作辅助；计算与非计算研究者差异不大；学者普遍担忧社会和环境后果；对AI输出信任度低；对AI改进持乐观但对其领域影响意见分歧。

Conclusion: 社会学学者谨慎使用GenAI，主要用于写作辅助，对其社会环境影响高度关注，信任度有限，但对技术改进持乐观态度。
...
### [139] [Toward Sustainable Generative AI: A Scoping Review of Carbon Footprint and Environmental Impacts Across Training and Inference Stages](https://arxiv.org/abs/2511.17179)
*Min-Kyu Kim,Tae-An Yoo,Ji-Bum Chung*

Main category: cs.CY

TL;DR: 本文通过范围综述分析了AI碳足迹评估方法，重点关注推理阶段的环境影响，揭示了当前碳核算实践的局限性，并提出了标准化测量协议、动态评估框架等未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 生成式AI快速发展带来巨大价值，但其高能耗和环境可持续性问题日益突出。现有研究主要关注训练阶段，而大规模服务运营中推理阶段的累积环境足迹研究相对不足。

Method: 采用范围综述方法，分析AI碳足迹评估的方法论和研究趋势，比较现有测量工具和方法的分类与标准化状况，考察训练和推理阶段的环境影响。

Result: 发现当前AI碳核算实践存在方法不一致、技术特定偏见、缺乏端到端系统视角等关键局限。识别出模型大小、提示复杂度、服务环境等多维因素对碳足迹的影响。

Conclusion: 提出建立标准化测量协议、设计动态评估框架、开发生命周期监测系统、推进多维度可持续性评估框架等未来研究和治理方向，为构建可持续AI生态系统提供基础。
...
### [140] [Cross-cultural value alignment frameworks for responsible AI governance: Evidence from China-West comparative analysis](https://arxiv.org/abs/2511.17256)
*Haijiang Liu,Jinguang Gu,Xun Wu,Daniel Hershcovich,Qiaoling Xiao*

Main category: cs.CY

TL;DR: 该研究开发了一个多层审计平台，用于评估中美大语言模型在跨文化价值对齐方面的表现，发现普遍存在价值系统不稳定、年轻群体代表性不足等问题，同时揭示了不同地区模型的发展轨迹差异。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在全球高风险决策中的影响力增强，确保其与不同文化价值观的对齐已成为关键治理挑战，需要系统评估跨文化价值对齐。

Method: 采用四种集成方法：伦理困境语料库评估时间稳定性、多样性增强框架量化文化保真度、首词概率对齐分析分布准确性、多阶段推理框架提供可解释决策。

Result: 分析20多个领先模型发现普遍挑战：价值系统基本不稳定、年轻人口系统性代表性不足、模型规模与对齐质量非线性关系，同时发现地区发展轨迹差异。中国模型强调多语言数据集成，西方模型更多架构实验但存在美国中心偏见。

Conclusion: 两种范式都未能实现稳健的跨文化泛化。Mistral架构在跨文化对齐方面显著优于LLaMA3，全参数微调在保持文化多样性方面优于人类反馈强化学习。
...
### [141] [AI Workers, Geopolitics, and Algorithmic Collective Action](https://arxiv.org/abs/2511.17331)
*Sydney Reis*

Main category: cs.CY

TL;DR: 本文从国际政治经济学视角分析AI治理，认为仅靠自上而下的治理不足以保证AI的负责任发展，应关注AI工作者作为变革个体的作用，通过参与式设计方法促进算法集体行动。


<details>
  <summary>Details</summary>
Motivation: 基于国际政治经济学理论，解释为何AI治理在国际和国家层面发展、应用和执行不均衡，强调需要关注自下而上的干预措施。

Method: 采用参与式设计方法，将AI工作者视为知识、相对权力和意向性的来源，促进更负责任和公正的AI发展，创造促进算法集体行动的条件。

Result: 提出将AI工作者定位为地缘政治行为体，强调他们在促进算法集体行动中的潜力，为AI治理提供新的视角和方法。

Conclusion: AI治理不能仅依赖自上而下的监管，需要重视AI工作者作为变革个体的作用，通过参与式设计促进自下而上的干预，实现更负责任和公正的AI发展。
