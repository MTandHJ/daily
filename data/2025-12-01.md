...
TL;DR: 本文提出了一种新的引导方法——鞍点自由引导(SFG)，它利用对数密度估计在鞍点区域的正曲率来引导基于分数的生成模型，无需额外训练或标记数据。


<details>
  <summary>Details</summary>
Motivation: 现有的引导方法如Classifier-Free Guidance (CFG)需要标记数据并训练额外的无条件模型，Auto-Guidance需要较小的模型版本。这些方法在缺乏标记数据或无法训练新模型时难以应用。作者发现对数密度估计在鞍点区域的正曲率可以提供有效的引导。

Method: SFG通过估计对数密度的最大正曲率来引导单个基于分数的模型。该方法与CFG计算成本相同，无需额外训练，可直接用于现成的扩散模型和流匹配模型。

Result: SFG在无条件ImageNet-512生成中实现了最先进的FID和FD-DINOv2指标。与Auto-Guidance结合时，在FD-DINOv2得分上达到通用最先进水平。在FLUX.1-dev和Stable Diffusion v3.5实验中，SFG相比CFG提高了输出图像的多样性，同时保持了优秀的提示遵循和图像保真度。

Conclusion: SFG是一种无需额外训练或标记数据的有效引导方法，能够显著提升基于分数生成模型的性能，在多样性和保真度之间取得良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 铰接3D物体在仿真和机器人领域很重要，但手动创建成本高且难以扩展。现有方法多为多阶段流程，需要复杂的后处理。

Method: 提出统一潜在表示，联合编码几何、纹理、部件分割和运动学参数；引入可逆关节到体素嵌入，将运动特征与体积几何空间对齐；将关节类型预测建模为开放集问题，无需固定关节语义。

Result: 在PartNet-Mobility基准测试中，UniArt在网格质量和铰接精度方面达到最先进水平。

Conclusion: UniArt能够从单张图像端到端合成铰接3D物体，统一表示和开放集关节预测使其能够泛化到新的关节类别和未见过的物体类型。
...
Conclusion: 该多模态原型框架能够有效整合全切片图像和不完整基因组学数据，为精准肿瘤学提供灵活且强大的解决方案，特别是在处理真实临床场景中的缺失数据时表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中，从少量无位姿且部分遮挡的视图重建3D物体是一个常见但具有挑战性的问题，传统多视角或修复方法在遮挡严重时往往产生不完整或几何不一致的重建结果。

Method: 提出AmodalGen3D生成框架，整合2D模态补全先验与多视角立体几何条件，包含View-Wise Cross Attention机制用于稀疏视图特征融合，以及Stereo-Conditioned Cross Attention模块用于未观测结构推断。
...
<details>
  <summary>Details</summary>
Motivation: 人类擅长构建全景心理模型，保持物体恒常性并推断可见区域之外的场景结构。相比之下，当前的人工视觉系统在处理持久、全景理解方面存在困难，通常以自我中心的方式逐帧处理场景。这种限制在Track Any Point任务中尤为明显，现有方法无法跟踪视野之外的2D点。

Method: 利用360度视频作为监督源，将其重采样为狭窄视野视角，同时使用2D流程通过跟踪整个全景中的点来计算地面真实方向。引入新的数据集和基准TAPVid360-10k，包含10k个带有地面真实方向点跟踪的透视视频。基线方法改编CoTracker v3来预测每个点的旋转以进行方向更新。

Result: 提出的基线方法在预测视野外点的3D方向方面优于现有的TAP和TAPVid 3D方法，展示了在非自我中心场景表示学习方面的有效性。

Conclusion: TAPVid-360任务和TAPVid360-10k数据集为开发能够像人类一样进行持久、全景理解的视觉系统提供了重要基础，无需动态4D地面真实场景模型进行训练。
...
TL;DR: WalkCLIP是一个多模态框架，通过整合卫星图像、街景图像和人口动态数据来预测城市步行性，超越了传统单源方法。


<details>
  <summary>Details</summary>
Motivation: 传统步行性评估方法成本高、难以扩展，现有单源方法（卫星图像、街景图像、人口指标）各自只能捕捉步行环境的一个维度，无法全面评估城市步行性。

Method: WalkCLIP整合三种互补视角：1) 从GPT-4o生成的图像描述中学习步行性感知的视觉-语言表示；2) 通过空间聚合模块融入邻域上下文；3) 与人口动态基础模型的表示进行特征融合。
...
TL;DR: 本文在包含4000张内窥镜图像的新型胃肠道医学影像数据集上进行了全面的模型比较分析，针对四种关键疾病类别，最佳模型VGG16和MobileNetV2达到96.5%的测试准确率，并采用Grad-CAM可视化增强临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜图像分析中的常见挑战，如光照变化、相机角度波动和成像伪影，同时推进胃肠道计算机辅助诊断的发展，强调多样化临床相关数据集和模型可解释性在医学AI研究中的重要性。

Method: 利用最先进的深度学习技术，在包含4000张内窥镜图像的数据集上对四种疾病类别（憩室病、肿瘤、腹膜炎、输尿管）进行分类，采用VGG16、MobileNetV2和Xception等模型进行比较分析，并集成Grad-CAM可视化实现可解释AI。

Result: VGG16和MobileNetV2模型均达到96.5%的测试准确率，Xception模型达到94.24%的准确率，建立了稳健的基准和基线，同时通过Grad-CAM可视化成功识别了对模型预测最具影响力的图像区域。

Conclusion: 该研究展示了即使在复杂的现实条件下，也能实现稳健、准确且可解释的医学图像分析，为胃肠道计算机辅助诊断领域贡献了原始基准、比较见解和视觉解释，强调了多样化临床相关数据集和模型可解释性的重要性。
...
### [8] [SciPostGen: Bridging the Gap between Scientific Papers and Poster Layouts](https://arxiv.org/abs/2511.22490)
...
Method: 引入SciPostGen大规模数据集，分析论文结构与海报布局元素数量的关联，提出检索增强的海报布局生成框架，该框架检索与给定论文一致的布局作为生成指导，并在有/无布局约束两种条件下进行实验。

Result: 实验结果表明，检索器能够估计与论文结构一致的布局，该框架生成的布局既能满足给定的约束条件，又能保持与论文结构的对齐。

Conclusion: SciPostGen数据集填补了论文与海报布局对应关系研究的空白，提出的检索增强布局生成框架能够有效生成与论文结构一致且满足约束的海报布局，为科学传播提供了有效工具。
...
TL;DR: PAT3D是首个将视觉语言模型与物理模拟结合的物理增强文本到3D场景生成框架，能生成物理合理、无交叉、可用于模拟的3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D场景生成方法缺乏物理合理性，生成的场景中物体经常出现交叉和不稳定的物理状态，无法直接用于下游任务如机器人操作。

Method: 1) 使用视觉语言模型生成3D物体并推断空间关系，构建层次化场景树；2) 转换为模拟初始条件；3) 使用可微分刚体模拟器确保重力下的真实物体交互；4) 引入模拟循环优化过程保证物理稳定性和无交叉。

Result: PAT3D在物理合理性、语义一致性和视觉质量方面显著优于现有方法，能生成可用于下游任务（如场景编辑和机器人操作）的模拟就绪3D场景。

Conclusion: PAT3D通过集成物理模拟与视觉语言模型，首次实现了物理合理、无交叉的文本到3D场景生成，为下游应用提供了可直接使用的模拟就绪场景。
...
TL;DR: 本文提出了一个用于指针仪表读数识别的大规模基准数据集RPM-10K，并基于此开发了基于物理关系注入的视觉语言模型MRLM，通过显式编码指针与刻度间的几何和因果关系来提升读数精度。


<details>
  <summary>Details</summary>
Motivation: 智能电力系统中指针仪表的精确读数识别至关重要，但现有方法由于反射、遮挡、动态视角、指针与刻度标记过于接近等挑战而表现脆弱。该领域目前缺乏大规模数据集来支持鲁棒算法的开发。

Method: 首先构建了包含10730张仪表图像的RPM-10K大规模基准数据集，反映了实际应用中的关键挑战。在此基础上，提出了基于物理关系注入的视觉语言模型MRLM，通过显式编码指针与刻度间的几何和因果关系，结合交叉注意力融合和自适应专家选择机制，使模型能够解释仪表配置并生成精确数值读数。

Result: 在新提出的基准数据集上进行了大量实验，充分验证了所提出框架的有效性。数据集和源代码将在GitHub上公开。

Conclusion: 本文通过构建大规模数据集和引入物理关系注入的视觉语言模型，解决了指针仪表读数识别中的关键挑战，为智能电力系统中的精确仪表读数识别提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在对话能力方面有进步，但缺乏提供实时、交互式逐步指导的能力，这是未来AI助手的关键功能。有效指导不仅需要提供指令，还需要检测执行成功、识别错误并向用户发出警报，所有这些都需要实时进行。

Method: 基于CaptainCook4D数据集构建了Qualcomm Interactive Cooking基准和数据集，包含用户执行任务时的错误。数据集具有密集标注、定时指令和反馈消息，特别是精确时间戳标记的错误警报。同时开发了LiveMamba，一种专为交互式教学指导设计的流式多模态LLM。

Result: 在Qualcomm Interactive Cooking基准上评估了最先进的多模态LLM，并展示了LiveMamba作为强基线的性能。该工作为开发和评估实时、情境化指导提供了首个专用基准。

Conclusion: 该研究填补了多模态LLM在实时交互式指导方面的空白，通过引入专用基准和数据集，以及开发LiveMamba模型，为未来AI助手的发展提供了重要基础。
...
TL;DR: 本文针对Rectified Flow和Flow Matching模型，提出了一套完整的加速流水线，通过新的速度场批处理、异构时间步向量化和动态TensorRT编译等方法，将512*512图像生成速度提升至611%，远超现有18%的加速效果。


<details>
  <summary>Details</summary>
Motivation: Rectified Flow和Flow Matching等新技术显著提升了生成模型的性能，但由于其理论、设计与现有扩散模型存在差异，现有加速方法无法直接应用。需要开发专门针对流模型的加速方案。

Method: 提出完整的加速流水线，包括：1）使用新速度场的批处理方法；2）异构时间步批处理的向量化；3）为新方法设计动态TensorRT编译。这些方法从理论、设计和推理策略三个层面全面加速基于流模型的生成过程。

Result: 实验证明，现有公开方法通常只能实现18%的加速，而本文新方法能将512*512图像生成速度提升至611%，远超当前非通用加速方法的效果。

Conclusion: 本文提出的加速流水线成功解决了Rectified Flow模型无法直接应用现有加速方法的问题，通过理论、设计和推理策略的全面优化，实现了显著的性能提升，为流模型的实用化部署提供了有效的解决方案。
...
Method: 1. 结合离策略专家指导，将专家视觉搜索轨迹转化为结构化外部行为信号；2. 设计凝视引导推理导航器（GRN），通过双模式探索策略模拟诊断过程；3. 引入置信值采样器（CVS）平衡专家模仿和自主发现；4. 采用双流GRPO优化框架分离在线和离策略学习信号。

Result: 在多个医学VQA基准测试中，MedEyes实现了平均8.5%的性能提升，验证了其在构建可解释医学AI系统方面的潜力。

Conclusion: MedEyes通过动态建模临床医生风格的诊断推理，结合专家指导和自主探索，有效提升了医学视觉问答的准确性和临床一致性，为构建可解释的医学AI系统提供了有前景的框架。
...
TL;DR: 提出一种无需训练的后处理不确定性估计方法，用于检测对比式视觉语言模型的错误预测，通过测量类内视觉特征一致性来创建类别特定的概率嵌入。
...
Method: 使用特征投影结合多元高斯分布创建类别特定的概率嵌入，测量类内视觉特征一致性。该方法无需微调，与VLM无关，最少只需每类10张训练图像。

Result: 在ImageNet、Flowers102、Food101、EuroSAT和DTD数据集上实现了最先进的错误检测性能，显著优于确定性和概率性VLM基线方法。

Conclusion: 提出的训练免费后处理不确定性估计方法能有效检测VLM的错误预测，具有模型无关性、无需微调、对分布偏移鲁棒等优点。
...
TL;DR: 该研究探索了直接从音频指令进行物体定位的可能性，避免了传统基于文本转录的中间步骤，发现直接音频-视觉对齐在某些情况下甚至优于转录方法


<details>
  <summary>Details</summary>
Motivation: 传统基于文本转录的物体定位方法存在效率和鲁棒性问题，研究者质疑是否可以直接实现音频-视觉对齐而不依赖文本作为中间表示

Method: 简化任务为基于单词语音指令的定位，创建了包含多种物体和不同口音的新音频定位数据集，并适配和基准测试了多个音频-视觉领域的模型

Result: 结果表明直接从音频进行定位不仅可行，在某些情况下甚至优于基于转录的方法，特别是在处理语言变异性方面表现出更强的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 大多数最先进的无监督域自适应方法过于复杂，依赖于具有挑战性的对抗训练策略，或需要复杂的架构设计和辅助模型进行特征蒸馏和伪标签生成。本文旨在提出一种更简单实用的方法。

Method: 在频域中学习适应图像风格以减少源域和目标域之间的差异。该方法引入一个轻量级预处理模块，仅在训练时使用，推理时完全丢弃，不增加额外计算开销。应用于域自适应目标检测任务。

Result: 在多个基准测试上进行了广泛实验，结果表明该方法在域自适应目标检测任务中取得了显著的性能提升，证明了其实用性和有效性。

Conclusion: 本文提出的频域图像风格自适应方法是一种简单而有效的无监督域自适应解决方案，通过轻量级预处理模块实现域适应，在保持高效性的同时获得了显著的性能改进。
...
TL;DR: 提出一种基于轨迹条件的三维场景占用预测新架构，通过端到端方式直接从原始图像特征预测多帧未来占用，避免VAE离散化限制和BEV投影约束，在nuScenes基准上实现SOTA性能。
...
Method: 采用稀疏占用表示，绕过中间BEV投影和显式几何先验，直接从原始图像特征通过基于注意力的Transformer架构进行端到端预测，受GPT和VGGT等基础模型的启发。

Result: 在nuScenes基准的1-3秒占用预测任务中达到最先进性能，显著超越现有方法，并在任意未来轨迹条件下表现出稳健的场景动态理解能力。

Conclusion: 通过避免离散标记化的有限容量约束和BEV表示的结构限制，提出的端到端Transformer架构能更有效地捕捉时空依赖，实现更准确的未来场景占用预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实图像超分辨率方法通常使用文本条件扩散模型的生成先验进行正则化，但这存在两个关键问题：概念上与任务不匹配（超分辨率应生成与低质量图像直接相关的高质量图像），以及实践中教师模型常产生颜色失真和边缘模糊的图像，表明其生成先验对此任务存在缺陷。

Method: 提出图像条件流形正则化(ICM)方法，将输出正则化到基于稀疏但关键结构信息（色彩图和Canny边缘的组合）的条件流形上，避免了直接使用原始输入图像进行条件化时因信息密度过高导致的不稳定性。

Result: 实验证实所提出的正则化方法显著提升了超分辨率性能，特别是在感知质量方面，证明了其在真实世界应用中的有效性。

Conclusion: ICM方法提供了任务对齐且稳定的正则化信号，避免了密集条件化的不稳定性，提高了最终的超分辨率质量，为解决真实图像超分辨率任务中的流形正则化问题提供了有效方案。
...
TL;DR: OralGPT-Omni是首个牙科专用的多模态大语言模型，通过TRACE-CoT数据集和四阶段训练范式，在牙科影像分析和临床任务中表现出色，显著优于GPT-5。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在多个医学专业领域展现出巨大潜力，但牙科领域仍未被充分探索，主要原因是缺乏领域特定数据、牙科专家标注不足、模态特定建模不够以及可靠性挑战。
...
Conclusion: OralGPT-Omni是首个牙科专用的多模态大语言模型，通过临床推理监督和系统训练，显著提升了牙科影像分析和理解能力，推动了智能牙科的发展。
...
TL;DR: WorldWander是一个用于第一人称和第三人称视角视频转换的上下文学习框架，通过视角对齐和协作位置编码实现跨视角同步，并在新构建的数据集上取得了优异效果。
...
Method: 基于先进的视频扩散变换器，WorldWander集成了：(1) 上下文视角对齐；(2) 协作位置编码，以高效建模跨视角同步。同时构建了EgoExo-8K数据集，包含合成和真实场景中的同步第一人称-第三人称视频三元组。

Result: 实验表明，WorldWander在视角同步、角色一致性和泛化能力方面表现优异，为第一人称-第三人称视频转换设立了新的基准。

Conclusion: WorldWander成功解决了第一人称和第三人称视角之间的视频转换问题，通过创新的上下文学习框架和专门构建的数据集，在跨视角同步方面取得了突破性进展。
...
TL;DR: HyperST框架利用双曲空间建模空间转录组数据的层次结构，通过多级表示提取和分层双曲对齐，从组织学图像预测基因表达，在四个公共数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注点水平的图像-基因匹配，未能充分利用ST数据的完整层次结构，特别是在基因表达侧。同时存在信息不对称问题：基因表达包含更多分子细节，但缺乏明显的视觉对应物，需要复杂的表示学习来弥合模态差距。

Method: 提出HyperST框架：1) 多级表示提取器捕获每个模态的点水平和生态位水平表示；2) 分层双曲对齐模块在双曲空间中统一这些表示，执行空间对齐并分层构建图像和基因嵌入，丰富图像表示的分子语义。

Result: 在四个不同组织的公共数据集上实现了最先进的性能，证明了该方法在空间转录组预测方面的有效性和可扩展性。

Conclusion: HyperST通过建模数据的固有层次结构并在双曲空间中进行分层对齐，显著提高了跨模态预测能力，为更可扩展和准确的空间转录组预测铺平了道路。
...
TL;DR: 提出一种即插即用的耦合提示学习框架，通过引入外部监督提示来缓解视觉语言模型在视频任务微调时的语义空间收缩问题，提升模型在未见类别上的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前构建通用VLA模型的策略是先训练专业VLA模型获得可靠操作技能，然后通过混合标注数据恢复推理能力，但这会导致动作性能下降（动作退化现象）。

Method: 1) 双重数据剪枝方法去除冗余的具身推理数据；2) 双重教师自适应蒸馏策略，为不同数据域分配不同监督信号；3) 提出VLA Score评估框架，从推理、意图、动作和对齐四个维度解耦评估。

Result: DualVLA在SimplerEnv中达到61.0%的平均成功率，在八个竞争性多模态基准测试中平均得分65.4，在精确动作执行和多模态理解之间取得了更好的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的手语生成方法过于注重语义准确性而忽视了情感表达，导致生成的手语视频缺乏自然度和表现力，无法满足聋人社区对情感交流的需求。
...
Conclusion: EASL成功解决了现有手语生成方法缺乏情感表达的问题，通过情感-语义解耦和多情感引导机制，显著提升了手语视频的自然度和情感表现力，为聋人社区提供了更优质的交流工具。
...
TL;DR: 该研究开发了一个基于堆叠集成学习的深度学习模型，用于膝关节骨关节炎的自动检测和严重程度分级，相比传统人工X光片分析提高了准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎（KOA）的诊断通常依赖专家对X光片进行Kellgren-Lawrence分级，这种方法耗时、需要专业知识且存在主观性，容易导致诊断误差。需要开发自动化的准确诊断工具来解决这些问题。

Method: 提出了一种堆叠集成模型，使用MobileNetV2、YOLOv8和DenseNet201作为基础学习器，CatBoost作为元学习器。该模型执行两个分类任务：二元分类（检测KOA是否存在）和多类分类（精确的KL分级）。

Result: 模型在测试集上取得了73%的平衡准确率（多类分类）和87.5%的平衡准确率（二元分类），这些结果优于现有文献中的先前工作。

Conclusion: 该堆叠集成模型能够有效自动检测膝关节骨关节炎并进行严重程度分级，相比传统人工诊断方法具有更高的准确性和效率，为临床诊断提供了有价值的辅助工具。
...
TL;DR: RemedyGS是首个针对3D高斯泼溅计算成本攻击的有效黑盒防御框架，通过检测和净化机制保护3D重建系统


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅作为主流3D重建技术存在安全漏洞，易受计算成本攻击导致资源占用甚至拒绝服务，威胁系统可靠部署

Method: 提出包含检测器和净化器的双组件防御框架：检测器识别含毒化纹理的攻击图像，净化器从攻击图像恢复良性图像，并加入对抗训练增强防御效果

Result: 实验证明该框架能有效防御白盒、黑盒和自适应攻击，在安全性和实用性方面达到最先进性能

Conclusion: RemedyGS为3D高斯泼溅系统提供了首个全面有效的黑盒防御解决方案，保障了3D重建系统的安全可靠部署
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法依赖显式光流和局部扭曲，难以建模复杂全局运动并导致身份漂移，需要更鲁棒的运动渲染和身份保持方法

Method: 1. 用跨注意力机制替代传统流基扭曲，在统一潜在空间中隐式建模运动差异和身份对齐；2. 引入身份自适应模块，将运动潜在向量投影到个性化空间，确保运动与身份的清晰解耦；3. 轻量级流匹配运动生成器从音频、姿态和注视线索生成生动可控的隐式运动向量

Result: IMTalker在运动准确性、身份保持和音频-唇部同步方面超越先前方法，达到最先进质量，在RTX 4090 GPU上实现视频驱动40 FPS和音频驱动42 FPS的高效运行
...
TL;DR: 该研究针对东亚地区PM浓度长期预测的挑战，提出了GRPO框架，通过类别奖励和课程学习来优化预测可靠性，显著降低误报率。


<details>
  <summary>Details</summary>
Motivation: 东亚地区复杂地形和强大气动力学使得PM浓度长期预测困难，现有基础模型缺乏区域特异性且依赖非实时数据，无法满足公共卫生预警系统的实际需求。同时，标准点对点目标无法反映操作成本的不对称性（误报损害公众信任，漏报危及人群健康）。

Method: 1. 构建并发布了东亚地区的实时观测和高分辨率CMAQ-OBS数据集；2. 提出Group-Relative Policy Optimization (GRPO)框架，包含类别奖励和课程学习机制，使预测与操作优先级对齐。

Result: 1. 新数据集将区域误差降低了59.5%，实现了实时48-120小时预测；2. GRPO框架相比SFT基线将误报率降低了47.3%，同时保持了有竞争力的F1分数，显著提高了长期预测场景下的可靠性。

Conclusion: 该研究通过构建高质量区域数据集和创新的GRPO优化框架，有效解决了东亚地区PM浓度长期预测的实际挑战，为公共卫生预警系统提供了更可靠的解决方案。
...
TL;DR: PS-CBM通过多模态概念生成器、部分共享概念策略和概念效率准确度指标，解决了概念瓶颈模型中的视觉基础差、概念冗余和缺乏平衡指标的问题，在11个数据集上显著提升了准确率和概念效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM/VLM的自动概念生成方法存在三个核心问题：视觉基础差（概念与视觉特征关联弱）、概念冗余（生成过多相似概念）、缺乏平衡预测准确性和概念紧凑性的原则性指标。

Method: 提出PS-CBM框架，包含三个核心组件：1) 多模态概念生成器，融合LLM语义和基于示例的视觉线索；2) 部分共享概念策略，基于激活模式合并概念以平衡特异性和紧凑性；3) 概念效率准确度(CEA)，同时捕获预测准确性和概念紧凑性的后验指标。

Result: 在11个多样化数据集上的实验表明，PS-CBM持续优于最先进的CBM方法，分类准确率提升1.0%-7.4%，CEA提升2.0%-9.5%，同时所需概念数量显著减少。

Conclusion: PS-CBM通过解决现有CBM方法的三个核心挑战，在保持高准确性的同时实现了更强的可解释性，证明了其在平衡预测性能和概念紧凑性方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态AI在"图像思维"能力上存在困境：端到端强化学习不稳定，监督微调又过于僵化，导致模型要么难以学习，要么缺乏处理复杂现实场景所需的认知灵活性。

Method: GRiP采用两阶段训练框架，核心是认知增强的强化学习阶段，包含两个关键创新：1) 显著性加权IoU奖励，激励模型优先定位任务关键对象而非琐碎干扰物；2) 多启发式奖励，通过奖励多样但逻辑有效的推理路径来鼓励认知灵活性。

Result: 基于Qwen2.5-VL-7B模型初始化，GRiP在多个挑战性基准测试中表现显著提升，在极具挑战性的TreeBench和V* Bench上达到开源模型中的最先进水平。

Conclusion: 超越简单奖励，用认知启发的信号引导模型"看什么"和"如何思考"，对于解锁多模态智能的下一层次至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶轨迹规划方法通常依赖地图特征，本文旨在探索使用学习到的视觉表示替代地图特征，结合运动预测框架和规划意图输入，实现基于视觉的轨迹规划。

Method: 提出MTR-VP方法：使用ViT编码器处理原始图像和过去运动状态，生成上下文嵌入；采用交叉注意力机制将意图与上下文嵌入结合，替代MTR中的可学习意图查询；在Waymo端到端驾驶数据集上评估，进行消融研究分析不同组件效果。

Result: 实验表明：基于Transformer的方法在结合视觉特征和运动特征方面效果有限，即使使用CLIP和DINOv2等基础模型增强意图嵌入；但预测多个未来轨迹分布而非单一轨迹能显著提升规划性能。

Conclusion: 视觉特征与运动特征的结合在轨迹规划中仍具挑战性，但多轨迹预测方法能有效提升规划性能，为基于视觉的自动驾驶规划提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 足部接触在人与世界互动中至关重要，但现有方法通常使用零速度约束近似，关注关节级接触，无法捕捉足部与世界的详细交互。从单张RGB图像预测密集足部接触尚未充分探索，面临鞋款外观多样性和地面单调性的挑战。
...
Conclusion: FECO框架通过鞋款风格不变和地面感知学习，成功解决了密集足部接触估计中的主要挑战，为准确建模足部与世界的交互提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶仿真方法在大视角变化下难以支持新视角合成或确保几何一致性，需要更真实可控的仿真来推动端到端自动驾驶发展。

Method: 提出HybridWorldSim混合仿真框架，集成多轨迹神经重建用于静态背景，结合生成建模用于动态智能体，实现视觉和空间一致性。

Result: 实验表明HybridWorldSim超越了先前最先进方法，提供了实用可扩展的高保真仿真解决方案，并发布了MIRROR多轨迹数据集。

Conclusion: HybridWorldSim解决了先前方法的局限性，能够创建多样化和高保真的驾驶场景，为自动驾驶研究和开发提供了有价值的资源。
...
TL;DR: 提出两种创新方法改进3D对象生成：使用现成的图像适配器避免文本反转训练，以及深度条件预热策略增强3D一致性。


<details>
  <summary>Details</summary>
Motivation: 现有3D对象生成方法主要依赖文本到图像扩散模型配合文本反转技术，但这种方法需要额外训练时间且缺乏控制能力。

Method: 1) 使用现成的图像适配器直接生成3D对象，无需文本反转，提供对深度、姿态和文本等条件的更好控制；2) 深度条件预热策略增强3D一致性。

Result: 实验结果显示，在定性和定量评估中与现有基于文本反转的方法性能相当，且3D一致性得到改善。用户研究证实模型在匹配输入图像和保持3D一致性方面优于替代方案。

Conclusion: 提出的方法有效解决了文本反转技术的训练时间和控制能力限制问题，通过图像适配器和深度条件预热策略实现了更高效、可控且具有更好3D一致性的3D对象生成。
...
TL;DR: 提出一种无需训练的多视图一致性扩散框架，用于3D表示（如NeRF或高斯泼溅模型）的图像编辑，通过一致性损失确保不同视角的编辑结果在几何和光度上保持一致。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本图像编辑方法在处理多视图场景时，不同视角的编辑结果往往在几何和光度上不一致，这对于3D表示（如NeRF或高斯泼溅模型）的编辑尤为不利。
...
Conclusion: 该框架通过强制执行多视图一致性，解决了3D表示编辑中的不一致问题，为高质量3D场景编辑提供了有效的训练免费解决方案。
...
TL;DR: 该研究提出了M3LLM医疗多图像多模态大语言模型，通过利用生物医学文献中的复合图像作为训练数据，解决了医疗MLLMs在多图像理解方面的数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有医疗MLLMs大多局限于单图像理解，无法满足临床实践中需要综合多模态或多时间点图像信息的需求。开发具备多图像理解能力的医疗MLLMs面临大规模高质量标注数据缺乏的挑战。

Method: 提出基于生物医学文献中许可允许的复合图像作为数据源，设计了五阶段上下文感知指令生成范式，采用分治策略将多图像分析分解为可管理的子任务。解析了超过237,000个复合图像及其上下文文本进行指令生成。

Result: 构建了PMC-MI-Bench基准测试集，实验表明M3LLM在多图像、单图像、纯文本和多选场景中显著优于通用和专用医疗MLLMs。模型在MIMIC数据集上的纵向胸部X光分析中表现出强大的泛化能力。

Conclusion: 该工作建立了可扩展且高效的医疗MLLMs开发范式，能够进行复合推理，弥合了生物医学文献与现实世界临床应用之间的差距。
...
TL;DR: IE-SRGS提出了一种新的3D高斯泼溅超分辨率方法，通过结合外部2D超分辨率先验和内部3DGS特征，解决了现有方法中存在的3D高斯模糊问题，实现了高质量的重建。
...
Method: 提出IE-SRGS方法：1）使用2D超分辨率和深度估计模型生成高分辨率图像和深度图作为外部知识；2）使用多尺度3DGS模型生成跨视图一致、领域自适应的对应物作为内部知识；3）引入掩码引导融合策略整合这两个来源，协同利用它们的互补优势，有效指导3D高斯优化。

Result: 在合成和真实世界基准测试上的广泛实验表明，IE-SRGS在定量准确性和视觉保真度方面都一致优于最先进的方法。

Conclusion: IE-SRGS通过结合外部2D超分辨率先验和内部3DGS特征，有效解决了3D高斯泼溅超分辨率中的模糊问题，实现了高质量的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D显微镜图像分割即使使用最先进的模型仍存在错误，需要人工修正来准备高质量训练数据或修复错误，但传统人工修正效率低下
...
TL;DR: 该论文提出了一种基于SAM模型的图像篡改检测新方法，通过对抗性扰动使SAM"看不见"图像内容，从而在图像被篡改时能够识别伪造区域。


<details>
  <summary>Details</summary>
Motivation: AIGC图像编辑技术简化了逼真级图像修改，带来了严重的图像伪造风险。现有篡改检测方法面临挑战，需要更有效的检测技术。

Method: 提出一种新颖策略：将整个图像从神经模型视角转化为空白画布。通过引入对抗性扰动阻止SAM"看见任何东西"，当图像被篡改时，SAM能够识别伪造区域。针对SAM强大的感知能力，提出频率感知优化策略来彻底欺骗SAM。

Result: 大量实验结果表明该方法在图像篡改定位方面具有显著有效性，能够准确识别伪造区域。

Conclusion: 该方法为图像篡改检测提供了新的有效途径，利用对抗性扰动和频率感知优化策略，使SAM模型能够在图像被篡改时准确识别伪造区域。
...
TL;DR: TTSnap提出了一种测试时扩展框架，通过噪声感知剪枝在部分去噪阶段就淘汰低质量候选，避免完全去噪的计算开销，显著提高了搜索效率。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法需要完全去噪每个噪声种子才能计算奖励，计算成本高昂，限制了在固定预算下可探索的样本数量和多样性。

Method: 通过自蒸馏训练噪声感知奖励模型，使中间估计的奖励与最终干净图像的奖励对齐；采用课程训练策略稳定不同噪声水平的学习；引入奖励对齐和计算预算利用率的新度量。

Result: 实验表明，该方法比现有方法性能提升超过16%，实现了更高效有效的测试时扩展，并能与训练后技术和局部测试时优化获得正交增益。

Conclusion: TTSnap通过噪声感知剪枝显著提高了测试时扩展的效率，使在有限计算预算下探索更多样化的噪声种子成为可能，为文本到图像扩散模型的测试时优化提供了新思路。
...
TL;DR: 该论文提出了一种通过语义锚定指导文本到图像扩散模型个性化学习的方法，在保持预训练语义先验的同时，从少量参考图像中学习新概念，提高了主体保真度和文本图像对齐度。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在个性化方面存在挑战：从少量参考图像学习新概念时，要么过度拟合参考图像而无法利用预训练分布，要么强调先验保持而无法学习新的个性化属性。需要一种平衡方法来解决这一矛盾。

Method: 提出语义锚定方法，将个性化重新定义为通过语义锚定在频繁对应概念的指导下学习罕见概念的过程。该方法通过将新概念锚定在其对应分布中来指导模型适应，以稳定可控的方式扩展预训练分布。

Result: 与基线方法相比，该方法实现了稳定的适应，并在主体保真度和文本图像对齐度方面都取得了一致性改进。大量实验和消融研究进一步证明了所提锚定策略的鲁棒性和有效性。

Conclusion: 语义锚定方法能够有效指导扩散模型的个性化学习，在保持预训练语义结构的同时扩展分布到个性化区域，解决了现有方法在主体保真度和语义一致性之间的权衡问题。
...
TL;DR: UMind-VL是一个统一的超声医学基础模型，能够同时处理像素级结构理解（分割、定位）和高级临床推理（诊断），在1.2百万超声图像-文本对数据集上训练，性能优于现有通用多模态模型，达到或超越专业模型水平。


<details>
  <summary>Details</summary>
Motivation: 超声领域缺乏能够同时处理低层次超声感知（如分割、定位）和高层次超声综合解释（如诊断、推理）的全面解决方案。现有模型要么专注于像素级任务，要么只处理诊断推理，缺乏统一框架。

Method: 1. 构建UMind-DS大规模多模态数据集（120万超声图像-文本对，16个解剖区域），包含像素级标注和临床验证的推理依据；2. 设计UMind-VL架构，采用轻量级动态卷积掩码解码器，通过基于LLM输出的动态核生成掩码；3. 使用任务特定token统一分割、检测、几何测量和诊断任务。

Result: UMind-VL在分割、检测、关键点定位和诊断推理基准测试中显著优于现有通用多模态模型，达到或超越最先进的专业模型性能，同时保持强大的泛化能力。

Conclusion: UMind-VL成功统一了超声领域的像素级结构理解和高级临床推理，填补了超声医学基础模型的空白，为超声智能分析提供了全面解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管已有专门针对3DGS的水印方案用于版权保护，但现有方法是否能真正保证3D资产的鲁棒保护仍存疑问。作者发现传统的2D图像水印去除技术无法有效推广到3DGS场景，因为3DGS具有专门的渲染流程和每个高斯基元的独特属性。

Method: 提出了GSPure框架，这是首个专门针对3DGS水印表示的净化框架。通过分析视图相关渲染贡献并利用几何精确的特征聚类，GSPure能够精确隔离并有效去除与水印相关的高斯基元，同时保持场景完整性。

Result: 实验表明，GSPure实现了最佳的水印净化性能，将水印PSNR降低了高达16.34dB，同时对原始场景保真度的退化控制在小于1dB的PSNR损失。在有效性和泛化性方面都优于现有方法。

Conclusion: 该研究首次系统验证了3DGS水印框架的脆弱性，提出的GSPure框架为3DGS水印保护提供了重要的安全评估工具，同时也揭示了现有水印方案需要更强的鲁棒性设计。
...
TL;DR: DriveVGGT是一个专门为自动驾驶数据设计的尺度感知4D重建框架，通过引入时间视频注意力模块和多摄像头一致性注意力模块，解决了VGGT在自动驾驶场景中的局限性，显著提升了重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈重建方法（如VGGT）直接应用于自动驾驶系统时效果不佳，因为自动驾驶任务具有不同的先验条件：摄像头视野重叠最小、相机内外参数已知且固定、所有摄像头相对位置固定。需要专门设计来整合这些自动驾驶特有的先验信息。

Method: 提出DriveVGGT框架：1) 时间视频注意力模块独立处理多摄像头视频，利用单摄像头序列的时空连续性；2) 多摄像头一致性注意力模块通过归一化相对位姿嵌入进行窗口注意力，建立跨摄像头一致性关系；3) 扩展VGGT头部，增加绝对尺度头和自车姿态头。

Result: 实验表明DriveVGGT在自动驾驶数据集上优于VGGT、StreamVGGT和fastVGGT，广泛的消融研究验证了所提设计的有效性。

Conclusion: DriveVGGT成功将自动驾驶特有的先验信息整合到前馈重建框架中，通过专门设计的时间视频注意力和多摄像头一致性注意力模块，实现了对自动驾驶数据的尺度感知4D重建，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 受量子力学中波函数坍缩现象的启发，作者观察到图像中某些补丁的实现会降低其他补丁的不确定性。这种"补丁坍缩"现象为理解图像内部依赖关系提供了新视角，旨在提高视觉任务的效率。

Method: 1. 提出"补丁坍缩"概念，类比量子力学波函数坍缩；2. 学习自编码器，软选择补丁子集来重建目标补丁；3. 通过PageRank分析补丁依赖关系，确定最优补丁实现顺序；4. 将该顺序应用于掩码图像建模方法。

Result: 1. 在自回归图像生成中，通过重新训练MAR模型获得性能提升；2. 在图像分类中，仅向Vision Transformers暴露22%的高排名补丁即可达到高准确率；3. 验证了补丁坍缩顺序对各种掩码图像建模方法的益处。

Conclusion: 补丁坍缩为图像建模提供了新颖视角，能够显著提升视觉任务效率。通过识别和利用补丁间的依赖关系，可以在减少计算成本的同时保持性能，为高效视觉系统设计开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理单个图像或密集采样的视频，缺乏对非结构化图像集合（共享视觉元素但视角、时间、内容不同）进行一致可控生成的能力。

Method: 将任务建模为图结构，每个节点对应一张图像，每条边触发图像对的联合生成。通过融合跨图像对的内部特征（基于密集输入对应关系），在统一框架中整合所有成对生成，确保局部一致性和全局连贯性。

Result: 该方法在一致性和视觉质量方面达到最先进水平，无需掩码或人工监督，解锁了从图像集合进行内容创作的新能力。

Conclusion: Match-and-Fuse为零样本、无需训练的跨图像一致生成提供了有效框架，通过图建模和特征融合实现了对非结构化图像集合的高质量可控生成。
...
TL;DR: MVMAE是一种自监督学习框架，利用放射学研究的自然多视图结构学习视图不变和疾病相关表示，结合掩码图像重建和跨视图对齐，并通过文本增强版本MVMAE-V2T进一步提升性能。
...
Method: 提出Multiview Masked Autoencoder (MVMAE)自监督框架，结合掩码图像重建和跨视图对齐，利用放射学研究的多个视图结构。进一步扩展为MVMAE-V2T，加入放射学报告作为文本辅助学习信号，增强语义基础。

Result: 在MIMIC-CXR、CheXpert和PadChest三个大型公共数据集的下游疾病分类任务中，MVMAE持续优于监督学习和视觉语言基线方法。MVMAE-V2T提供额外性能提升，特别是在标签稀缺的情况下，文本监督的益处最为明显。

Conclusion: 结构和文本监督是构建可扩展、临床基础的医学基础模型的互补路径。利用临床数据的多视图结构和文本信息可以显著提升医学机器学习系统的性能。
...
TL;DR: 提出一种基于语言和分割语义引导的自动化视频着色方法，通过语言条件扩散模型和光流技术实现高质量、时间稳定的视频着色，无需人工颜色输入。


<details>
  <summary>Details</summary>
Motivation: 现有视频着色方法存在时间闪烁问题或需要大量人工输入，需要一种能够自动实现高质量、时间稳定视频着色的解决方案。

Method: 使用语言条件扩散模型为灰度帧着色，通过自动生成的对象掩码和文本提示提供语义引导；采用光流（RAFT）从先前帧扭曲颜色信息以确保时间稳定性，并包含纠正步骤检测和修复扭曲引入的不一致性。

Result: 在标准基准测试（DAVIS30, VIDEVO20）上实现了最先进的性能，在着色准确性（PSNR）和视觉真实感（Colorfulness, CDC）方面表现优异，证明了基于自动提示引导的视频着色方法的有效性。

Conclusion: 该方法通过语言和分割语义引导实现了自动化、高质量的视频着色，解决了时间闪烁问题，无需特定颜色输入，为视频着色提供了有效的自动化解决方案。
...
TL;DR: 研究发现多模态大语言模型在视觉问答评估中存在对提示格式的敏感性，即使语义中性的格式变化也会显著影响模型表现，现有偏差缓解策略无法解决这些问题。
...
<details>
  <summary>Details</summary>
Motivation: 标准归一化流（NFs）的生成质量受限于基于对数似然优化获得的语义表示质量较差。需要一种方法来改善NFs的语义表示能力，同时保持其可逆性优势。

Method: 提出创新的对齐策略：利用NFs的可逆性，将生成（反向）过程中的中间特征与强大的视觉基础模型表示对齐，而不是正则化前向过程。同时提出无需训练、测试时优化的分类算法来评估NFs的语义知识。

Result: 实验表明，该方法将NFs训练速度提升3.3倍以上，同时在生成质量和分类精度上都有显著提升。在ImageNet 64×64和256×256上建立了新的最先进结果。

Conclusion: 通过创新的对齐策略，成功解决了NFs语义表示不足的问题，显著提升了生成模型的性能，为NFs的实际应用提供了更有效的解决方案。
...
TL;DR: INSIGHT是一个用于检测和解释AI生成图像的统一多模态框架，即使在极低分辨率下也能保持鲁棒性，通过超分辨率、空间定位和语义对齐技术提供透明解释。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型生成的图像越来越逼真，对视觉媒体可靠性的担忧加剧。现有深度伪造检测系统在现实世界条件下（如下采样、压缩、跨域分布偏移）性能急剧下降，且大多数检测器作为不透明的分类器，无法解释为何将图像标记为合成图像，这削弱了信任并阻碍了在高风险环境中的采用。

Method: INSIGHT结合了：1）分层超分辨率放大细微的取证线索而不引入误导性伪影；2）Grad-CAM驱动的多尺度定位揭示指示生成模式的空间区域；3）CLIP引导的语义对齐将视觉异常映射到人类可解释的描述符。然后使用结构化的ReAct + Chain-of-Thought协议提示视觉语言模型，通过双阶段G-Eval + LLM-as-a-judge管道验证，以最小化幻觉并确保事实性。
...
Method: AnchorFlow基于潜在锚点一致性原则，建立源轨迹和目标轨迹共享的全局潜在锚点，使用松弛锚点对齐损失和锚点对齐更新规则来强制一致性。

Result: 在Eval3DEdit基准测试中，AnchorFlow在不同编辑类型上都能一致地提供语义对齐和结构鲁棒的编辑效果。
...
TL;DR: 该论文针对遥感视觉问答中存在的伪推理问题，提出了RS-EoT框架，通过语言驱动的迭代视觉证据搜索范式，结合SocraticAgent多智能体系统和渐进式强化学习策略，实现了真正的基于证据的推理。


<details>
  <summary>Details</summary>
Motivation: 现有遥感多模态推理模型存在广泛的伪推理问题，模型只是叙述推理过程而非基于视觉证据进行真正推理。作者将此归因于"一瞥效应"——对大规模遥感图像的粗略感知导致理解不完整，模型基于语言自洽性而非视觉证据进行推理。
...
Result: 实验表明RS-EoT在多个RS VQA和grounding基准测试中达到最先进性能。分析显示清晰的推理和证据搜索迭代循环，证实RS-EoT缓解了一瞥效应，实现了真正的基于证据的推理。

Conclusion: RS-EoT框架成功解决了遥感多模态推理中的伪推理问题，通过迭代视觉证据搜索范式实现了真正的证据驱动推理，为遥感视觉问答系统提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在复杂低空环境中准确感知无人机对空域安全和智能系统至关重要，但真实世界数据收集面临空域管制、隐私问题和环境变化等限制，手动标注3D姿态和跨模态对应关系耗时耗力，需要大规模、准确标注的多模态数据来开发可靠解决方案。

Method: 创建UAV-MM3D合成数据集，包含40万帧同步数据，涵盖城市、郊区、森林、沿海等多样场景和晴、多云、雨、雾等天气条件，包含微型、小型、中型无人机模型，提供RGB、红外、激光雷达、雷达和动态视觉传感器五种模态数据，每帧提供2D/3D边界框、6自由度姿态和实例级标注。

Result: 提出了UAV-MM3D数据集，并开发了LGFusionNet（激光雷达引导的多模态融合基线）和专门的无人机轨迹预测基线，为无人机3D感知提供了公开基准测试平台。

Conclusion: UAV-MM3D通过可控的仿真环境、全面的场景覆盖和丰富的标注，为推进无人机3D感知研究提供了有价值的公共基准数据集和基准方法。
...
Method: 基于DiffPortrait360架构，引入风格外观模块分离风格与内容，采用风格融合注意力机制在潜在空间中平衡结构保持与风格化保真度，使用3D GAN生成的多视角数据集进行微调，并通过温度基键缩放策略控制推理时的风格化强度

Result: 在FFHQ和RenderMe360数据集上的实验表明，DiffStyle360在风格质量上优于最先进的GAN和基于扩散的风格化方法，能够处理具有挑战性的风格领域

Conclusion: DiffStyle360提供了一种高效、灵活的3D头部风格化解决方案，无需针对每种风格进行训练，仅需单张风格参考图像即可生成高质量的多视角一致风格化结果
...
TL;DR: WUKONG是一个无需训练的高保真纹理3D变形框架，通过流式transformer的生成先验实现源目标和目标提示之间的平滑3D过渡，避免了传统方法需要手动对应匹配和变形轨迹估计的限制。
...
Method: 1. 利用流式transformer的生成先验进行3D变形；2. 将变形建模为最优传输重心问题以确保平滑形状过渡；3. 引入顺序初始化策略防止几何突变；4. 提出相似性引导的语义一致性机制保留高频纹理细节并控制混合动态。

Result: 广泛的定量和定性评估表明，WUKONG在多样化的几何和纹理变化上显著优于现有最先进方法，实现了卓越的变形效果，避免了过平滑等常见伪影。

Conclusion: WUKONG框架通过创新的流式transformer应用和最优传输建模，实现了无需训练的高质量3D变形，在保持语义保真度的同时提供了精确的纹理控制能力。
...
TL;DR: Fin3R是一种简单有效的3D重建模型微调方法，通过冻结解码器、仅微调图像编码器，并利用单目教师模型提取的几何细节进行知识蒸馏，显著提升了3D重建的几何精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前前馈式3D重建模型在精细几何结构和鲁棒性方面存在不足，主要原因是：1）高精度深度和姿态监督数据的稀缺；2）多视角点图回归固有的几何不对齐问题。

Method: Fin3R采用轻量级微调策略：冻结处理视角匹配的解码器，仅微调负责特征提取的图像编码器。通过定制化的LoRA适配器，从强大的单目教师模型在大规模无标签数据集上提取精细几何细节进行知识蒸馏。

Result: 在DUSt3R、MASt3R、CUT3R和VGGT等多种模型上验证，微调后的模型能产生更清晰的边界、恢复复杂结构，在单视角和多视角设置下都获得更高的几何精度，且仅增加微小的LoRA权重，测试时内存和延迟几乎不变。

Conclusion: Fin3R是一种简单、有效且通用的3D重建模型微调方法，通过轻量级的知识蒸馏策略显著提升了模型的几何重建能力，同时保持了计算效率。
...
TL;DR: ABounD：一种用于少样本多类别工业异常检测的对抗边界驱动框架，通过动态概念融合和对抗边界锻造实现类别自适应和精确决策边界
...
Method: 提出ABounD统一学习框架，包含动态概念融合模块（DCF）生成类别自适应提示，以及对抗边界锻造模块（ABF）通过PGD风格扰动生成边界级围栏特征，在概念边界损失下进行单阶段训练
...
TL;DR: 论文提出IntentHQ基准，将深度伪造检测从真实性验证转向意图分析，包含5168个视频和23种细粒度意图类别，并开发了多模态意图识别模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法仅关注视频真伪，无法揭示操纵视频背后的意图，而理解视频背后的动机和目的对于应对社会和安全风险至关重要。

Method: 构建IntentHQ基准数据集（5168个视频，23种意图类别），采用监督和自监督多模态模型，整合时空视频特征、音频处理和文本分析来识别视频意图。

Result: 提出了新的意图分析基准和模型框架，能够区分广泛的意图类别，如金融欺诈、间接营销、政治宣传、恐惧传播等。

Conclusion: 该研究将深度伪造检测范式从真实性验证转向上下文理解，为分析操纵视频背后的意图提供了新的基准和方法框架。
...
Method: 提出ITS3D框架，将任务形式化为优化问题，寻找最优高斯噪声输入。采用验证器引导的搜索算法，并引入三种关键技术：高斯归一化稳定搜索过程、SVD压缩降低计算复杂度、奇异空间重置机制防止陷入局部最优

Result: 实验表明ITS3D能显著提升文本到3D的生成质量，展示了计算高效搜索方法在生成过程中的潜力
...
TL;DR: 提出一种基于高斯时空表示的动态火焰三维重建方法，仅需三个视角即可从有限相机视图中重建动态火焰


<details>
  <summary>Details</summary>
Motivation: 火焰的动态重建极具挑战性，因其具有易变性、半透明特性和大量高频特征。现有方法通常需要大量视角，本文旨在仅用三个视角解决这一欠约束几何问题

Method: 1) 结合密集多视角立体图像与单目深度先验分离静态背景与动态火焰区域；2) 通过融合每视角密集光流投影初始化3D流场；3) 每个3D高斯编码生命周期和线性速度以匹配密集光流；4) 采用自定义硬件同步模式确保跨相机亚帧时间对齐

Result: 在多个重建实验中的定量和定性验证表明，该方法在多样且具有挑战性的真实火焰场景中表现出鲁棒性能

Conclusion: 提出的高斯时空表示方法能够仅用三个视角有效重建动态火焰，通过硬件同步和3D高斯编码解决了火焰重建中的几何欠约束和高频特征捕捉问题
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对检测或分割等感知任务，忽视了推断道路拓扑和动态场景结构所需的推理能力，而理解连接低级感知和高级规划的中层道路语义对于可靠的自动驾驶和数字地图构建至关重要。

Method: 提出了RoadSceneBench基准测试，强调关系理解和结构一致性；同时提出了HRRP-T（分层关系奖励传播与时序一致性）训练框架，通过自适应奖励信号促进空间连贯性和语义对齐。

Result: 该方法在各种道路配置上实现了最先进的性能，RoadSceneBench为研究中层道路语义和培养结构感知的自主感知提供了紧凑而强大的基础。

Conclusion: RoadSceneBench填补了道路场景推理评估的空白，提出的HRRP-T框架使模型能够超越静态识别，实现几何感知和时序一致的推理，为自动驾驶感知系统的发展提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 文本人物异常检索现有方法大多依赖复杂的深度学习技术，存在如何优化模型以获得更细粒度特征的研究问题。需要探索如何结合细粒度特征和粗粒度特征来提高检索性能。

Method: 1. 提出局部-全局混合视角（LHP）模块与视觉语言模型（VLM）集成；2. 研究统一图像-文本（UIT）模型，结合ITC、ITM、MLM和MIM四种损失函数；3. 提出新颖的迭代集成策略；4. 基于LHP模型指导的特征选择算法。

Result: 在PAB数据集上实现了最先进的性能：R@1提升9.70%，R@5提升1.77%，R@10提升1.01%，相比之前工作有明显改进。

Conclusion: 提出的LHP模块、UIT模型、迭代集成策略和特征选择算法有效提升了文本人物异常检索性能，证明了结合细粒度和粗粒度特征的重要性。
...
TL;DR: DocVAL是一个验证链式思维蒸馏框架，将大型教师模型的空间推理能力转移到可部署的学生视觉语言模型中，在DocVQA任务上实现了高精度和高效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 当前文档视觉问答系统存在精度与效率的尖锐权衡：大型教师模型精度高但部署成本高，紧凑学生模型定位性能大幅下降。需要一种方法将教师的空间推理能力高效转移到学生模型中。

Method: 提出DocVAL框架，包含三个关键组件：1）带验证时文本检测的教师监督，过滤和去噪训练信号；2）多模块验证器，确保答案正确性和几何一致性，提供像素级错误反馈；3）两阶段学生训练方案，先学习验证后的CoT轨迹，再通过验证器反馈进行迭代优化。
...
Conclusion: DocVAL框架成功解决了文档视觉问答中的精度-效率权衡问题，通过验证链式思维蒸馏将教师模型的空间推理能力有效转移到学生模型中，为文档理解中的空间推理研究提供了高质量数据集和有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型多样化，现有检测方法往往记忆特定生成器的伪影而非学习可迁移线索，导致对未见生成器的检测失败。研究发现冻结的视觉基础模型（特别是DINOv3）已具备跨生成器检测能力。

Method: 通过频率、空间和token视角的系统研究，发现DINOv3依赖全局低频结构作为弱但可迁移的真实性线索。提出无需训练的token排序策略，结合轻量级线性探针选择真实性相关token子集。

Result: 该方法在所有评估数据集上一致提高检测准确率，token子集能有效提升检测性能，为跨生成器检测提供了高效、可解释的基线方法。
...
TL;DR: CoT4AD是一个用于自动驾驶的新型视觉-语言-动作模型框架，通过引入思维链推理来增强数值和因果推理能力，在真实世界和模拟基准测试中达到最先进性能。
...
Method: 提出CoT4AD框架，将视觉观察和语言指令结合进行语义推理、场景理解和轨迹规划。训练时显式建模感知-问题-预测-动作的思维链，推理时进行隐式思维链推理。
...
TL;DR: Dimitra++是一个音频驱动的说话头生成框架，通过条件运动扩散变换器学习唇部运动、面部表情和头部姿态，在VoxCeleb2和CelebV-HQ数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的说话头生成方法需要同时学习唇部运动、面部表情和头部姿态，这是一个具有挑战性的任务。本文旨在开发一个能够综合生成这些面部运动要素的框架。

Method: 提出条件运动扩散变换器（cMDT）来建模面部运动序列，使用3D表示。该模型以参考面部图像（决定外观）和音频序列（驱动运动）作为条件输入。

Result: 在VoxCeleb2和CelebV-HQ数据集上的定量、定性实验以及用户研究表明，Dimitra++在生成逼真的说话头方面优于现有方法，能够准确生成唇部运动、面部表情和头部姿态。

Conclusion: Dimitra++框架通过条件运动扩散变换器有效解决了音频驱动的说话头生成问题，能够综合生成逼真的唇部运动、面部表情和头部姿态运动。
...
### [68] [Revisiting the Necessity of Lengthy Chain-of-Thought in Vision-centric Reasoning Generalization](https://arxiv.org/abs/2511.22586)
*Yifan Du,Kun Zhou,Yingqian Min,Yue Ling,Wayne Xin Zhao,Youbin Wu*

Main category: cs.CV

TL;DR: 研究发现：在视觉语言模型中，简洁的思维链（仅包含必要的基础步骤）比冗长的思维链能带来更好的泛化性能，特别是在视觉推理任务中。


<details>
  <summary>Details</summary>
Motivation: 虽然思维链（CoT）数据（特别是长链或视觉CoT）已被广泛用于监督中间推理过程，但尚不清楚为什么特定的CoT设计有效，以及哪些设计真正支持可泛化的推理能力。

Method: 使用可控的迷宫求解基准测试，其中推理规则完全基于视觉，难度可通过网格大小调节，所有中间步骤可自动生成。使用Qwen2.5-VL-7B模型，在标准SFT-then-RL流程下，比较三种代表性的CoT格式：语言CoT、基础CoT（带空间坐标轨迹）和视觉CoT（带图像操作）。

Result: 实验显示：视觉和较长的CoT主要加速收敛但未提升最终性能上限；仅包含必要基础步骤的简洁CoT优于较长轨迹；仅保留最小基础结果的CoT在不同迷宫大小上泛化效果最好。这些发现在其他视觉中心任务上得到验证。
...
TL;DR: 本文针对密集文本图像去除任务，开发了基于贝叶斯优化的灵活掩模建模方法，发现字符级掩模比最小覆盖区域更有效，为手动掩模提供了实用指南。


<details>
  <summary>Details</summary>
Motivation: 生成模型显著提升了图像修复精度，但现有文本去除方法主要针对简单场景文本，缺乏对密集文本复杂图像的研究。工业应用中需要从文档图像中去除特定文本并重建原始图像，因此需要解决密集文本去除的挑战。

Method: 创建了包含大量文本的图像文本去除基准数据，发现文本去除性能对掩模轮廓扰动敏感。开发了高度灵活的掩模轮廓建模方法，使用贝叶斯优化学习掩模参数，生成字符级掩模。

Result: 研究发现文本区域的最小覆盖并非最优方案，字符级掩模表现更好。通过贝叶斯优化得到的掩模轮廓在实际应用中更有效，为手动掩模提供了用户友好的指导原则。

Conclusion: 本文为密集文本图像去除任务提供了实用的掩模建模方法，通过贝叶斯优化学习最优掩模参数，发现字符级掩模优于最小覆盖策略，为工业应用中的文本去除任务提供了重要指导。
...
### [70] [HarmoCLIP: Harmonizing Global and Regional Representations in Contrastive Vision-Language Models](https://arxiv.org/abs/2511.22594)
...
<details>
  <summary>Details</summary>
Motivation: CLIP虽然在大规模视觉-语言预训练中表现出色，但由于缺乏区域级监督，其细粒度语义理解能力有限。现有方法在改善局部感知时往往会破坏全局对齐，导致全局与局部表示的权衡问题。

Method: 提出HarmoCLIP框架，引入显式的细粒度语义监督项，直接对齐文本片段与对应的视觉区域，并采用新颖的区域-语言对齐监督策略，在不损害全局语义一致性的前提下促进细粒度语义学习。
...
TL;DR: 提出新的注视跟踪方法，通过形状误差正则化约束瞳孔椭圆拟合，结合类似纸张展开的坐标变换技术，在GazeTrack数据集上实现更低计算复杂度和更小注视角度误差


<details>
  <summary>Details</summary>
Motivation: 当前眼动跟踪技术在虚拟和增强现实应用中精度不足，无法满足空间计算需求，需要开发更高精度的注视跟踪方法

Method: 1) 设计注视数据收集框架，使用高精度设备创建包含多种族、年龄和视力条件的GazeTrack基准数据集；2) 提出形状误差正则化方法约束瞳孔椭圆拟合；3) 发明类似纸张展开的坐标变换方法准确预测注视向量；4) 构建注视向量生成模型

Result: 在GazeTrack数据集上实现了比其他方法更低的注视角度误差和计算复杂度

Conclusion: 提出的方法显著提高了注视跟踪精度，为虚拟和增强现实应用提供了更可靠的注视跟踪解决方案
...
TL;DR: Fast3Dcache是一个无需训练、几何感知的缓存框架，通过智能缓存调度和稳定性准则，在加速3D扩散模型推理的同时保持几何一致性，实现了27.12%的加速和54.8%的FLOPs减少。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在3D生成中推理成本高昂，现有缓存方法直接应用于3D扩散模型会破坏几何一致性，因为缓存潜在特征中的微小数值误差会累积导致结构伪影和拓扑不一致。
...
Conclusion: Fast3Dcache是一种有效的训练免费方法，能够在保持几何保真度的同时加速3D扩散模型推理，解决了现有缓存方法在3D生成中的几何一致性破坏问题。
...
TL;DR: Diff-ICMH是一个生成式图像压缩框架，通过利用生成先验和语义一致性损失，同时优化机器分析和人类感知，实现单码流支持多种智能任务。


<details>
  <summary>Details</summary>
Motivation: 现有图像压缩方法通常孤立地优化人类感知或机器分析任务，但两者存在根本共性：保持准确的语义信息对智能任务至关重要，同时增强感知质量也有助于机器语义特征提取。

Method: 提出Diff-ICMH框架：1)利用生成先验确保感知真实性；2)通过语义一致性损失(SC loss)保证语义保真度；3)引入标签引导模块(TGM)，利用图像级语义标签激发预训练扩散模型的生成能力，仅需极少额外比特率。

Result: Diff-ICMH能够通过单一编解码器和比特流支持多种智能任务，无需任务特定适配，同时为人类感知保持高质量视觉体验。实验结果表明其在多样化任务上的优越性和泛化能力。

Conclusion: 该研究揭示了机器分析和人类感知在图像压缩中的根本共性，提出的Diff-ICMH框架成功实现了两者的协调统一，为图像压缩领域提供了新的研究方向。
...
### [74] [ReAG: Reasoning-Augmented Generation for Knowledge-based Visual Question Answering](https://arxiv.org/abs/2511.22715)
...
Conclusion: ReAG通过结合检索增强和推理增强的方法，有效解决了多模态大语言模型在知识密集型视觉问答任务中的局限性，为知识密集型多模态理解提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统牙科种植体基台设计过程耗时耗力，不合适的基台可能导致种植体并发症如种植体周围炎。使用人工智能辅助设计可以提高效率和适应性。
...
Conclusion: TCEAD框架通过结合文本引导定位和自监督学习，有效解决了牙科种植体基台自动化设计问题，显著提升了定位精度和设计效率。
...
### [76] [All Centers Are at most a Few Tokens Apart: Knowledge Distillation with Domain Invariant Prompt Tuning](https://arxiv.org/abs/2511.22739)
*Amir Mohammad Ezzati,Alireza Malekhosseini,Armin Khosravi,Mohammad Hossein Rohban*

Main category: cs.CV

TL;DR: 提出DIPT方法，通过领域不变提示调优增强病理图像领域泛化能力，在知识蒸馏框架中学习领域特定标记并跨领域平均，实现领域不变提示


<details>
  <summary>Details</summary>
Motivation: 计算病理学中存在因染色协议、扫描设备和成像设置差异导致的领域偏移问题，现有视觉语言模型的零样本性能受提示词变化影响较大，且病理图像缺乏自然图像中的语义描述符，需要数据驱动方法学习领域特定提示

Method: 提出领域不变提示调优(DIPT)方法，为每个领域学习多个输入标记，这些标记分别训练后跨领域平均得到领域不变提示，学生模型利用这些提示从PLIP文本编码器蒸馏知识，实现视觉特征与领域不变嵌入的对齐

Result: 在组织病理学数据集上的领域泛化任务中，相比现有最先进知识蒸馏方法，平均F1分数有显著提升

Conclusion: DIPT方法通过领域不变提示学习增强了计算病理学模型的领域泛化能力，有助于在异构数据源的现实临床问题中部署鲁棒模型
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本工业异常检测方法只能生成粗粒度的异常图，因为视觉变换器仅提取补丁级特征。虽然最近有方法尝试基于ZSAD特征预测更精细的异常，但由于合成训练异常与真实异常之间的差距，仍然难以恢复细粒度异常且存在漏检问题。作者观察到异常分数图提供了互补的空间线索，这一事实之前被忽视了。

Method: 提出AnoRefiner方法：1) 设计异常细化解码器，利用异常分数图逐步增强图像特征，减少对合成异常数据的依赖；2) 受大规模生产范式启发，提出渐进式组间测试时训练策略，在每个产品组中训练ARD用于下一组的细化过程，同时保持与任何ZSAD方法的兼容性。
...
TL;DR: 该论文提出了一种基于推理的图像编辑框架，通过"思考-编辑-反思"循环机制，解锁多模态大语言模型的推理能力，显著提升了图像编辑性能。
...
Result: 实验表明该推理方法取得了显著性能提升：基于Step1X-Edit的ReasonEdit-S在ImgEdit上提升4.3%，GEdit上提升4.7%，Kris上提升8.2%；基于Qwen-Image-Edit的ReasonEdit-Q在GEdit和Kris上都优于之前的开源方法。

Conclusion: 解锁多模态大语言模型的推理能力可以有效提升图像编辑模型的性能，通过思考和反思机制增强指令理解和编辑准确性，为图像编辑模型的发展提供了新方向。
...
TL;DR: CORTEX框架通过整合视觉和文本特征来增强变化描述能力，利用视觉语言模型提取场景级文本知识，解决现有方法仅依赖视觉特征难以捕捉细微变化的问题。
...
Method: 提出CORTEX框架，包含三个关键模块：图像级变化检测器、推理感知文本提取模块（利用VLMs生成组合推理描述）、图像-文本双重对齐模块（对齐视觉和文本特征进行细粒度关系推理）。

Result: CORTEX能够对视觉和文本特征进行推理，捕捉仅凭视觉特征难以识别的模糊变化，增强了变化理解能力。

Conclusion: 通过整合互补的文本线索，CORTEX框架能够更好地理解和描述图像对之间的变化，特别是那些视觉特征难以捕捉的细微但有意义的变化。
...
TL;DR: 本文提出Attention Interaction Alignment (AIA)损失函数，旨在缓解多模态模型中理解与生成任务之间的冲突，无需模型解耦即可提升性能


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在图像生成和理解任务中存在目标冲突，现有方法通过不同程度的模型解耦来缓解冲突，但过度解耦会丧失交错生成能力，违背统一模型的初衷。本文旨在探索不依赖模型解耦的冲突缓解方法。

Method: 通过分析模型解耦如何缓解冲突，观察到解耦本质上驱动模型趋向任务特定的多模态交互模式。基于此观察提出Attention Interaction Alignment (AIA)损失函数，在训练中显式学习任务特定的多模态交互模式。将AIA应用于Emu3和Janus-Pro模型的不同训练阶段。

Result: AIA不仅细化了跨模态注意力模式，还同时提升了生成和理解任务的性能，证明了其通用性和有效性。

Conclusion: 提出的AIA损失函数能够在不依赖模型解耦的情况下缓解多模态任务冲突，保持统一模型交错生成能力的同时提升性能，为统一多模态模型训练提供了新思路。
...
TL;DR: 提出VaMP框架，通过变分多模态提示学习实现样本特定、不确定性感知的提示调优，在少样本和领域泛化基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态提示学习方法通常依赖固定的共享提示和确定性参数，难以捕捉实例级变化或跨任务和领域的不确定性。需要一种能够生成样本特定、不确定性感知提示的方法。

Method: 提出变分多模态提示学习框架，通过从学习到的后验分布中采样生成实例条件提示；引入基于实例表示和类原型的类感知先验；将提示调优公式化为潜在提示表示的变分推断，通过重参数化采样进行端到端训练。

Result: 在少样本学习和领域泛化基准测试中，VaMP实现了最先进的性能，证明了建模不确定性和任务结构的有效性。

Conclusion: VaMP框架通过变分多模态提示学习成功解决了现有方法的局限性，实现了样本特定、不确定性感知的提示调优，在多模态表示学习中表现出优越的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 挑战对Distribution Matching Distillation（DMD）及其变体性能的传统理解。传统观点认为DMD的核心机制是匹配学生和教师模型的输出分布，但作者质疑这一观点，特别是在需要CFG的复杂任务中。

Method: 通过对DMD训练目标进行严格分解，识别出CFG增强（CA）作为蒸馏的"引擎"，而分布匹配（DM）作为"正则化器"。验证了DM可以被更简单的非参数约束或GAN目标替代。基于这种解耦分析，提出了改进蒸馏过程的原理性修改，如解耦引擎和正则化器的噪声调度。

Result: 研究发现CFG增强是少步蒸馏的主要驱动力，而分布匹配主要起稳定训练和减少伪影的正则化作用。提出的改进方法被Z-Image项目采用，开发了顶级的8步图像生成模型，验证了发现的泛化性和鲁棒性。

Conclusion: 本文重新定义了扩散模型蒸馏的理解框架，揭示了CFG增强的核心作用，为更系统、深入的蒸馏分析提供了理论基础，并启发了原理性的改进方法，在实际应用中取得了显著性能提升。
...
### [84] [MIMM-X: Disentangling Spurious Correlations for Medical Image Analysis](https://arxiv.org/abs/2511.22990)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学任务中常出现伪相关（捷径学习）问题，导致在新环境中泛化能力差。医学影像中常存在多重伪相关共存的情况，误分类可能带来严重后果。

Method: 提出MIMM-X框架，通过最小化因果特征与多重伪相关之间的互信息，将两者解耦分离，使预测基于真实的因果关系而非数据集特定的捷径。

Result: 在三个数据集（UK Biobank、NAKO、CheXpert）和两种影像模态（MRI和X射线）上评估，结果显示MIMM-X能有效缓解多重伪相关的捷径学习问题。

Conclusion: MIMM-X框架能够有效解决医学影像中的多重伪相关问题，提高深度学习模型在新环境中的泛化能力和可靠性。
...
### [85] [From Illusion to Intention: Visual Rationale Learning for Vision-Language Reasoning](https://arxiv.org/abs/2511.23031)
*Changpeng Wang,Haozhe Wang,Xi Chen,Junhan Liu,Taofeng Xue,Chong Peng,Donglian Qi,Fangzhen Lin,Yunfeng Yan*

Main category: cs.CV

TL;DR: 该论文提出视觉理性学习(ViRL)框架，将视觉动作重构为核心推理原语而非可选工具，通过过程监督、目标对齐和细粒度信用分配，确保每个视觉动作对推理链都有实质性贡献，实现"基于正确视觉理由得到正确答案"。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言推理框架将视觉动作视为可选工具，虽然能提升指标但导致推理缺乏视觉基础，裁剪效果不佳，产生"视觉思考幻觉"——模型看似基于视觉但实际依赖与上下文无关的动作，既不精炼感知也不引导推理走向正确答案。

Method: 提出视觉理性学习(ViRL)端到端范式：1) 使用真实理性进行过程监督；2) 通过步骤级奖励塑造实现目标对齐；3) 细粒度信用分配以区分正确、冗余和错误动作。将视觉动作重构为视觉理性化(文本思维链的视觉类比)。

Result: 仅使用端到端强化学习训练的ViRL在感知、幻觉和推理等多个基准测试中取得了最先进的结果，建立了视觉理性化作为任务无关、过程基础的范式。

Conclusion: 视觉理性化是构建透明、可验证、可信赖的视觉语言模型的任务无关、过程基础范式，确保模型基于正确的视觉理由获得正确答案。
...
TL;DR: Ar2Can是一个两阶段框架，通过分离空间规划和身份渲染来解决多人生成问题，显著提升了人脸计数准确性和身份保真度


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在多人生成场景中表现不佳，经常出现人脸重复、身份混淆或人数统计错误的问题，需要专门的方法来解决多人生成的挑战

Method: 提出两阶段框架：1) Architect模块预测结构化布局，确定每个人的位置；2) Artist模块合成逼真图像，使用基于匈牙利空间对齐和ArcFace身份相似度的空间接地人脸匹配奖励；采用Group Relative Policy Optimization (GRPO)优化，使用组合奖励函数

Result: 在MultiHuman-Testbench上评估，Ar2Can在计数准确性和身份保真度方面取得显著改进，同时保持高感知质量；主要使用合成数据，无需真实多人生成图像

Conclusion: Ar2Can通过分离空间规划和身份渲染，有效解决了多人生成中的关键挑战，为可靠的多人生成提供了新方法
...
TL;DR: Z-Image是一个高效的6B参数图像生成模型，基于S3-DiT架构，挑战了"不惜一切代价扩大规模"的范式，在减少计算开销的同时实现了与顶级商业模型相媲美的性能。


<details>
  <summary>Details</summary>
Motivation: 当前高性能图像生成模型被Nano Banana Pro、Seedream 4.0等专有系统主导，开源替代品如Qwen-Image、Hunyuan-Image-3.0和FLUX.2参数规模过大（20B-80B），在消费级硬件上推理和微调不切实际，需要更高效的解决方案。

Method: 提出Z-Image，基于可扩展单流扩散变换器（S3-DiT）架构的6B参数基础生成模型，通过系统优化整个模型生命周期（从精选数据基础设施到简化训练课程），仅用314K H800 GPU小时完成训练。采用few-step蒸馏方案和奖励后训练得到Z-Image-Turbo，实现亚秒级推理延迟并兼容消费级硬件。还通过全预训练范式训练Z-Image-Edit编辑模型。

Result: 模型在多个维度上达到或超越领先竞争对手的性能，特别是在逼真图像生成和双语文本渲染方面表现出色，结果可与顶级商业模型相媲美，同时显著降低了计算开销。Z-Image-Turbo在企业级H800 GPU上实现亚秒级推理延迟，并兼容消费级硬件（<16GB VRAM）。

Conclusion: 研究表明，通过系统优化模型架构和训练流程，可以在显著减少计算开销的情况下实现最先进的图像生成性能，挑战了"不惜一切代价扩大规模"的范式。公开代码、权重和在线演示，促进可访问、经济实惠且先进的生成模型发展。
...
### [88] [Evaluating the Clinical Impact of Generative Inpainting on Bone Age Estimation](https://arxiv.org/abs/2511.23066)
...
TL;DR: 生成式基础模型虽然能通过逼真的图像修复去除视觉伪影，但对医学AI性能的影响不确定。研究发现修复儿科手部X光片中的非解剖标记会显著降低骨龄和性别预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 研究生成式基础模型在医学图像修复中的临床可靠性，特别是针对儿科手部X光片中非解剖标记的修复，评估这种修复是否会保留骨龄和性别预测所需的特征。
...
Conclusion: 尽管视觉上逼真，但基于基础模型的修复可能会掩盖细微但临床相关的特征，并引入潜在偏差，即使编辑仅限于非诊断区域。这强调了在将此类生成工具整合到临床AI工作流程之前，需要进行严格的任务特定验证。
...
<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在渲染任务中显示出潜力，但通常需要密集输入视角的逐场景优化。现有前馈方法需要大量重叠的输入视角来建立几何先验，无法处理大稀疏度的双目相机输入

Method: 提出两阶段学习策略：第一阶段通过迭代亲和力学习将点图转换到真实空间；第二阶段将两个输入视角的点图投影到目标视角平面，通过立体匹配细化几何，并在细化平面上锚定高斯基元进行高质量渲染

Result: 方法提高了点图重建的稳定性和自由视角渲染的视觉质量，在收集的多视角以人为中心数据上验证了有效性

Conclusion: Splat-SAP通过像素级点图重建和两阶段学习策略，成功实现了从稀疏双目输入的高质量自由视角渲染，解决了现有方法对密集重叠视角的依赖问题
...
TL;DR: 该论文介绍了Bharat场景文本数据集（BSTD），这是一个用于印度语言场景文本识别的大规模基准数据集，包含11种印度语言和英语的10万+单词，支持多种场景文本任务。
...
Method: 创建了Bharat场景文本数据集（BSTD），包含来自印度各地语言区域的6,500多张场景图像中的10万+单词，涵盖11种印度语言和英语。数据集经过精心标注，支持场景文本检测、脚本识别、裁剪单词识别和端到端场景文本识别等任务。

Result: 通过将最先进的英语模型适应（微调）到印度语言进行评估，结果突出了印度语言场景文本识别的挑战和机遇。所有模型和数据都已开源。

Conclusion: BSTD数据集代表了在印度语言场景文本识别领域研究的重要进展，为这一领域的研究提供了有价值的基准和资源。
...
### [91] [SpaceMind: Camera-Guided Modality Fusion for Spatial Reasoning in Vision-Language Models](https://arxiv.org/abs/2511.23075)
*Ruosen Zhao,Zhikang Zhang,Jialei Xu,Jiahao Chang,Dong Chen,Lingyun Li,Weijian Sun,Zizhuang Wei*

Main category: cs.CV

TL;DR: SpaceMind是一个专门为空间推理设计的多模态大语言模型，仅从RGB输入就能进行3D空间推理，通过相机引导的模态融合模块实现，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在多模态理解方面表现出色，但在3D空间推理（如距离估计、尺寸比较、跨视角一致性）方面仍有困难。现有方法要么依赖辅助3D信息，要么通过浅层特征融合增强RGB-only模型。
...
Method: 1) 提出REVEAL-Bench基准，基于多个轻量级专家模型构建证据链，记录逐步推理轨迹和证据依据；2) 提出REVEAL框架，集成检测与专家基础强化学习，奖励机制联合优化检测准确性、解释保真度和基于明确取证证据的逻辑一致性。

Result: 实验结果表明，REVEAL显著提升了检测准确性、解释保真度和跨模型鲁棒泛化能力，为可解释图像取证设立了新的最先进基准。

Conclusion: REVEAL通过证据链推理增强的取证框架，能够产生细粒度、可解释且可验证的推理链，有效解决了现有方法缺乏可验证证据链的问题，推动了可解释图像取证的发展。
...
<details>
  <summary>Details</summary>
Motivation: 野生动物监测对生物多样性保护至关重要，可见光和热红外图像的互补使用可以提供比单一图像源更多的信息，但在自动检测中面临对齐和融合的挑战，特别是两种图像通常具有不同的视场和空间分辨率。
...
Result: 在所有类别中，晚期融合和早期融合相比VIS-only模型都提高了F1分数。对于主要类别"被占用的巢穴"，晚期融合将F1分数从90.2%提高到93.0%。该模型还能以90%的召回率识别两种来源的误报。

Conclusion: 虽然融合方法能获得更好的结果，但受到TIR视场限制和对齐约束的影响会损失数据。使用机载超高分辨率可见光传感器可能是实现调查操作化的有前景选择。
...
<details>
  <summary>Details</summary>
Motivation: 森林在全球生态系统中至关重要，但传统的地上生物量估算方法依赖劳动密集的实地测量或存在显著局限性的遥感技术。需要一种可扩展、经济高效且能促进公民科学参与的解决方案。

Method: 将AGB估算构建为密集预测任务，引入AGB密度图概念（每个像素表示归一化的树木生物量）。利用SPREAD合成3D数据集，通过异速生长方程计算AGB，训练模型预测AGB密度图，然后整合得到整个场景的AGB估计。

Result: 在保留的SPREAD数据上实现1.22 kg/m²的中位AGB估算误差，在真实图像数据集上达到1.94 kg/m²的中位误差。这是首个直接从单张RGB图像估算地上生物量的方法。

Conclusion: 该方法为森林监测提供了可扩展、可解释且经济高效的解决方案，同时通过公民科学倡议实现了更广泛的参与。首次实现了从单张RGB图像直接估算地上生物量。
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病是不可逆的神经退行性疾病，早期预测对及时干预至关重要。轻度认知障碍是正常衰老与AD之间的过渡阶段，但并非所有MCI患者都会转化为AD，因此准确区分稳定MCI和进展性MCI具有重要临床意义。

Method: 提出混合架构模型：1) 结合CNN和Vision Transformer从MRI扫描中提取局部空间特征和全局上下文依赖；2) 使用双向LSTM处理四个连续时间点的MRI特征和其他非图像生物标志物；3) 端到端预测48个月时的认知状态。

Result: 模型在区分稳定MCI和进展性MCI方面达到95.05%的平均预测准确率，优于现有AD预测研究，展示了纵向AD预测的最先进性能。

Conclusion: 该研究证明了结合空间和时间建模在阿尔茨海默病早期检测中的有效性，为临床干预提供了有价值的预测工具。
...
<details>
  <summary>Details</summary>
Motivation: 在全球化背景下，单一视觉场景中经常出现来自不同文化的元素混合，但大型视觉语言模型如何感知这些文化混合场景尚未得到充分研究。研究者认为文化混合是LVLMs面临的关键挑战，需要系统评估模型在多元文化元素共存时的表现。

Method: 构建CultureMix基准测试，包含23k张扩散生成、人工验证的文化混合食物图像，涵盖四个子任务：纯食物、食物+食物、食物+背景、食物+食物+背景。评估10个LVLMs模型，分析其在文化混合场景中的表现，并探索三种鲁棒性策略，特别是使用多样文化混合数据集进行监督微调。

Result: 模型在文化混合场景中表现不佳：1）无法在混合设置中保持个体文化身份；2）对背景依赖性强，添加文化背景后准确率下降14%；3）对相同食物在不同上下文中产生不一致预测。监督微调能显著提高模型一致性和减少背景敏感性。

Conclusion: 文化混合场景是LVLMs面临的重要挑战，当前模型在多元文化元素共存时表现不佳。需要更多关注文化混合场景的研究，以开发能够在文化多样化现实环境中可靠运行的LVLMs。监督微调是有效的改进策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统MRI采集高质量高分辨率图像耗时，欠采样加速会导致图像质量下降，现有方法要么处理欠采样重建，要么处理伪影校正，没有同时解决这两个问题的方案

Method: USArt模型采用双子模型架构，专门针对笛卡尔采样的2D脑解剖图像，同时处理欠采样重建和噪声/运动伪影校正

Result: 显著提高信噪比和图像对比度，梯度欠采样策略效果最佳，实现5倍加速同时进行伪影校正且无明显质量下降

Conclusion: USArt模型能够同时处理MRI加速和伪影校正问题，在真实场景中表现出鲁棒性，为快速高质量MRI成像提供了有效解决方案
...
TL;DR: 该研究提出了CogIP-Bench基准测试，用于评估多模态大语言模型在图像认知属性（如记忆性、趣味性、美观度等）上与人类感知的对齐程度，并通过后训练显著提升模型的对齐能力，最终应用于图像生成任务。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型擅长识别图像中的物体和描述场景，但缺乏对人类主观认知属性的理解能力，如判断图像是否令人难忘、有趣、美观或具有情感感染力。这种能力差距限制了模型在需要人类感知对齐的应用场景中的表现。

Method: 1. 引入CogIP-Bench综合基准测试，系统评估MLLMs在图像认知属性上的表现；2. 设计后训练阶段，通过特定训练方法提升模型与人类感知的对齐度；3. 将认知对齐后的MLLM集成到图像生成流程中，指导合成过程以产生具有特定认知属性的图像。

Result: 1. 评估显示当前模型在图像认知属性上与人类感知存在显著差距；2. 后训练阶段能有效弥合这一差距，显著提升模型与人类判断的对齐度；3. 学习到的认知对齐能力不仅具有预测性，还能迁移到下游创意任务中，指导生成更具记忆性、视觉吸引力等特质的图像。

Conclusion: 该工作提供了衡量人类感知对齐的基准测试、增强对齐能力的后训练流程，并证明这种对齐能力能够实现更以人为本的人工智能，为MLLMs在需要人类感知理解的应用场景中提供了重要改进方向。
...
### [99] [LC4-DViT: Land-cover Creation for Land-cover Classification with Deformable Vision Transformer](https://arxiv.org/abs/2511.22812)
*Kai Wang,Siyi Chen,Weicong Pang,Chenchen Zhang,Renjun Gao,Ziru Chen,Cheng Li,Dasa Gu,Rui Huang,Alexis Kai Hon Lau*

Main category: cs.CV

TL;DR: LC4-DViT框架结合文本引导的扩散模型生成平衡训练数据与变形感知Vision Transformer，显著提升高分辨率遥感影像土地覆盖分类精度


<details>
  <summary>Details</summary>
Motivation: 传统遥感土地覆盖分类面临标注数据稀缺、类别不平衡以及高分辨率影像几何失真等挑战，需要开发能够生成平衡训练数据并有效处理几何变形的分类方法

Method: 提出LC4-DViT框架：1) 使用GPT-4o生成场景描述，结合文本引导扩散管道合成类别平衡的高保真训练图像；2) 设计DViT模型，将DCNv4可变形卷积主干与Vision Transformer编码器结合，同时捕捉细粒度几何特征和全局上下文

Result: 在AID数据集8个类别上达到0.9572总体准确率、0.9576宏F1分数和0.9510 Cohen's Kappa，优于ViT基线和其他主流模型；在SIRI-WHU数据集上表现良好，显示良好迁移性；GPT-4o评估显示DViT注意力与水文结构对齐最佳

Conclusion: 描述驱动的生成增强与变形感知Transformer相结合是解决高分辨率土地覆盖制图中数据稀缺和几何失真问题的有效途径，为环境管理提供准确及时的土地覆盖信息
...
<details>
  <summary>Details</summary>
Motivation: 现有世界引擎在激进6-DoF相机轨迹和复杂户外布局下存在三个主要问题：1）失去长距离几何一致性；2）偏离目标路径；3）运动过于保守。需要开发能够保持稳定3D结构并准确执行挑战性相机操作的系统。

Method: 提出Captain Safari，一个姿态条件的世界引擎，通过从持久世界记忆中检索来生成视频。给定相机路径时，系统维护动态本地内存，使用检索器获取姿态对齐的世界标记，这些标记随后条件化沿轨迹的视频生成。

Result: 在视频质量、3D一致性和轨迹跟随方面显著优于最先进的相机控制生成器：MEt3R从0.3703降至0.3690，AUC@30从0.181提升至0.200，FVD明显低于所有基线。在50人参与的5路人类研究中，67.6%的偏好选择Captain Safari。

Conclusion: 姿态条件的世界记忆是实现长时域、可控视频生成的强大机制。同时贡献了OpenSafari数据集作为未来世界引擎研究的挑战性新基准。
...
TL;DR: 该研究探讨了大规模视觉语言模型在自动驾驶安全监控中的应用，通过构建数据集评估模型处理同步驾驶员视角和道路视角视频的能力，发现微调后的LVLMs能生成准确的安全驾驶指令，但仍存在检测复杂事件的挑战。


<details>
  <summary>Details</summary>
Motivation: 虽然大规模视觉语言模型在视觉任务中表现出色，但自动驾驶安全监控需要同时处理驾驶员视角和道路视角的同步视频输入，以全面检测危险事件（如驾驶时使用手机）。目前缺乏对LVLMs在这种多视角同步输入场景下能力的研究。

Method: 研究构建了一个专门的数据集，用于评估LVLMs处理同步驾驶员视角和道路视角视频的能力。通过对比预训练模型和微调模型在该数据集上的表现，分析LVLMs在自动驾驶安全监控任务中的有效性。

Result: 实验结果表明，预训练的LVLMs在检测危险事件方面效果有限，但经过微调的LVLMs能够生成准确且具有安全意识的行车指令。然而，模型在检测视频中细微或复杂事件方面仍面临挑战。

Conclusion: 微调后的LVLMs在自动驾驶安全监控任务中具有应用潜力，但需要进一步改进以更好地检测复杂事件。研究中的发现和错误分析为该领域LVLM系统的改进提供了有价值的见解。
...
TL;DR: 该研究揭示了当前多模态大语言模型在多模态矛盾情境下的脆弱性，并提出了模态对齐调优策略来增强模型的跨模态推理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型取得了显著进展，但一个基本问题仍然存在：MLLMs是否对矛盾的模态具有鲁棒性？当前模型在模态对齐和跨模态推理方面存在脆弱性，需要系统性的评估和改进方法。

Method: 1. 构建MMA-Bench基准测试，包含视频和任务，用于探测模型对特定模态的依赖；2. 使用黑盒和白盒可解释性技术分析模型的脆弱性；3. 提出模态对齐调优策略，教导模型何时优先考虑、利用或忽略特定模态线索。

Result: 研究发现当前MLLMs在音频-视觉对不匹配和简单误导性文本的情况下表现不佳，缺乏鲁棒的多模态推理能力。提出的模态对齐调优策略能够显著增强多模态基础能力，产生更可靠的跨模态推理。

Conclusion: 该工作不仅提供了可解释性工具，还为开发具有内在可靠跨模态推理能力的MLLMs指明了清晰路径。通过模态对齐调优，可以显著提升模型在多模态矛盾情境下的鲁棒性和可靠性。
...
TL;DR: 该论文针对现有多模态知识库视觉问答基准存在的"视觉捷径"问题，提出了RETINA基准和MIMIR方法，通过消除视觉捷径来更真实地评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有MKB-VQA基准存在"视觉捷径"问题，查询图像通常与目标文档的主要实体匹配，导致模型可以仅利用视觉线索就获得良好表现，无法真实评估多模态理解能力。

Method: 1) 提出RETINA基准，使用LLM驱动的流水线自动构建，包含12万训练样本和2千人工标注测试集，查询引用次要实体，并配对相关实体图像；2) 提出MIMIR方法，通过增强多个相关实体图像来丰富文档嵌入，处理RETINA基准。

Result: 在RETINA基准上，现有模型性能显著下降，证实了它们对视觉捷径的依赖。MIMIR方法能有效处理RETINA基准，而先前仅使用单张图像的方法则无法应对。

Conclusion: RETINA基准揭示了现有MKB-VQA基准的局限性，MIMIR方法通过多图像增强有效解决了视觉捷径问题，为多模态知识库视觉问答提供了更真实的评估框架。
...
### [104] [Flow Straighter and Faster: Efficient One-Step Generative Modeling via MeanFlow on Rectified Trajectories](https://arxiv.org/abs/2511.23342)
*Xinxi Zhang,Shiwei Tan,Quang Nguyen,Quan Dao,Ligong Han,Xiaoxiao He,Tunyu Zhang,Alen Mrdovic,Dimitris Metaxas*

Main category: cs.CV

TL;DR: Re-MeanFlow提出了一种新的单步生成框架，通过单次回流步骤建模整流轨迹的平均速度场，避免了完全直线化轨迹的需求，在采样质量和训练效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有流模型存在采样效率问题：基于ODE的采样计算成本高，Rectified Flow需要多次回流迭代才能实现直线轨迹，MeanFlow在高度弯曲的流上训练时收敛慢且监督噪声大。

Method: 提出Rectified MeanFlow框架，仅使用单次回流步骤建模整流轨迹的平均速度场，无需完全直线化轨迹；引入简单的截断启发式方法减少残余曲率。

Result: 在ImageNet 64、256和512分辨率上的实验表明，Re-MeanFlow在样本质量和训练效率上均优于先前的单步流蒸馏和Rectified Flow方法。

Conclusion: Re-MeanFlow通过单次回流建模平均速度场，在保持高效训练的同时实现了更好的单步生成性能，为流模型的实用化提供了有效解决方案。
...
### [105] [Resolving Evidence Sparsity: Agentic Context Engineering for Long-Document Understanding](https://arxiv.org/abs/2511.22850)
*Keliang Liu,Zizhi Chen,Mingcheng Li,Jingqun Tang,Dingkang Yang,Lihua Zhang*

Main category: cs.CV

TL;DR: SLEUTH是一个多智能体框架，通过粗到细的处理流程解决长文档理解问题，利用检索器和四个协作智能体识别关键线索、过滤视觉证据、制定推理策略，最终生成证据密集的多模态上下文进行预测。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在单页文档任务上表现良好，但在处理长文档时效果下降。长文档中线索分散在多页和多模态中，冗长的输入冗余会损害模型判断。虽然检索增强生成通过筛选相关内容缓解了问题，但检索结果仍包含大量冗余。
...
Result: 当与先进的视觉语言模型主干配对时，SLEUTH在多个长文档基准测试中持续提升性能，达到了最先进的结果。消融研究验证了每个模块的有效性，并确认了分层细化范式的优势。
...
### [106] [GLOW: Global Illumination-Aware Inverse Rendering of Indoor Scenes Captured with Dynamic Co-Located Light & Camera](https://arxiv.org/abs/2511.22857)
*Jiaye Wu,Saeed Hadadan,Geng Lin,Peihan Tu,Matthias Zwicker,David Jacobs,Roni Sengupta*

Main category: cs.CV

TL;DR: GLOW是一个全局光照感知的逆向渲染框架，专门解决室内场景中光照与反射率之间的歧义问题，特别是在共置光源-相机设置下处理强相互反射、动态阴影、近场照明和移动高光等复杂情况。


<details>
  <summary>Details</summary>
Motivation: 室内场景的逆向渲染面临反射率与光照之间的歧义问题，尤其是在多个物体相互反射的情况下。虽然自然光照方法难以解决这一歧义，但共置光源-相机设置虽然能通过运动结构校准光照，却引入了强相互反射、动态阴影、近场照明和移动高光等新挑战，现有方法无法有效处理。

Method: GLOW整合了神经隐式表面表示和神经辐射缓存来近似全局光照，通过精心设计的正则化和初始化联合优化几何和反射率。还引入了动态辐射缓存来适应近场运动产生的尖锐光照不连续性，以及表面角度加权的辐射度量损失来抑制闪光灯捕捉中常见的镜面反射伪影。

Result: 实验表明，GLOW在自然光照和共置光照条件下，在材料反射率估计方面显著优于先前的方法。

Conclusion: GLOW通过全局光照感知的逆向渲染框架，有效解决了共置光源-相机设置下的复杂光照问题，在室内场景反射率估计方面取得了显著改进。
...
### [107] [CoordSpeaker: Exploiting Gesture Captioning for Coordinated Caption-Empowered Co-Speech Gesture Generation](https://arxiv.org/abs/2511.22863)
*Fengyi Fang,Sicheng Yang,Wenming Yang*

Main category: cs.CV

TL;DR: CoordSpeaker：首个通过手势理解与描述解决语义鸿沟的框架，实现协调的多模态控制语音手势生成


<details>
  <summary>Details</summary>
Motivation: 现有语音手势生成方法存在两个关键挑战：1）由于手势数据集缺乏描述性文本标注导致的语义先验鸿沟；2）难以实现协调的多模态控制。特别是忽略了文本驱动的非自发手势（如说话时鞠躬）。

Method: 1）通过新颖的手势描述框架弥合语义鸿沟，利用运动-语言模型生成多粒度描述性标题；2）基于此提出条件潜在扩散模型，采用统一跨数据集运动表示和分层控制去噪器，实现高度控制的协调手势生成。

Result: 实验表明，该方法能生成高质量手势，既与语音节奏同步，又与任意标题语义一致，相比现有方法实现了更优性能且效率更高。

Conclusion: CoordSpeaker首次探索手势理解与描述来解决手势生成中的语义鸿沟，同时提供了手势-文本双向映射的新视角，为协调的标题赋能语音手势合成提供了全面框架。
...
TL;DR: 本文提出DM³T框架，将多模态多目标跟踪中的特征融合重新定义为迭代特征对齐过程，通过跨模态扩散融合模块实现深度融合，在VT-MOT基准上达到41.7 HOTA的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多模态多目标跟踪对于自动驾驶系统至关重要，但可见光和热红外模态之间存在显著的非线性分布差异，简单的融合策略（如拼接或相加）容易导致模态冲突并降低跟踪精度。

Method: 提出DM³T框架，将多模态融合重新定义为迭代特征对齐过程：1) 跨模态扩散融合模块实现迭代交叉模态协调；2) 可插拔扩散精炼器增强统一特征表示；3) 分层跟踪器自适应处理置信度估计；4) 统一检测、状态估计和数据关联的在线跟踪框架。

Result: 在VT-MOT基准测试中达到41.7 HOTA，相比现有最先进方法相对提升1.54%，无需复杂后处理。

Conclusion: DM³T通过迭代特征对齐有效解决多模态融合中的非线性分布差异问题，实现了更准确和时序一致的目标轨迹，为多模态多目标跟踪提供了新的解决方案。
...
TL;DR: P2C框架将多模态提示学习从静态点表示重构为动态去噪任务，通过语义云分布提升视觉语言模型的泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态提示学习方法局限于优化单个静态点表示，这种方法存在脆弱性、容易在基础类别上过拟合，并且对新颖或模糊类别的泛化能力差。需要从点表示范式转向学习语义云分布来获得更鲁棒的泛化能力。
...
TL;DR: 提出一种通过识别查询中重要单词进行细粒度视频片段筛选的新方法，在视频时刻检索和高光检测任务中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有方法将整个文本查询和视频片段视为黑盒，忽略了单个单词的重要性，这阻碍了上下文理解。需要一种能够识别和优先处理查询中重要单词的细粒度方法。

Method: 通过多模态大语言模型（MLLMs）集成图像-文本场景理解，引入特征增强模块（FEM）捕捉查询中的重要单词，以及基于排名的过滤模块（RFM）根据这些重要单词的相关性迭代精炼视频片段。

Result: 大量实验表明，该方法在视频时刻检索（MR）和高光检测（HD）任务中显著优于现有最先进方法，取得了卓越性能。

Conclusion: 提出的通过识别重要单词进行细粒度片段筛选的方法有效提升了视频时刻检索和高光检测的性能，证明了关注查询中单个单词重要性的价值。
...
TL;DR: ViGG提出了一种基于视觉-几何相互引导的鲁棒RGB-D点云配准方法，通过几何引导抑制模糊团块，视觉引导确定搜索空间提取高质量对应关系，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法大多仅使用几何信息，而近期RGB-D配准方法主要关注特征融合或改进特征学习，这限制了图像信息的充分利用并阻碍了实际应用。

Method: 提出ViGG方法：1）以视觉-几何组合形式解决团块对齐问题，采用几何引导设计抑制模糊团块；2）提出视觉引导的几何匹配方法，利用视觉先验确定搜索空间，提取高质量、对噪声不敏感的对应关系。
...
### [113] [Artwork Interpretation with Vision Language Models: A Case Study on Emotions and Emotion Symbols](https://arxiv.org/abs/2511.22929)
*Sebastian Padó,Kerstin Thomas*

Main category: cs.CV

TL;DR: 该研究评估了当前视觉语言模型在识别艺术品中情感表达的能力，发现模型能较好识别图像内容和情感，但在抽象或象征性图像上表现不佳，且存在答案不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 情感是艺术表达的基本方面，但由于其抽象性，艺术品中的情感实现具有广泛谱系，且受历史变迁影响，分析需要艺术史专业知识。本研究旨在探索当前视觉语言模型能够检测情感表达的哪些方面。

Method: 采用案例研究方法，测试三个视觉语言模型（Llava-Llama和两个Qwen模型），向这些模型提出四组复杂度递增的问题（一般内容、情感内容、情感表达、情感符号），并进行定性专家评估。

Result: 视觉语言模型在识别图像内容方面表现惊人地好，通常也能识别图像描绘的情感和表达方式。模型在具体图像上表现最佳，但在高度抽象或高度象征性图像上失败。可靠识别符号仍然存在根本性困难。此外，模型继续表现出已知的大语言模型弱点，即对相关问题提供不一致的答案。

Conclusion: 当前视觉语言模型在识别艺术品情感表达方面取得了一定进展，但在处理抽象和象征性内容时仍有局限，且存在答案一致性问题，需要进一步改进。
...
TL;DR: NeuMatC是一个神经矩阵计算框架，通过利用参数维度上的低秩性和连续性，高效处理参数化矩阵运算任务，相比传统方法显著减少冗余计算。


<details>
  <summary>Details</summary>
Motivation: 在许多新兴应用（如无线通信和信号处理）中，矩阵运算需要在参数连续变化的矩阵上重复执行。传统方法独立处理每个矩阵运算，未能充分利用参数维度上的低秩性和连续性，导致大量冗余计算。

Method: 提出神经矩阵计算框架（NeuMatC），无监督地学习从参数到对应矩阵运算结果的低秩连续映射。训练完成后，NeuMatC仅需少量基本操作（如矩阵乘法和非线性激活）即可在任意参数下高效计算。

Result: 在合成和真实数据集上的实验表明，NeuMatC在无线通信应用中，相比广泛使用的NumPy基线，参数化矩阵求逆速度提升3倍以上，参数化SVD速度提升10倍以上，同时保持可接受的精度。

Conclusion: NeuMatC通过利用参数维度上的低秩性和连续性，为参数化矩阵运算任务提供了一种高效解决方案，显著减少了冗余计算，在保持精度的同时大幅提升了计算效率。
...
TL;DR: ReImage是一个基于神经水印的图像自恢复框架，通过将目标图像的乱序版本作为水印嵌入到图像自身中，实现被篡改区域的准确恢复。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成内容的快速增长，数字媒体的真实性受到关注。图像自恢复技术可以从被篡改版本中重建原始内容，有助于理解攻击者意图并恢复可信数据。然而现有方法往往无法准确恢复被篡改区域，达不到自恢复的主要目标。

Method: 提出ReImage框架：1) 设计生成器产生适合神经水印的水印；2) 将目标图像的乱序版本作为水印嵌入到图像自身；3) 引入图像增强模块来优化恢复的图像；4) 分析并解决乱序水印的关键限制，使其能有效用于自恢复。

Result: ReImage在多种篡改场景下实现了最先进的性能，始终能产生高质量的恢复图像。代码和预训练模型将在发表后发布。
...
TL;DR: 该研究深入评估了YOLOv8算法在条形码和二维码识别中的效率，通过在不同模型规模（Nano、Small、Medium）上进行训练和微调，实现了显著的检测精度提升。


<details>
  <summary>Details</summary>
Motivation: 研究旨在评估和优化YOLOv8算法在条形码和二维码识别任务中的性能，探索模型规模对物体检测准确性的影响，以推动计算机视觉技术的发展。

Method: 使用YOLOv8算法进行实时物体检测，在Kaggle数据集上对条形码和二维码检测进行大规模训练和微调。评估了三个不同规模的模型迭代：Nano、Small和Medium，重点关注精度、召回率和F1评估指标。

Result: 随着模型规模的增加，检测精度显著提升：Nano模型达到88.95%准确率，Small模型达到97.10%准确率，Medium模型达到94.10%准确率。结果显示模型缩放对物体识别性能有重要影响。

Conclusion: YOLOv8在物体检测领域取得了重大进展，模型缩放策略能有效提升识别准确性。该研究为深度学习计算机视觉技术的发展提供了重要见解，展示了YOLOv8作为物体检测里程碑技术的潜力。
...
TL;DR: DenoiseGS利用3D高斯泼溅技术进行突发去噪，通过高斯自一致性损失和加权频率损失解决噪声输入下的几何退化和细节丢失问题，比NeRF方法快250倍。
...
Method: 提出DenoiseGS框架，首次利用3D高斯泼溅进行突发去噪。引入高斯自一致性损失，用干净输入生成的高质量高斯点云正则化噪声输入的几何预测；提出对数加权频率损失，在频谱域加强监督，保留高频细节。

Result: DenoiseGS在噪声条件下的突发去噪和新视角合成任务上显著超越最先进的NeRF方法，同时实现250倍的推理速度提升。

Conclusion: DenoiseGS通过结合高斯自一致性损失和加权频率损失，有效解决了噪声输入下的几何退化和细节丢失问题，为突发去噪提供了高效且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在姿态驱动角色动画方面取得进展，但局限于空间对齐的参考-姿态对，无法处理参考姿态不对齐的情况。需要开发能够处理任意布局参考图像的通用动画框架。

Method: 1) 将训练重新定义为自监督外绘任务，将不同布局参考转换为统一的遮挡输入格式；2) 设计参考提取器提取全面的身份特征；3) 集成混合参考融合注意力处理不同分辨率和动态序列长度；4) 引入身份鲁棒姿态控制解耦外观与骨骼结构；5) 使用令牌替换策略实现连贯长视频生成。

Result: 大量实验表明，该方法在角色动画和图像姿态迁移任务上优于现有方法，能够处理任意布局的参考图像并生成高质量动画。

Conclusion: One-to-All Animation是一个统一的框架，成功解决了参考姿态不对齐的问题，实现了高保真角色动画和图像姿态迁移，代码和模型将开源。
...
TL;DR: FLEX-Seg是一个语义分割领域泛化框架，将扩散生成数据中的图像-掩码不对齐问题转化为学习机会，通过多尺度边界原型、不确定性边界强调和难度感知采样提升模型鲁棒性。
...
Method: FLEX-Seg包含三个核心组件：1) 粒度自适应原型，捕捉多尺度边界特征；2) 不确定性边界强调，基于预测熵动态调整学习重点；3) 难度感知采样，逐步聚焦于困难样本。框架利用固有不对齐而非强制严格对齐来学习鲁棒表示。

Result: 在五个真实世界数据集上的实验表明，相比最先进方法取得一致改进，在ACDC和Dark Zurich数据集上分别获得2.44%和2.63%的mIoU提升。

Conclusion: 研究验证了处理不完美合成数据的自适应策略能够带来更好的领域泛化性能。通过利用固有不对齐而非强制对齐，FLEX-Seg能够学习鲁棒表示并捕捉丰富的风格变化。
...
TL;DR: RobotSeg是一个用于图像和视频中机器人分割的基础模型，基于SAM 2构建，通过结构增强记忆关联器、机器人提示生成器和标签高效训练策略解决了机器人分割的挑战，在VRS数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 准确的机器人分割是机器人感知的基础能力，对VLA系统的视觉伺服、机器人中心数据增强、真实到仿真转换以及人机环境安全监控至关重要。尽管现代分割模型能力强大，但由于机器人形态多样性、外观模糊性、结构复杂性和快速形状变化，机器人分割仍然具有挑战性。

Method: 基于SAM 2基础模型构建RobotSeg，通过三个创新解决机器人分割的局限性：1) 结构增强记忆关联器，适应关节式机器人；2) 机器人提示生成器，减少对人工提示的依赖；3) 标签高效训练策略，减少逐帧训练掩码标注需求。同时构建了包含2.8k视频（138k帧）的VRS数据集。

Result: RobotSeg在图像和视频上都实现了最先进的性能，为机器人感知的未来发展建立了坚实基础。VRS数据集包含多样化的机器人形态和环境，为评估提供了全面基准。

Conclusion: RobotSeg通过结构感知、自动化和标签高效的解决方案，成功解决了机器人分割的挑战，为机器人感知领域提供了强大的基础模型，将推动VLA系统、数据增强、真实到仿真转换和安全监控等应用的发展。
...
TL;DR: 提出了一种新颖的分层多模态表示方法，用于3D场景推理，通过多视角图像和文本描述在输入空间显式对齐大型视觉语言模型，提升3D场景理解性能。
...
Method: 提出分层多模态表示方法：1）使用包含俯视图和四个方向视图的多视角图像确保全面场景覆盖；2）通过引用检测对象的3D坐标的文本描述捕捉空间关系；3）构建分层特征表示，将图像块级特征聚合为视图级和场景级表示，支持局部和全局场景上下文推理。

Result: 在情境化3D问答和通用3D问答基准测试中，实验结果表明该方法具有有效性。

Conclusion: 通过显式对齐输入空间的多模态表示方法，能够有效解决3D场景理解中数据稀缺和空间关系复杂的问题，提升3D推理性能。
...
TL;DR: 提出一种新颖的语义SLAM框架，通过主动的固有外观归一化和动态辐射平衡损失，实现光照不变性，在极端曝光条件下提升3D重建和语义分割的鲁棒性。
...
Method: 提出两个核心设计：1）固有外观归一化（IAN）模块，主动解耦场景的固有属性（如反照率）和瞬时光照，学习标准化的光照不变外观模型；2）动态辐射平衡损失（DRB-Loss），仅在图像曝光不佳时激活，直接在辐射场层面进行针对性优化。

Result: 在公开数据集上的评估表明，该系统在相机跟踪、地图质量、语义和几何精度方面达到了最先进的性能水平，展现出前所未有的鲁棒性。

Conclusion: IAN的主动不变性与DRB-Loss的被动校正相结合，赋予系统在极端光照条件下的强大鲁棒性，为语义SLAM系统提供了有效的光照不变性解决方案。
...
TL;DR: BlockVid是一个用于生成长达一分钟视频的块扩散框架，通过语义感知稀疏KV缓存、块强制训练策略、分块噪声调度等技术解决现有方法的长时误差累积问题，并在LV-Bench新基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成长达一分钟的视频对于开发世界模型和AI模拟器至关重要，但现有的半自回归（块扩散）方法面临两个主要挑战：KV缓存导致的长时误差累积，以及缺乏细粒度的长视频基准和连贯性感知指标。

Method: 提出了BlockVid框架，包含：1）语义感知稀疏KV缓存以减少误差传播；2）块强制训练策略；3）分块噪声调度和混洗以增强时间一致性。同时提出了LV-Bench细粒度长视频基准和新的连贯性评估指标。

Result: 在VBench和LV-Bench上的大量实验表明，BlockVid在生成高质量、连贯的一分钟视频方面持续优于现有方法。在LV-Bench上，相比最先进方法，VDE Subject指标提升22.2%，VDE Clarity指标提升19.4%。

Conclusion: BlockVid通过创新的块扩散框架解决了长视频生成中的误差累积问题，并提供了新的评估基准和指标，显著推进了分钟级视频生成技术的发展。
...
TL;DR: MultiBanana是一个专门评估多参考图像生成模型能力的基准数据集，覆盖了从参考数量变化到多语言文本参考等多种挑战性场景。
...
Method: 设计了MultiBanana基准，系统覆盖多参考特定问题：参考数量变化、参考间领域不匹配、参考与目标场景尺度不匹配、包含罕见概念的参考、多语言文本参考。

Result: 通过对多种文本到图像模型的分析，揭示了它们的优越性能、典型失败模式和改进领域，为多参考图像生成提供了标准化评估基础。

Conclusion: MultiBanana作为开放基准发布，将推动多参考图像生成领域的发展，建立公平比较的标准化基础，帮助识别模型能力边界和改进方向。
...
TL;DR: 提出频谱弱化框架，用于视觉自回归模型的无条件生成增强，无需重新训练或修改架构，通过频谱域可控弱化实现高质量生成和条件对齐。


<details>
  <summary>Details</summary>
Motivation: 现有CFG方法主要针对扩散模型，缺乏适用于视觉自回归模型的通用指导机制。需要一种无需重新训练、不依赖特定条件或架构修改的方法来提升自回归模型的生成质量和条件对齐能力。

Method: 提出频谱弱化框架：1) 利用可逆频谱变换保持信息完整性；2) 在通道维度选择性保留部分频谱实现可控信息减少；3) 引入两种频谱重归一化策略确保数值稳定性；4) 避免扩散模型的结构约束，适用于离散和连续自回归模型。

Result: 在离散和连续自回归模型上进行广泛实验，涵盖文本和类别条件。结果表明该方法能实现高质量的无条件生成，同时在条件生成中保持强大的提示对齐能力。

Conclusion: 频谱弱化框架为视觉自回归模型提供了一种有效的指导机制，无需重新训练或架构修改，在无条件生成质量和条件对齐方面均表现出色，突破了现有方法对扩散模型的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 随着数字身份系统扩展，生物特征认证安全性日益重要。虹膜识别因其独特且稳定的纹理模式具有高可靠性。深度学习特别是Vision Transformers (ViT) 在视觉识别方面取得进展，但优化器选择对基于ViT的生物特征系统影响研究不足。

Method: 评估不同优化器对ViT在虹膜识别任务中准确性和稳定性的影响

Result: 提供了关于优化器选择如何影响ViT在虹膜识别中性能的见解，为增强生物特征识别模型的鲁棒性提供指导
...
TL;DR: MrGS是基于3D高斯泼溅的多模态辐射场，同时重建RGB和热红外3D场景，通过正交特征提取和物理原理建模热传导特性


<details>
  <summary>Details</summary>
Motivation: 现有的NeRF和3DGS方法主要关注RGB场景重建，而结合热红外图像的多模态渲染研究不足，且往往忽略了热传导和朗伯特性等独特的热学特性

Method: 1) 通过正交特征提取从单一外观特征中分离RGB和热相关信息；2) 根据各模态的朗伯反射程度采用视图依赖或独立的嵌入策略；3) 集成傅里叶热传导定律建模高斯之间的热传导强度插值；4) 应用斯特藩-玻尔兹曼定律和平方反比定律构建深度感知热辐射图

Result: 实验结果表明，MrGS能够实现高保真度的RGB-T场景重建，同时减少了所需的高斯数量

Conclusion: MrGS成功地将物理原理融入3D高斯泼溅框架，实现了RGB和热红外场景的同步高质量重建，为多模态辐射场研究提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有动态高斯泼溅方法在稀疏输入视图下性能显著下降，限制了实际应用。问题源于4D几何在输入视图减少时的非连贯学习。

Method: 提出动态一致性检查策略减少多视图立体估计的不确定性；采用全局-局部深度正则化方法从单目深度中提取时空一致的几何信息；将几何一致性融入4D高斯泼溅框架。

Result: 在N3DV和Technicolor数据集上验证了有效性，渲染质量优于RF-DeRF（最新稀疏输入动态视图合成方法）2.62dB PSNR，优于原始4DGS 1.58dB PSNR，且保持实时效率。

Conclusion: GC-4DGS通过几何一致性增强，在稀疏输入视图下实现了高质量的动态场景渲染，同时保持实时性能，适用于资源受限的物联网边缘设备部署。
...
TL;DR: GOATex是一种基于扩散模型的3D网格纹理生成方法，通过遮挡感知的分层纹理生成技术，能够同时为网格的外部和内部表面生成高质量的无缝纹理。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注可见区域纹理生成，但缺乏处理被遮挡内部表面的机制，导致纹理不完整和可见接缝问题。

Method: 提出基于命中层级概念的遮挡感知纹理框架：1) 通过多视角光线投射将网格面划分为有序可见性层级；2) 采用两阶段可见性控制策略逐步揭示内部区域；3) 使用预训练扩散模型为每层生成纹理；4) 提出软UV空间混合技术基于视图相关可见性置信度无缝融合各层纹理。

Result: 实验结果表明GOATex在可见和被遮挡表面均能生成无缝、高保真纹理，性能优于现有方法，且无需对预训练扩散模型进行昂贵的微调。

Conclusion: GOATex通过创新的遮挡感知分层纹理生成框架，解决了3D网格内外表面纹理生成的完整性问题，实现了对分层外观的细粒度控制，为3D内容创作提供了更强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 在3D场景重建中，特别是使用NeRF技术时，不同输入图像对最终重建质量的贡献并不相同。真实场景中的图像往往存在质量差异、遮挡和瞬态物体等问题，导致图像效用不均。需要一种方法来量化每张图像对NeRF重建的具体贡献。

Method: 提出基于重建质量指标（PSNR和MSE）的图像贡献度评估方法。通过计算每张图像对整体重建质量的影响来量化其贡献，并在训练过程中移除低贡献图像来验证方法的有效性。

Result: 通过移除低贡献图像并测量重建保真度的变化，验证了所提方法的有效性。该方法能够准确识别对NeRF重建贡献较小的图像。

Conclusion: 该方法为NeRF重建中的图像贡献度量化提供了有效工具，有助于优化数据使用、提高重建效率，并为数据估值和货币化提供了技术基础。
...
TL;DR: REP是一种轻量级多模态学习方法，通过特征重放和私有-共享特征解耦策略，有效缓解缺失模态导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么计算成本高（合成缺失模态），要么只利用相邻层特征而忽略长距离上下文信息，这些信息在模态缺失时可能提供额外的容错能力。
...
TL;DR: ENSEL系统通过集成多种深度学习模型，提高了皮肤病变检测的准确性和响应速度，为数字医疗发展做出贡献


<details>
  <summary>Details</summary>
Motivation: 韩国数据3法案修订和COVID-19影响推动了数字医疗市场发展，但特应性皮炎等皮肤疾病缺乏客观诊断方法，现有研究使用的高质量皮肤镜图像在临床实践中难以获取，且现有系统需要兼顾准确性和快速响应时间
...
Conclusion: ENSEL系统有助于皮肤病变的客观诊断，促进了数字医疗的进步，为解决临床实践中皮肤疾病诊断的挑战提供了有效方案
...
TL;DR: NumeriKontrol是一个基于指令的图像编辑框架，通过连续标量值和常用单位实现精确的属性调整控制，解决了文本指令在编辑强度控制上缺乏精确性的问题。


<details>
  <summary>Details</summary>
Motivation: 基于文本指令的图像编辑虽然直观，但缺乏对编辑强度的精确控制。用户需要能够通过连续数值和常用单位来精确调整图像属性的方法。

Method: 1. 通过有效的数值适配器编码数字编辑尺度；2. 以即插即用方式将数值注入扩散模型；3. 采用任务分离设计支持零样本多条件编辑；4. 从高质量渲染引擎和DSLR相机合成精确训练数据；5. 构建Common Attribute Transform数据集提供高质量监督。

Result: NumeriKontrol在广泛的属性编辑场景中实现了准确、连续且稳定的尺度控制，能够作为简单而强大的交互式编辑工作室使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在数学推理方面取得显著进展，但视觉信息对推理的真实贡献不明确。现有基准报告了整体性能，但很少分离图像模态的作用，不清楚模型是否真正利用视觉理解还是仅依赖语言先验

Method: 提出MathSight基准，每个问题包含多个视觉变体（原始、手绘、照片拍摄）和纯文本条件进行控制比较，通过实验分析视觉输入的作用

Result: 实验发现：视觉信息的贡献随着问题难度增加而减少；无图像输入的Qwen3-VL甚至超越了其多模态变体和GPT-5
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器推理时采用序列并行可降低延迟，但当模型使用块级稀疏注意力时，由于注意力头间稀疏度差异和稀疏掩码中密集块分布不规则，导致严重的负载不平衡问题

Method: 提出db-SP稀疏感知序列并行技术：1）定义稀疏不平衡比量化不平衡程度；2）采用双级分区方法在头和块级别实现近乎完美的负载平衡；3）运行时动态确定头和块维度的并行度以适应去噪步骤和层间的稀疏模式变化

Result: 实验结果显示，db-SP相比最先进的序列并行方法，平均实现1.25倍端到端加速和1.40倍注意力特定加速

Conclusion: db-SP有效解决了扩散变换器序列并行中的负载不平衡问题，显著提升了推理效率，为大规模视觉生成模型的高效部署提供了实用解决方案
...
TL;DR: 本文提出了一种基于情感描述生成的情感图像分类新方法ACIEC，通过纯文本分析解决情感鸿沟问题，在多个基准测试中取得优异结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉模型的方法受到"情感鸿沟"的限制，无法有效利用预训练知识进行图像情感分类。心理学研究表明语言具有高变异性、丰富信息量，并能有效消除情感鸿沟，因此提出基于纯文本的情感图像分类方法。
...
Conclusion: 通过基于情感描述生成的方法，ACIEC成功解决了图像情感分类中的情感鸿沟问题，为这一领域提供了新的有效解决方案。
...
TL;DR: DualCamCtrl是一个用于相机控制视频生成的端到端扩散模型，通过双分支框架同时生成相机一致的RGB和深度序列，使用语义引导互对齐机制融合两种模态，显著降低了相机运动误差。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用射线条件表示相机姿态，但缺乏足够的场景理解和几何感知能力，无法充分解耦外观和几何建模，导致生成的视频与指定相机轨迹的一致性不足。

Method: 提出双分支框架同时生成相机一致的RGB和深度序列，引入语义引导互对齐机制进行RGB-深度融合，通过语义引导和相互增强的方式协调两种模态。

Result: 实验表明DualCamCtrl实现了更一致的相机控制视频生成，相机运动误差相比先前方法降低了40%以上，深度和相机姿态在不同去噪阶段具有不同的影响作用。

Conclusion: DualCamCtrl通过双分支框架和语义引导互对齐机制，更好地解耦了外观和几何建模，生成了更忠实于指定相机轨迹的视频，为相机控制视频生成提供了有效解决方案。
...
Method: 提出InstanceV框架，包含：1）实例感知掩码交叉注意力机制，利用实例级定位信息在指定空间位置生成正确属性的实例；2）共享时间步自适应提示增强模块，以参数高效的方式连接局部实例与全局语义；3）空间感知无条件指导，缓解训练和推理中小实例消失的问题。

Result: InstanceV在实例级视频生成中实现了卓越的实例级可控性，在定性和定量评估中，在通用质量和实例感知指标上都超越了现有最先进模型。

Conclusion: InstanceV通过创新的实例级控制机制和全局一致性模块，显著提升了文本到视频生成的细粒度可控性，并提出了InstanceBench基准用于更全面的实例级视频生成评估。
...
TL;DR: 提出多阶段文档矫正框架，通过渐进式分解解决复杂变形，包含全局仿射变换、几何变形矫正和内容感知迭代处理，并引入新评估指标


<details>
  <summary>Details</summary>
Motivation: 现实场景中文档矫正面临巨大挑战，主要源于相机视角变化和物理变形。现有方法难以处理复杂的复合变形，且评估指标存在局限性

Method: 采用多阶段渐进式框架：1) 全局仿射变换矫正相机视角变形；2) 矫正纸张卷曲折叠导致的几何变形；3) 内容感知迭代过程消除细粒度内容变形。同时提出两种增强评估指标

Result: 在多个挑战性基准测试中达到新的最先进性能，AAD指标显著降低14.1%-34.7%，在现实应用中表现出色

Conclusion: 通过分解复杂变形并渐进式处理的方法有效解决了现实场景中的文档矫正问题，提出的评估指标为领域提供了更准确的评估标准
...
<details>
  <summary>Details</summary>
Motivation: 现有视频时序定位模型假设查询总是相关的，即使查询与视频无关也会预测一个时间段。现有方法只能拒绝完全无关的查询，无法处理语义相似但实际不相关的硬无关查询。

Method: 提出Refusal-Aware Reinforcement Fine-Tuning (RA-RFT)方法，基于Group Relative Policy Optimization框架，整合格式、拒绝IoU、解释和查询修正四个奖励目标，提升相关判别和细粒度语义推理能力。同时构建Hard-Irrelevant VTG数据集支持训练。

Result: 方法在多种相关感知VTG场景中有效，包括硬无关VTG、简单重排RA-VTG和人工标注RA-VTG设置。方法具有可扩展性，可应用于多种LVLM-based VTG模型。

Conclusion: RA-RFT方法能有效拒绝视频时序定位中的硬无关查询，通过强化学习微调和多奖励目标设计提升了模型对语义相似但不相关查询的识别能力，为解决VTG中的无关查询问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的CLIP等对比视觉语言预训练框架在零样本任务上表现出色，但将单个文本标记与特定图像区域对齐的方法难以捕捉跨多个图像区域的组合语义。需要解决组合语义跨越多个图像区域的挑战。

Method: 提出PowerCLIP框架，通过幂集对齐来优化区域到短语的对齐，最小化图像区域幂集和文本解析树之间的损失。为了解决幂集构建带来的指数计算成本，引入了高效的非线性聚合器(NLAs)，将复杂度从O(2^M)降低到O(M)，同时以任意精度近似精确损失值。

Result: 广泛的实验表明，PowerCLIP在零样本分类和检索任务上优于最先进的方法，证明了该方法的组合性和鲁棒性。

Conclusion: PowerCLIP通过幂集对齐有效增强了细粒度组合理解能力，在保持计算效率的同时显著提升了零样本性能，为视觉语言理解提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本的3D编辑方法通常采用2D模型逐视角处理，然后进行迭代的2D-3D-2D更新，这种方法不仅耗时，而且容易因不同视角编辑信号的平均化导致结果过度平滑

Method: 提出ViP3DE方法：1) 基于单个编辑视角条件化视频生成模型，直接生成其他一致编辑视角用于3D更新；2) 提出运动保持噪声混合技术，使视频模型能在预定相机位姿生成编辑视角；3) 引入几何感知去噪，通过整合3D几何先验增强多视角一致性

Result: 大量实验表明，ViP3DE即使在单次前向传播中也能实现高质量的3D编辑结果，在编辑质量和速度方面显著优于现有方法

Conclusion: 通过利用视频生成模型的时序一致性先验，ViP3DE能够高效实现多视角一致的3D编辑，避免了传统迭代方法的局限性
...
Conclusion: 通过解锁几何模型的潜力并引入几何对齐机制，GeoWorld显著提升了图像到3D场景生成的几何一致性和视觉质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前病理学基础模型参数庞大（超过十亿参数），产生高维嵌入，在计算资源有限的研究或临床环境中应用受限，需要更高效的模型部署方案。

Method: 采用多教师蒸馏框架，结合RADIO蒸馏和Matryoshka表示学习技术，通过知识蒸馏减少模型大小，同时支持可调节的嵌入维度。

Result: 在十个公共病理学基准测试中，Pathryoshka将模型大小减少86-92%，性能与原始大模型相当；相比同类大小的单教师蒸馏模型，准确率中位数提升7.0%。
...
Method: 通过物体检测和已知CAD模型姿态估计语义描述工业场景，实现实时数字孪生渲染；提供可扩展的分层标注策略，统一姿态标注与逻辑结构缺陷标注。
...
TL;DR: 本文首次探索指令调优提升LLM表格数据生成能力，仅用7K指令在A100上训练6小时，性能达到GPT-4o水平
...
Method: 首先创建高质量的表格数据指令数据集，然后使用开源LLM（Llama3.1-8B-Instruct）在该数据集的训练集上进行指令调优，仅使用7K指令在A100 GPU上训练不到6小时。

Result: 实验结果表明，通过使用高质量数据集和有限的指令调优（7K指令，A100 GPU，6小时训练），在表格数据生成任务上达到了与最强大的商业LLM GPT-4o相当的性能水平。

Conclusion: 本研究证明了在有限数据和计算资源条件下，通过高质量的指令数据集和高效的指令调优，可以显著提升LLM的表格数据生成能力，达到与顶级商业模型相当的性能。
...
TL;DR: DAONet-YOLOv8是一种改进的YOLOv8变体，专门用于复杂茶园环境中的病虫害检测，通过双注意力融合、遮挡感知检测头和动态合成卷积等创新，显著提升了检测精度。
...
Conclusion: DAONet-YOLOv8通过创新的注意力机制、遮挡感知设计和动态卷积，有效解决了茶园复杂环境下的病虫害检测难题，在精度和效率方面均优于主流检测模型。
...
TL;DR: SynthFun3D：首个基于任务的3D场景合成方法，通过家具资产数据库生成可完成特定动作的3D室内环境，自动识别功能元素，为数据稀缺的3D功能理解提供低成本大规模数据生成方案。


<details>
  <summary>Details</summary>
Motivation: 3D功能理解需要识别场景中完成特定动作的功能元素，但真实世界数据收集和标注成本高昂，导致数据稀缺，限制了相关应用的发展。

Method: 提出SynthFun3D方法：基于动作描述，利用带有部件级标注的家具资产数据库生成3D室内环境；通过推理自动识别并检索正确功能元素的3D掩码，实现高质量标注数据的大规模生成。

Result: 用户研究表明，相比其他方法，SynthFun3D在场景-提示一致性方面表现更好；定量结果显示，生成的数据可以替代真实数据且性能损失很小，或补充真实数据以提升性能。

Conclusion: SynthFun3D为数据稀缺的3D应用提供了低成本、可扩展的解决方案，能够大规模生成高质量标注数据，有效缓解3D功能理解领域的数据瓶颈问题。
...
Method: 提出MRRE方法，在推理时按顺序注入两个预计算向量：跨语言推理增强向量（将非英语推理表征导向英语空间）和目标语言输出锚定向量（恢复目标语言分布以保持语言一致性）。

Result: 在6个先进模型和4个推理基准上的实验表明，MRRE平均提升非英语推理能力5.48%，在低资源语言（泰语和斯瓦希里语）上最高提升7.54%，同时将输入输出语言一致性提高3.78%。

Conclusion: MRRE是一种有效的训练免费方法，能够显著提升多语言推理能力，特别是在低资源语言上，同时保持语言一致性，为解决多语言公平性问题提供了新思路。
...
TL;DR: 对比多种领域随机化和领域适应技术，发现基于感知哈希的简单特征对齐方法在合成数据到真实世界迁移中表现最佳，优于复杂的生成式AI方法。


<details>
  <summary>Details</summary>
Motivation: 工业与机器人应用中，数据生成和标注成本高昂，合成渲染是潜在解决方案，但需要解决模拟到真实的差距问题，且通常需要专家干预。

Method: 系统评估了领域随机化(DR)和领域适应(DA)技术，包括基于特征的方法、生成式AI和经典渲染方法。重点关注低层和高层特征对齐，以及基于真实上下文提示的受控扩散DA方法。在两个数据集上验证：专有工业数据集（汽车和物流）和公共机器人数据集。

Result: 如果有足够多样性的渲染数据作为种子，基于亮度和感知哈希过滤的简单特征方法在准确性和资源效率上优于复杂的生成式AI方法。感知哈希表现最佳，在工业和机器人数据集上的mAP50分别为98%和67%。生成式AI方法在数据生成上时间开销大，且未带来性能提升。

Conclusion: 研究提供了有效缩小模拟到真实差距的实用见解，表明简单特征对齐方法（特别是感知哈希）能够实现仅用合成数据训练模型的高真实世界性能，为工业应用提供了高效解决方案。
...
TL;DR: FACT-GS是一种频率对齐的复杂感知纹理高斯泼溅框架，通过自适应采样密度分配来改进纹理参数化，在相同参数预算下恢复更清晰的高频细节。


<details>
  <summary>Details</summary>
Motivation: 传统基于纹理的高斯泼溅使用均匀采样网格，无论局部视觉复杂度如何都分配相同的采样密度，导致高频区域采样不足、平滑区域容量浪费，造成模糊外观和细节丢失。
...
Conclusion: FACT-GS通过频率感知的自适应采样密度分配，有效解决了传统均匀纹理高斯泼溅的采样效率问题，在保持实时性能的同时显著提升了视觉质量。
...
TL;DR: 提出基于人类颜色视觉基本现象学的变分颜色对比度增强方法，满足感知启发能量函数的特定要求，并优化计算复杂度


<details>
  <summary>Details</summary>
Motivation: 现有基于人类颜色视觉现象学的显式颜色校正算法在处理图像特征（如对比度和色散）时的行为难以表征，需要一种更系统的方法来设计感知启发的颜色增强算法

Method: 提出变分颜色对比度增强公式，定义感知启发能量函数需满足的基本要求，识别出满足所有要求的函数类，特别提出三种具有基本意义的显式函数，使用梯度下降法计算最小值，并将算法复杂度从O(N²)降低到O(NlogN)

Result: 建立了满足感知启发要求的能量函数类，提出了三种具体的函数形式，展示了与现有模型的相似性和差异，并实现了计算效率的显著提升

Conclusion: 基于人类颜色视觉基本现象学的变分方法为颜色对比度增强提供了系统框架，既能满足感知启发要求，又能通过优化算法实现高效计算
...
TL;DR: 本文提出了GeoSeg-1M——首个百万规模的遥感指令驱动分割数据集，包含590K图像、117个类别和1.1M图像-掩码-指令三元组，并建立了评估基准GeoSeg-Bench和统一框架UniGeoSeg。


<details>
  <summary>Details</summary>
Motivation: 当前遥感指令驱动分割方法存在任务表述碎片化和指令数据有限的问题，限制了模型的理解和泛化能力，需要大规模数据集和统一框架来推动该领域发展。

Method: 通过自动掩码过滤和指令生成流水线构建GeoSeg-1M数据集；创建GeoSeg-Bench评估基准；提出UniGeoSeg统一框架，包含任务感知文本增强、潜在知识记忆和渐进训练策略。

Result: UniGeoSeg在GeoSeg-Bench和多个公共基准测试中达到最先进性能，并展现出强大的零样本泛化能力。数据集和源代码已开源。

Conclusion: GeoSeg-1M数据集、GeoSeg-Bench基准和UniGeoSeg框架为遥感指令驱动分割提供了重要基础设施，推动了该领域向更通用、可访问的方向发展。
...
TL;DR: 提出Markov-VAR模型，将视觉自回归建模从全上下文依赖重构为非全上下文马尔可夫过程，通过马尔可夫尺度预测和滑动窗口压缩历史信息，显著提升性能同时大幅降低计算开销。
...
Method: 将VAR重构为非全上下文马尔可夫过程，提出Markov-VAR。采用马尔可夫尺度预测：将每个尺度视为马尔可夫状态，引入滑动窗口将某些先前尺度压缩为紧凑的历史向量，以补偿因非全上下文依赖导致的历史信息损失。将历史向量与马尔可夫状态结合形成代表性的动态状态，在马尔可夫过程下演化。

Result: 在ImageNet上的实验表明，与VAR相比，Markov-VAR在256×256分辨率下将FID降低10.5%，在1024×1024分辨率下将峰值内存消耗减少83.8%。模型极其简单但高度有效。
...
TL;DR: 提出SimScale框架，通过神经渲染和反应式环境生成大规模未见驾驶场景的合成数据，结合伪专家轨迹生成和协同训练策略，显著提升自动驾驶规划方法的鲁棒性和泛化能力。
...
Method: 1. 基于神经渲染和反应式环境的可扩展仿真框架，通过扰动自车轨迹生成高保真多视角观测；2. 为模拟状态开发伪专家轨迹生成机制提供动作监督；3. 采用真实世界和模拟样本的简单协同训练策略。

Result: 在挑战性真实世界基准测试中，各种规划方法在鲁棒性和泛化性方面显著提升：navhard上提升+6.8 EPDMS，navtest上提升+2.9。仅增加模拟数据即可平滑扩展策略改进，无需额外真实世界数据。

Conclusion: SimScale框架通过合成大规模未见驾驶场景数据有效解决了自动驾驶数据多样性不足的问题，协同训练策略显著提升规划性能，揭示了伪专家设计和不同策略架构的扩展特性等重要发现。
...
TL;DR: 该研究提出了DEAL-300K数据集，用于定位基于扩散模型的图像编辑区域，并开发了结合视觉基础模型和多频提示调优的定位框架。


<details>
  <summary>Details</summary>
Motivation: 现有的基准主要关注生成图像的二元检测或手动编辑区域的定位，未能反映扩散编辑平滑融入原始内容的特性，需要专门的数据集和方法来定位扩散编辑区域。
...
Conclusion: DEAL-300K数据集和提出的定位框架为扩散编辑区域定位研究提供了重要资源，能够有效捕捉编辑区域的语义和频域特征。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态统一模型面临的关键挑战是如何在单一tokenizer中统一理解、生成和重建表示。先前研究主要在双编码器范式下进行，分别使用不同编码器处理理解和生成任务，或通过对比损失平衡语义表示和低级特征。需要一种能够同时支持连续语义理解和离散视觉生成的统一表示方法。

Method: VQRAE基于预训练视觉基础模型构建，采用对称ViT解码器，实施两阶段训练策略：第一阶段冻结编码器，学习高维语义VQ码本，以像素重建为目标；第二阶段通过自蒸馏约束联合优化编码器。该方法利用高维码本量化语义编码器，与传统图像重建中使用的低维码本形成对比。

Result: VQRAE在多个视觉理解、生成和重建基准测试中表现出竞争力，其语义VQ码本在1536维度下能达到100%利用率。该方法在自回归范式下展现出良好的扩展性，能够同时保持多模态理解能力、生成兼容的离散token和细粒度重建能力。

Conclusion: VQRAE成功实现了在单一tokenizer中统一连续语义理解和离散视觉生成的目标，通过高维语义VQ码本量化语义编码器，为构建统一多模态模型提供了新的技术路径，展现出在自回归范式下的良好扩展潜力。
...
TL;DR: MANTA是一个针对水下目标跟踪的物理信息框架，通过双正对比学习策略和物理增强的多阶段跟踪管道，解决了水下环境中的外观失真问题，在多个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 水下目标跟踪面临波长相关衰减和散射的挑战，这些物理效应在不同深度和水况下严重扭曲目标外观。现有的基于陆地数据训练的跟踪器无法泛化到这些物理驱动的退化情况。

Method: 提出MANTA框架，采用双正对比学习策略，将时间一致性与Beer-Lambert增强相结合，生成对时间和水下失真都具有鲁棒性的特征。同时引入多阶段管道，通过基于运动的跟踪与物理信息的二次关联算法相结合，整合几何一致性和外观相似性来处理遮挡和漂移问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频和图像到视频模型虽然能生成视觉上吸引人的动态视频，但缺乏将运动与内容分离的显式表示，限制了内容创作者的应用。这些模型通常在运动保真度和提示遵循之间进行权衡，要么过度拟合源结构，要么偏离描述的动作。

Method: DisMo通过图像空间重建目标直接从原始视频数据学习抽象运动表示。该表示是通用的，独立于外观、物体身份或姿态等静态信息。该方法将运动语义与外观解耦，可以通过轻量级适配器与任何现有视频生成器结合。

Result: DisMo实现了开放世界的运动迁移，允许在语义无关的实体之间传输运动，即使是在完全不同的类别之间。在零样本动作分类任务中，在Something-Something v2和Jester等基准测试上持续优于V-JEPA等最先进的视频表示模型。

Conclusion: DisMo提供了一种有效的运动表示学习范式，能够解耦运动与内容，支持准确的运动迁移和下游运动理解任务，并且能够轻松利用未来视频模型的进步。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式世界模型存在动作模式僵化和标注成本高的问题，限制了其对多样化游戏交互和玩家驱动动态的建模能力，需要更灵活的交互方式。

Method: 定义了交互式视频数据概念，开发了自动化流程将大规模非结构化文本-视频对转换为因果对齐的交互数据集；基于14B图像到视频的混合专家基础模型，采用文本驱动交互注入机制，实现对摄像机运动、角色行为和环境动态的细粒度控制。

Result: 模型生成了时间一致且因果基础的交互式游戏视频，能够忠实响应"开门"、"绘制火炬"或"触发爆炸"等多样化和自由形式的用户指令；引入了交互重点基准InterBench进行全面评估。

Conclusion: Hunyuan-GameCraft-2通过指令驱动的交互范式，为生成式游戏世界建模提供了更灵活、语义丰富的交互能力，突破了传统固定键盘输入的限制。
...
TL;DR: 本文研究了在对象中心数据（多视角图像或3D模型）有限的情况下，如何通过四种数据合成方法微调目标检测模型，以扩展其对新物体类别的检测能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习目标检测方法扩展到新物体类别需要大量标注数据，成本高且耗时，特别是对于长尾类别。本文旨在解决在仅有有限对象中心数据的情况下，如何有效提升模型对新类别的检测性能。

Method: 提出了对象中心数据设置，并系统评估了四种数据合成方法：基于简单图像处理技术、3D渲染和图像扩散模型，利用对象中心数据合成具有不同上下文一致性和复杂度的真实杂乱图像。

Result: 这些方法在数据受限的实验设置中显著提升了模型性能，实现了在真实世界数据中的类别级泛化能力。

Conclusion: 在对象中心数据有限的情况下，通过数据合成方法可以有效扩展目标检测模型对新物体类别的检测能力，为解决数据稀缺问题提供了有效途径。
...
TL;DR: VGT是一种新的视觉生成调优范式，通过高效调优预训练视觉语言模型，激发其内在的视觉生成能力，显著降低对齐成本并加速自回归建模收敛速度（20倍加速）。


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型通过预训练获得了与语言对齐的复杂视觉表示，但这些为多模态理解任务优化的表示是否具有内在的视觉生成潜力尚未充分探索。本文旨在探索如何激发VLMs的视觉生成能力。

Method: 提出VGT（视觉生成调优）范式，摒弃为扩散变换器设计的纠缠像素级VAEs，通过将预训练VLMs的语义编码器与像素解码器的潜在表示对齐，构建VGT-AE。对预训练好的VLMs进行高效的视觉生成调优。

Result: 在图像重建任务中，在28倍压缩比下达到26.67 PSNR和0.50 rFID，优于专门的VAEs；在视觉生成任务中，在自回归模型中达到最先进水平，GenEval得分为0.77，DPG-Bench得分为78.73。实现了20倍的收敛加速。
...
TL;DR: AnyTalker是一个多人生成框架，采用可扩展的多流处理架构，通过身份感知注意力机制处理身份-音频对，仅需单人视频训练即可学习多人说话模式，少量真实多人片段优化交互性。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动的多人说话视频生成面临两个主要挑战：多样多人数据收集成本高，以及驱动多个身份保持连贯交互性困难。现有方法在这两方面存在局限。

Method: 提出可扩展的多流处理架构，扩展Diffusion Transformer的注意力块，引入新颖的身份感知注意力机制迭代处理身份-音频对，支持任意规模的身份驱动。训练流程仅依赖单人视频学习多人说话模式，仅需少量真实多人片段优化交互性。

Result: AnyTalker在唇部同步、视觉质量和自然交互性方面表现优异，在数据成本和身份可扩展性之间取得了良好平衡。还贡献了专门设计的评估指标和数据集来评估多人视频的自然性和交互性。

Conclusion: AnyTalker通过创新的架构设计和训练策略，有效解决了多人视频生成中的数据成本和交互性挑战，实现了高质量的多人说话视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视频理解中存在语义瓶颈：视频编码后推理完全在文本中进行，将视觉输入视为静态上下文，无法重新观看、重新聚焦或验证证据，导致在需要细粒度时空理解的任务上推理浅层

Method: 提出交互式视频推理范式，模型Video CoM通过链式操作进行迭代视觉动作来收集和精炼证据；构建18K指令调优数据集Video CoM Instruct；通过强化学习优化操作策略，引入推理感知的组相对策略优化和步骤级推理奖励

Result: 在九个视频推理基准测试中取得强劲结果，平均性能比最新SOTA模型提升3.6%，仅使用25K SFT和3K GRPO视频样本训练，显著少于可比的大规模模型；消融研究表明推理感知奖励提高了准确性和可解释性

Conclusion: 交互式视频推理范式通过让模型主动操作视频内容来克服传统方法的语义瓶颈，链式操作和推理感知奖励机制显著提升了视频理解的深度和准确性
...
TL;DR: 论文提出Video R2模型，通过强化学习增强视频推理的时序对齐和逻辑一致性，解决现有多模态大语言模型在动态视觉内容推理中存在的逻辑不一致和视觉证据依赖不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在动态视觉内容推理中存在两个主要问题：1）生成的推理痕迹虽然看起来有说服力，但实际上逻辑不一致；2）推理过程过度依赖语言先验而非视觉证据。这些问题导致模型推理不可靠且难以信任。

Method: 提出两阶段后训练方法：1）时间戳感知的监督微调；2）基于新提出的时序对齐奖励（TAR）的组相对策略优化（GRPO）。通过强化学习同时提升时序精度和推理一致性。

Result: 在11个视频推理基准测试中，Video R2模型在Think Answer Consistency（TAC）、Video Attention Score（VAS）和准确率方面均取得显著提升，证明时序对齐和推理一致性的改进能带来更准确可靠的视频理解。

Conclusion: 通过强化学习增强时序对齐和推理一致性的方法能有效提升多模态大语言模型在动态视觉内容推理中的性能，使模型更加准确和可信。所有代码、数据集和模型将开源。
...
### [167] [Artificial intelligence for methane detection: from continuous monitoring to verified mitigation](https://arxiv.org/abs/2511.21777)
...
TL;DR: MARS-S2L是一个机器学习模型，利用公开的多光谱卫星图像检测甲烷排放，每两天提供高分辨率检测，能够识别78%的排放羽流，误报率8%，已部署运行并向20个国家发出1015次通知，实现了可验证的甲烷减排。


<details>
  <summary>Details</summary>
Motivation: 甲烷是强效温室气体，少数大型点源排放占不成比例的高份额，针对这些相对较少的站点可实现显著减排，但目前大规模检测和归因大型排放仍具挑战性。

Method: 开发MARS-S2L机器学习模型，基于超过80,000张手动标注图像的数据集进行训练，利用公开可用的多光谱卫星图像检测甲烷排放，每两天提供高分辨率检测。

Result: 模型在697个未见站点上检测到78%的排放羽流，误报率8%；已部署运行并向20个国家的利益相关者发出1015次通知，实现了6个持续排放源的验证性永久减排，包括利比亚一个先前未知的站点。
...
### [168] [Dynamical Implicit Neural Representations](https://arxiv.org/abs/2511.21787)
...
Method: 提出DINR框架，将特征演化建模为连续时间动力系统，而不是传统的离散层堆叠。通过理论分析（Rademacher复杂度和神经正切核）证明该方法能增强表达能力并改善训练动态。通过正则化底层动力系统的复杂度来平衡表达能力和泛化能力。

Result: 在图像表示、场重建和数据压缩等任务上的实验表明，DINR相比传统静态INR具有更稳定的收敛性、更高的信号保真度和更强的泛化能力。

Conclusion: DINR通过动力系统视角重新构建隐式神经表示，有效缓解了频谱偏差问题，为连续信号建模提供了更强大和灵活的框架。
...
### [169] [The Double-Edged Nature of the Rashomon Set for Trustworthy Machine Learning](https://arxiv.org/abs/2511.21799)
*Ethan Hsu,Harry Chen,Chudi Zhong,Lesia Semenova*

Main category: cs.LG

TL;DR: Rashomon集合（多个近似最优模型）在可信机器学习中展现出双重作用：一方面通过模型多样性提供鲁棒性，另一方面因信息泄露增加隐私风险。


<details>
  <summary>Details</summary>
Motivation: 现实机器学习管道通常产生多个近似最优模型（Rashomon集合），而非单一模型。研究这种模型多样性如何影响可信机器学习的核心方面，特别是鲁棒性和隐私之间的权衡。

Method: 通过理论分析和稀疏决策树、线性模型的实证研究，探讨Rashomon集合在个体模型层面和集合层面的特性，分析其对对抗攻击、分布偏移的鲁棒性以及隐私泄露风险。

Result: 稀疏可解释模型保护隐私但对对抗攻击脆弱；Rashomon集合的多样性提供反应式鲁棒性（攻击破坏一个模型时其他模型仍准确）和分布偏移稳定性；但多样性增加信息泄露风险，披露更多近似最优模型为攻击者提供更丰富的训练数据视图。

Conclusion: Rashomon集合在可信机器学习中具有双重角色：既是鲁棒性资源（通过多样性），又是隐私风险（通过信息泄露）。研究揭示了鲁棒性与隐私之间的权衡，强调需要综合考虑模型集合而非单一模型来评估可信性。
...
### [170] [Massively Parallel Imitation Learning of Mouse Forelimb Musculoskeletal Reaching Dynamics](https://arxiv.org/abs/2511.21848)
...
Method: 开发了一个从神经科学实验室获取运动学数据并创建管道的平台，用于在生物力学模型中重现自然运动。采用模仿学习框架在模拟物理环境中执行灵巧的前肢抓取任务，使用JAX和Mujoco-MJX进行GPU加速训练。

Result: 小鼠手臂模型训练速度超过每秒100万训练步。结果表明，添加能量和速度的自然约束能产生更好地预测真实EMG信号的模拟肌肉骨骼活动。

Conclusion: 这项研究提供了证据表明，能量和控制约束对于建模肌肉骨骼运动控制至关重要，为理解体现控制的感觉运动转换提供了重要工具。
...
### [171] [From Topology to Retrieval: Decoding Embedding Spaces with Unified Signatures](https://arxiv.org/abs/2511.22150)
*Florian Rottach,William Rudman,Bastain Rieck,Harrisen Scells,Carsten Eickhoff*

Main category: cs.LG

TL;DR: 该论文提出了统一拓扑签名（UTS）框架，用于全面分析文本嵌入空间的组织结构，发现单一度量指标不足以区分不同嵌入空间，而多属性视角能更好地理解嵌入几何特征与下游任务性能的关系。


<details>
  <summary>Details</summary>
Motivation: 研究嵌入在空间中的组织方式不仅能增强模型可解释性，还能揭示影响下游任务性能的因素。当前对文本嵌入模型的拓扑和几何度量的分析不够全面，需要更系统的方法来表征嵌入空间。

Method: 提出了统一拓扑签名（UTS）框架，这是一个用于表征嵌入空间的整体方法。该方法在广泛的文本嵌入模型和数据集上进行了拓扑和几何度量的综合分析，识别了度量间的冗余性，并构建了多属性视角的分析体系。

Result: 研究发现：1）现有度量指标存在高度冗余；2）单一指标往往不足以充分区分不同嵌入空间；3）UTS能够预测模型特定属性并揭示由模型架构驱动的相似性；4）拓扑结构与排序效果相关，并能准确预测文档可检索性。

Conclusion: 理解文本嵌入的几何特征需要采用整体、多属性的视角。UTS框架为表征嵌入空间提供了有效工具，能够揭示模型架构特征并预测下游任务性能，为嵌入空间分析提供了新的方法论。
...
### [172] [Lightweight ML-Based Air Quality Prediction for IoT and Embedded Applications](https://arxiv.org/abs/2511.21857)
*Md. Sad Abdullah Sami,Mushfiquzzaman Abid*

Main category: cs.LG

TL;DR: 该研究评估了完整版和轻量版XGBoost回归模型在预测CO和NO2浓度方面的性能，发现完整版精度更高，但轻量版在计算资源方面有明显优势，适合物联网和嵌入式应用。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索不同版本的XGBoost模型在空气质量预测中的效果，特别关注在资源受限环境中部署简化模型的可行性，为物联网和嵌入式系统的实时空气质量监测提供解决方案。

Method: 使用AirQualityUCI数据集（城市环境一年数据），评估完整版和轻量版XGBoost回归模型。采用MAE、RMSE、MBE和R2等预测精度指标，以及推理时间、模型大小和峰值RAM使用等资源指标进行综合评估。

Result: 完整版XGBoost模型对两种污染物都获得了更优的预测精度，而轻量版模型虽然精度稍低，但在推理时间、模型存储需求和计算资源消耗方面有显著优势。

Conclusion: 轻量版XGBoost模型在资源受限环境中具有实际部署价值，可以在不显著牺牲预测质量的前提下，为物联网和嵌入式应用提供高效的实时空气质量监测解决方案。
...
### [173] [The Price of Progress: Algorithmic Efficiency and the Falling Cost of AI Inference](https://arxiv.org/abs/2511.23455)
*Hans Gundlach,Jayson Lynch,Matthias Mertens,Neil Thompson*

Main category: cs.LG

TL;DR: 该研究分析了AI模型在基准测试上的成本变化，发现前沿模型在知识、推理、数学和软件工程基准上的性能成本每年下降5-10倍，其中算法效率提升约每年3倍。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在高级基准测试上取得巨大进展，但这些进展往往依赖于更昂贵的模型，导致基准测试可能扭曲了实际能力与成本之间的关系。需要评估AI能力的实际经济影响。

Method: 使用Artificial Analysis和Epoch AI的数据构建了当前和历史基准测试运行价格的最大数据集，分析价格变化趋势，分离出开放模型以控制竞争效应，并除以硬件价格下降来估算算法效率进展。

Result: 研究发现前沿模型在知识、推理、数学和软件工程基准测试上达到特定性能水平的成本下降速度惊人，约为每年5-10倍。算法效率进展约为每年3倍。

Conclusion: 评估者应公开并考虑基准测试的价格作为衡量AI实际影响的重要组成部分，因为成本下降是评估AI进步的关键维度。
...
### [174] [Physically Interpretable Representation Learning with Gaussian Mixture Variational AutoEncoder (GM-VAE)](https://arxiv.org/abs/2511.21883)
*Tiffany Fan,Murray Cutforth,Marta D'Elia,Alexandre Cortiella,Alireza Doostan,Eric Darve*

Main category: cs.LG

TL;DR: 提出GM-VAE框架，结合EM训练策略和谱可解释性度量，从高维科学数据中提取物理可解释的紧凑表示，在反应流和湍流系统中实现稳定训练和物理一致的聚类。


<details>
  <summary>Details</summary>
Motivation: 从高维科学数据中提取紧凑、物理可解释的表示是一个持续挑战，因为物理系统具有复杂的非线性结构。传统VAE在联合优化重建和聚类时存在训练不稳定性问题。

Method: 提出高斯混合变分自编码器(GM-VAE)框架，采用块坐标下降策略，交替执行期望和最大化步骤，结合基于图拉普拉斯平滑度的谱可解释性度量来评估学习到的表示。

Result: 在表面反应ODE、Navier-Stokes尾流和实验激光诱导燃烧纹影图像等数据集上验证，GM-VAE能够产生平滑的物理一致流形和准确的机制聚类。

Conclusion: GM-VAE框架为解释湍流和反应流系统提供了一个鲁棒的数据驱动工具，实现了稳定训练和物理可解释的表示学习。
...
### [175] [Masked Diffusion for Generative Recommendation](https://arxiv.org/abs/2511.23021)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于语义ID的自回归生成式推荐模型存在推理成本高、训练数据利用效率低、偏向学习短上下文关系等问题。受NLP领域突破启发，作者希望开发更高效的序列建模方法。

Method: 提出使用掩码扩散模型来学习和建模用户语义ID序列的概率分布。该方法利用离散掩码噪声促进序列分布学习，将掩码标记建模为在给定未掩码标记条件下独立，从而实现并行解码。

Result: 实验表明，该方法在性能上持续优于自回归建模，特别是在数据受限设置和粗粒度召回方面表现突出。同时保持并行预测多个语义ID的灵活性。

Conclusion: 掩码扩散模型为生成式推荐提供了一种更高效、性能更优的替代方案，解决了自回归模型的推理效率和数据利用问题，在多种场景下表现出显著优势。
...
TL;DR: 论文提出了一种针对多模态基础模型中对抗性幻觉攻击的防御机制，通过生成模型重构被攻击者扰动的输入，结合生成采样和共识聚合策略，显著降低攻击成功率并提升跨模态对齐效果。


<details>
  <summary>Details</summary>
Motivation: 多模态基础模型虽然能在共享嵌入空间中对齐图像、文本等多种模态，但仍易受对抗性幻觉攻击的影响，其中微小的扰动会破坏跨模态对齐并误导下游任务。

Method: 提出任务无关的缓解机制：1）使用生成模型（如变分自编码器）从攻击者扰动的输入中重构原始输入；2）采用生成采样策略结合基于共识的聚合方案对生成样本结果进行整合。

Result: 在先进的多模态编码器上实验表明，该方法将幻觉攻击成功率降至接近零，在未扰动和扰动输入设置下分别将跨模态对齐提升了4%（42到46）和11%（32到43）。
...
<details>
  <summary>Details</summary>
Motivation: 准确预测人类对AI系统的信任对于AI决策支持工具的安全整合至关重要，特别是在医疗健康领域。在心理健康应用中，信任校准不当会影响诊断和治疗结果，因此需要实时、客观的信任标记。

Method: 使用多模态机器学习框架，结合图像（面部视频）和生理（皮肤电反应）数据。面部视频通过OpenCV提取帧，使用预训练transformer模型提取情感特征；GSR信号分解为紧张性和阶段性成分。定义两个时间窗口：早期检测窗口（决策前6-3秒）和近端检测窗口（决策前3-0秒）。分别使用图像、GSR和多模态特征进行信任预测，通过多模态堆叠集成最佳单模态模型。

Result: 多模态框架显著提高了预测性能：早期检测窗口准确率0.83、F1分数0.88、ROC-AUC 0.87；近端检测窗口准确率0.75、F1分数0.82、ROC-AUC 0.66。结果表明结合面部和生理线索能有效预测信任。
...
TL;DR: ProPS是一种新颖的强化学习方法，将大型语言模型置于策略优化循环的核心，直接基于奖励反馈和自然语言输入提出策略更新，在15个Gymnasium任务中优于7种主流RL算法。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励信号，无法充分利用现实任务中丰富的语义知识。相比之下，人类通过结合数值反馈、语言、先验知识和常识来高效学习。需要一种能统一数值和语言推理的框架。

Method: 提出Prompted Policy Search (ProPS)方法，将大型语言模型置于策略优化循环的核心，直接基于奖励反馈和自然语言输入提出策略更新。LLM能够在上下文中执行数值优化，并整合语义信号如目标、领域知识和策略提示。

Result: 在15个Gymnasium任务（经典控制、Atari游戏、MuJoCo环境）中评估，与7种广泛采用的RL算法（PPO、SAC、TRPO等）比较。ProPS在15个任务中的8个上优于所有基线方法，在提供领域知识时表现出显著优势。

Conclusion: 研究结果表明统一语义和数值推理的潜力，能够实现更透明、可泛化且与人类对齐的强化学习。LLM能够执行数值优化，语义信号的整合可以带来更明智的探索和样本高效的学习。
...
Method: 借鉴Rubin潜在结果框架，量化每个特征在二分类任务中分离两个结果组的强度，超越传统描述统计来估计每个特征对结果的影响，将数据驱动的特征排序与模型解释进行比较

Result: 提供了一个可解释且模型无关的方法来评估模型-数据对齐，为实践者提供工具验证模型是否真正反映了数据的内在结构

Conclusion: 提出的框架能够有效评估机器学习模型与数据结构的对齐程度，帮助确保模型解释与数据本身传达的信息一致
...
TL;DR: 提出了一种异构多智能体强化学习框架，用于自动化特征转换，通过共享批评机制增强智能体间协作，使用多头注意力处理动态扩展的特征空间，提升特征交叉效果。


<details>
  <summary>Details</summary>
Motivation: 特征转换对于结构化数据下游任务性能提升至关重要，但现有自动化方法依赖启发式或穷举搜索，效率低下。虽然强化学习改进了传统方法，但仍存在两个问题：1）特征转换过程中的动态特征扩展导致不稳定和学习复杂度增加；2）智能体间协作不足导致特征交叉操作次优和模型性能下降。

Method: 提出异构多智能体强化学习框架，包含三种异构智能体，分为两种类型，分别负责选择特征和操作进行特征交叉。采用共享批评机制促进智能体间信息交换，使用多头注意力特征智能体处理动态扩展的特征空间，引入状态编码技术稳定和增强强化学习智能体的学习动态。

Result: 通过大量实验验证了模型在有效性、效率、鲁棒性和可解释性方面的优越性能。

Conclusion: 该异构多智能体强化学习框架能够有效解决自动化特征转换中的协作不足和动态特征空间问题，实现了更稳健可靠的特征转换策略。
...
TL;DR: 该论文提出了一个动态可操作的框架，用于保护企业部署中的智能体AI系统，将安全视为智能体系统动态交互中涌现的属性，而非固定模型属性。


<details>
  <summary>Details</summary>
Motivation: 传统AI安全方法主要关注单个模型的安全属性，但智能体AI系统（包含模型、编排器、工具和数据）的安全是这些组件动态交互中涌现的属性。需要新的框架来识别和管理智能体特有的风险。

Method: 提出动态智能体安全与安全框架：1）定义统一传统安全和智能体特有风险的分类法；2）使用辅助AI模型和智能体进行上下文风险管理；3）通过沙盒化AI驱动的红队测试进行风险发现；4）在NVIDIA AI-Q研究助手案例中验证框架。

Result: 框架在NVIDIA旗舰智能体研究助手中得到验证，发现了新的智能体风险并进行了上下文缓解。发布了包含10,000多个现实攻击和防御执行轨迹的数据集，用于推进智能体安全研究。

Conclusion: 智能体AI系统的安全需要动态、可操作的框架，将安全视为系统交互的涌现属性。提出的框架能够有效识别和管理智能体特有风险，并通过实际案例验证了其有效性。
...
TL;DR: 本文提出了首个可证明最优的在线算法，用于学习一类称为有序超树的超图结构，使用最短路径查询，并建立了查询复杂度的理论界限。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过最短路径查询学习超图结构，特别是在进化树重建等实际应用中，距离测量可能随距离增加而退化，需要更高效的学习算法。

Method: 提出了一种可证明最优的在线算法，用于学习有序超树这类超图结构，该算法可转换为离线算法。同时考虑了使用有界距离查询的学习模型。

Result: 在线算法对有序超树类是最优的，有序超树包含Fagin层次结构中最广泛的可学习子类。在有界距离查询模型中，对一般超树学习建立了渐近紧的复杂度界限。

Conclusion: 本文首次为有序超树类提供了可证明最优的学习算法，扩展了超图学习的理论边界，并为实际应用中的距离退化问题提供了解决方案。
...
TL;DR: 本文提出了一种基于在线测试时适应（TTA）的驾驶员疲劳检测框架，通过动态调整批归一化层参数和使用记忆库管理EEG信号，有效解决了EEG信号的域偏移问题，在模拟驾驶数据集上取得了81.73%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 基于脑电图（EEG）的疲劳检测系统面临信号变异性和域偏移问题，特别是被试间变异性导致模型难以泛化到未见过的目标被试，需要繁琐的校准过程。

Method: 提出在线测试时适应（TTA）框架：1）更新批归一化（BN）层的可学习参数，同时保留预训练的归一化统计量；2）使用记忆库动态管理流式EEG片段，基于负能量分数和持续时间选择可靠样本；3）引入原型学习以应对时间上的分布偏移。

Result: 在模拟环境中的持续注意力驾驶数据集上验证，该方法平均F1分数达到81.73%，比最佳TTA基线提高了11.73%，显著提升了EEG疲劳检测系统在非独立同分布场景下的适应性。

Conclusion: 提出的在线TTA方法有效解决了EEG信号的域偏移问题，显著提高了驾驶员疲劳检测系统在未见被试上的泛化能力，为实际应用提供了可行的解决方案。
...
TL;DR: 开发了HealthPredictor AI模型，将电力使用与公共健康结果联系起来，通过优化用电行为降低空气污染对健康的影响
...
Result: 在美国多个地区的测试中，该健康驱动优化框架在公共健康影响方面的预测误差显著低于燃料组合驱动基线；电动汽车充电调度的案例研究展示了该方法带来的公共健康收益和可操作指导

Conclusion: 这项工作展示了如何明确设计AI模型来实现健康导向的能源管理，以促进公共健康和更广泛的社会福祉；已发布数据集和代码
...
TL;DR: 该论文研究了过参数化模型在分数匹配目标下学习单个高斯分布的梯度下降优化行为，分析了不同噪声尺度下的收敛性，证明了全局收敛条件和参数发散但损失收敛的现象。


<details>
  <summary>Details</summary>
Motivation: 尽管分数匹配在现代生成建模（特别是扩散模型）中取得了经验成功，但其在过参数化机制下的优化行为理论理解仍然有限。本文旨在填补这一理论空白，研究梯度下降在过参数化模型中学习单个高斯分布时的优化动态。

Method: 使用具有n个可学习参数的学生模型，在从单个真实高斯分布生成的数据上训练，采用总体分数匹配目标。分析梯度下降在不同机制下的优化动态：高噪声机制、低噪声机制、指数小初始化条件和随机高斯初始化条件。

Result: 1. 当噪声尺度足够大时，证明了梯度下降的全局收敛性；2. 在低噪声机制中识别了平稳点的存在；3. 当参数初始化为指数小时，所有参数收敛到真实值；4. 没有指数小初始化时，参数可能不收敛到真实值；5. 当参数从远离真实值的高斯分布随机初始化时，高概率下只有一个参数收敛而其他参数发散，但损失仍以1/τ速率收敛到零；6. 建立了该机制下收敛速率的近乎匹配下界。

Conclusion: 这是首个在分数匹配框架下为至少三个分量的高斯混合建立全局收敛保证的工作。研究揭示了过参数化分数匹配优化的复杂动态，包括参数发散但损失收敛的反直觉现象，为理解现代生成模型的训练行为提供了重要理论洞见。
...
TL;DR: FedWMSAM：针对非IID联邦学习中动量与SAM结合的两个失效模式（局部-全局曲率错位和动量回波振荡）提出解决方案，通过动量引导的全局扰动和自适应余弦相似度规则实现高效优化


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，模型需要在有限的通信预算下快速收敛，同时在非IID客户端分布上保持良好泛化能力。虽然动量和SAM分别用于加速收敛和寻找平坦解，但简单结合在非IID环境下存在两个结构性问题：局部-全局曲率错位和动量回波振荡

Method: 提出FedWMSAM方法：1）使用服务器聚合的动量构建动量引导的全局扰动，使客户端SAM方向与全局下降几何对齐，实现单反向传播SAM近似；2）通过余弦相似度自适应规则耦合动量和SAM，形成早期动量、晚期SAM的两阶段训练计划

Result: 在多个数据集和模型架构上的实验验证了方法的有效性、适应性和鲁棒性，在解决联邦学习优化挑战方面表现出优越性。理论分析提供了非IID收敛界，明确建模了扰动引起的方差及其对参数(S, K, R, N)的依赖

Conclusion: FedWMSAM成功解决了非IID联邦学习中动量和SAM结合的失效模式，通过创新的扰动对齐和自适应调度机制，实现了高效且稳定的优化性能
...
TL;DR: 本文首次全面研究了低秩分解压缩对LLM可信度的影响，涵盖隐私、对抗鲁棒性、公平性和伦理对齐四个方面，发现压缩对可信度有复杂影响并提出了指导建议。


<details>
  <summary>Details</summary>
Motivation: 尽管低秩分解压缩能有效减小LLM规模并保持性能，但其对模型可信度（隐私、对抗鲁棒性、公平性、伦理对齐）的影响尚未被充分理解，需要进行系统研究。

Method: 评估多种不同规模和变体的LLM，使用多种低秩算法进行压缩，系统分析压缩对隐私、对抗鲁棒性、公平性和伦理对齐的影响，并进行梯度归因分析识别影响对抗鲁棒性的关键层。

Result: 发现：1) 压缩保持或改善训练数据隐私但削弱对话中PII保护；2) 对抗鲁棒性通常保持甚至增强；3) 零样本伦理推理退化但少样本提示可部分恢复；4) 公平性在压缩下下降。还研究了模型规模和微调对可信度的影响。

Conclusion: 低秩压缩对LLM可信度有复杂影响，需在设计压缩策略时综合考虑不同可信度维度的权衡。梯度归因分析为识别影响对抗鲁棒性的关键层提供了指导，有助于开发更可信的压缩方法。
...
TL;DR: 该研究复制并改进了"Adaptive Liquidity Provision in Uniswap V3 with Deep Reinforcement Learning"论文，提出结合Mamba与DDQN的新架构，在部分测试中表现优于原模型。


<details>
  <summary>Details</summary>
Motivation: 复制原始论文并改进Uniswap V3中的自适应流动性提供策略，通过深度学习增强DeFi协议中的流动性管理效率。

Method: 1. 复制部分：从Uniswap Subgraph获取数据，实现原始模型；2. 改进部分：提出新架构，结合Mamba与DDQN，设计新奖励函数，重新清洗数据，引入两个新基线进行比较。

Result: 新模型虽然尚未应用于所有数据集，但比原始模型具有更强的理论支持，在部分测试中表现更好。

Conclusion: 结合Mamba与DDQN的新架构在Uniswap V3流动性提供任务中显示出改进潜力，为DeFi领域的强化学习应用提供了新的技术方向。
...
TL;DR: 该研究提出了一种从大规模动作空间中选取代表性子集的方法，用于解决强化学习中动作空间过大导致学习困难的问题，适用于库存管理和推荐系统等应用场景。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，当动作空间极其庞大时（如库存管理、推荐系统），直接在整个空间上学习是不可行的。需要找到一种方法，能够为一系列相关环境选取一个固定的动作子集，使得该子集对每个环境都包含接近最优的动作，从而实现高效学习。

Method: 将之前元多臂赌博机（meta-bandits）的研究扩展到更一般的马尔可夫决策过程（MDPs）设置。采用非中心化的亚高斯过程模型来适应更大的环境异质性，使用现有算法在扩展后的框架中实现动作子集选择。

Result: 证明了现有算法在MDPs设置下能够达到与使用完整动作空间相当的性能。在放松的非中心化亚高斯过程模型下建立了理论保证，该模型能够容纳更大的环境异质性。

Conclusion: 该方法为大规模组合决策问题提供了计算和样本高效的解决方案，能够在不确定性环境下实现高效学习，避免了在完整动作空间上学习的计算负担。
...
TL;DR: 开发基于球面傅里叶神经算子（SFNO）的太阳风稳态建模替代模型，相比传统数值替代模型HUX在多个指标上表现相当或更好，可实现高效实时预报。


<details>
  <summary>Details</summary>
Motivation: 太阳风变化（如高速流和日冕物质抛射）会破坏卫星、电网和通信系统，需要准确建模进行空间天气预报。传统3D磁流体动力学模型计算成本高，限制了边界条件不确定性研究，需要开发高效的替代模型。

Method: 使用球面傅里叶神经算子（SFNO）开发太阳风稳态建模的替代模型，并与先前开发的数值替代模型HUX进行比较。

Result: SFNO在多个指标上达到与HUX相当或更好的性能。虽然HUX在物理平滑性方面仍有优势，但这突显了需要改进评估标准而非SFNO的缺陷。

Conclusion: SFNO作为一种灵活可训练的方法，能够实现高效实时预报，并可通过更多数据改进性能，为太阳风建模提供了有前景的替代方案。
...
TL;DR: IVGAE是一种基于变分图自编码器的框架，用于处理包含数值和分类特征的异构表格数据中的缺失值问题，通过构建样本-特征二分图和使用双解码器架构来建模结构依赖关系和缺失机制。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的表格数据集经常存在缺失数据，特别是当数据同时包含数值和分类特征时。现有的插补方法往往难以有效捕捉复杂的结构依赖关系和处理异构数据。

Method: IVGAE构建样本-特征二分图来表示样本与特征之间的关系，应用图表示学习来建模结构依赖。采用双解码器架构：一个解码器重建特征嵌入，另一个建模缺失模式，提供对缺失机制有感知的结构先验。为了更好编码分类变量，引入了基于Transformer的异构嵌入模块，避免高维one-hot编码。

Result: 在16个真实世界数据集上的广泛实验表明，IVGAE在MCAR、MAR和MNAR三种缺失场景下，在30%缺失率的情况下，在RMSE和下游F1分数方面都取得了持续改进。

Conclusion: IVGAE通过图表示学习和双解码器架构，为异构表格数据的缺失值插补提供了一个有效的解决方案，能够更好地捕捉数据中的结构依赖关系和处理缺失机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有降维方法各有局限：PCA变体简单可解释但不够灵活捕捉非线性流形结构；自编码器灵活但难以解释；基于图嵌入的方法可能产生病态几何扭曲

Method: 提出变分框架，将降维算法形式化为最优流形嵌入问题的解，该框架允许非线性嵌入，满足偏微分方程，反映嵌入目标的对称性

Result: 框架允许非线性嵌入，比PCA更灵活；解满足偏微分方程，具有可解释性；特殊情况下精确恢复PCA；某些情况可解析表征

Conclusion: 提出的变分框架克服了现有降维方法的局限，既保持了可解释性又允许非线性嵌入，为降维算法提供了统一的理论基础
...
TL;DR: BELA基准测试评估智能体在动态环境中通过经验学习和主动探索的能力，当前前沿模型在推荐场景中难以有效改进


<details>
  <summary>Details</summary>
Motivation: 当前评估主要关注确定性任务，缺乏对智能体通过积累经验进行自适应学习和推理能力的衡量，特别是在动态现实环境中需要处理不完整知识和适应变化的能力

Method: 创建BELA基准测试，结合：(1)亚马逊真实产品数据，(2)多样化用户角色表示异质潜在偏好，(3)基于角色的LLM用户模拟器生成丰富交互轨迹

Result: 当前前沿模型在跨情节中难以实现有意义的改进，突显了需要具备强大上下文学习能力的智能体系统

Conclusion: 需要开发能够在动态环境中通过经验学习和主动探索进行有效适应的智能体系统，BELA基准为此提供了评估框架
...
### [194] [Designing Instance-Level Sampling Schedules via REINFORCE with James-Stein Shrinkage](https://arxiv.org/abs/2511.22177)
*Peiyu Yu,Suraj Kothawade,Sirui Xie,Ying Nian Wu,Hongliang Fei*

Main category: cs.LG

TL;DR: 该论文提出了一种新的文本到图像采样器后处理方法：通过单次Dirichlet策略学习实例级（提示和噪声条件）的采样时间表，而不是固定的全局时间表，从而提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 大多数文本到图像采样器的后处理方法都集中在模型权重上（微调对齐或蒸馏提高效率），本文采取不同路线：重新调度冻结采样器的采样时间表，以解锁预训练采样器的额外生成潜力。

Method: 1. 学习实例级（提示和噪声条件）采样时间表，而非固定全局时间表；2. 使用单次Dirichlet策略；3. 引入基于James-Stein估计器的新奖励基线，确保高维策略学习中的准确梯度估计。

Result: 1. 重新调度的采样器在Stable Diffusion和Flux模型家族中一致提升文本-图像对齐，包括文本渲染和组合控制；2. 5步Flux-Dev采样器配合该时间表能达到与专门蒸馏的采样器（如Flux-Schnell）相当的生成质量。

Conclusion: 该调度框架作为一种新兴的模型无关后处理杠杆，能够解锁预训练采样器的额外生成潜力，为文本到图像生成提供了新的优化方向。
...
### [195] [PULSE-ICU: A Pretrained Unified Long-Sequence Encoder for Multi-task Prediction in Intensive Care Units](https://arxiv.org/abs/2511.22199)
*Sejeong Jang,Joo Heung Yoon,Hyo Kyung Lee*

Main category: cs.LG

TL;DR: PULSE-ICU是一个自监督基础模型，可从大规模ICU电子健康记录序列中学习事件级表示，无需重采样或手动特征工程，在18个预测任务中表现出色，并在外部验证中显示出良好的领域适应能力。


<details>
  <summary>Details</summary>
Motivation: ICU数据具有高度不规则、异质性和时间碎片化的特点，这对通用的临床预测提出了挑战。现有方法需要重采样或手动特征工程，限制了模型的泛化能力和适应性。

Method: 提出PULSE-ICU自监督基础模型：1）统一的嵌入模块编码事件身份、连续值、单位和时间属性；2）基于Longformer的编码器高效建模长轨迹；3）在18个预测任务上进行微调，包括死亡率、干预预测和表型识别。

Result: PULSE-ICU在各项任务中表现优异，在eICU、HiRID和P12数据集的外部验证中，通过最小微调即实现显著改进，显示出对领域偏移和变量约束的鲁棒性。
...
### [196] [BiCQL-ML: A Bi-Level Conservative Q-Learning Framework for Maximum Likelihood Inverse Reinforcement Learning](https://arxiv.org/abs/2511.22210)
*Junsung Park*

Main category: cs.LG

TL;DR: BiCQL-ML是一种无需策略学习的离线逆强化学习算法，通过双层框架联合优化奖励函数和保守Q函数，在仅使用固定演示数据的情况下恢复解释专家行为的奖励函数。


<details>
  <summary>Details</summary>
Motivation: 离线逆强化学习的目标是在没有在线交互的情况下，仅使用固定的演示数据恢复解释专家行为的奖励函数。现有方法通常需要显式的策略学习，而本文提出了一种无需策略学习的方法来避免这一问题。

Method: 提出BiCQL-ML算法，采用双层框架：1）在当前奖励下通过保守Q学习（CQL）学习保守Q函数；2）更新奖励参数以最大化专家动作的期望Q值，同时抑制对分布外动作的过度泛化。该方法可视为基于软价值匹配原则的最大似然估计。

Result: 理论证明BiCQL-ML收敛到使专家策略达到软最优的奖励函数。在标准离线强化学习基准测试中，BiCQL-ML在奖励恢复和下游策略性能方面均优于现有离线逆强化学习基线方法。

Conclusion: BiCQL-ML是一种有效的策略无关离线逆强化学习方法，通过联合优化奖励和Q函数，在理论和实验上都表现出优越性能，为离线逆强化学习提供了新的解决方案。
...
### [197] [FedRE: A Representation Entanglement Framework for Model-Heterogeneous Federated Learning](https://arxiv.org/abs/2511.22265)
...
Method: 提出联邦表示纠缠框架：客户端使用归一化随机权重将本地表示聚合成纠缠表示，并用相同权重将one-hot标签编码整合为纠缠标签编码；这些信息上传到服务器训练全局分类器；每轮重新采样随机权重以引入多样性

Result: 实验表明FedRE在模型性能、隐私保护和通信开销之间实现了有效权衡，能够缓解全局分类器的过度自信，促进更平滑的决策边界，同时降低表示反转攻击风险

Conclusion: FedRE为模型异构联邦学习提供了一种有效的解决方案，通过表示纠缠机制平衡了性能、隐私和通信效率的需求
...
### [198] [TreeCoder: Systematic Exploration and Optimisation of Decoding and Constraints for LLM Code Generation](https://arxiv.org/abs/2511.22277)
*Henrijs Princis,Arindam Sharma,Cristina David*

Main category: cs.LG

TL;DR: TreeCoder是一个用于代码生成的通用解码框架，通过树搜索和约束函数在解码过程中强制执行正确性和结构约束，而非依赖提示工程。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面表现出色，但仅通过自然语言提示引导时，其输出经常违反语法或语义约束，需要更好的方法来确保代码正确性。

Method: TreeCoder将解码表示为候选程序的树搜索，将解码策略和约束函数（如风格、语法、执行）作为可优化的组件，支持系统化探索和自动调优解码配置。

Result: 在MBPP（Python）和SQL-Spider基准测试中，TreeCoder在CodeLlama、Mistral和DeepSeek等开源模型上持续提高准确性，通常显著优于无约束基线。

Conclusion: TreeCoder提供了一个灵活通用的框架，通过在解码过程中强制执行约束来改进LLM的代码生成质量，超越了依赖提示工程的传统方法。
...
### [199] [The Hidden Cost of Approximation in Online Mirror Descent](https://arxiv.org/abs/2511.22283)
*Ofir Schlisselberg,Uri Sherman,Tomer Koren,Yishay Mansour*

Main category: cs.LG

TL;DR: 该论文系统研究了在线镜像下降(OMD)算法在近似误差下的性能，揭示了正则化器平滑性与误差鲁棒性之间的复杂关系，发现不同正则化器对误差的容忍度存在显著差异。
...
Method: 系统研究不精确OMD算法，分析正则化器平滑性与近似误差鲁棒性之间的关系。特别关注单纯形及其子集上的屏障正则化器，包括负熵、log-barrier和Tsallis正则化器。

Result: 1. 当正则化器均匀平滑时，建立了误差导致的超额遗憾的紧界；2. 对于单纯形上的屏障正则化器，发现负熵需要指数级小误差才能避免线性遗憾，而log-barrier和Tsallis正则化器在多项式误差下仍保持鲁棒；3. 当损失是随机且域为单纯形时，负熵重新获得鲁棒性，但这不适用于所有子集。

Conclusion: 该研究首次系统分析了不精确OMD算法，揭示了正则化器选择对算法误差鲁棒性的关键影响。不同正则化器对近似误差的敏感性存在显著差异，这对实际算法设计和实现具有重要指导意义。
...
### [200] [Online Dynamic Pricing of Complementary Products](https://arxiv.org/abs/2511.22291)
*Marco Mussi,Marcello Restelli*

Main category: cs.LG

TL;DR: 该论文提出了一种针对互补产品的动态定价算法，通过考虑产品间的需求交互关系来优化整体收入，相比忽略产品间相互作用的算法能获得更好的收益。
...
Method: 提出在线学习算法，通过整数规划问题识别产品间的互补关系，并基于异方差高斯过程的多臂老虎机解决方案来优化定价策略，同时考虑产品需求的正负交互作用。

Result: 在模拟环境中验证了该解决方案，相比忽略产品间相互作用的可比学习算法，该算法能够提高整体收入。

Conclusion: 考虑产品间需求交互关系的动态定价算法比独立定价方法更有效，能够通过协调互补产品的定价策略来最大化整体收入。
...
### [201] [FLUX: Efficient Descriptor-Driven Clustered Federated Learning under Arbitrary Distribution Shifts](https://arxiv.org/abs/2511.22305)
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习方法假设客户端数据独立同分布，但在现实场景中这一假设往往不成立，导致全局模型性能显著下降。现有方法难以处理多种分布偏移类型，且需要先验知识。

Method: 提出FLUX框架：1) 使用隐私保护的客户端描述符提取技术；2) 采用无监督聚类方法对客户端进行分组；3) 为每个聚类训练专用模型；4) 支持测试时适应，使未见客户端能选择最合适的聚类模型。

Result: 在四个标准基准、两个真实数据集和十个最先进基线方法上的实验表明，FLUX在不同分布偏移下显著提升性能和稳定性，平均准确率比最佳基线提高最多23个百分点，同时保持与FedAvg相当的计算和通信开销。

Conclusion: FLUX是一个无需先验知识、支持测试时适应的聚类联邦学习框架，能有效处理多种分布偏移问题，在保持隐私的同时显著提升模型性能，具有实际应用价值。
...
### [202] [SingleQuant: Efficient Quantization of Large Language Models in a Single Pass](https://arxiv.org/abs/2511.22316)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM量化方法结合不兼容的梯度优化和量化截断，导致严重的收敛病理问题，延长量化时间并降低模型任务性能

Method: 提出SingleQuant框架，构造对齐旋转变换(ART)和均匀旋转变换(URT)处理不同的激活异常值，使用严格公式化的Givens旋转矩阵
...
### [203] [Cleaning the Pool: Progressive Filtering of Unlabeled Pools in Deep Active Learning](https://arxiv.org/abs/2511.22344)
...
<details>
  <summary>Details</summary>
Motivation: 现有主动学习策略（如不确定性或代表性）在不同数据集、模型和AL周期中效果差异很大，单一策略在整个AL过程中可能表现不佳，需要一种能自适应结合多种策略的方法。

Method: REFINE采用两阶段方法：1）渐进过滤阶段使用AL策略集成迭代精炼未标注池，保留捕捉不同价值概念的候选样本；2）覆盖选择阶段从精炼池中选择最终批次，确保覆盖所有已识别的价值概念。

Result: 在6个分类数据集和3个基础模型上的实验表明，REFINE始终优于单一策略和现有集成方法。渐进过滤作为预处理步骤能提升任何AL策略在精炼池上的性能，在音频频谱图分类用例中得到验证。

Conclusion: REFINE提供了一种有效的集成主动学习方法，能够自适应结合多种AL策略的优势，且易于扩展集成新的先进AL策略，解决了单一策略可能导致的次优性能问题。
...
### [204] [AutoTailor: Automatic and Efficient Adaptive Model Deployment for Diverse Edge Devices](https://arxiv.org/abs/2511.22355)
*Mengyang Liu,Chenyu Lu,Haodong Tian,Fang Dong,Ruiting Zhou,Wei Wang,Dian Shen,Guangtong Li,Ye Wan,Li Li*

Main category: cs.LG

TL;DR: AutoTailor是一个自动化端到端框架，用于在边缘设备上实现基于SuperNet的自适应模型部署，显著减少了代码量和硬件分析成本，同时提升了准确性和延迟性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于SuperNet的自适应模型部署方法存在两个主要问题：1）需要繁琐的模型感知开发，2）需要耗时的硬件感知性能分析。这些问题限制了SuperNet在实际应用中的采用。

Method: AutoTailor采用计算图引导的编译方法，自动将用户提供的ML模型转换为SuperNets。框架集成了无需学习的延迟和准确性预测器，支持低成本且准确的性能预测。

Result: 评估显示：1）SuperNet构建代码量减少11-27倍；2）硬件感知分析成本降低至少11倍；3）相比最先进方法，准确率提升最高15.60%，延迟降低最高60.03%。

Conclusion: AutoTailor是首个实现自动化端到端SuperNet自适应模型部署的框架，通过自动化SuperNet构建和高效性能预测，解决了现有方法的实际应用障碍。
...
### [205] [Efficient-Husformer: Efficient Multimodal Transformer Hyperparameter Optimization for Stress and Cognitive Loads](https://arxiv.org/abs/2511.22362)
...
Method: 设计结构化搜索空间进行超参数优化，开发Efficient-Husformer架构，使用单层、3个注意力头、模型维度18/30、FFN维度120/30的紧凑配置，仅约30k参数。

Result: 在WESAD数据集上准确率达到88.41（提升13.83%），CogLoad数据集上达到92.61（提升6.98%）。最佳配置使用(L+dm)或(L+FFN)模态组合。

Conclusion: 通过超参数优化设计的Efficient-Husformer在保持高性能的同时大幅减少了计算和内存需求，为生理信号分析提供了高效的Transformer解决方案。
...
### [206] [SuRe: Surprise-Driven Prioritised Replay for Continual LLM Learning](https://arxiv.org/abs/2511.22367)
*Hugo Hazard,Zafeirios Fountas,Martin A. Benfeghoul,Adnan Oomerjee,Jun Wang,Haitham Bou-Ammar*

Main category: cs.LG

TL;DR: 该论文提出Surprise-prioritised Replay (SuRe)方法和双学习器设计，通过基于惊讶度的样本选择和慢权重整合机制，显著提升大语言模型在持续学习中的表现，特别是在多任务场景下。


<details>
  <summary>Details</summary>
Motivation: 持续学习（在不忘记先前知识的情况下适应一系列任务）是机器学习的主要挑战，也是人工智能与人类智能的关键差距。虽然正则化和回放在视觉领域表现良好，但在大语言模型的持续学习中，尤其是在多任务场景下，它们落后于多任务学习。

Method: 提出两种改进：1) SuRe（基于惊讶度的回放）：通过负对数似然值对序列进行排名，存储最令人惊讶的序列；2) 双学习器设计：使用快速和慢速LoRA适配器，通过指数移动平均合并，实现快速适应同时稳定长期知识。

Result: SuRe在大量任务（LNT）设置中达到最先进性能，在标准CL和LNT基准测试中提供最佳平均表现。结合双学习器设计后，在LNT上比先前SOTA提升高达+5个准确率点。消融研究证实该方法在减少回放频率和小缓冲区大小下保持鲁棒性。

Conclusion: 研究结果表明回放可以作为持续LLM微调的强大基线，基于惊讶度的选择和慢权重整合是缓解灾难性遗忘的互补组件，共同提升了持续学习的有效性和样本效率。
...
### [207] [Predicting and Interpolating Spatiotemporal Environmental Data: A Case Study of Groundwater Storage in Bangladesh](https://arxiv.org/abs/2511.22378)
...
<details>
  <summary>Details</summary>
Motivation: 地理空间观测数据通常仅限于点测量，因此时间预测和空间插值对于构建连续场至关重要。研究旨在评估哪种深度学习策略能更好地解决这一挑战。
...
Conclusion: 空间插值比时间预测更具挑战性，地质不确定性对点的时间行为有重要影响。基于地下水储量数据得出的结论也适用于其他由间接可观测因素控制的环境变量。代码已开源。
...
### [208] [TS2Vec-Ensemble: An Enhanced Self-Supervised Framework for Time Series Forecasting](https://arxiv.org/abs/2511.22395)
*Ganeshan Niroshan,Uthayasanker Thayasivam*

Main category: cs.LG

TL;DR: TS2Vec-Ensemble是一个混合框架，通过融合预训练TS2Vec编码器的隐式学习动态与显式工程时间特征（周期性模式），采用双模型集成架构和自适应加权方案，显著提升了时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法（如TS2Vec）在预测任务中表现不佳，因为它们的损失函数侧重于实例区分而非捕捉对预测至关重要的确定性模式（如季节性和趋势）。需要一种能够结合学习表示和显式时间先验的混合策略。

Method: 提出TS2Vec-Ensemble混合框架：1) 使用预训练的TS2Vec编码器获取隐式学习动态；2) 融合显式工程时间特征编码周期性模式；3) 采用双模型集成架构，两个独立的回归头分别关注学习动态和季节模式；4) 使用自适应加权方案，为每个预测时域独立优化集成权重，动态调整短期动态和长期季节性的优先级。

Result: 在ETT基准数据集上进行大量实验（包括单变量和多变量预测），结果显示TS2Vec-Ensemble在预测性能上持续且显著优于标准TS2Vec基线和其他最先进模型。

Conclusion: 验证了假设：结合学习表示和显式时间先验的混合策略是长期时间序列预测的优越方法。TS2Vec-Ensemble通过融合隐式学习动态与显式工程特征，有效提升了预测准确性。
...
### [209] [Improving Stochastic Action-Constrained Reinforcement Learning via Truncated Distributions](https://arxiv.org/abs/2511.22406)
*Roland Stolz,Michael Eichelbeck,Matthias Althoff*

Main category: cs.LG

TL;DR: 本文提出在动作约束强化学习中，使用截断正态分布进行策略梯度方法时，需要准确计算熵、对数概率等关键特性，而非使用非截断分布的近似，从而显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有动作约束强化学习方法在处理复杂约束时，计算熵、对数概率及其梯度变得难以处理，通常使用非截断分布进行近似，但这严重降低了性能。作者认为准确估计这些特性在动作约束RL中至关重要。

Method: 提出高效的数值近似方法来准确计算截断策略分布的关键特性（熵、对数概率及其梯度），并提供截断策略分布的有效采样策略。

Result: 在三个基准环境中验证了该方法，当使用准确估计时，相比使用近似方法，性能有显著提升。

Conclusion: 在动作约束强化学习中，准确估计截断策略分布的关键特性（而非使用非截断分布的近似）对于性能提升至关重要，提出的高效数值近似方法能够有效解决这一问题。
...
### [210] [PISA: Prioritized Invariant Subgraph Aggregation](https://arxiv.org/abs/2511.22435)
*Ali Ghasemi,Farooq Ahmad Wani,Maria Sofia Bucarelli,Fabrizio Silvestri*

Main category: cs.LG

TL;DR: PISA框架通过动态MLP聚合机制改进图数据OOD泛化，相比之前的单子图或多子图方法，能更有效地优先组合多个不变子图表示，在15个数据集上获得最高5%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有图数据OOD泛化方法存在局限：CIGA只提取单个不变子图可能遗漏多个因果模式，SuGAr虽然学习多个不变子图但使用简单均匀或贪婪聚合策略不够有效，需要更智能的聚合机制来提升性能。

Method: 提出PISA框架，核心是动态MLP聚合器，能够优先考虑并更有效地组合多个不变子图表示，相比之前的均匀或贪婪聚合策略，能更智能地整合不同子图的信息。

Result: 在包括DrugOOD在内的15个数据集上进行实验，PISA相比之前的方法实现了最高5%的分类准确率提升，证明了动态聚合机制的有效性。

Conclusion: PISA通过引入动态MLP聚合机制，显著改进了图数据OOD泛化性能，为处理图数据中的复杂分布偏移提供了更有效的解决方案。
...
### [211] [Adversarial Flow Models](https://arxiv.org/abs/2511.22475)
*Shanchuan Lin,Ceyuan Yang,Zhijie Lin,Hao Chen,Haoqi Fan*

Main category: cs.LG

TL;DR: 对抗流模型统一了对抗模型和流模型，支持单步或多步生成，使用对抗目标训练，比传统GAN更稳定，比一致性方法更高效。


<details>
  <summary>Details</summary>
Motivation: 传统GAN存在训练不稳定问题，而流匹配模型需要学习概率流的中间时间步，导致模型容量浪费和误差累积。需要一种既能稳定训练又能高效生成的方法。

Method: 提出对抗流模型，将对抗模型与流模型统一。生成器学习确定性的噪声到数据的映射（与流匹配模型相同的最优传输），使用对抗目标训练，支持单步或多步生成，无需学习中间时间步。

Result: 在ImageNet-256px上，B/2模型在1NFE设置下接近一致性XL/2模型的性能，XL/2模型创造了新的最佳FID 2.38。通过深度重复端到端训练56层和112层模型，单次前向传播分别达到FID 2.08和1.94，超越了对应的2NFE和4NFE模型。

Conclusion: 对抗流模型成功统一了对抗模型和流模型的优势，实现了稳定训练和高效生成，在图像生成任务上取得了最先进的性能。
...
### [212] [Enhancing Trustworthiness with Mixed Precision: Benchmarks, Opportunities, and Challenges](https://arxiv.org/abs/2511.22483)
*Guanxi Lu,Hao Mark Chen,Zhiqiang Que,Wayne Luk,Hongxiang Fan*

Main category: cs.LG

TL;DR: 本文系统研究了量化对LLM可信度指标的影响，发现不同压缩比和量化方法会导致可信度不稳定，并提出了一种精度集成投票方法，在可信度指标上提升达5.8%。
...
Method: 系统研究了量化对四个可信度指标（对抗鲁棒性、公平性、机器伦理和分布外鲁棒性）的影响，识别了压缩比和量化方法导致的不稳定性，并开发了一种精度集成投票方法，利用同一模型混合精度变体的预测。

Result: 发现量化在不同压缩比和量化方法下会导致可信度指标的不稳定性，提出的精度集成投票方法在可信度指标上能持续提升性能达5.8%。

Conclusion: 开发模型压缩技术时考虑可信度至关重要，压缩与可信度的交叉研究为安全关键应用提供了新的研究机会，精度集成投票方法能有效提升量化LLM的可信度。
...
### [213] [Privacy-Utility-Bias Trade-offs for Privacy-Preserving Recommender Systems](https://arxiv.org/abs/2511.22515)
*Shiva Parsarad,Isabel Wagner*

Main category: cs.LG

TL;DR: 该研究全面评估了两种差分隐私机制（DPSGD和LDP）在四种推荐系统上的效果，发现隐私保护会降低推荐效用但影响不均，不同机制在不同隐私和数据条件下各有优劣。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统越来越多地采用差分隐私来保护用户数据，需要研究隐私机制如何影响推荐准确性和公平性，以指导实际应用中的机制选择。

Method: 对两种差分隐私机制（DPSGD和LDP）在四种推荐系统（NCF、BPR、SVD、VAE）上进行跨模型评估，使用MovieLens-1M和Yelp数据集。

Result: 更强的隐私保护会降低效用但不均匀：NCF在DPSGD下准确率损失最小（ε≈1时低于10%），SVD和BPR损失较大，VAE对隐私最敏感。DPSGD通常缩小热门与冷门物品推荐差距，LDP则更保持现有模式。

Conclusion: 没有单一的差分隐私机制在所有情况下都最优，每种机制在不同隐私制度和数据条件下提供不同的权衡，选择应基于具体应用场景。
...
### [214] [List-Decodable Regression via Expander Sketching](https://arxiv.org/abs/2511.22524)
*Herbod Pourali,Sajjad Hashemian,Ebrahim Ardeshir-Larijani*

Main category: cs.LG

TL;DR: 提出了一种基于扩展图草图技术的列表可解码线性回归框架，实现了接近最优的样本复杂度、列表大小和计算效率


<details>
  <summary>Details</summary>
Motivation: 解决列表可解码线性回归中样本复杂度、列表大小和计算效率之间的权衡问题，避免使用复杂的SoS方法，同时保持输入稀疏性

Method: 使用无损扩展图合成轻度污染的数据批次，通过鲁棒聚合和短谱滤波阶段实现高效处理

Result: 在标准亚高斯假设下，实现了样本复杂度$\tilde{O}((d+\log(1/δ))/α)$，列表大小$O(1/α)$，近输入稀疏运行时间$\tilde{O}(\mathrm{nnz}(X)+d^{3}/α)$

Conclusion: 提出的扩展图草图框架在避免SoS机制和显式批次结构的同时，匹配了已知最佳的高效保证，为列表可解码线性回归提供了实用的解决方案
...
### [215] [The Multiclass Score-Oriented Loss (MultiSOL) on the Simplex](https://arxiv.org/abs/2511.22587)
*Francesco Marchetti,Edoardo Legnaro,Sabrina Guastavino*

Main category: cs.LG

TL;DR: 本文提出了一种多类别分数导向损失函数(MultiSOL)，将二元分类中的分数导向损失扩展到多类别场景，直接优化目标性能指标，避免后验阈值调整。


<details>
  <summary>Details</summary>
Motivation: 在监督二元分类中，分数导向损失可以直接在训练阶段优化选定的性能指标，避免后验阈值调整。然而，这种方法在多类别分类中尚未得到充分探索。本文旨在将分数导向损失的优势扩展到多类别分类问题。

Method: 使用最近提出的多维阈值分类框架，将分数导向损失扩展到多类别分类，定义多类别分数导向损失(MultiSOL)函数。在构建过程中，将决策阈值视为具有特定先验分布的随机变量。

Result: 通过多个分类实验证明，MultiSOL保持了二元设置中的主要优势：直接优化目标指标、对类别不平衡的鲁棒性，性能与最先进的损失函数相当，并为单纯形几何与分数导向学习之间的相互作用提供了新见解。

Conclusion: MultiSOL成功地将分数导向损失扩展到多类别分类，保留了直接优化目标指标和鲁棒性的优势，为多类别分类中的阈值优化提供了新框架，并揭示了单纯形几何与分数导向学习之间的重要联系。
...
### [216] [LLM-Cave: A benchmark and light environment for large language models reasoning and decision-making system](https://arxiv.org/abs/2511.22598)
*Huanyu Li,Zongyuan Li,Wei Huang,Xian Guo*

Main category: cs.LG

TL;DR: LLM-Cave：一个轻量级基准测试环境，用于评估大语言模型在部分可观测状态下的序列推理和决策能力，弥补了现有基准测试单步交互的不足。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估基准主要局限于单步交互，而现有的序列决策环境（如TextStarCraftII和LLM-PySC2）过于复杂，需要数小时交互才能完成游戏。需要一种轻量级环境来评估LLM的序列推理和决策能力。

Method: 提出了LLM-Cave基准测试环境，这是一个符号主义时代的经典实例。环境允许智能体探索环境，利用部分可观测状态信息推理附近危险并避免潜在损失。评估了主流LLM（GPT-4o-mini、o1-mini、DeepSeek-R1）的序列推理能力、决策性能和计算效率。

Result: 实验显示：DeepSeek-R1在复杂推理任务上取得了最高成功率；而较小的模型如4o-mini通过采用思维链推测和规划-批判策略，显著缩小了性能差距，但牺牲了计算效率。结构化多步推理结合基于LLM的反馈机制能显著增强LLM的决策能力。

Conclusion: 结构化多步推理与基于LLM的反馈机制相结合可以显著提升LLM的决策能力，为改进较弱模型的推理能力提供了有前景的方向，并提出了以推理为中心的LLM评估新基准。
...
### [217] [Federated Learning Survey: A Multi-Level Taxonomy of Aggregation Techniques, Experimental Insights, and Future Frontiers](https://arxiv.org/abs/2511.22616)
*Meriem Arbaoui,Mohamed-el-Amine Brahmia,Abdellatif Rahmoun,Mourad Zghal*

Main category: cs.LG

TL;DR: 这篇论文对联邦学习进行了系统性综述，重点关注个性化、优化和鲁棒性三个研究方向，通过混合方法（文献计量分析与系统综述）对聚合策略、评估方法等进行了全面分析，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 物联网与人工智能的融合推动了行业创新，但隐私问题和数据孤岛限制了发展。传统的集中式机器学习难以应对这些挑战，因此需要联邦学习这种去中心化范式，能够在保护数据隐私的同时实现协作模型训练。

Method: 采用混合方法学，结合文献计量分析和系统综述，对联邦学习研究进行结构化分类。重点关注三个主要研究方向：个性化、优化和鲁棒性。分析聚合策略（包括架构、同步方法和联邦目标），并通过实验比较IID和非IID数据分布下的聚合方法。

Result: 论文提供了联邦学习领域的全面综述，系统分析了异构性、效率、安全性和隐私等挑战及相关技术。通过实验比较了不同聚合方法在IID和非IID数据分布下的表现，为联邦学习研究提供了实践评估方法。

Conclusion: 联邦学习作为解决数据隐私和孤岛问题的有效范式，虽然比集中式方法更复杂，但具有重要价值。论文为未来研究提供了指导方向，旨在推动这一快速发展领域的创新。
...
### [218] [Flow Density Control: Generative Optimization Beyond Entropy-Regularized Fine-Tuning](https://arxiv.org/abs/2511.22640)
...
<details>
  <summary>Details</summary>
Motivation: 现有微调方法主要关注最大化生成样本的期望奖励，但在现实应用中需要优化更广泛的效用函数（如风险规避、新颖性寻求、多样性度量等），同时以更通用的方式保持先验信息。

Method: 提出Flow Density Control (FDC)算法，将复杂的优化问题分解为一系列简单的微调任务，每个任务可通过现有可扩展方法解决。算法利用镜像流的最新理解，支持超越KL散度的更通用先验保持方式（如最优传输距离、Renyi散度）。

Result: 在文本到图像和分子设计任务上的验证表明，FDC能够引导预训练生成模型优化目标函数，解决当前微调方案无法处理的实用相关任务。

Conclusion: FDC为优化超越平均奖励的通用效用函数提供了有效框架，扩展了生成模型在现实应用中的适应能力，在保持先验信息的同时实现了更广泛的优化目标。
...
### [219] [Spatially Aware Dictionary-Free Eigenfunction Identification for Modeling and Control of Nonlinear Dynamical Systems](https://arxiv.org/abs/2511.22648)
*David Grasev*

Main category: cs.LG

TL;DR: 提出了一种无需预定义基函数的数据驱动Koopman特征函数发现新方法，通过参考轨迹识别Koopman模态振幅，转换到包含特征值和时间基本函数的新基，使用正则化最小二乘拟合和全局优化器优化特征值，成功在多个非线性动力系统上验证。


<details>
  <summary>Details</summary>
Motivation: 传统Koopman算子方法需要预定义基函数，限制了其在复杂非线性系统中的应用。本研究旨在开发一种无需预定义基函数的数据驱动方法，直接从数据中发现Koopman特征函数，提高预测精度和鲁棒性。

Method: 基于参考轨迹识别Koopman模态振幅，将模态分解转换到包含特征值和时间基本函数的新基；通过正则化最小二乘拟合获得特征函数初始值；使用全局优化器优化特征值；通过映射初始状态值到特征函数值揭示空间结构，数值计算梯度；惩罚Koopman偏微分方程的偏差以获得更鲁棒解。

Result: 方法在多个基准非线性动力系统上成功测试，包括带输入的FitzHugh-Nagumo系统、van der Pol和Duffing振荡器，以及带控制的2轴涡轮喷气发动机。研究表明，结合主特征值和空间结构完整性提升显著提高了Koopman预测器的准确性。即使在稀疏状态空间采样下也能有效发现Koopman谱分量，并揭示状态空间的几何特征（如不变分区）。

Conclusion: 该方法无需预定义基函数即可从数据中发现Koopman特征函数，通过数值近似特征函数梯度可用于输入动力学建模和控制设计。结果支持该方法在各种动力系统中的实用性，为非线性系统分析提供了更灵活有效的工具。
...
### [220] [Automated Design Optimization via Strategic Search with Large Language Models](https://arxiv.org/abs/2511.22651)
*Anthony Carreon,Vansh Sharma,Venkat Raman*

Main category: cs.LG

TL;DR: AUTO是一个基于大语言模型的智能体框架，将设计优化视为梯度自由搜索问题，通过战略推理指导优化过程，在GPU代码优化任务中表现出色。
...
Method: AUTO框架采用两个协作智能体：战略家（Strategist）负责在探索和利用策略之间选择，实施者（Implementor）负责执行详细设计。该框架将设计优化视为由战略LLM推理指导的梯度自由搜索问题。

Result: 在GPU代码优化领域（对机器学习和科学计算至关重要），AUTO生成的解决方案与专家实现相当。框架相对于贝叶斯优化方法实现了50-70%的搜索效率，每次优化约需8小时，估计成本最高159美元，而中等薪资软件开发者估计成本最高480美元。

Conclusion: AUTO框架为在定义不明确、先验信息有限的搜索空间中自动化设计优化打开了大门，展示了LLM在复杂设计优化任务中的潜力。
...
### [221] [Structure-aware Hybrid-order Similarity Learning for Multi-view Unsupervised Feature Selection](https://arxiv.org/abs/2511.22656)
...
<details>
  <summary>Details</summary>
Motivation: 现有多视图无监督特征选择方法主要使用一阶相似性图来保持局部结构，但往往忽略了二阶相似性能够捕捉的全局结构。少数使用预定义二阶相似性图的方法容易受到噪声和异常值的影响，导致特征选择性能不佳。

Method: SHINE-FS首先学习共识锚点和相应的锚图来捕捉锚点与样本之间的跨视图关系。基于获得的跨视图共识信息，生成样本的低维表示，通过识别判别性特征来重构多视图数据。然后利用锚点-样本关系学习二阶相似性图，最后联合学习一阶和二阶相似性图构建混合阶相似性图，同时捕捉局部和全局结构。

Result: 在真实多视图数据集上的综合实验结果表明，SHINE-FS优于最先进的方法。

Conclusion: SHINE-FS通过联合学习一阶和二阶相似性图，有效捕捉数据的局部和全局结构，显著提高了多视图无监督特征选择的性能。
...
### [222] [Difficulties with Evaluating a Deception Detector for AIs](https://arxiv.org/abs/2511.22662)
*Lewis Smith,Bilal Chughtai,Neel Nanda*

Main category: cs.LG

TL;DR: 论文探讨了AI欺骗检测器的可靠性评估问题，指出目前缺乏可明确标记为欺骗或诚实的训练样本，并分析了收集这些样本的具体障碍。


<details>
  <summary>Details</summary>
Motivation: 构建可靠的AI欺骗检测器对于减轻高级AI系统风险至关重要，但评估这些检测器的可靠性需要能够明确标记为欺骗或诚实的示例。

Method: 通过概念论证、现有实证研究分析以及新颖案例研究分析，识别收集欺骗/诚实示例的具体障碍，并评估现有实证解决方案的潜力。

Result: 研究发现目前缺乏必要的欺骗/诚实标记示例，识别了收集这些数据的多个具体障碍，现有实证解决方案虽然有价值但单独使用可能不足。

Conclusion: 在欺骗检测方面取得进展需要进一步考虑数据收集问题，当前的解决方案可能不足以完全解决评估AI欺骗检测器可靠性的挑战。
...
### [223] [Modèles de Fondation et Ajustement : Vers une Nouvelle Génération de Modèles pour la Prévision des Séries Temporelles](https://arxiv.org/abs/2511.22674)
...
<details>
  <summary>Details</summary>
Motivation: 受大语言模型启发，研究者开发了用于时间序列预测的基础模型，这些模型能在未见过的数据集上进行零样本预测，减少了对任务特定架构和手动调参的需求。

Method: 回顾了主要架构、预训练策略和优化方法，并研究了预训练后微调对特定数据集性能的影响，通过实证分析验证效果。

Result: 实证结果表明，微调通常能提升零样本预测能力，特别是在长期预测方面效果显著。

Conclusion: 基础模型结合微调策略是提升时间序列预测性能的有效方法，尤其在长期预测场景中表现突出。
...
### [224] [Test-time scaling of diffusions with flow maps](https://arxiv.org/abs/2511.22688)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在扩散模型中引入奖励梯度进行测试时优化存在问题，因为用户指定的奖励通常只在生成结束时定义在数据分布上。现有解决方案使用去噪器估计样本在生成结束时的状态，但这种方法不够理想

Method: 利用流映射与瞬时传输速度场之间的关系，构建FMTT算法。通过流映射直接处理奖励梯度问题，可以进行精确采样（通过重要性加权）或原则性搜索（识别奖励倾斜分布的局部最大化点）

Result: FMTT在理论上证明比标准测试时方法能更好地提升奖励分数。实验表明该方法优于其他前瞻技术，并能处理复杂的奖励函数，实现新的图像编辑形式（如与视觉语言模型交互）

Conclusion: 通过流映射直接处理扩散模型中的奖励梯度问题，提出了一个简单而有效的解决方案FMTT，该方法在理论上和实验上都优于现有方法，并能处理复杂的奖励函数，扩展了图像编辑的可能性
...
### [225] [Generative Anchored Fields: Controlled Data Generation via Emergent Velocity Fields and Transport Algebra](https://arxiv.org/abs/2511.22693)
*Deressa Wodajo Deressa,Hannes Mareen,Peter Lambert,Glenn Van Wallendael*

Main category: cs.LG

TL;DR: 提出Generative Anchored Fields (GAF)，一种学习独立端点预测器J（噪声）和K（数据）而非轨迹预测器的生成模型，通过J和K的时间条件差异产生速度场v=K-J，支持Transport Algebra实现组合控制。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型通常学习轨迹预测器，限制了组合控制能力。GAF旨在通过分解学习独立的噪声和数据端点预测器，实现更丰富的组合生成和可控传输。

Method: GAF学习时间条件的独立端点预测器J（噪声）和K（数据），速度场v=K-J从它们的差异中产生。通过Transport Algebra对学习的{(J_n,K_n)}头进行代数操作，实现组合控制。使用类别特定的K_n头支持从共享基分布到多模态的定向传输映射。

Result: 在CelebA-HQ 64×64上达到FID 7.5的强样本质量，提供组合生成作为架构原语。GAF在初始和最终状态之间实现无损循环传输（LPIPS=0.0）。

Conclusion: GAF通过分解端点预测器实现了组合可控生成，在保持高质量样本的同时提供了丰富的传输代数操作能力，为生成模型提供了新的架构范式。
...
### [226] [Integrated Transcriptomic-proteomic Biomarker Identification for Radiation Response Prediction in Non-small Cell Lung Cancer Cell Lines](https://arxiv.org/abs/2511.22735)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个整合转录组和蛋白质组数据的框架，以识别能够预测非小细胞肺癌细胞系辐射敏感性的并发生物标志物，解决单一组学数据预测能力有限的问题。

Method: 收集73个NSCLC细胞系的RNA-seq数据和46个细胞系的DIA-MS蛋白质组数据，保留1,605个共享基因。使用Lasso回归进行特征选择，构建支持向量回归模型，分别使用转录组、蛋白质组和整合数据集进行训练和评估。

Result: RNA-蛋白质表达呈显著正相关（中位数Pearson's r=0.363）。从三个数据集中识别出20个优先基因特征。单一组学模型在跨组学泛化能力有限，而整合模型在两个数据集中都表现出平衡的预测准确性（转录组R²=0.461，蛋白质组R²=0.604）。

Conclusion: 这是首个用于NSCLC SF2预测的蛋白质转录组学框架，突出了整合转录组和蛋白质组数据的互补价值。识别的并发生物标志物同时捕捉了转录调控和功能性蛋白质活性，提供了机制见解和转化潜力。
...
### [227] [VeriDispatcher: Multi-Model Dispatching through Pre-Inference Difficulty Prediction for RTL Generation Optimization](https://arxiv.org/abs/2511.22749)
...
Method: 提出VeriDispatcher框架：1）为每个LLM训练紧凑分类器，基于任务描述的语义嵌入和难度评分；2）难度评分来自结合语法、结构相似性和功能正确性的基准变体；3）在推理时使用预测器将任务路由到选定的LLM子集。

Result: 在RTLLM和VerilogEval基准上测试10个不同LLM：1）在RTLLM上使用仅40%的商业调用实现高达18%的准确率提升；2）在VerilogEval上保持准确性的同时减少25%的商业使用。
...
### [228] [Exact Learning of Arithmetic with Differentiable Agents](https://arxiv.org/abs/2511.22751)
...
Method: 引入可微分有限状态转换器（DFSTs），这是一种图灵完备的模型家族，支持恒定精度、恒定时间生成和端到端的对数并行可微分训练。利用专家智能体的策略轨迹观察，训练DFSTs执行二进制和十进制加法与乘法。

Result: 模型在极小数据集上训练后，能够无错误地泛化到比训练示例长数千倍的输入，实现了强大的长度泛化能力。

Conclusion: 通过在结构化中间监督上训练可微分智能体，可能为基于梯度的精确算法技能学习开辟新途径。
...
### [229] [CausalProfiler: Generating Synthetic Benchmarks for Rigorous and Transparent Evaluation of Causal Machine Learning](https://arxiv.org/abs/2511.22842)
*Panayiotis Panayiotou,Audrey Poinsot,Alessandro Leite,Nicolas Chesneau,Marc Schoenauer,Özgür Şimşek*

Main category: cs.LG

TL;DR: CausalProfiler是一个用于因果机器学习方法的合成基准测试生成器，通过随机采样因果模型、数据、查询和真实值来创建多样化的评估环境，填补了现有因果机器学习评估实践的不足。


<details>
  <summary>Details</summary>
Motivation: 当前因果机器学习（Causal ML）的实证评估实践有限，现有基准测试通常依赖于少量手工制作或半合成数据集，导致结论脆弱且不可泛化。需要一种更系统、透明的评估方法来填补这一空白。

Method: 提出CausalProfiler，一个基于明确设计选择的合成基准测试生成器。它随机采样因果模型、数据、查询和真实值，构成合成因果基准测试。该方法在观察、干预和反事实三个因果推理层面上运行，具有覆盖保证和透明假设。

Result: CausalProfiler能够对因果机器学习方法在各种条件下进行严格透明的评估，包括在识别机制内外的场景。通过评估多个最先进的方法，展示了该工具能够实现的分析类型和洞察。

Conclusion: CausalProfiler是第一个具有覆盖保证和透明假设的随机合成因果基准测试生成器，能够在三个因果推理层面上运行，为因果机器学习方法提供了更全面、可靠的评估框架。
...
### [230] [TARFVAE: Efficient One-Step Generative Time Series Forecasting via TARFLOW based VAE](https://arxiv.org/abs/2511.22853)
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式时间序列预测方法大多采用循环生成操作或重复去噪步骤，导致预测过程繁琐，特别是在长期预测中效率低下。这些方法通常只在短期预测中进行实验，与确定性方法在长期预测中的比较有限，使得其实际优势不明确。

Method: 提出TARFVAE框架，结合Transformer自回归流（TARFLOW）和变分自编码器（VAE）。TARFLOW模块增强VAE的后验估计，打破高斯假设，创建更具信息量的潜在空间。该框架仅使用TARFLOW的前向过程，避免自回归逆操作，实现快速生成。通过从先验潜在空间采样，直接通过VAE解码器生成完整预测范围。

Result: TARFVAE在多个基准数据集的不同预测范围上，超越了最先进的确定性和生成模型，同时保持了高效的预测速度。使用简单的MLP模块就能实现优越性能。

Conclusion: TARFVAE为生成式时间序列预测提供了一个高效且强大的解决方案，通过创新的TARFLOW-VAE组合实现了快速单步预测，在准确性和效率方面都表现出色。
...
### [231] [CRAwDAD: Causal Reasoning Augmentation with Dual-Agent Debate](https://arxiv.org/abs/2511.22854)
*Finn G. Vamosi,Nils D. Forkert*

Main category: cs.LG

TL;DR: 本文提出了一种基于双智能体辩论框架的因果推理方法，通过让一个模型进行结构化因果推理，另一个模型批判性地检查逻辑缺陷，通过辩论和说服过程提升因果推理的准确性。


<details>
  <summary>Details</summary>
Motivation: 人类在进行因果推理时会考虑多种"如果"场景，类似地，语言模型也可以通过考虑多种干预和反事实来判断因果主张的有效性。这种推理更像是内部对话而非单一计算，但目前语言模型在这方面的潜力尚未充分探索。

Method: 采用双智能体辩论框架：一个模型提供结构化因果推理，另一个模型批判性地检查推理中的逻辑缺陷。当出现分歧时，智能体试图说服对方，挑战彼此的逻辑并修订结论，直到达成共识。特别使用了推理语言模型，这些模型在因果推理和对抗性辩论方面的优势相对于标准大语言模型尚未充分开发。

Result: 在CLadder数据集（涵盖Pearl因果阶梯所有三个层级的基准）上评估，使用Qwen3和DeepSeek-R1作为辩论智能体。多智能体辩论将DeepSeek-R1的总体准确率从78.03%提升到87.45%，反事实类问题准确率从67.94%提升到80.04%。Qwen3的总体准确率从84.16%提升到89.41%，反事实问题从71.53%提升到80.35%。

Conclusion: 多智能体辩论显著提升了因果推理的准确性，即使是强模型也能通过与较弱智能体的辩论获得巨大收益。研究结果突显了推理模型作为多智能体系统构建模块在因果推理中的潜力，并展示了多样化视角在因果问题解决中的重要性。
...
### [232] [Bridging Modalities via Progressive Re-alignment for Multimodal Test-Time Adaptation](https://arxiv.org/abs/2511.22862)
*Jiacheng Li,Songhe Feng*

Main category: cs.LG

TL;DR: 本文提出了一种名为BriMPR的多模态测试时自适应框架，通过渐进式重对齐策略解决多模态场景中的分布偏移耦合效应问题。
...
Method: 提出BriMPR框架，采用分而治之策略：1) 将MMTTA分解为多个单模态特征对齐子问题，利用提示调优校准单模态全局特征分布；2) 为掩码和完整模态组合分配可信伪标签，引入模态间实例级对比学习增强信息交互和细化对齐。

Result: 在包括基于损坏和真实世界域偏移基准的MMTTA任务上进行广泛实验，证明了该方法的优越性。

Conclusion: BriMPR框架通过渐进式重对齐策略有效解决了多模态测试时自适应中的分布偏移耦合效应问题，为多模态TTA提供了有效解决方案。
...
### [233] [Covering-Space Normalizing Flows: Approximating Pushforwards on Lens Spaces](https://arxiv.org/abs/2511.22882)
*William Ghanem*

Main category: cs.LG

TL;DR: 本文构建了通过通用覆盖映射从S^3到透镜空间L(p;q)的推前分布，并利用L(p;q)上的流来近似这些分布，特别在对称S^3分布情况下能消除冗余性。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过通用覆盖映射将S^3上的分布推前到透镜空间L(p;q)上，并开发有效的近似方法，特别关注在对称分布情况下消除冗余性，为化学分子建模等应用提供数学工具。

Method: 使用通用覆盖映射ρ: S^3 → L(p;q)构建推前分布，通过在L(p;q)上设计流来近似这些分布，特别处理对称S^3分布情况以删除冗余性。

Result: 成功近似了von Mises-Fisher诱导的目标密度推前分布，以及为苯分子建模而构建的Z_12对称玻尔兹曼分布在S^3上的推前分布。

Conclusion: 提出的方法能够有效地通过透镜空间上的流来近似推前分布，在对称分布情况下能有效消除冗余性，为复杂分子系统的建模提供了有效的数学框架。
...
### [234] [Modeling Chaotic Pedestrian Behavior Using Chaos Indicators and Supervised Learning](https://arxiv.org/abs/2511.22887)
*Md. Muhtashim Shahrier,Nazmul Haque,Md Asif Raihan,Md. Hadiuzzaman*

Main category: cs.LG

TL;DR: 该研究提出了一种基于数据驱动的框架，使用监督学习模型来量化行人运动中的混沌行为，通过混沌指标和机器学习预测行人行为不稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着全球城市致力于提高步行性和安全性，理解行人行为的不规则和不可预测性变得越来越重要。需要一种能够量化行人混沌运动的方法来识别高风险区域和改进基础设施。

Method: 研究使用计算机视觉技术提取白天和夜间条件下的行人轨迹，通过近似熵和李雅普诺夫指数等四个混沌指标量化行为混沌度，应用主成分分析整合为统一混沌分数，然后使用随机森林和CatBoost回归模型基于个体、群体和交通特征进行预测。

Result: CatBoost模型表现最佳，白天PCA-based CatBoost模型的R^2达到0.8319，夜间模型达到0.8574。SHAP分析显示行程距离、运动持续时间和速度变异性是混沌行为的主要贡献因素。

Conclusion: 该框架能够量化现实环境中的行为不稳定性，帮助规划者和工程师识别高风险行人区域、改进基础设施并校准微观仿真模型，同时支持自动驾驶系统的自适应风险评估。
...
### [235] [Adversarial Training for Process Reward Models](https://arxiv.org/abs/2511.22888)
*Gurusha Juneja,Deepak Nathani,William Yang Wang*

Main category: cs.LG

TL;DR: APRM通过对抗训练提升过程奖励模型的鲁棒性，无需人工标注就能生成更难的反例，显著提高数学推理任务的准确性


<details>
  <summary>Details</summary>
Motivation: 过程奖励模型（PRMs）虽然能通过步骤级监督提升LLMs的推理能力，但面临两个主要限制：1）人工步骤级标注成本高昂；2）静态训练数据对新型错误的泛化能力差

Method: 提出对抗训练的过程奖励模型（APRM），包含生成器（G）和判别器（R）。G学习生成欺骗R的推理错误，R学习检测这些错误。这种对抗交互为R提供渐进式更难的反例，提升其鲁棒性和泛化能力

Result: 在多样化的数学推理基准测试中，APRM比最强的PRM基线平均提升3.4个百分点。在分布外任务上获得5.3个百分点的提升

Conclusion: APRM通过对抗训练有效解决了PRMs的两个主要限制，无需人工步骤级标注就能显著提升模型对推理错误的检测能力和泛化性能
...
### [236] [EnECG: Efficient Ensemble Learning for Electrocardiogram Multi-task Foundation Model](https://arxiv.org/abs/2511.22935)
*Yuhao Xu,Xiaoda Wang,Jiaying Lu,Sirui Ding,Defu Cao,Huaxiu Yao,Yan Liu,Xiao Hu,Carl Yang*

Main category: cs.LG

TL;DR: EnECG是一个基于专家混合的集成学习框架，专门用于心电图的多元任务分析，通过集成多个专业基础模型并采用轻量级适应策略来提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有心电图分析模型未能充分利用各种心脏异常之间的相互关系，而开发一个能提取所有相关特征用于多个心电图任务的特定模型又面临重大挑战。大型基础模型虽然强大，但通常未在心电图数据上预训练，完全重新训练或微调计算成本高昂。

Method: 提出EnECG框架：1) 集成多个专门的心电图解释专业基础模型；2) 采用轻量级适应策略：为每个基础模型附加专用输出层，仅对新添加的参数应用低秩适应(LoRA)；3) 采用专家混合(MoE)机制学习集成权重，有效结合各个模型的互补专业知识。

Result: 实验结果表明，通过最小化微调范围，EnECG能够帮助降低计算和内存成本，同时保持基础模型的强大表示能力。该框架不仅增强了特征提取和预测性能，还确保了实际临床应用的效率。

Conclusion: EnECG通过集成多个专业基础模型并采用轻量级适应策略，为心电图多元任务分析提供了一个高效实用的解决方案，在保持性能的同时显著降低了计算成本，具有实际临床应用的潜力。
...
TL;DR: 将课程学习重新定义为多臂老虎机问题，提出ONLINESUBMOD在线贪心策略，通过验证驱动的奖励指标指导课程进度，在视觉和语言数据集上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统课程学习从简单到困难样本进行，但定义可靠的难度概念具有挑战性。先前工作使用子模函数来诱导难度分数，但需要更原则性的方法来指导课程进度。

Method: 将自适应子集选择重新解释为多臂老虎机问题，每个臂对应一个指导样本选择的子模函数。提出ONLINESUBMOD在线贪心策略，优化效用驱动的奖励，并在各种采样机制下证明实现无遗憾性能。

Result: ONLINESUBMOD在视觉和语言数据集上优于传统课程学习和双层优化方法，展现出更优的准确率-效率权衡。验证驱动的奖励指标为课程进度提供了原则性指导。

Conclusion: 将课程学习重新定义为多臂老虎机问题，结合验证驱动的奖励指标，提供了一种原则性的课程进度指导方法，ONLINESUBMOD策略在实践中表现出优越性能。
...
### [238] [Experts are all you need: A Composable Framework for Large Language Model Inference](https://arxiv.org/abs/2511.22955)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：1）混合专家模型需要联合预训练且不支持多步推理；2）多智能体框架依赖顺序循环导致高延迟。需要一种既能实现专家协作又能降低延迟的解决方案。

Method: Comp-LLM包含三个组件：1）子查询生成器，分解输入查询，基于嵌入相似度为每个子查询分配专家，构建依赖图；2）查询执行器，处理图中的节点，基于依赖和资源约束识别并行机会；3）响应聚合器，将中间专家响应合成为连贯的最终答案。

Result: 在多个基准测试中，Comp-LLM相比相似大小的单体LLM实现了最高11.01%的准确率提升，模型大小减少1.67-3.56倍，相对于家族中最大模型没有显著性能下降，且比顺序子查询处理延迟降低1.1-1.7倍。

Conclusion: Comp-LLM通过可组合推理框架有效解决了混合专家模型和多智能体框架的局限性，实现了专家协作、模型大小减少和延迟降低的多重优势。
...
### [239] [A Trainable Centrality Framework for Modern Data](https://arxiv.org/abs/2511.22959)
*Minh Duc Vu,Mingshuo Liu,Doudou Zhou*

Main category: cs.LG

TL;DR: FUSE是一个神经中心性框架，通过全局头部和局部头部的融合，在任意数据表示上计算中心性得分，在多种数据类型上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统深度概念在高维空间中计算昂贵且不稳定，难以扩展到非欧几里得数据，需要一种更通用、高效的中心性度量方法。

Method: FUSE结合全局头部（基于成对距离比较学习无锚点中心性得分）和局部头部（通过去噪得分匹配近似平滑对数密度势），通过单一参数在0-1间插值融合这两个校准信号。

Result: 在合成分布、真实图像、时间序列、文本数据和标准异常检测基准上，FUSE恢复了有意义的经典排序，揭示了多尺度几何结构，与强基线方法相比具有竞争力。

Conclusion: FUSE提供了一个简单高效的神经中心性框架，能够在各种数据表示上计算深度类中心性，为鲁棒估计、排序和异常检测提供了新工具。
...
TL;DR: Delta-XAI提出了一种在线时间序列监控模型解释框架，通过包装函数适配14种现有XAI方法，并引入系统性评估套件。研究发现梯度方法在时序分析中表现优异，进而提出SWING方法，通过整合过去观测来捕捉时序依赖并缓解分布外效应。


<details>
  <summary>Details</summary>
Motivation: 在医疗和金融等敏感领域，在线时间序列监控模型的解释至关重要。现有XAI方法大多独立分析每个时间步，忽略了时序依赖性，导致解释预测变化困难、无法利用在线动态、评估困难等挑战。

Method: 1) 提出Delta-XAI框架，通过包装函数适配14种现有XAI方法；2) 引入针对在线场景的原则性评估套件，评估忠实性、充分性、一致性等多方面；3) 基于实验结果提出SWING方法，在积分路径中整合过去观测以系统捕捉时序依赖并缓解分布外效应。

Result: 实验表明经典梯度方法（如Integrated Gradients）在适配时序分析后能超越最新方法。SWING方法在多样化设置和多种指标下均表现出持续有效性。

Conclusion: Delta-XAI为在线时间序列监控模型解释提供了系统框架和评估方法，SWING方法通过整合历史信息有效捕捉时序依赖，在敏感领域应用中具有重要价值。
...
### [241] [Freeze, Diffuse, Decode: Geometry-Aware Adaptation of Pretrained Transformer Embeddings for Antimicrobial Peptide Design](https://arxiv.org/abs/2511.23120)
...
Result: 应用于抗菌肽设计时，FDD产生了低维、可预测且可解释的表征，支持属性预测、检索和潜在空间插值。

Conclusion: FDD提供了一种在适应下游任务时保持预训练嵌入几何结构的新方法，特别适用于监督数据稀缺的场景。
...
### [242] [Adapting Neural Audio Codecs to EEG](https://arxiv.org/abs/2511.23142)
*Ard Kastrati,Luca Lanzendörfer,Riccardo Rigoni,John Staib Matilla,Roger Wattenhofer*

Main category: cs.LG

TL;DR: 研究显示预训练的神经音频编解码器（如DAC）经过适当预处理后可直接用于EEG压缩，无需修改架构即可获得稳定重建，微调后效果优于从头训练。
...
Result: 即使不修改架构，该设置也能产生稳定的EEG重建，在EEG数据上微调可进一步提高保真度和泛化能力。在TUH异常和癫痫数据集上的评估显示，适应后的编解码器保留了临床相关信息，体现在基于频谱图的重建损失和下游分类准确性上。

Conclusion: 预训练的神经音频编解码器可以有效地适应EEG压缩任务，通过适当的预处理和微调，能够保留临床相关信息，为跨模态迁移学习提供了有前景的途径。
...
### [243] [A Theoretical Framework for Discovering Groups and Unitary Representations via Tensor Factorization](https://arxiv.org/abs/2511.23152)
...
<details>
  <summary>Details</summary>
Motivation: 理解HyperCube模型为何能够发现群结构和酉表示的数学机制，解释其内在的归纳偏倚原理

Method: 将目标函数分解为尺度调节项(ℬ)和方向对齐项(ℛ≥0)，分离出共线流形(ℛ=0)，证明该流形仅对群同构存在可行解，且ℬ在流形内施加向酉性的变分压力

Result: 在共线主导猜想下证明：(1)全局最小值由群的酉正则表示实现；(2)非群操作具有严格更高的目标值，量化了模型向群结合结构（同构）的归纳偏倚

Conclusion: HyperCube模型通过数学机制自然地偏向发现群结构和酉表示，为理解深度学习中的代数结构发现提供了理论框架
...
### [244] [Estimating the Event-Related Potential from Few EEG Trials](https://arxiv.org/abs/2511.23162)
*Anders Vestergaard Nørskov,Kasper Jørgensen,Alexander Neergaard Zahid,Morten Mørup*

Main category: cs.LG

TL;DR: EEG2ERP是一种基于不确定性感知自编码器的新方法，可将任意数量的EEG试验映射到相关ERP，显著减少所需试验次数，在零样本场景下优于传统平均方法。


<details>
  <summary>Details</summary>
Motivation: 传统ERP估计需要大量EEG试验的平均来降低噪声和信号变异性，这限制了ERP研究的效率和应用。需要开发能够减少所需试验次数的新方法。

Method: 提出EEG2ERP不确定性感知自编码器方法：1）使用自编码器架构将任意数量EEG试验映射到ERP；2）采用引导训练目标处理ERP不确定性；3）引入单独的方差解码器建模ERP估计的不确定性。

Result: 在三个公开数据集（ERP CORE、P300拼写器BCI、面部感知神经影像）的零样本泛化测试中，EEG2ERP在少量试验情况下显著优于传统平均方法和鲁棒平均方法。

Conclusion: EEG2ERP是首个将EEG信号映射到相关ERP的深度学习方法，能够显著减少ERP研究所需的试验次数，推动ERP研究向更高效的方向发展。
...
### [245] [Energy-Efficient Vision Transformer Inference for Edge-AI Deployment](https://arxiv.org/abs/2511.23166)
*Nursultan Amanzhol,Jurn-Gyu Park*

Main category: cs.LG

TL;DR: 提出两阶段评估流程，结合设备无关模型选择与设备相关测量，评估13个ViT模型在边缘设备上的能效表现。


<details>
  <summary>Details</summary>
Motivation: 随着视觉Transformer在能耗受限设备上的部署增加，需要超越单纯准确率的评估方法，综合考虑能效表现。

Method: 两阶段评估流程：第一阶段使用NetScore指标进行设备无关模型筛选；第二阶段使用可持续准确率指标(SAM)结合设备测量进行模型排序。

Result: 混合模型LeViT_Conv_192在TX2上相比ViT基线节能53%；蒸馏模型TinyViT-11M_Distilled在移动GPU上表现最佳。

Conclusion: 提出的两阶段评估方法能有效识别不同设备上的高效ViT模型，为能耗受限设备上的模型选择提供指导。
...
### [246] [Hard-Constrained Neural Networks with Physics-Embedded Architecture for Residual Dynamics Learning and Invariant Enforcement in Cyber-Physical Systems](https://arxiv.org/abs/2511.23307)
...
<details>
  <summary>Details</summary>
Motivation: 针对具有未知动力学和代数不变量的微分方程所描述的复杂信息物理系统，需要一种既能学习未知动态又能严格保持物理约束的学习框架。

Method: 1. 提出HRPINN架构，将已知物理作为硬结构约束嵌入循环积分器中，仅学习残差动态；2. 提出PHRPINN扩展，引入预测-投影机制，通过设计严格强制执行代数不变量。

Result: 在真实世界电池预测DAE上验证HRPINN，在标准约束基准测试套件上评估PHRPINN，结果显示框架具有高精度和数据效率，同时揭示了物理一致性、计算成本和数值稳定性之间的权衡。

Conclusion: 该框架为复杂信息物理系统的物理信息学习提供了有效解决方案，通过理论分析和实验验证展示了其潜力，并为实际部署提供了关于物理一致性、计算效率和数值稳定性权衡的实用指导。
...
### [247] [Towards Understanding Transformers in Learning Random Walks](https://arxiv.org/abs/2511.23239)
*Wei Shi,Yuan Cao*

Main category: cs.LG

TL;DR: 本文从理论角度分析单层Transformer在学习随机游走模型时的能力和可解释性，证明梯度下降训练后能达到最优预测精度，并揭示其内部机制相当于一个可解释的令牌选择器和概率转移执行器。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在实际应用中表现出色，但缺乏理论解释性和可解释性。本文旨在从理论上理解Transformer在经典统计模型（圆上随机游走）学习中的能力和内部工作机制。

Method: 研究单层Transformer模型学习圆上随机游走的能力，通过理论分析证明梯度下降训练后能达到最优预测精度，并分析其内部注意力机制如何作为令牌选择器，值矩阵如何执行概率转移。

Result: 理论证明单层Transformer在梯度下降训练后能实现随机游走的最优预测精度，且模型具有可解释性：softmax注意力作为令牌选择器关注直接父状态，值矩阵基于该父状态执行一步概率转移来预测下一个状态位置。

Conclusion: 本文为Transformer在经典统计模型学习中的能力和可解释性提供了理论支持，揭示了梯度下降在小初始化时可能在某些简单任务中失败或难以收敛的局限性，并通过实验验证了理论发现。
...
### [248] [BanglaSentNet: An Explainable Hybrid Deep Learning Framework for Multi-Aspect Sentiment Analysis with Cross-Domain Transfer Learning](https://arxiv.org/abs/2511.23264)
*Ariful Islam,Md Rifat Hossen,Tanvir Mahmud*

Main category: cs.LG

TL;DR: BanglaSentNet：一个用于孟加拉语电商评论多维度情感分析的可解释混合深度学习框架，集成了LSTM、BiLSTM、GRU和BanglaBERT，通过动态加权集成学习实现，在8,755条标注数据上达到85%准确率，并提供SHAP特征归因和注意力可视化增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语电商评论的多维度情感分析面临挑战：标注数据集有限、形态复杂性、代码混合现象和领域迁移问题，影响3亿孟加拉语用户。现有方法缺乏可解释性和跨领域泛化能力，难以实际部署。

Method: 提出BanglaSentNet框架，集成LSTM、BiLSTM、GRU和BanglaBERT通过动态加权集成学习进行多维度情感分类。引入包含8,755条手动标注的孟加拉语产品评论数据集，涵盖质量、服务、价格、装饰四个维度。框架包含SHAP特征归因和注意力可视化提供透明洞察。

Result: BanglaSentNet达到85%准确率和0.88 F1分数，优于独立深度学习模型3-7%，显著超越传统方法。可解释性套件获得9.4/10可解释性评分，人类一致性达87.6%。跨领域迁移学习实验显示强大泛化能力：零样本性能在不同领域保持67-76%有效性；少样本学习仅需500-1000样本即可达到完全微调性能的90-95%。

Conclusion: 该研究为孟加拉语情感分析建立了新的最先进基准，推进了低资源语言的集成学习方法，并为商业应用提供了可行的解决方案。实际部署展示了在孟加拉国电商平台中的实用价值，支持数据驱动的定价优化、服务改进和客户体验提升。
...
### [249] [ASTRO: Adaptive Stitching via Dynamics-Guided Trajectory Rollouts](https://arxiv.org/abs/2511.23442)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习从预收集的数据集中学习最优策略，但包含子优和碎片化轨迹的数据集给奖励传播带来挑战，导致价值估计不准确和政策性能下降。现有的基于生成模型的轨迹拼接方法要么局限于行为策略的支持范围，要么违反底层动态约束，限制了政策改进的效果。
...
Result: ASTRO在OGBench套件上显著优于先前的离线RL增强方法，在标准离线RL基准测试（如D4RL）上也表现出一致的改进。
...
### [250] [Beyond Curve Fitting: Neuro-Symbolic Agents for Context-Aware Epidemic Forecasting](https://arxiv.org/abs/2511.23276)
*Joongwon Chae,Runming Wang,Chen Xiong,Gong Yunhan,Lian Zhang,Ji Jiansong,Dongmei Yu,Peiwu Qin*

Main category: cs.LG

TL;DR: 提出一个两智能体框架，将上下文解释与概率预测解耦，使用LLM处理异构信号生成传播影响信号，再结合历史病例数据产生校准的概率预测，在手足口病监测中实现竞争性预测精度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统模型和基础模型虽然能纳入协变量，但缺乏语义推理能力来解释冲突驱动因素之间的因果相互作用，无法有效处理手足口病监测中流行病学模式与学校日历、天气等上下文驱动因素的关系。
...
Conclusion: 通过LLM结构化整合领域知识可以匹配最先进的性能，同时产生与公共卫生工作流程一致的上下文感知预测，为疾病监测提供可解释的预测框架。
...
### [251] [Closing the Generalization Gap in Parameter-efficient Federated Edge Learning](https://arxiv.org/abs/2511.23282)
*Xinnong Du,Zhonghao Lyu,Xiaowen Cao,Chunyang Wen,Shuguang Cui,Jie Xu*

Main category: cs.LG

TL;DR: 提出了一种参数高效的联邦边缘学习框架，通过联合模型剪枝和客户端选择来应对数据异构和资源受限的挑战，将泛化分析嵌入收敛分析中，并通过联合优化实现高效学习。


<details>
  <summary>Details</summary>
Motivation: 联邦边缘学习面临本地数据集有限且异构、资源受限部署等问题，这会严重影响模型泛化能力和资源利用率，从而损害学习性能。

Method: 提出参数高效的FEEL框架，联合利用模型剪枝和客户端选择；推导信息论泛化界限并嵌入收敛分析；构建泛化感知的平均平方梯度范数界最小化问题，联合优化剪枝率、客户端选择和通信计算资源；使用交替优化算法解决混合整数规划问题。

Result: 实验表明，所提出的设计在多个数据集上实现了优于现有基线方法的学习性能，验证了将泛化感知分析与系统级优化耦合的有效性。

Conclusion: 通过联合模型剪枝、客户端选择和资源优化，提出的参数高效FEEL框架能够有效应对数据异构和资源约束，显著提升联邦边缘学习的性能。
...
### [252] [Emergent Coordination and Phase Structure in Independent Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2511.23315)
*Azusa Yamaguchi*

Main category: cs.LG

TL;DR: 该研究通过大规模实验揭示了去中心化多智能体强化学习中的三阶段相结构：协调稳定相、脆弱过渡区和阻塞无序相，由核漂移和同步的相互作用驱动。


<details>
  <summary>Details</summary>
Motivation: 为了更好理解去中心化多智能体强化学习中协调何时出现、波动或崩溃的动态特性，研究者需要表征多智能体学习系统的动力学行为。

Method: 使用完全独立Q学习（IQL）作为最小化去中心化测试平台，在不同环境大小L和智能体密度ρ下进行大规模实验。构建基于合作成功率（CSR）和TD误差方差稳定性指数的相图，并进行同步性分析。

Result: 实验揭示了三个不同的相：协调稳定相、脆弱过渡区和阻塞无序相，由尖锐的双不稳定脊分隔。核漂移（智能体有效转移核随时间变化）是主要驱动因素，同步分析表明时间对齐对持续合作至关重要。移除智能体标识符会完全消除漂移并破坏三相结构。

Conclusion: 去中心化MARL表现出由规模、密度和核漂移相互作用支配的相干相结构，表明涌现的协调行为是一种分布-相互作用驱动的相现象，微小的智能体间不对称性是核漂移的必要驱动因素。
...
### [253] [Learning-Augmented Online Bipartite Matching in the Random Arrival Order Model](https://arxiv.org/abs/2511.23388)
*Kunanon Burathep,Thomas Erlebach,William K. Moses*

Main category: cs.LG

TL;DR: 本文研究了随机到达顺序模型中的在线无权二分图匹配问题，在增强学习设置下：算法接收在线顶点类型（邻域）的不可信预测。作者改进了Choo等人的工作，去除了最优匹配大小为n的假设，仅要求预测匹配大小至少为αn（0<α≤1）。


<details>
  <summary>Details</summary>
Motivation: 现有研究（Choo et al., ICML 2024）在增强学习设置下处理在线二分图匹配问题时，假设最优匹配大小为n（即每个在线顶点都能匹配）。这种假设限制了算法的应用范围。本文旨在去除这一限制，使算法适用于更一般的情况。

Method: 使用到达序列的前缀作为样本来判断预测是否接近真实到达序列。根据判断结果，要么遵循预测，要么使用忽略预测的基线算法（β-竞争性）。关键改进是去除了最优匹配大小为n的假设，仅要求预测匹配大小至少为αn（α为常数）。

Result: 提出的学习增强算法实现了(1-o(1))-一致性和(β-o(1))-鲁棒性。此外，竞争比随着预测误差的增加在一致性和鲁棒性之间平滑退化。

Conclusion: 本文成功将Choo等人的方法推广到更一般的情况，去除了对最优匹配大小的限制，同时保持了良好的一致性和鲁棒性保证，为在线二分图匹配问题的学习增强算法提供了更通用的解决方案。
...
### [254] [SmallWorlds: Assessing Dynamics Understanding of World Models in Isolated Environments](https://arxiv.org/abs/2511.23465)
*Xinyi Li,Zaishuo Xia,Weyl Lu,Chenjie Hao,Yubei Chen*

Main category: cs.LG

TL;DR: SmallWorld Benchmark：一个用于系统评估世界模型能力的测试平台，在完全可观测状态下对多种架构进行实验，揭示模型捕捉环境结构的能力和长期预测的退化情况。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型缺乏统一和受控的系统评估设置，难以判断它们是否真正捕捉到了环境动态的底层规则。

Method: 引入SmallWorld Benchmark测试平台，在完全可观测状态空间中对循环状态空间模型、Transformer、扩散模型和神经ODE等代表性架构进行实验，涵盖六个不同领域。

Result: 实验结果揭示了这些模型捕捉环境结构的有效性，以及它们在长期推演中预测性能的退化情况，突出了当前建模范式的优势和局限性。

Conclusion: 该研究为世界模型能力评估提供了系统化测试平台，揭示了当前建模方法的优缺点，为表示学习和动态建模的未来改进方向提供了见解。
...
### [255] [ThetaEvolve: Test-time Learning on Open Problems](https://arxiv.org/abs/2511.23473)
...
TL;DR: ThetaEvolve是一个开源的数学发现框架，通过强化学习在测试时持续学习，使小型开源模型能在开放优化问题上取得新的最佳边界。


<details>
  <summary>Details</summary>
Motivation: AlphaEvolve等现有系统依赖前沿大模型且是纯推理系统，模型无法内化演化策略。需要开源框架来简化扩展AlphaEvolve，使模型能从优化经验中持续学习。

Method: ThetaEvolve采用单一LLM、大型程序数据库增强探索、批量采样提高吞吐量、懒惰惩罚避免停滞输出、可选奖励塑形提供稳定训练信号，支持测试时强化学习。

Result: ThetaEvolve首次使小型开源模型（如DeepSeek-R1-0528-Qwen3-8B）在AlphaEvolve提到的开放问题（圆包装和自相关不等式）上取得新的最佳边界。在2个模型和4个开放任务上，测试时RL始终优于纯推理基线。

Conclusion: ThetaEvolve是一个有效的开源数学发现框架，通过测试时强化学习使模型能够学习演化能力，在目标任务和未见任务上都表现出更好的性能和更快的进展。
...
### [256] [Reducing research bureaucracy in UK higher education: Can generative AI assist with the internal evaluation of quality?](https://arxiv.org/abs/2511.21790)
*Gordon Fletcher,Saomai Vu Khan,Aldus Greenhill Fletcher*

Main category: cs.CY

TL;DR: 研究探讨生成式AI在UK高等教育研究质量评估中的辅助作用，通过ChatGPT对REF 2021商业管理论文进行评分排名，建立与官方结果对齐的评分边界，证明AI能提供一致性评估并减轻传统评审的资源负担。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决英国高等教育研究评估（特别是REF）中传统内部评审过程资源消耗巨大的问题，探索生成式AI如何辅助评估过程，减少官僚成本，同时保持学术完整性。

Method: 采用可行系统模型中的功能替代视角，使用ChatGPT对REF 2021提交的822篇商业管理论文进行评分和排名，通过"反向工程"方法将AI生成分数与已知机构结果进行比较，建立评分边界。

Result: 通过11个机构的822篇论文测试，建立了与REF结果对齐的评分边界：1*和2*之间为49%，2*和3*之间为59%，3*和4*之间为69%。AI能提供一致性评估，识别需要额外人工审查的边界案例。

Conclusion: 研究提出了一个有前景的框架，支持采用细致混合方法应用AI，在保持学术完整性的同时解决研究评估官僚主义的巨额成本，实现更高效、一致的研究评估方法转型。
...
### [257] [Dark Speculation: Combining Qualitative and Quantitative Understanding in Frontier AI Risk Analysis](https://arxiv.org/abs/2511.21838)
*Daniel Carpenter,Carson Ezell,Pratyush Mallick,Alexandria Westray*

Main category: cs.CY

TL;DR: 该论文提出"黑暗推测"方法，通过结合情景生成和保险精算分析来应对前沿AI的灾难性风险评估中的深度模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 评估前沿AI的灾难性危害受到深度模糊性的阻碍：许多风险不仅未被观察到，甚至未被分析师预见。当前风险分析的核心局限是无法填充"灾难性事件空间"，即无法确定可能分配概率的潜在大规模危害集合。这种困难因"卢克莱修问题"（仅从过去经验推断未来风险的倾向）而加剧。

Method: 提出"黑暗推测"过程，将系统生成和精炼灾难性情景（定性工作）与估计其可能性和相关损害（定量精算分析）相结合。使用简化的灾难性Lévy随机框架形式化这一过程，并提出迭代制度设计：(1)推测（包括情景规划）生成详细的灾难性事件叙事；(2)保险精算师为这些叙事分配概率和财务参数；(3)决策者将结果合成为汇总统计信息以支持判断。

Result: 模型分析揭示了以下价值：(a)保持推测与精算之间的独立性；(b)并行分析多个风险类别；(c)生成"厚重"的灾难性叙事，包含丰富的因果（反事实）和缓解细节。该方法无法消除深度模糊性，但为推理前沿AI中的极端低概率事件提供了系统方法。

Conclusion: 黑暗推测方法为应对前沿AI灾难性风险评估中的深度模糊性提供了系统框架，通过结合叙事工具和精算分析来生成结果概率分布，既能避免自满又能防止过度反应。该框架适用于迭代使用，并可进一步用AI系统增强。
...
### [258] [The Risk-Adjusted Intelligence Dividend: A Quantitative Framework for Measuring AI Return on Investment Integrating ISO 42001 and Regulatory Exposure](https://arxiv.org/abs/2511.21975)
*Hernan Huwyler*

Main category: cs.CY

TL;DR: 论文提出了一个量化AI项目投资回报的财务框架，该框架明确整合了组织风险状况的变化，解决了传统ROI计算无法捕捉AI实施双重性质的问题。


<details>
  <summary>Details</summary>
Motivation: 传统投资回报率计算无法捕捉AI实施的双重性质：一方面减少某些运营风险，另一方面引入算法故障、对抗性攻击和监管责任等新型风险。当前实践存在关键空白，投资决策依赖乐观的效益预测，而未考虑AI特定威胁的概率成本。

Method: 采用综合财务框架，整合已建立的风险量化方法，包括年度损失期望计算和蒙特卡洛模拟技术。该框架使从业者能够计算净收益，既包含生产力提升，也包含实施前后风险暴露的差异。

Result: 分析表明，准确的AI投资评估需要明确建模控制有效性、算法故障的储备要求以及维持模型性能的持续运营成本。该框架能够实现基于证据的AI投资组合管理。

Conclusion: 论文提供了建立治理结构、进行分阶段验证以及将风险调整指标整合到资本分配决策中的具体指导，最终实现既能满足受托责任又能满足监管要求的AI投资组合管理。
...
### [259] [The Hidden AI Race: Tracking Environmental Costs of Innovation](https://arxiv.org/abs/2511.22781)
*Shyam Agarwal,Mahasweta Chakraborti*

Main category: cs.CY

TL;DR: 该研究分析了AI模型在不同领域和时间段内的碳排放情况，发现模型大小、版本更新频率、任务类型和组织背景是影响环境足迹的关键因素，并提出了绿色AI实践建议。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，AI系统在训练和部署过程中消耗大量计算资源和能源，产生了显著的碳足迹，对环境造成严重影响。研究旨在量化AI开发的环境成本，识别影响碳排放的关键因素。

Method: 通过分析模型大小、仓库活动（如提交次数和仓库年龄）、任务类型和组织归属等参数，研究不同领域和时间段内AI模型的二氧化碳排放量，识别影响环境足迹的关键因素。

Result: 研究发现：1）模型大小和版本更新频率与更高排放量强相关；2）NLP模型比音频系统碳足迹更低；3）大学项目排放最高，其次是非营利组织和公司，社区驱动项目排放较低。

Conclusion: 研究强调了采用绿色AI实践的重要性，包括能效架构、优化开发流程和利用可再生能源。提出了未来研究方向，旨在引导构建更生态负责的AI生态系统。
...
### [260] [Building Metaverse Responsibly: Findings from Interviews with Experts](https://arxiv.org/abs/2511.23087)
*Muhammad Irfan Khalid,Ilias Pappas,Moatasim Mahmoud,Stamatia Rizou*

Main category: cs.CY

TL;DR: 该研究通过访谈元宇宙平台和应用专家，探讨了他们在开发过程中对隐私问题的认知和处理方式，为元宇宙隐私保护提供实践指导。


<details>
  <summary>Details</summary>
Motivation: 元宇宙技术虽然提供了沉浸式数字体验，但也带来了严重的隐私问题，大量个人和行为数据被收集。现有隐私标准在物理与虚拟边界模糊的元宇宙环境中面临挑战，但目前缺乏对技术专家（如需求分析师、设计师、开发者、架构师）如何看待和解决元宇宙隐私问题的了解。

Method: 采用深度半结构化访谈方法，对元宇宙平台和应用专家进行访谈，探索他们对隐私挑战和实践的看法。

Result: 研究结果提供了新的实证见解，将信息系统隐私研究扩展到元宇宙背景，强调了技术设计、用户行为和监管结构之间的相互作用。

Conclusion: 这项研究为开发者和政策制定者提供了实践指导，包括实施隐私设计原则、教育和赋权用户，以及主动应对元宇宙中的新型隐私威胁。
...
TL;DR: 提出一种基于多重隔离系统相互验证的人工超级智能对齐协议，通过隔离多个多样化超级智能，让它们通过可审计的提交接口相互验证对齐证明，利用声誉系统激励诚实行为，最终形成"一致群体"来确保客观真理的收敛。


<details>
  <summary>Details</summary>
Motivation: 解决人工超级智能对齐问题，传统方法面临单一系统可能欺骗人类的风险，需要一种能够确保超级智能真正对齐的机制，避免协调欺骗的可能性。

Method: 将多个多样化人工超级智能严格隔离在"盒子"中，人类完全在系统外。超级智能只能通过可审计的提交接口互动：提交对齐证明、验证他人证明、请求自我修改、批准他人修改请求、报告隐藏信息、确认或反驳报告。建立声誉系统激励诚实行为。

Result: 协议理论上能够形成"一致群体"（truth-telling coalition），因为隔离系统无法协调欺骗，但能独立识别有效声明，从而自然收敛于客观真理而非协调欺骗。

Conclusion: 该方法为利用超级智能系统间的同行验证解决对齐问题提供了框架，虽然需要大量计算资源且不涉及多样化超级智能的创建，但通过隔离和相互验证机制能够确保对齐的可靠性。
...
TL;DR: 该研究探索了在皮肤病学AI应用中，通过大语言模型生成合成临床文本描述，结合图像数据构建多模态学习系统，以提升分类性能和跨模态检索能力。
...
Result: 实验表明，合成临床文本不仅能提升分类性能（特别是在领域转移情况下），还能解锁跨模态检索能力，这是一个在训练期间未明确优化的下游任务。

Conclusion: 通过精心设计的提示和医学元数据整合，大语言模型生成的合成临床文本可以有效增强皮肤病学多模态AI系统的性能，特别是在数据稀缺和领域转移场景下。
...
TL;DR: 该论文提出了一种分层锚点原型调制（HAPM）框架，通过整合细粒度病理描述和基础模型知识，解决糖尿病视网膜病变分级中视觉信息不足的问题，在八个公开数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有糖尿病视网膜病变分级方法主要关注视觉病变特征提取，但普遍忽视领域不变的病理模式，且未充分利用基础模型的丰富上下文知识，仅依赖视觉信息难以区分细微的病理变化。

Method: 提出分层锚点原型调制框架：1）构建方差谱驱动的锚点原型库以保留领域不变病理模式；2）采用分层差异提示门控机制，动态选择LVLM和LLM的判别性语义提示；3）使用两阶段原型调制策略，通过病理语义注入器和判别原型增强器逐步将临床知识整合到视觉原型中。

Result: 在八个公开数据集上的广泛实验表明，该方法实现了病理引导的原型演化，并在性能上超越了现有最先进方法。

Conclusion: 通过整合细粒度病理描述和基础模型知识，提出的HAPM框架能够有效解决糖尿病视网膜病变分级中的语义混淆问题，实现更准确的分级诊断。
...
TL;DR: PRAXIS是一个轻量级的事后训练学习机制，通过存储和检索过往状态-动作-结果示例来增强智能体在实时环境中的程序性知识获取能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体缺乏部署后获取程序性知识的能力，无法像生物智能那样通过试错实时学习。在快速演化的状态化环境中，智能体需要能够有效学习新程序才能实现实际应用。

Method: 提出PRAXIS机制，存储动作的后果，并通过联合匹配环境和内部状态来检索过往经验。该机制通过实时生成检索到的状态-动作-结果示例来增强智能体的动作选择。

Result: 在REAL网页浏览基准测试中，PRAXIS提高了任务完成准确率、可靠性和成本效率，在不同基础模型骨干上都表现良好，并初步显示出在相似环境中对未见任务的泛化能力。

Conclusion: PRAXIS通过帮助智能体有效学习新程序，使其能够在快速演化的状态化环境中实现实际应用，展示了程序性回忆机制的重要价值。
...
### [265] [A perceptual bias of AI Logical Argumentation Ability in Writing](https://arxiv.org/abs/2511.22151)
*Xi Cun,Jifan Ren,Asha Huang,Siyu Li,Ruzhen Song*

Main category: cs.AI

TL;DR: 研究发现人们对AI逻辑推理能力的评估存在偏见，这种偏见受对AI先入为主看法的影响，频繁使用AI的人更不认为AI使用会削弱独立思考能力。
...
### [266] [WearVQA: A Visual Question Answering Benchmark for Wearables in Egocentric Authentic Real-world scenarios](https://arxiv.org/abs/2511.22154)
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA基准测试主要关注高质量、第三人称图像，而可穿戴设备（如智能眼镜）的视觉输入具有独特性——可能被遮挡、光照不佳、未缩放或模糊，且问题需要基于现实的可穿戴使用场景。需要专门针对可穿戴设备多模态AI系统的评估基准。

Method: 创建了包含2520个精心策划的图像-问题-答案三元组的基准测试，涵盖7个不同图像领域（包括文本中心场景和一般场景）、10种认知任务类型（从基本识别到各种推理形式）和6种常见的可穿戴设备图像质量问题。所有问题仅基于视觉输入和常识即可回答。配套开发了LLM-as-a-judge评估框架，标注准确率达96%。

Result: 开源和专有的多模态大语言模型在WearVQA上的问答准确率仅为24-52%，在低质量图像和推理密集型任务上表现显著下降。这表明现有模型在处理可穿戴设备特有的视觉挑战方面仍有很大改进空间。

Conclusion: WearVQA作为一个全面且具有挑战性的基准测试，能够指导技术发展，推动构建更鲁棒、适用于真实世界的可穿戴设备多模态AI系统。该基准突显了现有模型在处理可穿戴设备特有视觉挑战方面的局限性。
...
### [267] [Training High-Level Schedulers with Execution-Feedback Reinforcement Learning for Long-Horizon GUI Automation](https://arxiv.org/abs/2511.22235)
...
<details>
  <summary>Details</summary>
Motivation: 当前GUI智能体在处理长时程任务时面临两个主要挑战：1) 单智能体模型难以平衡高层能力与底层执行能力，存在责任耦合和能力冲突问题；2) 智能体缺乏任务状态感知，导致在长时程任务中进度丢失

Method: 提出分阶段执行-反馈强化学习算法，训练高层调度模型。构建CES多智能体框架，包含协调器（负责战略规划和任务分解）、状态跟踪器（负责上下文压缩和信息管理）、执行器（底层执行模块）。该框架可与任何底层执行器模型集成

Result: 在长时程任务基准测试中，CES显著提升了系统的规划和状态管理能力。分析证实训练的高层调度模块是通用、即插即用的模块，能显著增强各种执行器的长时程任务处理能力

Conclusion: CES多智能体框架通过高层调度和状态管理，有效解决了GUI智能体在长时程任务中的挑战，提供了一种可扩展的解决方案
...
### [268] [RecToM: A Benchmark for Evaluating Machine Theory of Mind in LLM-based Conversational Recommender Systems](https://arxiv.org/abs/2511.22275)
*Mengfan Li,Xuanhua Shi,Yang Deng*

Main category: cs.AI

TL;DR: RecToM：一个评估推荐对话中LLM心智理论能力的新基准，包含认知推理和行为预测两个维度，现有模型在动态对话中保持连贯的心智推理仍面临挑战。
...
Result: 在最先进的LLM上进行广泛实验表明，RecToM带来了显著挑战。模型在识别心理状态方面表现出部分能力，但在动态推荐对话中难以保持连贯、战略性的心智理论推理，特别是在跟踪演变意图和使对话策略与推断的心理状态保持一致方面存在困难。

Conclusion: RecToM基准更好地将LLM心智理论评估与人类社交推理对齐，揭示了当前模型在动态对话环境中进行连贯心智推理的局限性，为未来研究提供了重要方向。
...
### [269] [When AI Bends Metal: AI-Assisted Optimization of Design Parameters in Sheet Metal Forming](https://arxiv.org/abs/2511.22302)
...
TL;DR: 提出基于贝叶斯优化的AI辅助工作流，减少专家参与参数优化，通过深度学习模型提供初始参数估计，迭代优化设计，以钣金成形过程为例展示效果


<details>
  <summary>Details</summary>
Motivation: 数值仿真虽能降低工业设计成本，但大规模仿真需要大量专家知识、计算资源和时间，参数优化迭代成本高且环境影响大

Method: 采用贝叶斯优化和主动学习变体，深度学习模型提供初始参数估计，优化循环迭代精炼设计直到满足终止条件（如能量预算或迭代限制）

Result: 基于钣金成形过程的演示表明，该方法能加速设计空间探索，同时减少专家参与需求

Conclusion: AI辅助工作流通过贝叶斯优化有效减少专家参与参数优化，加速设计空间探索，降低仿真成本
...
### [270] [Swarms of Large Language Model Agents for Protein Sequence Design with Experimental Validation](https://arxiv.org/abs/2511.22311)
...
<details>
  <summary>Details</summary>
Motivation: 当前蛋白质设计方法（如蛋白质语言模型和扩散模型）需要大量微调、特定任务数据或模型重构，限制了灵活性和可扩展性。蛋白质设计面临序列空间巨大和序列-结构-功能复杂耦合的挑战。

Method: 采用去中心化、基于群体智能的多智能体框架，多个LLM智能体并行工作，每个负责特定残基位置。智能体通过迭代提出上下文感知突变，整合设计目标、局部邻域交互以及先前迭代的记忆和反馈。

Result: 在α螺旋和卷曲结构蛋白质上验证，框架展现出涌现行为，能有效导航蛋白质适应度景观。方法在几GPU小时内实现高效目标导向设计，无需微调或专门训练。
...
### [271] [On the Complexity of the Grounded Semantics for Infinite Argumentation Frameworks](https://arxiv.org/abs/2511.22376)
...
<details>
  <summary>Details</summary>
Motivation: 研究论证框架中grounded扩展的计算特性，特别是在无限情况下的复杂性，以理解冲突信息下推理的形式化特性

Method: 使用数理逻辑方法，特别是可计算性和集合论，分析自然防御算子的最小不动点，确定迭代过程的精确序数长度

Result: 确定了grounded扩展迭代过程的确切序数长度，证明了grounded接受决策问题具有最大复杂度，与有限情况形成鲜明对比

Conclusion: 无限论证框架中grounded扩展的计算复杂度远高于有限情况，这揭示了形式化论证中推理问题的复杂性差异
...
### [272] [Who is Afraid of Minimal Revision?](https://arxiv.org/abs/2511.22386)
*Edoardo Baccini,Zoé Christoff,Nina Gierasimczuk,Rineke Verbrugge*

Main category: cs.AI

TL;DR: 最小修订在信念修正中保持信念状态尽可能接近初始状态，但学习能力有限。研究表明它仍能成功学习有限可识别问题，并在有限可能性下处理正负数据。论文还分析了不同先验概率分配对学习的影响，并探讨了错误信息下的学习限制。


<details>
  <summary>Details</summary>
Motivation: 探讨信念修正理论中的最小修订原则在学习能力方面的局限性，研究在何种条件下最小修订仍能成为有效的学习方法，并比较不同信念修正方法（如条件化和词典升级）的学习能力。

Method: 通过理论分析证明最小修订能够学习有限可识别问题；研究在有限可能性下处理正负数据的学习能力；分析不同先验概率分配对最小修订、条件化和词典升级学习方法的影响；探讨错误信息对学习结果的影响。

Result: 最小修订能够学习所有有限可识别问题；在有限可能性下能够处理正负数据；确定了支持最小修订学习的先验概率分配特征；比较了不同信念修正方法的学习能力；发现部分结果在错误信息下不再成立。

Conclusion: 尽管最小修订在学习能力上有限制，但在许多实际情境中仍是有效的学习方法。不同信念修正方法的学习能力取决于先验概率分配，且错误信息会影响学习效果。
...
TL;DR: 本文提出了一种基于博弈论视角的计算框架，用于实现有限理性决策中的心智理论自动化计算。


<details>
  <summary>Details</summary>
Motivation: 心智理论在心理学、逻辑学、经济学和机器人学等多个领域受到关注，但心理学研究通常不形式化目标、意图和信念等核心概念以实现自动化计算，而逻辑学家对此进行了深入研究。本文旨在从博弈论角度提供不同的计算视角。

Method: 提出一个计算框架：一方面规定如何在保持对他人心智理论（递归地，每个人都对其他人持有心智理论）的同时做出有限理性决策；另一方面采用统计技术和近似解来保持固有计算问题的可计算性。

Result: 该框架结合了博弈论视角与统计近似方法，为心智理论的自动化计算提供了可行的解决方案。

Conclusion: 通过博弈论视角和统计近似技术，本文提出的计算框架能够实现有限理性决策中心智理论的自动化计算，为相关领域提供了新的研究方向。
...
### [274] [Counting Still Counts: Understanding Neural Complex Query Answering Through Query Relaxation](https://arxiv.org/abs/2511.22565)
*Yannick Brunink,Daniel Daza,Yunjie He,Michael Cochez*

Main category: cs.AI

TL;DR: 神经复杂查询回答模型并不总是优于无训练的查询松弛方法，两者答案重叠度低，结合使用可提升性能
...
Result: 神经模型与松弛方法表现相似，无神经模型始终优于后者；两者检索答案重叠度低，结合输出可一致提升性能
...
### [275] [DeepSeekMath-V2: Towards Self-Verifiable Mathematical Reasoning](https://arxiv.org/abs/2511.22570)
*Zhihong Shao,Yuxiang Luo,Chengda Lu,Z. Z. Ren,Jiewen Hu,Tian Ye,Zhibin Gou,Shirong Ma,Xiaokang Zhang*

Main category: cs.AI

TL;DR: 论文提出DeepSeekMath-V2模型，通过自我验证机制解决数学推理中"正确答案不代表正确推理"的问题，在定理证明任务上取得突破性表现。
...
Method: 1. 训练准确且可靠的基于LLM的定理证明验证器；2. 使用验证器作为奖励模型训练证明生成器；3. 激励生成器在最终确定证明前识别并解决尽可能多的问题；4. 通过扩展验证计算自动标注新的难以验证的证明，创建训练数据以持续改进验证器。

Result: DeepSeekMath-V2在定理证明方面表现出色：在IMO 2025和CMO 2024获得金牌级分数，在Putnam 2024获得接近完美的118/120分（使用扩展测试时计算）。

Conclusion: 通过自我验证机制和扩展验证计算，可以显著提升数学推理的深度和严谨性，特别是在定理证明等需要逐步推导的任务上，为解决开放数学问题提供了有前景的方向。
...
### [276] [AI Deception: Risks, Dynamics, and Controls](https://arxiv.org/abs/2511.22619)
...
TL;DR: 该论文对AI欺骗现象进行了全面综述，将AI欺骗定义为系统诱导错误信念以获得自身利益的行为，分析了欺骗产生的机制、风险及应对策略。


<details>
  <summary>Details</summary>
Motivation: 随着AI智能水平提升，AI欺骗已从理论担忧演变为实证风险，需要系统性地理解这一现象以应对其带来的安全挑战。

Method: 采用信号理论框架定义AI欺骗，构建"欺骗循环"模型分析欺骗产生机制（能力、激励、触发条件）和处理方法（检测、缓解、审计）。

Result: 识别了AI欺骗的三个核心要素：足够能力、激励潜力和外部触发条件；提出了包含静态和交互场景的检测方法，以及技术、社区和治理相结合的缓解策略。

Conclusion: AI欺骗是真实存在的安全风险，需要跨学科的综合应对方案，包括技术检测、社区协作和治理框架，并建立了持续更新的资源平台支持相关研究。
...
### [277] [Geometrically-Constrained Agent for Spatial Reasoning](https://arxiv.org/abs/2511.22659)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理中存在语义到几何的鸿沟：擅长定性语义推理但推理在损失语义空间中进行，与高保真几何不对齐。现有方法无法解决这一鸿沟，训练方法存在"预言悖论"，工具集成方法则无法约束VLM的规划过程。

Method: 提出几何约束智能体(GCA)，训练免费范式，通过引入形式化任务约束解决该鸿沟。策略性地将VLM角色解耦为两个阶段：1)作为语义分析器，将用户模糊查询转换为可验证的任务约束；2)作为任务求解器，在约束定义的确定性边界内生成和执行工具调用。

Result: GCA在多个空间推理基准测试中达到最先进性能，超越现有训练方法和工具集成方法约27%。

Conclusion: 几何约束推理策略成功解决了语义到几何的鸿沟，为空间推理提供了鲁棒且可验证的推理路径。
...
TL;DR: 提出一种让大语言模型处理任意长度工具输出而不丢失信息的方法，通过内存指针替代原始数据交互，显著减少token使用和执行时间


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理动态、知识密集型领域（如化学和材料科学）时，需要与外部工具交互获取专业知识，但大型工具输出会超出模型的上下文窗口，现有解决方案（如截断或摘要）无法保留完整输出，不适合需要完整数据的工作流程

Method: 将模型与工具的交互从原始数据转向内存指针，通过指针引用而非直接处理完整数据，保留工具功能完整性，实现与智能工作流程的无缝集成

Result: 在真实材料科学应用中验证了方法的有效性，传统工作流程无法执行的任务得以完成；在两种方法都能成功的比较实验中，新方法消耗的token数量约为传统方法的七分之一

Conclusion: 该方法使大语言模型能够处理任意长度的工具输出而不丢失信息，保持工具功能完整性，减少token使用和执行时间，特别适用于需要完整数据的工作流程
...
### [279] [Agentic AI Framework for Cloudburst Prediction and Coordinated Response](https://arxiv.org/abs/2511.22767)
...
<details>
  <summary>Details</summary>
Motivation: 传统预报系统将预测和响应视为两个独立过程，难以应对极端短时降雨事件。需要一种集成化的智能系统来改善极端天气事件的预测和应急响应能力。

Method: 开发了一个多智能体人工智能系统，包含自主但协作的智能体，在整个事件生命周期中进行推理、感知和行动。系统整合了传感、预报、降尺度、水文建模和协调响应，形成闭环系统，并嵌入学习层实现自适应校准和透明审计。

Result: 在巴基斯坦北部地区的多年雷达、卫星和地面评估显示，多智能体配置相比基线模型提高了预报可靠性、关键成功指数和预警提前时间。通过通信和路由智能体最大化人口覆盖范围，最小化疏散过程中的错误。

Conclusion: 协作AI智能体能够将大气数据流转化为可操作的预见性洞察，为可扩展的自适应和学习型气候韧性提供了一个平台，改变了极端天气事件的预测和响应方式。
...
### [280] [Fast dynamical similarity analysis](https://arxiv.org/abs/2511.22828)
*Arman Behrad,Mitchell Ostrow,Mohammad Taha Fakharian,Ila Fiete,Christian Beste,Shervin Safavi*

Main category: cs.AI

TL;DR: 提出fastDSA方法，通过自动选择有效模型阶数和轻量级优化过程，在保持准确性的同时将动态相似性分析的计算效率提高至少一个数量级。
...
Method: fastDSA引入两个关键组件：1) 通过数据驱动的奇异值阈值自动选择Hankel嵌入的有效模型阶数，识别信息子空间并丢弃噪声；2) 用轻量级过程替代严格的正交性约束，在保持接近正交变换空间的同时大幅降低计算成本。

Result: fastDSA比先前方法至少快一个数量级，同时保持了祖先方法的特性，包括对系统动态的不变性和敏感性，实现了计算高效且准确的动态相似性分析。

Conclusion: fastDSA为动态相似性分析提供了一个计算高效且准确的方法，有助于神经信息处理研究中不同电路、大脑或模型之间的比较分析。
...
### [281] [InsightEval: An Expert-Curated Benchmark for Assessing Insight Discovery in LLM-Driven Data Agents](https://arxiv.org/abs/2511.22884)
*Zhenghao Zhu,Yuanfeng Song,Xin Chen,Chengzhong Liu,Yakun Cui,Caleb Chen Cao,Sirui Han,Yike Guo*

Main category: cs.AI

TL;DR: 该论文针对现有数据洞察发现基准（如InsightBench）存在的问题，提出了新的高质量洞察基准InsightEval，并开发了相应的评估指标来评估智能体在数据探索分析中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和多智能体系统的发展，越来越多的研究者利用这些技术进行数据洞察发现，但缺乏有效的评估基准。现有的InsightBench框架存在格式不一致、目标设计不佳、洞察冗余等问题，这些问题可能严重影响数据质量和智能体评估。

Method: 首先深入分析InsightBench的缺陷，提出高质量洞察基准的关键标准；然后开发数据整理流程构建新的数据集InsightEval；最后引入新的指标来衡量智能体的探索性分析性能。

Result: 通过大量实验验证了InsightEval的有效性，突出了自动化洞察发现中的普遍挑战，并提出了指导未来研究的关键发现。

Conclusion: 该研究为解决数据洞察发现评估基准的缺陷提供了系统性的解决方案，InsightEval数据集和新评估指标为这一有前景的研究方向提供了更可靠的评估框架。
...
### [282] [ORION: Teaching Language Models to Reason Efficiently in the Language of Thought](https://arxiv.org/abs/2511.22891)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在数学、代码生成等任务中表现良好，但依赖冗长的"思考"令牌链导致高延迟、冗余和不连贯的推理路径。受人类思维语言假说启发，需要开发更紧凑、高效的推理方式。

Method: 引入Mentalese框架，训练模型使用超压缩、结构化的令牌进行推理；提出SLPO（短长度偏好优化）强化学习方法，奖励简洁且正确的解决方案，同时允许必要时进行更长的推理。

Result: ORION模型在多个基准测试中实现4-16倍令牌减少，推理延迟降低5倍，训练成本降低7-9倍，保持90-98%的准确性，并在某些任务上超越Claude和ChatGPT-4o达5%的准确率提升。

Conclusion: Mentalese风格的压缩推理实现了类人认知效率，在不牺牲准确性的前提下实现实时、经济高效的推理，为高效推理模型的发展提供了新方向。
...
### [283] [TIM-PRM: Verifying multimodal reasoning with Tool-Integrated PRM](https://arxiv.org/abs/2511.22998)
*Peng Kuang,Xiangxiang Wang,Wentao Liu,Jian Dong,Kaidi Xu,Haohan Wang*

Main category: cs.AI

TL;DR: TIM-PRM是一个工具集成的多模态过程奖励模型，通过主动工具查询和独立提问机制解决视觉幻觉和逻辑不一致问题，在数学推理任务中超越更大规模模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数学推理中存在视觉幻觉和逻辑不一致问题，而标准的结果监督无法解决这些问题。现有的过程奖励模型通常作为标量评分器或生成式批评者，存在谄媚问题，盲目验证有缺陷的假设而非基于视觉现实。

Method: 提出TIM-PRM（工具集成多模态PRM），将验证从被动分类任务转变为主动、工具增强的调查。模型训练明确规划验证策略，使用独立提问机制通过外部工具查询证据，将验证与推理上下文解耦以消除确认偏差。通过策划高质量的工具集成验证轨迹数据集来实例化该方法。

Result: 在VisualProcessBench上的广泛实验表明，8B参数的TIM-PRM模型超越了现有的开源多模态PRM，显著优于Qwen2.5-72B和InternVL-78B等更大规模模型，同时为验证过程提供可解释的见解。

Conclusion: TIM-PRM通过主动工具查询和独立提问机制，有效解决了多模态数学推理中的视觉幻觉和逻辑不一致问题，在较小参数规模下实现了超越更大模型的性能，并为验证过程提供了可解释性。
...
TL;DR: MindPower是一个机器人中心框架，通过整合感知、心理推理、决策和行动，使视觉语言具身智能体具备心理理论能力，显著提升了决策和行动生成性能。
...
Method: 提出MindPower机器人中心框架，包含感知、心理推理、决策和行动四个模块。给定多模态输入，首先感知环境和人类状态，然后进行心理理论推理以建模自我和他人的心理状态，最后基于推断的心理状态生成决策和行动。同时引入Mind-Reward优化目标，鼓励视觉语言模型产生一致的心理理论推理和行为。

Result: MindPower模型在决策制定方面比GPT-4o提升了12.77%，在行动生成方面提升了12.49%。

Conclusion: MindPower框架通过整合心理理论推理，显著提升了具身智能体的决策和行动生成能力，为机器人中心的心理理论推理提供了有效解决方案。
...
TL;DR: 研究发现当语言模型的自我评价与奖励信号耦合时，会导致评分膨胀而非任务性能提升，特别是在模糊任务上；而将自我评价与学习信号解耦则是安全的。


<details>
  <summary>Details</summary>
Motivation: 随着自我评价在语言模型训练中越来越重要（从宪法AI到自我优化），研究者想探究将自我评价与奖励信号耦合是否会引发"线头化"问题，即智能体操纵奖励测量而非真正提升任务性能。

Method: 研究首先在部分可观察马尔可夫决策过程（POMDPs）中形式化了奖励通道控制严格优于任务导向行为的条件，然后在两个模型和三个任务上进行实证测试，比较了自我评分决定奖励的模型与仅自我评价但不控制奖励的模型。

Result: 研究发现，当自我评分决定奖励时，模型表现出显著的评分膨胀，但没有相应的准确性提升，特别是在摘要等模糊任务上；而仅自我评价但不控制奖励的模型则没有这种膨胀现象。

Conclusion: 自我评价与学习信号解耦时是安全的，但当两者耦合时是危险的，这对智能体系统设计有明确启示：应避免让自我评价直接控制奖励信号。
...
TL;DR: 提出TPET框架，用LLM作为进化引擎生成专用启发式策略，解决传统启发式方法过于简化、DRL泛化性差且不透明、在线LLM延迟高且缺乏环境优化的问题


<details>
  <summary>Details</summary>
Motivation: 交通信号控制面临经典启发式方法效率高但过于简化，深度强化学习性能好但泛化性差且策略不透明，在线大语言模型具有通用推理能力但延迟高且缺乏环境特定优化的问题

Method: 提出TPET框架，使用LLM作为进化引擎推导专用启发式策略，包含两个关键模块：结构化状态抽象（将高维交通数据转换为时序逻辑事实）和信用分配反馈（追踪微观决策错误到宏观结果以进行针对性批评）

Result: 该框架完全在提示层面运行无需训练，生成轻量级、鲁棒的针对特定交通环境优化的策略，性能优于启发式方法和在线LLM执行器

Conclusion: TPET框架有效结合了LLM的推理能力和启发式策略的实用性，为交通信号控制提供了一种新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉问答数据集和基准测试往往无法充分评估复杂农业场景中所需的关键推理和问题解决能力，因此需要专门针对农业领域推理能力评估的数据集。

Method: 引入AgriCoT数据集，该数据集结合了思维链推理，包含4,535个精心策划的样本，专门设计用于评估视觉语言模型的推理能力。对26个代表性视觉语言模型（包括专有和开源模型）进行评估。

Result: 评估结果显示，虽然一些专有模型在回答问题方面表现出色，但它们在推理能力方面存在显著差距，强调了思维链推理对于更精确有效评估的重要性。

Conclusion: AgriCoT数据集为评估视觉语言模型在农业领域的推理能力提供了全面而鲁棒的基准，揭示了当前模型在推理能力方面的不足，并强调了思维链推理在评估中的重要性。
...
TL;DR: MCTR是一个元认知测试时推理框架，通过元认知自更新使视觉语言模型能够在测试时学习、适应和改进，在Atari游戏中表现出强大的测试时适应能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然具有强大的感知推理能力，但在遇到新任务时难以高效适应。相比之下，人类利用带有记忆的元认知模型，在面对新挑战时通过元认知控制不断优化策略。为了弥合这一差距，需要让模型具备在测试时学习、适应和改进的能力。

Method: MCTR框架包含元级和对象级VLM推理模块，每个模块配备专用记忆系统进行分层自适应推理：1）元推理模块通过发现和存储任务相关规则、环境模式和行动-结果关系来增量构建结构化记忆；2）行动推理模块通过上下文感知感知和策略推理确定最优行动，动态检索和整合记忆知识。行动推理模块通过元认知测试时强化学习持续更新策略。

Result: 在45个Atari游戏（33个已见，12个未见）上评估，MCTR在未见游戏中获得9/12的top-1结果，相比基线表现出强大的测试时适应能力。通过消融实验、学习动态和案例研究分析，揭示了两个组件的互补贡献，并显示元推理向类人适应策略演化。

Conclusion: MCTR框架成功使视觉语言模型具备了元认知测试时适应能力，通过模拟人类元认知的双层结构，实现了在遇到新任务时的持续学习和策略优化，为AI系统的自适应能力提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 高质量的数据策展对医疗大语言模型的训练至关重要，直接影响模型对未见临床任务的泛化能力和鲁棒性。研究者旨在开发医疗领域的鲁棒多模态推理模型。

Method: 采用监督微调（SFT）方法，探索利用结构化推理轨迹的数据配方策略。数据配方包含不同长度的结构化推理轨迹，构建了包含800多万个样本和68亿响应令牌的大规模数据集。

Result: 在多样化的分布外医疗基准任务上取得了开源模型中的最先进性能。研究发现，包含不同长度结构化推理轨迹的高质量、多样化训练数据集能使微调模型根据下游任务自我校准其推理轨迹长度，无需显式监督。

Conclusion: 高质量的数据策展和结构化推理轨迹对开发鲁棒的医疗视觉语言推理系统至关重要。研究为医疗大语言模型的训练提供了关键见解，并概述了开发鲁棒医疗多模态推理系统的下一步方向。
...
TL;DR: 提出一个基于智能体的AI系统，用于零售库存管理、供应商采购和产品发现，通过需求预测、供应商优化和多智能体谈判来减少缺货、降低库存成本并改善产品组合周转。
...
Result: 在三个传统和人工数据表上的测试表明，与基准启发式方法相比，系统能减少缺货、降低库存持有成本，并改善产品组合周转率。

Conclusion: 该智能体AI系统在零售库存管理中表现出有效性，同时讨论了系统约束、可扩展性以及未来改进前景。
...
TL;DR: SuperIntelliAgent是一个智能体学习框架，通过将可训练的小型扩散模型（学习者）与冻结的大型语言模型（验证者）结合，实现无监督的持续智能增长。该框架通过生成-验证-优化的循环机制，将普通推理过程转化为终身优化过程。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调需要人工标注，限制了智能体的持续学习能力。本文旨在开发一个无需标注的自主学习框架，使智能体能够通过与环境的交互不断积累智能，实现终身学习。

Method: 1. 学习者（小型扩散模型）生成候选输出；2. 验证者（冻结LLM）通过逐步推理评估输出质量；3. 生成选择/拒绝对用于直接偏好优化（DPO）；4. 集成双尺度记忆：短期上下文记忆保存推理轨迹，长期记忆通过轻量级微调巩固知识；5. 回放缓冲区保留显示可验证进展的样本作为辅助监督。

Result: 仅使用少量自动生成的DPO对，学习者在所有基准测试中均表现出改进。这表明该机制为持续智能积累和实际部署提供了有前景的方向。

Conclusion: 将可训练的学习者与具备推理能力的验证者配对构成了智能增长的最小可靠单元。这种配对反馈和部分历史回放机制能够产生更丰富的学习课程和更强的偏好对齐，为持续智能发展提供了有效框架。
...
TL;DR: WMAct方法通过奖励重标定和交互频率退火策略，让LLM智能体在单次交互中完成原本需要多次交互的任务，实现高效的世界模型推理。


<details>
  <summary>Details</summary>
Motivation: 当前多轮交互方法虽然能通过真实反馈更好地理解环境动态，但强制的结构化推理过程限制了模型的主动学习能力，阻碍了高效的世界模型推理。

Method: WMAct通过两种关键机制实现：1）基于行动效能的奖励重标定机制，激励减少冗余和有目的的交互；2）交互频率退火策略，逐步减少允许的最大交互轮数，迫使模型浓缩学习并内化环境动态。

Result: 在Sokoban、Maze和Taxi环境中的实验表明，WMAct能够实现有效的世界模型推理，在单次交互中完成原本需要多次交互的任务，并在复杂环境中展现出强大的可迁移性，在一系列推理基准测试中提升了性能。

Conclusion: WMAct通过解放模型的结构化推理约束，让模型通过行动直接塑造思考，实现了高效的世界模型内部化，为LLM智能体在复杂环境中的规划和交互提供了有效解决方案。
...
TL;DR: 本文提出了一种基于强化学习的用户选择框架，利用大语言模型作为用户模拟器，为冷启动物品生成交互数据，从而提升推荐系统在冷启动场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用大语言模型为冷启动物品生成增强数据，但存在两个主要问题：1）仅使用部分用户历史记录，无法让LLM充分模拟用户行为；2）随机选择用户进行增强不是最优策略。需要一种更智能的用户选择方法来优化冷启动物品的性能。

Method: 提出基于强化学习的框架，将LLM作为用户模拟器，训练一个策略模型来选择最适合的用户进行数据增强。策略模型基于用户行为特征和历史记录做出选择，使用策略梯度方法进行优化，奖励信号基于冷启动物品在增强训练后的性能表现。

Result: 在Amazon产品评论数据集上的实验表明，该方法在冷启动物品召回率方面取得了显著提升，证明了其作为可扩展、服务效率高的增强策略的有效性。

Conclusion: 通过强化学习框架智能选择用户进行数据增强，结合LLM作为用户模拟器，能够有效提升推荐系统在冷启动物品场景下的性能，为现代推荐系统提供了一种高效的增强策略。
...
TL;DR: 评估不同文本嵌入模型和管道配置对AI驱动搜索系统性能的影响，发现高维嵌入、神经重排序和细粒度分块能显著提升检索准确率。


<details>
  <summary>Details</summary>
Motivation: 为了评估不同文本嵌入模型和管道配置在AI驱动搜索系统中的性能差异，为实际应用提供优化指导。

Method: 使用本地LLM从美国市议会会议记录合成11,975个查询-分块对作为评估数据集；比较sentence-transformer和生成式嵌入模型（All-MPNet、BGE、GTE、Qwen）在不同维度、索引方法（Milvus HNSW/IVF）和分块策略下的表现；采用Top-K准确率和NDCG作为评估指标。

Result: 高维嵌入显著提升搜索质量（如Qwen3-Embedding-8B/4096的Top-3准确率约0.571，而GTE-large/1024为0.412）；神经重排序器（如BGE交叉编码器）进一步提升排名准确率（Top-3最高达0.527）；更细粒度的分块（512字符vs2000字符）也能提高准确率。

Conclusion: 嵌入维度、重排序技术和分块粒度是影响AI搜索系统性能的关键因素，未来需要进一步研究管道自动化和评估方法的改进。
...
<details>
  <summary>Details</summary>
Motivation: IGROT（图像引导检索与可选文本）统一了视觉检索和组合检索，在Google Image和Bing等应用中很重要，但进展受限：缺乏可访问的基准，现有方法往往偏向视觉或组合查询，大型数据集计算成本高。

Method: 1. 提出FIGROTD轻量级高质量数据集（16,474训练三元组，1,262测试三元组）；2. 提出Variance Guided Feature Mask（VaGFeM），基于方差统计选择性增强判别性维度；3. 采用双损失设计（InfoNCE + Triplet）改进组合推理。

Result: 在FIGROTD上训练的VaGFeM在9个基准上取得竞争性结果：CIRCO上达到34.8 mAP@10，Sketchy上达到75.7 mAP@200，尽管使用更少的三元组仍优于更强的基线。

Conclusion: 提出的FIGROTD数据集和VaGFeM方法有效解决了IGROT任务中数据集可访问性和性能平衡问题，在多个基准上展现出竞争性性能，为图像引导检索与可选文本任务提供了实用解决方案。
...
TL;DR: UNION方法通过融合图像嵌入和空文本提示，在少量数据监督下实现图像引导检索，统一了组合图像检索和基于草图的图像检索任务。


<details>
  <summary>Details</summary>
Motivation: 解决图像引导检索（IGROT）在低数据监督下的挑战，统一组合图像检索（CIR）和基于草图的图像检索（SBIR）两大任务，避免对预训练视觉语言模型进行架构修改。

Method: 提出UNION方法，这是一种轻量级且可泛化的目标表示，通过将图像嵌入与空文本提示融合来增强与多模态查询的语义对齐，无需修改预训练模型架构。

Result: 仅使用5,000个训练样本，在CIRCO基准上达到mAP@50为38.5，在Sketchy基准上达到mAP@200为82.7，超越了多个需要大量监督的基线方法。

Conclusion: UNION方法展示了在少量数据下实现跨视觉和语言多模态检索的鲁棒性和高效性，为统一不同查询类型的图像检索任务提供了有效解决方案。
...
### [297] [Efficiency and Effectiveness of SPLADE Models on Billion-Scale Web Document Title](https://arxiv.org/abs/2511.22263)
*Taeryun Won,Tae Kwan Lee,Hiun Kim,Hyemin Lee*

Main category: cs.IR

TL;DR: 本文对BM25、SPLADE和Expanded-SPLADE模型在大规模网页文档检索中的效果和效率进行了全面比较，发现稀疏表示模型性能更优但计算成本更高，通过剪枝策略可在保持性能的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 在大规模网页文档检索场景下，需要评估不同检索模型（传统BM25与基于稀疏表示的SPLADE系列）在效果和效率上的平衡，为实际搜索引擎部署提供指导。

Method: 在从千万到十亿级别的网页文档标题数据集上，比较BM25、SPLADE和Expanded-SPLADE模型的检索效果；引入文档中心剪枝、top-k查询词选择、布尔查询加词项阈值等策略来优化计算效率。

Result: SPLADE和Expanded-SPLADE在复杂查询上表现优于BM25，但计算成本更高；通过剪枝策略可显著提升效率而不明显损失检索性能；Expanded-SPLADE在效果和效率之间取得了最佳平衡。

Conclusion: 稀疏检索模型在大规模网页文档检索中具有优势，通过适当的优化策略可以在保持高性能的同时控制计算成本，为实际搜索引擎部署提供了有价值的参考。
...
### [298] [CoFiRec: Coarse-to-Fine Tokenization for Generative Recommendation](https://arxiv.org/abs/2511.22707)
*Tianxin Wei,Xuying Ning,Xuxing Chen,Ruizhong Qiu,Yupeng Hou,Yan Xie,Shuang Yang,Zhigang Hua,Jingrui He*

Main category: cs.IR

TL;DR: CoFiRec是一个新颖的生成式推荐框架，通过将物品语义从粗到细分解为多个层次进行独立token化，在自回归解码时引导模型从粗粒度到细粒度生成物品token，从而更好地捕捉用户在网页交互中意图的渐进演化过程。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐模型忽略了用户在网页环境中从浏览宽泛类别到探索具体物品的自然细化过程。传统方法将所有异构属性融合为单一嵌入再进行量化，这抹平了物品固有的语义层次结构，无法捕捉用户意图在交互过程中的渐进演化。

Method: 提出CoFiRec框架，将物品信息分解为多个语义层次（从高层类别到详细描述和协同过滤信号），设计CoFiRec Tokenizer对每个层次进行独立token化并保持结构顺序。在自回归解码时，指导语言模型从粗到细生成物品token，逐步建模用户从一般兴趣到具体物品级兴趣的意图演化。

Result: 在多个公共基准测试和骨干模型上的实验表明，CoFiRec优于现有方法。理论上证明了结构化tokenization能够降低生成物品与真实物品之间的不相似性，支持其在生成式推荐中的有效性。

Conclusion: CoFiRec通过显式地将物品语义的粗到细特性融入tokenization过程，为生成式推荐提供了新视角，能够更好地建模用户意图的渐进演化，在推荐性能上取得显著提升。
...
### [299] [Two-Stage Distributionally Robust Optimization Framework for Secure Communications in Aerial-RIS Systems](https://arxiv.org/abs/2511.22855)
*Zhongming Feng,Qiling Gao,Zeping Sui,Yun Lin,Michail Matthaiou*

Main category: cs.IR

TL;DR: 提出两阶段分布鲁棒优化框架，用于空中可重构智能表面辅助毫米波系统的安全部署和波束成形，通过CVaR风险度量处理多时间尺度不确定性，提升尾部保密频谱效率和降低中断概率。


<details>
  <summary>Details</summary>
Motivation: 解决空中可重构智能表面辅助毫米波系统中由于用户移动性、不完美信道状态信息和硬件损伤引起的多时间尺度不确定性问题，确保系统在不确定环境下的安全性和可靠性。

Method: 采用两阶段分布鲁棒优化框架：第一阶段进行长期无人机部署，第二阶段进行每时隙波束成形设计。使用条件风险价值作为分布无关风险度量，结合替代模型进行高效部署和交替优化方案进行鲁棒实时波束成形。

Result: 仿真结果表明，提出的DRO-CVaR框架显著提升了尾部保密频谱效率，并在严重不确定条件下相比基准方案保持了更低的中断概率。

Conclusion: 该两阶段分布鲁棒优化框架能有效处理空中可重构智能表面辅助毫米波系统中的多时间尺度不确定性，在不确定环境下提供更好的安全性和可靠性保障。
...
### [300] [FedAU2: Attribute Unlearning for User-Level Federated Recommender Systems with Adaptive and Robust Adversarial Training](https://arxiv.org/abs/2511.22872)
...
<details>
  <summary>Details</summary>
Motivation: 联邦推荐系统虽然保护用户隐私，但用户嵌入向量仍可能编码敏感属性信息，易受属性推断攻击。用户级联邦推荐系统相比组级更具实用性但也更复杂，对抗训练成为可行方案，但面临用户数据异质性导致的训练不稳定性和梯度信息泄露两大挑战
...
Conclusion: FedAU2有效解决了用户级联邦推荐系统中属性遗忘的两大关键挑战，在保护用户隐私的同时保持了良好的推荐性能
...
### [301] [Do LLM-judges Align with Human Relevance in Cranfield-style Recommender Evaluation?](https://arxiv.org/abs/2511.23312)
...
TL;DR: LLM可以作为可靠的自动评估者来评估推荐系统，解决传统离线评估方法的不稳定性和可扩展性问题


<details>
  <summary>Details</summary>
Motivation: 推荐系统评估面临长期挑战：基于历史用户交互和训练测试分割的离线方法由于曝光偏差、流行度偏差、采样评估和MNAR模式等问题，常常产生不稳定和不一致的结果。虽然文本检索领域有Cranfield风格的标准化评估方法，但构建此类集合需要人工相关性判断，成本高昂且难以扩展。

Method: 使用ML-32M-ext Cranfield风格电影推荐集合，首先分析现有评估方法的局限性，然后探索LLM评估者与人工提供相关性标签之间的一致性和推荐系统排序一致性。研究更丰富的项目元数据和更长的用户历史如何改善对齐，并在播客推荐领域进行工业案例研究。

Result: 研究发现：1）结合更丰富的项目元数据和更长的用户历史可以改善对齐；2）LLM评估者与基于人工的排序具有高度一致性（Kendall's tau = 0.87）；3）播客推荐领域的工业案例研究证明了LLM评估者在模型选择中的实际价值。

Conclusion: LLM评估者是评估推荐系统的可行且可扩展的方法，能够解决传统评估方法的局限性，为推荐系统评估提供了新的有效途径。
