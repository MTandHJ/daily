...
TL;DR: 该论文开发了一个分析框架来研究AI系统的递归自我改进，提出了可测试的条件来判定能力是否会无限增长，并提供了可实施的安全控制措施。
...
Result: 推导出将可观测序列映射为失控与非奇异行为的决策规则，提供了基于功率限制、吞吐量节流和评估门等可直接实施的安全控制。

Conclusion: 该框架用可测试的条件和可部署的控制措施取代了对AI奇点的猜测，为认证或排除AI奇点提供了理论基础。
...
TL;DR: 提出了一种可扩展且高效的数据归属方法，通过将基于反学习的慢速归属方法蒸馏到特征嵌入空间，实现高效检索高影响力训练图像，比现有方法快2500-400000倍。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型数据归属方法需要大量计算资源，使得在实际应用中不切实际。

Method: 将基于反学习的慢速归属方法蒸馏到特征嵌入空间，结合高效索引和搜索方法，无需运行昂贵的归属算法即可找到高影响力图像。

Result: 在MSCOCO训练的中等规模模型和LAION训练的大规模Stable Diffusion模型上均取得更好或竞争性性能，几秒内完成归属，比现有方法快2500-400000倍。
...
Method: 通过Delphi流程达成关于通用工作流程描述符的共识，开发ColoWorkflow工具，并在多中心视频数据集上由独立评分者应用该工具评估适用性和评分者间可靠性。

Result: 达成了10个程序无关阶段和34个程序特定步骤的共识，工具在54个手术视频中表现出广泛适用性，评分者间可靠性中等（阶段K=0.71，步骤K=0.66）。

Conclusion: ColoWorkflow是首个基于共识的验证VBA工具，为微创结直肠手术建立了可重复的工作流程分析框架，支持机构间基准测试和AI驱动的工作流程识别发展。
...
TL;DR: 本文提出了一种基于LRP的CNN剪枝方法，能够在少量数据情况下实现更高的剪枝率同时保持更好的模型精度。
...
Result: 该方法比现有方法在保持精度方面表现更好，实现了更高的剪枝率。

Conclusion: 该方法为在计算资源有限和数据稀缺环境下的模型压缩提供了更实用的解决方案。
...
TL;DR: 提出了一种短窗口滑动学习框架，用于CCTV监控视频的实时暴力检测。该方法将视频分割为1-2秒的片段，利用大语言模型自动标注构建细粒度数据集，在保持时间连续性的同时实现快速暴力事件的精确识别。


<details>
  <summary>Details</summary>
Motivation: 传统长视频训练方法难以捕捉快速发生的暴力事件，且标注成本高昂。需要一种能够实时检测、充分利用所有帧信息并保持时间连续性的暴力检测方法。

Method: 采用短窗口滑动学习框架，将视频分割为1-2秒的短片段，使用大语言模型进行自动标注构建数据集，每个短片段充分利用所有帧信息。

Result: 在RWF-2000数据集上达到95.25%的准确率，在长视频数据集UCF-Crime上达到83.25%的准确率，显著提升了性能。
...
### [6] [SP-Guard: Selective Prompt-adaptive Guidance for Safe Text-to-Image Generation](https://arxiv.org/abs/2511.11014)
...
<details>
  <summary>Details</summary>
Motivation: 现有的推理时引导方法缺乏自适应性和选择性，无法根据提示词调整引导强度，也无法仅针对图像中的不安全区域进行引导，导致在提高安全性的同时可能过度修改内容。

Method: SP-Guard通过估计提示词危害性，并应用选择性引导掩码来仅对图像中的不安全区域进行引导，实现了自适应和选择性的安全防护。
...
TL;DR: 本文提出了一种竞争性强且计算效率高的概率性降雨临近预报方法，使用预训练的卫星视觉编码器结合视频投影器和轻量级概率头来预测4小时累积降雨量的离散经验累积分布函数。


<details>
  <summary>Details</summary>
Motivation: 开发一种更准确和计算效率高的概率性降雨临近预报方法，以改进传统的确定性预报方法，提供更可靠的不确定性量化。

Method: 将视频投影器(V-JEPA Vision Transformer)和轻量级概率头连接到预训练的卫星视觉编码器(DINOv3-SAT493M)，将编码器标记映射到降雨量的离散经验CDF上，通过连续排序概率得分进行端到端优化。

Result: 在Weather4Cast 2025基准测试中，该方法取得了有前景的性能，CRPS得分为3.5102，相对于最佳3D-UNET基线实现了约26%的效果提升。

Conclusion: 所提出的方法在概率性降雨临近预报任务中表现出色，显著优于传统的3D-UNET方法，为天气预报提供了更准确和高效的概率预测解决方案。
...
TL;DR: PhaseWin是一种新颖的相位窗口搜索算法，用于对象级基础模型的忠实区域归因，以近线性复杂度替代传统二次成本贪婪选择，显著减少模型评估次数。


<details>
  <summary>Details</summary>
Motivation: 现有基于子模子集选择的方法虽然忠实度高，但效率限制阻碍了在实际场景中的部署应用。

Method: 采用分阶段粗到精搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，近似贪婪行为。

Result: 理论上有近贪婪近似保证，实证中仅用20%计算预算就达到贪婪归因95%以上的忠实度，在物体检测和视觉定位任务中优于其他基线方法。
...
TL;DR: 本文提出了一种正负提示监督方法，通过优化正负提示来增强基于能量的OOD检测性能，在多个基准数据集上优于现有方法。
...
Method: 使用LLM初始化类特定的正负提示，然后优化这些提示：正提示关注类内特征，负提示关注类别边界特征。采用基于图的结构聚合语义感知监督并传播到视觉分支。

Result: 在CIFAR-100和ImageNet-1K两个基准数据集上，针对8个OOD数据集和5种不同LLM的广泛实验表明，该方法优于最先进的基线方法。

Conclusion: 所提出的正负提示监督方法能有效提升OOD检测性能，通过优化提示表示和语义知识传递实现了更好的检测效果。
...
TL;DR: 本研究评估了YOLOv11n和YOLOv12n两种轻量级模型在面部表情识别中的表现，发现在干净数据集上YOLOv12n表现更优，而在噪声环境下YOLOv11n具有更高精度。


<details>
  <summary>Details</summary>
Motivation: 在无约束真实环境中，面部表情识别仍具挑战性，需要平衡性能与效率的轻量级解决方案。
...
Conclusion: 轻量级YOLO模型能有效平衡敏感性和精确性，适用于实时、资源受限的情感感知AI应用。
...
TL;DR: HCD是一个简单有效的异构知识蒸馏框架，通过整合教师和学生的互补特征来对齐共享logits表示，解决ViT到ResNet等异构架构蒸馏中的特征表示差异问题。


<details>
  <summary>Details</summary>
Motivation: 异构架构蒸馏（如ViT到ResNet）面临空间特征表示差异的挑战，传统同构KD方法难以有效解决。现有异构KD方法计算成本高、设计复杂或过度依赖logit对齐，限制了互补特征的利用。

Method: 提出HCD框架：通过卷积投影器和自适应池化处理学生中间特征，与教师倒数第二层特征拼接，通过CFM模块映射生成共享logits；引入SDD将共享logits划分为n个子logits与教师logits融合修正分类；使用正交损失确保子logit多样性。
...
TL;DR: 本文提出FedBCS方法解决联邦学习中医疗图像分割的特征异质性问题，通过领域不变上下文原型对齐来弥合特征表示差距。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中不同扫描仪或协议导致的特征异质性是一个主要挑战。现有方法主要关注最终层特征，忽略了多级线索，且中间层的领域特定偏差会累积降低模型鲁棒性。

Method: 提出频率域自适应风格重校准来构建原型，解耦内容-风格表示并学习最优风格参数。设计上下文感知的双层原型对齐方法，从编码器和解码器的不同层提取领域不变原型并与上下文信息融合。

Result: 在两个公共数据集上的大量实验表明，该方法表现出显著性能。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情绪至关重要。现有方法存在两个核心挑战：静态外观与动态运动线索的纠缠阻碍模型关注细微运动；现有数据集中的文本标签与面部肌肉运动不完全对应，存在语义差距。

Method: 提出DEFT-LLM模型，通过多专家解耦实现运动语义对齐。首先构建Uni-MER运动驱动指令数据集，利用光流和动作单元标签的双重约束确保时空一致性。然后设计包含三个专家的架构，将面部动态解耦为独立可解释的表征（结构、动态纹理和运动语义）。

Result: 在多个具有挑战性的MER基准测试中表现出最先进的性能，并在局部面部运动的可解释建模方面具有特殊优势。

Conclusion: DEFT-LLM通过注入有效的物理先验知识并利用大语言模型的跨模态推理能力，能够精确捕捉细微的情绪线索，实现了微表情识别的高性能和可解释性。
...
TL;DR: ERMoE是一种稀疏MoE架构，通过将专家重新参数化在学习的正交特征基中，并用特征基评分（输入特征与专家基的余弦相似度）替代学习门控逻辑，解决了路由不稳定和专家利用率不足的问题。


<details>
  <summary>Details</summary>
Motivation: 解决MoE架构中路由器逻辑与专家内部结构不对齐导致的路由不稳定、专家利用率不足，以及负载不平衡造成的瓶颈问题。

Method: 将每个专家重新参数化在学习的正交特征基中，用特征基评分（余弦相似度）替代学习门控逻辑，实现内容感知路由，无需显式平衡损失。

Result: 在ImageNet分类和跨模态图像-文本检索基准测试中达到最先进精度，自然产生更平坦的专家负载分布，3D MRI变体将脑龄预测精度提高7%以上。

Conclusion: ERMoE为稀疏专家模型引入了新的架构原则，直接解决路由不稳定性，实现了可扩展、可解释的专业化改进性能。
...
TL;DR: 提出DMC和DMC-OT两个框架，用于解决基于CLIP的类增量学习中的分类器偏差和分布漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型在持续学习新类别时，学习任务特定的软提示会导致严重的分类器偏差，文本原型会过度拟合新类别。同时，生成式重放中存储的类级高斯统计量忽略了视觉编码器更新时产生的分布漂移。

Method: DMC采用两阶段框架，解耦视觉编码器适配和文本软提示优化，每个阶段训练时冻结另一模态。DMC-OT在DMC基础上引入最优传输引导的校准策略来对齐演化编码器间的记忆统计量，并设计任务特定提示增强任务间可分性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的广泛实验表明，DMC和DMC-OT都达到了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: 提出的DMC框架通过解耦视觉和文本模态训练有效缓解了分类器偏差问题，而DMC-OT通过最优传输校准进一步解决了分布漂移问题，在多个数据集上取得了优异的类增量学习性能。
...
TL;DR: 本文提出了Phase Aggregated Smoothing (PAS)方法，通过应用相反相位偏移并聚合输出来解决视频LLMs中的时间不一致性问题，无需训练即可提升时间编码的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小变化会翻转注意力并抑制相关帧。这种不稳定性源于通过多模态RoPE将Rotary Position Embeddings扩展到视频时产生的时间核波纹。

Method: 提出PAS方法：在多个头之间应用小的相反相位偏移，然后聚合它们的输出。该方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核并减少相位敏感性，而不改变位置编码结构。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本视觉任务中处理开放性问题时准确率有限，需要一种无需额外训练的统一方法来提升推理性能。

Method: 包含两个步骤：量化将开放性问题转化为多项选择题；二值化将每个候选答案转化为真/假问题，通过确定性规则进行选择。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，该方法相比直接回答开放性问题获得了显著提升，二值化步骤提供了额外的性能增益。

Conclusion: 该工作流提供了一种简单实用的方法，强调推理时设计而非任务特定训练，为当前视觉语言模型提供了更强的零样本视觉能力。
...
TL;DR: CLUE是一个基于Stable Diffusion的生成模型框架，通过固定格式提示实现多样且稳定的图像生成，无需额外数据。在医学领域（中耳炎数据集）上显著提升了生成质量和分类器性能。


<details>
  <summary>Details</summary>
Motivation: 解决在专业领域（如医学）中数据有限且类型单一的情况下，现有文本到图像生成方法难以实现多样性和稳定性的问题。

Method: 基于Stable Diffusion架构，引入Style Encoder处理图像和提示生成风格嵌入，通过新的第二注意力层和Kullback-Leibler散度实现与提示无关的连续潜在空间表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在现实场景中遇到模态缺失时性能显著下降，主要原因是完整多模态数据与不完整模态场景之间的表示学习不一致。现有方法通过简单生成方式处理缺失模态，但无法充分保持跨模态一致性。

Method: 提出PROMISE框架，将多模态提示学习融入分层对比学习框架，配备专门设计的提示注意力机制，动态生成对缺失模态场景的鲁棒且一致的表示。

Result: 在基准数据集上的大量实验和消融研究显示，PROMISE相比当前最先进的多模态方法具有优越性能。

Conclusion: PROMISE通过创新的提示注意力机制有效弥合完整数据与不完整数据之间的表示差距，为模态缺失场景提供了鲁棒的跨模态表示解决方案。
...
TL;DR: EmoVid是首个专门为创意媒体设计的多模态情感标注视频数据集，包含卡通动画、电影片段和动画贴纸，通过情感标注和视觉属性分析揭示了情感感知的时空模式，并开发了基于Wan2.1模型的情感条件视频生成技术，显著提升了生成视频的质量。
...
Method: 引入EmoVid数据集，包含情感标签、视觉属性和文本标注；通过系统分析发现情感感知的时空模式；基于这些洞察开发了情感条件视频生成技术，通过微调Wan2.1模型实现。

Result: 在文本到视频和图像到视频任务中，生成视频的定量指标和视觉质量均有显著提升。

Conclusion: EmoVid为情感视频计算建立了新基准，不仅为艺术风格视频的视觉情感分析提供了宝贵见解，还为增强视频生成中的情感表达提供了实用方法。
...
TL;DR: VisMem是一个基于人类认知记忆理论的视觉语言模型增强框架，通过短期和长期视觉记忆模块解决视觉处理瓶颈问题，显著提升模型性能。
...
Method: 受人类认知记忆理论启发，提出VisMem框架，包含短期模块用于细粒度感知保留和长期模块用于抽象语义整合，通过动态潜在视觉记忆在推理过程中无缝调用。
...
Result: 在CrackVision12K数据集上显著提升细裂纹分割性能（特别是宽度小于4像素的裂纹），在SIDD智能手机图像去噪中PSNR有适度提升，验证了其在低频和高频场景下的鲁棒性。

Conclusion: SUPER解码器块具有即插即用的通用性，可在统一的重构感知框架内实现高频保真度和全局一致性，同时保持可比较的计算成本。
...
TL;DR: 提出了AirCopBench——首个用于评估多模态大语言模型在具身空中协作感知中表现的基准测试，包含14.6k+问题，涵盖4个关键任务维度，在挑战性感知条件下测试模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对单智能体高质量图像的基本感知任务，缺乏评估多智能体协作感知的基准，而多无人机系统相比单传感器具有更好的覆盖范围、鲁棒性和协作能力。

Method: 使用模拟器和真实世界数据构建基准，包含14.6k+问题，涵盖场景理解、目标理解、感知评估和协作决策4个维度14种任务类型，通过模型、规则和人工方法生成问题并严格质量控制。

Result: 在40个MLLMs上的评估显示协作感知任务存在显著性能差距，最佳模型平均落后人类24.38%，且在不同任务间表现不一致。微调实验证实了空中协作感知和推理中模拟到真实迁移的可行性。

Conclusion: AirCopBench填补了多智能体协作感知评估的空白，揭示了当前MLLMs在复杂协作场景下的局限性，为未来研究提供了重要基准。
...
Method: 提出两阶段扩散框架：首先训练冻结帧级编码器提取鲁棒多焦点特征；然后引入多焦点特征融合策略构建3D感知形态表示，并设计混合语义边界条件块注入扩散去噪过程。
...
TL;DR: 提出了一种混合粒度缓存(HGC)方法，通过在不同计算阶段采用不同粒度的缓存策略来降低可控生成模型的计算开销，包括基于特征重用的块级粗粒度缓存和模块内提示级细粒度缓存。


<details>
  <summary>Details</summary>
Motivation: 解决可控生成模型在处理控制条件和内容生成时计算效率低下的问题，通过缓存策略减少冗余计算。

Method: 采用混合粒度缓存策略：1) 块级粗粒度缓存基于特征重用在模型推理步骤间动态绕过编码器-解码器块中的冗余计算；2) 提示级细粒度缓存重用连续推理步骤中的交叉注意力图，并将其扩展到相邻步骤的相应模块计算中。

Result: 在COCO-Stuff分割基准测试中，HGC显著降低了63%的计算成本(MACs从18.22T降至6.70T)，同时将语义保真度损失控制在1.5%以内。

Conclusion: HGC方法在四个基准数据集上验证了有效性，特别是在平衡生成效率和视觉质量方面具有优势，能够显著降低计算开销同时保持生成质量。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集重新构建为统一的视觉问答格式，评估LLaVA-Vicuna-7B和Qwen2-VL-7B在相关、不相关和零重叠MAT分割下的表现。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降。跨任务转移显示仅使用分类数据训练也能提升分割性能7% cIoU。

Conclusion: 多模态大语言模型在组合泛化方面表现优异，CrossMed为评估医学视觉语言模型的零样本、跨任务和模态无关泛化能力提供了严格测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 解决4D流MRI高时空分辨率需求与长扫描时间之间的矛盾，以及现有PINNs方法需要为每个患者单独训练的局限性。

Method: 提出轴对齐时空高斯表示方法，包括归一化高斯渲染、轴对齐高斯简化训练、高斯合并过程等技术创新。

Result: 在计算流体动力学和真实4D流MRI数据集上的实验表明，PINGS-X显著减少训练时间，同时实现优越的超分辨率精度。

Conclusion: PINGS-X框架为4D流MRI超分辨率提供了高效准确的解决方案，具有实际应用价值。
...
TL;DR: 本文提出了一种多参考生成式图像合成方法，通过校准前景参考图像的全局和局部特征来解决现有方法在细节保持和前景姿态/视角调整方面的困难。
...
Method: 将现有生成式合成模型扩展到多参考版本，允许使用任意数量的前景参考图像，并提出校准前景参考图像的全局和局部特征以使其与背景信息兼容。
...
TL;DR: LiteAttention是一种利用扩散注意力中时间相干性来加速视频扩散变换器的方法，通过早期标记非必要tile并向前传播跳过决策，消除冗余注意力计算，在保持质量的同时显著提升速度。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散变换器加速方法面临基本权衡：动态估计稀疏注意力模式计算开销大且存在估计误差，而静态稀疏模式固定且在整个去噪过程中通常不是最优的。

Method: 识别扩散注意力的关键结构特性——其稀疏模式在去噪步骤间表现出强时间相干性，利用这一观察引入LiteAttention方法，通过标记非必要tile并传播跳过决策来实现跨去噪序列的进化计算跳过。

Result: 在FlashAttention基础上实现高度优化的LiteAttention内核，在生产视频扩散模型上展示了显著加速效果，且质量没有下降。
...
TL;DR: 本文提出了一种名为S2D-Align的新型监督微调范式，通过从粗到深的策略利用多粒度辅助信号来建立解剖学基础的跨模态对齐，以改进放射学报告生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告生成方法主要依赖标准监督微调范式进行图像-文本对的实例级对齐，但由于报告的模板化特性，无法建立解剖学基础的对齐，导致生成质量不理想。

Method: 提出S2D-Align方法，采用从粗到深的策略：首先进行粗粒度的放射图像-报告配对，然后引入参考报告进行实例级指导，最后使用关键短语将生成过程锚定在特定解剖细节上。通过基于记忆的适配器桥接不同对齐阶段，实现特征共享。

Result: 在公开的MIMIC-CXR和IU X-Ray基准测试中，S2D-Align相比现有方法取得了最先进的性能。消融研究验证了多阶段辅助引导方法的有效性。

Conclusion: S2D-Align为增强复杂多模态生成任务中的基础能力提供了一个有前景的方向，通过渐进式对齐策略显著提升了放射学报告生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 尽管生成模型在3D数据等多种模态中得到了广泛应用，但对于哪种模型最适合特定任务尚未达成共识。此外，文本和图像等条件信息经常被用于引导生成过程，而部分3D数据等条件信息尚未得到充分评估。
...
TL;DR: 提出了Phys-Liquid数据集，包含97,200张模拟图像和对应的3D网格，用于透明变形液体的几何和体积属性估计，通过四阶段重建管道验证了数据集的真实性和有效性。


<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器移动会引发液体表面动态变形，而现有数据集缺乏全面的物理信息模拟数据来表征真实液体行为。

Method: 引入Phys-Liquid物理信息数据集，包含多个实验室场景、光照条件、液体颜色和容器旋转下的液体动态；提出四阶段重建和估计管道：液体分割、多视角掩码生成、3D网格重建和真实世界缩放。
...
TL;DR: SaTformer是一个基于全时空注意力的视频变换器模型，用于从卫星辐射数据中准确预测极端降水，在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报模型在临近预报时间尺度（0-4小时）性能下降，且计算成本高。AI天气预报方法虽然成功，但视频理解架构在天气预报中的应用仍未被充分探索。

Method: 提出SaTformer架构，使用全时空注意力的视频变换器；将降水回归问题重新表述为分类问题；采用类别加权损失函数处理标签不平衡问题。

Result: 该模型在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名。

Conclusion: SaTformer通过视频变换器架构和专门处理长尾数据集的技术，成功实现了从卫星辐射数据中预测极端降水，为AI天气预报提供了新的解决方案。
...
TL;DR: VideoP2R是一个面向视频语言模型的强化微调框架，通过将感知和推理建模为独立过程来提升视频推理能力，在7个基准测试中的6个达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 将强化微调(RFT)扩展到大型视频语言模型(LVLMs)存在挑战，需要专门的方法来处理视频推理任务。

Method: 提出两阶段框架：SFT阶段使用三步流程生成高质量的过程感知链式思维数据集；RL阶段引入过程感知组相对策略优化算法，为感知和推理提供独立奖励。

Result: 在7个视频推理和理解基准测试中的6个达到最先进性能，消融研究证实了过程感知建模和PA-GRPO的有效性。

Conclusion: VideoP2R通过过程感知建模成功提升了视频推理能力，证明了感知输出为下游推理提供了充分信息。
...
TL;DR: 本文回顾了24篇关于AI生成媒体检测的最新研究，总结了现有方法的局限性和关键挑战，并提出以多模态深度学习模型为重点的研究方向，以提供更鲁棒和通用的检测能力。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，生成逼真合成媒体的能力大幅提升，导致难以区分真实与合成内容。深度伪造等技术的滥用可能传播虚假信息、侵犯隐私和进行欺诈，因此需要开发有效的检测方法。

Method: 通过系统回顾24篇最新研究，逐一分析每项研究的贡献和弱点，总结现有检测方法的共同局限性，并提出基于多模态深度学习模型的研究方向。

Result: 现有检测方法主要基于CNN和ViT等深度学习技术，寻找视觉、空间或时间异常，但往往难以泛化到未见数据，对不同模型生成的内容效果不佳，且在多模态数据和高度修改内容上表现不足。

Conclusion: 多模态深度学习模型有望提供更鲁棒和通用的AI生成媒体检测能力，为未来研究者构建更强防御系统提供明确的起点。
...
TL;DR: 本文提出了一种大型矢量化字形模型LVGM，通过预测下一个笔画来生成矢量化中文字形，并发布了基于笔画的大规模中文SVG数据集。


<details>
  <summary>Details</summary>
Motivation: 利用大语言模型的序列预测能力，通过笔画建模实现矢量化字符生成，解决矢量化字形在多种应用场景中的需求。

Method: 将笔画编码为离散潜在变量（笔画嵌入），通过微调DeepSeek LLM来预测下一个笔画嵌入，从而生成完整的矢量化字符。

Result: 模型能够在给定有限笔画的情况下生成完整字符、语义优美的词语甚至未见过的诗句，实验显示模型在数据规模上具有扩展性。

Conclusion: LVGM模型成功实现了矢量化中文字形的生成，生成的矢量化字形得到了专家和相关人员的验证。
...
TL;DR: 本文提出了Hindsight Distilled Reasoning (HinD)框架和Knowledge Encouragement Preference Optimization (KEPO)方法，用于激发多模态大语言模型的内在知识推理能力，解决KBVQA中推理过程不明确的问题。
...
Method: 首先通过提示冻结的7B规模MLLM完成问题与真实答案之间的推理过程，构建Hindsight-Zero训练数据；然后将其自蒸馏为思维链生成器和知识生成器；最后使用KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA上的实验验证了HinD的有效性，使用7B规模MLLM激发的推理实现了优越性能，无需商业模型API或外部知识。
...
<details>
  <summary>Details</summary>
Motivation: DDIB方法虽然能保持循环一致性并具有灵活性，但面临翻译效率低和潜在分布不匹配导致的翻译轨迹偏差问题，需要改进。
...
Conclusion: OT-ALD有效平衡了图像翻译速度与质量，通过最优传输映射解决了潜在分布不匹配问题，显著提升了翻译性能。
...
TL;DR: 本文提出了一种基于混响变换的轨迹预测模型Rev，通过两个可学习的混响核来模拟和预测不同智能体对轨迹变化事件的延迟响应偏好及其随机性，实现基于预测延迟的可控轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法难以显式学习和预测智能体响应轨迹变化事件时的延迟（latency），不同智能体对特定事件可能有不同的注意、处理和反应延迟偏好，忽略这些延迟会削弱预测系统的因果连续性并导致不合理的轨迹。

Method: 受声学中混响曲线的启发，提出了混响变换和相应的Rev轨迹预测模型，使用两个显式可学习的混响核来模拟每个智能体的不同延迟偏好及其随机性。

Result: 在多个数据集上的实验表明，Rev在保持竞争力的预测精度的同时，能够揭示跨智能体和场景的可解释延迟动态。

Conclusion: 提出的混响变换具有作为通用延迟建模方法的潜力，定性分析进一步验证了其特性。
...
TL;DR: MultiTypeFCDD是一个轻量级卷积框架，用于可解释的多类型异常检测，能够区分不同异常类型，无需为每个对象类别训练单独模型。


<details>
  <summary>Details</summary>
Motivation: 现有可解释异常检测方法无法区分异常类型，且需要为每个对象类别训练和维护单独模型，这在实际应用中成本高昂。同时，现有大规模视觉语言模型计算密集，难以在实时或嵌入式系统中使用。

Method: 提出MultiTypeFCDD框架，仅使用图像级标签学习并生成多通道热图，每个通道对应特定异常类型。该模型作为统一框架，能够跨多个对象类别区分异常类型。

Result: 在Real-IAD数据集上的评估表明，该方法在显著减少参数负载和推理时间的同时，与最先进复杂模型的结果相当。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注普通交通场景，限制了协同感知在复杂不利交通场景中的潜力。为克服这一限制，需要专门针对复杂不利交通场景的数据集。

Method: 使用两辆硬件时间同步的车辆收集数据，覆盖10种天气和光照条件，提供LiDAR点云、相机图像、GNSS/IMU记录，并提出基于目标的时序对齐方法确保多模态数据精确对齐。

Result: 构建了包含60K帧LiDAR点云、1.26M张相机图像、750K条GNSS/IMU记录的100片段数据集，提供时间一致的3D边界框标注和静态场景4D BEV表示。
...
TL;DR: AlignVQA是一个基于辩论的多智能体框架，通过多样化提示策略的视觉语言模型生成候选答案，经过两阶段交互（通用智能体批判、精炼和聚合）来产生更准确的置信度估计。


<details>
  <summary>Details</summary>
Motivation: 现代VQA系统在医疗诊断和自动驾驶等高风险领域应用广泛，但其置信度估计的可靠性研究不足，常产生过度自信的响应，需要提高校准度。

Method: 提出AlignVQA辩论框架，使用多样化提示策略的专门化VLM生成答案，通过两阶段交互过程（通用智能体批判、精炼和聚合）来校准置信度，并引入可微分的校准感知损失函数aligncal。

Result: 在多个基准VQA数据集上的实证结果表明，该方法显著减少了校准差异，更校准的专门化智能体产生更好对齐的置信度。

Conclusion: AlignVQA框架通过多智能体辩论和校准感知损失函数，有效提高了VQA系统的置信度校准性能，使其置信度估计更准确反映真实预测性能。
...
TL;DR: 提出了一种用于从非同步多视角视频进行高质量4D高斯溅射重建的新型时间对齐策略，通过粗到细的对齐模块估计和补偿每个相机的时间偏移。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多视角视频通常由于相机触发延迟或独立录制设置而存在时间不同步问题，这降低了现有4D高斯溅射方法的重建质量。

Method: 采用粗到细对齐模块，首先确定粗粒度的帧级偏移，然后细化为亚帧精度，可轻松集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时间错位视频，并显著提升基线方法的性能。

Conclusion: 该方法增强了4DGS框架在处理异步数据时的鲁棒性，为多视角视频重建提供了实用的时间对齐解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型计算成本高，难以在资源受限场景部署，且小模型在捕捉细粒度视觉区域方面表现不佳，影响实际应用效果。

Method: 提出Viper-F1模型，用液态状态空间动力学替代Transformer交叉注意力；引入Token-Grid相关模块计算文本标记与图像块之间的轻量相关性，通过FiLM条件调节状态空间动力学。
...
TL;DR: 本文提出了一个高效框架，用于印度次大陆及周边地区PM1、PM2.5和PM10的6小时领先时间临近预报，采用轻量级参数特定架构，相比Aurora基础模型有显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对有限空间域的紧凑深度学习模型，以提高短期空气质量预报的准确性，减少系统偏差，并实现快速推理。

Method: 利用CAMS全球大气成分预报的分析场作为输入，构建三个轻量级参数特定架构，在256x256空间区域输入，预测中心128x128区域，使用2021-2023年数据进行训练。

Result: 使用RMSE、MAE、Bias和SSIM评估显示，相比Aurora基础模型取得了显著的性能提升，证明了紧凑专门化模型在有限空间域短期预报中的有效性。

Conclusion: 紧凑且专门化的深度学习模型在有限空间域的短期预报中表现出色，为空气质量临近预报提供了高效解决方案。
...
### [46] [3D Gaussian and Diffusion-Based Gaze Redirection](https://arxiv.org/abs/2511.11231)
*Abiram Panchalingam,Indu Bodala,Stuart Middleton*

Main category: cs.CV

TL;DR: DiT-Gaze是一个基于扩散变换器和3D高斯溅射的视线重定向框架，通过弱监督训练和正交约束损失实现高质量视线重定向，在感知质量和重定向精度方面达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射模型在渲染细微连续视线变化时存在困难，需要提升视线重定向的保真度以生成更好的增强数据来改进视线估计器的泛化能力。

Method: 结合扩散变换器(DiT)、跨视线角度的弱监督训练和正交约束损失，DiT实现更高保真度图像合成，弱监督策略使用合成的中间视线角度提供平滑视线方向流形，正交约束损失数学上强制视线、头部姿态和表情的内部表示解耦。

Result: DiT-Gaze在感知质量和重定向精度方面达到新SOTA，将SOTA视线误差降低4.1%至6.353度，为创建合成训练数据提供了更优方法。

Conclusion: DiT-Gaze通过结合DiT、弱监督和正交约束，显著提升了视线重定向的质量和精度，为视线估计研究提供了更高质量的合成数据生成方法。
...
TL;DR: 该研究提出了基于ConvGRU的迁移学习框架，用于Weather4Cast 2025竞赛中的短期降雨预测。使用单一SEVIRI红外通道作为输入，采用两阶段训练策略，在累积降雨任务中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 开发一个有效的短期降雨预测框架，能够从有限的观测数据中预测未来4小时的降雨情况，并识别降水事件。

Method: 使用ConvGRU模型，采用两阶段训练：第一阶段预测SEVIRI亮度温度以捕捉时空模式，第二阶段通过非线性变换将预测场映射到OPERA兼容的降雨率。对于事件预测，使用3D事件检测和时空特征提取。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉问答模型在处理复杂地理空间查询时缺乏结构化推理能力，而地理空间思维链推理对于气候相关应用（如灾害监测、基础设施风险评估等）至关重要。

Method: 集成思维链推理与直接偏好优化，通过生成中间推理过程来处理检测、分类、空间关系和比较分析等任务。

Result: 思维链监督比直接基线准确率提升34.9%，直接偏好优化进一步提升了准确率和推理质量。
...
TL;DR: 研究发现现代视觉语言模型对微小的语义保持扰动高度敏感，即使是像素级偏移、几何变换、重述等不影响语义的修改也会导致预测答案改变。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型取得了显著进展，但其在微小语义保持输入变化下的可靠性仍不清楚，需要系统研究其鲁棒性。

Method: 进行了首个大规模系统研究，评估VLM对多种良性视觉和文本扰动的鲁棒性，包括像素级偏移、几何变换、重缩放、重述和多语言改写。

Result: 现代VLM对微小扰动高度敏感，即使是GPT-4o、Gemini 2.0 Flash等最先进系统也经常在像素级偏移或无害重述下失败。样本稳定性与正确性高度相关，稳定样本更可能被正确回答。
...
### [50] [Toward Gaze Target Detection of Young Autistic Children](https://arxiv.org/abs/2511.11244)
...
TL;DR: 本文提出了一种用于自闭症儿童注视目标检测的新框架SACF，通过利用社交上下文来克服数据集类别不平衡问题，在自闭症儿童注视目标检测任务上达到了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 通过人工智能自动检测自闭症儿童的注视目标具有重要意义，特别是对于那些缺乏足够专业资源来改善生活质量的儿童。该任务对于构建能够测量联合注意力的自动化系统至关重要。

Method: 提出了新颖的社交感知粗到细(SACF)注视检测框架，采用双路径架构，包含专门处理社交和非社交注视的专家模型，并通过上下文感知门模块进行指导。

Result: 综合实验结果表明，该框架在自闭症儿童注视目标检测任务上达到了新的最先进性能，显著优于现有方法，特别是在关键的少数类别——面向面部的注视上表现突出。

Conclusion: 该研究为自闭症儿童的注视目标检测提供了有效的解决方案，通过利用社交上下文有效解决了数据集类别不平衡问题，在关键的面部注视检测上取得了显著改进。
...
TL;DR: 本文提出了首个3D视觉中的一对多后门攻击框架，基于可配置的球形触发器，允许单个触发器设计编码多个目标类别。
...
Result: 在多个数据集和模型架构上的实验验证了该方法的有效性，攻击成功率高达100%，同时在干净数据上保持准确性。
...
<details>
  <summary>Details</summary>
Motivation: 医学成像中数据稀缺使得为每个领域、模态或任务预训练模型具有挑战性，需要一种统一且可扩展的框架来适应多任务和多模态需求。
...
### [53] [AUVIC: Adversarial Unlearning of Visual Concepts for Multi-modal Large Language Models](https://arxiv.org/abs/2511.11299)
*Haokun Chen,Jianing Li,Yao Zhang,Jinhe Bi,Yan Xia,Jindong Gu,Volker Tresp*

Main category: cs.CV

TL;DR: AUVIC是一个针对多模态大语言模型的视觉概念遗忘框架，通过对抗性扰动实现精确的目标概念移除，同时避免对相关实体的影响。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型训练数据常包含敏感或版权内容，需要满足'被遗忘权'的法规要求，但视觉概念遗忘在MLLMs中研究不足，主要挑战是精确移除目标概念而不影响相关实体性能。

Method: 提出AUVIC框架，采用对抗性扰动技术实现精确遗忘，有效隔离目标概念并避免对相似实体的意外影响。

Result: 构建了首个视觉概念遗忘基准VCUBench，实验表明AUVIC在目标遗忘率上达到最先进水平，同时对非目标概念的性能影响最小。

Conclusion: AUVIC为MLLMs提供了有效的视觉概念遗忘解决方案，在满足数据隐私要求的同时保持了模型性能。
...
TL;DR: 本文提出了一种名为Guidance Score Distillation (GSD)的框架，通过利用预训练视频扩散模型的多视角一致性先验，解决3D高斯溅射在稀疏训练视图下容易过拟合的问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏训练视图下容易过拟合，主要原因是缺乏中间视角的监督。受视频扩散模型成功的启发，作者希望利用其丰富的多视角一致性先验来改善这一问题。

Method: 基于Score Distillation Sampling (SDS)的见解，GSD监督来自多个相邻视角的渲染图像，引导高斯溅射表示朝向视频扩散模型的生成方向。为了解决生成方向可能包含物体运动和随机相机轨迹的问题，引入了统一的指导形式来修正视频扩散模型的噪声预测结果，包括基于真实深度图的深度扭曲指导和基于语义图像特征的指导。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: 通过利用视频扩散模型的多视角一致性先验，并结合深度和语义指导，GSD框架有效解决了3D高斯溅射在稀疏视图下的过拟合问题，提升了表示质量。
...
TL;DR: 本文研究了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中，发现文本编码器倾向于关注输入开头，而图像编码器在开头和结尾都表现出偏见。


<details>
  <summary>Details</summary>
Motivation: 位置偏见（模型过度关注某些位置而忽略内容）已被证明对各种任务产生负面影响。虽然最近研究广泛探讨了文本生成模型中的位置偏见，但在表示模型中的存在和影响仍未充分探索，特别是在多模态模型中。

Method: 首先区分上下文重要性和位置偏见，然后评估不同模型和数据集中的位置偏见存在程度。通过实验分析多模态模型中的位置偏见表现模式。

Result: 实验表明位置偏见在多模态模型中普遍存在，但不同模态表现不同：文本编码器倾向于对输入开头产生偏见，而图像编码器在开头和结尾都表现出偏见。

Conclusion: 这种偏见源于或由多种因素共同作用放大，包括位置编码方案、训练损失、上下文重要性以及多模态训练中使用图像-文本对的性质。
...
### [56] [The Persistence of Cultural Memory: Investigating Multimodal Iconicity in Diffusion Models](https://arxiv.org/abs/2511.11435)
*Maria-Teresa De Rosa Palmini,Eva Cetinic*

Main category: cs.CV

TL;DR: 本文研究了文本到图像扩散模型中泛化与记忆之间的模糊性，特别关注多模态标志性现象，即图像和文本唤起文化共享关联的情况。作者提出了一个评估框架，区分识别（模型是否识别出文化引用）和实现（模型如何通过复制或重新诠释来描绘引用），并评估了五个扩散模型在767个维基数据文化引用上的表现。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型中泛化与记忆的模糊界限问题，特别关注文化引用在模型中的处理方式，探索模型如何平衡识别文化参考与复制它们之间的关系。

Method: 引入评估框架分离识别和实现两个维度，量化测量这两个方面。评估五个扩散模型在767个维基数据文化引用上的表现，包括静态和动态图像。进行提示扰动实验，使用同义词替换和字面图像描述来评估语言敏感性。

Result: 框架比现有基于相似性的方法更有效地区分复制与转换。模型即使在文本线索改变时也经常复制标志性视觉结构。文化对齐不仅与训练数据频率相关，还与文本独特性、引用流行度和创建日期相关。

Conclusion: 扩散模型的价值不仅在于它们复制的内容，还在于它们如何转换和重新情境化文化知识。这项工作推动了评估从简单的文本-图像匹配向更丰富的上下文理解发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D深度学习在多领域泛化受限的问题，主要由于现有数据集规模有限和多源点云的高度异构性，不同传感器采集的点云在密度和噪声分布上存在显著差异，导致多领域融合时出现负迁移。

Method: 提出DoReMi框架，包含领域感知专家分支和统一表示分支。通过领域引导空间路由动态激活专家分支，使用熵控制动态分配实现稳定高效的专家利用，同时采用冻结的统一表示分支保持跨领域几何和结构先验。

Result: 在多个3D理解基准测试中表现优异，ScanNet Val上达到80.1% mIoU，S3DIS上达到77.2% mIoU，相比现有方法具有竞争力或更优性能。

Conclusion: DoReMi展示了作为未来3D理解研究基础框架的强大潜力，能够有效解决多源点云数据异构性问题，实现专业化与泛化知识的协同学习。
...
<details>
  <summary>Details</summary>
Motivation: 通用图像编辑模型在面对新风格时往往无法产生令人满意的结果，需要解决如何用有限的配对数据有效微调模型到新风格的问题。

Method: 提出参数高效的多风格MoE LoRA方法，包含风格特定和风格共享路由机制；采用度量引导方法自动确定每层最优秩；探索在DiT模型中插入LoRA的最佳位置；集成对抗学习和流匹配来指导扩散训练过程。

Result: 实验结果表明，所提方法在显著减少LoRA参数的情况下，性能优于现有最先进方法。

Conclusion: 该框架能够有效解决少样本风格编辑问题，通过智能路由机制和参数优化策略实现了高效的多风格适应。
...
TL;DR: GEODE是一个解决视觉语言模型在3D空间理解上双重瓶颈的新架构，通过解耦3D推理和数值生成，使用可插拔模块实现先进的3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解真实世界3D空间智能方面存在根本性困难，主要由于输入阶段的几何感知编码器与2D特征冲突，以及输出阶段的离散分词器无法生成精确连续数值的双重瓶颈。

Method: 引入GEODE架构，包含两个可插拔模块：解耦推理模块(DRM)作为空间协处理器，通过交叉注意力对齐3D数据与2D视觉特征；直接回归头(DRH)采用"嵌入即值"范式，通过轻量级MLP实现精确连续回归。

Result: 仅1.5B参数的模型实现了最先进的3D空间推理性能，可与7B+参数的模型相媲美。

Conclusion: GEODE通过解耦3D推理和数值生成的双模块架构，有效解决了视觉语言模型在3D空间理解上的双重瓶颈，为3D空间智能提供了新的解决方案。
...
### [60] [PAS : Prelim Attention Score for Detecting Object Hallucinations in Large Vision--Language Models](https://arxiv.org/abs/2511.11502)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，研究发现模型在幻觉预测时会忽略图像而依赖先前生成的token来推断新物体。

Method: 通过分析图像与预测物体间的互信息，引入Prelim Attention Score (PAS)，从注意力权重中计算信号来检测幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但往往无法遵循文本中的数值指令，这揭示了语言与视觉表示之间的差距。研究发现这些模型并非完全对数字不敏感，它们隐式地意识到自己的计数准确性。

Method: 基于模型内部对数值正确性的潜在认知，提出CountSteer方法，在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练。

Result: 实验表明，CountSteer将对象计数准确率提高了约4%，同时保持视觉质量不受影响。

Conclusion: CountSteer是朝着更可控和语义可靠的文本到图像生成迈出的简单而有效的一步，展示了利用模型内部信号改进生成质量的可能性。
...
TL;DR: 该论文提出了一种通过分组caption tokens来获取细粒度视觉语言表示的方法，将语言表示与图像中对象级别的信息对齐，从而提升模型对视觉和语言的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型大多专注于将图像块与语言token对齐，但图像块对人类视觉没有意义，单个token也不一定携带可接地的图像信息。需要将token组合来描述场景的不同方面，以获得对现实世界更好的细粒度理解。

Method: 提出了一种在架构中分组caption tokens的模型，通过将语言表示与经过训练发现对象的图像编码器输出对齐，捕捉语言的细粒度表示。模型学习对token进行分组，使表示达到图像中对象的级别。

Result: 通过学习token分组，视觉语言模型对视觉和语言有了更好的细粒度理解。模型发现的token组在定性和定量上都与文本中的可接地短语高度相似。

Conclusion: 通过分组caption tokens来获取细粒度语言表示的方法有效提升了视觉语言模型的细粒度理解能力，且发现的token组与可接地短语具有高度相似性。
...
TL;DR: 提出了一种模型无关的方法，通过交通场景图来增强基于语言的驾驶模型，显著提升了自动驾驶性能，在LangAuto基准测试中驾驶分数提升高达17.5%。
...
Conclusion: 模型能够通过场景图条件化训练更好地内化和基础关系先验，即使在测试时不需要场景图输入也能保持性能提升。
...
TL;DR: 提出CORAL方法解决体积医学图像分割中解剖结构建模问题，通过对比排序和序数目标来捕获局部和全局解剖结构，在有限标注下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用硬二值阈值定义正负样本，丢弃了连续解剖相似性信息，且忽略了全局解剖进展的方向一致性，导致特征空间扭曲无法捕获跨患者的规范解剖流形。

Method: CORAL采用对比排序目标利用连续解剖相似性，确保切片间特征距离与其解剖位置差异成比例；同时加入序数目标强制全局方向一致性，使学习特征分布与跨患者规范解剖进展对齐。

Result: 在基准数据集上，CORAL在有限标注设置下实现了最先进的性能，同时学习了具有有意义解剖结构的表示。

Conclusion: 通过协调学习框架，CORAL能够学习解剖感知表示，有效提升下游分割任务的性能，代码已开源。
...
TL;DR: SimuFreeMark是一个免噪声模拟的水印框架，利用图像低频分量的固有稳定性来嵌入水印，无需在训练中使用噪声模拟层，能够抵抗传统信号处理和语义编辑攻击。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法依赖于手工制作的噪声模拟层进行训练，这限制了它们对未知失真的泛化能力。需要一种能够抵抗传统和新型语义编辑攻击的鲁棒图像水印方法。

Method: 利用图像低频分量对多种攻击的鲁棒性，将水印直接嵌入到低频分量的深度特征空间中，使用预训练的变分自编码器将水印与结构稳定的图像表示绑定，完全消除了训练过程中的噪声模拟需求。

Result: 广泛的实验表明，SimuFreeMark在多种传统和语义攻击下均优于现有最先进方法，同时保持优异的视觉质量。

Conclusion: SimuFreeMark通过利用低频分量的稳定性，成功实现了免噪声模拟的鲁棒水印嵌入，在多种攻击下表现出色，为AIGC内容保护提供了有效解决方案。
...
TL;DR: DocSLM是一种高效的小型视觉语言模型，专为内存受限环境下的长文档理解设计，通过分层多模态压缩器和流式弃权机制，在减少82%视觉标记、75%参数和71%延迟的同时，保持与最先进方法相当的性能。
...
Method: 采用分层多模态压缩器联合编码视觉、文本和布局信息为固定长度序列，并引入基于熵的不确定性校准器的流式弃权机制进行可扩展处理。
...
TL;DR: BOFA框架通过仅调整CLIP的跨模态桥接层来实现类增量学习，无需额外参数，采用正交低秩融合防止遗忘，结合跨模态混合原型提升分类性能。
...
Method: 提出BOFA框架，仅调整CLIP现有的跨模态桥接层，使用正交低秩融合机制将参数更新限制在低秩安全子空间内，结合跨模态混合原型进行分类。

Result: 在标准基准测试中，BOFA相比现有方法实现了更高的准确率和效率。

Conclusion: BOFA通过桥接层正交融合策略，在无需数据回放的情况下实现了稳定的类增量学习，显著提升了CLIP在增量学习场景下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决事件视觉领域缺乏与帧基计算机视觉相媲美的综合合成数据资源的问题，为6DoF物体姿态估计提供高质量的事件相机数据。

Method: 使用基于物理的渲染(PBR)场景和BOP方法生成合成数据，采用模拟线性相机运动确保完整场景覆盖，系统评估不同事件表示方法用于CNN推理。

Result: 发现具有线性衰减和双通道极性编码的时间表面在姿态估计性能上表现最佳，显著优于指数衰减和单通道替代方案，极性信息对性能提升贡献最大。

Conclusion: 成功创建了首个全面的标准分辨率事件相机数据集，确定了最优的事件表示方法，为事件视觉研究提供了可直接使用的资源和可复现的基准测试。
...
TL;DR: VoxTell是一个基于视觉语言模型的医学图像分割系统，能够将自由形式的文本描述映射到3D分割掩码，支持从单个词语到完整临床句子的多种输入形式。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解自由形式文本描述并执行3D医学图像分割的通用模型，以解决传统分割方法对特定类别标注的依赖问题。

Method: 使用多阶段视觉语言融合策略，在解码器层间对齐文本和视觉特征，在62K+ CT、MRI和PET体积数据上训练，涵盖1000多个解剖和病理类别。

Result: 在未见数据集上实现最先进的零样本性能，在熟悉概念上表现出色，同时能够泛化到相关的未见类别，展示了强大的跨模态迁移能力和对临床语言的鲁棒性。

Conclusion: VoxTell为医学图像分割提供了一种灵活且强大的文本驱动方法，能够准确处理真实世界的文本描述，具有重要的临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体运动生成模型虽然重建精度高，但难以泛化到训练分布之外，这源于精确3D监督导致模型过度拟合固定坐标模式，而非学习必要的3D结构和运动语义线索。

Method: 提出Motion-Lifting Residual Quantized VAE (ML-RQ)将2D运动序列映射到3D一致潜在空间，并设计3D自由正则化目标，包括视图一致性、方向连贯性和物理合理性约束。
...
TL;DR: 提出了一种结合隐马尔可夫随机场(HMRF)理论和CNN分割的方法，实现无监督学习和快速分割，在PU泡沫结构的μCT图像数据集上达到高分割精度，并提出减少标注数据需求的预训练策略。


<details>
  <summary>Details</summary>
Motivation: 从图像中提取材料表征是定量分析材料特性的必要前提。传统分割方法在精度或速度上存在不足，有监督CNN需要大量标注数据，而无监督方法分割时间长且精度较差。

Method: 集成HMRF理论和CNN分割，结合无监督学习和快速分割的优势。研究不同邻域项和组件对无监督HMRF损失的贡献，使用HMRF-UNet架构。
...
TL;DR: LSEF框架通过低秩稀疏原理分层解耦情感基态和瞬时波动，解决了视频情感计算中的模型不稳定和表征退化问题。


<details>
  <summary>Details</summary>
Motivation: 视频情感计算因复杂情感动态导致模型不稳定和表征退化，核心问题在于缺乏分层结构机制来解耦不同的情感成分（情感基态和瞬时波动）。

Method: 提出LSEF框架，包含三个即插即用模块：稳定性编码模块捕获低秩情感基态，动态解耦模块分离稀疏瞬时信号，一致性集成模块重建多尺度稳定性与反应性一致性，采用秩感知优化策略自适应平衡梯度平滑性和敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法忽视了视觉与脑电信号之间的根本不对称性，包括保真度差距（EEG噪声与视觉高保真特征）和语义差距（EEG浅层概念与视觉丰富语义）。这种不对称性导致现有方法泛化能力差。

Method: 提出自适应教学范式，让"教师"模态（视觉）在任务指导下动态缩小和调整知识结构，使用ShrinkAdapter模块（无残差设计和瓶颈结构）来匹配"学生"模态（EEG）的能力。
...
TL;DR: WEAVE是首个用于上下文交错跨模态理解和生成的套件，包含WEAVE-100k数据集和WEAVEBench基准，旨在解决现有方法在多轮、上下文依赖的图像创建和编辑方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型主要关注单轮交互，无法捕捉现实世界图像创建和编辑的多轮、上下文依赖特性。

Method: 提出WEAVE套件：WEAVE-100k包含10万个交错样本，覆盖37万对话轮次和50万图像；WEAVEBench包含100个基于480张图像的任务，采用混合VLM评估框架。

Result: 在WEAVE-100k上训练能够提升视觉理解、图像编辑和理解-生成协作能力，促进视觉记忆能力的涌现。WEAVEBench评估揭示了当前方法在多轮上下文感知图像生成和编辑方面的局限性。

Conclusion: WEAVE为研究上下文交错理解和生成提供了视角和基础，推动了多模态社区的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的解码器大多直接基于fMRI特征，忽略了视觉信息在皮层中的层次组织方式，这模糊了早期、中期和晚期视觉区域的不同作用。
...
TL;DR: VP-Bench是一个评估多模态大语言模型理解和使用视觉提示能力的基准测试，包含感知和下游任务两个阶段，评估了28个模型并分析了影响视觉提示理解的因素。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试没有系统评估MLLMs解释视觉提示的能力，不清楚当前模型是否能有效识别这种直观的人类提示方法并用于解决问题。

Method: 采用两阶段评估框架：第一阶段测试模型在自然场景中感知视觉提示的能力，使用30k可视化提示；第二阶段研究视觉提示对下游任务的影响。

Result: 评估了28个MLLMs，包括专有系统和开源模型，提供了关于视觉提示属性变化、问题安排和模型规模等因素影响VP理解的综合分析。
...
TL;DR: 该论文提出了一种通过生成平衡合成数据来微调视觉语言模型的新方法，解决了传统微调过程中的偏见、分布不平衡和标注错误问题，显著提升了模型在真实世界数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型微调过程存在偏见、错误和分布不平衡问题，导致过拟合和性能不平衡。现有合成数据方法缺乏对分布偏见和标注质量的控制。

Method: 重新设计微调过程：1）控制数据和标注生成，确保无偏见、分布平衡和无标注错误；2）自动构建数据集，全面采样对象属性（颜色、形状、大小、位置）；3）使用该数据集微调最先进的视觉语言模型。

Result: 实验发现：1）在平衡合成数据上微调可在整个视觉场景中产生均匀性能并减轻常见偏见；2）在合成刺激上微调显著提升在真实世界数据（COCO）上的性能，优于匹配设置下微调的模型。

Conclusion: 通过控制生成平衡合成数据的方法能够有效解决传统微调过程中的问题，提升模型在真实世界任务中的泛化能力和性能表现。
...
TL;DR: 提出了一种名为MaMOL的框架，通过双路由机制处理遥感多模态分类中的模态缺失问题，将模态缺失重新定义为多任务学习问题，实现了参数高效的适应。
...
Method: 提出Missing-aware Mixture-of-Loras (MaMOL)框架，引入双路由机制：任务导向的动态路由器自适应激活专家处理不同缺失模式，模态特定共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效适应。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于免疫组化的PD-L1表达评估方法资源密集，需要开发更高效、可扩展的替代方案。

Method: 基于nnUNet-v2构建，在循环训练期间采样多样化模型检查点来近似后验分布，通过熵和标准差实现不确定性估计。

Result: 在肺鳞状细胞癌数据集上，平均Dice得分0.805，平均IoU 0.709，性能与现有基线方法相当，同时提供像素级不确定性图谱。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型要么在编码器早期融合特征，要么在后期比较池化嵌入，但两种模态的隐藏状态已经包含丰富的模态特定结构，直接对齐这些状态是更自然的融合方式。

Method: 在视觉和文本编码器顶部放置少量仅跨模态的双向注意力层，将隐藏状态投影到共享空间，进行跨模态注意力计算，并通过门控残差更新返回，使用简单稳定器改善对齐效果。

Result: 在标准检索、VQA和视觉推理基准测试中，BRIDGE优于可比视觉语言模型，同时保持对比模型的双编码器效率。

Conclusion: BRIDGE提供了一种高效的多模态融合方法，在编码器层面实现模态对齐，同时保持生成任务的解耦设计。
...
Conclusion: DocLens通过增强的定位能力证明了其在长视觉文档理解中的有效性，特别是在处理视觉中心和不可回答查询时展现出强大优势。
...
### [83] [The Second Law of Intelligence: Controlling Ethical Entropy in Autonomous Systems](https://arxiv.org/abs/2511.10704)
*Samih Fadli*
...
### [84] [Picking a Representative Set of Solutions in Multiobjective Optimization: Axioms, Algorithms, and Experiments](https://arxiv.org/abs/2511.10716)
*Niclas Boehmer,Maximilian T. Wittmann*

Main category: cs.AI

TL;DR: 本文研究多目标决策中的帕累托剪枝问题，提出新的质量度量方法并分析计算复杂性，通过实验验证新方法在不同场景下的竞争力。


<details>
  <summary>Details</summary>
Motivation: 多目标决策问题中需要从帕累托最优解集中选择代表性子集，现有质量度量方法存在不直观行为，需要改进。

Method: 将帕累托剪枝问题重新构建为多赢家投票问题，进行公理化分析，提出新的定向覆盖度量方法，并分析各种质量度量的计算复杂性。

Result: 发现现有质量度量存在反直觉行为，新提出的定向覆盖度量在实验中表现具有竞争力甚至更优。

Conclusion: 质量度量的选择对所选解集特性有决定性影响，新提出的定向覆盖度量在多种设置下表现良好。
...
### [85] [Structure-Aware Encodings of Argumentation Properties for Clique-width](https://arxiv.org/abs/2511.10767)
*Yasir Mahmood,Markus Hecher,Johanna Groven,Johannes K. Fichte*

Main category: cs.AI

TL;DR: 本文研究了图参数 clique-width 在编码中的应用，特别针对抽象论证框架设计了新的归约方法，将论证问题转化为(Q)SAT问题，并在线性保持 clique-width 的前提下实现了高效的编码。


<details>
  <summary>Details</summary>
Motivation: 现有的SAT求解器在小树宽实例上表现高效，但对于更一般的图参数 clique-width（即使在稠密图上也可能很小）的编码能力了解甚少。抽象论证框架作为基于有向图的推理框架，是研究计算性质的理想候选。

Method: 设计了从论证问题到(Q)SAT的新归约方法，这些归约线性保持 clique-width，形成了有向分解引导(DDG)归约。

Result: 为所有论证语义（包括计数）建立了新结果，证明DDG归约引起的开销在合理假设下无法显著改进。

Conclusion: 研究开启了理解 clique-width 编码能力的新方向，为基于 clique-width 的高效算法和编码提供了理论基础。
...
### [86] [Potential Outcome Rankings for Counterfactual Decision Making](https://arxiv.org/abs/2511.10776)
*Yuta Kawakami,Jin Tian*

Main category: cs.AI

TL;DR: 本文研究了反事实决策中的新规则，引入了潜在结果排序概率(PoR)和获得最佳潜在结果概率(PoB)两个新指标，建立了识别定理和边界，提出了估计方法，并通过数值实验验证了估计器的有限样本性质。


<details>
  <summary>Details</summary>
Motivation: 在不确定性下的反事实决策中，决策者通常通过比较候选行动的期望潜在结果来进行选择。本文旨在开发新的决策规则，通过引入PoR和PoB指标来更准确地评估行动偏好。

Method: 引入PoR和PoB两个新指标，建立识别定理和边界，提出估计方法，并通过数值实验验证估计器的性能。

Result: 成功建立了PoR和PoB的识别定理和边界，提出的估计方法在数值实验中表现出良好的有限样本性质，并在真实数据集上得到应用验证。

Conclusion: PoR和PoB为反事实决策提供了新的有效工具，能够更准确地评估个体层面的行动偏好，在不确定性决策中具有重要应用价值。
...
### [87] [From Efficiency to Adaptivity: A Deeper Look at Adaptive Reasoning in Large Language Models](https://arxiv.org/abs/2511.10788)
*Chao Wu,Baoheng Li,Mingchen Gao,Zhenyi Wang*

Main category: cs.AI

TL;DR: 本文从自适应性的角度重新审视大语言模型的推理能力，提出将推理努力根据输入特征进行动态分配，并建立了系统性的分类法来组织现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在推理时采用统一的策略，无论任务复杂度如何，这导致对简单问题生成过长推理链，而对困难任务又无法扩展推理。本文旨在通过自适应性来解决这一根本挑战。

Method: 首先形式化了演绎、归纳和溯因推理在LLM上下文中的定义；其次将自适应推理形式化为控制增强的策略优化问题；最后提出了系统分类法，将现有方法分为基于训练的方法和无需训练的方法。

Result: 建立了一个清晰的框架，阐明了不同机制如何在实际中实现自适应推理，并使得跨不同策略的系统比较成为可能。
...
### [88] [HyperComplEx: Adaptive Multi-Space Knowledge Graph Embeddings](https://arxiv.org/abs/2511.10842)
...
Result: 在计算机科学研究知识图谱上评估，从1K论文到10M论文规模，均优于TransE、RotatE、DistMult等基线方法。在10M论文数据集上达到0.612 MRR，相对最佳基线提升4.8%，同时保持高效训练和85ms推理时间。
...
### [89] [Advanced Tool for Traffic Crash Analysis: An AI-Driven Multi-Agent Approach to Pre-Crash Reconstruction](https://arxiv.org/abs/2511.10853)
...
<details>
  <summary>Details</summary>
Motivation: 传统的交通事故重建依赖人类专家，在处理不完整多模态数据时往往产生不一致的结果。需要开发能够处理异构碰撞数据并提供精确重建的AI系统。

Method: 提出两阶段协作框架：第一阶段从多模态输入生成自然语言事故重建；第二阶段结合重建结果与时序事件数据记录器数据进行深入事故推理。处理了277个追尾前车减速碰撞案例。
...
TL;DR: 本文提出了一种利用大型语言模型（LLMs）作为专家指导的新框架，用于从不规则采样的纵向患者数据中学习神经退行性疾病进展，特别关注阿尔茨海默病中病理蛋白在大脑区域间的传播。


<details>
  <summary>Details</summary>
Motivation: 当前方法过于简化大脑连接性的复杂关系，假设单一模态的脑连接组作为疾病传播基质，导致病理传播预测不准确。同时，纯粹数据驱动的方法由于缺乏适当约束而面临可识别性问题。

Method: 利用LLMs合成多模态关系并整合多种疾病驱动机制的能力，同时优化：1）从个体层面观察构建长期疾病轨迹；2）生物约束的图结构，以更好地捕获大脑区域间的相互作用。

Result: 在阿尔茨海默病队列的tau-PET成像数据上验证，新框架相比传统方法展现出更优的预测准确性和可解释性，同时揭示了超越传统连接性测量的额外疾病驱动因素。

Conclusion: 该框架通过LLMs的专家指导，显著改善了神经退行性疾病进展建模的准确性和可解释性，为理解疾病传播机制提供了新视角。
...
### [91] [Multi-Agent Legal Verifier Systems for Data Transfer Planning](https://arxiv.org/abs/2511.10925)
*Ha-Thanh Nguyen,Wachara Fungwacharakorn,Ken Satoh*

Main category: cs.AI

TL;DR: 提出多智能体法律验证器，通过专业化分工和协调推理，在AI驱动的数据传输规划中提高法律合规性检查的准确性，相比单智能体基线提升21个百分点。


<details>
  <summary>Details</summary>
Motivation: 在严格的隐私法规（如日本个人信息保护法APPI）下，AI驱动的数据传输规划中的法律合规性变得越来越重要，需要更准确和可解释的自动化合规验证方法。

Method: 设计多智能体法律验证器，将合规检查分解为法规解释、业务背景评估和风险评估三个专业智能体，通过结构化合成协议进行协调。

Result: 在200个APPI第16条修正案案例数据集上，系统达到72%准确率，比单智能体基线高21个百分点；在明确合规案例上达到90%准确率（基线仅16%），同时保持对明确违规的完美检测。

Conclusion: 领域专业化和协调推理能够显著提高法律AI性能，为可信赖和可解释的自动化合规验证提供了可扩展且法规感知的框架，尽管在模糊场景中仍存在挑战。
...
### [92] [Requirements for Aligned, Dynamic Resolution of Conflicts in Operational Constraints](https://arxiv.org/abs/2511.10952)
*Steven J. Jones,Robert E. Wray,John E. Laird*

Main category: cs.AI

TL;DR: 论文探讨了自主AI系统在遇到无法完全满足所有操作约束的场景时，需要超越训练策略来构建、评估和证明候选行动方案，并分析了支持此类决策所需的知识类型。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统在部署后必然会遇到训练中未覆盖的复杂场景，需要能够处理不完全满足所有约束条件的情况，以符合人类期望和价值观。

Method: 通过理论分析和实证案例研究，考察了智能体如何整合规范性、实用性和情境性理解来选择更符合期望的行动方案。

Result: 识别了智能体在复杂现实环境中做出稳健且符合人类期望决策所需的知识类型，包括规范性知识、实用知识和情境理解。

Conclusion: 为了实现与人类期望和价值观一致的目标，自主AI系统需要超越训练策略，具备构建、评估和证明候选行动方案的能力，这需要整合多种类型的知识。
...
### [93] [Faster Symmetry Breaking Constraints for Abstract Structures](https://arxiv.org/abs/2511.11029)
*Özgür Akgün,Mun See Chang,Ian P. Gent,Christopher Jefferson*

Main category: cs.AI

TL;DR: 本文提出了一种新的不完全方法来打破抽象结构的对称性，通过更好地利用其表示来处理约束编程中常见的对称性问题。


<details>
  <summary>Details</summary>
Motivation: 在约束编程中，使用高级建模语言（如Essence）表达问题时涉及抽象结构，这些结构需要转换为求解器支持的表示。对称性破坏是加速求解的重要技术，但传统方法应用于抽象变量时会产生大量复杂约束，性能较差。

Method: 提出了一种新的不完全对称性破坏方法，通过更好地利用抽象结构的表示来处理对称性。该方法特别针对不可区分对象产生的对称性进行优化。

Result: 实验表明，该方法比之前（Akgün et al. 2025）提出的方法更快。

Conclusion: 新方法通过更有效地利用抽象结构的表示，在对称性破坏方面取得了更好的性能，为约束编程中的对称性处理提供了改进方案。
...
### [94] [Key Decision-Makers in Multi-Agent Debates: Who Holds the Power?](https://arxiv.org/abs/2511.11040)
*Qian Zhang,Yan Zheng,Jinyi Liu,Hebin Liang,Lanjun Wang*

Main category: cs.AI

TL;DR: 研究发现角色分配策略对多智能体辩论性能有显著影响，提出'真理最后'策略可提升22%性能，并开发了MADC策略来应对实际应用中真理未知的问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论在提升推理能力方面有潜力，但角色分配策略这一关键方面尚未充分探索，特别是在实际应用中真理未知的情况下。

Method: 提出'真理最后'角色分配策略，并开发多智能体辩论一致性(MADC)策略，通过路径一致性评估独立角色间的一致性，模拟一致性得分最高的角色作为真理。

Result: 在9个LLM模型上的验证显示，MADC策略持续表现出先进性能，有效克服了MAD的性能瓶颈，在推理任务中性能提升高达22%。
...
### [95] [Autonomous Vehicle Path Planning by Searching With Differentiable Simulation](https://arxiv.org/abs/2511.11043)
...
Conclusion: DSS框架通过结合规划梯度和随机搜索，为自动驾驶规划提供了有效解决方案，能够显著提升性能。
...
TL;DR: 本文提出了一种新的广义规划方法，通过从训练问题中学习条件-动作规则来构建广义规划程序，这些程序可以直接执行或用于剪枝规划搜索空间。


<details>
  <summary>Details</summary>
Motivation: 广义规划旨在合成能够解决相关规划问题族的程序，传统方法存在效率问题，需要更简单有效的方法来学习广义规划。
...
TL;DR: GGBench是一个专门评估几何生成推理的基准测试，旨在填补现有评估方法在衡量统一多模态模型集成认知过程方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估判别性理解或无约束图像生成，无法衡量生成推理的集成认知过程。几何构造需要语言理解和精确视觉生成的融合，为评估提供了理想测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 解决多智能体辩论（MAD）方法中依赖所有智能体都理性且反思的不现实假设问题，当智能体本身容易产生幻觉时，MAD方法效果受限。

Method: 引入多智能体卧底游戏协议，通过修改参考图像引入反事实证据，观察智能体是否能准确识别这些变化，从而检测幻觉智能体，实现稳健的多模态推理。

Result: MUG在三个关键维度上改进了MAD协议：实现基于反事实测试的事实验证、引入动态修改证据源的跨证据推理、促进主动推理而非被动回答问题。

Conclusion: MUG为LLM的多模态推理提供了一个更可靠和有效的框架，通过反事实测试和主动推理机制显著提升了推理的可靠性。
...
TL;DR: STaR框架通过慢思考机制增强LLMs在表格推理中的认知能力，使用两阶段难度感知强化学习和轨迹级不确定性量化，显著提升推理性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在表格推理中存在两个关键问题：推理过程缺乏人类认知的深度和迭代优化特性，以及推理过程不稳定影响下游应用可靠性。

Method: 提出STaR框架，通过显式建模逐步思考和不确定性感知推理来增强LLMs的慢思考能力。训练阶段使用两阶段难度感知强化学习，从简单到复杂查询逐步学习；推理阶段通过整合token级置信度和答案一致性进行轨迹级不确定性量化。

Result: 在多个基准测试上的广泛实验表明，STaR实现了卓越的性能和增强的推理稳定性，在领域外数据集上也表现出强大的泛化能力。

Conclusion: STaR作为一个可靠且受认知启发的解决方案，在LLMs表格推理中展现出巨大潜力。
...
Method: 开发了基于大语言模型的AIonopedia智能体，采用LLM增强的多模态领域基础模型，结合分层搜索架构进行分子筛选和设计。

Result: 在新构建的综合离子液体数据集上训练和评估，模型表现出优越性能；在文献报告系统上的评估显示智能体能有效进行离子液体修饰；真实湿实验验证证实了其在实际应用中的有效性。

Conclusion: AIonopedia能够加速真实世界离子液体的发现，在具有挑战性的分布外任务中展现出卓越的泛化能力。
...
TL;DR: 本文提出了一种通过强制记录AI决策过程中每个组件的文档化方法，使用机密计算技术实现防篡改、可验证的AI决策追踪工作流。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在决策过程文档化方面存在不足，这阻碍了责任链的追溯，特别是在AI决策违反法律时无法在法庭上提供有效证据。

Method: 扩展DBOM概念，利用机密计算技术构建运行工作流，强制记录训练和推理过程中的每个组件，生成防篡改、可验证的完整决策痕迹。

Result: 开发了一个区分有毒和可食用蘑菇的应用程序作为示例，展示了该工作流的实际运行机制。

Conclusion: 该方法为AI决策提供了可靠的追溯能力，是解决AI系统责任链问题的实用方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法只能单独解释为什么C(a)被知识库蕴含或为什么C(b)不被蕴含，缺乏同时考虑两者的对比视角，无法有效回答"为什么a是C的实例而b不是"这类对比性问题。

Method: 为描述逻辑本体论中的ABox推理开发了对比解释的适当概念，分析了不同变体在不同最优性标准下的计算复杂性，涵盖了轻量级和更表达性的描述逻辑。实现了计算对比解释变体的方法，并在现实知识库的生成问题上进行了评估。

Result: 开发了对比ABox解释的形式化框架，分析了不同描述逻辑下对比解释的计算复杂性，并实现了可运行的对比解释计算方法。

Conclusion: 对比解释能够同时考虑正蕴含和缺失蕴含，聚焦于实例间的相关共性和差异，为"为什么a是C的实例而b不是"这类问题提供了更全面的解释框架。
...
TL;DR: EcoAlign是一个推理时框架，将大型视觉语言模型的对齐问题重新定义为经济理性的搜索过程，通过前瞻性评估函数动态权衡安全性、效用和成本，在降低计算成本的同时实现更好的安全性和实用性。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型存在复杂的越狱漏洞，现有对齐方法在安全性、实用性和运营成本之间存在权衡困难。仅关注最终输出的过程盲目性会浪费大量计算预算在不安全的推理上，允许有害推理通过良性论证伪装。

Method: 将LVLM视为有限理性智能体，逐步扩展思维图，使用前瞻性函数（类似于净现值）对行动进行评分，动态权衡预期安全性、效用和成本与剩余预算的关系。通过最弱环节原则强制执行路径安全性。

Result: 在3个闭源和2个开源模型的6个数据集上进行广泛实验，显示EcoAlign在较低计算成本下匹配或超越了最先进的安全性和实用性。

Conclusion: EcoAlign为稳健的LVLM对齐提供了一条原则性、经济性的路径，解决了安全性、效用和成本之间的权衡问题。
...
TL;DR: RLSLM是一个混合强化学习框架，将基于规则的社交运动模型集成到强化学习的奖励函数中，实现社会对齐的导航策略，在用户体验上优于现有基于规则的模型。


<details>
  <summary>Details</summary>
Motivation: 在人类密集环境中导航而不引起不适是社会感知智能体的关键能力。基于规则的方法虽然可解释但缺乏泛化性，数据驱动方法虽然能学习复杂行为但效率低且不透明。需要结合两者的优势。

Method: 提出RLSLM混合强化学习框架，将基于经验行为实验的社交运动模型集成到强化学习奖励函数中，生成方向敏感的社交舒适场，共同优化机械能量和社交舒适度。

Result: 通过沉浸式VR人机交互实验证明，RLSLM在用户体验上优于最先进的基于规则模型。消融和敏感性分析显示模型相比传统数据驱动方法显著提高了可解释性。

Conclusion: 这项工作提出了一个可扩展的、以人为中心的方法论，有效整合了认知科学和机器学习，用于现实世界的社交导航。
...
TL;DR: MarsRL是一个新颖的强化学习框架，通过智能体管道并行化联合优化多智能体推理系统中的所有智能体，解决了开源模型在多智能体推理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型受限于输出长度，多智能体推理系统通过迭代优化解决方案提供有前景的替代方案，但开源模型由于批评和修正能力不足而难以推广。

Method: 提出MarsRL框架，采用智能体管道并行化，引入智能体特定奖励机制以减少奖励噪声，并使用管道式训练提高长轨迹处理效率。

Result: 在Qwen3-30B-A3B-Thinking-2507上应用MarsRL，AIME2025准确率从86.5%提升至93.3%，BeyondAIME从64.9%提升至73.8%，甚至超过Qwen3-235B-A22B-Thinking-2507。

Conclusion: MarsRL有潜力推进多智能体推理系统的发展，并扩大其在多样化推理任务中的适用性。
...
TL;DR: 本文系统综述了多智能体强化学习（MARL）在现实通信约束下的鲁棒高效通信策略，包括消息扰动、传输延迟和带宽限制等挑战，并聚焦于协作自动驾驶、分布式SLAM和联邦学习三个应用场景。
...
Method: 通过系统综述方法，分析MARL在消息扰动、传输延迟和有限带宽等现实通信约束下的最新进展，重点关注协作自动驾驶、分布式SLAM和联邦学习三个应用场景。

Result: 识别了MARL在实际部署中的关键挑战，包括低延迟可靠性、带宽密集型数据共享和通信隐私权衡等问题。

Conclusion: 提出了统一方法的未来研究方向，倡导共同设计通信、学习和鲁棒性，以弥合理论MARL模型与实际实现之间的差距。
...
TL;DR: CURENet是一个多模态模型，整合了非结构化临床笔记、实验室测试和患者时间序列数据，利用LLM处理临床文本和文本实验室测试，使用transformer编码器处理纵向序列访问，在慢性疾病预测中达到94%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录包含多种数据类型，但大多数预测模型未能充分捕捉多模态数据之间的交互、冗余和时间模式，往往只关注单一数据类型或忽略这些复杂性。

Method: 提出CURENet模型，结合大型语言模型处理临床文本和文本实验室测试，使用transformer编码器处理纵向序列访问数据，整合多种临床数据形式。
...
### [108] [Experience-Guided Adaptation of Inference-Time Reasoning Strategies](https://arxiv.org/abs/2511.11519)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有AI系统在训练后无法灵活调整问题解决方法的局限性，现有系统要么只能修改文本输入，要么需要离线优化且部署后无法改变。

Method: 使用基于LLM的元策略，包含两个组件：Guide根据当前问题和结构化经验记忆生成候选策略，Consolidator整合执行反馈来改进策略生成。

Result: 在五个挑战性基准测试中，EGuR比最强基线准确率提升高达14%，计算成本降低111倍，且随着经验积累性能持续提升。

Conclusion: EGuR通过动态策略生成实现了推理时的灵活自适应，显著提升了AI系统的性能和效率。
...
TL;DR: 本文提出LT-Soups方法，通过两阶段模型融合框架解决长尾分布数据中参数高效微调方法在头尾类性能上的权衡问题。
...
Result: 在六个基准数据集上的实验表明，LT-Soups在广泛的失衡机制下相比PEFT和传统模型融合方法实现了更优的权衡。

Conclusion: LT-Soups能够有效解决长尾分布中头尾类性能的权衡问题，在不同失衡机制下都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统稀疏回归技术在识别有理函数和对噪声敏感方面存在困难，而现有的拉格朗日识别方法受测量噪声和数据可用性的显著影响。

Method: 集成三次B样条近似到拉格朗日系统识别中，开发鲁棒方程发现机制，并基于B样条基函数实现递归导数计算方案。

Result: 该方法在复杂机械系统中表现出优越性能，相比基线方法能够更准确可靠地从噪声数据中提取物理定律。

Conclusion: 所提出的框架为从噪声数据中发现物理定律提供了更准确和可靠的方法，特别是在复杂机械系统中。
...
<details>
  <summary>Details</summary>
Motivation: 现有的表示微调(ReFT)方法在数学推理任务上表现不佳，主要原因是难以生成有效的推理前缀以及干扰数值编码导致错误累积。
...
Conclusion: BREP ReFT成功解决了ReFT在数学推理任务上的性能瓶颈，为参数高效微调在复杂推理任务上的应用提供了有效解决方案。
...
TL;DR: 本文提出了生成模型学习中不确定性量化的问题，讨论了基于集成方法的精度-召回曲线等研究方向，并在合成数据集上验证了聚合精度-召回曲线在捕捉模型近似不确定性方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 生成模型在各领域应用日益广泛，但其可靠性存在根本性担忧。当前评估方法主要关注学习分布与目标分布的接近程度，而忽略了这些测量中固有的不确定性。

Method: 形式化生成模型学习中的不确定性量化问题，提出使用基于集成方法的精度-召回曲线来量化模型近似的不确定性。

Result: 在合成数据集上的初步实验表明，聚合精度-召回曲线能有效捕捉模型近似不确定性，使基于不确定性特征对不同模型架构进行系统比较成为可能。

Conclusion: 不确定性量化是生成模型评估中被忽视但至关重要的方面，基于集成方法的精度-召回曲线为系统评估和比较不同模型架构的不确定性特征提供了有效途径。
...
### [113] [Near-optimal Linear Predictive Clustering in Non-separable Spaces via Mixed Integer Programming and Quadratic Pseudo-Boolean Reductions](https://arxiv.org/abs/2511.10809)
*Jiazhou Liang,Hassan Khurram,Scott Sanner*

Main category: cs.LG

TL;DR: 本文提出了两种改进线性预测聚类（LPC）全局优化的新方法，通过利用可分离性的理论特性推导出具有可证明误差界的近似最优解，显著降低了MIP公式的复杂性并提高了可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统的贪婪优化方法缺乏全局最优性，在非可分聚类设置中表现不佳；而现有的MIP方法虽然能保证全局最优性但可扩展性差。本文旨在改进LPC的全局优化效率。

Method: 1. 基于可分离性理论特性推导近似最优解；2. 将LPC近似为二次伪布尔优化（QPBO）问题；3. 显著降低MIP公式复杂性。

Result: 在合成和真实数据集上的比较分析表明，该方法始终获得近似最优解，回归误差显著低于贪婪优化，同时比现有MIP公式具有更好的可扩展性。

Conclusion: 提出的方法在保持近似最优性的同时，显著提高了LPC全局优化的计算效率和可扩展性。
...
### [114] [Transformers know more than they can tell -- Learning the Collatz sequence](https://arxiv.org/abs/2511.10811)
*François Charton,Ashvni Narayanan*

Main category: cs.LG

TL;DR: 本文研究了Transformer模型预测长Collatz步数的能力，发现模型准确率随编码基数变化，最高可达99.7%，最低仅25%。所有模型都学习到相同的模式：按输入模2^p的余数分类学习，准确率接近完美，反映了Collatz序列的数学特性。
...
Method: 使用不同基数编码输入和输出，训练Transformer模型预测长Collatz步数，分析模型的学习模式和错误类型。

Result: 模型准确率随基数变化显著（99.7%到25%），但都学习到相同的模式：按模2^p余数分类学习。90%以上的错误是由于正确计算但错误估计循环长度，几乎不发生幻觉。

Conclusion: 学习复杂算术函数的困难在于理解计算的控制结构（循环长度）。使用数学问题分析语言模型的方法可广泛应用于其他问题并取得成果。
...
### [115] [Towards Universal Neural Operators through Multiphysics Pretraining](https://arxiv.org/abs/2511.10829)
...
<details>
  <summary>Details</summary>
Motivation: 尽管神经算子广泛应用于数据驱动的物理模拟，但其训练成本高昂。现有研究通过下游学习（在简单问题上预训练，在复杂问题上微调）来解决这个问题，但Transformer架构的神经算子在此领域的应用仍有限。

Method: 在通用迁移学习设置下评估基于Transformer的神经算子，测试场景包括：未见参数的泛化、新变量的引入、从多方程数据集的迁移。

Result: 研究结果表明，先进的神经算子架构能够有效地在PDE问题间迁移知识。

Conclusion: 基于Transformer的神经算子在PDE问题的迁移学习中表现出良好的性能，能够有效实现知识迁移。
...
### [116] [Benchmarking Quantum Kernels Across Diverse and Complex Data](https://arxiv.org/abs/2511.10831)
*Yuhan Jiang,Matthew Otten*

Main category: cs.LG

TL;DR: 本文提出了一个变分量子核框架，在8个高维真实世界数据集上的基准测试显示，量子核在性能上优于经典RBF核，证明了量子核作为高性能工具的潜力。
...
Method: 开发了使用资源高效ansätze的变分量子核框架，并引入了参数缩放技术来加速收敛，在8个具有挑战性的真实世界高维数据集上进行了全面基准测试。
...
### [117] [SURFACEBENCH: Can Self-Evolving LLMs Find the Equations of 3D Scientific Surfaces?](https://arxiv.org/abs/2511.10833)
*Sanchit Kabra,Shobhnik Kriplani,Parshin Shojaee,Chandan K. Reddy*

Main category: cs.LG

TL;DR: SurfaceBench是首个用于符号曲面发现的综合基准测试，包含183个任务，涵盖15种符号复杂度类别，涉及显式、隐式和参数方程表示形式。该基准通过新颖的符号组合抵抗LLM记忆，并采用几何感知指标评估方程发现质量。


<details>
  <summary>Details</summary>
Motivation: 现有的符号回归基准存在局限性：主要关注标量函数、忽略领域基础、依赖脆弱的字符串匹配指标，无法捕捉科学等价性。需要建立一个更全面、具有挑战性的测试平台来评估符号推理与几何重建的结合能力。

Method: 构建包含183个任务的SurfaceBench基准，涵盖15种符号复杂度类别，采用显式、隐式和参数方程三种表示形式。每个任务包含真实方程、变量语义和合成的三维数据。评估时结合符号检查和几何感知指标（如Chamfer和Hausdorff距离）。

Result: 实验表明，最先进的框架虽然在特定函数族上偶尔成功，但在跨表示类型和曲面复杂度方面难以泛化。SurfaceBench为组合泛化、数据驱动的科学归纳和几何感知推理提供了具有挑战性的诊断测试平台。

Conclusion: SurfaceBench建立了一个连接符号推理与几何重建的挑战性测试平台，能够对组合泛化、数据驱动的科学归纳和几何感知推理的进展进行原则性基准测试。
...
TL;DR: 本文提出了一个区分外在和内在幻觉的评估框架，并利用基于注意力的不确定性量化算法来改进幻觉检测性能。
...
Method: 使用基于注意力的不确定性量化算法，提出新颖的注意力聚合策略来改进可解释性和检测性能。

Result: 基于采样的方法如语义熵能有效检测外在幻觉但通常无法检测内在幻觉，而基于输入令牌注意力聚合的方法更适合内在幻觉。
...
TL;DR: FlowPath提出了一种通过可逆神经流学习控制路径几何形状的新方法，用于处理稀疏和不规则采样的时间序列建模问题，相比固定插值方法在分类准确率上取得显著提升。
...
Result: 在18个基准数据集和真实案例研究中，FlowPath相比使用固定插值或不可逆架构的基线方法，在分类准确率上持续取得统计显著的改进。

Conclusion: 研究强调了不仅需要建模路径上的动态，还需要建模路径本身的几何形状，为从不规则时间序列中学习提供了稳健且可泛化的解决方案。
...
TL;DR: 本文提出通过精心设计的行为策略收集离策略数据，可以显著降低回报估计的方差，从而提高强化学习算法的样本效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习算法依赖回报估计进行策略改进，但高方差的回报估计会导致样本效率低下和训练不稳定。最近离策略评估研究表明，设计良好的行为策略可以收集方差更低的离策略数据。

Method: 将离策略评估的关键见解扩展到在线强化学习设置，在单一行为策略下收集数据用于策略改进，利用重要性加权样本进行去偏和方差管理，并在两个策略梯度方法中实现这种机制。

Result: 实验表明，该方法在多样化环境中展现出更好的样本效率和性能表现。

Conclusion: 收集离策略数据可以获得方差更低的回报估计，这挑战了传统认为在线策略数据收集是最优的观念，为强化学习算法提供了新的改进方向。
...
TL;DR: 本文提出了STAMP适配器，将通用时间序列基础模型应用于EEG数据分析，在8个基准数据集上达到与专用EEG基础模型相当的性能。
...
Result: STAMP在8个临床EEG分类数据集上表现优异，性能与最先进的EEG专用基础模型相当，且参数轻量、输入灵活。

Conclusion: 通用TSFM通过STAMP适配器可以有效处理EEG数据，为EEG建模提供了灵活轻量的解决方案。
...
TL;DR: ExPairT-LLM是一种基于LLM的精确代码选择算法，通过成对成员资格和成对等价性查询进行锦标赛式程序选择，在四个流行代码数据集上平均提升13.0%的pass@1性能。


<details>
  <summary>Details</summary>
Motivation: 现有代码选择算法存在两个问题：可能错误识别非等价程序，以及过度依赖LLM假设其总能正确确定每个输入输出。需要一种更鲁棒的代码选择方法。

Method: 提出ExPairT-LLM算法，使用两种新的查询类型：成对成员资格查询和成对等价性查询，通过锦标赛机制选择正确程序，对LLM错误具有一定鲁棒性。

Result: 在四个流行代码数据集上，ExPairT-LLM的pass@1（成功率）平均比最先进的代码选择算法高出13.0%，最高提升27.1%，同时将执行复杂推理的LLM的pass@1提升了24.0%。

Conclusion: ExPairT-LLM通过简化的查询类型和锦标赛机制，有效提升了代码选择的准确性和鲁棒性，显著优于现有方法。
...
TL;DR: 提出了PAZO框架，利用公开数据指导私有零阶优化算法，在保持隐私的同时提高效用，在高度私有化场景中优于一阶基线方法，并提供16倍运行速度提升。


<details>
  <summary>Details</summary>
Motivation: 部署一阶差分隐私机器学习算法（如DP-SGD）存在高计算和内存成本问题，零阶方法虽然更容易实现隐私保护，但效用相对较低且应用领域有限。

Method: 利用公开信息来指导和改进私有零阶算法的梯度近似，探索了一套具有最小开销的公开数据辅助零阶优化器（PAZO）。

Result: 在视觉和文本任务的预训练和微调设置中，PAZO实现了优越的隐私/效用权衡，在高度私有化场景中优于最佳的一阶基线方法，并提供高达16倍的运行速度提升。

Conclusion: PAZO框架通过利用公开数据有效提升了私有零阶优化算法的性能，在保持隐私保护的同时显著提高了模型效用和运行效率。
...
TL;DR: GO UT Bench是一个包含5264对代码和单元测试的基准数据集，用于解决代码LLM在训练数据不平衡问题，特别是在Golang等低资源语言中。通过微调，模型在超过75%的基准任务上表现优于基础模型。


<details>
  <summary>Details</summary>
Motivation: 解决代码LLM训练数据不平衡问题，特别是Golang等低资源语言中开源代码过度表示而软件工程任务（如单元测试生成）表示不足的问题。

Method: 引入GO UT Bench基准数据集，从10个许可宽松的Golang仓库中收集5264对代码和单元测试，涵盖多个领域。在两个LLM家族（专家混合和密集解码器）上进行微调评估。
...
### [125] [Multi-Joint Physics-Informed Deep Learning Framework for Time-Efficient Inverse Dynamics](https://arxiv.org/abs/2511.10878)
*Shuhao Ma,Zeyi Huang,Yu Cao,Wesley Doorsamy,Chaoyang Shi,Jun Li,Zhi-Qiang Zhang*

Main category: cs.LG

TL;DR: 提出了一种物理信息深度学习框架PI-MJCA-BiGRU，用于从运动学数据直接估计肌肉激活和力量，无需标记数据即可实现与传统监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在多关节系统中计算成本高且缺乏高质量标记数据集的问题，实现高效、准确的肌肉激活和力量估计。

Method: 使用新颖的多关节交叉注意力模块和双向门控循环单元层来捕捉关节间协调，通过嵌入多关节动力学、关节间耦合和外部力交互到损失函数中实现物理一致性预测。

Result: 在两个数据集上的实验验证表明，该方法在不需要真实标签的情况下达到与传统监督方法相当的性能，MJCA模块显著提升了关节间协调建模能力。

Conclusion: PI-MJCA-BiGRU框架能够提供生理一致的预测，实现时间高效的推理，为临床评估和辅助设备控制提供了有效的解决方案。
...
### [126] [Towards Federated Clustering: A Client-wise Private Graph Aggregation Framework](https://arxiv.org/abs/2511.10915)
*Guanxiong He,Jie Wang,Liaoyuan Tang,Zheng Wang,Rong Wang,Feiping Nie*

Main category: cs.LG

TL;DR: 提出SPP-FGC框架，通过本地结构图进行隐私保护的联邦聚类，解决传统方法在性能与隐私之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 联邦聚类面临性能与隐私的妥协困境：传输嵌入表示会泄露敏感数据，而仅共享聚类原型会降低模型准确性。

Method: SPP-FGC使用本地结构图作为隐私保护知识共享媒介，采用客户端-服务器架构。SPP-FGC为单次通信方法，SPP-FGC+为迭代方法用于复杂数据。

Result: 实验表明该框架达到最先进性能，相比联邦基线方法将聚类准确率提升高达10%（NMI），同时保持可证明的隐私保证。

Conclusion: SPP-FGC框架成功解决了联邦聚类中性能与隐私的权衡问题，通过结构图实现高效且隐私保护的聚类分析。
...
### [127] [Cascading Bandits With Feedback](https://arxiv.org/abs/2511.10938)
*R Sri Prakash,Nikhil Karamchandani,Sharayu Moharir*

Main category: cs.LG

TL;DR: 本文研究了边缘推理中的级联赌博机模型，分析了四种决策策略的理论遗憾保证。与经典赌博机设置不同，探索后提交和行动消除策略由于在探索阶段后固定排序而具有次优遗憾，而LCB和Thompson采样通过持续更新决策实现了常数O(1)遗憾。
...
Method: 分析四种决策策略：探索后提交、行动消除、下置信界(LCB)和Thompson采样，并为每种策略提供尖锐的理论遗憾保证。

Result: 探索后提交和行动消除策略由于在探索阶段后提交固定排序而具有次优遗憾，限制了其适应能力。相比之下，LCB和Thompson采样根据观察到的反馈持续更新决策，实现了常数O(1)遗憾。仿真验证了这些理论发现。

Conclusion: 在不确定性下的高效边缘推理中，适应性起着关键作用，LCB和Thompson采样等持续适应策略优于固定排序策略。
...
### [128] [From Parameter to Representation: A Closed-Form Approach for Controllable Model Merging](https://arxiv.org/abs/2511.10943)
...
<details>
  <summary>Details</summary>
Motivation: 传统模型合并方法面临参数干扰问题，现有可控合并方法采用编译-查询范式，离线优化成本高昂且复杂度随任务数量指数增长。

Method: 将视角从参数空间优化转向直接修正模型最终表示，将该修正建模为最优线性变换，获得闭式解，用单步架构无关计算替代整个离线优化过程。

Result: 实验结果表明该方法生成更优的Pareto前沿，具有更精确的偏好对齐和显著降低的计算成本。

Conclusion: 该方法通过直接表示修正实现了高效可控的模型合并，复杂度线性增长，支持实时生成Pareto最优模型。
...
### [129] [How Data Quality Affects Machine Learning Models for Credit Risk Assessment](https://arxiv.org/abs/2511.10964)
*Andrea Maurino*
...
Method: 使用开源数据集，通过Pucktrick库引入受控数据损坏，评估包括随机森林、SVM和逻辑回归在内的10种常用机器学习模型的鲁棒性。

Result: 实验表明，不同模型对数据退化的鲁棒性存在显著差异，具体取决于数据质量问题的性质和严重程度。
...
### [130] [Unsupervised Robust Domain Adaptation: Paradigm, Theory and Algorithm](https://arxiv.org/abs/2511.11009)
*Fuxiang Huang,Xiaowei Fu,Shiyu Ye,Lina Ma,Wen Li,Xinbo Gao,David Zhang,Lei Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种无监督鲁棒域自适应（URDA）范式，解决了传统UDA方法忽视对抗攻击鲁棒性的问题，并提出了DART算法来实现鲁棒性训练。


<details>
  <summary>Details</summary>
Motivation: 大多数无监督域自适应方法强调迁移能力但忽视对抗攻击鲁棒性，而传统的对抗训练在UDA中效果不佳。本文旨在解决UDA+VAT范式中的内在纠缠问题。

Method: 提出了URDA范式和DART算法，采用两步训练过程：首先预训练任意UDA模型，然后通过解纠缠蒸馏进行瞬时鲁棒化后训练步骤。

Result: 在四个基准数据集上的实验表明，DART能有效增强鲁棒性同时保持域适应性，验证了URDA范式和理论。

Conclusion: 本文首次建立了URDA范式和理论，提出的DART算法简单有效，能同时确保迁移性和鲁棒性。
...
### [131] [Improving Continual Learning of Knowledge Graph Embeddings via Informed Initialization](https://arxiv.org/abs/2511.11118)
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱频繁更新需要其嵌入模型持续适应变化，而新实体嵌入初始化对最终准确性和训练时间有重要影响，特别是对于小型频繁更新。

Method: 利用知识图谱模式和先前学习的嵌入，基于实体所属类别为新实体获得初始表示，可无缝集成到现有的KGE持续学习方法中。

Result: 实验分析显示该初始化策略提高了预测性能，增强了知识保留，加速了知识获取，减少了训练轮次和时间，并在多种KGE学习模型中均有效。

Conclusion: 提出的初始化策略能有效提升知识图谱嵌入在持续学习中的表现，同时减少灾难性遗忘并加速训练过程。
...
### [132] [Anomaly Detection in High-Dimensional Bank Account Balances via Robust Methods](https://arxiv.org/abs/2511.11143)
*Federico Maddanu,Tommaso Proietti,Riccardo Crupi*

Main category: cs.LG

TL;DR: 本文提出并评估了几种在中等和高维数据集中计算高效的稳健方法，用于检测银行账户余额中的点异常，应用于260万条匿名用户银行账户余额的每日记录。
...
Method: 提出并实证评估了几种具有高崩溃点和低计算时间的稳健方法，这些方法在中等和高维数据集中可能计算高效。
...
### [133] [Deep Learning for Short-Term Precipitation Prediction in Four Major Indian Cities: A ConvLSTM Approach with Explainable AI](https://arxiv.org/abs/2511.11152)
*Tanmay Ghosh,Shaurabh Anand,Rakesh Gomaji Nannewar,Nithin Nagaraj*

Main category: cs.LG

TL;DR: 开发了一个可解释的深度学习框架用于印度四个主要城市的短期降水预测，结合CNN-ConvLSTM架构和多种可解释性分析方法，实现了准确预测并揭示了模型决策模式。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习降水预测模型作为黑箱的问题，在保持准确性的同时增强透明度，促进在实际天气预报中的采用。

Method: 采用混合时间分布式CNN-ConvLSTM架构，基于多年代ERA5再分析数据训练，为每个城市优化卷积滤波器数量，并使用排列重要性、Grad-CAM、时间遮挡和反事实扰动进行可解释性分析。

Result: 模型在四个城市达到不同的RMSE值：班加罗尔0.21毫米/天、孟买0.52毫米/天、德里0.48毫米/天、加尔各答1.80毫米/天，预测时间范围从1天到5天不等。

Conclusion: 研究表明可解释AI能够提供准确的降水预测，并为不同城市环境中的降水模式提供透明洞察，展示了xAI在天气预报中的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 改进对气候极端事件（特别是热浪）的预测能力，解决现有方法在预测极端温度事件方面的不足。

Method: 将问题构建为分类任务，预测地表气温是否会在指定时间内超过局部q分位数；通过使机器学习天气预报模型具有生成性，并应用幂均值非线性聚合方法来整合集成预测。

Result: 幂均值聚合方法显著提升了分类器性能，在预测极端热事件方面比相同模型的典型均值预测获得更好的准确率；该方法的最优性能随所选分位数阈值而变化，对更高极端值的预测效果更好。

Conclusion: 幂均值聚合方法在改进极端热事件预测方面表现出良好的潜力和适应性，特别适用于更高极端值的预测场景。
...
TL;DR: 本文提出了一种名为Sindy Kalman Filter (SKF)的新方法，将稀疏识别非线性动力学(Sindy)算法与卡尔曼滤波结合，实现了复杂时变非线性系统的实时建模。


<details>
  <summary>Details</summary>
Motivation: 从数据中学习控制方程对于理解物理系统行为至关重要，但现有方法难以实现复杂时变非线性系统的实时学习。

Method: 将Sindy算法与卡尔曼滤波统一，将未知系统参数作为状态变量处理，并利用前瞻误差增强参数识别策略。

Result: 在参数漂移或切换的混沌Lorenz系统上验证了SKF的有效性，并在真实飞行数据构建的稀疏非线性飞机模型中实现了实时识别。

Conclusion: SKF方法能够实时推断复杂时变非线性模型，显著简化了稀疏水平、方差参数和切换时刻的估计，超越了单独使用任一方法的能力。
...
TL;DR: 本文提出了一种用于不完整多视图聚类的动态深度图学习方法DGIMVCM，通过构建缺失鲁棒的全局图、图结构对比学习和掩码图重构损失来解决现有方法中的噪声问题和梯度噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多视图数据的普遍性使得不完整多视图聚类成为重要研究方向。现有基于GNN的方法存在两个主要挑战：(1) 使用KNN构建静态图会引入噪声并降低图拓扑的鲁棒性；(2) 使用MSE损失作为图重构损失会在优化过程中产生大量梯度噪声。
...
Conclusion: DGIMVCM通过动态图学习、图结构对比学习和掩码图重构损失，有效解决了不完整多视图聚类中的噪声和梯度噪声问题，取得了优异的性能。
...
TL;DR: 提出LoRaCompass强化学习模型，用于在未知环境中高效定位周期性广播的LoRa标签，通过空间感知特征提取器和策略蒸馏损失函数解决领域偏移和信号波动问题，在80km²测试区域中成功率超过90%。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的搜索方法对领域偏移和信号波动敏感，导致级联决策错误和显著定位误差，需要开发更鲁棒的LoRa标签定位方法。

Method: LoRaCompass结合空间感知特征提取器、策略蒸馏损失函数和受UCB启发的探索函数，从RSSI学习鲁棒空间表示，最大化向标签靠近的概率。

Result: 在80km²多样化未见环境中验证，定位成功率>90%（比现有方法提升40%），搜索路径长度与初始距离呈线性关系，效率高。

Conclusion: LoRaCompass能够实现鲁棒高效的LoRa标签搜索，在领域偏移和信号波动下仍能保持高定位精度和效率。
...
TL;DR: 提出了一种零样本合成验证框架，利用生成式AI监控联邦学习模型性能并确定早期停止点，显著减少训练轮次同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习方法通常运行预定义的全局轮次，导致在达到最佳性能后仍进行不必要的计算，或在模型无法获得有意义的性能时继续训练，造成效率低下。

Method: 引入零样本合成验证框架，利用生成式AI监控模型性能并自适应地确定早期停止点，在接近最优轮次时停止训练。

Result: 在多标签胸部X光分类任务中，该方法将训练轮次减少高达74%，同时保持准确率在最优值的1%以内。

Conclusion: 该方法有效解决了联邦学习中的计算效率问题，能够显著节省计算资源并支持快速超参数调整。
...
### [139] [A Best-of-Both-Worlds Proof for Tsallis-INF without Fenchel Conjugates](https://arxiv.org/abs/2511.11211)
*Wei-Cheng Lee,Francesco Orabona*

Main category: cs.LG

TL;DR: 本文提供了一个对Tsallis-INF多臂老虎机算法最佳性能保证的简化推导，避免了共轭函数的使用，并采用在线凸优化工具来简化证明。


<details>
  <summary>Details</summary>
Motivation: 动机是简化Tsallis-INF算法在随机和对抗性老虎机环境中的最佳性能保证的证明过程，使其更加简洁易懂。

Method: 方法包括使用现代在线凸优化工具，避免使用共轭函数，并且不优化边界常数以保持证明的简洁性。

Result: 结果是对Tsallis-INF算法的最佳性能保证提供了一个更简单、更直接的推导。

Conclusion: 结论是成功简化了Tsallis-INF算法的性能保证证明，使其更加易于理解和应用。
...
### [140] [HealSplit: Towards Self-Healing through Adversarial Distillation in Split Federated Learning](https://arxiv.org/abs/2511.11240)
*Yuhan Xie,Chen Lyu*
...
<details>
  <summary>Details</summary>
Motivation: Split Federated Learning在隐私保护分布式学习中具有重要价值，但容易受到针对本地特征、标签、粉碎数据和模型权重的复杂数据投毒攻击。现有防御方法主要从传统联邦学习改编而来，在SFL中效果有限。

Method: HealSplit包含三个关键组件：1）基于拓扑的检测模块，通过拓扑异常评分识别中毒样本；2）生成式恢复管道，合成语义一致的替代样本；3）对抗性多教师蒸馏框架，使用语义监督和异常感知信号训练学生模型。

Result: 在四个基准数据集上的广泛实验表明，HealSplit在十种最先进的防御方法中表现最佳，在各种攻击场景下实现了卓越的鲁棒性和防御效果。

Conclusion: HealSplit为Split Federated Learning提供了一个有效的统一防御解决方案，显著提升了系统对复杂投毒攻击的抵抗能力。
...
### [141] [Toward Scalable Early Cancer Detection: Evaluating EHR-Based Predictive Models Against Traditional Screening Criteria](https://arxiv.org/abs/2511.11293)
*Jiheum Park,Chao Pang,Tristan Y. Lee,Jeong Yun Yang,Jacob Berkowitz,Alexander Z. Wei,Nicholas Tatonetti*

Main category: cs.LG

TL;DR: 本研究评估了基于电子健康记录（EHR）的预测模型与传统风险因素在识别癌症高风险人群方面的临床效用。结果显示EHR模型在识别高风险个体中真实癌症病例的富集度比传统风险因素高3-6倍，且EHR基础模型进一步提升了26种癌症类型的预测性能。
...
Method: 使用All of Us研究计划的数据（整合了超过865,000名参与者的EHR、基因组和调查数据），系统评估了EHR预测模型与传统风险因素（包括基因突变和癌症家族史）在八种主要癌症中的临床效用。

Result: 即使使用基线建模方法，EHR模型在识别的高风险个体中真实癌症病例的富集度比单独使用传统风险因素高3-6倍。EHR基础模型进一步提升了26种癌症类型的预测性能。

Conclusion: 基于EHR的预测建模具有支持更精确和可扩展的早期检测策略的临床潜力。
...
### [142] [Toward Multi-Fidelity Machine Learning Force Field for Cathode Materials](https://arxiv.org/abs/2511.11361)
...
### [143] [On-Device Fine-Tuning via Backprop-Free Zeroth-Order Optimization](https://arxiv.org/abs/2511.11362)
*Prabodh Katti,Sangwoo Park,Bipin Rajendran,Osvaldo Simeone*

Main category: cs.LG

TL;DR: 本文研究了在边缘设备上使用内存高效的零阶优化（MeZO）进行微调，相比传统的反向传播（BP）方法，MeZO通过仅使用前向评估来估计梯度，消除了存储中间激活和优化器状态的需求，从而在内存受限的设备上支持更大的模型部署。


<details>
  <summary>Details</summary>
Motivation: 边缘AI系统需要在严格的内存约束下适应不同的智能任务。传统基于反向传播的训练需要存储层激活和优化器状态，这在模型权重必须完全驻留在设备内存中的边缘部署中严重限制了可部署的最大模型尺寸。

Method: 使用内存高效的零阶优化（MeZO）方法，仅通过前向评估来估计梯度，无需存储中间激活或优化器状态。首先提供BP和MeZO训练下可容纳模型尺寸的理论估计，然后进行数值验证。

Result: MeZO在设备内存约束下表现出精度优势，前提是有足够的挂钟时间用于微调。该方法能够显著增大在片上内存中可容纳的模型尺寸，尽管可能需要更长的微调时间。

Conclusion: MeZO是一种有效的边缘设备微调方法，通过消除存储中间状态的需求，在内存受限的环境中支持更大的模型部署，为边缘AI系统的适应性提供了重要解决方案。
...
### [144] [Multicalibration yields better matchings](https://arxiv.org/abs/2511.11413)
...
TL;DR: 本文提出使用多校准方法来处理加权图匹配问题中的预测误差，通过构造多校准预测器来补偿不完美预测器带来的误差，使基于预测权重的匹配决策能与原始预测器上的最佳决策规则竞争。


<details>
  <summary>Details</summary>
Motivation: 在加权图匹配问题中，如果预测器是贝叶斯最优的，那么基于预测权重计算最佳匹配是最优的。但在实践中，预测器通常不完美，需要设计能够补偿预测误差的决策规则。

Method: 提出多校准方法，要求预测器在受保护上下文集合的每个元素上无偏。给定匹配算法类$\mathcal C$和任意边权重预测器$γ$，构造特定的多校准预测器$\hat γ$。

Result: 基于$\hat γ$输出的最佳匹配决策能够与在原始预测器$γ$上应用$\mathcal C$中最佳决策规则的性能竞争。

Conclusion: 多校准方法能有效处理不完美预测器带来的误差，并提供了样本复杂度界限来支持理论结果。
...
### [145] [Differentiation Strategies for Acoustic Inverse Problems: Admittance Estimation and Shape Optimization](https://arxiv.org/abs/2511.11415)
*Nikolas Borrel-Jensen,Josiah Bjorgaard*

Main category: cs.LG

TL;DR: 本文提出了一种实用的可微分编程方法，用于声学逆问题，包括导纳估计和形状优化两个应用。通过JAX-FEM的自动微分实现边界导纳的精确估计，以及结合有限差分和AD的几何形状优化方法。
...
Method: 使用JAX-FEM进行自动微分实现边界导纳估计，结合随机有限差分和PyTorch3D进行网格操作的声学形状优化，将物理驱动的边界优化与几何驱动的内部网格适应分离。

Result: 边界导纳估计达到3位数精度，形状优化在目标频率下实现48.1%的能量减少，相比标准有限差分方法减少了30倍的FEM求解次数。

Conclusion: 现代可微分软件栈能够实现基于物理的逆问题的快速优化工作流原型设计，自动微分用于参数估计，有限差分和AD组合用于几何设计。
...
### [146] [Low-Bit, High-Fidelity: Optimal Transport Quantization for Flow Matching](https://arxiv.org/abs/2511.11418)
*Dara Varam,Diaa A. Abuhani,Imran Zualkernan,Raghad AlDamani,Lujain Khalil*

Main category: cs.LG

TL;DR: 本文研究了Flow Matching生成模型的量化压缩方法，提出基于最优传输的量化方案，在2-3比特精度下仍能保持生成质量，适用于边缘AI应用。


<details>
  <summary>Details</summary>
Motivation: Flow Matching模型虽然具有高效的无模拟训练和确定性采样优势，但其实际部署受到高精度参数需求的挑战，需要有效的量化压缩方法。

Method: 采用基于最优传输的后训练量化方法，最小化量化权重与原始权重之间的2-Wasserstein距离，并与均匀、分段和对数量化方案进行系统比较。

Result: 在五个不同复杂度的基准数据集上的实验表明，基于最优传输的量化在2-3比特精度下仍能保持视觉生成质量和潜在空间稳定性，而其他方法在此精度下失效。

Conclusion: 基于最优传输的量化是一种有原则、有效的Flow Matching生成模型压缩方法，适用于边缘和嵌入式AI应用。
...
### [147] [Retrofit: Continual Learning with Bounded Forgetting for Security Applications](https://arxiv.org/abs/2511.11439)
*Yiling He,Junchi Lei,Hongyu She,Shuo Shao,Xinran Zheng,Yiping Liu,Zhan Qin,Lorenzo Cavallaro*

Main category: cs.LG

TL;DR: RETROFIT是一种无需历史数据的持续学习方法，通过参数级模型合并和知识仲裁机制，在安全分析场景中有效缓解灾难性遗忘问题，同时在恶意软件检测和二进制摘要任务上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在安全分析中因威胁环境演变和数据表示变化导致的性能退化问题，克服现有持续学习方法需要完整重训练或数据回放的局限性，特别是在数据敏感环境中无法使用历史数据的挑战。

Method: 提出RETROFIT方法：1) 将先前训练和新微调的模型作为新旧知识的教师模型进行参数级合并；2) 使用低秩和稀疏更新将参数变化限制在独立子空间；3) 基于模型置信度的知识仲裁机制动态平衡教师贡献。

Result: 在恶意软件检测任务中，保留分数从基线的20.2%提升至38.6%，并在新数据上超过oracle上界；在二进制摘要任务中，BLEU分数达到先前工作中迁移学习方法的两倍，在跨表示泛化方面超越所有基线。

Conclusion: RETROFIT通过无历史数据的持续学习框架，在安全关键场景中实现了有界遗忘和有效知识传递，为安全分析中的模型持续适应提供了可行解决方案。
...
### [148] [DiffPro: Joint Timestep and Layer-Wise Precision Optimization for Efficient Diffusion Inference](https://arxiv.org/abs/2511.11446)
*Farhana Amin,Sabiha Afroz,Kanchon Gharami,Mona Moghadampanah,Dimitrios S. Nikolopoulos*

Main category: cs.LG

TL;DR: DiffPro是一个后训练框架，通过联合优化扩散模型的时间步长和每层精度，在不训练的情况下显著降低推理延迟和内存使用，实现高达6.25倍的模型压缩和2.8倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但由于需要大量去噪步骤和繁重的矩阵运算，推理成本高昂。需要一种无需训练的方法来优化部署效率。

Method: DiffPro结合三个关键技术：流形感知敏感度指标分配权重比特、动态激活量化稳定跨时间步的激活值、基于师生漂移的预算时间步选择器。

Result: 在标准基准测试中，DiffPro实现高达6.25倍模型压缩、减少50%时间步数、2.8倍推理加速，Delta FID ≤ 10。

Conclusion: DiffPro将步数减少和精度规划统一为单一可部署计划，为实时节能扩散推理提供了实用解决方案。
...
### [149] [FairReweighing: Density Estimation-Based Reweighing Framework for Improving Separation in Fair Regression](https://arxiv.org/abs/2511.11459)
*Xiaoyin Xi,Zhe Yu*
...
<details>
  <summary>Details</summary>
Motivation: AI软件在公共部门和工业应用中日益普及，但缺乏透明度引发了关于公平性的担忧。现有研究主要集中在二元分类任务，回归中的公平性相对较少探索。

Method: 采用基于互信息的度量来评估分离违规，并扩展到分类和回归问题。提出基于密度估计的FairReweighing预处理算法，确保学习模型满足分离准则。

Result: 理论上证明FairReweighing算法在数据独立性假设下能保证训练数据的分离。实证表明，在合成和真实数据上，该算法在提高分离性的同时保持高准确率，优于现有最先进的回归公平性解决方案。
...
### [150] [Epistemic Error Decomposition for Multi-step Time Series Forecasting: Rethinking Bias-Variance in Recursive and Direct Strategies](https://arxiv.org/abs/2511.11461)
*Riku Green,Huw Day,Zahraa S. Abdallah,Telmo M. Silva Filho*

Main category: cs.LG

TL;DR: 本文重新审视了多步预测中递归策略和直接策略的传统偏差-方差权衡观点，通过误差分解揭示了非线性模型中递归策略可能同时具有更低偏差和更高方差的原因。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为递归策略具有高偏差低方差，直接策略具有低偏差高方差。本文旨在重新检验这一信念，通过理论分析揭示更复杂的偏差-方差关系。

Method: 将多步预测误差分解为三部分：不可约噪声、结构近似差距和估计方差项。对于线性预测器，证明结构差距为零；对于非线性预测器，分析递归组合如何影响模型表达能力。还推导了递归策略估计方差的雅可比放大因子。

Result: 理论分析表明，递归预测可能同时具有比直接预测更低的偏差和更高的方差。在ETTm1数据集上的多层感知机实验验证了这一发现。

Conclusion: 研究结果为选择递归和直接策略提供了实用指导，建议基于模型非线性和噪声特性而非传统偏差-方差直觉来做出选择。
...
### [151] [MoCap2Radar: A Spatiotemporal Transformer for Synthesizing Micro-Doppler Radar Signatures from Motion Capture](https://arxiv.org/abs/2511.11462)
...
<details>
  <summary>Details</summary>
Motivation: 利用更丰富的MoCap数据来增强稀缺的雷达数据集，为更高级别的应用训练提供支持，同时相比基于物理的方法需要更少的计算资源。

Method: 将MoCap到频谱图的转换建模为窗口化序列到序列任务，使用基于Transformer的模型共同捕捉MoCap标记之间的空间关系和跨帧的时间动态。

Result: 实验表明该方法能够生成视觉和数量上合理的多普勒雷达频谱图，具有良好的泛化能力，并且能够理解人体不同部位之间的空间关系。

Conclusion: 这是一个使用Transformer进行时间序列信号处理的有趣示例，特别适用于边缘计算和物联网雷达，能够有效增强雷达数据集并减少计算需求。
...
### [152] [Quantifying and Improving Adaptivity in Conformal Prediction through Input Transformations](https://arxiv.org/abs/2511.11472)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法存在分箱不平衡问题，导致覆盖率和集合大小估计不准确，需要更可靠的适应性评估指标。

Method: 利用输入变换对示例按难度排序，然后进行均匀质量分箱，在此基础上提出两个新评估指标，并开发了基于组条件共形预测的自适应预测集算法。

Result: 实验表明，新指标与期望的适应性属性相关性更强，新算法在图像分类和医疗任务上优于现有方法。

Conclusion: 提出的分箱方法和评估指标能更准确地评估适应性，基于此开发的自适应预测集算法在多个任务上表现优异。
...
### [153] [Data-efficient U-Net for Segmentation of Carbide Microstructures in SEM Images of Steel Alloys](https://arxiv.org/abs/2511.11485)
*Alinda Ezgi Gerçek,Till Korten,Paul Chekhonin,Maleeha Hassan,Peter Steinbach*

Main category: cs.LG

TL;DR: 提出了一种数据高效的轻量级U-Net分割管道，仅使用10张标注的SEM图像就能准确分割反应堆压力容器钢中的碳化物，Dice系数达到0.98，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 反应堆压力容器钢的微观结构分析对预测机械性能至关重要，但SEM图像中碳化物与基体的灰度值重叠使得简单阈值分割无效，需要更精确的分割方法。

Method: 使用轻量级U-Net模型（3070万参数），仅训练10张标注的扫描电镜图像，构建数据高效的分割管道。

Result: 模型在有限数据下达到Dice-Sørensen系数0.98，显著优于冶金领域最先进方法（传统图像分析：0.85），并将标注工作量减少一个数量级。
...
### [154] [FarSkip-Collective: Unhobbling Blocking Communication in Mixture of Experts Models](https://arxiv.org/abs/2511.11505)
*Yonatan Dukler,Guihong Li,Deval Shah,Vikram Appia,Emad Barsoum*

Main category: cs.LG

TL;DR: FarSkip-Collective通过修改MoE模型架构，跳过连接层来重叠计算与通信，在16B到109B参数的大模型中实现了通信重叠，同时保持与原模型相当的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决MoE模型在分布式环境中通信阻塞的问题，提高运行效率。

Method: 修改模型架构，跳过连接层，通过自蒸馏方法转换模型，实现计算与通信的重叠。

Result: 成功转换了16B到109B参数的一系列最先进模型，在Llama 4 Scout (109B)上通过自蒸馏实现了与原始指令调优版本平均准确率相差不到1%的性能。

Conclusion: FarSkip-Collective方法能够有效重叠通信与计算，在保持模型准确率的同时加速训练和推理过程。
...
### [155] [Generalizing Fair Clustering to Multiple Groups: Algorithms and Applications](https://arxiv.org/abs/2511.11539)
*Diptarka Chakraborty,Kushagra Chatterjee,Debarati Das,Tien-Long Nguyen*

Main category: cs.LG

TL;DR: 本文研究了多群体（超过两个）的最接近公平聚类问题，证明了该问题在NP难解性，并提出了近似算法，同时改进了公平相关聚类和公平共识聚类的近似保证。


<details>
  <summary>Details</summary>
Motivation: 传统聚类算法经常无法为多个受保护属性定义的边缘化群体提供公平表示，这通常由训练数据中的偏见引起。现有研究仅限于两个群体的情况，而实际数据通常涉及多个群体（如年龄、种族、性别等）。

Method: 将最接近公平聚类问题推广到任意数量群体的设置，证明该问题的NP难解性，并提出近线性时间的近似算法来处理任意大小的多个群体。

Result: 开发了高效的多群体最接近公平聚类算法，改进了公平相关聚类问题的近似保证，并为多群体公平共识聚类问题提供了首个近似算法。

Conclusion: 本研究解决了多群体公平聚类中的关键挑战，为处理多个受保护属性的公平聚类问题提供了有效的算法解决方案，推进了该领域的研究进展。
...
### [156] [Multistability of Self-Attention Dynamics in Transformers](https://arxiv.org/abs/2511.11553)
*Claudio Altafini*

Main category: cs.LG

TL;DR: 本文揭示了自注意力动力学与多智能体Oja流的关系，将单头自注意力系统的平衡点分为四类：共识、二分共识、聚类和多边形平衡点。前两类平衡点总是与价值矩阵的特征向量对齐。
...
Method: 将自注意力动力学建模为连续时间多智能体系统，分析其与多智能体Oja流的关联，并对单头自注意力系统的平衡点进行分类研究。

Result: 发现自注意力动力学中经常共存多个来自前三类的渐近稳定平衡点，共识和二分共识平衡点总是与价值矩阵的特征向量对齐。

Conclusion: 自注意力动力学与多智能体Oja流密切相关，其平衡点具有明确的分类特征，这有助于深入理解transformer注意力机制的工作原理。
...
### [157] [Optimizing Mixture of Block Attention](https://arxiv.org/abs/2511.11571)
*Guangxuan Xiao,Junxian Guo,Kasra Mazaheri,Song Han*

Main category: cs.LG

TL;DR: 本文分析了MoBA（混合块注意力）机制，开发了统计模型揭示其性能关键取决于路由器准确区分相关与不相关块的能力，提出了FlashMoBA高效GPU实现，在保持性能的同时实现14.7倍加速。


<details>
  <summary>Details</summary>
Motivation: MoBA作为处理长上下文的高效机制，其设计原则理解不足且缺乏高效GPU实现，限制了实际应用。

Method: 开发统计模型分析MoBA机制，识别改进路径（小分块大小和键上的短卷积），并设计FlashMoBA硬件感知CUDA内核实现高效执行。

Result: 改进的MoBA模型在从头训练LLM时达到密集注意力基线的性能，FlashMoBA对小分块实现14.7倍于FlashAttention-2的加速。

Conclusion: 通过理论分析和硬件优化，成功将MoBA的理论优势转化为实际性能，为长上下文处理提供了高效解决方案。
...
### [158] [LEMUR: Large scale End-to-end MUltimodal Recommendation](https://arxiv.org/abs/2511.10962)
...
Method: 提出LEMUR端到端训练框架，采用记忆库机制增量累积历史多模态表示以降低计算成本，联合优化多模态和推荐组件。

Result: 在抖音搜索部署一个月后，查询变更率衰减减少0.843%，QAUC提升0.81%，在抖音广告关键离线指标上也有显著提升。
...
### [159] [Enhancing Group Recommendation using Soft Impute Singular Value Decomposition](https://arxiv.org/abs/2511.11172)
...
<details>
  <summary>Details</summary>
Motivation: 随着群组活动的日益普及，需要开发基于群组成员集体偏好的群组推荐方法。现有方法在处理现实应用中常见的稀疏高维数据时存在困难。

Method: 利用软插补奇异值分解进行低秩矩阵补全，通过Group Soft-Impute SVD方法来增强群组推荐效果。

Result: 在Goodbooks、Movielens和合成数据集上的实验表明，该方法在小用户群组的召回率上优于基于Group MF的基线方法，在所有群组规模下都取得了可比的结果，并且能够恢复比基线方法更低的矩阵秩。

Conclusion: Group Soft-Impute SVD方法在处理高维数据方面表现出有效性，特别适合解决群组推荐中的数据稀疏性问题。
...
### [160] [Align$^3$GR: Unified Multi-Level Alignment for LLM-based Generative Recommendation](https://arxiv.org/abs/2511.11255)
*Wencai Ye,Mingjie Sun,Shuhang Chen,Wenjin Wu,Peng Jiang*

Main category: cs.IR

TL;DR: Align³GR是一个统一token级、行为建模级和偏好级对齐的推荐系统框架，通过双token化、双向语义对齐和渐进式DPO策略，在公开数据集上比SOTA基线提升17.8%的Recall@10和20.2%的NDCG@10。


<details>
  <summary>Details</summary>
Motivation: 将大语言模型转化为实际推荐系统时存在语义和行为不对齐的挑战，需要解决这些根本性问题。

Method: 提出Align³GR框架，包含：双token化融合用户-物品语义和协同信号；双向语义对齐增强行为建模；结合自博弈和真实反馈的渐进式DPO策略进行动态偏好适应。

Result: 在公开数据集上比SOTA基线提升17.8%的Recall@10和20.2%的NDCG@10，在线A/B测试和工业级大规模推荐平台部署中取得显著收益。
...
### [161] [An External Fairness Evaluation of LinkedIn Talent Search](https://arxiv.org/abs/2511.10752)
*Tina Behzad,Siddartha Devic,Vatsal Sharan,Aleksandra Korolova,David Kempe*

Main category: cs.CY

TL;DR: 对LinkedIn人才搜索排名系统进行第三方偏见审计，发现少数群体在早期排名中存在代表性不足问题，并揭示了排名稳定性的群体差异。


<details>
  <summary>Details</summary>
Motivation: 作为独立第三方评估LinkedIn人才搜索系统在性别和种族方面的潜在排名偏见，补充平台自我审计的局限性。

Method: 构建真实世界排名数据集，开发人口属性推断流程，使用曝光差异指标（群体比例偏差和MinSkew）评估偏见，并分析时间维度上的子群公平性。

Result: 发现许多查询中少数群体在早期排名代表性不足，不同群体在排名稳定性上存在差异，某些群体排名位置波动更大。

Conclusion: 外部黑盒评估面临观察有限性和人口属性推断噪音等约束，早期排名中的偏见难以完全消除，需要关注时间维度的公平性问题。
...
### [162] [An International Agreement to Prevent the Premature Creation of Artificial Superintelligence](https://arxiv.org/abs/2511.10783)
*Aaron Scher,David Abecassis,Peter Barnett,Brian Abeyta*

Main category: cs.CY

TL;DR: 该报告提出了一个国际协议框架，旨在防止过早开发人工超级智能（ASI），通过限制AI训练规模和危险AI研究来降低灾难性风险，同时保留当前有益AI应用的使用。


<details>
  <summary>Details</summary>
Motivation: 防止人工超级智能的过早开发带来的灾难性风险，包括未对齐ASI导致的人类灭绝、地缘政治不稳定和恶意行为者的滥用。

Method: 建立由美国和中国领导的联盟，通过FLOP阈值限制AI训练规模，追踪AI芯片使用进行验证；通过法律禁止和多方面验证停止危险的AI研究。

Result: 该提案在技术上是充分的，但AI能力或开发方法的进步会削弱其有效性，且目前缺乏实施该协议的政治意愿。

Conclusion: 尽管存在挑战，该协议可为AI治理研究和政策提供方向，强调在确保安全的前提下推进AI发展。
...
### [163] [Generative Artificial Intelligence Adoption Among Bangladeshi Journalists: Exploring Journalists' Awareness, Acceptance, Usage, and Organizational Stance on Generative AI](https://arxiv.org/abs/2511.10862)
*H. M. Murtuza,Md Oliullah*

Main category: cs.CY

TL;DR: 研究通过访谈23名孟加拉国记者，探讨他们对生成式AI的认知、接受度、使用模式及媒体机构立场，发现尽管缺乏机构支持和AI政策，记者仍高度依赖AI，与西方同行相似。研究对UTAUT理论提出两点修正：促进条件在非西方环境中不影响行为意图；社会影响通过同行压力而非正式层级压力发挥作用。


<details>
  <summary>Details</summary>
Motivation: 了解全球新闻业采用生成式AI的背景下，孟加拉国记者在缺乏机构支持和政策指导的情况下如何接受和使用AI技术，以及东西方记者对AI担忧的异同。

Method: 采用深度访谈法，对23名孟加拉国记者进行访谈，分析他们对生成式AI的认知、接受度、使用模式和所在媒体机构的态度。

Result: 发现孟加拉国记者高度依赖生成式AI，尽管缺乏机构支持和AI政策；与西方记者对AI影响的担忧基本一致；在非西方环境中，促进条件不影响行为意图，社会影响通过同行压力而非正式层级压力发挥作用。

Conclusion: 研究揭示了非西方新闻业背景下技术采纳轨迹如何受情境因素影响，对UTAUT理论在非西方环境中的应用提出了重要修正，强调了同行压力和职业需求在技术采纳中的关键作用。
...
### [164] [Ethical conundrums: Hacked data in the study of far-right violent extremism](https://arxiv.org/abs/2511.10924)
*Lise Waldek,Brian Ballsun-Stanton,Muhammad Iqbal,David Kernot,Debra Smith*

Main category: cs.CY

TL;DR: 本文探讨了在数字研究中使用黑客数据的伦理问题，通过一个关于在线极右翼暴力极端主义的案例研究，论证了在某些情况下研究人员可以道德地使用黑客数据，但需要持续深入地应对伦理困境。


<details>
  <summary>Details</summary>
Motivation: 数字研究中的伦理行为存在许多灰色地带，特别是在使用黑客数据方面。现有的学科、机构和个体规范往往无法提供明确指导，导致研究人员感到不安或困惑。黑客数据的使用在研究中缺乏充分讨论，这为发展反思性伦理实践提供了机会。

Method: 采用案例研究方法，通过分析使用黑客数据研究在线极右翼暴力极端主义时出现的伦理辩论，探讨黑客数据研究的伦理框架。

Result: 研究发现，在特定情况下，研究人员可以道德地使用黑客数据进行研究。但成功的关键在于积极主动且持续地深入应对伦理困境和两难问题。

Conclusion: 使用黑客数据进行研究在伦理上是可行的，但需要研究人员不断深入参与伦理辩论，建立反思性的伦理实践，而不是回避这些复杂的伦理问题。
...
### [165] [Specification, Application, and Operationalization of a Metamodel of Fairness](https://arxiv.org/abs/2511.11144)
*Julian Alfredo Mendez,Timotheus Kampik*

Main category: cs.CY

TL;DR: 本文提出了AR公平性元模型，用于形式化表示、分析和比较公平性场景。该元模型通过Tiles框架实现，支持模块化组件连接来表示不同的公平性定义，特别关注平等与公平概念的比较。


<details>
  <summary>Details</summary>
Motivation: 旨在为增强现实(AR)系统提供形式化的公平性定义和比较方法，解决不同公平性概念在AR场景中的操作化问题。

Method: 使用Tiles框架构建模块化组件，通过组件互连来表示各种公平性定义，并实例化元模型进行案例研究。

Result: 开发了开源的公平性建模和评估实现，能够形式化定义公平性概念，并支持不同公平性定义的比较分析。

Conclusion: AR公平性元模型为定义、比较和评估AR系统中的公平性提供了有效方法，特别在区分平等与公平概念方面具有实用价值。
...
### [166] [Beyond the Hype: Critical Analysis of Student Motivations and Ethical Boundaries in Educational AI Use in Higher Education](https://arxiv.org/abs/2511.11369)
*Adeleh Mazaheriyan,Erfan Nourbakhsh*

Main category: cs.CY

TL;DR: 该研究分析了高等教育中生成式AI的快速应用与学生伦理认知、机构准备度之间的差距，揭示了学生普遍使用AI但缺乏指导的现状，提出了以AI素养、性别包容支持和评估改革为核心的伦理AI整合模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决2023年以来生成式AI在高等教育中快速应用与机构准备度不足之间的矛盾，特别是学生AI使用实践与现有伦理标准之间的脱节问题。

Method: 采用混合方法调查和文献综述，分析学生动机、伦理困境、性别差异反应以及机构对AI应用的准备情况。

Result: 研究发现92%的学生使用AI工具主要是为了节省时间和提高工作质量，但只有36%获得正式指导；18%的学生将AI生成内容整合到作业中；女性学生对信息滥用和扭曲表现出更大担忧；72%的教育工作者使用AI，但仅14%感到自在。

Conclusion: 机构必须采用全面的AI素养计划，整合技术技能和伦理推理，同时制定清晰的AI使用政策和促进透明度的评估实践，以负责任地采用AI、保护学术诚信并在AI驱动环境中促进公平教育成果。
...
### [167] [AI as a component in the action research tradition of learning-by-doing](https://arxiv.org/abs/2511.11445)
