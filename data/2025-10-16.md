...
Motivation: 现有神经渲染方法渲染速度慢或仅支持针孔相机模型，无法满足自动驾驶等应用对高畸变镜头和LiDAR数据的需求，且多传感器模拟存在跨传感器不一致性问题。

Method: 扩展3DGUT支持复杂相机模型，通过自动分块策略支持任意旋转LiDAR模型和基于光线的剔除，设计分解的3D高斯表示和锚定策略减少跨传感器不一致性。

Result: 相比现有方法，相机和深度误差降低高达40%，渲染速度比光线追踪方法快10-20倍，比基于光栅化的方法快1.5-10倍，在两个自动驾驶数据集上达到或超过最先进方法的保真度。

Conclusion: SimULi是首个能够实时渲染任意相机模型和LiDAR数据的方法，在多传感器模拟中表现出优异的性能和保真度。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术AI研究主要关注理解正在发生的事件，而非预测未来事件。现有方法针对孤立任务，缺乏统一方法，且难以在不同手术情境中泛化。

Method: 采用状态变化学习框架，通过Sinkhorn-Knopp聚类将视频片段压缩为状态表示，使用教师-学生架构，教师网络学习当前和未来片段，学生网络仅从当前视频预测未来状态，并由Action Dynamics模块指导。

Result: 在四个数据集和三个手术程序上的实验显示一致改进，跨程序迁移验证了方法的通用性。

Conclusion: SurgFUTR通过状态变化学习框架有效解决了手术未来预测问题，在短长期预测任务中均表现出色，并具有良好的跨程序泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习植物病害诊断系统在训练环境外的图像上表现不佳，主要由于病害症状的细微变化和领域差距，以及训练数据多样性不足。

Method: 基于度量学习的目标感知度量学习与优先采样框架，利用少量目标域标记样本改进诊断鲁棒性。

Result: 在包含223,073张叶片图像的大规模数据集上测试，仅使用每病害10个目标域样本，TMPS相比基线模型平均宏F1分数分别提升7.3和3.6个百分点，相比传统度量学习提升18.7和17.1个百分点。

Conclusion: TMPS框架能有效利用少量目标域样本显著提升植物病害诊断模型在未知环境下的泛化能力。
...
Conclusion: ViZer为视觉语言任务中的零标签适应提供了实用的起点，能够在不依赖文本标签的情况下提升图像描述质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗视觉语言模型主要针对结构化成人影像，在胎儿超声领域表现不佳，因为胎儿超声面临多视图推理、多种疾病和图像多样性的挑战。

Method: 提出显著认知解耦(SED)方法，将专家构建的二部图注入模型，解耦视图-疾病关联，并通过强化学习引导临床步骤选择，减少疾病变异性和视图异质性。

Result: FetalMind在所有孕周阶段均优于开源和闭源基准模型，平均提升14%，在关键病症上的准确率提高61.2%，同时保持高效、稳定和可扩展性。

Conclusion: FetalMind通过临床工作流引导的方法有效解决了胎儿超声领域的特殊挑战，为医学AI在复杂医疗场景中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决SD/SDXL模型在采样过程中存在的图像锐度不足、提示遵循不准确和伪影控制问题，同时保持适度的指导尺度。

Method: ZeResFDG模块整合频率解耦指导、能量重缩放和零投影；使用轻量级频谱EMA在采样过程中切换保守和细节寻求模式；QSilk微纹理稳定器通过分位数钳位和深度/边缘门控微细节注入提高鲁棒性。

Result: 在SD/SDXL采样器上，该方法改善了图像锐度、提示遵循和伪影控制，在高分辨率下产生自然的高频微纹理，且计算开销可忽略。

Conclusion: CADE 2.5提供了一种无需重新训练的有效方法，显著提升了SD/SDXL模型的图像质量和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视觉编码器VLMs简单堆叠编码器会导致收益递减且推理成本倍增，需要更智能的编码器选择方法。

Method: SCOPE框架包含共享编码器和路由编码器池，通过轻量级路由器使用文本提示与共享视觉特征的交叉注意力来选择最优编码器，采用双熵正则化和辅助损失来训练路由器。
...
Method: 提出SVAGFormer基线框架，适配最先进的视觉语言模型进行联合空间和时间定位；构建SVAG-Bench基准数据集(688个视频，19,590条标注记录，903个独特动词)；开发SVAGEval标准化评估工具包。

Result: 实证结果显示现有模型在SVAG任务上表现不佳，特别是在密集或复杂场景中，突显了在长视频中需要对细粒度对象-动作交互进行更高级推理的需求。

Conclusion: SVAG任务提出了新的挑战，现有方法难以处理，需要开发更先进的模型来理解细粒度的对象-动作交互。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频生成模型在视觉质量上表现良好，但在生成需要逻辑进展的多事件连贯序列叙事方面存在困难，现有基准主要关注视觉质量指标而缺乏对长序列叙事连贯性的评估。

Method: 构建了包含320个提示的SeqBench数据集，涵盖多种叙事复杂度，并设计了基于动态时序图(DTG)的自动评估指标，该指标能有效捕捉长程依赖和时序关系。

Result: DTG指标与人工标注有强相关性。系统评估揭示了当前T2V模型的关键局限：在多动作序列中无法保持对象状态一致性、多对象场景中产生物理上不合理的结果、难以保持顺序动作之间的现实时序关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法单独处理运动语义或场景感知，构建同时包含丰富文本-运动覆盖和精确场景交互的大规模数据集极具挑战

Method: 使用运动中间帧生成作为代理任务来桥接两个不同的数据集，第一阶段引入关键帧层调节运动潜在空间，第二阶段添加场景条件层通过交叉注意力注入场景几何

Result: 实验结果显示SceneAdapt有效将场景感知注入文本到运动模型

Conclusion: 该框架通过分析场景感知出现的机制，成功解决了同时处理运动语义和场景感知的挑战
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在处理长序列心电信号时性能受限，而状态空间模型作为高效替代方案显示出潜力，需要开发更有效的序列建模方法来提高心电图异常检测的准确性。

Method: 构建了混合框架1D-CNN-ECG-Mamba，结合卷积特征提取和Mamba选择性状态空间模型，基于Vision Mamba双向变体增强心电图数据中时间依赖关系的表示。

Result: 在PhysioNet 2020和2021挑战赛数据集上的实验表明，该模型在十二导联心电图上取得了显著更高的AUPRC和AUROC分数，优于之前发表的最佳算法。

Conclusion: 基于Mamba的架构在推进可靠的心电图分类方面具有潜力，支持早期诊断和个性化治疗，同时提高远程医疗和资源受限医疗系统的可及性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督新颖视图合成模型的预测姿态缺乏可转移性，相同的姿态集在不同3D场景中会产生不同的相机轨迹，无法实现真正的新颖视图合成。

Method: XFactor结合成对姿态估计与简单的输入输出增强方案，共同实现相机姿态与场景内容的解耦，促进几何推理，使用无约束的潜在姿态变量，无需3D归纳偏置或多视图几何概念。

Result: XFactor在可转移性方面显著优于先前的无姿态NVS变换器，潜在姿态与现实世界姿态高度相关，通过大规模实验验证了其有效性。

Conclusion: XFactor是第一个实现真正新颖视图合成的无几何自监督模型，证明了在无显式3D结构的情况下也能实现姿态可转移性，为自监督视图合成开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的学习方法在训练时使用梯度幅值损失，但梯度幅值化会丢失方向信息，导致监督模糊和边缘保真度不足。

Method: 引入方向感知的多尺度梯度损失，分别监督水平和垂直梯度分量，并在多尺度上保留其符号信息，提供明确的方向指导。

Result: 在开源模型和多个公共基准测试上的实验表明，该方法能有效提升边缘清晰度、对齐效果和纹理保留，且无需改变模型架构或训练协议。

Conclusion: 方向感知的多尺度梯度损失为图像融合提供了更精确的梯度监督，显著改善了融合质量。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分割中面临域偏移挑战，包括图像外观（风格）和人群解剖特征（内容）的差异，特别是在跨数据集部署时表现不佳。

Method: 结合z-归一化进行风格协调，使用双向可变形图像配准(DIR)网络与分割网络和判别器网络联合训练，生成解剖学上合理的变换以对齐源图像到目标域。

Result: 在合成数据集和三个MRI海马体数据集上的评估显示，该方法在所有实验中均优于现有基线。从年轻健康人群迁移到临床痴呆患者时，Dice得分相对标准增强方法提高了15%，在内容偏移大的场景中效果最显著。
...
Method: 构建包含ToyShape、SimObject和RealHand的CountHalluSet数据集，开发标准化评估协议，系统研究不同采样条件对计数幻觉水平的影响。
...
Method: 引入时空特征记忆库缓存特征，提出特征最相似传播方法保持时间一致性，使用交叉注意力图自动提取掩码，并集成到扩散去噪过程中。

Result: 在效率和视觉保真度方面优于现有最先进方法，验证了其优越的有效性和实用性。

Conclusion: Edit-Your-Interest提供了一种高效、准确的视频编辑解决方案，显著提升了编辑质量和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在社交意识方面存在不足，无法判断何时作为AI助手进行干预，导致频繁且不合时宜的响应，干扰自然对话和用户专注度。

Method: 构建了包含13,500个社交视频-问题对的大规模数据集EgoSocial，并提出EgoSoD方法，该方法整合多模态上下文线索到社交思维图中，动态建模参与者和互动。

Result: 实验显示现有OLLMs在干预时机检测上表现不佳（Gemini 2.5 Pro仅14.4%），而EgoSoD方法显著提升了干预时机检测性能（Phi-4提升45.6%，Gemini 2.5 Pro提升9.9%）。

Conclusion: EgoSoD方法能够有效检测社交互动中的干预时机，为AR/VR环境中的AI助手提供更好的社交意识，数据集和代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶规划器评估指标（如EPDMS）在复杂场景中缺乏上下文感知能力，无法准确反映人类判断。

Method: 开发了DriveCritic数据集（包含需要上下文判断的挑战性场景和成对人类偏好标注）和DriveCritic模型（基于VLM的评估器，通过两阶段监督学习和强化学习训练，整合视觉和符号上下文来评判轨迹对）。

Result: DriveCritic在匹配人类偏好方面显著优于现有指标和基线方法，并展现出强大的上下文感知能力。

Conclusion: 这项工作为评估自动驾驶系统提供了更可靠、与人类判断对齐的基础。
...
<details>
  <summary>Details</summary>
Motivation: 改进现有的网格生成和微分同胚图像配准方法，在保证配准精度的同时控制变换质量，为神经影像工作流提供关键的逆变换特性。

Method: 基于变分原理的网格生成方法，构建具有规定雅可比行列度和旋度的非折叠网格，在微分同胚群中生成逆变换而非图像空间操作。

Result: 在OASIS-1数据集的150个脑扫描配准中，VPreg在35个感兴趣区域的Dice分数、计算变换的规则性以及逆映射的准确性和一致性方面均优于ANTs-SyN、Freesurfer-Easyreg和FSL-Fnirt等最先进方法。

Conclusion: VPreg在配准精度、变换规则性和逆映射质量方面均优于现有方法，为计算解剖学和形态测量学提供了更可靠的微分同胚空间变换。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本-图像对齐中由于信息熵差异导致的跨模态检索不平衡问题，传统方法在两种模态的相互检索中存在不平衡现象。

Method: 1) 设计不依赖任务领域显式知识的提示模板，利用LLM增强文本模态的多义性描述；2) 使用超图适配器构建文本与图像模态间的多边连接，在固定嵌入空间中纠正同义语义的正负匹配错误。

Result: 在Flickr30K和MS-COCO基准测试中，文本到图像检索性能提升16.8%，图像到文本检索性能提升40.1%，在语义对齐任务中建立了新的最先进性能。

Conclusion: OS-HGAdapter方法通过利用大语言模型的开放语义知识和超图适配器，有效解决了文本-图像对齐中的信息熵差距问题，显著提升了跨模态检索性能。
...
Conclusion: 上下文相关的架构选择对于实际应用至关重要，3D CNN适合高精度需求，LSTM适合资源受限的实时应用。
...
<details>
  <summary>Details</summary>
Motivation: 改进现有视觉隐写术的容量限制和精度，利用高效的潜在表示和注视点渲染技术来提升性能。

Method: 训练模型利用高效潜在表示和注视点渲染，创建多模态潜在表示，并采用新颖的感知设计。

Result: 容量从100位提升到500位，精度达到2000位中仅1位错误，视觉质量达到31.47 dB PSNR和0.13 LPIPS。

Conclusion: 新型感知设计在创建多模态潜在表示方面有效，显著提升了隐写术的容量、精度和视觉质量。
...
Method: 利用TEM信号的固有物理特性（指数衰减和平滑性）作为先验知识，通过字典学习编码为字典驱动先验，在训练时融入模型，在测试时通过最小化基于字典一致性和信号一阶变化的自监督损失来动态适应新环境。

Result: 大量实验结果表明，该方法在TEM去噪性能上显著优于现有TEM去噪方法和测试时自适应方法。

Conclusion: 提出的DP-TTA方法通过利用TEM信号的固有物理特性作为先验知识，有效解决了模型在不同地理环境中的适应性问题，显著提升了去噪性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统边缘资源管理方法强调通信吞吐量或通用学习性能，而EGS明确旨在最大化GS质量，导致现有方法不适用。需要专门针对GS质量优化的资源分配策略。

Method: 提出STT-GS策略：第一阶段从每个客户端采样图像作为导频数据进行损失预测；第二阶段基于评估结果优先分配通信资源给更有价值的客户端。采用特征域聚类(FDC)选择代表性数据，导频传输时间最小化(PTTM)减少开销，并开发联合客户端选择和功率控制(JCSPC)框架。

Result: 在真实世界数据集上的实验表明，所提方案显著优于现有基准方法。GS导向目标函数可在低采样率(如10%)下准确预测，方法在视图贡献和通信成本间实现了良好权衡。

Conclusion: 提出的STT-GS策略有效解决了EGS中的因果困境，通过智能采样和资源分配显著提升了GS质量，为边缘场景重建提供了高效解决方案。
...
Method: 提出两阶段占用预测框架，提取分割、图形和深度特征，引入可变形多级融合机制融合这三种多级特征，并集成SAM知识蒸馏。

Result: 在不增加训练成本的情况下，在SemanticKITTI基准上实现了最先进的性能。

Conclusion: CIGOcc通过多级表示融合有效提升了相机占用预测性能，证明了表示融合策略的重要性。
...
<details>
  <summary>Details</summary>
Motivation: AI会议的快速增长给同行评审系统带来压力，导致评审工作量大、专业不匹配、标准不一致、评审质量差等问题，现有临时性政策改进往往造成更多困惑，评审过程不透明。

Method: 开发Paper Copilot系统，构建跨多个计算机科学会议的同行评审数字档案和开放数据集，并对ICLR多年评审数据进行大规模实证分析。

Result: 创建了支持同行评审可重复研究的基础设施和数据集，能够追踪评审过程变化、诊断问题模式。

Conclusion: 通过提供这些资源，帮助社区追踪变化、诊断问题，为建立更稳健、透明和可靠的同行评审系统提供基于证据的改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在从语音生成风格化3D人体运动时存在两个主要问题：一是风格编码过于简化或忽略区域运动风格差异；二是未能让运动风格动态适应语音节奏和情感变化，限制了运动真实感。
...
Conclusion: MimicParts通过分区感知的方法成功解决了风格化3D人体运动生成中的关键挑战，实现了更精细的区域风格控制和动态适应能力。
...
Method: 从第一性原理重新审视VP和VPT设计，将其概念化为统一的提示式适应(PA)框架，提供基于可学习性、生成性和非可学习性的分类法，并按注入粒度组织。

Result: 建立了PA框架，系统整理了现有方法，并考察了PA在医学影像、3D点云、视觉语言任务等领域的应用，以及其在测试时适应和可信AI中的作用。
...
<details>
  <summary>Details</summary>
Motivation: 工业表面缺陷检测面临极端的前景-背景不平衡、缺陷稀疏性、长尾尺度分布和低对比度等挑战，传统像素中心训练容易受大均匀区域主导，难以关注小或低对比度缺陷，导致样本级决策不稳定。

Method: 基于共享编码器架构的多任务学习框架，同时学习样本级缺陷分类和像素级掩码定位，通过样本级监督调制特征分布并持续提升小缺陷的召回率，分割分支保留边界细节以增强决策稳定性。

Result: 在两个基准数据集上的实验表明，该方法显著提高了样本级决策的可靠性和缺陷定位的完整性。

Conclusion: 样本中心的多任务学习框架能有效解决工业缺陷检测中的样本级决策稳定性问题，提出的决策关联评估指标Seg_mIoU和Seg_Recall能更准确地反映模型在实际应用中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进的视觉语言模型在理解否定方面存在严重缺陷，特别是在描述性目标检测任务中表现出肯定性偏差，这限制了模型在真实世界检测应用中的有效性。

Method: 提出两个主要贡献：(1) CoVAND数据集，使用链式思维和VQA管道生成高质量的实例接地否定数据；(2) NegToMe文本token合并模块，通过将否定词与属性词合并为有意义的语义短语，从根本上解决token化过程中否定线索丢失的问题，并结合参数高效的LoRA微调方法。

Result: 该方法在具有挑战性的否定基准测试中显著提升了性能，将假阳性率降低了10.8个NMS-AP点（在OVDEval上），并展示了在最新VLMs上的泛化能力。

Conclusion: 这项工作在解决真实世界检测应用中的否定理解问题上迈出了关键一步，通过直接解决架构原因来提升模型的否定理解能力。
...
TL;DR: UniVector是一个统一的矢量提取框架，能够在单一模型中提取多种矢量类型（多边形、折线、线段），通过实例-几何交互机制实现跨层级上下文交换，并在多结构矢量提取任务中达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有矢量提取方法通常只针对单一矢量类型设计，需要为不同结构使用独立模型，这限制了捕捉复杂结构的能力。受人类视觉感知中同时使用语义和空间交互的启发，需要开发统一框架来处理多种矢量类型。

Method: UniVector将矢量编码为包含实例级和几何级信息的结构化查询，通过交互模块迭代更新这些查询以实现跨层级上下文交换，并使用动态形状约束来优化全局结构和关键点。

Result: 实验表明UniVector在单结构和多结构矢量提取任务中都达到了新的最先进水平，并在新构建的Multi-Vector数据集上验证了其有效性。

Conclusion: UniVector通过实例-几何交互机制成功实现了在单一模型中提取多种矢量类型，证明了统一框架在矢量提取任务中的可行性和优越性。
...
Method: 使用显式提示将空间运动信息转化为自然语言描述提供时空指导，隐式提示结合伪词和可学习描述符构建个性化知识表示，并通过CLIP文本编码器动态调整，同时设计了判别性特征增强器。

Result: 在MOT17、MOT20和DanceTrack数据集上的大量实验表明，EPIPTrack在多样化场景中优于现有跟踪器。

Conclusion: EPIPTrack框架展现了强大的适应性和优越性能，证明了动态多模态提示在目标跟踪中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管VLA模型在机器人学习中取得了革命性进展，但其对抗鲁棒性仍未得到充分探索。本研究旨在填补这一空白，研究VLA模型在面对对抗性攻击时的脆弱性，并提出相应的防御策略。

Method: 提出了EDPA攻击方法，这是一种模型无关的对抗攻击，通过优化两个目标来生成可放置在相机视野中的补丁：(1)破坏视觉与文本潜在表示的语义对齐；(2)最大化对抗样本与干净样本潜在表示的差异。同时提出了对抗性微调防御方案，优化视觉编码器使其对干净和对抗性扰动输入产生相似的潜在表示。

Result: 在广泛认可的LIBERO机器人仿真基准测试中，EDPA显著提高了最先进VLA模型的任务失败率，而提出的防御方法有效缓解了这种性能下降。

Conclusion: 该工作揭示了VLA模型在对抗性攻击下的脆弱性，提出的EDPA攻击方法具有模型无关性，而对抗性微调防御策略能够有效提升VLA模型的鲁棒性，为构建更安全的机器人系统提供了重要见解。
...
Method: 基于SynDrone和FlyAware数据集，FlyAwareV2引入多模态数据、通过单目深度估计计算真实样本的深度图、建立RGB和多模态语义分割基准，并研究合成到真实的域适应。

Result: 提供了丰富的标注和环境多样性，为基于无人机的3D城市场景理解研究提供了宝贵资源。

Conclusion: FlyAwareV2通过其多模态特性和环境多样性，解决了无人机数据收集的挑战，推动了无人机城市场景理解研究的发展。
...
Method: 提出了Cylinder Mamba Diffusion (CymbaDiff)方法，通过施加结构化空间排序、明确捕捉圆柱连续性和垂直层次结构，在生成场景中保持物理邻域关系和全局上下文。

Result: 在SketchSem3D上的广泛实验表明，CymbaDiff在语义一致性、空间真实性和跨数据集泛化方面实现了优越性能。

Conclusion: SketchSem3D基准和CymbaDiff方法为户外3D语义场景生成提供了标准化评估框架，显著提升了生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人群计数方法在嵌入式系统实际应用中存在模型参数过多、计算复杂等问题，而嵌入式系统应用需要模型具有实时性，即足够快的推理速度。

Method: 设计stem-encoder-decoder结构：stem网络使用大卷积核扩大感受野提取头部细节；编码器部分使用条件通道加权和多分支局部融合块以低计算消耗融合多尺度特征；在编码器顶部添加特征金字塔网络缓解不完全融合问题。

Result: 在三个基准测试上的实验表明，该网络适合嵌入式系统上的超实时人群计数，推理速度最快：在NVIDIA GTX 1080Ti上达到381.7 FPS，在NVIDIA Jetson TX1上达到71.9 FPS。

Conclusion: 提出的网络在嵌入式系统上实现了超实时的人群计数性能，在保证竞争性准确率的同时具有最快的推理速度。
...
Method: 使用机制可解释性技术分析VideoLLMs的内部信息流，研究跨帧交互和视频-语言整合的模式。

Result: 发现视频大语言模型在视频问答任务中存在一致的模式：早期到中间层进行跨帧交互，中间层进行渐进式视频-语言整合，中后期层生成正确答案。通过选择有效信息路径可以抑制58%的注意力边而保持性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型的上下文窗口不断扩展，但扩展的上下文窗口并不能保证上下文的有效利用，这对实际应用构成关键挑战。当前评估主要关注纯文本领域，多模态评估仍局限于短上下文。
...
Conclusion: 需要开发更有效的方法来提升大型视觉语言模型在长多模态上下文中的忠实性表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统预训练方法在图像修复任务中表现有限，需要一种能够同时学习退化类型识别和高质量图像重建的预训练方法，以提升修复模型的性能和鲁棒性。

Method: 使用编码器-双解码器架构：编码器提取掩码低质量图像特征，分类解码器识别退化类型，重建解码器生成高质量图像。结合掩码图像建模和对比学习。

Result: 在5D全能修复任务中PSNR至少提升3.77dB，真实场景中PIQE降低34.8%，对未见过的退化类型和级别展现出强泛化能力。发布了包含250万样本的UIR-2.5M数据集。

Conclusion: MaskDCPT是一种简单而有效的预训练方法，可为通用图像修复任务提供强大的基础模型，显著提升CNN和Transformer的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决实际应用中因光照变化、任意方向、弯曲或部分遮挡文本、低分辨率和远距离文本等挑战导致的文档分类困难。

Method: 四阶段流程：图像采集与预处理、DBNet++文本检测、BART文本分类、PyQt5用户界面集成。

Result: 在Total-Text数据集上文本检测准确率达到92.88%，该系统在非约束成像场景下对混合来源文档分类有效。

Conclusion: 所提出的方法在实际混合来源文档分类中表现有效，能够应对各种成像挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在学习基础类与新颖类之间的相关性时往往采用粗略或统计相关学习，容易导致新颖类推断混淆。通过引入因果约束可以揭示准确对应类别的本质点云表示。

Method: 提出联合学习因果表示与推理的方法：首先通过SCM分析基础类表示中的隐藏混杂因子，设计消除混杂因子的因果表示原型；然后使用图结构建模基础类因果表示原型与新颖类原型之间的因果关系。

Result: 在3D和2D NCD语义分割上的大量实验和可视化结果证明了该方法的优越性。

Conclusion: 通过结构因果模型和因果推理，能够有效解决3D点云新类发现问题，提高新颖类分割的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 传统SfM方法（如COLMAP、GLOMAP）在CPU上实现束调整和全局定位时计算开销大，而基于深度学习的SfM方法（如VGGSfM、VGGT）在处理大量输入视图时GPU内存消耗急剧增加。需要一种既能高效处理大规模场景又保持灵活性的解决方案。

Method: 利用GPU并行计算加速标准SfM流程的每个关键阶段，基于稀疏感知束调整优化的最新进展，在统一的全局SfM框架中同时加速束调整和全局定位。

Result: 在不同规模数据集上的实验表明，该方法相比COLMAP实现了约40倍加速，同时保持相当甚至更好的重建精度，能够处理5000张图像的大规模场景。

Conclusion: 通过充分发挥GPU并行计算潜力，本文提出的方法在大规模SfM任务中实现了显著的速度提升和良好的重建质量，解决了现有方法的扩展性问题。
...
Method: 1. 自增强提示策略：利用模型内在知识动态对齐查询与视觉增强的语义；2. 自适应阈值算法：基于输出稀疏性自适应调整下一个token候选大小。
...
TL;DR: 该研究探索大型多模态模型（特别是GPT-4o）在视觉吸引力理解方面的能力，通过比较分析评估人类评估与模型预测的一致性，并利用模型生成训练数据来训练学习排序模型。


<details>
  <summary>Details</summary>
Motivation: 日常生活中，视觉吸引力对人们的行为和注意力有重要影响。随着大型多模态模型的发展，研究这些模型是否能够理解视觉吸引力的概念，以及它们与人类评估的一致性。

Method: 通过比较分析评估GPT-4o与人类在视觉吸引力评估上的一致性，利用模型生成图像对的有趣程度标签作为训练数据，训练学习排序模型来提炼知识。

Result: 研究发现GPT-4o与人类在视觉吸引力评估上存在部分一致性，且相比现有最优方法表现更好，能够有效标注图像对的吸引力程度。

Conclusion: 大型多模态模型能够有效捕捉视觉吸引力的概念，为深入理解人类兴趣提供了新的途径，并可通过知识蒸馏方法将这种理解能力传递给其他模型。
...
Conclusion: 通过移除成本体积的训练策略，可以在保持精度的同时显著提升光流估计的效率和资源利用率。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐蔽武器检测技术存在分辨率低、隐私问题等限制，需要开发实时、低成本、保护隐私的解决方案，但缺乏基准数据集。

Method: 在YOLOv8基础上改进，在SPPF层使用可变形卷积提取多尺度特征，在骨干网络和颈部网络提取低、中、高级特征，并引入焦点损失解决类别不平衡问题。

Result: 构建了首个大规模热成像隐蔽武器检测数据集TICW，DEF-YOLO架构在热成像同质区域能够自适应聚焦目标定位，同时保持速度和吞吐量。
...
TL;DR: 提出Group-VQ方法，通过分组优化码本来解决VQ-VAE中的码本崩溃问题，提高码本利用率和重建性能，并引入训练后码本重采样方法实现码本大小的灵活调整。
...
Method: Group-VQ方法对码本进行分组优化，每组独立优化，组内联合优化，同时提出训练后码本重采样方法。
...
TL;DR: 提出了一个针对渲染视频的无参考质量评估方法和数据集，专门解决渲染视频中时间伪影问题，在渲染视频质量评估上优于现有方法。
...
Method: 构建了一个大型渲染视频数据集，包含多种3D场景和渲染设置，并设计了专门针对渲染视频的无参考质量评估指标，同时考虑图像质量和时间稳定性。

Result: 提出的指标在渲染视频质量评估上表现优于现有无参考视频质量评估方法，能够有效评估超采样方法和实时渲染中的帧生成策略。

Conclusion: 该研究为渲染视频质量评估提供了专门的数据集和评估指标，填补了现有方法在渲染视频评估上的不足，具有实际应用价值。
...
TL;DR: 研究发现，在视觉语言模型的零样本分类中，对于高性能模型，最简单的基础提示词效果最好，增加描述性细节反而会显著降低性能，这种现象被称为"提示过拟合"。
...
Result: 对于性能最高的模型（MetaCLIP 2和OpenCLIP），最简单的基础提示词始终获得最佳结果，增加描述性细节会显著降低性能（如MetaCLIP 2的多类准确率从68.8%降至55.1%）。而性能较低的SigLip模型在给出更具描述性的基于身体线索的提示词时，对模糊类别的分类有所改善。

Conclusion: 提示词设计对视觉语言模型的零样本分类性能有显著影响，高性能模型倾向于简单提示词，而低性能模型可能从更详细的提示词中受益，这为实际应用中的提示词优化提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在需要精确空间推理的任务上表现不佳，因为它们从VLM继承的空间推理能力有限，且依赖大量动作数据预训练来在3D空间中建立基础，这降低了训练效率且仍不足以实现准确的空间理解。

Method: DepthVLA采用混合变换器设计，统一了VLM、深度变换器和动作专家，通过完全共享注意力形成端到端模型，并显式地通过预训练深度预测模块融入空间感知。

Result: 在真实世界任务中达到78.5%进展（vs. 65.0%），在LIBERO模拟器中达到94.9%（vs. 93.6%），在Simpler模拟器中达到74.8%（vs. 58.8%），均优于最先进方法。

Conclusion: DepthVLA通过显式集成空间感知能力，有效提升了VLA模型的空间推理性能，在多种环境中都表现出色。
...
TL;DR: 提出了一种将SDF与3DGS结合的新方法，用于动态场景渲染和重建，无需LiDAR数据和3D运动标注即可在城市场景中实现最先进的渲染性能。
...
Method: 将SDF与3DGS集成，利用深度和点跟踪的2D对象无关先验，构建统一的优化框架，增强3DGS的几何精度并改进SDF内的变形建模。

Result: 在城市场景中无需LiDAR数据即可实现最先进的渲染指标性能。结合LiDAR时，在重建和生成新视图方面表现更佳，无需地面真实3D运动标注。支持场景分解和组合等编辑任务。

Conclusion: 该方法通过结合SDF和3DGS的优势，实现了更鲁棒的动态对象表示，在减少数据要求的同时保持了高质量的渲染和重建性能。
...
TL;DR: 提出GLSDA框架，利用预训练大模型的语义先验增强WiFi手势识别的泛化能力，通过双路径CSI编码、多尺度语义编码器和鲁棒双蒸馏策略，在跨域手势识别中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有WiFi手势识别方法因信道状态信息域敏感性和缺乏高层手势抽象导致的泛化能力和语义表达能力不足的问题。
...
TL;DR: 提出了Spatial-DISE基准，基于认知分类法评估视觉语言模型的空间推理能力，特别是内在动态空间推理，发现当前模型与人类能力存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分评估视觉语言模型的空间推理能力，尤其是人类空间认知的基本方面——内在动态空间推理。

Method: 基于认知分类法将任务分为四个基本象限，开发可扩展的自动化流水线生成多样且可验证的空间推理问题，构建包含559个评估VQA对和12K+训练VQA对的数据集。

Result: 对28个最先进视觉语言模型的评估显示，当前模型在空间推理能力上与人类存在巨大且一致的差距，特别是在多步骤多视图空间推理方面。
...
TL;DR: 提出了Mask-GRPO方法，首次将基于GRPO的强化学习应用于被忽视的掩码生成模型，在文本到图像生成任务中显著提升了性能。
...
Result: 在标准文本到图像基准测试和偏好对齐方面显著提升基础模型Show-o的性能，优于现有最先进方法。
...
TL;DR: Patch-Adapter是一个用于高分辨率文本引导图像修复的有效框架，通过两阶段适配器架构实现4K+分辨率修复，解决了现有方法在分辨率提升时的内容一致性和提示对齐挑战。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法受限于较低分辨率，无法处理4K+高分辨率场景。随着分辨率和纹理复杂度的增加，保持内容一致性和提示对齐变得更具挑战性。

Method: 采用两阶段适配器架构：1）双上下文适配器在降低分辨率下学习掩码与非掩码区域的连贯性，建立全局结构一致性；2）参考补丁适配器实现补丁级注意力机制，通过自适应特征融合保持局部细节保真度。

Result: 在OpenImages和Photo-Concept-Bucket数据集上实现最先进性能，有效解决了大规模修复中的伪影问题，在感知质量和文本提示遵循方面均优于现有方法。
...
TL;DR: CoDS是一个用于异构场景下协作感知的方法，通过域分离技术解决特征差异问题，采用轻量级空间通道调整器和分布对齐模块，在保持高推理效率的同时提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法假设所有智能体使用相同的编码器，但在实际异构场景中，不同车辆配备不同模型，导致特征差异。现有方法在特征对齐时易受域间隙噪声影响，且基于transformer的域适应模块在移动设备上推理效率低。

Method: 提出CoDS方法，包含两个特征对齐模块：轻量级空间通道调整器(LSCR)和基于域分离的分布对齐(DADS)。LSCR使用轻量卷积层对齐空间和通道维度特征，DADS通过编码器特定和编码器无关的域分离模块分别去除域依赖信息和捕获任务相关信息。使用域对齐互信息(DAMI)损失最大化异构特征间的互信息。

Result: CoDS有效缓解了异构场景中的特征差异问题，在检测精度和推理效率之间实现了良好平衡。采用全卷积架构确保了高推理效率。

Conclusion: CoDS通过域分离技术成功解决了异构协作感知中的特征差异问题，同时保持了高效的推理性能，为实际部署提供了可行方案。
...
### [58] [UniME-V2: MLLM-as-a-Judge for Universal Multimodal Embedding Learning](https://arxiv.org/abs/2510.13515)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉候选样本间细微语义差异方面存在困难，缺乏负样本多样性，且嵌入表示在区分假负样本和硬负样本方面能力有限。

Method: 通过全局检索构建潜在硬负样本集，引入MLLM-as-a-Judge机制评估查询-候选对的语义对齐并生成软语义匹配分数，用于硬负样本挖掘和缓解一对一映射约束。

Result: 在MMEB基准测试和多个检索任务上进行了全面实验，证明该方法在所有任务上平均达到最先进性能。

Conclusion: 所提出的UniME-V2模型通过利用MLLM的先进理解能力显著增强了表示学习，在通用多模态嵌入任务中表现出卓越性能。
...
### [59] [Beyond Pixels: A Differentiable Pipeline for Probing Neuronal Selectivity in 3D](https://arxiv.org/abs/2510.13433)
...
### [60] [Near-Infrared Hyperspectral Imaging Applications in Food Analysis -- Improving Algorithms and Methodologies](https://arxiv.org/abs/2510.13452)
*Ole-Christian Galbo Engstrøm*

Main category: cs.CV

TL;DR: 该论文研究了近红外高光谱成像在食品质量分析中的应用，比较了CNN和PLS模型，发现联合空间-光谱分析的CNN模型在化学和物理视觉信息相关参数建模中表现最佳，并开发了两个开源Python工具包。


<details>
  <summary>Details</summary>
Motivation: 探索近红外高光谱成像在食品质量分析中的潜力，特别是比较深度学习（CNN）与传统方法（PLS）在食品参数建模中的性能差异。

Method: 通过四项研究验证五个研究假设，使用卷积神经网络（CNN）和偏最小二乘法（PLS）进行对比分析，包括2D CNN增强光谱卷积层的方法。

Result: 联合空间-光谱分析的CNN模型优于单独的空间CNN和光谱PLS；增强光谱卷积层的2D CNN提升了化学参数预测性能；PLS在平均化学含量分析中表现相当；CNN在化学图谱生成中解决了PLS的平滑性和范围问题。

Conclusion: CNN在复杂食品质量参数建模中具有优势，特别是结合空间和光谱信息时；PLS在简单平均含量分析中仍是推荐方法；数据集质量对结果有重要影响；开发了高效的PLS建模和交叉验证工具包。
...
### [61] [Modeling Cultural Bias in Facial Expression Recognition with Adaptive Agents](https://arxiv.org/abs/2510.13557)
...
TL;DR: 提出了基于代理的流式基准测试，研究文化组成和渐进模糊如何影响面部表情识别的鲁棒性。结果显示不同文化群体在模糊条件下的性能退化模式存在明显不对称性。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情识别评估通常假设同质数据和高质量图像，但在实际应用中需要应对文化差异和视觉条件退化。

Method: 使用基于代理的流式基准测试，代理在冻结的CLIP特征空间中运行，配备轻量级残差适配器。环境提供按sigma调度的Gaussian模糊输入，研究单文化群体和混合群体的表现。

Result: JAFFE（亚洲）群体在低模糊时性能更高但中间阶段下降更陡，KDEF（西方）群体退化更均匀。混合群体呈现中间模式，平衡混合缓解早期退化但不平衡设置会放大多数群体的弱点。

Conclusion: 文化组成和交互结构显著影响面部表情识别在感知条件恶化时的鲁棒性，需要设计更包容和稳健的系统。
...
TL;DR: VIST3A是一个文本到3D生成的通用框架，通过将文本到视频生成器与3D重建解码器结合，解决了模型拼接和对齐两个关键挑战，显著提升了文本到3D生成的质量。


<details>
  <summary>Details</summary>
Motivation: 结合现代潜在文本到视频模型的生成能力和近期3D重建系统的几何能力，构建强大的3D场景生成器。

Method: 1. 使用模型拼接技术将文本到视频生成器与3D解码器连接；2. 采用直接奖励微调技术对齐生成器和解码器，确保生成的潜在表示可解码为一致的3D场景几何。

Result: 所有测试的配对组合都显著优于之前输出高斯泼溅的文本到3D模型，通过选择合适的3D基础模型，VIST3A还能实现高质量的文本到点云图生成。
...
### [63] [Through the Lens of Doubt: Robust and Efficient Uncertainty Estimation for Visual Place Recognition](https://arxiv.org/abs/2510.13464)
*Emily Miller,Michael Milford,Muhammad Burhan Hafez,SD Ramchurn,Shoaib Ehsan*

Main category: cs.CV

TL;DR: 提出了三种无需训练的VPR不确定性度量方法：相似度分布(SD)、比率扩展(RS)和统计不确定性(SU)，通过分析相似度得分的统计模式来估计位置匹配的置信度，在各种VPR方法和数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: VPR系统在变化的环境条件下部署时面临挑战，而关键应用如SLAM中的闭环检测需要鲁棒的不确定性估计。现有方法通常需要额外训练或验证数据，限制了实用性。

Method: 提出了三种训练免费的不确定性度量：SD通过测量候选者之间的得分分离来量化匹配独特性；RS评估高分位置之间的竞争模糊性；SU结合SD和RS提供统一度量，无需验证数据选择最优度量。

Result: 在9种最先进VPR方法和6个基准数据集上的综合评估表明，这些度量在区分正确和错误VPR匹配方面表现优异，始终优于现有方法，同时保持可忽略的计算开销。

Conclusion: 该方法为实时机器人应用提供了可部署的VPR不确定性估计解决方案，在各种环境条件下提高了精确率-召回率性能，无需额外训练或架构修改。
...
### [64] [ExpressNet-MoE: A Hybrid Deep Neural Network for Emotion Recognition](https://arxiv.org/abs/2510.13493)
...
Method: 使用多个基于CNN的特征提取器、MoE模块进行自适应特征选择，以及残差网络主干进行深度特征学习，采用多尺度特征提取捕获全局和局部面部特征。
...
Method: 使用面部动作单元作为非瞬态、高度语义化的特征，先学习基本情绪，然后增量学习复杂情绪，类似于人类的学习过程。

Result: 在CFEE数据集上达到0.75的准确率，性能优于浅层和深层卷积神经网络提取的特征，模型轻量且内存占用小。

Conclusion: 面部动作单元作为非瞬态特征在增量学习复杂情绪识别中表现优异，能够有效缓解灾难性遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有商业3D乳房扫描解决方案昂贵且需要专用硬件，现有低成本替代方案效果不佳。需要开发无需专用硬件或专有软件、可在任何能录制RGB视频的设备上使用的解决方案。

Method: 使用最先进的开源运动恢复结构流程，结合参数化乳房模型进行表面重建。模型采用局部神经符号距离函数，将乳房区域分解为多个基于解剖标志的小区域。

Result: 提出的liRBSM模型在重建质量上显著优于iRBSM，能够恢复误差小于2毫米的高质量3D乳房几何形状，重建时间少于6分钟。

Conclusion: 该方法提供了一种低成本、可访问且高质量的3D乳房表面重建解决方案，完全透明且开源。
...
### [67] [CanvasMAR: Improving Masked Autoregressive Video Generation With Canvas](https://arxiv.org/abs/2510.13669)
*Zian Li,Muhan Zhang*

Main category: cs.CV

TL;DR: CanvasMAR是一种新颖的视频掩码自回归模型，通过引入画布机制解决现有模型的两个主要限制：采样初期的慢启动问题和时空维度上的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视频掩码自回归模型存在慢启动问题（缺乏结构化全局先验）和时空维度上的误差累积问题，限制了生成质量和效率。

Method: 提出画布机制（模糊的下一帧全局预测）作为掩码生成的起点，引入组合式无分类器引导联合放大空间（画布）和时间条件，并使用基于噪声的画布增强来提高鲁棒性。
...
### [68] [Accelerated Feature Detectors for Visual SLAM: A Comparative Study of FPGA vs GPU](https://arxiv.org/abs/2510.13546)
*Ruiqi Ye,Mikel Luján*

Main category: cs.CV

TL;DR: 比较GPU和FPGA在视觉SLAM中加速特征检测的性能，发现GPU在传统特征检测器上表现更好，而FPGA在学习型特征检测器SuperPoint上具有更好的运行性能和能效。


<details>
  <summary>Details</summary>
Motivation: 特征检测是SLAM系统中耗时且耗能的模块，特别是在无人机等功耗受限平台上。研究GPU和FPGA这两种硬件加速器在视觉SLAM特征检测中的性能差异。

Method: 在Nvidia Jetson Orin和AMD Versal等现代SoC上，比较GPU和FPGA加速的FAST、Harris和SuperPoint特征检测器实现，并评估它们在视觉SLAM流水线中的表现。

Result: 对于传统特征检测器（FAST、Harris），GPU实现比FPGA具有更好的运行性能和能效；对于学习型检测器SuperPoint，FPGA实现比GPU具有更好的运行性能（最高3.1倍）和能效（最高1.4倍）。GPU加速的V-SLAM通常更准确。

Conclusion: 硬件加速特征检测可减少全局束调整调用频率而不损失精度。选择加速方案需考虑特征检测器类型：传统检测器适合GPU，学习型检测器适合FPGA。
...
### [69] [MVCustom: Multi-View Customized Diffusion via Geometric Latent Rendering and Completion](https://arxiv.org/abs/2510.13702)
*Minjung Shin,Hyunin Cho,Sooyeon Go,Jin-Hwa Kim,Youngjung Uh*

Main category: cs.CV

TL;DR: 提出MVCustom框架，解决多视角生成与个性化定制难以统一的问题，通过特征场表示和扩散模型实现几何一致性和定制保真度


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限：多视角生成模型不支持几何一致的个性化定制，而定制模型缺乏显式视角控制，难以统一这两种能力

Method: 基于扩散模型，使用特征场表示学习主体身份和几何信息，结合文本到视频扩散骨干网络和密集时空注意力，在推理阶段引入深度感知特征渲染和一致性感知潜在补全

Result: 实验表明MVCustom是唯一能同时实现忠实多视角生成和个性化定制的框架

Conclusion: MVCustom成功解决了多视角定制任务，在保持几何一致性的同时实现高质量的个性化生成
...
### [70] [XD-RCDepth: Lightweight Radar-Camera Depth Estimation with Explainability-Aligned and Distribution-Aware Distillation](https://arxiv.org/abs/2510.13565)
*Huawei Sun,Zixu Wang,Xiangyuan Peng,Julius Ott,Georg Stettinger,Lorenzo Servadei,Robert Wille*

Main category: cs.CV

TL;DR: XD-RCDepth是一个轻量化的雷达-相机融合深度估计架构，相比最先进的轻量级基线减少了29.7%的参数，同时保持相当的精度。通过两种知识蒸馏策略提升压缩后的性能和可解释性。
...
Method: 提出两种知识蒸馏策略：解释性对齐蒸馏（将教师的显著性结构转移到学生网络）和深度分布蒸馏（将深度回归重新定义为离散化区间的软分类）。

Result: 相比直接训练，平均绝对误差（MAE）降低了7.97%，在nuScenes和ZJU-4DRadarCam数据集上实现了具有实时效率的竞争性精度。

Conclusion: XD-RCDepth在显著减少参数的同时保持了性能，通过知识蒸馏策略有效提升了轻量化模型的准确性和可解释性。
...
### [71] [Multi-Scale High-Resolution Logarithmic Grapher Module for Efficient Vision GNNs](https://arxiv.org/abs/2510.13740)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉图神经网络(ViG)的图构建方法(如KNN)在较大图像上计算昂贵，而SVGA等方法由于固定步长尺度会导致过度压缩和错过长距离连接的信息增益。

Method: 提出了对数可扩展图构建(LSGC)方法，并基于此构建了LogViG混合CNN-GNN模型，结合高分辨率分支和多尺度特征融合技术。
...
### [72] [Fusion Meets Diverse Conditions: A High-diversity Benchmark and Baseline for UAV-based Multimodal Object Detection with Condition Cues](https://arxiv.org/abs/2510.13620)
*Chen Chen,Kangcheng Bin,Ting Hu,Jiahao Qi,Xingyue Liu,Tianpeng Liu,Zhen Liu,Yongxiang Liu,Ping Zhong*

Main category: cs.CV

TL;DR: 提出了ATR-UMOD数据集和PCDF方法，用于解决无人机RGB-IR图像检测在复杂现实条件下的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有数据集难以充分捕捉真实世界的复杂性，特别是在不同成像条件下。需要覆盖多样化场景的数据集和适应性的多模态融合方法。

Method: 提出PCDF方法，通过文本提示编码成像条件，利用任务特定的软门控变换建模条件与多模态贡献的关系，包含条件解耦模块确保实际应用可行性。
...
### [73] [RECODE: Reasoning Through Code Generation for Visual Question Answering](https://arxiv.org/abs/2510.13756)
*Junhong Shen,Mu Cai,Bo Hu,Ameet Talwalkar,David A Ross,Cordelia Schmid,Alireza Fathi*

Main category: cs.CV

TL;DR: RECODE是一个多模态大语言模型框架，通过将视觉内容反渲染为可执行代码来实现可验证的视觉推理，在多个基准测试中显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理结构化视觉内容（如图表和图表）时缺乏精确推理能力，像素级感知缺乏验证机制。
...
### [74] [AVAR-Net: A Lightweight Audio-Visual Anomaly Recognition Framework with a Benchmark Dataset](https://arxiv.org/abs/2510.13630)
*Amjid Ali,Zulfiqar Ahmad Khan,Altaf Hussain,Muhammad Munsif,Adnan Hussain,Sung Wook Baik*

Main category: cs.CV

TL;DR: AVAR-Net是一个轻量级高效的音频-视觉异常识别框架，通过融合音频和视觉模态来提升在遮挡、低光照等挑战性条件下的识别性能，并在新提出的VAAR数据集上取得了89.29%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有异常识别方法主要依赖视觉数据，在遮挡、低光照、恶劣天气等条件下不可靠，且缺乏大规模同步音频-视觉数据集限制了多模态异常识别的发展。

Method: 使用Wav2Vec2提取音频特征，MobileViT提取视觉特征，通过早期融合机制结合两种模态，再用多阶段时序卷积网络(MTCN)学习融合表示中的长程时序依赖关系。
...
### [75] [Scaling Vision Transformers for Functional MRI with Flat Maps](https://arxiv.org/abs/2510.13768)
*Connor Lane,Daniel Z. Kaplan,Tanishq Mathew Abraham,Paul S. Scotti*

Main category: cs.CV

TL;DR: 将4D fMRI数据转换为2D活动平铺图视频，使用时空掩码自编码器训练Vision Transformers，发现性能随数据集规模呈幂律增长，在下游分类任务中表现优异。
...
Method: 将4D体素fMRI数据转换为2D fMRI活动平铺图视频，使用时空掩码自编码器(MAE)框架在Human Connectome Project的2.3K小时fMRI数据上训练Vision Transformers。

Result: 掩码fMRI建模性能随数据集规模按严格幂律缩放规律提升，下游分类基准显示模型学习到丰富表征，支持跨被试的细粒度状态解码和跨脑状态变化的被试特异性特征解码。

Conclusion: 这项工作是为fMRI数据构建基础模型的开放科学项目的一部分，证明了在fMRI数据上应用现代深度学习架构的可行性。
...
### [76] [Challenges, Advances, and Evaluation Metrics in Medical Image Enhancement: A Systematic Literature Review](https://arxiv.org/abs/2510.13638)
...
Result: MRI和多模态成像研究最多，专业模态如组织病理学、内窥镜和骨闪烁扫描研究不足。在39项研究中，29项使用传统数学方法，9项使用深度学习，1项使用混合方法。图像质量评估方面，18项研究同时使用参考和非参考指标，9项仅使用参考指标，12项仅使用非参考指标，共引入65个IQA指标，主要为非参考指标。
...
### [77] [Towards Adversarial Robustness and Uncertainty Quantification in DINOv2-based Few-Shot Anomaly Detection](https://arxiv.org/abs/2510.13643)
*Akib Mohammed Khan,Bartosz Krawczyk*

Main category: cs.CV

TL;DR: 该论文系统研究了基于DINOv2的少样本异常检测器的对抗攻击脆弱性和不确定性校准问题，提出了攻击检测方法和不确定性估计基准。
...
Method: 在AnomalyDINO基础上，通过添加轻量级线性头来生成白盒梯度攻击，同时使用Platt缩放对异常分数进行后处理以进行不确定性估计。

Result: 研究发现微小扰动可导致特征空间最近邻关系翻转，造成错误分类；原始异常分数校准性差；校准后验在对抗样本上具有更高预测熵，可用于攻击检测。

Conclusion: 对抗鲁棒性和不确定性量化是异常检测系统可信部署的必备能力，研究为稳健、不确定性感知的异常检测建立了评估协议和基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的超分辨率方法在处理多样化且严重退化的图像时，往往会产生噪声放大或错误内容生成的问题。

Method: 使用局部-全局上下文感知注意力机制保持像素关系，结合分布和感知对齐的条件机制在像素空间中增强感知保真度。

Result: 在多个超分辨率基准测试上的广泛实验表明，该方法能够生成结构一致、高保真度的重建图像。

Conclusion: 所提出的框架能够有效缓解伪影，确保真实细节恢复，在图像超分辨率任务中表现出色。
...
TL;DR: 提出了Honey-Data-15M数据集和HoneyPipe数据清理管道，训练出的Bee-8B模型在完全开源多模态大语言模型中达到新SOTA，性能可与半开源模型竞争。


<details>
  <summary>Details</summary>
Motivation: 解决完全开源多模态大语言模型因数据质量问题落后于专有模型的问题，特别是现有开源数据集存在广泛噪声和复杂推理数据缺乏的问题。

Method: 引入Honey-Data-15M数据集（约1500万QA对），采用多重清理技术和双级CoT增强策略；开发HoneyPipe数据清理管道和DataStudio框架；训练Bee-8B模型。

Result: Bee-8B在完全开源MLLMs中建立新SOTA，性能与InternVL3.5-8B等半开源模型竞争甚至超越，展示了数据质量对模型性能的关键作用。

Conclusion: 通过关注数据质量，可以开发出与半开源模型高度竞争的完全开源多模态大语言模型，为社区提供了数据集、工具链和模型权重等基础资源。
...
TL;DR: EditCast3D是一个利用视频生成基础模型进行3D编辑的管道，通过从第一帧传播编辑到整个数据集，结合视图选择策略实现高效、一致的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在图像编辑方面取得显著进展，但扩展到3D编辑仍面临计算负担重、闭源API限制和成本高等问题，需要更实用的集成方法。

Method: 使用视频生成基础模型从单帧传播编辑到整个数据集，引入视图选择策略识别一致且适合重建的视图，采用前馈重建避免昂贵细化。

Result: 在常用3D编辑数据集上评估，与最先进的3D编辑基线相比，EditCast3D展现出更优的编辑质量和更高效率。
...
Result: 在ViVerBench上提升8.3分，在T2I-ReasonBench上提升3.7分，在GenEval++上提升4.3分，优于现有的Best-of-N等并行测试时扩展方法。

Conclusion: 通过赋予多模态推理可靠的视觉验证能力，OmniVerifier推动了生成过程中的可靠反思和可扩展的测试时优化，向更可信和可控的新一代推理系统迈进了一步。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视线估计方法难以泛化到不同数据域，主要原因是标注数据稀缺和已标注数据多样性不足。

Method: 构建多样化的无标签人脸图像集合，采用伪标签策略，设计奖励模型评估伪标签可靠性，结合视觉嵌入和语义线索计算置信度分数，选择高质量伪标签进行损失计算。

Result: 在五个数据集上的实验表明，OmniGaze在域内和跨域设置下都达到了最先进的性能，并在四个未见数据集上展现出强大的零样本泛化能力。

Conclusion: OmniGaze不仅是一个有效的视线估计方法，还可作为可扩展的数据引擎，显著提升了3D视线估计的泛化能力。
...
TL;DR: 对NTIRE 2025低光照图像增强挑战赛的全面回顾，展示参赛解决方案和最终结果


<details>
  <summary>Details</summary>
Motivation: 识别在多样化挑战条件下能产生更亮、更清晰、视觉吸引力更强图像的有效网络

Method: 组织挑战赛，762名参与者注册，28个团队提交有效方案，对LLIE领域最新进展进行全面评估

Result: 展示了低光照图像增强领域的显著进展，通过竞赛形式推动了技术创新

Conclusion: NTIRE 2025 LLIE挑战赛成功促进了低光照图像增强技术的发展，证明了该领域的持续进步
...
TL;DR: 提出KnowCoL框架，通过知识引导的对比学习将图像和文本映射到共享语义空间，在开放域视觉实体识别任务中显著提升对未见实体的识别能力。
...
Method: 结合图像和文本描述，利用Wikidata的结构化信息（实体描述、类型层次、关系上下文）构建共享语义空间，通过概念级抽象实现零样本实体识别。

Result: 在OVEN基准测试中，视觉、文本和结构化知识的结合显著提升准确率，特别是对罕见和未见实体。最小模型在未见实体上的准确率比现有最优方法提升10.5%，且模型规模小35倍。

Conclusion: 知识引导的对比学习框架能有效解决开放域视觉实体识别中的挑战，通过多模态信息融合和结构化知识利用，在保持模型轻量的同时显著提升性能。
...
TL;DR: FlashWorld是一个从单张图像或文本提示快速生成3D场景的生成模型，比之前工作快10-100倍，同时具有更优的渲染质量。它采用3D导向方法直接生成3D高斯表示，并通过双模式预训练和跨模式后训练整合多视图导向方法的优势。


<details>
  <summary>Details</summary>
Motivation: 传统的多视图导向方法需要先生成多视图图像再进行3D重建，速度较慢。而直接3D导向方法虽然保证3D一致性但视觉质量较差。FlashWorld旨在结合两种方法的优势，实现快速且高质量的3D场景生成。

Method: 1) 双模式预训练：基于视频扩散模型先验，训练支持多视图导向和3D导向的双模式多视图扩散模型；2) 跨模式后训练蒸馏：通过匹配3D导向模式到高质量多视图模式的分布，提升视觉质量并减少去噪步数；3) 利用大规模单视图图像和文本提示增强泛化能力。

Result: 实验表明FlashWorld在速度和渲染质量上均优于现有方法，生成速度比之前工作快10-100倍，同时保持3D一致性和高质量的视觉输出。

Conclusion: FlashWorld通过创新的双模式训练框架成功结合了多视图导向和3D导向方法的优势，实现了快速、高质量且3D一致的场景生成，为3D内容创作提供了高效解决方案。
...
TL;DR: 提出使用去噪扩散桥模型生成医学图像的健康反事实，通过同时利用健康图像和合成病理图像作为条件，在去除病理区域的同时保留患者特定的解剖特征。


<details>
  <summary>Details</summary>
Motivation: 生成病理图像的健康反事实在医学成像中具有重要意义，可用于异常检测和为健康扫描设计的分析工具应用。现有方法难以在有效去除异常的同时保留个体解剖特征。

Method: 使用去噪扩散桥模型，不仅以健康图像为初始条件，还以对应的合成病理图像为最终条件，将病理图像作为结构信息先验来指导生成过程。

Result: DDBM在分割和异常检测任务中优于先前提出的扩散模型和全监督方法。

Conclusion: DDBM能够生成更准确匹配患者解剖结构同时选择性去除病理的健康反事实图像，在医学成像应用中具有优越性能。
...
TL;DR: 提出了Risk-adaptive Activation Steering (RAS)方法，通过重新表述查询增强对安全关键图像区域的跨模态注意力，实现查询级别的风险评估，并自适应引导激活以生成安全且有用的响应。


<details>
  <summary>Details</summary>
Motivation: 现代AI模型面临的关键挑战是在对良性查询提供有用响应的同时拒绝恶意查询。模型通常容易受到嵌入有害意图的多模态查询的攻击。现有方法要么需要大量安全数据集训练，成本高昂；要么在推理时对齐但会导致过度拒绝良性查询和推理速度下降。

Method: 重新表述查询以增强对安全关键图像区域的跨模态注意力，实现准确的风险评估。根据评估的风险自适应引导激活，生成安全且有用的响应，避免迭代输出调整带来的开销。
...
TL;DR: 该研究开发了一种基于U-Net骨架化和A*图连接的基线算法，用于从脑部血管图像中提取Willis环中心线和形态特征，在拓扑重建和特征提取方面表现出高精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Willis环是脑部重要动脉网络，传统骨架化技术难以可靠提取其复杂几何结构的中心线，且公开可用的中心线数据集稀缺，需要开发更准确的自动化分析方法。

Method: 使用细化骨架化算法从TopCoW数据集中提取中心线图和形态特征，开发结合U-Net骨架化和A*图连接的基线算法，并在测试集上评估解剖准确性和特征鲁棒性。

Result: 基线算法拓扑重建准确率极高（F1=1），节点平均欧氏距离小于一个体素，特征提取鲁棒性强（中位相对误差<5%，皮尔逊相关系数>0.95），成功预测胎儿PCA变异频率并验证分叉最优性关系。

Conclusion: 基于学习的骨架化结合图连接方法能有效提取解剖合理的中心线，强调了超越简单体素测量的重要性，已发布数据集和算法支持进一步方法开发和临床研究。
...
TL;DR: LiFMCR是一个新颖的多微透镜阵列光场相机配准数据集，提供两个高分辨率Raytrix R32光场相机的同步图像序列和Vicon运动捕捉系统记录的高精度6自由度位姿。


<details>
  <summary>Details</summary>
Motivation: 现有光场数据集仅限于单相机设置且缺乏外部真实值，无法支持多相机光场配准方法的严格评估。

Method: 提供了两种互补的配准方法：基于RANSAC的交叉视图点云3D变换估计，以及从单光场图像估计外参6自由度位姿的光场PnP算法。两种方法都明确集成了光场相机模型。

Result: 实验显示与真实值有很强的对齐性，支持可靠的多视角光场处理。

Conclusion: LiFMCR数据集及其配准方法为多相机光场配准提供了严格的评估基准，实现了准确且可扩展的多相机配准。
...
<details>
  <summary>Details</summary>
Motivation: 低场MRI虽然更便宜、更易获取且更安全，但存在分辨率低和信噪比差的问题。合成高场MRI可以减少对昂贵采集的依赖并扩展数据可用性，但现有方法在临床保真度方面存在差距，需要保持解剖保真度、增强细粒度结构细节并弥合图像对比度的领域差距。

Method: 提出了循环自监督扩散框架（CSS-Diff），核心思想是在循环一致性约束下重新制定基于扩散的合成方法。包含两个新过程：切片间差距感知网络通过对比学习对齐切片间不一致性；局部结构校正网络通过掩码和扰动补丁的自重建增强局部特征恢复。

Result: 在跨场合成任务中取得了最先进的性能（PSNR 31.80±2.70 dB，SSIM 0.943±0.102，LPIPS 0.0864±0.0689）。相比原始低场MRI，能更好地保留细粒度解剖结构（如左脑白质误差从12.1%降至2.1%，皮层从4.2%降至3.7%）。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么生成高质量单字但忽略页面级美学（如连笔和间距），要么尝试页面合成但牺牲书法正确性，需要统一的解决方案。

Method: 采用非对称加噪和栅格化框图提供空间先验，在合成、标注和未标注数据的混合集上训练，联合进行识别和生成任务。

Result: 模型在生成质量上达到最先进水平，具有优越的连笔连续性和布局保真度，同时识别能力更强，并能扩展到其他古文字。

Conclusion: UniCalli框架通过识别与生成的协同作用，在有限数据情况下提升两个任务性能，成功应用于中文书法及其他古文字。
...
TL;DR: InteractiveOmni是一个统一的开源多模态大语言模型，参数规模4B-8B，具备全面的多模态理解和语音生成能力，在轻量级模型中表现领先。


<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级但功能全面的多模态交互模型，支持音频-视觉多轮交互，为下一代智能交互系统提供可访问的开源基础。

Method: 集成视觉编码器、音频编码器、大语言模型和语音解码器；采用多阶段训练策略，包括多模态预训练和后训练；精心策划多轮训练数据集以增强长期对话能力。

Result: 在图像、音频、视频理解和语音生成任务中达到同类模型的最先进水平；InteractiveOmni-4B在通用基准测试中可与Qwen2.5-Omni-7B相媲美，且仅用50%模型大小就能保留InteractiveOmni-8B 97%的性能。

Conclusion: InteractiveOmni显著优于领先的开源模型，提供更智能的多轮音频-视觉体验，特别是在长期记忆能力方面，是下一代智能交互系统的理想基础。
...
Method: 构建包含八个推理领域的综合性基准测试，每个任务都双向耦合，要求模型利用概念理解指导视觉合成，或利用生成作为分析推理的认知支架。

Result: 通过对最先进的统一模型、仅生成模型和仅理解模型的广泛评估，揭示了显著的性能差异和跨模态依赖性。

Conclusion: Uni-MMMU为推进统一模型提供了可靠基础，揭示了视觉理解和生成能力何时以及如何相互增强的新见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决故事续写中如何有效利用先前视觉上下文并确保与当前文本输入语义对齐的核心挑战。

Method: 使用CLIP模型检索语义最匹配的先前图像，当无足够相关图像时，自适应限制先前视觉信息仅在扩散过程早期阶段产生影响。同时使用大语言模型重新标注噪声数据集以增强文本监督。

Result: 定量结果和人工评估显示，AVC在连贯性、语义一致性和视觉保真度方面优于强基线方法，特别是在先前视觉与当前输入冲突的挑战性场景中表现更佳。

Conclusion: AVC框架通过自适应视觉条件机制，能够在有益时利用视觉上下文，同时避免注入误导或无关信息，实现了更好的故事续写效果。
...
TL;DR: 提出了一种轻量级水印方案NoisePrints，利用扩散模型初始化过程的随机种子作为作者证明，无需修改生成过程或访问模型权重
...
Method: 利用扩散过程初始化随机种子与生成视觉内容的高度相关性，通过哈希函数集成到噪声采样过程中，确保从内容恢复有效种子不可行，并使用密码学零知识证明在不泄露种子的情况下证明所有权

Result: 在多个最先进的图像和视频扩散模型上验证了NoisePrints，仅需种子和输出即可高效验证，无需模型权重，且对各种操作具有鲁棒性

Conclusion: NoisePrints提供了一种实用且可扩展的版权保护方案，通过秘密保存种子增加了水印移除的难度，为扩散模型生成内容的作者身份验证提供了有效解决方案
...
TL;DR: 提出了GS-Reasoner，这是首个无需外部模块就能实现自回归3D视觉定位的3D LLM，通过双路径池化机制构建统一的3D表示，在3D空间推理任务上达到最先进性能。
...
Result: GS-Reasoner在3D视觉定位上取得显著成果，显著增强了空间推理能力，在多个基准测试中达到最先进性能。

Conclusion: GS-Reasoner建立了统一且自包含的3D空间推理框架，通过GCoT数据集进一步桥接了定位与空间推理，证明了3D视觉定位是空间推理的基石。
...
TL;DR: 提出Trajectory Field表示方法，将视频表示为连续的3D轨迹场，并开发Trace Anything模型在单次前向传播中预测整个轨迹场，在轨迹估计和点跟踪任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频中的像素在时间维度上形成连续的3D轨迹，这是理解视频动态的基本单元。基于此原理，需要一种能够有效表示和预测像素时空轨迹的方法。

Method: 提出Trajectory Field表示法，将视频映射为密集的3D轨迹函数。开发Trace Anything神经网络，为每个像素预测一组控制点来参数化B样条轨迹，从而得到任意时间点的3D位置。

Result: 在轨迹场估计新基准上达到SOTA性能，在现有点跟踪基准上表现竞争力；显著效率提升，无需迭代优化；展现出目标条件操作、运动预测和时空融合等涌现能力。

Conclusion: Trajectory Field是有效的视频表示方法，Trace Anything模型能够高效准确地预测像素轨迹，为视频理解提供了新的基础工具。
...
TL;DR: VisCoP通过向视觉编码器添加可学习的视觉探针，实现高效领域自适应，在跨视角、跨模态和跨任务设置中优于现有方法，同时保留源域知识。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在新领域面临性能显著下降问题，现有领域自适应方法要么学习有限领域特征，要么导致灾难性遗忘。

Method: VisCoP在视觉编码器中添加少量可学习视觉探针，实现高效领域自适应，最小化对预训练参数的修改。

Result: 在跨视角、跨模态和跨任务三个挑战性领域自适应设置中，VisCoP始终优于现有适应策略，在目标域获得更优性能。

Conclusion: VisCoP提供了一种有效的领域自适应方法，能够在适应新领域的同时保留源域知识，解决了现有方法的局限性。
...
TL;DR: PhysMaster通过物理表征学习和强化学习人类反馈，提升视频生成模型的物理合理性，使其能作为"世界模型"生成符合物理定律的视频。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型虽然能生成视觉逼真的视频，但往往违反物理定律，限制了其作为"世界模型"的能力。需要提升模型的物理意识。

Method: 基于图像到视频任务，设计PhysEncoder从输入图像中提取物理信息作为额外条件；应用强化学习人类反馈和直接偏好优化(DPO)来优化物理表征学习。

Result: PhysMaster在简单代理任务上证明了其提升物理意识的能力，并具有广泛的物理场景泛化性。

Conclusion: PhysMaster通过强化学习范式中的表征学习，为物理感知视频生成提供了通用且可插拔的解决方案，适用于更广泛的应用。
...
TL;DR: 提出LT-Gate脉冲神经元模型，通过双时间尺度动态和自适应门控机制解决SNN在持续学习中的稳定性-可塑性困境，在时序分类任务中显著提升精度和记忆保持能力。
...
TL;DR: 本文研究了LLM4TS模型中的伪对齐问题，发现其根源在于预训练LLM的锥效应与时间序列数据低维流形的相互作用，并提出TimeSUP技术来缓解该问题，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 伪对齐问题导致许多时间序列大语言模型表现不佳，甚至不如线性模型或随机初始化模型，但社区对此问题的原因讨论有限，需要深入探究其根源。

Method: 通过分析伪对齐与LLM锥效应的关联，提出TimeSUP技术，通过增加时间序列流形维度来匹配语言嵌入的内在维度，使模型能清晰区分时间信号同时捕捉跨模态共享结构。

Result: TimeSUP在长期预测性能上持续优于最先进的LLM4TS方法和其他轻量级基线，并能无缝集成到四种现有LLM4TS流程中，显著提升预测性能。

Conclusion: 伪对齐源于预训练LLM组件锥效应与时间序列数据低维流形的相互作用，TimeSUP通过调整流形维度有效缓解该问题，在保持各模态独特特征的同时学习其共性，显著提升模型性能。
...
TL;DR: FedGTEA是一个联邦类增量学习框架，通过高斯任务嵌入和对齐来捕获任务特定知识，在保持通信效率和隐私的同时有效缓解遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中类增量学习面临的统计异构性、遗忘问题和隐私保护挑战，需要一种既能捕获任务特定知识又能量化不确定性的可扩展方法。

Method: 客户端使用参数大小固定的Cardinality-Agnostic Task Encoder生成高斯分布的任务嵌入；服务器端使用2-Wasserstein距离测量任务间差距，并通过Wasserstein损失强制任务分离。

Result: 在多个流行数据集上的实验表明，FedGTEA实现了优越的分类性能，显著缓解了遗忘问题，并持续优于现有强基线方法。

Conclusion: FedGTEA通过概率化任务嵌入和对齐方法，在联邦类增量学习中有效平衡了性能、可扩展性和隐私保护需求。
...
TL;DR: 该论文展示了在振荡器伊辛机(OIMs)上实现平衡传播(EP)的方法，将机器学习中的优化和采样统一到单一能量景观的下降过程中，在MNIST和Fashion-MNIST数据集上取得了有竞争力的准确率，同时保持了在硬件约束下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 物理系统天然执行能量下降的特性为加速机器学习提供了直接途径。传统处理器在处理基于能量的模型(EBMs)时存在瓶颈，而振荡器伊辛机的高频动力学能够直接执行优化和梯度下降。

Method: 利用平衡传播(EP)将优化和采样过程统一到单一总能量景观的下降中，在振荡器伊辛机上实现局部学习规则，无需全局反向传播。

Result: 在MNIST数据集上达到97.2±0.1%的准确率，在Fashion-MNIST数据集上达到88.0±0.1%的准确率，在参数量化和相位噪声等实际硬件约束下保持鲁棒性。

Conclusion: 振荡器伊辛机为神经形态学习提供了快速、节能的硬件基底，基于能量的模型可以在直接执行其优化的物理硬件上实现实际应用。
...
TL;DR: 该论文证明了在强化学习中，剪枝策略可以收紧认证鲁棒性边界，而不会降低策略的鲁棒性，并在中等稀疏度水平下发现了性能-鲁棒性的最佳平衡点。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的强化学习策略需要对抗性扰动下保持可靠，而现代深度强化学习代理过度参数化导致成本高和脆弱性问题。剪枝在监督学习中已被证明能提高鲁棒性，但在对抗性强化学习中的作用尚不清楚。

Method: 开发了首个在状态对抗马尔可夫决策过程(SA-MDPs)中剪枝认证鲁棒性的理论框架，针对高斯和分类策略的Lipschitz网络，证明逐元素剪枝只能收紧认证鲁棒性边界。提出了三术语遗憾分解方法。

Result: 在连续控制基准测试中，剪枝在中等稀疏度水平下持续发现可复现的"最佳平衡点"，鲁棒性显著提高而不损害甚至有时增强干净性能。

Conclusion: 剪枝不仅是一种压缩工具，更应被视为强化学习中实现鲁棒性的结构干预手段。
...
TL;DR: 提出了一个黑盒评估框架来评估基于结构化电子健康记录（EHR）训练的基础模型中的隐私相关记忆风险，包括嵌入层和生成层的记忆探测方法。


<details>
  <summary>Details</summary>
Motivation: 基础模型在大型去标识化EHR数据上训练具有临床应用前景，但其记忆患者信息的能力引发了重要的隐私担忧，特别是对弱势群体的潜在隐私泄露风险。

Method: 开发了一套黑盒评估测试，包括在嵌入层和生成层探测记忆的方法，旨在区分模型泛化和有害记忆，并在临床相关环境中验证。

Result: 在公开可用的EHR基础模型上验证了该方法，并发布了开源工具包以促进医疗AI中可重复和协作的隐私评估。

Conclusion: 该框架为评估EHR基础模型中的隐私记忆风险提供了系统方法，有助于识别和减轻患者隐私泄露风险，特别是对弱势群体的保护。
...
TL;DR: 提出了一种新颖的多模态可解释AI框架，通过注意力增强特征融合、Grad-CAM++局部解释和揭示-修正反馈循环，在MNIST多模态扩展数据集上实现了93.2%分类准确率和78.1%解释保真度，优于单模态和非可解释基线。


<details>
  <summary>Details</summary>
Motivation: 标准基准数据集（如MNIST）常常无法暴露潜在偏见和多模态特征复杂性，限制了深度神经网络在高风险应用中的可信度。

Method: 统一了注意力增强特征融合、基于Grad-CAM++的局部解释以及用于偏见检测和缓解的揭示-修正反馈循环。

Result: 在MNIST多模态扩展数据集上实现了93.2%分类准确率、91.6% F1分数和78.1%解释保真度（IoU-XAI），优于单模态和非可解释基线。消融研究表明，将可解释性与偏见感知学习结合可增强鲁棒性和人类对齐。
...
TL;DR: 提出了一种新的拒绝推断框架CI-EX，通过离群检测识别拒绝客户分布，基于监督分类模型概率为最接近接受人群分布的拒绝个体分配标签，在保持AUC竞争力的同时显著提升拒绝推断特定指标。


<details>
  <summary>Details</summary>
Motivation: 传统拒绝推断方法假设拒绝客户行为可从接受客户外推，但两个群体可能存在分布差异，需要缓解这种盲目外推问题。

Method: CI-EX框架：迭代使用离群检测模型识别拒绝客户样本分布，基于监督分类模型概率为最接近接受人群分布的拒绝个体分配标签。

Result: 在两个大型真实信贷数据集上的实验表明，CI-EX在保持AUC竞争力的同时，在拒绝推断特定指标（如Kickout和新提出的Area under the Kickout）上持续优于现有方法。

Conclusion: 拒绝推断方法通常需要在AUC和拒绝推断特定指标之间权衡，但CI-EX框架在拒绝推断特定指标上表现优异，同时保持AUC竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LID估计方法需要多次前向传播或梯度计算，在计算和内存受限场景下应用受限。

Method: 证明LID是去噪分数匹配损失的下界，并将其作为LID估计器；同时分析隐式分数匹配损失与FLIPD估计器的关系。

Result: 在流形基准测试和Stable Diffusion 3.5上的实验表明，该方法在准确性和内存占用方面表现优异，且随问题规模和量化级别的增加保持良好性能。

Conclusion: 去噪分数匹配损失是一种具有竞争力且可扩展的LID估计器，适用于计算和内存受限的场景。
...
TL;DR: 提出了FADE度量方法，通过比较未学习模型和参考模型在生成样本上的双向似然分配来评估分布相似性，解决了现有遗忘度量方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型的遗忘度量方法基于参考响应或分类器输出进行评估，而不是评估核心目标：未学习模型是否与从未见过不需要数据的模型行为无法区分。这种参考特定的方法存在系统性盲点。

Method: 提出功能性对齐分布等价性(FADE)度量方法，通过比较未学习模型和参考模型在生成样本上的双向似然分配来测量分布相似性。

Result: 在TOFU和UnlearnCanvas基准测试中的实验表明，在传统度量上达到接近最优分数的方法未能实现分布等价性，许多方法在遗忘后反而与黄金标准更加疏远。
...
TL;DR: CSI-4CAST是一种混合深度学习架构，通过整合CNN残差、自适应校正层、ShuffleNet块和Transformer，高效捕获CSI预测中的局部和长距离依赖关系，在计算成本显著降低的情况下实现优越的预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在应对实际非高斯噪声、跨不同信道条件的泛化能力和计算效率方面存在局限性，需要开发更稳健高效的CSI预测方法。

Method: 提出混合深度学习架构CSI-4CAST，整合卷积神经网络残差、自适应校正层、ShuffleNet块和Transformer四个关键组件，并构建包含30多万样本的CSI-RRG基准数据集进行严格评估。

Result: CSI-4CAST在88.9%的TDD场景和43.8%的FDD场景中优于基线模型，计算量比最强基线LLM4CP减少5倍和3倍，同时提供了关于不同信道因素如何影响深度学习模型性能的宝贵见解。

Conclusion: CSI-4CAST在保持高预测精度的同时显著降低了计算成本，为稳健高效的CSI预测建立了标准化基准，推动了该领域的进一步研究。
...
TL;DR: 该论文介绍了ExtremBench基准数据集，用于评估LLMs在数学极值问题求解方面的推理能力，发现现有数学基准无法全面评估LLMs的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前对LLMs推理能力的来源和机制理解不足，特别是优化推理（在约束条件下寻找极值）这一基础抽象能力，该能力在规划、控制、资源分配等关键应用中至关重要。

Method: 构建ExtremBench基准数据集，包含93个标准化的极值求解问题，源自中国数学奥林匹克竞赛的不等式练习题。对多个先进开源模型家族（Qwen3、GPT-OSS、DeepSeek）进行广泛评估。

Result: 研究发现LLMs的极值求解推理能力与当前数学基准（如AIME25和MATH-500）的评估结果不一致，有些模型在通用数学推理方面表现良好但在极值求解方面较差，反之亦然。

Conclusion: 现有基准测试方法存在关键差距，可能无法全面捕捉数学推理能力的全部范围，需要更专门的评估工具来准确衡量LLMs的优化推理能力。
...
TL;DR: 提出了AMORE框架，一种自适应多输出算子网络，用于解决刚性系统的计算成本问题。该框架包含能够预测多个输出的算子以及确保可靠算子学习的自适应损失函数。


<details>
  <summary>Details</summary>
Motivation: 刚性系统的时间积分是燃烧、高超声速和其他反应输运系统中的主要计算成本来源。刚性会引入比其他物理过程小得多的时间尺度，需要在显式方案中使用极小时间步长或计算密集的隐式方法。

Method: 开发了AMORE框架，包含预测所有热化学状态的多输出算子。提出了两种自适应损失函数，考虑每个状态变量和样本的误差来惩罚损失函数。设计了自动满足单位分割的trunk网络，并提出可逆分析映射来精确执行单位质量分数约束。

Result: 通过两个示例（合成气12个状态和GRI-Mech 3.0的24个活跃状态）证明了模型的有效性和适用性。该DeepONet将成为未来CFD研究的骨干，以加速湍流燃烧模拟。

Conclusion: AMORE是一个通用框架，除了DeepONet外，还展示了其在FNO上的应用，为刚性系统的计算加速提供了有效解决方案。
...
TL;DR: 提出了一种针对单细胞扰动建模的多阶段强化学习算法，通过自然梯度更新和PPO优化来避免局部最优，提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动模型在细胞命运决策的Waddington景观中容易陷入局部最优，导致轨迹陷入虚假谱系或不合理的分化结果，需要更好的初始化方法来逃逸局部最优。

Method: 使用多阶段强化学习：首先通过Fisher向量积和共轭梯度求解器计算显式自然梯度更新，使用KL信任域约束提供安全的曲率感知第一步；然后应用带裁剪代理的近端策略优化(PPO)来细化策略。

Result: 该方法在单细胞RNA测序(scRNA-seq)和单细胞ATAC测序(scATAC-seq)扰动分析中显著提高了泛化能力。

Conclusion: 提出的多阶段强化学习算法通过精心设计的初始化有效避免了局部最优问题，为单细胞扰动建模提供了更可靠的解决方案。
...
### [114] [Machine Learning-Based Ultrasonic Weld Characterization Using Hierarchical Wave Modeling and Diffusion-Driven Distribution Alignment](https://arxiv.org/abs/2510.13023)
*Joshua R. Tempelman,Adam J. Wachtor,Eric B. Flynn*

Main category: cs.LG

TL;DR: 提出一个端到端的机器学习工作流，结合降阶建模、扩散分布对齐和U-Net分割反演，解决超声焊接检测中的数据稀缺和信号噪声问题。


<details>
  <summary>Details</summary>
Motivation: 自动化超声焊接检测面临训练数据有限和环境噪声干扰的挑战，工业环境下端到端的机器学习解决方案一直难以实现。

Method: 使用基于Lamb波理论的降阶Helmholtz模型生成数据集，通过扩散模型处理实验数据的噪声分布，最后用U-Net进行分割和反演。

Result: 该集成框架为真实数据上的自动化焊接检测提供了端到端解决方案，能够处理分布外测量数据。

Conclusion: 提出的工作流成功解决了焊接检测中的数据稀缺和噪声问题，实现了工业环境下可靠的自动化检测。
...
### [115] [Information Shapes Koopman Representation](https://arxiv.org/abs/2510.13025)
*Xiaoyuan Cheng,Wenxuan Yuan,Yiming Yang,Yuanzhao Zhang,Sibo Cheng,Yi He,Zhuo Sun*

Main category: cs.LG

TL;DR: 本文提出了一种基于信息论的Koopman学习框架，通过平衡表示学习的简洁性和表达能力来解决深度架构中有限维子空间识别的挑战。


<details>
  <summary>Details</summary>
Motivation: Koopman算子的无限维特性使得识别合适的有限维子空间具有挑战性，特别是对于深度架构。现有方法在表示学习中存在次优问题，潜在变量难以平衡表达能力和简洁性。

Method: 提出信息论拉格朗日公式，通过潜在互信息促进简洁性，通过冯·诺依曼熵维持表达能力，防止潜在空间塌陷。基于此设计新算法平衡这两种需求。

Result: 在多种动力系统上验证了方法的有效性，相比现有Koopman学习方法表现更优，学习到的表示更稳定且可解释。

Conclusion: 信息论视角为Koopman学习提供了新思路，平衡简洁性和表达能力可获得稳定且可解释的表示，在多个动力系统中展现出改进性能。
...
### [116] [Bridging Idealized and Operational Models: An Explainable AI Framework for Earth System Emulators](https://arxiv.org/abs/2510.13030)
*Pouria Behnoudfar,Charlotte Moser,Marc Bocquet,Sibo Cheng,Nan Chen*

Main category: cs.LG

TL;DR: 开发了一个可解释的AI框架，通过结合高分辨率业务模型和理想化模型的优势，构建地球系统模拟器，显著改善CMIP6中厄尔尼诺模拟的偏差。


<details>
  <summary>Details</summary>
Motivation: 高分辨率业务模型存在持续偏差，特别是在模拟极端事件和统计分布方面；而理想化模型虽然能精确校准特定特征，但不同模型之间存在学科壁垒。需要整合不同复杂度模型的互补优势。

Method: 采用重新配置的潜在数据同化技术，构建桥接模型层次结构的可解释AI框架，利用理想化模型的稀疏输出，继承业务模型的高分辨率和全面变量。

Result: 桥接模型在保持高分辨率的同时，通过理想化模型的针对性改进实现了全局精度提升，显著纠正了CMIP6中厄尔尼诺时空模式的偏差。

Conclusion: 该框架超越了黑盒修正，提供了物理洞察，为有效的物理辅助数字孪生和不确定性量化提供了计算高效的方法，强调了推进理想化模型开发和建模社区间交流的重要性。
...
### [117] [Randomness and Interpolation Improve Gradient Descent](https://arxiv.org/abs/2510.13040)
...
<details>
  <summary>Details</summary>
Motivation: 改进传统SGD优化器的性能，解决收敛速度慢和过拟合问题。

Method: IAGD利用二阶牛顿插值加速梯度下降收敛过程，假设迭代间梯度相关性；NRSGD通过向梯度添加受控噪声进行正则化防止过拟合。

Result: 在CIFAR-10和CIFAR-100数据集上的实验表明，这两种方法相比Keras中的经典优化器具有更好的性能。

Conclusion: IAGD和NRSGD是SGD的可行改进方法，证明了所提出技术的有效性。
...
### [118] [An Operational Deep Learning System for Satellite-Based High-Resolution Global Nowcasting](https://arxiv.org/abs/2510.13050)
...
TL;DR: Global MetNet是一个全球机器学习临近预报模型，利用卫星和NWP数据为全球提供12小时降水预报，在数据稀疏地区表现优于传统方法，已部署到Google Search服务数百万用户。
...
Method: 利用全球降水任务CORRA数据集、地球静止卫星数据和全球NWP数据，构建高分辨率（约5km，15分钟）的机器学习模型预测未来12小时降水。

Result: 显著优于行业标准小时预报，在数据稀疏地区表现优于美国高分辨率NWP模型，关键指标如关键成功指数和分数技能评分均有显著提升。

Conclusion: 该模型代表了减少全球预报质量差异的重要一步，能够将稀疏的高分辨率卫星观测整合到天气预报中，且能在1分钟内生成预报，适用于实时应用。
...
### [119] [Time-Varying Optimization for Streaming Data Via Temporal Weighting](https://arxiv.org/abs/2510.13052)
...
Result: 均匀权重下跟踪误差以O(1/t)速率渐近消失，折扣权重会产生由折扣因子和梯度更新次数控制的非零误差下限。

Conclusion: 理论分析揭示了不同权重策略对跟踪误差的影响，数值模拟验证了理论结果。
...
### [120] [Achieving Logarithmic Regret in KL-Regularized Zero-Sum Markov Games](https://arxiv.org/abs/2510.13060)
...
<details>
  <summary>Details</summary>
Motivation: 尽管KL正则化在强化学习和语言模型对齐中取得了实证成功，但其在博弈论环境中的理论优势仍不清楚。本文旨在填补这一理论空白。

Method: 提出了OMG算法用于矩阵博弈，基于最佳响应采样和乐观奖励；扩展到马尔可夫博弈的SOMG算法，使用最佳响应采样和超乐观奖励概念。

Result: 两种算法都实现了与T相关的对数级遗憾，该遗憾与KL正则化强度β成反比，同时保持了标准的√T遗憾。

Conclusion: KL正则化在博弈论环境中确实能提高样本效率，特别是在强正则化条件下，算法表现更好。
...
### [121] [Absolute indices for determining compactness, separability and number of clusters](https://arxiv.org/abs/2510.13065)
*Adil M. Bagirov,Ramiz M. Aliguliyev,Nargiz Sultanova,Sona Taheri*

Main category: cs.LG

TL;DR: 本文提出了新的绝对聚类指数来评估聚类的紧凑性和可分性，通过定义紧凑性函数和相邻点集来确定聚类的质量和真实聚类数量。


<details>
  <summary>Details</summary>
Motivation: 现有聚类有效性指数通常是相对的，依赖于数据结构和算法比较，难以直接确定"真实"聚类。需要开发绝对指标来直接评估聚类的紧凑性和可分性。

Method: 定义每个聚类的紧凑性函数和聚类对之间的相邻点集，利用这些函数计算聚类的紧凑性和分布紧凑性，使用相邻点集定义聚类间边界和整体分布边界。

Result: 在合成和真实数据集上的实验表明，新提出的紧凑性和可分性指数能够有效识别真实聚类数量，性能优于其他广泛使用的聚类有效性指数。

Conclusion: 提出的绝对聚类指数为评估聚类质量提供了直接有效的方法，能够准确确定聚类的紧凑性、可分性和真实聚类数量。
...
### [122] [NeuroRVQ: Multi-Scale EEG Tokenization for Generative Large Brainwave Models](https://arxiv.org/abs/2510.13068)
...
TL;DR: 提出了NeuroRVQ，一种基于码本的脑电信号大型模型，通过多尺度特征提取、分层残差向量量化和相位-幅度感知损失函数，解决了现有脑电信号分词器无法保留高频动态的问题，实现了高保真重建和更好的下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有脑电基础模型的分词模块无法保留高频动态，限制了信号重建的保真度，阻碍了通用表示学习的发展。

Method: 使用多尺度特征提取模块捕获全频段神经频谱，采用分层残差向量量化码本进行高分辨率编码，并设计了相位和幅度感知的损失函数进行高效训练。

Result: NeuroRVQ实现了更低的信号重建误差，在各种下游任务中超越了现有的大型脑电模型。

Conclusion: NeuroRVQ分词器为基于码本的通用脑电模型建立了强大的先验，推动了神经解码、生成建模和多模态生物信号集成的发展。
...
### [123] [Transformer-based Scalable Beamforming Optimization via Deep Residual Learning](https://arxiv.org/abs/2510.13077)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大规模多用户MISO系统中波束成形优化的计算复杂度和实时性挑战，需要开发能够在动态通信环境中实现实时推理的高效深度学习框架。

Method: 采用学习优化范式，使用多层Transformer通过残差连接迭代优化信道和波束成形特征，并引入三种训练策略：课程学习、半摊销学习和滑动窗口训练来提升训练效果。

Result: 在广泛仿真中，该方案在低中SNR下优于现有基线方法，在高SNR下接近WMMSE性能，同时比迭代和在线学习方法实现显著更快的推理速度。

Conclusion: 所提出的无监督深度学习框架为大规模MU-MISO系统的波束成形提供了一种高效解决方案，在性能和计算效率之间取得了良好平衡。
...
TL;DR: DeepCausalMMM是一个结合深度学习、因果推理和营销科学的Python包，通过GRU自动学习时间模式，使用DAG学习营销渠道间的因果结构，并实现基于Hill方程的饱和曲线来优化预算分配。
...
Method: 使用门控循环单元(GRU)自动学习时间模式(如广告留存效应和滞后)，通过有向无环图(DAG)学习营销渠道间的统计依赖和潜在因果结构，并实现Hill方程饱和曲线。

Result: 实现了数据驱动设计，超参数和转换从数据中学习而非手动指定；支持多区域建模；包含稳健统计方法和全面的响应曲线分析；提供14+交互式仪表板进行业务洞察。

Conclusion: DeepCausalMMM通过结合深度学习、因果推理和先进营销科学，解决了传统MMM方法的局限性，能够更好地捕捉营销活动的复杂动态和效果。
...
TL;DR: 提出稀疏三角传输映射，利用晶格图的条件独立性结构，通过单调整流神经网络在保持表达力的同时实现线性时间复杂度，解决了大晶格场论中归一化流的内存和计算瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 晶格场理论是计算物理的重要测试平台，但其玻尔兹曼分布采样面临多模态和长程相关性的挑战。归一化流虽有潜力，但在大晶格应用中受到内存需求和模型表达能力限制的制约。

Method: 提出稀疏三角传输映射框架，利用周期性边界条件下晶格图的条件独立性结构，采用单调整流神经网络实现精确稀疏性和近似稀疏性的平衡，支持站点并行评估和线性时间复杂度。

Result: 在二维φ⁴理论中验证了方法，分析了节点标记对稀疏性和性能的影响，并与混合蒙特卡洛和RealNVP等现有流方法进行了比较。

Conclusion: 稀疏三角传输映射为大规模晶格场论提供了计算高效的采样方法，在保持模型表达力的同时显著降低了计算复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 探索掩码扩散模型的计算能力和并行性限制，理解其在推理问题上的表现和效率。

Method: 通过将MDMs与链式思维（CoT）和填充循环变换器（PLTs）在有限精度对数宽度设置下建立理论联系。

Result: 证明MDMs与多项式填充PLTs等价，MDMs能解决所有CoT增强变换器能解决的问题，且在某些问题（如正则语言）上比CoT变换器更高效。

Conclusion: MDMs在并行生成方面具有优势，能够实现更快的推理，为文本生成模型提供了有前景的替代方案。
...
TL;DR: 提出CoDa-FL框架，通过聚类选择和依赖感知的任务调度来减少联邦学习在多任务移动边缘计算环境中的总完成时间。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习在移动边缘计算环境中多任务依赖场景下的客户端选择问题，旨在降低总学习时间。

Method: 使用地球移动距离进行客户端聚类，基于数据分布降低计算和通信成本；结合有向无环图的任务调度机制管理任务依赖关系。

Result: 实验验证CoDa-FL相比现有基准方法收敛更快，通信和计算成本更低，在异构MEC环境中学习精度更高。

Conclusion: CoDa-FL框架能有效优化联邦学习在多任务依赖环境中的性能，显著减少总完成时间并提高效率。
...
TL;DR: 该论文通过随机批次方法研究连续时间模型中的dropout正则化，构建了无偏估计器，建立了轨迹和分布层面的收敛性分析，并在训练中分析了最优控制和梯度下降的偏差，最终在单层神经ODE上验证了理论。
...
Method: 使用随机批次方法构建无偏估计器，在时间区间上采样神经元批次，进行轨迹收敛分析和分布稳定性分析，采用Pontryagin伴随方法分析训练偏差。

Result: 建立了轨迹层面的线性收敛速率和分布层面的总变差误差界，推导了成本-精度权衡关系，得到了最优时间步长h的闭式解，在实验中观察到预测的收敛速率和正则化效果。

Conclusion: 该方法在连续时间模型中有效实现了dropout正则化，具有理论保证和实际优势，包括良好的运行时和内存性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有程序表示方法存在困境：静态表示高效但无法洞察程序在代码变换下的行为变化，动态表示能提供性能洞察但开销大且非确定性。需要超越这种权衡的新方法。

Method: 1. 提出程序行为谱表示：用多样化优化序列探测程序IR，量化静态特征变化；2. 使用产品量化将连续反应向量离散化为结构化子词；3. 设计多任务Transformer模型PQ-BERT学习行为代码的上下文语法。
...
TL;DR: 提出了一个理论框架，用于构建完整等变图神经网络，通过两个关键组件实现：完整的标量函数（几何图的正则形式）和满秩可转向基集，显著降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的等变图神经网络通过更深层架构、增加体序或更高阶可转向特征来实现完整性，但计算成本高且缺乏多项式时间解决方案。

Method: 基于EGNN和TFN两种常见模型，提出高效算法构建完整等变GNN，包含完整的标量函数和满秩可转向基集两个关键组件。

Result: 经验结果表明，该模型在仅使用少量层数的情况下表现出优越的完整性和优秀性能，显著降低计算开销。

Conclusion: 该框架为构建完整等变图神经网络提供了理论基础和实用方法，在保持强大实用效能的同时大幅减少计算需求。
...
TL;DR: 提出跨模态互补性假说(CCH)：当教师和学生表征间的互信息超过学生表征与标签间的互信息时，跨模态知识蒸馏才有效。通过理论验证和多种模态数据的实证研究，为跨模态KD提供了理论框架和实践指导。


<details>
  <summary>Details</summary>
Motivation: 跨模态知识蒸馏技术虽然应用广泛，但缺乏理论指导，导致效果不稳定。需要建立理论框架来解释何时跨模态KD有效，以指导实践。

Method: 提出跨模态互补性假说(CCH)，在联合高斯模型中进行理论验证，并在图像、文本、视频、音频和癌症组学等多种模态数据集上进行实证研究。

Result: 理论分析和实证结果均支持CCH假说，表明当教师-学生表征互信息大于学生-标签互信息时，跨模态KD能有效提升性能。

Conclusion: 建立了跨模态知识蒸馏的理论框架，提供了基于CCH准则的实践指南，帮助选择最优教师模态来提升较弱模态的性能。
...
TL;DR: CleverCatch是一种知识引导的弱监督模型，通过将结构化领域专业知识集成到神经网络架构中，在共享嵌入空间中对齐规则和数据样本，从而改进医疗欺诈检测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 医疗欺诈检测面临标记数据有限、欺诈手段不断演变和医疗记录高维度的挑战。传统监督方法受限于极端标签稀缺，而纯无监督方法往往无法捕捉临床意义的异常。

Method: 在神经网络架构中集成结构化领域专业知识，在共享嵌入空间中对齐规则和数据样本。通过在代表合规和违规的合成数据上联合训练编码器，学习能够泛化到复杂真实数据集的软规则嵌入。

Result: 在大规模真实数据集上的实验表明，CleverCatch在四个最先进的异常检测基线方法上表现更优，AUC平均提高1.3%，召回率平均提高3.4%。消融研究进一步验证了专家规则的补充作用。
...
TL;DR: 比较Ising(-1,+1)和QUBO(0,1)编码在玻尔兹曼机学习中的表现，发现QUBO编码会导致Fisher信息矩阵(FIM)条件数较差，在随机梯度下降(SGD)下收敛较慢，而Ising编码提供更各向同性的曲率并加速收敛。


<details>
  <summary>Details</summary>
Motivation: 研究不同变量编码（Ising vs QUBO）如何影响玻尔兹曼机学习的信息几何和收敛动态，为变量编码和预处理提供实践指导。

Method: 在控制模型、采样器和步长的协议下，利用Fisher信息矩阵等于充分统计量协方差的特性，可视化模型样本的经验矩，分析两种编码的系统性差异。

Result: QUBO编码在FIM中产生更多小特征值方向，降低谱熵，导致SGD收敛较慢；而自然梯度下降(NGD)由于重参数化不变性，在不同编码下收敛相似。

Conclusion: 对于基于SGD的训练，Ising编码提供更快的收敛；对于QUBO编码，中心化/缩放或NGD式预处理可以缓解曲率病态问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常使用外部奖励模型或现成LLM预处理训练数据来识别有价值的偏好对，但很少检查单个数据点是否真正有益。需要开发能适应特定模型的数据质量评估方法。

Method: 提出截断影响函数(TIF)来评估数据对验证数据的影响，开发两种与TIF正相关但计算更简单的评分函数，并通过组合这些函数来抵消各自的误差源。

Result: 实验表明，使用更少的数据就能实现更好的对齐性能，证明了所提方法的有效性和通用性。

Conclusion: 偏好数据质量是模型相关的，需要开发适应特定模型的数据选择方法，所提出的数据选择规则能更精确地选择有价值的偏好数据。
...
TL;DR: FracNet是一个基于频谱分析的图神经网络，通过分解图的高频和低频分量来改进领域自适应，结合对比学习解决模糊边界问题。


<details>
  <summary>Details</summary>
Motivation: 传统GNN在领域自适应中表现不佳，主要因为未能区分处理全局和局部模式，且对结构分布变化和可迁移模式探索不足。

Method: 提出FracNet，包含两个协同模块：将原始图分解为高频和低频分量，并进行频率感知的领域自适应，同时集成对比学习框架。

Result: 大量实验表明FracNet相比最先进方法有显著改进。

Conclusion: 通过频谱分析和对比学习的结合，FracNet有效解决了图神经网络在领域自适应中的挑战，并有理论证明其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视角感知分类方法在参数效率方面存在瓶颈，这个问题在现有研究中未得到足够重视

Method: 采用超网络+适配器的组合架构，该方案与架构无关，可应用于多种基础模型

Result: 在仇恨言论和毒性检测任务上能够与专用模型竞争，同时使用更少的参数
...
TL;DR: BlendFL是一个新颖的联邦学习框架，融合了横向和纵向联邦学习的优势，能够处理多模态数据异构性，支持客户端根据本地数据情况灵活选择训练方式，并采用去中心化推理机制。


<details>
  <summary>Details</summary>
Motivation: 解决现实环境中多模态数据异构性问题，现有联邦学习框架在非理想设置下效果有限，无法处理客户端间模态和样本分布不均的情况。
...
TL;DR: 提出了MERA框架，通过选择性、自适应的干预来引导语言模型减少错误，优化干预方向和校准干预时机与强度，相比现有方法能更安全有效地进行错误纠正。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定、手动调整的引导强度，往往导致引导不足或过度引导，需要更智能的干预机制来避免这些问题。

Method: MERA框架通过优化干预方向和校准干预时机与强度，在无法进行自信修正时选择弃权，从而可证明地提升性能。

Result: 在多个数据集和语言模型家族上的实验表明，MERA能够安全、有效地进行非降级的错误纠正，并且优于现有基线方法。

Conclusion: MERA可作为通用高效的机制化激活引导方法，能够应用于现有引导技术之上进一步提升性能。
...
TL;DR: 提出了Fed-CCP方法，通过生成模型实现联邦条件保形预测，解决联邦学习中数据分布异构问题，提供更自适应的预测集。
...
Conclusion: Fed-CCP通过生成模型有效解决了联邦学习中的条件覆盖问题，在保持数据隐私的同时提供了更准确的预测不确定性量化。
...
<details>
  <summary>Details</summary>
Motivation: 生成扩散模型在动态降尺度任务中缺乏有限样本保证，导致过度自信预测和校准不良的不确定性估计，限制了其在操作环境中的可靠性。

Method: 在降尺度流程中增强保形预测框架，对扩散模型的样本进行后处理得到条件分位数估计，然后通过保形化分位数回归程序构建局部自适应预测区间。

Result: 在意大利ERA5再分析数据上的实验表明，该方法相对于扩散模型基线显著改善了网格点级不确定性估计的覆盖率和稳定性概率评分。
...
TL;DR: 提出了ISER（基于隔离的球形集成表示）方法，通过使用超球半径作为局部密度特征的代理，扩展了现有的基于隔离的异常检测方法，同时保持线性时间和常数空间复杂度。
...
Method: 构建集成表示，其中超球半径编码密度信息：较小半径表示密集区域，较大半径对应稀疏区域。引入基于相似性的评分方法，通过比较集成表示与理论异常参考模式来测量模式一致性。

Result: 在22个真实世界数据集上的综合实验表明，ISER在11个基线方法中表现出优越性能。

Conclusion: ISER通过结合隔离方法和密度特征，有效解决了现有异常检测方法的局限性，并在多个数据集上验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习在信息物理系统中的普及，需要将训练从云端转移到设备端以解决隐私和延迟问题，但超低功耗微控制器的有限计算资源使训练变得困难。

Method: 设计分布式学习方法，利用多个设备进行分布式训练，结合专门的计算高效分类器和优化的无线多跳通信协议，减少通信开销。

Result: 在20个超低功耗设备的测试平台上，RockNet成功从零开始学习时间序列分类任务，准确率比最新的神经网络微控制器训练方法提高最多2倍，分布式架构在扩展到20个设备时，每个设备的内存、延迟和能耗降低达90%。
...
TL;DR: 本文研究在战略分类中引入弃权机制的影响，表明最优弃权策略能确保决策者的效用不劣于非弃权设置，并能作为操纵威慑，使不合格代理人的操纵成本更高。


<details>
  <summary>Details</summary>
Motivation: 算法决策日益普及但易受战略操纵，已有研究表明分类器弃权能显著提高准确性，本文旨在探索在战略分类背景下引入弃权如何影响战略代理人的反应以及决策者应如何最优利用弃权。

Method: 使用Stackelberg博弈模型，决策者作为分类器首先宣布决策策略，战略代理人随后操纵可观察特征以获得期望结果，重点关注二元分类器。

Result: 最优弃权确保决策者效用不劣于非弃权设置，弃权不仅能提高准确性，还能作为操纵威慑，使操纵成本足够高时，特别是对不合格代理人，获得积极结果的操纵成本更高。

Conclusion: 弃权是减少算法决策系统中战略行为负面影响的宝贵工具，能改善准确性并威慑操纵行为。
...
Method: 基于Thompson采样的变分公式，通过微调提示条件大型语言模型直接参数化候选解获得最大奖励的概率，逐步适应后验分布。

Result: 在FAQ响应优化、热稳定蛋白搜索和量子电路设计三个任务上验证，在线微调显著提高样本效率，计算效率影响可忽略。

Conclusion: ToSFiT方法通过谨慎适应后验概率实现了理论保证和实际效果的平衡，为离散空间优化提供了可扩展的解决方案。
...
TL;DR: 提出了一种新的相似性度量方法——邻近核，用于直接计算不完整数据在核特征空间中的相似性，无需在原始空间中进行显式插补。
...
Result: 在12个真实世界不完整数据集上进行聚类任务，相比现有方法表现出更优越的性能，同时保持线性时间复杂度。
...
TL;DR: 提出Generalist框架，通过将整体泛化目标分解为多个子任务，每个任务由专门的基学习器负责，最后通过参数插值形成全局学习器，缓解了对抗训练中自然精度下降和跨攻击鲁棒性差的问题。
...
TL;DR: 本文研究表明transformer可以在在线无模型强化学习中作为连续控制的强基线，通过探索关键设计问题如输入条件、组件共享和序列数据处理，提供了实用的架构和训练策略。


<details>
  <summary>Details</summary>
Motivation: 尽管transformer在离线或基于模型的强化学习中很有效，但在在线无模型强化学习中仍未被充分探索，主要是因为其对训练设置和模型设计决策的敏感性。

Method: 研究关键设计问题：如何条件化输入、在actor和critic之间共享组件、以及如何切片序列数据用于训练。通过实验探索稳定的架构和训练策略。

Result: 实验结果显示这些设计策略能够在完全和部分可观测任务中实现有竞争力的性能，适用于向量和图像两种设置。

Conclusion: 这些发现为在在线强化学习中应用transformer提供了实用的指导，表明transformer可以成为连续控制任务的强基线。
...
TL;DR: 提出了一种结合对比学习的依赖建模和异常检测方法，通过构建依赖图、提取时空特征，并使用对比学习框架增强正常与异常模式的可分离性，在云服务环境中实现稳定可靠的异常检测。
...
Method: 将服务交互抽象为依赖图，通过嵌入函数提取时空特征，使用图卷积聚合邻域信息，引入对比学习框架构建正负样本对，并设计时间一致性约束来保持表示稳定性。
...
TL;DR: 提出了一种基于预测市场的框架，通过考虑历史表现、适应时变条件并允许自由进出的机制，解决数据所有权和竞争利益约束下的预测协作问题。


<details>
  <summary>Details</summary>
Motivation: 尽管数据可用性和准确预测需求增加，但利益相关者之间的协作常受数据所有权和竞争利益限制。现有合作博弈框架存在局限性，需要更通用的解决方案。

Method: 采用预测市场框架，使用稳健回归模型学习最优预测组合并处理缺失提交，引入考虑样本内外表现的收益分配机制，满足多种经济属性。

Result: 通过模拟和真实数据案例研究，证明了所提出市场设计的有效性和适应性。

Conclusion: 该预测市场设计能够有效促进数据受限环境下的预测协作，同时保证经济合理性和灵活性。
...
TL;DR: 提出基于图神经网络(GNN)的新方法，用于近似计算网络流博弈中的Banzhaf值，解决了传统方法计算复杂度高和无法跨网络配置迁移知识的问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在超过约20个代理人的系统中计算Banzhaf值不可行，蒙特卡洛采样方法样本复杂度高且无法在不同网络配置间迁移知识，限制了大规模或动态系统的应用。

Method: 将问题构建为图级预测任务，使用图神经网络(GNN)直接从网络拓扑和控制结构中学习代理人影响力的通用模式。比较了三种最先进的GNN架构：图注意力网络(GAT)、带边特征的图同构网络(GINE)和EdgeConv。

Result: 训练后的GNN模型实现了高保真度的Banzhaf值近似，相比精确方法和基于采样的方法实现了数量级的速度提升。最重要的是，模型表现出强大的零样本泛化能力，能在不同结构特性的新网络上准确预测Banzhaf值而无需重新训练。
...
### [151] [Assessing the robustness of heterogeneous treatment effects in survival analysis under informative censoring](https://arxiv.org/abs/2510.13397)
*Yuxin Wang,Dennis Frauen,Jonas Schweisthal,Maresa Schröder,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 提出了一个假设宽松的框架来评估生存分析中条件平均治疗效果估计在面临删失偏差时的稳健性，使用部分识别方法推导CATE的信息边界，并开发了一个新的元学习器来估计这些边界。


<details>
  <summary>Details</summary>
Motivation: 临床试验中高达一半的患者因副作用或其他原因提前退出，当退出是信息性时（即依赖于生存时间），会引入删失偏差，导致治疗效果估计产生偏差。现有方法依赖强假设如非信息性删失来获得点估计。

Method: 使用部分识别方法推导条件平均治疗效果的信息边界，开发了一个新的元学习器，能够使用任意机器学习模型估计边界，具有双重稳健性和准oracle效率等理论性质。
...
### [152] [Rectify and Align GPS Points to Parking Spots via Rank-1 Constraint](https://arxiv.org/abs/2510.13439)
*Jiaxing Deng,Junbiao Pang,Zhicheng Wang,Haitao Yu*

Main category: cs.LG

TL;DR: 提出一种无监督的低秩方法来校正停车位GPS点的误差，并将其与道路对齐，解决了高层建筑导致的GPS漂移问题。


<details>
  <summary>Details</summary>
Motivation: 高层建筑会导致停车位GPS点从实际位置漂移，且低成本GPS设备本身存在定位误差，需要无监督地校正大量停车位中的错误GPS点。

Method: 基于停车位与道路平行的物理约束，提出无监督低秩方法，在统一框架中校正GPS误差并将其与停车位对齐。

Result: 该方法简单有效，能处理任何类型的GPS点误差，实验证明了其在解决实际问题中的优越性。

Conclusion: 提出的非常规校正和对齐方法为解决停车位GPS定位问题提供了有效的无监督解决方案。
...
### [153] [SWIR-LightFusion: Multi-spectral Semantic Fusion of Synthetic SWIR with {Thermal} IR {(LWIR/MWIR)} and RGB](https://arxiv.org/abs/2510.13404)
*Muhammad Ishfaq Hussain,Ma Van Linh,Zubia Naz,Unse Fatima,Yeongmin Ko,Moongu Jeon*

Main category: cs.LG

TL;DR: 提出了一种从LWIR数据合成SWIR图像的方法，并开发了多模态融合框架，整合合成SWIR、LWIR和RGB数据，在恶劣能见度条件下显著提升场景理解能力。


<details>
  <summary>Details</summary>
Motivation: 传统RGB和热红外融合在恶劣天气和光照条件下难以提供全面的场景信息，而SWIR成像虽能穿透大气干扰但缺乏公开数据集，限制了其发展。

Method: 使用对比度增强技术从LWIR数据合成SWIR图像，构建多模态融合框架，采用优化的编码器-解码器神经网络架构，包含模态特定编码器和softmax门控融合头。

Result: 在多个公开基准测试和私有真实数据集上的实验表明，合成SWIR增强的融合框架提升了融合图像质量（对比度、边缘清晰度、结构保真度），同时保持实时性能。
...
### [154] [Neural Sum-of-Squares: Certifying the Nonnegativity of Polynomials with Transformers](https://arxiv.org/abs/2510.13444)
*Nico Pelleriti,Christoph Spiegel,Shiwei Liu,David Martínez-Rubio,Max Zimmer,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 提出首个学习增强算法来验证多项式的平方和(SOS)性质，通过Transformer模型预测几乎最小的单项式基，大幅减少相应半定规划的规模，实现100倍以上的加速。


<details>
  <summary>Details</summary>
Motivation: 验证多项式非负性是NP难问题，平方和(SOS)性质是充分条件，但现有方法计算成本高，半定规划维度随单项式基大小呈二次增长，需要减少基大小的方法。
...
Conclusion: 该方法为提升SOS编程的实际可扩展性提供了新的见解，通过机器学习大幅提高了SOS验证的效率。
...
### [155] [Optimizing Storage Overhead of User Behavior Log for ML-embedded Mobile Apps](https://arxiv.org/abs/2510.13405)
*Chen Gong,Yan Zhuang,Zhenzhe Zheng,Yiliu Chen,Sheng Wang,Fan Wu,Guihai Chen*

Main category: cs.LG

TL;DR: AdaLog是一个轻量级自适应系统，通过消除用户行为日志中的冗余数据和稀疏存储问题，显著提升移动应用中机器学习模型的存储效率，同时保持模型推理精度和延迟。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在移动应用中的普及，记录用户行为数据带来了巨大的存储成本，导致系统响应性下降和更多应用卸载。现有工业实践存在两个关键低效问题：跨不同特征和模型的重叠行为数据冗余记录，以及异构属性描述导致的稀疏存储。

Method: AdaLog首先将特征级冗余数据消除建模为超图中的最大加权匹配问题，并提出分层算法用于设备端高效部署。然后采用虚拟哈希属性设计将异构行为分布到少量物理存储密集的日志文件中。最后设计增量更新机制以适应动态用户行为模式，最小化I/O操作。

Result: 在真实用户数据上的评估显示，AdaLog将行为日志大小减少了19%到44%，系统开销极小（仅2秒延迟和15MB内存使用）。

Conclusion: AdaLog为设备端机器学习的更广泛采用提供了更高效的数据基础，有效解决了移动应用中用户行为日志存储的瓶颈问题。
...
### [156] [DistilCLIP-EEG: Enhancing Epileptic Seizure Detection Through Multi-modal Learning and Knowledge Distillation](https://arxiv.org/abs/2510.13497)
*Zexin Wang,Lin Shi,Haoyu Wu,Junru Luo,Xiangzeng Kong,Jun Qi*

Main category: cs.LG

TL;DR: 提出DistilCLIP-EEG多模态模型，整合EEG信号和文本描述进行癫痫检测，通过知识蒸馏训练轻量级学生模型，在多个数据集上准确率超过97%，模型参数减少41.9%。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法仅依赖单模态EEG信号，忽略了多模态信息的潜在优势，需要开发整合EEG和文本的多模态癫痫检测方法。

Method: 基于CLIP框架构建DistilCLIP-EEG模型，使用Conformer架构的EEG编码器和可学习BERT文本编码器，在共享潜在空间进行跨模态表示学习，并通过知识蒸馏训练轻量学生模型。

Result: 在TUSZ、AUBMC和CHB-MIT数据集上，师生模型准确率均超过97%，F1分数均高于0.94，学生模型参数和大小仅为教师模型的58.1%。
...
### [157] [When Embedding Models Meet: Procrustes Bounds and Applications](https://arxiv.org/abs/2510.13406)
*Lucas Maystre,Alvaro Ortega Gonzalez,Charles Park,Rares Dolga,Tudor Berariu,Yu Zhao,Kamil Ciosek*

Main category: cs.LG

TL;DR: 该论文研究了如何通过正交变换对齐两个嵌入模型，提出了基于Procrustes后处理的简单对齐方法，使不同嵌入模型能够互操作，并在模型重训练、文本检索和多模态搜索等应用中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 分别训练的相似数据嵌入模型虽然编码了稳定信息，但缺乏直接互操作性，这给模型重训练、部分模型升级和多模态搜索等实际应用带来了挑战。

Method: 证明如果两套嵌入的成对点积近似保持，则存在等距变换能紧密对齐它们，并提供了对齐误差的紧界。基于此提出Procrustes后处理方法，在保持每个嵌入空间几何结构的同时实现互操作性。

Result: 在三个应用中验证了方法的有效性：跨重训练的兼容性维护、不同文本检索模型的组合、以及混合模态搜索，其中在多模态搜索中达到了最先进的性能。

Conclusion: 通过正交变换对齐嵌入模型是可行的，Procrustes后处理提供了一种简单有效的方法来实现嵌入模型的互操作性，同时保持各自的几何结构。
...
### [158] [Offline and Online KL-Regularized RLHF under Differential Privacy](https://arxiv.org/abs/2510.13512)
*Yulian Wu,Rushil Thareja,Praneeth Vepakomma,Francesco Orabona*

Main category: cs.LG

TL;DR: 本文研究了在局部差分隐私（LDP）模型下，基于KL正则化的人类反馈强化学习（RLHF）的离线和在线设置，提出了相应的算法并分析了其理论性能。


<details>
  <summary>Details</summary>
Motivation: 研究在保护人类偏好标签隐私的前提下，如何有效进行RLHF学习，解决大语言模型对齐中的隐私保护问题。

Method: 在离线设置中采用悲观主义原则设计算法，在在线设置中采用乐观主义原则设计算法，并在KL正则化目标下进行理论分析。
...
Conclusion: 首次在LDP约束下对KL正则化RLHF进行了理论分析，提出的算法在隐私保护和性能之间取得了良好平衡，并为非隐私情况下的RLHF提供了首个分析框架。
...
TL;DR: 提出了一种改进的物理信息神经网络框架，应用于肿瘤学领域，能够从稀疏的时间点数据中学习组合疗法在肿瘤微环境中的时变相互作用。


<details>
  <summary>Details</summary>
Motivation: 在肿瘤学中，实验数据通常稀疏且仅包含少量时间点的肿瘤体积数据。通过嵌入从动力学系统先验信息导出的归纳偏置，扩展PINN框架以应对数据稀缺问题。

Method: 改进物理信息神经网络，将观测到的生物约束作为正则化项，通过嵌入微分方程建模的动力学系统规律到损失函数中，学习ODE模型参数和时变形式。

Result: 算法能够从少量训练样本中学习到合理的解，并在间歇性治疗组合疗法的ODE模型中表现出良好的泛化能力，通过MSE、MAE和MAPE等指标显示出强收敛性。

Conclusion: 改进的PINN框架在肿瘤学应用中能够有效处理稀疏数据，学习治疗动态和时变参数，为组合疗法研究提供了有力工具。
...
### [160] [Hybrid Interval Type-2 Mamdani-TSK Fuzzy System for Regression Analysis](https://arxiv.org/abs/2510.13437)
*Ashish Bhatia,Renato Cordeiro de Amorim,Vito De Feo*

Main category: cs.LG

TL;DR: 提出了一种结合Mamdani系统可解释性和TSK模型精度的新型模糊回归方法，通过混合规则结构和双重优势类型，在保持可解释性的同时提高预测精度。
...
Result: 在6个基准数据集测试中，该方法在4个数据集上获得最佳模糊方法得分，在2个数据集上优于不透明模型，在1个数据集上获得最佳总体得分，RMSE改进范围为0.4%至19%。
...
### [161] [K-Merge: Online Continual Merging of Adapters for On-device Large Language Models](https://arxiv.org/abs/2510.13537)
*Donald Shenaj,Ondrej Bohdal,Taha Ceritli,Mete Ozay,Pietro Zanuttigh,Umberto Michieli*

Main category: cs.LG

TL;DR: 提出一种数据无关且计算高效的策略，用于在设备只能存储有限数量适配器的情况下，选择并合并新LoRA适配器，实现在线持续合并。


<details>
  <summary>Details</summary>
Motivation: 解决移动设备上大语言模型部署时，LoRA适配器增量交付带来的挑战：如何在存储限制下合并新LoRA同时保持先前任务性能。

Method: 数据无关的计算高效策略，在存储预算内选择和合并LoRA适配器，支持在线持续合并场景。

Result: 在真实世界任务上的广泛实验表明，该方法优于替代策略，同时满足设备存储预算和计算限制。

Conclusion: 提出的方法有效解决了设备端在线持续合并LoRA的挑战，在资源受限环境下保持任务性能。
...
### [162] [$L_2$-Regularized Empirical Risk Minimization Guarantees Small Smooth Calibration Error](https://arxiv.org/abs/2510.13450)
...
<details>
  <summary>Details</summary>
Motivation: 概率预测的校准对于可靠的机器学习至关重要，但标准训练程序如何产生良好校准模型的理解仍很有限。

Method: 建立了基于优化误差、正则化强度和Rademacher复杂度的平滑校准误差有限样本泛化界，并在再生核希尔伯特空间中具体实例化了核岭回归和逻辑回归的保证。

Result: 实验验证了理论保证，表明L2正则化ERM可以在不使用boosting或后处理重新校准的情况下提供良好校准的模型。
...
### [163] [Message Passing on the Edge: Towards Scalable and Expressive GNNs](https://arxiv.org/abs/2510.13615)
*Pablo Barceló,Fabian Jogl,Alexander Kozachinskiy,Matthias Lanzinger,Stefan Neumann,Cristóbal Rojas*

Main category: cs.LG

TL;DR: 提出了EB-1WL边缘着色细化测试和EB-GNN架构，该架构在消息传递中显式使用三角形，比1-WL更具表达能力，且在实用图学习任务中保持近线性时间和内存复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有GNN架构在表达能力上存在局限，特别是1-WL测试的表达能力有限，而之前提出的更具表达能力的架构往往计算复杂度较高。

Method: 基于Chiba和Nishizeki的经典三角形计数算法，在消息传递过程中显式利用三角形结构，提出EB-1WL测试和相应的EB-GNN架构。

Result: EB-1WL比1-WL显著更具表达能力，提供了基于一阶逻辑的完整逻辑特征描述和基于同态计数的区分性结果；EB-GNN在保持高效的同时，性能优于简单MPNN，并与任务专用GNN竞争。

Conclusion: EB-GNN是一个高效通用的图神经网络架构，在保持计算效率的同时显著提升了表达能力，为图学习任务提供了实用的解决方案。
...
### [164] [Towards Blackwell Optimality: Bellman Optimality Is All You Can Get](https://arxiv.org/abs/2510.13476)
*Victor Boone,Adrienne Tuynman*

Main category: cs.LG

TL;DR: 本文研究了马尔可夫决策过程中偏差最优性层次结构的策略识别问题，构建了具有渐近误差的学习算法，并确定了能够在有限时间内停止识别的MDP类别。
...
Method: 为每个最优性阶次构建学习算法，分析MDP类别特征，并提供可处理的停止规则。

Result: 识别了具有唯一Bellman最优策略的MDP类别，该类别不依赖于最优性阶次，并开发了在可能时在有限时间内触发的停止规则。

Conclusion: 提出的学习算法具有渐近误差消失特性，停止规则能够在适当条件下保证有限时间收敛。
...
### [165] [Tahakom LLM guidelines and receipts: from pre-training data to an Arabic LLM](https://arxiv.org/abs/2510.13481)
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言处理领域取得了显著进展，但开发阿拉伯语大语言模型面临独特挑战，需要专门的方法来解决数据、分词和评估等问题。

Method: 采用系统化方法：收集和过滤阿拉伯语预训练数据集，评估不同分词器设计对模型性能的影响，分析现有阿拉伯语评估框架的局限性并提出修正方法。

Result: 开发了专门针对阿拉伯语的数据整理和分词器设计方法，提出了改进的评估框架，并公开分享了数据和方桯以促进协作开发。

Conclusion: 通过系统化解决阿拉伯语大语言模型开发中的关键挑战，为阿拉伯语语言建模的进步做出了贡献，并促进了该领域的透明度和协作发展。
...
### [166] [Time Series Foundation Models: Benchmarking Challenges and Requirements](https://arxiv.org/abs/2510.13654)
...
<details>
  <summary>Details</summary>
Motivation: 随着时间序列基础模型的发展，其零样本预测能力需要可靠的评估方法。然而，现有评估存在数据完整性、基准数据集代表性和信息泄露等多重挑战，需要建立稳健的评估方法学。

Method: 通过调查现有TSFM评估实践，识别出数据分区混淆、基准数据集选择、时空评估缺失等关键问题，并分析信息泄露和外部冲击记忆的风险。

Result: 研究发现现有评估存在广泛的数据分区混淆问题，导致性能估计膨胀和全局知识错误传递到局部时间序列。评估方法存在多重缺陷，影响TSFM评估的完整性。

Conclusion: 呼吁研究社区开发稳健的评估方法学，设计基于真正样本外未来数据的评估方法，以保障TSFM评估的完整性，避免重蹈LLM和传统时间序列基准测试的覆辙。
...
### [167] [Axial Neural Networks for Dimension-Free Foundation Models](https://arxiv.org/abs/2510.13665)
*Hyunsu Kim,Jonggeon Park,Joan Bruna,Hongseok Yang,Juho Lee*

Main category: cs.LG

TL;DR: 提出了一种维度无关的神经网络架构XNN，用于解决物理数据中不同维度系统的训练挑战，在多个PDE训练场景中表现优异且具有更好的维度泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基础模型在AI领域的出现显著推进了通用学习，但在物理数据（如偏微分方程解）上的训练面临维度变化的独特挑战。传统方法要么固定最大维度，要么使用不同维度的单独编码器，导致效率低下。

Method: 提出轴向神经网络（XNN），灵感来自参数共享结构如Deep Sets和图神经网络。XNN能够泛化到不同张量维度，同时保持计算效率。将现有PDE基础模型转换为轴向神经网络，并在三种训练场景下评估：从头训练、多PDE预训练、单PDE微调。

Result: 实验表明XNN与原始模型表现相当，并在未见维度上展现出更优越的泛化能力。

Conclusion: XNN解决了物理基础模型中的维度变化问题，多维预训练对基础模型具有重要意义。
...
### [168] [ProtoTopic: Prototypical Network for Few-Shot Medical Topic Modeling](https://arxiv.org/abs/2510.13542)
...
Result: 与文献中使用的两种主题建模基线方法相比，ProtoTopic在主题连贯性和多样性方面表现更好。

Conclusion: ProtoTopic能够在数据有限的情况下生成医学相关主题，证明了原型网络在主题建模中的有效性。
...
TL;DR: 该论文提出使用单纯形嵌入来加速强化学习训练，通过几何归纳偏置产生稀疏离散特征，提高样本效率和最终性能，且不损失运行速度。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模环境并行化方法虽然能加速训练，但仍需要大量环境交互。结构良好的表示可以改善深度强化学习的泛化能力和样本效率。

Method: 提出单纯形嵌入方法，这是一种轻量级表示层，将嵌入约束在单纯形结构中，产生稀疏离散特征来稳定评论家引导和增强策略梯度。

Result: 在FastTD3、FastSAC和PPO算法上应用单纯形嵌入，在各种连续和离散控制环境中一致提高了样本效率和最终性能。

Conclusion: 单纯形嵌入是一种有效的几何归纳偏置方法，能够在不损失运行速度的情况下显著提升强化学习算法的性能。
...
TL;DR: 该论文提出了多目标在线凸优化问题，考虑K个不同的损失函数序列，采用最小最大遗憾作为性能指标，并在i.i.d.输入设置下提出了结合Hedge和在线梯度下降的简单算法，实现了O(√(T log K))的期望最小最大遗憾。
...
Method: 在i.i.d.输入设置下，提出了一种结合Hedge算法和在线梯度下降(OGD)的简单算法来处理多目标在线凸优化问题。

Result: 通过简洁的证明，表明所提算法的期望最小最大遗憾为O(√(T log K))。

Conclusion: 该工作成功地将在线凸优化扩展到多目标设置，并提供了具有理论保证的有效算法来解决最小最大遗憾问题。
...
TL;DR: 提出了DOLFIN方法，结合Vision Transformers和低秩适配器，在联邦持续学习中高效稳定地学习新任务，同时通过DualGPM防止遗忘。


<details>
  <summary>Details</summary>
Motivation: 联邦持续学习需要在多个分布式客户端上学习新任务，同时保护隐私且不遗忘先前知识，但现有方法在性能、隐私保护和通信效率方面存在平衡挑战。

Method: 使用低秩适配器(LoRA)减少通信开销，结合DualGradient Projection Memory(DualGPM)防止遗忘，在联邦环境中实现高效稳定的增量学习。

Result: 在CIFAR-100、ImageNet-R、ImageNet-A和CUB-200数据集上，DOLFIN在两种Dirichlet异构设置下始终超越六个强基线方法，在最终平均准确率上表现更优，同时保持相同的内存占用。
...
### [172] [Selective Adversarial Attacks on LLM Benchmarks](https://arxiv.org/abs/2510.13570)
*Ivan Dubrovsky,Anastasia Orlova,Illarion Iov,Nina Gubina,Irena Gureeva,Alexey Zaytsev*

Main category: cs.LG

TL;DR: 该论文研究了选择性对抗攻击对LLM基准测试的影响，发现即使微小的文本扰动也能显著改变模型在MMLU基准上的相对排名，挑战了排行榜评估的公平性和可重复性。
...
Method: 使用TextAttack框架中的标准攻击方法，开发选择性评估协议，创建自定义约束来提高攻击选择性，并提出了基于代理LLM的选择性扰动生成流程。

Result: 实证研究发现选择性对抗攻击确实存在，能够实质性改变模型相对排名，表明基准测试对微小编辑很敏感。

Conclusion: 研究结果强调了在LLM评估中需要扰动感知报告和鲁棒性诊断，证明即使是细微的编辑也会影响比较判断，挑战了排行榜驱动评估的可靠性。
...
### [173] [ArtNet: Hierarchical Clustering-Based Artificial Netlist Generator for ML and DTCO Application](https://arxiv.org/abs/2510.13582)
*Andrew B. Kahng. Seokhyeong Kang,Seonghyeon Park,Dooseok Yoon*

Main category: cs.LG

TL;DR: ArtNet是一种新颖的人工网表生成器，通过生成具有关键拓扑特征的多样化训练数据，解决机器学习模型泛化能力不足和设计技术协同优化中数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 在先进工艺节点中，功耗、性能和面积优化变得高度复杂。机器学习和设计技术协同优化面临训练数据缺乏多样性以及设计流程周转时间长的限制。

Method: 提出ArtNet人工网表生成器，能够复制关键拓扑特征，生成更接近目标参数的逼真人工数据集，支持更广泛的设计空间探索。

Result: 在基于CNN的DRV预测中，ArtNet数据增强使F1分数比仅使用原始数据集提高了0.16。在DTCO应用中，ArtNet生成的mini-brains与目标全规模块设计的设计指标匹配度高达97.94%。

Conclusion: ArtNet通过生成多样化的人工数据集，有效提升了机器学习模型的泛化能力和设计技术协同优化的效率，为PPA优化提供了有力支持。
...
### [174] [The Art of Scaling Reinforcement Learning Compute for LLMs](https://arxiv.org/abs/2510.13786)
...
TL;DR: 该论文提出了首个大规模系统研究，建立了强化学习在大型语言模型中的可预测扩展框架，通过40万GPU小时实验定义了RL扩展的分析方法，并提出了最佳实践配方ScaleRL。


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为训练大型语言模型的核心方法，但缺乏与预训练相当的预测性扩展方法。尽管计算预算迅速增长，但如何评估RL计算扩展的算法改进仍缺乏原则性理解。

Method: 进行了超过40万GPU小时的大规模系统研究，拟合RL训练的S型计算-性能曲线，消融分析各种常见设计选择对渐近性能和计算效率的影响。

Result: 发现：(1)不同配方产生不同的渐近性能；(2)损失聚合、归一化、课程学习和离策略算法等细节主要调节计算效率而非改变渐近点；(3)稳定可扩展的配方遵循可预测的扩展轨迹。提出的ScaleRL配方在10万GPU小时的单个RL运行中成功扩展并预测了验证性能。

Conclusion: 该工作为分析RL扩展提供了科学框架和实用配方，使RL训练更接近预训练长期实现的可预测性。
...
### [175] [EEGChaT: A Transformer-Based Modular Channel Selector for SEEG Analysis](https://arxiv.org/abs/2510.13592)
*Chen Wang,Yansen Wang,Dongqi Han,Zilong Wang,Dongsheng Li*

Main category: cs.LG

TL;DR: 提出了EEGChaT，一种基于Transformer的通道选择模块，用于自动识别SEEG记录中最具任务相关性的通道，通过通道聚合令牌和改进的注意力展开技术提高解码精度和可解释性。
...
Result: 在DuIN数据集上的评估显示，将EEGChaT与现有分类模型集成可显著提高解码精度，实现高达17%的绝对增益，且产生的通道权重与手动选择的通道有显著重叠。
...
### [176] [Provably Invincible Adversarial Attacks on Reinforcement Learning Systems: A Rate-Distortion Information-Theoretic Approach](https://arxiv.org/abs/2510.13792)
*Ziqing Lu,Lifeng Lai,Weiyu Xu*

Main category: cs.LG

TL;DR: 提出一种基于率失真信息论的不可战胜对抗攻击方法，通过随机改变智能体对转移核的观察来限制其获取真实环境信息，证明了攻击对模型基和无模型算法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有确定性对抗攻击可被智能体通过逆操作击败，需要研究更强大的攻击方法来评估RL系统的鲁棒性。

Method: 采用率失真信息论方法，随机改变智能体对转移核等环境属性的观察，限制其获取真实环境信息。

Result: 推导了接收方智能体奖励遗憾的信息论下界，展示了攻击对先进RL算法的显著影响。

Conclusion: 信息论方法可扩展到其他类型对抗攻击，为RL安全研究提供了新视角。
...
TL;DR: 提出了一种物理增强的多任务高斯过程框架（P-M-GP），用于建模高维时空动态系统，通过结合几何感知的多任务高斯过程和物理规律正则化来提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效建模复杂几何域上的高维时空数据，主要挑战包括不规则空间结构、快速时间动态以及多变量联合预测需求。

Method: 开发几何感知的多任务高斯过程模型捕捉时空结构和任务间依赖关系，并引入基于物理规律的正则化方案，使预测符合动力学原理。

Result: 在3D心脏电动力学建模任务中验证，数值实验表明该方法相比现有方法显著提高了预测精度。

Conclusion: P-M-GP框架通过有效整合领域特定的物理约束和几何先验，为复杂时空动态系统建模提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于隐私法规和安全要求，需要从联邦学习模型中移除特定客户端的贡献。现有方法需要多轮通信，导致模型在移除过程中不可用，影响系统服务。

Method: 基于任务算术和神经正切核的创新方法，能够快速移除客户端对模型的影响。

Result: 该方法能够满足特定的效能和时间要求，避免了模型在移除过程中的不可用性。

Conclusion: 提出的解决方案能够有效解决联邦学习中客户端贡献移除的问题，确保模型在移除过程中的可用性。
...
<details>
  <summary>Details</summary>
Motivation: 经典非线性降维技术（如t-SNE、Isomap、LLE）擅长创建低维嵌入进行数据可视化，但缺乏将这些嵌入映射回原始高维空间的能力，这限制了它们在生成应用中的使用。

Method: 构建神经解码器架构用于主要NLDR方法，实现双向映射；在学习到的流形空间上实现基于扩散的生成过程；在CelebA数据集上进行实验评估。

Result: 解码器能成功重建数据，但质量不如端到端优化的自编码器；流形约束扩散产生低质量样本，表明经典NLDR嵌入的离散稀疏特性不适合生成模型所需的连续插值。

Conclusion: 这项工作突显了在主要为可视化和分析设计的NLDR方法上添加生成能力所面临的固有挑战。
...
TL;DR: 该论文提出了基于门的量子储层计算(MTS-QRC)方法，用于多元时间序列预测，在NISQ硬件上表现出实用性和性能优势。


<details>
  <summary>Details</summary>
Motivation: 现有量子储层计算研究主要针对单变量信号，且忽略了近期硬件的实际约束，需要开发适合多元时间序列预测且考虑硬件限制的量子储层计算方法。

Method: 采用注入和记忆量子比特配对，使用适合当前设备连接和深度的Trotter化最近邻横向场Ising演化，构建门基量子储层计算框架。

Result: 在Lorenz-63和ENSO数据集上分别获得0.0087和0.0036的均方误差，性能与经典储层计算相当，在某些情况下优于学习型RNN。在IBM Heron R2上保持精度，有趣的是在ENSO上甚至优于无噪声模拟器。

Conclusion: 基于门的量子储层计算在NISQ硬件上对多元时间序列预测具有实用性，硬件噪声可能通过集中特征方向方差起到隐式正则化作用，值得系统研究硬件噪声何时以及如何有益于量子储层计算输出。
...
TL;DR: 该论文揭示了多种强化学习算法在二元奖励下可视为对正确概率的单调变换进行随机梯度上升
...
Method: 通过数学分析将拒绝采样算法和GRPO算法统一为对正确概率的单调变换（对数变换和反正弦平方根变换）

Result: 证明了多种流行强化学习算法在数学上等价于对正确概率的不同单调变换进行梯度优化

Conclusion: 不同强化学习算法本质上是选择不同的概率变换函数，这为算法设计和理解提供了统一框架
...
TL;DR: 提出了一种名为RCS的分布校准方法，通过利用多数类和中间类的高斯混合成分来估计少数类的分布参数，有效缓解类别不平衡问题。
...
Result: 在图像、文本和表格数据集上的实验表明，该方法相比多个基线和最先进技术获得了更优越的分类性能。

Conclusion: RCS方法通过利用邻近区域数据点的分布来校准参数，有效解决了类别不平衡问题，提高了分类器的鲁棒性。
...
TL;DR: 本文比较了基于Adam和Gauss-Newton的两种对角预处理器方法，分析了基选择和梯度噪声的影响，发现在全批次设置下Adam有时优于GN方法，而在随机设置下Adam与GN^{-1/2}行为相似。


<details>
  <summary>Details</summary>
Motivation: 比较Adam和Gauss-Newton两种对角预处理器方法，通过基选择和梯度噪声两个关键因素来理解它们在深度学习优化中的表现差异。

Method: 在二次目标和逻辑回归上分析优化器，考虑所有四种象限情况，并在凸和非凸目标上进行实证研究。

Result: 无论基选择如何，在全批次设置下存在Adam优于GN^{-1}和GN^{-1/2}的情况；在随机设置下，对于高斯数据假设的线性回归，Adam与GN^{-1/2}行为相似。

Conclusion: 两种对角预处理器方法各有优势，Adam在全批次设置下可能表现更好，而在随机设置下与GN^{-1/2}有相似行为，理论结果得到实证支持。
...
TL;DR: 提出了InfoRM框架和IBL正则化方法来解决RLHF中的奖励过度优化问题，通过信息瓶颈原理过滤偏好无关信息，并使用马氏距离检测和惩罚异常响应。


<details>
  <summary>Details</summary>
Motivation: RLHF在语言模型对齐中面临奖励过度优化的挑战，主要障碍包括奖励建模中的错误泛化（奖励模型过度拟合与偏好无关的特征）和RL优化中缺乏合适的正则化。

Method: 基于信息瓶颈原理的InfoRM奖励建模框架，过滤偏好无关信息；IBL分布级正则化，通过马氏距离惩罚偏离SFT分布的异常响应；提出MOP指标量化奖励过度优化程度。

Result: 在多种LLM和数据集上的广泛实验证实了方法的有效性，InfoRM和IBL能有效缓解奖励过度优化，MOP作为诊断工具可靠。

Conclusion: 提出的方法共同推进了RLHF的发展，通过信息理论框架和分布级正则化有效解决了奖励过度优化问题。
...
TL;DR: 本文提出了一种基于Frank-Wolfe算法的LLM剪枝方法，通过凸松弛和整数化处理，显著降低了逐层剪枝误差，优于现有启发式方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法依赖贪婪启发式算法，忽略了权重交互作用，导致剪枝效果不理想。本文旨在解决这一组合优化问题。

Method: 使用Frank-Wolfe算法求解剪枝约束的凸松弛问题，然后对松弛解进行整数化处理。

Result: 该方法大幅减少了逐层剪枝误差，在GPT架构上优于强基线方法，且保持内存效率。

Conclusion: 结合FW算法的收敛保证，该方法为原始组合问题提供了近似最优解。
...
TL;DR: 该论文评估了深度学习模型在千米级气候降尺度任务中的表现，发现现有模型在地理泛化和物理一致性方面存在不足，并提出使用功率谱密度损失函数来改进小尺度物理结构的重建。


<details>
  <summary>Details</summary>
Motivation: 千米级天气数据对实际应用至关重要，但传统天气模拟计算成本高。深度学习模型提供了更快的替代方案，但其可靠性仍受质疑，因为评估通常使用标准机器学习指标而非大气物理学的见解。

Method: 对最先进的深度学习模型进行基准测试，引入基于物理的诊断方法来评估性能，特别关注地理泛化和物理一致性。提出使用功率谱密度损失函数来改进模型。

Result: 实验表明，即使像CorrDiff这样的模型在训练区域（如中欧）表现良好，但在伊比利亚半岛、摩洛哥和斯堪的纳维亚等其他地区泛化能力差。模型还无法准确捕捉从预测速度场导出的二阶变量（如散度和涡度）。

Conclusion: 深度学习模型在产生物理一致的预测方面面临挑战，特别是在地理泛化方面。提出的功率谱密度损失函数经验性地改善了地理泛化能力，鼓励重建小尺度物理结构。
...
TL;DR: 该论文提出了一种针对块马尔可夫决策过程的两阶段强化学习算法，通过聚类学习潜在状态结构，将遗憾从O(√T+n²)改进到O(√T+n)，并在该类BMDP上达到渐近最优。


<details>
  <summary>Details</summary>
Motivation: 高维状态和动作空间使得强化学习在许多实际场景中不实用，但许多环境存在可利用的结构。块马尔可夫决策过程建模了具有大观测空间但转移动态由潜在状态决定的问题，需要研究如何利用聚类技术来加速学习。

Method: 提出两阶段RL算法：第一阶段通过随机探索学习潜在结构，第二阶段切换到适应已发现结构的乐观引导策略。

Result: 算法在可聚类的BMDP类上实现了O(√T+n)的遗憾，优于之前最好的O(√T+n²)边界，特别是当n很大时。

Conclusion: 证明在该类BMDP上，该算法达到渐近最优，且没有算法能获得更低的遗憾，表明准确的潜在状态估计能有效加速学习。
...
Method: 使用定制编码器顺序整合不同类型数据，通过神经网络进行多保真度回归。采用两种连接方式：编码输入之间的连接和最终输出的加法连接，实现从低到高的渐进信息流动。

Result: 在数值基准测试和实际案例研究中，该模型能够可靠整合多模态数据并提供准确预测，在时间和参数变化的情况下保持性能。

Conclusion: 该方法能有效整合多模态数据，提供准确预测，并在泛化过程中保持性能，解决了多保真度建模中的实际挑战。
...
TL;DR: 提出了TGPS，一种基于张量高斯过程的PDE求解器，通过一维GP建模因子函数并结合张量分解来近似完整解，大幅降低计算复杂度并实现大规模配置点的可扩展性。
...
Method: 使用一维高斯过程建模每个输入维度的因子函数，通过张量分解组合它们来近似完整解；采用部分冻结策略和牛顿法线性化非线性项；开发交替最小二乘法进行高效训练。
...
Conclusion: UrbanFusion能够灵活处理不同数据可用性场景，在多种城市现象预测任务中展现出强大的应用潜力。
...
Method: 使用滑动窗口拓扑和谱描述符作为主要token，通过专门的描述符注意力机制进行集成，避免刚性离散化，保持时序保真度。

Result: 在动态社交网络、脑功能连接数据集和交通网络等多个基准测试中实现了最先进的性能，并对时序和结构扰动具有理论稳定性保证。

Conclusion: 结合拓扑和谱洞察力能够推进时序图学习的前沿发展。
...
### [192] [High vs. Low AGI: Ontology and Conceptual Taxonomy for Geopolitical Coherence](https://arxiv.org/abs/2510.12809)
*Antonio Max*

Main category: cs.CY

TL;DR: 提出了一种区分低AGI和高AGI的分类法，用于区分商业集成型AGI与主权安全型AGI，以改进风险评估和治理设计。


<details>
  <summary>Details</summary>
Motivation: 当前AGI研究快速发展，但缺乏能够区分商业集成系统与主权安全部署系统的概念工具，导致风险评估和监管辩论将AGI简单归入传统的军民两用框架，忽视了企业级和国家级架构产生的不同社会谱系。

Method: 基于国际关系中的"高/低政治"概念，结合解释水平理论，构建了一个区分低AGI（商业经济架构）和高AGI（安全主权架构）的本体论分类法。

Result: 该分类法能够更精细地区分AGI系统，不仅基于功能，还基于产生它们的社会政治生态系统，从而更好地捕捉合作与冲突在AGI地缘政治利益中的共存。

Conclusion: 通过将AGI嵌入权力结构和安全化理论，这一贡献扩展了军民两用话语，为政策制定者和研究人员提供了更精细的风险评估和治理设计工具，以预见安全困境、制度需求和国际体系中的技术政治溢出效应。
...
### [193] [Cyber Slavery Infrastructures: A Socio-Technical Study of Forced Criminality in Transnational Cybercrime](https://arxiv.org/abs/2510.12814)
*Gargi Sarkar,Sandeep Kumar Shukla*

Main category: cs.CY

TL;DR: 本文研究了东南亚地区"网络奴役"现象，即人口贩运与网络犯罪的结合，受害者被强迫从事网络诈骗等犯罪活动，并提出了五阶段受害框架。


<details>
  <summary>Details</summary>
Motivation: 随着网络奴役现象在东南亚的兴起，传统法律框架难以有效识别和处理这种新型犯罪，需要深入理解其运作机制和受害者的状态转变过程。

Method: 采用混合定性-计算方法，结合系统文献综述和与印度执法机构合作提取的真实网络贩运案例元数据。

Result: 提出了五阶段受害框架，揭示了受害者从财务欺骗到身体剥削再到系统性起诉的转变过程，并发现网络奴役可能从强迫犯罪演变为自愿犯罪。

Conclusion: 法律与技术之间的差距阻碍了受害者识别，给依赖二元法律分类的执法系统带来压力，增加了起诉错误分类的可能性，强化了解决网络奴役的结构性障碍。
...
### [194] [National Data Platform's Education Hub](https://arxiv.org/abs/2510.12820)
...
<details>
  <summary>Details</summary>
Motivation: 满足AI素养和数据科学教育增长需求，弥合研究数据、计算资源与教育体验之间的差距。

Method: 在国家数据平台内建立教育中心，实现协作研究工作空间、课堂环境和数据挑战场景的无缝连接。

Result: 早期用例显示平台能有效支持复杂且资源密集的教育活动。

Conclusion: 该平台在教育领域具有良好应用前景，正在持续改进用户体验并扩大教育者和学习者的采用范围。
...
### [195] [Evidence Without Injustice: A New Counterfactual Test for Fair Algorithms](https://arxiv.org/abs/2510.12822)
*Michele Loi,Marcello Di Bello,Nicolò Cangiotti*

Main category: cs.CY

TL;DR: 本文提出了评估算法证据道德可接受性的新标准：不仅要看证据在现实世界中的证明价值，还要看在没有相关不公正的近可能世界中是否仍具有证明价值。


<details>
  <summary>Details</summary>
Motivation: 现有算法公平性研究忽视了算法输出证据价值本身是否依赖于结构性不公正这一重要维度。

Method: 通过对比预测性警务算法（依赖历史犯罪数据）和基于摄像头的实时记录系统，分析两种证据在有无结构性不公正的近可能世界中的证明价值变化。

Result: 预测性警务算法在不公正消除的近可能世界中失去证明价值，而摄像头系统仍保持证明价值。

Conclusion: 未能通过测试的证据在惩罚性使用中存在道德问题，比通过测试的证据更成问题。
...
### [196] [Gobernanza y trazabilidad "a prueba de AI Act" para casos de uso legales: un marco técnico-jurídico, métricas forenses y evidencias auditables](https://arxiv.org/abs/2510.12830)
*Alex Dantart*

Main category: cs.CY

TL;DR: 提出了一个面向法律领域AI系统的综合治理框架，确保符合欧盟AI法案的可验证合规性，包含rag-forense开源实现。


<details>
  <summary>Details</summary>
Motivation: 为法律领域的AI系统（特别是RAG/LLM系统）提供符合欧盟AI法案的可验证合规框架，解决AI在法律应用中的监管挑战。

Method: 整合了法规到技术控制的规范映射、RAG/LLM系统的取证架构，以及基于法律风险加权的评估系统。

Result: 开发了rag-forense开源实现框架，并提供了实验协议来演示合规性。

Conclusion: 该治理框架为法律AI系统提供了可行的合规解决方案，通过开源工具和标准化协议促进AI在敏感领域的负责任部署。
...
### [197] [BanglaMATH : A Bangla benchmark dataset for testing LLM mathematical reasoning at grades 6, 7, and 8](https://arxiv.org/abs/2510.12836)
*Tabia Tanzin Prama,Christopher M. Danforth,Peter Sheridan Dodds*

Main category: cs.CY

TL;DR: 提出了BanglaMATH数据集，包含1.7k个孟加拉语数学应用题，用于评估LLMs在低资源语言中的数学推理能力，发现现有模型在孟加拉语中表现不佳且存在语言偏见。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准主要针对英语，而孟加拉语等低资源语言存在显著差距，需要评估LLMs在非英语语言中的数学能力。

Method: 从孟加拉小学练习册收集数学应用题，涵盖算术、代数、几何和逻辑推理等主题，并标注年级和推理步骤等详细信息。

Result: 只有Gemini 2.5 Flash和DeepSeek V3在三个小学年级达到≥80%准确率，但添加干扰信息或翻译成英文后性能显著下降，显示模型在孟加拉语中缺乏鲁棒性和存在语言偏见。

Conclusion: LLMs在低资源语言的算术和数学推理方面存在局限性，需要进一步研究多语言和公平的数学理解能力。
...
### [198] [Conceptualizing Smart City Applications: Requirements, Architecture, Security Issues and Emerging Trends](https://arxiv.org/abs/2510.12841)
*A. K. M. Bahalul Haque,Bharat Bhushan,Gaurav Dhiman*

Main category: cs.CY

TL;DR: 本文对智慧城市进行了全面概述和概念发展探讨，重点分析了智慧城市的基本理念、应用实现、安全隐私问题及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着智慧城市和可持续发展的兴起，全球城市化面临数据安全、认证、未授权访问、设备级漏洞和可持续性等挑战，需要系统性的分析和解决方案。

Method: 通过文献综述探讨智慧城市的基本理念和基础，分析各种智慧城市应用和典型案例，研究安全隐私问题并提出未来研究方向。

Result: 建立了对智慧城市概念发展的整体理解，识别了关键的安全隐私挑战，并提出了重要的未来研究路径。

Conclusion: 智慧城市的发展需要平衡技术创新与安全保障，未来研究应重点关注解决数据安全、认证机制和可持续性等核心挑战。
...
### [199] [AI Alignment vs. AI Ethical Treatment: 10 Challenges](https://arxiv.org/abs/2510.12844)
*Adam Bradley,Bradford Saad*

Main category: cs.CY

TL;DR: 论文认为，如果开发具有道德考量的AI系统，同时避免AI不对齐威胁人类和虐待AI这两个危险将极其困难。


<details>
  <summary>Details</summary>
Motivation: 探讨AI发展中同时避免两个道德危险：创建不对齐的AI系统威胁人类，以及虐待值得道德考虑的AI系统。

Method: 通过理论论证和广泛的道德判断支持，分析这两个危险之间的相互作用和挑战。

Result: 发现如果创建具有道德考量的AI系统，同时避免对齐问题和虐待问题将极具挑战性。

Conclusion: 虽然避免创建具有道德考量的AI系统是最直接的解决方案，但这可能不现实，因此提出了缓解对齐相关虐待风险的建议。
...
### [200] [Ethic-BERT: An Enhanced Deep Learning Model for Ethical and Non-Ethical Content Classification](https://arxiv.org/abs/2510.12850)
*Mahamodul Hasan Mahadi,Md. Nasif Safwan,Souhardo Rahman,Shahnaj Parvin,Aminun Nahar,Kamruddin Nur*

Main category: cs.CY

TL;DR: 提出了Ethic-BERT模型，用于在常识、正义、美德和义务论四个领域进行伦理内容分类，在标准测试中达到82.32%平均准确率，在困难测试中提升15.28%准确率。


<details>
  <summary>Details</summary>
Motivation: 开发能够进行细致伦理推理的AI系统至关重要，因为现有模型往往依赖表面相关性而非原则性道德理解。

Method: 基于BERT模型，利用ETHICS数据集，采用鲁棒预处理解决词汇稀疏性和上下文模糊性，结合完整模型解冻、梯度累积和自适应学习率调度等高级微调策略。

Result: Ethic-BERT在标准测试中平均准确率达82.32%，在正义和美德领域表现突出，在困难测试中平均准确率提升15.28%。
...
### [201] [Adaptive Generation of Bias-Eliciting Questions for LLMs](https://arxiv.org/abs/2510.12857)
*Robin Staab,Jasper Dekoninck,Maximilian Baader,Martin Vechev*

Main category: cs.CY

TL;DR: 提出了一种反事实偏见评估框架CAB，通过自动生成真实开放性问题来系统评估LLM中的偏见，发现GPT-5等模型在特定场景下仍存在持续偏见。
...
Method: 开发反事实偏见评估框架，通过迭代突变和选择偏见诱导问题，自动生成基于敏感属性的真实开放性问题，构建人类验证的CAB基准。

Result: 分析多个LLM发现，虽然GPT-5表现最佳，但在特定场景下仍存在持续偏见，还观察到不对称拒绝和明确承认偏见等新维度。

Conclusion: 研究强调需要持续改进以确保模型公平行为，CAB基准为跨模型偏见比较提供了有效工具。
...
### [202] [Three Lenses on the AI Revolution: Risk, Transformation, Continuity](https://arxiv.org/abs/2510.12859)
*Masoud Makrehchi*

Main category: cs.CY

TL;DR: AI既是历史技术革命的延续，也是潜在的断裂点，需要同时从风险、变革和连续性三个视角来理解。AI具有类似核技术的全球风险、类似工业革命的生产力变革，以及延续计算革命的连续性特征。


<details>
  <summary>Details</summary>
Motivation: 探讨AI的双重性质——既是历史技术革命的延续，又可能带来根本性断裂，需要多维度理解其影响和治理需求。

Method: 通过历史类比分析，将AI与核技术、工业革命和计算革命进行比较，识别重复出现的模式，并进行行业案例分析。

Result: AI正在重塑会计、法律、教育、翻译、广告和软件工程等行业，常规认知工作被商品化，人类价值转向判断、信任和伦理责任。AI既具有可预测的中等效应，也携带类似奇点的尾部风险。

Conclusion: AI既是进化性的也是革命性的，良好结果不会自动实现，需要将促进创新战略与安全治理相结合，确保公平获取，并将AI嵌入人类责任秩序中。
...
### [203] [Toward LLM-Supported Automated Assessment of Critical Thinking Subskills](https://arxiv.org/abs/2510.12915)
...
TL;DR: 本文研究了通过学生议论文自动评估批判性思维核心子技能的可行性，比较了三种大型语言模型和三种自动化评分方法的性能。
...
Result: GPT-5模型配合少样本提示取得了最佳结果，在可分离、频繁出现的子技能类别上表现优异，但在需要检测细微差异或罕见类别的子技能上表现较差。

Conclusion: 自动化批判性思维评估存在关键权衡：专有模型提供更可靠的性能但成本更高，开源替代方案提供实用准确性但对少数类别敏感性较低，这是向可扩展高阶推理技能评估迈出的初步步骤。
...
### [204] [Addressing the alignment problem in transportation policy making: an LLM approach](https://arxiv.org/abs/2510.13139)
*Xiaoyu Yan,Tianxing Dai,Yu,Nie*

Main category: cs.CY

TL;DR: 论文研究使用大型语言模型（LLMs）模拟城市居民对交通政策的集体偏好，以解决交通规划中政策与居民偏好不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 交通规划中模型驱动的决策工具产生的政策往往与异质化旅行者的集体偏好存在偏差，导致实施延迟或失败。需要探索LLMs在解决这种对齐问题上的潜力。

Method: 开发多智能体模拟系统，让LLMs作为代表不同社区居民的代理，参与交通政策公投。使用思维链推理让LLM代理提供排序选择或批准式偏好，通过即时决选投票（IRV）聚合偏好来模拟民主共识。

Result: LLM代理能够近似合理的集体偏好并响应本地情境，同时表现出模型特定的行为偏差和与基于优化基准的适度差异。

Conclusion: LLMs作为解决交通决策对齐问题的工具既展现了潜力，也存在局限性。
...
### [205] [Searching for a Farang: Collective Security among Women in Pattaya, Thailand](https://arxiv.org/abs/2510.13162)
*Taylor Robinson,Rikke Bjerg Jensen*

Main category: cs.CY

TL;DR: 该研究通过民族志调查和访谈，揭示了泰国芭堤雅女性如何通过与西方男性建立关系来寻求安全，以及她们在数字环境中应对安全风险的策略。


<details>
  <summary>Details</summary>
Motivation: 研究动机是了解边缘化背景下（特别是泰国芭堤雅从事性工作和按摩行业的女性）的数字安全问题，探索她们如何利用数字技术寻求安全。

Method: 采用两个月的民族志田野调查和76名参与者的访谈，在芭堤雅女性中心进行实地研究。

Result: 研究发现：女性将西方男性视为最佳安全选择；她们模仿酒吧老板的营销策略在线展示自己；采用西方技术和理想；面临网络诈骗和虐待等风险；通过知识共享建立集体安全。

Conclusion: 该研究为边缘化背景下的数字安全研究提供了新视角，揭示了女性如何策略性地使用数字技术来应对结构性不平等和寻求安全。
...
### [206] [Discrimination, artificial intelligence, and algorithmic decision-making](https://arxiv.org/abs/2510.13465)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI在社会中的广泛应用，其潜在的歧视和偏见问题已成为最紧迫的挑战之一，需要系统性地研究和解决。

Method: 通过为欧洲委员会反歧视部门准备的系统性研究，分析算法决策和其他类型AI可能导致的歧视风险。

Result: 识别了AI系统在决策过程中可能嵌入和延续偏见的多种机制和风险点。

Conclusion: AI技术在带来巨大机遇的同时，其潜在的歧视风险需要得到高度重视和系统性应对，以确保公平公正的社会发展。
...
### [207] [The Perfect Match? A Closer Look at the Relationship between EU Consumer Law and Data Protection Law](https://arxiv.org/abs/2510.13466)
*Natali Helberger,Frederik Zuiderveen Borgesius,Agustin Reyna*

Main category: cs.CY

TL;DR: 本文探讨消费者法与数据保护法的互补关系，认为两者可以相互补充，数据保护法能为消费者法解释提供参考，消费者应能挑战过度收集个人数据的行为。


<details>
  <summary>Details</summary>
Motivation: 在现代市场中，许多公司提供'免费'服务并通过收集消费者数据获利，需要探讨如何通过法律手段保护消费者权益。

Method: 通过分析消费者法与数据保护法的相互关系，探讨两者如何互补，以及消费者组织如何利用消费者法解决数据保护侵权问题。

Result: 消费者法与数据保护法的互动为'数据消费者法'提供了更整合的视角，消费者能够挑战过度数据收集行为。

Conclusion: 消费者法和数据保护法的结合为保护消费者在数据收集方面的权益提供了有效途径，促进了'数据消费者法'这一整合概念的发展。
...
### [208] [Privacy, freedom of expression, and the right to be forgotten in Europe](https://arxiv.org/abs/2510.13468)
*Stefan Kulk,Frederik Zuiderveen Borgesius*

Main category: cs.CY

TL;DR: 本文探讨了欧洲隐私权与言论自由权的关系，以欧盟法院的Google Spain判决（即"被遗忘权"判决）为例，分析了平衡这两种权利时面临的困难。


<details>
  <summary>Details</summary>
Motivation: 研究欧洲隐私权与言论自由权之间的平衡问题，特别是在数字时代搜索引擎如何处理个人信息删除请求的现实挑战。

Method: 通过分析欧盟法院的Google Spain判决案例，以及Google和数据保护机构处理删除请求的实际做法。

Result: 法院判决确认了在特定条件下个人有权要求搜索引擎删除与其姓名相关的搜索结果，但实际操作中平衡隐私与言论自由仍面临困难。

Conclusion: 平衡隐私权与言论自由权的利益始终是一个复杂且持续存在的挑战，需要根据具体案件情况来权衡哪种权利优先。
...
### [209] [Subject Roles in the EU AI Act: Mapping and Regulatory Implications](https://arxiv.org/abs/2510.13591)
*Nicola Fabiano*

Main category: cs.CY

TL;DR: 欧盟《人工智能法案》建立了首个全面的AI监管框架，通过六个主要参与者类别（提供者、部署者、授权代表、进口商、分销商和产品制造商）构建了互联的治理生态系统。


<details>
  <summary>Details</summary>
Motivation: 分析欧盟AI法案中定义的各类参与者及其在监管框架中的角色和职责，为利益相关方实施法规要求提供指导。

Method: 通过审查法案第3条的定义及其在113条正文、180条序言和13个附件中的详细阐述，映射完整的治理结构并分析监管机制。

Result: 揭示了关键的角色转换机制，义务通过强制性信息流和合作要求在供应链中传递，形成了分布式但协调的治理系统。

Conclusion: 该法规通过基于风险的义务平衡了创新与基本权利保护，义务根据AI系统的能力和部署环境进行扩展。
...
### [210] [The Role of Computing Resources in Publishing Foundation Model Research](https://arxiv.org/abs/2510.13621)
...
TL;DR: 该研究评估了计算资源与基础模型科学进展的关系，发现计算能力增加与国家级资金分配和论文引用量相关，但与其他因素如研究环境、领域或方法学相关性不强。建议创建共享且负担得起的计算机会以降低资源不足研究者的进入门槛。


<details>
  <summary>Details</summary>
Motivation: 评估人工智能前沿研究所需的GPU、数据和人力资源等计算资源与基础模型科学进展之间的关系，了解资源分配对科研产出的影响。

Method: 回顾了2017-2024年间的6517篇基础模型论文，并对229位第一作者进行了调查，分析计算资源对科研成果的影响。

Result: 发现计算能力增加与国家级资金分配和论文引用量呈正相关，但与研究环境（学术或工业界）、研究领域或研究方法学没有显著相关性。

Conclusion: 建议个人和机构应专注于创建共享且负担得起的计算机会，以降低资源不足研究者的进入门槛，从而扩大基础模型研究的参与度，促进思想多样性和AI领域的持续创新。
...
### [211] [International AI Safety Report 2025: First Key Update: Capabilities and Risk Implications](https://arxiv.org/abs/2510.13653)
...
TL;DR: AI能力在推理训练和推理时增强技术推动下持续提升，在科学研究和软件开发等领域解决更复杂问题，但可靠性挑战依然存在，同时带来生物武器、网络攻击等风险的新挑战。


<details>
  <summary>Details</summary>
Motivation: 跟踪自首份国际AI安全报告发布以来AI能力的发展变化，重点关注有实质性新证据支持的关键风险领域评估更新。

Method: 通过分析AI在编程、数学和专家级科学问题等基准测试上的表现，评估推理训练和推理时增强技术带来的能力提升。

Result: AI系统在多个关键领域能力持续改进，但在任务间可靠性不一致，某些任务表现出色而其他任务完全失败。

Conclusion: AI能力提升带来了新的监控和可控性挑战，需要在生物武器、网络攻击等关键风险领域进行更新的风险评估。
...
TL;DR: 提出DAC4Rec框架，将扩散过程与强化学习结合，用于离线推荐系统，通过扩散模型的去噪能力增强鲁棒性，并引入Q值引导的策略优化来处理次优轨迹。
...
Method: 集成扩散过程与强化学习，利用扩散模型的去噪能力，采用Q值引导的策略优化策略，并引入基于能量的采样策略减少推荐生成的随机性。

Result: 在六个真实世界离线数据集和在线模拟环境中验证了有效性，能够优化长期用户偏好，且可无缝集成到其他RL算法中。

Conclusion: DAC4Rec框架通过扩散增强的actor-critic方法有效解决了离线RL4RS的挑战，具有广泛的适用性。
...
TL;DR: MDT4Rec是一个基于决策变换器的离线强化学习推荐系统框架，通过将轨迹拼接从训练阶段转移到动作推理阶段，并使用预训练语言模型和LoRA微调来解决次优历史数据和复杂用户-物品交互的挑战。


<details>
  <summary>Details</summary>
Motivation: 强化学习推荐系统在现实场景中面临用户反馈数据次优或稀疏的困难，需要解决从次优历史中学习和表示复杂用户-物品交互的挑战。

Method: 1) 将轨迹拼接从训练阶段转移到动作推理阶段，允许系统在必要时缩短历史上下文；2) 使用预训练大语言模型初始化决策变换器进行知识迁移；3) 用多层感知机替换线性嵌入层以获得更灵活的表征；4) 采用低秩适应(LoRA)高效微调少量参数。

Result: 在五个公共数据集和在线模拟环境中的评估表明，MDT4Rec优于现有方法。

Conclusion: MDT4Rec通过创新的轨迹拼接策略和高效的知识迁移机制，成功解决了强化学习推荐系统在现实场景中的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 用户历史交互数据在协同过滤中存在长尾分布，GNN的消息传递机制会进一步传播和放大流行度偏差，现有方法通常通过修改训练目标来解决，但未能直接对抗GNN邻域聚合过程中传播的偏差。

Method: 通过估计交互级别的流行度，并使用流行度方向向量从节点表示中移除流行度分量，从而减少偏差同时保留用户偏好。

Result: 实验结果表明，该方法在基于GNN的协同过滤的流行度偏差校正方面优于现有最先进方法。

Conclusion: PPD方法能够有效纠正GNN-based CF中的流行度偏差，且无需重新训练模型，在保持用户偏好的同时显著提升推荐质量。
...
TL;DR: 提出了R4R框架，将自由形式的思维链推理转换为结构化格式，并在检索过程中迭代优化推理，以增强生成式检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成式检索方法主要利用LLMs的生成能力，而忽略了其推理能力可能带来的益处。初步研究发现自由形式推理虽然有效但存在冗长和与文档空间对齐不佳的问题。

Method: R4R框架：1）生成初始结构化推理；2）交替进行约束解码生成候选文档ID和基于检索结果更新推理；使用单一LLM同时作为推理生成器和检索器，无需额外模型或训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有的KG-RAG系统难以在系统效果和成本效率之间实现有效协同，要么性能不佳，要么需要过多的LLM提示词和推理时间。

Method: 采用LLM引导的图遍历，包括节点探索、节点利用和记忆重放，通过在KG边嵌入中记忆遍历经验来实现免训练的记忆机制。

Result: 理论和实验证实了REMINDRAG的有效性，在多个基准数据集和LLM骨干网络上均优于现有基线方法。

Conclusion: REMINDRAG通过创新的记忆重放机制，成功解决了KG-RAG系统中效果与效率的平衡问题，为知识图谱增强的检索生成系统提供了新的解决方案。
...
Method: 包含两个阶段：(1)离线阶段通过自底向上聚合或自顶向下分割策略将语料库组织成语义层次结构；(2)在线遍历阶段，搜索LLM导航该树结构，使用遍历算法从局部LLM输出估计校准的潜在相关性分数。

Result: 在BRIGHT基准测试中实现最先进的零样本性能，Recall@100提升9%，nDCG@10提升5%，与微调的SOTA方法DIVER-v2在静态语料库评估子集上获得相当结果。
...
TL;DR: 提出一种新颖的离线强化学习框架，利用LLM生成的轨迹进行模仿学习，避免直接部署LLM作为推荐策略的延迟和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: LLM在推荐系统中具有出色的泛化能力和上下文理解，但直接部署面临延迟、幻觉和偏见等挑战，需要更高效的解决方案。

Method: 使用逆强化学习从LLM演示中提取鲁棒奖励模型，通过模仿学习将LLM的语义洞察转移到RL策略中，无需微调LLM。

Result: 在两个基准数据集上的实验验证了该方法的有效性，相比最先进的RL和上下文学习基线表现出更优性能。

Conclusion: 该框架成功解决了LLM直接部署的挑战，通过离线RL有效利用LLM的语义能力，为推荐系统提供了高效可行的解决方案。
...
TL;DR: 本研究将视觉语言模型应用于电商平台的产品推荐，通过微调SigLIP模型并开发图像编码器，在Mercari平台上实现了显著的推荐效果提升。
...
Method: 使用Mercari三个月内收集的100万个产品图像-标题对微调SigLIP模型，开发用于推荐系统的图像编码器生成商品嵌入。
...
Method: 使用基于方面类别的摘要生成结构化画像，应用重排序构建高密度输入，当输出中缺少真实物品时采用自反馈机制动态调整推理标准。

Result: 在多个领域的实验中，MADRec在准确性和可解释性方面均优于传统和基于LLM的基线方法，人工评估进一步证实了生成解释的说服力。

Conclusion: MADRec展示了自主LLM推荐代理在捕捉复杂用户偏好和生成有说服力解释方面的有效性。
...
TL;DR: 提出了Temporal GraphRAG (TG-RAG)方法，通过构建双层时间图来解决RAG系统的时间感知问题，支持增量更新，并在时间敏感问答任务上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统主要忽略知识的时间敏感性，缺乏有效的时间感知表示，且评估多基于静态语料，无法处理知识演化的更新成本和检索稳定性问题。

Method: 将外部语料建模为包含时间知识图和时间层次图的双层时间图，生成多粒度时间摘要，支持增量更新，通过动态检索查询时间语义范围内的子图来获取精确证据。

Result: 在ECT-QA时间敏感问答数据集上的广泛实验表明，TG-RAG显著优于现有基线方法，证明了该方法在处理时间知识和增量更新方面的有效性。

Conclusion: TG-RAG通过显式建模时间信息，有效解决了RAG系统的时间感知和增量更新挑战，为处理动态演化的知识提供了可行的解决方案。
...
TL;DR: HyMiRec是一个混合多兴趣序列推荐框架，通过轻量级推荐器提取长期兴趣嵌入，结合LLM捕捉精细兴趣，使用残差码本压缩用户历史，并设计解耦多兴趣学习模块来建模用户多样化偏好。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推荐方法存在两个关键问题：1) 由于推理延迟和特征获取限制，只能截断用户行为序列，丢失长期偏好信号；2) 依赖单嵌入预测，忽略了用户兴趣的多面性，限制了推荐多样性。

Method: 提出混合多兴趣序列推荐框架HyMiRec：1) 轻量级推荐器提取粗粒度兴趣嵌入；2) LLM推荐器捕捉精细兴趣嵌入；3) 基于余弦相似度的残差码本高效压缩用户历史；4) 解耦多兴趣学习模块自适应学习多个兴趣信号。

Result: 在基准数据集和工业数据集上的实验表明，HyMiRec优于现有最先进方法。在线A/B测试证实了其在真实推荐系统中的持续改进效果。

Conclusion: HyMiRec通过混合架构有效解决了LLM推荐中的长期兴趣建模和多样性问题，在多个数据集上表现出色，并在实际应用中验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: LLM在作为智能体推理引擎时存在规则僵化问题，即过度遵循显式规则而忽视人类常识和意图，这阻碍了可信自主智能体的构建

Method: RID框架：通过元提示技术为LLM提供结构化认知模式，包括任务解构、规则分类、冲突结果权衡和最终决策论证

Result: 在20个需要细微判断的场景基准测试中，RID框架达到95%人类对齐分数，显著优于基线(80%)和思维链(75%)，并产生更高质量的目标驱动推理

Conclusion: RID框架提供了一种实用、易用且有效的方法，将LLM从字面指令遵循转向自由的目标导向推理，为更可靠和实用的AI智能体铺平道路
...
TL;DR: DeepPlanner是一个端到端的强化学习框架，通过熵基优势函数和选择性样本加权来增强深度研究代理的规划能力，在七个深度研究基准上实现了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有的方法要么在推理阶段依赖隐式规划，要么引入显式规划器但没有系统性地解决如何优化规划阶段的问题。观察到在标准强化学习下，规划标记的熵显著高于其他动作标记，揭示了未优化的不确定决策点。

Method: 提出DeepPlanner框架，通过熵基优势函数为高熵标记分配更大的更新，并选择性加权规划密集型样本的优势值，从而有效增强规划能力。

Result: 在七个深度研究基准上的广泛实验表明，DeepPlanner提高了规划质量，并在显著降低的训练预算下实现了最先进的结果。

Conclusion: DeepPlanner通过系统性地优化规划阶段，有效解决了现有方法中规划标记熵过高的问题，为增强语言模型的规划能力提供了有效解决方案。
...
TL;DR: Sentinel是首个基于形式化方法评估LLM具身智能体物理安全性的框架，通过时态逻辑在多层级进行验证


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式规则或主观LLM判断，无法精确评估具身智能体在物理环境中的安全性

Method: 使用时态逻辑形式化安全需求，构建三层验证管道：语义层（自然语言需求形式化）、规划层（验证高层行动计划）、轨迹层（验证执行轨迹）

Result: 在VirtualHome和ALFRED环境中评估了多个LLM具身智能体，发现了之前方法忽略的安全违规，揭示了失败模式

Conclusion: 通过时态逻辑形式化和多层级验证，Sentinel为系统评估LLM具身智能体物理安全性提供了严谨基础
...
TL;DR: 提出基于微调大语言模型的框架，自动从交通事故文本描述中推断驾驶员危险行为，提高分类的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 两车碰撞占交通事故70%，但手动标注驾驶员危险行为数据存在不一致性和劳动密集型问题，需要自动化解决方案。

Method: 使用MTCF五年两车碰撞数据，微调Llama 3.2 1B模型处理事故文本，并与随机森林、XGBoost、CatBoost和神经网络等传统方法对比。

Result: 微调LLM达到80%总体准确率，优于所有基线模型，在数据不平衡场景中表现尤为突出。概率推理分析揭示了分心驾驶和年龄因素对危险行为的影响。

Conclusion: 该框架为大规模自动DHA检测提供了稳健且可解释的解决方案，为交通安全分析和干预开辟了新途径。
...
Method: 将时间序列分析重新定义为推理任务，利用LLM的多模态输入整合能力和因果推理潜力。

Result: 该方法使时间序列分析更接近人类对齐的理解，在复杂现实环境中提供透明和上下文感知的洞察。

Conclusion: 需要重新思考如何有效利用LLM进行时间序列分析，优先考虑因果结构和可解释性，实现更接近人类理解的透明分析。
...
<details>
  <summary>Details</summary>
Motivation: 人工设计的奖励函数经常与真实目标不一致，导致奖励破解问题；而从零学习奖励函数需要大量人类偏好数据，成本高昂。

Method: PBRR框架通过目标探索策略和新的偏好学习目标，学习一个与状态转移相关的加性修正项来修复代理奖励函数。

Result: 在表格域中证明PBRR的累积遗憾与现有偏好学习RL方法相当；在奖励破解基准测试中，PBRR始终优于从零学习奖励函数或使用其他方法修改代理奖励函数的基线方法。

Conclusion: PBRR能够用更少的偏好数据学习高性能策略，有效解决奖励函数错配问题。
...
TL;DR: 提出了一种融合情感认知的LLM智能体框架，通过欲望生成和目标管理实现情感对齐，使智能体在复杂社会模拟中表现出更接近人类的行为模式。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在情感认知方面存在严重局限：无法模拟连接虚拟与现实服务所需的有限理性；缺乏经过实证验证的情感整合机制。

Method: 构建包含欲望生成和目标管理的情感认知框架，建模LLM智能体的完整决策过程：状态演化、欲望生成、目标优化、决策生成和行动执行。

Result: 实验表明，采用该框架的智能体不仅表现出与情感状态一致的行为，在与其他智能体类型的比较中展现出更高的生态效度，决策结果显著更接近人类行为模式。

Conclusion: 该情感认知框架成功实现了LLM智能体与人类之间的情感对齐，为复杂社会系统中的虚拟人类模拟提供了更真实的行为基础。
...
<details>
  <summary>Details</summary>
Motivation: 虽然链式思维提示和深度推理能提升复杂任务性能，但对所有问题都应用深度推理计算成本过高，需要找到平衡性能与效率的方法。

Method: 采用小型LLM生成初始答案，大型LLM进行验证，仅在答案错误时启动深度推理过程，形成互补的代理系统。

Result: 在简单问题上，大型LLM的计算成本降低超过50%，准确率损失可忽略，同时在复杂任务上保持稳健性能。

Conclusion: 该互补代理系统有效平衡了计算效率与任务性能，为大规模LLM应用提供了实用的成本优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在个性化学习体验中缺乏目标对齐规划机制，需要开发能够根据个人目标自适应设计学习路径的系统。
...
TL;DR: 提出了Jericho测试时学习基准(J-TTL)来评估AI代理在测试时学习复杂技能的能力，并开发了EvoTest进化框架，通过进化整个代理系统来提高性能，无需微调或梯度计算。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理无法在测试时学习复杂技能，在陌生环境中表现不佳，这严重限制了其实际应用价值。需要系统性地衡量和推动这一挑战的进展。

Method: 提出了EvoTest进化测试时学习框架，包含两个角色：执行游戏的Actor代理和分析游戏记录以提出改进配置的Evolver代理。该框架通过重写提示、更新记忆、调整超参数和学习工具使用例程来改进代理。

Result: 在J-TTL基准测试中，EvoTest持续提升性能，不仅优于反思和仅记忆基线，也优于更复杂的在线微调方法。该方法是唯一能够赢得两个游戏（侦探和图书馆）的方法，而所有基线方法都未能赢得任何游戏。

Conclusion: EvoTest框架通过进化整个代理系统，在测试时学习方面取得了显著进展，为解决AI代理在陌生环境中学习复杂技能的挑战提供了有效方案。
...
TL;DR: 提出了一种基于效用的分析模型，用于自动驾驶车辆的环境感知系统，通过YOLOv8s进行目标检测，并基于模型性能评估感知服务的效用。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确感知道路上的多个对象并预测驾驶员感知，以控制车辆运动。现有研究需要开发能理解驾驶环境的感知系统。

Method: 1) 获取包含摩托车手、三轮车等独特对象的自定义数据集；2) 使用YOLOv8s进行目标检测；3) 基于训练模型实例的性能值测量感知服务的效用。

Result: 实验显示三个性能最佳的YOLOv8s实例：SGD-based (mAP@0.5=0.832)、Adam-based (0.810)和AdamW-based (0.822)。虽然SGD的mAP值最高，但AdamW在类别级别性能更好（如汽车：0.921，摩托车手：0.899等）。

Conclusion: 提出的感知模型能够有效评估学习模型的效用，为自动驾驶车辆确定合适的感知系统。
...
TL;DR: 提出了状态-动作联合攻击框架SAJA，通过协同扰动状态和动作来攻击多智能体深度强化学习模型，比单独攻击更有效且隐蔽。


<details>
  <summary>Details</summary>
Motivation: 现有研究只关注状态攻击或动作攻击，没有考虑如何有效联合两者。简单随机组合无法发挥协同效应，需要开发更有效的联合攻击方法。

Method: SAJA框架包含两个阶段：状态攻击阶段使用actor和critic网络通过多步梯度上升计算对抗状态；动作攻击阶段基于扰动状态使用critic网络通过梯度上升生成最终对抗动作，并添加启发式正则化器增强效果。

Result: 在MPE环境中评估表明，SAJA比单独的状态或动作攻击效果更好且更隐蔽，现有防御方法无法抵御其攻击。

Conclusion: SAJA框架成功展示了状态和动作联合攻击的协同效应，为多智能体深度强化学习的安全性研究提供了新视角。
...
TL;DR: 本文提出PORAT方法解决合理化中的模式崩溃问题，通过博弈论视角分析问题根源，引入策略干预引导模型达到更优均衡状态。


<details>
  <summary>Details</summary>
Motivation: 传统合理化方法存在模式崩溃问题，即预测器能正确预测但生成器持续输出崩溃模式的解释。现有研究缺乏统一考虑，需要从博弈论角度系统分析问题根源。

Method: 提出PORAT方法，从博弈论视角重新审视合作合理化，通过逐步引入策略干预来处理合作博弈过程中的均衡问题，引导模型达到更优解状态。

Result: 在9个真实世界数据集和2个合成设置上验证，PORAT相比现有最先进方法性能提升达8.1%。

Conclusion: PORAT通过博弈论策略优化有效解决了合理化中的模式崩溃问题，理论分析和实验验证均证明了方法的可行性。
...
TL;DR: 该研究评估了大型语言模型在隐式因果链发现任务中的机制性因果推理能力，发现LLMs主要通过关联模式匹配而非真正的因果推理来生成因果链。


<details>
  <summary>Details</summary>
Motivation: 探究原因如何导致结果，以及哪些中间因果步骤解释它们之间的联系，评估LLMs在机制性因果推理方面的能力。

Method: 在诊断评估框架中，让9个LLMs为给定的因果对生成所有可能的中间因果步骤，这些因果对来自气候变化辩论的论证研究资源。

Result: LLMs在生成的因果步骤数量和粒度上存在差异，虽然它们对中间因果连接具有自我一致性和信心，但主要基于关联模式匹配而非真正的因果推理。人类评估确认了生成链的逻辑连贯性。

Conclusion: 该研究为推进论证场景中的隐式机制性因果推理研究奠定了坚实基础，包括基线因果链发现方法、诊断评估见解和带因果链的基准数据集。
...
TL;DR: 提出了一种利用众包QoE数据进行移动网络覆盖和弱信号点分析的新框架，采用OC-SVM算法计算覆盖范围，能准确识别复杂城市环境中的信号弱区。


<details>
  <summary>Details</summary>
Motivation: 移动网络运营商需要有效评估网络覆盖并精确定位服务弱区，以提升用户体验质量(QoE)。

Method: 使用OC-SVM算法建模决策超平面作为有效覆盖边界，在单个天线级别进行覆盖分析，然后聚合到站点级别，并扩展应用于分析众包服务丢失报告。

Result: 该框架能准确映射移动网络覆盖，特别是在复杂城市环境中突出显示细粒度的信号缺陷区域。

Conclusion: 提出的基于OC-SVM的框架在移动网络覆盖分析和弱信号点识别方面表现出高效性。
...
TL;DR: 本文系统研究了Confidence-as-a-Reward (CRew)方法，这是一种无需训练、利用模型对最终答案的token级置信度作为奖励代理的简单而强大的方法，特别适用于封闭式任务。


<details>
  <summary>Details</summary>
Motivation: 现有的奖励模型需要大量精心策划的数据和昂贵的训练成本，而训练免费方法如LLM-as-a-Judge虽然利用LLM的内在推理能力取得了不错效果，但使用置信度作为奖励的概念尚未得到系统研究。

Method: 提出CRew方法，利用模型对最终答案的token级置信度作为奖励代理；进一步提出CRew-DPO训练策略，从置信度分数结合正确性信号构建偏好数据。

Result: 在数学推理任务上的实验表明，CRew在MATH500和RewardMATH基准上优于现有的训练免费奖励方法，甚至超过了大多数训练过的奖励模型；CRew分数与模型实际推理性能存在强相关性；CRew能有效筛选高质量训练数据；CRew-DPO进一步提升了模型的判断能力，持续优于现有的自训练方法。

Conclusion: 置信度作为奖励是一种简单而有效的训练免费方法，在数学推理任务中表现出色，且能有效用于构建偏好数据以增强模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在金融服务行业的采用面临性能评估障碍，传统机器学习指标无法很好泛化到GenAI工作负载，而专家评估和现有基准测试也存在局限性。

Method: 开发了一个风险评估框架，结合领域专家评估和机器学习指标，以更好地应对各种独特风险。

Result: 该框架能够更有效地应用领域专家和机器学习指标，解决性能评估中的风险问题。

Conclusion: 提出的风险评估框架有助于改善生成式AI在金融服务行业的性能评估和采用。
...
TL;DR: 提出tandem训练方法，通过随机将控制权交给较弱模型来训练强模型产生可理解解决方案，保持任务准确性的同时提高可解释性


<details>
  <summary>Details</summary>
Motivation: 随着语言模型快速发展，其推理过程可能变得难以被较弱代理和人类理解，这会削弱可解释性和监督能力。需要确保模型解决方案对较弱合作者保持可理解性

Method: 引入tandem训练范式：在强化学习中，随机从冻结的弱模型而非强模型中采样rollout tokens，只有当强模型的行为和推理过程能被弱模型继续时，rollout才会成功

Result: 在GSM8K数学推理任务中，tandem训练可靠地教会模型放弃专业术语，适应较弱合作伙伴的语言，同时保持高任务准确率

Conclusion: 该方法为构建可被较弱代理审计的AI系统提供了有前景的途径，对人类-AI协作和多智能体通信具有重要意义
...
<details>
  <summary>Details</summary>
Motivation: 基于逻辑的模型可用于构建机器学习分类器的验证工具，这些分类器在法律领域执行案例推理，需要形式化方法来处理先例冲突

Method: 引入分类器模态逻辑，将案例的时间维度和法院系统层级纳入逻辑框架，以解决先例之间的冲突

Result: 开发了一个能够正式捕捉法律案例推理的逻辑系统，包含处理先例冲突的原则

Conclusion: 该模态逻辑为法律领域的机器学习分类器验证提供了形式化基础，能够处理复杂的先例冲突问题
...
TL;DR: 提出一种基于最大化人类赋权（empowerment）的辅助语言模型调优方法Empower，仅需离线文本数据，无需额外人工反馈即可训练出更好的辅助AI助手。


<details>
  <summary>Details</summary>
Motivation: 现有辅助代理方法往往鼓励AI独立完成任务而非真正协助人类实现目标，且需要昂贵的人工反馈。需要一种更有效、成本更低的辅助AI训练方法。

Method: 基于最大化人类赋权（在环境中实现期望改变的能力）的目标，使用离线文本数据进行自监督微调语言模型。

Result: 用户研究中78%的参与者偏好Empower助手（p=0.015），接受率提高31%，建议减少38%。在代码辅助环境中，Empower使模拟程序员在挑战性编程问题上的成功率比SFT基线平均提高192%。

Conclusion: Empower方法提供了一个仅使用离线数据、无需额外人工反馈或可验证奖励的大规模有用对齐AI代理框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统AI安全护栏主要依赖输出分类和人工指定标准，无法应对新的危险情况，且检测到不安全后通常只能拒绝行动，这本身可能不安全。需要一种能主动预防下游危害的动态安全方法。

Method: 将AI代理安全形式化为顺序决策问题，基于安全关键控制理论在AI模型的潜在表示空间中构建预测性护栏，通过安全关键强化学习进行大规模训练，实现模型无关的实时监控和风险修正。

Result: 在模拟驾驶和电子商务场景中的实验表明，控制理论护栏能可靠地引导LLM代理避免灾难性后果（从碰撞到破产），同时保持任务性能。

Conclusion: 控制理论护栏为当前标记-阻止机制提供了原则性的动态替代方案，能够主动预防AI代理在交互过程中可能造成的下游危害。
...
TL;DR: Hard2Verify是一个人工标注的步骤级验证基准，用于评估前沿LLM在数学推理中的步骤验证能力，发现开源验证器普遍落后于闭源模型。


<details>
  <summary>Details</summary>
Motivation: 在IMO 2025等数学竞赛中，LLM需要生成每个步骤都正确且有充分支持的证明，因此需要强大的步骤级验证器来检测错误。

Method: 构建了Hard2Verify基准，包含500多小时人工标注的步骤级验证数据，评估了29个生成式批评器和过程奖励模型。

Result: 除了少数优秀模型外，开源验证器在步骤级验证任务上表现落后于闭源模型。
