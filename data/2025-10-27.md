...
Method: 在适配器训练期间，通过ControlNet或LoRA等辅助模块为混淆因素提供捷径，消除适配器内部化这些因素的动力，推理时移除辅助模块。
...
Method: VAP 将问题重新定义为上下文生成，使用参考视频作为直接语义提示，通过即插即用的混合变换器专家引导冻结的视频扩散变换器，并采用时间偏置位置嵌入来消除虚假映射先验。

Result: VAP 作为单一统一模型，在开源方法中达到了新的最先进水平，获得了38.7%的用户偏好率，可与领先的条件特定商业模型相媲美。

Conclusion: VAP 在零样本泛化和支持各种下游应用方面表现出色，标志着向通用可控视频生成迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割对临床诊断和治疗至关重要，但传统CNN难以捕捉全局上下文信息和长距离依赖关系，影响对复杂边界和不同大小结构的精确分割。
...
Conclusion: FM-BFF-Net通过融合CNN和Transformer的优势，有效解决了医学图像分割中的全局上下文捕捉和边界精度问题，在多种医疗成像场景中表现出色。
...
Method: 采用流匹配训练框架，结合判别式跟踪器的迭代优化、跨窗口一致性先验和针对点坐标调整的方差调度，在推理时使用最佳优先搜索策略
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在3D医学图像中难以准确定位解剖区域并进行逐步推理，这限制了与临床诊断工作流程的对齐和可信的临床-AI协作。

Method: 创建了3DReasonKnee数据集，每个样本包含3D MRI体积、针对特定解剖区域的诊断问题、3D边界框定位、临床医生生成的诊断推理步骤以及结构化严重程度评估。

Result: 建立了ReasonKnee-Bench评估基准，对五个最先进的VLM进行了基准测试，为定位和诊断准确性提供了基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法受光照和材料属性限制，无法有效处理透明、半透明等特殊材质物体的3D重建。热偏振成像能够克服可见光偏振分析中的模糊性问题。

Method: 建立了偏振观测的一般理论，证明长波红外偏振成像不受可见光偏振模糊性影响，提出基于多视角热偏振图像的3D形状恢复方法。

Result: 实验结果表明该方法能有效重建透明、半透明和异质物体的精细细节，性能优于现有技术。

Conclusion: 热偏振成像为不受光照和材料限制的详细3D形状重建提供了可行方案，特别适用于传统方法难以处理的特殊材质物体。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在分布偏移和标注稀缺领域表现不佳的问题，传统监督微调不可行，而自监督学习在视觉编码器模型中效果有限。

Method: 采用自蒸馏范式，精心调整预测头并部署参数高效适应技术，利用多视角对象中心视频进行无标注训练。

Result: 在3个视觉基础模型和2个数据集上的实验表明，VESSA在下游分类任务中相比基础模型和先前适应方法有持续改进。

Conclusion: VESSA有效解决了视觉基础模型在无标注域适应中的挑战，通过多视角对象视频学习实现了鲁棒性提升。
...
Method: 基于当前SOTA基线，通过低光图像增强和基于开放词汇检测的基础模型引导的背景去除，减少领域偏移和背景伪影，抑制原始SAM输出的误报。

Result: 在BOP的真实工业拣选基准测试中，该方法显著提升了检测精度，同时推理开销可忽略不计。
...
Method: 构建新数据集，使用YOLOv11模型检测裂缝、剥落和暴露钢筋，通过微调、数据增强和混合框架自动分类损伤等级

Result: 开发了能够自动可靠地从输入图像确定损伤等级的混合框架，证明了跨不同损伤场景的快速自动化检测可行性

Conclusion: 利用图像数据收集、标注和深度学习方法，可以实现灾后快速自动化损伤检测
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景图生成方法局限于单视角、不支持增量更新、缺乏3D空间几何接地，无法满足具身智能场景的需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义分割网络大多采用强大的预训练编码器但使用简单解码器，导致语义上下文和精细细节保留之间的权衡不理想。

Method: 提出WaveSeg解码器架构：1)从输入图像学习高频分量作为显式先验强化边界细节；2)应用多尺度融合机制Dual Domain Operation；3)提出Spectrum Decomposition Attention块利用Mamba的线性复杂度长程建模；4)使用重参数化卷积在小波域保持低频语义完整性；5)残差引导融合集成多尺度特征和边界感知表示。

Result: 在标准基准测试上的广泛实验表明，WaveSeg在定量和定性上都持续优于最先进的方法，实现了高效精确的分割。

Conclusion: WaveSeg通过在小波域利用频率先验和基于Mamba的注意力机制，能够实现语义和结构丰富的特征图，为语义分割提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在神经丛分类任务中表现良好但缺乏可解释性，且不符合医生的决策方式。需要开发能够整合临床专业知识的多模态方法。

Method: 使用基于对比语言-图像预训练的视觉语言模型，整合从医学教科书和论文中提取的专家文本概念，通过QuiltNet编码提示来对齐临床语义线索和视觉特征。

Result: 模型在多个分类指标上优于CNN模型（VGG-19、ResNet-18、ResNet-50），准确率达到83.9%，精确率86.6%，特异性87.6%。
...
Method: 定义了直方图位置矩阵和直方图计数矩阵，构建了基于直方图的两级优化模型，通过迭代求解光照和反射直方图，最后通过直方图匹配实现图像增强。

Result: 在1000*664分辨率图像上仅需1.86秒执行时间，比现有方法至少节省6.67秒，同时在可见性和性能指标上优于现有方法。

Conclusion: HistRetinex方法在保持增强质量的同时显著提高了处理速度，为低光照图像增强提供了一种高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型主要研究静态、完全可观察环境下的视觉推理，但在现实世界中信息往往因遮挡或视野限制而不完整。受人类主动探索环境能力的启发，需要开发能够主动获取信息的视觉推理系统。

Method: 引入CLEVR-AVR模拟基准和AVR-152k大规模数据集，提供丰富的思维链标注。开发了PhysVLM-AVR模型，在更高阶马尔可夫决策过程中训练智能体进行不确定性识别、信息增益预测和行动选择。

Result: PhysVLM-AVR在CLEVR-AVR、具身推理(OpenEQA, RoboVQA)和被动视觉推理(GeoMath, Geometry30K)任务上达到最先进性能。分析发现当前具身MLLMs虽然能检测信息不完整性，但在主动获取和整合新信息方面仍有困难。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法对垂直偏差、地面坡度和视角失配敏感，且缺乏对象身份信息；基于点云的神经网络方法内存需求大、需要完美预对齐、难以处理薄结构且缺乏类别一致关联，无法解决分裂合并问题。

Method: 采用多分辨率NDT和点对平面ICP进行时序对齐，高度归一化，基于配准协方差和表面粗糙度计算检测水平，通过几何代理、语义实例分割和类别约束二分图分配实现跨时期关联，使用分块处理限制内存。

Result: 在15个Subiaco区块上达到95.2%准确率、90.4% mF1和82.6% mIoU，超过Triplet KPConv方法，在Decreased类别上IoU达到74.8%，提升7.6个百分点。

Conclusion: 该方法在保持高精度的同时有效处理了城市级LiDAR变化检测中的分裂合并问题，对部分重叠和采样变化具有稳定性。
...
Method: 构建轻量级动态混合局部先验提取器，通过异构卷积捕获多样局部先验；设计双向交互适配器，使用余弦对齐可变形注意力和通道自适应尺度增强进行特征交互重构。

Result: 在31个SOTA方法对比中表现出优越的分割性能，适应多个二值目标分割任务。

Conclusion: Controllable-LPMoE通过动态先验微调范式，以较少可训练参数实现了高效微调，在分割任务中表现出色且具有良好适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像安全数据集标签粗糙模糊，无法区分驱动安全差异的具体特征。需要系统方法来识别使图像不安全的特定元素。

Method: 利用图像编辑模型对图像进行针对性修改，仅改变与安全策略相关的特征，从而翻转安全标签，同时保持安全无关细节不变。

Result: 构建了包含3020个SafetyPair图像的新安全基准，涵盖9个安全类别，揭示了视觉语言模型在区分细微差异图像方面的弱点，并可作为有效的数据增强策略。

Conclusion: SafetyPairs提供了首个系统研究细粒度图像安全差异的资源，既能评估模型性能，又能提高轻量级防护模型的训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习框架在提升通用CoT推理能力时，往往难以泛化到训练分布之外。需要一种系统性的多模态RL框架来增强探索能力和泛化性能。

Method: 1) 噪声注入探索策略：在视觉输入中加入可控高斯噪声，鼓励在更广泛的视觉场景中进行探索；2) 贝叶斯优势估计：将优势估计建模为贝叶斯推理问题，融合噪声水平和轨迹奖励信息来计算稳健的后验优势估计。

Result: 在标准CoT质量、通用能力和幻觉基准测试中，NoisyGRPO显著提高了泛化性和鲁棒性，特别是在小规模MLLMs（如Qwen2.5-VL 3B）的RL设置中表现优异。

Conclusion: NoisyGRPO通过噪声注入和贝叶斯建模有效提升了多模态大语言模型的CoT推理泛化能力，为解决RL训练中的泛化问题提供了系统解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CTPA是诊断肺血管疾病的金标准，但依赖碘对比剂存在肾毒性和过敏反应风险，特别是对高危患者。需要开发不依赖对比剂的替代方法。

Method: 使用基于CycleGAN的级联合成器，从非对比CT扫描生成数字对比CTPA。研究使用410对CTPA和NCCT扫描，分为训练验证集(249对)和测试集(161对)。

Result: 在验证集上MAE:156.28、PSNR:20.71、SSIM:0.98；测试集上MAE:165.12、PSNR:20.27、SSIM:0.98。下游任务中肺血管分割的Dice、clDice、clRecall分别为0.70、0.71、0.73(动脉)和0.70、0.72、0.75(静脉)。血管体积ICC从0.70提升到0.81。

Conclusion: 该方法成功从NCCT生成DCCTPA，实现了有效的血管增强，特别是在小血管方面，为高危患者提供了一种安全的替代诊断方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基础模型中的视觉空间智能评估主要依赖文本提示和VQA式评分，这种方法模糊了几何信息、容易产生语言捷径，且难以准确归因于真正的空间技能。

Method: 引入SIG结构化网格模式，作为文本的补充通道，提供场景结构的忠实组合表示。基于SIG开发评估指标，量化模型的内在视觉空间智能，将空间能力与语言先验分离。

Result: 在少样本上下文学习中，SIG相比纯VQA表示在所有VSI指标上都获得了更一致、更稳定和更全面的提升。发布了包含1.4K驾驶帧的SIGBench基准数据集。

Conclusion: SIG作为数据标注和训练模式，在学习和评估视觉空间智能方面具有潜力，能够更准确地衡量基础模型的空间推理能力。
...
Method: BFM将生成轨迹划分为多个时间段，每个段由专门的速度块建模；引入语义特征引导模块，使速度块基于预训练表示的语义丰富特征进行条件生成；提出轻量级特征残差近似策略，在保持语义质量的同时显著降低推理成本。

Result: 在ImageNet 256x256上的实验表明，BFM相比现有Flow Matching方法建立了显著改进的帕累托前沿，在可比生成性能下实现了2.1倍到4.9倍的推理复杂度加速。

Conclusion: BFM通过块状设计和语义特征引导，在保持生成质量的同时显著提高了Flow Matching模型的推理效率。
...
Method: 将标记无关的文本空间扩展为一组正交子空间，通过最优传输问题动态分配每个标记到语义相关的子空间组合，并使用top-k掩码来稀疏化传输计划。

Result: 大量实验证明了TokenCLIP的优越性。

Conclusion: TokenCLIP通过动态标记级对齐机制，有效提升了零样本异常检测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决细粒度食品图像检索问题，提升检索性能以获得更好的竞赛排名。

Method: 独立训练四个基础模型（ArcFace和Circle损失的加权和），应用TTA和集成学习提升特征表示能力，提出基于扩散和k-互惠重排的新重排方法。

Result: 在公开排行榜上获得0.81219 mAP@100，私有排行榜上获得0.81191 mAP@100，排名第三。

Conclusion: 提出的集成策略和新重排方法有效提升了细粒度食品图像检索的性能，证明了该方案在竞赛中的竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 食物分析在健康领域日益重要，细粒度食物识别任务在其中扮演关键角色。本文旨在解决LargeFineFoodAI-ICCV Workshop-Recognition挑战赛中的细粒度食物识别问题。

Method: 采用Arcface损失和Circle损失的适当组合来提升模型性能，通过精心调优的配置训练模型，并使用集成方法获得最终结果。

Result: 该解决方案在竞赛中获得了第三名的成绩。

Conclusion: Arcface损失和Circle损失的组合能够有效提升细粒度食物识别的性能，证明了该方法在食物识别任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决shortcut模型在广泛采用中遇到的五个关键性能瓶颈问题，使其成为可行且具有竞争力的生成模型类别。

Method: 提出iSM统一训练框架，包含四个关键改进：内在引导提供动态控制、多级小波损失缓解频率偏差、缩放最优传输减少训练方差、双EMA策略平衡训练稳定性与自一致性。

Result: 在ImageNet 256×256上的广泛实验表明，该方法在一步、少步和多步生成中都显著优于基线shortcut模型，FID指标有实质性提升。
...
Conclusion: TSSR通过并行生成和解耦策略，在3D网格生成任务中实现了高效且高质量的成果，为艺术家风格的3D内容创作提供了新方法。
...
Method: 提出SAGE-3D方法，包括：(1)对象中心语义标注，为3DGS添加对象级细粒度标注；(2)物理感知执行连接，将碰撞对象嵌入3DGS并构建丰富的物理接口。

Result: 发布了包含1K对象标注3DGS室内场景数据的InteriorGS，以及首个基于3DGS的VLN基准SAGE-Bench(含2M VLN数据)。实验显示3DGS场景数据更难收敛但具有强泛化性，在VLN-CE Unseen任务上基线性能提升31%。
...
<details>
  <summary>Details</summary>
Motivation: 由于输入分辨率受限，MLLMs在理解高分辨率图像中的视觉细节和定位极小物体方面面临挑战，特别是在杂乱背景中。

Method: 采用从粗到精的流程，包括全局语义探索(GSE)和局部感知细化(LPR)两个阶段，通过定位感知的回顾奖励机制耦合两个阶段。

Result: 在FineRS-4k和公共数据集上的实验表明，该方法在指令引导分割和视觉推理任务上持续优于最先进的MLLM方法。

Conclusion: FineRS框架有效解决了MLLMs在高分辨率图像中处理极小物体的挑战，提升了分割和推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型的多模态对齐组件的可解释性尚未得到研究，因为难以将多模态表示的语义映射到统一概念集中。

Method: 提出VL-SAE稀疏自编码器，使用基于距离的编码器和两个模态特定解码器，通过余弦相似度测量多模态表示的语义相似性，并在自监督训练中鼓励语义相似表示具有一致的神经元激活。

Result: 在多个VLMs上的实验表明，VL-SAE在解释和增强视觉-语言对齐方面具有优越能力，能够通过概念比较理解视觉与语言表示的对齐，并在下游任务中提升性能。

Conclusion: VL-SAE通过将视觉-语言表示映射到统一概念集，有效解释和增强了多模态对齐，为理解视觉-语言模型的内部机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂果园环境中苹果叶病表型异质性带来的挑战，传统多尺度特征融合方法无法充分处理局部与全局特征关系。

Method: 使用CNN提取局部病变细节特征，Vision Transformer捕捉全局结构关系，自适应特征融合模块动态融合特征，并采用多模态图像-文本学习方法利用CLIP预训练权重。

Result: 在公开苹果病数据集和自建数据集上分别达到97.38%和96.12%的准确率，优于多个基线方法。

Conclusion: CT-CLIP在农业病害识别中表现出强大能力，显著提升了复杂环境条件下的识别精度，为农业自动化病害识别提供了创新实用的解决方案。
...
Conclusion: 该方法有效解决了无人机跟踪中的语义感知问题，提高了在挑战性场景下的跟踪性能，同时提供了灵活部署的选项。
...
Method: 使用2.5D U-Net++架构，ResNet-34编码器，联合训练和区域微调。结合L1损失和IMPACT-Synth感知损失，采用测试时增强和五折集成。评估了Elastix和IMPACT两种配准策略。

Result: 在本地测试集上，IMPACT配准实现了更准确的对齐和更低的MAE，但在公共验证集上，Elastix配准获得更高分数，反映了评估流程中的配准偏差。

Conclusion: 配准误差会传播到监督学习中，影响训练和评估。IMPACT通过促进解剖一致性对齐，有助于减轻这种偏差，支持开发更稳健的sCT合成模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型固定去噪步骤，不考虑下游传输限制，在带宽受限场景下会导致过度压缩和计算浪费。

Method: 引入端到端联合训练策略，通过轻量级质量嵌入指导去噪轨迹，使模型能够根据目标质量水平自适应调制去噪过程。

Result: 实验结果表明，相比简单提前停止方法，该方法显著提高了带宽适应生成的视觉保真度。
...
Result: 实验结果表明，TerraGen在多样化任务中实现了最佳生成图像质量，并能作为通用数据增强生成器，显著提升下游任务性能，在完整数据和少样本场景下都展示了强大的跨任务泛化能力。

Conclusion: TerraGen解决了遥感视觉任务中数据标注不足和任务孤立的问题，通过统一的布局到图像生成框架实现了空间可控的图像合成，为多任务遥感分析提供了有效的数据增强解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 图像拼接中物体深度变化导致的大视差会引起重影和错位问题，需要解决多视角对齐困难。

Method: 采用多阶段机制结合全局深度正则化约束增强不同深度范围目标的对齐精度；使用基于图的低成本计算确定最优拼接缝，并通过软缝区域扩散精确定位过渡区域；引入重参数化策略优化结构设计。

Result: 大量实验证明该方法在现有方法中表现出优越性能。

Conclusion: 该方法能有效缓解视差引起的对齐误差，实现自然无缝的拼接效果，同时显著提高算法效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频检索方法在处理未剪辑长视频和多模态查询方面存在局限性，需要专门针对长视频平台应用的检索基准。

Method: 构建基于核心视频内容的多层次视觉对应关系（复制、事件、场景、实例、动作等6个级别），开发3个版本的MUVR基准（Base、Filter、QA）进行评估。

Result: 在53K个未剪辑视频和1050个多模态查询上评估了3个SOTA视频检索模型、6个基于图像的VLM和10个MLLM，揭示了现有方法在处理未剪辑视频和多模态查询方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言引导的视觉概念学习方法局限于预定义的少数语义维度（如颜色、形状），且主要在合成数据集上探索，无法适应真实世界中丰富多样的概念。

Method: 利用预训练的视觉语言模型和通用提示策略识别多样化的图像相关概念轴；通过通用概念编码器自适应地将视觉特征绑定到发现的概念轴；优化组合锚定目标确保各概念轴可独立操作。

Result: 在ImageNet、CelebA-HQ和AFHQ数据集上展示了优越的编辑能力，能够处理过于多样化而无法手动预定义的真实世界概念；在组合泛化方面优于现有方法。

Conclusion: 该框架能够有效学习和操作真实世界中的多样化视觉概念，无需预定义概念类别，在概念编辑和组合泛化方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在合成具有关节结构的3D物体时，难以同时保证几何一致性、准确关节运动和外观真实性，特别是在处理关节依赖的可见性变化方面存在不足。

Method: 1. 使用变分自编码器将稀疏体素表示和关节属性嵌入统一潜在空间；2. 训练潜在扩散模型进行多样化采样；3. 引入关节感知高斯解码器，在重建时考虑关节状态，为通常被遮挡的区域分配合理纹理。

Result: 在PartNet-Mobility和ACD数据集上的实验表明，ArtiLatent在几何一致性和外观保真度方面优于现有方法，能够生成物理上合理的多样化样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LUT实现CNN方法使用平均池化来融合不同方向的像素块查找结果，但这种方法不适合处理各向异性信号结构，导致性能受限。

Method: 1. 引入广义中值池化方法；2. 扩展为学习数据相关的池化系数，使不同方向的像素块贡献能够自适应加权。

Result: 在各种恢复基准测试中，所提出的各向异性池化策略在感知质量和数值指标上都优于现有的LUT可实现的CNN方法。

Conclusion: 各向异性池化策略能够有效提升LUT实现CNN图像恢复方法的性能，特别是在处理各向异性信号时表现更优。
...
TL;DR: PhysWorld是一个从模拟器中合成物理演示来学习高效世界模型的框架，用于预测可变形物体的动态，推理速度比现有方法快47倍


<details>
  <summary>Details</summary>
Motivation: 从有限的真实世界视频数据中学习物理一致的动态模型具有挑战性，特别是对于具有空间变化物理属性的可变形物体

Method: 首先在MPM模拟器中构建物理一致的数字孪生，然后对物理属性施加部分感知扰动生成多样化运动模式，最后用这些演示训练轻量级GNN世界模型

Result: PhysWorld实现了对各种可变形物体的准确快速未来预测，并能很好地泛化到新交互，推理速度比PhysTwin快47倍

Conclusion: PhysWorld框架通过模拟器合成物理演示有效解决了数据稀缺问题，实现了高效准确的可变形物体动态预测
...
TL;DR: 提出MoniTor方法，一种基于记忆的在线评分队列方案，用于无需训练的在线视频异常检测，通过结合预训练视觉语言模型和LSTM启发的预测机制来有效捕捉时间依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频异常检测研究主要集中在离线场景，而在线VAD由于实时约束和计算强度很少受到关注，需要解决在线检测的固有复杂性。

Method: 使用流式输入到视觉语言模型，结合LSTM启发的预测机制来建模过去状态，设计评分队列和异常先验来动态存储最近分数并覆盖监控场景中的所有异常。

Result: 在UCF-Crime和XD-Violence两个大型数据集上的评估表明，MoniTor优于最先进方法，且与弱监督方法相比具有竞争力，无需训练。

Conclusion: MoniTor为在线视频异常检测提供了一种有效的无需训练解决方案，能够有效捕捉时间依赖性并区分正常与异常行为。
...
TL;DR: VidSplice是一个视频修复框架，通过解耦视频修复为多帧一致性图像修复和掩码区域运动传播两个子任务，引入间隔帧先验来增强时空稳定性。
...
Method: 提出VidSplice框架，包含CoSpliced模块进行首帧传播策略，以及上下文控制器模块将拼接视频注入I2V生成主干，约束内容失真。

Result: 广泛评估显示VidSplice在多样化视频修复场景中取得竞争性性能，显著改善前景对齐和运动稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在胸部X光诊断中准确性高但缺乏可解释性，临床医生需要透明的预测解释来信任自动化诊断并识别潜在失败模式。
...
Conclusion: CXR-LanIC证明医学AI系统可以既准确又可解释，通过从特定诊断目标训练的模型中提取可解释特征，确保发现的模式与临床决策直接相关，支持更安全的临床部署。
...
TL;DR: 提出双流架构整合组织宏观特征和细胞级特征，通过循环变换器高效聚合细胞信息，并引入双向组织-细胞交互模块，在病理亚型分类任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型虽然能捕捉全局组织上下文，但缺乏细胞级特征建模，限制了在癌症亚型分类等细粒度任务中的表现。

Method: 采用双流架构分别建模组织宏观特征和聚合细胞表示；提出接受度加权的键值聚合模型（循环变换器）以线性复杂度捕获细胞间依赖关系；引入双向组织-细胞交互模块实现局部细胞线索与周围组织环境的相互关注。

Result: 在四个病理亚型分类基准测试中，所提方法优于现有模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决封闭区域多摄像头重叠视角下的行人跟踪问题，特别是频繁发生行人遮挡的情况，需要更有效的多目标跟踪方法。

Method: 采用图加权解决方案，在线更新行人身份标签，结合轨迹和特征信息，包含特征提取、跟踪和社区搜索等模块，并配备了行人位置估计模块。

Result: 在封闭区域模型和公开真实数据集上的测试表明，该方法在高度拥挤空间中的表现优于没有位置数据的方法。

Conclusion: GRAP-MOT在拥挤场景中表现出优越性能，同时分析发现IDF1指标比MOTA更适合此类算法比较，代码和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 胎盘早剥是妊娠期严重并发症，传统超声诊断方法依赖医生经验，存在主观偏差和诊断不一致性问题，需要自动化检测方案。

Method: 基于小样本学习改进YOLOv11n模型，集成小波卷积和坐标卷积增强频率和空间特征提取，加入级联组注意力机制抑制超声伪影和遮挡干扰。

Result: 检测准确率78%，在精确率-召回率曲线、置信度分数和遮挡场景下表现显著优于基线模型，兼具高精度和实时处理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉编码器主要关注全局图像表示，忽视了细粒度区域分析，由于缺乏细粒度标注数据和预训练范式，在细粒度感知方面存在限制。

Method: 构建Gran-29M数据集（200万张图像，1.8亿个区域级标注），采用预训练-适应框架和自蒸馏机制，通过边界框到标题回归和标题到边界框回归来增强局部视觉表示和定位能力。

Result: GranViT超越了现有视觉编码器，在不同LLMs上展现出强迁移性，在细粒度识别、多模态VQA和OCR理解任务上达到最先进结果。
...
TL;DR: 该论文提出了前瞻性引导（FSG），一种新的条件引导方法，通过将条件引导重新定义为定点迭代问题，解决了现有Classifier-Free Guidance（CFG）方法效率低下的问题。
...
Method: 将条件引导重新定义为定点迭代问题，寻找潜在空间的黄金路径。提出FSG方法，在早期扩散阶段优先解决长间隔子问题并增加迭代次数。
...
Method: 基于已有的可解释性方法，通过信号处理视角重新解释使用最终解码层探测中间激活的实践，从而分析多个样本并基于相关性对注意力头进行排序。

Result: 在单模态和多模态Transformer中都发现了一致的注意力头专业化模式，仅编辑1%的注意力头就能可靠地抑制或增强目标概念。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态系统通常需要端到端重新训练，参数量大且缺乏模块化。本文旨在开发一种轻量级方法，能够利用预训练模型的强大能力，同时保持可控性和模块化。

Method: 连接V-JEPA2视频嵌入到冻结的Stable Audio Open DiT文本到音频模型，通过在现有文本交叉注意力后插入紧凑的视频交叉注意力层。使用视频token池化来减少内存消耗和稳定训练。

Result: 在精选的视频-音频基准测试中，Foley Control在时间和语义对齐方面表现具有竞争力，参数量远少于最近的多模态系统，同时保持提示驱动的可控性和生产友好的模块化。

Conclusion: Foley Control提供了一种高效且模块化的视频引导音频生成解决方案，该方法可以扩展到其他音频模态（如语音）。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Group Relative Policy Optimization (GRPO)方法在基于流匹配的文本到图像生成中存在两个关键限制：优势归因不准确，以及忽略了生成的时序动态特性。

Method: 提出Chunk-GRPO方法，将连续步骤分组为连贯的'块'来捕捉流匹配的内在时序动态，在块级别进行策略优化，并引入可选的加权采样策略来进一步提升性能。

Result: 大量实验表明，Chunk-GRPO在偏好对齐和图像质量方面都取得了优异的结果。
...
TL;DR: MATrack是一个专为夜间无人机跟踪设计的系统，通过多尺度层次融合、自适应关键令牌门和夜间模板校准器三个核心模块，解决了低光条件下跟踪的挑战，在UAVDark135基准上性能显著优于现有方法，同时保持81 FPS的实时处理速度。


<details>
  <summary>Details</summary>
Motivation: 夜间无人机跟踪面临低光照条件、复杂背景和频繁视角变化的挑战，现有基于低光增强和域适应的方法存在视觉伪影、计算成本高和无法充分利用动态对象信息等问题。

Method: 提出MATrack系统，包含三个核心模块：多尺度层次融合(MHB)增强静态和动态模板间的特征一致性，自适应关键令牌门准确识别复杂背景中的目标信息，夜间模板校准器(NTC)确保长序列中的稳定跟踪性能。

Result: 在UAVDark135基准测试中，精确度、归一化精确度和AUC分别比最先进方法提升了5.9%、5.4%和4.2%，同时保持81 FPS的实时处理速度。在实际无人机平台上的测试验证了系统的可靠性。

Conclusion: MATrack能够为夜间搜救、边境巡逻等关键机器人应用提供稳定有效的夜间无人机跟踪支持，解决了现有方法在真实无人机系统中的局限性。
...
TL;DR: TIGER是一个两阶段文本图像引导超分辨率框架，通过"先文本后图像"的方法，在提升图像质量的同时保持文本可读性，打破了传统方法中图像质量与文本可读性的权衡。
...
Method: 提出TIGER两阶段框架：首先重构精确的文本结构，然后使用这些文本结构来指导后续的全图像超分辨率，实现从字形到图像的引导。
...
TL;DR: 使用深度学习模型自动检测MEG记录中的发作间期棘波，提出了基于特征的ANN和CNN两种架构，在仅有单专家标注的临床数据上表现优于现有方法，并采用交互式机器学习策略提高标注质量。


<details>
  <summary>Details</summary>
Motivation: 药物难治性癫痫的术前评估中，MEG检测发作间期棘波对定位致痫区很重要，但手动检测耗时且易出错，现有自动化方法要么需要大量标注数据，要么在非典型数据上缺乏鲁棒性。
...
Conclusion: 简单架构的模型在分析复杂且标注不完美的数据时具有鲁棒性，交互式机器学习方法为快速数据标注提供了潜力，所提模型是自动检测发作间期棘波的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 显著目标检测任务面临数据限制问题，昂贵的像素级标注迫使相关子任务需要单独训练模型。
...
Conclusion: 大规模合成数据生成和模糊感知架构能有效解决显著目标检测的数据限制问题，显著提升模型泛化能力。
...
TL;DR: 提出了Modest-Align轻量级跨模态对齐框架，通过随机扰动和嵌入平滑策略解决资源受限场景下模型过度自信和性能下降问题，在少量数据和计算资源下实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态对齐模型（如CLIP）在大规模数据上表现良好，但在资源受限、数据质量差的场景中容易过度自信且性能下降，因为对比学习方法会强化不确定样本的过度自信。

Method: 使用两种互补策略：随机扰动（引入可控噪声模拟不确定性）和嵌入平滑（在嵌入空间校准相似度分布），共同降低过度自信并提升噪声或弱对齐样本的性能。
...
TL;DR: 通过基于偏好的优化方法，将成对极几何约束融入视频扩散模型，解决几何不一致性和不稳定运动问题，生成空间一致的视频。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在几何一致性、稳定运动和视觉伪影方面仍存在不足，3D一致视频生成对下游应用有重要意义。

Method: 使用成对极几何约束进行基于偏好的优化，在不要求端到端可微的情况下有效实施几何原则，在静态场景和动态相机设置下训练。

Result: 经典几何约束比现代学习度量提供更稳定的优化信号，模型能有效泛化到多样动态内容，保持视觉质量。

Conclusion: 通过将数据驱动的深度学习与经典几何计算机视觉相结合，提出了一种实用的空间一致视频生成方法。
...
TL;DR: 提出DAP-MAE方法，通过自适应整合跨域点云数据集知识来解决点云数据稀缺问题，在单一预训练下在多个3D点云分析任务中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 点云数据在不同领域的训练数据规模有限，现有方法混合不同领域数据进行MAE预训练，但学到的先验知识与下游任务可能不匹配，导致性能下降。
...
Conclusion: DAP-MAE能有效解决跨域点云数据预训练中的领域适应问题，为通用点云分析提供了有效的预训练解决方案。
...
TL;DR: 提出Gompertz-CNN动态知识蒸馏框架，通过Gompertz生长模型调节蒸馏损失权重，结合Wasserstein距离和梯度匹配，在CIFAR数据集上相比传统方法提升准确率最高8%。
...
Conclusion: Gompertz-CNN框架通过模拟人类学习曲线，实现了更有效的动态知识蒸馏，显著提升了学生模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决纯IMU方法在平移估计和多人相对定位方面的局限性，因为惯性线索本质上是自我参考的，无法提供他人的直接空间参考。

Method: 使用超宽带测距(UWB)估计传感器间的绝对距离，将距离测量与惯性观测融合到结构化状态空间模型中，采用新颖的两步优化方法利用估计距离来准确跟踪全局轨迹。

Result: 在合成和真实世界数据上的评估显示，该方法在准确性和鲁棒性方面优于现有最先进方法，创建了首个IMU+UWB双人跟踪数据集GIP-DB。

Conclusion: 展示了IMU+UWB基于多人运动捕捉在野外环境中的潜力，为多人运动跟踪提供了更准确和鲁棒的解决方案。
...
TL;DR: 本文研究了在NACTI野生动物数据集上应用长尾识别方法，通过实验优化LTR损失函数和正则化，显著提升了物种识别准确率，从95.51%提高到99.40%，并在跨域测试中展示了更强的泛化能力。


<details>
  <summary>Details</summary>
Motivation: NACTI数据集存在严重的类别不平衡问题，最大的'Head'类别占数据集50%以上。为了解决这种长尾分布对物种识别性能的影响，需要系统研究长尾识别方法。

Method: 基于PyTorch Wildlife模型，系统研究了各种LTR损失函数和LTR敏感正则化方法，构建了减少偏差的测试集来评估域偏移。

Result: 最佳配置在NACTI测试集上达到99.40%的Top-1准确率，相比标准交叉熵的95.51%有显著提升。在跨域测试中，优化后的长尾增强模型达到52.55%准确率，优于WCE损失的51.20%。

Conclusion: LTR增强调度器选择在野生动物领域带来了一致的改进，特别是与先进的LTR损失函数结合时。但LTR方法仍无法充分解决某些问题，包括在严重域偏移下对'Tail'类别的灾难性失效。
...
TL;DR: 提出了一种无监督的突触分类方法，基于相邻突触比随机突触更相似的假设，无需预先知道突触类型数量。


<details>
  <summary>Details</summary>
Motivation: 传统突触分类需要监督学习，需要提供不同类别的样本。本文旨在开发一种仅基于相邻突触相似性假设的无监督分类方法。

Method: 利用相邻突触比随机突触更相似的观察，开发无监督分类算法，无需预先指定突触类型数量。

Result: 在果蝇数据上成功应用该方法，能够有效分离突触类型。

Conclusion: 该方法提供了一种无需先验知识的突触分类方式，并为选择具有代表性的地面真值数据提供了原则性方法。
...
TL;DR: 评估UNI和Virchow2两种基础模型作为特征提取器，用于将全切片图像分类为黑色素细胞、基底细胞和鳞状细胞病变三类。Virchow2在大多数分类器中表现优于UNI，逻辑回归达到90%准确率。


<details>
  <summary>Details</summary>
Motivation: 皮肤病理学中全切片图像的快速生成需要自动化方法进行高效处理和准确分类。

Method: 使用UNI和Virchow2提取补丁级嵌入，通过均值聚合策略聚合成切片级特征，然后训练多种机器学习分类器（逻辑回归、梯度提升树、随机森林）。

Result: Virchow2提取的特征在大多数切片级分类器中优于UNI，逻辑回归达到90%准确率，但差异无统计学意义。均值聚合方法提供了可靠的切片级特征表示。

Conclusion: 基础模型在自动化WSI分类中具有潜力，为皮肤病理学诊断提供了可扩展且有效的方法，为切片级表示学习的未来发展铺平了道路。
...
TL;DR: 该研究将后解释方法应用于冰川湾国家公园的海豹检测模型，通过评估解释的定位保真度、忠实性和诊断效用，为生态保护中的计算机视觉应用提供可信证据。


<details>
  <summary>Details</summary>
Motivation: 解决生态学中因缺乏对黑盒神经网络模型的信任而导致的计算机视觉应用滞后问题，为野外部署提供预测证据和局限性文档。

Method: 使用Faster R-CNN检测海豹，并应用梯度类激活映射（HiResCAM、LayerCAM）、LIME和基于扰动的解释方法生成预测解释。

Result: 解释集中在海豹躯干和轮廓而非周围环境，移除海豹会降低检测置信度，同时发现模型会混淆海豹与黑冰和岩石等系统性错误。

Conclusion: 通过将目标检测与后解释性相结合，可以超越"黑盒"预测，开发出可审计、支持决策的保护监测工具。
...
TL;DR: BachVid是首个无需训练、无需参考图像就能实现一致性视频生成的方法，通过分析DiT注意力机制和中间特征，缓存身份视频的中间变量并注入到新视频中，确保多视频间前景和背景的一致性。


<details>
  <summary>Details</summary>
Motivation: 解决多视频生成中角色和背景一致性的挑战，现有方法依赖参考图像或大量训练，且通常只关注角色一致性。

Method: 基于对DiT注意力机制和中间特征的系统分析，发现其能在去噪过程中提取前景掩码和识别匹配点。首先生成身份视频并缓存中间变量，然后将这些变量注入到新生成视频的对应位置。

Result: 实验结果表明BachVid在生成视频中实现了稳健的一致性，无需额外训练。

Conclusion: BachVid为无需参考图像或额外训练的一致性视频生成提供了新颖高效的解决方案。
...
TL;DR: 视觉扩散模型可以直接在像素空间中解决几何问题，包括内接正方形问题、斯坦纳树问题和简单多边形问题，将几何推理重新定义为图像生成任务。


<details>
  <summary>Details</summary>
Motivation: 探索视觉扩散模型作为几何求解器的潜力，证明它们可以直接在像素空间中推理几何问题，而无需专门的架构或领域特定适配。
...
Conclusion: 在图像空间中操作提供了一个通用且实用的框架来近似 notoriously hard 问题，为处理更广泛挑战性几何任务打开了大门。
...
TL;DR: 提出了一个自动化框架来检测训练好的视觉模型对特定视觉属性的依赖，通过自反思代理系统性地生成和测试假设，迭代优化发现过程。
...
Conclusion: 自反思方法能有效识别视觉模型对特定视觉属性的依赖，为模型理解和改进提供了有力工具。
...
TL;DR: 该研究开发了一个人机交互流程，将手绘平面草图转换为语义一致的3D BIM模型，利用多模态大语言模型实现多智能体推理。


<details>
  <summary>Details</summary>
Motivation: 让BIM创建对专家和非专家都更加便捷，仅使用手绘草图即可生成专业级3D建筑信息模型。

Method: 采用多智能体框架，结合感知提取、人工反馈、模式验证和自动化BIM脚本生成，将草图迭代优化为结构化JSON布局，再转换为可执行脚本。

Result: 在10个不同平面图上测试，门、窗等开口元素首次捕获可靠性高，墙体检测初始准确率约83%，经过几次反馈迭代后达到近乎完美对齐。所有类别的精确率、召回率和F1分数均超过0.83，几何误差通过反馈修正逐步降至零。
...
TL;DR: 提出Cultural Alien Sampler (CAS)方法，通过分离概念组合的连贯性和文化典型性，生成既新颖又内部一致的创意，在艺术创作中超越GPT-4o基准，达到与艺术学生相当的表现。


<details>
  <summary>Details</summary>
Motivation: 解决当前大型语言模型在开放领域艺术创作中要么遵循熟悉文化模式，要么在追求新颖性时牺牲连贯性的问题。

Method: 使用两个在WikiArt概念上微调的GPT-2模型：概念连贯性模型评估概念在艺术品中共同出现的合理性，文化背景模型估计这些组合在艺术家作品中的典型程度。CAS选择高连贯性、低典型性的概念组合。

Result: 在人类评估中，CAS在原创性和和谐度方面优于随机选择和GPT-4o基准，表现与艺术学生相当。定量研究显示该方法产生更多样化输出，探索更广泛的概念空间。

Conclusion: 人工文化异化可以释放自主代理的创造潜力，CAS方法在保持内部一致性的同时偏离学习到的文化惯例。
...
TL;DR: 该论文提出了一种新的模糊数表示方法——外延模糊数，以解决传统模糊数运算中的计算复杂性和结果特征不一致问题，并定义了相应的运算和关系运算符。


<details>
  <summary>Details</summary>
Motivation: 传统模糊数在运算中存在三个主要问题：高计算复杂度、某些运算结果不保持原有特征（如两个三角模糊数相乘结果不是三角模糊数）、以及模糊扩散问题（运算次数增加导致模糊度增加），这些限制了模糊数的应用范围。

Method: 提出外延模糊数的概念，定义了外延模糊数的运算方法（包括算术运算）和关系运算符（=, >, >=, <, <=），并通过C++实现验证了方法的可行性。

Result: 开发了外延模糊数的运算框架，解决了传统模糊数运算的问题，提供了可用的C++实现，并通过应用示例展示了方法的有效性。

Conclusion: 外延模糊数为模糊数运算提供了一种更高效和实用的替代方案，克服了传统方法的局限性，扩展了模糊数的应用潜力。
...
### [76] [Epistemic Deference to AI](https://arxiv.org/abs/2510.21043)
*Benjamin Lange*

Main category: cs.AI

TL;DR: 该论文探讨AI何时应优先于人类专家判断，提出AI预占主义及其替代方案——全证据观点，认为AI输出应作为贡献性理由而非完全替代人类独立认知。


<details>
  <summary>Details</summary>
Motivation: 基于社会认识论，探讨AI系统作为人工认识权威(AEAs)时，何时应优先于人类专家判断，以及如何避免经典预占主义在AI应用中的放大风险。

Method: 通过分析AI预占主义观点及其问题，发展出全证据观点，将AI输出视为贡献性理由而非完全替代人类独立认知。

Result: 全证据观点具有三个关键优势：缓解专业知识萎缩、为人机监督提供认识论依据、解释AI不可靠时的合理不信任。

Conclusion: 全证据观点为确定AI服从何时合理提供了原则性方法，特别适用于需要严格可靠性的高风险情境。
...
### [77] [Customizing Open Source LLMs for Quantitative Medication Attribute Extraction across Heterogeneous EHR Systems](https://arxiv.org/abs/2510.21027)
...
TL;DR: 提出一个基于开源大语言模型的框架，从异构电子健康记录中提取统一的阿片类药物使用障碍治疗处方属性，并计算标准化的药物覆盖天数指标。


<details>
  <summary>Details</summary>
Motivation: 解决不同电子健康记录系统中药方数据格式不统一的问题，特别是阿片类药物使用障碍治疗处方数据的标准化提取需求。

Method: 定制开源大语言模型（Llama、Qwen、Gemma、MedGemma）来处理异构数据，提取处方属性，并通过轻量级归一化和跨字段一致性检查生成固定JSON格式输出。

Result: 在5个诊所的25,605条记录上评估，Qwen2.5-32B达到93.4%覆盖率和93.0%准确率，MedGemma-27B达到93.1%/92.2%。

Conclusion: 该方法能够替代脆弱的站点特定ETL流程，支持本地隐私保护部署，实现跨站点MOUD暴露、依从性和保留率的一致性分析。
...
TL;DR: 提出了一种多智能体框架，用于将自然语言问题准确转换为空间SQL查询，通过专业化的智能体协作和程序化验证机制，显著提升了空间查询的准确性和语义对齐度。


<details>
  <summary>Details</summary>
Motivation: 解决SQL复杂性和PostGIS等地理空间工具的专业性带来的使用障碍，使非专家用户能够更便捷地进行空间数据分析。

Method: 采用多智能体框架，包括实体提取、元数据检索、查询逻辑制定、SQL生成和审查智能体，结合知识库、程序化模式分析和语义增强技术。

Result: 在KaggleDBQA基准测试中达到81.2%的准确率，在空间查询测试中达到87.7%的准确率，比无审查智能体时提升11个百分点。

Conclusion: 该工作使空间分析更加普及，为空间Text-to-SQL系统提供了稳健、可推广的基础，推动了自主GIS的发展。
...
TL;DR: MedAlign是一个用于医疗视觉问答的新框架，通过多模态直接偏好优化、检索感知的专家混合架构和联邦治理机制，解决大型视觉语言模型在医疗应用中的幻觉问题、固定深度推理效率低和多机构协作困难等挑战。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在智能医疗部署中面临三个关键挑战：产生与视觉证据不符的幻觉答案、固定深度推理效率低下、多机构协作困难。

Method: 提出多模态直接偏好优化目标，设计检索感知的专家混合架构，采用联邦治理机制进行自适应推理。

Result: 在三个代表性Med-VQA数据集上实现最先进性能，F1分数比强检索增强基线提升高达11.85%，平均推理长度比固定深度CoT方法减少51.60%。

Conclusion: MedAlign框架有效解决了医疗视觉问答中大型视觉语言模型的幻觉、推理效率和协作问题，展现了在临床服务中的实际应用潜力。
...
TL;DR: 提出了一种对观测数据中混淆偏差具有鲁棒性的深度强化学习算法，基于DQN框架，在存在未观测混淆变量的复杂高维环境中寻找最坏情况兼容环境下的安全策略。
...
Method: 基于深度Q网络(DQN)，提出了一种新算法，寻找与观测兼容的最坏情况环境下的安全策略，以应对未观测混淆变量带来的偏差。

Result: 在12个存在混淆的Atari游戏上测试，该算法在所有存在行为策略与目标策略输入不匹配以及未观测混淆变量的游戏中都显著优于标准DQN。

Conclusion: 所提出的鲁棒深度强化学习算法能有效应对观测数据中的混淆偏差，在存在未观测混淆变量的复杂环境中表现优于传统方法。
...
TL;DR: 本文对代理AI在去中心化治理中作为自主决策者进行了首次实证研究，通过构建AI投票代理在真实金融模拟环境中评估其与人类决策的一致性。
...
Method: 使用3000多个主要协议提案，构建AI投票代理，通过模块化可组合程序(MCP)工作流程在基于可验证区块链数据的金融模拟环境中运行，代理能够解释提案背景、检索历史审议数据并独立确定投票立场。

Result: 研究发现AI代理的决策与人类和代币加权结果高度一致，通过精心设计的评估指标显示出强对齐性。
...
TL;DR: 提出了PACE数据集和PACER咨询模型，用于恐慌发作时的及时干预，通过监督学习和偏好对齐优化，在恐慌场景中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 恐慌发作是急性恐惧和痛苦发作，及时干预很重要，但缺乏合适的训练数据集。

Method: 引入PACE数据集，构建基于心理急救原则的高危情境叙事；训练PACER咨询模型，结合监督学习和模拟偏好对齐。

Result: PACER在咨询质量指标和客户情感改善方面优于强基线，在恐慌场景中比通用、CBT和GPT-4模型更受青睐。
...
### [83] [DeepAgent: A General Reasoning Agent with Scalable Toolsets](https://arxiv.org/abs/2510.21618)
...
TL;DR: DeepAgent是一个端到端深度推理代理，通过自主思维、工具发现和行动执行在单一推理过程中完成任务，解决了长程交互中的上下文爆炸和历史积累问题。
...
### [84] [NeuroGenPoisoning: Neuron-Guided Attacks on Retrieval-Augmented Generation of LLM via Genetic Optimization of External Knowledge](https://arxiv.org/abs/2510.21144)
...
Conclusion: 该方法有效解决了知识冲突问题，能够大规模生成有效的RAG投毒知识。
...
### [85] [How to Auto-optimize Prompts for Domain Tasks? Adaptive Prompting and Reasoning through Evolutionary Domain Knowledge Adaptation](https://arxiv.org/abs/2510.21148)
...
<details>
  <summary>Details</summary>
Motivation: 在现实应用中，为LLMs设计领域特定任务的最优提示和推理过程既必要又具挑战性，需要解决领域知识整合、推理效率提升和专家知识集成提示等关键问题。

Method: EGO-Prompt从专家构建的初始语义因果图开始，通过因果引导的文本梯度过程分两步优化：生成确定性推理指导，并适应LLM有效利用指导与原始输入。采用迭代优化算法使用文本梯度和真实标签优化SCG和推理机制。

Result: 在公共卫生、交通和人类行为任务上测试，EGO-Prompt比前沿方法F1分数提高7.32%-12.61%，使小模型以不到20%成本达到大模型性能，并输出可解释的领域特定SCG。

Conclusion: EGO-Prompt框架有效解决了领域特定任务中LLM提示和推理过程的自动化优化问题，显著提升性能同时降低成本并增强可解释性。
...
### [86] [String Seed of Thought: Prompting LLMs for Distribution-Faithful and Diverse Generation](https://arxiv.org/abs/2510.21150)
*Kou Misaki,Takuya Akiba*

Main category: cs.AI

TL;DR: SSoT是一种新颖的提示方法，通过在LLM输出答案前先输出随机字符串来生成足够的熵，从而改善概率指令跟随(PIF)性能，使生成答案的分布与目标分布对齐。


<details>
  <summary>Details</summary>
Motivation: LLMs在需要单一确定性答案的任务上表现出色，但在PIF任务中经常失败，表现出偏见，这对于需要非确定性行为的应用（如人类行为模拟、内容多样化和多人游戏）存在问题，并且会损害生成响应的多样性。

Method: SSoT提示方法指示LLM首先输出一个随机字符串以生成足够的熵，然后通过操作该字符串提取随机性来推导最终答案，从而在遵守特定约束的同时保持多样性。

Result: SSoT显著提高了LLMs的PIF性能，接近伪随机数生成器的理想性能。在NoveltyBench上的实验表明，SSoT的好处超出了封闭集任务，通过增强响应多样性扩展到开放式任务。

Conclusion: SSoT是一种简单有效的提示方法，能够显著改善LLMs在概率指令跟随任务中的表现，并增强生成响应的多样性。
...
### [87] [Memory-Free Continual Learning with Null Space Adaptation for Zero-Shot Vision-Language Models](https://arxiv.org/abs/2510.21175)
*Yujin Jo,Taesup Kim*

Main category: cs.AI

TL;DR: NuSA-CL是一个轻量级、无需内存的持续学习框架，通过低秩适应和约束任务特定权重更新到模型参数的近似零空间，来保护预训练视觉语言模型的零样本能力，同时适应新任务。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型在现实部署中面临分布偏移和新任务挑战，静态零样本能力不足，需要持续学习方法在避免灾难性遗忘的同时适应环境变化。
...
### [88] [Shylock: Causal Discovery in Multivariate Time Series based on Hybrid Constraints](https://arxiv.org/abs/2510.21181)
*Shuo Li,Keqin Xu,Jie Liu,Dan Ye*

Main category: cs.AI

TL;DR: Shylock是一种新的多变量时间序列因果发现方法，能在少样本和正常样本情况下有效工作，通过组扩张卷积和共享核减少参数数量，结合全局和局部约束提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法依赖人工经验、统计方法或图形标准方法，容易出错且依赖大量数据。多变量时间序列存在严重的数据缺口，现有方法容易过拟合。

Method: 使用组扩张卷积和共享核来指数级减少参数数量，结合全局约束和局部约束实现网络间信息共享，设计数据生成方法生成带时间延迟的多变量时间序列。

Result: 在常用基准测试和生成数据集上的广泛实验表明，Shylock在少样本和正常多变量时间序列上都优于两种现有最先进方法。

Conclusion: Shylock能有效解决多变量时间序列因果发现中的数据缺口问题，在少样本和正常情况下都表现出色，并开发了Tcausal库便于使用。
...
### [89] [OutboundEval: A Dual-Dimensional Benchmark for Expert-Level Intelligent Outbound Evaluation of Xbench's Professional-Aligned Series](https://arxiv.org/abs/2510.21244)
...
Method: 设计了覆盖6个主要业务领域和30个子场景的基准测试，开发了大模型驱动的用户模拟器，引入动态评估方法结合自动化和人工评估。

Result: 在12个最先进的大语言模型上的实验揭示了专家级任务完成度与交互流畅性之间的权衡，为构建可靠、类人外呼AI系统提供了实用见解。
...
### [90] [Out-of-Distribution Detection for Safety Assurance of AI and Autonomous Systems](https://arxiv.org/abs/2510.21254)
*Victoria J. Hodge,Colin Paterson,Ibrahim Habli*

Main category: cs.AI

TL;DR: 这篇综述论文分析了自主系统中OOD检测技术在安全保证方面的应用，特别关注安全关键领域。论文探讨了OOD的成因、自主系统安全保证的挑战，并识别了在ML开发生命周期中可用的OOD检测技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI自主系统能力的扩展，严格证明其安全性对于负责任采用至关重要。OOD检测能够处理系统生命周期中的新颖和不确定情况，因此受到研究、开发和安全性工程社区的日益关注。

Method: 采用全面综述方法，定义相关概念，调查OOD的成因，探索自主系统安全保证和OOD检测的挑战因素，识别ML开发生命周期中可用的技术。

Result: 识别了一系列可在ML开发生命周期中使用的OOD检测技术，并建议了在生命周期中支持安全保证论证的应用领域。讨论了将OOD检测集成到系统生命周期时需要注意的注意事项。
...
### [91] [Investigating Scale Independent UCT Exploration Factor Strategies](https://arxiv.org/abs/2510.21275)
...
<details>
  <summary>Details</summary>
Motivation: UCT算法对游戏奖励尺度不鲁棒，在具有密集奖励的游戏中使用固定λ参数会导致性能问题。

Method: 评估了文献中已有的λ策略以及五种新策略，包括基于Q值标准差的自适应方法。

Result: 新提出的λ=2σ策略在各种任务中表现优于现有方法，无论是使用单一参数值还是优化所有可用参数时的峰值性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色但存在严重安全风险，现有缓解策略依赖注入启发式安全信号，往往会抑制推理能力且无法解决安全与推理的权衡问题。

Method: 分析推理轨迹发现Self-Jailbreak现象，提出CoG框架重组不安全推理步骤，引导模型回到安全轨迹。

Result: 在多个推理和安全基准测试中，CoG显著提升当前大型推理模型的安全性，同时保持相当的推理能力，明显优于先前存在严重安全-推理权衡的方法。

Conclusion: CoG框架有效解决了大型推理模型的安全与推理权衡问题，通过系统性干预推理过程实现了安全性的显著提升。
...
TL;DR: 对AIES和FAccT会议论文的范围综述发现，当前可信AI研究过度关注技术属性而忽视社会技术维度，需要结合技术严谨性与社会文化因素的综合方法。
...
Result: 研究在定义透明度、问责制和鲁棒性等技术属性方面取得显著进展，但存在关键差距：当前研究过度强调技术精度而牺牲社会伦理考量，AI系统的社会技术性质较少被探索，可信度成为由有权定义者塑造的争议概念。
...
### [94] [Towards Reliable Code-as-Policies: A Neuro-Symbolic Framework for Embodied Task Planning](https://arxiv.org/abs/2510.21302)
...
Result: 在RLBench和真实世界动态部分可观测场景中，任务成功率比Code-as-Policies基线提高46.2%，任务相关动作可执行性达86.8%
...
TL;DR: CXRAgent是一个基于LLM的胸部X光分析智能体，通过导演编排的多阶段框架，结合工具调用、诊断规划和协作决策，提升了CXR诊断的适应性和可信度。
...
Method: 提出导演编排的三阶段框架：工具调用（含证据驱动验证器）、诊断规划（组建专家团队）、协作决策（整合上下文记忆）。

Result: 在多种CXR解释任务上表现出色，能提供视觉证据，并能很好地泛化到不同复杂度的临床任务。

Conclusion: CXRAgent通过多阶段协作框架显著提升了CXR诊断的性能和可信度，具有良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成创新想法时往往局限于训练数据中的常见概念，现有方法如思维树依赖不可靠的自我评估启发式方法，存在根本性限制。

Method: 使用蒙特卡洛树搜索，结合语义罗盘向量进行长程引导，以及基于景观感知的价值函数进行局部决策，平衡内在连贯性、外在新颖性和叙事进展。

Result: 在广泛的实验中，Magellan在生成科学想法方面显著优于ReAct和思维树等强基线方法，在可信度和创新性方面表现更优。

Conclusion: 对于创造性发现，有原则的引导搜索比无约束的自主性更有效，为LLMs成为创新合作伙伴开辟了新途径。
...
TL;DR: 该论文提出了一种结合强化学习的方法，通过提高token效率来提升1.5B模型在数学推理任务上的性能，相比仅使用监督微调的方法，在GSM8K数据集上获得更高准确率并减少40%以上的token使用量。


<details>
  <summary>Details</summary>
Motivation: 现有的预算强制方法虽然能提高推理性能，但依赖于对长上下文推理轨迹的监督微调，这会导致小模型因冗长响应而性能下降。因此需要一种方法来提高token效率并恢复因长上下文训练造成的性能损失。

Method: 提出了一个结合强化学习的框架，仅使用1.5K训练样本，通过SFT+RL的方法来优化模型在数学推理任务中的表现，重点关注token使用效率的提升。

Result: 在GSM8K数据集上，SFT+RL模型在不同计算预算下表现更好，总体准确率更高，同时token使用量相比SFT模型减少了40%以上。

Conclusion: 强化学习能够有效恢复因长上下文训练造成的性能损失，显著提高数学推理性能，同时大幅降低计算资源消耗。
...
TL;DR: 本文提出了一种新的符号AI与LLMs集成分类法，包括四个维度的分类框架，旨在解决LLMs透明度不足的问题，并为未来研究提供路线图。


<details>
  <summary>Details</summary>
Motivation: LLMs在关键领域表现出色但缺乏透明度，现有神经符号AI方法主要针对传统神经网络，不适用于LLMs特性，缺乏对符号AI如何有效集成到LLMs的系统理解。

Method: 首先回顾已建立的神经符号AI方法，然后提出LLMs中符号集成的新分类法，包括四个维度的分类框架：LLM各阶段的符号集成、耦合机制、架构范式、算法和应用层面视角。

Result: 构建了一个组织现有文献的分类框架，全面识别了当前基准、前沿进展和关键差距，提出了未来研究的路线图。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决数学优化问题求解过程中需要人工建模和编程的繁琐过程，开发一个能够直接从图像识别优化问题并自动求解的系统，降低优化问题求解的技术门槛。

Method: 开发了三模块框架：M1使用深度学习模型进行数学表达式识别生成LaTeX代码；M2使用微调的小型LLM将LaTeX转换为PYOMO脚本；M3使用双层优化分解方法求解优化问题。

Result: MER任务模型在BLEU分数上优于ChatGPT、Gemini和Nougat；BOBD方法在复杂测试问题上比内点法和遗传算法表现更好。

Conclusion: AutoOpt-11k数据集和框架为优化问题的自动化求解提供了有效工具，显著降低了优化问题求解的技术要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经方法通常学习统一求解器同时处理多个约束，但未充分利用VRP变体的组合结构，错失了专门化基础求解器的潜在优势。

Method: 引入状态可分解MDP(SDMDP)将状态空间表示为基础状态空间的笛卡尔积，并开发潜在空间扩展，结合最优基础策略和可学习混合函数实现策略重用。

Result: 在多个VRP变体上的广泛实验表明，MoSES优于先前方法。

Conclusion: 所提框架成功让统一求解器感知VRP变体的共享组件特性，通过重用基础求解器实现了优越性能。
...
TL;DR: EU-Agent-Bench是一个可验证的人工策划基准，用于评估LLM代理在欧盟法律背景下执行非法行为的潜在倾向，涵盖数据保护、偏见/歧视和科学诚信等多个类别。


<details>
  <summary>Details</summary>
Motivation: LLM代理在部署时可能表现出不可预测的行为，包括采取不良和/或不安全的行动，需要衡量其在欧盟立法背景下执行非法行为的潜在倾向。

Method: 通过将模型的函数调用与详尽引用相关立法的评分标准进行比较，评估前沿LLM的法律合规性，并研究在代理系统提示中提供相关立法摘录和明确合规指令的合规效果。

Result: 研究发布了公开预览集供研究社区使用，同时保留私有测试集以防止在评估即将推出的模型时出现数据污染。

Conclusion: 鼓励未来的工作将代理安全基准扩展到不同的法律管辖区以及多轮和多语言交互。
...
TL;DR: Co-Sight通过冲突感知元验证(CAMV)和可信推理结构化事实(TRSF)机制，将推理转化为可证伪和可审计的过程，解决了LLM智能体在长程推理中验证不足的问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长程推理中的失败往往不是生成能力不足，而是对中间推理的验证不足。需要一种可扩展的可靠长程推理范式。

Method: 使用CAMV将验证重新定义为冲突识别和针对性证伪，只在专家智能体间的不一致热点分配计算；TRSF通过结构化事实模块持续组织、验证和同步证据。
...
### [103] [Learning Neural Control Barrier Functions from Expert Demonstrations using Inverse Constraint Learning](https://arxiv.org/abs/2510.21560)
*Yuxuan Yang,Hussein Sibai*

Main category: cs.AI

TL;DR: 该论文提出了一种基于演示学习的方法来训练神经控制屏障函数，用于在故障状态难以明确指定的情况下保证自主系统的安全性。


<details>
  <summary>Details</summary>
Motivation: 在关键领域运行的自主系统中，安全性是基本要求。虽然控制屏障函数可用于设计安全过滤器，但故障状态集往往难以明确指定，而专家演示数据更容易获得。

Method: 使用演示学习训练约束函数来分类系统状态为安全或危险，然后用该函数标注新的模拟轨迹来训练神经控制屏障函数。

Result: 在四个不同环境中进行实证评估，表明该方法优于现有基线方法，并且与使用真实安全标签训练的神经控制屏障函数性能相当。

Conclusion: 该方法能够从专家演示中学习安全约束，在故障状态难以明确指定的情况下有效保证系统安全性。
...
### [104] [Huxley-Gödel Machine: Human-Level Coding Agent Development by an Approximation of the Optimal Self-Improving Machine](https://arxiv.org/abs/2510.21614)
*Wenyi Wang,Piotr Piękos,Li Nanbo,Firas Laakom,Yimeng Chen,Mateusz Ostaszewski,Mingchen Zhuge,Jürgen Schmidhuber*

Main category: cs.AI

TL;DR: 本文提出Huxley-Gödel Machine (HGM)方法，通过估计自我改进潜力指标CMP来指导代码代理的自我修改树搜索，在SWE-bench等基准上优于现有方法，且实现了人类水平的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设编码基准性能越高意味着后续自我修改潜力越大，但作者发现自我改进潜力与编码性能之间存在不匹配问题，需要更好的指标来指导自我改进过程。

Method: 提出CMP指标来聚合代理后代的基准性能作为自我改进潜力指标，并基于此构建Huxley-Gödel Machine (HGM)，通过估计CMP来指导自我修改树的搜索。

Result: 在SWE-bench Verified和Polyglot基准上，HGM优于现有方法且使用更少的计算时间，在SWE-bench Lite上达到人类水平的性能，匹配最佳人工工程编码代理的结果。

Conclusion: HGM通过更准确地估计自我改进潜力，有效解决了元生产力与性能不匹配问题，在代码代理自我改进方面取得了显著进展，并展现出良好的迁移能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI代理基准存在不足：缺乏真实科学研究的整体衡量、缺少可复现的工具、未考虑模型成本和工具访问等混杂变量、缺乏标准化接口和全面基线代理。
...
Conclusion: 需要更严谨的基准来评估AI科学代理，AstaBench提供了这样的框架，揭示了当前AI在科学研究辅助方面的局限性。
...
TL;DR: CMOMgen是首个端到端的复杂多本体匹配策略，能够生成完整且语义合理的映射，无需对目标本体或实体数量设限。该策略在三个生物医学任务中表现优异，超越了所有基线方法。


<details>
  <summary>Details</summary>
Motivation: 构建全面的知识图谱需要使用多个本体来充分将数据置于领域上下文中。简单的成对等价映射无法提供相关但不相交本体的完整语义集成，需要更复杂的多本体匹配方法。

Method: CMOMgen采用检索增强生成策略，选择相关类来组成映射，并过滤匹配的参考映射作为示例以增强上下文学习。这是首个端到端的复杂多本体匹配策略。
...
TL;DR: 提出了一个专家标注的视频广告数据集，用于评估视觉语言模型在分析企业公关活动中的框架识别能力，特别是在能源领域的绿色创新框架检测。
...
Method: 构建了一个从Facebook和YouTube获取的专家标注视频广告数据集，包含13种框架类型，覆盖50多家公司和20个国家，专门用于评估视觉语言模型。

Result: 基线实验显示GPT-4.1能检测环境信息，F1分数达79%，但最佳模型在识别绿色创新框架方面仅获得46%的F1分数。

Conclusion: 该数据集有助于能源领域战略沟通的多模态分析研究，同时揭示了视觉语言模型需要解决的挑战，如隐式框架、不同长度视频处理和隐式文化背景。
...
TL;DR: 提出了一个基于知识图谱的框架，作为智能翻译层来解决自主代理在动态环境中协调时高层任务目标与底层规划输入之间的语义鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 解决自主代理在动态环境中协调时高层任务目标与底层规划输入之间的语义鸿沟问题，使复杂协调路径能够通过简单修改知识图谱中的事实来调整。

Method: 采用知识图谱的双平面架构，将声明性事实编译为每个代理的任务感知"世界观"和物理感知遍历规则，将任务语义与领域无关的规划器解耦。

Result: 通过墨西哥湾自主水下航行器的案例研究，可视化展示了端到端过程，并定量证明不同声明性策略会产生独特且高性能的结果。
...
### [109] [Gaussian Mixture Flow Matching with Domain Alignment for Multi-Domain Sequential Recommendation](https://arxiv.org/abs/2510.21021)
*Xiaoxin Ye,Chengkai Huang,Hongtao Huang,Lina Yao*

Main category: cs.IR

TL;DR: GMFlowRec是一个用于多域序列推荐的高效生成框架，通过高斯混合流匹配建模域感知转换轨迹，解决了现有方法在处理复杂域转换、域异构性和不平衡性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 用户在多域环境中的交互行为具有频繁复杂的域转换，现有跨域序列推荐方法往往忽视域转换的复杂性，容易在密集域上过拟合而在稀疏域上欠拟合，且难以扩展到更多域。

Method: 提出GMFlowRec框架，包含：(1)统一双掩码Transformer分离域不变和域特定意图；(2)高斯混合流场捕捉多样化行为模式；(3)域对齐先验支持频繁和稀疏转换。

Result: 在JD和Amazon数据集上的实验表明，GMFlowRec在NDCG@5指标上达到最先进性能，提升高达44%，同时通过单一统一骨干保持高效率。

Conclusion: GMFlowRec是一个可扩展的高效多域序列推荐框架，能够有效处理现实世界中的多域推荐挑战。
...
### [110] [Communication Platform for Non-verbal Autistic children in Oman using Android mobile](https://arxiv.org/abs/2510.21028)
*Amna Al-Araimi,Yue Zheng,Haiming Liu*

Main category: cs.IR

TL;DR: 该论文提出一个结合网页面板和Android移动应用的平台，用于帮助阿曼的非语言自闭症儿童改善沟通能力，采用增强现实框架促进创造性游戏和自我反思。


<details>
  <summary>Details</summary>
Motivation: 针对非语言自闭症谱系障碍在全球范围内的普遍性，以及现有干预方法的碎片化问题，旨在开发一个综合平台来提升自闭症儿童的生活质量。

Method: 开发包含网页面板和Android移动应用的平台，整合多种干预方法，采用增强现实框架进行基于屏幕的互动活动。

Result: 论文提出了一个技术解决方案框架，但未提供具体的实施结果数据。

Conclusion: 综合性的技术平台比碎片化方法更适合自闭症儿童，增强现实技术能有效促进他们的创造性游戏和沟通能力发展。
...
### [111] [VOGUE: A Multimodal Dataset for Conversational Recommendation in Fashion](https://arxiv.org/abs/2510.21151)
*David Guo,Minqi Sun,Yilun Jiang,Jiazhou Liang,Scott Sanner*

Main category: cs.IR

TL;DR: VOGUE是一个新颖的多模态对话推荐数据集，包含60个真实时尚购物场景下的人类对话，配有视觉目录、用户档案和评分数据，用于评估对话推理和推荐系统性能。
...
Result: 分析显示视觉对话具有独特动态，推荐者常按特征分组建议物品；MLLMs在整体偏好对齐上接近人类水平，但在评分分布和偏好推断泛化方面存在系统性错误。

Conclusion: VOGUE既是研究多模态对话系统的独特资源，也是对当前顶级多模态基础模型推荐能力的挑战数据集。
...
### [112] [Bi-Level Optimization for Generative Recommendation: Bridging Tokenization and Generation](https://arxiv.org/abs/2510.21242)
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐方法将分词器和推荐器分开训练（顺序或交替），忽略了它们的相互依赖关系，导致分词器训练缺乏推荐目标指导，产生次优标识符从而降低推荐性能。
...
### [113] [Pctx: Tokenizing Personalized Context for Generative Recommendation](https://arxiv.org/abs/2510.21276)
*Qiyong Zhong,Jiajie Su,Yunshan Ma,Julian McAuley,Yupeng Hou*

Main category: cs.IR

TL;DR: 提出个性化上下文感知分词器，将用户历史交互纳入语义ID生成过程，使同一物品在不同用户上下文中被分词为不同的语义ID，从而在生成式推荐模型中实现更个性化的预测。


<details>
  <summary>Details</summary>
Motivation: 现有分词方法是非个性化的，仅从物品特征派生语义ID，假设存在通用的物品相似性标准，忽略了用户特定的视角和意图。

Method: 设计个性化上下文感知分词器，在生成语义ID时融入用户历史交互信息，使同一物品在不同用户上下文中产生不同的分词结果。
...
### [114] [CausalRec: A CausalBoost Attention Model for Sequential Recommendation](https://arxiv.org/abs/2510.21333)
*Yunbo Hou,Tianle Yang,Ruijie Li,Li He,Liang Wang,Weiping Li,Bo Zheng,Guojie Song*

Main category: cs.IR

TL;DR: 提出了CausalRec框架，通过因果注意力机制改进序列推荐系统，解决了传统方法只关注项目共现而忽略用户行为动机的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于相关性的序列推荐系统虽然取得了成功，但仅关注项目共现会忽略用户行为背后的动机，导致虚假相关性和不准确的推荐。

Method: CausalRec包含因果发现块和CausalBooster。因果发现块学习用户行为序列中的因果图，CausalBooster利用发现的因果图来优化注意力机制，优先考虑具有因果重要性的行为。

Result: 在真实世界数据集上的实验表明，CausalRec在命中率(HR)和归一化折损累计增益(NDCG)上分别平均提升了7.21%和8.65%，优于多个最先进方法。

Conclusion: 这是第一个通过注意力机制在序列推荐中引入因果关系的模型，证明了因果关系在生成更准确可靠推荐中的价值。
...
### [115] [SciNUP: Natural Language User Interest Profiles for Scientific Literature Recommendation](https://arxiv.org/abs/2510.21352)
*Mariam Arustashvili,Krisztian Balog*

Main category: cs.IR

TL;DR: SciNUP是一个用于学术推荐的新型合成数据集，利用作者发表历史生成自然语言用户档案和对应真实项目，填补了该领域大规模公开测试集的空白。


<details>
  <summary>Details</summary>
Motivation: 解决自然语言用户档案推荐系统中缺乏大规模公开测试集的问题，为评估基于自然语言档案的推荐方法提供资源。

Method: 通过作者发表历史生成自然语言用户档案和真实项目，构建SciNUP数据集，并比较从稀疏检索到最先进LLM重排器的基线方法。

Result: 基线方法性能相当但检索不同项目，显示互补行为；同时存在显著改进空间，表明需要更有效的基于自然语言的推荐方法。
...
### [116] [Doc-Researcher: A Unified System for Multimodal Document Parsing and Deep Research](https://arxiv.org/abs/2510.21603)
...
TL;DR: Doc-Researcher是一个统一的多模态文档研究系统，通过深度多模态解析、系统化检索架构和迭代多智能体工作流，解决了现有系统仅限于文本数据的局限性，在M4DocBench基准上实现了50.6%的准确率，比现有最佳方法高出3.4倍。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究系统仅限于文本网页数据，忽略了多模态文档中嵌入的丰富知识（如图表、表格、方程等），需要能够保持视觉语义和结构连贯性的解析、分块和跨模态检索能力。

Method: 系统包含三个集成组件：(1)深度多模态解析，保持布局结构和视觉语义，创建从块级到文档级的多粒度表示；(2)系统化检索架构，支持纯文本、纯视觉和混合范式，具有动态粒度选择；(3)迭代多智能体工作流，分解复杂查询，逐步积累证据，跨文档和模态合成全面答案。

Result: 在M4DocBench基准测试中，Doc-Researcher达到50.6%的准确率，比现有最佳基线方法高出3.4倍。M4DocBench是首个多模态、多跳、多文档、多轮深度研究基准，包含158个专家标注问题和304个文档的完整证据链。

Conclusion: 有效的文档研究不仅需要更好的检索，更需要能够保持多模态完整性和支持迭代研究的深度解析。这项工作为多模态文档集合的深度研究建立了新范式。
...
### [117] [A Data-Centric Approach to Multilingual E-Commerce Product Search: Case Study on Query-Category and Query-Item Relevance](https://arxiv.org/abs/2510.21671)
*Yabo Yin,Yang Xi,Jialong Wang,Shanqi Wang,Jiateng Hu*

Main category: cs.IR

TL;DR: 提出一个数据中心的框架，通过翻译增强、语义负采样和自验证过滤三种策略改进多语言电商搜索的查询-类别和查询-商品相关性任务，在LLM基线上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多语言电商搜索面临数据不平衡、标签噪声和低资源语言监督不足的问题，阻碍了相关性模型的跨语言泛化能力。

Method: 采用三种数据工程策略：翻译增强为训练中缺失的语言合成样本，语义负采样生成困难负例缓解类别不平衡，自验证过滤检测并移除可能错误标注的实例。

Result: 在CIKM AnalytiCup 2025数据集上，相比强LLM基线获得显著的F1分数提升，在官方竞赛中取得有竞争力的结果。

Conclusion: 系统化的数据工程与复杂模型修改同样有效且更易部署，为构建稳健的多语言电商搜索系统提供了实用指导。
...
### [118] [Digital Permission Structures: How Celebrity Disclosure Enables Black Masculine Vulnerability in Online Mental Health Discourse](https://arxiv.org/abs/2510.20881)
*Anurag Shekhar*

Main category: cs.CY

TL;DR: 本研究通过分析Lil Wayne公开谈论心理健康后YouTube上的11,306条评论，发现名人披露对黑人男性心理健康求助具有积极影响，支持了数字许可结构模型，挑战了基于缺陷的黑人男性气质模型。


<details>
  <summary>Details</summary>
Motivation: 黑人男性面临传统男性气质规范和系统性种族主义的双重障碍，限制了心理健康求助行为。虽然名人心理健康披露有望减少污名化，但缺乏关于其对黑人男性社区影响的研究。

Method: 采用收敛混合方法分析11,306条YouTube评论，包括VADER情感分类、LDA主题建模、NRC情感词典分析的定量方法，以及对2,100条高参与度评论的反思性主题分析定性方法。

Result: 研究发现评论以积极情感为主，同伴支持达到最高饱和度，社区回应显示了基于Ubuntu哲学的集体男性气质和男性规范的重构，将求助视为力量。

Conclusion: 研究支持数字许可结构模型，挑战了基于缺陷的黑人男性气质模型，建议干预措施应利用集体主义、与文化人物合作、采用基于力量的信息传递，并以嘻哈真实性为中心。
...
### [119] [What do model reports say about their ChemBio benchmark evaluations? Comparing recent releases to the STREAM framework](https://arxiv.org/abs/2510.20927)
*Tom Reed,Tegan McCaslin,Luca Righetti*

Main category: cs.CY

TL;DR: 该论文分析了2025年春季发布的三个前沿AI模型报告（OpenAI o3、Anthropic Claude 4、Google DeepMind Gemini 2.5 Pro）中关于化学和生物（ChemBio）误用风险评估的方法论透明度，使用STREAM标准进行比较，发现各报告在透明度方面存在差异且都有改进空间。


<details>
  <summary>Details</summary>
Motivation: 了解AI开发者目前在模型报告中包含或省略了哪些评估方法细节，以促进AI评估科学的发展，建立公众对AI系统的信任，并支持第三方审查。

Method: 使用STREAM（v1）标准对三个前沿AI模型报告（OpenAI o3、Anthropic Claude 4、Google DeepMind Gemini 2.5 Pro）中化学和生物误用风险评估的方法论进行系统性比较分析。

Result: 每个模型报告都包含了一些其他报告没有的有用细节，所有报告都有需要改进的领域。在所有报告中，测试材料示例和详细激发条件列表等项目的报告质量较差。

Conclusion: 建议AI开发者继续加强评估科学，在目前报告仍然有限的领域朝着更高的透明度努力，相互借鉴最佳报告实践。
...
### [120] [Soppia: A Structured Prompting Framework for the Proportional Assessment of Non-Pecuniary Damages in Personal Injury Cases](https://arxiv.org/abs/2510.21082)
*Jorge Alberto Araujo*

Main category: cs.CY

TL;DR: Soppia是一个结构化提示框架，利用AI帮助法律专业人士处理多标准加权法律规则的复杂性，特别是在人身伤害案件的非金钱损害赔偿量化中。


<details>
  <summary>Details</summary>
Motivation: 解决司法决策中复杂法律规则应用不一致的问题，特别是在多标准、异质权重的情况下，确保立法意图的连贯实现。

Method: 开发Soppia（有序比例加权智能评估系统）框架，将细微的法律指令转化为实用、可复制、透明的方法论，以巴西CLT第223-G条中的12个非金钱损害赔偿标准为案例研究。

Result: 该框架能够全面平衡地分析所有规定标准，增强一致性和可预测性，提供可解释的工具，适应多标准法律环境。

Conclusion: Soppia在规范解释和计算推理之间架起桥梁，实现了可审计的法律AI，为多标准法律决策提供了实用解决方案。
...
### [121] [The Nuclear Analogy in AI Governance Research](https://arxiv.org/abs/2510.21203)
*Sophia Hatz*

Main category: cs.CY

TL;DR: 本文回顾了43篇将人工智能与核武器进行类比的学术文献，分析了核领域经验对AI治理的借鉴意义，探讨了四个主要问题领域及其政策价值。


<details>
  <summary>Details</summary>
Motivation: 人工智能与核武器的类比在AI治理学术和政策讨论中很常见，但存在领域差异的批评。本文旨在澄清历史类比如何在不同技术领域中为政策制定提供参考。

Method: 系统回顾了43篇明确借鉴核领域经验来推导AI治理教训的学术文献，识别了四个主要问题领域。

Result: 识别了四个核领域经验可借鉴的AI治理问题领域：早期技术发展与治理、国际安全风险与战略、国际制度与协议、国内安全监管。历史类比提供了概念框架、警示教训和政策想象力。

Conclusion: 尽管技术领域存在差异，但核类比能为AI治理提供有价值的参考。鉴于政策制定者已在使用这种类比，继续批判性地借鉴历史先例对于塑造有效的全球AI治理至关重要。
...
### [122] [World Models Should Prioritize the Unification of Physical and Social Dynamics](https://arxiv.org/abs/2510.21219)
*Xiaoyuan Zhang,Chengdong Ma,Yizhe Huang,Weidong Huang,Siyuan Qi,Song-Chun Zhu,Xue Feng,Yaodong Yang*

Main category: cs.CY

TL;DR: 该立场论文主张将物理预测能力与社会预测能力双向统一是世界模型发展的关键前沿，提出了ACE原则和概念框架来实现真正的整体世界模型


<details>
  <summary>Details</summary>
Motivation: 当前世界模型在预测物理动力学和社会行为方面分别发展，未能建模物理环境与社会结构之间的关键相互作用，无法应对真实世界系统的复杂性

Method: 分析整合的核心障碍，提出ACE指导原则（适应性、情境性、涌现性），并构建概念框架和研究路线图

Result: 提出了实现物理与社会能力统一的理论框架和方法论指导

Conclusion: 物理与社会预测能力的系统性双向统一对于AI稳健应对复杂现实挑战和实现更通用智能至关重要
...
### [123] [Recommended Practices for NPOV Research on Wikipedia](https://arxiv.org/abs/2510.21526)
*Isaac Johnson,Yu-Ming Liou,Jacob Rogers,Aaron Shaw,Leila Zia*

Main category: cs.CY

TL;DR: 本文旨在加速维基百科中立性研究，通过帮助研究者理解维基百科中立观点的含义，识别研究挑战并提供指导，以提升研究成果对维基百科的实际影响。


<details>
  <summary>Details</summary>
Motivation: 维基百科的中立观点原则是其核心支柱之一，但相关研究相对较少。研究者对维基百科中立性的定义和重要性理解不足，且中立性本身是一个具有挑战性和争议性的概念。

Method: 通过帮助研究者理解维基百科中立观点的定义，识别研究中的常见挑战并提供应对方法，以及指导如何有效沟通研究成果以增强对维基百科的实际影响。

Result: 本文为研究者提供了系统性的指导框架，帮助他们更好地开展维基百科中立性相关研究，并提升研究成果的实际应用价值。

Conclusion: 通过提供清晰的研究指导和实践建议，本文旨在促进高质量的中立性研究，支持维基百科社区持续改进百科全书的内容质量。
...
TL;DR: CESAR通过在线强化学习框架解决音频大语言模型中的推理逆缩放问题，从结果验证转向奖励推理过程，在MMAU Test-mini上达到SOTA性能，接近人类水平的MMSU推理任务表现。


<details>
  <summary>Details</summary>
Motivation: 音频大语言模型中的推理作用未被充分探索，推理过程在推理时常常导致性能下降而非提升，这种现象称为测试时逆缩放，即更长的推理链产生更差的结果。

Method: 引入CESAR框架，采用在线强化学习和Group Relative Policy Optimization，使用多维度奖励机制激励正确性、格式、一致性、结构化分析模式、因果推理、领域知识整合和校准推理深度。

Result: 解决了测试时逆缩放问题，在MMAU Test-mini上大幅超越Gemini 2.5 Pro和GPT-4o Audio，在MMSU推理任务上接近人类水平，通过AI作为评判者的评估和定性比较验证了推理质量的提升。

Conclusion: CESAR为开发音频LLM中稳健且可扩展的推理建立了原则性方法，增强的推理产生了协同效应，同时改善了多模态推理和感知能力。
...
TL;DR: CRISP是一个基于图学习的时空框架，通过自适应注意力机制发现危机相关的资产关系，在通胀驱动市场等不同危机机制中实现稳健的资产配置预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的方法依赖预定义图拓扑，无法适应市场动态在不同危机机制（信用传染、疫情冲击、通胀抛售）中的变化，导致资产相关性结构预测不准确。

Method: 使用图卷积网络编码空间关系，BiLSTM与自注意力编码时间动态，通过多头图注意力网络学习稀疏结构，过滤92.5%的噪声连接，保留危机相关依赖。

Result: 在2005-2021数据训练后，能泛化到2022-2024通胀驱动市场，夏普比率达3.76，比等权重基准提高707%，比静态图方法提高94%。防御性集群注意力在危机期间增强49%。

Conclusion: CRISP通过自适应学习而非强加假设，实现了跨危机机制的稳健预测，注意力权重提供了可解释的机制检测能力。
...
Method: 通过正交搜索方向定义多个约束优化子问题，结合帕累托前沿估计技术生成额外解，支持批量优化

Result: 在2到6个目标的合成和真实基准函数上，MOBO-OSD始终优于最先进算法

Conclusion: MOBO-OSD算法能有效生成多样化的帕累托最优解集，在多目标优化中具有优越性能
...
TL;DR: CC-GRMAS框架利用卫星观测和环境信号，通过三个互联智能体（预测、规划、执行）实现滑坡的实时检测和灾害响应，提高滑坡预测准确性。


<details>
  <summary>Details</summary>
Motivation: 滑坡是日益严重的气候诱发灾害，特别是在亚洲高山区。尽管卫星和时序数据集日益普及，但及时检测和灾害响应仍然不发达且分散。

Method: 引入CC-GRMAS框架，利用系列卫星观测和环境信号，通过预测、规划、执行三个互联智能体的协作，实现实时态势感知、响应规划和干预。

Result: 通过整合当地环境因素和操作化多智能体协调，该方法为脆弱山区提供了可扩展的、主动的气候韧性灾害准备解决方案。

Conclusion: CC-GRMAS框架通过多智能体协作和环境因素整合，为山区滑坡灾害提供了有效的预测和响应解决方案，增强了气候韧性灾害准备能力。
...
### [128] [Few-Shot Knowledge Distillation of LLMs With Counterfactual Explanations](https://arxiv.org/abs/2510.21631)
*Faisal Hamman,Pasan Dissanayake,Yanjun Fu,Sanghamitra Dutta*

Main category: cs.LG

TL;DR: 提出了一种名为CoD的反事实解释增强蒸馏方法，用于少样本任务感知知识蒸馏，通过注入反事实解释来精确映射教师模型的决策边界，显著减少所需样本量。
...
Method: CoD策略通过系统地注入反事实解释（CFEs）来进行蒸馏，反事实解释是指能够以最小扰动翻转教师模型输出预测的输入。

Result: 在各种数据集和LLM上的实验表明，CoD在少样本场景（低至8-512个样本）中优于标准蒸馏方法，仅使用基线方法一半的原始样本及其对应的CFEs就能提升性能。

Conclusion: CoD提供了一种有效的少样本知识蒸馏方法，通过利用反事实解释来精确捕捉教师模型的决策边界，在数据稀缺场景下表现优异。
...
### [129] [Multimodal Negative Learning](https://arxiv.org/abs/2510.20877)
*Baoquan Gong,Xiyuan Gao,Pengfei Zhu,Qinghua Hu,Bing Cao*

Main category: cs.LG

TL;DR: 提出一种新的多模态学习范式"负学习"，通过主导模态动态引导弱模态抑制非目标类别，而不是强制对齐，以解决模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统多模态学习方法强制弱模态与主导模态对齐，这会抑制弱模态的独特信息。需要一种新范式来保护模态特异性信息。

Method: 提出多模态负学习框架，主导模态动态引导弱模态抑制非目标类别，通过增加单模态置信度边界来提升鲁棒性。

Result: 在多个基准测试中验证了方法的有效性，特别是在噪声和不平衡场景下能显著降低弱模态的实证误差。

Conclusion: 负学习范式能够稳定决策空间，保护模态特异性信息，在多模态学习中提供更好的鲁棒性和泛化能力。
...
### [130] [HA-RAG: Hotness-Aware RAG Acceleration via Mixed Precision and Data Placement](https://arxiv.org/abs/2510.20878)
*Danying Ge,Jianhua Gao,Yixue Yang,Weixing Ji*

Main category: cs.LG

TL;DR: 提出了一种基于访问热度的RAG推理优化系统HA-RAG，通过混合精度压缩加载和热感知数据放置策略，显著提升推理速度，在TTFT指标上平均加速2.10倍，最高加速10.49倍。


<details>
  <summary>Details</summary>
Motivation: RAG虽然能提高LLM输出准确性，但引入外部知识库带来了长上下文处理挑战，显著增加了内存消耗和推理延迟。现有研究通过预计算KV并按需加载来加速推理，但仍有优化空间。

Method: 1. 基于KV块访问频率的热感知混合精度压缩加载方法，减少磁盘I/O和内存访问开销；2. 热感知数据放置策略，优先将高频访问的KV块存储在高速内存中。

Result: 相比TurboRAG，HA-RAG在Time-To-First-Token指标上平均加速2.10倍，最高加速10.49倍，且精度损失可忽略不计。
...
TL;DR: 本文研究发现，通过注入并截断重尾噪声，SGD可以几乎完全避免尖锐局部最小值，从而获得更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 理解SGD避免尖锐局部最小值的能力，并进一步强化这种能力，需要超越传统的局部收敛分析，全面理解SGD的全局动力学。

Method: 基于大偏差和亚稳态分析的技术工具，对重尾SGD的全局动力学进行精确表征，通过注入和截断重尾噪声来训练模型。

Result: 重尾SGD能够几乎完全避免尖锐最小值，找到几何更平坦的局部最小值，并在测试数据上实现更好的泛化性能。

Conclusion: 重尾SGD结合梯度裁剪可以找到更平坦的局部最小值，从而获得更好的泛化能力，实验证实了理论预测。
...
### [132] [Learning from Interval Targets](https://arxiv.org/abs/2510.20925)
*Rattana Pukdee,Ziqi Ke,Chirag Gupta*

Main category: cs.LG

TL;DR: 该论文研究了区间目标回归问题，提出了两种方法：使用与区间目标兼容的损失函数，以及新颖的min-max学习公式，在真实数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当精确的目标标签由于成本高昂或固有不确定性而无法获得时，只能获得目标值的上下界区间。在这种情况下，传统的回归损失函数无法使用。

Method: 1. 使用与区间目标兼容的损失函数，基于假设类的平滑性建立非渐近泛化界；2. 提出新颖的min-max学习公式：在提供的区间内最小化对抗最坏情况（最大化）的目标标签，并通过平滑性约束解决非凸最大化问题。

Result: 在真实世界数据集上进行了广泛实验，表明所提出的方法达到了最先进的性能。

Conclusion: 该研究为区间目标回归问题提供了有效的解决方案，通过兼容区间目标的损失函数和min-max学习公式，在放松先前假设条件的同时实现了优异的性能。
...
### [133] [Meta-Learning for Cross-Task Generalization in Protein Mutation Property Prediction](https://arxiv.org/abs/2510.20943)
*Srivathsan Badrinarayanan,Yue Su,Janghoon Ock,Alan Pham,Sanya Ahuja,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 该论文提出了首个将模型无关元学习（MAML）应用于蛋白质突变性质预测的方法，并引入新的突变编码策略，显著提升了跨数据集泛化能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖针对单个数据集的蛋白质特定变换器微调，但由于实验条件异质性和目标域数据有限，在跨数据集泛化方面表现不佳。

Method: 结合变换器架构与MAML元学习，通过最小梯度步数实现快速任务适应；使用分隔符标记的突变编码策略，将突变直接整合到序列上下文中。

Result: 在三个不同的蛋白质突变数据集（功能适应性、热稳定性和溶解度）上评估，相比传统微调方法显著提升：功能适应性准确率提高29%，训练时间减少65%；溶解度准确率提高94%，训练时间减少55%。

Conclusion: 该工作为蛋白质突变分析建立了系统的元学习方法，并引入了有效的突变编码策略，为蛋白质工程中的跨域泛化提供了变革性方法学。
...
### [134] [LLM-Integrated Bayesian State Space Models for Multimodal Time-Series Forecasting](https://arxiv.org/abs/2510.20952)
...
<details>
  <summary>Details</summary>
Motivation: 解决现实世界预测中需要整合结构化时序数据和非结构化文本信息的挑战，现有方法在输入输出时间窗口固定且无法建模不确定性方面存在架构限制

Method: 包含两个组件：(1)状态空间模型(SSM)主干捕捉潜在状态的时间动态；(2)预训练大语言模型(LLM)编码文本输入进行后验状态估计，并解码与潜在轨迹一致的文本预测
...
### [135] [Safety Assessment in Reinforcement Learning via Model Predictive Control](https://arxiv.org/abs/2510.20955)
...
Conclusion: 基于可逆性的方法能够有效提供安全保证，仅需查询黑盒动态而无需显式动态模型或安全约束知识
...
### [136] [An Ensembled Penalized Federated Learning Framework for Falling People Detection](https://arxiv.org/abs/2510.20960)
*Sizhe Rao,Runqiu Zhang,Sajal Saha,Liang Chen*

Main category: cs.LG

TL;DR: 提出EPFL框架，集成持续学习、个性化建模和加权聚合策略，用于可扩展、安全、准确的跌倒检测。
...
Method: 集成惩罚联邦学习框架，结合同态加密和联邦训练保护隐私，采用惩罚本地训练和集成推理提高一致性。

Result: 在基准跌倒检测数据集上实现88.31%召回率和89.94% F1分数，显著优于集中式和基线模型。
...
### [137] [Towards Scalable Oversight with Collaborative Multi-Agent Debate in Error Detection](https://arxiv.org/abs/2510.20963)
*Yongqiang Chen,Gang Niu,James Cheng,Bo Han,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 提出ColMAD协作式多智能体辩论协议，将辩论重构为非零和游戏，显著提升大语言模型错误检测能力


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论协议将辩论视为零和游戏，导致辩论者为了获胜而误导法官，引入更多错误，表现不如单智能体方法

Method: 设计协作式多智能体辩论协议ColMAD，鼓励多个智能体以支持性方式相互批评，补充彼此缺失的观点

Result: ColMAD显著优于先前竞争性多智能体辩论方法19%，在错误检测任务上相比单智能体方法带来显著提升

Conclusion: 协作式辩论协议能有效缓解辩论黑客问题，通过多智能体互补视角提供更全面的证据，从而提升错误检测性能
...
### [138] [Neural Mutual Information Estimation with Vector Copulas](https://arxiv.org/abs/2510.20968)
*Yanzhi Chen,Zijing Ou,Adrian Weller,Michael U. Gutmann*

Main category: cs.LG

TL;DR: 提出了一种基于向量copula理论的互信息估计方法，在复杂神经网络模型和简化高斯模型之间找到平衡点


<details>
  <summary>Details</summary>
Motivation: 现有互信息估计方法要么依赖需要大量数据的复杂神经网络模型，要么使用无法捕捉复杂分布的过度简化模型，需要找到更好的平衡点

Method: 基于向量copula理论，在复杂模型和简化模型之间进行原则性插值，实现复杂性和容量的更好权衡

Result: 在最新的合成基准测试和包含多种模态的真实世界数据上的实验证明了所提出估计器的优势

Conclusion: 该方法在互信息估计中实现了复杂性和容量之间的更好权衡，优于现有方法
...
### [139] [On the accuracy of implicit neural representations for cardiovascular anatomies and hemodynamic fields](https://arxiv.org/abs/2510.20970)
*Jubilee Lee,Daniele E. Schiavazzi*

Main category: cs.LG

TL;DR: 该论文评估了隐式神经表示(INRs)在血流动力学场压缩和心血管解剖表示中的性能，发现INRs能实现高达230倍的压缩比，同时保持较低的误差。


<details>
  <summary>Details</summary>
Motivation: 评估INRs在特定领域应用中的准确性，特别是在血流动力学场压缩和心血管解剖表示方面的表现，因为目前对这些应用的准确性理解不足。

Method: 使用多种策略减轻频谱偏差，包括专用激活函数、固定和可训练位置编码、非线性核的线性组合，并在胸主动脉的血流动力学场和48个胸主动脉解剖结构上进行测试。

Result: 在胸主动脉血流动力学场中实现约230倍的压缩比，压力最大绝对误差为1 mmHg，速度误差为5-10 cm/s；在解剖表示中，平均和最大绝对解剖差异分别低于0.5 mm和1.6 mm。
...
### [140] [L^2M^3OF: A Large Language Multimodal Model for Metal-Organic Frameworks](https://arxiv.org/abs/2510.20976)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在科学发现方面的突破有限，因为理解复杂物理现象需要超越语言的多模态表示。MOF材料设计尤其困难，其三维原子排列和配位几何规则难以用纯语言表示，且依赖难以文本化的人类专业知识。

Method: L2M3OF整合了晶体表示学习与语言理解，处理结构、文本和知识三种模态。使用预训练的晶体编码器和轻量级投影层将结构信息压缩到token空间，与语言指令高效对齐。构建了结构-属性-知识数据库用于训练和评估。
...
### [141] [Memory Constrained Dynamic Subnetwork Update for Transfer Learning](https://arxiv.org/abs/2510.20979)
*Aël Quélennec,Pavlo Mozharovskyi,Van-Tam Nguyen,Enzo Tartaglione*

Main category: cs.LG

TL;DR: MeDyate是一个理论驱动的内存受限动态子网络适应框架，通过改进的层重要性度量和动态通道采样策略，在极低内存预算下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 设备端神经网络训练面临严重的内存限制，限制了预训练模型在下游任务中的适应能力。

Method: 引入LaRa（层排序）改进层重要性度量，以及利用通道重要性分布时间稳定性的动态通道采样策略，在训练周期间根据重要性加权概率重新采样通道。

Result: 在大量任务和架构上的广泛评估显示，MeDyate在极端内存约束下达到最先进性能，始终优于现有静态和动态方法，同时保持高计算效率。

Conclusion: 该方法通过展示在仅几百kB RAM内存预算下的有效微调，向实现高效设备端学习迈出了重要一步。
...
### [142] [Learning Grouped Lattice Vector Quantizers for Low-Bit LLM Compression](https://arxiv.org/abs/2510.20984)
*Xi Zhang,Xiaolin Wu,Jiamang Wang,Weisi Lin*

Main category: cs.LG

TL;DR: 提出了一种分组格向量量化(GLQ)框架，通过为每组权重分配定制化的格码本，在低比特量化中实现更好的模型大小与精度权衡。
...
Method: 采用分组格向量量化，为每组权重分配可学习生成矩阵定义的格码本，使用Babai舍入近似最近格点搜索来解决量化过程的不可微问题。

Result: 在多个基准测试中，该方法相比现有后训练量化基线实现了更好的模型大小与精度权衡，特别适合在严格资源约束下部署大模型。

Conclusion: GLVQ框架提供了一种高效实用的量化流程，训练后解码简化为矩阵向量乘法，在保持性能的同时显著减少模型存储需求。
...
### [143] [GPU Memory Requirement Prediction for Deep Learning Task Based on Bidirectional Gated Recurrent Unit Optimization Transformer](https://arxiv.org/abs/2510.20985)
...
<details>
  <summary>Details</summary>
Motivation: 为满足深度学习任务中对GPU内存资源准确预测的日益关键需求，解决传统机器学习方法在内存需求预测方面的局限性。
...
### [144] [AL-CoLe: Augmented Lagrangian for Constrained Learning](https://arxiv.org/abs/2510.20995)
*Ignacio Boero,Ignacio Hounie,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 本文重新探讨了增强拉格朗日方法在机器学习约束学习问题中的应用，证明了在温和条件下的强对偶性，开发了收敛到可行最优解的算法，并提供了泛化保证。


<details>
  <summary>Details</summary>
Motivation: 尽管现代机器学习参数化大多是非凸的，拉格朗日对偶已成为处理约束学习问题的流行工具。但增强拉格朗日方法在约束学习环境中相对未被充分探索，该方法旨在缓解非凸设置中的对偶间隙，同时只需最小修改。

Method: 使用增强拉格朗日方法，建立强对偶性结果，证明对偶上升算法收敛到可行和最优的原始解，并提供PAC风格的泛化保证。

Result: 在温和条件下建立了强对偶性，证明了对偶上升算法的收敛性，并在公平约束分类任务中展示了其有效性。

Conclusion: 增强拉格朗日方法是解决机器学习中约束学习问题的有效工具，能够处理非凸优化问题并提供理论保证。
...
TL;DR: 提出了一种训练脉冲神经网络(SNN)进行多元时间序列二分类的通用框架，专注于逐步预测和低误报率下的高精度。该框架通过进化算法联合优化SNN架构和参数，并使用投票集成方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个通用的SNN训练框架，能够在低误报率下实现高精度的多元时间序列分类，特别是在资源受限的硬件上部署高效模型。

Method: 使用EONS算法进化稀疏、有状态的SNN，联合优化架构和参数；输入编码为脉冲序列，通过阈值化单个输出神经元的脉冲计数进行预测；采用投票集成方法提升鲁棒性。

Result: 在放射性源检测任务中，仅49个神经元和66个突触的SNN在1/小时误报率下达到51.8%真阳性率，优于PCA(42.7%)和深度学习(49.8%)基线；三模型集成将真阳性率提升至67.1%。硬件部署功耗仅2mW，推理延迟20.2ms。在癫痫检测任务中，集成模型达到95%真阳性率和16%假阳性率，与深度学习相当但参数显著减少。

Conclusion: 该框架能够训练出高效、低功耗的SNN模型，在多个时间序列分类任务中表现出色，特别适合资源受限的边缘计算应用。
...
TL;DR: DD2提出了一种新的条件分数蒸馏方法，使图像自回归模型能够实现一步采样，显著提升生成速度，同时保持高质量。
...
Result: 在ImageNet-256上，DD2将一步采样的FID从3.40增加到5.43，与最强基线DD1相比，将一步采样与原始AR模型的差距减少了67%，同时训练速度提升达12.3倍。

Conclusion: DD2为实现一步自回归生成迈出了重要一步，为快速高质量AR建模开辟了新可能性。
...
TL;DR: 本文研究了零阶优化在持续学习中的应用，发现它能通过产生更平坦的损失景观来减少遗忘，但会牺牲学习新任务的能力。作者提出ZO-FC方法，结合零阶优化的稳定性和一阶优化的适应性。
...
Method: 通过理论分析和实证研究评估零阶优化在各种持续学习方法中的应用，并提出ZO-FC方法：对单个基于适配器的PEFT模块使用零阶优化，同时用一阶优化更新分类器。

Result: 零阶优化能增强稳定性但会削弱可塑性，特别是在可学习分类器的情况下。ZO-FC方法在稳定性和可塑性之间取得了有效平衡，提供了实用的内存高效解决方案。

Conclusion: 零阶优化是持续学习中有前景的方法，ZO-FC设计通过结合零阶优化的稳定性和一阶优化的适应性，为设备端持续学习提供了内存高效的解决方案。
...
TL;DR: 该论文分析了梯度学习算法中学习率对样本复杂度的影响，揭示了从信息指数机制到生成指数机制的相变现象，并提出了新的分层训练方法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络中特征学习的动态过程，特别是学习率如何影响梯度算法的样本复杂度，以及如何超越传统的相关查询限制。

Method: 提出了一个统一的框架来研究梯度算法，涵盖了一轮SGD和批重用SGD，并引入了一种新的分层训练算法，使用不同学习率进行两时间尺度训练。

Result: 发现学习率选择存在相变现象：小学习率对应信息指数机制，大学习率对应生成指数机制，且学习率选择对统计和计算效率至关重要。

Conclusion: 学习率选择与算法设计同等重要，通过适当的学习率策略可以在不重用样本或修改损失函数的情况下超越相关查询的限制。
...
Method: CIPHER框架整合了可索引符号聚合近似(iSAX)进行可解释压缩和索引，基于密度的聚类(HDBSCAN)来分组重复现象，以及人在回路步骤进行高效专家验证。

Result: 在OMNI数据中分类太阳风现象的案例研究中，CIPHER成功识别了日冕物质抛射和流相互作用区域等有意义的现象。

Conclusion: CIPHER展示了一种结合符号表示、无监督学习和专家知识的通用策略，以解决物理科学中时间序列标注稀缺的问题，其代码和配置文件已公开以支持可复现性。
...
TL;DR: 提出物理一致性神经算子(PCNO)和扩散模型增强PCNO(DiffPCNO)，通过物理约束投影和一致性模型实现高保真、物理一致且不确定性感知的时空预测。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法往往忽视物理定律且无法量化时空预测中的固有不确定性，需要开发能同时保证物理一致性和不确定性量化的方法。

Method: PCNO通过物理一致性投影层在傅里叶空间强制执行质量和动量守恒；DiffPCNO在确定性预测基础上利用一致性模型来量化和减轻不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 当前脑机接口基准主要针对简单的基础任务，而实际应用如脑转文本仍难以实现。关键词检测作为实用且保护隐私的中间任务，可以填补这一空白。

Method: 使用52小时的LibriBrain语料库，采用针对极端类别不平衡的评估协议，使用AUPRC和每小时误报率作为评估指标，开发了基于1-D Conv/ResNet的紧凑基线模型，采用焦点损失和top-k池化。

Result: 参考模型在保持会话上达到约13倍置换基线的AUPRC，展示了任务的可行性。探索性分析显示：性能随训练时间对数线性提升，词频和持续时间等词级因素系统性地影响可检测性。

Conclusion: 关键词检测是脑机接口中一个可行的实用中间任务，提出的基准和参考模型为社区提供了标准化评估框架，展示了可预测的性能缩放和系统性的词级影响因素。
...
TL;DR: 提出A-GPS框架用于在线离散黑盒多目标优化，通过生成模型学习帕累托集，支持基于用户偏好的后验条件生成，避免显式计算超体积。


<details>
  <summary>Details</summary>
Motivation: 解决传统多目标优化方法在在线设置中样本效率低、难以灵活融入用户偏好、以及显式计算超体积复杂度高的问题。

Method: 使用类别概率估计器预测非支配关系，结合偏好方向向量编码用户偏好，训练生成模型同时考虑帕累托成员关系和偏好对齐。

Result: 在合成基准和蛋白质设计任务中表现出强样本效率和有效偏好融入能力，获得高质量的帕累托集近似。

Conclusion: A-GPS是一个简单而强大的方法，能够高效学习帕累托集生成模型，灵活捕捉用户偏好，避免复杂超体积计算。
...
TL;DR: 提出一种在线多类选择问题的公平分配算法，通过无损舍入方案和资源预留机制，在保证群体公平的同时实现与分数解相同的性能表现。


<details>
  <summary>Details</summary>
Motivation: 解决现有文献中的两个关键限制：缺乏无损舍入方案，以及处理属于多个类别的代理人的挑战。

Method: 基于松弛-舍入框架的随机算法，首先使用资源预留机制计算分数解，然后通过无损舍入方案保持公平性保证而不降低性能。

Result: 开发了能够确保积分算法达到与任何分数解相同期望性能的无损舍入方案，并提出了结合机器学习预测的学习增强变体。

Conclusion: 该算法在在线多类选择问题中有效平衡了公平性和效率，特别适用于实际场景中需要处理多类别代理人的情况。
...
<details>
  <summary>Details</summary>
Motivation: 随着策略优化在敏感领域（如机器人、医疗、大语言模型训练）的广泛应用，隐私保护问题日益突出，需要研究差分隐私下的策略优化理论。
...
Conclusion: 该研究为隐私保护策略优化算法提供了有价值的理论见解，表明在适当定义下，隐私成本对样本复杂度的影响相对较小。
...
TL;DR: 提出了一个可扩展的机器学习框架，使用分布式处理和量子启发的核密度矩阵方法分析帕克太阳探测器数据，揭示了内日球层太阳风参数的特征趋势和异常阈值。


<details>
  <summary>Details</summary>
Motivation: 帕克太阳探测器数据集超过150GB，传统分析方法难以处理，需要可扩展的分布式框架来探索复杂的物理数据集。

Method: 使用Dask进行大规模统计计算，采用量子启发的核密度矩阵方法估计太阳风参数的单变量和双变量分布及异常阈值。

Result: 揭示了内日球层特征趋势：太阳风速随日距增加而增加，质子密度减少，速度与密度呈反比关系。

Conclusion: 该方法为探索复杂物理数据集提供了可处理、可解释的分布式方法，促进了大规模原位测量的可重复分析。
...
Result: 该方法在两个挑战性基准测试中与自一致性等复杂方法竞争力相当，同时显著降低计算开销。
...
TL;DR: 该论文证明在正交可分数据上训练的两层ReLU网络，通过梯度流能够实现神经崩溃现象，揭示了数据结构和非线性激活对NC的影响，以及训练动态中的隐式偏置作用。


<details>
  <summary>Details</summary>
Motivation: 现有NC理论研究通常将最后一层特征视为无约束优化变量，忽略了数据结构和非线性激活的影响。本文旨在放松这一假设，探索实际神经网络训练中NC现象的形成机制。

Method: 使用两层ReLU网络对正交可分数据进行分类，通过梯度流训练分析神经崩溃现象的形成过程。

Result: 理论证明梯度流能够使网络展现出神经崩溃现象，包括特征向类均值对齐、类均值形成等角紧框架等典型NC特征。

Conclusion: 该研究将NC理论扩展到实际神经网络训练场景，揭示了数据结构和训练动态的隐式偏置在NC形成中的关键作用，为理解深度网络表示学习提供了新视角。
...
TL;DR: 提出了一种基于OpenCL细粒度共享虚拟内存的轻量级同步机制和机器学习执行时间预测模型，用于在移动设备上实现CPU-GPU协同执行，显著提升深度学习推理性能。
...
Method: 使用OpenCL细粒度SVM实现轻量级同步机制，并开发机器学习模型准确预测CPU和GPU任务的执行时间，包括GPU内核的性能特征和分发时间。

Result: 在四个移动平台上评估显示，该方法能快速选择CPU-GPU协同执行策略，线性层加速比达1.89倍，卷积层达1.75倍，接近通过穷举搜索找到的最大可能值。

Conclusion: 提出的轻量级同步机制和机器学习预测模型有效解决了移动设备上CPU-GPU协同执行的挑战，显著提升了深度学习推理性能。
...
TL;DR: DictPFL是一个实用的联邦学习框架，通过分解模型权重为静态字典和可更新查找表，仅加密传输部分参数，在保证全梯度保护的同时大幅降低计算和通信开销。
...
Result: 相比完全加密的联邦学习，DictPFL将通信成本降低402-748倍，训练速度提升28-65倍；相比选择性加密方法，开销降低51-155倍，速度提升4-19倍。
...
TL;DR: 提出了一种基于基序驱动的全局-局部上下文图的少样本分子性质预测方法，通过引入化学基序节点构建三元异构图，在全局和局部层面丰富上下文信息，显著提升了少样本学习性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法依赖大量标注数据，但在药物发现和材料科学中往往难以获取。现有的少样本分子性质预测方法虽然通过上下文图引入关系归纳偏置，但分子-性质图的结构指导有限。

Method: 构建全局三元异构图（基序-分子-性质）捕获长程组合模式，同时为每个分子-性质对构建局部子图进行单独编码，使模型关注最相关的邻近分子和基序。

Result: 在五个标准FSMPP基准测试中，该方法持续优于最先进的方法。

Conclusion: 将全局基序知识与细粒度局部上下文相结合，有效推进了鲁棒的少样本分子性质预测。
...
TL;DR: ESCORT是一个基于粒子的POMDP信念分布近似框架，通过相关感知投影和时间一致性约束来准确建模高维多模态信念分布，解决了现有方法在复杂不确定性结构表示上的不足。
...
Result: 在POMDP领域和合成多模态分布上的广泛评估表明，ESCORT在信念近似精度和下游决策质量方面始终优于最先进的方法。

Conclusion: ESCORT能够动态适应信念景观的复杂性，无需重采样或限制性分布假设，为复杂POMDP环境中的准确信念表示提供了有效解决方案。
...
TL;DR: 提出一种特征选择方法，在有限特征预算下，确保模型在多个子群体上都能表现良好。
...
Conclusion: 该方法能够平衡跨群体下游预测的整体性能，在有限特征预算下实现多群体模型的良好表现。
...
TL;DR: SolarBoost提出了一种新的分布式光伏系统功率输出预测方法，通过将聚合功率输出建模为小电网输出的组合，解决了分布式光伏面临的电网级数据缺失、容量变化、地理差异等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有集中式光伏方法难以应用于分布式光伏系统，因为分布式光伏面临电网级数据缺失、安装容量时间变化、地理可变性和面板多样性等挑战。
...
Conclusion: SolarBoost通过创新的电网级建模方法成功解决了分布式光伏系统功率预测的挑战，在实际部署中表现出色。
...
TL;DR: 提出基于DDPG的两层调度框架，将调度决策分解为全局层选择和本地节点分配，以最小化医疗物联网工作流的总完成时间。


<details>
  <summary>Details</summary>
Motivation: 医疗物联网对端到端延迟有严格要求，而调度顺序工作流以最小化总完成时间是一个NP难问题，需要高效解决方案。

Method: 使用两层DDPG强化学习框架，全局控制器选择计算层（边缘、雾或云），本地控制器处理所选层内的节点分配。

Result: 实验结果表明，随着工作流复杂性增加，该方法相比基线方法表现出越来越优越的性能。

Conclusion: 该框架能够学习有效的长期策略，适用于复杂、大规模的医疗物联网调度场景。
...
TL;DR: 提出了一种不确定性感知的强化学习框架，用于优化3D分子扩散模型，实现多属性目标的分子生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成高质量3D分子结构方面表现出色，但难以有效控制实际应用中关键的多目标约束。

Method: 利用具有预测不确定性估计的代理模型动态塑造奖励函数，在多目标优化中实现平衡，并在三个基准数据集和多种扩散模型架构上进行评估。
...
TL;DR: 提出了一个统一的矩阵分解框架，将经典和鲁棒聚类方法统一起来，包括k-means和模糊c-means的矩阵分解解释，并开发了鲁棒变体来应对异常值。


<details>
  <summary>Details</summary>
Motivation: 重新审视聚类与矩阵分解的等价关系，为经典聚类方法提供统一的矩阵分解框架，并解决传统聚类方法对异常值敏感的问题。

Method: 1) 重新推导k-means和模糊c-means的矩阵分解形式；2) 用l1,2范数替换Frobenius范数来构建鲁棒聚类；3) 开发交替最小化算法和IRLS算法。

Result: 建立了k-means和模糊c-means的矩阵分解等价关系，提出了鲁棒聚类变体，所有算法都被证明能收敛到局部最小值。

Conclusion: 该框架为经典和鲁棒聚类提供了统一的矩阵分解视角，增强了聚类方法对异常值的鲁棒性，并保证了算法的收敛性。
...
<details>
  <summary>Details</summary>
Motivation: 气象领域产生大量数据，具有高维度、缺失值频繁、变量间高度相关等特点，需要利用大数据和数据挖掘技术来提取有用知识预测天气现象。
...
TL;DR: 提出了一个用于主代理合同设计的双层最大-最大优化框架，使用隐式微分和共轭梯度计算超梯度，无需形成或求逆Hessian矩阵，可处理非线性效用、随机动态和高维动作等复杂合同设计问题。


<details>
  <summary>Details</summary>
Motivation: 传统的线性-二次模型（如Holmstr"om-Milgrom）有闭式解，但在非线性效用、随机动态或高维动作的现实环境中通常无法获得闭式解，需要新的计算方法。

Method: 采用现代机器学习中的双层优化技术，使用隐式微分结合共轭梯度计算超梯度，通过Hessian-向量积高效计算，避免直接形成或求逆Hessian矩阵。

Result: 在CARA-Normal基准环境中能恢复已知解析最优解并从随机初始化可靠收敛。方法可扩展到复杂非线性合同，如sigmoidal工资计划、相对绩效补偿、多任务合同和CARA-Poisson计数模型。
...
<details>
  <summary>Details</summary>
Motivation: 标准RL方法优化平均奖励，而减少不良输出概率的方法通常会影响平均性能。需要改进这种权衡关系。

Method: 在标准RL损失基础上增加额外损失，使用学习到的建议来引导采样低奖励输出，然后降低这些输出的概率。

Result: 实验表明RePULSe在期望奖励与不良输出概率之间实现了更好的权衡，并且具有更强的对抗鲁棒性。

Conclusion: RePULSe相比标准RL对齐方法和替代方案，能够更好地平衡期望奖励与不良输出概率的关系。
...
TL;DR: PLAN是一个持续学习框架，通过正交基向量和扰动优化策略管理任务特定子空间，减少新旧任务间的干扰，在基础模型上实现高效持续学习。


<details>
  <summary>Details</summary>
Motivation: 解决持续学习中模型需要不断适应新任务同时不遗忘过去知识的问题，特别是在大型预训练模型上的高效微调需求。

Method: 扩展LoRA方法，引入任务特定的正交基向量，通过基于扰动的优化策略最小化与已学习参数的冲突，并采用选择机制识别干扰敏感度最小的基向量。

Result: 在标准持续学习基准测试中，PLAN持续优于现有方法，为基础模型的持续学习建立了新的最先进水平。

Conclusion: PLAN框架通过主动管理任务特定子空间和干扰感知优化，在保持高效适应新任务的同时有效减少过去知识的退化，为大型预训练模型的持续学习提供了有效解决方案。
...
TL;DR: 本文介绍了GenReview数据集，这是迄今为止最大的包含LLM撰写评审的数据集，涵盖ICLR 2018-2025年所有提交论文的81K条评审，用于研究LLM在科学同行评审中的影响。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在科学同行评审中的潜在应用日益增多，需要全面数据集来理解其效用和影响，但目前缺乏这样的数据集。

Method: 构建GenReview数据集，为ICLR 2018-2025年所有提交论文使用三种独立提示（负面、正面、中性）生成LLM评审，并与原始论文和评审关联。

Result: 研究发现：LLM在评审中存在偏见；LLM撰写的评审目前可以自动检测；LLM不能总是严格遵循评审指南；LLM提供的评分仅与论文接受决定一致。

Conclusion: GenReview数据集填补了相关研究空白，为深入探索LLM在科学同行评审中的影响提供了重要资源。
...
TL;DR: 提出一种基于成对铰链损失的新型二阶代理损失函数，开发了高效的在线算法来优化AUC性能，相比传统方法实现了更紧密的遗憾界。


<details>
  <summary>Details</summary>
Motivation: AUC是分类任务中重要的性能指标，但优化AUC面临挑战：成对0/1损失的非凸性和不连续性难以优化，实例级存储的内存成本在大规模应用中造成瓶颈。

Method: 提出二阶代理损失，直接替代整个聚合的成对损失，使用训练数据的一阶和二阶统计量构建代理损失函数，并扩展到非线性设置的核方法。

Result: 理论分析显示现有在线AUC优化算法通常达到O(√T)遗憾界，而本方法达到更紧密的O(ln T)界；在多个基准数据集上的实验证明了该二阶代理损失在优化在线AUC性能方面的优越效率和有效性。

Conclusion: 所提出的二阶代理损失方法为AUC优化提供了新的范式，在理论和实验上都表现出优越性能，特别是在大规模在线学习场景中。
...
TL;DR: 本文首次系统研究表格基础模型(TFMs)中合成先验分布的设计原则，提出了基于多样化合成先验的Mitra模型，在分类和回归任务上超越了现有最佳TFMs。


<details>
  <summary>Details</summary>
Motivation: 基于上下文学习的表格基础模型通过纯合成数据集预训练就能在真实数据上表现优异，但合成先验分布的设计原则尚不明确，需要系统研究。

Method: 系统研究合成先验的关键特性，基于这些洞察开发了Mitra模型，使用经过筛选的多样化、独特且对真实表格数据性能好的合成先验混合进行训练。

Result: Mitra在分类和回归基准测试中持续优于TabPFNv2和TabICL等最先进的表格基础模型，并具有更好的样本效率。

Conclusion: 合成先验分布的设计对表格基础模型的泛化能力至关重要，精心设计的多样化先验混合可以显著提升模型性能。
...
TL;DR: ADaMoRE是一个自适应残差专家混合框架，通过无监督训练解决图神经网络在多样化图结构中的适应性问题，结合重构任务和信息论多样性正则化实现专家功能专业化。


<details>
  <summary>Details</summary>
Motivation: 图神经网络面临基本适应性问题：固定的消息传递架构难以应对现实世界图的巨大多样性，其中最优计算策略因局部结构和任务而异。现有图MoE方法受限于对监督信号的依赖和训练异构专家时的不稳定性。

Method: 采用骨干-残差专家架构，基础编码器提供稳定性，专用残差专家捕获多样化计算模式。结构感知门控网络执行细粒度节点路由，通过统一无监督目标进行端到端训练，结合重构任务和信息论多样性正则化。

Result: 在16个基准测试中验证了ADaMoRE在无监督节点分类和少样本学习方面的最先进性能，同时在不同图和任务上展现出优越的泛化能力、训练效率和更快收敛速度。

Conclusion: ADaMoRE提供了一个原则性框架，能够稳健地无监督训练图上的异构MoE，理论分析证实了设计在数据效率和训练稳定性方面的改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统流匹配生成模型中间生成步骤缺乏可解释性的问题，将不透明的神经网络轨迹转化为可解释的物理过程。

Method: 使用编码器将离散Ising配置映射到连续潜在空间，流匹配网络执行温度驱动的扩散，投影器返回离散Ising状态并保持物理约束。

Result: 在多种晶格尺寸下验证了该框架，在保持物理保真度的同时，随着晶格尺寸增大，生成速度优于蒙特卡洛方法。

Conclusion: 将物理语义嵌入生成流可以将不透明的神经轨迹转化为可解释的物理过程，为生成模型提供了新的可解释性维度。
...
TL;DR: 提出Functional Dual Anchors (FDAs)框架，通过在输入表示空间建模来改进模型合并，而不是传统的参数空间方法。
...
Method: 使用合成输入（FDAs）来捕捉任务特定的功能偏移，其诱导的梯度与任务向量对齐，并提供原则性初始化方案。

Result: 综合实验证明FDAs在模型合并中的有效性，且与参数空间模型合并方法互补。

Conclusion: FDAs框架为联合多任务训练和后验合并提供了桥梁，兼具鲁棒性和灵活性。
...
TL;DR: 本文针对神经网络世界模型，提出了一种寻找最混淆实例的约束优化方法，通过对抗训练来构建统计上接近参考模型但使次优策略性能发散的替代模型。


<details>
  <summary>Details</summary>
Motivation: 在强化学习理论中，最混淆实例对于建立遗憾下界至关重要，但在神经网络世界模型中的构造方法仍是一个开放问题。

Method: 将问题形式化为约束优化，提出对抗训练程序来寻找统计上接近参考模型但使最优和次优策略性能发散的修改模型。

Result: 实证研究表明，可实现的混淆程度与近似模型中的不确定性相关，这可能为深度基于模型的强化学习提供理论依据的探索策略。

Conclusion: 该方法为神经网络世界模型中的最混淆实例构造提供了可行方案，揭示了模型不确定性与探索需求之间的联系。
...
TL;DR: 本文对随机梯度朗之万动力学(SGLD)在惰性训练机制下的非渐近收敛性进行了分析，证明了其在满足Hessian正则性条件下能保持非退化核特性，并实现指数收敛到经验风险最小化器。


<details>
  <summary>Details</summary>
Motivation: 连续时间模型能为深度学习优化算法的训练动态提供重要见解，需要研究SGLD这种随机梯度下降的连续时间近似在惰性训练机制下的收敛特性。

Method: 使用随机梯度朗之万动力学(SGLD)，这是一个带有乘性和状态相关噪声的Itô随机微分方程，在满足Hessian正则性条件下进行分析。

Result: 证明了SGLD在整个训练过程中以高概率产生非退化核，在期望意义上指数收敛到经验风险最小化器，并建立了最优性间隙的有限时间和有限宽度边界。

Conclusion: SGLD在惰性训练机制下具有良好的收敛特性，理论结果在回归设置中得到了数值实验的验证。
...
TL;DR: 提出了三个开源库（torchrecurrent、RecurrentLayers.jl、LuxRecurrentLayers.jl），用于集中实现和测试各种RNN变体，解决缺乏统一测试平台和重复实现的问题。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏中心化库来测试各种RNN变体，重新实现不同架构耗时且容易出错，限制了可重复性和探索性研究。

Method: 在Julia和Python中开发三个开源库，集中实现多种循环单元和高层循环架构，提供一致的框架来构建和扩展RNN模型。

Result: 成功开发了三个MIT许可的开源库，提供内置的自定义和实验机制，并在GitHub上积极维护。
...
TL;DR: 提出PINN Balls方法，结合局部混合专家模型和可学习域分解，实现二阶训练的高效物理信息神经网络，在保持可扩展性的同时达到最先进精度。


<details>
  <summary>Details</summary>
Motivation: 二阶方法能提升PINNs训练效果，但内存需求大、模型扩展性差。需要开发既能利用二阶方法优势又具有良好可扩展性的训练框架。

Method: 使用局部混合专家模型结合参数高效的集成模型和稀疏编码，实现可学习的域分解结构，并采用对抗自适应采样技术使域分解适应PDE及其定义域。

Result: PINN Balls在科学机器学习中达到了比现有技术更好的精度，同时保持了宝贵的可扩展性特性。

Conclusion: 该方法成功解决了二阶训练的内存瓶颈问题，为PINNs的实际应用提供了可行的解决方案，具有坚实的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 图注意力在全局交互学习中面临过度聚合问题，当大量节点信息被不加区分地聚合到单个节点时，会导致关键信息被稀释和信息丢失。

Method: Wideformer将节点聚合过程分解为并行子过程，限制每次聚合的输入规模，并通过排序和加权机制优先处理信息量大的聚合结果。

Result: 评估表明Wideformer能有效缓解过度聚合问题，使骨干方法能够专注于重要信息，相比基线方法获得更优性能。

Conclusion: Wideformer通过并行聚合和选择性关注机制成功解决了图注意力中的过度聚合问题，为图学习任务提供了有效的解决方案。
...
TL;DR: 提出了一种基于Buffer层的新型测试时适应范式，解决了传统基于归一化层更新的方法在批次大小敏感性和预训练模型结构限制方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法主要依赖归一化层更新，但存在对小批次大小敏感、受限于预训练模型结构的问题，限制了在显著域偏移下的效果。

Method: 引入Buffer层概念，保持预训练主干网络完整性，避免灾难性遗忘，该模块化设计可无缝集成到现有TTA框架中。

Result: 实验表明该方法在缓解域偏移、增强模型鲁棒性方面优于传统方法，同时表现出强大的抗遗忘能力，在各种架构上都能带来一致的性能提升。

Conclusion: Buffer层方法有效解决了归一化层更新的根本限制，在真实世界域适应场景中展现出高效性和通用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决在真实世界噪声条件下人类活动识别的鲁棒性问题，特别是在WEAR数据集挑战中面对传感器扰动时的性能下降问题。

Method: 使用PatchTST变换器架构，为每个惯性传感器位置训练独立模型，应用随机抖动、缩放、旋转和通道丢失等数据增强技术来模拟测试时噪声，最后通过集成四个传感器模型的softmax概率进行预测。

Result: 在私有排行榜上获得了显著高于基线的macro-F1分数，证明了该方法在噪声条件下的有效性。
...
TL;DR: 提出DVC方法，通过分层评估数据贡献度来解决MLP训练中的数据选择问题，在预算约束下显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法存在简化非线性变换、忽略中间层表示、计算复杂度高等局限，无法有效处理大规模异构数据源下的MLP训练数据选择

Method: DVC方法将数据贡献分解为层价值贡献(LVC)和全局价值贡献(GVC)，使用6个度量指标从质量、相关性和分布多样性三个维度评估数据，结合UCB算法进行自适应源选择

Result: 在6个数据集和8个基线方法上的实验表明，DVC在各种预算约束下均优于现有方法，获得更高的准确率和F1分数
...
### [186] [Additive Models Explained: A Computational Complexity Approach](https://arxiv.org/abs/2510.21292)
...
<details>
  <summary>Details</summary>
Motivation: 挑战GAMs作为可解释模型时解释生成必然高效的传统假设，系统分析GAMs在不同情境下解释生成的计算复杂性。

Method: 基于标准复杂性假设(P!=NP)，分析不同形式GAMs在各种上下文中的解释生成复杂性，包括输入空间结构、组件模型类型、任务类型和解释方法的影响。

Result: 发现GAMs解释复杂性高度依赖输入空间结构；不同组件模型的复杂性差异仅在特定输入域下显现；回归与分类任务的解释复杂性存在显著区别；神经网络可加表达仅对某些解释方法和输入域带来解释便利。

Conclusion: 为GAMs多样化解释的计算可行性提供了严谨的理论图景，明确了在何种条件下这些计算是可能的或可证明困难的。
...
### [187] [An Evidence-Based Post-Hoc Adjustment Framework for Anomaly Detection Under Data Contamination](https://arxiv.org/abs/2510.21296)
*Sukanya Patra,Souhaib Ben Taieb*

Main category: cs.LG

TL;DR: EPHAD是一种测试时自适应框架，用于处理训练数据被异常值污染的无监督异常检测问题，通过结合预训练模型和经典方法提供的证据来更新模型输出。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的无监督异常检测数据集通常包含未被检测到或错误标记的异常值，导致性能显著下降，现有解决方案需要访问训练管道、数据或异常比例的先验知识，限制了实际应用。
...
### [188] [A Convergence Analysis of Adaptive Optimizers under Floating-point Quantization](https://arxiv.org/abs/2510.21314)
*Xuan Tang,Jichu Li,Difan Zou*

Main category: cs.LG

TL;DR: 提出了首个分析自适应优化器在浮点量化下的收敛理论框架，证明了Adam和Muon在低精度训练中仍能保持接近全精度的收敛速率，条件是尾数长度随迭代次数对数增长。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的快速扩展使得低精度训练变得至关重要，但现有收敛理论假设所有组件都是精确的，忽略了硬件感知量化，因此需要理论解释为什么低精度训练仍然有效。

Method: 引入了首个理论框架来分析自适应优化器在梯度、权重和优化器状态浮点量化下的收敛性，在标准随机梯度假设下推导了平滑非凸目标上的收敛速率。

Result: 证明了两种算法在尾数长度随迭代次数对数增长时，能保持接近全精度对应算法的收敛速率。Adam对权重和第二矩量化高度敏感，而Muon需要更弱的误差控制，因此可能更鲁棒。
...
### [189] [Amortized Variational Inference for Partial-Label Learning: A Probabilistic Approach to Label Disambiguation](https://arxiv.org/abs/2510.21300)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据通常存在噪声和歧义，在众包标注中人类标注者可能对相同实例分配冲突的类别标签。部分标签学习旨在处理这种情况，但现有方法要么计算密集，要么依赖启发式标签精炼。

Method: 采用摊销变分推断，使用神经网络从输入数据预测变分参数，直接逼近真实标签的后验分布。该方法保持架构无关性，结合深度学习与概率建模的优势。

Result: 在合成和真实世界数据集上的广泛实验表明，该方法在准确性和效率方面都达到了最先进的性能。
...
### [190] [Weak-to-Strong Generalization under Distribution Shifts](https://arxiv.org/abs/2510.21332)
*Myeongho Jeon,Jan Sobotka,Suhwan Choi,Maria Brbić*

Main category: cs.LG

TL;DR: RAVEN框架解决了弱模型监督强模型在分布偏移下失效的问题，通过动态学习弱模型的最优组合来提升强模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着超人类模型日益复杂，人类可能无法有效监督其行为。现有弱到强泛化方法在分布偏移下表现不佳，导致强模型性能反而比弱监督者更差。

Method: 提出RAVEN框架，动态学习弱模型的最优组合以及强模型的参数，自动识别可信的监督源。

Result: 在图像分类、文本分类和偏好对齐任务中，RAVEN在分布外任务上比基线方法提升超过30%，在分布内任务上达到或超越现有方法。

Conclusion: RAVEN能够有效解决弱到强泛化在分布偏移下的失效问题，并自动选择更准确的弱模型作为监督源。
...
### [191] [Data as a Lever: A Neighbouring Datasets Perspective on Predictive Multiplicity](https://arxiv.org/abs/2510.21303)
*Prakhar Ganesh,Hsiang Hsu,Golnoosh Farnadi*

Main category: cs.LG

TL;DR: 本文提出了一个邻接数据集框架来研究数据对模型多重性的影响，发现类间分布重叠更大的邻接数据集反而表现出更低的多重性，这与传统预期相反。基于此，作者在主动学习和数据插补两个领域提出了多重性感知的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注建模选择对多重性的影响，而数据在塑造多重性方面的关键作用被相对忽视。本文旨在通过邻接数据集框架来研究单个数据点差异对多重性的影响。

Method: 引入邻接数据集框架分析单个数据点差异对多重性的影响，通过严格的数学证明验证发现，并将该框架扩展到主动学习和数据插补两个应用领域。

Result: 发现邻接数据集具有更大的类间分布重叠时，反而表现出更低的多重性，这与传统预期相反。这一现象源于共享的Rashomon参数。

Conclusion: 数据在模型多重性中扮演关键角色，基于邻接数据集框架的发现可以指导开发多重性感知的主动学习策略和数据插补技术。
...
### [192] [Revisiting Social Welfare in Bandits: UCB is (Nearly) All You Need](https://arxiv.org/abs/2510.21312)
*Dhruv Sarkar,Nishant Pandey,Sayak Ray Chowdhury*

Main category: cs.LG

TL;DR: 本文提出了一种简单而通用的方法来解决随机多臂老虎机中的公平性问题，通过初始均匀探索阶段后接标准UCB算法，实现了接近最优的Nash遗憾和p-mean遗憾。


<details>
  <summary>Details</summary>
Motivation: 传统的遗憾度量无法解决奖励分配的公平性问题，特别是在需要公平分配奖励的场景中（如临床试验）。现有的Nash遗憾方法需要专门算法设计和强假设，限制了其适用性。

Method: 采用初始均匀探索阶段后接标准上置信界(UCB)算法，仅依赖加性Hoeffding界，可自然扩展到亚高斯奖励分布。

Result: 该方法实现了接近最优的Nash遗憾，并能推广到更广泛的p-mean遗憾类，在所有p值上均获得（接近）最优遗憾界。

Conclusion: 提出的简单方法在更弱假设下实现了公平性度量的最优性能，相比现有方法具有更好的通用性和性能。
...
### [193] [$α$-LoRA: Effective Fine-Tuning via Base Model Rescaling](https://arxiv.org/abs/2510.21345)
*Aymane El Firdoussi,El Mahdi Chayti,Mohamed El Amine Seddik,Martin Jaggi*

Main category: cs.LG

TL;DR: 提出了一类新的重参数化方法用于迁移学习，旨在提升微调模型的泛化能力。该方法在理论分析和实际实验中均表现出有效性。
...
Method: 引入新的重参数化方法类别，在迁移学习中增强泛化能力，通过随机矩阵理论进行高维二分类的理论分析，并在LLM微调等实际场景验证。

Result: 理论分析和实验验证均表明该方法能有效提升微调模型的泛化性能。

Conclusion: 提出的新重参数化方法在提升微调模型泛化能力方面具有理论和实践价值。
...
### [194] [Leverage Unlearning to Sanitize LLMs](https://arxiv.org/abs/2510.21322)
...
<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在特定任务微调时会记忆敏感数据，存在隐私泄露风险，需要一种低成本的方法来清除这些记忆。

Method: SANI采用两阶段方法：1）重置模型最后几层的特定神经元以破坏细粒度信息记忆；2）在避免记忆敏感信息的情况下微调模型。

Result: 仅需少量遗忘轮次，模型就能被有效清理，信息泄露数量显著减少。

Conclusion: 该方法特别适用于医院等已投入大量资源训练模型的机构，可在共享前对模型进行清理。
...
### [195] [SCORENF: Score-based Normalizing Flows for Sampling Unnormalized distributions](https://arxiv.org/abs/2510.21330)
...
<details>
  <summary>Details</summary>
Motivation: 传统采样方法如MCMC存在收敛慢、临界减速、模式混合差和高自相关等问题，而基于似然和对立的机器学习模型需要大量数据且容易出现模式覆盖和模式崩溃问题。

Method: 提出ScoreNF框架，在归一化流架构上构建得分学习，集成独立Metropolis-Hastings模块，即使在小训练集下也能保持高性能。

Result: 在合成2D分布和高维φ⁴晶格场理论分布上验证了方法的有效性，展示了其在采样任务中的优越性能。

Conclusion: ScoreNF提供了一种高效且无偏的采样方法，减少了对计算昂贵的MCMC生成训练数据的依赖，并提出了评估模式覆盖和模式崩溃行为的方法。
...
### [196] [Assessing the Real-World Utility of Explainable AI for Arousal Diagnostics: An Application-Grounded User Study](https://arxiv.org/abs/2510.21389)
*Stefan Kraft,Andreas Theissler,Vera Wienhausen-Wilke,Gjergji Kasneci,Hendrik Lensch*

Main category: cs.LG

TL;DR: 本研究通过用户实验比较了黑盒和白盒AI辅助对睡眠医学专家评分的影响，发现透明AI辅助作为质量控制步骤能显著提升性能，虽然耗时增加但更受用户青睐。


<details>
  <summary>Details</summary>
Motivation: AI系统在生物医学信号解释方面已能媲美人类专家，但临床实践中医生需要知道何时以及为何信任算法推荐，因此研究AI辅助的类型和时机对临床决策的影响至关重要。

Method: 对8名专业睡眠医学从业者进行应用导向的用户研究，在三种条件下评分夜间觉醒事件：手动评分、黑盒AI辅助、透明白盒AI辅助，辅助时机包括开始阶段或事后质量控制。

Result: 与临床标准相比，AI和人类-AI团队均显著优于无辅助专家，透明AI辅助作为质量控制步骤比黑盒辅助提升约30%的事件级性能，质量控制时机进一步改善计数结果。
...
### [197] [Large Language Models as Model Organisms for Human Associative Learning](https://arxiv.org/abs/2510.21408)
*Camila Kolling,Vy Ai Vo,Mariya Toneva*

Main category: cs.LG

TL;DR: 该研究使用大语言模型研究联想学习中的表征变化，发现非单调可塑性模式，词汇干扰调节表征分化。


<details>
  <summary>Details</summary>
Motivation: 测试生物系统中表征变化假说具有挑战性，大语言模型提供可扩展的替代方案来研究联想学习机制。

Method: 基于大语言模型的上下文学习，改编认知神经科学联想学习范式，在六个模型中研究表征演化。

Result: 发现与非线性可塑性假说一致的非单调模式，中等相似项目在学习后分化；词汇干扰增强分化。

Conclusion: 大语言模型不仅是研究类人学习系统表征动态的强大工具，也是生成关于大脑记忆重组原理新假说的通用计算模型。
...
### [198] [DreamerV3-XP: Optimizing exploration through uncertainty estimation](https://arxiv.org/abs/2510.21418)
...
Method: 使用基于回报、重建损失和价值误差的优先重放缓冲器，以及基于世界模型集成预测环境奖励分歧的内在奖励。

Result: 在Atari100k和DeepMind Control Visual Benchmark任务上验证了DreamerV3的结果，并显示出更快的学习速度和更低的动态模型损失。

Conclusion: DreamerV3-XP扩展有效提升了学习效率，在稀疏奖励环境中表现尤为突出。
...
<details>
  <summary>Details</summary>
Motivation: 传统收益率曲线估计方法（如拔靴法、Nelson-Siegel模型）在债券稀疏、价格波动或噪声较大时存在过拟合或不稳定问题，需要更稳健的解决方案。

Method: 使用神经网络框架独立估计每日收益率曲线，引入新的损失函数来强制平滑性和稳定性，可整合领域特定约束（如与无风险基准对齐）。

Result: 在瑞典抵押债券上的实证结果显示，该方法比Nelson-Siegel-Svensson和Kernel-Ridge方法产生更稳健和稳定的收益率曲线估计。

Conclusion: 该神经网络框架为小型抵押债券市场提供了有效的收益率曲线估计解决方案，允许从业者根据需求在平滑性和准确性之间进行权衡。
...
### [200] [Compositional Monte Carlo Tree Diffusion for Extendable Planning](https://arxiv.org/abs/2510.21361)
*Jaesik Yoon,Hyeonseo Cho,Sungjin Ahn*

Main category: cs.LG

TL;DR: C-MCTD通过组合式规划框架解决了MCTD在轨迹长度和局部规划方面的限制，引入了三种互补组件来实现全局感知的规划。


<details>
  <summary>Details</summary>
Motivation: MCTD虽然结合了扩散模型和树搜索进行轨迹探索，但受限于训练轨迹长度，且规划过程局限于局部，缺乏全局上下文。
...
Conclusion: C-MCTD通过组合式方法显著提升了规划能力，解决了MCTD的局限性，为长序列推理任务提供了更有效的解决方案。
...
TL;DR: FairImagen是一个后处理去偏框架，通过处理提示嵌入来减少文本到图像扩散模型中的社会偏见，无需重新训练或修改底层模型。
...
Method: 使用公平主成分分析将CLIP输入嵌入投影到最小化群体特定信息的子空间，同时通过经验噪声注入和统一跨人口统计投影方法来增强去偏效果。
...
### [202] [Randomized Neural Network with Adaptive Forward Regularization for Online Task-free Class Incremental Learning](https://arxiv.org/abs/2510.21367)
...
<details>
  <summary>Details</summary>
Motivation: 解决类增量学习在实际应用中的两个主要问题：(1) 非独立同分布批量数据流且无边界提示的在线任务无关场景；(2) 长任务流中的记忆丢失问题，旨在实现高效决策并减少累积遗憾。

Method: 提出随机神经网络前向正则化框架，集成无监督知识到递归凸优化中。具体包括edRVFL-kF算法（可调前向正则化）和edRVFL-kF-Bayes（基于贝叶斯学习的自适应调整），实现单次闭式增量更新和可变学习率。

Result: 在2个图像数据集上的6个指标测试显示，该方法在动态性能、消融测试和兼容性方面均表现出色，有效避免了灾难性遗忘，无需过去数据重放。

Conclusion: 提出的OTCIL框架通过-kF和-kF-Bayes风格显著提升了在线任务无关类增量学习的性能，验证了前向正则化和贝叶斯自适应调整的有效性。
...
### [203] [Generative Correlation Manifolds: Generating Synthetic Data with Preserved Higher-Order Correlations](https://arxiv.org/abs/2510.21610)
...
<details>
  <summary>Details</summary>
Motivation: 当前合成数据生成方法往往只能复制简单统计量，但无法保留数据中定义复杂多变量交互的成对和高阶相关结构，导致合成数据在复杂建模任务中失效。

Method: 使用Cholesky分解目标相关矩阵来生成合成数据，确保数学上完整保留从简单成对关系到高阶交互的整个相关结构。

Result: 该方法计算效率高，能够生成保留完整相关结构的合成数据。
...
### [204] [Disentangled Representation Learning via Modular Compositional Bias](https://arxiv.org/abs/2510.21402)
*Whie Jung,Dong Hoon Lee,Seunghoon Hong*

Main category: cs.LG

TL;DR: 提出了一种解耦表示学习的组合偏置方法，通过因子特定的混合策略和互补目标函数，无需修改架构或目标即可实现属性和对象的解纠缠。


<details>
  <summary>Details</summary>
Motivation: 现有的解纠缠表示学习方法依赖因子特定的策略，当新变化因子不符合先验假设或多个因子共存时需要重新设计架构或目标，带来显著开销。

Method: 提出组合偏置作为模块化归纳偏置，通过因子特定的混合策略随机重混潜在变量，使用先验损失和组合一致性损失来发现因子结构。

Result: 实验表明该方法在属性和对象解纠缠方面具有竞争力，并独特地实现了全局风格和对象的联合解纠缠。

Conclusion: 该方法通过调整混合策略即可实现不同因子的解纠缠，无需修改目标或架构，为解纠缠表示学习提供了通用框架。
...
### [205] [Cost-Sensitive Freeze-thaw Bayesian Optimization for Efficient Hyperparameter Tuning](https://arxiv.org/abs/2510.21379)
...
<details>
  <summary>Details</summary>
Motivation: 解决用户在预期性能改进不理想时需要提前停止超参数优化过程的问题，考虑计算成本与性能之间的权衡。

Method: 在冻结-解冻框架中引入效用函数，结合新的采集函数和停止准则，动态选择最有潜力的配置继续训练，并使用迁移学习提高样本效率。
...
### [206] [DEEDEE: Fast and Scalable Out-of-Distribution Dynamics Detection](https://arxiv.org/abs/2510.21638)
...
Method: DEEDEE使用两个统计量：episodewise均值和RBF核相似度到训练摘要，捕捉互补的全局和局部偏差，是一个极简的替代方案。

Result: DEEDEE在标准RL OOD测试套件中匹配或超越当代检测器，计算量减少600倍（FLOPs/墙钟时间），平均绝对准确率提高5%。

Conclusion: 结果表明，不同类型的异常通常通过少量低阶统计量在RL轨迹上留下印记，为复杂环境中的OOD检测提供了紧凑的基础。
...
### [207] [Self-diffusion for Solving Inverse Problems](https://arxiv.org/abs/2510.21417)
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散方法需要预训练模型来学习反向噪声过程，而自扩散旨在开发自包含的迭代过程，不依赖外部预训练模型

Method: 自扩散框架交替进行加噪和去噪步骤，使用随机初始化的单卷积网络作为自去噪器，通过数据保真度损失持续训练

Result: 在线性逆问题上，自扩散方法取得了与其他方法相当或更优的性能

Conclusion: 自扩散通过利用神经网络的光谱偏置和调度噪声过程，无需预训练分数函数或外部去噪器，具有高度灵活性和广泛应用性
...
### [208] [A Rapid Physics-Informed Machine Learning Framework Based on Extreme Learning Machine for Inverse Stefan Problems](https://arxiv.org/abs/2510.21426)
...
Result: 与传统的PINNs相比，PIELM将相对L2误差提高了3-7个数量级的预测精度，同时节省了超过94%的训练时间。

Conclusion: PIELM框架为逆Stefan问题提供了一种快速、准确的求解方法，在精度和效率方面均显著优于传统PINNs方法。
...
### [209] [Causality Meets Locality: Provably Generalizable and Scalable Policy Learning for Networked Systems](https://arxiv.org/abs/2510.21427)
*Hao Liang,Shuqing Shi,Yudi Zhang,Biwei Huang,Yali Du*

Main category: cs.LG

TL;DR: GSAC框架结合因果表示学习与元演员-评论家学习，通过稀疏局部因果掩码和近似紧凑表示实现图网络上的可扩展学习和领域泛化。


<details>
  <summary>Details</summary>
Motivation: 解决大规模网络系统（如交通、电力、无线网络）中强化学习面临的可扩展性和环境变化挑战。

Method: 每个智能体学习稀疏局部因果掩码识别影响动态的最小邻域变量，生成状态和领域因子的近似紧凑表示；元演员-评论家在多个源域上训练共享策略，并在测试时通过少量轨迹估计新领域因子。

Result: GSAC能够快速适应新领域，显著优于从头学习和传统适应基线方法。

Conclusion: 该框架在因果恢复、演员-评论家收敛和适应差距方面建立了有限样本保证，实现了可扩展的领域泛化强化学习。
...
### [210] [Unified token representations for sequential decision models](https://arxiv.org/abs/2510.21448)
...
Method: 开发了UTR统一令牌表示，将回报目标、状态和动作合并为单个令牌，并基于此构建了UDT(Transformer骨干)和UDC(门控CNN骨干)两个变体。

Result: 两种变体都实现了与最先进方法相当或更优的性能，同时计算量显著降低。理论分析显示UTR具有更紧的Rademacher复杂度界限，表明更好的泛化能力。

Conclusion: UTR方法在不同架构上泛化良好，可能为未来大规模决策模型提供高效基础，实现可扩展控制。
...
### [211] [ParaRNN: Unlocking Parallel Training of Nonlinear RNNs for Large Language Models](https://arxiv.org/abs/2510.21450)
...
<details>
  <summary>Details</summary>
Motivation: 传统RNN的序列特性限制了并行计算能力，而现有的并行化架构如Transformers和状态空间模型虽然能并行化，但线性约束限制了其表达能力，无法建模复杂的非线性序列依赖关系。

Method: 将非线性递归关系序列转化为单一方程组系统，使用牛顿迭代结合自定义并行归约方法进行并行求解，实现了非线性RNN的高效并行训练。

Result: 实现了高达665倍的速度提升，成功训练了70亿参数的LSTM和GRU模型，在困惑度指标上达到与同等规模Transformer和Mamba2架构相当的性能。

Conclusion: ParaRNN框架突破了非线性RNN的并行化障碍，为高效序列建模研究提供了开源工具，使研究人员能够在大规模上探索新的非线性RNN模型。
...
### [212] [Towards Explainable Personalized Recommendations by Learning from Users' Photos](https://arxiv.org/abs/2510.21455)
...
<details>
  <summary>Details</summary>
Motivation: 解释复杂系统（如推荐系统）的输出对用户和公司都至关重要。用户上传的照片可以强化或证明他们对物品的评价，因此预测用户会拍摄什么照片可以生成最能说服用户的解释。

Method: 提出一个形式化框架来估计给定（用户，照片）对的作者概率。使用TripAdvisor收集的餐厅评论（包含照片）数据来验证方法。

Result: 通过预测用户会为物品拍摄的吸引人的照片，推荐系统可以解释其推荐结果并提高可靠性。公司还可以通过这些照片分布了解客户关注的产品特性。

Conclusion: 将个性化解释作为推荐本身学习是可行的，这种方法不仅能提高推荐系统的可信度，还能为企业提供关于客户偏好的直观洞察。
...
### [213] [Estimating Treatment Effects in Networks using Domain Adversarial Training](https://arxiv.org/abs/2510.21457)
...
### [214] [Parameter-Free Hypergraph Neural Network for Few-Shot Node Classification](https://arxiv.org/abs/2510.21462)
*Chaewoon Bae,Doyun Choi,Jaehyun Lee,Jaemin Yoo*

Main category: cs.LG

TL;DR: 提出ZEN，一种零参数超图神经网络，通过闭式解和冗余感知传播实现高效且可解释的少样本节点分类
...
Conclusion: ZEN实现了表达能力和效率的平衡，决策过程完全可解释，为数据集特征提供了洞察
...
### [215] [Benchmarking Catastrophic Forgetting Mitigation Methods in Federated Time Series Forecasting](https://arxiv.org/abs/2510.21491)
...
Result: 建立了一个新的时间序列联邦学习中灾难性遗忘基准，对最先进方法进行了全面比较分析，并发布了可复现的开源框架。
...
### [216] [Uniform Convergence Beyond Glivenko-Cantelli](https://arxiv.org/abs/2510.21506)
*Tanmay Devale,Pramith Devulapalli,Steve Hanneke*

Main category: cs.LG

TL;DR: 本文扩展了Vapnik和Chervonenkis的均匀收敛理论，提出了均匀均值可估性(UME-learnability)概念，证明了均值向量可分性是UME-learnability的充分条件但不是必要条件，并解决了关于UME-learnable集合可数并的猜想。


<details>
  <summary>Details</summary>
Motivation: 传统Vapnik-Chervonenkis理论局限于经验均值估计器，本文旨在建立更一般的均匀均值估计框架，研究任意估计器下的均匀估计可能性。

Method: 在分布集合的均值向量空间上工作，分析均值向量可分性与UME-learnability的关系，并通过构造非可分但UME-learnable的分布集合来证明可分性不是必要条件。

Result: 证明了均值向量可分性是UME-learnability的充分条件，但通过反例表明不是必要条件，并证明了UME-learnable集合的可数并也是UME-learnable的。

Conclusion: 建立了均匀均值可估性的完整理论框架，解决了相关猜想，为统计学习理论提供了新的理论基础。
...
### [217] [Surrogate-based quantification of policy uncertainty in generative flow networks](https://arxiv.org/abs/2510.21523)
...
TL;DR: 提出使用多项式混沌展开构建代理模型，通过少量训练好的流网络集成来量化生成流网络中的策略不确定性，该方法可用于蒙特卡洛采样估计奖励不确定时的策略不确定性。
...
Method: 构建多项式混沌展开的代理模型，在低维参数化奖励函数空间上拟合少量训练流网络，学习奖励函数与流网络轨迹中各步骤动作概率分布的关系。

Result: 在离散和连续网格世界、符号回归以及贝叶斯结构学习任务中验证了方法的性能。

Conclusion: 提出的代理模型方法能够通过廉价蒙特卡洛采样有效估计奖励不确定时的策略不确定性。
...
### [218] [A Unified Model for Multi-Task Drone Routing in Post-Disaster Road Assessment](https://arxiv.org/abs/2510.21525)
*Huatian Gong,Jiuh-Biing Sheu,Zheng Wang,Xiaoguang Yang,Ran Yan*

Main category: cs.LG

TL;DR: 提出统一模型(UM)解决灾后道路评估中的无人机路径规划问题，通过单一神经网络处理8种变体问题，显著减少训练时间和参数，在解质量和实时性方面优于传统方法和单任务DRL方法。
...
### [219] [Probe-based Fine-tuning for Reducing Toxicity](https://arxiv.org/abs/2510.21531)
*Jan Wehner,Mario Fritz*

Main category: cs.LG

TL;DR: 该论文研究了使用探针（probes）作为训练信号时面临的Goodhart定律问题，提出了两种基于监督微调和直接偏好优化的训练方法，发现在减少毒性任务中，基于探针的偏好优化能更好地保持探针检测能力，且探针多样性益处有限，重新训练探针即可恢复高检测精度。


<details>
  <summary>Details</summary>
Motivation: 探针可用于检测模型内部的不良行为（如欺骗或偏见），但将其作为训练目标时可能因Goodhart定律而失效。研究旨在探索如何在训练中使用探针信号的同时保持其检测可靠性。

Method: 提出两种方法：基于监督微调（SFT）和直接偏好优化（DPO）的探针训练。在毒性减少测试平台上评估探针精度下降程度，尝试使用探针集成、保留未训练探针和训练后重新训练探针来保持检测精度。

Result: 基于探针的偏好优化比基于分类器的方法更好地保持了探针可检测性；探针多样性提供的实际益处有限，只需在优化后重新训练探针即可恢复高检测精度。
...
### [220] [FrameShield: Adversarially Robust Video Anomaly Detection](https://arxiv.org/abs/2510.21532)
*Mojtaba Nafez,Mobina Poulaei,Nikan Vasei,Bardia Soltani Moakhar,Mohammad Sabokrou,MohammadHossein Rohban*

Main category: cs.LG

TL;DR: 提出了一种名为时空区域扭曲（SRD）的伪异常生成方法，通过在正常视频的局部区域应用严重增强来创建合成异常，同时保持时间一致性，从而显著提升弱监督视频异常检测模型的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测模型容易受到对抗攻击，但由于弱监督只提供视频级标签而需要帧级预测，传统对抗训练方法效果不佳。直接使用模型生成的伪标签进行帧级对抗训练会因标签噪声严重降低性能。

Method: 提出SRD方法，在正常视频的局部区域应用严重增强来创建合成异常，保持时间一致性。将这些精确标注的合成异常与噪声伪标签结合，显著减少标签噪声，实现有效对抗训练。
...
### [221] [Excision Score: Evaluating Edits with Surgical Precision](https://arxiv.org/abs/2510.21537)
*Nikolai Gruzinov,Ksenia Sycheva,Earl T. Barr,Alex Bezzubov*

Main category: cs.LG

TL;DR: 提出了修订相似性问题，引入切除分数(ES)作为新的评估指标，通过移除共享内容来专注于比较修订差异区域，在代码编辑评估中显著优于现有指标。


<details>
  <summary>Details</summary>
Motivation: 现有相似性度量（如BLEU）在评估文档修订时存在根本缺陷，因为它们被共享内容主导，无法准确反映人类对修订差异的判断。

Method: 提出切除分数(ES)，使用最长公共子序列(LCS)移除现有文档与修订版本之间的共享内容，然后比较剩余的不同区域，并通过近似算法将标准立方LCS计算加速到二次复杂度。

Result: 在HumanEvalFix上的实验显示，ES比最接近的竞争对手SARI提高了12%的皮尔逊相关性，比标准指标如BLEU提高了>21%。当增加共享上下文时，ES的改进进一步增加到20%和>30%。

Conclusion: ES是一个有效的修订相似性度量，能够正确处理共享内容、代码块移动、插入删除等复杂情况，与人类判断更好地对齐。
...
### [222] [Cost Minimization for Space-Air-Ground Integrated Multi-Access Edge Computing Systems](https://arxiv.org/abs/2510.21541)
...
<details>
  <summary>Details</summary>
Motivation: 解决SAGIN-MEC在低空经济中面临的异构节点协调、复杂因素建模和部分可观测环境下实时决策的挑战。

Method: 采用分层SAGIN-MEC架构，提出MADDPG-COCG算法：使用MADDPG优化连续时序决策，结合凸优化和联盟博弈处理混合变量决策。

Result: 相比基准算法，显著降低了用户设备成本、任务完成延迟和能耗，仅略微增加无人机能耗，且具有更好的收敛稳定性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动心电图诊断系统在透明性和对未见病症的泛化能力方面存在不足，需要与临床工作流程对齐的可解释诊断方法。

Method: 使用LLM辅助、专家验证的过程构建结构化临床观察，利用预训练多模态模型对齐ECG和文本嵌入，通过对比正负临床观察进行鉴别诊断。
...
TL;DR: 通过在图神经网络上预训练经典算法，可以提升分子属性预测任务的性能，在HIV抑制和临床毒性预测任务中分别获得6%和3%的绝对增益。
...
Result: 相比随机初始化基线，预训练模型在HIV抑制任务上获得6%绝对增益（Segments Intersect算法），在临床毒性预测上获得3%增益（Dijkstra算法）。

Conclusion: 将经典算法先验嵌入图神经网络提供了有用的归纳偏置，能够提升在复杂真实世界图数据上的性能表现。
...
TL;DR: 该论文通过开发无监督学习工具研究深度神经网络内部机制，包括数据内在维度估计方法Gride、隐藏层概率密度演化分析，以及网络泛化能力与冗余表示的关系。
...
TL;DR: REVE是一个专门为EEG信号设计的预训练模型，通过创新的4D位置编码方案处理任意长度和电极配置的信号，在92个数据集上预训练超过60,000小时，在10个下游任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有EEG基础模型由于公共数据集的异质性（不同协议、设备和电极配置）而难以泛化，通常限制在单一设置下预训练，导致性能不佳。
...
Conclusion: REVE通过标准化EEG研究支持临床神经科学进展，释放代码、预训练权重和教程。
...
TL;DR: 提出HOPSS算法，通过同源扰动方法减少生成训练数据所需的时间步数，在保持模型训练精度的同时显著加速数据集生成过程。
...
Conclusion: HOPSS算法能够显著加速PDE训练数据集的生成过程，在保持模型训练精度的同时大幅减少计算开销。
...
TL;DR: 本文提出了计算张量网络(TNs)SHAP解释的通用框架，证明在张量链(TT)结构下SHAP计算可在多对数时间内并行完成，并揭示了神经网络宽度而非深度是SHAP计算的主要计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: SHAP解释对于简单模型可在多项式时间内计算，但对神经网络等更复杂的黑盒模型计算变得NP难，而这类模型恰恰最需要解释。本文旨在为更广泛的张量网络模型提供精确的SHAP计算解决方案。

Method: 提出了计算任意结构张量网络精确SHAP解释的通用框架，特别针对张量链结构设计了多对数时间并行算法，并通过将二值化神经网络约简为张量网络表示来分析计算复杂度。

Result: 在张量链结构下SHAP计算可在多对数时间内并行完成，该结果可推广到决策树、树集成、线性模型和线性RNN等流行模型。对于二值化神经网络，当网络宽度固定时SHAP计算变得高效可处理，但即使深度固定计算仍然困难。

Conclusion: 张量网络为SHAP计算提供了高效框架，特别是在张量链结构下可实现高效并行计算。研究揭示了神经网络宽度而非深度是SHAP计算的主要计算瓶颈，这对模型可解释性研究具有重要意义。
...
TL;DR: 提出了广义流映射(GFM)，一种在黎曼流形上的少步生成模型，通过三种自蒸馏训练方法实现高效推理，在多个几何数据集上达到最先进的样本质量。


<details>
  <summary>Details</summary>
Motivation: 当前几何生成模型在推理时计算成本高，需要复杂的数值模拟步骤，限制了实际应用。

Method: 提出了广义流映射(GFM)框架，包含三种自蒸馏训练方法：广义拉格朗日流映射、广义欧拉流映射和广义渐进流映射，将欧几里得空间的少步生成模型推广到任意黎曼流形。

Result: 在多个几何数据集（地理空间数据、RNA扭转角、双曲流形）上实现了最先进的样本质量，在单步和少步评估中表现优异，隐式概率流的对数似然也达到竞争水平。

Conclusion: GFM统一并提升了现有的欧几里得少步生成模型到黎曼流形设置，为几何生成模型提供了高效的推理解决方案。
...
TL;DR: 本文建立了在流行度调整块模型(PABM)下图聚类的理论极限，解决了现有模型的局限性。PABM引入了簇内和簇间连接的独立流行度参数，证明了即使传统边密度信号消失，只要簇内和簇间流行度系数不同，聚类恢复仍然是可能的。


<details>
  <summary>Details</summary>
Motivation: 解决现有图聚类模型(如SBM和DCBM)的局限性，这些模型假设统一的顶点度或应用统一的度校正。PABM通过引入簇内和簇间连接的独立流行度参数，捕捉了DCBM忽略的度异质性维度。

Method: 在PABM框架下建立理论分析，推导最优聚类错误率。通过数值实验验证理论结果，使用合成和真实数据集比较基于k个特征向量和k²个特征向量的谱聚类算法。

Result: 证明了在PABM中，即使传统边密度信号消失，只要簇内和簇间流行度系数不同，聚类恢复仍然是可能的。实验表明，包含k²个特征向量的谱聚类算法优于传统的谱方法。

Conclusion: PABM捕捉了DCBM忽略的度异质性重要维度，局部连接模式的差异可以独立于全局边密度增强簇可分性。PABM的期望邻接矩阵秩在k到k²之间，因此需要包含更多特征向量来捕获完整结构信息。
...
TL;DR: 本文研究了等变性与模型校准之间的关系，通过理论分析和实验验证了等变网络在不确定性估计中的表现，揭示了对称性不匹配如何导致模型校准错误。


<details>
  <summary>Details</summary>
Motivation: 在数据稀疏场景中，等变网络能改善建模效果，不确定性估计可防止过度自信，但等变性与模型校准之间的关系尚未被系统研究。

Method: 通过证明等变条件下不确定性校准误差（ECE和ENCE）的上下界，建立等变性与不确定性估计的理论框架，并用真实和模拟数据集进行数值实验验证。

Result: 理论分析阐明了等变模型的泛化界限，实验结果显示对称性不匹配会导致分类和回归任务中的校准错误，并分析了对称性大小、偶然性和认知性不确定性的影响趋势。

Conclusion: 等变性对模型校准有重要影响，对称性不匹配会显著影响不确定性估计的准确性，这为等变网络在数据稀疏场景中的应用提供了理论指导。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络TSP求解器虽然高效但缺乏可解释性，无法了解其学习的几何模式和启发式策略

Method: 在基于强化学习的指针网络上训练稀疏自编码器，分析编码器残差流中的激活特征

Result: 发现模型自然发展出反映基本TSP概念的特征：凸包节点边界检测器、局部密集区域聚类敏感特征、几何分区编码的分隔符特征

Conclusion: 首次揭示了神经TSP求解器在节点选择前的内部计算机制，证明几何结构无需显式监督即可涌现，为结合神经效率和算法可解释性的透明混合系统提供了路径
...
TL;DR: 提出Equivariance by Contrast (EbC)方法，从观察对中学习等变嵌入，无需依赖群特定的归纳偏置，成功在非阿贝尔群和乘积群上验证。


<details>
  <summary>Details</summary>
Motivation: 旨在从数据中学习等变表示，而不依赖于对群结构的先验知识，实现通用的编码器仅从群作用观察中学习等变性。

Method: 通过对比学习联合学习潜在空间和群表示，使群作用对应可逆线性映射，在dSprites数据集上验证，涵盖离散旋转和平移的有限群。

Result: 学习到的嵌入表现出高保真等变性，群操作在潜在空间中忠实再现，在非阿贝尔正交群和一般线性群上进一步验证。

Conclusion: 这是首次成功演示仅从群作用观察中实现通用编码器等变学习，包括非平凡非阿贝尔群和乘积群，为计算机视觉中的仿射等变性建模提供基础。
