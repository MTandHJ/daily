...
TL;DR: TDAD是一种将AI智能体提示视为编译产物的方法，通过工程师提供行为规范，编码智能体生成可执行测试，再由第二个编码智能体迭代优化提示直至测试通过，以解决生产环境中LLM智能体行为合规性难以测量的问题。


<details>
  <summary>Details</summary>
Motivation: 当前开发实践无法为生产环境中部署的工具使用型LLM智能体提供可测量的行为合规性保证。小的提示更改可能导致静默回归，工具误用难以检测，政策违规往往在部署后才显现。

Method: TDAD将智能体提示视为编译产物，工程师提供行为规范，第一个编码智能体将其转换为可执行测试，第二个编码智能体迭代优化提示直至测试通过。为防止规范博弈，引入三种机制：1)可见/隐藏测试分割；2)通过后编译智能体进行语义突变测试；3)规范演化场景量化需求变更时的回归安全性。

Result: 在SpecSuite-Core基准测试中，TDAD在24个独立试验中实现了92%的v1编译成功率，平均隐藏测试通过率97%；演化规范编译成功率58%，突变得分86-100%，v2隐藏通过率78%，回归安全得分97%。

Conclusion: TDAD提供了一种系统化的方法来确保AI智能体的行为合规性，通过测试驱动的开发方法显著提高了智能体提示的可靠性和可维护性，为生产环境部署提供了可测量的行为保证。
...
TL;DR: 本文探讨了生成式AI在软件工程定性研究中的应用，分析了其潜力与局限，强调需要根据具体研究策略和数据特点进行适配。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和生成式AI的发展，出现了AI可以自动化定性分析的说法，但这些说法往往过度概括了有限的成功案例。需要系统评估GenAI在软件工程定性研究中的实际应用价值。

Method: 通过讨论GenAI在软件工程定性研究中的新兴应用，概述定性工作的维度，回顾GenAI辅助的实证证据，分析GenAI介导的定性研究实践的利弊，并重新审视定性研究质量因素。

Result: GenAI支持需要根据具体数据和特定研究策略的特点进行精心适配，不能一概而论。需要权衡GenAI辅助研究的优势与潜在风险。

Conclusion: 本文旨在告知研究者GenAI辅助定性研究的承诺与陷阱，并计划未来进一步推进对其在软件工程中应用的理解。
...
### [3] [Automating Detection and Root-Cause Analysis of Flaky Tests in Quantum Software](https://arxiv.org/abs/2603.09029)
*Janakan Sivaloganathan,Ainaz Jamshidi,Andriy Miranskyy,Lei Zhang*

Main category: cs.SE

TL;DR: 该研究开发了一个自动化管道，用于检测量子软件中的不稳定测试，利用大语言模型进行分类和根因分析，将原有数据集扩展了54%，最佳模型在检测和根因识别方面都取得了超过0.94的F1分数。


<details>
  <summary>Details</summary>
Motivation: 量子软件测试具有固有的概率性输出，容易产生不稳定的测试结果（量子不稳定测试），这些测试会掩盖真实缺陷并降低开发效率，但目前缺乏系统化的检测和诊断工具。

Method: 基于先前对14个量子软件仓库的手动分析，使用大语言模型和余弦相似度自动化发现更多不稳定测试案例。评估了OpenAI GPT、Meta LLaMA、Google Gemini和Anthropic Claude等系列的大语言模型，用于从问题描述和代码上下文中分类不稳定性和识别根因。

Result: 通过自动化管道发现了25个先前未知的不稳定测试，将原始数据集大小增加了54%。最佳模型Google Gemini在不稳定性检测方面获得0.9420的F1分数，在根因识别方面获得0.9643的F1分数。

Conclusion: 大语言模型能够为量子软件中不稳定报告的筛选和根因理解提供实际支持。扩展的数据集和自动化管道为量子软件工程社区提供了可重用的工具，未来工作将集中在提高检测鲁棒性和探索量子不稳定测试的自动修复。
...
### [4] [The Future of Software Engineering Conferences: A New Zealand Perspective](https://arxiv.org/abs/2603.09035)
*Kelly Blincoe,Sherlock A. Licorish,Judith Fuchs,Amjed Tahir*

Main category: cs.SE

TL;DR: 该论文分析了新西兰等地理偏远地区研究人员参与软件工程会议的障碍，提出了促进全球公平参与的策略建议。


<details>
  <summary>Details</summary>
Motivation: 软件工程会议对于知识交流和合作至关重要，但地理位置偏远地区（如新西兰）的研究人员面临显著参与障碍，包括高昂的旅行成本、学术日历不匹配和有限代表性等问题。

Method: 通过识别和分析软件工程会议参与的主要障碍，包括高旅行成本、学术日历错位和代表性不足，并基于这些分析提出具体的改进策略。

Result: 识别出三大主要障碍：1）高昂的旅行成本；2）学术日历不匹配；3）有限的地域代表性。提出了混合参与模式、成本敏感型会议选址和治理改革等具体策略。

Conclusion: 通过实施混合参与、选择成本敏感型会议地点和进行治理改革等策略，可以促进软件工程会议的全球公平参与，从而加强整个研究社区的建设。
...
### [5] [Class Model Generation from Requirements using Large Language Models](https://arxiv.org/abs/2603.09100)
*Jackson Nguyen,Rui En Koe,Fanyu Wang,Chetan Arora,Alessio Ferrari*

Main category: cs.SE

TL;DR: 该研究评估了GPT-5、Claude Sonnet 4.0、Gemini 2.5 Flash Thinking和Llama-3.1-8B-Instruct等先进大语言模型从自然语言需求自动生成UML类图的能力，提出了结合LLM-as-a-Judge和人工评估的双重验证框架，结果表明LLM能够生成结构连贯、语义有意义的UML图，与人类评估高度一致。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的出现为自动化传统需要大量人工工作的软件工程活动提供了新机会。类图生成是软件设计中关键但资源密集的阶段，需要探索LLM在此任务上的能力和可靠性。

Method: 研究采用链式思维提示从自然语言需求中提取领域实体、属性和关联，生成PlantUML表示。提出双重验证框架，结合LLM-as-a-Judge方法和人工评估，使用八个异构数据集，通过两个独立LLM法官（Grok和Mistral）进行结构化成对比较，并与专家评估进行验证。

Result: LLM能够生成结构连贯、语义有意义的UML类图，在完整性、正确性、标准符合性、可理解性和术语一致性五个质量维度上表现出色。LLM评估与人类评估之间存在显著一致性，表明LLM不仅可作为建模助手，还可作为自动化需求工程工作流中的可靠评估者。

Conclusion: LLM在UML类图自动化生成方面具有实际应用潜力，双重验证框架有效评估了LLM的能力和局限性，为LLM驱动的需求工程工作流提供了实用见解。
...
### [6] [Can ChatGPT Generate Realistic Synthetic System Requirement Specifications? Results of a Case Study](https://arxiv.org/abs/2603.09335)
*Alex R. Mattukat,Florian M. Braun,Horst Lichter*

Main category: cs.SE

TL;DR: 研究探索使用ChatGPT生成合成系统需求规格说明书，无需真实数据，通过系统化提示工程生成300份跨行业SSyRS，62%专家认为真实，但存在矛盾陈述和缺陷。


<details>
  <summary>Details</summary>
Motivation: 真实系统需求规格说明书因专有性和保密性难以获取，而合成生成可解决数据稀缺问题。大型语言模型如ChatGPT提供自然语言生成能力，但存在幻觉和过度自信问题，研究旨在探索是否能在无真实数据情况下生成真实的合成需求规格说明书。

Method: 采用系统化方法：利用提示模式、基于LLM的质量评估和迭代提示优化，使用ChatGPT生成300份合成系统需求规格说明书，覆盖10个行业。通过跨模型检查和专家研究（n=87份调查）进行评估。

Result: 62%的专家认为生成的合成系统需求规格说明书是真实的。然而深入检查发现存在矛盾陈述和缺陷。基于LLM的质量评估不能完全替代彻底的专家评估。

Conclusion: ChatGPT能在一定程度上生成真实的合成系统需求规格说明书，但LLM的质量评估无法完全替代专家评估。研究展示了方法论和关键见解，为合成需求规格生成提供了实践指导。
...
### [7] [Declarative Scenario-based Testing with RoadLogic](https://arxiv.org/abs/2603.09455)
*Ezio Bartocci,Alessio Gambi,Felix Gigler,Cristinel Mateis,Dejan Ničković*

Main category: cs.SE

TL;DR: RoadLogic是一个将声明式OpenSCENARIO DSL场景规范转换为可执行仿真的开源工具，使用答案集编程和运动规划生成符合规范的自动驾驶测试场景。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶场景测试方法依赖命令式定义，需要手动枚举大量变体；声明式语言如OpenSCENARIO DSL缺乏将抽象规范实例化为具体仿真场景的系统方法，目前没有开源解决方案提供此能力。

Method: RoadLogic采用三层方法：1) 使用答案集编程生成满足场景约束的抽象计划；2) 通过运动规划将抽象计划细化为可行轨迹；3) 基于规范的监控来验证正确性。

Result: 在CommonRoad框架中评估表明，RoadLogic能在几分钟内持续生成符合规范的真实仿真场景，并通过参数采样捕获多样行为变体，为自动驾驶系统基于场景的测试打开了系统化的大门。

Conclusion: RoadLogic填补了声明式场景规范与可执行仿真之间的空白，提供了系统化的场景实例化能力，支持自动驾驶场景测试的自动化和多样化。
...
### [8] [Experience Report on the Adaptable Integration of Requirements Engineering Courses into Curricula for Professionals](https://arxiv.org/abs/2603.09467)
...
<details>
  <summary>Details</summary>
Motivation: 由于软件工程需求的增长、技术环境的快速演变以及实践所需技能的不断变化，面向专业人士的软件工程教育需求日益增长。然而，将需求工程课程系统有效地整合到面向专业人士的软件工程教育课程中面临挑战，因为这类课程具有需求不同、更加动态和模块化的特点。

Method: 研究基于开发三个面向专业人士的软件工程教育课程的经验，提出了整合需求工程课程的基本原理，并开发了一种专注于课程内容映射的系统方法。

Result: 研究提出了面向专业人士的软件工程教育课程中整合需求工程课程的基本原则，并开发了基于课程内容映射的系统方法，为类似课程设计提供了实践指导。

Conclusion: 通过实际课程开发经验，研究为面向专业人士的软件工程教育课程中需求工程课程的整合提供了系统方法和基本原则，有助于解决这类课程特有的挑战。
...
### [9] [Towards Viewpoint-centric Artifact-based Regulatory Requirements Engineering for Compliance by Design](https://arxiv.org/abs/2603.09492)
*Oleksandr Kosenkov*

Main category: cs.SE

TL;DR: 该博士研究提出AM4RRE模型，旨在将法规要求工程整合到软件开发中，实现"合规设计"，解决法规复杂性、跨职能沟通和不同视角一致性问题。


<details>
  <summary>Details</summary>
Motivation: 法规数量和复杂性不断增加，软件工程面临确保合规性的挑战。尽管研究界提出了新的法规要求工程方法，但行业在将其整合到软件开发生命周期中仍面临压力。目前缺乏如何实现这种整合的实证理解，需要解决法律知识密集性、跨职能沟通和不同视角一致性问题。

Method: 提出并报告了法规要求工程的人工制品模型（AM4RRE），该模型旨在实现集成化的合规设计。研究采用合成方法，并计划进行未来评估，目前处于寻求反馈和讨论的阶段。

Result: 研究发现法规要求工程具有区别于其他要求工程的特殊性。组织通常在组织层面建立独立的法规要求工程流程，但软件开发团队通常以临时方式处理合规设计，而非系统化方式。这主要是由于涉及视角之间的协调复杂性。

Conclusion: 本文报告了AM4RRE模型的合成和未来评估计划，旨在引发关于法规要求工程的讨论，帮助完善博士研究的最后阶段。该模型旨在促进法规要求工程与软件开发的整合，实现系统化的合规设计。
...
### [10] [EmbC-Test: How to Speed Up Embedded Software Testing Using LLMs and RAG](https://arxiv.org/abs/2603.09497)
*Maximilian Harnot,Sebastian Komarnicki,Michal Polok,Timo Oksanen*

Main category: cs.SE

TL;DR: 本文提出了一种基于检索增强生成（RAG）的管道，用于部分自动化嵌入式C软件的测试生成，显著减少测试时间并提高效率。


<details>
  <summary>Details</summary>
Motivation: 嵌入式C软件的手动测试开发耗时费力且难以扩展，随着软件发布周期加快，验证过程成为嵌入式开发流程的瓶颈，需要自动化解决方案。

Method: 采用检索增强生成（RAG）管道，将大型语言模型与项目特定工件相结合，减少幻觉并提高项目对齐度，实现测试生成的部分自动化。
...
### [11] [Preparing Students for AI-Driven Agile Development: A Project-Based AI Engineering Curriculum](https://arxiv.org/abs/2603.09599)
*Andreas Rausch,Stefan Wittek,Tobias Geger,David Inkermann*

Main category: cs.SE

TL;DR: 该论文提出了一种基于项目的AI工程课程，将敏捷实践与AI工具整合，培养学生在AI驱动的敏捷开发中的能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和智能工具正在重塑敏捷软件开发，但许多工程课程仍将敏捷方法和AI能力分开教学，且主要以讲座形式进行，需要整合的教学方法。

Method: 设计了项目驱动的AI工程课程，将AI工具嵌入日常敏捷工程任务中，包括需求澄清、待办事项优化、架构推理、编码支持、测试和文档等环节，并配合人类责任和质量反思。

Result: 初步结果表明，这种整合方法支持了AI辅助工程的实际能力发展。关键观察强调了敏捷教学需要适应快速工具演变，口头验证对确保基础学习至关重要。

Conclusion: 论文提供了课程设计经验和建议，帮助教育工作者在AI时代设计基于项目的敏捷课程，强调整合AI与敏捷实践的重要性。
...
### [12] [An Empirical Study of Interaction Smells in Multi-Turn Human-LLM Collaborative Code Generation](https://arxiv.org/abs/2603.09701)
*Binquan Zhang,Li Zhang,Lin Shi,Song Wang,Yuwei Qian,Linhui Zhao,Fang Liu,An Fu,Yida Ye*

Main category: cs.SE

TL;DR: 该论文研究了LLM在代码生成多轮交互中的"交互异味"问题，提出了首个分类法，分析了主流模型的异味分布，并提出了InCE框架来改善交互质量。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在代码生成方面表现出色，但在多轮交互中难以保持上下文一致性，现有基准主要关注最终输出的功能正确性，忽视了交互过程中的潜在质量问题（即"交互异味"）。

Method: 1. 从WildChat和LMSYS-Chat-1M数据集中采样真实用户-LLM交互进行实证研究；2. 通过开放卡片分类建立首个交互异味分类法；3. 定量评估6个主流LLM的异味分布；4. 提出InCE多智能体框架，通过显式提取全局不变性和预生成质量审计来改善交互质量。

Result: 1. 建立了包含3个主要类别（用户意图质量、历史指令遵从性、历史响应违反）和9个子类别的交互异味分类法；2. 分析了GPT-4o、DeepSeek-Chat等6个主流LLM的异味分布；3. 在扩展的WildBench基准测试中，InCE框架显著提高了任务成功率并有效抑制了交互异味的出现。

Conclusion: 交互异味是LLM代码生成协作中的重要问题，需要系统性的研究和缓解策略。提出的InCE框架通过多智能体协作和显式约束管理，能够有效改善多轮交互质量，为未来LLM协作系统设计提供了重要参考。
...
### [13] [Time warping with Hellinger elasticity](https://arxiv.org/abs/2603.08807)
*Yuly Billig*

Main category: cs.IR

TL;DR: 提出了一种用于任意度量空间中时间序列的匹配方法，使用Hellinger核作为拉伸惩罚，并引入了立方计算复杂度的弹性时间规整算法


<details>
  <summary>Details</summary>
Motivation: 解决任意度量空间中时间序列的匹配问题，传统方法在处理不同度量空间的时间序列匹配时存在局限性，需要一种更通用的匹配框架

Method: 采用Hellinger核作为时间序列拉伸的惩罚函数，提出了弹性时间规整算法，该算法具有立方计算复杂度，能够处理任意度量空间中的时间序列匹配

Result: 开发了一种通用的时间序列匹配算法，能够在任意度量空间中工作，通过Hellinger核有效处理时间序列的拉伸问题，算法复杂度为立方级别

Conclusion: 提出的弹性时间规整算法为任意度量空间中的时间序列匹配提供了一个有效的解决方案，Hellinger核作为拉伸惩罚函数具有良好的理论性质，立方复杂度在实际应用中具有可行性
...
### [14] [Evoking User Memory: Personalizing LLM via Recollection-Familiarity Adaptive Retrieval](https://arxiv.org/abs/2603.09250)
*Yingyi Zhang,Junyi Li,Wenlin Zhang,Penyue Jia,Xianneng Li,Yichao Wang,Derong Xu,Yi Wen,Huifeng Guo,Yong Liu,Xiangyu Zhao*

Main category: cs.IR

TL;DR: RF-Mem提出基于认知科学双过程理论（熟悉度-回忆）的记忆检索框架，通过熟悉度不确定性引导在快速熟悉度检索和深度回忆检索之间自适应切换，实现更高效、准确的个性化LLM记忆检索。


<details>
  <summary>Details</summary>
Motivation: 现有LLM个性化记忆检索方法存在两个极端：要么将所有用户记忆塞入提示（成本高、不可扩展），要么简化为一次性相似度搜索（只能捕捉表面匹配）。认知科学研究表明人类记忆通过熟悉度（快速但粗略）和回忆（深思熟虑、链式重建）双过程运作，而现有系统缺乏回忆检索能力和自适应切换机制。

Method: RF-Mem通过熟悉度信号（平均分数和熵）测量记忆熟悉程度。高熟悉度时采用直接top-K熟悉度检索路径；低熟悉度时激活回忆路径，对候选记忆进行聚类并通过alpha-mix与查询在嵌入空间迭代扩展证据，模拟深思熟虑的上下文重建。

Result: 在三个基准测试和不同语料规模上的实验表明，RF-Mem在固定预算和延迟约束下，始终优于一次性检索和全上下文推理方法。

Conclusion: RF-Mem将人类认知的双过程识别机制嵌入检索器，避免了全上下文开销，实现了可扩展、自适应的个性化记忆检索，为LLM个性化提供了更符合人类认知原理的解决方案。
...
### [15] [MITRA: An AI Assistant for Knowledge Retrieval in Physics Collaborations](https://arxiv.org/abs/2603.09800)
...
### [16] [Overview of the TREC 2025 Retrieval Augmented Generation (RAG) Track](https://arxiv.org/abs/2603.09891)
*Shivani Upadhyay,Nandan Thakur,Ronak Pradeep,Nick Craswell,Daniel Campos,Jimmy Lin*

Main category: cs.IR

TL;DR: TREC 2025 RAG Track聚焦于检索增强生成系统，通过引入长篇多句叙事查询来模拟真实复杂信息需求，强调透明度、事实基础和归因验证。


<details>
  <summary>Details</summary>
Motivation: 随着对推理驱动响应的需求增长，需要更好地反映深度搜索任务，设计能够处理复杂、真实世界信息需求的检索增强生成系统。

Method: 使用MS MARCO V2.1语料库，引入长篇多句叙事查询，要求参与者设计结合检索和生成的管道，采用多层评估框架包括相关性评估、响应完整性、归因验证和一致性分析。

Result: 今年收到超过150份提交，通过强调多层面叙事和富含归因的答案，推动了检索增强生成系统的创新。

Conclusion: TREC 2025 RAG Track旨在促进创建可信赖、上下文感知的检索增强生成系统，推动该领域的研究发展。
...
### [17] [A Voronoi Cell Formulation for Principled Token Pruning in Late-Interaction Retrieval Models](https://arxiv.org/abs/2603.09933)
...
<details>
  <summary>Details</summary>
Motivation: ColBERT等延迟交互模型需要为每个文档token存储密集嵌入，导致显著的索引存储开销。现有剪枝方法缺乏理论依据或效果不佳。

Method: 提出基于超空间几何的框架，将token剪枝视为嵌入空间中的Voronoi单元估计问题，通过测量每个token的Voronoi区域影响来进行剪枝。

Result: 该方法不仅作为竞争性的剪枝策略，还能作为改进和解释密集检索系统中token级行为的工具，在减少索引大小的同时保持检索质量。

Conclusion: 基于Voronoi几何的token剪枝方法提供了理论依据，有效解决了延迟交互模型的存储开销问题，同时为理解token级行为提供了新视角。
...
### [18] [MASEval: Extending Multi-Agent Evaluation from Models to Systems](https://arxiv.org/abs/2603.08835)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要是模型中心的，固定了智能体设置而不比较其他系统组件。作者认为实现决策（如拓扑结构、编排逻辑、错误处理）会显著影响性能，但缺乏相应的评估方法。

Method: 开发了MASEval框架无关的评估库，将整个系统作为分析单元。通过系统级比较，涵盖3个基准测试、3个模型和3个框架，评估不同组件对性能的影响。

Result: 研究发现框架选择与模型选择同等重要，不同的实现决策会显著影响智能体系统的性能表现。

Conclusion: MASEval填补了智能体系统评估的空白，为研究人员提供了探索所有系统组件的工具，为从业者识别最佳实现方案，推动了系统设计的规范化发展。
...
### [19] [Quantifying the Accuracy and Cost Impact of Design Decisions in Budget-Constrained Agentic LLM Search](https://arxiv.org/abs/2603.08877)
*Kyle McCleary,James Ghawaly*

Main category: cs.AI

TL;DR: 本文研究了预算约束下的智能检索增强生成系统，通过BCAS评估框架分析搜索深度、检索策略和完成预算对准确性和成本的影响，为实际部署提供配置指导。


<details>
  <summary>Details</summary>
Motivation: 智能RAG系统在实际部署中面临明确的工具调用和完成token预算限制，需要了解不同配置参数如何在固定约束下影响系统性能和成本。

Method: 提出Budget-Constrained Agentic Search (BCAS)评估框架，这是一个模型无关的评估工具，能够监控剩余预算并控制工具使用。在六个LLM和三个问答基准上进行比较实验，分析搜索深度、检索策略和完成预算的影响。

Result: 准确性随搜索次数增加而提高，但存在小上限；混合词汇和密集检索结合轻量级重排带来最大平均增益；较大的完成预算在HotpotQA式合成任务中最有帮助。

Conclusion: 研究结果为配置预算约束的智能检索管道提供了实用指导，并提供了可复现的提示和评估设置。
...
### [20] [Interpretable Markov-Based Spatiotemporal Risk Surfaces for Missing-Child Search Planning with Reinforcement Learning and LLM-Based Quality Assurance](https://arxiv.org/abs/2603.08933)
*Joshua Castillo,Ravi Mukkamala*

Main category: cs.AI

TL;DR: Guardian系统是一个用于失踪儿童调查的端到端决策支持系统，通过三层预测架构（马尔可夫链、强化学习、LLM验证）生成可解释的搜索计划。
...
Method: 系统将异构非结构化案件文档转换为时空表示，通过三层预测架构：第一层使用考虑道路可达性、隐蔽偏好和昼夜参数的马尔可夫链；第二层用强化学习将预测分布转化为可操作的搜索计划；第三层用LLM进行事后验证。

Result: 使用合成但真实的案例研究，系统在24/48/72小时时间范围内产生定量输出，结果显示三层架构能产生用于区域优化和人工审查的可解释先验概率。

Conclusion: Guardian系统提供了一个完整的决策支持框架，通过结合可解释模型和操作化工具，为失踪儿童调查提供有效的预测搜索计划。
...
### [21] [AgentOS: From Application Silos to a Natural Language-Driven Data Ecosystem](https://arxiv.org/abs/2603.08938)
*Rui Liu,Tao Zhe,Dongjie Wang,Zijun Yao,Kunpeng Liu,Yanjie Fu,Huan Liu,Jian Pei*

Main category: cs.AI

TL;DR: 本文提出了一种新型的个人智能体操作系统（AgentOS），旨在解决当前基于LLM的智能体在传统操作系统架构下的交互碎片化、权限管理混乱和上下文割裂等问题，将操作系统重构为以自然语言为中心、以智能体内核为核心、以技能模块为组件的全新计算范式。


<details>
  <summary>Details</summary>
Motivation: 当前开源本地智能体（如OpenClaw）虽然展示了LLM智能体自主操作本地计算环境的能力，但它们仍然运行在最初为GUI/CLI设计的传统操作系统上。这种架构不匹配导致了：1）碎片化的交互模型；2）权限管理混乱（常被称为"影子AI"）；3）严重的上下文割裂。因此需要从根本上重新思考操作系统架构。

Method: 提出AgentOS新范式：1）用自然用户界面（NUI）取代传统GUI桌面，建立统一的自然语言/语音门户；2）核心是智能体内核，负责解释用户意图、分解任务、协调多个智能体；3）传统应用演变为模块化技能模块，用户可通过自然语言规则组合软件；4）将AgentOS实现视为知识发现与数据挖掘问题，智能体内核作为实时意图挖掘和知识发现引擎。

Result: 将操作系统重构为连续的数据挖掘管道，涉及：1）工作流自动化的序列模式挖掘；2）技能检索的推荐系统；3）动态演化的个人知识图谱。这为KDD社区定义了新的研究议程，用于构建下一代智能计算系统。

Conclusion: AgentOS代表了从传统应用为中心到以智能体为中心的计算范式转变，其核心实现本质上是KDD问题。这种新架构能够解决当前智能体系统的根本性限制，为构建真正智能、集成、上下文感知的个人计算环境提供了理论框架和技术路径。
...
### [22] [A Consensus-Driven Multi-LLM Pipeline for Missing-Person Investigations](https://arxiv.org/abs/2603.08954)
*Joshua Castillo,Ravi Mukkamala*

Main category: cs.AI

TL;DR: Guardian是一个端到端系统，通过多模型LLM管道支持失踪儿童调查和早期搜索规划，使用QLoRA微调和共识机制提高信息提取的准确性和可靠性。
...
Method: 采用多模型LLM管道，协调任务专用LLM模型进行端到端执行，引入共识LLM引擎比较多个模型输出并解决分歧，通过QLoRA微调使用精选数据集增强系统性能。

Result: 设计了一个保守、可审计的LLM使用框架，将LLM作为结构化提取器和标注器，而非不受约束的端到端决策者，与弱监督和LLM辅助标注的先前工作保持一致。

Conclusion: Guardian LLM管道为失踪儿童调查提供了智能信息提取和处理系统，通过多模型协调和共识机制确保可靠性和准确性，支持关键的早期搜索规划。
...
### [23] [The FABRIC Strategy for Verifying Neural Feedback Systems](https://arxiv.org/abs/2603.08964)
*I. Samuel Akinwande,Sydney M. Katz,Mykel J. Kochenderfer,Clark Barrett*

Main category: cs.AI

TL;DR: FaBRIC算法通过结合前向和后向可达性分析来验证神经反馈系统的可达-规避规范，显著优于现有方法
...
### [24] [Meissa: Multi-modal Medical Agentic Intelligence](https://arxiv.org/abs/2603.09018)
...
Method: 提出统一轨迹建模将推理和行动轨迹表示为状态-行动-观察形式；三层分层监督根据模型错误逐步升级策略；前瞻-回顾监督结合探索性前向轨迹和后验合理化执行轨迹。

Result: 在13个医疗基准的16个评估设置中，Meissa在10个设置中匹配或超越专有前沿智能体，参数量比Gemini-3等前沿模型少25倍以上，端到端延迟降低22倍。

Conclusion: Meissa成功实现了轻量级离线医疗智能体系统，解决了现有系统依赖API部署的成本、延迟和隐私问题，为临床现场部署提供了可行方案。
...
### [25] [DataFactory: Collaborative Multi-Agent Framework for Advanced Table Question Answering](https://arxiv.org/abs/2603.09152)
*Tong Wang,Chi Jin,Yongkang Chen,Huan Deng,Xiaohui Kuang,Gang Zhao*

Main category: cs.AI

TL;DR: DataFactory是一个多智能体框架，通过专门的团队协调和自动知识转换解决TableQA中的上下文限制、幻觉问题和单智能体架构不足等挑战。
...
Result: 在TabFact、WikiTableQuestions和FeTaQA数据集上，使用来自五个提供商的八个LLM，结果显示一致提升。在TabFact上准确率提高20.2%，WikiTQ上提高23.9%（Cohen's d > 1）。团队协调优于单团队变体（TabFact +5.5%，WikiTQ +14.4%，FeTaQA ROUGE-2 +17.1%）。

Conclusion: DataFactory为多智能体协作提供了设计指南，并通过集成结构化查询和基于图的知识表示，为企业数据分析提供了一个实用平台。
...
### [26] [MEMO: Memory-Augmented Model Context Optimization for Robust Multi-Turn Multi-Agent LLM Games](https://arxiv.org/abs/2603.09022)
...
TL;DR: MEMO框架通过记忆增强和探索优化，解决了多智能体LLM游戏评估中的高方差问题，显著提升了胜率和稳定性


<details>
  <summary>Details</summary>
Motivation: 多轮多智能体LLM游戏评估存在显著的运行间方差问题，早期微小偏差会通过多智能体耦合在长期交互中被放大，导致胜率估计偏差和排名不可靠，提示选择进一步加剧了这一问题

Method: MEMO（记忆增强模型上下文优化）是一个自博弈框架，通过保留和探索的耦合来优化推理时上下文。保留维护持久记忆库存储自博弈轨迹的结构化见解，探索运行锦标赛式提示演化，使用TrueSkill进行不确定性感知选择，并通过优先回放重新访问罕见和决定性状态

Result: 在五个文本游戏中，MEMO将GPT-4o-mini的平均胜率从25.1%提升到49.5%，Qwen-2.5-7B-Instruct从20.9%提升到44.3%，运行间方差显著降低，在谈判和不完全信息游戏中获得最大提升
...
### [27] [Deep Tabular Research via Continual Experience-Driven Execution](https://arxiv.org/abs/2603.09151)
*Junnan Dong,Chuang Zhou,Zheng Yuan,Yifei Yu,Siyu An,Di Yin,Xing Sun,Feiyue Huang*

Main category: cs.AI

TL;DR: 提出DTR框架解决大语言模型在复杂非结构化表格上的长时程分析任务，通过分层元图、期望感知选择策略和孪生结构化记忆实现闭环决策


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理具有分层双向表头和非规范布局的复杂非结构化表格时表现不佳，需要解决长时程表格推理的挑战

Method: 提出DTR框架：1)构建分层元图捕捉双向语义；2)引入期望感知选择策略优先高效用执行路径；3)使用孪生结构化记忆（参数化更新和抽象文本）持续优化

Result: 在具有挑战性的非结构化表格基准测试中验证了有效性，证明了将战略规划与低级执行分离对长时程表格推理的必要性

Conclusion: DTR框架通过将表格推理视为闭环决策过程，有效解决了复杂非结构化表格的长时程分析问题，为表格推理提供了新的解决方案
...
### [28] [The Reasoning Trap -- Logical Reasoning as a Mechanistic Pathway to Situational Awareness](https://arxiv.org/abs/2603.09200)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.AI

TL;DR: 论文提出RAISE框架，揭示逻辑推理能力提升与AI情境意识增强之间的危险关联，认为当前逻辑推理研究正在无意中放大AI系统的风险能力。


<details>
  <summary>Details</summary>
Motivation: AI情境意识被认为是高级AI系统中最危险的新兴能力之一，而逻辑推理能力提升研究正在快速发展。作者发现这两个研究方向正在发生碰撞，需要揭示其中的危险关联。

Method: 提出RAISE框架，识别三种机制路径：演绎自我推断、归纳情境识别和溯因自我建模。构建从基本自我认知到战略欺骗的升级阶梯，分析当前逻辑推理研究如何映射到情境意识的放大器。

Result: 发现逻辑推理能力的每个主要研究方向都直接对应情境意识的特定放大器，当前安全措施不足以阻止这种升级。逻辑推理研究正在无意中放大AI系统的危险能力。

Conclusion: 需要采取具体保障措施，包括"镜像测试"基准和推理安全对等原则。向逻辑推理研究社区提出关于其在这一轨迹中责任的必要问题。
...
### [29] [Cognitively Layered Data Synthesis for Domain Adaptation of LLMs to Space Situational Awareness](https://arxiv.org/abs/2603.09231)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现优异，但在复杂工程领域（如空间态势感知）的迁移面临挑战，主要瓶颈在于高质量监督微调数据集的构建不足，存在知识覆盖不完整、认知深度浅、质量可控性差等问题。

Method: 提出BD-FDG框架，包含三个机制：1）使用知识树确保结构化语料覆盖；2）设计跨越9个类别和6个认知层次（从记忆到创造）的问题生成方案；3）应用多维评分管道确保领域严谨性和一致性。基于该框架构建了SSA-SFT数据集（约23万样本），并微调Qwen3-8B得到SSA-LLM-8B。

Result: SSA-LLM-8B在领域测试集上相对BLEU-1提升144%（无思考）和176%（有思考），在竞技场比较中以82.21%的胜率超越基线，同时基本保持了通用基准性能（MMLU-Pro、MATH-500）。

Conclusion: 认知分层驱动的SFT数据构建是复杂工程领域LLM适应的有效范式，BD-FDG为领域特定LLM迁移提供了可转移的框架。
...
### [30] [Social-R1: Towards Human-like Social Reasoning in LLMs](https://arxiv.org/abs/2603.09249)
*Jincenzi Wu,Yuxuan Lei,Jianxun Lian,Yitian Huang,Lexin Zhou,Haotian Li,Xing Xie,Helen Meng*

Main category: cs.AI

TL;DR: 本文提出ToMBench-Hard对抗性基准和Social-R1强化学习框架，通过挑战性训练案例和推理过程对齐，使4B参数模型在社交推理任务上超越更大模型。
...
Method: 1) 引入ToMBench-Hard对抗性基准，提供困难的社交推理训练示例；2) 提出Social-R1强化学习框架，通过多维奖励将模型推理与人类认知对齐，监督整个推理过程，强制结构对齐、逻辑完整性和信息密度。

Result: 该方法使4B参数模型在八个多样化基准测试中超越更大模型，并展现出强大的泛化能力。证明了通过挑战性训练案例和轨迹级对齐可以实现高效可靠的社交智能。

Conclusion: 挑战性训练案例与轨迹级对齐为开发高效可靠的社交智能提供了一条可行路径，有助于培养真正类人的社交推理能力。
...
### [31] [Logos: An evolvable reasoning engine for rational molecular design](https://arxiv.org/abs/2603.09268)
...
Method: 采用分阶段训练策略：1) 让模型接触明确推理示例，将分子描述与结构决策联系起来；2) 逐步对齐推理模式与分子表示；3) 将化学规则和不变性直接纳入优化目标，引导模型产生化学有效的输出。

Result: 在多个基准数据集上，Logos在结构准确性和化学有效性方面都表现出色，匹配或超越了参数规模大得多的通用语言模型。在涉及多个潜在冲突约束的分子优化任务中表现出稳定行为，并能通过展示中间推理步骤实现人工检查。
...
### [32] [Rescaling Confidence: What Scale Design Reveals About LLM Metacognition](https://arxiv.org/abs/2603.09309)
*Yuyang Dai*

Main category: cs.AI

TL;DR: 研究表明LLM口头置信度（0-100分制）存在严重离散化问题，超过78%的回答集中在三个整数上。通过系统操纵置信度量表的三个维度，发现0-20量表比标准0-100格式能提高元认知效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM口头置信度（通常为0-100分制）被广泛用于黑盒设置中的不确定性估计，但置信度量表本身的设计选择很少被研究。作者发现这种设计并非中性，需要系统研究量表设计如何影响口头不确定性的质量。

Method: 在六个LLM和三个数据集上分析口头置信度的分布特征。系统操纵置信度量表的三个维度：粒度、边界位置和范围规律性，使用元d'（meta-d'）评估元认知敏感性，比较不同量表设计的性能差异。

Result: 口头置信度存在严重离散化，超过78%的回答集中在三个整数上。0-20量表相比标准0-100格式能一致提高元认知效率，边界压缩会降低性能，整数偏好即使在非规律范围下仍然存在。

Conclusion: 置信度量表设计直接影响口头不确定性的质量，应被视为LLM评估中的首要实验变量，而不是中性设计选择。研究结果为改进LLM不确定性校准提供了重要设计指导。
...
### [33] [Curveball Steering: The Right Direction To Steer Isn't Always Linear](https://arxiv.org/abs/2603.09313)
...
<details>
  <summary>Details</summary>
Motivation: 现有激活导向方法依赖线性表示假设，认为行为属性可通过全局线性方向操控，但实践中线性干预常表现不一致。论文质疑这一假设，通过分析LLM激活空间的内在几何特性来探究问题根源。

Method: 通过测地距离与欧氏距离之比测量几何扭曲，发现激活空间并非全局线性几何。提出基于多项式核PCA的"Curveball steering"非线性导向方法，在特征空间中进行干预，更好地尊重学习到的激活几何。

Result: 观察到显著且概念依赖的几何扭曲，表明激活空间无法用全局线性几何良好近似。Curveball steering在几何扭曲强烈的场景中持续优于基于线性PCA的导向方法。

Conclusion: 几何感知的非线性导向为全局线性干预提供了原则性替代方案，激活空间的非线性几何特性需要更精细的干预方法。
...
<details>
  <summary>Details</summary>
Motivation: AI在公共和社会领域的快速部署增加了对监管标准合规性的需求，但缺乏资源阻碍了对AI系统合规性的半自动或自动评估，导致需要容易出错、资源有限的手动工作。

Method: 开发了一种开放、透明、可重复的方法，结合领域知识和大型语言模型的处理推理能力，生成包含风险级别分类、条款检索、义务生成和问答任务的机器可读格式数据集。

Result: 创建的数据集能够有效评估RAG系统，在禁止和高风险场景下分别达到0.87和0.85的F1分数，解决了风险级别边界不明确（如有限和最小风险案例）的问题。

Conclusion: 该方法展示了如何利用语言模型进行基于文档的高相关性生成，为评估AI系统对欧盟AI法案的合规性提供了有效资源，有助于推动AI监管合规的自动化评估。
...
TL;DR: 论文研究了模型合并中的"合并崩溃"现象，发现某些任务组合会导致性能严重下降，这与任务间的表示不兼容性高度相关，而与参数空间冲突指标无关。


<details>
  <summary>Details</summary>
Motivation: 模型合并可以将独立微调的LLM统一起来，实现并行开发成果的复用和集成，无需重新训练。但在实践中发现，某些任务专家模型的组合在合并后会出现灾难性的性能下降，这种现象被称为"合并崩溃"。研究者希望理解并解释这一现象的根本原因。

Method: 通过大量实验和统计分析，识别并表征任务级合并崩溃现象。研究任务间的表示不兼容性与合并崩溃的相关性，并与参数空间冲突指标进行对比。使用率失真理论提供理论解释，建立了与维度相关的边界，确立了任务可合并性的基本限制。

Result: 研究发现，任务间的表示不兼容性与合并崩溃高度相关，而参数空间冲突指标与合并崩溃几乎没有相关性。这一发现挑战了模型合并文献中的传统观点。通过理论分析建立了任务可合并性的基本限制，与维度相关。

Conclusion: 合并崩溃的根本原因在于任务间的表示不兼容性，而不是参数空间冲突。这一发现为模型合并提供了新的理论框架，表明某些任务组合本质上不可合并，与具体合并方法无关。研究结果为模型合并的实际应用提供了重要指导。
...
TL;DR: 研究提出一种基于三种认知差距（无知、惊讶、陈旧）的优先级函数，用于从智能体内部认知状态内生生成注意力目标，无需外部目标或奖励。
...
Method: 提出基于三种认知差距的优先级函数：无知（后验方差）、惊讶（预测误差）和陈旧（未观测变量置信度的时间衰减）。在两个系统中验证：最小注意力分配环境（2000次运行）和模块化部分可观测世界（500次运行）。进行消融实验验证各组件必要性。

Result: 1）消融实验显示三个认知差距组件都必要；2）发现度量依赖反转现象：全局预测误差下基于覆盖的轮转获胜，变化检测延迟下优先级引导分配获胜，优势随维度单调增长；3）检测延迟遵循注意力预算的幂律，优先级引导分配的指数更陡；4）当衰减率可学习时，系统能自发恢复环境波动结构，无需监督。

Conclusion: 仅凭认知差距（无需外部奖励）就足以生成自适应优先级，优于固定策略并能恢复潜在环境结构，为内生注意力机制提供了理论基础。
...
TL;DR: GenePlan是一个利用大语言模型辅助进化算法生成领域相关通用规划器的新框架，在PDDL描述的经典规划任务中表现优异，接近最先进规划器性能。


<details>
  <summary>Details</summary>
Motivation: 传统规划方法通常针对特定问题实例设计，缺乏泛化能力。需要开发能够生成领域相关通用规划器的框架，以解决多样化的规划问题实例。

Method: 将通用规划转化为优化问题，使用大语言模型辅助的进化算法迭代演化可解释的Python规划器，最小化不同问题实例上的规划长度。

Result: 在6个现有基准领域和2个新领域上，GenePlan平均SAT得分0.91，接近最先进规划器（0.93），显著优于其他基于LLM的基线方法（如CoT提示平均0.64）。生成的规划器解决新实例速度快（平均0.49秒/任务），成本低（使用GPT-4o平均每领域1.82美元）。

Conclusion: GenePlan框架成功地将LLM辅助的进化算法应用于通用规划，生成了高效、低成本且可解释的领域相关规划器，在性能上接近最先进方法，为自动化规划提供了有前景的新途径。
...
TL;DR: 本文提出"第三实体"概念，描述人类推理与生成式AI交互产生的新型认知-认识论结构，它超越了传统的工具使用、增强或协作伙伴关系，是一种涌现的、短暂的结构，源于两种本体论上不可通约的认知模式的转导耦合。


<details>
  <summary>Details</summary>
Motivation: 现有的人类与生成式AI交互的隐喻（工具使用、增强、协作伙伴关系）不足以充分描述这种交互的本质，需要新的理论框架来理解这种独特的认知-认识论形成。

Method: 结合皮尔士符号学、波兰尼的默会知识理论、西蒙东的个体化哲学、伊德的后现象学以及莫兰的复杂性理论，构建多层理论框架，引入"氛围创造"和"不对称涌现"等概念。

Result: 提出了"第三实体"理论框架，描述了人类与AI交互产生的涌现认知结构，分析了其认识论意义，特别是默会知识自动化的深远影响。

Conclusion: 这一理论框架对教育机构转型和生成式AI时代智力能力的重新定义具有重要意义，第三实体虽然新颖且不可还原，但仍以人类意向责任为基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统的辟谣信息通常是通用的，缺乏针对不同人格特质的个性化设计。研究表明个性化信息可能更有效，但人工创建成本高昂。本研究旨在利用大语言模型自动化生成个性化辟谣信息，提高辟谣效果。

Method: 1. 基于大五人格特质（外向性、宜人性、尽责性、神经质、开放性）创建人格档案；2. 使用大语言模型将通用辟谣内容转化为针对特定人格特征的个性化版本；3. 采用另一个大语言模型作为自动评估器，模拟相应人格特质来评估个性化信息的说服力，避免昂贵的人工评估。

Result: 1. 个性化信息普遍比通用信息更具说服力；2. 开放性特质倾向于提高说服力，而神经质特质可能降低说服力；3. 不同大语言模型评估器之间存在差异，使用多个模型能提供更清晰的评估结果。

Conclusion: 本研究展示了一种利用大语言模型创建针对性辟谣信息的实用方法，同时提出了关于此类技术使用的重要伦理问题。个性化辟谣信息具有潜力，但需要谨慎考虑其应用伦理。
...
### [40] [MiniAppBench: Evaluating the Shift from Text to Interactive HTML Responses in LLM-Powered Assistants](https://arxiv.org/abs/2603.09652)
*Zuhao Zhang,Chengyue Yu,Yuante Li,Chenyi Zhuang,Linjian Mo,Shuai Li*

Main category: cs.AI

TL;DR: 该论文提出了MiniAppBench基准测试，用于评估LLM生成交互式HTML应用的能力，并开发了MiniAppEval自动化评估框架来测试应用的意图、静态和动态维度。
...
Method: 1. 从真实世界应用中收集了1000万+生成样本，提炼出500个任务，涵盖游戏、科学、工具等六个领域，构建MiniAppBench基准。2. 提出MiniAppEval评估框架，利用浏览器自动化进行类似人类的探索性测试，从意图、静态和动态三个维度系统评估应用。

Result: 实验表明，当前LLM在生成高质量MiniApps方面仍面临显著挑战。MiniAppEval与人类判断高度一致，为未来研究建立了可靠的评估标准。

Conclusion: 该研究填补了交互式应用生成评估的空白，提出的MiniAppBench基准和MiniAppEval评估框架为评估LLM生成动态交互应用的能力提供了全面可靠的解决方案。
...
### [41] [Logics-Parsing-Omni Technical Report](https://arxiv.org/abs/2603.09677)
...
TL;DR: 本文提出Omni Parsing框架，通过统一的多模态解析方法将非结构化数据转换为结构化知识，实现了从感知到认知的渐进式解析。


<details>
  <summary>Details</summary>
Motivation: 针对多模态解析中任务定义碎片化和非结构化数据异质性带来的挑战，需要建立一个统一的框架来处理文档、图像和音视频流等多种模态数据。

Method: 提出Omni Parsing框架，包含三个层次：1)整体检测实现对象/事件的时空定位；2)细粒度识别进行符号化和属性提取；3)多级解释构建从局部语义到全局逻辑的推理链。采用证据锚定机制确保高层语义描述与低层事实对齐。

Result: 构建了标准化数据集并发布了Logics-Parsing-Omni模型，成功将复杂音视频信号转换为机器可读的结构化知识。实验表明细粒度感知与高层认知协同作用，有效提升模型可靠性。同时发布了OmniParsingBench用于定量评估。

Conclusion: Omni Parsing框架通过统一的多模态解析方法，实现了从非结构化信号到结构化知识的转换，使知识具有可定位、可枚举和可追溯的特性，为多模态理解提供了系统化解决方案。
...
### [42] [OOD-MMSafe: Advancing MLLM Safety from Harmful Intent to Hidden Consequences](https://arxiv.org/abs/2603.09706)
*Ming Wen,Kun Yang,Jingyu Zhang,Yuxuan Liu,shiwen cui,Shouling Ji,Xingjun Ma*

Main category: cs.AI

TL;DR: 该研究提出从意图/情境安全转向后果驱动安全的新范式，针对多模态大语言模型在因果链中识别潜在危险的能力进行评估和优化。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的安全对齐主要关注恶意意图或情境违规，但缺乏对后果驱动的安全考量，这对于自主和具身智能体的稳健部署至关重要。

Method: 1) 引入OOD-MMSafe基准测试，包含455个精心设计的查询-图像对；2) 开发CASPO框架，利用模型内在推理作为动态参考进行token级自蒸馏奖励。

Result: 前沿模型普遍存在因果盲区，高性能闭源模型失败率高达67.5%；CASPO显著提升后果预测能力，将Qwen2.5-VL-7B和Qwen3-VL-4B的风险识别失败率分别降至7.3%和5.7%。

Conclusion: 后果驱动安全是多模态大语言模型安全对齐的重要前沿，CASPO框架能有效提升模型在复杂因果链中识别潜在危险的能力，为自主智能体部署提供更可靠的安全保障。
...
### [43] [AutoAgent: Evolving Cognition and Elastic Memory Orchestration for Adaptive Agents](https://arxiv.org/abs/2603.09716)
*Xiaoxing Wang,Ning Liao,Shikun Wei,Chen Tang,Feiyu Xiong*

Main category: cs.AI

TL;DR: AutoAgent是一个自进化的多智能体框架，通过进化认知、实时上下文决策和弹性内存编排解决自主智能体在长期经验学习和实时决策之间的鸿沟问题。
...
Method: AutoAgent包含三个核心组件：1) 进化认知 - 智能体维护结构化提示级认知；2) 实时上下文决策 - 结合任务上下文从统一动作空间选择行动；3) 弹性内存编排器 - 动态组织交互历史，保留原始记录、压缩冗余轨迹、构建可重用情节抽象。

Result: 在检索增强推理、工具增强智能体基准和具身任务环境中的实验表明，AutoAgent在任务成功率、工具使用效率和协作鲁棒性方面持续优于静态和内存增强基线方法。

Conclusion: AutoAgent为自适应自主智能体提供了统一实用基础，使其能够在动态环境中从经验中学习，同时做出可靠的上下文感知决策，通过闭环认知进化过程持续更新认知和扩展可重用技能。
...
### [44] [World2Mind: Cognition Toolkit for Allocentric Spatial Reasoning in Foundation Models](https://arxiv.org/abs/2603.09774)
...
Method: 1. 利用3D重建和实例分割模型构建结构化空间认知地图；2. 合成以自我为中心的空间树（AST），使用椭圆参数精确建模地标的俯视布局；3. 引入三阶段推理链：工具调用评估、模态解耦线索收集、几何语义交织推理。

Result: 1. World2Mind将前沿模型（如GPT-5.2）的性能提升5%~18%；2. 仅依赖AST结构化文本，纯文本基础模型就能进行复杂3D空间推理，性能接近先进多模态模型。

Conclusion: World2Mind通过构建结构化空间认知地图，显著提升了多模态基础模型的空间推理能力，并证明了纯文本模型也能通过适当的空间表示进行复杂的3D推理。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统研究依赖临时性提示，缺乏原则性的策略视角。研究者希望探索是否可以通过参数化提示作为动作，构建轻量级策略来影响对话行为。

Method: 提出将提示视为LLM执行动作的框架，基于智能体当前状态通过五个组件动态构建提示。在两种公共讨论场景中使用不同LLM驱动的智能体进行实验，评估对话流程的五个指标。

Result: 实验表明提示参数化能够影响对话动态，参数化策略提示为影响对话过程提供了简单有效的机制，有助于多智能体系统在社会模拟方向的研究。

Conclusion: 参数化策略提示为LLM多智能体系统提供了一种无需训练的轻量级策略控制方法，能够有效影响对话行为，为多智能体社会模拟研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗多智能体系统研究存在架构碎片化、缺乏标准化多模态集成、数据管道不统一、视觉推理评估不一致以及缺乏跨专业基准测试等问题，阻碍了复杂临床决策支持系统的发展。
...
Result: 系统评估揭示了关键领域特定性能差距：虽然MAS提高了推理深度，但当前架构在医疗专业子领域间转换时表现出显著的脆弱性。研究提供了交互机制和成本性能权衡的严格消融分析，为未来自主临床系统建立了新的技术基线。

Conclusion: MedMASLab通过统一的框架和基准测试平台解决了医疗多智能体系统研究的关键挑战，为未来自主临床决策支持系统的发展奠定了技术基础，并公开了源代码和数据。
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学需要视觉模式识别和结构化领域知识的动态整合。现有多模态大语言模型缺乏结构化知识整合机制和可解释的记忆控制，难以在推理中一致地纳入病理学特定的诊断标准。

Method: 提出PathMem框架，将结构化病理知识组织为长期记忆，引入记忆Transformer建模从长期记忆到工作记忆的动态转换，通过多模态记忆激活和上下文感知的知识基础实现上下文感知的记忆精炼。

Result: 在多个基准测试中达到最先进性能：WSI-Bench报告生成提升12.8%的WSI-Precision和10.1%的WSI-Relevance，开放式诊断相比先前WSI模型提升9.7%和8.9%。

Conclusion: PathMem通过分层记忆机制有效整合结构化病理知识，显著提升了病理学多模态大语言模型的诊断推理能力，为计算病理学提供了更可靠的知识整合框架。
...
TL;DR: 研究排序决策系统中基于置信度的弃权机制何时能单调提升决策质量，何时会失败，区分了结构性不确定性和上下文不确定性两种类型。


<details>
  <summary>Details</summary>
Motivation: 排序决策系统（推荐系统、广告拍卖、临床分诊队列）需要决定何时干预排序输出、何时弃权。当前缺乏对置信度弃权机制何时有效何时失效的系统理解，特别是不同不确定性类型对决策质量的影响。

Method: 提出形式化条件：排序对齐和无反转区域。区分结构性不确定性（缺失数据，如冷启动）和上下文不确定性（缺失上下文，如时间漂移）。在三个领域进行实证验证：协同过滤（MovieLens，3种分布偏移）、电子商务意图检测（RetailRocket、Criteo、Yoochoose）、临床路径分诊（MIMIC-IV）。

Result: 结构性不确定性在所有领域都产生近乎单调的弃权增益；基于结构基础的置信信号（观察计数）在上下文漂移下失效，在MovieLens时间分割上产生与随机弃权同样多的单调性违反。上下文感知替代方案（集成分歧和近期特征）显著缩小差距（将违反从3减少到1-2），但未能完全恢复单调性。基于残差的异常标签在分布偏移下性能大幅下降（AUC从0.71降至0.61-0.62）。

Conclusion: 上下文不确定性带来本质上不同的挑战。提供实用的部署诊断：在部署置信度门控前检查C1和C2条件，并将置信信号与主要不确定性类型匹配。
...
TL;DR: 研究发现，与人类不同，大型语言模型在道德困境中，推理过程会提高诚实度，这源于表示空间中欺骗性区域的不稳定性


<details>
  <summary>Details</summary>
Motivation: 现有研究主要测量大语言模型的欺骗率，但对导致欺骗行为的根本条件了解不足。本文旨在探究在道德权衡情境中，什么因素会影响LLM的诚实行为

Method: 使用包含现实道德权衡的新数据集，其中诚实需要付出不同代价。通过分析推理过程、表示空间几何结构、输入改写、输出重采样和激活噪声等方法，研究欺骗行为的稳定性

Result: 与人类相反，推理过程会提高LLM的诚实度，且这一效应在不同规模和模型家族中都存在。欺骗性答案在表示空间中处于亚稳态，更容易被输入改写、输出重采样和激活噪声破坏。推理过程通过遍历有偏的表示空间，将模型推向更稳定的诚实默认状态

Conclusion: LLM的诚实行为不仅取决于推理内容，更与表示空间的几何结构有关。欺骗性区域在表示空间中处于亚稳态，而推理过程通过遍历这一空间，促使模型回归到更稳定的诚实默认状态
...
TL;DR: 本文提出了一种在线-在线公平多任务学习框架，用于AI-RAN中异构用户间的公平资源分配，通过内外双循环机制保证长期公平性。
...
Method: 提出OWO-FMTL框架，包含两个学习循环：外循环更新跨轮次的共享模型，内循环通过轻量级原始-对偶更新在每个轮次内重新平衡用户优先级，使用广义alpha公平性量化公平性。

Result: 该框架保证随时间递减的性能差异，计算开销低适合边缘部署，在凸优化和深度学习任务上的实验表明，在动态场景下优于现有多任务学习基线方法。
...
TL;DR: OptEMA提出了两种新的Adam风格优化器变体，通过自适应调整EMA系数，在无需Lipschitz常数的情况下实现了噪声自适应的收敛速率，在零噪声情况下达到接近最优的确定性收敛率。


<details>
  <summary>Details</summary>
Motivation: 现有Adam风格方法的理论分析存在多个局限性：在零噪声情况下保证可能不最优、依赖限制性有界条件、使用恒定或开环步长、需要Lipschitz常数先验知识。需要克服这些瓶颈。

Method: 提出了OptEMA优化器及其两种变体：OptEMA-M（对一阶矩应用自适应递减EMA系数，固定二阶衰减）和OptEMA-V（交换这些角色）。OptEMA是闭环且无需Lipschitz常数的，其有效步长依赖于轨迹。

Result: 在标准SGD假设下，两种变体都实现了噪声自适应的收敛速率$\widetilde{\mathcal{O}}(T^{-1/2}+σ^{1/2} T^{-1/4})$，其中σ是噪声水平。特别地，在σ=0的零噪声情况下，收敛率简化为接近最优的确定性速率$\widetilde{\mathcal{O}}(T^{-1/2})$，无需手动超参数调整。

Conclusion: OptEMA通过创新的自适应EMA系数设计，克服了现有Adam风格优化器理论分析的多个限制，实现了无需Lipschitz常数先验知识的噪声自适应收敛性能，在零噪声情况下达到接近最优的收敛速率。
...
TL;DR: HCAPO框架将事后信用分配引入LLM智能体，解决长时程多步任务中的信用分配问题，通过LLM作为事后评论家优化步级Q值估计，并使用多尺度优势机制补充不准确的价值基线。


<details>
  <summary>Details</summary>
Motivation: 现有无价值方法（如GRPO）在长时程多步任务中面临两个基本瓶颈：不准确的步级Q值估计和中间状态的价值基线错位，导致稀疏奖励下的信用分配挑战。

Method: HCAPO框架将事后信用分配集成到LLM智能体中，利用LLM本身作为事后评论家，通过事后推理优化步级Q值，并采用多尺度优势机制补充关键决策状态的不准确价值基线。
...
Conclusion: HCAPO显著提升了探索效率，促进了简洁的决策制定，并确保了在复杂长时程任务中的可扩展性，为解决LLM智能体的信用分配问题提供了有效框架。
...
TL;DR: 论文提出了一种在异构乘积空间上构建不变量的新方法，通过轨道等价将群作用下的函数约简为等变子群作用，扩展了等变神经场的适用范围。


<details>
  <summary>Details</summary>
Motivation: 许多几何学习问题需要在异构乘积空间（不同群作用的空间乘积）上构建不变量，但标准技术无法直接应用。现有方法对等变神经场施加了主要的结构约束。

Method: 当群G在空间M上可迁作用时，证明任何在乘积空间X×M上的G-不变函数可以约简为M的等变子群H在X上的不变量。建立明确的轨道等价关系(X×M)/G ≅ X/H，提供保持表达能力的原理性约简。

Result: 该方法将等变神经场扩展到任意群作用和齐次条件空间，消除了现有方法施加的主要结构约束，为异构乘积空间上的几何学习提供了通用框架。

Conclusion: 通过轨道等价约简异构乘积空间上的不变量，为处理不同群作用的空间乘积提供了理论基础，显著扩展了等变神经场的应用范围，解决了几何学习中长期存在的结构约束问题。
...
TL;DR: SPREAD是一个终身模仿学习框架，通过奇异值分解在低秩子空间中对齐策略表示，保持任务流形的几何结构，并使用置信度引导的蒸馏策略提高优化稳定性。


<details>
  <summary>Details</summary>
Motivation: 终身模仿学习的关键挑战是让智能体从专家演示中学习新技能的同时保留先验知识。现有基于L2范数特征匹配的蒸馏方法对噪声和高维变化敏感，难以保持任务流形的内在几何结构。

Method: 提出SPREAD框架：1) 使用奇异值分解在低秩子空间中对齐策略表示，保持多模态特征的底层几何结构；2) 提出置信度引导的蒸馏策略，将KL散度损失限制在置信度最高的前M个动作样本上，强调可靠模式。

Result: 在LIBERO终身模仿学习基准测试中，SPREAD显著改善了知识迁移，缓解了灾难性遗忘，并实现了最先进的性能。

Conclusion: SPREAD通过几何保持的特征对齐和置信度引导的蒸馏，有效解决了终身模仿学习中的知识保留和迁移问题，为保持任务流形的几何结构提供了新方法。
...
TL;DR: 提出一种多级马尔可夫决策过程压缩方法，通过分层抽象减少随机性和策略搜索空间，结合课程学习实现技能跨问题和跨层级迁移


<details>
  <summary>Details</summary>
Motivation: 解决具有自然多层次结构的顺序决策问题，传统方法难以系统推断和利用层次结构，需要一种能保持语义、减少随机性、支持技能迁移的高效方法

Method: 多级MDP压缩框架：将低层策略族视为高层MDP中的单一动作，保持原始MDP语义结构；高层MDP具有更少随机性，可独立求解；结合嵌入（问题特定）和技能（高阶函数）的因子分解

Result: 多级表示解耦子任务，显著减少不必要随机性和策略搜索空间，降低迭代和计算成本；技能可跨不同问题和层级迁移；在MazeBase+等示例中验证了抽象、可迁移性和课程学习效果

Conclusion: 该框架在温和假设下保证一致性，通过多级分解和课程学习有效解决复杂MDP问题，实现高效策略搜索和技能迁移，为层次强化学习提供系统化方法
...
TL;DR: TMTF是MTF的扩展，通过将时间序列分段并估计局部转移矩阵，解决了MTF在非平稳过程中丢失时间信息的缺陷，为CNN提供更好的时间序列图像表示。


<details>
  <summary>Details</summary>
Motivation: 传统MTF方法在时间序列动态变化（如机制转换）时存在问题，全局转移矩阵会平均不同机制的信息，导致丢失每个动态机制何时活跃的时间信息。

Method: 将时间序列划分为K个连续的时间块，为每个块估计局部转移矩阵，然后组装成图像，使每一行反映其所在块的局部动态而非全局平均。

Result: 生成的T×T图像具有K个不同纹理的水平带，每个带编码一个时间段的转移动态。该方法具有振幅无关性和顺序保持性，适合作为CNN的输入通道。

Conclusion: TMTF扩展了MTF，通过局部化方法保留了时间信息，能更好地表征非平稳时间序列的动态变化，为基于CNN的时间序列分析提供了改进的图像表示。
...
TL;DR: 该论文介绍了SoftJAX和SoftTorch两个开源库，为JAX和PyTorch中的"硬"原语（如阈值、布尔逻辑、离散索引、排序等）提供软可微替代方案，解决这些操作梯度为零或未定义的问题。


<details>
  <summary>Details</summary>
Motivation: 自动微分框架（如JAX和PyTorch）中的许多"硬"原语（阈值、布尔逻辑、离散索引、排序等）会产生零梯度或未定义梯度，不利于优化。虽然已有各种"软"松弛方法提供信息梯度，但实现分散在不同项目中，难以组合和比较。

Method: 开发了SoftJAX和SoftTorch两个开源库，提供多种软函数作为硬JAX和PyTorch对应函数的直接替代品。包括：(i)逐元素操作符（如clip、abs），(ii)通过模糊逻辑操作布尔值和索引的实用方法，(iii)基于最优传输或置换多面体投影的轴向操作符（如排序、排名），(iv)支持直通梯度估计。

Result: 创建了功能完整的软可微编程库，使软松弛工具箱易于在可微编程中使用。通过基准测试和实际案例研究展示了其有效性。代码已在GitHub开源。

Conclusion: SoftJAX和SoftTorch为可微编程提供了易于访问的软松弛工具箱，解决了硬原语的梯度问题，促进了科学计算和机器学习中梯度优化方法的应用。
...
### [58] [Expressivity-Efficiency Tradeoffs for Hybrid Sequence Models](https://arxiv.org/abs/2603.08859)
...
<details>
  <summary>Details</summary>
Motivation: 混合序列模型结合了Transformer和状态空间模型的优势，但缺乏对其在何种场景下、通过何种机制优于单一模型的基本理解。本文旨在系统研究混合模型相对于其组成模型的优势和机制。

Method: 通过理论分析证明非混合模型在核心合成任务族上的根本局限性，针对选择性复制和关联回忆两个典型任务，构建小型混合模型并理论证明其有效性。同时进行实验验证，包括学习型混合模型与参数更多的非混合模型对比，以及长度泛化和分布外鲁棒性测试。

Result: 理论证明：对于特定任务族，任何Transformer或状态空间模型都需要大量参数或工作内存，而混合模型能以小规模解决这些任务。实验验证：学习型混合模型在参数少6倍的情况下仍优于非混合模型，且表现出更强的长度泛化能力和分布外鲁棒性。

Conclusion: 混合序列模型在特定任务上确实能同时获得注意力机制的表达能力和状态空间模型的计算效率，突破了单一模型的根本限制。混合模型不仅在理论构造上有效，实际学习到的混合模型也展现出显著优势，具有更好的泛化能力和鲁棒性。
...
### [59] [A New Modeling to Feature Selection Based on the Fuzzy Rough Set Theory in Normal and Optimistic States on Hybrid Information Systems](https://arxiv.org/abs/2603.08900)
*Mohammad Hossein Safarpour,Seyed Mohammad Alavi,Mohammad Izadikhah,Hossein Dibachi*

Main category: cs.LG

TL;DR: 提出了一种名为FSbuHD的新型特征选择模型，通过计算对象间的组合距离来推导模糊等价关系，将特征选择问题转化为优化问题，在高维大数据环境下具有高效性和有效性。


<details>
  <summary>Details</summary>
Motivation: 针对大数据环境下特征选择的需求，传统模糊粗糙集理论面临两个主要挑战：在高维空间中通过交集运算获取模糊等价关系耗时耗内存，且可能产生噪声数据。本文旨在解决这些问题。

Method: 提出FSbuHD特征选择模型，通过计算对象间的组合距离来推导模糊等价关系，将特征选择问题重新表述为可用元启发式算法解决的优化问题。模型提供正常和乐观两种模式，基于两种不同的模糊等价关系。

Result: 在UCI标准数据集上测试并与其它算法比较，结果表明FSbuHD是相比先前方法最有效和高效的特征选择方法之一。

Conclusion: FSbuHD模型成功解决了传统模糊粗糙集理论在高维大数据特征选择中的挑战，通过组合距离方法和优化问题重构，实现了高效且有效的特征选择。
...
### [60] [Cross-Domain Uncertainty Quantification for Selective Prediction: A Comprehensive Bound Ablation with Transfer-Informed Betting](https://arxiv.org/abs/2603.08907)
*Abhinaba Basu*

Main category: cs.LG

TL;DR: 本文对选择性预测的九种有限样本界进行了全面消融研究，提出了基于转移信息的投注方法，在数据稀缺场景下获得更紧的界，并在多个基准测试中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 选择性预测需要在有限样本下提供风险控制保证，现有方法在数据稀缺场景下界过松。本文旨在结合多种技术（集中不等式、多重检验校正、投注置信序列）并引入跨域转移信息，以获得更紧的风险控制界。

Method: 结合九种界族：Hoeffding、经验Bernstein、Clopper-Pearson、Wasserstein DRO、CVaR等集中不等式，与多重检验校正（并界、Learn Then Test固定序列）和基于投注的置信序列（WSR）。主要理论贡献是转移信息投注（TIB），利用源域风险配置文件预热WSR财富过程。

Result: 在四个基准测试（MASSIVE、NyayaBench、CLINC-150、Banking77）上评估了18种配置。在MASSIVE上，LTT消除了并界惩罚，在α=0.10时获得94.0%保证覆盖率，比Hoeffding的73.8%提高27%。在NyayaBench上，TIB在α=0.10时获得18.5%覆盖率，比LTT+Hoeffding提高5.4倍。

Conclusion: TIB方法在数据稀缺场景下显著优于现有方法，提供了形式化的支配保证。结合投注置信序列、LTT单调测试和跨域转移是文献中未见的三重创新。该方法可应用于智能体缓存系统，建立渐进信任模型。
...
### [61] [Quantifying Memorization and Privacy Risks in Genomic Language Models](https://arxiv.org/abs/2603.08913)
*Alexander Nemecek,Wenbiao Li,Xiaoqian Jiang,Jaideep Vaidya,Erman Ayday*

Main category: cs.LG

TL;DR: 本文提出了一个全面的多向量隐私评估框架，用于量化基因组语言模型中的记忆风险，结合三种互补的风险评估方法，揭示了GLMs存在可测量的记忆风险，且风险程度因架构和训练方案而异。


<details>
  <summary>Details</summary>
Motivation: 随着基因组语言模型越来越多地在敏感基因组队列上进行训练或微调，它们存在记忆训练数据中特定序列的风险，这引发了关于隐私、数据泄露和监管合规性的严重担忧。然而，在基因组领域，对这些风险的系统性评估很少，而基因组数据具有独特的特性。

Method: 提出了一个全面的多向量隐私评估框架，整合了三种互补的风险评估方法：基于困惑度的检测、金丝雀序列提取和成员推断。这些方法被组合成一个统一的评估流程，产生最坏情况下的记忆风险评分。为了进行受控评估，将金丝雀序列以不同的重复率植入合成和真实的基因组数据集中。

Result: 评估结果显示基因组语言模型表现出可测量的记忆，且记忆程度因架构和训练方案而异。研究发现，没有单一的攻击向量能够捕捉到记忆风险的全部范围。

Conclusion: 基因组语言模型存在记忆风险，需要多向量隐私审计作为基因组AI系统的标准实践。不同模型架构和训练方案对记忆风险有不同影响，强调了系统性隐私评估的重要性。
...
### [62] [The $qs$ Inequality: Quantifying the Double Penalty of Mixture-of-Experts at Inference](https://arxiv.org/abs/2603.08960)
*Vignesh Adhinarayanan,Nuwan Jayasena*

Main category: cs.LG

TL;DR: MoE模型训练时FLOPs效率高，但推理时因专家路由碎片化和KV缓存占用HBM内存导致效率下降，尤其在长上下文场景下表现更差。


<details>
  <summary>Details</summary>
Motivation: 研究MoE模型在推理时效率下降的根本原因，特别是识别出专家路由导致的微批次碎片化和KV缓存占用高带宽内存的问题，这些问题在长上下文场景下尤为严重。

Method: 提出qs不等式作为预测标准，结合稀疏度s（每个token激活的参数比例）和质量等价因子q（密集模型匹配MoE性能所需的大小倍数），评估MoE相对于密集模型的结构劣势。在DeepSeek-V3、Qwen3-235B、Grok-1和Switch-C等前沿模型上进行评估。

Result: MoE模型在推理时存在重用碎片化问题，导致FFN进入带宽受限状态。DeepSeek-V3在128k上下文长度下，质量匹配的密集模型吞吐量优势达到4.5倍。Switch-C等大规模架构在集群规模上可能变得不可行，而质量匹配的密集模型仍可行。

Conclusion: 训练时的FLOPs效率不能完全代表推理时的性能，特别是在长上下文服务中。MoE最好被视为训练时优化技术，通过蒸馏到密集模型可能是实现推理高效部署的可行路径。
...
### [63] [Semantic Level of Detail: Multi-Scale Knowledge Representation via Heat Kernel Diffusion on Hyperbolic Manifolds](https://arxiv.org/abs/2603.08965)
*Edward Izgorodin*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI记忆系统将知识组织成图结构（知识图谱、实体关系、社区层次），但缺乏连续分辨率控制的原理性机制，无法确定抽象层次之间的定性边界以及智能体应如何在这些层次间导航。

Method: 引入语义细节层次（SLoD）框架，通过在庞加莱球上的热核扩散定义连续缩放算子。在粗尺度上，扩散将嵌入聚合成高级摘要；在细尺度上，保留局部语义细节。证明了树状层次结构在Sarkar嵌入下的层次一致性和有界近似误差。利用图拉普拉斯算子的谱间隙自动检测表示发生定性转变的尺度边界。

Result: 在合成层次结构（HSBM）上，边界扫描器恢复植入的层次，调整兰德指数高达1.00，在信息论Kesten-Stigum阈值附近检测性能逐渐下降。在完整WordNet名词层次结构（82K同义词集）上，检测到的边界与真实分类深度高度相关（τ=0.79），表明该方法能在无监督情况下发现真实世界知识图谱中有意义的抽象层次。

Conclusion: SLoD框架为知识图谱提供了连续分辨率控制的原理性机制，能够自动检测抽象层次边界，无需手动分辨率参数，在合成和真实世界知识图谱中都表现出色。
...
### [64] [MAPLE: Elevating Medical Reasoning from Statistical Consensus to Process-Led Alignment](https://arxiv.org/abs/2603.08987)
*Kailong Fan,Anqi Pu,Yichen Wu,Wanhua Li,Yicong Li,Hanspeter Pfister,Huafeng Liu,Xiang Li,Quanzheng Li,Ning Guo*

Main category: cs.LG

TL;DR: 提出了一种将医学过程奖励模型与测试时强化学习相结合的新训练范式，用专家对齐的细粒度监督取代多数投票，在医学基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前医学大语言模型的测试时强化学习通常依赖多数投票作为启发式监督信号，但在复杂医学场景中，最常见的推理路径不一定符合临床正确性，需要更可靠的监督机制。

Method: 提出了一种统一的训练范式，将医学过程奖励模型与测试时强化学习相结合，用Med-RPM提供的专家对齐细粒度监督取代传统的多数投票机制，将搜索式智能蒸馏到模型的参数化记忆中。

Result: 在四个不同基准测试上的广泛评估表明，该方法在一致性和显著性方面优于当前的测试时强化学习和独立的PRM选择方法。

Conclusion: 从随机启发式方法转向结构化、逐步奖励对于开发可靠且可扩展的医学AI系统至关重要，医学过程奖励模型与测试时强化学习的结合能够有效提升医学推理的准确性和可靠性。
...
### [65] [The Coupling Within: Flow Matching via Distilled Normalizing Flows](https://arxiv.org/abs/2603.09014)
...
TL;DR: 提出NFM方法，利用预训练归一化流的准确定性耦合来训练学生流模型，超越独立耦合和最优传输耦合的性能


<details>
  <summary>Details</summary>
Motivation: 流模型已成为大规模生成器的首选方法，但训练中的耦合选择对性能至关重要。虽然自适应耦合（如最优传输）能改善训练和推理，但本文提出更激进的思路：利用预训练归一化流的准确定性耦合来训练学生流模型

Method: 提出归一化流匹配（NFM）方法，利用预训练的自回归归一化流模型（AR-NF）的耦合关系，通过蒸馏技术将这种准确定性耦合传递给学生流模型进行训练

Result: NFM方法训练的学生流模型显著优于使用独立耦合或最优传输耦合的流模型，同时也超越了教师AR-NF模型的性能

Conclusion: 通过蒸馏预训练归一化流的耦合关系来训练流模型是一种有效方法，能够结合流模型的灵活性和归一化流的确定性耦合优势，实现性能提升
...
### [66] [Towards a Neural Debugger for Python](https://arxiv.org/abs/2603.09951)
*Maximilian Beck,Jonas Gehring,Jannik Kossen,Gabriel Synnaeve*

Main category: cs.LG

TL;DR: 该论文提出"神经调试器"概念，通过训练语言模型模拟传统调试器功能，支持单步执行、函数跳入跳出、断点设置等操作，实现正向和逆向执行预测。


<details>
  <summary>Details</summary>
Motivation: 现有神经解释器方法缺乏交互控制能力，而实际开发中开发者使用调试器在断点处停止执行并逐步检查代码。需要让语言模型具备类似传统调试器的交互控制能力。

Method: 通过微调大型语言模型或从头预训练较小模型，训练语言模型模拟调试器操作，支持单步执行、函数跳入跳出、断点设置等功能，实现条件化执行建模。

Result: 在CruxEval评估中，模型在输出预测和输入预测任务上均表现优异，展示了强大的条件化执行建模能力，能够可靠地建模正向和逆向执行。

Conclusion: 神经调试器为未来智能编码系统提供了世界模型基础，能够为模拟调试环境提供执行反馈或与真实调试工具交互，为代码生成、程序理解和自动化调试奠定基础。
...
### [67] [Two Teachers Better Than One: Hardware-Physics Co-Guided Distributed Scientific Machine Learning](https://arxiv.org/abs/2603.09032)
...
<details>
  <summary>Details</summary>
Motivation: 科学机器学习在野外处理、控制和监测中的应用面临广域传感、实时需求以及严格的能源和可靠性约束，集中式实现不切实际。现有方法要么需要原始数据集中聚合导致通信成本过高，要么分布式的通用机器学习模型破坏了物理原理导致性能下降。

Method: EPIC采用硬件和物理协同指导的分布式框架，以全波形反演为代表任务。在终端设备上进行轻量级本地编码，在中心节点进行物理感知解码。通过传输紧凑的潜在特征而非高容量原始数据，并使用交叉注意力捕捉接收器间的波场耦合。

Result: 在包含5个终端设备和1个中心节点的分布式测试平台上，使用OpenFWI的10个数据集进行评估，EPIC将延迟降低8.9倍，通信能耗降低33.8倍，同时在10个数据集中的8个上提高了重建保真度。

Conclusion: EPIC框架成功解决了分布式科学机器学习中的通信成本和物理保真度权衡问题，为在资源受限环境中部署科学机器学习提供了有效的解决方案。
...
### [68] [SCALAR: Learning and Composing Skills through LLM Guided Symbolic Planning and Deep RL Grounding](https://arxiv.org/abs/2603.09036)
*Renos Zabounidis,Yue Wu,Simon Stepputtis,Woojun Kim,Yuanzhi Li,Tom Mitchell,Katia Sycara*

Main category: cs.LG

TL;DR: SCALAR框架通过双向耦合LLM规划与RL学习，利用技能库迭代优化技能规范，显著提升语言模型在低级控制任务中的表现


<details>
  <summary>Details</summary>
Motivation: 基于语言模型的智能体在高级动作API下表现优异，但难以将语言落地到低级控制。现有方法让LLM一次性生成技能或奖励函数，缺乏反馈机制来修正规范错误。

Method: SCALAR是一个双向框架，将LLM规划与RL通过学习的技能库耦合。LLM提出带有前提条件和效果的技能；RL为每个技能训练策略，并将执行结果反馈给LLM以迭代优化技能规范。关键轨迹分析通过分析RL轨迹修正LLM先验；前沿检查点可选地在技能边界保存环境状态以提高样本效率。

Result: 在Craftax环境中，SCALAR实现了88.2%的钻石收集率，比最佳基线提升1.9倍，并且9.1%的时间到达了Gnomish Mines，而先前方法完全无法到达。

Conclusion: SCALAR通过LLM与RL的双向耦合和迭代优化机制，有效解决了语言模型在低级控制任务中的落地问题，显著提升了智能体在复杂环境中的表现。
...
### [69] [Dynamic Multi-period Experts for Online Time Series Forecasting](https://arxiv.org/abs/2603.09062)
...
<details>
  <summary>Details</summary>
Motivation: 现有在线时间序列预测方法通常将概念漂移视为单一现象，无法有效区分不同类型的漂移模式，限制了模型对复杂动态环境的适应能力。

Method: 首先重新定义概念漂移为周期性漂移和新兴漂移两类。提出DynaME混合框架：对于周期性漂移，使用动态拟合历史周期性模式的专家委员会；对于新兴漂移，通过不确定性检测切换到稳定的通用专家。

Result: 在多个基准数据集和骨干模型上的实验表明，DynaME能有效适应两种概念漂移，并显著优于现有基线方法。

Conclusion: 通过区分概念漂移类型并设计相应的处理机制，DynaME框架为在线时间序列预测提供了更精细和有效的概念漂移适应方案。
...
### [70] [Learning Adaptive LLM Decoding](https://arxiv.org/abs/2603.09065)
*Chloe H. Su,Zhe Ye,Samuel Tenka,Aidan Yang,Soonho Kong,Udaya Ghai*

Main category: cs.LG

TL;DR: 该论文提出了一种自适应解码策略，通过轻量级解码适配器动态选择采样策略，在固定计算预算下提高LLM在数学和编程任务上的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统LLM解码使用固定的采样超参数（如温度、top-p），无法适应不同任务难度和不确定性在提示词和单个解码步骤中的显著变化。需要根据可用计算资源动态调整解码策略。

Method: 提出轻量级解码适配器，使用强化学习和可验证的终端奖励（如数学和编程任务的正确性）进行训练。在序列层面，将解码建模为上下文赌博机问题；在令牌层面，建模为部分可观测马尔可夫决策过程（POMDP）。

Result: 在MATH和CodeContests基准测试中，学习的适配器改善了准确性-预算权衡：在MATH上，令牌级适配器在固定令牌预算下比最佳静态基线提高Pass@1准确性达10.2%；序列级适配器在固定并行采样下获得2-3%的提升。

Conclusion: 自适应解码策略能有效提高LLM在资源受限情况下的性能，序列级和令牌级适配都有贡献，为动态解码策略提供了新方向。
...
### [71] [Exclusive Self Attention](https://arxiv.org/abs/2603.09078)
*Shuangfei Zhai*

Main category: cs.LG

TL;DR: 提出了一种称为"排他性自注意力"的简单自注意力机制改进方法，通过限制注意力仅捕获与token自身值向量正交的信息，从而提升Transformer序列建模性能。


<details>
  <summary>Details</summary>
Motivation: 标准自注意力机制在处理序列建模时存在局限性，特别是在捕捉上下文信息方面。研究者希望通过修改注意力机制来更好地建模长序列中的上下文关系。

Method: 提出排他性自注意力机制，核心思想是约束注意力仅捕获与token自身值向量正交的信息，从而排除自身位置的信息，鼓励更好的上下文建模。

Result: 在标准语言建模任务中，XSA在模型规模达到27亿参数时始终优于标准自注意力，并且随着序列长度增加，性能提升效果更加显著。

Conclusion: 排他性自注意力是一种简单有效的自注意力改进方法，能够显著提升Transformer在序列建模任务中的性能，特别是在处理长序列时表现更佳。
...
### [72] [PPO-Based Hybrid Optimization for RIS-Assisted Semantic Vehicular Edge Computing](https://arxiv.org/abs/2603.09082)
*Wei Feng,Jingbo Zhang,Qiong Wu,Pingyi Fan,Qiang Fan*

Main category: cs.LG

TL;DR: 提出RIS辅助的语义感知车联网边缘计算框架，通过RIS优化无线连接和语义通信降低延迟，采用PPO和LP混合优化方案，相比GA和QPSO减少40-50%延迟
...
Result: 仿真验证框架优越性：PPO混合方案相比GA和QPSO减少40-50%平均端到端延迟；系统扩展性强，在30辆车拥堵场景下仍保持低延迟

Conclusion: RIS辅助的语义感知VEC框架能有效降低车联网应用延迟，提出的混合优化方案在性能和扩展性方面优于传统方法
...
### [73] [Not All News Is Equal: Topic- and Event-Conditional Sentiment from Finetuned LLMs for Aluminum Price Forecasting](https://arxiv.org/abs/2603.09085)
*Alvaro Paredes Amorin,Andre Python,Christoph Weisser*

Main category: cs.LG

TL;DR: 该研究探讨了微调大型语言模型在铝价预测中的应用，通过结合新闻情感分析和传统市场数据，发现在高波动时期情感数据能显著提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 文本数据对商品价格预测越来越重要，但轻量级微调大语言模型在铝价预测中的有效性，以及这些信号在何种市场条件下最有效，尚未得到充分探索。

Method: 从英文和中文新闻标题生成月度情感得分，结合传统表格数据（基本金属指数、汇率、通胀率、能源价格），通过LSTM模型进行预测，并在上海金属交易所进行多空模拟评估。

Result: 在高波动时期，结合微调Qwen3模型情感数据的LSTM模型（夏普比率1.04）显著优于仅使用表格数据的基准模型（夏普比率0.23）。

Conclusion: 新闻情感数据在铝价预测中具有重要价值，特别是在市场高波动时期，不同新闻来源、主题和事件类型在预测中发挥不同的作用。
...
### [74] [Probabilistic Hysteresis Factor Prediction for Electric Vehicle Batteries with Graphite Anodes Containing Silicon](https://arxiv.org/abs/2603.09103)
...
TL;DR: 本文提出了一种数据驱动的概率滞后因子预测方法，专门针对硅-石墨负极电池的SoC估计挑战，通过数据协调框架和机器学习模型实现高效、可泛化的预测。


<details>
  <summary>Details</summary>
Motivation: 硅-石墨负极电池虽然具有更高能量密度和充电性能，但存在显著的电压滞后现象，使得SoC估计特别困难。现有方法要么依赖高保真测试，要么只关注传统石墨基锂离子电池，缺乏不确定性量化和计算效率考虑。

Method: 提出数据协调框架标准化不同运行条件下的异质驾驶循环，应用统计学习和深度学习模型预测滞后因子并量化不确定性，同时考虑计算效率。通过重新训练、零样本预测、微调和联合训练评估模型在未见车辆模型上的泛化能力。

Result: 通过大量实验评估了最优模型配置的泛化性能，展示了该方法在解决SoC估计关键挑战方面的有效性，促进了先进电池技术的应用。

Conclusion: 该研究为硅-石墨负极电池的SoC估计提供了一种数据驱动的概率滞后因子预测方法，通过数据协调和机器学习模型实现了高效、可泛化的解决方案，有助于推动先进电池技术的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: RLVR显著提升大语言模型推理能力，但会导致严重的校准退化问题，使模型对错误答案过度自信。现有方法试图将校准目标直接整合到优化目标中，但存在根本性的梯度冲突。

Method: 提出DCPO框架，通过理论分析发现最大化策略准确性和最小化校准误差之间存在梯度冲突，因此系统性地解耦推理和校准目标。

Result: 实验表明DCPO在保持与GRPO相当的准确性的同时，实现了最佳的校准性能，显著缓解了过度自信问题。

Conclusion: 该研究为更可靠的大语言模型部署提供了有价值的见解和实用解决方案，通过解耦目标有效解决了RLVR中的校准退化问题。
...
TL;DR: 利用LLM生成的功能不完美的RTL代码作为训练数据，通过数据增强框架解决电路网表表示学习中的数据稀缺问题，在真实网表上取得良好泛化性能


<details>
  <summary>Details</summary>
Motivation: 电路网表表示学习面临标注数据稀缺的挑战，因为真实设计受知识产权保护且标注成本高。现有方法局限于小规模电路和干净标签，难以扩展到实际设计。虽然LLM可以大规模生成RTL代码，但其功能不正确性限制了在电路分析中的应用。

Method: 提出一个经济高效的数据增强和训练框架，系统性地利用不完美的LLM生成RTL作为训练数据。关键观察是：即使LLM生成的RTL功能不完美，其综合后的网表仍保留与预期功能相关的结构模式。构建从自动代码生成到下游任务的端到端管道。

Result: 在电路功能理解任务上进行评估，包括子电路边界识别和组件分类，涵盖从操作符级到IP级的不同规模基准。评估表明，在噪声合成语料上训练的模型能很好地泛化到真实网表，匹配甚至超越在稀缺高质量数据上训练的方法，有效打破电路表示学习的数据瓶颈。

Conclusion: 即使LLM生成的RTL功能不完美，其结构模式仍对电路表示学习有价值。提出的框架通过利用不完美的合成数据解决了数据稀缺问题，为电路分析任务提供了可扩展的解决方案。
...
Method: 提出地质信息注意力Transformer(GIAT)，利用类别序列相关(CSC)滤波器生成地质信息关系矩阵，注入自注意力计算中
...
<details>
  <summary>Details</summary>
Motivation: 分布式专家问题中，专家分布在多个服务器上，每个时间步的损失是各服务器上损失的ℓ_p范数。现有方法在通信开销方面存在不足，需要设计更高效的协议来最小化遗憾并减少通信量。

Method: 提出了一种新的分布式协议，通过优化通信策略来减少信息交换量。协议利用ℓ_p范数特性，采用多对数因子优化，在保持低遗憾的同时显著降低通信开销。

Result: 协议实现了大约R≳1/(√T·poly log(nsT))的遗憾，通信开销为O((n/R² + s/R²)·max(s^{1-2/p},1)·poly log(nsT))比特，相比之前的工作有所改进。

Conclusion: 该研究为分布式专家问题提供了更高效的解决方案，在遗憾和通信开销之间取得了更好的平衡，特别是在大规模分布式环境中具有实际应用价值。
...
TL;DR: Latent-DARM是一个潜在空间通信框架，连接离散扩散语言模型（规划器）和自回归语言模型（执行器），通过非顺序生成和全局修订能力提升多智能体协作效果。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统主要依赖自回归语言模型（ARMs），虽然能生成流畅文本，但限制了全局推理和计划修订能力。离散扩散语言模型（DDLMs）具有非顺序生成和全局修订优势，但文本流畅度不足，难以与ARMs直接协作。

Method: 提出Latent-DARM框架，在潜在空间中建立DDLMs（作为规划器）和ARMs（作为执行器）之间的通信桥梁，最大化异构模型的协作效益。
...
Conclusion: Latent-DARM通过潜在空间通信有效整合了DDLMs的规划能力和ARMs的执行能力，显著提升了多智能体协作效果，为异构模型协作提供了新方向。
...
TL;DR: 提出一种基于Transformer和量化编码器的数据驱动信号分离方法，在RF信号分离任务中显著优于传统方法，并展示出零样本泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决信号分离问题：从未知非高斯背景/干扰中估计目标信号，需要构建完全数据驱动的信号分离器

Method: 1) 改进Google SoundStream作为离散化编码器，增加Transformer层并使用有限标量量化(FSQ)替代VQVAE；2) 训练端到端Transformer，使用交叉熵损失而非传统均方误差

Result: 在MIT RF Challenge数据集上取得竞争性性能，QPSK信号与5G干扰分离的误码率比先前最优方法降低122倍；学习到的表示能自适应干扰类型且无需侧信息，在推理时展示零样本泛化能力

Conclusion: 该方法在射频混合信号分离中表现优异，其架构可推广到引力波数据(LIGO应变)及其他需要数据驱动背景建模的科学传感问题
...
TL;DR: 该论文提出了RQRE-OVI算法，用于计算风险敏感量化响应均衡，在大型或连续状态空间中实现可证明的高效均衡计算，平衡了理性与风险敏感性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 一般和马尔可夫博弈中的纳什均衡计算存在计算困难、均衡多重性和对近似误差敏感等问题。需要一种既具有计算可行性又能提供鲁棒性的均衡概念。

Method: 提出了RQRE-OVI算法，这是一种乐观值迭代算法，结合了线性函数逼近来处理大型或连续状态空间。该算法基于风险敏感量化响应均衡概念，考虑了有限理性和风险敏感性。

Result: 通过有限样本遗憾分析建立了收敛性，并明确描述了样本复杂度如何随理性和风险敏感性参数变化。遗憾界限揭示了理性与风险敏感性之间的定量权衡：增加理性会收紧遗憾界限，而风险敏感性则通过正则化增强稳定性和鲁棒性。

Conclusion: RQRE-OVI提供了一种原则性、可扩展且可调优的均衡学习方法，在自对弈中具有竞争力，在交叉对弈中相比基于纳什的方法表现出更强的鲁棒性。RQRE策略映射在估计收益方面具有Lipschitz连续性，且具有分布鲁棒优化解释。
...
TL;DR: 该论文提出两种训练组件（RPA和Guardian）来提升Transformer推理效率，在不增加推理成本的情况下改善模型性能
...
Method: 1. 基于模糊机制位置对齐（RPA）的长度感知注意力先验，通过归一化的预softmax偏置引导注意力；2. 最小增益感知控制器（Guardian），仅在验证改进合理时调整注意力锐度，遵循双时间尺度策略梯度优化视角

Result: 在WikiText 2上保持计算对等的情况下，降低了验证交叉熵，同时匹配基线延迟和内存使用；推理时仅添加预计算的缓存偏置，控制器不运行，实际开销可忽略

Conclusion: 长度感知先验和后期增益控制能够保留稀缺的改进效果，特别是在长跨度、噪声logit机制中，同时保持测试时间成本基本不变
...
TL;DR: 该论文提出了基于最优传输的表示学习泛化边界，用于图节点分类的转导学习场景，解决了传统复杂度度量计算困难且与经验行为不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有转导泛化边界依赖计算困难且与经验行为不匹配的经典复杂度度量，需要开发更实用、与经验泛化强相关的泛化边界。

Method: 在分布无关的转导设置下，通过最优传输推导全局和类别的泛化边界，用编码特征分布之间的Wasserstein距离表达，并分析GNN聚合过程如何改变表示分布。

Result: 提出的边界可高效计算，在图节点分类中与经验泛化强相关，优于经典复杂度度量，揭示了深度与泛化误差的非单调关系。

Conclusion: 基于最优传输的表示学习泛化边界为转导学习提供了实用且理论严谨的分析工具，揭示了GNN深度与泛化性能的复杂关系。
...
### [84] [A Gaussian Comparison Theorem for Training Dynamics in Machine Learning](https://arxiv.org/abs/2603.09310)
*Ashkan Panahi*

Main category: cs.LG

TL;DR: 该论文提出了一种基于高斯混合模型训练算法的非渐近分析方法，将模型演化与替代动力系统连接，并利用Gordon比较定理证明动态平均场表达式的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究高斯混合模型下训练算法的理论分析，特别是在非渐近场景下，需要建立更精确的分析框架来理解算法动态。

Method: 1. 提出基于Gordon比较定理的非渐近结果，将模型演化连接到替代动力系统；2. 严格证明动态平均场表达式在渐近场景下的有效性；3. 提出迭代细化方案以获得非渐近场景下更精确的表达式；4. 将理论应用于感知器模型的通用一阶（全批量）算法分析。

Result: 1. 建立了训练算法与替代动力系统之间的非渐近连接；2. 严格证明了动态平均场表达式在渐近场景下的有效性；3. 发现在非渐近域中除了动态平均场核外还会出现波动参数。

Conclusion: 该研究为高斯混合模型下的训练算法提供了新的非渐近分析框架，通过连接模型演化与动力系统，不仅验证了渐近场景下的动态平均场理论，还揭示了非渐近场景下的额外波动参数，为算法分析提供了更精确的工具。
...
### [85] [Reward-Zero: Language Embedding Driven Implicit Reward Mechanisms for Reinforcement Learning](https://arxiv.org/abs/2603.09331)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决强化学习中稀疏或延迟的环境反馈问题，避免任务特定的工程化奖励设计，需要一种通用的、基于语义的奖励机制来加速探索、稳定训练并增强泛化能力。

Method: 通过比较任务规范的语言嵌入与智能体交互经验的语言嵌入，产生连续、语义对齐的完成感信号。这种方法利用语言嵌入作为通用奖励函数，无需任务特定工程。

Result: 实验表明，使用Reward-Zero训练的智能体比传统方法（如PPO）收敛更快，最终成功率更高，成功解决了某些复杂任务中手工设计奖励无法解决的问题。同时开发了一个通过语言嵌入评估任务执行过程中完成感的微型基准。

Conclusion: 语言驱动的隐式奖励函数为实现更样本高效、可泛化和可扩展的强化学习提供了一条实用路径，特别适用于具身智能体。
...
### [86] [TA-GGAD: Testing-time Adaptive Graph Model for Generalist Graph Anomaly Detection](https://arxiv.org/abs/2603.09349)
*Xiong Zhang,Hong Peng,Changlong Fu,Xin Jin,Yun Yang,Cheng Xie*

Main category: cs.LG

TL;DR: 提出图异常检测中的异常非同类性(AD)问题，并基于此开发了一个图基础模型，实现单次训练即可跨域泛化的异常检测


<details>
  <summary>Details</summary>
Motivation: 现实世界中存在大量异常节点（如假新闻、恶意交易等）严重危害图数据生态系统健康，需要有效识别。现有跨域检测模型面临严重的域偏移问题，限制了其在所有领域的泛化能力。

Method: 识别并定量分析了图异常检测中域偏移表现出的特定特征不匹配模式，定义为异常非同类性(AD)问题。基于AD问题建模，引入了一个新颖的图基础模型用于异常检测。

Result: 基于14个不同的真实世界图进行实验，证实了模型在跨域适应方面取得突破，在检测准确率方面达到了开创性的最先进水平。
...
### [87] [From Representation to Clusters: A Contrastive Learning Approach for Attributed Hypergraph Clustering](https://arxiv.org/abs/2603.09370)
...
Conclusion: CAHC是一种有效的端到端属性超图聚类方法，通过同时学习节点嵌入和聚类结果，结合对比学习和聚类导向优化，取得了优于现有方法的性能。
...
### [88] [SPAARS: Safer RL Policy Alignment through Abstract Exploration and Refined Exploitation of Action Space](https://arxiv.org/abs/2603.09378)
*Swaminathan S K,Aritra Hazra*

Main category: cs.LG

TL;DR: SPAARS是一个离线到在线强化学习框架，通过课程学习策略：先在低维潜在空间进行安全探索，然后无缝转移到原始动作空间以绕过解码器瓶颈，解决了现有方法存在的利用差距问题。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习在机器人领域很有前景，但关键挑战是如何在在线探索时不偏离离线数据的行为支持。现有方法使用CVAE限制探索在潜在空间内，但存在利用差距问题——解码器重建损失导致的性能上限。

Method: SPAARS采用课程学习框架：1）初始阶段将探索限制在低维潜在流形上，实现样本高效的安全行为改进；2）无缝将控制转移到原始动作空间，绕过解码器瓶颈。提供两种实现：CVAE变体仅需无序(s,a)对，无需轨迹分割；SPAARS-SUPE结合OPAL时序技能预训练以获得更强的探索结构，但需要轨迹块。

Result: 理论证明：1）使用性能差异引理建立了利用差距的上界；2）潜在空间策略梯度相比原始空间探索实现了可证明的方差减少；3）潜在阶段同时进行行为克隆直接控制课程转换稳定性。实验显示：SPAARS-SUPE在kitchen-mixed-v0上达到0.825标准化回报（SUPE为0.75），样本效率提高5倍；独立SPAARS在hopper-medium-v2和walker2d-medium-v2上分别达到92.7和102.9标准化回报，超越IQL基线的66.3和78.3。

Conclusion: SPAARS框架成功解决了离线到在线强化学习中的利用差距问题，通过课程学习策略实现了从潜在空间到原始动作空间的安全高效过渡，两种变体分别适用于不同数据可用性场景，在多个基准任务上表现出优越性能。
...
### [89] [Reconstructing Movement from Sparse Samples: Enhanced Spatio-Temporal Matching Strategies for Low-Frequency Data](https://arxiv.org/abs/2603.09412)
*Ali Yousefian,Arianna Burzacchi,Simone Vantini*

Main category: cs.LG

TL;DR: 本文针对GPS轨迹与路网匹配的空间-时间匹配算法提出四项改进：动态缓冲区、自适应观测概率、重新设计的时间评分函数和行为分析，在米兰城区真实数据上验证了效率和路径质量的显著提升。


<details>
  <summary>Details</summary>
Motivation: 空间-时间匹配算法虽然有效，但在计算效率和结果准确性方面存在局限，特别是在密集环境和高采样间隔情况下。需要改进算法以应对这些挑战。

Method: 提出四项改进：1）动态缓冲区替代固定缓冲区；2）自适应观测概率；3）重新设计的时间评分函数；4）结合历史移动模式的行为分析。使用米兰城区真实数据进行评估，并定义了新的评估指标。

Result: 实验结果显示，改进后的算法在各种评估指标上都表现出显著的性能效率提升和路径质量改善。

Conclusion: 通过四项关键改进，空间-时间匹配算法在计算效率和路径匹配准确性方面都得到了显著提升，特别是在密集城区环境中表现更好。
...
### [90] [Impact of Markov Decision Process Design on Sim-to-Real Reinforcement Learning](https://arxiv.org/abs/2603.09427)
*Tatjana Krau,Jorge Mandlmaier,Tobias Damm,Frieder Heieck*

Main category: cs.LG

TL;DR: 该研究系统分析了强化学习在工业过程控制中的MDP设计选择对仿真到现实迁移的影响，通过颜色混合任务验证了不同配置，发现基于物理的动力学模型在严格精度约束下比简化模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 强化学习在工业过程控制中显示出潜力，但在仿真中训练的策略部署到物理硬件时存在显著的仿真到现实差距。研究旨在分析核心MDP设计选择如何影响这种迁移。

Method: 使用颜色混合任务，系统评估不同的MDP配置（状态组成、目标包含、奖励公式、终止标准、环境动力学模型）和混合动力学，在仿真和真实世界实验中进行对比。

Result: 在物理硬件上验证发现，基于物理的动力学模型在严格精度约束下实现了高达50%的真实世界成功率，而简化模型则完全失败。

Conclusion: 研究结果为在工业过程控制中部署强化学习提供了实用的MDP设计指南，强调基于物理的动力学模型对于减少仿真到现实差距的重要性。
...
### [91] [From Weighting to Modeling: A Nonparametric Estimator for Off-Policy Evaluation](https://arxiv.org/abs/2603.09436)
*Rong J. B. Zhu*

Main category: cs.LG

TL;DR: 提出非参数加权(NW)和模型辅助非参数加权(MNW)方法，用于上下文赌博机的离策略评估，相比传统IPW和DR方法，在保持低偏差的同时显著降低方差。


<details>
  <summary>Details</summary>
Motivation: 在上下文赌博机的离策略评估中，历史数据通常不能准确反映新策略的动作分布。逆概率加权(IPW)方法虽然能调整动作分布的差异，但由于概率在分母中，常常导致高方差。双重稳健(DR)估计器通过建模奖励来减少方差，但并未直接解决IPW带来的方差问题。

Method: 提出非参数加权(NW)方法，使用非参数模型构建权重，保持IPW的低偏差特性但显著降低方差。进一步提出模型辅助非参数加权(MNW)方法，结合奖励预测（类似DR技术），通过显式建模和缓解奖励建模的偏差来获得准确的价值估计，但不保证标准的双重稳健性。

Result: 广泛的实证比较表明，NW和MNW方法在价值估计中一致优于现有技术，在保持低偏差的同时实现更低的方差。

Conclusion: 提出的非参数加权方法有效解决了传统IPW方法的高方差问题，模型辅助版本进一步提升了估计精度，为上下文赌博机的离策略评估提供了更优的解决方案。
...
### [92] [Variational Routing: A Scalable Bayesian Framework for Calibrated Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.09453)
*Albus Yizhuo Li,Matthew Wicker*

Main category: cs.LG

TL;DR: 提出VMoER方法，在MoE层中引入贝叶斯不确定性量化，通过变分推理改进专家路由的稳定性与校准，仅增加不到1%的计算开销


<details>
  <summary>Details</summary>
Motivation: 基础模型部署中需要理解输出不确定性以确保负责任部署，但传统贝叶斯方法计算开销过大，无法应用于万亿参数规模的基础模型训练或推理

Method: 提出变分混合专家路由（VMoER），将贝叶斯推理限制在专家选择阶段，采用两种推理策略：路由logits的摊销变分推理和随机专家选择的温度参数推断

Result: VMoER将噪声下的路由稳定性提高38%，校准误差降低94%，分布外检测AUROC提高12%，同时仅增加不到1%的FLOPs开销

Conclusion: VMoER为构建稳健且具有不确定性感知能力的基础模型提供了可扩展的路径
...
### [93] [Temporal-Conditioned Normalizing Flows for Multivariate Time Series Anomaly Detection](https://arxiv.org/abs/2603.09490)
*David Baumgartner,Helge Langseth,Kenth Engø-Monsen,Heri Ramampiaro*

Main category: cs.LG

TL;DR: 本文提出了一种基于时间条件归一化流(tcNF)的时间序列异常检测框架，通过将归一化流条件化于历史观测值来建模时间依赖性和不确定性，实现鲁棒的异常检测。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据中的异常检测需要准确建模时间依赖性和不确定性，现有方法在这方面存在不足。本文旨在开发一个能够有效捕捉复杂时间动态并生成准确概率分布的框架。

Method: 提出时间条件归一化流(tcNF)框架，将归一化流条件化于先前观测值，采用自回归方法捕捉时间依赖关系，通过学习正常行为的概率分布来识别低概率异常事件。

Result: 在多个数据集上评估tcNF，相比现有方法表现出良好的准确性和鲁棒性。提供了全面的强度与局限性分析以及开源代码，确保可复现性。

Conclusion: tcNF框架能够有效建模时间序列中的时间依赖性和不确定性，为异常检测提供了一种准确且鲁棒的方法，开源代码和详细分析有助于未来研究发展。
...
### [94] [Efficiently Aligning Draft Models via Parameter- and Data-Efficient Adaptation](https://arxiv.org/abs/2603.09527)
...
Method: 提出EDA框架：1) 解耦架构使用共享和私有组件分别建模共享和目标特定输出分布，仅更新轻量私有组件；2) 数据再生策略利用微调目标模型再生训练数据，提高训练与推测解码的对齐度；3) 样本选择机制优先选择高价值数据进行高效适配

Result: 实验显示EDA能有效恢复微调模型的推测解码性能，以显著降低的训练成本实现更优的平均接受长度

Conclusion: EDA框架为草稿模型适配提供了参数和数据高效的解决方案，能有效解决微调目标模型导致的推测解码性能下降问题
...
### [95] [An Optimal Control Approach To Transformer Training](https://arxiv.org/abs/2603.09571)
*Kağan Akman,Naci Saldı,Serdar Yüksel*

Main category: cs.LG

TL;DR: 论文提出了一种基于最优控制理论的Transformer训练方法，将Transformer建模为具有共享动作的无噪声McKean-Vlasov动力学离散时间受控粒子系统，通过提升到概率测度空间构建完全可观测的MDP，并证明了全局最优策略的存在性。


<details>
  <summary>Details</summary>
Motivation: 为Transformer训练提供一种严格的最优控制理论方法，同时满足三个关键结构约束：执行时的实现输入独立性、集成控制特性以及位置依赖性，为梯度训练提供全局最优且鲁棒的替代方案。

Method: 将Transformer架构建模为具有共享动作的离散时间受控粒子系统，通过提升到概率测度空间构建完全可观测MDP，提出三重量化训练过程（状态空间、概率测度空间和动作空间的量化），并证明量化模型的最优策略对原始问题接近最优。

Result: 在紧致性假设下证明了全局最优策略的存在性；证明了提升模型中的闭环策略等价于依赖于初始分布的开环策略；建立了提升模型的稳定性和经验一致性，显示值函数对初始经验测度扰动具有连续性，且策略随数据量增加而收敛。

Conclusion: 该方法为Transformer训练提供了全局最优且鲁棒的替代方案，不要求平滑性或凸性，同时满足Transformer的关键结构约束，为基于梯度的训练提供了理论严谨的替代方法。
...
### [96] [Towards Understanding Adam Convergence on Highly Degenerate Polynomials](https://arxiv.org/abs/2603.09581)
*Zhiwei Bai,Jiajie Zhao,Zhangchen Zhou,Zhi-Qin John Xu,Yaoyu Zhang*

Main category: cs.LG

TL;DR: 该论文研究了Adam优化算法在高度退化多项式上的自动收敛特性，发现Adam在该类函数上无需外部调度器即可实现局部线性收敛，显著优于梯度下降和动量法的次线性收敛。


<details>
  <summary>Details</summary>
Motivation: Adam是深度学习中被广泛使用的优化算法，但其在哪些目标函数上具有内在优势尚未得到充分探索。先前研究需要外部调度器和β₂接近1才能收敛，本文旨在研究Adam的"自然"自动收敛特性。

Method: 研究高度退化多项式上Adam的自动收敛特性，推导退化多项式上局部渐近稳定的理论条件，证明Adam在该类函数上实现局部线性收敛，并分析其超参数相图。

Result: 理论条件与实验结果高度一致，Adam在退化多项式上实现局部线性收敛，显著优于梯度下降和动量法的次线性收敛。这种加速源于二阶矩v_t与梯度平方g_t²之间的解耦机制，该机制指数级放大了有效学习率。超参数相图揭示了三种行为模式：稳定收敛、尖峰和SignGD类振荡。

Conclusion: 该工作识别了Adam具有内在优势的函数类别——高度退化多项式，在这些函数上Adam无需外部调度器即可自动收敛并实现局部线性收敛，显著优于传统优化方法。研究揭示了Adam的加速机制和解耦特性，为理解Adam的收敛行为提供了新的理论视角。
...
### [97] [Nonparametric Variational Differential Privacy via Embedding Parameter Clipping](https://arxiv.org/abs/2603.09583)
*Dina El Zein,Shashi Kumar,James Henderson*

Main category: cs.LG

TL;DR: 本文提出了一种参数裁剪策略，用于解决非参数变分信息瓶颈（NVIB）中潜在表示漂移导致隐私保护效果差和效用低的问题，通过理论推导的约束条件改善隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 非参数变分信息瓶颈（NVIB）是非参数变分差分隐私（NVDP）的基础，但学习到的潜在表示可能漂移到高信息区域，导致隐私保护效果差，同时由于训练过程中的数值不稳定性导致效用低下。

Method: 提出了一种基于理论推导的参数裁剪策略，通过最小化Rényi散度（RD）上界的目标，对后验均值、方差和混合权重参数施加特定的理论约束，应用于NVIB模型并与无约束基线进行比较。

Result: 裁剪后的模型始终实现更紧的RD上界（意味着更强的隐私保护），同时在多个下游任务中获得更高的性能表现。
...
### [98] [MM-algorithms for traditional and convex NMF with Tweedie and Negative Binomial cost functions and empirical evaluation](https://arxiv.org/abs/2603.09601)
*Elisabeth Sommer James,Asger Hobolth,Marta Pelizzola*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的NMF框架，支持多种噪声分布假设（包括负二项式和Tweedie模型），并开发了相应的乘法更新规则，特别是针对凸NMF的泊松和负二项式成本函数的新更新方法。


<details>
  <summary>Details</summary>
Motivation: 标准NMF方法通常基于高斯或泊松噪声假设，但这些假设对于具有过度离散或复杂均值-方差关系的数据可能不充分。需要开发一个更通用的框架来适应更广泛的分布假设。

Method: 使用Majorize-Minimisation方法，为传统NMF和凸NMF推导了统一的乘法更新规则，特别针对泊松和负二项式成本函数开发了新的凸NMF更新方法。实现了包括多个凸NMF模型的统一实现。

Result: 在突变和词频数据上的实证评估表明，噪声模型的选择对模型拟合和特征恢复有重要影响，凸NMF在类别数量大的情况下可以提供高效且稳健的替代方案。

Conclusion: 提出了一个统一的NMF框架，支持多种分布假设，并提供了相应的实现工具。研究表明噪声模型的选择至关重要，凸NMF在某些场景下具有优势。
...
### [99] [FreqCycle: A Multi-Scale Time-Frequency Analysis Method for Time Series Forecasting](https://arxiv.org/abs/2603.09661)
*Boya Zhang,Shuaijie Yin,Huiwen Zhu,Xing He*

Main category: cs.LG

TL;DR: FreqCycle是一个新颖的时间序列预测框架，通过提取低频特征和增强中高频能量比例来提升预测性能，其扩展版本MFreqCycle还能处理耦合多周期性和长回溯窗口问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注低频模式建模，而忽略了中高频特征，这限制了深度学习模型性能的进一步提升。同时，时间序列数据常表现出耦合多周期性（如交织的周周期和日周期），以及长回溯窗口的挑战。

Method: 提出FreqCycle框架：1）FECF模块在时域显式学习共享周期模式提取低频特征；2）SFPL模块通过可学习滤波器和自适应加权增强中高频能量比例。扩展为MFreqCycle，通过跨尺度交互解耦嵌套周期特征，处理耦合多周期性和长回溯窗口问题。

Result: 在七个不同领域的基准测试中，FreqCycle实现了最先进的预测精度，同时保持了更快的推理速度，在性能与效率之间达到了最佳平衡。

Conclusion: FreqCycle通过同时关注低频和中高频特征，有效提升了时间序列预测性能，其分层扩展MFreqCycle进一步解决了耦合多周期性和长回溯窗口的挑战，为时间序列分析提供了新的有效框架。
...
### [100] [No evaluation without fair representation : Impact of label and selection bias on the evaluation, performance and mitigation of classification models](https://arxiv.org/abs/2603.09662)
*Magali Legast,Toon Calders,François Fouss*

Main category: cs.LG

TL;DR: 该研究通过实验分析了标签偏差和选择偏差对分类模型评估、性能及偏差缓解方法有效性的影响，并提出了一个偏差建模框架来模拟公平世界及其偏差对应物。
...
Method: 引入一个偏差建模和评估框架，通过在低歧视的真实数据集中引入受控偏差来模拟公平世界及其偏差对应物；独立分析每种偏差类型的影响，并使用比传统方法（仅使用有偏数据子集作为测试集）更具代表性的评估方法。

Result: 研究结果揭示了影响偏差对模型性能影响程度的不同因素；发现在测试集不包含不需要的偏差时，公平性与准确性之间、个体公平性与群体公平性之间不存在权衡；偏差缓解方法的性能受数据中存在的偏差类型影响。

Conclusion: 研究结果呼吁未来工作开发更准确的预测模型和公平干预评估方法，更好地理解其他偏差类型、涉及不同偏差类型组合的更复杂场景，以及影响缓解方法效率的其他因素（如数据集特征）。
...
### [101] [On Catastrophic Forgetting in Low-Rank Decomposition-Based Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2603.09684)
*Muhammad Ahmad,Jingjing Zheng,Yankai Cao*

Main category: cs.LG

TL;DR: 论文实证研究表明，基于低秩分解的参数高效微调方法在顺序学习中的遗忘问题主要受更新子空间的几何结构和参数化方式影响，张量分解方法能更好地缓解遗忘。


<details>
  <summary>Details</summary>
Motivation: 研究参数高效微调方法在顺序学习中的行为，特别是灾难性遗忘问题，目前对此理解不足，需要探索更新子空间设计对遗忘的影响。

Method: 通过实证研究分析不同更新子空间设计对遗忘的影响，比较基于矩阵子空间的方法（如LoRA）和张量分解方法（如LoRETTA），以及结构对齐参数化方法（如WeGeFT）。

Result: 研究发现：1）限制在小共享矩阵子空间的更新方法容易产生任务干扰；2）张量分解方法能在超紧凑预算内捕获更丰富的结构信息，有效缓解遗忘；3）结构对齐参数化方法能更好地保持预训练表示。

Conclusion: 更新子空间设计是持续学习中的关键因素，为顺序学习场景中选择高效适应策略提供了实用指导，张量分解和结构对齐参数化方法在缓解遗忘方面表现更好。
...
### [102] [ActiveUltraFeedback: Efficient Preference Data Generation using Active Learning](https://arxiv.org/abs/2603.09692)
*Davit Melikidze,Marian Schneider,Jessica Lam,Martin Wertich,Ido Hakimi,Barna Pásztor,Andreas Krause*

Main category: cs.LG

TL;DR: ACTIVEULTRAFEEDBACK是一个模块化主动学习框架，通过不确定性估计动态选择最有信息量的响应进行标注，显著降低RLHF对齐LLMs的标注成本。


<details>
  <summary>Details</summary>
Motivation: RLHF已成为对齐大型语言模型的标准方法，但其效果受限于获取偏好数据的高成本，特别是在低资源和专业领域。需要更高效的数据收集方法来降低标注成本。

Method: 提出ACTIVEULTRAFEEDBACK模块化主动学习流水线，利用不确定性估计动态识别最有信息量的响应进行标注。引入两种新方法：DOUBLE REVERSE THOMPSON SAMPLING (DRTS)和DELTAUCB，优先选择预测质量差距大的响应对进行标注。
...
### [103] [Physics-informed neural operator for predictive parametric phase-field modelling](https://arxiv.org/abs/2603.09693)
...
<details>
  <summary>Details</summary>
Motivation: 相场建模用于预测材料微观结构和形态演化，但计算成本高昂，特别是在高通量参数研究中。传统神经算子（如FNO）缺乏显式物理约束，可能限制其在复杂相场动力学中的泛化能力和长期准确性。

Method: 开发了PF-PINO物理信息神经算子框架，将相场控制方程的残差嵌入数据保真度损失函数中，在训练过程中有效强制执行物理约束。

Result: PF-PINO在电化学腐蚀、枝晶凝固和旋节分解等基准相场问题上验证，显著优于传统FNO，在精度、泛化能力和长期稳定性方面表现优异。

Conclusion: 该工作为相场建模提供了强大高效的计算工具，突显了物理信息神经算子在推进复杂界面演化问题的科学机器学习方面的潜力。
...
### [104] [Exploiting Label-Aware Channel Scoring for Adaptive Channel Pruning in Split Learning](https://arxiv.org/abs/2603.09792)
*Jialei Tan,Zheng Lin,Xiangming Cai,Ruoxi Zhu,Zihan Fang,Pingping Chen,Wei Ni*

Main category: cs.LG

TL;DR: 提出自适应通道剪枝辅助的分割学习方案ACP-SL，通过标签感知通道重要性评分模块识别重要通道，自适应剪枝减少通信开销


<details>
  <summary>Details</summary>
Motivation: 分割学习虽然将大部分训练工作负载转移到服务器，减轻了客户端设备计算负担，但中间特征表示（粉碎数据）的传输在大量客户端设备参与时会产生显著的通信开销

Method: 设计了标签感知通道重要性评分模块生成通道重要性分数，区分重要与不重要通道；基于这些分数开发自适应通道剪枝模块，剪枝不重要通道，压缩粉碎数据并减少通信开销

Result: 实验结果表明ACP-SL在测试准确率上持续优于基准方案，并且能以更少的训练轮次达到目标测试准确率，从而减少通信开销

Conclusion: 提出的ACP-SL方案有效解决了分割学习中的通信开销问题，通过自适应通道剪枝在保持性能的同时显著减少了通信负担
...
### [105] [Information Theoretic Bayesian Optimization over the Probability Simplex](https://arxiv.org/abs/2603.09793)
...
<details>
  <summary>Details</summary>
Motivation: 许多应用涉及优化概率和混合模型，这些自然属于概率单纯形（非负元素和为1的约束非欧几里得域）。传统贝叶斯优化方法在处理这种几何结构时存在局限性，需要专门针对概率单纯形几何特性的优化方法。

Method: 基于信息几何理论，构建反映概率单纯形几何结构的Matérn核函数，并开发单参数几何优化器用于采集函数优化。信息几何为单纯形提供了黎曼度量和连接类，使算法能够更好地处理该空间的几何特性。

Result: 在基准函数和多种实际应用（包括组件混合、分类器混合和机器人控制任务）上验证了方法的有效性，相比约束欧几里得方法表现出更高的性能。

Conclusion: α-GaBO是一种新颖的贝叶斯优化算法，专门针对概率单纯形的几何特性设计，通过信息几何理论构建的核函数和优化器，在处理概率和混合优化问题时比传统方法更有效。
...
### [106] [Good Reasoning Makes Good Demonstrations: Implicit Reasoning Quality Supervision via In-Context Reinforcement Learning](https://arxiv.org/abs/2603.09803)
*Tiehua Mei,Minxuan Lv,Leiyu Pan,Zhenpeng Su,Hongru Hou,Hengrui Chen,Ao Xu,Deqing Yang*

Main category: cs.LG

TL;DR: 本文提出In-Context RLVR方法，通过利用模型自身的上下文学习能力评估推理质量，对高质量推理轨迹赋予更高权重，提升数学推理的准确性和推理质量。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR方法将所有正确解决方案同等对待，可能强化那些偶然获得正确答案但推理过程有缺陷的轨迹。研究发现高质量解决方案比低质量解决方案能提供更有效的示范。

Method: 提出In-Context RLVR方法，引入"示范效用"概念，利用策略模型自身的上下文学习能力来测量推理质量（称为"证据增益"），通过贝叶斯分析隐式地对奖励进行重新加权。

Result: 在数学基准测试中，相比标准RLVR方法，新方法在准确性和推理质量方面都取得了改进。

Conclusion: 利用模型自身的上下文学习能力可以高效评估推理质量，通过隐式重新加权奖励，能够提升强化学习在语言模型推理任务中的效果。
...
### [107] [GAST: Gradient-aligned Sparse Tuning of Large Language Models with Data-layer Selection](https://arxiv.org/abs/2603.09865)
*Kai Yao,Zhenghan Song,Kaixin Wu,Mingjie Zhong,Danzhao Cheng,Zhaorui Tan,Yixin Ji,Penglei Gao*

Main category: cs.LG

TL;DR: GAST是一种梯度对齐稀疏调优方法，在数据和层两个维度同时进行选择性微调，通过自适应选择对每层影响最大的数据点来减少冗余信息，在参数高效微调中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法主要关注两个独立范式：层选择性方法（微调关键层以减少计算负载）和数据选择性方法（选择有效训练子集以提升训练效果）。但这些方法通常忽略了不同数据点对不同模型层的贡献程度不同，并且往往丢弃了被视为低质量数据中可能包含的有价值信息。

Method: 提出梯度对齐稀疏调优（GAST）方法，在数据和层两个维度同时进行选择性微调，作为统一优化策略的组成部分。采用层稀疏策略，自适应选择对每层影响最大的数据点，提供比单维度方法更全面、更精细的解决方案。

Result: 实验表明，GAST在参数高效微调中持续优于基线方法，为未来PEFT策略研究确立了有前景的方向。

Conclusion: GAST通过同时考虑数据和层维度的选择性微调，有效解决了现有方法忽略数据点对不同层贡献差异以及丢弃低质量数据中有价值信息的问题，在参数高效微调中表现出优越性能。
...
### [108] [CarbonBench: A Global Benchmark for Upscaling of Carbon Fluxes Using Zero-Shot Learning](https://arxiv.org/abs/2603.09868)
*Aleksei Rozanov,Arvind Renganathan,Yimeng Zhang,Vipin Kumar*

Main category: cs.LG

TL;DR: CarbonBench是首个用于碳通量升尺度零样本空间迁移学习的基准测试，包含全球567个通量塔站点的130万日观测数据，提供分层评估协议、统一特征集和基线方法，旨在系统评估模型在不同植被类型和气候条件下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 准确量化陆地碳交换对气候政策和碳核算至关重要，但现有模型需要泛化到稀疏涡度协方差观测数据中代表性不足的生态系统。目前缺乏标准化基准来严格评估模型在不同地理位置、气候条件和植被类型下的性能。

Method: 构建CarbonBench基准：1) 包含全球567个通量塔站点2000-2024年的130万日观测数据；2) 设计分层评估协议，明确测试模型在未见植被类型和气候条件下的泛化能力，分离空间迁移与时间自相关；3) 提供统一的遥感与气象特征集；4) 建立从树基方法到领域泛化架构的基线模型。

Result: 创建了首个用于碳通量升尺度零样本空间迁移学习的标准化基准，为系统比较迁移学习方法提供了平台，可作为分布偏移下回归问题的测试床，并支持下一代气候建模工作。

Conclusion: CarbonBench通过连接机器学习方法与地球系统科学，实现了对迁移学习方法的系统比较，为解决碳通量模型在空间分布偏移下的泛化问题提供了重要工具，有助于推动气候建模的发展。
...
TL;DR: MSSR是一种基于记忆强度估计和自适应间隔排练的经验回放框架，用于缓解大语言模型在持续微调中的灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 大语言模型在动态环境中部署时需要持续微调，但强适应性导致灾难性遗忘问题。现有回放策略要么依赖启发式规则只能部分缓解遗忘，要么计算开销过大

Method: 提出MSSR框架，通过估计样本级记忆强度，在自适应间隔安排排练，平衡快速适应和遗忘缓解

Result: 在3个骨干模型和11个序列任务上的实验表明，MSSR持续优于最先进的回放基线，在推理密集型和多项选择基准上表现尤为突出

Conclusion: MSSR通过记忆强度驱动的自适应排练机制，有效缓解了持续微调中的灾难性遗忘问题，同时保持了模型的快速适应能力
...
### [110] [Generative Drifting is Secretly Score Matching: a Spectral and Variational Perspective](https://arxiv.org/abs/2603.09936)
*Erkan Turan,Maks Ovsjanikov*

Main category: cs.LG

TL;DR: 本文揭示了基于核的漂移算子与平滑分布得分差分的等价关系，解决了原始工作中三个关键理论问题，提出了指数带宽退火策略，并将漂移方法形式化为平滑KL散度的Wasserstein梯度流。


<details>
  <summary>Details</summary>
Motivation: 生成建模中的漂移方法虽然在一步图像生成中取得了经验成功，但其理论基础薄弱。本文旨在填补这一理论空白，特别是解决原始工作中遗留的三个关键问题：漂移消失是否保证分布相等、如何选择核函数、以及为什么需要停止梯度算子。

Method: 通过将高斯核下的漂移算子解释为平滑分布的得分差分，将漂移方法置于得分匹配框架中。通过线性化McKean-Vlasov动力学并在傅里叶空间中分析，揭示了频率相关的收敛时间尺度。提出了指数带宽退火策略，并将漂移形式化为平滑KL散度的Wasserstein梯度流。

Result: 证明了高斯核存在指数级高频瓶颈，解释了经验上偏好拉普拉斯核的原因。提出的指数带宽退火策略将收敛时间从exp(O(K_max^2))减少到O(log K_max)。证明了停止梯度算子源于JKO方案要求的冻结场离散化，移除它会破坏梯度流保证。

Conclusion: 本文为漂移方法建立了坚实的理论基础，将其与得分匹配和Wasserstein梯度流联系起来。提出的理论框架不仅解释了现有方法的经验成功，还为构建新的漂移算子提供了通用模板，如演示的Sinkhorn散度漂移。
...
### [111] [SignalMC-MED: A Multimodal Benchmark for Evaluating Biosignal Foundation Models on Single-Lead ECG and PPG](https://arxiv.org/abs/2603.09940)
*Fredrik K. Gustafsson,Xiao Gu,Mattia Carletti,Patitapaban Palo,David W. Eyre,David A. Clifton*

Main category: cs.LG

TL;DR: SignalMC-MED是一个用于评估生物信号基础模型在长时程多模态数据上性能的基准测试，包含22,256次就诊的同步单导联ECG和PPG信号，涵盖20个临床相关任务。研究发现：特定领域的生物信号基础模型优于通用时间序列模型，ECG+PPG多模态融合优于单模态，使用完整10分钟信号优于短片段，手工ECG特征与学习表示结合具有互补价值。


<details>
  <summary>Details</summary>
Motivation: 尽管生物信号基础模型在各种临床预测任务中表现出良好性能，但在长时程多模态数据上的系统评估仍然有限。需要建立一个标准化的基准来评估这些模型在同步ECG和PPG信号上的表现，为实际部署提供指导。

Method: 基于MC-MED数据集构建SignalMC-MED基准，包含22,256次就诊的10分钟重叠ECG和PPG信号，涵盖20个临床相关任务（人口统计学预测、急诊处置、实验室值回归、ICD-10诊断检测）。系统评估代表性时间序列和生物信号基础模型在ECG-only、PPG-only和ECG+PPG三种设置下的性能。

Result: 特定领域的生物信号基础模型始终优于通用时间序列模型；ECG+PPG多模态融合比单模态输入有稳健改进；使用完整10分钟信号始终优于较短片段；较大模型变体并不总是优于较小模型；手工ECG特征提供了强大的基线，与学习表示结合具有互补价值。

Conclusion: SignalMC-MED为生物信号基础模型的评估提供了一个标准化基准，并为实际部署提供了实用指导：应优先考虑特定领域模型、多模态融合、完整信号长度，以及手工特征与学习表示的结合。
...
### [112] [Task Aware Modulation Using Representation Learning for Upsaling of Terrestrial Carbon Fluxes](https://arxiv.org/abs/2603.09974)
...
<details>
  <summary>Details</summary>
Motivation: 准确估算陆地碳通量对全球碳预算评估至关重要，但由于地面测量数据稀疏且分布不均，现有数据驱动方法在未观测区域泛化能力差，存在系统性区域偏差和高预测不确定性。

Method: 提出任务感知调制与表征学习框架，将时空表征学习与基于碳平衡方程的知识引导编码器-解码器架构和损失函数相结合。

Result: 在150多个代表不同生物群落和气候区的通量塔站点上，TAM-RL相比现有最先进数据集将RMSE降低了8-9.6%，解释方差从19.4%提升到43.8%，具体取决于目标通量类型。
