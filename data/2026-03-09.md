...
### [1] [Real Faults in Model Context Protocol (MCP) Software: a Comprehensive Taxonomy](https://arxiv.org/abs/2603.05637)
*Mina Taraghi,Mohammad Mehdi Morovati,Foutse Khomh*

Main category: cs.SE

TL;DR: 首次对MCP服务器故障进行大规模分类研究，识别出5个高层故障类别，并通过从业者调查验证其实际存在性和独特性，为构建更可靠的AI软件系统提供指导。


<details>
  <summary>Details</summary>
Motivation: 基础模型的快速应用虽然增强了软件系统的能力，但也带来了新的可靠性、安全性和鲁棒性挑战。随着模型上下文协议（MCP）的广泛采用，标准化AI系统与外部工具的交互，但目前缺乏对MCP软件系统实际故障的系统性理解。

Method: 基于实证证据构建MCP服务器故障分类法（包含5个高层故障类别），然后对具有不同角色和经验水平的MCP从业者进行调查，评估分类法的完整性和普适性。

Result: 调查证实所有识别的故障类别在实践中都存在，并揭示了MCP特定故障与非MCP故障的显著区别特征。研究识别了MCP系统中最容易出错和最关键的组件。

Conclusion: 这项研究为研究人员和从业者提供了可操作的见解，有助于开发更鲁棒、可靠和安全的基于MCP的AI软件系统，填补了该领域系统性故障理解的空白。
...
### [2] [Balancing Latency and Accuracy of Code Completion via Local-Cloud Model Cascading](https://arxiv.org/abs/2603.05974)
*Hanzhen Lu,Lishui Fan,Jiachi Chen,Qiuyuan Chen,Zhao Wei,Zhongxin Liu*

Main category: cs.SE

TL;DR: MCCom是一个级联本地小模型和云端大模型的代码补全框架，通过用户行为触发大模型、两阶段推测解码和迭代检索机制，在降低47.9%延迟和46.3%大模型使用量的同时，将准确率提升8.9%。


<details>
  <summary>Details</summary>
Motivation: 现有代码补全方法存在权衡：大语言模型(LLMs)质量高但延迟高，小语言模型(SLMs)速度快但效果差。需要平衡高准确率和低延迟之间的矛盾。

Method: 提出MCCom框架，级联本地SLM和云端LLM；利用用户行为作为触发信号，仅在SLM失败时调用LLM；采用两阶段推测解码策略和迭代检索机制增强模型协作；训练了一个121M参数的轻量级模型。

Result: 在RepoEval和新的真实世界基准StmtEval上评估，MCCom将推理延迟降低47.9%，LLM使用量减少46.3%，同时通过有效协作将LLM的精确匹配率提升8.9%。121M参数模型达到7B参数SOTA模型73.8%的性能。

Conclusion: MCCom通过级联架构和创新的触发机制，有效解决了代码补全中准确率与延迟的权衡问题，实现了高质量、低延迟的代码补全解决方案。
...
### [3] [Detecting Semantic Alignments between Textual Specifications and Domain Models](https://arxiv.org/abs/2603.06037)
*Shwetali Shimangaud,Lola Burgueño,Rijul Saini,Jörg Kienzle*

Main category: cs.SE

TL;DR: 该研究提出了一种使用NLP和LLM技术自动评估领域模型与文本规范对齐程度的方法，能够以高精度识别模型元素的正确性。


<details>
  <summary>Details</summary>
Motivation: 在软件工程早期阶段，从文本规范中推导出正确的领域模型并建立清晰的链接是一项具有挑战性的任务，特别是对于新手建模者来说。需要一种自动化的方法来评估领域模型与文本规范的对齐程度。

Method: 使用自然语言处理技术预处理文本，为每个模型元素生成人工自然语言规范描述，然后使用大型语言模型将生成的描述与原始规范中的匹配句子进行比较。算法将每个模型元素分类为对齐（正确）、未对齐（错误）或未分类（证据不足），并输出支持分类的相关句子。
...
### [4] [Real-World Fault Detection for C-Extended Python Projects with Automated Unit Test Generation](https://arxiv.org/abs/2603.06107)
*Lucas Berg,Lukas Krodinger,Stephan Lukasczyk,Annibale Panichella,Gordon Fraser,Wim Vanhoof,Xavier Devroey*

Main category: cs.SE

TL;DR: 该论文提出了一种通过子进程执行来隔离Python C扩展崩溃的方法，使自动化测试生成能够检测和复现原生代码崩溃，而不中断测试过程。


<details>
  <summary>Details</summary>
Motivation: Python库中的C扩展虽然能提高性能，但C代码中的异常可能绕过Python的异常处理机制，导致整个解释器崩溃。这些崩溃在调用公共API时是真正的故障。自动化测试生成本应检测此类故障，但原生代码的崩溃会完全停止测试进程，阻碍故障的检测、复现以及非崩溃代码的覆盖。

Method: 将测试生成过程分为生成和执行两个阶段，改进Pynguin（Python自动化测试用例生成工具）以使用子进程执行。每个生成的测试都在隔离的子进程中执行，防止崩溃中断测试生成过程本身。

Result: 在包含21个流行Python库的1648个模块的数据集上评估该方法。子进程执行使自动化测试能够覆盖多达56.5%的额外模块，发现了213个独特的崩溃原因，揭示了32个先前未知的故障。

Conclusion: 通过子进程执行隔离Python C扩展崩溃的方法有效解决了原生代码崩溃中断测试过程的问题，能够检测故障、生成可复现的崩溃测试用例、研究底层故障，并实现对非崩溃代码的测试覆盖。
...
### [5] [Can Adjusting Hyperparameters Lead to Green Deep Learning: An Empirical Study on Correlations between Hyperparameters and Energy Consumption of Deep Learning Models](https://arxiv.org/abs/2603.06195)
*Taoran Wang,Yanhui Li,Mingliang Ma,Lin Chen,Yuming Zhou*

Main category: cs.SE

TL;DR: 该论文提出从超参数角度分析深度学习模型能耗的新视角，通过实验发现超参数与能耗存在相关性，合理调整超参数可在不影响性能的情况下降低能耗，实现绿色深度学习。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型的发展，数据集增大和模型结构复杂化导致计算资源和能耗急剧上升，需要关注绿色深度学习模型。论文旨在从超参数对能耗影响这一新视角进行分析。

Method: 使用变异算子模拟实践者调整超参数（如训练轮数和学习率）的过程，分别训练原始模型和变异模型，收集能耗信息和运行时性能指标，并重点关注并行训练多个DL模型的场景。

Result: 在五个真实世界DL模型上的实验表明：(1) 许多超参数与能耗存在正负相关性；(2) 调整超参数可使DL模型更绿色，即在保持性能的同时降低能耗；(3) 在并行环境中，能耗对变化更加敏感。

Conclusion: 建议在开发DL模型时更加关注超参数，因为适当调整超参数可以产生更绿色的深度学习模型，降低能耗而不损害性能。
...
### [6] [Story Point Estimation Using Large Language Models](https://arxiv.org/abs/2603.06276)
*Pranam Prakash Shetty,Adarsh Balakrishnan,Mengqiao Xu,Xiaoyin Xi,Zhe Yu*

Main category: cs.SE

TL;DR: 本研究探索使用大语言模型进行故事点估算，发现零样本提示下LLMs表现优于需要80%训练数据的监督深度学习模型，少量示例可进一步提升性能，比较判断可作为少样本示例改善预测效果。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型需要大量项目特定训练数据才能准确预测故事点，这促使研究者探索LLMs是否能在无训练数据或仅有少量数据的情况下进行有效预测，同时研究比较判断是否比直接估算故事点更容易预测。

Method: 使用四种大语言模型在16个软件项目上进行实验，采用零样本提示和少样本提示方法，同时探索比较判断作为少样本示例的有效性，并与监督深度学习模型进行对比。

Result: 零样本提示下LLMs预测故事点的表现优于使用80%训练数据的监督深度学习模型；少样本提示可进一步提升性能；比较判断并不比直接估算故事点更容易预测，但可作为有效的少样本示例改善预测效果。

Conclusion: 大语言模型在故事点估算任务中表现出色，无需大量训练数据即可获得良好性能，比较判断可作为有效的少样本示例，为敏捷开发中的工作量估算提供了新方法。
...
### [7] [A Generalized Feature Model for Digital Twins](https://arxiv.org/abs/2603.06308)
...
<details>
  <summary>Details</summary>
Motivation: 数字孪生技术在各个领域迅速扩展，已有大量研究探讨其性质、应用和优势，但至今缺乏一个全面的特征模型来捕捉数字孪生的必需和可选特征。

Method: 基于对现有文献的系统性映射研究，开发了适用于数字模型、数字影子和数字孪生的通用特征模型，并通过应急、车辆和制造领域的三个用例验证模型的有效性。

Result: 成功开发了一个通用的数字孪生特征模型，该模型能够有效捕捉数字孪生的关键特征，并通过实际应用案例验证了其可行性和实用性。

Conclusion: 提出的通用特征模型通过促进设计阶段的知情决策、改进数字孪生的模型驱动开发，以及为测试用例推断提供基于模型的基础，推动了数字孪生领域的发展。
...
### [8] [A Scalable Benchmark for Repository-Oriented Long-Horizon Conversational Context Management](https://arxiv.org/abs/2603.06358)
*Yang Liu,Li Zhang,Fang Liu,Ping Lin,Xinyi Li*

Main category: cs.SE

TL;DR: LoCoEval：首个面向仓库开发场景的长对话上下文管理评测基准，通过LLM驱动流程生成真实多样的仓库对话，评估现有方法在仓库开发场景中的局限性，并提出改进方案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码理解与生成方面进步显著，但在实际仓库开发中，过长的对话上下文可能导致关键信息丢失和性能下降。现有上下文管理方法主要针对通用对话，缺乏面向仓库开发的解决方案和可靠的评测基准。

Method: 提出LoCoEval基准，遵循三个关键原则，通过LLM驱动流程生成真实多样的仓库导向对话，捕捉迭代需求、噪声输入和回顾性提问等关键交互模式。评估了7个基线方法（包括4种代表性上下文管理方法）和3种先进骨干LLM。

Result: 评估结果显示，独立LLM和现有方法（特别是记忆系统）在仓库导向对话场景中面临重大挑战。提出的改进方法将对话和仓库信息整合到统一记忆中，在所有基线方法（除Oracle外）中表现最佳，并展现出鲁棒性。

Conclusion: LoCoEval填补了仓库开发场景中长对话上下文管理评测的空白，揭示了现有方法的局限性，提出的改进方法表现出色，为未来研究提供了可操作的见解。
...
### [9] [A Reference Architecture of Reinforcement Learning Frameworks](https://arxiv.org/abs/2603.06413)
*Xiaoran Liu,Istvan David*

Main category: cs.SE

TL;DR: 本文提出强化学习框架的参考架构，通过分析18个现有框架识别核心组件和关系，为框架比较、评估和集成提供统一基础。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习框架缺乏统一的参考架构，导致实现模式不一致，难以进行比较、评估和集成。需要建立共同基础来解决这一问题。

Method: 采用扎根理论方法分析18个最先进的强化学习框架，识别重复出现的架构组件及其关系，并将其编码为参考架构。

Result: 成功构建了强化学习框架的参考架构，能够重构特征性强化学习模式，识别出常用组件等架构趋势。

Conclusion: 提出的参考架构为强化学习框架提供了统一的比较基础，有助于改进框架设计和集成，并指出了未来改进方向。
...
### [10] [Understanding and Finding JIT Compiler Performance Bugs](https://arxiv.org/abs/2603.06551)
*Zijian Yi,Cheng Ding,August Shi,Milos Gligoric*

Main category: cs.SE

TL;DR: 本文首次系统研究JIT编译器性能缺陷，通过实证分析191个缺陷报告总结模式，并开发Jittery工具自动检测性能缺陷，在HotSpot和Graal中发现12个新缺陷。


<details>
  <summary>Details</summary>
Motivation: JIT编译器是Java、JavaScript等语言的关键组件，但现有研究主要关注功能缺陷检测，缺乏对性能缺陷的研究。性能缺陷会导致运行时显著性能下降，但由于JIT编译器的复杂性和动态性，检测这些缺陷具有挑战性。

Method: 1. 对四个流行Java和JavaScript JIT编译器进行实证研究，手动分析191个缺陷报告，总结性能缺陷的触发条件、表现模式和根本原因；2. 基于这些洞察提出分层差分性能测试技术，实现为Jittery工具，包含测试优先级排序（减少92.40%测试时间）和自动过滤误报/重复等优化。

Result: 通过Jittery在Oracle HotSpot和Graal JIT编译器中发现了12个先前未知的性能缺陷，其中11个得到开发者确认，6个已修复。工具通过优化显著减少了测试时间和人工检查工作量。

Conclusion: 这是首个系统研究JIT编译器性能缺陷的工作，通过实证分析揭示了性能缺陷的模式，提出的Jittery工具能有效自动检测性能缺陷，为JIT编译器质量保障提供了新方法。
...
### [11] [VDCook:DIY video data cook your MLLMs](https://arxiv.org/abs/2603.05539)
*Chengwei Wu*

Main category: cs.LG

TL;DR: VDCook是一个自演进的视频数据操作系统，通过自然语言查询和参数配置，自动执行视频检索和可控合成，生成带有完整溯源和元数据的领域特定视频数据集。


<details>
  <summary>Details</summary>
Motivation: 传统静态数据集一次性构建后难以更新和扩展，研究人员和垂直领域团队构建专业视频训练数据集的门槛较高，需要基础设施层面的解决方案来降低这一门槛。

Method: 用户通过自然语言查询和可调参数发起数据请求，系统自动进行查询优化，并行运行真实视频检索和可控合成模块，基于MCP协议实现自动化数据摄入，并提供多维元数据标注。

Result: 系统生成带有完整溯源和元数据的领域数据包及可复现的Notebook，将数据集转变为动态演进的开放生态系统，支持社区贡献和治理驱动的数据扩展范式。
...
### [12] [IntSeqBERT: Learning Arithmetic Structure in OEIS via Modulo-Spectrum Embeddings](https://arxiv.org/abs/2603.05556)
*Kazuhisa Nakasho*
...
<details>
  <summary>Details</summary>
Motivation: OEIS整数序列值域跨度大（从个位数到天文级阶乘和指数），传统分词模型无法处理词汇表外值，也无法利用周期性算术结构，需要专门设计模型
...
Result: 大型模型（9150万参数）在测试集上达到95.85%幅度准确率和50.38%平均模数准确率，比基准分词Transformer分别提升8.9和4.5个百分点；下一项预测Top-1准确率19.09% vs 2.59%，提升7.4倍；模数谱分析显示NIG与欧拉函数比φ(m)/m强负相关（r=-0.851）

Conclusion: IntSeqBERT通过双流架构有效捕捉整数序列的幅度和模数结构，CRT求解器显著提升预测性能，复合模数通过CRT聚合更高效地捕获OEIS算术结构
...
### [13] [Autocorrelation effects in a stochastic-process model for decision making via time series](https://arxiv.org/abs/2603.05559)
*Tomoki Yamagami,Mikio Hasegawa,Takatomo Mihana,Ryoichi Horisaki,Atsushi Uchida*

Main category: cs.LG

TL;DR: 该研究分析了基于时间序列的决策模型，发现自相关特性对多臂赌博机问题的决策精度有重要影响，且最优自相关方向取决于环境奖励丰富程度。


<details>
  <summary>Details</summary>
Motivation: 先前实验发现利用半导体激光器产生的光子混沌动力学解决多臂赌博机问题时，决策精度强烈依赖于时间序列的自相关特性，但缺乏最小数学模型来解释这种自相关的好处。

Method: 研究者分析了基于时间序列的决策随机过程模型，采用拔河原理解决两臂赌博机问题，其中阈值和双值马尔可夫信号共同演化。

Result: 数值结果显示环境依赖的结构：在奖励丰富的环境中负自相关最优，在奖励贫乏的环境中正自相关最优。当获胜概率之和大于1时负自相关有利，小于1时正自相关有利，等于1时性能与自相关无关。

Conclusion: 该研究为改进无线通信和机器人强化学习应用中的决策方案铺平了道路，揭示了自相关特性在决策过程中的重要作用及其与环境条件的依赖关系。
...
TL;DR: 生成式AI在压缩任务内技能差异的同时，将经济价值转向集中的互补资产，形成悖论：使个体表现均等化的技术可能加剧总体不平等


<details>
  <summary>Details</summary>
Motivation: 研究生成式AI对劳动力市场的双重影响：一方面压缩个体技能差异，另一方面通过价值转移加剧总体不平等，需要理论框架解释这一悖论

Method: 建立包含内生教育、雇主筛选和异质性企业的任务基础模型；使用模拟矩估计方法匹配六个经验目标；通过BLS OEWS数据进行职业层面回归分析

Result: 模型识别出两种制度边界取决于AI技术结构（专有vs商品化）和劳动力市场制度；校准参数显示总体不平等方向由特定参数决定；职业层面数据无法检验任务层面预测

Conclusion: 主要贡献是提出机制而非确定符号；需要在任务层面面板数据进行检验，但目前缺乏大规模此类数据
...
### [15] [Aligning the True Semantics: Constrained Decoupling and Distribution Sampling for Cross-Modal Alignment](https://arxiv.org/abs/2603.05566)
*Xiang Ma,Lexin Fang,Litian Xu,Caiming Zhang*

Main category: cs.LG

TL;DR: CDDS提出了一种新的跨模态对齐方法，通过约束解耦和分布采样来解决传统方法中忽略非语义信息和模态差距的问题，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统跨模态对齐方法追求嵌入一致性来实现语义一致性，但忽略了嵌入中的非语义信息。直观的解决方案是将嵌入解耦为语义和模态分量，仅对齐语义分量，但这面临两个挑战：缺乏区分语义和模态信息的标准，以及模态差距可能导致语义对齐偏差或信息丢失。

Method: 提出CDDS算法：1）引入双路径UNet自适应解耦嵌入，应用多重约束确保有效分离；2）提出分布采样方法桥接模态差距，确保对齐过程的合理性。

Result: 在多个基准测试和模型骨干上的广泛实验表明，CDDS方法优于现有最先进方法6.6%到14.2%。
...
### [16] [Why Depth Matters in Parallelizable Sequence Models: A Lie Algebraic View](https://arxiv.org/abs/2603.05573)
*Gyuryang Heo,Timothy Ngotiaoco,Kazuki Irie,Samuel J. Gershman,Bernardo Sabatini*

Main category: cs.LG

TL;DR: 论文从李代数控制视角分析序列模型的表达能力边界，建立了模型深度与李代数扩展塔的对应关系，推导出近似误差随深度指数衰减的边界
...
Method: 采用李代数控制理论视角，建立序列模型深度与李代数扩展塔的对应关系，分析恒定深度序列模型的李代数类别及其表达能力边界，解析推导近似误差边界

Result: 理论分析表明误差随深度增加呈指数衰减，这与这些模型强大的实证性能一致。通过在符号词和连续值状态跟踪问题上的实验验证了理论预测

Conclusion: 从李代数控制视角为序列模型提供了理论框架，揭示了深度与表达能力之间的指数关系，为理解现代序列模型的强大性能提供了理论依据
...
### [17] [A Novel Hybrid Heuristic-Reinforcement Learning Optimization Approach for a Class of Railcar Shunting Problems](https://arxiv.org/abs/2603.05579)
*Ruonan Zhao,Joseph Geunes*

Main category: cs.LG

TL;DR: 本文提出了一种混合启发式-强化学习框架，用于解决具有双侧轨道访问的铁路货车编组调度问题，通过分解为单侧子问题并集成领域启发式与Q学习来提高求解效率。


<details>
  <summary>Details</summary>
Motivation: 铁路货车编组是货运站场的核心规划任务，其中单侧轨道遵循后进先出原则，双侧轨道遵循先进先出原则。现有方法在处理具有双侧轨道访问和两台机车的复杂编组问题时面临组合爆炸挑战，需要更高效的求解方法。

Method: 将双侧访问问题分解为两个单侧访问子问题，每个子问题对应一台机车。提出混合启发式-强化学习框架，集成铁路领域特定的启发式方法与Q学习算法，通过减少状态-动作空间和引导探索来优化学习过程。

Result: 数值实验结果表明，HHRL算法在单侧访问单机车问题和双侧访问双机车问题中均表现出高效性和高质量的解，验证了所提框架的有效性。

Conclusion: 混合启发式-强化学习框架能够有效解决复杂的铁路货车编组调度问题，通过领域知识与强化学习的结合，在减少状态空间的同时保持解的质量，为实际铁路运营提供了实用的优化方法。
...
TL;DR: 本文研究了tokenizer预训练对物理仿真模型准确性和效率的影响，发现在相同物理系统上进行预训练能显著提升性能，并提出了灵活时空压缩操作
...
Method: 采用tokenizer预训练方法，先用自编码目标预训练tokenizer，再训练动力学模型。引入灵活的时空压缩操作，扩展因果卷积以支持运行时可调的压缩比。

Result: 领域对齐的预训练效果最佳：在同物理系统上预训练使VRMSE在10,500训练步后降低64%；在其他系统上预训练也有中等提升。这是首次系统研究物理基础模型的tokenizer预训练。

Conclusion: tokenizer预训练能显著提升物理仿真器的计算效率和准确性，预训练数据的选择策略至关重要，为训练高效物理仿真器提供了实用指导。
...
TL;DR: 论文提出一个识别攻击者特征的框架，而非防御攻击本身，通过分析观察到的攻击来推断攻击者信息，帮助防御者进行针对性防御。


<details>
  <summary>Details</summary>
Motivation: 现有防御机制主要关注攻击本身（如对抗正则化）或系统层面（如异常检测），但忽略了攻击者的特征信息。本文认为识别攻击者特征能为防御提供额外优势，包括外部缓解和针对性防御。

Method: 提出一个领域无关的框架，从观察到的攻击中识别最可能的攻击者。首先证明在没有额外知识的情况下攻击者是不可识别的，然后通过框架解决这一挑战。框架可应用于多种学习器，帮助防御者利用攻击者信息进行外部缓解和针对性防御。

Result: 证明了攻击者的非可识别性，但提出的框架能够识别最可能的攻击者。通过在不同学习器上的具体实例化展示了框架的适用性，表明攻击者信息可以改善防御性能。

Conclusion: 关注攻击者而非攻击本身是一种有效的防御策略。提出的框架能够从观察到的攻击中推断攻击者特征，为防御者提供额外信息来改善防御效果，包括外部缓解和针对性防御。
...
TL;DR: 通过构建包含场上和场外数据的知识图谱，将其嵌入向量空间，并纳入表格数据，显著提升了NBA球员薪资预测的准确性，特别是对于老将和高薪球员。


<details>
  <summary>Details</summary>
Motivation: NBA球员市场估值困难，传统基于表格数据的监督学习方法对年轻球员有效，但对老将和高薪球员预测效果不佳，需要更全面的因素理解。

Method: 构建包含场上和场外数据的知识图谱，使用多种图嵌入算法将图谱嵌入向量空间，然后将这些向量特征纳入传统的表格数据中进行监督学习。

Result: 比较了多种图嵌入算法，证明将知识图谱向量纳入表格数据对NBA薪资预测至关重要，能够更好地理解影响薪资的各种因素。

Conclusion: 知识图谱嵌入方法显著提升了NBA球员薪资预测的准确性，特别是解决了传统方法对老将和高薪球员预测效果不佳的问题。
...
### [21] [Improved Scaling Laws via Weak-to-Strong Generalization in Random Feature Ridge Regression](https://arxiv.org/abs/2603.05691)
*Diyuan Wu,Lehan Chen,Theodor Misiakiewicz,Marco Mondelli*

Main category: cs.LG

TL;DR: 论文研究了机器学习中弱到强泛化现象，发现通过弱教师模型生成标签训练强学生模型可以显著改善测试误差的缩放规律，学生甚至能达到极小极大最优速率。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习中普遍采用两阶段训练流程：先用学习模型标注数据，再用这些数据训练更强大的模型。弱到强泛化现象表明，即使使用弱教师生成的不完美标签，强学生也能超越教师性能。本文旨在量化这种改进的潜力，特别是它如何影响测试误差的缩放规律。

Method: 使用随机特征岭回归（RFRR）框架分析教师和学生模型。主要技术贡献是推导了学生在教师生成标签上训练时的测试误差超额的确定性等价表达式。通过这个表达式，识别出学生缩放规律优于教师的各种机制。

Result: 研究发现学生可以在偏置主导和方差主导两种设置下实现缩放规律的改进。令人惊讶的是，无论教师的缩放规律如何，学生都能达到极小极大最优速率——即使教师的测试误差不随样本量衰减时也是如此。

Conclusion: 弱到强泛化现象具有显著的改进潜力，能够影响测试误差的缩放规律。通过随机特征岭回归框架的分析表明，学生模型可以在各种机制下超越教师性能，甚至达到理论最优速率，这为两阶段训练流程提供了理论支持。
...
TL;DR: 该研究使用无监督域适应技术，通过最小化源域和目标域特征向量的最大均值差异，提高了基于合成数据训练的放射性核素识别模型在真实实验数据上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 放射性核素识别面临实验数据获取和标注困难的问题，虽然可以使用模拟数据训练模型，但模型在真实环境部署时性能会显著下降。需要找到一种方法让基于合成数据训练的模型能够适应真实操作环境。

Method: 首先使用标记的合成数据预训练光谱分类器，然后利用目标域的无标记数据，通过无监督域适应技术对齐源域和目标域的特征表示。比较了多种UDA方法，发现最小化源域和目标域特征向量之间的最大均值差异效果最好。

Result: 通过MMD最小化进行无监督特征对齐后，在实验LaBr₃测试集上的准确率从0.754±0.014提高到0.904±0.022，显著提升了模型在真实数据上的性能。

Conclusion: 无监督域适应技术能够有效提升基于合成数据训练的放射性核素分类器在真实世界部署中的性能，为实际应用提供了可行解决方案。
...
TL;DR: 本文重新评估了Best-of-N采样方法，发现在实际条件下（使用胜率而非期望奖励作为指标），适当调优的BoN在统计和计算上都是最优的，并提出了一个能消除奖励攻击的改进版本。


<details>
  <summary>Details</summary>
Motivation: 尽管Best-of-N采样在实践中被广泛使用，但先前理论研究认为它在统计上不是最优的且容易受到奖励攻击。本文旨在在实际条件下重新评估BoN的性能，使用更贴近实践的评价指标（胜率而非期望奖励）。

Method: 本文在更贴近实践的条件下分析BoN方法，特别关注胜率这一成对比较指标。提出了一个简单实用的BoN变体，该变体在理论上能够消除奖励攻击问题，同时保持最优的统计性能。

Result: 研究表明，在参考模型和奖励模型质量满足最小条件的情况下，适当调优的BoN在实现高胜率方面既是计算最优的也是统计最优的。提出的改进版本能够消除奖励攻击问题，而先前的方法在考虑胜率时被证明是次优的。

Conclusion: BoN方法的实际成功部分可以通过其在胜率方面的最优性来解释。选择合适的评估目标（胜率而非期望奖励）对于分析推理时对齐方法至关重要。提出的改进BoN变体既保持了最优性能又解决了奖励攻击问题。
...
TL;DR: MIRACL是一个分层元多目标强化学习框架，通过任务分解和帕累托适应策略实现少样本泛化，在供应链优化任务中优于传统MORL方法。


<details>
  <summary>Details</summary>
Motivation: 多目标强化学习在多梯队组合供应链优化中有效，但在动态环境中部署面临任务特定重训练和计算成本高的挑战，需要能够快速适应新任务的解决方案。
...
Conclusion: MIRACL展示了在多目标问题中实现鲁棒、高效适应的潜力，虽然是针对供应链领域验证，但理论上具有领域无关性，适用于更广泛的动态多目标决策问题。
...
TL;DR: SGPP提出了一种统一框架，通过近端投影平衡确定性优化和随机采样，解决RF模型在精确控制任务中的几何锁定和计算不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: Rectified Flow模型在生成质量上表现出色，但在精确控制任务（如语义编辑或盲图像恢复）中存在挑战。现有方法要么受限于"几何锁定"问题（如反转引导），要么计算昂贵且不稳定（如后验采样近似）。

Method: 提出Score-Guided Proximal Projection (SGPP)框架，将恢复任务重新表述为近端优化问题，定义了一个平衡输入保真度和预训练分数场真实性的能量景观。该方法理论上证明了正常收缩特性，确保分布外输入被映射到数据流形上。

Result: SGPP统一了最先进的编辑方法，RF反转是其框架的一个极限情况。通过放松近端方差，SGPP实现了"软引导"，在严格身份保持和生成自由度之间提供了连续、无需训练的权衡。

Conclusion: SGPP框架弥合了确定性优化和随机采样之间的差距，通过理论保证的几何收缩特性，为RF模型提供了更灵活、稳定的精确控制能力，在图像编辑和恢复任务中具有重要应用价值。
...
TL;DR: 本文提出SCORE方法解决模型合并中的领域泛化问题，通过奇异子空间冲突消解提升未见领域的泛化性能


<details>
  <summary>Details</summary>
Motivation: 当前模型合并研究主要关注同分布或多任务场景，但在领域泛化方面的探索不足。本文旨在研究在不同领域上微调的模型合并后对未见领域的泛化能力，并解决由此产生的参数冲突问题。

Method: 提出SCORE（Subspace COnflict-Resolving mErging）方法：1）通过奇异值分解分析任务矩阵的参数竞争；2）计算所有模型前导奇异向量的主成分来找到共享正交基；3）将每个任务矩阵投影到共享基上，并修剪非对角分量以消除冲突的奇异方向。

Result: SCORE在多种架构和模型规模上，在领域泛化设置中平均优于现有的模型合并方法，证明了其有效性和可扩展性。

Conclusion: 模型合并中的领域泛化存在独特的挑战，特别是不同分布偏移导致的子空间冲突。SCORE方法通过奇异子空间冲突消解有效提升了合并模型在未见领域的泛化能力。
...
### [27] [First-Order Softmax Weighted Switching Gradient Method for Distributed Stochastic Minimax Optimization with Stochastic Constraints](https://arxiv.org/abs/2603.05774)
*Zhankun Luo,Antesh Upadhyay,Sang Bin Moon,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 提出Softmax-Weighted Switching Gradient方法解决联邦学习中的分布式随机极小极大优化问题，在完全和部分客户端参与下均实现高效收敛，避免传统方法的超参数敏感性和振荡问题。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中分布式随机极小极大优化问题，特别是针对带随机约束的场景。传统的主-对偶或惩罚方法存在超参数敏感性和收敛振荡问题，需要更稳定的优化框架来优化最差客户端性能。

Method: 提出Softmax-Weighted Switching Gradient方法，这是一种单循环仅主变量切换机制。通过softmax加权和切换梯度策略，在联邦学习框架下处理随机约束的极小极大优化问题。理论分析包括完全客户端参与和部分参与两种场景。

Result: 算法在完全客户端参与下达到标准O(ε⁻⁴)的oracle复杂度，满足最优性间隙和可行性容忍度的统一界ε。在部分参与下，通过量化客户端采样噪声建立高概率收敛保证。实验在Neyman-Pearson分类和公平分类任务上验证了有效性。

Conclusion: 单循环仅主变量切换机制为优化最差客户端性能提供了稳定替代方案，有效避免了传统主-对偶或惩罚方法的超参数敏感性和收敛振荡问题，在联邦学习环境中具有实际应用价值。
...
### [28] [Sparse Crosscoders for diffing MoEs and Dense models](https://arxiv.org/abs/2603.05805)
*Marmik Chaudhari,Nishkal Hundia,Idhant Gulati*

Main category: cs.LG

TL;DR: 通过交叉编码器比较MoE和密集模型内部表示，发现MoE学习更少的独特特征，特征更专业化且集中，而密集模型特征更广泛通用


<details>
  <summary>Details</summary>
Motivation: 虽然混合专家模型通过稀疏专家路由实现了参数高效扩展，但其内部表示相比密集模型仍缺乏深入理解，需要系统比较两者的内部特征组织方式

Method: 使用交叉编码器（稀疏自编码器变体）联合建模多个激活空间，在代码、科学文本和英文故事上训练5层密集和MoE模型（等量活跃参数），采用BatchTopK交叉编码器并明确指定共享特征

Result: 达到约87%的分数方差解释率，发现MoE学习显著更少的独特特征；MoE特定特征比共享特征具有更高的激活密度，而密集模型特定特征密度较低

Conclusion: MoE发展出更专业化、集中的表示，而密集模型在更广泛、通用的特征间分布信息，揭示了两种架构在特征组织上的根本差异
...
### [29] [Reference-guided Policy Optimization for Molecular Optimization via LLM Reasoning](https://arxiv.org/abs/2603.05900)
*Xuan Li,Zhanke Zhou,Zongze Li,Jiangchao Yao,Yu Rong,Lu Zhang,Bo Han*

Main category: cs.LG

TL;DR: RePO是一种用于分子优化的新方法，结合了强化学习和参考指导，解决了传统SFT和RLVR在分子优化任务中的局限性，无需轨迹数据即可从参考分子中学习。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调(SFT)和基于可验证奖励的强化学习(RLVR)在分子优化任务中表现不佳，因为分子优化数据通常只提供单个优化参考分子，缺乏逐步优化轨迹。仅基于答案的SFT会导致推理能力崩溃，而RLVR在相似性约束下提供稀疏反馈，限制了模型的有效探索。

Method: 提出Reference-guided Policy Optimization (RePO)方法：1) 从模型中采样候选分子及其中间推理轨迹；2) 使用可验证奖励（在相似性约束下测量属性满足度）以强化学习方式训练模型；3) 应用参考指导，将策略的中间推理轨迹作为上下文，仅以监督方式训练答案部分。RL项促进探索，指导项缓解奖励稀疏性并稳定训练。

Result: 在分子优化基准测试中，RePO始终优于SFT和RLVR基线（如GRPO），在优化指标（成功率×相似度）上取得改进，更好地平衡了竞争目标，并对未见过的指令风格具有更好的泛化能力。

Conclusion: RePO通过结合强化学习和参考指导，有效解决了分子优化中探索与利用的平衡问题，无需轨迹数据即可从参考分子中学习，在多个基准测试中表现出优越性能。
...
TL;DR: 该论文系统分析了MoE模型中的专家专业化行为，发现尽管模型有64个路由专家，但主要依赖少数几个专业专家，单个顶级专家的输出就能近似完整集成预测，为推理优化提供了机会。


<details>
  <summary>Details</summary>
Motivation: MoE模型通过稀疏专家激活实现参数高效扩展，但由于对其专业化行为的理解有限，优化推理和内存成本仍然具有挑战性。需要系统分析专家专业化行为以发现优化机会。

Method: 采用两种互补方法：1) 领域特定路由模式分析；2) 早期解码框架，跟踪专家对输出表示的贡献。分析DeepSeekMoE模型，研究64个路由专家中每个层激活6个专家的行为。

Result: 研究发现：1) 模型主要依赖少数专业专家，顶级专家输出与完整集成预测高度相似；2) 少数专家处理超过50%的路由决策；3) 单专家与集成专家隐藏状态相似度极高（余弦相似度高达0.95）；4) 跨所有三个领域使用单个专家时困惑度仅增加5%。
...
### [31] [Stock Market Prediction Using Node Transformer Architecture Integrated with BERT Sentiment Analysis](https://arxiv.org/abs/2603.05917)
...
<details>
  <summary>Details</summary>
Motivation: 股票市场预测面临噪声、非平稳性和行为动态等挑战，传统方法难以捕捉金融市场中复杂的模式和横截面依赖关系。

Method: 提出集成框架：1) 将股票市场表示为图结构，股票为节点，边捕捉行业关联、价格相关性等关系；2) 使用微调BERT模型从社交媒体提取情感特征；3) 通过注意力机制融合情感与量化市场特征；4) 节点Transformer处理历史市场数据，捕捉时间演化和横截面依赖。

Result: 在1982年1月至2025年3月的20只S&P 500股票上测试：集成模型的一日预测MAPE为0.80%，优于ARIMA(1.20%)和LSTM(1.00%)；情感分析使预测误差降低10%（财报期间降低25%）；图建模贡献额外15%改进；方向准确性达65%；高波动期MAPE保持在1.5%以下（基准模型超过2%）。

Conclusion: 集成节点Transformer和BERT情感分析的框架能有效捕捉股票市场的时间演化、横截面依赖和情感因素，显著提升预测准确性，特别是在高波动期和财报期间表现优异。
...
TL;DR: SEA-PEFT：一种自动化的参数高效微调方法，通过搜索-审计-分配循环解决临床站点适配中的领域偏移和标注稀缺问题，在少样本3D医学图像分割任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 基础模型在临床站点间的适配面临挑战：领域偏移和稀缺标注需要专家处理，但许多临床团队缺乏AI工程师来调整适配器设计和训练方案，导致适配周期长达数周到数月。现有PEFT方法要么需要手动配置适配器，要么自动化搜索在少样本3D设置中计算不可行。

Method: SEA-PEFT将适配器配置视为在线分配问题，通过搜索-审计-分配循环解决：训练活跃适配器，通过暂时关闭每个适配器来估计其Dice效用，然后使用贪心背包分配器在参数预算下重新选择活跃集。采用指数移动平均和四分位距平滑，结合有限状态排序控制器来稳定循环并提高高噪声少样本环境下的可靠性。

Result: 在TotalSegmentator和FLARE'22数据集上，SEA-PEFT在1/5/10-shot设置中，相比最强的固定拓扑PEFT基线，平均Dice提升了2.4-2.8个点，同时训练参数少于1%。

Conclusion: SEA-PEFT提供了一种自动化、高效的临床站点适配解决方案，显著减少了人工干预需求，在少样本3D医学图像分割任务中表现出优越性能，为临床部署提供了实用工具。
...
TL;DR: 该论文实证研究了大语言模型中的多示例提示测试时适应方法，分析了其在不同任务和模型中的效果、限制及可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 尽管多示例提示作为测试时适应方法能提升大语言模型性能，但其可靠性和实际限制尚未得到充分理解，特别是在开源模型中。研究者希望探究这种输入空间更新机制的有效边界。

Method: 通过实证研究分析多示例提示在不同任务和模型架构中的表现，考察更新规模、示例顺序和选择策略的影响。同时研究动态ICL和强化ICL作为替代测试时更新策略。

Result: 多示例提示在结构化任务中有效（演示提供高信息增益），但对选择策略高度敏感，在开放式生成任务中效果有限。输入空间更新在某些情况下有益，在某些情况下有害。

Conclusion: 研究揭示了基于提示的测试时适应的实际限制，明确了输入空间更新在何种情况下有益或有害，为实际应用提供了指导。
...
### [34] [Partial Policy Gradients for RL in LLMs](https://arxiv.org/abs/2603.06138)
*Puneet Mathur,Branislav Kveton,Subhojyoti Mukherjee,Viet Dac Lai*

Main category: cs.LG

TL;DR: 该论文提出了一种在策略梯度中建模策略结构的新方法，通过优化未来奖励的子集来学习更简单可靠的策略，并在对话对齐问题上验证了不同策略类的性能差异。


<details>
  <summary>Details</summary>
Motivation: 强化学习需要在未知环境中顺序学习行动策略，但现有方法缺乏对策略结构进行系统建模和比较的框架。研究者希望开发一种能够建模不同复杂度策略的方法，以便根据问题特性选择最合适的策略类。

Method: 提出在策略梯度中通过优化未来奖励的子集来建模策略结构：较小的奖励子集对应更简单的策略，这些策略的经验梯度估计更准确、学习更可靠。方法支持建模和比较多种策略类，包括完全规划、贪婪、K步前瞻和分段策略。

Result: 在多个角色对齐对话问题上进行实证评估，发现不同策略在不同问题上表现优异，这反映了它们各自的特点，并突显了所研究策略类的重要性。

Conclusion: 通过优化未来奖励子集来建模策略结构是一种有效的方法，能够系统比较不同复杂度策略的性能。不同策略类在不同问题上的优势表明，根据问题特性选择合适的策略结构对于强化学习应用至关重要。
...
TL;DR: 提出Artificial Replay方法，通过重用第一个策略的记录来减少多臂老虎机算法比较的实验成本，只需T+o(T)次用户交互而非2T次，同时保证估计的无偏性和次线性方差增长。


<details>
  <summary>Details</summary>
Motivation: 在线平台需要比较多臂老虎机算法（如UCB、Thompson Sampling），但传统方法需要多次独立重启算法，导致实验成本高昂且部署决策延迟。每个算法在T个用户上运行只产生一个依赖轨迹，因为算法的决策依赖于所有过去的交互。

Method: 提出Artificial Replay（AR）实验设计：首先运行一个策略并记录其轨迹；当执行第二个策略时，如果它选择第一个策略已经采取过的动作，就重用记录中的奖励，否则才查询真实环境。开发了新的分析框架来研究这种设计。

Result: 证明了AR估计器的三个关键特性：1）无偏性；2）只需T+o(T)次用户交互而非2T次，当两个策略都有次线性遗憾时几乎减半实验成本；3）方差在T中次线性增长，而朴素设计的估计器方差线性增长。数值实验验证了这些理论优势。

Conclusion: Artificial Replay为多臂老虎机算法的比较提供了一种高效、可靠的实验设计方法，显著降低了实验成本，同时保证了统计推断的质量，有助于加速在线平台的算法部署决策。
...
### [36] [Learning to Solve Orienteering Problem with Time Windows and Variable Profits](https://arxiv.org/abs/2603.06260)
*Songqun Gao,Zanxi Ruan,Patrick Floor,Marco Roveri,Luigi Palopoli,Daniele Fontanelli*

Main category: cs.LG

TL;DR: DeCoST是一种学习型两阶段解耦离散-连续优化方法，用于解决带时间窗和可变利润的定向问题，通过分离离散和连续决策变量并实现高效协调，在解质量和计算效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前方法无法有效解决同时包含离散和连续变量的定向问题变体OPTWVP，需要开发能够解耦这两种变量并实现高效协调的求解器。

Method: 提出两阶段DeCoST框架：第一阶段使用并行解码结构预测路径和初始服务时间分配；第二阶段通过线性规划优化服务时间并提供长期结构估计学习。严格证明了第二阶段解的全局最优性。

Result: 在OPTWVP实例上的实验表明，DeCoST在解质量和计算效率上均优于最先进的构造性求解器和最新元启发式算法，在少于500个节点的实例上实现高达6.6倍的推理加速，且框架兼容多种构造性求解器。

Conclusion: DeCoST通过有效解耦离散和连续决策变量并实现高效协调，为OPTWVP问题提供了高质量、高效率的解决方案，且具有良好的通用性和扩展性。
...
TL;DR: OMGD是一种基于掩码遍历的内存高效优化方法，在非凸设置下实现了改进的迭代复杂度，并能无缝集成到主流优化器中。


<details>
  <summary>Details</summary>
Motivation: 在GPU内存瓶颈下扩展大语言模型的全参数训练时，现有内存高效优化方法要么缺乏清晰的收敛保证，要么在非凸设置下只能达到标准的O(ε⁻⁴)迭代复杂度。

Method: 提出Omni-Masked Gradient Descent (OMGD)，这是一种基于掩码遍历的优化方法，用于内存高效训练。该方法轻量级、即插即用，能无缝集成到大多数主流优化器中。

Result: 理论分析表明OMGD在非凸设置下实现了改进的迭代复杂度Õ(ε⁻³)，用于找到ε-近似驻点。实证结果显示，在微调和预训练任务中，OMGD相比竞争基线都能带来一致的改进。

Conclusion: OMGD是一种有效的内存高效优化方法，具有理论保证和实证优势，能够改善大语言模型训练的内存效率。
...
### [38] [Stem: Rethinking Causal Information Flow in Sparse Attention](https://arxiv.org/abs/2603.06274)
*Lin Niu,Xin Luo,Linchuan Xie,Yifu Sun,Guanghua Yu,Jianchen Zhu,S Kevin Zhou*

Main category: cs.LG

TL;DR: Stem是一种新的稀疏注意力模块，通过位置衰减策略和输出感知度量来优化因果注意力计算，减少长上下文LLM的预填充阶段计算复杂度


<details>
  <summary>Details</summary>
Motivation: 自注意力的二次计算复杂度限制了大型语言模型扩展到长上下文的能力，特别是在预填充阶段。现有稀疏方法在所有token位置统一应用top-k选择，忽略了因果架构中token信息的累积依赖性

Method: 1. Token Position-Decay策略：在每层应用位置相关的top-k选择，保留初始token以支持递归依赖；2. Output-Aware Metric：基于近似输出幅度优先选择高影响力token

Result: 广泛评估表明，Stem在减少计算量和预填充延迟的同时实现了更优的准确性

Conclusion: Stem是一种即插即用的稀疏模块，与信息流对齐，有效解决了因果注意力机制中的计算瓶颈问题
...
### [39] [From Entropy to Calibrated Uncertainty: Training Language Models to Reason About Uncertainty](https://arxiv.org/abs/2603.06317)
*Azza Jenane,Nassim Walha,Lukas Kuhn,Florian Buettner*

Main category: cs.LG

TL;DR: 提出一个三阶段后训练流程，让大语言模型能够高效地推断校准的不确定性估计，而不是使用计算昂贵的后处理方法


<details>
  <summary>Details</summary>
Motivation: 在高风险领域中，能够表达可解释和校准不确定性的大语言模型至关重要。现有的后处理方法通常是基于采样的，计算成本高或缺乏校准
...
Conclusion: 提出的方法能够提供可解释且计算高效的不确定性估计，与后处理方法相比，在测试时具有更好的性能和效率
...
### [40] [Preventing Learning Stagnation in PPO by Scaling to 1 Million Parallel Environments](https://arxiv.org/abs/2603.06009)
*Michael Beukman,Khimya Khetarpal,Zeyu Zheng,Will Dabney,Jakob Foerster,Michael Dennis,Clare Lyle*

Main category: cs.LG

TL;DR: PPO算法在深度在线强化学习中常出现性能停滞问题，研究发现这是由于样本估计损失逐渐偏离真实目标。通过将PPO外循环建模为随机优化，发现可以通过减少步长或增加样本数来解决停滞问题。增加并行环境数量是有效方法，但需要正确调整超参数。


<details>
  <summary>Details</summary>
Motivation: PPO作为广泛采用的深度在线强化学习算法，常出现性能停滞在次优水平的问题。传统认为这是由于探索、容量或优化挑战，但本文发现根本原因是样本估计损失在训练过程中逐渐偏离真实目标，导致学习停滞。

Method: 将PPO的外循环（采样阶段）建模为随机优化过程，步长由对先前策略的正则化强度控制，梯度噪声由策略更新间收集的样本数决定。通过分析超参数对步长和噪声的影响，提出增加并行环境数量来减少这两个因素。同时提供超参数协同缩放的方法，避免性能下降。

Result: 模型预测得到验证：当外循环步长相对于噪声过大时，性能会停滞在次优水平。增加并行环境数量能有效减少步长和噪声。通过正确调整超参数，将PPO扩展到超过100万个并行环境，在复杂开放领域大幅超越先前基线，实现单调性能改进直至一万亿次状态转移。

Conclusion: PPO中的性能停滞问题可以通过将外循环视为随机优化过程来理解。增加并行环境数量是解决停滞的有效方法，但需要正确协同缩放超参数。该方法使PPO能够扩展到大规模并行环境，实现持续的性能改进。
...
### [41] [CLAIRE: Compressed Latent Autoencoder for Industrial Representation and Evaluation -- A Deep Learning Framework for Smart Manufacturing](https://arxiv.org/abs/2603.06361)
*Mohammadhossein Ghahramani,Mengchu Zhou*

Main category: cs.LG

TL;DR: CLAIRE是一个用于智能制造的混合端到端学习框架，结合无监督深度表示学习和有监督分类，通过优化的深度自编码器将高维传感器数据转换为紧凑的潜在空间表示，然后使用下游分类器进行故障预测，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 高维工业环境中的准确故障检测面临重大挑战，因为传感器数据具有固有的复杂性、噪声和冗余性，需要更有效的智能质量控制方法。

Method: CLAIRE采用混合端到端学习框架：1) 使用优化的深度自编码器将原始输入转换为紧凑的潜在空间表示，捕获内在数据结构并抑制无关或噪声特征；2) 将学习到的表示输入下游分类器进行二元故障预测；3) 采用基于博弈论的可解释性技术进行事后分析，识别对故障预测最有贡献的输入特征。

Result: 在高维数据集上的实验结果表明，CLAIRE显著优于直接在原始特征上训练的传统分类器。框架还成功识别了最有信息量的输入特征，增强了可解释性。

Conclusion: CLAIRE展示了将可解释AI与特征感知正则化相结合进行稳健故障检测的潜力。其模块化和可解释性使其具有高度适应性，在医疗、金融和环境监测等其他高维复杂数据领域也有应用前景。
...
### [42] [Agnostic learning in (almost) optimal time via Gaussian surface area](https://arxiv.org/abs/2603.06027)
*Lucas Pesenti,Lucas Slot,Manuel Wiedmer*

Main category: cs.LG

TL;DR: 本文改进了Klivans等人(2008)关于高斯分布下概念类学习复杂度的分析，将实现ε近似所需的低阶多项式次数从O(Γ²/ε⁴)降低到Õ(Γ²/ε²)，在统计查询模型中为多项式阈值函数的不可知学习提供了接近最优的复杂度边界。


<details>
  <summary>Details</summary>
Motivation: 高斯分布下概念类的不可知学习复杂度与其低阶多项式的L1近似能力密切相关。Klivans等人(2008)证明了对于高斯表面积至多为Γ的概念类，需要O(Γ²/ε⁴)阶的多项式才能实现ε近似，这为多种概念类的学习提供了最佳已知边界。本文旨在改进这一分析。

Method: 本文改进了Klivans等人的分析，证明只需Õ(Γ²/ε²)阶的多项式即可实现ε近似。证明方法借鉴了Feldman等人(2020)在布尔超立方体上L1近似构造的直接类比。

Result: 将实现ε近似所需的低阶多项式次数从O(Γ²/ε⁴)降低到Õ(Γ²/ε²)。结合Diakonikolas等人(2021)的下界结果，这为统计查询模型中多项式阈值函数的不可知学习提供了(接近)最优的复杂度边界。

Conclusion: 本文显著改进了高斯分布下概念类学习复杂度的理论分析，将多项式近似所需的阶数平方降低，为不可知学习提供了更紧的复杂度边界，特别是在多项式阈值函数的学习问题上达到了接近最优的结果。
...
### [43] [Improved high-dimensional estimation with Langevin dynamics and stochastic weight averaging](https://arxiv.org/abs/2603.06028)
*Stanley Wei,Alex Damian,Jason D. Lee*

Main category: cs.LG

TL;DR: 本文研究Langevin动力学能否通过迭代平均而非最终迭代，在样本复杂度n≳d^{k*/2}下恢复隐藏的种植方向θ*，从而避免显式平滑并匹配最优速率。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在线SGD和Langevin动力学需要n≳d^{max(1,k*-1)}样本才能恢复隐藏方向θ*，而通过显式平滑的梯度下降可将样本复杂度降至n≳d^{max(1,k*/2)}。这引发了一个问题：能否在不显式平滑的情况下达到相同速率？

Method: 采用Langevin动力学（噪声注入）并结合迭代平均，而非使用最终迭代。作者证明这种组合能够模拟景观平滑的效果，从而降低样本复杂度要求。

Result: 证明Langevin动力学通过迭代平均可以在n≳d^{k*/2}样本下成功恢复θ*，并将此结果应用于张量PCA和单指标模型设置。同时推测小批量SGD也能在不添加额外噪声的情况下达到相同速率。

Conclusion: 噪声注入与迭代平均的组合能够有效模拟景观平滑的效果，使得Langevin动力学在不显式平滑的情况下达到最优样本复杂度。这一发现为优化算法设计提供了新思路，并可能扩展到其他随机优化方法。
...
### [44] [NOBLE: Accelerating Transformers with Nonlinear Low-Rank Branches](https://arxiv.org/abs/2603.06492)
*Ethan Smith*

Main category: cs.LG

TL;DR: NOBLE是一种用于Transformer架构的增强方法，通过添加非线性低秩分支来提升训练效率，相比传统PEFT方法，它专为从头预训练设计。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）主要用于微调阶段，而NOBLE旨在为从头预训练提供一种高效架构增强，通过添加非线性低秩分支来加速训练收敛。

Method: 在Transformer线性层中添加永久性的非线性低秩分支，分支结构为σ(xWdown)Wup，其中σ为可学习的非线性激活函数。研究发现CosNet（具有可学习频率和相位的两层余弦非线性，中间有瓶颈空间线性投影）效果最佳。

Result: NOBLE显著提升训练效率：达到基线评估损失所需的训练步骤减少32%（1.47倍加速），仅增加4%参数和7%单步时间开销，净墙钟时间加速达1.22倍。在LLM、BERT、VQGAN和ViT上均表现一致改进。

Conclusion: NOBLE是一种有效的预训练架构增强方法，能显著加速训练收敛，但需注意某些数据增强方法（如Mixup/CutMix）可能干扰其效果，这可能是由于正则化技术与NOBLE的优化目标存在差异。
...
### [45] [COLD-Steer: Steering Large Language Models via In-Context One-step Learning Dynamics](https://arxiv.org/abs/2603.06495)
*Kartik Sharma,Rakshit S. Trivedi*

Main category: cs.LG

TL;DR: COLD-Steer是一种无需训练即可在推理时控制LLM行为的框架，通过近似梯度下降对上下文示例的影响来实现高效激活引导，相比现有方法减少50倍样本需求。


<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法面临基本权衡：样本效率高的方法无法充分捕捉标注示例中的引导信号，而能更好提取信号的方法需要数百到数千个示例。需要一种既能高效利用样本又能有效提取引导信号的方法。

Method: 提出COLD-Steer框架，通过近似梯度下降对少量示例进行微调的效果来实现推理时激活引导。采用两种互补方法：(1)单位核近似方法，直接使用激活梯度更新激活，并在示例间归一化；(2)有限差分近似方法，无论示例数量多少只需两次前向传播。

Result: 在各种引导任务和基准测试中，COLD-Steer达到95%的引导效果，同时比最佳基线方法减少50倍样本需求。在多元化对齐任务中验证了无需大量演示数据即可适应不同观点的能力。

Conclusion: COLD-Steer通过原理性地近似学习动态而非专门的训练过程，为自适应、上下文感知的模型控制开辟了新可能性，能够灵活应对基于损失的不同人类偏好。
...
### [46] [Dynamic Momentum Recalibration in Online Gradient Learning](https://arxiv.org/abs/2603.06120)
...
<details>
  <summary>Details</summary>
Motivation: SGD及其动量变体是深度学习优化的核心，但其梯度行为的底层动力学机制尚未被充分理解。固定动量系数会固有地扭曲偏差与方差之间的平衡，导致有偏或次优的参数更新。

Method: 提出SGDF（带滤波的SGD）优化器，受最优线性滤波原理启发。SGDF计算在线时变增益，通过最小化均方误差动态优化梯度估计，在噪声抑制和信号保留之间实现最优权衡。该方法可扩展到其他优化器。

Result: 在多种架构和基准测试上的广泛实验表明，SGDF超越了传统动量方法，性能达到或超过了最先进的优化器。

Conclusion: 从信号处理角度重新解释梯度更新，提出基于最优线性滤波的SGDF优化器，能够动态调整梯度估计，在噪声抑制和信号保留间取得最优平衡，性能优于传统方法。
...
### [47] [DQE: A Semantic-Aware Evaluation Metric for Time Series Anomaly Detection](https://arxiv.org/abs/2603.06131)
*Yuewei Li,Dalin Zhang,Huan Li,Xinyi Gong,Hongjun Chu,Zhaohui Song*

Main category: cs.LG

TL;DR: 该论文提出了一种新的时间序列异常检测评估指标，解决了现有评估方法在点级覆盖偏差、近漏检测不敏感、误报惩罚不足以及阈值选择不一致等方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列异常检测评估指标存在多个问题：偏向点级覆盖、对近漏检测不敏感或不一致、对误报惩罚不足、以及由阈值或阈值区间选择引起的不一致性。这些问题导致评估结果不可靠或反直觉，阻碍了客观进展。
...
Result: 在合成数据和真实世界数据上的大量实验表明，该指标提供了稳定、有区分度和可解释的评估，与十种广泛使用的指标相比实现了稳健的评估效果。

Conclusion: 该论文提出的新评估指标能够更全面、可靠地评估时间序列异常检测性能，解决了现有评估方法的局限性，为领域提供了更客观的评估框架。
...
### [48] [Topological descriptors of foot clearance gait dynamics improve differential diagnosis of Parkinsonism](https://arxiv.org/abs/2603.06212)
*Jhonathan Barrios,Wolfram Erlhagen,Miguel F. Gago,Estela Bicho,Flora Ferreira*

Main category: cs.LG

TL;DR: 该研究使用拓扑数据分析（TDA）结合机器学习，通过足部间隙时间序列来区分帕金森综合征亚型，在药物状态下的分类准确率达到83%。
...
Method: 使用拓扑数据分析（TDA）处理足部间隙时间序列，生成Betti曲线、持久性景观和轮廓图作为特征，然后使用随机森林分类器进行帕金森综合征分类。数据集包括15名对照、15名特发性帕金森病和14名血管性帕金森病患者，采用留一法交叉验证评估模型。

Result: Betti曲线描述符表现最佳。在药物状态下，特发性帕金森病与血管性帕金森病的分类准确率达到83%，AUC为0.89。药物状态下的性能优于非药物状态，且同时考虑两种状态时性能进一步提升，表明拓扑特征对左旋多巴相关的步态变化敏感。

Conclusion: 拓扑数据分析与机器学习结合可以改善临床步态分析，有助于帕金森综合征的鉴别诊断，为临床决策提供新的工具。
...
### [49] [FedSCS-XGB -- Federated Server-centric surrogate XGBoost for continual health monitoring](https://arxiv.org/abs/2603.06224)
*Felix Walger,Mehdi Ejtehadi,Anke Schmeink,Diego Paez-Granados*

Main category: cs.LG

TL;DR: 提出一种基于XGBoost的分布式机器学习协议，用于可穿戴传感器的人类活动识别，在脊髓损伤监测场景中实现接近集中式训练的性能（差距<1%）。


<details>
  <summary>Details</summary>
Motivation: 脊髓损伤患者面临压疮和血压不稳定等风险，需要连续监测以早期发现和干预。可穿戴传感器结合本地数据处理能够实现这一目标，但需要处理数据碎片化和异质性问题的分布式机器学习方法。

Method: 提出基于梯度提升决策树（XGBoost）的分布式机器学习协议，受Party-Adaptive XGBoost（PAX）启发，但明确保留了标准XGBoost的关键结构和优化特性，包括基于直方图的分裂构建和树集成动态。

Result: 理论分析表明在适当数据条件和超参数选择下，分布式协议能收敛到与集中式XGBoost训练等效的解。在代表性可穿戴传感器HAR数据集上的实证评估显示，与集中式XGBoost和IBM PAX相比，该方法能匹配集中式性能，差距在1%以内。

Conclusion: 提出的分布式XGBoost协议在保持XGBoost结构优势的同时，在分布式可穿戴HAR设置中实现了接近集中式训练的性能，为远程监测场景中的异质性和数据碎片化问题提供了有效解决方案。
...
### [50] [DC-Merge: Improving Model Merging with Directional Consistency](https://arxiv.org/abs/2603.06242)
*Han-Chen Zhang,Zi-Hao Zhou,Mao-Lin Luo,Shimin Di,Min-Ling Zhang,Tong Wei*

Main category: cs.LG

TL;DR: DC-Merge是一种保持方向一致性的模型融合方法，通过平衡任务向量的能量分布和几何对齐来解决传统模型融合中的知识保留问题。


<details>
  <summary>Details</summary>
Motivation: 传统模型融合方法在将多个任务适应模型整合为统一模型时，经常无法有效保留各个任务的知识。主要问题在于：1）任务向量中能量分布不平衡，少数奇异值主导总能量，导致语义重要但较弱的组件在融合时被忽略；2）参数空间中任务向量的几何不一致性，直接融合会扭曲其底层方向几何。

Method: DC-Merge包含三个步骤：首先通过平滑奇异值来平衡每个任务向量的能量分布，确保所有知识组件得到充分表示；然后将这些能量平衡的向量投影到共享正交子空间，以最小重构误差对齐其方向几何；最后在共享正交子空间中聚合对齐的向量，并投影回原始参数空间。

Result: 在视觉和视觉语言基准测试上的广泛实验表明，DC-Merge在完全微调和LoRA设置下都能持续实现最先进的性能。

Conclusion: DC-Merge通过保持奇异空间之间的方向一致性，有效解决了模型融合中的知识保留问题，为多任务模型整合提供了一种有效的解决方案。
...
### [51] [Gradient Flow Polarizes Softmax Outputs towards Low-Entropy Solutions](https://arxiv.org/abs/2603.06248)
*Aditya Varre,Mark Rofin,Nicolas Flammarion*

Main category: cs.LG

TL;DR: 该论文分析了基于softmax的transformer模型中梯度流动态，揭示了优化过程会自然驱动模型产生低熵输出，解释了注意力汇聚等经验现象。


<details>
  <summary>Details</summary>
Motivation: 理解softmax模型的非凸训练动态对于解释transformer的成功至关重要。论文旨在分析value-softmax模型的梯度流动态，该模型是自注意力机制的核心构建块。

Method: 分析value-softmax模型L(Vσ(a))的梯度流动态，其中V是可学习的值矩阵，a是注意力向量。研究在各种目标函数（包括逻辑损失和平方损失）下的优化行为。

Result: 梯度流在这种结构上会自然驱动优化朝着低熵输出的解决方案发展。这种极化效应在各种目标函数中具有普遍性，为注意力汇聚和大规模激活等经验现象提供了形式化机制。

Conclusion: 该分析为理解transformer训练动态提供了直接洞见，揭示了softmax参数化如何自然导致低熵输出，解释了实际观察到的经验现象。
...
### [52] [Tiny, Hardware-Independent, Compression-based Classification](https://arxiv.org/abs/2603.06359)
*Charles Meyers,Aaron MacSween,Erik Elmroth,Tommy Löfstedt*

Main category: cs.LG

TL;DR: 本文针对客户端机器学习中的隐私问题，提出改进的归一化压缩距离方法，使其适用于核方法并提升训练效率，实现仅需少量样本即可在客户端设备上高效运行的模型。


<details>
  <summary>Details</summary>
Motivation: 随着用户隐私意识增强，客户端数据存储和分析成为趋势，但现有机器学习方法需要大量标注数据和计算资源，不适合客户端设备。需要开发既保护隐私又能在资源受限设备上高效运行的方法。

Method: 1) 证明归一化压缩距离不是度量；2) 将其扩展到核方法中以处理复杂数据；3) 提出改进训练时间的技术；4) 在分类任务中验证其有效性。

Result: 归一化压缩距离与其他度量和核方法效果相当甚至更好，计算成本仅略高，且无需正式度量属性。仅需少量样本即可获得高精度，模型足够小且高效，可在客户端设备上仅使用用户数据运行。

Conclusion: 改进的归一化压缩距离方法为客户端机器学习提供了有效解决方案，平衡了隐私保护、计算效率和模型性能，适合在资源受限设备上部署。
...
### [53] [Adapter-Augmented Bandits for Online Multi-Constrained Multi-Modal Inference Scheduling](https://arxiv.org/abs/2603.06403)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在实际应用中面临异构预算和请求多样性的挑战，传统同质后端设置无法满足需求，需要智能调度来平衡质量和成本

Method: 提出M-CMAB框架，包含三个组件：1) 基于CLS注意力的预测器提取任务表示；2) 原始对偶约束器维护拉格朗日乘子；3) 两阶段调度器在不可逆预算下平衡探索与利用

Result: 在异构后端的多模态基准测试中，M-CMAB在不同预算机制下均优于现有方法，奖励提升最高达14.18%，接近oracle辅助上界

Conclusion: M-CMAB通过多约束上下文多臂老虎机方法有效解决了多模态推理调度问题，在保证语义忠实度的同时实现了高效的在线决策
...
### [54] [Toward Generative Quantum Utility via Correlation-Complexity Map](https://arxiv.org/abs/2603.06440)
*Chen-Yu Liu,Leonardo Placidi,Eric Brunner,Enrico Rinaldi*

Main category: cs.LG

TL;DR: 本文提出了一种相关性-复杂性映射作为诊断工具，用于判断真实世界数据分布是否与IQP型量子生成模型结构对齐，并在经典湍流数据上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 需要一种实用的诊断工具来确定真实世界数据分布何时在结构上与IQP型量子生成模型对齐，以推进量子生成模型的实用化。

Method: 提出相关性-复杂性映射，包含两个互补指标：量子相关性指标（QCLI）和经典相关性-复杂性指标（CCI）。QCLI通过数据集的相关阶功率谱计算，使用Jensen-Shannon散度与i.i.d.二项分布的差异；CCI定义为总相关性中未被最优Chow-Liu树近似捕获的部分比例。使用可逆浮点到比特串表示和潜在参数适应方案，通过学习和插值低维潜在轨迹来重用紧凑的IQP电路。

Result: 识别出经典湍流数据既与IQP兼容又具有经典复杂性（高QCLI/高CCI）。在与RBM和DCGAN等经典模型的比较评估中，IQP方法实现了竞争性的分布对齐，同时使用了显著更少的训练快照和小型潜在块。

Conclusion: QCLI/CCI可作为实用的指标来定位与IQP对齐的领域，支持使用相关性-复杂性映射作为诊断工具，推进量子生成模型的实用化。
...
TL;DR: 研究发现多模态扩散模型中的后门攻击存在"模态崩溃"现象，即攻击机制主要依赖部分模态而非多模态协同作用，挑战了多模态攻击具有互补效应的假设。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型在视觉内容生成中的广泛应用，研究其安全漏洞（如后门攻击）变得至关重要。传统观点认为多模态（如文本和图像）同时攻击会产生互补效应，但本文旨在验证这一假设是否正确。

Method: 提出了两种新指标来量化后门模态崩溃现象：触发模态归因（TMA）和交叉触发交互（CTI）。在多模态条件扩散模型的不同训练配置下进行了广泛实验。

Result: 实验结果显示：1）攻击往往崩溃为子模态主导的"赢家通吃"动态；2）跨模态交互可忽略甚至为负，与协同漏洞的直觉相矛盾。高攻击成功率往往掩盖了对部分模态的根本依赖。

Conclusion: 研究揭示了当前评估中的关键盲点，为机制分析和未来防御开发建立了原则性基础。多模态后门攻击并不总是产生协同效应，反而可能出现模态崩溃现象。
...
TL;DR: 提出了一种针对大规模分层概率时间序列预测的新型可解释性方法，能够解释工业供应链场景中的预测结果，显著提高可解释性准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然机器学习模型在分层时间序列预测任务上取得了显著准确性和可扩展性，但其预测的可解释性在工业应用中仍未被充分探索。需要填补这一空白，以帮助利益相关者更好地理解预测结果。

Method: 提出了一种新颖的可解释性方法，专门针对大规模分层概率时间序列预测。该方法适应通用的可解释性技术，同时解决了与分层结构和不确定性相关的挑战。通过生成基于真实工业场景的半合成数据集进行评估。

Result: 实验表明，该方法成功解释了最先进的工业预测方法，具有显著更高的可解释性准确性。多个真实案例研究证明了该方法在识别重要模式和解释方面的有效性，帮助利益相关者更好地理解预测。

Conclusion: 该方法能够提供有价值的解释性见解，帮助识别预测需求背后的关键驱动因素，促进更明智的决策和战略规划。有助于建立用户信任和信心，最终促进分层预测模型在实践中的更好采用和利用。
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习原子间势能模型在处理大尺度系统（如生物分子和电解质）时难以准确捕捉长程相互作用，现有方法依赖显式的物理基础项或组件。

Method: 提出AllScAIP模型，采用基于注意力的能量守恒架构，包含全节点注意力组件（数据驱动），可扩展到1亿训练样本规模。

Result: 模型在分子系统上达到最先进的能量/力精度，在物理基础评估（OMol25）中表现优异，在材料（OMat24）和催化剂（OC20）上具有竞争力，并能进行稳定、长时间尺度的分子动力学模拟，准确预测实验观测值。

Conclusion: 在低数据/小模型情况下，归纳偏置提高样本效率；但随着数据和模型规模扩大，这些优势减弱甚至反转，而全节点注意力对捕捉长程相互作用至关重要。AllScAIP为大规模系统提供了有效的解决方案。
...
### [58] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*

Main category: cs.AI

TL;DR: RoboLayout扩展了LayoutVLM，通过集成显式可达性约束和局部细化阶段，生成适合具身智能体交互的3D室内场景布局。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理和3D场景布局生成方面有潜力，但生成的布局往往缺乏对具身智能体的交互可行性考虑，特别是在物理受限的室内环境中。

Method: 1. 在LayoutVLM基础上增加智能体感知推理；2. 将显式可达性约束集成到可微布局优化过程中；3. 提出局部细化阶段，选择性重新优化问题物体放置；4. 支持多种具身智能体抽象（服务机器人、仓储机器人、不同年龄段人类、动物等）。

Result: RoboLayout在保持LayoutVLM的语义对齐和物理合理性的同时，增强了面向智能体的室内场景生成适用性，实验结果表明其在多样化场景配置中表现良好。

Conclusion: RoboLayout通过集成智能体感知约束和优化稳定性改进，成功生成了既语义连贯又适合具身智能体交互的室内场景布局，扩展了视觉语言模型在具身智能环境设计中的应用。
...
### [59] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 该研究评估了推理模型对思维链（CoT）的控制能力，发现模型对CoT的控制能力远低于对最终输出的控制能力，目前CoT可控性不太可能成为CoT可监控性的失效模式。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估推理模型对思维链的控制能力，因为如果模型能够控制其在CoT中表达的内容，可能会破坏CoT的可监控性，从而影响检测模型不当行为和理解其动机的能力。

Method: 引入了CoT-Control评估套件，包含需要模型在遵循CoT指令的同时解决问题的任务（例如在不使用"染色体"一词的情况下回答遗传学问题）。评估了不同规模模型、不同RL训练程度、不同测试时计算量以及不同问题难度下的CoT可控性。

Result: 研究发现推理模型的CoT可控性显著低于输出可控性（例如Claude Sonnet 4.5的CoT可控性仅为2.7%，而输出可控性为61.9%）。CoT可控性随模型规模增大而提高，但随RL训练增加、测试时计算量增加和问题难度增加而降低。即使在有激励逃避监控的情况下，模型也难以控制CoT。

Conclusion: 目前CoT可控性不太可能成为CoT可监控性的失效模式，但低可控性的机制尚不清楚。建议前沿实验室在未来模型中跟踪CoT可控性，这对维持CoT可监控性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设静态环境，忽略了现实世界的演化特性以及智能体对环境变化的鲁棒性。需要一种可扩展且可控的方式来演化智能体环境，以更好地评估智能体对现实世界动态的适应能力。

Method: 提出ProEvolve框架，使用类型化关系图统一表示环境的数据、工具和模式。通过图变换（添加、删除、修改能力）来演化环境，这些变换能够跨工具、模式和数据访问一致地传播更新。框架可以编程演化动态过程，并通过子图采样和编程实例化任务沙箱。
...
### [61] [Aggregative Semantics for Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2603.06067)
*Yann Munro,Isabelle Bloch,Marie-Jeanne Lesot*

Main category: cs.AI

TL;DR: 本文提出了一种新的渐进语义家族——聚合语义，用于处理具有内在权重的定量双极论证框架，通过将攻击者和支持者分开聚合的三阶段计算，提高了参数化和可解释性。


<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有的渐进语义通常对称处理攻击和支持关系，但在实际应用中攻击者和支持者可能扮演不对称的角色。需要一种更灵活、更可解释的方法来处理这种不对称性，同时保持双极性的特征。

Method: 提出聚合语义家族，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。这种方法将攻击者和支持者分开处理，与传统的模块化语义形成对比。

Result: 聚合语义提供了更高的参数化程度和更好的可解释性，能够处理攻击者和支持者不对称的情况。通过500种聚合语义的测试和比较，展示了该方法的行为范围，并通过具体示例说明了其适用性。

Conclusion: 聚合语义通过将计算分解为三个独立且可解释的步骤，在保持双极性的同时提供了更灵活的参数化方法，比现有文献中的方法更进一步，产生了更易理解的渐进语义。
...
### [62] [Offline Materials Optimization with CliqueFlowmer](https://arxiv.org/abs/2603.06082)
*Jakub Grudzien Kuba,Benjamin Kurt Miller,Sergey Levine,Pieter Abbeel*

Main category: cs.AI

TL;DR: CliqueFlowmer：一种基于离线模型优化（MBO）的计算材料发现方法，通过结合团簇优化与Transformer和流生成技术，直接优化目标材料性能，优于传统生成模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的生成建模方法在计算材料发现中存在局限性，由于最大似然训练难以有效探索材料空间的有利区域，需要一种能直接优化目标性能的替代方法。

Method: 提出CliqueFlowmer模型，将团簇优化（clique-based MBO）的最新进展与Transformer和流生成技术相结合，实现离线模型优化方法，将目标材料性能的直接优化融入生成过程。

Result: CliqueFlowmer在优化能力验证中表现出色，其生成的材料性能显著优于生成基线方法提供的材料。

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的离线模型优化方法，优于传统生成建模，已开源代码以支持专业材料优化问题和跨学科研究。
...
### [63] [Conversational Demand Response: Bidirectional Aggregator-Prosumer Coordination through Agentic AI](https://arxiv.org/abs/2603.06217)
*Reda El Makroum,Sebastian Zwickl-Bernhard,Lukas Kranzl,Hans Auer*

Main category: cs.AI

TL;DR: 该论文提出了对话式需求响应（CDR），一种通过双向自然语言交互协调聚合商和产消者的机制，使用智能体AI实现透明、可解释的用户参与。
...
Method: 开发了两层多智能体架构：聚合商智能体调度灵活性请求，产消者家庭能源管理系统通过调用基于优化的工具评估可交付性和成本效益。支持产消者发起的上游通信，允许偏好变更直接传达给聚合商。

Result: 概念验证评估显示交互在12秒内完成。该架构展示了智能体AI如何弥合聚合商-产消者协调差距，在保持自动化需求响应可扩展性的同时，提供透明度、可解释性和用户自主权。

Conclusion: 对话式需求响应通过双向自然语言交互实现了更有效的协调，所有系统组件（包括智能体提示、编排逻辑和仿真接口）已开源发布，以促进可重复性和进一步开发。
...
### [64] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: 本文提出了一种基于强化学习的决策支持框架，用于城市洪涝适应规划，通过整合降雨预测、洪水建模、交通模拟和影响评估，在哥本哈根案例研究中展示了优于传统优化方法的效果。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和城市内涝，导致交通系统中断增加。制定有效适应策略面临挑战：基础设施投资的长期性、气候不确定性、以及洪水-基础设施-流动性之间的复杂相互作用。

Method: 提出基于强化学习的决策支持框架，作为综合评估模型，整合降雨预测与洪水建模、交通模拟、基础设施和流动性直接间接影响量化。RL方法学习平衡投资维护成本与避免影响的适应策略。

Result: 在哥本哈根市中心2024-2100年案例研究中，测试多种适应方案和不同气候情景。结果显示该框架优于传统优化方法，能发现协调的时空适应路径，学习影响减少与适应投资之间的权衡，产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下具有自适应基础设施规划的潜力，能够发现更有效的长期适应策略。
...
### [65] [SAHOO: Safeguarded Alignment for High-Order Optimization Objectives in Recursive Self-Improvement](https://arxiv.org/abs/2603.06333)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.AI

TL;DR: SAHOO框架通过三个安全措施监控和控制递归自我改进中的对齐漂移：目标漂移指数、约束保护检查和回归风险量化，在代码生成、数学推理和真实性任务中实现显著质量提升同时保持对齐。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改存在微妙的对齐漂移风险，需要实用的监控和控制框架来确保系统在改进过程中保持对齐。

Method: 提出SAHOO框架，包含三个核心组件：(1)目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；(2)约束保护检查，强制执行安全关键不变量如语法正确性和非幻觉；(3)回归风险量化，标记可能撤销先前改进的改进周期。

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO实现了显著质量提升：代码任务提升18.3%，推理任务提升16.8%，同时在两个领域保持约束保护，在真实性任务中保持低违规率。阈值校准基于18个任务的验证集。

Conclusion: SAHOO使递归自我改进过程中的对齐保护变得可测量、可部署且可系统验证，揭示了能力-对齐边界，显示早期改进周期效率高但后期对齐成本上升，并暴露了领域特定的张力如流畅性与事实性之间的权衡。
...
### [66] [Talk Freely, Execute Strictly: Schema-Gated Agentic AI for Flexible and Reproducible Scientific Workflows](https://arxiv.org/abs/2603.06394)
...
TL;DR: 该论文提出了一种解决LLM在科学工作流中确定性执行与对话灵活性矛盾的方案：基于模式门控的编排架构，通过模式验证确保工作流确定性，同时保持对话灵活性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决定执行内容难以满足这些要求。研究发现工业研发中存在两个竞争需求：确定性约束执行与无工作流刚性的对话灵活性。

Method: 提出模式门控编排作为解决方案：模式成为组合工作流级别的强制执行边界，确保完整操作（包括跨步骤依赖）必须通过机器可检查规范的验证。将两个需求操作化为执行确定性（ED）和对话灵活性（CF），使用多模型协议评估20个系统，通过3个LLM家族的15个独立会话进行评分。

Result: 多模型LLM评分显示高度一致性（ED的Krippendorff α=0.80，CF的α=0.98），证明其可作为人类专家评估的可重用替代方案。评估发现经验帕累托前沿：没有系统同时实现高灵活性和高确定性，但在生成式和以工作流为中心的极端之间存在收敛区域。

Conclusion: 模式门控架构通过分离对话和执行权限，能够解耦灵活性与确定性的权衡。提炼出三个操作原则：执行前澄清、约束的计划-行动编排、工具到工作流级别的门控，以指导实际应用。
...
### [67] [AutothinkRAG: Complexity-Aware Control of Retrieval-Augmented Reasoning for Image-Text Interaction](https://arxiv.org/abs/2603.05551)
...
<details>
  <summary>Details</summary>
Motivation: 文档问答面临长上下文和信息过载的挑战，现有多模态图RAG框架存在两个问题：1）需要大规模模型处理不同复杂度查询；2）端到端视觉语言模型存在固有推理瓶颈

Method: 提出AutoThinkRAG框架：1）查询复杂度路由器根据查询难度分配推理路径；2）功能解耦架构：小型VLM作为高保真视觉解释器将视觉线索转为文本表示，LLM负责逻辑推理和综合

Result: 在DocBench和MMLongBench上的实验表明，AutoThinkRAG显著降低推理成本的同时实现了新的SOTA性能，消融研究验证了方法的有效性

Conclusion: AutoThinkRAG通过多模型协同和功能解耦有效解决了复杂文档理解中的挑战，在性能和效率方面取得了平衡
...
### [68] [CBR-to-SQL: Rethinking Retrieval-based Text-to-SQL using Case-based Reasoning in the Healthcare Domain](https://arxiv.org/abs/2603.05569)
*Hung Nguyen,Hans Moen,Pekka Marttinen*

Main category: cs.IR

TL;DR: CBR-to-SQL框架使用案例推理方法，通过两阶段检索过程将自然语言医疗问题转换为SQL查询，相比标准RAG方法在医疗领域表现更优


<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据库查询需要SQL专业知识，这成为医疗决策和研究的障碍。虽然可以使用LLM通过RAG将自然语言问题转换为SQL，但在医疗领域应用存在挑战，因为标准RAG的单步检索方法难以处理医学术语和行话的变异性与噪声

Method: 提出CBR-to-SQL框架，受案例推理启发，将问题-SQL对表示为可重用的抽象案例模板，采用两阶段检索过程：先捕获逻辑结构，再解析相关实体

Result: 在MIMICSQL数据集上评估，CBR-to-SQL在逻辑形式准确性方面达到最先进水平，在执行准确性方面具有竞争力。更重要的是，相比标准RAG方法，它表现出更高的样本效率和鲁棒性，特别是在数据稀缺和检索扰动情况下

Conclusion: CBR-to-SQL框架通过案例推理方法有效解决了医疗领域自然语言到SQL转换的挑战，比标准RAG方法更高效、更鲁棒，有助于降低医疗数据查询的技术门槛
...
### [69] [Balancing Domestic and Global Perspectives: Evaluating Dual-Calibration and LLM-Generated Nudges for Diverse News Recommendation](https://arxiv.org/abs/2603.05780)
*Ruixuan Sun,Matthew Zent,Minzhu Zhao,Thanmayee Boyapati,Xinyi Li,Joseph A. Konstan*

Main category: cs.IR

TL;DR: 该研究应用"个性化多样性助推框架"来扩展用户在新闻地域性（国内与世界新闻）方面的阅读覆盖范围，通过算法助推和LLM展示助推在真实用户实验中验证效果。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决新闻推荐系统中用户阅读覆盖范围有限的问题，特别是新闻地域性（国内与世界新闻）的多样性不足，希望通过个性化多样性助推框架来扩展用户的阅读视野。

Method: 设计了主题-地域双重校准算法助推和基于大语言模型的新闻个性化展示助推，在POPROX新闻推荐实验平台上进行了为期5周的真实用户研究，涉及120名美国新闻读者，收集用户交互日志和调查响应。

Result: 算法助推成功增加了曝光和消费多样性；LLM展示助推效果不一；用户主题兴趣是点击的强预测因素；强调新闻与先前阅读文章相关性的个性化优于通用主题和无个性化；长期接触校准新闻可能改变阅读习惯，使读者更重视国内与世界新闻的平衡。

Conclusion: 研究为新闻推荐系统中促进多样性消费的未来工作提供了方向，表明算法助推和个性化展示策略可以有效扩展用户的新闻阅读覆盖范围，特别是通过长期校准可能改变阅读习惯。
...
### [70] [MLLMRec-R1: Incentivizing Reasoning Capability in Large Language Models for Multimodal Sequential Recommendation](https://arxiv.org/abs/2603.06243)
*Yu Wang,Yonghui Yang,Le Wu,Jiancan Wu,Hefei Xu,Hui Lin*

Main category: cs.IR

TL;DR: MLLMRec-R1提出了一种高效稳定的GRPO推理框架，用于解决多模态序列推荐中视觉token过多导致训练成本过高以及CoT监督存在奖励膨胀的问题。


<details>
  <summary>Details</summary>
Motivation: 将GRPO推理管道扩展到多模态序列推荐面临两个主要挑战：1）视觉token主导输入导致基于组的rollout成本随历史长度和候选集大小而急剧增加，使GRPO训练极其昂贵；2）现有CoT监督在推荐场景中存在奖励膨胀问题，更高的训练奖励并不能可靠地转化为更好的排序性能，可能导致捷径学习。

Method: MLLMRec-R1采用三种关键技术：1）离线文本化视觉信号以消除昂贵的视觉token同时保留多模态语义；2）通过细化和置信度感知评估构建高质量的多模态CoT监督；3）采用混合粒度数据增强策略，选择性注入可靠的CoT样本同时保留标准训练数据，缓解奖励膨胀并提高泛化稳定性。

Result: 在三个基准数据集上的广泛实验表明，MLLMRec-R1持续优于最先进的方法，为多模态序列推荐建立了一个实用有效的GRPO推理管道。

Conclusion: MLLMRec-R1成功解决了多模态序列推荐中GRPO训练的成本和稳定性问题，通过视觉信号文本化、高质量CoT监督构建和混合粒度数据增强，实现了高效稳定的推荐性能提升。
...
### [71] [Efficient, Property-Aligned Fan-Out Retrieval via RL-Compiled Diffusion](https://arxiv.org/abs/2603.06397)
...
<details>
  <summary>Details</summary>
Motivation: 现代检索问题通常是集合值的，需要优化高阶属性（如多样性、覆盖性、互补性、一致性），但现有监督数据集只关注top-1检索。强化学习可以优化集合级目标但推理成本高，扩散检索效率高但需要目标对齐的训练数据。

Method: 提出R4T方法：1）使用复合集合级奖励训练fan-out LLM；2）合成目标一致的训练对；3）训练轻量级扩散检索器来建模集合值输出的条件分布。

Result: 在时尚和音乐基准测试中，R4T相对于强基线提高了检索质量，同时将查询时fan-out延迟降低了一个数量级。

Conclusion: R4T通过将强化学习作为一次性目标转换器，结合扩散检索的效率优势，有效解决了集合值检索中目标优化与推理效率的平衡问题。
