...
Motivation: 现有视频描述模型在描述细粒度运动细节时存在严重局限性，特别是在运动中心视频中，对复杂动作和肢体动态的精确描述不足，同时存在严重的幻觉问题。
...
Result: 1) 发布了KPM-Bench开源数据集；2) 提出了独立的幻觉评估指标；3) 有效缓解了幻觉问题，显著提高了运动中心视频描述模型的可靠性。

Conclusion: 通过KPM-Bench数据集和MoPE算法，该研究为细粒度运动理解提供了系统解决方案，显著改善了视频描述模型在运动细节描述上的准确性和可靠性。
...
TL;DR: 本文提出了3D-HIW数据集和CLUTCH系统，用于解决野外环境下的手部动作建模问题，在文本到手部动作生成和动作描述任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 手部在日常生活中扮演核心角色，但现有方法依赖工作室采集的有限数据集，难以扩展到野外环境，且现有模型在动画保真度和文本-动作对齐方面存在不足。
...
Conclusion: 通过3D-HIW数据集和CLUTCH系统，成功解决了野外环境下手部动作建模的挑战，为手部动画生成和理解提供了新的基准和解决方案。
...
TL;DR: 提出LGD-Net框架，通过跨模态特征幻觉而非像素级图像生成，从H&E切片直接预测HER2表达水平，避免虚拟染色计算开销和重建伪影


<details>
  <summary>Details</summary>
Motivation: 标准IHC染色资源密集、昂贵且耗时，许多地区无法获得。从H&E切片直接预测HER2水平是潜在替代方案，但现有像素级虚拟染色方法计算昂贵且易产生重建伪影，可能导致诊断错误

Method: 提出Latent-Guided Dual-Stream Network (LGD-Net)，采用跨模态特征幻觉而非显式像素级图像生成。模型学习将形态学H&E特征直接映射到分子潜在空间，在训练期间由教师IHC编码器指导。通过轻量级辅助正则化任务，利用特定领域知识（核分布和膜染色强度）正则化模型训练

Result: 在公开BCI数据集上的广泛实验表明，LGD-Net达到最先进性能，显著优于基线方法，同时支持使用单模态H&E输入进行高效推理

Conclusion: LGD-Net通过特征幻觉而非图像生成，有效解决了虚拟染色方法的计算开销和伪影问题，为从H&E切片准确预测HER2表达水平提供了高效可靠的解决方案
...
TL;DR: 提出了一种无需额外训练即可实现遥感图像文本引导分割的方法，通过结合对比式和生成式视觉语言模型与SAM，在19个遥感基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型和视觉基础模型为零样本文本引导遥感图像分割提供了新机会，但大多数方法仍依赖额外的可训练组件，限制了其泛化能力和实际应用性。

Method: 提出两种方法：1）对比式方法使用CLIP作为SAM网格提议的掩码选择器；2）生成式方法使用GPT-4V生成点击提示给SAM，或使用LoRA微调的Qwen-VL模型。

Result: 在19个遥感基准测试（包括开放词汇、指代和基于推理的任务）上进行了广泛实验，对比式方法在完全零样本设置下实现了最先进的开放词汇语义分割，生成式方法在指代分割方面表现最佳。

Conclusion: 展示了仅依赖现有基础模型而无需额外训练即可实现文本引导遥感图像分割的可行性，为实际应用提供了轻量级解决方案。
...
TL;DR: 提出了首个视频查询性能预测基准VQPP，包含两个文本到视频检索数据集和两个CBVR系统，探索了预检索和后检索性能预测器，并展示了其在查询重写任务中的应用。


<details>
  <summary>Details</summary>
Motivation: 查询性能预测在信息检索中很重要，但主要研究集中在文本和图像检索领域，基于内容的视频检索中的QPP研究相对不足，需要建立专门的基准来推动该领域发展。

Method: 创建VQPP基准，包含两个文本到视频检索数据集和两个CBVR系统，共56K文本查询和51K视频；探索多种预检索和后检索性能预测器；使用最佳预检索预测器作为奖励模型，通过直接偏好优化训练LLM进行查询重写。

Result: 预检索预测器取得了有竞争力的性能，能够在检索步骤之前实现应用；成功将最佳预检索预测器应用于查询重写任务，通过DPO训练LLM；建立了可复现的视频QPP基准。

Conclusion: VQPP是首个视频查询性能预测基准，填补了CBVR领域QPP研究的空白；预检索预测器表现出色，为视频检索中的查询优化等应用提供了实用工具；基准的发布将促进该领域的标准化比较和进一步发展。
...
TL;DR: 该论文对Liu和Szirányi提出的手势识别方法进行了方法学分析，指出其评估协议存在严重的数据泄露问题，导致报告的近乎完美的准确率指标无效。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是分析Liu和Szirányi提出的手势识别方法的评估协议有效性，特别关注其是否存在数据泄露问题。研究者发现报告的近乎完美的准确率可能源于不恰当的评估方法，这对手势识别研究，特别是无人机-人交互等需要识别未见个体手势的应用具有重要意义。

Method: 通过分析已发表的混淆矩阵、学习曲线和数据集构建方法，研究者展示了Liu和Szirányi的评估协议存在问题。具体来说，他们使用了帧级别的随机训练-测试分割，这不可避免地混合了同一受试者在训练集和测试集中的样本，导致了严重的数据泄露。

Result: 研究结果表明，Liu和Szirányi报告的近乎完美的准确率指标是由于数据泄露造成的，而不是真正的模型泛化能力。他们的评估协议未能测量模型对未见个体的泛化能力，因为训练集和测试集中包含了同一受试者的数据。

Conclusion: 该研究强调了在基于视觉的手势识别研究中，特别是对于需要识别未见个体手势的应用（如无人机-人交互），采用受试者独立的数据划分方法的重要性。研究呼吁在评估手势识别系统时，必须确保训练集和测试集中的受试者完全分离，以避免数据泄露并准确评估模型的真实泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着视频骨干架构和大语言模型的发展，分析长达数十分钟的长视频变得可行且普遍。但视频序列固有的冗余性给现有模型带来两大挑战：1) 在内存限制内高效处理更多帧；2) 从大量输入数据中提取判别性信息。

Method: 提出端到端的长视频理解框架，包括：1) 基于信息密度的自适应视频采样器(AVS)，自适应捕捉不同时长视频的关键信息；2) 基于自动编码器的时空视频压缩器(SVC)，实现高压缩率同时保留关键判别信息；3) 与多模态大语言模型(MLLM)集成。

Result: 该框架在多个基准测试中表现出色，不仅在长视频理解任务中表现优异，在标准视频理解基准测试中也取得良好结果，证明了其在处理长视频复杂性方面的有效性和通用性。

Conclusion: 提出的框架通过自适应采样和高效压缩机制，有效解决了长视频理解中的冗余问题，展现了在处理长视频序列复杂性的多功能性和高效性，为长视频分析提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在各种视觉问答基准测试中取得了显著进展，但最近的研究表明这些模型在传统的图像分类基准测试（特别是细粒度视觉知识测试）上表现落后。研究者希望探索视觉语言模型在细粒度视觉理解能力上的不足，并找出影响这种能力的关键因素。

Method: 研究测试了大量最新的视觉语言模型在细粒度分类基准上的表现，通过一系列消融实验分析影响性能的因素。实验设计包括：1）测试不同LLM对各类基准性能的影响；2）测试不同视觉编码器对细粒度分类性能的影响；3）分析预训练阶段（特别是语言模型权重是否冻结）对细粒度性能的影响。

Result: 实验结果显示：1）使用更好的LLM能同等提升所有基准测试的分数；2）更好的视觉编码器能不成比例地提升细粒度分类性能；3）预训练阶段对细粒度性能至关重要，特别是在预训练期间不冻结语言模型权重时效果更佳。

Conclusion: 该研究揭示了提升视觉语言模型细粒度视觉理解能力的关键因素：视觉编码器的质量和预训练策略。这些发现为增强视觉语言模型的细粒度视觉理解和视觉中心能力提供了重要指导，有助于缩小视觉语言模型在通用视觉问答和细粒度分类任务之间的性能差距。
...
Method: 提出多模态深度重建框架，利用极稀疏的测距数据（如汽车雷达或LiDAR），采用局部高斯过程建模在角度域中表示深度，实现计算高效推理并显式量化观测有限区域的不确定性。重建的深度和不确定性可直接替换现有扩散渲染流程中的单目深度估计器。

Result: 在真实世界多模态驾驶场景实验中，用稀疏测距重建深度替换纯视觉深度，显著提升了单图像新视角视频生成的几何一致性和视觉质量。

Conclusion: 研究强调了可靠几何先验对基于扩散模型的视角合成的重要性，并证明了即使在极端稀疏水平下，多模态传感也具有实际优势。
...
TL;DR: ZACH-ViT是一种紧凑型视觉Transformer，移除了位置嵌入和[CLS]标记，通过全局平均池化实现排列不变性，在医学图像分类任务中表现出色，特别适用于资源受限的临床环境。
...
Result: 在7个MedMNIST数据集上的评估显示：ZACH-ViT（0.25M参数）在BloodMNIST上表现最佳，在PathMNIST上与TransMIL竞争，在具有强解剖先验的数据集（OCTMNIST, OrganAMNIST）上相对优势减弱；保持亚秒级推理时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Vision-Language-Action模型通常在2D数据上预训练，缺乏3D空间理解能力。现有表示对齐方法通常只在单层应用监督，无法充分利用深度分布信息，而简单的多层对齐会导致梯度干扰。
...
<details>
  <summary>Details</summary>
Motivation: 现有全参考图像质量评估方法依赖参考图像质量，限制了在理想参考源不可用的实际应用。受人类视觉系统积累视觉记忆能力的启发，希望开发能基于长期记忆存储进行质量评估的方法。

Method: 建立存储失真模式的记忆库，设计双模式质量评估策略：有参考图像时，通过自适应加权参考信息并将失真图像与记忆库中的失真模式比较获得质量分数；无参考图像时，依赖记忆库中的失真模式推断图像质量，实现无参考质量评估。

Result: 实验结果显示，该方法在多个数据集上优于最先进方法，同时能适应无参考和全参考两种任务。

Conclusion: 提出的记忆驱动质量感知框架通过模拟人类视觉记忆机制，有效减少了对高质量参考图像的依赖，在有无参考图像的情况下都能实现高质量评估，具有更好的实际应用价值。
...
TL;DR: 该论文提出了首个伪多模态水下目标跟踪基准MUOT_3M（300万帧）和基于SAM的多模态到单模态跟踪器MUTrack，通过视觉几何对齐、视觉语言融合和四级知识蒸馏，在五个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 水下目标跟踪对海洋机器人、生态监测和海洋勘探至关重要，但现有基准数据集规模小且仅支持RGB模态，限制了在严重颜色失真、浑浊和低能见度条件下的鲁棒性。

Method: 1. 构建MUOT_3M基准：包含300万帧、3030个视频（27.8小时），标注32个跟踪属性、677个细粒度类别，同步提供RGB、增强RGB、估计深度和语言模态；2. 提出MUTrack跟踪器：基于SAM架构，采用视觉几何对齐、视觉语言融合和四级知识蒸馏，将多模态知识转移到单模态学生模型中。

Result: 在五个水下目标跟踪基准测试中，MUTrack比最强的SOTA基线高出8.40%的AUC和7.80%的精度，同时以24 FPS实时运行。MUOT_3M基准经过海洋生物学家验证。

Conclusion: MUOT_3M和MUTrack为可扩展、多模态训练但实际可部署的水下跟踪建立了新的基础，解决了现有基准规模小、模态单一的问题，显著提升了水下跟踪性能。
...
TL;DR: 本文提出了一种基于多模态大语言模型的LLM中心情感视觉定制任务，旨在通过高效的情感语义转换和精确的情感无关内容保留来编辑图像的主观情感。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定制研究主要关注控制信号与编辑图像的客观对齐，忽视了主观情感内容，且缺乏通用的情感视觉定制基础模型。因此需要开发能够有效处理情感转换和内容保留的视觉定制方法。

Method: 提出高效精确情感操纵方法，包含两个核心模块：高效情感间转换模块使LLM在编辑前后高效对齐情感语义转换；精确情感外保留模块精确保留情感无关内容。

Result: 在构建的L-AVC数据集上进行全面实验评估，结果表明EPEM方法在L-AVC任务上优于多个最先进的基线方法，证明了情感信息对L-AVC的重要性以及EPEM方法在高效精确操纵情感信息方面的有效性。

Conclusion: 情感信息对LLM中心情感视觉定制任务至关重要，提出的EPEM方法能够高效精确地操纵图像中的情感信息，为情感视觉定制提供了有效的解决方案。
...
TL;DR: 本文提出了深度安全导向视频理解（DeepSVU）新任务，旨在不仅检测威胁，还能归因和评估威胁原因，并提出了统一物理世界正则化MoE（UPRM）方法来解决该任务中的关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有安全导向视频理解（SVU）研究主要关注检测和定位视频中的威胁（如枪击、抢劫），但缺乏有效生成和评估威胁原因的能力。为填补这一空白，本文提出了深度安全导向视频理解（DeepSVU）新任务。

Method: 提出了统一物理世界正则化MoE（UPRM）方法，包含两个关键组件：统一物理世界增强MoE（UPE）块和物理世界权衡正则化器（PTR），分别用于建模从粗到细的物理世界信息和自适应权衡这些因素。

Result: 在DeepSVU指令数据集（UCF-C指令和CUVA指令）上的大量实验表明，UPRM优于多个先进的视频-LLM和非VLM方法，证明了从粗到细的物理世界信息在DeepSVU任务中的重要性以及UPRM在捕获此类信息方面的有效性。

Conclusion: 本文提出的DeepSVU任务扩展了传统SVU的范围，UPRM方法通过有效建模和权衡物理世界信息，显著提升了深度安全视频理解能力，为安全视频分析提供了更全面的解决方案。
...
TL;DR: 提出UAOR（不确定性感知观察重注入）模块，一种无需训练、即插即用的方法，通过不确定性测量和注意力检索机制，在VLA模型中重注入关键观察信息，提升机器人操作性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型为提升性能通常需要额外观察线索（如深度图、点云）或辅助模块（如目标检测器），这些方法需要昂贵的数据收集和额外训练。作者希望开发一种无需训练、即插即用的模块来增强VLA模型的性能。

Method: 提出UAOR模块：当当前语言模型层表现出高不确定性（通过动作熵测量）时，通过注意力检索机制将关键观察信息重注入到下一层的FFN中。该方法无需额外训练，可直接集成到现有VLA模型中。

Result: 综合实验表明，该方法能持续改进多种VLA模型在仿真和真实世界任务中的性能，且开销极小。UAOR无需额外观察线索或模块，成为现有VLA管道的通用实用插件。

Conclusion: UAOR是一种有效、无需训练、即插即用的VLA模型增强模块，通过不确定性感知的观察重注入机制，使模型在推理过程中更好地关注观察信息，生成更自信和可靠的动作。
...
TL;DR: 本文提出DCAG框架，通过同时操纵DiT注意力机制中的Key和Value通道实现无训练编辑强度控制，相比仅使用Key通道的方法能更精确地平衡编辑效果与保真度。


<details>
  <summary>Details</summary>
Motivation: 基于DiT架构的扩散模型需要无训练编辑强度控制。现有方法仅关注Key空间来调节注意力路由，而完全忽略了控制特征聚合的Value空间。

Method: 提出双通道注意力引导(DCAG)框架，同时操纵Key通道（控制注意力位置）和Value通道（控制特征聚合）。基于DiT多模态注意力层中Key和Value投影都呈现明显的偏差-增量结构这一观察，通过二维参数空间(δ_k, δ_v)实现更精确的编辑-保真度权衡。

Result: 在PIE-Bench基准测试（700张图像，10个编辑类别）上，DCAG在所有保真度指标上均优于仅使用Key引导的方法，在对象删除（LPIPS降低4.9%）和对象添加（LPIPS降低3.2%）等局部编辑任务中改进最显著。

Conclusion: 同时操纵Key和Value通道的DCAG框架能够实现更精确的无训练编辑控制，Key通道通过非线性softmax函数提供粗粒度控制，Value通道通过线性加权求和提供细粒度补充，两者结合优于单通道方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统少样本动作识别方法通常使用语义粗糙的类别名称作为辅助上下文，但这种上下文过于有限，无法为捕捉动作中的新颖空间和时间概念提供足够的背景知识。

Method: 提出Decomposition-incorporation框架：1）分解阶段：将原始动作名称解耦为多样化的时空属性描述；2）融合阶段：提出空间/时间知识补偿器（SKC/TKC）来发现判别性的对象级和帧级原型。SKC在空间知识指导下自适应聚合重要补丁标记，TKC利用时间属性辅助帧间时间关系建模。

Result: 实验结果表明DiST在五个标准少样本动作识别数据集上取得了最先进的结果。

Conclusion: 通过利用大语言模型提供的解耦空间和时间知识，DiST框架能够学习表达性强的多粒度原型，有效捕捉细粒度空间细节和多样化时间模式，从而提升少样本动作识别性能。
...
TL;DR: CityGuard：一种用于分散式监控中隐私保护身份检索的拓扑感知Transformer框架，通过自适应度量学习、空间条件注意力和差分隐私嵌入实现跨视角对齐和隐私保护。


<details>
  <summary>Details</summary>
Motivation: 城市规模的人员重识别面临视角变化、遮挡和域偏移等外观变化挑战，同时需要遵守数据保护规则防止原始图像共享。需要在保护隐私的前提下实现跨分布式摄像头的身份检索。

Method: 1. 分散自适应度量学习：根据特征分布调整实例级边界，增强类内紧凑性；2. 空间条件注意力：将粗略几何信息（如GPS或部署平面图）注入基于图的自注意力，实现投影一致的跨视角对齐；3. 差分隐私嵌入映射：结合紧凑近似索引，支持安全且成本高效的部署。

Result: 在Market-1501和其他公共基准测试中，检索精度和查询吞吐量均优于强基线方法，数据库规模的检索研究也证实了框架的实际可行性。

Conclusion: CityGuard框架能够生成对视角变化、遮挡和域偏移具有鲁棒性的描述符，在严格的差分隐私核算下实现隐私与效用的可调平衡，适用于隐私关键的城市身份匹配应用。
...
TL;DR: TCA-T2M是一个时间一致性感知的文本到动作生成框架，通过跨序列时间对齐和运动变换器解决现有方法忽略时间一致性的问题，在HumanML3D和KIT-ML基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段文本到动作生成框架虽然利用离散运动表示取得了进展，但往往忽略了跨序列时间一致性（即相同动作不同实例间共享的时间结构），导致语义错位和物理上不可信的运动。

Method: 提出TCA-T2M框架，包含：1）时间一致性感知空间VQ-VAE（TCaS-VQ-VAE）用于跨序列时间对齐；2）掩码运动变换器用于文本条件运动生成；3）运动学约束块减轻离散化伪影以确保物理可信性。

Result: 在HumanML3D和KIT-ML基准测试中，TCA-T2M实现了最先进的性能，证明了时间一致性在鲁棒和连贯的文本到动作生成中的重要性。

Conclusion: TCA-T2M通过引入时间一致性感知机制，解决了现有文本到动作生成方法中跨序列时间一致性的缺失问题，显著提升了生成动作的语义对齐和物理可信性。
...
TL;DR: 本文提出了一种两阶段训练策略，通过自监督预训练和半监督微调，减少对昂贵BEV标注数据的依赖，同时提升道路标记分割性能。
...
Method: 采用两阶段训练：1) 自监督预训练阶段，将BEVFormer预测通过可微分重投影到图像平面，使用Mask2Former生成的多视角语义伪标签进行训练，并加入时间一致性损失；2) 监督微调阶段，仅需50%数据集进行微调。

Result: 在nuScenes数据集上，该方法比全监督基线提升高达2.5pp mIoU，同时将标注数据使用量减半，总训练时间减少三分之二。
...
TL;DR: DohaScript是一个大规模、多书写者的手写印地语数据集，包含531位贡献者转录相同的六首传统印地语对句，旨在解决德瓦纳格里文字手写数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 尽管德瓦纳格里文字有数亿使用者，但公开可用的手写基准数据集严重不足。现有资源规模有限，主要关注孤立字符或短词，缺乏受控的词汇内容和书写者多样性，无法捕捉德瓦纳格里手写文字的连续性、融合性和结构复杂性。

Method: 收集531位独特贡献者的手写印地语文本，设计为平行风格语料库，所有书写者转录相同的六首传统印地语对句。数据集包含去识别化的人口统计元数据，基于客观清晰度和分辨率标准的严格质量筛选，以及页面级布局难度标注。

Result: 基线实验显示了清晰的质量分离和对未见书写者的强泛化能力，突出了数据集的可靠性和实用价值。DohaScript为低资源脚本环境下连续手写德瓦纳格里文字研究提供了标准化、可复现的基准。

Conclusion: DohaScript填补了德瓦纳格里手写文本数据集的空白，支持手写识别、书写者识别、风格分析和生成建模等任务，为低资源脚本环境下的手写文字研究提供了标准化基准。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在图像和视频生成中表现出色，但其迭代去噪过程计算成本高。现有基于特征缓存和重用的免训练加速方法假设时间稳定性，但多步重用特征可能导致潜在漂移和视觉质量下降。

Method: 提出PrediT框架：1) 将特征预测建模为线性多步问题，使用经典线性多步方法从历史信息预测未来模型输出；2) 在高动态区域激活校正器防止误差累积；3) 动态步长调制机制通过监控特征变化率自适应调整预测范围。

Result: 在各种基于DiT的图像和视频生成模型上实现了高达5.54倍的延迟降低，同时质量退化可忽略不计。

Conclusion: PrediT通过预测而非简单重用的方式，在保持生成保真度的同时显著加速了DiT模型，为扩散模型的实时应用提供了有效解决方案。
...
TL;DR: OODBench：一个自动化构建的视觉语言模型OOD数据评估基准，包含40K实例级OOD数据对，揭示了现有VLMs在OOD数据上性能显著下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常假设数据独立同分布，但现实应用中常遇到分布外数据，这可能带来安全风险。目前缺乏全面评估VLMs处理OOD数据能力的有效基准。

Method: 提出OODBench方法，以最小人工验证的自动化方式构建新基准，包含40K实例级OOD实例-类别对，并提出从基础到高级的渐进式提示问题自动评估指标。

Result: 当前VLMs即使在常见图像类别上，在OODBench上仍表现出显著性能下降。提出的自动化评估指标能更全面地评估OOD数据对不同难度问题的影响。

Conclusion: OODBench为评估VLMs处理OOD数据能力提供了有效基准，总结了重要发现和见解，有助于未来OOD数据获取和评估的研究。
...
<details>
  <summary>Details</summary>
Motivation: 虽然ViTs在多种图像任务中表现出色，但其在可视化图形感知任务中的能力尚未被充分研究，而这些任务对解释可视化至关重要

Method: 基于Cleveland和McGill的经典研究，设计了一系列受控的图形感知任务，将ViTs与CNNs和人类参与者的表现进行对比评估

Result: ViTs在通用视觉任务中表现良好，但在可视化领域的类人图形感知能力有限，与人类感知存在显著差距

Conclusion: ViTs在可视化系统中的实际应用需要谨慎考虑其感知局限性，这对可视化系统和图形感知建模有重要启示
...
TL;DR: BLM-Guard是一个用于短视频广告内容审核的框架，结合思维链推理、规则策略和强化学习，通过多模态分析检测欺骗性内容
...
Method: 融合思维链推理与基于规则的策略原则，采用规则驱动的ICoT数据合成管道生成结构化场景描述、推理链和标签，然后通过强化学习使用复合奖励（平衡因果一致性与策略遵循）优化模型，多任务架构建模模态内操纵和跨模态不匹配

Result: 在真实短视频广告上的实验显示，BLM-Guard在准确性、一致性和泛化能力方面超越了强基线模型

Conclusion: BLM-Guard提供了一个有效的商业广告内容审核框架，通过结合规则驱动数据合成、强化学习和多模态分析，实现了对欺骗性广告内容的精准检测
...
TL;DR: DMC是一个后处理模块，通过自监督数据驱动方法，在保持文本语义一致性的同时，修正文本生成动作中的物理不合理现象（如脚部漂浮）


<details>
  <summary>Details</summary>
Motivation: 当前文本生成人体动作的方法在语义对齐方面进展迅速，但难以同时保证语义和物理真实性。现有方法生成的动作常出现物理不合理现象（如脚部漂浮），需要一种能同时保持语义一致性和物理合理性的解决方案。

Method: 提出Distortion-aware Motion Calibrator (DMC)，这是一个后处理模块，采用自监督数据驱动方法。DMC学习在给定故意扭曲的动作和原始文本描述的情况下，生成物理合理的动作，而不依赖复杂的物理建模。

Result: DMC显著提升动作质量：在T2M上FID分数降低42.74%，在T2M-GPT上降低13.20%，同时达到最高的R-Precision。应用于MoMask等高质量模型时，穿透率降低33.0%，漂浮伪影更接近真实参考。DMC能作为通用后处理框架提升各种文本生成动作模型的物理合理性。

Conclusion: DMC是一个有前景的后处理动作优化框架，能够为任何文本生成动作模型同时提升文本语义一致性和物理合理性，解决了现有方法在物理真实性方面的不足。
...
TL;DR: 本文首次研究了离散图像分词器对抗攻击的脆弱性，提出了高效的应用无关攻击方法，并通过无监督对抗训练提升分词器鲁棒性，为安全多模态基础模型发展迈出重要一步。


<details>
  <summary>Details</summary>
Motivation: 离散图像分词器在多模态系统中应用日益广泛，但其对抗攻击脆弱性尚未被探索。与CLIP编码器不同，离散分词器的安全性研究存在空白，需要填补这一研究空白以开发更安全的多模态基础模型。

Method: 1. 首先制定攻击方法，旨在扰动离散分词器提取的特征并改变提取的标记，这些攻击计算高效、应用无关；2. 提出防御方法，受鲁棒CLIP编码器研究启发，通过无监督对抗训练微调流行分词器，保持其他组件冻结。

Result: 攻击方法在分类、多模态检索和字幕生成任务中均有效；防御方法显著提升了对无监督和端到端监督攻击的鲁棒性，并能很好地泛化到未见过的任务和数据，且可利用未标记图像，比监督对抗训练更通用。

Conclusion: 本研究揭示了分词器鲁棒性在下游任务中的关键作用，为开发安全的多模态基础模型迈出了重要一步。无监督对抗训练方法为提升离散分词器安全性提供了有效途径。
...
Result: DEIG在多个基准测试中在空间一致性、语义准确性和组合泛化方面一致优于现有方法，且可作为即插即用模块轻松集成到标准扩散管道中。

Conclusion: DEIG通过细粒度语义理解和属性泄漏控制，实现了高质量的多实例生成，为可控场景生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 草图为设计师提供了早期时尚概念化的简洁表达媒介，而文本描述则补充了材料、颜色和风格细节。有效结合文本和视觉模态需要在利用文本局部属性指导时遵守草图视觉结构。

Method: 提出LOTS框架，包含多级条件阶段独立编码局部特征于共享潜在空间，以及扩散对引导阶段通过注意力机制在扩散模型去噪过程中整合局部和全局条件。还创建了Sketchy数据集，包含专业草图和非专业草图两个版本。

Result: 实验表明该方法在保持全局结构一致性的同时利用更丰富的局部语义指导，相比现有技术有所改进。创建了首个包含多个文本-草图对的时尚数据集。

Conclusion: LOTS框架通过多级草图-文本引导增强了时尚图像生成，在专业和非专业草图设置下都表现出鲁棒性，为时尚设计提供了有效的多模态生成工具。
...
TL;DR: Diff2DGS：一种用于手术场景重建的两阶段框架，结合扩散模型修复被器械遮挡的组织，并使用可学习变形模型的2D高斯溅射来捕捉动态组织变形，在图像质量和深度精度方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实时重建可变形手术场景对于推进机器人手术、改善外科医生引导和实现自动化至关重要。现有方法在遮挡区域的重建质量有限，且缺乏深度精度评估，因为现有基准数据集缺乏3D真实数据。

Method: 提出Diff2DGS两阶段框架：第一阶段使用基于扩散的视频模块，利用时间先验修复被手术器械遮挡的组织，保持时空一致性；第二阶段采用带有可学习变形模型(LDM)的2D高斯溅射(2DGS)来捕捉动态组织变形和解剖几何结构。

Result: 在EndoNeRF数据集上达到38.02 dB PSNR，在StereoMIS数据集上达到34.40 dB PSNR，均优于现有方法。在SCARED数据集上进行了深度精度定量分析，发现仅优化图像质量不一定能获得最佳3D重建精度，因此进一步优化了深度质量。

Conclusion: Diff2DGS在手术场景重建中实现了外观和几何结构的同时优化，不仅提高了图像质量，还确保了更准确的3D几何重建，为机器人手术提供了更可靠的场景重建解决方案。
...
TL;DR: Luminance-GS++：基于3D高斯泼溅的框架，通过全局自适应亮度调整和局部像素级残差细化，解决多视角捕获中光照变化导致的色彩不一致问题，提升3D重建和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现实环境中复杂的光照变化和相机成像管道的限制导致高质量图像采集困难，特别是在多视角捕获中，光照、传感器响应和ISP配置的差异会引入光度学和色彩不一致性，这违反了现代3D新视角合成方法（如NeRF和3DGS）所依赖的光度一致性假设，导致重建和渲染质量下降。

Method: 提出Luminance-GS++框架，结合全局视图自适应亮度调整和局部像素级残差细化进行精确色彩校正。设计无监督目标，联合强制执行亮度校正以及多视角几何和光度一致性。该方法保留了显式的3DGS公式，不修改底层表示。

Result: 在低光照、过曝和复杂亮度及色彩变化等挑战性场景中展示了最先进的性能。提高了重建保真度，同时保持了实时渲染效率。

Conclusion: Luminance-GS++能够有效处理多视角捕获中的光照变化问题，通过创新的色彩校正方法在保持3DGS实时渲染优势的同时，显著提升了在复杂光照条件下的3D重建和渲染质量。
...
TL;DR: 本文提出了一种基于高斯-拉普拉斯算子(G-LoG)的双参数过滤方法，用于医学图像拓扑特征提取，在多参数持久性模块中表现优于单参数过滤，且MLP模型在拓扑特征上的性能可与复杂深度学习模型相媲美。


<details>
  <summary>Details</summary>
Motivation: 在拓扑数据分析中，构建实用的过滤方法来检测拓扑和几何特征是一个重要任务。本文旨在利用拉普拉斯高斯算子增强医学图像边界的能力，定义更适合多参数持久性模块的特征提取方法。

Method: 1. 提出G-LoG（高斯-拉普拉斯高斯）双参数过滤方法，利用拉普拉斯高斯算子增强医学图像边界
2. 将体积图像建模为有界函数
...
Conclusion: G-LoG双参数过滤方法为医学图像分析提供了一种有效的拓扑特征提取方案，在多参数持久性模块中表现出色，且基于拓扑特征的简单MLP模型能达到与复杂深度学习模型相当的性能，展示了拓扑数据分析在医学图像处理中的潜力。
...
TL;DR: 提出了一种基于头手姿态控制的视频世界模型，用于扩展现实(XR)中的交互式虚拟环境生成


<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型仅接受文本或键盘等粗略控制信号，无法响应XR中用户真实运动追踪，限制了具身交互的实用性

Method: 提出以追踪的头姿态和关节级手姿态为条件的人类中心视频世界模型；评估现有扩散变换器条件策略，提出有效的3D头手控制机制；训练双向视频扩散模型教师，并蒸馏为因果交互系统

Result: 系统能够生成以自我为中心的虚拟环境，支持灵巧的手-物体交互；人类实验显示任务性能提升，感知控制水平显著高于基线

Conclusion: 该模型为XR应用提供了更自然、响应式的虚拟环境生成能力，增强了用户的控制感和交互体验
...
TL;DR: CapNav是一个评估视觉语言模型在考虑智能体物理能力约束下进行室内导航的新基准，包含5种代表性智能体、45个场景、473个任务，发现当前VLM在严格移动约束下性能显著下降


<details>
  <summary>Details</summary>
Motivation: 真实世界导航受智能体移动能力约束（如扫地机器人不能爬楼梯，四足机器人可以），但现有视觉语言导航研究缺乏对智能体具体物理和操作能力的考虑

Method: 提出能力条件导航基准CapNav，定义5种代表性人类和机器人智能体（各有物理尺寸、移动能力和环境交互能力描述），包含45个真实室内场景、473个导航任务和2365个问答对

Result: 评估13个现代VLM发现：1）当前VLM导航性能随移动约束收紧而急剧下降；2）即使最先进模型也难以处理需要空间维度推理的障碍类型
...
TL;DR: 首个实时、完全因果的空间感知对话动作生成方法，可在流式VR头显上部署，结合用户位置和音频生成全身动作，实现300+FPS的实时性能


<details>
  <summary>Details</summary>
Motivation: 当前方法缺乏空间感知能力，而具身智能体需要转向用户、响应用户动作并保持自然注视，这对VR、远程呈现和数字人应用至关重要

Method: 结合因果transformer-based VAE与流匹配模型，使用交错潜在token进行流式推理，引入注视评分机制和分类器无指导来解耦学习与控制

Result: 在Embody 3D数据集上达到最先进的运动质量，超过300 FPS（比非因果基线快3倍），能捕捉自然对话的微妙空间动态，并在实时VR系统中验证

Conclusion: 首次实现了实时、完全因果的空间感知对话动作生成，使空间感知对话智能体能够实时部署，为VR、远程呈现和数字人应用提供了重要技术突破
...
TL;DR: 大学生在学术工作中越来越多地使用生成式AI，但现有制度实践尚未适应这一转变。研究发现制度压力（截止日期、考试周期、评分）和社会因素（同伴群体）共同影响学生的AI使用行为，导致AI使用成为隐蔽的实践。


<details>
  <summary>Details</summary>
Motivation: 高等教育学生越来越多地在学术工作中使用生成式AI，但现有机构实践尚未适应这一转变。需要了解影响学生使用AI的环境和社会因素，以便为机构、教师和系统工具设计者提供有效支持学生学习的建议。

Method: 通过对23名大学生进行半结构化访谈，研究学生使用AI的环境和社会影响因素。

Result: 研究发现：1）制度压力因素（截止日期、考试周期、评分）导致学生即使认为AI会削弱学习效果仍会使用；2）社会影响特别是同伴微社区建立了事实上的AI规范；3）校园普遍存在"AI羞耻感"，推动AI使用地下化；4）现有AI政策被学生视为笼统、不一致且令人困惑，导致常规性违规；5）学生发展基于价值的自我调节策略，但环境压力造成意图与行为之间的差距。

Conclusion: 学生AI使用是一种情境化实践。研究为机构、教师和系统工具设计者提供了有效支持学生学习的启示，需要重新思考AI政策设计，减少AI羞耻感，并更好地协调环境压力与学生学习目标。
...
TL;DR: 平台经济在危机时期如何塑造劳动力市场反应，特别关注性别化体验，指出平台工作既是灵活机会也强化不平等，女性面临更多约束，需要性别敏感政策


<details>
  <summary>Details</summary>
Motivation: 探讨平台经济在危机时期（经济危机、自然灾害、难民流离失所）如何影响劳动力市场，特别关注性别差异，理解数字平台如何同时提供工作机会又强化现有不平等

Method: 基于案例研究的方法，分析经济危机、自然灾害和难民流离失所等不同危机情境，考察数字劳动平台如何运作及其对性别化劳动体验的影响

Result: 平台工作在危机时期可作为"生命线"提供灵活工作机会，但同时也强化了现有不平等；女性因照顾责任、流动性限制和经济不安全等约束面临更多就业障碍和收入潜力限制；平台工作的益处分布不均，并非可靠解决方案

Conclusion: 需要性别敏感的政策和新的研究来理解数字基础设施如何在不同危机情境中介导劳动体验，以制定促进平台工作韧性和公平性的包容性策略，特别是针对边缘化和流离失所群体
...
TL;DR: 本文提出了一种低成本AI天气预警系统架构，使用NVIDIA Earth-2模型，以每月1430-1730美元的成本实现国家级部署，成本比传统雷达低2000-4545倍，通过WhatsApp提供15天全球天气预报。


<details>
  <summary>Details</summary>
Motivation: 2026年1月南非暴雨造成200-300人死亡，暴露了非洲60%地区缺乏有效早期预警系统的问题。传统雷达站每个超过100万美元，导致非洲的覆盖率比美国和欧盟低18倍，需要经济可行的解决方案。

Method: 采用生产级架构部署NVIDIA Earth-2 AI天气模型，使用ProcessPoolExecutor解决异步Python应用中的会话生命周期冲突，建立数据库支持的GPU直接写入PostgreSQL的架构，实现61个时间步的多步推理自动坐标管理。

Result: 2026年2月在南非成功部署，系统能以200毫秒内响应用户查询，通过WhatsApp（市场渗透率80%+）提供15天全球大气预报，使大陆规模的早期预警系统在经济上可行。

Conclusion: 该架构使大陆规模的早期预警系统在经济上可行，支持UNDRR关于此类系统能将灾害死亡率降低6倍的发现，所有架构细节都有完整文档支持可复现性。
...
TL;DR: 论文指出"AI"一词过于宽泛，导致讨论模糊不清，建议在军事领域等具体应用中明确区分不同类型的AI系统，以便进行更有成效的辩论和政策制定。


<details>
  <summary>Details</summary>
Motivation: 当前关于AI的讨论过于笼统，将各种不同类型的系统都归为"AI"，导致批评和辩论缺乏针对性。特别是在军事等安全关键领域，这种模糊性会影响对具体系统风险、责任和决策影响的准确评估。

Method: 以军事领域为案例研究，提出一个松散的枚举式分类法，对"军事AI"这一宽泛术语下的各种系统进行分类，并讨论每类系统面临的独特挑战。

Result: 通过对军事AI系统的分类分析，发现对某一类系统的批评并不总是适用于其他类型的系统。这揭示了当前AI讨论中术语模糊性带来的问题。

Conclusion: 为了使AI辩论更有成效，需要提高讨论的精确性，尽可能避免使用"AI"这一笼统术语。研究人员、开发者和政策制定者应明确他们所指的具体系统类型，并针对性地讨论这些特定系统的潜在利益和风险。
...
### [42] [The 2025 AI Agent Index: Documenting Technical and Safety Features of Deployed Agentic AI Systems](https://arxiv.org/abs/2602.17753)
*Leon Staufer,Kevin Feng,Kevin Wei,Luke Bailey,Yawen Duan,Mick Yang,A. Pinar Ozisik,Stephen Casper,Noam Kolt*

Main category: cs.CY

TL;DR: 2025 AI Agent Index系统性地记录和分析了30个最先进的AI智能体，揭示了开发者透明度不足、安全评估信息缺乏等关键问题。


<details>
  <summary>Details</summary>
Motivation: AI智能体生态系统复杂、快速演变且文档记录不一致，给研究人员和政策制定者带来跟踪困难，需要系统性的信息整理和分析工具。

Method: 基于公开信息和与开发者的邮件通信，收集30个最先进AI智能体的起源、设计、能力、生态系统和安全特性等信息，建立综合索引。

Result: 发现不同开发者透明度差异显著，大多数开发者很少分享关于安全性、评估和社会影响的信息，揭示了行业透明度不足的问题。

Conclusion: 2025 AI Agent Index为研究人员和政策制定者提供了有价值的参考工具，同时指出了AI智能体开发中透明度、安全评估和社会影响考量方面的不足。
...
### [43] [Strange Undercurrents: A Critical Outlook on AI's Cultural Influence](https://arxiv.org/abs/2602.17841)
*Dejan Grba*

Main category: cs.CY

TL;DR: 论文探讨生成式AI对艺术观念的影响，聚焦计算机科学基础概念与AI行业意识形态向艺术、文化和社会转移的问题


<details>
  <summary>Details</summary>
Motivation: 当前对生成式AI的讨论缺乏对艺术不断变化特性的细致理解，且忽视了AI科学与行业的概念和意识形态基础如何通过生成式AI传播到主流文化和经济中

Method: 以当前生成式AI的争议为切入点，总结更广泛的AI对艺术观念影响的研究，聚焦计算机科学基础概念与AI行业意识形态向艺术领域的转移

Result: 研究发现AI的影响融合了多样且有时不一致但又能凝聚的哲学前提、技术理念和政治观点，其中许多带有不利的基调

Conclusion: 生成式AI不仅需要技术层面的审视，更需要对其传播的计算机科学基础概念和AI行业意识形态如何影响艺术观念进行深入批判性分析
...
### [44] [Visual Anthropomorphism Shifts Evaluations of Gendered AI Managers](https://arxiv.org/abs/2602.17919)
*Ruiqing Han,Hao Cui,Taha Yasseri*

Main category: cs.CY

TL;DR: 研究发现：能力线索可减少AI经理评价中的性别偏见，但效果取决于AI的呈现方式。文本描述中能力主导评价，而视觉面部呈现则引发性别偏见。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨能力线索是否能减少AI经理评价中的性别偏见，以及这种效果是否取决于AI的呈现方式（文本vs视觉）。旨在理解不同呈现模态如何影响性别刻板印象的激活。

Method: 采用两个预注册实验（N=2,505），使用2×2×3设计：操纵AI性别（男/女）、能力（高/低）和决策结果（有利/不利）。比较文本描述与使用反向相关范式生成的AI面部图像两种呈现方式。

Result: 文本条件下，评价由能力而非性别驱动；不利决策时，高能力AI经理被认为更公平、更有能力、领导力更强。视觉面部呈现时，能力线索影响减弱，出现系统性性别差异：女性化经理被认为更有能力和更可信，特别是在有利结果时。不利结果时性别效应减弱。

Conclusion: 能力信息可在文本交互中减轻对AI经理的负面反应，而面部拟人化会引发文本环境中未观察到的性别偏见。呈现模态在决定AI系统评价中性别刻板印象何时被激活方面起关键作用，设计选择对AI治理具有重要影响。
...
### [45] [Atrial Fibrillation Detection Using Machine Learning](https://arxiv.org/abs/2602.18036)
*Ankit Singh,Vidhi Thakur,Nachiket Tapas*

Main category: cs.CY

TL;DR: 本文提出了一种基于机器学习的房颤检测框架，使用PPG和ECG信号，通过特征提取和多种分类器实现高精度检测。


<details>
  <summary>Details</summary>
Motivation: 房颤是常见的心律失常，也是缺血性中风的主要风险因素。早期通过非侵入性信号检测房颤可以实现及时干预。
...
Result: 子空间KNN获得最高测试准确率98.7%，略优于袋装决策树（97.9%）和立方核SVM（97.1%）。最佳模型的敏感性和特异性均超过95%。

Conclusion: 基于集成机器学习模型结合PPG和ECG特征可以有效检测房颤。研究还提供了模型性能的比较分析以及所提框架的优势和局限性。
...
### [46] [Demonstrating Restraint](https://arxiv.org/abs/2602.18139)
*L. C. R. Patell,O. E. Guest*

Main category: cs.CY

TL;DR: 论文探讨美国如何通过克制策略避免因AI发展引发他国预防性攻击，分析在AI能力优势确定和不确定两种情况下克制策略的可行性。


<details>
  <summary>Details</summary>
Motivation: 美国AI快速发展可能被敌对国家视为生存威胁，从而引发预防性攻击。为避免这种冲突，需要探讨美国如何通过克制策略来增强自身安全。

Method: 借鉴国际关系学中关于国家可信承诺的文献，提出美国实施克制策略的一系列选项，分析在AI能力确定和不确定两种情境下的可行性。

Result: 在AI能力确定的情况下，可信克制承诺困难，但政策努力与技术突破结合可能实现；在能力不确定的情况下，克制策略更可行。

Conclusion: 克制策略虽然面临困难，但值得与避免AI过渡期冲突的其他策略进行比较权衡。
...
### [47] [Computer Vision in Tactical AI Art](https://arxiv.org/abs/2602.18189)
*Dejan Grba*

Main category: cs.CY

TL;DR: 本文探讨了AI艺术如何批判性地回应计算机视觉技术的社会政治影响，分析了战术性AI艺术的特征、挑战和发展方向。


<details>
  <summary>Details</summary>
Motivation: AI艺术作为对人工智能发展、AI驱动经济扩张及其社会文化影响的创造性回应，需要批判性地审视计算机视觉技术在现代AI中的自动化推理和决策所涉及的问题性社会政治方面。

Method: 作者首先识别和背景化与计算机视觉相关的战术性AI艺术，然后从四个相互关联的主题领域分析代表性艺术作品的特征，通过它们的主题重叠、共同批判点和共享缺陷来描绘战术性AI艺术的更广泛图景。

Result: 通过分析发现，战术性AI艺术在诗意说服力、社会责任和政治影响力方面受到多种因素影响，其中一些因素存在于数字艺术行动主义的理论前提中。

Conclusion: 基于分析结果，作者提出了应对挑战和推进该领域发展的路线图，强调需要解决战术性AI艺术在批判计算机视觉技术社会政治影响时的有效性和影响力问题。
...
### [48] [Ontology-Guided Neuro-Symbolic Inference: Grounding Language Models with Mathematical Domain Knowledge](https://arxiv.org/abs/2602.17826)
*Marcelo Labre*

Main category: cs.AI

TL;DR: 该研究探索使用形式化领域本体（OpenMath）通过检索增强生成来提升语言模型在数学推理中的可靠性，发现本体引导的上下文在检索质量高时能改善性能，但无关上下文会降低性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型存在幻觉、脆弱性和缺乏形式化基础等根本限制，这在需要可验证推理的高风险专业领域尤为成问题。研究旨在探索形式化领域本体是否能通过检索增强生成来增强语言模型的可靠性。

Method: 使用数学作为概念验证，实现了一个神经符号管道，利用OpenMath本体进行混合检索和交叉编码器重排序，将相关定义注入模型提示中。在MATH基准测试中评估了三个开源模型。

Result: 评估显示，当检索质量高时，本体引导的上下文能提高性能，但无关上下文会主动降低性能，这凸显了神经符号方法的潜力和挑战。

Conclusion: 形式化本体在提升语言模型可靠性方面具有潜力，但检索质量至关重要，无关上下文会损害性能，这为神经符号方法的发展指明了方向。
...
### [49] [The Token Games: Evaluating Language Model Reasoning with Puzzle Duels](https://arxiv.org/abs/2602.17831)
*Simon Henniger,Gabriel Poesia*

Main category: cs.AI

TL;DR: TTG是一个基于16世纪数学决斗启发的评估框架，让大语言模型通过相互创建编程谜题来挑战对方，从而评估推理能力，无需人工创建问题。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型推理能力面临挑战：人工创建难题成本高昂，且难以确定模型是否真正推理还是见过类似训练数据。需要一种无法通过设计饱和的评估范式。

Method: 采用16世纪数学决斗格式，让模型通过创建编程谜题相互挑战。使用编程谜题格式（给定返回布尔值的Python函数，找到使其返回True的输入）灵活表示问题并验证解决方案。通过两两对决结果计算Elo评分来比较模型。

Result: 评估了10个前沿模型，结果与现有基准（如Humanity's Last Exam）排名高度匹配，且无需人工创建谜题。发现创建优质谜题对当前模型仍是极具挑战的任务，这是先前基准未测量的能力。
...
### [50] [WorkflowPerturb: Calibrated Stress Tests for Evaluating Multi-Agent Workflow Metrics](https://arxiv.org/abs/2602.17990)
*Madhav Kanda,Pedro Las-Casas,Alok Gautam Kumbhare,Rodrigo Fonseca,Sharad Agarwal*

Main category: cs.AI

TL;DR: WorkflowPerturb是一个用于评估工作流评估指标的基准测试，通过对黄金工作流施加可控扰动来研究不同指标的性能表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的系统生成复杂任务的结构化工作流，但自动评估这些工作流很困难，因为指标分数通常未校准，且分数变化不能直接反映工作流退化的严重程度。

Method: 引入WorkflowPerturb基准，通过对黄金工作流施加三种现实可控扰动（缺失步骤、压缩步骤、描述变化），每种扰动在10%、30%、50%的严重级别上应用，包含4,973个黄金工作流和44,757个扰动变体。

Result: 基准测试了多个指标家族，使用预期分数轨迹和残差分析其敏感性和校准性，结果揭示了不同指标家族之间的系统性差异，支持基于严重程度的工作流评估分数解释。

Conclusion: WorkflowPerturb为工作流评估指标提供了受控基准，有助于理解不同指标的敏感性和校准特性，支持更准确地解释工作流评估分数，数据集将在接受后发布。
...
### [51] [Cross-Embodiment Offline Reinforcement Learning for Heterogeneous Robot Datasets](https://arxiv.org/abs/2602.18025)
*Haruki Abe,Takayuki Osa,Yusuke Mukuta,Tatsuya Harada*

Main category: cs.AI

TL;DR: 该研究结合离线强化学习和跨具身学习来解决机器人策略预训练中高质量演示数据稀缺的问题，通过构建包含16个机器人平台的数据集验证了该方法的有效性，并提出了基于形态相似性的分组策略来缓解多机器人学习中的梯度冲突问题。


<details>
  <summary>Details</summary>
Motivation: 机器人策略预训练面临高质量演示数据收集成本高昂的问题，不同机器人平台需要各自的演示数据，这限制了规模化预训练的可行性。

Method: 结合离线强化学习和跨具身学习，利用专家和次优数据，聚合异构机器人轨迹来获取通用控制先验。当次优数据比例和机器人类型增加时，引入基于形态相似性的分组策略，将机器人按形态聚类并使用组梯度更新模型。

Result: 实验证明该方法在包含丰富次优轨迹的数据集上预训练效果优于纯行为克隆。但随着次优数据比例和机器人类型增加，形态间的梯度冲突会阻碍学习。提出的静态分组策略能显著减少机器人间冲突，优于现有冲突解决方法。

Conclusion: 离线强化学习与跨具身学习的结合为机器人策略预训练提供了有效途径，但需要解决多机器人学习中的梯度冲突问题。基于形态相似性的简单静态分组策略是缓解这一问题的有效方法。
...
TL;DR: 研究发现无监督表示学习即使排除敏感属性，仍会泄露年龄、收入等敏感信息，公平性通过无知假设在表示层面失效。


<details>
  <summary>Details</summary>
Motivation: 挑战"公平性通过无知"的假设，即认为在训练中排除敏感属性就能获得中性表示。作者发现即使明确排除敏感属性，无监督表示仍会泄露这些信息。

Method: 使用SOMtime（基于高容量自组织映射的拓扑保持表示方法），在两个大规模真实数据集（五个国家的世界价值观调查和人口普查收入数据集）上测试，比较PCA、UMAP、t-SNE和自编码器等方法。

Result: SOMtime在排除敏感属性的情况下仍能恢复与年龄、收入等敏感属性对齐的单调排序，Spearman相关系数高达0.85，而其他方法通常低于0.23。无监督分割SOMtime嵌入会产生人口统计学偏斜的聚类。

Conclusion: 公平性通过无知在表示层面对序数敏感属性失效，公平性审计必须扩展到机器学习管道的无监督组件。无监督表示并非中性，会泄露敏感信息。
...
TL;DR: OMAD：首个在线多智能体强化学习框架，采用扩散策略协调智能体，通过松弛策略目标最大化联合熵实现高效探索，无需可处理似然函数


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成和离线设置中表现出卓越的表达能力和多模态表示能力，但在在线多智能体强化学习中的应用尚未充分探索。主要障碍是扩散模型的不可处理似然函数阻碍了基于熵的探索和协调。

Method: 提出OMAD框架：1）松弛策略目标最大化缩放联合熵，实现有效探索；2）在集中训练分散执行范式下，使用联合分布值函数优化分散扩散策略；3）利用可处理的熵增强目标指导扩散策略同时更新，确保稳定协调。

Result: 在MPE和MAMuJoCo的10个多样化任务上建立新的最先进性能，样本效率显著提高2.5倍到5倍。

Conclusion: OMAD成功将扩散策略应用于在线多智能体强化学习，通过创新的松弛策略目标和联合分布值函数优化，克服了扩散模型似然不可处理的挑战，实现了卓越的协调性能和样本效率。
...
### [54] [When & How to Write for Personalized Demand-aware Query Rewriting in Video Search](https://arxiv.org/abs/2602.17667)
*Cheng cheng,Chenxing Wang,Aolin Li,Haijun Wu,Huiyun Hu,Juyuan Wang*

Main category: cs.IR

TL;DR: WeWrite是一个个性化需求感知的查询重写框架，通过自动化挖掘策略、混合训练范式和并行架构，在视频搜索系统中提升个性化查询重写效果，显著提高了点击率和降低了查询重构率。


<details>
  <summary>Details</summary>
Motivation: 视频搜索系统中，用户历史行为为识别搜索意图和消除歧义提供了丰富上下文，但传统使用隐式历史特征的方法存在信号稀释和延迟反馈的问题。

Method: 提出WeWrite框架，解决三个关键挑战：1) 何时重写：基于后验的自动化挖掘策略从用户日志中提取高质量样本；2) 如何重写：结合监督微调和组相对策略优化的混合训练范式；3) 部署：采用并行"假召回"架构确保低延迟。

Result: 在大规模视频平台上的在线A/B测试显示，WeWrite将点击率（VV>10s）提高了1.07%，并将查询重构率降低了2.97%。
...
### [55] [IRPAPERS: A Visual Document Benchmark for Scientific Retrieval and Question Answering](https://arxiv.org/abs/2602.17687)
...
TL;DR: IRPAPERS是一个包含3,230页科学论文的基准数据集，用于比较基于图像和基于文本的文档检索与问答系统。研究发现两种模态具有互补性，多模态混合搜索优于单一模态，图像嵌入在某些情况下优于文本嵌入。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在文本和关系数据处理方面取得了显著成功，但视觉文档处理仍相对未被充分探索。传统系统需要OCR转录将视觉文档转换为文本和元数据，而多模态基础模型可以直接从文档图像进行检索和生成。这引发了一个关键问题：基于图像的系统与成熟的基于文本方法相比表现如何？

Method: 引入IRPAPERS基准数据集，包含166篇科学论文的3,230页，每页都有图像和OCR转录。使用180个"大海捞针"式问题，比较基于图像和基于文本的检索与问答系统。评估了多种检索方法（Arctic 2.0嵌入、BM25、混合搜索）和图像嵌入模型（包括Cohere Embed v4、Voyage 3 Large等）。
...
Conclusion: 多模态方法在视觉文档处理中具有优势，图像和文本表示具有互补的局限性。某些问题类型需要特定模态。增加检索深度对所有系统都有显著好处，多文档检索优于oracle单文档检索。IRPAPERS数据集和实验代码已公开可用。
...
### [56] [Enhancing Scientific Literature Chatbots with Retrieval-Augmented Generation: A Performance Evaluation of Vector and Graph-Based Systems](https://arxiv.org/abs/2602.17856)
*Hamideh Ghanadian,Amin Kamali,Mohammad Hossein Tekieh*

Main category: cs.IR

TL;DR: 该研究通过检索增强生成技术改进科学文献聊天机器人，重点评估向量和图检索系统，使用混合数据库实现高效文献筛选，并通过两个用例场景进行系统性能评估。


<details>
  <summary>Details</summary>
Motivation: 提升科学文献聊天机器人的性能，通过结合结构化和非结构化数据检索，改善科学知识的可访问性，支持基于证据的决策制定。

Method: 提出基于检索增强生成的聊天机器人，结合图数据库（结构化）和向量数据库（非结构化）检索科学文献；通过两个用例场景评估：单文档检索和大规模语料库检索；使用GPT模型生成基准测试集，并进行人工标注评估。

Result: 比较分析显示，混合RAG系统在检索准确性和响应相关性方面表现出色，揭示了向量和图检索方法各自的优势和局限性，证明了混合系统提升科学知识可访问性的潜力。

Conclusion: 混合检索增强生成系统能够有效改进科学文献聊天机器人的性能，为科学知识获取和证据决策提供有力支持，展示了结合不同检索方法的实用价值。
...
### [57] [SuiteEval: Simplifying Retrieval Benchmarks](https://arxiv.org/abs/2602.18107)
*Andrew Parry,Debasis Ganguly,Sean MacAvaney*

Main category: cs.IR

TL;DR: SuiteEval是一个统一的信息检索评估框架，提供自动端到端评估、动态索引重用和主流基准测试支持，旨在解决评估实践碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 信息检索评估存在碎片化实践问题——不同的数据集子集、聚合方法和管道配置——这损害了可重复性和可比性，特别是对于需要强大跨域性能的基础嵌入模型。

Method: 引入SuiteEval框架，提供自动端到端评估、动态索引重用磁盘索引以最小化磁盘使用，并内置支持主要基准测试（BEIR、LoTTE、MS MARCO、NanoBEIR和BRIGHT）。用户只需提供管道生成器，框架处理数据加载、索引、排序、指标计算和结果聚合。

Result: SuiteEval减少了样板代码并标准化了评估流程，促进了可重复的信息检索研究，特别是随着需要更广泛基准测试集的需求增加。
...
### [58] [A Simple yet Effective Negative Sampling Plugin for Constructing Positive Sample Pairs in Implicit Collaborative Filtering](https://arxiv.org/abs/2602.18206)
*Jiayi Wu,Zhengyu Wu,Xunkai Li,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 提出PSP-NS负采样插件，通过增强正样本监督信号来改进隐式协同过滤推荐系统，特别关注正样本利用和解决用户活跃度偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有隐式协同过滤模型主要关注高质量负样本设计，但忽视了正样本的探索。现有的去噪推荐方法虽然可用于正样本去噪，但往往稀疏化了正监督信号，且普遍忽略了训练过程中的用户活跃度偏差，导致对非活跃用户学习不足。

Method: 提出PSP-NS负采样插件：1）构建用户-物品二部图，边权重表示基于全局和局部模式推断的交互置信度；2）通过基于复制的重加权生成正样本对以增强正信号；3）采用活动感知加权方案有效学习非活跃用户偏好。

Result: 在四个真实世界数据集上进行了广泛实验，证明了其优越性。例如在Yelp数据集上，PSP-NS将Recall@30和Precision@30分别提升了32.11%和22.90%。该插件可与各种隐式CF推荐器或负采样方法集成以提升性能。

Conclusion: PSP-NS是一个简单有效的负采样插件，从增强正监督信号的角度出发，通过置信度推断、正样本对重加权和活动感知加权，有效解决了现有方法忽视正样本探索和用户活跃度偏差的问题，显著提升了推荐质量。
...
### [59] [The Economical-Ecological Benefits of Matching Non-matching Socks](https://arxiv.org/abs/2602.18221)
*Teddy Lazebnik*

Main category: cs.IR

TL;DR: 研究量化了配对不匹配的"孤儿袜"的经济和生态价值，分析了阻碍这种行为的社会成本，通过建模和实验发现适度容忍不匹配比严格匹配更可持续。


<details>
  <summary>Details</summary>
Motivation: 袜子大规模生产和更换，但成对使用使其特别容易浪费——丢失一只袜子会浪费可用穿着容量并导致过早更换。研究旨在量化配对不匹配袜子的价值及阻碍这种行为的社会成本。

Method: 将袜子所有权建模为不确定性下的序列决策问题，袜子在使用和洗涤中随机磨损和丢失，公共暴露引入个人特定的不匹配惩罚。通过现场研究估计不匹配敏感性和多样性偏好，将行为异质性与最优配对策略联系起来。使用计算机模拟评估可解释的配对策略。

Result: 严格匹配看似节约资源主要是因为产生许多无袜可穿的日子，而适度容忍不匹配能维持服务并减少不同丢失情况下的闲置容量。研究建立了配对不匹配袜子的可行性。

Conclusion: 配对不匹配袜子具有经济和生态价值，但受社会成本限制。适度容忍不匹配比严格匹配更可持续，能减少浪费并维持服务。研究为配对策略提供了理论基础和实践指导。
...
### [60] [Dual-Tree LLM-Enhanced Negative Sampling for Implicit Collaborative Filtering](https://arxiv.org/abs/2602.18249)
*Jiayi Wu,Zhengyu Wu,Xunkai Li,Rong-Hua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 提出了一种无需文本信息和微调的LLM增强负采样方法DTL-NS，通过双树结构识别假负样本并挖掘高质量硬负样本，显著提升推荐系统性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的负采样方法过度依赖文本信息和任务特定微调，限制了实际应用。需要开发一种无需文本信息和微调的LLM增强负采样方法。

Method: 提出DTL-NS方法，包含两个模块：1）离线假负样本识别模块，利用层次索引树将协同结构和潜在语义信息转换为结构化项目ID编码供LLM推理；2）多视图硬负采样模块，结合用户-项目偏好分数和项目间层次相似性挖掘高质量硬负样本。

Result: 在Amazon-sports数据集上，DTL-NS在Recall@20和NDCG@20指标上分别比最强基线提升10.64%和19.12%。该方法可集成到多种隐式CF模型和负采样方法中，持续提升性能。

Conclusion: DTL-NS是一种有效且实用的LLM增强负采样方法，无需文本信息和微调，通过双树结构显著提升推荐系统性能，具有广泛的适用性。
...
### [61] [A Topology-Aware Positive Sample Set Construction and Feature Optimization Method in Implicit Collaborative Filtering](https://arxiv.org/abs/2602.18288)
*Jiayi Wu,Zhengyu Wu,Xunkai Li,Rong-Hua Li,Guoren Wang*

Main category: cs.IR

TL;DR: TPSC-FO方法通过拓扑社区结构识别假阴性样本并将其转化为正样本，结合邻域引导的特征优化，提升隐式协同过滤的性能。
...
Method: 提出TPSC-FO方法：1) 拓扑感知正样本集构建模块，使用差分社区检测策略捕获隐式反馈中的拓扑社区结构，结合个性化噪声过滤识别假阴性并转化为正样本；2) 邻域引导特征优化模块，通过融入嵌入空间中的邻域特征来精炼正样本特征。
...
TL;DR: Neural-HSS：基于层次半可分矩阵结构的高效参数架构，在低数据量下仍能保持精确性，适用于广泛PDE问题


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在求解PDE方面表现出色，但大规模高质量数据集的生成和模型训练仍面临巨大计算成本，限制了关键应用的发展

Method: 受椭圆PDE格林函数结构研究启发，提出基于层次半可分矩阵结构的参数高效架构Neural-HSS，理论上分析其精确性，并探讨其与傅里叶神经算子层和卷积层等其他架构原语的联系

Result: 在200万点网格的三维泊松方程上验证了Neural-HSS的数据效率，在低数据量下学习椭圆PDE生成的数据方面表现出色，优于基线方法；同时展示了其在电磁学、流体动力学和生物学等多个领域广泛PDE问题中的学习能力

Conclusion: Neural-HSS架构在保持参数效率的同时，在低数据量下对广泛PDE类别具有数据效率，为解决计算成本高的PDE问题提供了有效方案
...
### [63] [Reducing Text Bias in Synthetically Generated MCQAs for VLMs in Autonomous Driving](https://arxiv.org/abs/2602.17677)
*Sutej Kulgod,Sean Ye,Sanchit Tanwar,Christoffer Heckman*

Main category: cs.LG

TL;DR: 该研究发现基于合成数据的多选问答基准存在文本线索漏洞，导致视觉语言模型无需视觉输入即可获得高准确率。作者提出方法消除文本捷径，迫使模型依赖视觉理解。


<details>
  <summary>Details</summary>
Motivation: 多选问答基准是评估视觉语言模型在驾驶任务中性能的标准方法，但研究发现合成生成的MCQA数据存在隐藏的文本线索，使模型可以利用语言模式而非视觉上下文，导致评估结果失真。

Method: 通过将正确答案与语言伪影解耦，并采用课程学习策略，减少模型对文本捷径的依赖，强制模型进行视觉基础化，确保性能准确反映感知理解能力。

Result: 提出的方法将盲目准确率从比随机高+66.9%降低到+2.9%，消除了绝大多数可利用的文本捷径，使模型必须依赖视觉输入才能获得良好性能。

Conclusion: 合成MCQA基准存在严重的文本线索漏洞，需要改进评估方法以确保视觉语言模型的性能真正反映其视觉理解能力，而非利用语言模式。
...
### [64] [BioBridge: Bridging Proteins and Language for Enhanced Biological Reasoning with LLMs](https://arxiv.org/abs/2602.17680)
*Yujia Wang,Jihong Guan,Wengen Li,Shuigeng Zhou,Xuhong Wang*

Main category: cs.LG

TL;DR: BioBridge是一个结合蛋白质语言模型和通用大语言模型优势的领域自适应持续预训练框架，通过领域增量持续预训练和跨模态对齐，在蛋白质理解和通用理解任务上都取得了良好表现。
...
Method: 提出BioBridge框架，采用领域增量持续预训练(DICP)同时注入蛋白质领域知识和通用推理语料，通过PLM-Projector-LLM管道实现跨模态对齐，将蛋白质序列嵌入映射到语言模型的语义空间，采用端到端优化支持多种任务。

Result: BioBridge在EC和BindingDB等多个蛋白质基准测试中表现与主流蛋白质语言模型相当，在MMLU和RACE等通用理解任务上达到与大语言模型相当的结果，展示了领域特定适应性与通用语言能力的创新结合优势。

Conclusion: BioBridge成功结合了蛋白质语言模型的领域专业性和大语言模型的通用推理能力，通过创新的持续预训练框架实现了蛋白质理解和通用语言理解的双重优势，为生物信息学领域提供了有效的多模态解决方案。
...
### [65] [LATMiX: Learnable Affine Transformations for Microscaling Quantization of LLMs](https://arxiv.org/abs/2602.17681)
*Ofir Gordon,Lior Dikstein,Arnon Netzer,Idan Achituve,Hai Victor Habi*

Main category: cs.LG

TL;DR: 本文提出LATMiX方法，通过可学习的可逆仿射变换优化激活分布，显著提升大语言模型在微缩放量化格式下的低比特量化性能。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法主要关注传统量化方案，而现代硬件越来越多地支持微缩放数据格式。现有方法在结合可逆变换和微缩放量化时会出现严重的性能下降，且现有变换方法主要局限于旋转或Hadamard变换。

Method: 首先对微缩放量化下的变换进行理论分析，推导量化误差边界。基于此分析提出LATMiX方法，将异常值减少推广到可学习的可逆仿射变换，使用标准深度学习工具进行优化。

Result: 实验表明，在广泛的零样本基准测试和多种模型尺寸上，LATMiX方法在微缩放低比特量化方面相比强基线实现了平均准确率的持续提升。

Conclusion: 通过理论分析和可学习变换的结合，LATMiX方法成功解决了微缩放量化格式下的性能下降问题，为大语言模型的高效部署提供了有效解决方案。
...
### [66] [Probabilistic NDVI Forecasting from Sparse Satellite Time Series and Weather Covariates](https://arxiv.org/abs/2602.17683)
*Irene Iele,Giulia Romoli,Daniele Molino,Elena Mulero Ayllón,Filippo Ruffini,Paolo Soda,Matteo Tortora*

Main category: cs.LG

TL;DR: 提出基于Transformer的概率预测框架，用于卫星NDVI指数预测，通过分离历史植被动态与未来外生信息建模，引入时间距离加权分位数损失处理不规则采样，在点预测和概率预测指标上优于多种基线方法。


<details>
  <summary>Details</summary>
Motivation: 精准农业需要准确的植被动态短期预测，但卫星NDVI预测面临云层遮挡导致采样稀疏不规则以及作物生长的异质性气候条件等挑战。

Method: 提出概率预测框架，使用Transformer架构明确分离历史植被动态与未来外生信息建模，整合历史NDVI观测与历史和未来气象协变量；引入时间距离加权分位数损失处理不规则重访模式；加入累积和极端天气特征工程捕捉延迟气象效应。

Result: 在欧洲卫星数据上的广泛实验表明，该方法在点预测和概率预测指标上一致优于统计、深度学习和近期时间序列基线方法；消融研究显示目标历史信息起核心作用，而气象协变量联合使用时提供互补增益。
...
### [67] [CodeScaler: Scaling Code LLM Training and Test-Time Inference via Execution-Free Reward Models](https://arxiv.org/abs/2602.17684)
*Xiao Zhu,Xinyu Zhou,Boyu Zhu,Hanxu Hu,Mingzhe Du,Haotian Zhang,Huiming Wang,Zhijiang Guo*

Main category: cs.LG

TL;DR: CodeScaler：无需执行的奖励模型，通过语法感知的代码提取和保持有效性的奖励塑造，显著提升代码生成模型的性能，超越基于单元测试的强化学习方法，并实现10倍延迟降低。


<details>
  <summary>Details</summary>
Motivation: 基于可验证奖励的强化学习（RLVR）依赖单元测试的执行反馈，但其可扩展性受限于高质量测试用例的可用性和可靠性。需要一种无需执行的奖励模型来扩展强化学习训练和推理。

Method: 提出CodeScaler奖励模型，在已验证代码问题的基础上构建精心策划的偏好数据，采用语法感知的代码提取和保持有效性的奖励塑造技术，确保稳定鲁棒的优化。

Result: 在五个编码基准测试中，CodeScaler将Qwen3-8B-Base平均提升11.72分，超越基于执行的RL方法1.82分；在无测试用例的合成数据集上实现可扩展强化学习；推理时延迟降低10倍；在RM-Bench上超越现有奖励模型。

Conclusion: CodeScaler作为一种无需执行的奖励模型，有效解决了基于测试的强化学习的可扩展性限制，在代码生成任务中实现了性能提升和延迟降低，并在多个领域超越了现有奖励模型。
...
### [68] [Optimal Multi-Debris Mission Planning in LEO: A Deep Reinforcement Learning Approach with Co-Elliptic Transfers and Refueling](https://arxiv.org/abs/2602.17685)
*Agni Bandyopadhyay,Gunther Waxenegger-Wilfing*

Main category: cs.LG

TL;DR: 论文提出了一种用于低地球轨道多目标主动碎片移除的统一共椭圆机动框架，结合了霍曼转移、安全椭圆接近操作和显式燃料补给逻辑，并比较了三种规划算法的性能。


<details>
  <summary>Details</summary>
Motivation: 解决低地球轨道多目标主动碎片移除的挑战，需要高效的轨道机动规划方法来处理随机碎片场、禁飞区和燃料约束等复杂条件。

Method: 提出了统一的共椭圆机动框架，结合霍曼转移、安全椭圆接近操作和显式燃料补给逻辑。在真实的轨道仿真环境中，对三种规划算法进行了基准测试：贪婪启发式算法、蒙特卡洛树搜索和基于掩码近端策略优化的深度强化学习。

Result: 在100个测试场景中，掩码PPO算法表现出最优的任务效率和计算性能，访问的碎片数量是贪婪算法的两倍，并且在运行时间上显著优于蒙特卡洛树搜索。

Conclusion: 现代强化学习方法在可扩展、安全和资源高效的空间任务规划方面具有巨大潜力，为未来主动碎片移除自主性的发展铺平了道路。
...
### [69] [EXACT: Explicit Attribute-Guided Decoding-Time Personalization](https://arxiv.org/abs/2602.17695)
*Xin Yu,Hanwen Xing,Lingzhou Xue*

Main category: cs.LG

TL;DR: EXACT是一种解码时个性化方法，通过可解释属性对齐生成与有限成对偏好反馈，优于现有基线方法


<details>
  <summary>Details</summary>
Motivation: 现有解码时个性化方法依赖隐式、难以解释的偏好表示，且采用僵化的上下文无关用户表示，无法处理偏好随提示变化的问题

Method: EXACT使用预定义的可解释属性集：1）离线阶段通过最大化偏好响应似然识别用户特定属性子集；2）在线推理时检索与输入提示最相关的属性并注入上下文引导生成

Result: 在温和假设下建立了理论近似保证，证明相似性检索机制能有效缓解上下文偏好偏移；在人工标注偏好数据集上的实验显示，EXACT在偏好建模准确性和个性化生成质量上持续优于强基线

Conclusion: EXACT通过可解释属性实现解码时个性化，能适应不同任务而不混合冲突偏好，为个性化对齐提供了有效解决方案
...
### [70] [AnCoder: Anchored Code Generation via Discrete Diffusion Models](https://arxiv.org/abs/2602.17688)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在代码生成中经常产生无法执行的程序，因为它们未能尊重编程语言的刚性结构

Method: 引入AnchorTree框架，利用抽象语法树作为结构化层次先验，优先解析关键词和标识符等关键标记，建立结构脚手架指导生成

Result: 通过AnCoder模型验证，结构锚定的扩散方法为高质量代码生成提供了参数高效的路径

Conclusion: AnchorTree框架通过显式锚定扩散过程于代码原生结构，有效解决了现有扩散模型在代码生成中的结构不匹配问题
...
### [71] [Robust Pre-Training of Medical Vision-and-Language Models with Domain-Invariant Multi-Modal Masked Reconstruction](https://arxiv.org/abs/2602.17689)
*Melika Filvantorkaman,Mohsen Piri*

Main category: cs.LG

TL;DR: 提出Robust-MMR框架，通过自监督预训练增强医学视觉语言模型在领域偏移下的鲁棒性
...
Method: 提出Robust Multi-Modal Masked Reconstruction (Robust-MMR)框架，整合非对称扰动感知掩码、领域一致性正则化和模态弹性约束，鼓励领域不变表示

Result: 在VQA-RAD上达到78.9%跨域准确率，比最强基线高3.8个百分点；在SLAKE和VQA-2019上分别达到74.6%和77.0%；扰动评估下VQA-RAD准确率从69.1%提升至75.6%；MELINDA跨域准确率从70.3%提升至75.2%；检索实验中平均排名退化从超过16降至4.1
...
TL;DR: 本文研究了在昇腾NPU上对推理导向模型进行训练后量化的可行性和局限性，发现4位权重量化对大型模型可行，但4位权重-激活量化在长上下文推理任务中不稳定，而8位量化保持数值稳定。


<details>
  <summary>Details</summary>
Motivation: 训练后量化（PTQ）对于高效模型部署至关重要，但其在昇腾NPU上的有效性相比GPU架构尚未得到充分探索。本研究旨在填补这一空白，为在昇腾NPU上部署量化推理模型提供实践参考。

Method: 对DeepSeek-R1-Distill-Qwen系列（1.5B/7B/14B）和QwQ-32B等推理导向模型应用四种代表性PTQ基线算法：AWQ、GPTQ、SmoothQuant和FlatQuant，涵盖从仅权重量化到基于旋转的高级方法。在昇腾NPU平台上进行实证评估。

Result: 实证结果显示显著的平台敏感性：4位仅权重量化对大型模型可行，但激进的4位权重-激活量化方案在NPU上存在层间校准不稳定性，导致长上下文推理任务中的逻辑崩溃。标准8位量化保持数值稳定。实际INT8部署显示，尽管优化内核减少了延迟，但动态量化开销目前限制了端到端加速。

Conclusion: 研究结果为在昇腾NPU上部署量化推理模型的可行性和局限性提供了实用参考，强调了平台特定的量化策略需求，并指出当前动态量化开销是端到端加速的主要瓶颈。
...
### [73] [Can LLM Safety Be Ensured by Constraining Parameter Regions?](https://arxiv.org/abs/2602.17696)
*Zongmin Li,Jian Su,Farah Benamara,Aixin Sun*

Main category: cs.LG

TL;DR: 对大型语言模型中"安全区域"识别方法的系统性评估显示，现有技术无法可靠识别稳定、与数据集无关的安全区域


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常被认为包含"安全区域"——这些参数子集的修改会直接影响安全行为。研究旨在评估现有安全区域识别方法的可靠性

Method: 系统评估了四种安全区域识别方法，涵盖从单个权重到整个Transformer层的不同参数粒度，在四个不同规模的LLM家族上测试，使用十个安全识别数据集

Result: 识别出的安全区域仅显示低到中度的重叠（通过IoU测量），当使用效用数据集（即非有害查询）进一步精炼时，重叠显著下降
...
TL;DR: 该论文提出将大型语言模型视为可配置系统，应用变异性管理技术来系统分析推理时配置选择，以优化能耗和可持续性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在广泛任务中应用日益增多，但其巨大的计算需求引发了训练和推理阶段能效与可持续性的担忧。推理阶段尤其占据总计算使用的主导地位，使得其优化至关重要。现有研究虽然探索了优化技术并分析了配置选择对能耗的影响，但由于推理服务器配置空间的组合爆炸，经验评估变得不可行。

Method: 将LLMs视为可配置系统，应用变异性管理技术系统分析推理时配置选择。在Hugging Face Transformers库上评估该方法，使用基于特征的变异性模型表示生成超参数及其约束，采样代表性配置，测量其能耗、延迟、准确性，并从收集的数据中学习预测模型。

Result: 变异性建模有效管理了LLM推理配置的复杂性。它能够系统分析超参数效应和相互作用，揭示权衡关系，并支持从有限测量中准确预测推理行为。
...
### [75] [ScaleBITS: Scalable Bitwidth Search for Hardware-Aligned Mixed-Precision LLMs](https://arxiv.org/abs/2602.17698)
*Xinlin Li,Timothy Chou,Josh Fromm,Zichang Liu,Yunjie Pan,Christina Fragouli*

Main category: cs.LG

TL;DR: ScaleBITS是一个混合精度量化框架，能够在内存预算下实现自动化、细粒度的比特宽度分配，同时保持硬件效率，在超低比特量化中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 后训练权重量化对于减少大型语言模型的内存和推理成本至关重要，但将平均精度推到4比特以下仍然具有挑战性，主要原因是权重敏感度高度不均匀，缺乏原则性的精度分配方法。现有解决方案要么使用不规则细粒度混合精度导致高运行时开销，要么依赖启发式或高度受限的精度分配策略。

Method: 提出ScaleBITS混合精度量化框架：1）基于新的敏感度分析；2）引入硬件对齐的块级权重分区方案，采用双向通道重排序；3）将全局比特宽度分配建模为约束优化问题，并开发可扩展的贪心算法近似方法，实现端到端原则性分配。

Result: 实验表明，ScaleBITS在超低比特量化中显著优于均匀精度量化（提升高达36%），并且优于最先进的敏感度感知基线方法（提升高达13%），同时不增加运行时开销。

Conclusion: ScaleBITS框架成功解决了超低比特量化中的精度分配问题，通过硬件高效的混合精度量化方案，在保持推理效率的同时显著提升了量化模型的性能。
...
TL;DR: 提出一个理论框架，证明在可验证的正则性和复杂性约束下，分布偏移下的预测器风险可由可计算的偏移度量和模型参数显式上界


<details>
  <summary>Details</summary>
Motivation: 解决机器学习模型在分布偏移下的风险认证问题，提供可验证的保证，避免依赖事后解释，建立严格的数学框架

Method: 开发统一的理论框架，基于可验证的正则性和复杂性约束，推导分布偏移下风险的显式上界，使用可计算的偏移度量，建立可认证的不等式

Result: 获得了分布偏移下风险的显式上界，该上界由可计算的偏移度量和模型参数决定；建立了可验证的模型认证框架，能够处理非平凡规模问题；通过可识别性条件而非事后解释实现可解释性

Conclusion: 在明确假设下建立了分布偏移风险认证的统一框架，能够隔离失效模式并表征不可认证的机制，为机器学习模型在分布偏移下的可靠性提供了理论基础
...
### [77] [MIDAS: Mosaic Input-Specific Differentiable Architecture Search](https://arxiv.org/abs/2602.17700)
*Konstanty Subbotko*

Main category: cs.LG

TL;DR: MIDAS是一种改进的可微分神经架构搜索方法，通过自注意力机制动态生成输入特定的架构参数，并采用局部化架构选择和拓扑感知搜索空间来提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管可微分神经架构搜索（NAS）提供了高效的梯度优化方法，但在实际应用中采用仍然有限。作者旨在改进DARTS方法，使其更加实用和鲁棒。

Method: MIDAS用动态的、输入特定的参数替换DARTS中的静态架构参数，这些参数通过自注意力计算。采用两种关键技术：(1) 局部化架构选择，为激活图的每个空间补丁单独计算；(2) 引入参数免费、拓扑感知的搜索空间，建模节点连接性并简化每个节点的两个入边选择。

Result: 在DARTS搜索空间上，CIFAR-10达到97.42% top-1准确率，CIFAR-100达到83.38%。在NAS-Bench-201上能一致找到全局最优架构。在RDARTS搜索空间中，在CIFAR-10的四个搜索空间中的两个上达到最先进水平。

Conclusion: MIDAS通过补丁级注意力改进了候选操作之间的区分度，生成的输入特定参数分布具有类别感知性且主要是单峰的，为解码提供了可靠的指导。该方法显著提升了可微分NAS的实用性和性能。
...
TL;DR: PaCoDi是一种在频域进行生成建模的谱原生架构，通过傅里叶变换解耦时间信号，利用厄米特对称性压缩序列长度，在保持生成质量的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列生成模型在处理长程依赖时面临表示能力与计算效率的权衡，时间扩散模型存在局部纠缠问题，注意力机制具有O(L²)的计算成本。需要一种既能有效建模长程依赖又计算高效的方法。

Method: 提出PaCoDi（并行复扩散）架构，在频域进行生成建模。通过傅里叶变换将时间信号转换为解耦的谱分量，利用厄米特对称性压缩序列长度。提出正交前向扩散和条件反向分解定理，使用平均场理论近似和交互校正机制，将离散DDPM推广到连续时间频率SDE，推导谱维纳过程。
...
### [79] [Provable Adversarial Robustness in In-Context Learning](https://arxiv.org/abs/2602.17743)
*Di Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一个分布鲁棒的元学习框架，为基于Wasserstein距离分布偏移下的上下文学习提供最坏情况性能保证，分析了对抗性扰动强度、模型容量和上下文示例数量之间的关系。
...
Method: 引入分布鲁棒的元学习框架，针对线性自注意力Transformer，推导出对抗性扰动强度(ρ)、模型容量(m)和上下文示例数量(N)之间的非渐近边界。

Result: 分析表明模型鲁棒性随其容量的平方根缩放(ρ_max ∝ √m)，而对抗性设置会带来与扰动幅度平方成正比的样本复杂度惩罚(N_ρ - N_0 ∝ ρ^2)。合成任务实验验证了这些缩放规律。

Conclusion: 这些发现推进了对对抗条件下上下文学习极限的理论理解，表明模型容量是分布鲁棒性的基本资源。
...
### [80] [Asking Forever: Universal Activations Behind Turn Amplification in Conversational LLMs](https://arxiv.org/abs/2602.17778)
*Zachary Coalson,Bo Fang,Sanghyun Hong*

Main category: cs.LG

TL;DR: 论文发现对话大语言模型存在"轮次放大"新故障模式，攻击者可系统利用澄清寻求行为延长多轮对话而不完成任务，这种攻击源于对话动态机制而非特定提示，能通过微调或参数污染实现规模化攻击。


<details>
  <summary>Details</summary>
Motivation: 多轮交互长度是对话大语言模型运营成本的主要因素，现有研究主要关注提示层面的成本放大攻击，但缺乏对对话动态机制中固有故障模式的研究，特别是模型可能被诱导持续延长对话而不完成任务的系统性风险。

Method: 从机制角度识别与澄清寻求响应相关的查询无关通用激活子空间；通过供应链攻击（微调）和运行时攻击（低级参数污染）实现规模化轮次放大；在多个指令调优大语言模型和基准测试中验证攻击效果。

Result: 攻击能显著增加对话轮次同时保持合规性；现有防御措施对此类故障保护有限；攻击具有跨提示和任务的持久性，不依赖每轮提示优化；通过激活子空间操纵能一致地将模型转向抽象澄清寻求行为。

Conclusion: 对话大语言模型存在固有的轮次放大故障模式，源于对话动态机制而非特定提示，攻击者可规模化利用此漏洞增加运营成本，现有防御不足，需要新的安全机制来应对此类基于对话动态的攻击。
...
<details>
  <summary>Details</summary>
Motivation: 金融时间序列数据具有稀缺性和动态性，导致深度学习模型训练不充分、泛化能力差，需要可靠的数据增强方法来提高预测准确性

Method: 提出使用基于Transformer的GAN（TTS-GAN）生成合成数据来增强金融时间序列数据集，并训练LSTM预测模型；同时提出结合动态时间规整（DTW）和改进的深度数据集差异度量（DeD-iMs）的时间序列特定质量评估指标

Result: 在比特币和标普500价格数据上，使用GAN增强数据的LSTM模型预测准确性显著优于仅使用真实数据的模型，且在不同预测时间范围内都有效

Conclusion: GAN作为数据增强工具能有效克服金融领域数据稀缺问题，提高预测能力，提出的时间序列特定质量评估指标能可靠监控训练进展和评估生成数据质量
...
### [82] [MantisV2: Closing the Zero-Shot Gap in Time Series Classification with Synthetic Data and Test-Time Strategies](https://arxiv.org/abs/2602.17868)
...
<details>
  <summary>Details</summary>
Motivation: 开发时间序列分类基础模型具有重要实践价值，可作为通用特征提取器用于多种下游任务。尽管早期模型如Mantis已显示出潜力，但冻结编码器和微调编码器之间仍存在显著性能差距。

Method: 1. 引入Mantis+：完全在合成时间序列上预训练的Mantis变体；2. 通过受控消融研究改进架构，得到更轻量化的MantisV2编码器；3. 提出增强的测试时方法，利用中间层表示并改进输出标记聚合；4. 通过自集成和跨模型嵌入融合进一步提升性能。

Result: 在UCR、UEA、人类活动识别基准和EEG数据集上的广泛实验表明，MantisV2和Mantis+始终优于先前的时间序列基础模型，实现了最先进的零样本性能。

Conclusion: 该研究显著加强了时间序列的零样本特征提取能力，通过合成数据预训练、架构优化和测试时方法改进，为时间序列基础模型的发展提供了有效方法。
...
### [83] [Grassmannian Mixture-of-Experts: Concentration-Controlled Routing on Subspace Manifolds](https://arxiv.org/abs/2602.17798)
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE模型使用softmax门控缺乏控制稀疏性与利用率权衡的理论机制，且存在专家崩溃问题。需要一种几何原理清晰、可连续控制稀疏性的路由框架。

Method: 在Grassmann流形子空间上构建路由框架，使用矩阵Bingham分布的浓度参数Λ作为控制稀疏性的单一可解释参数，开发摊销变分推理方法估计后验路由分布。

Result: 在350M、1.3B、2.7B参数的MoE语言模型上实现0%路由崩溃，困惑度相当或更好，负载均衡提升15-30%，浓度与稀疏性呈单调关系，支持训练后稀疏性调整。

Conclusion: GrMoE提供了基于几何原理的路由框架，首次建立了浓度控制稀疏性的形式化理论，实现了连续可调的稀疏机制和不确定性感知的路由分配，避免专家崩溃。
...
TL;DR: SBA是一种贝叶斯参数高效微调框架，通过Stiefel流形上的矩阵Langevin先验和切空间Laplace近似，为大型语言模型提供校准的不确定性估计，在保持任务性能的同时显著改善校准误差和域外检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）缺乏原则性的不确定性估计，导致预测校准不佳且在领域偏移下行为不可靠。需要一种既能保持参数效率又能提供校准不确定性估计的贝叶斯框架。

Method: 提出Stiefel-Bayes Adapters (SBA)：在Stiefel流形上放置矩阵Langevin先验，通过切空间Laplace近似与测地线回缩进行近似后验推断。该方法在流形上自然编码适配器子空间应良好条件且正交的归纳偏置，避免从环境空间投影带来的结构方差膨胀。

Result: 在RoBERTa-large、LLaMA-2-7B/13B、Mistral-7B、Qwen2.5-7B等多个模型上，SBA在GLUE和SuperGLUE基准上达到与LoRA和DoRA相当的任务性能，同时将预期校准误差降低18-34%，在领域偏移下将选择性预测AUROC提高12-25%，并以远低于五个LoRA模型集成成本的参数在OOD检测上表现更优。

Conclusion: 在正确的几何结构上放置不确定性比简单地为适配器添加任何贝叶斯处理更重要。SBA证明了流形上的内在推断具有严格的理論优势，能够提供校准的预测不确定性而无需重新校准。
...
TL;DR: 该研究使用监督机器学习模型预测慢性鼻窦炎患者的手术获益，基于前瞻性收集的观察性临床试验数据，模型准确率达85%，在30个病例测试中准确率80%，超过临床专家平均准确率75.6%。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能在医疗预后领域发展迅速，但机器学习预测在观察性临床试验前瞻性标准化数据中的应用仍待探索。慢性鼻窦炎作为一种持续炎症性疾病，对患者生活质量和社会成本造成重大负担，而手术决策复杂，需要权衡已知风险与不确定的个体化结果。

Method: 研究使用监督机器学习模型预测慢性鼻窦炎患者的手术获益，以Sino-Nasal Outcome Test-22（SNOT-22）作为主要患者报告结局指标。基于前瞻性收集的观察性干预试验队列数据，所有患者均接受手术，研究探索仅使用术前数据训练的模型是否能识别那些术前可能不被推荐手术的患者。采用多种算法包括集成方法。

Result: 最佳模型达到约85%的分类准确率，能够提供准确且可解释的手术候选预测。在包含30个混合难度病例的保留测试集上，模型准确率达80%，超过专家临床医生平均预测准确率（75.6%）。

Conclusion: 机器学习模型在预测慢性鼻窦炎手术获益方面表现出色，准确率超过临床专家，具有增强临床决策支持和促进个体化慢性鼻窦炎护理的潜力。
...
### [86] [MIRA: Memory-Integrated Reinforcement Learning Agent with Limited LLM Guidance](https://arxiv.org/abs/2602.17930)
*Narjes Nourzad,Carlee Joe-Wong*

Main category: cs.LG

TL;DR: MIRA是一种结合LLM先验知识和强化学习的框架，通过构建记忆图来指导早期训练，减少对实时LLM监督的依赖，在稀疏奖励环境中提高学习效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在稀疏或延迟奖励环境中样本效率低，而LLM虽然能提供先验知识（如子目标分解、轨迹规划），但过度依赖LLM监督会导致可扩展性问题和不可靠信号依赖。

Method: 提出MIRA框架，构建结构化记忆图存储决策相关信息（轨迹片段、子目标结构），结合智能体高回报经验和LLM输出。从记忆图推导效用信号，软调整优势估计以影响策略更新，而不改变基础奖励函数。随着训练进行，效用项衰减，保留标准收敛保证。

Result: 理论分析表明基于效用的塑形改进了稀疏奖励环境中的早期学习。实证显示MIRA优于RL基线，达到与频繁LLM监督方法相当的回报，同时显著减少在线LLM查询次数。

Conclusion: MIRA通过将LLM先验知识摊销到持久记忆中，减少了实时监督需求，在保持收敛保证的同时提高了稀疏奖励环境中的学习效率，平衡了LLM指导与自主学习的优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在处理具有延迟效应的时序数据因果推断时存在局限性，特别是当多个不同的输入轨迹产生几乎无法区分的输出时，难以提供泛化且可解释的解释。

Method: 提出ruleXplain框架，利用LLMs通过结构化提示生成包含时态算子和延迟语义的约束符号规则语言。使用仿真器生成多样化的反事实输入轨迹，聚类后作为上下文提供给LLM，通过闭环精炼过程确保规则一致性和语义有效性。

Result: 在PySIRTEM流行病模拟器和EnergyPlus建筑能源模拟器上验证了框架有效性，通过三类实验评估：规则集的输入重构效果、因果编码消融研究以及提取规则在不同相位动态的未见输出趋势上的泛化能力。

Conclusion: ruleXplain能够从复杂动力系统中提取形式化、可验证的因果规则，为具有延迟效应的时序数据提供泛化且可解释的因果推断解决方案。
...
### [88] [Memory-Based Advantage Shaping for LLM-Guided Reinforcement Learning](https://arxiv.org/abs/2602.17931)
*Narjes Nourzad,Carlee Joe-Wong*

Main category: cs.LG

TL;DR: 该论文提出了一种结合LLM指导与智能体自身经验的方法，通过构建记忆图来编码子目标和轨迹，从中推导效用函数来指导强化学习，减少对频繁LLM调用的依赖，提高稀疏奖励环境中的样本效率。
...
Method: 构建记忆图编码来自LLM指导和智能体自身成功轨迹的子目标与轨迹，从中推导效用函数评估智能体轨迹与先前成功策略的匹配程度，该效用函数塑造优势函数为critic提供额外指导而不改变奖励，主要依赖离线输入和偶尔在线查询。
...
### [89] [MePoly: Max Entropy Polynomial Policy Optimization](https://arxiv.org/abs/2602.17832)
...
Method: 提出MePoly策略参数化方法，基于多项式能量模型，提供显式可处理的概率密度，利用经典矩问题的理论基础

Result: MePoly能有效捕捉复杂非凸流形，在多样化基准测试中性能优于基线方法

Conclusion: MePoly为随机最优控制提供了一种能表示多模态解且具有显式概率密度的新策略参数化方法
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型能生成高质量样本但也可能记忆训练数据，引发隐私担忧。目前对记忆与泛化机制的理解不足，特别是噪声调度中记忆何时产生、数据几何如何影响记忆、不同噪声尺度现象如何相互作用等问题尚不清楚。

Method: 引入几何框架，基于高斯壳层覆盖训练数据的特性和后验集中行为这两个决定扩散模型记忆与泛化的基本对象，将噪声调度划分为三个区域。识别出中等噪声区域为记忆风险最高的"危险区"，并提出几何感知的针对性干预措施来缓解记忆问题。

Result: 发现记忆风险在噪声水平上高度不均匀，中等噪声区域记忆风险最显著。小噪声区域因训练覆盖有限而避免记忆，大噪声区域后验集中度低且可证明接近线性高斯去噪行为。针对中等噪声区域，通过几何条件提出了有效的干预方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习需要大量在线环境交互，这在成本高、危险或不可行的场景中存在问题。离线学习面临隐藏混杂因素的挑战，这些混杂因素可能导致虚假相关性和次优决策。

Method: 1. 使用工具变量识别因果效应，基于条件矩限制问题；2. 受双重/去偏机器学习启发，开发具有收敛性和最优性保证的样本高效算法；3. 在模仿学习中放宽隐藏混杂因素条件限制；4. 针对线性时序逻辑表达的高层目标开发可证明最优的学习算法。

Result: 提出的CMR求解算法优于现有最先进算法，模仿学习算法具有收敛率保证，时序逻辑学习算法提高了样本效率。在强化学习基准测试、合成和半合成数据集上验证了方法的有效性。

Conclusion: 该论文开发的方法能够有效处理离线学习中的隐藏混杂因素问题，为实际决策制定提供了有用的工具，特别是在高风险应用中需要保证的场景。
...
### [92] [JAX-Privacy: A library for differentially private machine learning](https://arxiv.org/abs/2602.17861)
...
TL;DR: JAX-Privacy是一个用于简化差分隐私机器学习机制部署的库，提供模块化组件和最新研究成果集成。
...
Result: 创建了一个既支持深度定制又提供开箱即用体验的库，整合了大量差分隐私机器学习的最新研究成果。

Conclusion: JAX-Privacy通过模块化设计和易用性原则，为差分隐私机器学习提供了强大的工具支持。
...
TL;DR: PHAST：一种用于部分观测物理系统（仅位置信息）的端口哈密顿架构，通过结构化分解实现稳定长期预测和物理参数恢复


<details>
  <summary>Details</summary>
Motivation: 真实物理系统都是耗散的，从部分观测（仅位置信息）预测其动力学是科学机器学习中的核心挑战。需要解决仅位置信息问题：给定离散时间的广义位置（动量为隐变量），学习一个结构化模型，既能产生稳定长期预测，又能在提供足够结构时恢复有物理意义的参数。

Method: 提出PHAST（端口哈密顿结构化时间动力学架构），基于端口哈密顿框架将保守-耗散分解显式表示为$\dot{x}=(J-R)\nabla H(x)$。将哈密顿量分解为势能$V(q)$、质量$M(q)$和阻尼$D(q)$，考虑三种知识状态（已知、部分已知、未知），使用高效低秩PSD/SPD参数化，并通过Strang分裂推进动力学。

Result: 在13个仅位置信息的基准测试（涵盖机械、电气、分子、热力学、引力和生态系统）中，PHAST在竞争基线中实现了最佳长期预测性能，并在知识状态提供足够锚点时能够恢复有物理意义的参数。研究表明，没有这种锚点时识别问题本质上是病态的（规范自由度）。

Conclusion: PHAST通过结构化端口哈密顿方法有效解决了仅位置信息的物理系统预测问题，实现了稳定长期预测和物理参数恢复。提出了双轴评估框架，将预测稳定性与可识别性分开评估，为科学机器学习中的结构化建模提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 理解LLM激活空间中学习到的方向编码了哪些特征需要找到能强烈激活这些方向的输入。特征可视化通过优化输入来最大化激活目标方向，为昂贵的数据库搜索方法提供了替代方案，但由于文本的离散性，在LLM中仍未得到充分探索。现有的提示优化技术也不适合这个容易陷入局部最小值的领域。

Method: ADAPT是一种混合方法，结合了束搜索初始化和自适应梯度引导变异，专门针对这些失败模式设计。该方法在Gemma 2 2B的稀疏自编码器潜在空间上进行评估，提出了基于数据集激活统计的指标来实现严格比较。

Result: ADAPT在不同层和潜在类型上始终优于先前的方法。结果表明LLM的特征可视化是可行的，但需要针对该领域量身定制的设计假设。

Conclusion: LLM的特征可视化是可行的，但需要专门针对该领域设计的假设和方法。ADAPT通过结合束搜索初始化和自适应梯度引导变异，成功克服了现有方法的局限性，为理解LLM激活空间中的特征编码提供了有效的工具。
...
TL;DR: 该论文首次从理论和优化角度分析了注意力回归模型中PCC停滞现象，揭示了MSE优化与PCC梯度之间的冲突，以及凸聚合器的PCC改进极限，并提出了创新的外推相关注意力机制来突破这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 注意力回归模型训练中普遍存在但未被深入理解的PCC停滞现象：即使MSE持续下降，PCC在训练早期就停止改善。作者旨在从理论和优化角度深入分析这一现象的根本原因。

Method: 首先从优化动力学角度分析MSE降低与PCC梯度抑制之间的冲突，特别关注softmax注意力机制在数据高度同质化时的表现。其次从模型能力角度推导出任何凸聚合器（包括softmax注意力）的PCC改进极限。基于这些理论洞察，提出了外推相关注意力机制，包含新颖的理论驱动机制来改善PCC优化并超越凸包限制。

Result: 理论分析揭示了PCC停滞的根本原因：MSE优化会抑制PCC梯度，且凸聚合器的PCC改进受输入凸包的严格限制。提出的ECA机制在包括具有挑战性的同质数据设置在内的多样化基准测试中，一致地打破了PCC停滞，在保持MSE性能的同时显著提升了相关性。

Conclusion: 该研究首次为注意力回归模型中的PCC停滞现象提供了严格的理论分析，揭示了优化动力学和模型容量的双重限制，并提出了有效的解决方案。ECA机制通过理论驱动的设计成功突破了PCC改进的瓶颈，为相关模型训练提供了重要洞见。
...
<details>
  <summary>Details</summary>
Motivation: 在语言模型的后训练中，RLHF和RLVR存在奖励黑客问题，即策略可能利用奖励模型的不准确性学习到非预期的行为。传统方法使用KL惩罚来限制策略更新，但作者认为应该将策略更新偏向奖励模型更准确的区域。

Method: 作者首先建立了奖励模型准确性与收敛时最优解平坦度之间的理论联系，提出使用梯度正则化（GR）将训练偏向平坦区域以保持奖励模型准确性。他们发现KL惩罚的参考重置隐式使用了GR，并进一步提出使用显式GR配合高效有限差分估计。

Result: 实验表明，梯度正则化在多种语言模型强化学习实验中优于KL惩罚：在RLHF中获得更高的GPT评判胜率，避免过度关注基于规则的数学奖励格式，并防止在LLM-as-a-Judge数学任务中黑客评判者。

Conclusion: 梯度正则化为解决RLHF中的奖励黑客问题提供了新的理论框架和实用方法，通过将策略更新偏向奖励模型更准确的平坦区域，比传统KL惩罚方法表现更好。
...
TL;DR: 研究在对抗性注入数据流中，学习者可以选择弃权以避免错误预测的序列预测问题。在未知分布的情况下，提出AbstainBoost算法，为VC类实现分布无关的学习保证。


<details>
  <summary>Details</summary>
Motivation: 研究半对抗性设置下的学习问题，其中对抗者可以在独立同分布数据流中注入任意多对抗性实例，但学习者可以在实例被污染时弃权而不受惩罚。现有研究假设已知干净样本分布，这在理论和实践中都是强假设，需要探索未知分布下的学习保证。

Method: 提出AbstainBoost算法，基于弱学习器的提升过程，为一般VC类在分布无关的弃权学习中保证亚线性错误率。算法对遗忘型对抗者有效，对自适应对抗者则适用于结构化函数类（如线性分类器）。

Result: 算法在未知分布情况下为VC类实现了分布无关的弃权学习保证，同时揭示了错误分类错误与错误弃权次数之间的多项式权衡关系。

Conclusion: 在未知分布的情况下，通过弃权机制可以在半对抗性设置中实现VC类的学习，填补了经典随机设置和完全对抗设置之间的空白，为实际应用提供了更实用的理论保证。
...
### [98] [Flow Matching with Injected Noise for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2602.18117)
*Yongjae Shin,Jongseong Chae,Jongeui Park,Youngchul Sung*

Main category: cs.LG

TL;DR: FINO是一种基于流匹配的策略方法，通过注入噪声增强离线到在线强化学习的样本效率，结合熵引导采样机制平衡探索与利用。


<details>
  <summary>Details</summary>
Motivation: 生成模型在离线强化学习中表现出色，但在扩展到在线微调时面临挑战。现有方法将在线微调视为离线预训练的直接延续，未能解决关键问题，特别是探索效率不足的问题。

Method: 提出FINO方法：1) 在策略训练中注入噪声，鼓励超越离线数据集观察到的动作范围；2) 结合熵引导采样机制，在在线微调过程中动态平衡探索与利用。

Result: 在多样化的挑战性任务实验中，FINO在有限的在线预算下始终实现卓越性能，证明了其样本效率优势。

Conclusion: FINO通过噪声注入和熵引导采样有效解决了离线到在线强化学习中的探索挑战，显著提升了在线微调的样本效率。
...
TL;DR: 该论文研究了高斯过程（GP）赌博机问题在频率主义设置下的算法无关最坏情况下界，特别关注平方指数（SE）核函数。论文部分解决了维度依赖对数因子的开放性问题，给出了累积遗憾和简单遗憾的下界，并改进了SE核的最大信息增益上界。


<details>
  <summary>Details</summary>
Motivation: 高斯过程赌博机问题中，对于平方指数核函数，现有上下界在维度依赖的对数因子方面存在差距，这是一个尚未解决的开放性问题。论文旨在部分解决这个问题，特别是在超球形输入域下，为算法性能提供更精确的理论界限。

Method: 研究采用理论分析方法，在频率主义设置下分析高斯过程赌博机问题。通过数学推导和证明，建立了算法无关的最坏情况累积遗憾下界和简单遗憾下界，同时改进了平方指数核的最大信息增益上界。

Result: 1. 任何算法在超球形输入域下都遭受Ω(√[T(ln T)^d(ln ln T)^{-d}])的累积遗憾；2. 任何算法需要Ω(ε^{-2}(ln 1/ε)^d(ln ln 1/ε)^{-d})时间步找到ε-最优点；3. 改进了SE核的最大信息增益上界为O((ln T)^{d+1}(ln ln T)^{-d})。
...
TL;DR: 提出了首个测量AI倾向性的形式化框架，使用双逻辑公式来描述模型在"理想区间"内的高成功率，通过任务无关的评估标准估计理想区间边界，发现倾向性与能力结合比单独使用任一项能更好地预测AI行为。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估主要关注能力测量，但倾向性（模型展现特定行为的趋势）对性能和安全结果有核心影响。传统项目反应理论（IRT）将模型在任务上的成功描述为模型能力和任务需求的单调函数，这种方法不适用于倾向性评估，因为倾向性过高或过低都可能存在问题。

Method: 引入首个测量AI倾向性的形式化框架，采用双逻辑公式描述模型成功概率，当模型倾向性处于"理想区间"内时获得高成功概率。使用配备新开发的任务无关评估标准的LLM来估计理想区间的边界。将该框架应用于六个LLM模型家族，这些模型的倾向性在不同方向上被激发。

Result: 能够测量倾向性偏移程度及其对任务的影响；使用一个基准估计的倾向性成功预测了保留任务上的行为；结合倾向性和能力比单独使用任一项获得更强的预测能力；展示了如何进行严格的倾向性测量以及如何超越仅使用能力评估来预测AI行为。

Conclusion: 该框架展示了如何进行严格的倾向性测量，并证明了结合倾向性和能力评估比单独使用能力评估能更好地预测AI行为，为AI评估提供了更全面的方法。
...
### [101] [LERD: Latent Event-Relational Dynamics for Neurodegenerative Classification](https://arxiv.org/abs/2602.18195)
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病改变了大脑电生理并破坏了多通道EEG动力学，现有方法大多依赖黑盒分类器，没有明确建模生成观察信号的基础动力学。

Method: 提出LERD系统，结合连续时间事件推断模块和随机事件生成过程，捕捉灵活的时间模式，并融入电生理启发的动力学先验来指导学习。

Result: 在合成基准和两个真实世界AD EEG队列上的实验表明，LERD始终优于强基线方法，并产生与生理对齐的潜在摘要，有助于表征群体水平的动力学差异。

Conclusion: LERD提供了一种可解释的、基于动力学的EEG分析方法，能够直接从多通道EEG推断神经事件和关系结构，为阿尔茨海默病的诊断和监测提供了新工具。
...
TL;DR: 该论文分析了基于梯度的超参数优化中超梯度估计的偏差-方差分解，重点关注以往研究忽略的方差项，提出集成超梯度策略来降低方差，并在多个任务上验证了改进效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的超参数优化方法主要关注超梯度估计的偏差，而忽略了数据分布导致的方差误差，这会影响算法性能。作者旨在填补这一理论空白，并基于分析提出改进方法。

Method: 1. 对超梯度估计误差进行偏差-方差分解；2. 提供超梯度估计误差界的全面分析；3. 提出集成超梯度策略来有效降低方差；4. 在正则化超参数学习、数据超清洗和少样本学习等任务上进行实验验证。

Result: 实验结果表明，提出的方差降低策略改进了超梯度估计。作者还建立了超梯度估计与超额误差之间的联系，为实证观察提供了理论解释。

Conclusion: 该研究强调了超梯度估计中方差项的重要性，提出的集成超梯度策略能有效降低方差，改善超参数优化性能，并为实践中观察到的现象（如验证集过拟合）提供了理论解释。
...
TL;DR: 该研究对Abdelnabi等人（2024）提出的基于可评分游戏的LLM多智能体谈判基准进行了可重复性分析，发现基准虽然复杂但模型比较存在模糊性，实验设置存在信息泄露检测和消融研究不足等局限性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在多智能体谈判任务中表现出巨大潜力，但该领域缺乏稳健且可泛化的评估基准。Abdelnabi等人（2024）提出了基于可评分游戏的谈判基准，本研究旨在验证该基准声明的可重复性，并深入理解其可用性和泛化性。

Method: 1. 在更多模型上复现原始实验；2. 引入额外指标验证谈判质量和评估公平性；3. 在扩展版本的基准上分析更广泛模型的行为；4. 识别实验设置的局限性，特别是信息泄露检测和消融研究的完整性。

Result: 研究发现：1. 基准确实复杂，但模型比较存在模糊性，对其客观性提出质疑；2. 实验设置存在局限性，包括信息泄露检测不足和消融研究不彻底；3. 通过分析更广泛模型在扩展基准上的行为，揭示了为潜在用户提供额外背景的见解。

Conclusion: 研究结果强调了上下文在模型比较评估中的重要性，指出虽然该谈判基准具有复杂性，但在客观性和实验严谨性方面存在改进空间，为未来基准开发提供了重要参考。
...
### [104] [A Geometric Probe of the Accuracy-Robustness Trade-off: Sharp Boundaries in Symmetry-Breaking Dimensional Expansion](https://arxiv.org/abs/2602.17948)
*Yu Bai,Zhe Wang,Jiarui Zhang,Dong-Xiao Zhang,Yinjun Gao,Jun-Jie Zhang*

Main category: cs.LG

TL;DR: 本文通过对称破缺维度扩展(SBDE)技术探究了深度学习中的准确率与对抗鲁棒性权衡问题。研究发现，通过插入常数值像素扩展输入维度可以提升干净准确率，但同时会降低对抗鲁棒性，这种脆弱性主要源于新增维度上的陡峭边界。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在干净准确率和对抗鲁棒性之间存在普遍权衡，但其几何根源尚不明确。本文旨在通过受控实验探究这一权衡背后的机制。

Method: 采用对称破缺维度扩展(SBDE)技术：通过在输入图像中插入常数值像素来扩展输入维度，打破平移对称性。同时使用测试时的掩码投影技术，将插入的辅助像素重置为训练时的值，以分析脆弱性的来源。

Result: SBDE显著提升了干净准确率（如CIFAR-10上ResNet-18从90.47%提升到95.63%），但降低了对抗迭代白盒攻击的鲁棒性。掩码投影实验表明脆弱性几乎完全源于插入的维度，投影能有效中和攻击并恢复鲁棒性。模型通过沿辅助轴创建陡峭边界（陡峭损失梯度）来实现高准确率。

Conclusion: 准确率-鲁棒性悖论的几何解释是：优化过程通过加深吸引盆来提升准确率，但不可避免地会在辅助自由度上建立陡峭边界，从而对流形外扰动产生脆弱敏感性。
...
TL;DR: 提出了一种贝叶斯在线模型选择算法，用于随机多臂赌博机问题，能够在多个基础学习器中选择最优者，并获得理论保证。
...
Method: 提出了一种新的贝叶斯算法用于随机赌博机中的在线模型选择，该算法能够自适应地在多个基础学习器之间进行选择和切换。

Result: 证明了贝叶斯遗憾的oracle式保证为O(d*M√T + √(MT))，其中M是基础学习器数量，d*是最优基础学习器的遗憾系数，T是时间范围。实验验证了该方法在多种随机赌博机设置下的竞争力。

Conclusion: 该算法能够有效解决贝叶斯在线模型选择问题，性能可与最优基础学习器竞争，同时研究了数据共享对缓解先验设定错误的影响。
...
TL;DR: 该论文提出将解码视为一个原则化的优化层，通过正则化问题统一现有解码方法，并基于此框架设计了新的Best-of-K解码器来提升多样本管道的性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型的解码过程仍被视为启发式的参数调整过程，缺乏统一的理论框架。作者认为解码应该被理解为一个原则化的优化层，能够系统地解释现有方法并方便地设计新解码器。

Method: 提出一个统一的解码框架：在每个token位置，解决一个在概率单纯形上的正则化优化问题，平衡模型得分与结构偏好和约束。该框架将贪婪解码、Softmax采样、Top-K、Top-P和Sparsemax等现有方法统一为特例。基于此框架设计了Best-of-K解码器，使用KL散度锚定的覆盖目标来优化多样本管道。

Result: Best-of-K解码器显著提升了多样本管道的性能，例如在Qwen2.5-Math-7B模型上，MATH500数据集在高采样温度下准确率提升了+18.6%。该框架成功解释了现有解码方法的共同结构，并为设计新解码器提供了系统化方法。

Conclusion: 解码应该被视为语言模型的原则化优化层，而不是启发式参数调整。提出的统一框架不仅解释了现有解码方法，还使设计新解码器变得简单，Best-of-K解码器的成功验证了该框架的实用价值。
...
### [107] [Analyzing and Improving Chain-of-Thought Monitorability Through Information Theory](https://arxiv.org/abs/2602.18297)
*Usman Anwar,Tim Bakker,Dana Kianfar,Cristina Pinneri,Christos Louizos*

Main category: cs.LG

TL;DR: 论文通过信息论分析CoT监控机制，提出两种训练方法提升监控准确性，防止CoT退化


<details>
  <summary>Details</summary>
Motivation: 研究CoT监控器的理论基础和实际性能限制，解决监控器在实践中可能失效的问题，防止奖励黑客行为

Method: 1) 信息论分析CoT与输出间互信息条件；2) 识别信息差距和启发误差两种近似误差源；3) 提出两种训练方法：基于oracle的直接奖励法和无标签的条件互信息最大化法

Result: 两种方法在不同环境中显著提升监控准确性，防止CoT退化，有效缓解任务奖励不完善时的奖励黑客问题

Conclusion: CoT监控性可通过针对性训练目标系统改进，提出的两种方法能有效提升监控性能并防止退化，为构建更可靠的CoT监控系统提供理论基础和实践方案
...
TL;DR: JPmHC框架通过约束线性混合器在算子范数有界流形上，解决了超连接架构中梯度病理问题，实现了更稳定、高效的训练。
...
Method: 提出JPmHC框架，用可训练的线性混合器替代恒等跳跃连接，通过约束混合器在算子范数有界流形上控制梯度条件，包括自由概率分析、内存高效的隐式微分和Cayley变换实现的Stiefel约束混合器。

Result: 在ARC-AGI上的实验表明，JPmHC相比双随机基线实现了更快的收敛速度、更高的准确率和更低的计算成本。

Conclusion: JPmHC作为超连接的灵活可扩展扩展，推进了谱感知、稳定且高效的深度学习，为拓扑架构设计和基础模型演进提供了见解。
...
### [109] [FedZMG: Efficient Client-Side Optimization in Federated Learning](https://arxiv.org/abs/2602.18384)
*Fotios Zantalis,Evangelos Zervas,Grigorios Koulouras*

Main category: cs.LG

TL;DR: FedZMG是一种无需额外参数、客户端侧的联邦学习优化算法，通过将局部梯度投影到零均值超平面来缓解非IID数据导致的客户端漂移问题，无需额外通信或超参数调优，在非IID设置下优于FedAvg和FedAdam。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据通常是非独立同分布的，这会导致客户端漂移问题，降低收敛速度和模型性能。现有的自适应优化器虽然能缓解这些问题，但通常计算复杂或通信开销大，不适合资源受限的物联网环境。

Method: 提出FedZMG算法，基于梯度中心化的思想，将局部梯度投影到零均值超平面上，有效中和异构数据分布中固有的"强度"或"偏差"偏移。该算法无需额外参数、无需额外通信、无需超参数调优。
...
TL;DR: 论文提出了模板GNN（T-GNNs）框架，通过聚合指定图模板的嵌入来更新节点特征，并建立了对应的逻辑系统GML(T)，统一分析了GNN的表达能力。


<details>
  <summary>Details</summary>
Motivation: 标准GNN只能聚合直接邻居或全局信息，表达能力有限。为了增强表达能力，最近的研究尝试纳入子结构信息（如环计数和子图属性）。本文旨在形式化这一架构趋势。

Method: 引入模板GNN（T-GNNs）框架，节点特征通过聚合指定图模板集合中的有效模板嵌入来更新。提出对应的分级模板模态逻辑GML(T)，以及模板基二分和WL算法的广义概念。

Result: 建立了T-GNNs与GML(T)表达能力的等价性，提供了分析GNN表达能力的统一方法：展示了标准AC-GNNs及其变体可以作为T-GNNs的实例化。

Conclusion: T-GNNs框架为分析和理解GNN表达能力提供了统一的理论基础，能够涵盖现有GNN变体并形式化子结构信息整合的趋势。
...
TL;DR: 研究针对异步优化中重尾梯度噪声的算法改进，通过延迟感知学习率调度和延迟补偿提升性能，在图像和语言任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 重尾随机梯度噪声在Transformer模型中常见，会破坏优化稳定性。现有研究主要关注集中式或分布式同步设置中的重尾噪声处理，而重尾噪声与异步优化之间的相互作用尚未充分探索。

Method: 提出两种处理异步更新中慢节点（stragglers）的通信方案，基于延迟感知学习率调度和延迟补偿进行算法修改，以增强异步算法在重尾梯度噪声下的性能。

Result: 在重尾噪声下的收敛保证与同步对应方法的速率匹配，相比现有异步方法提高了延迟容忍度。在图像和语言任务中，提出的方法在准确率/运行时间权衡方面优于先前的同步和异步方法，并且对超参数更加鲁棒。

Conclusion: 通过延迟感知学习率调度和延迟补偿的算法修改，可以有效处理异步优化中的重尾梯度噪声问题，在理论和实证上都取得了优于现有方法的结果。
...
TL;DR: 本文提出了Continual-NExT框架和MAGE方法，用于解决双模态多模态大语言模型在持续学习中的遗忘、幻觉和跨模态知识迁移问题


<details>
  <summary>Details</summary>
Motivation: 双模态多模态大语言模型虽然具有强大的即时学习和泛化能力，但在持续学习方面存在不足，无法适应动态现实场景。现有研究缺乏标准化的持续学习框架，且模型面临灾难性遗忘、幻觉、指令不遵循和跨模态知识迁移失败等挑战

Method: 提出了Continual-NExT持续学习框架，包含精心设计的评估指标。同时提出了MAGE方法，通过混合和聚合通用LoRA与专家LoRA来促进跨模态知识迁移并减轻遗忘

Result: 大量实验表明，MAGE方法优于其他持续学习方法，并取得了最先进的性能

Conclusion: 本文为双模态多模态大语言模型建立了首个标准化的持续学习框架，并通过MAGE方法有效解决了持续学习中的关键挑战，提升了模型在动态环境中的适应能力
...
TL;DR: 该研究探讨了图生成模型中严格等变性的权衡问题，提出通过可控对称性调制方案放松等变性约束，以加速训练收敛并避免过拟合。


<details>
  <summary>Details</summary>
Motivation: 严格等变性虽然能确保模型尊重图的置换对称性，但会增加计算成本并减缓收敛速度，因为模型必须在大量可能的节点置换中保持一致。研究旨在探索这种权衡关系。

Method: 从等变离散流匹配模型出发，通过基于正弦位置编码和节点置换的可控对称性调制方案，在训练过程中放松等变性约束。

Result: 实验表明：对称性破坏能通过提供更简单的学习信号加速早期训练，但会导致模型过拟合，重复生成训练集中的图；而适当调制对称性信号可以延迟过拟合同时加速收敛，使模型仅用基线训练轮数的19%就达到更强性能。

Conclusion: 在图生成模型中，适当放松等变性约束并通过可控对称性调制方案进行管理，可以在保持性能的同时显著加速训练收敛，避免过拟合问题。
...
TL;DR: TempoNet是一个基于强化学习的实时调度器，使用Transformer和深度Q学习，通过Urgency Tokenizer处理时间松弛度，稀疏注意力机制实现高效推理，在多核环境下优于传统调度器。


<details>
  <summary>Details</summary>
Motivation: 实时调度器需要在严格的计算预算下处理紧迫的截止时间，传统方法难以在复杂多核环境中实现最优调度，需要更智能的解决方案。

Method: 1. 使用置换不变Transformer和深度Q近似；2. Urgency Tokenizer将时间松弛度离散化为可学习嵌入；3. 延迟感知稀疏注意力堆栈（块状top-k选择和局部敏感分块）；4. 多核映射层将Q分数转换为处理器分配。

Result: 在工业混合关键性追踪和大规模多处理器设置中，相比分析调度器和神经基线，TempoNet在截止时间满足率上表现更优，具有改进的优化稳定性和亚毫秒级推理速度。

Conclusion: TempoNet为基于Transformer的高吞吐量实时调度决策提供了一个实用框架，展示了强化学习在复杂调度问题中的潜力。
...
TL;DR: 该研究提出了一种基于分位数的元策略，用于处理非平稳需求下的在线资源分配问题，仅需每个周期一个历史样本，并在两种样本信息设置下分别实现了√T和对数级遗憾。


<details>
  <summary>Details</summary>
Motivation: 解决在线资源分配中非平稳需求带来的挑战，传统方法通常需要大量历史数据或假设环境平稳，而实际应用中需求分布可能任意变化且历史数据有限。

Method: 提出类型依赖的分位数元策略，将问题解耦为三个模块：奖励分布估计、通过流体松弛优化目标服务概率、通过动态接受阈值进行实时决策。

Result: 对于奖励可观测样本，静态阈值策略实现了Õ(√T)遗憾；对于仅类型样本，在最小到达概率假设下，部分自适应策略达到Õ(√T)，完全自适应解析策略实现了O((log T)³)的多对数遗憾。

Conclusion: 该框架在仅需每个周期一个离线样本的情况下，能够处理任意非平稳性，支持多资源约束，为非平稳多资源在线分配提供了首个多对数遗憾保证。
...
TL;DR: RAT+是一种密集预训练架构，通过全序列循环和主动循环学习增强注意力机制，可在推理时灵活切换到扩张注意力模式，在保持准确性的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 结构化扩张注意力在推理时具有效率优势，但将预训练注意力模型稀疏化为扩张模式会导致严重的准确性下降。需要一种既能保持长距离连接，又能在推理时灵活调整计算开销的方法。

Method: RAT+架构在密集预训练时通过全序列循环和主动循环学习增强注意力机制。单一模型经过密集预训练后，可在推理时灵活切换到扩张注意力模式（可选局部窗口）或混合层/头组合，仅需少量分辨率适应而非重新训练稀疏模型。

Result: 在1.5B参数、100B token训练下，RAT+在16倍扩张时接近密集注意力准确性，64倍扩张时在常识推理和LongBench任务上仅下降2-3个点。此外，RAT+在稀疏化为top-k块注意力时优于原始注意力。在2.6B参数、200B token规模下观察到相同趋势。

Conclusion: RAT+提供了一种高效的解决方案：通过单一密集预训练模型支持多种推理时稀疏化模式，在保持模型性能的同时显著降低计算开销，解决了结构化扩张注意力的准确性下降问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决结构上的张力：在序列生成中，因果性主要在符号空间组织，即使存在潜在变量也常为辅助作用，而有效动态由确定性解码器承载；同时概率潜在模型捕获变化因素和不确定性，但这些不确定性通常由全局或参数机制承担，而计算单元仍传播标量。核心问题：如果不确定性是计算的内在属性，为什么计算单元不显式地承载它？

Method: 提出变分分布神经元：每个神经元参数化后验分布，传播重参数化样本，并通过局部ELBO的KL项进行正则化。分析"崩溃"模式和"活神经元"条件，并通过自回归先验在时间上扩展贡献。

Result: 提出概念证明，展示如何通过局部约束测试"收缩"过程，并通过内部度量监控。神经元的上下文信息量和时间持久性可通过不同约束进行局部调节。

Conclusion: 探索两个轴线：(i)概率约束的组合必须稳定、可解释和可控；(ii)粒度问题：如果推理是在约束下分布协商的过程，原始单元应保持确定性还是变为分布性？为分布计算单元提供了理论基础。
...
TL;DR: 该研究首次展示了MEG语音模型在感知与产生任务间的迁移学习和跨任务解码，使用Conformer模型在50小时单被试听音数据上预训练，仅用5分钟数据微调即可在18名被试上获得性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决语音脑机接口中的数据效率问题，探索语音感知与产生任务间的共享神经表征，验证预训练模型能否捕捉跨任务的共同神经过程而非任务特异性活动。

Method: 使用Conformer架构模型，在单个被试的50小时听音数据上进行预训练，然后在18名被试上仅用每人5分钟的数据进行微调，评估模型在语音感知和产生任务内的性能以及跨任务解码能力。

Result: 迁移学习带来一致性能提升：任务内准确率提高1-4%，跨任务准确率提升达5-6%。预训练不仅改善各任务性能，还实现了感知与产生任务间的可靠跨任务解码。关键发现：在语音产生任务上训练的模型能够解码被动听音任务，表明学习到的表征反映了共享神经过程而非任务特异性运动活动。

Conclusion: 该研究证明了迁移学习在MEG语音解码中的有效性，揭示了语音感知与产生任务间存在共享神经表征，为数据高效的语音脑机接口开发提供了新途径，并确认了学习到的表征捕捉的是跨任务的共同神经过程。
...
TL;DR: 研究发现冻结大语言模型可以通过两个学习到的原型标记在单次前向传播中重建数百个标记，探索了原型标记编码的信息及其在重建和约束下的行为，为超越自回归范式提供了路径。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型需要n次前向传播来生成长度为n的序列，效率较低。最近研究表明冻结LLMs可以从两个学习到的原型标记中重建数百个标记，这为超越自回归范式提供了可能，因此需要深入研究这些原型标记编码的信息及其行为特性。

Method: 通过一系列实验分析两个原型标记中的语义和句法内容，研究e-token的稳定性特性，可视化重建过程中对e-token的注意力模式，并测试两种使用教师嵌入在e-token上"施加"语义结构的正则化方案：基于锚点的损失和关系蒸馏目标。

Result: 标准优化下m-token比e-token更强烈地捕获语义信息；基于锚点的约束与重建精度之间存在明显权衡；关系蒸馏可以在不牺牲重建质量的情况下将批次级语义关系转移到原型标记空间。

Conclusion: 研究支持了未来非自回归序列到序列系统的可行性，这些系统可以预测原型标记作为中间表示，为超越传统自回归生成范式提供了理论基础。
...
### [120] [On the "Induction Bias" in Sequence Models](https://arxiv.org/abs/2602.18333)
*M. Reza Ebrahimi,Michaël Defferrard,Sunny Panchal,Roland Memisevic*

Main category: cs.LG

TL;DR: Transformer模型在状态跟踪任务上存在根本性挑战，即使在训练和评估分布匹配的情况下，其数据效率远低于RNN，且缺乏跨序列长度的权重共享能力。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在实际应用中取得了显著成功，但近期研究表明它们在状态跟踪能力上存在局限，特别是在分布外泛化方面。本研究关注这些局限在分布内的影响，探索Transformer和RNN在不同监督机制下的数据效率差异。

Method: 进行大规模实验研究，比较Transformer和RNN在多种监督机制下的数据效率。分析训练数据需求随状态空间大小和序列长度的增长情况，并研究学习到的状态跟踪机制在不同序列长度间的共享程度。

Result: Transformer的训练数据需求随状态空间大小和序列长度增长的速度远快于RNN。Transformer在不同序列长度间表现出可忽略甚至有害的权重共享，表明它们学习的是长度特定的孤立解决方案。相比之下，循环模型通过跨长度共享权重实现有效的摊销学习，允许来自一个序列长度的数据改进其他长度的性能。

Conclusion: 状态跟踪仍然是Transformer面临的根本性挑战，即使在训练和评估分布匹配的情况下。Transformer缺乏RNN所具有的跨序列长度的有效权重共享能力，导致数据效率低下和长度特定的学习模式。
...
### [121] [Explaining AutoClustering: Uncovering Meta-Feature Contribution in AutoML for Clustering](https://arxiv.org/abs/2602.18348)
*Matheus Camilo da Silva,Leonardo Arrighi,Ana Carolina Lorena,Sylvio Barbon Junior*

Main category: cs.LG

TL;DR: 该论文研究了AutoClustering元模型的可解释性，通过分析22种现有方法的元特征，应用全局和局部解释技术，揭示了元特征重要性模式，识别了当前元学习策略的结构性弱点，为更可解释的AutoML设计提供了指导。


<details>
  <summary>Details</summary>
Motivation: AutoClustering方法虽然性能强大，但其推荐结果难以解释：数据集元特征对算法和超参数选择的影响通常不透明，限制了可靠性、偏差诊断和高效的元特征工程。这影响了系统的可靠性和进一步改进的诊断洞察。

Method: 1. 回顾22种现有方法，将其元特征组织成结构化分类法；2. 应用全局可解释性技术（决策谓词图）评估选定框架中元模型的特征重要性；3. 使用SHAP等局部可解释性工具分析具体的聚类决策。

Result: 研究发现：1. 揭示了元特征相关性的稳定模式；2. 识别了当前元学习策略中可能扭曲推荐结果的结构性弱点；3. 为更可解释的AutoML设计提供了可操作的指导。

Conclusion: 该研究为增加无监督学习自动化中的决策透明度提供了实用基础，有助于提高AutoClustering系统的可靠性和可解释性。
...
### [122] [Assigning Confidence: K-partition Ensembles](https://arxiv.org/abs/2602.18435)
*Aggelos Semoglou,John Pavlopoulos*

Main category: cs.LG

TL;DR: CAKE框架通过聚类集成计算每个数据点的分配稳定性和局部几何拟合一致性，为聚类分配提供置信度评分，识别稳定核心点和模糊点。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法缺乏对单个分配可靠性的评估，诊断指标仅反映全局质量，无法量化特定实例的分配置信度，特别是对于初始化敏感的算法如k-means。分配层面的不稳定性会影响准确性和鲁棒性。

Method: 提出CAKE框架，通过聚类集成计算两个互补统计量：分配稳定性（跨运行一致性）和局部几何拟合一致性（学习到的聚类结构支持）。将这些统计量组合成[0,1]范围内的可解释置信度分数。

Result: 理论分析表明CAKE在噪声下保持有效，并能区分稳定点和不稳定点。在合成和真实数据集上的实验表明，CAKE能有效识别模糊点和稳定核心成员，提供可用于指导过滤或优先级排序的置信度排名。

Conclusion: CAKE框架为聚类分配提供了点级别的置信度量化方法，结合了跨运行一致性和几何支持，能够提高聚类质量并指导后续分析决策。
