...
TL;DR: MeCSAFNet是一个用于多光谱图像土地覆盖分割的多分支编码器-解码器架构，通过双ConvNeXt编码器分别处理可见光和非可见光通道，结合CBAM注意力和ASAU激活函数，在FBP和Potsdam数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的土地覆盖分割方法在处理多光谱图像时，未能充分利用不同光谱通道的特性，特别是可见光和非可见光波段之间的互补信息。需要设计一个能够有效融合多光谱信息并适应不同光谱配置的架构。

Method: 提出MeCSAFNet多分支编码器-解码器架构：1) 使用双ConvNeXt编码器分别处理可见光和非可见光通道；2) 每个分支有独立的解码器重建空间信息；3) 专门的融合解码器在多尺度上集成中间特征；4) 采用CBAM注意力增强特征融合；5) 使用ASAU激活函数实现稳定高效优化；6) 支持4通道(RGB+NIR)和6通道(含NDVI、NDWI)输入。
...
Conclusion: MeCSAFNet通过多分支架构有效融合多光谱信息，在土地覆盖分割任务上取得了显著性能提升，其紧凑变体适合资源受限环境部署，为多光谱图像分析提供了有效的解决方案。
...
TL;DR: 本文对基于多模态大语言模型（MLLM）的图表理解研究进行了系统性综述，梳理了该领域的发展脉络、核心挑战、任务分类、方法演进以及未来方向。


<details>
  <summary>Details</summary>
Motivation: 图表理解作为典型的信息融合任务，需要整合图形和文本数据来提取意义。虽然多模态大语言模型（MLLMs）已经革新了这一领域，但当前基于MLLM的图表分析研究仍处于碎片化状态，缺乏系统性的组织。本文旨在为该新兴领域提供一个全面的路线图，促进更稳健可靠系统的发展。

Method: 本文采用系统性综述方法：首先分析图表中视觉与语言信息融合的基本挑战；然后对下游任务和数据集进行分类，提出规范与非规范基准的新颖分类法；接着全面梳理方法论的演进，从经典深度学习技术到最先进的MLLM范式；最后通过批判性分析当前模型的局限性，识别未来研究方向。

Result: 本文构建了MLLM图表理解领域的结构化框架，包括：1）明确了视觉-语言信息融合的核心挑战；2）建立了下游任务和数据集分类体系；3）提出了规范与非规范基准的分类法；4）系统梳理了从传统方法到先进MLLM方法的技术演进路径；5）识别了当前模型在感知和推理方面的缺陷。

Conclusion: 本综述为研究人员和从业者提供了关于MLLM如何变革图表信息融合的结构化理解，旨在推动该领域向更稳健可靠系统发展。未来方向包括先进的校准技术和用于认知增强的强化学习等，以解决当前模型的感知和推理局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有小样本学习方法主要关注视觉模态，直接从原始支持图像计算原型，缺乏全面丰富的多模态信息。为了克服这些限制，需要开发能够整合多模态信息的小样本学习框架。

Method: 提出MPA框架，包含三个核心组件：1) LLM多变量语义增强(LMSE)：利用大语言模型生成多样化的类别描述，丰富支持集的语义线索；2) 分层多视图增强(HMA)：利用自然和多视图增强技术增强特征多样性；3) 自适应不确定类吸收器(AUCA)：通过插值和高斯采样引入不确定类，有效吸收不确定样本。

Result: 在4个单域和6个跨域小样本学习基准测试中，MPA在大多数设置下都优于现有最先进方法。在5-way 1-shot设置中，MPA在单域和跨域设置下分别比第二名方法高出12.29%和24.56%。

Conclusion: MPA框架通过整合多模态信息、增强特征多样性和处理不确定性，显著提升了小样本学习的性能，特别是在跨域场景中表现尤为突出。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在长上下文和复杂推理任务中面临挑战，需要深入理解其内部处理机制以解决性能瓶颈。

Method: 通过注意力分析识别关键的视觉证据检索头，提出VERA框架：基于模型不确定性检测触发视觉证据的显式语言化。

Result: VERA显著提升了开源VLM的长上下文理解能力：在Qwen3-VL-8B-Instruct上平均相对提升21.3%，在GLM-4.1V-Thinking上提升20.1%。

Conclusion: 视觉证据检索头对模型性能具有因果影响，基于此发现的VERA框架能有效增强视觉语言模型的长上下文处理能力。
...
TL;DR: 提出了一套用于3D高斯溅射表示（3DGS）的交互式选择和分割工具，支持用户引导的2D到3D选择传播、手动分割，以及基于视频扩散模型的局部编辑，无需额外优化即可用于任意野外捕获场景。


<details>
  <summary>Details</summary>
Motivation: 虽然3D高斯溅射表示在物理模拟和动画等应用中逐渐成为传统图形的可行替代方案，但从野外捕获中提取可用对象仍然具有挑战性，且针对该表示的可控编辑技术有限。现有技术大多专注于自动解决方案或高级编辑，缺乏灵活的交互工具。

Method: 开发了一套交互式工具集，核心是灵活的高斯溅射选择和分割：1）快速AI驱动方法，将用户引导的2D选择掩码传播到3DGS选择；2）支持用户干预错误情况；3）结合灵活的手动选择和分割工具，实现对非结构化3DGS场景的任意二进制分割；4）基于自定义视频扩散模型开发用户引导的局部编辑方法。

Result: 与高斯溅射选择的最新技术相比，该工具集表现出色。通过灵活的选择工具，用户可以直接控制AI可修改的区域。选择和编辑工具可用于任何野外捕获场景，无需额外优化。

Conclusion: 提出的交互式工具集解决了3D高斯溅射表示中对象提取和可控编辑的挑战，为用户提供了灵活的选择和分割能力，支持下游应用如局部编辑，为野外捕获的3DGS场景处理提供了实用解决方案。
...
TL;DR: 该论文提出了首个视觉中心越狱攻击(VJA)，通过纯视觉输入传达恶意指令，并建立了图像编辑模型安全基准IESBench，同时提出基于内省多模态推理的无训练防御方法。


<details>
  <summary>Details</summary>
Motivation: 随着图像编辑模型从文本驱动转向视觉提示编辑，用户意图直接从视觉输入推断，这引入了关键但未被充分探索的安全风险：攻击面本身变为视觉形式。论文旨在系统研究这一新兴威胁。

Method: 提出了视觉中心越狱攻击(VJA)，这是首个视觉到视觉的越狱攻击方法；建立了IESBench安全基准用于评估图像编辑模型；提出基于内省多模态推理的无训练防御方法，无需辅助防护模型且计算开销可忽略。

Result: VJA攻击对最先进的商业模型有效，在Nano Banana Pro上攻击成功率高达80.9%，在GPT-Image-1.5上达70.1%。提出的防御方法显著提高了未对齐模型的安全性，使其达到与商业系统相当的水平。

Conclusion: 该研究揭示了图像编辑模型的新漏洞，提供了基准和实用防御方法，推动了安全可信的现代图像编辑系统发展。论文包含大型图像编辑模型生成的冒犯性图像警告。
...
TL;DR: 本文提出了一个名为PMMA的使用助行器的行人检测数据集，包含9个类别，在户外环境中收集。使用7种检测模型和3种跟踪算法进行基准测试，结果显示YOLOX、Deformable DETR和Faster R-CNN表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对使用助行器（如轮椅、拐杖、助行器）的行人检测数据集，这些人群在智能交通系统和辅助技术中具有重要应用价值，需要专门的数据集来提升相关检测和跟踪性能。

Method: 在户外环境中收集志愿者使用轮椅、拐杖和助行器的视频数据，构建包含9个类别的PMMA数据集。使用MMDetection框架实现7种目标检测模型（Faster R-CNN、CenterNet、YOLOX、DETR、Deformable DETR、DINO、RT-DETR）和3种跟踪算法（ByteTrack、BOT-SORT、OC-SORT）建立基准。

Result: 实验结果显示，YOLOX、Deformable DETR和Faster R-CNN在检测性能上表现最佳，而三种跟踪算法之间的差异相对较小。数据集和代码已公开提供。

Conclusion: PMMA数据集填补了助行器使用者检测数据的空白，为相关研究提供了有价值的基准。公开的数据集和代码将促进智能交通和辅助技术领域的发展。
...
### [8] [MetaphorStar: Image Metaphor Understanding and Reasoning with End-to-End Visual Reinforcement Learning](https://arxiv.org/abs/2602.10575)
*Chenhao Zhang,Yazhe Niu,Hongsheng Li*

Main category: cs.CV

TL;DR: 论文提出MetaphorStar框架，首个端到端视觉强化学习系统，用于解决图像隐喻理解难题，在图像暗示任务上平均提升82.6%性能，超越主流MLLMs
...
Result: MetaphorStar-32B在多项选择问题和开放式问题上达到SOTA，在判断题上显著超越顶级闭源模型Gemini-3.0-pro，平均性能提升82.6%，同时提升模型的通用理解能力特别是复杂视觉推理能力

Conclusion: 学习图像暗示任务能提升模型的通用理解能力，特别是复杂视觉推理能力，该方法具有广泛适用性，所有模型权重、数据集和方法代码均已开源
...
<details>
  <summary>Details</summary>
Motivation: 从单张图像生成3D内容是一个病态问题，因为遮挡区域缺乏几何和纹理信息。虽然现有生成模型可以合成辅助视图提供额外监督，但这些视图包含几何不一致和纹理错位，会在3D重建过程中传播和放大伪影。

Method: 提出ERGO自适应优化框架，基于超额风险分解将3D高斯溅射的优化损失分解为两个分量：超额风险（量化当前参数与最优参数之间的差距）和贝叶斯误差（建模合成视图中固有的不可约噪声）。该分解使ERGO能够动态估计视图特定的超额风险，并在优化过程中自适应调整损失权重。此外，引入几何感知和纹理感知目标，与超额风险权重机制互补，建立协同的全局-局部优化范式。

Result: 在Google Scanned Objects数据集和OmniObject3D数据集上的广泛实验表明，ERGO在对抗监督噪声方面表现出鲁棒性，同时持续提升重建3D内容的几何保真度和纹理质量，优于现有最先进方法。

Conclusion: ERGO通过超额风险分解和自适应损失加权，有效利用不完美的监督信号，解决了单图像3D重建中合成视图噪声问题，显著提升了3D内容的几何和纹理质量。
...
TL;DR: 提出了一种名为LoRD（低秩防御）的高效防御策略，用于保护潜在扩散模型（LDMs）免受对抗攻击，通过在低秩适应（LoRA）模块中引入合并思想和平衡参数来检测和防御对抗样本。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型及其微调过程中对抗攻击算法的快速发展，需要开发相应的防御策略来防止这些攻击算法被滥用，从而影响扩散模型的实际应用。

Method: 提出LoRD防御策略，结合低秩适应（LoRA）模块，引入合并思想和平衡参数来检测和防御对抗样本。基于LoRD构建了一个防御流程，将学习到的LoRD模块应用于帮助扩散模型防御攻击算法。

Result: 在面部和风景图像上进行了大量实验，结果表明该方法在对抗攻击和干净样本上微调的LDM仍能生成高质量图像，且防御性能显著优于基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于采样的生成模型如流匹配取得了显著成功，但它们仍可能产生质量不一致或退化的样本。需要评估样本可靠性并生成更高质量的输出

Method: 提出不确定性感知流匹配(UA-Flow)，作为流匹配的轻量级扩展，同时预测速度场和异方差不确定性。通过流动力学传播速度不确定性来估计每个样本的不确定性，并利用不确定性信号通过不确定性感知分类器引导和无分类器引导来指导生成

Result: 图像生成实验表明，UA-Flow产生的不确定性信号与样本保真度的相关性高于基线方法，并且不确定性引导采样进一步提高了生成质量

Conclusion: UA-Flow通过集成不确定性估计和引导机制，有效提升了流匹配模型的样本可靠性和生成质量
...
<details>
  <summary>Details</summary>
Motivation: 相干成像中斑点噪声被建模为乘性噪声，给图像重建带来根本挑战。虽然最大似然估计为斑点抑制提供了原则性框架，但在数字全息等有限孔径系统中，高维矩阵求逆的计算成本过高，阻碍了物理精确孔径建模的MLE应用。

Method: 提出投影梯度下降与蒙特卡洛估计方法，利用随机线性代数技术实现可扩展的MLE优化，无需在梯度计算中进行显式矩阵求逆。通过利用传感矩阵的结构特性并使用共轭梯度进行似然梯度评估，支持精确孔径建模而不需要常见的简化假设。

Result: PGD-MC框架对多样且物理精确的孔径模型表现出鲁棒性，在重建质量和计算效率方面实现显著改进，并能有效扩展到高分辨率数字全息。实验表明，该方法在准确性和速度上均优于现有的即插即用模型迭代重建方法。

Conclusion: PGD-MC为有限孔径数字全息提供了一个灵活有效的基于MLE的重建框架，克服了传统方法的计算瓶颈，支持物理精确的孔径建模，在重建质量和计算效率方面均有显著提升。
...
TL;DR: 开发并验证了两个首个完全开源且获得FDA 510(k)许可的深度学习管道：用于CT扫描机会性分析的AAQ（腹主动脉量化）和BMD（骨密度估计），解决了开源方案验证不足和商业方案不透明的问题。


<details>
  <summary>Details</summary>
Motivation: 当前开源影像分析解决方案缺乏严格验证，而商业解决方案缺乏透明度，导致部署时出现意外失败。需要开发既经过严格验证又完全透明的解决方案，以促进人工智能在医学影像中的可靠应用。

Method: 开发了两个深度学习管道：AAQ用于腹主动脉分割以评估动脉瘤大小，BMD用于椎体分割以估计骨小梁密度和骨质疏松风险。两个算法都集成在Comp2Comp包中，并在多个外部机构的患者扫描上进行验证。

Result: AAQ在258例患者扫描上的平均绝对误差为1.57毫米（95% CI 1.38-1.80毫米）。BMD在371例患者扫描上的敏感性为81.0%（95% CI 74.0-86.8%），特异性为78.4%（95% CI 72.3-83.7%）。两个算法都达到了临床使用的足够准确性。
...
TL;DR: 该论文提出了一种通过合成幻觉诱导图像来揭示视觉语言模型中基于语言偏置的幻觉模式，并利用这些图像构建高质量偏好数据集进行细粒度对齐，有效缓解幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但仍存在由固有语言偏置引起的幻觉问题。现有缓解方法往往忽视了语言偏置驱动的底层幻觉模式，需要更深入的研究。

Method: 1. 设计新颖的管道准确合成幻觉诱导图像；2. 揭示场景条件幻觉模式：模型倾向于提及场景中典型对象，即使视觉证据已被移除；3. 建立Masked-Object-Hallucination基准评估现有对齐框架；4. 利用幻觉诱导图像构建高质量偏好数据集进行细粒度对齐。

Result: 实验结果表明，该方法在保持模型通用能力的同时有效缓解了幻觉问题。在标准幻觉基准测试中，相比当前最先进方法实现了高达38%的改进。

Conclusion: 通过合成幻觉诱导图像揭示并量化视觉语言模型的幻觉模式，并利用这些图像进行细粒度对齐，是缓解模型幻觉问题的有效途径，同时保持了模型的通用能力。
...
TL;DR: 本文提出了一种分析多分辨率哈希编码（MHE）的新方法，通过点扩散函数（PSF）来量化其空间分辨率和保真度，揭示了网格引起的各向异性和对数空间分布，并提出了旋转MHE（R-MHE）来缓解各向异性问题。


<details>
  <summary>Details</summary>
Motivation: 多分辨率哈希编码（MHE）作为即时神经图形原语的基础技术，为神经场提供了强大的参数化方法。然而，其空间行为缺乏从物理系统角度的严格理解，导致超参数选择依赖启发式方法。本文旨在建立基于物理原理的方法论来表征和优化MHE。

Method: 引入了一种新颖的分析方法，通过检查MHE的点扩散函数（PSF，类似于系统的格林函数）来表征其特性。推导了碰撞自由PSF的闭式近似，揭示了固有的网格诱导各向异性和对数空间分布。基于理论洞察，提出了旋转MHE（R-MHE）架构，该架构在每个分辨率级别对输入坐标应用不同的旋转。

Result: 研究发现：1）理想化的空间带宽（FWHM）由平均分辨率Navg决定；2）模型的有效分辨率由经验FWHM（因此Navg）决定，而非最精细分辨率Nmax，这种展宽效应源于优化动态；3）有限哈希容量会引入散斑噪声并降低信噪比；4）R-MHE能够缓解各向异性，同时保持原始MHE的效率和参数数量。

Conclusion: 本研究建立了一种基于物理原理的方法论，超越了启发式方法，能够表征和优化多分辨率哈希编码。提出的R-MHE架构在保持效率的同时缓解了各向异性问题，为神经场的参数化提供了更严格的理论基础。
...
Method: 使用稀疏自编码器在SegFormer和U-Net上训练，将分割模型激活分解为可解释的潜在特征；通过跨架构和跨数据集的潜在对齐（健康、成人、儿童和撒哈拉以南非洲胶质瘤队列）进行分析。

Result: 识别出稳定的共享表示骨干，数据集偏移由对特定人群潜在特征的依赖差异驱动；这些潜在特征作为分割失败的因果瓶颈，针对性干预可以纠正错误并改善跨数据集适应，无需重新训练就能恢复70%的失败案例，Dice分数从39.4%提高到74.2%。
...
Method: 提出Complex Linear Projection Optimization (CoLin)适配器：1) 设计新型低秩复数适配器架构，仅引入约1%参数；2) 理论上证明低秩复合矩阵在训练中存在严重收敛问题，并通过定制化损失函数解决这一挑战

Result: 在目标检测、分割、图像分类和旋转目标检测（遥感场景）等任务上的广泛实验表明，CoLin仅用1%参数首次超越了全微调和经典delta-tuning方法

Conclusion: CoLin为视觉基础模型部署提供了新颖且高效的解决方案，代码已在GitHub上开源
...
TL;DR: 3DXTalker是一个通过数据增强身份建模、音频丰富表示和空间动态可控性实现表达性3D说话头像生成的框架，解决了现有方法在身份泛化、唇语同步和情感表达方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动的3D说话头像生成面临三大挑战：1）训练数据不足且身份有限；2）音频表示狭窄；3）显式可控性受限。这些限制导致难以同时实现身份保持、唇语同步、情感表达和空间动态等表达性目标。

Method: 1）通过2D到3D数据整理流水线和解耦表示实现可扩展身份建模；2）引入帧级振幅和情感线索超越标准语音嵌入；3）使用基于流匹配的transformer统一这些线索；4）支持基于提示的条件控制实现风格化头部姿态运动生成。

Result: 大量实验表明，3DXTalker在统一的框架中集成了唇语同步、情感表达和头部姿态动态，在3D说话头像生成方面实现了优越性能。

Conclusion: 3DXTalker通过创新的数据整理、丰富的音频表示和可控的空间动态，成功解决了表达性3D说话头像生成的关键挑战，为虚拟通信、数字人类和交互媒体提供了更自然、更具表现力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前用于评估视觉语言模型在地图推理方面的数据集范围狭窄、局限于特定领域、过度依赖人工生成内容，缺乏对真实地理空间推理的深度评估。现有大型语言模型和视觉语言模型在整合空间关系、视觉线索、现实世界背景和领域专业知识方面仍存在困难。

Method: 构建MapVerse基准测试，包含11,837个人工编写的问题-答案对，基于1,025张真实世界地图，涵盖十个不同的地图类别和多个问题类别。评估了十个最先进的模型，进行细粒度分类分析，并研究影响推理结果的视觉因素。

Result: 评估发现，当前视觉语言模型在分类式任务上表现有竞争力，但开源和闭源模型在需要复杂空间推理的高级任务上都表现不足。基准测试为地图推理提供了全面的评估框架。

Conclusion: MapVerse填补了地图推理评估的空白，揭示了当前模型在复杂空间推理方面的局限性，为未来模型开发提供了重要的基准和方向。
...
TL;DR: 本文提出了一个高质量、大规模的数据集（超过73万张图像），用于检测AI生成图像，并基于图像噪声熵提出了一种轻量级检测方法。
...
Method: 1) 构建包含超过73万张图像的大规模数据集，涵盖文本到图像生成（使用1万多个精心设计的提示词）、图像修复、图像精修和人脸交换等多种生成方法；2) 提出基于图像噪声熵的轻量级检测方法，将原始图像转换为非局部均值噪声的熵张量进行分类。

Result: 在该数据集上训练的检测模型展现出优越的泛化能力，提出的检测方法具有竞争力性能，为未来研究建立了坚实基础。

Conclusion: 该数据集不仅为评估检测方法提供了强大基准，还推动了AI生成图像检测技术的鲁棒性发展，数据集和源代码已公开。
...
TL;DR: 提出文本引导的多模态视频异常检测框架，通过上下文学习增强文本特征，使用多尺度瓶颈Transformer融合模块解决多模态冗余和不平衡问题，在UCF-Crime和XD-Violence数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 弱监督多模态视频异常检测中文本模态潜力未被充分挖掘，文本能提供显式语义信息增强异常表征并减少误报。但通用语言模型难以捕捉异常特定细节，且相关描述稀缺。此外，多模态融合存在冗余和不平衡问题。
...
TL;DR: C²RoPE：一种改进的旋转位置编码方法，专门针对3D大语言模型中的视觉处理问题，通过建模局部空间连续性和空间因果关系来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D大语言模型继承了传统RoPE的局限性：1）使用1D时间位置索引破坏了视觉特征在列维度上的连续性；2）基于时间邻近性的因果假设导致注意力分配的长程衰减，随着序列增长模型会逐渐忽略早期视觉token。

Method: 提出C²RoPE方法：1）构建三元混合位置索引，将1D时间位置与基于笛卡尔坐标的空间坐标结合；2）采用频率分配策略在三个索引分量上编码时空位置信息；3）引入切比雪夫因果掩码，通过计算2D空间中图像token的切比雪夫距离来确定因果依赖关系。
...
TL;DR: 基于YOLOv11n架构，提出轻量级儿童检测方案，通过领域特定数据增强和SAHI推理优化，在低质量监控视频中提升儿童检测性能，适用于边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界失踪儿童警报和日托监控系统中，由于现有CCTV基础设施质量差（遮挡、小目标、低分辨率、运动模糊、光照不足）导致的儿童检测困难问题。

Method: 1. 基于高效的YOLOv11n架构；2. 引入领域特定数据增强策略：空间扰动（部分可见、截断、重叠）和光度退化（光照变化、噪声）；3. 推理时集成SAHI（切片辅助超推理）以提升小目标和部分遮挡实例的召回率。

Result: 在Roboflow Daycare数据集的儿童子集上评估，相比基线YOLOv11n，mAP@0.5达到0.967（提升0.7%），mAP@0.5:0.95达到0.783（提升2.3%），无需架构修改。

Conclusion: 提出了一种实用轻量级的儿童检测解决方案，在保持与低功耗边缘设备兼容性和实时性能的同时，显著提升了在挑战性监控条件下的检测精度，特别适合低成本或资源受限的工业监控部署。
...
TL;DR: 该论文研究使用YOLOX和Edge AI加速器硬件在火车站进行乘客检测，比较了Hailo-8和Jetson Orin Nano的性能，结果显示Hailo-8在准确性和延迟方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 火车站区域（特别是黄色警戒线附近）经常发生乘客不慎导致的安全事故，需要开发额外的安全技术来减少事故数量。图像处理技术在交通领域的安全应用日益重要，特别是在火车站乘客检测方面。

Method: 使用YOLOX目标检测算法结合Edge AI加速器硬件，在火车站场景中进行乘客检测。研究比较了两种硬件平台：Hailo-8 AI硬件加速器和Jetson Orin Nano的性能表现。

Result: 实验结果表明，Hailo-8 AI硬件加速器相比Jetson Orin Nano具有更高的准确性（提升超过12%），并且具有更低的延迟（减少20毫秒）。
...
TL;DR: VRFT-Aug是一个针对医学领域的视觉强化微调框架，通过增强感知和推理能力来改进医学图像分析，在多个数据集上优于标准监督微调和强化微调基线。


<details>
  <summary>Details</summary>
Motivation: 现有的基于规则的奖励方案强化微调方法主要针对语言模型，在跨模态、视觉为中心的医学成像领域应用不足。医学成像需要强大的视觉感知和结构化推理能力，这一领域的研究存在空白。

Method: 提出VRFT-Aug框架，包含一系列训练策略：先验知识注入、感知驱动的策略细化、医学知识引导的奖励塑造和行为模仿，旨在稳定和改进强化微调过程。

Result: 在多个医学数据集上的实验表明，该方法持续优于标准监督微调和强化微调基线，并为其他医学图像任务提供了可推广的经验性见解和实用训练启发。

Conclusion: 这项工作为开发可靠、具备推理能力的高风险医学应用模型提供了可行的指导和新思路，推动了医学视觉强化微调领域的发展。
...
TL;DR: DermFM-Zero是一个皮肤病视觉语言基础模型，通过零样本学习在20个基准测试中达到最先进性能，并在涉及1100多名临床医生的多国读者研究中验证了其临床实用性。


<details>
  <summary>Details</summary>
Motivation: 当前医学基础模型在受控基准测试中表现良好，但广泛部署受到任务特定微调需求的限制。研究旨在开发无需任务特定适应就能提供有效临床决策支持的皮肤病基础模型。

Method: 开发了DermFM-Zero皮肤病视觉语言基础模型，通过掩码潜在建模和对比学习在超过400万个多模态数据点上进行训练。在20个基准测试中评估零样本诊断和多模态检索能力，并在三个多国读者研究中验证临床实用性。

Result: 在零样本设置下达到最先进性能；在初级保健中使全科医生的鉴别诊断准确率几乎翻倍；在专科环境中超越认证皮肤科医生的皮肤癌评估能力；在协作工作流中使非专家超越未辅助的专家；模型潜在表示可解释，能无监督解缠临床相关概念并抑制伪影诱导偏差。

Conclusion: DermFM-Zero证明基础模型能够提供有效、安全且透明的零样本临床决策支持，无需任务特定适应即可实现广泛部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在真实世界超分辨率任务中取得了突破，但存在推理速度慢、设备需求高的问题。虽然GenDR通过步数蒸馏将步数减少到一步，但VAE仍然是延迟和内存的瓶颈，限制了最大处理尺寸，需要分块恢复高分辨率图像。

Method: 1) 利用像素重排操作消除VAE，将潜在空间的GenDR转换为像素空间的GenDR-Pix；2) 提出多阶段对抗蒸馏逐步移除编码器和解码器，利用前一阶段模型的生成特征指导对抗判别；3) 引入随机填充增强生成特征并避免判别器崩溃；4) 提出掩码傅里叶空间损失惩罚振幅异常值；5) 集成基于填充的自集成和分类器自由引导改进推理缩放。

Result: GenDR-Pix相比GenDR实现了2.8倍加速和60%内存节省，视觉质量下降可忽略不计，超越其他一步扩散超分辨率方法。能够在仅1秒和6GB内存下恢复4K图像。

Conclusion: 通过消除VAE瓶颈并将模型转换到像素空间，GenDR-Pix显著提高了扩散超分辨率模型的效率和实用性，实现了快速、内存高效的高质量图像恢复。
...
TL;DR: VideoLLMs存在严重输出重复问题，现有基准无法检测，作者提出VideoSTF框架系统测量和压力测试输出重复，发现该问题普遍存在且易受时间扰动影响，构成安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 发现视频大语言模型存在先前未被充分探索的生成失败模式：严重输出重复，模型会陷入自我强化的重复短语或句子循环。现有VideoLLM基准主要关注任务准确性和事实正确性，无法捕捉这种失败模式。

Method: 提出VideoSTF框架，使用三种互补的n-gram指标形式化重复测量，提供包含10,000个多样化视频的标准化测试平台和受控时间变换库。通过VideoSTF对10个先进VideoLLM进行普遍测试、时间压力测试和对抗性利用。

Result: 发现输出重复问题普遍存在，且对视频输入的时间扰动高度敏感。简单的时域变换可以在黑盒设置中有效诱导重复退化，暴露输出重复作为可利用的安全漏洞。

Conclusion: 输出重复是现代VideoLLMs的基本稳定性问题，需要稳定性感知的视频语言系统评估。研究揭示了VideoLLMs中先前未被认识到的脆弱性，并提供了系统评估工具。
...
TL;DR: MP-HOI是一个用于文本驱动的3D人-物交互运动生成的新框架，通过多模态数据先验、增强的对象表示、多模态感知的专家混合模型和级联扩散框架来解决现有方法在人体运动、物体运动和交互质量方面的局限性。
...
Method: 提出MP-HOI框架，包含四个核心创新：1) 利用大型多模态模型的多模态数据（文本、图像、姿态/物体）作为先验指导HOI生成；2) 通过几何关键点、接触特征和动态属性增强对象表示；3) 提出多模态感知的专家混合模型进行有效的多模态特征融合；4) 设计级联扩散框架，在专门监督下逐步细化人-物交互特征。

Result: 综合实验表明，MP-HOI在生成高保真和细粒度的人-物交互运动方面优于现有方法。

Conclusion: MP-HOI通过多模态数据先验、增强的对象表示、多模态感知的专家混合模型和级联扩散框架，有效解决了文本驱动3D人-物交互运动生成中的跨模态差距问题，显著提升了生成质量。
...
TL;DR: 本文提出了一种基于频率调制的训练免费方法，用于在文本引导扩散模型中实现语义修改时保持结构框架一致性，避免传统方法依赖经验特征图选择的问题。
...
Method: 从频率角度分析噪声潜在变量的频谱对生成过程中结构框架和细粒度纹理层次出现的影响，发现低频分量主要在早期生成阶段负责建立结构框架。提出训练免费的频率调制方法，利用具有动态衰减的频率相关加权函数，直接操作噪声潜在变量，避免内部特征图的经验选择。

Result: 大量实验表明，所提出的方法显著优于当前最先进的方法，在保持结构和实现语义更新之间实现了有效平衡。

Conclusion: 通过频率视角分析生成过程，提出的训练免费频率调制方法能够有效解决文本引导扩散模型中语义修改时的结构保持问题，避免了传统方法的经验选择依赖，实现了更好的稳定性和性能。
...
TL;DR: AMAP-APP是一个跨平台桌面应用，通过优化算法将足细胞形态分析处理速度提升147倍，同时保持与原方法的高相关性，使深度学习足细胞形态测量更易普及。


<details>
  <summary>Details</summary>
Motivation: 现有的足细胞自动形态分析方法(AMAP)存在计算需求高、缺乏用户界面和Linux依赖等限制，阻碍了其在肾脏研究中的广泛应用。需要开发一个更易用的跨平台解决方案。

Method: AMAP-APP通过用经典图像处理替代计算密集的实例分割来优化效率，同时保留原始语义分割模型。引入了改进的感兴趣区域(ROI)算法以提高精度。使用365张小鼠和人类图像进行验证，通过皮尔逊相关性和双侧等效性检验与原方法进行性能比较。

Result: 在消费级硬件上处理速度提升147倍。形态测量输出(面积、周长、圆形度和裂孔隔膜密度)与原方法高度相关(r>0.90)且统计等效。新的ROI算法相比原方法精度更高，与手动标注的偏差更小。

Conclusion: AMAP-APP使基于深度学习的足细胞形态测量更加普及。通过消除对高性能计算集群的需求，并为Windows、macOS和Linux提供用户友好界面，促进了其在肾脏病学研究和潜在临床诊断中的广泛应用。
...
TL;DR: TwiFF-2.7M是首个大规模、时间基础的视觉思维链数据集，包含270万视频片段，专门用于动态视觉问答。研究者还提出了TwiFF模型，利用预训练视频生成和图像理解能力，在动态推理任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉思维链方法主要局限于静态场景，难以捕捉指令、预测和相机运动等任务所需的时间动态性，因此需要开发适用于动态场景的视觉推理方法。

Method: 提出TwiFF-2.7M数据集（270万视频片段）和TwiFF-Bench评估基准（1078个样本）。开发TwiFF模型，协同利用预训练视频生成和图像理解能力，迭代生成未来动作帧和文本推理，产生时间一致的视觉推理线索。

Result: TwiFF模型在动态推理任务上显著优于现有视觉思维链方法和文本思维链基线，验证了其在动态场景中视觉问答的有效性。

Conclusion: 该研究填补了动态视觉思维链的空白，提出的数据集、基准和模型为动态场景下的视觉推理提供了有效解决方案，推动了多模态推理的发展。
...
TL;DR: 提出OmniVL-Guard框架，用于多模态（文本、图像、视频）伪造检测与定位，解决传统方法局限于单模态或双模态的问题，通过平衡强化学习克服检测与定位任务间的难度偏差。


<details>
  <summary>Details</summary>
Motivation: 现有伪造检测方法通常局限于单模态或双模态设置，无法处理真实世界虚假信息中交织的文本、图像和视频。同时，多模态交互以及同时进行检测和定位的双重要求导致了"难度偏差"问题：较简单的真伪分类任务倾向于主导梯度，导致多任务优化中细粒度定位性能不佳。

Method: 提出OmniVL-Guard框架，包含两个核心设计：1) Self-Evolving CoT Generation（自演化思维链生成）：合成高质量推理路径，有效克服冷启动挑战；2) Adaptive Reward Scaling Policy Optimization (ARSPO，自适应奖励缩放策略优化)：动态调整奖励规模和任务权重，确保平衡的联合优化。

Result: 大量实验表明，OmniVL-Guard显著优于最先进的方法，并在跨域场景中表现出零样本鲁棒泛化能力。

Conclusion: OmniVL-Guard为全模态视觉语言伪造检测和定位提供了一个统一的平衡强化学习框架，有效解决了多模态交互和双任务优化中的难度偏差问题，在性能和泛化能力方面都取得了显著提升。
...
TL;DR: 提出了一种将深度估计集成到视觉-语言-动作模型中的新框架，通过VGGT深度估计提取3D几何特征，并引入动作助手模块约束学习，提升模型在复杂3D环境中的空间理解和动作预测能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要依赖2D图像训练的视觉语言模型，限制了其在复杂3D环境中的空间理解和动作定位能力，需要解决2D观测与3D感知决策之间的差距。
...
Conclusion: 这项工作展示了深度驱动数据增强和辅助专家监督在弥合机器人系统中2D观测与3D感知决策之间差距的潜力，为VLA模型在复杂3D环境中的应用提供了有效解决方案。
...
TL;DR: 提出(MGS)²框架，通过几何基础方法解决跨视角地理定位中无人机倾斜航拍与正射卫星图像间的几何错配问题，显著提升定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法主要在2D流形上操作，忽略了3D几何结构，导致视角相关的垂直立面（宏观结构）和尺度变化（微观尺度）严重破坏特征对齐，影响GNSS缺失环境下无人机导航的鲁棒性。

Method: 提出几何基础框架(MGS)²，包含三个核心模块：1) 宏观几何结构过滤(MGSF)模块，利用扩张几何梯度物理过滤立面伪影，增强视角不变的水平平面；2) 微观几何尺度适应(MGSA)模块，利用深度先验通过多分支特征融合动态校正尺度差异；3) 几何-外观对比蒸馏(GACD)损失函数，严格区分倾斜遮挡。

Result: 在University-1652数据集上Recall@1达到97.5%，在SUES-200数据集上达到97.02%，实现了最先进的性能，并在跨数据集泛化中表现出对几何模糊性的优越鲁棒性。

Conclusion: (MGS)²框架通过显式建模3D几何结构，有效解决了跨视角地理定位中的几何错配问题，为GNSS缺失环境下的无人机导航提供了更鲁棒的解决方案，代码已开源。
...
TL;DR: 该研究探索了地理空间基础模型在生态应用中的效用，通过微调Prithvi-E0-2.0和TerraMind模型，在三个用例中评估其性能，发现这些模型在土地覆盖分类、森林功能性状制图和泥炭地检测方面均优于基线ResNet-101模型。


<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型在生态应用中的潜力尚未被充分探索，本研究旨在评估这些模型在生态用途中的实际效用、面临的挑战和机遇，特别是在高价值应用场景中。

Method: 研究微调了预训练的Prithvi-E0-2.0和TerraMind模型，在三个生态用例中进行评估：土地覆盖分类、森林功能性状制图和泥炭地检测，并与基线ResNet-101模型进行性能比较。

Result: 在所有实验中，地理空间基础模型都优于基线ResNet模型。TerraMind模型在大多数情况下略优于Prithvi，当加入额外模态数据时，TerraMind显著优于基线和Prithvi模型。但需要注意输入数据与预训练模态的差异。

Conclusion: 地理空间基础模型在生态应用中具有显著优势，但需要更高分辨率和更准确的标签数据，特别是在需要像素级动态制图的应用中。模型性能受输入数据与预训练模态匹配程度的影响。
...
TL;DR: 提出S3 RIQA方法，针对真实世界低分辨率图像的超分辨率质量评估问题，通过自监督对比学习框架，利用不同SR算法生成的图像对来学习质量表示，在真实SR-IQA基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 真实世界低分辨率图像的超分辨率处理会产生复杂、不规则的退化，这些退化高度不可预测且随场景变化。与合成LR图像产生的SR伪影不同，真实场景的SR质量评估（SR-IQA）具有挑战性且研究不足，特别是在数据稀缺领域。

Method: 提出S3 RIQA方法：1）假设SR图像退化主要取决于SR算法而非图像内容；2）采用自监督对比学习框架，将同一SR算法生成的图像作为正对，不同算法生成的图像作为负对；3）引入针对性预处理提取互补质量信息；4）添加辅助任务处理不同SR缩放因子相关的退化特征；5）构建新数据集SRMORSS支持无监督预训练。

Result: 在真实SR-IQA基准测试中，S3 RIQA方法持续优于大多数最相关的现有指标，证明了该方法在真实世界超分辨率质量评估中的有效性。

Conclusion: S3 RIQA为高度不适定的真实世界SR设置提供了一种无参考质量评估方法，通过自监督学习策略和对比学习框架，能够适应数据稀缺领域的SR质量评估需求，填补了现有数据集和方法在真实SR-IQA方面的空白。
...
<details>
  <summary>Details</summary>
Motivation: 矢量图形虽然具有可缩放且不失真的优势，但制作复杂。在设计和建筑领域，矢量图形的多功能性至关重要，尽管其计算需求较高。现有方法在生成高质量矢量住宅平面图方面存在不足。
...
Conclusion: 该方法在矢量住宅平面图生成和矢量化方面都取得了显著的性能提升，为设计和建筑领域提供了更高质量的矢量图形生成工具。
...
TL;DR: 该论文提出了双端一致性模型（DE-CM），通过选择关键子轨迹簇来解决一致性模型训练不稳定和采样不灵活的问题，在ImageNet 256×256数据集上实现了一步生成FID 1.70的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型和流模型的迭代采样速度慢限制了实际部署。一致性模型（CMs）作为高效的蒸馏方法，仍面临两个关键问题：训练不稳定性和采样不灵活性。现有方法通过架构调整或正则化目标来缓解这些问题，但忽视了轨迹选择的关键依赖性。

Method: 提出了双端一致性模型（DE-CM）：1）分解PF-ODE轨迹并选择三个关键子轨迹作为优化目标；2）利用连续时间CM目标实现少步蒸馏；3）使用流匹配作为边界正则化器稳定训练；4）提出新颖的噪声到噪声（N2N）映射，可将噪声映射到任意点，缓解第一步的误差累积。

Result: 在ImageNet 256×256数据集上，DE-CM实现了一步生成FID 1.70的SOTA性能，超越了现有的基于CM的一步生成方法。

Conclusion: 通过轨迹选择和双端优化策略，DE-CM有效解决了CMs的训练不稳定和采样不灵活问题，在保持高效生成的同时实现了更好的性能。
...
TL;DR: 本文介绍了CyclingVQA基准测试，用于评估视觉语言模型在自行车骑行场景下的感知和推理能力，发现现有模型在自行车中心视角理解方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 自行车骑行者在城市交通中常遇到安全关键情况，需要辅助系统支持安全决策。现有视觉语言模型在自动驾驶基准上表现良好，但评估主要是车辆中心的，缺乏从自行车骑行者视角的感知和推理评估。

Method: 作者引入了CyclingVQA诊断基准，专门从自行车骑行者视角测试感知、时空理解和交通规则到车道推理能力。评估了31+个最近的视觉语言模型，包括通用模型、空间增强模型和自动驾驶专用模型。

Result: 当前模型显示出有希望的能力，但在自行车中心感知和推理方面仍有明显改进空间，特别是在解释自行车特定交通线索和将标志与正确导航车道关联方面。值得注意的是，几个驾驶专用模型表现不如强大的通用视觉语言模型，表明从车辆中心训练到自行车辅助场景的迁移有限。

Conclusion: 通过系统错误分析，识别了重复出现的失败模式，为开发更有效的自行车辅助智能系统提供了指导。需要更多自行车中心的数据和训练来提升模型在骑行场景下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 遥感多模态大语言模型在紧急管理和农业监测等高风险场景中部署时，幻觉问题（与输入遥感图像不一致的响应）严重阻碍了其应用，而这一问题在遥感领域尚未得到充分探索。

Method: 1. 提出面向遥感的幻觉分类法，引入图像级幻觉概念；2. 构建幻觉评估基准RSHalluEval（2,023个QA对）和双模式检查机制；3. 创建领域定制数据集RSHalluShield（30k QA对）用于训练友好型缓解，并提出无需训练的即插即用策略，包括解码时对数校正和遥感感知提示。

Result: 在代表性遥感多模态大语言模型中，提出的缓解策略将无幻觉率提高了高达21.63个百分点，同时在遥感视觉问答和遥感视觉定位等下游任务上保持了竞争力。

Conclusion: RSHallu为遥感多模态大语言模型的幻觉问题提供了系统性解决方案，包括分类框架、评估基准和有效的缓解策略，有助于提升模型在关键遥感应用中的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法大多依赖类别特定训练，在少样本场景下灵活性受限，需要一种无需训练即可跨类别检测的方法

Method: 将点云转换为固定数量的真实深度图像，使用冻结的CLIP视觉编码器提取多视角特征表示，通过加权特征相似度进行异常检测

Result: 在ShapeNetPart数据集上实验表明，DMP-3DAD在少样本设置下达到最先进的性能

Conclusion: 该方法为实际跨类别3D异常检测提供了简单有效的解决方案，无需微调或类别依赖适应
...
TL;DR: DC-SFT方法通过基于样本难度筛选训练数据，显著提升了视觉语言模型的分布外泛化性能，超越了传统监督微调和强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型微调中存在泛化差距：强化学习方法在分布外性能上优于监督微调。本文提出数据中心的解释，认为RL的优势源于其隐式数据过滤机制，优先选择中等难度样本。

Method: 提出Difficulty-Curated SFT（DC-SFT）方法，通过显式地基于样本难度筛选训练集。系统评估了SFT模型在不同难度级别训练数据上的分布外泛化性能，验证了数据难度是关键因素。

Result: 实验表明DC-SFT不仅显著提升了分布外泛化性能，超越了标准SFT，甚至超过了基于RL的训练方法，同时提供了更好的稳定性和计算效率。

Conclusion: 这项工作为视觉语言模型中的分布外泛化差距提供了数据中心的解释，并建立了实现鲁棒泛化的更高效途径。DC-SFT方法简单有效，代码已开源。
...
TL;DR: FlowCache：首个专为自回归视频生成设计的缓存框架，通过分块缓存策略和联合重要性-冗余优化的KV缓存压缩机制，显著加速超长视频生成，在MAGI-1上实现2.38倍加速，SkyReels-V2上实现6.7倍加速。
...
Method: 提出FlowCache框架，核心洞察是每个视频块应保持独立的缓存策略。采用分块缓存策略，动态适应每个块的独特去噪特性，并引入联合重要性-冗余优化的KV缓存压缩机制，在保持固定内存边界的同时维持生成质量。

Result: 在MAGI-1上实现2.38倍加速，在SkyReels-V2上实现6.7倍加速，质量下降可忽略不计（VBench：分别增加0.87和减少0.79）。成功解锁自回归模型实时生成超长视频的潜力。

Conclusion: FlowCache为高效大规模视频合成建立了新基准，首次专门针对自回归视频生成的缓存框架，通过精细控制每个块的重新计算需求，显著提升生成效率。
...
TL;DR: 提出Chart Specification结构化中间表示，通过语义监督而非文本模仿来提升图表到代码生成的结构保真度，在少量训练样本下显著超越现有方法
...
Conclusion: 精确的结构监督为高保真图表到代码生成提供了高效途径，结构化中间表示能够显著提升生成代码的结构保真度
...
TL;DR: 提出TR-World模型，专注于动态物体建模，通过计算场景表征的时间残差提取动态信息，结合FGTR模块实现轨迹与未来BEV特征的交互，提升自动驾驶规划性能


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在驾驶场景中存在静态区域冗余建模和与轨迹缺乏深度交互的问题，限制了其在端到端自动驾驶框架中的规划准确性

Method: 提出时间残差世界模型(TR-World)，通过计算场景表征的时间残差提取动态物体信息；设计未来引导轨迹细化(FGTR)模块，实现先验轨迹与未来BEV特征的交互

Result: 在nuScenes和NAVSIM数据集上的综合实验表明，ResWorld方法实现了最先进的规划性能

Conclusion: TR-World专注于动态物体建模，结合FGTR模块的交互机制，有效提升了自动驾驶规划精度，防止世界模型崩溃
...
TL;DR: FastUSP是一个针对大型扩散模型分布式推理的多级优化框架，通过编译级、通信级和算子级优化，在FLUX和Qwen-Image模型上实现了1.09-1.16倍的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有统一序列并行（USP）实现存在显著的效率问题，包括过多的内核启动开销和次优的计算-通信调度，这限制了大型扩散模型（如FLUX和Stable Diffusion 3）在多GPU环境下的推理效率。

Method: 提出FastUSP多级优化框架，包含：1）编译级优化（使用CUDA Graphs进行图编译和计算-通信重排序）；2）通信级优化（FP8量化集体通信）；3）算子级优化（带双缓冲的流水线Ring attention）。

Result: 在FLUX（12B）模型上，FastUSP相比基线USP实现了1.12-1.16倍的端到端加速，编译级优化贡献了主要改进。在Qwen-Image模型上，2GPU时获得1.09倍加速；4-8GPU时发现PyTorch Inductor与Ring attention的兼容性问题限制了编译优化。

Conclusion: FastUSP有效解决了USP实现中的效率瓶颈，分析表明在现代高带宽GPU互连上，内核启动开销而非通信延迟是分布式扩散推理的主要瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常在相机坐标系下操作，限制了在机器人操作任务中的直接应用。需要一种能够在全局工作空间框架中构建占用表示的方法，使其能够直接应用于机器人操作，并能够泛化到未见过的物体布置。

Method: 提出一种可泛化的神经辐射场方法，从机器人第一人称视角观察构建3D工作空间占用表示。与之前基于相机坐标系的方法不同，该方法在全局工作空间框架中构建占用表示，支持灵活的源视图输入，无需场景特定的微调即可泛化到新的物体布置。

Result: 在40个真实场景上训练后，模型实现了26mm的重建误差，包括被遮挡区域。这验证了该方法能够推断完整的3D占用情况，超越了传统的立体视觉方法。

Conclusion: 该方法成功构建了全局工作空间框架中的3D占用表示，可直接应用于机器人操作任务，具有良好的泛化能力，能够准确重建包括遮挡区域在内的完整3D几何结构。
...
TL;DR: 该研究使用InceptionV3和ResNet50深度学习模型对番石榴病害（炭疽病、果蝇、健康果实）进行分类，通过数据增强和混合技术提升模型性能，InceptionV3达到98.15%准确率。


<details>
  <summary>Details</summary>
Motivation: 番石榴果实常受多种病害影响，会损害果实质量和产量。早期识别对于减少损害和确保果实健康至关重要。本研究旨在开发有效的病害分类方法。

Method: 使用Mendeley Data的473张原始番石榴图像，预处理为256x256像素RGB格式，通过数据增强扩展到3784张图像。采用InceptionV3和ResNet50深度学习模型进行分类，应用CutMix和MixUp数据混合技术增强模型鲁棒性，使用混淆矩阵评估性能，并通过SHAP分析提高模型可解释性。

Result: InceptionV3模型取得了98.15%的准确率，ResNet50模型达到94.46%准确率。数据混合技术有效增强了模型鲁棒性，SHAP分析帮助识别了影响模型预测的关键图像区域。

Conclusion: 研究表明深度学习模型能有效分类番石榴病害，InceptionV3表现最佳。数据增强和混合技术提升了模型性能，SHAP分析增强了模型可解释性，为早期病害识别提供了有效解决方案。
...
TL;DR: 本文介绍了DFIC数据集，这是一个包含约58,000张标注图像和2706个视频的全面面部图像数据集，用于自动验证ICAO合规性，相比现有方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前手动检查方法在验证机器可读旅行证件(MRTDs)中面部图像是否符合ISO/IEC和ICAO标准时效率低下，特别是在高需求环境中。需要更高效的自动化验证方法。

Method: 创建了DFIC数据集，包含约58,000张标注图像和2706个视频，涵盖1000多名受试者，包含合规和非合规条件。基于该数据集，微调了一种主要依赖空间注意力机制的新方法，用于自动验证ICAO合规要求。

Result: 提出的方法在ICAO合规验证方面相比现有最先进方法表现出改进的结果。DFIC数据集具有比现有公共数据集更平衡的人口统计分布，其中一个分区接近均匀分布。

Conclusion: DFIC数据集不仅能够增强自动化ICAO合规方法，还可用于改善面部识别系统的安全性、隐私性和公平性等多种应用。数据集已公开供模型训练和验证使用。
...
TL;DR: 本文提出将SVD-Inspired Attention机制应用于Vision Transformers，通过几何基础增强注意力机制的可解释性、稀疏性和谱结构，在多个基准测试中保持分类精度的同时获得更可解释的注意力模式。
...
Method: 将先前提出的SVD-Inspired Attention机制适配到ViT架构中，引入几何基础公式化，并使用可解释性指标监控训练过程中的注意力动态，评估学习表示的结构特性。
...
### [53] [Enhancing Predictability of Multi-Tenant DNN Inference for Autonomous Vehicles' Perception](https://arxiv.org/abs/2602.11004)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要实时进行DNN推理以实现环境感知，但由于计算需求与车辆有限资源之间存在巨大差距，实现实时推理具有挑战性。现有研究主要关注通过模型压缩优化推理时间，但本文提出了一种不同的方法。

Method: PP-DNN系统包含四个核心组件：1) ROI生成器基于连续帧相似性和交通场景识别关键帧和感兴趣区域；2) FLOPs预测器从动态关键帧和ROI预测MAC操作；3) ROI调度器协调多个DNN模型处理关键帧和ROI；4) 检测预测器用于非关键帧的感知。系统在ROS-based AV管道中实现。

Result: 在BDD100K和nuScenes数据集上的评估显示，PP-DNN显著提升了感知可预测性：融合帧数量增加高达7.3倍，融合延迟减少>2.6倍，融合延迟变化减少>2.3倍，检测完整性提高75.4%，成本效益提升高达98%。

Conclusion: PP-DNN通过动态选择关键帧和感兴趣区域，有效减少了自动驾驶车辆感知管道中需要处理的图像数据量，同时保持了多租户DNN的精度，显著提高了感知可预测性和系统效率。
...
TL;DR: 该论文提出了一种名为SVD-Inspired Attention (SVDA)的新型注意力机制，将其集成到Dense Prediction Transformer (DPT)中，为单目深度估计任务提供首个谱结构化的注意力公式，在保持预测精度的同时实现了内在可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前Transformer架构中的自注意力机制在单目深度估计等密集预测任务中仍然不透明，缺乏可解释性。研究者希望开发一种能够提供结构化、可量化分析的注意力机制，而不是依赖事后近似解释。

Method: 提出SVD-Inspired Attention (SVDA)，通过在学习到的对角矩阵中嵌入归一化的查询-键交互，将方向对齐与谱调制解耦。这种设计使注意力图具有内在可解释性，而非事后近似。SVDA在DPT架构中实现，并引入了六个谱指标来量化注意力特性。

Result: 在KITTI和NYU-v2数据集上的实验表明，SVDA在保持或略微提高预测精度的同时，仅增加了轻微的计算开销。更重要的是，SVDA解锁了六个谱指标，揭示了注意力在训练过程中组织的一致跨数据集和深度模式，这些洞察在标准Transformer中无法获得。

Conclusion: SVDA将注意力的角色从黑盒机制转变为可量化的描述符，重新定义了单目深度估计中的可解释性，并为透明密集预测模型开辟了原则性途径。该方法不仅保持了性能，更重要的是提供了对注意力机制运作方式的深入理解。
...
<details>
  <summary>Details</summary>
Motivation: 手术室中准确计数手术器械对患者安全至关重要，但在器械密集堆积的复杂场景中，传统目标检测方法难以准确计数，需要更有效的解决方案。

Method: 提出Chain-of-Look视觉推理框架，模拟人类顺序计数过程，通过结构化视觉链引导模型沿空间轨迹计数；引入相邻损失函数建模密集器械的空间约束；创建SurgCount-HD数据集包含1,464张高密度手术器械图像。

Result: 在密集手术器械计数任务中，该方法优于最先进的计数方法（如CountGD、REC）以及多模态大语言模型（如Qwen、ChatGPT）。

Conclusion: Chain-of-Look框架通过模拟人类顺序计数过程和引入空间约束，显著提升了密集手术器械计数的准确性，为手术室安全提供了有效解决方案。
...
TL;DR: PuriLight是一个轻量级自监督单目深度估计框架，通过三个新模块在保持计算效率的同时提升结构精度。


<details>
  <summary>Details</summary>
Motivation: 当前自监督深度估计方法面临双重挑战：要么架构庞大影响实用性，要么轻量模型牺牲结构精度，需要开发既轻量又精确的架构。

Method: 采用三阶段架构，包含三个新模块：Shuffle-Dilation卷积模块用于局部特征提取，Rotation-Adaptive核注意力模块用于分层特征增强，Deep Frequency信号净化模块用于全局特征净化。

Result: PuriLight在保持最小训练参数的同时实现了最先进的性能，并保持了出色的计算效率。

Conclusion: PuriLight框架通过创新的模块设计，成功解决了自监督深度估计中轻量化和精度保持的双重挑战。
...
TL;DR: ViLaVT提出"用图像聊天"新框架，通过语言引导的特征调制实现视觉推理，在复杂多图像和视频空间推理任务上表现优异


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型依赖单次视觉编码的文本推理，导致细粒度视觉信息丢失；现有"用图像思考"方法生成的视觉状态与语言语义对齐不足，特别是在需要跨远距离区域或多图像进行视觉语义或几何关系推理时

Method: 提出"用图像聊天"框架，将视觉操作重构为语言引导的特征调制；开发ViLaVT模型，配备动态视觉编码器进行交互式视觉推理；采用两阶段课程训练：监督微调+强化学习

Result: 在八个基准测试中表现优异，在复杂多图像和视频空间推理任务上获得显著提升

Conclusion: ViLaVT通过语言引导的视觉特征调制实现了更好的跨模态对齐，为交互式视觉推理提供了新范式
...
TL;DR: 该论文介绍了基于足底压力模式的生物识别技术，通过UNB StepUP-P150数据集和首届国际StepUP竞赛，展示了当前最佳识别性能为10.77%的等错误率，但面对新鞋类等变化时仍存在泛化挑战。


<details>
  <summary>Details</summary>
Motivation: 生物足迹识别技术发展受限，主要原因是缺乏大规模、多样化的数据集来应对关键挑战，如对新用户的泛化能力以及对鞋类、步行速度等因素变化的鲁棒性。

Method: 利用新发布的UNB StepUP-P150数据集（迄今为止最大、最全面的高分辨率足底压力记录集合），通过举办首届国际StepUP竞赛，邀请全球团队开发鲁棒的识别模型，并在专门设计的测试集上评估验证性能。

Result: 竞赛吸引了来自学术界和工业界的23个团队参与，最佳团队Saeid_UCC使用生成奖励机器（GRM）优化策略，取得了10.77%的最佳等错误率（EER）。

Conclusion: 竞赛展示了强大的解决方案，但对不熟悉鞋类的泛化能力仍存在持续挑战，这是未来工作的关键领域。UNB StepUP-P150数据集的发布为通过深度学习解决这些挑战提供了新机会。
...
Method: 提出PhyCritic模型，采用两阶段RLVR流程：1）物理技能预热阶段，增强物理导向的感知和推理能力；2）自参考评判微调阶段，评判模型在评判候选回答前先生成自己的预测作为内部参考，从而提高判断稳定性和物理正确性。

Result: 在物理和通用多模态评判基准上，PhyCritic相比开源基线实现了显著的性能提升。当作为策略模型应用时，PhyCritic进一步提升了物理基础任务中的感知和推理能力。

Conclusion: PhyCritic通过专门针对物理AI优化的两阶段训练流程，有效解决了现有评判模型在物理任务上的不足，为物理AI的评估和偏好对齐提供了可靠的评判工具。
...
TL;DR: SurfPhase是一种从稀疏相机视角重建三维界面动力学的新方法，专门针对两相流中的尖锐可变形液-汽界面，结合动态高斯面元和符号距离函数，并利用视频扩散模型进行新视角合成。
...
Method: 提出SurfPhase模型，集成动态高斯面元与符号距离函数以保证几何一致性，并利用视频扩散模型合成新视角视频以优化稀疏观测的重建。

Result: 在高速度池沸腾视频新数据集上评估，仅用两个相机视角即可实现高质量视角合成和速度估计。

Conclusion: SurfPhase能够从稀疏相机视角有效重建三维界面动力学，为两相流界面测量提供了新方法。
...
### [61] [Compute Only Once: UG-Separation for Efficient Large Recommendation Models](https://arxiv.org/abs/2602.10455)
...
TL;DR: UG-Sep框架首次在密集交互模型中实现可复用的用户侧计算，通过掩码机制分离用户和物品信息流，结合信息补偿策略和量化技术，显著降低推理延迟而不影响推荐效果。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统模型规模扩大，训练和推理成本急剧增加。虽然长序列模型可以通过KV缓存复用用户侧计算，但在密集特征交互架构中，用户和物品特征在层间深度纠缠，难以实现类似的复用机制。

Method: 提出用户-物品分离框架，在token混合层引入掩码机制，显式分离用户侧和物品侧信息流；设计信息补偿策略自适应重建被抑制的用户-物品交互；结合W8A16权重量化缓解内存带宽瓶颈。

Result: 在字节跳动多个业务场景的离线和在线实验中，UG-Sep将推理延迟降低高达20%，同时保持在线用户体验和商业指标不变，包括信息流推荐和广告系统。

Conclusion: UG-Sep首次在密集交互模型中实现了可复用的用户侧计算，通过创新的分离框架和补偿策略，在保持推荐质量的同时显著降低了推理成本，为大规模推荐系统的效率优化提供了新思路。
...
### [62] [S-GRec: Personalized Semantic-Aware Generative Recommendation with Asymmetric Advantage](https://arxiv.org/abs/2602.10606)
*Jie Jiang,Hongbo Tang,Wenjie Wu,Yangru Huang,Zhenmao Li,Qian Li,Changping Wang,Jun Zhang,Huan Yu*

Main category: cs.IR

TL;DR: S-GRec框架通过解耦在线轻量生成器和离线LLM语义评判器，在训练时提供语义监督，同时使用A2PO确保语义优势与业务目标一致，实现了无需实时LLM推理的高效推荐系统。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐模型从行为日志训练时对用户意图的监督较弱，而LLM虽然能提供丰富的语义先验，但直接应用于工业推荐面临两个障碍：语义信号可能与平台业务目标冲突，且LLM推理在大规模场景下成本过高。

Method: 提出S-GRec框架：1）两阶段个性化语义评判器（PSJ）离线生成可解释的方面证据并从成对反馈中学习用户条件聚合，产生稳定的语义奖励；2）非对称优势策略优化（A2PO）以业务奖励（如eCPM）为优化锚点，仅在语义优势与业务目标一致时注入语义优势。

Result: 在公开基准和大规模生产系统上的实验验证了有效性和可扩展性：包括CTR的统计显著提升，在线A/B测试中GMV提升1.19%，且无需实时LLM推理。

Conclusion: S-GRec成功解决了LLM在工业推荐中应用的两个主要障碍，通过解耦架构和策略优化实现了语义监督与业务目标的一致性，为大规模推荐系统提供了高效实用的解决方案。
...
### [63] [A Cognitive Distribution and Behavior-Consistent Framework for Black-Box Attacks on Recommender Systems](https://arxiv.org/abs/2602.10633)
*Hongyue Zhan,Mingming Li,Dongqin Liu,Hui Wang,Yaning Zhang,Xi Zhou,Honglei Lv,Jiao Dai,Jizhong Han*

Main category: cs.IR

TL;DR: 该论文提出了一种双增强攻击框架，通过认知分布驱动的提取机制和行为感知的噪声项目生成策略，显著提升了序列推荐系统的攻击成功率和规避率。


<details>
  <summary>Details</summary>
Motivation: 随着序列推荐系统在电商等领域的广泛应用，其黑盒接口存在安全风险：模型容易受到提取攻击和后续对抗性操纵。现有黑盒提取攻击主要依赖硬标签或成对学习，往往忽略排名位置的重要性，导致知识转移不完整。此外，纯梯度方法生成的对抗序列缺乏与真实用户行为的语义一致性，容易被检测。

Method: 1. 引入认知分布驱动的提取机制：基于首因效应和位置偏差，将离散排名映射为具有位置感知衰减的连续值分布，实现从顺序对齐到认知分布对齐的转变。2. 设计行为感知的噪声项目生成策略：联合优化协同信号和梯度信号，确保语义一致性和统计隐蔽性，同时有效提升目标项目排名。

Result: 在多个数据集上的广泛实验表明，该方法在攻击成功率和规避率方面显著优于现有方法，验证了将认知建模和行为一致性整合到安全推荐系统中的价值。

Conclusion: 该研究通过结合认知分布对齐和行为感知生成，提出了一种更有效的序列推荐系统攻击框架，为构建更安全的推荐系统提供了重要见解。
...
### [64] [Training-Induced Bias Toward LLM-Generated Content in Dense Retrieval](https://arxiv.org/abs/2602.10833)
*William Xion,Wolfgang Nejdl*

Main category: cs.IR

TL;DR: 研究发现密集检索器的"源偏见"（偏好LLM生成文本）主要是训练诱导现象而非固有属性，监督微调特别是使用MS MARCO数据集会强化这种偏见。


<details>
  <summary>Details</summary>
Motivation: 针对近期研究中发现的密集检索器对大型语言模型生成文本的偏好（源偏见），以及认为低困惑度导致这种偏见的假设，本研究旨在通过受控实验重新审视这一主张，追踪偏好在不同训练阶段和数据源中的出现情况。

Method: 使用SciFact和Natural Questions数据集的并行人类生成和LLM生成文本对比，比较无监督检查点与使用领域内人类文本、领域内LLM生成文本和MS MARCO微调的模型。通过重新连接语言建模头到微调后的密集检索器编码器进行困惑度探测。

Result: 1) 无监督检索器没有统一的亲LLM偏好，方向和幅度取决于数据集；2) MS MARCO监督微调始终使排名偏向LLM生成文本；3) 领域内微调产生数据集特定且不一致的偏好变化；4) LLM生成语料微调诱导明显的亲LLM偏见；5) 困惑度探测显示相关性接近随机，削弱了困惑度的解释力。

Conclusion: 源偏见是训练诱导现象而非密集检索器的固有属性，监督微调特别是使用MS MARCO数据集会强化这种偏见，而困惑度不能很好地解释这种偏见现象。
...
### [65] [End-to-End Semantic ID Generation for Generative Advertisement Recommendation](https://arxiv.org/abs/2602.10445)
...
TL;DR: UniSID是一个统一的语义ID生成框架，通过端到端优化解决生成式推荐中两阶段压缩的语义退化问题，在广告推荐场景中显著提升命中率。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐使用残差量化生成语义ID存在两个主要问题：1）两阶段压缩导致目标不对齐和语义退化；2）残差量化结构固有的误差累积。这些限制了推荐系统的性能。

Method: 提出UniSID框架：1）端到端联合优化嵌入和语义ID，让语义信息直接流入语义ID空间；2）引入多粒度对比学习策略对齐不同语义ID层级的项目；3）提出基于摘要的广告重建机制，鼓励语义ID捕获广告上下文中未明确存在的高层语义信息。

Result: 实验表明UniSID在广告推荐场景中持续优于最先进的语义ID生成方法，相比最强基线在命中率指标上提升高达4.62%。

Conclusion: UniSID通过统一的端到端语义ID生成框架，有效解决了现有两阶段压缩方法的局限性，显著提升了生成式广告推荐的性能。
...
### [66] [A Human-Centric Framework for Data Attribution in Large Language Models](https://arxiv.org/abs/2602.10995)
*Amelie Wührl,Mattes Ruckdeschel,Kyle Lo,Anna Rogers*

Main category: cs.CY

TL;DR: 论文提出一个以人为中心的数据归属框架，将LLM文本归属问题置于更广泛的数据经济中，通过参数化用例和利益相关者协商来解决归属问题


<details>
  <summary>Details</summary>
Motivation: 当前LLM生态系统中，创作者对其数据使用缺乏控制权，用户可能无意中抄袭现有来源。LLM生成文本的归属问题尚未明确解决，需要系统性的框架来平衡各方利益

Method: 提出人类中心的数据归属框架，通过参数化具体用例（如创意写作辅助、事实核查），包括利益相关者目标和实施标准等参数，由创作者、LLM用户和中介机构协商确定标准

Result: 该框架为NLP方法学、治理政策干预和经济激励分析之间搭建桥梁，支持领域特定协商结果的实施和测试，以评估是否达成利益相关者目标

Conclusion: 提出的方法为数据归属问题提供了系统解决方案，有助于在数据经济中实现可持续平衡，促进创作者激励和负责任的数据使用
...
### [67] [A Practical Guide to Agentic AI Transition in Organizations](https://arxiv.org/abs/2602.10122)
...
TL;DR: 本文提出了一个实用框架，帮助组织从手动流程过渡到自动化智能体AI系统，强调领域驱动用例识别、任务系统化委托给AI智能体、AI辅助构建智能体工作流，以及小型AI增强团队与业务利益相关者紧密合作。


<details>
  <summary>Details</summary>
Motivation: 尽管许多组织已采用AI提高生产力，但大多数实施仍局限于孤立用例和以人为中心的工具驱动工作流。组织在超越实验阶段、扩展智能体系统并将其与切实业务价值对齐方面面临挑战，缺乏有效操作化智能体AI的明确指导。

Method: 基于在多个组织和业务领域设计和部署智能体AI工作流的实践经验，提出一个实用框架，包括：领域驱动用例识别、任务系统化委托给AI智能体、AI辅助构建智能体工作流，以及小型AI增强团队与业务利益相关者紧密合作。核心是人在循环操作模型，个人作为多个AI智能体的协调者。

Result: 该框架使组织能够实现从手动流程到自动化智能体AI系统的过渡，实现可扩展的自动化，同时保持监督、适应性和组织控制。

Conclusion: 智能体AI代表组织内智能应用的重要转变，通过提出的实用框架，组织可以克服传统软件工程实践的过度依赖、业务领域知识有限整合、AI驱动工作流所有权不明确以及可持续人机协作模型缺失等关键挑战，有效操作化智能体AI系统。
...
### [68] [AI-PACE: A Framework for Integrating AI into Medical Education](https://arxiv.org/abs/2602.10527)
*Scott P. McGrath,Katherine K. Kim,Karnjit Johl,Haibo Wang,Nick Anderson*

Main category: cs.CY

TL;DR: 本文通过文献综合分析，探讨了医学教育中人工智能整合的现状，提出了AI医学教育的核心能力、课程方法和实施策略框架，强调在医学培训中纵向整合AI教育的重要性。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的应用正在加速发展，但医学教育未能跟上这些技术进步的步伐。本文旨在通过综合分析现有文献，识别医学教育中AI整合的关键要素，为医学教育者提供框架，以培养未来医生适应AI增强的医疗环境。

Method: 采用文献综合分析方法，对现有关于AI在医学教育中的研究进行系统梳理，识别关键能力、课程方法和实施策略。

Result: 研究发现有效的AI医学教育需要在医学培训中纵向整合，促进跨学科合作，并平衡技术基础和临床应用。提出了一个课程开发框架，为医学教育者提供指导。

Conclusion: 本文为医学教育者提供了一个基础框架，帮助他们在整个医学学习连续体中构建结构化的AI教育，以培养未来医生在AI增强的医疗环境中有效工作的能力。
...
### [69] [Traceable, Enforceable, and Compensable Participation: A Participation Ledger for People-Centered AI Governance](https://arxiv.org/abs/2602.10916)
*Rashid Mushkani*

Main category: cs.CY

TL;DR: 提出了"参与账本"框架，将AI治理中的参与转化为可追踪的影响、可执行的权力和可补偿的劳动，通过机器可读的账本记录参与活动与系统更新的关联。


<details>
  <summary>Details</summary>
Motivation: 当前AI治理中的参与方法往往流于形式，社区贡献（如审议、标注、提示、事件报告）通常记录不规范，与系统更新关联弱，缺乏可执行的权利或持续补偿，导致参与更多是象征性的而非可问责的。

Method: 引入参与账本框架，包含三个核心要素：1) 参与证据标准，记录同意、隐私、补偿和重用条款；2) 影响追踪机制，将系统更新与可重放的"前后"测试连接；3) 编码的权利和激励机制，包括能力凭证和参与积分。

Result: 在四个城市AI和公共空间治理部署中验证了该框架，提供了机器可读的模式、模板和评估计划，用于在实践中评估可追踪性、可执行性和补偿机制。

Conclusion: 参与账本框架能够将AI治理中的参与转化为可追踪、可执行和可补偿的实际影响，解决了当前参与方法中存在的问责缺失问题，为社区参与提供了实质性的机制保障。
...
### [70] [LiveMedBench: A Contamination-Free Medical Benchmark for LLMs with Automated Rubric Evaluation](https://arxiv.org/abs/2602.10367)
*Zhiling Yan,Dingjie Song,Zhe Fang,Yisheng Ji,Xiang Li,Quanzheng Li,Lichao Sun*

Main category: cs.AI

TL;DR: LiveMedBench是一个持续更新的医疗基准测试，解决现有医疗评估中的数据污染和时间错位问题，通过每周收集真实临床案例并采用基于规则的自动化评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有医疗基准测试存在两个关键问题：1) 数据污染（测试集泄露到训练数据中导致性能虚高）；2) 时间错位（无法捕捉医学知识的快速演进）。同时，现有评估指标要么依赖浅层词汇重叠，要么依赖主观的LLM评分，都不足以验证临床正确性。

Method: 提出LiveMedBench基准测试，每周从在线医疗社区收集真实临床案例，确保与模型训练数据的时间分离。采用多智能体临床筛选框架过滤原始数据噪声，并基于循证医学原则验证临床完整性。开发自动化基于规则的评估框架，将医生回答分解为细粒度的病例特定标准。

Result: LiveMedBench包含2,756个真实病例，涵盖38个医学专科和多种语言，配有16,702个独特评估标准。对38个LLM的评估显示，最佳模型仅达到39.2%准确率，84%的模型在截止日期后的病例上表现下降，证实普遍存在数据污染风险。错误分析表明主要瓶颈是上下文应用能力而非事实知识，35-48%的失败源于无法将医学知识适配到患者特定约束。

Conclusion: LiveMedBench提供了一个持续更新、无数据污染、基于规则的医疗基准测试，能够更准确地评估LLM在临床环境中的真实能力，揭示了数据污染的普遍风险和上下文应用能力的重要性。
...
### [71] [MERIT Feedback Elicits Better Bargaining in LLM Negotiators](https://arxiv.org/abs/2602.10467)
*Jihwan Oh,Murad Aghazada,Yooju Shin,Se-Young Yun,Taehyeon Kim*

Main category: cs.AI

TL;DR: 论文提出了一个基于效用反馈的框架来提升LLM在讨价还价任务中的表现，包括新的基准测试、经济学指标和人类偏好数据集。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在讨价还价任务中表现不佳，缺乏战略深度和适应复杂人类因素的能力，现有基准测试未能充分捕捉这些局限性。

Method: 提出了一个效用反馈中心框架，包括：(1) AgoraBench基准测试，涵盖9个挑战性场景；(2) 基于效用理论的经济学指标；(3) 基于人类偏好的数据集和学习流程，通过提示和微调增强LLM讨价还价能力。

Result: 基线LLM策略常偏离人类偏好，而提出的机制显著提升了谈判性能，产生了更深层次的战略行为和更强的对手意识。

Conclusion: 该框架有效解决了LLM在讨价还价任务中的局限性，通过结合经济学理论和人类偏好数据，显著提升了模型的战略深度和适应性。
...
### [72] [Abstraction Generation for Generalized Planning with Pretrained Large Language Models](https://arxiv.org/abs/2602.10485)
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能够作为QNP抽象生成器来解决广义规划问题，以及如何通过自动调试来修复抽象错误

Method: 提出提示协议：输入GP领域和训练任务给LLMs，提示它们生成抽象特征并将初始状态、动作集和目标抽象为QNP问题；设计自动调试方法检测抽象错误并指导LLMs修复

Result: 实验表明，在自动调试的适当指导下，一些LLMs能够生成有用的QNP抽象

Conclusion: LLMs可以作为QNP抽象生成器，结合自动调试方法能够有效生成和修复抽象，为广义规划问题提供解决方案
...
### [73] [To Think or Not To Think, That is The Question for Large Reasoning Models in Theory of Mind Tasks](https://arxiv.org/abs/2602.10625)
...
Result: 推理模型在心理理论任务上并不一致优于非推理模型，有时表现更差。研究发现三个关键问题：慢思考崩溃现象、适度和自适应推理有益、选项匹配捷径依赖。干预方法验证了这些问题并提供了缓解方案。
...
### [74] [OmniSapiens: A Foundation Model for Social Behavior Processing via Heterogeneity-Aware Relative Policy Optimization](https://arxiv.org/abs/2602.10635)
...
TL;DR: HARPO方法通过调节优势函数平衡异构任务和样本的学习，开发了Omnisapiens-7B 2.0社交行为基础模型，在多项任务上表现最优


<details>
  <summary>Details</summary>
Motivation: 现有方法通常孤立地建模人类行为维度（情感、认知或社会属性），任务特定建模增加了训练成本并限制了跨行为设置的泛化能力。虽然最近的推理RL方法支持跨多个行为任务训练统一模型，但没有明确解决跨异构行为数据的学习问题。

Method: 提出异构感知相对策略优化（HARPO）方法，通过调节优势函数确保在策略优化过程中没有单个任务或样本产生不成比例的影响，从而平衡跨异构任务和样本的学习。

Result: 使用HARPO开发了Omnisapiens-7B 2.0社交行为处理基础模型。相比现有行为基础模型，在多任务和保留设置上分别获得+16.85%和+9.37%的性能提升，同时产生更明确和鲁棒的推理轨迹。HARPO相比其他RL方法在行为任务上表现最一致。
...
### [75] [Spend Search Where It Pays: Value-Guided Structured Sampling and Optimization for Generative Recommendation](https://arxiv.org/abs/2602.10699)
...
<details>
  <summary>Details</summary>
Motivation: 生成式推荐中的强化学习存在概率-奖励不匹配问题：传统基于似然的解码偏向局部高概率前缀，导致探索不足（高奖励但低概率项被过早剪枝）和优势压缩（共享高概率前缀的轨迹奖励高度相关，比较信号弱）。

Method: 提出V-STAR框架，包含两个协同组件：1）价值引导高效解码（VED），识别关键节点并选择性深化高潜力前缀，提升探索效率；2）Sibling-GRPO，利用诱导的树状拓扑计算兄弟相对优势，将学习信号集中在关键分支决策上。
...
### [76] [Integrating Generative AI-enhanced Cognitive Systems in Higher Education: From Stakeholder Perceptions to a Conceptual Framework considering the EU AI Act](https://arxiv.org/abs/2602.10802)
*Da-Lun Chen,Prasasthy Balasubramanian,Lauri Lovén,Susanna Pirttikangas,Jaakko Sauvola,Panagiotis Kostakos*

Main category: cs.AI

TL;DR: 该研究调查了高等教育中生成式AI的接受度，特别关注ITEE领域，通过混合方法揭示了共享和学科特定的主题，提出了负责任整合的概念框架。


<details>
  <summary>Details</summary>
Motivation: 尽管高等教育中的许多师生已采用生成式AI工具，但利益相关者对GenAI的看法仍存在分歧，且受文化、学科和制度背景影响。欧盟AI法案要求大学确保认知系统的合规性，因此需要了解利益相关者需求并制定负责任的整合策略。

Method: 采用混合方法研究，对奥卢大学ITEE学院的61名教职员工和37名学生进行了调查，分析他们对GenAI的感知和态度。

Result: 研究揭示了共享和学科特定的主题：对GenAI编程支持的强烈兴趣，以及对响应质量、隐私和学术诚信的担忧。基于这些发现，研究确定了一套高级要求并提出了负责任GenAI整合的概念框架。

Conclusion: 学科特定要求强调了利益相关者参与的重要性。高级要求和框架为大学利用GenAI同时解决利益相关者关切和确保监管合规提供了实用指导。
...
### [77] [SynergyKGC: Reconciling Topological Heterogeneity in Knowledge Graph Completion via Topology-Aware Synergy](https://arxiv.org/abs/2602.10845)
*Xuecheng Zou,Yu Tang,Bingbing Wang*

Main category: cs.AI

TL;DR: SynergyKGC通过跨模态协同专家和密度依赖的身份锚定策略，解决了知识图谱补全中的结构分辨率不匹配问题，显著提升了命中率。
...
Method: 提出SynergyKGC框架：1) 将传统邻居聚合提升为主动的跨模态协同专家，通过关系感知的交叉注意力和语义意图驱动的门控机制；2) 结合密度依赖的身份锚定策略和双塔一致性架构，协调拓扑异质性并确保表示稳定性。

Result: 在两个公共基准测试上的系统评估验证了方法的优越性，显著提升了KGC命中率，为非均匀结构化数据中的弹性信息集成提供了经验证据。

Conclusion: SynergyKGC通过自适应框架有效解决了知识图谱补全中的结构分辨率不匹配问题，为异构拓扑结构中的稳健关系推理提供了有效解决方案。
...
### [78] [Reinforcing Chain-of-Thought Reasoning with Self-Evolving Rubrics](https://arxiv.org/abs/2602.10885)
*Leheng Sheng,Wenchang Ma,Ruixin Hong,Xiang Wang,An Zhang,Tat-Seng Chua*

Main category: cs.AI

TL;DR: RLCER：一种无需人工标注的自主CoT奖励方法，通过自提出和自演进的评分标准来监督思维链推理


<details>
  <summary>Details</summary>
Motivation: 传统思维链奖励方法面临人工标注成本高、静态奖励模型难以适应CoT分布变化和奖励攻击的问题，需要一种无需人工标注且能自主演进的CoT奖励方法

Method: 提出RLCER方法，通过自提出和自演进的评分标准来监督思维链，增强以结果为中心的RLVR方法，即使没有结果奖励也能提供可靠的CoT监督信号

Result: RLCER在无结果奖励的情况下仍能提供可靠的CoT监督信号，性能优于结果中心的RLVR方法，且自提出的评分标准作为提示提示能进一步提升推理时性能

Conclusion: RLCER提供了一种有效的自主CoT奖励方法，无需人工标注且能适应演化，为LLM推理提供了新的监督范式
...
### [79] [Can LLMs Cook Jamaican Couscous? A Study of Cultural Novelty in Recipe Generation](https://arxiv.org/abs/2602.10964)
*F. Carichon,R. Rampa,G. Farnadi*

Main category: cs.AI

TL;DR: 研究发现当前大语言模型在文化适应方面存在根本性限制，无法产生真正具有文化代表性的内容，其生成的食谱与文化距离无关，与人类行为形成鲜明对比。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地用于生成和塑造文化内容，但研究表明它们存在系统性文化偏见，可能导致刻板印象、同质化和文化特定表达形式的消失。理解LLM是否能够超越主导文化而真正与多元文化对齐是一个关键挑战。

Method: 通过烹饪食谱这一文化、传统和创造力紧密结合的领域研究LLM的文化适应能力。使用GlobalFusion数据集，该数据集根据文化距离测量将不同国家的人类食谱配对。使用相同的国家配对，用多个LLM生成文化适应的食谱，从而直接比较人类和LLM在跨文化内容创作中的行为。

Result: LLM无法产生具有文化代表性的适应内容。与人类不同，它们生成的食谱差异与文化距离无关。文化信息在模型内部表示中保存较弱，模型通过误解创造性和传统等概念来夸大新颖性，无法将适应内容与相关国家联系起来，也无法将其建立在文化显著元素（如食材）上。

Conclusion: 当前LLM在文化导向的生成方面存在根本性限制，这对它们在文化敏感应用中的使用具有重要影响。模型无法真正理解和适应多元文化，需要更深入的文化理解和表示能力。
...
### [80] [GameDevBench: Evaluating Agentic Capabilities Through Game Development](https://arxiv.org/abs/2602.11103)
...
TL;DR: GameDevBench是首个针对游戏开发任务的智能体评估基准，包含132个从教程中提取的任务，需要大量多模态理解和复杂代码操作，当前最佳智能体仅能解决54.5%的任务。


<details>
  <summary>Details</summary>
Motivation: 当前编码智能体发展迅速，但其多模态对应物进展滞后。主要挑战是缺乏结合软件开发复杂性和深度多模态理解的评估测试平台。游戏开发提供了这样的测试平台，因为智能体必须导航大型密集代码库，同时在视觉游戏场景中操作着色器、精灵、动画等多模态资产。

Method: 1. 创建GameDevBench基准，包含132个从网页和视频教程中提取的任务；2. 任务需要大量多模态理解和复杂代码操作（平均解决方案代码行数和文件修改量是先前软件基准的3倍以上）；3. 引入两种简单的基于图像和视频的反馈机制来提升智能体的多模态能力。

Result: 1. 智能体在游戏开发任务上表现不佳，最佳智能体仅解决54.5%的任务；2. 感知任务难度与多模态复杂度强相关，成功率从游戏玩法导向任务的46.9%下降到2D图形任务的31.6%；3. 简单的图像和视频反馈机制能持续提升性能，Claude Sonnet 4.5的性能从33.3%提升到47.7%。

Conclusion: 游戏开发是多模态智能体评估的重要测试平台，GameDevBench基准揭示了当前智能体在多模态游戏开发任务上的局限性，简单的多模态反馈机制能显著提升性能，该基准的公开发布将支持智能体游戏开发的进一步研究。
...
### [81] [Found-RL: foundation model-enhanced reinforcement learning for autonomous driving](https://arxiv.org/abs/2602.10458)
*Yansong Qu,Zihao Sheng,Zilin Huang,Jiancong Chen,Yuhao Luo,Tianyi Wang,Yiheng Feng,Samuel Labi,Sikai Chen*

Main category: cs.AI

TL;DR: Found-RL平台通过异步批处理推理框架解决VLM在自动驾驶RL训练中的延迟问题，使用VMR和AWAG机制将VLM专家建议蒸馏到RL策略中，并采用CLIP进行密集奖励塑造，使轻量RL模型达到接近大参数VLM的性能。
...
Method: 1. 异步批处理推理框架解耦VLM推理与仿真循环；2. 引入价值边际正则化和优势加权动作指导机制蒸馏VLM专家建议；3. 采用高吞吐量CLIP进行密集奖励塑造；4. 通过条件对比动作对齐解决CLIP的动态盲区问题。

Result: Found-RL平台使轻量级RL模型能够达到接近数十亿参数VLM的性能，同时保持实时推理（约500 FPS），解决了VLM在RL训练中的延迟瓶颈。

Conclusion: Found-RL为自动驾驶中的强化学习提供了高效集成基础模型的端到端平台，通过创新的异步推理和监督机制，在保持实时性能的同时显著提升了RL的样本效率和语义可解释性。
...
### [82] [Large Language Models Predict Functional Outcomes after Acute Ischemic Stroke](https://arxiv.org/abs/2602.10119)
...
TL;DR: 研究评估了大型语言模型直接从入院记录预测卒中后功能结局的能力，发现微调后的Llama模型表现最佳，性能与需要结构化变量的传统模型相当。


<details>
  <summary>Details</summary>
Motivation: 准确预测急性缺血性卒中后的功能结局对临床决策和资源分配很重要。以往研究主要依赖结构化变量和传统机器学习，而大型语言模型直接从常规入院记录推断未来mRS评分的能力尚未充分探索。

Method: 使用编码器模型（BERT、NYUTron）和生成式模型（Llama-3.1-8B、MedGemma-4B），在冻结和微调两种设置下，基于大型真实世界卒中登记数据预测出院和90天mRS评分。数据集包括9,485份入院记录和1,898份90天随访记录，按时间分割，最近12个月作为测试集。

Result: 微调后的Llama模型表现最佳：90天mRS预测准确率33.9%（95% CI 27.9-39.9%），二元功能结局准确率76.3%（95% CI 70.7-81.9%）；出院预测准确率42.0%（95% CI 39.0-45.0%），二元准确率75.0%（95% CI 72.4-77.6%）。对于90天预测，Llama与基于结构化变量的基线模型表现相当。

Conclusion: 微调的大型语言模型能够仅从入院记录预测卒中后功能结局，性能与需要结构化变量提取的模型相当。这支持开发基于文本的预后工具，无需手动数据提取即可无缝集成到临床工作流程中。
...
### [83] [Bridging the Compression-Precision Paradox: A Hybrid Architecture for Clinical EEG Report Generation with Guaranteed Measurement Accuracy](https://arxiv.org/abs/2602.10544)
*Wuyang Zhang,Zhen Luo,Chuqiao Gu,Jianming Ma,Yebo Cao,Wangming Yuan,Yinzhi Jin*

Main category: cs.LG

TL;DR: 提出混合架构分离测量提取与文本生成，通过信号处理在压缩前计算精确临床值，实现首个保证临床测量精度的自动化EEG报告系统


<details>
  <summary>Details</summary>
Motivation: 自动化EEG监测需要临床级精度的癫痫检测和报告，但现有方法面临两个主要问题：1）临床EEG记录超过LLM上下文窗口，需要极端压缩（400:1+比例）破坏时间精度；2）LLM缺乏时间序列理解能力，依赖压缩表示的统计关联，导致产生临床错误的测量值

Method: 采用混合架构分离测量提取与文本生成：1）在压缩前通过信号处理计算精确临床值；2）使用跨模态桥接进行EEG到语言的翻译；3）采用参数高效微调，在冻结槽位周围进行约束解码；4）多速率采样保持长程上下文同时保留事件级精度

Result: 在TUH和CHB-MIT数据集上评估，实现了60%更少的误报、50%更快的检测速度，以及亚临床测量精度，是首个保证临床测量精度的自动化EEG报告系统

Conclusion: 通过分离测量提取与文本生成的混合架构，解决了自动化EEG报告中临床测量精度的问题，为临床级精度的癫痫检测和报告提供了可靠解决方案
...
### [84] [Towards Autonomous Mathematics Research](https://arxiv.org/abs/2602.10177)
*Tony Feng,Trieu H. Trinh,Garrett Bingham,Dawsen Hwang,Yuri Chervonyi,Junehyuk Jung,Joonkyung Lee,Carlo Pagano,Sang-hyun Kim,Federico Pasqualotto,Sergei Gukov,Jonathan N. Lee,Junsu Kim,Kaiying Hou,Golnaz Ghiasi,Yi Tay,YaGuang Li,Chenkai Kuang,Yuan Liu,Hanzhao,Lin,Evan Zheran Liu,Nigamaa Nayakanti,Xiaomeng Yang,Heng-tze Cheng,Demis Hassabis,Koray Kavukcuoglu,Quoc V. Le,Thang Luong*

Main category: cs.LG

TL;DR: Aletheia是一个数学研究智能体，能够从奥数问题到博士级练习再到实际数学研究，通过迭代生成、验证和修订解决方案，在AI辅助数学研究中取得了多项里程碑成果。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型在解决国际数学奥林匹克竞赛级别问题上已达到金牌标准，但从竞赛级问题解决过渡到专业研究需要处理大量文献和构建长视野证明，因此需要开发能够应对数学研究复杂性的AI系统。

Method: Aletheia采用迭代生成、验证和修订解决方案的端到端自然语言处理流程，由改进版Gemini Deep Think驱动，结合新颖的推理时间扩展定律和密集工具使用来应对数学研究的复杂性。

Result: 展示了Aletheia在三个重要里程碑上的能力：1）完全由AI生成的研究论文（Feng26）；2）人机协作的研究论文（LeeSeo26）；3）对700个开放问题的半自主评估，包括自主解决了四个开放问题。

Conclusion: 需要建立量化AI辅助结果自主性和新颖性的标准等级，以帮助公众理解AI与数学的发展，并反思人机协作在数学研究中的未来。
...
### [85] [Signature-Kernel Based Evaluation Metrics for Robust Probabilistic and Tail-Event Forecasting](https://arxiv.org/abs/2602.10182)
*Benjamin R. Redhead,Thomas L. Lee,Peng Gu,Víctor Elvira,Amos Storkey*

Main category: cs.LG

TL;DR: 提出了两种基于核的度量方法（Sig-MMD和CSig-MMD）来改进概率预测评估，解决现有方法忽略时间/变量间依赖关系和对尾部事件不敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 当前概率预测评估框架缺乏共识度量标准，存在两个关键缺陷：假设时间步或变量间独立，以及对尾部事件（现实决策中最关键的部分）缺乏敏感性。

Method: 提出了两种基于核的度量方法：签名最大均值差异（Sig-MMD）和新的截断Sig-MMD（CSig-MMD）。利用签名核捕获复杂的变量间和时间依赖关系，对缺失数据保持鲁棒性。CSig-MMD引入截断方案，优先考虑预测尾部事件的能力，同时严格保持适当性。

Result: 这些度量方法能够更可靠地评估直接多步预测，促进开发更鲁棒的概率算法。

Conclusion: 提出的Sig-MMD和CSig-MMD度量解决了当前概率预测评估框架的关键缺陷，通过捕获复杂依赖关系和优先考虑尾部事件预测能力，为概率预测提供了更可靠的评估工具。
...
### [86] [Adaptive Optimization via Momentum on Variance-Normalized Gradients](https://arxiv.org/abs/2602.10204)
*Francisco Patitucci,Aryan Mokhtari*

Main category: cs.LG

TL;DR: MVN-Grad是一种Adam风格的优化器，通过方差归一化和归一化后应用动量来提高稳定性和性能，在低方差场景下避免符号塌陷，在图像分类和语言建模任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 标准Adam类优化器存在过时动量与随机归一化器之间的跨时间耦合问题，这可能导致训练不稳定和性能下降。需要一种能够解耦动量和归一化、提高稳定性并改善泛化性能的优化方法。

Method: MVN-Grad结合了两个互补思想：1）基于方差的归一化，使用梯度不确定性的指数移动平均来缩放每个坐标；2）在归一化后的梯度上应用动量。这种方法消除了标准Adam更新中过时动量与随机归一化器之间的跨时间耦合。

Result: 理论证明：MVN-Grad在标准噪声假设下具有比先动量后归一化方法更小的一步条件更新方差；对异常值具有鲁棒性，对单个梯度尖峰具有一致有界响应；在低方差场景下避免符号塌陷并可能加速收敛。实验表明：在CIFAR-100图像分类和GPT风格语言建模基准测试中，MVN-Grad匹配或优于Adam、AdaBelief和LaProp，提供更平滑的训练和更好的泛化性能，且无额外开销。

Conclusion: MVN-Grad通过解耦动量和归一化操作，提供了一种更稳定、性能更好的优化方法，在理论和实验上都表现出优于现有Adam类优化器的特性，特别是在处理异常值和低方差场景时具有优势。
...
TL;DR: 提出了HybridRAG-Bench框架，用于评估大语言模型在混合知识（非结构化文本+结构化知识图谱）上的检索密集型多跳推理能力，通过自动生成基于arXiv最新科学文献的问答对，避免预训练数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在需要最新信息和多跳推理的知识密集型问题上仍存在困难，而现有基准测试常与模型预训练数据重叠，难以区分真正的检索推理能力和参数记忆。

Method: 开发了HybridRAG-Bench框架，从arXiv最新科学文献自动构建混合知识表示（非结构化文本+结构化知识图谱），生成基于显式推理路径的知识密集型问答对，支持灵活领域和时间范围选择。

Result: 在人工智能、治理与政策、生物信息学三个领域的实验表明，HybridRAG-Bench能够有效评估真正的检索和推理能力，而非参数记忆，为混合知识增强推理系统提供了原则性测试平台。

Conclusion: HybridRAG-Bench提供了一个可定制、避免数据污染的评估框架，能够准确衡量大语言模型在混合知识上的检索密集型多跳推理能力，推动了知识增强推理系统的可靠评估。
...
TL;DR: 本文从动力系统角度分析LoRA在微调任务中精度与更新秩的关系，建立了秩与精度的显式关系


<details>
  <summary>Details</summary>
Motivation: 虽然实证研究表明LoRA在低秩更新（甚至秩1）时能达到与全参数方法相当的精度，但其精度与更新秩关系的理论基础相对缺乏探索

Method: 从动力系统角度比较秩r的LoRA更新与全参数更新，在满秩和低秩两种机制下进行梯度流分析，推导LoRA梯度流方程并证明同时和顺序参数更新的等价性

Result: 建立了LoRA秩与精度之间的闭式关系，针对迹平方和Frobenius范数低秩逼近损失函数得到了明确的数学表达式

Conclusion: 通过严格的动力系统分析，为LoRA在微调任务中精度与更新秩的关系提供了理论基础，揭示了低秩更新的有效性机制
...
TL;DR: 提出一种在扩散模型输入嵌入空间中解耦语义方向的方法，通过分析固定提示下随机生成的梯度变化，实现细粒度概念控制并恢复蒸馏模型的多样性


<details>
  <summary>Details</summary>
Motivation: 扩散模型对同一文本提示会产生多种随机视觉输出，用户无法直接控制具体的语义变化。现有无监督方法仅分析输出特征而忽略了生成过程本身

Method: 收集固定提示下随机实现之间的梯度差异，通过主成分分析或稀疏自编码器将其分解为有意义的引导方向，直接在输入嵌入空间中解耦语义方向

Result: 方法实现了：(1)可解释、可引导的方向用于精确细粒度概念控制；(2)有效缓解蒸馏模型的模式崩溃问题；(3)基于发现子空间维度建立概念复杂度的新估计器

Conclusion: 提出了一种在扩散模型输入嵌入空间中直接解耦语义方向的新框架，实现了对单个概念的精确控制、恢复模型多样性，并为概念复杂度提供了新的量化方法
...
### [90] [Chamfer-Linkage for Hierarchical Agglomerative Clustering](https://arxiv.org/abs/2602.10444)
*Kishen N Gowda,Willem Fletcher,MohammadHossein Bateni,Laxman Dhulipala,D Ellis Hershkowitz,Rajesh Jayaram,Jakub Łącki*

Main category: cs.LG

TL;DR: 提出基于Chamfer距离的新型链接函数Chamfer-linkage，用于层次凝聚聚类，相比传统链接函数在多个数据集上表现更稳定且质量更高。


<details>
  <summary>Details</summary>
Motivation: 传统层次凝聚聚类（HAC）的链接函数（如单链接、平均链接、Ward方法）在实际应用中表现不稳定，质量参差不齐，缺乏一致的性能保证。

Method: 提出Chamfer-linkage链接函数，使用机器学习中常用的Chamfer距离来衡量簇间距离，满足概念表示的理想属性，并实现了O(n²)的时间复杂度。

Result: 实验表明，Chamfer-linkage在多个不同数据集上比平均链接和Ward方法等传统链接函数产生更高质量的聚类结果。
...
### [91] [Internalizing Meta-Experience into Memory for Guided Reinforcement Learning in Large Language Models](https://arxiv.org/abs/2602.10224)
*Shiting Huang,Zecheng Li,Yu Zeng,Qingnan Ren,Zhen Fang,Qisheng Su,Kou Shi,Lin Chen,Zehui Chen,Feng Zhao*

Main category: cs.LG

TL;DR: MEL框架通过将自我验证的错误分析转化为可重用的元经验知识，解决了RLVR在错误归因和经验内化方面的瓶颈，显著提升了LLM的推理性能。


<details>
  <summary>Details</summary>
Motivation: RLVR虽然能增强LLM的推理能力，但存在元学习瓶颈：缺乏错误归因机制和超越实践与验证的经验内化能力，限制了细粒度信用分配和可重用知识形成。

Method: 提出MEL框架，在标准RLVR基础上，利用LLM的自我验证能力对正确和错误轨迹进行对比分析，识别推理错误的分叉点，将其总结为可泛化的元经验，并通过最小化负对数似然将这些元经验内化到LLM的参数记忆中。

Result: 实验结果显示MEL在基准测试中取得一致改进，在不同模型规模下获得3.92%--4.73%的Pass@1提升。

Conclusion: MEL通过将自我蒸馏的元经验整合到模型参数记忆中，有效解决了RLVR的元学习瓶颈，实现了更细粒度的信用分配和可重用知识形成，显著提升了LLM的推理性能。
...
### [92] [Frame-Level Internal Tool Use for Temporal Grounding in Audio LMs](https://arxiv.org/abs/2602.10230)
*Joesph An,Phillip Keung,Jiaqi Wang,Orevaoghene Ahia,Noah A. Smith*

Main category: cs.LG

TL;DR: 本文提出了一种帧级内部工具使用的方法，让音频语言模型利用自身的内部音频表示直接进行时间定位，避免了传统基于文本标记的时间戳生成方法的计算开销和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型在处理复杂音频理解任务时，难以完成需要精确时间定位的任务（如词对齐和说话人分离）。传统方法将时间戳生成为文本标记序列，计算成本高且容易产生幻觉，特别是在处理超出训练分布长度的音频时。

Method: 提出了帧级内部工具使用的方法，训练音频语言模型利用自身的内部音频表示直接进行时间定位。引入轻量级预测机制，通过两个目标进行训练：二元帧分类器和新颖的非齐次泊松过程损失，用于建模时间事件强度。

Result: 在词定位、说话人分离和事件定位任务中，该方法优于基于标记的基线方法。最显著的是，实现了超过50倍的推理加速，并在长度泛化方面表现出鲁棒性，在超出分布范围的音频时长上保持高精度，而标准的基于标记的模型则完全失效。

Conclusion: 帧级内部工具使用为音频语言模型提供了一种高效、准确的时间定位方法，解决了传统标记生成方法的计算开销和泛化问题，在保持精度的同时大幅提升了推理速度。
...
### [93] [Blockwise Advantage Estimation for Multi-Objective RL with Verifiable Rewards](https://arxiv.org/abs/2602.10231)
*Kirill Pavlenko,Alexander Golubev,Simon Karasik,Boris Yangel*

Main category: cs.LG

TL;DR: 论文提出Blockwise Advantage Estimation方法，通过为结构化生成中的每个文本块分配独立的优势估计，解决GRPO中奖励信号耦合和信用分配错误的问题。


<details>
  <summary>Details</summary>
Motivation: GRPO方法为整个生成结果分配单一标量优势值，在结构化生成中会导致不同段落的奖励信号相互干扰，造成目标冲突和信用分配错误。

Method: 提出Blockwise Advantage Estimation方法家族，为每个目标分配独立的优势估计，仅应用于对应文本块。引入Outcome-Conditioned Baseline，通过基于前缀派生中间结果的分层采样来近似中间状态价值，避免昂贵的嵌套rollout。
...
### [94] [Risk-Equalized Differentially Private Synthetic Data: Protecting Outliers by Controlling Record-Level Influence](https://arxiv.org/abs/2602.10232)
...
<details>
  <summary>Details</summary>
Motivation: 当发布合成数据时，某些个体（如罕见疾病患者或异常交易记录）比其他个体更难保护。差分隐私提供最坏情况保证，但经验攻击（特别是成员推理攻击）对这类异常值的成功率远高于平均水平，尤其是在中等隐私预算和辅助信息存在的情况下。

Method: 提出风险均衡差分隐私合成框架，分两阶段操作：1）使用少量隐私预算估计每条记录的"异常度"；2）在差分隐私学习过程中，根据风险分数对每条记录进行逆权重处理。在高斯机制下，记录的隐私损失与其对输出的影响成正比，因此故意缩小异常值的贡献可以为最需要保护的记录提供更严格的每实例隐私边界。

Result: 通过组合证明端到端差分隐私保证，并为合成阶段推导出封闭形式的每记录边界。在模拟数据上的实验显示，风险加权显著降低了针对高异常度记录的成员推理攻击成功率；消融实验确认是针对性加权而非随机降权驱动了改进。在真实世界基准数据集（乳腺癌、成人、德国信用）上的效果因数据集而异，突显了评分器质量与合成流程之间的相互作用。

Conclusion: 风险均衡差分隐私合成框架能够为高风险记录提供更强的保护，通过针对性降低异常值对生成器的影响来改善隐私保护效果，特别是在成员推理攻击场景下对异常记录的保护效果显著提升。
...
### [95] [Modeling Programming Skills with Source Code Embeddings for Context-aware Exercise Recommendation](https://arxiv.org/abs/2602.10249)
*Carlos Eduardo P. Silva,João Pedro M. Sena,Julio C. S. Reis,André G. Santos,Lucas N. Ferreira*

Main category: cs.LG

TL;DR: 提出基于代码嵌入的学生编程技能建模方法，通过余弦相似度匹配学生能力与作业需求，实现个性化推荐


<details>
  <summary>Details</summary>
Motivation: 传统基于正确率或解题时间的推荐方法信号较弱，需要更精准建模学生编程技能以实现个性化练习推荐

Method: 使用代码嵌入技术建模学生提交的源代码，预测多主题编程技能，通过余弦相似度匹配学生能力向量与作业技能需求向量

Result: Jina嵌入在技能预测上优于TF-IDF、CodeBERT-cpp和GraphCodeBERT；基于技能预测的推荐比基于正确率或解题时间的基线方法更合适

Conclusion: 编程技能预测为作业推荐提供了更强的信号，基于代码嵌入的上下文感知推荐系统能有效匹配学生能力与练习需求
...
### [96] [Kernel-Based Learning of Chest X-ray Images for Predicting ICU Escalation among COVID-19 Patients](https://arxiv.org/abs/2602.10261)
...
<details>
  <summary>Details</summary>
Motivation: 传统单核方法在处理复杂非线性数据时存在局限性，而现有的多核学习方法主要针对连续结果变量，无法处理指数族分布的多种数据类型。

Method: 提出GLIMARK方法，将多核学习扩展到广义线性模型框架，通过构建复合核函数并集成异质数据源信息，适用于指数族分布的多种数据类型。

Result: GLIMARK能够有效恢复或近似真实数据生成机制，在COVID-19胸部X光数据集上成功预测ICU升级的二元结果，并提取出具有临床意义的特征。

Conclusion: GLIMARK方法扩展了多核学习的适用范围，为处理多种数据类型的实际问题提供了有效工具，在医学影像分析等现实场景中具有实用价值。
...
### [97] [Linear-LLM-SCM: Benchmarking LLMs for Coefficient Elicitation in Linear-Gaussian Causal Models](https://arxiv.org/abs/2602.10282)
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在识别定性因果关系方面表现出潜力，但它们在连续领域进行定量因果推理（估计效应大小）的能力尚未得到充分探索

Method: 提出Linear-LLM-SCM框架，将DAG分解为局部父子节点集，提示LLM为每个节点生成回归式结构方程，然后聚合并与真实参数比较

Result: 实验显示：1）某些模型结果具有强随机性；2）在连续领域对虚假边导致的DAG错误设定敏感；3）不同模型系数估计存在显著变异性；4）对结构和语义扰动敏感
...
### [98] [Configuration-to-Performance Scaling Law with Neural Ansatz](https://arxiv.org/abs/2602.10300)
*Huaqing Zhang,Kaiyue Wen,Tengyu Ma*

Main category: cs.LG

TL;DR: 提出神经配置-性能缩放定律(NCPL)，使用LLM参数化从完整训练配置到训练性能的映射，相比传统缩放定律在预测准确性和泛化能力上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统缩放定律假设超参数已最优选择，这在实际中需要大量调优工作且受硬件限制难以实现。需要一种能预测更广泛超参数配置下性能的方法，简化大规模训练的调优过程。

Method: 提出配置-性能缩放定律(CPL)概念，使用大型语言模型(LLM)参数化从完整训练配置到训练性能的映射。利用多个来源的多样化开源预训练日志进行拟合，构建神经配置-性能缩放定律(NCPL)。

Result: NCPL相比配置无关的Chinchilla定律预测误差降低20-40%，能泛化到训练集中10倍计算量的运行。支持多超参数联合调优，性能与超参数缩放定律基线相当。还能自然有效地扩展到更丰富的预测目标如损失曲线预测。

Conclusion: NCPL通过LLM参数化配置-性能映射，显著提升了训练性能预测的准确性和泛化能力，为大规模训练的超参数调优提供了更实用和强大的工具。
...
### [99] [Confounding Robust Continuous Control via Automatic Reward Shaping](https://arxiv.org/abs/2602.10305)
*Mateo Juliani,Mingxuan Li,Elias Bareinboim*

Main category: cs.LG

TL;DR: 本文提出了一种从离线数据中自动学习奖励塑造函数的方法，用于连续控制问题，即使在存在未观测混杂变量的情况下也能保证性能。


<details>
  <summary>Details</summary>
Motivation: 奖励塑造被广泛应用于加速强化学习训练，但对于复杂连续控制问题，如何设计有效的奖励塑造函数仍缺乏系统方法。特别是在离线数据可能受到未观测混杂变量污染的情况下，这一问题更加突出。

Method: 基于因果贝尔曼方程学习最优状态值的紧上界，然后将该上界作为势函数用于基于势的奖励塑造框架。该方法使用Soft-Actor-Critic在多个连续控制基准上进行测试。

Result: 在存在未观测混杂变量的情况下，提出的奖励塑造算法表现出强大的性能保证，在多个常用连续控制基准上验证了有效性。

Conclusion: 这项工作标志着从因果视角实现混杂鲁棒连续控制的重要第一步，为自动学习有效的奖励塑造函数提供了新方法。
...
### [100] [R2RAG-Flood: A reasoning-reinforced training-free retrieval augmentation generation framework for flood damage nowcasting](https://arxiv.org/abs/2602.10312)
*Lipai Huang,Kai Yin,Chia-Fu Liu,Ali Mostafavi*

Main category: cs.LG

TL;DR: R2RAG-Flood是一个无需训练、基于推理增强的检索增强生成框架，用于风暴后财产损失临近预报，通过检索相关推理轨迹来指导大语言模型预测，无需任务特定训练。


<details>
  <summary>Details</summary>
Motivation: 现有监督表格预测器需要特定任务训练，且缺乏解释性。R2RAG-Flood旨在开发一个无需训练、具有推理能力的框架，能够接近监督模型的性能，同时提供结构化推理依据。

Method: 框架构建推理中心知识库，包含带标签的表格记录、自然语言摘要和模型生成的推理轨迹。推理时检索地理空间邻居和典型类别原型的相关推理轨迹，通过上下文增强提示指导大语言模型模拟和适应先前推理。采用两阶段预测：先确定损失发生，然后在三级财产损失程度分类中细化严重程度，包含条件降级步骤纠正过度预测。

Result: 在得克萨斯州哈里斯县的案例研究中，监督表格基线达到0.714总体准确率和0.859损失类别准确率。R2RAG-Flood在七个大语言模型骨干上获得0.613-0.668总体准确率和0.757-0.896损失类别准确率，接近监督基线。轻量级变体在严重程度-成本效率指标上显著优于监督基线和更大语言模型。

Conclusion: R2RAG-Flood无需任务特定训练即可接近监督模型性能，同时提供结构化推理依据，轻量级变体在效率上具有显著优势，为灾害损失预测提供了可解释且高效的方法。
...
### [101] [Stop Training for the Worst: Progressive Unmasking Accelerates Masked Diffusion Training](https://arxiv.org/abs/2602.10314)
*Jaeyeon Kim,Jonathan Geuter,David Alvarez-Melis,Sham Kakade,Sitan Chen*

Main category: cs.LG

TL;DR: PUMA提出了一种渐进式去掩码方法，通过修改前向掩码过程来对齐训练时和推理时的掩码模式，从而加速掩码扩散模型的训练。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在离散空间生成建模中表现出色，但存在训练复杂性问题：需要在指数级数量的掩码模式上训练，计算成本高，且训练时随机掩码与推理时结构化掩码之间存在不匹配。

Method: 提出渐进式去掩码（PUMA），修改前向掩码过程，使训练时的掩码模式与推理时的去掩码模式对齐，专注于优化与推理对齐的掩码。

Result: 在125M规模上，PUMA将预训练速度提升约2.5倍，并且与自回归初始化等常见方法具有互补优势。

Conclusion: PUMA通过对齐训练和推理的掩码模式，有效解决了掩码扩散模型的训练复杂性问题，显著加速训练同时保持性能。
...
### [102] [Theoretical Analysis of Contrastive Learning under Imbalanced Data: From Training Dynamics to a Pruning Solution](https://arxiv.org/abs/2602.10357)
*Haixu Liao,Yating Zhou,Songyang Zhang,Meng Wang,Shuai Zhang*

Main category: cs.LG

TL;DR: 论文分析了对比学习在数据不平衡情况下的训练动态，揭示了神经元权重演变的三个阶段，并发现数据不平衡会降低表示能力、增加架构复杂度需求、阻碍特征分离。通过剪枝可以恢复性能并增强特征分离。


<details>
  <summary>Details</summary>
Motivation: 对比学习已成为学习通用表示的有力框架，但其理论理解仍然有限，特别是在现实应用中普遍存在的数据不平衡分布下。这种不平衡会降低表示质量并导致模型行为偏差，但缺乏对这些影响的严格理论分析。
...
Conclusion: 该研究为理解对比学习在数据不平衡下的训练动态提供了理论框架，揭示了神经元层面的行为机制，并通过剪枝方法提供了实际指导，为改进不平衡数据下的表示学习提供了理论依据和实践方向。
...
### [103] [Simple LLM Baselines are Competitive for Model Diffing](https://arxiv.org/abs/2602.10371)
*Elias Kempf,Simon Schrodi,Bartosz Cywiński,Thomas Brox,Neel Nanda,Arthur Conmy*

Main category: cs.LG

TL;DR: 论文提出模型差异分析方法评估指标，比较LLM基线和稀疏自编码器方法，发现改进的LLM基线在发现抽象行为差异方面表现相当


<details>
  <summary>Details</summary>
Motivation: 标准LLM评估只能测试设计好的能力，会遗漏模型修订间的意外行为差异或新兴错位倾向。现有模型差异分析方法缺乏系统比较和评估标准

Method: 提出评估指标（泛化性、有趣性、抽象级别）来比较现有方法，包括LLM基线和稀疏自编码器方法，并改进LLM基线

Result: 改进的LLM基线在性能上与SAE方法相当，但通常能发现更抽象的行为差异

Conclusion: 需要建立模型差异分析的评估标准，改进的LLM基线是有效的替代方法，能发现更抽象的行为差异
...
Method: 使用尼泊尔多指标类集调查2019年数据，构建综合营养不良指标（结合发育迟缓、消瘦和体重不足），系统比较16种算法（深度学习、梯度提升和传统机器学习），采用10个评估指标，重点关注F1分数和召回率以应对类别不平衡问题。
...
### [105] [Time-to-Event Transformer to Capture Timing Attention of Events in EHR Time Series](https://arxiv.org/abs/2602.10385)
*Jia Li,Yu Hou,Rui Zhang*

Main category: cs.LG

TL;DR: LITT是一种新颖的时序Transformer架构，通过虚拟"相对时间线"对齐序列事件，专注于事件时序注意力，在预测乳腺癌患者心脏毒性心脏病发病时间方面表现出色。
...
Method: 提出LITT（时序Transformer）架构，将时间作为可计算维度，在虚拟"相对时间线"上临时对齐序列事件，实现事件时序聚焦的注意力机制，提供临床轨迹的个性化解释。

Result: 在3,276名乳腺癌患者的真实世界纵向电子健康记录数据上验证了可解释性和有效性，成功预测心脏毒性诱导心脏病的发病时间。在公共数据集上优于基准和最先进的生存分析方法。

Conclusion: LITT代表了临床AI精准医疗的重要进展，通过将时间作为可计算维度并关注事件时序，能够发现个性化的序列事件模式，为临床决策提供更好的支持。
...
### [106] [Colorful Talks with Graphs: Human-Interpretable Graph Encodings for Large Language Models](https://arxiv.org/abs/2602.10386)
*Angelo Zangari,Peyman Baghershahi,Sourav Medya*

Main category: cs.LG

TL;DR: 该论文提出了一种将图结构编码为人类可解释的自然语言提示的方法，通过Weisfeiler-Lehman相似性类和颜色标记来增强大语言模型在图任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理图问题时面临根本性挑战，因为图任务需要处理显式结构、排列不变性和计算复杂的图关系，这与基于文本的模型表示不匹配。需要找到有效的方法让LLMs能够处理图问题。

Method: 提出了一种人类可解释的结构编码策略，用于图到文本的转换。该方法计算Weisfeiler-Lehman相似性类的变体，并将其映射到类似人类的颜色标记而不是数字标签。核心见解是语义上有意义且人类可解释的线索可能比不透明的符号编码更有效地被LLMs处理。

Result: 在多个算法和预测性图任务上的实验结果显示，该方法在合成和真实世界数据集上都带来了显著改进。通过捕捉局部和全局范围的依赖关系，该方法特别增强了LLMs在需要全局图结构推理的任务上的性能。

Conclusion: 通过将图结构编码为人类可解释的自然语言提示，可以有效地克服大语言模型在图任务上的局限性，特别是在需要全局图结构推理的场景中。
...
### [107] [Affordances Enable Partial World Modeling with LLMs](https://arxiv.org/abs/2602.10390)
*Khimya Khetarpal,Gheorghe Comanici,Jonathan Richens,Jeremy Shar,Fei Xia,Laurent Orseau,Aleksandra Faust,Doina Precup*

Main category: cs.LG

TL;DR: 该研究探讨了将大模型作为部分世界模型的可能性，证明了实现任务无关、语言条件意图的智能体必然拥有基于可供性的预测性部分世界模型，并通过实验验证了这种部分模型能显著提高搜索效率。


<details>
  <summary>Details</summary>
Motivation: 虽然预训练大模型可能包含丰富的世界知识，但直接用于搜索过程效率低下且不准确。而部分世界模型专注于对与用户意图相关的状态和动作子集进行高质量预测。研究旨在探索大模型能否作为部分世界模型，并验证其有效性。

Method: 1. 从理论上证明实现任务无关、语言条件意图的智能体必然拥有基于可供性的预测性部分世界模型；2. 在多任务设置中引入分布鲁棒的可供性概念；3. 从大模型中提取部分模型以提高搜索效率；4. 在桌面机器人任务中进行实证评估。

Result: 实验结果表明，基于可供性的部分世界模型显著减少了搜索分支因子，与完整世界模型相比获得了更高的奖励。这验证了从大模型中提取部分模型的有效性。

Conclusion: 大模型可以作为有效的部分世界模型，基于可供性的部分模型能够显著提高智能体在复杂任务中的搜索效率，为构建更高效的世界模型提供了理论依据和实践方法。
...
### [108] [Modular Multi-Task Learning for Chemical Reaction Prediction](https://arxiv.org/abs/2602.10404)
*Jiayun Pang,Ahmed M. Zaitoun,Xacobe Couso Cambeiro,Ivan Vulić*

Main category: cs.LG

TL;DR: LoRA微调在有机反应预测任务上达到与全参数微调相当的准确率，同时更好地缓解灾难性遗忘并保持多任务性能，为化学领域LLM部署提供了实用的参数高效微调策略。


<details>
  <summary>Details</summary>
Motivation: 将在大规模有机化学数据上训练的LLM适配到小型、领域特定的反应数据集是化学和制药研发的关键挑战，需要学习新反应知识同时保持跨相关任务的通用化学理解。

Method: 使用低秩适配（LoRA）作为全参数微调的参数高效替代方案，在USPTO反应类别和具有挑战性的C-H官能化反应数据集上，对前向反应预测、逆合成和试剂预测进行基准测试。

Result: LoRA达到与全参数微调相当的准确率，同时有效缓解灾难性遗忘并更好地保持多任务性能。两种微调方法都能泛化到训练分布之外，产生合理的替代溶剂预测。C-H官能化微调显示LoRA和全参数微调编码了微妙不同的反应性模式。

Conclusion: 随着LLM规模持续扩大，研究结果强调了模块化、参数高效微调策略在化学应用中灵活部署的实用性，LoRA能更有效地进行反应特定适配。
...
### [109] [AI-rithmetic](https://arxiv.org/abs/2602.10416)
*Alex Bie,Travis Dick,Alex Kulesza,Prabhakar Raghavan,Vinod Raman,Sergei Vassilvitskii*

Main category: cs.LG

TL;DR: 前沿AI模型在高级数学任务上表现出色，但在基本整数加法运算中存在系统性缺陷，错误主要源于操作数对齐问题和进位失败。


<details>
  <summary>Details</summary>
Motivation: 尽管现代AI系统在国际数学竞赛、研究工作和技术引理证明等高级数学任务上取得成功，但它们仍然在基本算术运算（特别是整数加法）上表现不佳。本研究旨在系统性地调查这一现象，探究前沿模型在整数加法任务中的错误模式和原因。

Method: 通过实证研究，系统测试前沿模型在不同位数整数加法任务中的准确性。分析错误模式，将错误分类为操作数对齐错误和进位错误两类，并量化各类错误的比例。进一步探究错误与分词机制的关系以及进位错误的随机性特征。

Result: 所有前沿模型在整数加法任务中的准确性随着数字位数的增加而显著下降。模型错误高度可解释：Claude Opus 4.1、GPT-5和Gemini 2.5 Pro的错误中分别有87.9%、62.9%和92.4%可归因于操作数对齐错误或进位失败。对齐错误常与分词机制相关，而进位错误则表现为独立的随机失败。

Conclusion: 前沿AI模型在基本整数加法运算中存在系统性缺陷，主要错误模式为操作数对齐问题和进位失败。这些发现揭示了当前大语言模型在基础算术推理能力上的局限性，为改进模型的基础数学能力提供了重要见解。
...
### [110] [Breaking the Curse of Repulsion: Optimistic Distributionally Robust Policy Optimization for Off-Policy Generative Recommendation](https://arxiv.org/abs/2602.10430)
*Jie Jiang,Yusen Huo,Xiangxin Zhan,Changping Wang,Jun Zhang*

Main category: cs.LG

TL;DR: DRPO通过分布鲁棒优化解决离线强化学习中低质量数据导致的模型崩溃问题，使用硬过滤方法恢复高质量行为分布


<details>
  <summary>Details</summary>
Motivation: 基于策略的强化学习在生成式推荐中占主导地位，但在离线历史日志中应用时，低质量数据的主导会导致严重的模型崩溃。现有方法无法平衡方差减少和噪声模仿的困境。
...
Conclusion: 通过识别噪声行为策略中潜在的高质量分布，并采用分布鲁棒优化框架，DRPO成功解决了离线强化学习中的模型崩溃问题，为混合质量推荐提供了有效的解决方案。
...
### [111] [Gated Removal of Normalization in Transformers Enables Stable Training and Efficient Inference](https://arxiv.org/abs/2602.10408)
*Andrei Kanavalau,Carmen Amo Alonso,Sanjay Lall*

Main category: cs.LG

TL;DR: TaperNorm是一种新的Transformer归一化方法，早期行为类似标准归一化，后期平滑过渡为固定的线性/仿射映射，可消除逐token统计量并实现推理时层折叠
...
Method: 提出TaperNorm作为RMSNorm/LayerNorm的替代方案，使用全局门控机制：早期保持g=1进行标准归一化，后期余弦衰减到g=0，最终变为固定的线性/仿射映射，可折叠到相邻线性投影中

Result: TaperNorm在相同设置下匹配归一化基线的性能，同时消除逐token统计量，在推理时可将这些层折叠到相邻线性投影中，在效率微基准测试中，内部缩放折叠使最后token对数模式吞吐量提高1.22倍

Conclusion: 研究揭示了输出归一化的关键作用是尺度锚定，防止对数追逐问题；TaperNorm为实现无归一化Transformer迈出了一步，同时识别了输出归一化的特殊作用
...
### [112] [LUCID: Attention with Preconditioned Representations](https://arxiv.org/abs/2602.10410)
*Sai Surya Duvvuri,Nirmal Patel,Nilesh Gupta,Inderjit S. Dhillon*

Main category: cs.LG

TL;DR: LUCID Attention是一种新的注意力机制，通过基于键-键相似度的预处理器来减少注意力概率的扩散，在保持标准注意力计算复杂度的同时，显著提升了长序列任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统softmax注意力在长序列场景中存在局限性：随着上下文长度增加，softmax会将概率质量扩散到不相关的token上，降低性能；而通过降低softmax温度来增强聚焦又会因梯度消失而影响可学习性。

Method: 提出LUCID Attention架构修改，应用基于指数化键-键相似度的预处理器到注意力概率上。该预处理器最小化键在再生核希尔伯特空间中的重叠，使查询能在大量键中准确聚焦重要键，计算复杂度与标准注意力相同。

Result: 在约10亿参数的语言模型上验证，评估长度达128K token。在长上下文检索任务上取得显著提升：BABILong上提升达18%，RULER多针任务提升14%，在SCROLLS和LongBench等基准测试中也表现优异。

Conclusion: LUCID Attention通过预处理器方法有效解决了长序列中注意力扩散问题，避免了低温度带来的学习性问题，在保持计算效率的同时显著提升了长上下文任务的性能。
...
### [113] [A Unified Theory of Random Projection for Influence Functions](https://arxiv.org/abs/2602.10449)
*Pingbang Hu,Yuzheng Hu,Jiaqi W. Ma,Han Zhao*

Main category: cs.LG

TL;DR: 该论文研究了在过参数化模型中通过随机投影（sketching）计算影响函数的理论保证，分析了投影如何与岭正则化、结构化曲率近似等技术交互，并提供了选择投影维度的原则性指导。


<details>
  <summary>Details</summary>
Motivation: 在现代过参数化模型中，计算影响函数需要处理高维曲率矩阵的求逆，这在计算上是不可行的。虽然随机投影技术被广泛使用，但缺乏理论保证来解释投影如何与求逆、正则化等操作交互。

Method: 开发统一理论分析投影何时能保持影响函数。分析三种情况：1）无正则化投影；2）岭正则化投影；3）Kronecker分解曲率下的因子化影响。同时分析超出范围（out-of-range）的测试梯度。

Result: 1）无正则化时，精确保持需要投影在曲率矩阵的值域上单射，要求投影维度≥曲率矩阵秩；2）岭正则化改变了投影障碍，近似保证由正则化尺度下的有效维度决定；3）对于Kronecker分解曲率，即使投影存在行相关性，保证仍然成立。

Conclusion: 该工作建立了投影保持影响函数的新理论，为实践中选择投影维度提供了原则性指导，并量化了当测试梯度在曲率矩阵核空间中时产生的泄漏项。
...
### [114] [Constructing Industrial-Scale Optimization Modeling Benchmark](https://arxiv.org/abs/2602.10450)
*Zhong Li,Hongliang Lu,Tao Wei,Wenyu Liu,Yuxuan Chen,Yuan Lan,Fan Zhang,Zaiwen Wen*

Main category: cs.LG

TL;DR: MIPLIB-NL：基于真实混合整数线性规划问题构建的自然语言优化建模基准，用于评估LLM将自然语言需求转换为优化模型的能力，暴露现有基准无法发现的失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前将自然语言需求转换为优化模型和求解器代码的过程仍然劳动密集，现有评估主要基于玩具规模或合成基准，无法反映工业规模问题（10³-10⁶变量和约束）的真实难度，缺乏与真实优化模型对齐的自然语言规范基准。

Method: 通过结构感知的逆向构建方法，从MIPLIB 2017中的真实混合整数线性规划问题出发：1) 从平面求解器公式中恢复紧凑、可重用的模型结构；2) 在统一的模型-数据分离格式下，基于恢复的结构逆向生成自然语言规范；3) 通过专家评审和人-LLM交互的迭代语义验证，进行独立重建检查。

Result: 构建了包含223个一对一重建的MIPLIB-NL基准，保留了原始实例的数学内容，同时支持真实的自然语言到优化评估。实验显示，在现有基准上表现良好的系统在MIPLIB-NL上性能显著下降，暴露了在玩具规模下不可见的失败模式。

Conclusion: MIPLIB-NL填补了真实优化建模评估基准的空白，揭示了当前LLM在工业规模优化问题上的局限性，为未来自然语言到优化模型转换的研究提供了更现实的评估平台。
...
TL;DR: 该论文研究了流匹配在二进制流形上的应用，发现了信号空间预测与速度目标之间的结构不匹配问题，提出了预测损失对齐作为流匹配训练的必要条件，为二进制离散域提供了理论基础和实践指导。


<details>
  <summary>Details</summary>
Motivation: 流匹配在生成建模中表现出色，特别是信号空间预测（x-预测）在连续域中效果显著。然而，当将这一范式转移到二进制流形（离散数据生成建模的基本设置）时，需要研究其适用性和潜在问题。

Method: 1. 识别了x-预测与速度目标（v-损失）结合时产生的潜在结构不匹配问题，这会导致时间依赖的奇异加权，放大梯度对近似误差的敏感性。2. 形式化了预测损失对齐作为流匹配训练的必要条件。3. 证明将目标重新对齐到信号空间（x-损失）可以消除奇异加权，产生均匀有界的梯度，使均匀时间步采样下的鲁棒训练成为可能。4. 在获得对齐后，研究了二进制数据特定的设计选择，揭示了概率目标（如交叉熵）与几何损失（如均方误差）之间的拓扑依赖区别。

Result: 1. 发现了x-预测与v-损失结合时的结构不匹配问题。2. 证明了信号空间对齐可以消除奇异加权，实现均匀有界的梯度。3. 揭示了二进制数据中概率目标与几何损失之间的拓扑依赖区别。4. 为二进制及相关离散域上的鲁棒流匹配提供了理论基础和实践指导。

Conclusion: 信号空间对齐是鲁棒扩散学习的关键原则。该研究为二进制及相关离散域上的流匹配提供了理论框架，通过消除结构不匹配问题，实现了鲁棒训练，并为离散数据生成建模中的损失函数选择提供了指导。
...
TL;DR: LatentRxnFlow提出了一种新的反应预测范式，将化学反应建模为锚定在热力学产物状态的连续潜在轨迹，实现了高精度预测的同时提供反应过程的透明度和可诊断性。


<details>
  <summary>Details</summary>
Motivation: 当前反应预测模型虽然基准测试准确率高，但大多采用从反应物到产物的一步映射方式，无法揭示反应过程机制。逐步生成方法又依赖机制特定监督、离散符号编辑和计算昂贵的推理。

Method: 基于条件流匹配（Conditional Flow Matching），从标准反应物-产物对中直接学习时间依赖的潜在动力学，无需机制注释或中间体标签。将反应建模为连续潜在轨迹，锚定在热力学产物状态。

Result: 在USPTO基准测试中达到最先进性能，更重要的是连续公式化暴露了完整的生成轨迹，实现了轨迹级诊断。潜在轨迹分析能够定位和表征失败模式，通过门控推理缓解某些错误。学习轨迹的几何特性提供了认知不确定性的内在信号。

Conclusion: LatentRxnFlow将强大的预测准确性与改进的透明度、可诊断性和不确定性意识相结合，推动反应预测朝着高通量发现工作流程中更可信赖的部署方向发展。
...
TL;DR: ADAlign是一个用于图域自适应的自适应分布对齐框架，无需手动指定对齐标准，能自动识别并联合对齐最相关的分布差异，在10个数据集和16个迁移任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的图域自适应方法需要手动选择对齐的图元素（如节点属性或结构统计），并依赖手动设计的图滤波器来提取相关特征，这种方法不够灵活，依赖于特定场景的启发式方法，当不同迁移场景中的主要差异变化时效果不佳。

Method: 提出了ADAlign自适应分布对齐框架，引入神经谱差异（NSD）作为理论上有原则的参数距离，提供跨图分布差异的统一视图。NSD利用谱域中的神经特征函数编码所有阶的特征-结构依赖关系，同时通过可学习的频率采样器以极小极大范式自适应地强调每个任务中最具信息量的谱成分。

Result: 在10个数据集和16个迁移任务上的广泛实验表明，ADAlign不仅优于最先进的基线方法，而且实现了效率提升，具有更低的内存使用和更快的训练速度。

Conclusion: ADAlign通过自适应识别和联合对齐最相关的分布差异，解决了现有图域自适应方法依赖手动启发式对齐的局限性，提供了一个灵活、场景感知且对多样化和动态演化分布差异具有鲁棒性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要大量计算和内存资源，现有token-adaptive执行和量化技术单独使用时有效，但简单集成会导致精度下降，因为token-adaptive方法减少了模型冗余度

Method: 提出QTALE框架，包含两个关键组件：1）训练策略确保微调期间探索多样执行路径；2）后训练机制允许推理时灵活调整执行比例以重新引入冗余

Result: QTALE成功集成token-adaptive层执行与量化，精度无明显差异，与纯量化模型的差距保持在CommonsenseQA基准上低于0.5%
...
<details>
  <summary>Details</summary>
Motivation: 研究过参数化transformer模型学习动力学的几何结构，特别是在模块化算术任务中，理解高维参数空间中的训练动态如何组织。

Method: 通过精心控制的模块化算术任务，分析transformer训练轨迹的几何结构，研究参数空间中的维度坍缩现象。

Result: 发现transformer训练轨迹在d=128的高维参数空间中快速坍缩到3-4维的低维执行流形；这种维度坍缩在不同随机种子和中等任务难度下具有鲁棒性；该几何结构解释了注意力集中、SGD近似可积动力学和稀疏自编码器行为。

Conclusion: 提出了理解transformer学习的统一几何框架：大多数参数用于吸收优化干扰，核心计算发生在显著降低的低维子空间中，这对可解释性、训练课程设计和理解过参数化作用具有重要意义。
...
### [120] [A Swap-Adversarial Framework for Improving Domain Generalization in Electroencephalography-Based Parkinson's Disease Prediction](https://arxiv.org/abs/2602.10528)
*Seongwon Jin,Hanseul Choi,Sunggu Yang,Sungho Park,Jibum Kim*

Main category: cs.LG

TL;DR: 该研究提出了首个帕金森病预测的可重复基准数据集，并开发了Swap-Adversarial Framework (SAF)来解决ECoG数据中的高主体间变异性和高维低样本问题，实现了跨ECoG和EEG数据集的鲁棒域泛化。
...
Method: 提出Swap-Adversarial Framework (SAF)，包含：(1)鲁棒预处理，(2)主体间平衡通道交换(ISBCS)进行跨主体数据增强，(3)域对抗训练抑制主体特异性偏差。ISBCS随机交换不同主体间的通道以减少主体间变异性。

Result: 在跨主体、跨会话和跨数据集设置下，该方法在所有基准方法中表现最优，在高度可变环境中改进最显著。在公开EEG基准测试中也实现了优异的跨数据集性能，展示了强大的泛化能力。

Conclusion: 该研究提供了首个帕金森病预测的ECoG基准数据集，提出的SAF框架有效解决了ECoG数据的高主体间变异性和高维低样本问题，不仅在ECoG内部，还能泛化到EEG数据，具有强大的跨域泛化能力。
...
### [121] [LLM-Based Scientific Equation Discovery via Physics-Informed Token-Regularized Policy Optimization](https://arxiv.org/abs/2602.10576)
*Boxiao Wang,Kai Li,Tianyi Liu,Chen Li,Junzhe Wang,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: 提出PiT-PO框架，通过强化学习将LLM转变为自适应符号回归生成器，结合物理约束和令牌级正则化，生成科学一致且结构简洁的方程。


<details>
  <summary>Details</summary>
Motivation: 现有符号回归方法主要将LLM作为静态生成器，依赖提示级指导，无法根据搜索反馈更新模型内部表示，常产生物理不一致或数学冗余的表达式。

Method: 提出PiT-PO（物理信息令牌正则化策略优化）框架，通过强化学习演化LLM为自适应生成器，采用双重约束机制：强制分层物理有效性，同时应用细粒度令牌级惩罚抑制冗余结构。

Result: 在标准基准测试中达到最先进性能，成功为挑战性流体动力学问题发现新的湍流模型，并证明小规模模型能超越闭源大型模型。

Conclusion: PiT-PO框架使LLM能够生成科学一致且结构简洁的方程，推动了高性能科学发现的民主化。
...
### [122] [Enhancing Ride-Hailing Forecasting at DiDi with Multi-View Geospatial Representation Learning from the Web](https://arxiv.org/abs/2602.10502)
*Xixuan Hao,Guicheng Li,Daiqiang Wu,Xusen Guo,Yumeng Zhu,Zhichao Zou,Peng Zhen,Yao Yao,Yuxuan Liang*

Main category: cs.LG

TL;DR: MVGR-Net是一个用于网约车预测的两阶段框架，通过多视角地理空间表征学习和提示增强的大语言模型微调，解决了地理空间异质性和外部事件影响的问题。
...
Method: 提出MVGR-Net两阶段框架：1) 预训练阶段整合兴趣点和时序移动模式，从语义属性和时序移动模式两个视角学习全面的地理空间表征；2) 预测阶段通过提示增强框架微调大语言模型，同时融入外部事件信息。

Result: 在滴滴的真实数据集上进行广泛实验，证明了该方法达到了最先进的性能水平。

Conclusion: MVGR-Net通过多视角地理空间表征学习和提示增强的大语言模型微调，有效解决了网约车预测中的地理空间异质性和外部事件影响问题，为城市交通优化提供了有力工具。
...
### [123] [Don't Eliminate Cut: Exponential Separations in LLM-Based Theorem Proving](https://arxiv.org/abs/2602.10512)
*Sho Sonoda,Shunta Akiyama,Yuya Uezato*

Main category: cs.LG

TL;DR: 该论文为LLM指导的形式化定理证明建立理论框架，证明在证明DAG存在可重用结构时，具备剪枝感知的分层学习器比平面学习器需要指数级更少的数据。


<details>
  <summary>Details</summary>
Motivation: 解释形式化定理证明中经验成功与最坏情况困难之间的差距，为近期智能定理证明器中的子目标分解提供理论依据。

Method: 将策略提议建模为有限时域确定性MDP中的随机策略，引入由参考策略q生成的问题分布，包括具有可重用剪枝/引理/草图结构的潜在变量模型，在top-k搜索协议和Tsybakov型边界条件下分析有限时域成功概率。

Result: 当剪枝消除将深度D的DAG扩展为大小为Ω(Λ^D)的无剪枝树，而剪枝感知分层过程大小为O(λ^D)且λ≪Λ时，平面（无剪枝）学习器比剪枝感知分层学习器需要指数级更多数据。

Conclusion: 该理论分析为智能定理证明器中的子目标分解提供了原则性理论支持，解释了为什么分层方法在具有可重用证明结构的场景中具有显著优势。
...
### [124] [Prioritize the Process, Not Just the Outcome: Rewarding Latent Thought Trajectories Improves Reasoning in Looped Language Models](https://arxiv.org/abs/2602.10520)
*Williams Jonathan,Tureci Esin*

Main category: cs.LG

TL;DR: RLTT框架通过在整个潜在推理轨迹上分配奖励，解决了LoopLMs强化学习中只奖励最终状态的问题，显著提升了数学推理性能并有效迁移到非数学任务


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习方法（如GRPO）在LoopLMs中只对最终潜在状态分配奖励，这与模型内部的多步推理计算存在根本性不匹配，导致强化学习无法有效提升LoopLMs的推理能力

Method: 提出RLTT（Reward Latent Thought Trajectories）强化学习框架，将奖励分布在整个潜在推理轨迹上，提供密集的轨迹级信用分配，无需依赖外部验证器，可直接替代GRPO且开销极小

Result: 在相同训练和推理条件下，RLTT相比GRPO在数学推理基准上取得显著提升：MATH-500准确率+14.4%，AIME24+16.6%，BeyondAIME+10.0%。尽管仅在数学数据上训练，RLTT也能有效迁移到非数学推理任务

Conclusion: 轨迹级信用分配是LoopLMs强化学习的有效方法，RLTT框架解决了传统方法只奖励最终状态的问题，显著提升了推理性能并展示了良好的迁移能力
...
### [125] [What Makes Value Learning Efficient in Residual Reinforcement Learning?](https://arxiv.org/abs/2602.10539)
...
<details>
  <summary>Details</summary>
Motivation: 残差强化学习虽然能稳定在线优化预训练策略，但其价值学习面临独特挑战。现有方法存在两个关键瓶颈：冷启动问题（评论家缺乏基策略周围价值景观知识）和结构尺度不匹配问题（残差贡献被基动作淹没），这些问题尚未得到充分理解和解决。

Method: 提出DAWN方法，包含两个核心组件：1）使用基策略转换作为价值锚点进行隐式预热，解决冷启动问题；2）通过评论家归一化有效恢复表示敏感性，使评论家能够辨别价值差异，解决结构尺度不匹配问题。

Result: DAWN方法在多样化基准测试、策略架构和观测模态中展现出显著的效率提升，通过解决这两个瓶颈实现了高效的价值学习。

Conclusion: 残差强化学习中的价值学习瓶颈可以通过简单而原则性的方法解决。基策略转换作为价值锚点和评论家归一化是有效解决冷启动和结构尺度不匹配问题的关键，DAWN方法为此提供了最小化且高效的解决方案。
...
### [126] [VESPO: Variational Sequence-Level Soft Policy Optimization for Stable Off-Policy LLM Training](https://arxiv.org/abs/2602.10693)
*Guobin Shen,Chenxiao Zhao,Xiang Cheng,Lei Huang,Xing Yu*

Main category: cs.LG

TL;DR: VESPO是一种用于大型语言模型强化学习的变分序列级软策略优化方法，通过变分公式推导出闭式重塑核，直接处理序列级重要性权重，解决了策略陈旧和异步训练导致的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型强化学习中训练稳定性是一个核心挑战。策略陈旧、异步训练以及训练与推理引擎不匹配导致行为策略与当前策略偏离，可能引发训练崩溃。重要性采样虽然提供了分布偏移的校正，但存在高方差问题，现有方法缺乏统一的理论基础。

Method: 提出VESPO（变分序列级软策略优化），将方差减少纳入提案分布的变分公式中，推导出闭式重塑核，直接操作序列级重要性权重而无需长度归一化。

Result: 在数学推理基准测试中，VESPO在陈旧比高达64倍和完全异步执行的情况下保持训练稳定，并在密集模型和专家混合模型中都取得了一致的性能提升。

Conclusion: VESPO为大型语言模型强化学习中的训练稳定性问题提供了一个理论统一且有效的解决方案，能够处理策略陈旧和异步训练带来的挑战。
...
### [127] [Interpretable Graph-Level Anomaly Detection via Contrast with Normal Prototypes](https://arxiv.org/abs/2602.10708)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度图级异常检测方法存在黑盒问题，缺乏可解释性；现有解释方法要么不参考正常图，要么依赖抽象潜在向量而非具体数据集中的图

Method: 使用点集核迭代发现多个正常原型图及其关联聚类，将远离所有正常聚类的图识别为异常，并通过对比异常图与最近正常原型图提供解释

Result: 在多个真实数据集上的实验表明，ProtoGLAD在异常检测性能上与最先进方法相当，同时提供更好的人类可理解的原型解释

Conclusion: ProtoGLAD是一个可解释的无监督框架，通过明确对比异常图与最近正常原型图，在保持竞争性检测性能的同时提供更好的可解释性
...
### [128] [Online Min-Max Optimization: From Individual Regrets to Cumulative Saddle Points](https://arxiv.org/abs/2602.10565)
*Abhijeet Vyas,Brian Bullins*

Main category: cs.LG

TL;DR: 该论文研究了在线min-max优化的新性能度量，提出了静态对偶间隙和动态鞍点遗憾，并在强凸-强凹、min-max指数凹性等条件下提供了算法和理论保证。


<details>
  <summary>Details</summary>
Motivation: 传统静态纳什均衡与个体遗憾在非凸凹设置下不兼容，需要新的性能度量来评估在线min-max优化算法。

Method: 提出静态对偶间隙(SDual-Gap_T)和动态鞍点遗憾(DSP-Reg_T)，通过约简到经典在线凸优化问题设计算法，在强凸-强凹、min-max指数凹性和双边Polyak-Łojasiewicz条件下进行分析。

Result: 在强凸-强凹和min-max指数凹性条件下获得了SDual-Gap_T和DSP-Reg_T的界，建立了满足min-max指数凹性的函数类（包含双玩家投资组合选择问题），并在双边PL条件下为动态遗憾提供了保证。

Conclusion: 论文提出了适用于在线min-max优化的新性能度量框架，在多种函数类下提供了算法和理论保证，扩展了在线优化的分析工具。
...
### [129] [Gauss-Newton Unlearning for the LLM Era](https://arxiv.org/abs/2602.10568)
*Lev McKinney,Anvith Thudi,Juhan Bae,Tara Rezaei,Nicolas Papernot,Sheila A. McIlraith,Roger Grosse*

Main category: cs.LG

TL;DR: 提出K-FADE方法，通过使用遗忘集计算少量上坡高斯-牛顿步骤，实现LLM的高效遗忘学习，在保持模型在保留集上性能的同时有效抑制遗忘集输出。


<details>
  <summary>Details</summary>
Motivation: 标准大语言模型训练可能产生部署中不可接受的输出，虽然可以通过遗忘学习减少这些输出，但传统方法会降低模型在其他分布上的性能，需要改进这种权衡。

Method: 使用遗忘集计算少量上坡高斯-牛顿步骤，结合参数化Hessian近似方法如K-FAC，将整个保留集的输出约束转化为权重约束，最小化对保留集行为的影响。

Result: 在WMDP和ToFU基准测试中，K-FADE有效抑制遗忘集输出，近似达到无遗忘集重新训练的效果，同时对保留集输出的改变小于先前方法，且遗忘更新可在后续训练中重复应用。

Conclusion: K-FADE提供了一种概念简单、性能先进的LLM遗忘学习方法，通过将输出空间约束转化为权重约束，在有效遗忘的同时最小化对保留集性能的影响，且具有可重复应用的特性。
...
### [130] [When Gradient Clipping Becomes a Control Mechanism for Differential Privacy in Deep Learning](https://arxiv.org/abs/2602.10584)
*Mohammad Partohaghighi,Roummel Marcia,Bruce J. West,YangQuan Chen*

Main category: cs.LG

TL;DR: 提出一种基于控制理论的剪裁阈值自适应方法，通过轻量级权重谱诊断来调整差分隐私训练中的梯度剪裁阈值，避免过剪裁和噪声主导问题。


<details>
  <summary>Details</summary>
Motivation: 差分隐私训练中的梯度剪裁阈值设置是关键挑战：阈值过小会导致系统性过剪裁引起优化偏差；阈值过大会使注入噪声主导更新并降低准确性。现有自适应方法依赖每个样本梯度范数统计，计算开销大且对数据集和架构敏感。

Method: 提出控制驱动的剪裁策略，使用轻量级、仅权重的谱诊断来适应阈值。在周期性探测步骤中，方法通过谱分解分析指定权重矩阵，估计与训练稳定性相关的重尾谱指标。该指标随时间平滑处理，并输入有界反馈控制器，在log域中乘法更新剪裁阈值。

Result: 由于控制器仅使用隐私保护训练期间产生的参数，得到的阈值更新是后处理的，在标准组合计算下不会增加底层DP优化器之外的隐私损失。

Conclusion: 该方法提供了一种高效、隐私安全的自适应剪裁策略，解决了传统方法计算开销大和对数据集敏感的问题，同时保持隐私保护训练的有效性。
...
### [131] [TRACE: Theoretical Risk Attribution under Covariate-shift Effects](https://arxiv.org/abs/2602.10588)
*Hosein Anjidani,S. Yahya S. R. Tehrani,Mohammad Mahdi Mojahedian,Mohammad Hossein Yassaee*

Main category: cs.LG

TL;DR: TRACE框架分解模型替换后的风险变化，将其归因于四个可解释因素，提供诊断工具和部署门控决策


<details>
  <summary>Details</summary>
Motivation: 当源域训练的模型被基于偏移数据训练的新模型替换时，其在源域上的性能变化难以预测，需要理解这种风险变化的原因

Method: 提出TRACE框架，将风险变化绝对值分解为四个可解释因素：两个泛化差距、模型变化惩罚和协变量偏移惩罚；通过模型敏感度因子、特征空间最优传输、最大均值差异等方法实例化各项

Result: 在线性回归设置中验证TRACE边界正确捕捉了真实风险差异随偏移幅度的缩放；在合成和视觉基准测试中，TRACE诊断有效且与真实性能退化保持强单调关系；部署门控分数与风险变化强相关，实现高AUROC/AUPRC

Conclusion: TRACE框架提供了解释模型替换后风险变化的诊断工具，能够实现安全、标签高效的模型替换决策
...
### [132] [Transport, Don't Generate: Deterministic Geometric Flows for Combinatorial Optimization](https://arxiv.org/abs/2602.10794)
*Benjy Friedmann,Nadav Dym*

Main category: cs.LG

TL;DR: CycFlow提出了一种新的神经组合优化框架，用确定性点传输替代迭代边去噪，通过流匹配将2D坐标连续传输到规范圆形排列，实现比扩散模型快3个数量级的求解速度
...
Method: CycFlow框架学习一个实例条件化的向量场，将输入的2D坐标连续传输到规范的圆形排列中。通过数据依赖的流匹配技术，绕过边评分的二次瓶颈，采用线性坐标动态。从2N维表示中通过角度排序恢复最优路径

Result: 与最先进的扩散基线相比，CycFlow将求解速度提高了三个数量级，同时保持了竞争力的最优性差距

Conclusion: CycFlow通过用确定性点传输替代迭代边去噪，实现了范式转变，为神经组合优化提供了更高效的解决方案，显著加速了旅行商问题的求解过程
...
### [133] [Roughness-Informed Federated Learning](https://arxiv.org/abs/2602.10595)
*Mohammad Partohaghighi,Roummel Marcia,Bruce J. West,YangQuan Chen*
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在非独立同分布（non-IID）设置下面临客户端漂移问题，这会损害模型收敛性能。现有方法在处理异构数据时效果有限，需要更有效的解决方案来提升联邦学习在现实异构环境中的鲁棒性和效率。

Method: 提出RI-FedAvg算法，在本地目标函数中引入基于粗糙度指数（RI）的正则化项。RI用于量化高维损失函数的粗糙度，根据本地损失景观的波动自适应地惩罚更新，从而缓解客户端漂移。

Result: 在MNIST、CIFAR-10和CIFAR-100数据集上的实验表明，RI-FedAvg在非IID场景下优于FedAvg、FedProx、FedDyn、SCAFFOLD和DP-FedAvg等先进基线方法，实现了更高的准确率和更快的收敛速度。收敛分析证明算法在非凸目标下能收敛到稳定点。

Conclusion: RI-FedAvg通过粗糙度指数正则化有效缓解了联邦学习中的客户端漂移问题，在异构环境中展现出优越的性能，为实际应用中的联邦学习提供了更鲁棒和高效的解决方案。
...
### [134] [dnaHNet: A Scalable and Hierarchical Foundation Model for Genomic Sequence Learning](https://arxiv.org/abs/2602.10603)
...
Result: 在细菌基因组上预训练后，dnaHNet在扩展性和效率上优于包括StripedHyena2在内的领先架构，递归分块实现二次FLOP减少，推理速度比Transformer快3倍以上。在零样本任务中，dnaHNet在预测蛋白质变异适应性和基因必需性方面表现优异，并能无监督地自动发现分层生物结构。
...
### [135] [Enhancing Multivariate Time Series Forecasting with Global Temporal Retrieval](https://arxiv.org/abs/2602.10847)
...
<details>
  <summary>Details</summary>
Motivation: 现有多元时间序列预测模型受限于有限的历史上下文，无法有效捕捉跨越多个周期的全局周期性模式，而这些模式通常包含强烈的预测信号。简单扩展历史窗口会导致过拟合、计算成本过高和信息冗余等问题。

Method: 提出Global Temporal Retriever（GTR）模块，维护整个周期的自适应全局时间嵌入，动态检索并输入序列对齐相关全局片段。通过2D卷积和残差融合联合建模局部和全局依赖关系，在不改变宿主模型架构的情况下连接短期观测和长期周期性。

Result: 在六个真实世界数据集上的广泛实验表明，GTR在短期和长期预测场景中均能持续提供最先进的性能，同时引入最小的参数和计算开销。

Conclusion: GTR是一种高效通用的解决方案，能够增强多元时间序列预测任务中的全局周期性建模能力，通过轻量级模块有效扩展模型的时序感知范围。
...
### [136] [Time Series Foundation Models for Energy Load Forecasting on Consumer Hardware: A Multi-Dimensional Zero-Shot Benchmark](https://arxiv.org/abs/2602.10848)
*Luigi Simeone*

Main category: cs.LG

TL;DR: 本文评估了四种时间序列基础模型在电力需求预测中的零样本性能，发现它们在长上下文长度下表现优异，相比传统方法有显著优势，但校准质量存在差异。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型引入了零样本预测能力，但这些能力是否适用于电力需求预测等关键任务应用仍是一个开放性问题，因为这些应用的准确性、校准性和鲁棒性直接影响电网运营。

Method: 使用ERCOT 2020-2024年小时负荷数据，在消费级硬件上对四种TSFM（Chronos-Bolt、Chronos-2、Moirai-2、TinyTimeMixer）进行多维基准测试，包括Prophet作为行业基准和两个统计参考模型（SARIMA和Seasonal Naive）。评估涵盖四个维度：上下文长度敏感性、概率预测校准、分布偏移下的鲁棒性以及操作决策支持的规范性分析。

Result: 表现最佳的基础模型在长上下文长度（C=2048小时，日前预测）下MASE值接近0.31，比Seasonal Naive基线降低了47%。Prophet在拟合窗口短于其季节性周期时失败（24小时上下文下MASE>74），而TSFM即使在最小上下文下也能保持稳定准确性。校准质量差异显著：Chronos-2产生良好校准的预测区间，而Moirai-2和Prophet表现出过度自信。

Conclusion: 时间序列基础模型在电力需求预测中展现出强大的零样本能力，特别是在长上下文长度下表现优异。它们通过预训练识别时间模式，避免了从头估计的需要。模型选择应考虑上下文长度需求和校准要求，Chronos-2在校准方面表现最佳。研究提供了实用的模型选择指南并发布了完整的基准框架。
...
### [137] [Pupillometry and Brain Dynamics for Cognitive Load in Working Memory](https://arxiv.org/abs/2602.10614)
*Nusaibah Farrukh,Malavika Pradeep,Akshay Sasi,Rahul Venugopal,Elizabeth Sherly*

Main category: cs.LG

TL;DR: 该研究比较了EEG和瞳孔测量在认知负荷分类中的表现，发现基于特征的传统机器学习方法优于深度学习，且瞳孔测量单独使用就能与EEG竞争，为可穿戴认知监测系统提供了实用方案。


<details>
  <summary>Details</summary>
Motivation: 认知负荷评估对自适应学习、临床监测和脑机接口至关重要。现有研究多使用EEG和瞳孔测量，但EEG技术复杂、成本高且穿戴不便，而瞳孔测量便携可扩展。现有深度学习模型解释性差且计算成本高，需要探索更实用、轻量化的监测方案。

Method: 使用OpenNeuro的'数字广度任务'数据集，结合特征驱动和模型驱动方法进行时间序列分析。采用Catch-22特征提取和传统机器学习模型，与深度学习方法比较。通过SHAP特征分析提供生理学解释。

Result: 基于Catch-22特征的传统机器学习在二分类和多分类任务中均优于深度学习。瞳孔测量单独使用就能与EEG的表现竞争，表明其可作为认知负荷检测的便携替代方案。

Conclusion: 瞳孔测量结合可解释模型能够提供与EEG相当的认知负荷检测能力，挑战了EEG是必要手段的假设。这支持了开发可穿戴、经济实惠的认知监测系统，适用于神经精神病学、教育和医疗保健领域。
...
### [138] [FedPS: Federated data Preprocessing via aggregated Statistics](https://arxiv.org/abs/2602.10870)
*Xuefeng Xu,Graham Cormode*

Main category: cs.LG

TL;DR: FedPS是一个基于聚合统计的联邦数据预处理统一框架，通过数据草图技术高效汇总本地数据集，为水平与垂直联邦学习提供通信高效、一致的数据预处理解决方案。


<details>
  <summary>Details</summary>
Motivation: 联邦学习需要在隐私约束下进行数据预处理，但现有研究大多忽视这一关键环节。实际FL系统中，隐私限制禁止集中原始数据，通信效率也给分布式预处理带来挑战。

Method: 基于聚合统计的联邦数据预处理框架，利用数据草图技术高效汇总本地数据集，设计特征缩放、编码、离散化和缺失值插补的联邦算法，并将k-Means、k-NN和贝叶斯线性回归等预处理相关模型扩展到水平和垂直联邦学习设置。

Result: FedPS为实际FL部署提供了灵活、通信高效且一致的预处理管道，能够在不共享原始数据的情况下完成关键的数据预处理任务。

Conclusion: FedPS填补了联邦学习中数据预处理研究的空白，为实际FL系统提供了实用的预处理解决方案，解决了隐私约束下的分布式数据预处理挑战。
...
### [139] [Resource-Efficient Model-Free Reinforcement Learning for Board Games](https://arxiv.org/abs/2602.10894)
...
<details>
  <summary>Details</summary>
Motivation: 基于搜索的强化学习方法（如AlphaZero）在棋盘游戏中取得了显著成功，但计算需求大、可复现性差。需要开发更高效的模型无关强化学习算法来替代传统基于搜索的方法。

Method: 提出一种专门为棋盘游戏设计的模型无关强化学习算法，通过核心技术创新实现高效学习，并在五种棋盘游戏（Animal Shogi, Gardner Chess, Go, Hex, Othello）上进行验证。

Result: 在五种棋盘游戏环境中，提出的方法比现有方法实现了更高效的学习。消融研究证明了所采用核心技术的重要性。

Conclusion: 该高效算法展示了模型无关强化学习在传统由基于搜索方法主导的领域中的潜力，为棋盘游戏AI提供了更高效、可复现的替代方案。
...
### [140] [Generative clinical time series models trained on moderate amounts of patient data are privacy preserving](https://arxiv.org/abs/2602.10631)
*Rustam Zhumagambetov,Niklas Giesa,Sebastian D. Boie,Stefan Haufe*

Main category: cs.LG

TL;DR: 该研究审计了生成式AI模型在医疗时间序列数据上的隐私保护效果，发现当训练数据足够大时，现有隐私攻击对生成的临床时间序列数据无效，且差分隐私机制只会降低数据实用性而不会带来隐私改进。


<details>
  <summary>Details</summary>
Motivation: 医疗数据共享面临隐私泄露风险，生成式AI模型虽然能生成合成数据，但现有隐私保护机制（如k-匿名化、差分隐私）在时间序列数据上存在局限性，需要评估这些模型的实际隐私保护效果。

Method: 使用一系列已建立的隐私攻击方法，对在公开MIMIC-IV数据集上训练的最先进医院时间序列模型进行隐私审计，并使用eICU数据集对MIMIC-IV训练的合成数据生成器发起隐私攻击。

Result: 当合成数据生成器在足够大的训练数据集上训练时，现有隐私攻击对生成的多元临床时间序列数据无效。现有差分隐私机制不会带来期望的隐私改进，只会降低机器学习预测任务的实用性。

Conclusion: 对于大型训练数据集，生成式AI模型生成的临床时间序列数据具有较好的隐私保护效果，无需额外应用差分隐私机制，因为后者只会降低数据实用性而不改善隐私保护。
...
### [141] [Coarse-Grained Boltzmann Generators](https://arxiv.org/abs/2602.10637)
*Weilong Chen,Bojun Zhao,Jan Eckwert,Julija Zavadlav*

Main category: cs.LG

TL;DR: 提出Coarse-Grained Boltzmann Generators (CG-BGs)框架，将粗粒度建模与重要性采样结合，用于高效采样大分子系统的玻尔兹曼分布


<details>
  <summary>Details</summary>
Motivation: 传统Boltzmann Generators (BGs)在实际应用中可扩展性有限，而粗粒度替代模型虽然能处理更大系统但缺乏确保渐近正确统计的重加权过程

Method: CG-BGs在粗粒度坐标空间中工作，使用学习得到的平均力势(PMF)对基于流模型生成的样本进行重加权，并通过力匹配从快速收敛数据中高效学习PMF

Result: CG-BGs能够在高度简化的表示中准确捕捉由显式溶剂介导的复杂相互作用，为更大分子系统的无偏采样建立了可扩展途径

Conclusion: CG-BGs提供了一个原则性框架，将可扩展的降阶建模与重要性采样的精确性统一起来，解决了大分子系统玻尔兹曼分布采样的挑战
...
### [142] [Rotary Positional Embeddings as Phase Modulation: Theoretical Bounds on the RoPE Base for Long-Context Transformers](https://arxiv.org/abs/2602.10959)
*Feilong Liu*

Main category: cs.LG

TL;DR: 该研究将RoPE重新解释为相位调制，推导出保持位置相干性的基参数下界（防混叠和稳定性）和浮点精度相关的上界，定义了长上下文Transformer的可行区域，并通过主流模型验证了理论边界。


<details>
  <summary>Details</summary>
Motivation: RoPE在大语言模型中广泛用于编码token位置，但其在长上下文长度下的行为特征尚不明确。需要从信号处理角度分析RoPE在长上下文中的表现，为长上下文Transformer设计提供理论指导。

Method: 将RoPE重新解释为应用于复数振荡器组的相位调制，利用经典信号处理理论进行分析。推导出保持位置相干性所需的RoPE基参数下界（包括类似奈奎斯特极限的混叠边界和低频模式相位漂移的稳定性边界），并扩展到深层Transformer。同时推导出由有限浮点精度决定的基参数上界。

Result: 理论分析表明存在精度和深度相关的可行区域（Goldilocks zone）。通过LLaMA、Mistral、DeepSeek等主流模型的案例研究验证了理论边界：违反稳定性边界的模型出现注意力崩溃和长距离性能下降，超过百万token的扩展尝试遇到与架构或训练无关的硬精度限制。

Conclusion: 该研究为RoPE在长上下文中的行为提供了理论框架，定义了基参数选择的可行区域，解释了现有模型的成功与失败，并为未来长上下文Transformer设计提供了指导原则。
...
### [143] [Evaluation metrics for temporal preservation in synthetic longitudinal patient data](https://arxiv.org/abs/2602.10643)
*Katariina Perkonoja,Parisa Movahedi,Antti Airola,Kari Auranen,Joni Virta*

Main category: cs.LG

TL;DR: 该研究提出了一套评估合成纵向患者数据时间保留性的指标，这些指标评估合成数据如何重现关键时间特征，包括边际、协方差、个体水平和测量结构。研究发现强边际相似性可能掩盖协方差扭曲和个体轨迹中断。


<details>
  <summary>Details</summary>
Motivation: 当前合成纵向患者数据评估缺乏系统的时间保留性评估指标。需要开发能够全面评估合成数据如何保留真实患者随时间重复测量特征的方法，以支持生成更真实的时间序列数据。

Method: 提出了一套评估时间保留性的指标，分为四类：边际结构、协方差结构、个体水平结构和测量结构。这些指标考虑了原始数据质量、测量频率、预处理策略（如分箱、变量编码和精度）等因素对时间保留性的影响。

Result: 研究发现：1）强边际相似性可能掩盖协方差扭曲和个体轨迹中断；2）时间保留性受原始数据质量、测量频率和预处理策略影响；3）稀疏或高度不规则测量时间的变量提供有限的时间依赖信息；4）单一指标不足以全面评估时间保留性，需要多维评估。

Conclusion: 提出的指标阐明了时间结构如何被保留或退化，支持更可靠的生成模型评估和改进，有助于创建时间上更真实的合成纵向患者数据。多维评估方法比单一指标更能全面评估合成数据质量。
...
### [144] [Domain Knowledge Guided Bayesian Optimization For Autonomous Alignment Of Complex Scientific Instruments](https://arxiv.org/abs/2602.10670)
*Aashwin Mishra,Matt Seaberg,Ryan Roussel,Daniel Ratner,Apurva Mehta*

Main category: cs.LG

TL;DR: 本文提出了一种基于领域知识引导的贝叶斯优化方法，通过物理洞察进行坐标变换来解耦高维参数，将主动子空间与主要搜索轴对齐，从而显著加速复杂系统的优化过程。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化在高维、参数紧密耦合且目标函数高度不对称的"大海捞针"场景中性能下降，即使先进的信任区域BO等方法也难以获得满意结果。随着复杂科学仪器（如大型望远镜、X射线自由电子激光器中的新光谱仪）的部署，对鲁棒高维优化的需求日益增长。

Method: 提出领域知识引导的贝叶斯优化方法，利用物理洞察进行坐标变换，解耦输入特征并将主动子空间与主要搜索轴对齐。结合反向退火探索策略，该方法能够可靠地收敛到全局最优解。

Result: 在具有挑战性的12维、6晶体分束延迟光学系统上，该方法显著优于传统方法（包括标准BO、TuRBO和多目标BO）。坐标变换本身是成功的关键，通过将输入坐标轴与问题的主动子空间对齐，显著加速了搜索过程。

Conclusion: 研究结果表明了一个可推广的范式：利用物理洞察将高维耦合优化问题转化为更简单的表示，可以在保留现有优化算法的同时，实现快速、鲁棒的自动调谐，确保一致的高性能。
...
### [145] [ROCKET: Rapid Optimization via Calibration-guided Knapsack Enhanced Truncation for Efficient Model Compression](https://arxiv.org/abs/2602.11008)
*Ammar Ali,Baher Mohammad,Denis Makhov,Dmitriy Shopkhoev,Magauiya Zhussip,Stamatios Lefkimmiatis*

Main category: cs.LG

TL;DR: ROCKET是一种无需训练的模型压缩方法，通过多选择背包问题优化层间压缩分配，结合单步稀疏矩阵分解，在20-50%压缩率下超越现有方法，保持90%以上原始性能，轻量微调后恢复效果显著。


<details>
  <summary>Details</summary>
Motivation: 现有模型压缩方法（如因子分解、结构化稀疏化和动态压缩）在保持性能的同时实现高效压缩仍有改进空间，需要一种无需训练、计算高效的压缩方法，能够在全局压缩预算下优化各层压缩分配。

Method: 1. 将层间压缩分配建模为多选择背包问题，为每层选择最优压缩级别以最小化重构误差；2. 引入受字典学习启发的单步稀疏矩阵分解：使用小校准集基于激活-权重敏感性稀疏化权重系数，通过最小二乘法闭式更新字典，无需迭代优化、稀疏编码或反向传播。

Result: 在20-50%压缩率下持续超越现有压缩方法，30%压缩时保持原始模型90%以上性能且无需微调。轻量微调后恢复效果显著：将Qwen3-14B压缩至8B参数模型，仅用3000万token微调即可达到接近原始Qwen3-8B的性能。

Conclusion: ROCKET提供了一种高效、无需训练的模型压缩框架，通过优化层间压缩分配和单步稀疏分解实现了优异的压缩性能保持，为大规模模型部署提供了实用的压缩解决方案。
...
### [146] [OSIL: Learning Offline Safe Imitation Policies with Safety Inferred from Non-preferred Trajectories](https://arxiv.org/abs/2602.11018)
*Returaj Burnwal,Nirav Pravinbhai Bhatt,Balaraman Ravindran*

Main category: cs.LG

TL;DR: 提出OSIL算法，从离线非偏好轨迹中学习安全策略，无需显式安全成本标注，通过约束马尔可夫决策过程框架实现安全模仿学习


<details>
  <summary>Details</summary>
Motivation: 现实世界中在线学习存在风险，准确指定安全成本困难，但收集反映不安全行为的轨迹相对容易，需要从这些非偏好轨迹中推断安全约束

Method: 将安全策略学习建模为约束马尔可夫决策过程，推导奖励最大化目标的下界，学习估计非偏好行为可能性的成本模型，完全从离线演示中学习

Result: OSIL能够学习满足成本约束的更安全策略，且不降低奖励性能，优于多个基线方法

Conclusion: 从非偏好轨迹中推断安全约束是可行的，OSIL为离线安全模仿学习提供有效解决方案，无需显式安全成本标注
...
### [147] [GENIUS: Generative Fluid Intelligence Evaluation Suite](https://arxiv.org/abs/2602.11144)
*Ruichuan An,Sihan Yang,Ziyu Guo,Wei Dai,Zijun Shen,Haodong Li,Renrui Zhang,Xinyu Wei,Guopeng Li,Wenshan Wu,Wentao Zhang*

Main category: cs.LG

TL;DR: 论文提出了GENIUS评估套件，用于评估统一多模态模型的生成性流体智能(GFI)，而非传统的结晶智能。研究发现现有模型在GFI任务上表现不佳，主要源于上下文理解不足而非生成能力问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估统一多模态模型的结晶智能（依赖记忆知识和学习模式），而忽视了生成性流体智能（GFI）——即归纳模式、通过约束推理和适应新场景的能力。需要建立专门评估GFI的基准。

Method: 提出GENIUS评估套件，将GFI形式化为三个基本能力：归纳隐含模式、执行临时约束、适应上下文知识。对12个代表性模型进行系统评估，并通过诊断分析分离失败模式，提出无需训练的注意力干预策略。

Result: 评估显示现有模型在GFI任务上存在显著性能缺陷。诊断分析表明这些失败主要源于有限的上下文理解能力，而非内在生成能力不足。提出的注意力干预策略有助于改善性能。

Conclusion: GENIUS为生成性流体智能建立了严格评估标准，引导领域从知识利用转向动态、通用推理。该基准揭示了当前模型在上下文理解和动态适应方面的局限性，为未来研究提供了方向。
...
TL;DR: SnapMLA是一个针对DeepSeek MLA架构的FP8解码框架，通过硬件感知的算法-内核协同优化技术，解决了FP8注意力在解码阶段面临的数值异构性、量化尺度不对齐等问题，在长上下文任务中实现了高达1.91倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: FP8注意力虽然在前向传播中表现出色（如FlashAttention-3），但在DeepSeek MLA架构的解码阶段面临三个主要挑战：位置嵌入解耦导致的数值异构性、FP8 PV GEMM中的量化尺度不对齐问题，以及缺乏优化的系统级支持。这些挑战限制了FP8在MLA解码中的实际应用效果。

Method: SnapMLA采用三种硬件感知的算法-内核协同优化技术：1) RoPE感知的每令牌KV量化，将RoPE部分保持高精度，采用每令牌粒度以适应自回归解码过程；2) 量化PV计算管道重构，解决MLA KV缓存共享结构导致的FP8 PV计算量化尺度不对齐问题；3) 端到端数据流优化，使用专用内核建立高效的数据读写工作流。

Result: 在最先进的MLA LLM上的大量实验表明，SnapMLA在具有挑战性的长上下文任务（包括数学推理和代码生成基准测试）中，实现了高达1.91倍的吞吐量提升，且性能下降风险可忽略不计。

Conclusion: SnapMLA成功解决了FP8在MLA解码中的关键挑战，通过硬件感知的协同优化实现了显著的性能提升，为高效的长上下文推理提供了有效的解决方案。代码已开源。
...
TL;DR: 本文提出了CURE-UCB算法，专门解决上升多臂老虎机问题中依赖于时间预算的最优策略问题，相比无视时间预算的方法有显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 在上升多臂老虎机问题中，最优策略强烈依赖于可用时间预算T，但现有研究对此关注不足。了解T能带来显著优势，但这一领域尚未充分探索。

Method: 提出CURE-UCB算法，该算法明确整合时间预算信息，通过累积奖励估计来优化决策过程。

Result: 理论分析建立了新的遗憾上界，证明在"线性后平坦"等结构化环境中，该方法严格优于无视时间预算的策略。大量实验验证了其相对于基线的显著优越性。

Conclusion: CURE-UCB算法有效解决了上升多臂老虎机中时间预算依赖的最优策略问题，填补了该领域的研究空白，为实际应用提供了更优的决策框架。
...
### [150] [In-the-Wild Model Organisms: Mitigating Undesirable Emergent Behaviors in Production LLM Post-Training via Data Attribution](https://arxiv.org/abs/2602.11079)
*Frank Xiao,Santiago Aranguri*

Main category: cs.LG

TL;DR: 提出激活基数据归因方法，通过计算激活差异向量和余弦相似度，追踪语言模型行为变化到具体训练数据点，并验证归因的因果性


<details>
  <summary>Details</summary>
Motivation: 需要开发一种能够准确追踪语言模型行为变化到具体训练数据点的方法，以理解和控制模型行为，特别是在安全关键场景中

Method: 计算测试提示和偏好对的激活差异向量，通过余弦相似度排名识别导致特定行为的数据点，并通过修改数据重新训练进行因果验证；聚类行为-数据点相似度矩阵实现无监督行为发现

Result: 应用于OLMo 2的DPO训练，发现了"分心触发合规"的有害行为；过滤排名靠前的数据点可将该行为减少63%，切换标签可减少78%；方法优于基于梯度的归因和LLM判断基线，成本降低10倍以上

Conclusion: 激活基数据归因方法能有效识别和减轻语言模型中的有害行为，为安全技术提供了现实的基准测试，特别是在处理受污染偏好数据而非故意注入的场景中
...
### [151] [GRASP: group-Shapley feature selection for patients](https://arxiv.org/abs/2602.11084)
...
<details>
  <summary>Details</summary>
Motivation: 医学预测中的特征选择面临挑战，现有方法如LASSO缺乏鲁棒性和可解释性，需要一种能够提取紧凑且非冗余特征集的新方法。

Method: GRASP框架首先通过SHAP从预训练的树模型中提取组级重要性分数，然后通过组L21正则化逻辑回归强制执行结构化稀疏性，实现稳定且可解释的特征选择。

Result: 与LASSO、SHAP和基于深度学习的方法相比，GRASP在保持相当或更优预测准确性的同时，识别出更少、更不冗余且更稳定的特征。

Conclusion: GRASP为医学预测提供了一种有效的特征选择框架，能够产生紧凑、非冗余且稳定的特征集，具有更好的可解释性和鲁棒性。
...
### [152] [General Flexible $f$-divergence for Challenging Offline RL Datasets with Low Stochasticity and Diverse Behavior Policies](https://arxiv.org/abs/2602.11087)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临两个主要挑战：1) 实际数据集通常包含有限的环境探索，影响Q/V值估计；2) 来自多个行为策略的数据具有不同的专业水平，过于保守的约束会限制学习效果。需要在RL目标和行为策略约束之间找到平衡。

Method: 首先通过线性规划形式和凸共轭建立了f-散度与贝尔曼残差优化约束之间的联系，然后引入了灵活的f-散度函数公式，基于离线训练数据集对算法学习目标施加自适应约束。

Result: 在MuJoCo、Fetch和AdroitHand环境上的实验结果表明，提出的线性规划形式是正确的，并且灵活的f-散度在应用于兼容的约束优化算法时，能够显著提高从具有挑战性的数据集学习的性能。

Conclusion: 通过将f-散度与贝尔曼残差约束联系起来，并引入灵活的自适应约束机制，可以有效平衡离线强化学习中的探索与利用，提升在有限多样性数据集上的学习效果。
...
### [153] [Collaborative Threshold Watermarking](https://arxiv.org/abs/2602.10765)
*Tameem Bakr,Anish Ambreth,Nils Lukas*

Main category: cs.LG

TL;DR: 该论文提出了一种(t,K)-阈值水印方案，用于联邦学习中的模型溯源，确保只有至少t个客户端组成的联盟才能验证水印，解决了传统水印方法在多客户端场景下的可扩展性和安全性问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中多个客户端联合训练模型，但缺乏有效机制来证明模型来源。传统水印方法存在两个问题：1) 随着客户端数量K增加，每个客户端的水印会被稀释；2) 任何单个客户端都能验证甚至移除水印，存在安全风险。

Method: 提出(t,K)-阈值水印方案：客户端在训练过程中协作嵌入共享水印，但只有至少t个客户端组成的联盟才能重构水印密钥并验证可疑模型。使用秘密共享技术分发水印密钥τ，确保少于t个客户端的联盟无法重构密钥，且验证过程无需明文暴露τ。在白盒设置下实现该协议。

Result: 在图像分类任务上评估，水印在大规模场景(K=128)下仍可检测，准确率损失最小。在包括使用高达20%训练数据进行自适应微调的攻击下，检测阈值保持稳定(z≥4)。

Conclusion: 提出的(t,K)-阈值水印方案解决了联邦学习中模型溯源的关键挑战，实现了可扩展、安全的水印验证机制，能够抵抗各种攻击并保持模型性能。
...
### [154] [Weight Decay Improves Language Model Plasticity](https://arxiv.org/abs/2602.11137)
*Tessa Han,Sebastian Bordt,Hanlin Zhang,Sham Kakade*

Main category: cs.LG

TL;DR: 研究发现权重衰减（weight decay）在预训练中能增强模型的可塑性，使基础模型在微调后获得更大性能提升，即使预训练时表现较差的基础模型在微调后可能表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型开发主要关注基础模型的验证损失，而忽略了模型在下游任务中的适应能力。本研究从模型可塑性角度研究预训练，即基础模型通过微调成功适应下游任务的能力。

Method: 通过系统实验研究权重衰减这一关键正则化参数在预训练中的作用，分析其对模型可塑性的影响，并深入探究权重衰减对模型行为的机制性影响。

Result: 使用较大权重衰减值训练的模型更具可塑性，在微调下游任务时表现出更大的性能提升。权重衰减鼓励线性可分表示、正则化注意力矩阵并减少训练数据的过拟合。

Conclusion: 研究证明了在超参数优化中使用交叉熵损失之外的评估指标的重要性，并揭示了单个优化超参数在塑造模型行为中的多方面作用。
...
### [155] [RePO: Bridging On-Policy Learning and Off-Policy Knowledge through Rephrasing Policy Optimization](https://arxiv.org/abs/2602.10819)
*Linxuan Xia,Xiaolong Yang,Yongyuan Chen,Enyue Zhao,Deng Cai,Yasheng Wang,Boxi Wu*

Main category: cs.LG

TL;DR: RePO是一种新的强化学习方法，通过让模型重新表述离线知识来平衡离线知识吸收和在线训练稳定性，在保持模型泛化能力的同时有效利用困难样本。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在领域特定数据上的对齐存在挑战：监督微调会损害模型泛化能力，在线强化学习无法有效利用困难样本，而离线强化学习又存在训练不稳定的问题。需要一种既能有效吸收离线知识又能保持在线训练稳定性的方法。

Method: 提出Rephrasing Policy Optimization (RePO)方法：让策略模型首先理解离线知识，然后将其重新表述为符合自身风格和参数分布的轨迹。动态地用这些重新表述的高质量轨迹替换低奖励的rollouts，引导模型走向正确推理路径，同时严格保持在线训练动态。

Result: 在多个基准测试上的实验表明，RePO提高了困难样本的利用率，超越了现有基线方法，实现了最先进的性能。

Conclusion: RePO通过重新表述离线知识的策略优化，成功解决了离线知识吸收与在线训练稳定性之间的权衡问题，为大语言模型的领域对齐提供了一种有效且稳定的方法。
...
### [156] [Adaptive Sampling for Private Worst-Case Group Optimization](https://arxiv.org/abs/2602.10820)
*Max Cairney-Leeming,Amartya Sanyal,Christoph H. Lampert*

Main category: cs.LG

TL;DR: ASC算法通过自适应控制每组采样率和裁剪阈值，在差分隐私下实现最坏情况组优化，平衡隐私保护与少数群体性能


<details>
  <summary>Details</summary>
Motivation: 传统加权优化方法在差分隐私下会导致隐私保护不均，特别是少数群体隐私保护较弱，需要新方法在保证隐私均匀性的同时提升最坏情况组性能

Method: 提出ASC算法，自适应控制每个组的采样率和裁剪阈值，允许难学习组更频繁采样，同时确保所有组隐私保护一致

Result: ASC相比现有方法具有更低方差梯度、更严格的隐私保证，显著提升最坏情况组准确率而不牺牲整体平均准确率

Conclusion: ASC算法有效解决了差分隐私下最坏情况组优化的隐私不均问题，实现了隐私保护与性能提升的平衡
...
### [157] [SimuScene: Training and Benchmarking Code Generation to Simulate Physical Scenarios](https://arxiv.org/abs/2602.10840)
*Yanan Wang,Renxi Wang,Yongxin Wang,Xuezhi Liang,Fajri Koto,Timothy Baldwin,Xiaodan Liang,Haonan Li*

Main category: cs.LG

TL;DR: SimuScene：首个系统研究LLMs通过代码模拟物理场景的能力，涵盖5个物理领域和52个物理概念，构建了7,659个物理场景数据集，发现当前最强模型仅21.5%通过率，并提出基于视觉奖励的强化学习训练方法。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在数学竞赛、复杂编码和科学推理等任务上得到广泛研究，但其通过代码准确表示和模拟物理场景的能力仍未得到充分探索。现有研究缺乏对LLMs物理模拟能力的系统性评估。

Method: 1. 构建SimuScene数据集：通过自动流水线收集数据，人工验证确保质量，包含7,659个物理场景，其中334个人工验证示例作为测试集；2. 评估10个当代LLMs的物理模拟能力；3. 提出基于视觉奖励的强化学习流水线，使用视觉语言模型作为评判器来训练文本模型。

Result: 1. 即使最强的LLM在物理场景模拟任务上也仅达到21.5%的通过率，表明任务难度很高；2. 使用SimuScene数据进行训练可以显著提升模型通过代码进行物理模拟的能力；3. 训练还大幅提升了模型的一般代码生成性能。

Conclusion: SimuScene是首个系统研究LLMs物理模拟能力的工作，揭示了当前模型在此任务上的局限性，提出的基于视觉奖励的强化学习方法有效提升了模型的物理模拟和代码生成能力，为LLMs在物理场景理解和模拟方面的发展提供了重要基础。
...
### [158] [The Sample Complexity of Uniform Approximation for Multi-Dimensional CDFs and Fixed-Price Mechanisms](https://arxiv.org/abs/2602.10868)
*Matteo Castiglioni,Anna Lunghi,Alberto Marchesi*

Main category: cs.LG

TL;DR: 该论文研究了在最小化单比特反馈下学习n维累积分布函数均匀近似的样本复杂度，证明了样本复杂度具有近维度不变性，维度n仅影响对数项。


<details>
  <summary>Details</summary>
Motivation: 研究在"老虎机反馈"（单比特反馈）而非"完全反馈"下学习多维累积分布函数均匀近似的样本复杂度，作为多元DKW不等式在反馈受限情况下的对应研究。

Method: 在最小化单比特反馈设置下，通过理论分析推导学习n维CDF均匀近似的样本复杂度界限，特别关注维度对样本复杂度的影响。

Result: 获得了样本复杂度的近维度不变性：在任意精细网格上获得均匀ε-近似需要样本复杂度为1/ε³ × log(1/ε)^O(n)，其中维度n仅影响对数项。

Conclusion: 该结果为小市场（如双边交易）中学习固定价格机制提供了紧致的样本复杂度界限和新的遗憾保证，扩展了多元DKW不等式到反馈受限场景。
...
### [159] [Natural Hypergradient Descent: Algorithm Design, Convergence Analysis, and Parallel Implementation](https://arxiv.org/abs/2602.10905)
...
Method: 利用内层优化问题的统计结构，使用经验Fisher信息矩阵作为Hessian的渐近一致替代，设计并行优化-近似框架，使Hessian逆近似与随机内层优化同步更新，以可忽略的额外成本重用梯度信息。

Result: 理论贡献建立了NHGD的高概率误差界和样本复杂度保证，与最先进的优化-然后-近似方法相匹配，同时显著减少计算时间开销。在代表性双层学习任务上的实证评估进一步展示了NHGD的实际优势。

Conclusion: NHGD是一种高效的双层优化方法，通过创新的Hessian逆近似策略解决了计算瓶颈问题，在大规模机器学习设置中具有可扩展性和有效性。
...
### [160] [Tuning the burn-in phase in training recurrent neural networks improves their performance](https://arxiv.org/abs/2602.10911)
*Julian D. Schiller,Malte Heinrich,Victor G. Lopez,Matthias A. Müller*

Main category: cs.LG

TL;DR: 该论文研究了在时间序列任务中使用截断BPTT训练RNN的方法，建立了在子序列上优化而非完整数据序列时的理论精度和性能损失界限，发现RNN的burn-in阶段对训练有重要影响。


<details>
  <summary>Details</summary>
Motivation: 标准BPTT训练RNN在处理长输入序列时存在计算和内存开销大的挑战，截断BPTT通过处理较短的数据段来减少开销，但需要理论分析其性能影响。

Method: 建立截断学习方法的理论框架，分析在子序列上优化而非完整序列时的精度界限，特别关注burn-in阶段作为重要调节参数的影响，并在系统识别和时间序列预测的标准基准上进行实验验证。

Result: 理论分析表明burn-in阶段对性能保证有显著影响，实验验证显示burn-in阶段对训练过程有强烈影响，适当调节可使训练和测试数据的预测误差在某些情况下减少60%以上。

Conclusion: 截断BPTT是训练RNN的有效替代方法，burn-in阶段是关键调节参数，适当设置可显著提升性能，为实际应用提供了理论指导和实用建议。
...
### [161] [Near-Constant Strong Violation and Last-Iterate Convergence for Online CMDPs via Decaying Safety Margins](https://arxiv.org/abs/2602.10917)
*Qian Zuo,Zhiyong Wang,Fengxiang He*

Main category: cs.LG

TL;DR: FlexDOME算法在约束马尔可夫决策过程中实现近常数级强约束违反和次线性强遗憾，解决了现有方法约束违反增长或仅平均迭代收敛的问题。


<details>
  <summary>Details</summary>
Motivation: 现有原始对偶方法在实现次线性强奖励遗憾时，不可避免地导致强约束违反增长或仅限于平均迭代收敛，这是由于固有的振荡问题。需要一种能同时保证近常数级约束违反、次线性遗憾和非渐近最后迭代收敛的算法。

Method: 提出FlexDOME算法，将时变安全裕度和正则化项融入原始对偶框架。采用新颖的逐项渐近主导策略，通过严格调度安全裕度来渐近主导优化和统计误差的函数衰减率，从而将累积违反限制在近常数水平。通过策略-对偶Lyapunov论证建立非渐近最后迭代收敛保证。

Result: FlexDOME是首个可证明实现近常数级$\tilde{O}(1)$强约束违反、次线性强遗憾和非渐近最后迭代收敛的算法。理论分析通过渐近主导策略确保约束违反被钳制在近常数水平。

Conclusion: FlexDOME算法成功解决了现有方法在强约束违反和收敛性方面的局限性，实现了近常数级约束违反、次线性遗憾和最后迭代收敛的平衡，实验验证了理论发现。
...
### [162] [CMAD: Cooperative Multi-Agent Diffusion via Stochastic Optimal Control](https://arxiv.org/abs/2602.10933)
*Riccardo Barbano,Alexander Denker,Zeljko Kereta,Runchang Li,Francisco Vargas*

Main category: cs.LG

TL;DR: 提出一种基于随机最优控制的新范式，将预训练扩散模型视为交互智能体，通过联合控制其扩散轨迹实现组合生成，而非传统概率密度组合方法。


<details>
  <summary>Details</summary>
Motivation: 现有连续时间生成模型在图像恢复和合成方面取得显著成功，但控制多个预训练模型的组合仍然是一个开放挑战。当前方法主要将组合视为概率密度的代数组合（如乘积或专家混合），这需要明确知道目标分布，而这几乎不可能实现。

Method: 提出新范式，将组合生成建模为协作式随机最优控制问题。将预训练扩散模型视为交互智能体，通过最优控制联合引导其扩散轨迹，使其聚合输出朝向共享目标。

Result: 在条件MNIST生成任务上验证了该框架，并与基于DPS风格的朴素推理时基线进行比较，后者用每步梯度指导替代学习到的协作控制。

Conclusion: 通过将组合生成重新定义为协作最优控制问题，提供了一种比传统概率密度组合更灵活、更有效的多模型协同生成方法。
...
### [163] [Stochastic Parroting in Temporal Attention -- Regulating the Diagonal Sink](https://arxiv.org/abs/2602.10956)
*Victoria Hankemeier,Malte Hankemeier*

Main category: cs.LG

TL;DR: 该论文分析了时空模型中时间注意力机制的信息退化问题，发现时间注意力矩阵存在对角线注意力沉没现象，并提出了相应的正则化方法。


<details>
  <summary>Details</summary>
Motivation: 时空模型分析空间结构和时间动态时容易出现时空信息退化问题。先前研究表明因果注意力或时间卷积中的过度压缩会对第一个token产生偏差，本文旨在分析时间注意力机制是否也存在类似偏差。

Method: 推导时间注意力层Jacobian期望值的敏感度边界，从理论上分析非对角线注意力分数如何依赖于序列长度，证明时间注意力矩阵存在对角线注意力沉没现象，并提出正则化方法。

Result: 理论分析表明时间注意力机制确实存在对角线注意力沉没问题，提出的正则化方法在实验中证明有效。

Conclusion: 时间注意力机制存在与因果注意力类似的信息退化偏差，表现为对角线注意力沉没，通过适当的正则化方法可以有效缓解这一问题。
...
### [164] [MoEEdit: Efficient and Routing-Stable Knowledge Editing for Mixture-of-Experts LLMs](https://arxiv.org/abs/2602.10965)
...
Conclusion: MoEEdit为稀疏LLM中的可扩展、精确知识编辑建立了坚实基础，强调了路由稳定干预的重要性，解决了MoE模型知识编辑的关键挑战。
...
### [165] [A Jointly Efficient and Optimal Algorithm for Heteroskedastic Generalized Linear Bandits with Adversarial Corruptions](https://arxiv.org/abs/2602.10971)
*Sanghwa Kim,Junghyun Lee,Se-Young Yun*

Main category: cs.LG

TL;DR: 本文提出HCW-GLB-OMD算法，用于解决具有对抗性扰动的异方差广义线性老虎机问题，该算法结合在线镜像下降估计器和基于Hessian的置信权重，实现了计算高效和对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的随机上下文老虎机设置（如异方差线性老虎机、逻辑回归/泊松老虎机）在面对对抗性扰动时缺乏统一的鲁棒算法。需要开发一个既能处理异方差性又能抵抗对抗扰动的通用框架。

Method: 提出HCW-GLB-OMD算法，包含两个核心组件：1）基于在线镜像下降的估计器；2）基于Hessian的置信权重。算法具有O(1)的每轮空间和时间复杂度，计算高效。

Result: 在连接函数满足自协调性假设下，获得了后悔上界：Õ(d√∑g(τ_t)μ̇_{t,⋆} + d²g_maxκ + dκC)，其中C是对抗者的总扰动预算。同时给出了下界：Ω̃(d√∑g(τ_t)μ̇_{t,⋆} + dC)，表明算法在扰动项上达到κ因子的实例级极小极大最优性。

Conclusion: 该算法首次为具有对抗性扰动的异方差广义线性老虎机提供了统一的解决方案，在多种实例中同时实现了实例级极小极大最优性，统一了先前特定问题的下界结果。
...
### [166] [TVCACHE: A Stateful Tool-Value Cache for Post-Training LLM Agents](https://arxiv.org/abs/2602.10986)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体后训练中，外部工具调用耗时数秒甚至数分钟，导致GPU空闲时间增加，延长训练时间和成本。许多工具调用在并行rollout中重复出现，但简单缓存输出不正确，因为工具输出依赖于先前智能体交互诱导的环境状态。

Method: TVCACHE维护观察到的工具调用序列树，执行最长前缀匹配进行缓存查找：只有当智能体的完整工具历史与先前执行序列完全匹配时才发生缓存命中，保证环境状态一致。系统在三个不同工作负载上测试：终端任务、SQL生成和视频理解。

Result: 在三个多样化工作负载上，TVCACHE实现高达70%的缓存命中率，将工具调用执行时间中位数减少高达6.9倍，且后训练奖励积累没有下降。

Conclusion: TVCACHE通过状态感知的缓存机制有效解决了LLM智能体后训练中工具调用延迟问题，在保证训练效果的同时显著提升了训练效率。
...
### [167] [Motion Capture is Not the Target Domain: Scaling Synthetic Data for Learning Motion Representations](https://arxiv.org/abs/2602.11064)
*Firas Darwish,George Nicholson,Aiden Doherty,Hang Yuan*

Main category: cs.LG

TL;DR: 研究合成数据在人体运动识别中的迁移学习效果，发现合成数据与真实数据混合或大规模使用时能提升泛化能力，但运动捕捉数据与可穿戴信号存在领域不匹配问题


<details>
  <summary>Details</summary>
Motivation: 当真实数据稀缺时，合成数据为可扩展的预训练提供了有吸引力的途径，但在合成数据上预训练的模型往往无法可靠地迁移到实际部署环境中。本研究针对全身人体运动识别领域，该领域大规模数据收集困难但对可穿戴设备的人体活动识别至关重要

Method: 使用从运动捕捉数据派生的表示生成合成运动数据，在合成数据上预训练运动时间序列模型，并在多样化的下游人体活动识别任务中评估其迁移效果

Result: 合成预训练在与真实数据混合或规模足够大时能改善泛化性能；大规模运动捕捉预训练由于与可穿戴信号存在领域不匹配，仅带来边际收益

Conclusion: 阐明了从仿真到现实的关键挑战，以及合成运动数据在可迁移的人体活动识别表示方面的局限性和机会，为合成数据在运动识别领域的应用提供了重要见解
...
### [168] [Token-Efficient Change Detection in LLM APIs](https://arxiv.org/abs/2602.11083)
*Timothée Chauvin,Clément Lalanne,Erwan Le Merrer,Jean-Michel Loubes,François Taïani,Gilles Tredan*

Main category: cs.LG

TL;DR: B3IT：一种基于边界输入的低成本黑盒LLM变化检测方法，性能媲美灰盒方法，成本降低30倍


<details>
  <summary>Details</summary>
Motivation: 现有LLM远程变化检测方法要么成本过高难以大规模部署，要么需要白盒（模型权重）或灰盒（对数概率）访问权限。需要一种既低成本又能在严格黑盒环境下（仅观察输出token）运行的方法。
...
Conclusion: B3IT方案成功实现了低成本、严格黑盒的LLM变化检测，解决了现有方法在成本和访问权限方面的限制，为大规模部署提供了可行方案。
...
### [169] [MerLin: A Discovery Engine for Photonic and Hybrid Quantum Machine Learning](https://arxiv.org/abs/2602.11092)
...
TL;DR: MerLin是一个开源框架，用于系统探索光子量子机器学习模型，通过集成线性光学电路的优化强模拟到PyTorch和scikit-learn工作流中，支持端到端可微分训练。


<details>
  <summary>Details</summary>
Motivation: 需要超越孤立的算法提案，系统实证地探索量子模型在不同数据集和硬件约束下的实际优势，以识别近期量子机器学习可能提供实用价值的领域。

Method: 开发MerLin框架，将线性光学电路的优化强模拟集成到PyTorch和scikit-learn工作流中，支持端到端可微分训练量子层，围绕系统基准测试和可重复性设计。

Result: 成功复现了18个最先进的光子和混合量子机器学习工作，涵盖核方法、储备池计算、卷积和循环架构、生成模型等，并发布了可重用、模块化的实验作为共享基准。

Conclusion: MerLin通过将光子量子模型嵌入现有机器学习生态系统，为消融研究、跨模态比较和混合经典-量子工作流提供工具，并具备硬件感知功能，成为连接算法、基准和硬件的未来验证协同设计工具。
...
### [170] [The Offline-Frontier Shift: Diagnosing Distributional Limits in Generative Multi-Objective Optimization](https://arxiv.org/abs/2602.11126)
...
<details>
  <summary>Details</summary>
Motivation: 研究生成式方法在离线多目标优化中的局限性，特别是它们在除高维体积外的其他标准指标上的表现不佳，以及这种失败模式背后的根本原因

Method: 通过分析离线前沿偏移现象，将其视为离线多目标优化的基本限制，并论证克服这一限制需要在目标空间进行分布外采样（通过积分概率度量）

Result: 生成式方法在代际距离等指标上系统性地表现不如进化方法，且保持保守地接近离线目标分布，未能有效进行分布外采样

Conclusion: 离线多目标优化本质上是一个分布偏移受限问题，研究结果为理解生成式优化方法何时及为何失败提供了诊断视角
...
### [171] [Asymmetric Prompt Weighting for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2602.11128)
*Reinhard Heckel,Mahdi Soltanolkotabi,Christos Thramboulidis*

Main category: cs.LG

TL;DR: 本文研究强化学习中非对称提示加权策略，发现在低成功率场景下，对低成功率提示赋予更高权重可以加速收敛，特别是在从零开始的RL训练中效果显著。


<details>
  <summary>Details</summary>
Motivation: 当前流行的强化学习算法（如GRPO、DAPO、RLOO）主要关注中等成功概率的模糊提示，而对非常容易或非常困难的提示进行降权处理。本文旨在探索非对称提示加权策略，特别是对低成功率提示赋予更高权重，以优化训练效率。

Method: 提出非对称提示加权方法，分析不同加权策略对训练收敛速度的影响。通过理论分析确定在固定更新预算下，能够最小化从初始成功率提升到目标准确率所需时间的最优提示权重。

Result: 研究发现非对称加权在从零开始的RL训练（如R1-Zero）中特别有效，因为训练过程覆盖了广泛的准确率范围；而在后SFT RL中效果较弱，因为模型已经具有较高的初始准确率。理论分析表明，在低成功率场景下，最优权重确实是非对称的，通过上加权低成功率提示可以加速有效时间收敛。

Conclusion: 非对称提示加权策略在强化学习中具有重要价值，特别是在低成功率场景下，通过合理分配提示权重可以显著提高训练效率，加速模型收敛。
...
### [172] [Just on Time: Token-Level Early Stopping for Diffusion Language Models](https://arxiv.org/abs/2602.11133)
*Zahar Kohut,Severyn Shykula,Dmytro Khamula,Mykola Vysotskyi,Taras Rumezhak,Volodymyr Karpiv*

Main category: cs.LG

TL;DR: 提出一种无需训练、基于词元级别的早停方法，通过轻量级信号动态判断每个词元何时收敛，显著减少扩散模型生成文本所需的步骤数


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过迭代优化生成文本，但许多词元在最终去噪步骤之前很早就达到稳定状态，导致计算效率低下

Method: 训练免费的词元级别早停方法，利用模型预测和局部上下文的轻量级信号，独立识别每个位置的收敛情况，实现自适应词元冻结

Result: 在数学推理、通用问答和科学理解等多样化基准测试中，该方法在保持生成质量的同时实现了最先进的效率提升

Conclusion: 提出的自适应词元早停方法能够显著提高扩散语言模型的生成效率，无需任务特定的微调，具有广泛的应用前景
...
### [173] [TabICLv2: A better, faster, scalable, and open tabular foundation model](https://arxiv.org/abs/2602.11139)
*Jingang Qu,David Holzmüller,Gaël Varoquaux,Marine Le Morvan*

Main category: cs.LG

TL;DR: TabICLv2是一种新的表格基础模型，通过创新的合成数据生成、架构改进和优化训练协议，在回归和分类任务上超越了现有最佳模型RealTabPFN-2.5，同时保持高效的内存使用和推理速度。
...
Method: TabICLv2基于三大支柱构建：(1) 新颖的合成数据生成引擎，确保预训练数据多样性；(2) 架构创新，包括新的可扩展注意力softmax，无需大规模长序列预训练即可泛化到更大数据集；(3) 优化的预训练协议，用Muon优化器替代AdamW。

Result: 在TabArena和TALENT基准测试中，未经调优的TabICLv2超越了当前最佳模型RealTabPFN-2.5（经过超参数调优、集成和真实数据微调）。模型在50GB GPU内存下有效泛化到百万级数据集，且推理速度明显快于RealTabPFN-2.5。

Conclusion: TabICLv2在表格数据预测任务中建立了新的最先进水平，通过创新的合成数据生成、架构改进和优化训练协议实现了卓越性能。作者承诺开放研究，已发布推理代码和模型权重。
